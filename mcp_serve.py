"""
Clover MCP Server — expose messaging conversations as MCP tools.
...
Plus: channels_list (Clover-specific extra)

Usage:
    clover mcp serve
    clover mcp serve --verbose
...
logger = logging.getLogger("clover.mcp_serve")
...
def _get_sessions_dir() -> Path:
    """Return the sessions directory using CLOVER_HOME."""
    try:
        from clover_constants import get_clover_home
        return get_clover_home() / "sessions"
    except ImportError:
        return Path(os.environ.get("CLOVER_HOME", Path.home() / ".clover")) / "sessions"
...
def _load_channel_directory() -> dict:
    ...
    try:
        from clover_constants import get_clover_home
        directory_file = get_clover_home() / "channel_directory.json"
    except ImportError:
        directory_file = Path(
            os.environ.get("CLOVER_HOME", Path.home() / ".clover")
...
class EventBridge:
    ...
    This is the Clover equivalent of OpenClaw's WebSocket gateway bridge.
    ...
    def _establish_baseline_with_db(self, db) -> None:
        ...
        try:
            from clover_constants import get_clover_home
            db_file = get_clover_home() / "state.db"
        except ImportError:
            db_file = Path(os.environ.get("CLOVER_HOME", Path.home() / ".clover")) / "state.db"
        try:
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "MCPServer":
    """Create and return the Clover MCP server with all tools registered."""
    ...
    mcp = MCPServer(
        "clover",
        instructions=(
            "Clover Cognition messaging bridge. Use these tools to interact with "
...
def run_mcp_server(verbose: bool = False) -> None:
    """Start the Clover MCP server on stdio."""
    if not _MCP_SERVER_AVAILABLE:
        print(
