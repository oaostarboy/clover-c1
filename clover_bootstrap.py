"""Windows UTF-8 bootstrap for Clover entry points.
...
should be imported at the very top of every Clover entry point
(``clover``, ``clover-c1``, ``clover-acp``, ``python -m gateway.run``,
...
def suppress_platform_ver_console() -> None:
    ...
    Mirrors ``clover_cli._subprocess_compat.suppress_platform_ver_console``
    ...
    ``clover_bootstrap``, never ``clover_cli.main``.
    """
    if not _IS_WINDOWS:
        return
    try:
...
def harden_import_path(src_root: str | None = None) -> None:
    """Stop a package in the current directory from shadowing Clover modules.

    Clover ships top-level modules with common names (``utils``, ``proxy``,
    ...
    We drop the relative forms outright, then force the real Clover source root
    ...
    """
    root = src_root or os.environ.get("CLOVER_PYTHON_SRC_ROOT") or os.path.dirname(
...
def activate_durable_lazy_target() -> None:
    ...
    (``CLOVER_LAZY_INSTALL_TARGET``, e.g. ``/opt/data/lazy-packages``).
    ...
    """
    if not os.environ.get("CLOVER_LAZY_INSTALL_TARGET", "").strip():
        return
    try:
...
# Apply on import — entry points just need ``import clover_bootstrap``
# (or ``from clover_bootstrap import apply_windows_utf8_bootstrap``) at
