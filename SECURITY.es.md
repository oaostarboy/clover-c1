# Política de Seguridad de Clover Cognition

Este documento describe el modelo de confianza de Clover Cognition, identifica el
...
Reporta de forma privada a través de [GitHub Security Advisories](https://github.com/CloverCognition/clover-c1/security/advisories/new)
o **security@clover-c1.local**. No abras issues públicos para
...
- Detalles del entorno (`clover --version`, SHA del commit, SO, versión de Python).
...
Clover Cognition es un agente personal de un solo inquilino. Su postura es
...
- **Proceso del agente.** El intérprete Python que ejecuta Clover Cognition,
...
- **Envolvente de confianza.** El conjunto de recursos a los que un operador ha otorgado
  implícitamente acceso a Clover Cognition al ejecutarlo — típicamente, todo lo que
  la propia cuenta de usuario del operador puede alcanzar en el host.
- **Postura.** Una declaración explícita en la documentación o código de Clover Cognition
...
Clover Cognition admite dos posturas de aislamiento a nivel de SO. Abordan
...
Clover Cognition admite esto de dos maneras:

- **La propia imagen Docker de Clover Cognition y la configuración de Compose.** Más
...
Bajo una envoltura de proceso completo, las heurísticas en proceso de Clover Cognition
...
Clover Cognition filtra el entorno que pasa a sus componentes en proceso de
...
Un plugin malicioso o con errores no es una vulnerabilidad en Clover Cognition
en sí mismo. Los errores en la ruta de instalación o descubrimiento de plugins de Clover Cognition
...
**Superficies en Clover Cognition:**
...
4. **Dentro del conjunto autorizado, todos los llamadores tienen la misma confianza.**
   Clover Cognition no modela capacidades por llamador dentro de un único adaptador.
...
- Violaciones de la documentación del modelo de confianza: código que se comporta
  contrariamente a lo que esta política, la propia documentación de Clover Cognition o
  las expectativas razonables del operador predecirían — incluyendo casos donde
  Clover Cognition ha documentado una postura sobre cómo su salida debe ser
...
- **Configuraciones documentadas de emergencia.** Compensaciones seleccionadas por el operador
  ...
  la seguridad de clover-home, y similares. Los informes contra esas
...
- **Habilidades y plugins contribuidos por la comunidad.** Las habilidades de terceros
  ...
  están en la superficie de revisión del operador, no en la superficie de confianza de Clover Cognition
  (§2.4, §2.5). Una habilidad o plugin que haga algo
  malicioso es el modo de falla esperado de uno que no fue
  revisado, no una vulnerabilidad en Clover Cognition. Los errores en la ruta de
  instalación de habilidades o plugins de Clover Cognition que impidan al
...
- Clover Cognition incluye guardias de cadena de suministro para lanzamientos de servidores
