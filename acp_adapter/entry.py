"""CLI entry point for the clover-c1 ACP adapter.

Loads environment variables from ``~/.clover/.env``, configures logging
...
try:
    ...
    # shadowing Clover's own modules — ``clover acp`` can be started from any
...
def _load_env() -> None:
    """Load .env from CLOVER_HOME (default ``~/.clover``)."""
    from clover_cli.env_loader import load_clover_dotenv

    clover_home = get_clover_home()
    loaded = load_clover_dotenv(clover_home=clover_home)
    if loaded:
        for env_file in loaded:
            logging.getLogger(__name__).info("Loaded env from %s", env_file)
    else:
        logging.getLogger(__name__).info(
            "No .env found at %s, using system env", clover_home / ".env"
        )
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        prog="clover-acp",
        description="Run Clover Cognition as an ACP stdio server.",
    )
    parser.add_argument("--version", action="store_true", help="Print Clover version and exit")
    parser.add_argument(
        "--check",
        action="store_true",
    ...
    parser.add_argument(
        "--setup",
        action="store_true",
        help="Run interactive Clover provider/model setup for ACP terminal auth",
    )
    parser.add_argument(
        "--setup-browser",
        action="store_true",
        help="Install agent-browser + Playwright Chromium into ~/.clover/node/ "
...
def _print_version() -> None:
    ...
    print(clover_version)


def _run_check() -> None:
    import acp  # noqa: F401
    from acp_adapter.server import CloverACPAgent  # noqa: F401

    print("Clover ACP check OK")


def _run_setup() -> None:
    from clover_cli.main import main as clover_main

    old_argv = sys.argv[:]
    try:
        sys.argv = [old_argv[0] if old_argv else "clover", "model"]
        clover_main()
    finally:
        sys.argv = old_argv
...
def main(argv: list[str] | None = None) -> None:
    ...
    logger.info("Starting clover-c1 ACP adapter")
    ...
    import acp
    from .server import CloverACPAgent
    ...
    if os.environ.get("CLOVER_ACP_SKIP_CONFIGURED_MCP", "").strip() != "1":
        try:
