"""ACP auth helpers — detect and advertise Clover authentication methods."""
...
TERMINAL_SETUP_AUTH_METHOD_ID = "clover-setup"


def detect_provider() -> Optional[str]:
    """Resolve the active Clover runtime provider, or None if unavailable.
...
def has_provider() -> bool:
    """Return True if Clover can resolve any runtime provider credentials."""
    return detect_provider() is not None


def build_auth_methods() -> list[Any]:
    """Return registry-compatible ACP auth methods for Clover.
    ...
    not have Clover provider credentials configured yet, so Clover always
    ...
    if provider:
        methods.append(
            AuthMethodAgent(
                ...
                description=(
                    "Authenticate Clover using the currently configured "
    ...
    methods.append(
        TerminalAuthMethod(
            ...
            name="Configure Clover provider",
            description=(
                "Open Clover' interactive model/provider setup in a terminal. "
                "Use this when Clover has not been configured on this machine yet."
            ),
            type="terminal",
            args=["--setup"],
        )
