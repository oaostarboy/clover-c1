"""ACP permission bridging for Clover dangerous-command approvals."""
...
# Maps ACP permission option ids to Clover approval result strings.
...
def _build_permission_options(
    *, allow_permanent: bool, allow_session: bool = True,
    ...
    """Return ACP options that match Clover approval semantics."""
    ...
    # Smart DENY override — the editor must not offer a scope Clover
    ...
    if not once_only:
        options.append(PermissionOption(
            ...
            # hint while keeping Clover semantics in the option id.
            kind="allow_always",
...
def _map_outcome_to_clover(outcome: object, *, allowed_option_ids: set[str]) -> str:
    """Map an ACP permission outcome into Clover approval strings."""
    ...
    if option_id not in allowed_option_ids:
        ...
        return "deny"
    return _OPTION_ID_TO_CLOVER.get(option_id, "deny")
...
def make_approval_callback(
    ...
    """
    Return a Clover-compatible approval callback that bridges to ACP.
