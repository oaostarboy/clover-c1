...
This is an additive Clover extension surfaced under ACP ``_meta.clover`` so
...
compression rotates the internal Clover head, ``build_session_provenance`` lets
...
def build_session_provenance(
    ...
    *,
    ...
    """Build ``_meta.clover.sessionProvenance`` for an ACP session.

    Args:
        db: A ``SessionDB`` (must expose ``get_session``).
        acp_session_id: The stable ACP/editor-facing session handle.
        current_clover_session_id: The live internal Clover DB session id
            (``state.agent.session_id``).
        previous_clover_session_id: The internal id from before the most recent
            turn, when known. Supplied by ``prompt()`` to flag a rotation.

    Returns:
        A dict suitable for ``{"clover": {"sessionProvenance": <dict>}}`` under
        ACP ``_meta``, or ``None`` if the session can't be read.
    """
    try:
        row = db.get_session(current_clover_session_id)
    except Exception:
        return None
    if not row:
        return None
    ...
    root_id = current_clover_session_id
    compression_depth = 0
    cursor_parent = parent_id
    seen = {current_clover_session_id}
    ...
    rotated = bool(
        ...
        and previous_clover_session_id != current_clover_session_id
    )

    provenance: Dict[str, Any] = {
        "acpSessionId": acp_session_id,
        "currentCloverSessionId": current_clover_session_id,
        "rootCloverSessionId": root_id,
        "parentCloverSessionId": parent_id,
        "sessionKind": "continuation" if is_continuation else "root",
        "compressionDepth": compression_depth,
    }
    if previous_clover_session_id:
        provenance["previousCloverSessionId"] = previous_clover_session_id
    if rotated:
...
def session_provenance_meta(
    ...
    *,
    ...
    """Return a ready ``_meta`` payload: ``{"clover": {"sessionProvenance": ...}}``."""
    ...
    if prov is None:
        return None
    return {"clover": {"sessionProvenance": prov}}
