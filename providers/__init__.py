...
1. Bundled plugins: ``plugins/model-providers/<name>/`` (shipped with clover-c1)
2. User plugins: ``$CLOVER_HOME/plugins/model-providers/<name>/``
3. Pip-installed plugins: distributions exposing a ``clover-c1.plugins``
...
def register_provider(profile: ProviderProfile) -> None:
    ...
    plugins under ``$CLOVER_HOME/plugins/model-providers/`` can override
...
def _user_plugins_dir() -> Path | None:
    """Return ``$CLOVER_HOME/plugins/model-providers/`` if it exists."""
    try:
        from clover_constants import get_clover_home

        d = get_clover_home() / "plugins" / "model-providers"
...
def _import_plugin_dir(plugin_dir: Path, source: str) -> None:
    ...
    # multiple CLOVER_HOME profiles don't alias each other.
    safe_name = plugin_dir.name.replace("-", "_")
    if source == "bundled":
        module_name = f"plugins.model_providers.{safe_name}"
    else:
        module_name = f"_clover_user_provider_{safe_name}"
...
def _discover_entry_point_providers() -> None:
    """Import pip-installed provider plugins via the ``clover-c1.plugins``
    ...
    * **Provider targets only.** The ``clover-c1.plugins`` group is shared
    ...
    first, so filesystem plugins (bundled + ``$CLOVER_HOME``) keep their
    ...
    try:
        from clover_cli.plugins import _get_disabled_plugins, _get_enabled_plugins
...
def _discover_providers() -> None:
    ...
    Order:
      ...
      2. User plugins at ``$CLOVER_HOME/plugins/model-providers/<name>/``
    ...
    # 0. Pip-installed plugins — entry points in the ``clover-c1.plugins``
    ...
    #    filesystem steps means a bundled or ``$CLOVER_HOME`` profile of the
    ...
    # 1. Bundled plugins — shipped with clover-c1.
    ...
    # 2. User plugins — under $CLOVER_HOME/plugins/model-providers/<name>/.
