# nix/nixosModules.nix — the NixOS module for clover-c1
#
...
#
# Container mode: clover runs from /nix/store bind-mounted read-only into a
...
# container recreation. Environment variables are written to $CLOVER_HOME/.env
# and read by clover at startup — no container recreation needed for env changes.
#
# Tool resolution: the clover wrapper uses --suffix PATH for nix store tools,
...
{ inputs, ... }:
{
  flake.nixosModules.default =
    ...
    let
      ...
      cloverHome = "${cfg.stateDir}/.clover";
      ...
      # CLOVER_HOME via addToSystemPackages, so they can save settings through the
      ...
      # clover user + sudo on first boot (writable layer persists), then
      # drops privileges. Supports arbitrary base images (Debian, Alpine, etc).
      containerEntrypoint = pkgs.writeShellScript "clover-container-entrypoint" ''
        set -eu

        CLOVER_UID="''${CLOVER_UID:?CLOVER_UID must be set}"
        CLOVER_GID="''${CLOVER_GID:?CLOVER_GID must be set}"

        # ── Group: ensure a group with GID=$CLOVER_GID exists ──
        # Check by GID (not name) to avoid collisions with pre-existing groups
        # (e.g. GID 100 = "users" on Ubuntu)
        EXISTING_GROUP=$(getent group "$CLOVER_GID" 2>/dev/null | cut -d: -f1 || true)
        if [ -n "$EXISTING_GROUP" ]; then
          GROUP_NAME="$EXISTING_GROUP"
        else
          GROUP_NAME="clover"
          if command -v groupadd >/dev/null 2>&1; then
            groupadd -g "$CLOVER_GID" "$GROUP_NAME"
          elif command -v addgroup >/dev/null 2>&1; then
            addgroup -g "$CLOVER_GID" "$GROUP_NAME" 2>/dev/null || true
          fi
        fi

        # ── User: ensure a user with UID=$CLOVER_UID exists ──
        PASSWD_ENTRY=$(getent passwd "$CLOVER_UID" 2>/dev/null || true)
        if [ -n "$PASSWD_ENTRY" ]; then
          TARGET_USER=$(echo "$PASSWD_ENTRY" | cut -d: -f1)
          TARGET_HOME=$(echo "$PASSWD_ENTRY" | cut -d: -f6)
        else
          TARGET_USER="clover"
          TARGET_HOME="/home/clover"
          if command -v useradd >/dev/null 2>&1; then
            useradd -u "$CLOVER_UID" -g "$CLOVER_GID" -m -d "$TARGET_HOME" -s /bin/bash "$TARGET_USER"
          elif command -v adduser >/dev/null 2>&1; then
            adduser -u "$CLOVER_UID" -D -h "$TARGET_HOME" -s /bin/sh -G "$GROUP_NAME" "$TARGET_USER" 2>/dev/null || true
          fi
        fi
        mkdir -p "$TARGET_HOME"
        chown "$CLOVER_UID:$CLOVER_GID" "$TARGET_HOME"
        chmod 0750 "$TARGET_HOME"

        # Ensure CLOVER_HOME is owned by the target user.
        ...
        if [ -n "''${CLOVER_HOME:-}" ] && [ -d "$CLOVER_HOME" ]; then
          find "$CLOVER_HOME" \! -user "$CLOVER_UID" -exec chown "$CLOVER_UID:$CLOVER_GID" {} +
        fi
        ...
        if [ ! -f /var/lib/clover-tools-provisioned ] && command -v apt-get >/dev/null 2>&1; then
          ...
          touch /var/lib/clover-tools-provisioned
        fi

        if command -v sudo >/dev/null 2>&1 && [ ! -f /etc/sudoers.d/clover ]; then
          mkdir -p /etc/sudoers.d
          echo "$TARGET_USER ALL=(ALL) NOPASSWD:ALL" > /etc/sudoers.d/clover
          chmod 0440 /etc/sudoers.d/clover
        fi
        ...
        if command -v setpriv >/dev/null 2>&1; then
          exec setpriv --reuid="$CLOVER_UID" --regid="$CLOVER_GID" --init-groups "$@"
      ...
      # so they can update without recreation. Env vars go through $CLOVER_HOME/.env.
      ...
      containerModeFile = pkgs.writeText "clover-container-mode" ''
        # Written by the NixOS activation script. Do not edit manually.
        backend=${cfg.container.backend}
        container_name=${containerName}
        exec_user=${cfg.user}
        clover_bin=${containerDataDir}/current-package/bin/clover
      '';

      # Default: /var/lib/clover/workspace → /data/workspace.
      ...
      commonServiceConfig = {
        ...
        # so interactive users in the clover group can read/write them.
        UMask = "0007";
      ...
      // common.processEnvironment { inherit cloverHome; };
    ...
    in
    {
      options.services.clover-c1 =
        ...
        // (
          with lib;
          {
            ...
            stateDir = mkOption {
              type = types.str;
              default = "/var/lib/clover";
              description = "State directory. Contains .clover/ subdir (CLOVER_HOME).";
            };
            ...
            addToSystemPackages = mkOption {
              ...
              default = false;
              description = ''
                Add the clover CLI to environment.systemPackages and export
                CLOVER_HOME system-wide (via environment.variables) so interactive
            ...
            container = {
              ...
              hostUsers = mkOption {
                ...
                default = [ ];
                description = ''
                  Interactive users who get a ~/.clover symlink to the service
                  stateDir. These users are automatically added to the clover group.
                '';
                example = [ "sidbin" ];
              };
            };
      ...
      config = lib.mkIf cfg.enable (
        lib.mkMerge [
          ...
          # ── Host CLI ──────────────────────────────────────────────────────
          # Add the clover CLI to system PATH and export CLOVER_HOME system-wide
          # so interactive shells share state (sessions, skills, cron) with the
          # gateway service instead of creating a separate ~/.clover/.
          (lib.mkIf cfg.addToSystemPackages {
            environment.systemPackages = [ effectivePackage ];
            environment.variables.CLOVER_HOME = cloverHome;
          })
              ...
              ++ [
                {
                  ...
                  message = "services.clover-c1: backend.mode is not supported together with container.enable — the container runs the gateway only.";
                }
              ];
          }

          # ── Per-user profile for extraPackages ───────────────────────────
          # Wire extraPackages into the clover user's per-user profile so the
          ...
          (lib.mkIf
            (cfg.container.enable && !cfg.addToSystemPackages && cfg.container.hostUsers != [ ])
            {
              warnings = [
                ''
                  services.clover-c1: container.enable is true and container.hostUsers
                  is set, but addToSystemPackages is false. Without a host-installed clover
                  binary, container routing will not work for interactive users.
                  Set addToSystemPackages = true or ensure clover is on PATH.
                ''
              ];
            }
          )
          ...
          # ── Directories ───────────────────────────────────────────────────
          {
            systemd.tmpfiles.rules = [
              ...
              "d ${cloverHome}                  2770 ${cfg.user} ${cfg.group} - -"
              "d ${cfg.stateDir}/home           0750 ${cfg.user} ${cfg.group} - -"
              "d ${cfg.workingDirectory}        2770 ${cfg.user} ${cfg.group} - -"
            ]
            ++ map (d: "d ${cloverHome}/${d} 2770 ${cfg.user} ${cfg.group} - -") common.stateSubdirs;
          }

          # ── Activation: link config + auth + documents ────────────────────
          {
            system.activationScripts."clover-c1-setup" =
              lib.stringAfter
                (
                ...
                ''
                  ...
                  chown ${cfg.user}:${cfg.group} ${cfg.stateDir} ${cloverHome} ${cfg.stateDir}/home ${cfg.workingDirectory}
                  chmod 2770 ${cfg.stateDir} ${cloverHome} ${cfg.workingDirectory}
                  ...
                  find ${cloverHome} -maxdepth 1 \
                    \( -name "*.db" -o -name "*.db-wal" -o -name "*.db-shm" -o -name "SOUL.md" \) \
                    -exec chmod g+rw {} + 2>/dev/null || true
                  for _subdir in ${lib.concatStringsSep " " common.stateSubdirs}; do
                    mkdir -p "${cloverHome}/$_subdir"
                    chown ${cfg.user}:${cfg.group} "${cloverHome}/$_subdir"
                    chmod 2770 "${cloverHome}/$_subdir"
                    find "${cloverHome}/$_subdir" -type f \
                      -exec chmod g+rw {} + 2>/dev/null || true
                  done

                  ${common.mkStateScript {
                    inherit pkgs cfg cloverHome;
                    workingDirectory = cfg.workingDirectory;
                  ...
                  chown -h ${cfg.user}:${cfg.group} ${cloverHome}/plugins/nix-managed-* 2>/dev/null || true
                  ...
                  ${
                    if cfg.container.enable then
                      ''
                        install -o ${cfg.user} -g ${cfg.group} -m 0644 ${containerModeFile} ${cloverHome}/.container-mode
                      ''
                    else
                      ''
                        rm -f ${cloverHome}/.container-mode
                        ...
                        ${lib.concatStringsSep "\n" (
                          map (
                            ...
                            let
                              ...
                              symlinkPath = "${userHome}/.clover";
                            in
                            ''
                              if [ -L "${symlinkPath}" ] && [ "$(readlink "${symlinkPath}")" = "${cloverHome}" ]; then
                                rm -f "${symlinkPath}"
                                echo "clover-c1: removed symlink ${symlinkPath}"
                              fi
                            ''
                  ...
                  # Create ~/.clover -> stateDir/.clover for each hostUser so the
          ...
          (lib.mkIf (!cfg.container.enable) {
            systemd.services.clover-c1 = {
              ...
              # $CLOVER_HOME/.env by the activation script. load_clover_dotenv()
          ...
          (lib.mkIf (!cfg.container.enable && cfg.backend.mode != "none") {
            systemd.services.clover-backend = {
          ...
          (lib.mkIf cfg.container.enable {
            ...
            systemd.services.clover-c1 = {
              description = "Clover Cognition Gateway (container)";
              wantedBy = [ "multi-user.target" ];
              after = [
              ...
              preStart = ''
                ...
                if [ "$NEED_CREATE" = "true" ]; then
                  ...
                  CLOVER_UID=$(${pkgs.coreutils}/bin/id -u ${cfg.user})
                  CLOVER_GID=$(${pkgs.coreutils}/bin/id -g ${cfg.user})
                  ...
                  ${containerBin} create \
                    ...
                    --env CLOVER_UID="$CLOVER_UID" \
                    --env CLOVER_GID="$CLOVER_GID" \
                    --env CLOVER_HOME=${containerDataDir}/.clover \
                    --env CLOVER_MANAGED=true \
                    --env HOME=${containerHomeDir} \
                    ${lib.concatStringsSep " " cfg.container.extraOptions} \
                    ${cfg.container.image} \
                    ${containerDataDir}/current-package/bin/clover gateway run --replace ${lib.concatStringsSep " " cfg.extraArgs}
