...
#
# `services.clover-c1` is the same option set on both modules. Both modules
...
#   homeManagerModules.nix  cloverHome, programs.clover-c1 (the CLI and
...
let
  ...
  deepConfigType = types.mkOptionType {
    name = "clover-config-attrs";
    description = "Clover YAML config (attrset), merged deeply via lib.recursiveUpdate.";
    ...
    options = {
      ...
      auth = mkOption {
        ...
        description = ''
          ...
          (remote MCP servers). Tokens are stored in $CLOVER_HOME/mcp-tokens/.
        '';
      };
  ...
  sharedOptions =
    ...
    {
      enable = lib.mkEnableOption "Clover Cognition";
      ...
      package = mkOption {
        ...
        description = "The clover-c1 package to use.";
      };
      ...
      settings = mkOption {
        ...
        default = { };
        description = ''
          The Clover configuration, as an attribute set. The module joins the
          ...
          which includes the keys that `clover config set` and the settings
      ...
      environmentFiles = mkOption {
        ...
        description = ''
          ...
          $CLOVER_HOME/.env. Clover reads that file at each start, with
          load_clover_dotenv().

          Each activation writes .env again from the start. Thus a secret
          file cannot go into .env two times.
        '';
        example = literalExpression ''[ config.sops.secrets."clover/env".path ]'';
      };
      ...
      authFile = mkOption {
        ...
        description = ''
          ...
          does not exist. Thus a token that Clover refreshes at runtime stays
          after an activation.
        '';
      };
      ...
      documents = mkOption {
        ...
        description = ''
          ...
          Clover reads SOUL.md and memories/ from CLOVER_HOME, so put those
          files in `cloverHomeFiles`.
      ...
      cloverHomeFiles = mkOption {
        type = documentsType;
        default = { };
        description = ''
          Files that the module installs into CLOVER_HOME. Each key is a path
          relative to that directory, and the module makes the necessary
          subdirectories. Each value is a string or a path.

          Clover reads SOUL.md and the memory files from CLOVER_HOME and not
          from the working directory. Declare those files here, or Clover
          does not load them.
        '';
        example = literalExpression ''
          {
      ...
      extraPlugins = mkOption {
        ...
        default = [ ];
        description = ''
          Directory-based plugin packages to symlink into the clover plugins
          directory. Each package must contain a plugin.yaml and __init__.py
          at its root. Clover discovers these automatically on startup.
        '';
        example = literalExpression ''
          [
      ...
      extraPythonPackages = mkOption {
        ...
        description = ''
          ...
          clover-c1.plugins entry-point group. Each package must be built
          with the same Python interpreter as clover (python312).
        '';
        example = literalExpression ''
          [
      ...
      extraDependencyGroups = mkOption {
        ...
        description = ''
          ...
          Use this for optional extras already declared in clover-c1's
      ...
      extraArgs = mkOption {
        type = types.listOf types.str;
        default = [ ];
        description = "Extra command-line arguments for `clover gateway`.";
      };
      ...
      # ── The backend: `clover serve` or `clover dashboard` ──────────────
      # `clover serve` and `clover dashboard` are the same entry point,
      # clover_cli.main:cmd_dashboard, with one flag of difference. serve runs
      # without a user interface. dashboard also serves the web application.
      # Both give the /api/ws and /api/pty sockets that Clover Desktop
      ...
      # controls an external gateway, with `clover gateway restart`. It does
      ...
      backend = {
        mode = mkOption {
          ...
          description = ''
            ...
            - "serve"     — the backend without a user interface. It gives
                            the /api/ws and /api/pty sockets that Clover
                            Desktop connects to.
        ...
        sessionTokenFile = mkOption {
          ...
          description = ''
            ...
            CLOVER_DASHBOARD_SESSION_TOKEN. That token authorizes the /api
            routes and the /api/ws socket. Clover Desktop presents the same
          ...
          '';
          example = literalExpression ''config.sops.secrets."clover/desktop-token".path'';
        };
      };
    };
  ...
  # removal leaves them with no `clover` on the PATH and no message. The
  ...
  installPackageRemovedMessage =
    value:
    ''
      services.clover-c1.installPackage was removed. Clover now
      separates the installation from the services, which is the
      Home Manager convention:

        programs.clover-c1.enable = ${lib.boolToString (value != false)};  # the clover CLI, and CLOVER_HOME for your shells
        programs.clover-c1.desktop.enable = true;  # the desktop application

      `services.clover-c1` keeps the state, the configuration and
  ...
  mkConfigFiles =
    ...
    let
      generated = pkgs.writeText "clover-config.yaml" (
  ...
  mkDocumentTree =
    { pkgs, documents }:
    pkgs.runCommand "clover-documents" { } (
      ''
        mkdir -p $out
      ''
  ...
  mkEnvScript =
    { pkgs, environment }:
    let
      base = pkgs.writeText "clover-env-base" (
        lib.concatStringsSep "\n" (lib.mapAttrsToList (k: v: "${k}=${v}") environment)
        + lib.optionalString (environment != { }) "\n"
      );
    in
    pkgs.writeShellScript "clover-env-merge" ''
      set -eu
      ...
      for file in "$@"; do
        if [ -r "$file" ]; then
          ...
          echo "clover-c1: WARNING cannot read environmentFile $file" >&2
        fi
      done
    '';
  ...
  mkStateScript =
    {
      ...
      # CLOVER_MANAGED variable of the service. The value tells the shell
    ...
    in
    ''
      # Directories. The service units and Clover make most of these
      ...
      ${run}mkdir -p ${
        lib.escapeShellArgs (
          ...
          ++ map (d: "${cloverHome}/${d}") stateDirs
        )
      }

      # config.yaml: merge the Nix settings into the file on disk. Clover
      ...
      ${
        if cfg.configFile != null then
          "${inst} -m ${modes.config} -D ${configFiles.effective} ${cloverHome}/config.yaml"
        else
          ''
            ${run}${configFiles.mergeScript} ${configFiles.generated} ${cloverHome}/config.yaml
            ${run}chmod ${modes.config} ${cloverHome}/config.yaml
          ''
      }

      # The managed-mode marker. It makes an interactive shell also refuse to
      # change the configuration that Nix owns.
      ${inst} -m ${modes.managed} ${pkgs.writeText "clover-managed" managedSystem} ${cloverHome}/.managed

      ${lib.optionalString (cfg.environment != { } || cfg.environmentFiles != [ ]) ''
        ${run}${envScript} ${cloverHome}/.env ${modes.env} ${lib.escapeShellArgs cfg.environmentFiles}
        ${lib.optionalString (owner != null) "${run}chown ${owner} ${cloverHome}/.env"}
      ''}

      ${lib.optionalString (cfg.authFile != null) (
        if cfg.authFileForceOverwrite then
          "${inst} -m ${modes.auth} ${cfg.authFile} ${cloverHome}/auth.json"
        else
          ''
            if [ ! -e ${cloverHome}/auth.json ]; then
              ${inst} -m ${modes.auth} ${cfg.authFile} ${cloverHome}/auth.json
            fi
          ''
      )}
      ...
      ${installDocuments homeDocumentTree cloverHome cfg.cloverHomeFiles}
      ...
      ${run}find ${cloverHome}/plugins -maxdepth 1 -type l -name 'nix-managed-*' -delete 2>/dev/null || true
      ${lib.concatMapStringsSep "\n" (plugin: ''
        if [ ! -f ${plugin}/plugin.yaml ]; then
          echo "clover-c1: ERROR extraPlugins entry '${plugin}' has no plugin.yaml" >&2
          exit 1
        fi
        ${run}ln -sfn ${plugin} ${cloverHome}/plugins/nix-managed-${lib.getName plugin}
      '') cfg.extraPlugins}
    '';
  ...
  gatewayArgv =
    cfg:
    [
      "${effectivePackage cfg}/bin/clover"
      "gateway"
    ]
    ++ cfg.extraArgs;
  ...
  #
  # `exec` on the last line keeps clover as the MainPID of the unit. No shell
  ...
  backendLauncher =
    ...
    pkgs.writeShellScript "clover-backend-launch" (
      builtins.replaceStrings [ "@HOST@" ] [ ''"$_target"'' ] ''
        ...
        ${lib.optionalString (cfg.backend.sessionTokenFile != null) ''
          ...
          if [ ! -r "$_token_file" ]; then
            echo "clover-backend: cannot read the session token file '$_token_file'. The unit stops." >&2
            echo "clover-backend: backend.sessionTokenFile must name a runtime path that this user can read." >&2
            exit 1
          fi

          CLOVER_DASHBOARD_SESSION_TOKEN="$(${pkgs.coreutils}/bin/tr -d '\r\n' < "$_token_file")"
          export CLOVER_DASHBOARD_SESSION_TOKEN

          if [ -z "$CLOVER_DASHBOARD_SESSION_TOKEN" ]; then
            echo "clover-backend: the session token file '$_token_file' is empty. The unit stops." >&2
            exit 1
          fi
        ''}
        ${
          if cfg.backend.waitFor == null then
            ...
            ''
              ...
              while :; do
                ...
                if [ "$_waited" -ge "$_timeout" ]; then
                  echo "clover-backend: '$_target' did not resolve after ''${_timeout}s. The unit stops." >&2
                  exit 1
                fi

                if [ "$_waited" = 0 ]; then
                  echo "clover-backend: waits for '$_target' to resolve..." >&2
                fi
            ...
            ''
              ...
              while :; do
                ...
                if [ "$_waited" -ge "$_timeout" ]; then
                  echo "clover-backend: interface '$_iface' had no IPv4 address after ''${_timeout}s. The unit stops." >&2
                  echo "clover-backend: a fallback address can expose the backend more widely than you intend." >&2
                  exit 1
                fi

                if [ "$_waited" = 0 ]; then
                  echo "clover-backend: waits for an IPv4 address on '$_iface'..." >&2
                fi
                ${pkgs.coreutils}/bin/sleep 2
                _waited=$(( _waited + 2 ))
              done
        ...
        echo "clover-backend: binds to $_target:${toString cfg.backend.port} (from $_how)" >&2
  ...
  backendDescription =
    ...
    if cfg.backend.mode == "dashboard" then
      "Clover Cognition web dashboard and desktop backend"
    else
      "Clover Cognition backend for Clover Desktop";

  # The environment that each Clover process needs, from either module.
  #
  # managedSystem gives the value of CLOVER_MANAGED. The CLI reads that
    ...
    [
      {
        ...
        message = ''
          ...
          To give Clover an identity and a memory, use
          ${optionPath}.cloverHomeFiles instead. Those files go to
          CLOVER_HOME. Clover reads SOUL.md and memories/ only from there.
        '';
      }
    ];
  ...
  # The subdirectories of CLOVER_HOME that both modules make.
  stateSubdirs = [
    "cron"
    "sessions"
    "logs"
