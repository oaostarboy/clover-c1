...
let
    ...
    (callPackage pyproject-nix.build.packages {
      ...
      (
        lib.composeManyExtensions [
          ...
          # Clover derivation. This is deliberately a derivation environment
          # variable, not a devShell variable: ``nix develop -c uv build``
          # must remain blocked.
          (final: prev: {
            clover-c1 = prev.clover-c1.overrideAttrs (_old: {
              CLOVER_NIX_BUILD = "1";
            });
          })
        ]
  ...
  # $CLOVER_PYTHON_SRC_ROOT at runtime.
  workspaceRoot = ./..;
  ...
  editableOverlay = editableWorkspace.mkEditablePyprojectOverlay {
    root = "$CLOVER_PYTHON_SRC_ROOT"; # resolved at shellHook time
  };
  ...
  editableSet = pythonSet.overrideScope (
    lib.composeManyExtensions [
      editableOverlay
      (final: prev: {
        clover-c1 = prev.clover-c1.overrideAttrs (old: {
...
in
{
  venv = pythonSet.mkVirtualEnv "clover-c1-env" {
    clover-c1 = dependency-groups;
  };
  editableVenv = editableSet.mkVirtualEnv "clover-c1-editable-env" {
    clover-c1 = dependency-groups;
  };
}
