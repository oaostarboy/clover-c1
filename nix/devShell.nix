...
{ ... }:
{
  perSystem =
    ...
    let
      ...
      cloverNpmLib = self'.packages.default.passthru.cloverNpmLib;
      ...
      cloverAgentDevShellHook = self'.packages.default.passthru.devShellHook;
    in
    {
      devShells.default = pkgs.mkShell {
        packages = with pkgs; [
          (pkgs.runCommand "clover" { } ''
            mkdir -p $out/bin
            install -Dm755 ${../clover} $out/bin/clover
          '')
        ...
        shellHook = ''
          ${cloverAgentDevShellHook}
          ${cloverNpmLib.mkNpmDevShellHook npmPackageJsonPaths}
          ...
          export CLOVER_PYTHON_SRC_ROOT=$(git rev-parse --show-toplevel)
          ...
          echo "Clover Cognition dev shell in $CLOVER_PYTHON_SRC_ROOT"
          echo "Ready. Run 'clover' or 'sandbox clover' to start."
        '';
      };
    };
}
