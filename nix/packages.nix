# nix/packages.nix — Clover Cognition package built with uv2nix
{ inputs, ... }:
{
  perSystem =
    {
    ...
    let
      ...
      minimal = pkgs.callPackage ./clover-c1.nix {
    ...
    in
    {
      packages = {
        ...
        tui = full.cloverTui;
        web = full.cloverWeb;
        desktop = full.cloverDesktop;

        update-npm-lockfile = full.cloverNpmLib.updateNpmLockfile;
      };
    };
}
