...
{ inputs, ... }: {
  perSystem = { pkgs, lib, self', ... }:
    let
      clover-c1 = self'.packages.default;
      cloverVenv = clover-c1.cloverVenv;
      ...
      evalHomeModule =
        ...
        inputs.home-manager.lib.homeManagerConfiguration {
          ...
          modules = [
            ...
            {
              home = {
                username = "clover-check";
                homeDirectory = "/home/clover-check";
                stateVersion = "24.11";
              };
            }
            { services.clover-c1 = settings; }
      ...
      evalHomeSplit =
        ...
        inputs.home-manager.lib.homeManagerConfiguration {
          ...
          modules = [
            ...
            {
              programs.clover-c1 = programs;
              services.clover-c1 = services;
            }
          ];
        };

      # The option names that each module defines under
      # services.clover-c1. The internal names that the module system adds
      # are not in the list.
      moduleOptionNames =
        eval: lib.attrNames (lib.filterAttrs (n: _: !lib.hasPrefix "_" n) eval.options.services.clover-c1);
      ...
      configKeys = pkgs.runCommand "clover-config-keys" {} ''
        set -euo pipefail
        export HOME=$TMPDIR
        ${cloverVenv}/bin/python3 -c '
import json, sys
...
' > $out
    ...
    in {
      ...
      checks = {
        ...
        in pkgs.runCommand "clover-cross-eval" { } (
          if failures != [] then
        ...
        build-package = pkgs.runCommand "clover-build-package" { } ''
          echo "PASS: package built at ${clover-c1}"
          mkdir -p $out
          echo "ok" > $out/result
        '';

        # Verify the devShell builds successfully (cross-platform).
        build-devshell = pkgs.runCommand "clover-build-devshell" { } ''
          ...
          let
            enabled = evalHomeSplit {
              ...
              services = {
                ...
                environment.CLOVER_TEST = "1";
                environmentFiles = [ "/run/secrets/clover-env" ];
                cloverHomeFiles."SOUL.md" = "test soul";
            ...
            processes =
              if pkgs.stdenv.hostPlatform.isDarwin then
                ...
                }) (lib.filterAttrs (n: _: lib.hasPrefix "clover" n) cfg.launchd.agents)
              else
                lib.mapAttrs (_: unit: {
                  argv = [ unit.Service.ExecStart ];
                  env = unit.Service.Environment;
                }) (lib.filterAttrs (n: _: lib.hasPrefix "clover" n) cfg.systemd.user.services);
            ...
            failures =
              ...
              ) "gateway process does not run `clover gateway`: ${argvOf "clover-c1"}"
              ++ lib.optional (
                !lib.hasInfix "bin/clover serve" (argvOf "clover-backend")
              ) "backend process does not run `clover serve`: ${argvOf "clover-backend"}"
              ++ lib.optional (
                !lib.hasInfix "--no-open" (argvOf "clover-backend")
              ) "backend must pass --no-open so a service never opens a browser"
              ++ lib.optional (
                lib.any (n: !lib.hasInfix "/home/clover-check/.clover" (envOf n)) names
              ) "gateway and backend must share one CLOVER_HOME"
              ++ lib.optional (
                cfg.home.sessionVariables.CLOVER_HOME or null != "/home/clover-check/.clover"
              ) "programs.clover-c1.enable must export CLOVER_HOME for interactive shells"
              ++ lib.optional (
                !lib.hasInfix "clover-config-merge" activation
              ) "activation must deep-merge config.yaml, not overwrite it"
              ++ lib.optional (
                !lib.hasInfix "/home/clover-check/.clover/SOUL.md" activation
              ) "cloverHomeFiles must install into CLOVER_HOME"
              ++ lib.optional (
                !lib.hasInfix "/home/test-user/workspace/AGENTS.md" activation
              ) "documents must install into workingDirectory"
              # The CLI reads CLOVER_MANAGED to name the rebuild command when
              # it refuses to write the configuration. A Home Manager install
              # has no nixos-rebuild command. Thus it must not report NixOS.
              ++ lib.optional (
                !lib.any (n: lib.hasInfix "CLOVER_MANAGED=home-manager" (envOf n)) names
              ) "processes must report CLOVER_MANAGED=home-manager"
              ++ lib.optional (
                !lib.hasInfix "clover-managed" activation
              ) "activation must write a .managed marker naming the managing system";
          in
          pkgs.runCommand "clover-home-manager-module" { } (
            if failures != [ ] then
        ...
        workspace-files-need-a-directory =
          let
            ...
            # the priority.
            sameAsDefault = "/home/clover-check";
            ...
            cases = [
              ...
              {
                name = "cloverHomeFiles needs no directory";
                ok = accepts { cloverHomeFiles."SOUL.md" = "x"; };
              }
              {
          ...
          in
          pkgs.runCommand "clover-workspace-files-need-a-directory" { } (
            if failed != [ ] then
        ...
        # ── The desktop application shares one CLOVER_HOME ───────────────
        # `programs.enable` exports CLOVER_HOME with home.sessionVariables,
        ...
        # no shell profile. Thus the desktop application would open ~/.clover
        # while the services use the CLOVER_HOME of the module, and the user
        ...
        home-manager-desktop =
          let
            tokenFile = "/run/secrets/clover-desktop-token";
            ...
            enabled = evalHomeSplit {
              ...
              services = {
                enable = true;
                cloverHome = "/home/clover-check/.clover-work";
            ...
            desktopPackages = builtins.filter (p: (p.pname or "") == "clover-desktop") cfg.home.packages;
            ...
            # second Clover runtime beside the services is the fault that
            # `programs.enable` plus a plain desktop package would give.
            agentPackages = builtins.filter (p: (p.pname or "") == "clover-c1") cfg.home.packages;
            ...
            backendScript =
              let
                argv =
                  if pkgs.stdenv.hostPlatform.isDarwin then
                    cfg.launchd.agents.clover-backend.config.ProgramArguments
                  else
                    [ cfg.systemd.user.services.clover-backend.Service.ExecStart ];
            ...
            failures =
              ...
              ) "programs.desktop.enable must install exactly one clover-desktop package, got ${toString (lib.length desktopPackages)}"
              ++ lib.optional (
                setValue "CLOVER_HOME" != "/home/clover-check/.clover-work"
              ) "the launcher must carry CLOVER_HOME: a GUI launcher reads no shell profile, so home.sessionVariables never reaches it (got: ${toString (setValue "CLOVER_HOME")})"
              ++ lib.optional (
                setValue "CLOVER_MANAGED" != "home-manager"
              ) "the launcher must report CLOVER_MANAGED=home-manager while the services own the configuration (got: ${toString (setValue "CLOVER_MANAGED")})"
              ++ lib.optional (
                lib.length agentPackages == 1
                && setValue "CLOVER_DESKTOP_CLOVER" != "${lib.head agentPackages}/bin/clover"
              ) "the launcher must pin the agent package that programs.enable installs, and not a second runtime: ${toString (setValue "CLOVER_DESKTOP_CLOVER")}"

              # ── The application reaches the backend of the service ──────
              ++ lib.optional (
                setValue "CLOVER_DESKTOP_REMOTE_URL" != "http://127.0.0.1:9231"
              ) "the launcher must name the backend of the service, or the application starts a second one (got: ${toString (setValue "CLOVER_DESKTOP_REMOTE_URL")})"
              ++ lib.optional (
                !lib.hasInfix "CLOVER_DESKTOP_REMOTE_TOKEN" wrapper
              ) "the launcher must give a token with the URL: the desktop resolver throws when the URL is set alone"
              ++ lib.optional (
                !lib.hasInfix "CLOVER_DASHBOARD_SESSION_TOKEN" backendScript
              ...
              ++ lib.optional (
                setValue "CLOVER_DESKTOP_REMOTE_TOKEN" != null
              ) "the token must never be a --set value: makeWrapper writes it into the world-readable Nix store";
          in
          pkgs.runCommand "clover-home-manager-desktop" { } (
            if failures != [ ] then
              throw "Home Manager desktop check failed:\n${lib.concatMapStringsSep "\n" (f: "  - ${f}") failures}"
            else
              ''
                echo "PASS: the desktop launcher shares CLOVER_HOME, the runtime and the backend of the service"
                mkdir -p $out
                echo "ok" > $out/result
              ''
          );
        ...
        home-manager-desktop-standalone =
          let
            ...
            failures =
              ...
              ++ lib.optional (
                !lib.hasInfix "--set CLOVER_HOME" wrapper
              ) "the launcher must carry CLOVER_HOME even with no services"
              ++ lib.optional (
                lib.hasInfix "CLOVER_MANAGED" wrapper
              ) "the launcher must not claim a managed install when no activation writes one"
              ++ lib.optional (
                lib.hasInfix "CLOVER_DESKTOP_REMOTE_URL" wrapper
              ) "the launcher must not name a backend when the services run none"
              ++ lib.optional (
                cfg.systemd.user.services ? clover-backend || cfg.launchd.agents ? clover-backend
              ) "programs.enable alone must start no service";
          in
          pkgs.runCommand "clover-home-manager-desktop-standalone" { } (
            if failures != [ ] then
        ...
        # command line. A silent removal thus leaves them with no `clover`
        ...
        home-manager-install-package-removed =
          let
            ...
            cases = [
              {
                value = true;
                expect = "programs.clover-c1.enable = true;";
              }
              {
                value = false;
                expect = "programs.clover-c1.enable = false;";
              }
            ];
          ...
          in
          pkgs.runCommand "clover-home-manager-install-package-removed" { } (
            if failures != [ ] then
        ...
        module-option-parity =
          ...
          in
          pkgs.runCommand "clover-module-option-parity" { } (
            if failures != [ ] then
        ...
        nixos-module =
          let
            ...
            units = lib.filterAttrs (n: _: lib.hasPrefix "clover" n) cfg.systemd.services;
            ...
            failures =
              ...
              ) "gateway unit does not run `clover gateway`: ${execOf "clover-c1"}"
              ++ lib.optional (
                !lib.hasInfix "bin/clover dashboard" (execOf "clover-backend")
              ) "backend unit does not run `clover dashboard`: ${execOf "clover-backend"}"
              ++ lib.optional (
                units.clover-c1.environment.CLOVER_HOME != units.clover-backend.environment.CLOVER_HOME
              ) "gateway and backend must share one CLOVER_HOME"
              ++ lib.optional (
                !lib.hasInfix "/var/lib/clover/.clover/SOUL.md" activation
              ) "cloverHomeFiles must install into CLOVER_HOME";
          ...
          in
          pkgs.runCommand "clover-nixos-module" { } (
            if failures != [ ] then
        ...
        # polls and then execs clover, and the assertions reject a
        # configuration that cannot work.
        backend-bind-wait =
          let
            execOf =
              settings:
              (evalNixosModule ({ enable = true; } // settings)).config.systemd.services.clover-backend.serviceConfig.ExecStart;
            ...
            failures =
              ...
              lib.optional (!lib.hasInfix "bin/clover serve --host 127.0.0.1" direct)
                "without waitFor the backend must exec clover directly, got: ${direct}"
              ++ lib.optional (lib.hasInfix "clover-backend-launch" direct)
                "without waitFor the backend must not use the launcher"

              # The hostname mode polls the resolver, then binds the name.
              ++ lib.optional (!lib.hasInfix "clover-backend-launch" hostnameWait)
              ...
              ++ lib.optional (!lib.hasInfix "exec " hostnameScript)
                "the launcher must exec clover, so that it keeps the MainPID"
              ...
              ++ lib.optional (!lib.hasInfix "bin/clover dashboard" interfaceScript)
          ...
          in
          pkgs.runCommand "clover-backend-bind-wait" { } (
            if failures != [ ] then
        ...
        # $CLOVER_HOME/.env. The important property is that a second run
        ...
        env-file-assembly =
          let
            envScript = (import ./moduleCommon.nix { inherit lib; }).mkEnvScript {
              inherit pkgs;
              environment = {
                CLOVER_PUBLIC = "visible";
              };
            };
          in
          pkgs.runCommand "clover-env-file-assembly" { } ''
            set -e
            workdir=$(mktemp -d)
            ...
            grep -qx 'CLOVER_PUBLIC=visible' "$workdir/.env" || \
        ...
        service-argv =
          let
            ...
            sentinel = "--clover-nix-argv-probe";
            probe = argv: lib.escapeShellArgs (argv ++ [ sentinel ]);
          in
          pkgs.runCommand "clover-service-argv" { } ''
            set -e
        ...
        package-contents = pkgs.runCommand "clover-package-contents" { } ''
          set -e
          echo "=== Checking binaries ==="
          test -x ${clover-c1}/bin/clover || (echo "FAIL: clover binary missing"; exit 1)
          test -x ${clover-c1}/bin/clover-c1 || (echo "FAIL: clover-c1 binary missing"; exit 1)
          echo "PASS: All binaries present"

          echo "=== Checking version ==="
          ${clover-c1}/bin/clover --version 2>&1 | grep -qi "clover" || (echo "FAIL: version check"; exit 1)
        ...
        entry-points-sync = pkgs.runCommand "clover-entry-points-sync" { } ''
          set -e
          echo "=== Checking entry points match pyproject.toml [project.scripts] ==="
          for bin in clover clover-c1 clover-acp; do
            test -x ${clover-c1}/bin/$bin || (echo "FAIL: $bin binary missing from Nix package"; exit 1)
        ...
        cli-commands = pkgs.runCommand "clover-cli-commands" { } ''
          set -e
          export HOME=$(mktemp -d)

          echo "=== Checking clover --help ==="
          ${clover-c1}/bin/clover --help 2>&1 | grep -q "gateway" || (echo "FAIL: gateway subcommand missing"; exit 1)
          ${clover-c1}/bin/clover --help 2>&1 | grep -q "config" || (echo "FAIL: config subcommand missing"; exit 1)
        ...
        bundled-skills = pkgs.runCommand "clover-bundled-skills" { } ''
          set -e
          echo "=== Checking bundled skills ==="
          test -d ${clover-c1}/share/clover-c1/skills || (echo "FAIL: skills directory missing"; exit 1)
          echo "PASS: skills directory exists"

          # -L: skills/ is a symlink to the filtered source store path
          SKILL_COUNT=$(find -L ${clover-c1}/share/clover-c1/skills -name "SKILL.md" | wc -l)
          test "$SKILL_COUNT" -gt 0 || (echo "FAIL: no SKILL.md files found in skills directory"; exit 1)
          echo "PASS: $SKILL_COUNT bundled skills found"

          grep -q "CLOVER_BUNDLED_SKILLS" ${clover-c1}/bin/clover || \
            (echo "FAIL: CLOVER_BUNDLED_SKILLS not in wrapper"; exit 1)
          echo "PASS: CLOVER_BUNDLED_SKILLS set in wrapper"

          # Optional skills ship via the wrapper too (pythonSrc excludes
          # them from the wheel, so the env var is the only path in nix).
          test -d ${clover-c1}/share/clover-c1/optional-skills || \
            (echo "FAIL: optional-skills directory missing"; exit 1)
          OPT_COUNT=$(find -L ${clover-c1}/share/clover-c1/optional-skills -name "SKILL.md" | wc -l)
          test "$OPT_COUNT" -gt 0 || (echo "FAIL: no SKILL.md files in optional-skills"; exit 1)
          grep -q "CLOVER_OPTIONAL_SKILLS" ${clover-c1}/bin/clover || \
            (echo "FAIL: CLOVER_OPTIONAL_SKILLS not in wrapper"; exit 1)
          echo "PASS: $OPT_COUNT optional skills found, CLOVER_OPTIONAL_SKILLS set in wrapper"
        ...
        bundled-plugins = pkgs.runCommand "clover-bundled-plugins" { } ''
          set -e
          echo "=== Checking bundled plugins ==="
          test -d ${clover-c1}/share/clover-c1/plugins || (echo "FAIL: plugins directory missing"; exit 1)
          echo "PASS: plugins directory exists"

          test -f ${clover-c1}/share/clover-c1/plugins/platforms/irc/plugin.yaml || \
            (echo "FAIL: irc plugin manifest missing"; exit 1)
          echo "PASS: irc plugin manifest present"

          grep -q "CLOVER_BUNDLED_PLUGINS" ${clover-c1}/bin/clover || \
            (echo "FAIL: CLOVER_BUNDLED_PLUGINS not in wrapper"; exit 1)
          echo "PASS: CLOVER_BUNDLED_PLUGINS set in wrapper"
        ...
        bundled-locales = pkgs.runCommand "clover-bundled-locales" { } ''
          set -e
          echo "=== Checking bundled locales ==="
          test -d ${clover-c1}/share/clover-c1/locales || (echo "FAIL: locales directory missing"; exit 1)
          echo "PASS: locales directory exists"

          # -L: locales/ is a symlink to the source store path
          LOC_COUNT=$(find -L ${clover-c1}/share/clover-c1/locales -name "*.yaml" | wc -l)
          test "$LOC_COUNT" -ge 16 || (echo "FAIL: expected >=16 catalogs, found $LOC_COUNT"; exit 1)
          echo "PASS: $LOC_COUNT locale catalogs found"

          test -f ${clover-c1}/share/clover-c1/locales/en.yaml || (echo "FAIL: en.yaml missing"; exit 1)
          echo "PASS: en.yaml present"

          grep -q "CLOVER_BUNDLED_LOCALES" ${clover-c1}/bin/clover || \
            (echo "FAIL: CLOVER_BUNDLED_LOCALES not in wrapper"; exit 1)
          echo "PASS: CLOVER_BUNDLED_LOCALES set in wrapper"

          # locales/ is a bare data dir (no __init__.py), shipped via a
          # symlink + CLOVER_BUNDLED_LOCALES (not via wheel data-files).
          # Verify the wrapper override resolves real strings.
          export HOME=$(mktemp -d)
          RENDERED=$(cd "$HOME" && CLOVER_BUNDLED_LOCALES=${clover-c1}/share/clover-c1/locales \
            ${cloverVenv}/bin/python3 -c "from agent import i18n; print(i18n.t('gateway.reset.header_default', lang='en'))")
          echo "rendered: $RENDERED"
          test "$RENDERED" != "gateway.reset.header_default" || (echo "FAIL: i18n returned the raw key with CLOVER_BUNDLED_LOCALES set"; exit 1)
        ...
        # CLOVER_OPTIONAL_MCPS (not via wheel data-files).
        bundled-mcps = pkgs.runCommand "clover-bundled-mcps" { } ''
          set -e
          echo "=== Checking bundled optional-mcps ==="
          test -d ${clover-c1}/share/clover-c1/optional-mcps || (echo "FAIL: optional-mcps directory missing"; exit 1)
          echo "PASS: optional-mcps directory exists"

          MANIFEST_COUNT=$(find -L ${clover-c1}/share/clover-c1/optional-mcps -name "manifest.yaml" | wc -l)
          test "$MANIFEST_COUNT" -gt 0 || (echo "FAIL: no manifest.yaml files found"; exit 1)
          echo "PASS: $MANIFEST_COUNT catalog manifests found"

          grep -q "CLOVER_OPTIONAL_MCPS" ${clover-c1}/bin/clover || \
            (echo "FAIL: CLOVER_OPTIONAL_MCPS not in wrapper"; exit 1)
          echo "PASS: CLOVER_OPTIONAL_MCPS set in wrapper"

          export HOME=$(mktemp -d)
          CATALOG=$(cd "$HOME" && ${clover-c1}/bin/clover mcp catalog 2>/dev/null || true)
          echo "catalog output: $CATALOG"
          test -n "$CATALOG" || (echo "FAIL: clover mcp catalog returned empty"; exit 1)
        ...
        bundled-tui = pkgs.runCommand "clover-bundled-tui" { } ''
          set -e
          echo "=== Checking bundled TUI ==="
          test -d ${clover-c1}/ui-tui || (echo "FAIL: ui-tui directory missing"; exit 1)
          echo "PASS: ui-tui directory exists"

          test -f ${clover-c1}/ui-tui/dist/entry.js || (echo "FAIL: compiled entry.js missing"; exit 1)
          echo "PASS: compiled entry.js present"

          # self-contained bundle; no runtime node_modules expected

          grep -q "CLOVER_TUI_DIR" ${clover-c1}/bin/clover || \
            (echo "FAIL: CLOVER_TUI_DIR not in wrapper"; exit 1)
          echo "PASS: CLOVER_TUI_DIR set in wrapper"
        ...
        # Verify CLOVER_NODE is set in wrapper and points to Node 26+
        # (Clover pins its toolchain to Node 26 everywhere)
        clover-node = pkgs.runCommand "clover-node-version" { } ''
          set -e
          echo "=== Checking CLOVER_NODE in wrapper ==="
          grep -q "CLOVER_NODE" ${clover-c1}/bin/clover || \
            (echo "FAIL: CLOVER_NODE not set in wrapper"; exit 1)
          echo "PASS: CLOVER_NODE present in wrapper"

          CLOVER_NODE=$(sed -n "s/^export CLOVER_NODE='\(.*\)'/\1/p" ${clover-c1}/bin/clover)
          test -x "$CLOVER_NODE" || (echo "FAIL: CLOVER_NODE=$CLOVER_NODE not executable"; exit 1)
          echo "PASS: CLOVER_NODE executable at $CLOVER_NODE"

          NODE_MAJOR=$("$CLOVER_NODE" --version | sed 's/^v//' | cut -d. -f1)
          test "$NODE_MAJOR" -ge 26 || \
            (echo "FAIL: Node v$NODE_MAJOR < 26, Clover requires Node 26"; exit 1)
          echo "PASS: Node v$NODE_MAJOR >= 26"

          echo "=== All CLOVER_NODE checks passed ==="
          mkdir -p $out
          echo "ok" > $out/result
        '';

        # Verify CLOVER_MANAGED guard works on all mutation commands
        managed-guard = pkgs.runCommand "clover-managed-guard" { } ''
          set -e
          export HOME=$(mktemp -d)
          ...
          check_blocked() {
            local label="$1"
            shift
            OUTPUT=$(CLOVER_MANAGED=true "$@" 2>&1 || true)
          ...
          echo "=== Checking CLOVER_MANAGED guards ==="
          check_blocked "config set" ${clover-c1}/bin/clover config set model foo
          check_blocked "config edit" ${clover-c1}/bin/clover config edit
        ...
        extra-python-packages = let
          ...
          cloverWithExtra = clover-c1.override {
            extraPythonPackages = [ testPkg ];
          };
        in pkgs.runCommand "clover-extra-python-packages" { } ''
          set -e
          echo "=== Checking extraPythonPackages PYTHONPATH injection ==="

          grep -q "PYTHONPATH" ${cloverWithExtra}/bin/clover || \
            (echo "FAIL: PYTHONPATH not in wrapper"; exit 1)
          echo "PASS: PYTHONPATH present in wrapper"

          grep -q "${testPkg}" ${cloverWithExtra}/bin/clover || \
          ...
          if grep -q "PYTHONPATH" ${clover-c1}/bin/clover; then
        ...
        extra-dependency-groups = let
          cloverWithGroups = clover-c1.override {
            extraDependencyGroups = [ "honcho" ];
          };
        in pkgs.runCommand "clover-extra-dependency-groups" { } ''
          set -e
          ...
          echo "derivation: ${cloverWithGroups}"
          echo "venv: ${cloverWithGroups.cloverVenv}"
        ...
        messaging-variant = pkgs.runCommand "clover-messaging-variant" { } ''
          set -e
          echo "=== Checking discord.py importable from messaging variant ==="
          ${self'.packages.messaging.cloverVenv}/bin/python3 -c \
        ...
        in pkgs.runCommand "clover-config-roundtrip" {
          ...
          merge_and_load() {
            local clover_home="$1"
            export CLOVER_HOME="$clover_home"
            ${configMergeScript} ${nixSettings} "$clover_home/config.yaml"
            ${cloverVenv}/bin/python3 -c '
import json, sys
