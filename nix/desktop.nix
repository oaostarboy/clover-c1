# nix/desktop.nix — Clover Desktop (Electron) app build + wrapper
#
# `cloverAgent` is the fully-built `.#default` package — it ships the
# `clover` binary with the venv, runtime PATH, bundled skills/plugins, etc.
# already wired up.  We point the desktop at it via the existing
# `CLOVER_DESKTOP_CLOVER` override env var, so the desktop's resolver
# uses our fully wrapped binary at step 4 ("existing Clover CLI").
# No reimplementation of the agent resolution in this wrapper.
{
  ...
  # CLOVER_HOME and CLOVER_MANAGED here, which gives the app the same state
...
let
  ...
  targetPlatform =
    if stdenv.hostPlatform.isDarwin then
      ...
      throw "clover-desktop: unsupported host platform for node-pty staging";
  ...
  targetArch =
    if stdenv.hostPlatform.isAarch64 then
      ...
      throw "clover-desktop: unsupported host arch for node-pty staging";

  # Build the renderer (dist/ + electron/ + package.json).
  renderer = cloverNpmLib.buildNpmPackage {
    dirs = [
      "apps/desktop"
      "apps/shared"
    ];
    pname = "clover-desktop-renderer";
    ...
    buildPhase = ''
      ...
      pushd apps/desktop
        ...
        ${lib.getExe cloverNpmLib.node-gyp} rebuild \
...
stdenv.mkDerivation {
  ...
  installPhase = ''
    ...
    mkdir -p $out/share/clover-desktop $out/bin
    cp -r ${renderer}/* $out/share/clover-desktop/
    ...
    substituteInPlace $out/share/clover-desktop/dist/electron-main.mjs \
      --replace-fail "process.resourcesPath" "'$out/share/clover-desktop'"

    # Wrap the nixpkgs electron binary to launch our app.  Set
    # CLOVER_DESKTOP_CLOVER to the absolute path of the nix-built `clover`
    # binary so the desktop's resolver step 4 ("existing Clover CLI on
    # PATH") uses our fully wrapped binary — venv with all deps,
    # bundled skills/plugins, runtime PATH (ripgrep/git/ffmpeg/etc).
    # No reimplementation of the agent resolver in the wrapper.
    makeWrapper ${lib.getExe electron} $out/bin/clover-desktop \
      --add-flags "$out/share/clover-desktop" \
      --set CLOVER_DESKTOP_CLOVER "${lib.getExe cloverAgent}" \
    ...
    install -m 0644 ${../apps/desktop/assets/icon.png} \
      $out/share/icons/hicolor/1024x1024/apps/clover.png
    export PYTHONPATH=$(mktemp -d)
    cp ${../clover_cli/linux_desktop_entry.py} "$PYTHONPATH/linux_desktop_entry.py"
    export DESKTOP_EXEC="$out/bin/clover-desktop"
    export DESKTOP_ICON="$out/share/icons/hicolor/1024x1024/apps/clover.png"
    python3 -c 'import os; from linux_desktop_entry import render_desktop_entry; print(render_desktop_entry(os.environ["DESKTOP_EXEC"], os.environ["DESKTOP_ICON"]))' > $out/share/applications/clover.desktop
    runHook postInstall
  '';
