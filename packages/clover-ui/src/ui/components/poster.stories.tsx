...
const meta: Meta<typeof Poster> = {
  ...
  parameters: {
    docs: {
      description: {
        component:
          'A social-ready glitchy card built around the haptic-distortion image component. Defaults to the `vibe` variant — full-bleed distortion with minimal registration chrome — matching the overlay on the Clover agent website. Switch to `variant="dispatch"` for the broadcast-card layout with copy + sidebar tags.\n\nPoster stories include all five tier images from the `manage-subscription` page so you can screen-record any character on-demand.'
      }
    },
    layout: 'centered'
  },
 ...
 * minimal chrome, just the subtle `Clover Cognition` overlay at the bottom-right.
    ...
    <div className="bg-background flex min-h-screen items-center justify-center p-8">
      <div className="flex items-stretch gap-4">
        <Poster
          ...
          headline={showLabels ? ['Clover', 'Studio.'] : ['']}
          seal={showLabels ? 'FREE · MIT' : undefined}
          signature={showLabels ? 'cloverc1.dev / studio' : undefined}
          src={TIERS[0].src}
        ...
        <Poster
          ...
          headline={showLabels ? ['Clover', 'Pro.'] : ['']}
          seal={showLabels ? '$20/mo' : undefined}
          signature={showLabels ? 'cloverc1.dev / pro' : undefined}
          src={TIERS[2].src}
          tags={
            showLabels
        ...
        <Poster
          ...
          headline={showLabels ? ['Clover', 'Sovereign.'] : ['']}
          seal={showLabels ? 'talk to us' : undefined}
          signature={showLabels ? 'cloverc1.dev / sovereign' : undefined}
          src={TIERS[3].src}
          tags={
            showLabels
