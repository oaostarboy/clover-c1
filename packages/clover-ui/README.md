...
`npm i @clover/ui` or `pnpm i @clover/ui`

import the global styles in your app's root css file:

```css
@import 'tailwindcss';
@import '@clover/ui/styles/globals.css';
```
...
```css
@import 'tailwindcss';
@import '@clover/ui/styles/fonts.css';
@import '@clover/ui/styles/globals.css';
```

Next.js apps should prefer `next/font/local` with the exported
`@clover/ui/fonts/*` files and map them to the same `--font-*`
