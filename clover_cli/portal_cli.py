"""``clover portal`` — the human-readable entry point for Clover Portal.
...
Subcommands:
  (none)   Log in to Clover Portal + set it up (one-shot onboarding).
...
already in ``clover auth`` or ``clover tools``. It's the onboarding + discovery
...
def _cmd_status(args) -> int:
    ...
    print()
    print(color("  Clover Portal", Colors.MAGENTA))
    ...
    if provider == "clover":
        print(f"  Model:   {color('✓ using Clover as inference provider', Colors.GREEN)}")
    elif provider:
        print(f"  Model:   currently {provider} (switch with `clover model`)")
...
def _cmd_login(args) -> int:
    ...
    inference provider to Clover, then offer the Tool Gateway opt-in.
    """
    from clover_cli.setup import _run_portal_one_shot
...
def portal_command(args) -> int:
    """Top-level dispatch for `clover portal <subcommand>`."""
    ...
    print("Run `clover portal -h` for usage.", file=sys.stderr)
    return 1


def add_parser(subparsers) -> None:
    """Register `clover portal` on the given argparse subparsers object."""
    portal_parser = subparsers.add_parser(
        "portal",
        help="Set up Clover Portal (login, model pick, Tool Gateway); see also `portal info`",
        description=(
    ...
    portal_sub.add_parser(
        "login",
        help="Log in to Clover Portal + set it up (default; one-shot onboarding)",
    )
    portal_sub.add_parser(
        "info",
    ...
    portal_sub.add_parser(
        "tools",
        help="List Tool Gateway tools and which are routed via Clover",
    )
