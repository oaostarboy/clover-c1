...
def _run_post_setup(post_setup_key: str):
    ...
    if post_setup_key in {"agent_browser", "browserbase"}:
        ...
        if _running_in_docker():
            ...
            _print_info(
                "      docker pull ghcr.io/clovercognition/clover-c1:latest"
            )
            return
...
#:
#: Not gated on a Clover sign-in here: the six ``bfl_flux3_*`` tools carry
#: ``check_fn=check_bfl_requirements``, so an enabled toolset still ships zero
#: schemas to a user with no Clover credential — the same split Home Assistant
...
def tools_command(args=None, first_install: bool = False, config: dict = None):
    ...
    print(color("  Guide: https://clover-c1.local/docs/user-guide/features/tools", Colors.DIM))
    print()
...
def _configure_mcp_tools_interactive(config: dict):
    ...
    for server_name, tools in server_tools.items():
        ...
        # tools.include across the codebase (catalog installs, clover mcp
