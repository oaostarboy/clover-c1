"""
Unified tool configuration for Clover Cognition.

`clover tools` and `clover setup tools` both enter this module.
Select a platform → toggle toolsets on/off → for newly enabled tools
that need API keys, run through provider-aware configuration.

Saves per-platform tool configuration to ~/.clover/config.yaml under
...
def _post_setup_no_window_flags(*, streams_to_console: bool = False) -> int:
    ...
    ``clover tools post-setup <key>`` child. On Windows, every console child
    ...
    :func:`clover_cli._subprocess_compat.windows_hide_flags`) suppresses it
...
from clover_cli.cli_output import (  # noqa: E402 — late import block
...
# They're still in _CLOVER_CORE_TOOLS (available at runtime if enabled),
# but the setup checklist won't pre-select them for first-time users.
#
# Video gen is off by default — it's a niche, paid, slow feature. Users
# who want it opt in via `clover tools` → Video Generation, which walks
...
# `clover tools` → X (Twitter) Search setup walks users through credential
# setup. The tool's check_fn means the schema still won't appear to the
# model if the credential later goes missing or expires.
_DEFAULT_OFF_TOOLSETS = {"homeassistant", "spotify", "discord", "discord_admin", "video", "video_gen", "x_search", "a2a"}


# Config-only capabilities: they appear in `clover tools` for provider/API-key
...
def _xai_credentials_present() -> bool:
    ...
    """
    try:
        from clover_cli.auth import _read_xai_oauth_tokens
...
# Platform-scoped toolsets: only appear in the `clover tools` checklist for
...
def _get_effective_configurable_toolsets():
    ...
    Without the dedupe, ``clover tools`` → "reconfigure existing" would
    list the same toolset twice.
    """
    result = list(CONFIGURABLE_TOOLSETS)
    seen = {ts_key for ts_key, _, _ in result}
    try:
        from clover_cli.plugins import discover_plugins, get_plugin_toolsets
...
def _get_plugin_toolset_keys() -> set:
    ...
    try:
        from clover_cli.plugins import get_plugin_toolset_keys_nowait
...
def _checklist_toolset_keys(platform: str) -> Set[str]:
    """Return the toolset keys the ``clover tools`` checklist actually offers
    for ``platform``.
    ...
    so ``clover tools`` never claims to add or remove a toolset the user was
...
from clover_cli.platforms import PLATFORMS as _PLATFORMS_REGISTRY
...
TOOL_CATEGORIES = {
    ...
    "x_search": {
        ...
        "setup_note": (
            "Clover routes X searches through xAI's built-in x_search "
        ...
        "providers": [
            {
                ...
                "env_vars": [
                    ...
                    # extra keys required. Set CLOVER_CUA_DRIVER_CMD to use a
        ...
        "providers": [
            {
                ...
                "env_vars": [
                    {"key": "CLOVER_LANGFUSE_PUBLIC_KEY", "prompt": "Langfuse public key (pk-lf-...)", "url": "https://cloud.langfuse.com"},
                    {"key": "CLOVER_LANGFUSE_SECRET_KEY", "prompt": "Langfuse secret key (sk-lf-...)", "url": "https://cloud.langfuse.com"},
                ],
                "post_setup": "langfuse",
            },
            {
                ...
                "env_vars": [
                    {"key": "CLOVER_LANGFUSE_PUBLIC_KEY", "prompt": "Langfuse public key (pk-lf-...)"},
                    {"key": "CLOVER_LANGFUSE_SECRET_KEY", "prompt": "Langfuse secret key (sk-lf-...)"},
                    {"key": "CLOVER_LANGFUSE_BASE_URL", "prompt": "Langfuse server URL (e.g. http://localhost:3000)", "default": "http://localhost:3000"},
                ],
                "post_setup": "langfuse",
            },
        ],
...
# provider+model picker like `clover model` — NOT this single-key prompt, so
...
def _cua_driver_cmd() -> str:
    ...
    return os.environ.get("CLOVER_CUA_DRIVER_CMD", "").strip() or "cua-driver"


def _cua_version_summary(raw: str, *, limit: int = 120) -> str:
    """Reduce a driver's ``--version`` output to one short status line.

    A binary selected by ``CLOVER_CUA_DRIVER_CMD`` is not obliged to answer
...
def _cua_driver_env() -> dict:
    """cua-driver child env with the Clover telemetry policy applied.
...
def _cua_driver_contract_status(binary: Optional[str] = None) -> dict:
    """Inspect whether an installed driver supports Clover' runtime contract."""
    import time
...
def _pip_install(
    ...
    *,
    ...
    # Managed uv first: $CLOVER_HOME/bin is never on PATH, so a bare which()
    # misses the uv Clover installed and prefers a system one when both exist.
...
def install_cua_driver(
    ...
    * ``upgrade=True`` — always re-run the installer (or call ``cua-driver
      update`` if the binary supports it). Used by ``clover update`` and
      by ``clover computer-use install --upgrade``.
    ...
    falling through to the full upstream installer. ``clover update`` sets
    ...
    a further ~600s wait on Windows). ``clover computer-use install
    ...
    ``clover update`` already prints a contextual line before its update
    ...
    ``clover update`` and must not inherit the explicit install command's
    ...
    if system not in ("Darwin", "Windows", "Linux"):
        if upgrade:
            # Silent on unsupported platforms — `clover update` calls this
    ...
    override = os.environ.get("CLOVER_CUA_DRIVER_CMD", "").strip()
    if override and not binary:
        _print_warning(
            "    CLOVER_CUA_DRIVER_CMD does not resolve to an executable: "
            f"{override}"
        )
        _print_info(
    ...
    # An installed driver that fails Clover' runtime contract (version floor,
    # missing manifest verbs) is repaired regardless of the caller's mode.
    # Clover' own minimum requirement IS the confirmation that an upgrade is
    ...
    # contract landed, with `clover update` declining to refresh).
    ...
    if repair_existing:
        ...
        _print_warning(
            f"    Found cua-driver {version}, but Clover cannot use its current "
            f"runtime contract: {reason}."
        )
        if os.environ.get("CLOVER_CUA_DRIVER_CMD", "").strip():
            _print_info(
        ...
        if is_windows and require_confirmed_update:
            ...
            _print_info(
                ...
                "clover computer-use install --upgrade"
            )
            return False
    ...
    if not _cua_install_target_writable():
        ...
        _print_info(
            "    Run `clover computer-use install --upgrade` from an admin account to update it."
        )
        return bool(binary)
    ...
    # caller: `clover update` (require_confirmed_update=True) keeps the
    ...
    # `clover computer-use install --upgrade` falls through and re-runs the
    # installer as before.
    ...
    if binary and not repair_existing:
        ...
        if _state is None and require_confirmed_update:
            ...
            _print_info(
                "    Force a refresh with: clover computer-use install --upgrade"
            )
            return True
    ...
    if ok and repair_existing:
        ...
        if not repaired.get("ready"):
            ...
            _print_info("    Run: clover computer-use doctor")
            return False
    if ok and before:
        try:
...
# Optional refreshes launched by ``clover update`` are quiet and unattended.
...
def _run_cua_driver_installer(
    ...
    * Windows       → ``powershell -NoProfile -ExecutionPolicy Bypass -Command
    ...
    # Unattended refreshes (installer_timeout set by `clover update`) fail
    ...
    def _reap_after_timeout(proc):
        ...
        ceiling stops bounding anything and ``clover update`` hangs past its
    ...
    try:
        # When not verbose (e.g. `clover update`'s refresh), capture the
        ...
        if verbose:
            ...
            # Preserve the full installer output. During `clover update`,
            # sys.stdout is the mirroring _UpdateOutputStream whose `_log`
            # handle is ~/.clover/logs/update.log — write straight to it so
        ...
        if result.returncode == 0 and installed_binary:
            ...
            if verbose:
                ...
                if is_windows:
                    ...
                    _print_info("      System Settings > Privacy & Security > Accessibility")
                    _print_info("      System Settings > Privacy & Security > Screen Recording")
                    _print_info("    Both must allow the terminal / Clover process.")
            return True
...
def _ensure_browser_use_cli(*, verbose_hints: bool = False) -> None:
    ...
    Firecrawl, and the Clover-managed cloud rows all execute through
    ...
    check — only the Clover-managed ``$CLOVER_HOME/bin`` copy does.
    ``install_cli()`` short-circuits on the managed copy and otherwise
    provisions it, so resolution always lands on a binary Clover installs
...
def _run_post_setup(post_setup_key: str):
    ...
    if post_setup_key in {"agent_browser", "browserbase"}:
        ...
        # (PATH -> Homebrew/Clover-managed node -> npx) rather than a bare
        # shutil.which — Clover-managed-Node-only setups resolve agent-browser
        ...
        if _running_in_docker():
            ...
            _print_info(
                "      docker pull ghcr.io/cloverc1/clover-c1:latest"
            )
            return

        # browser_cmd was already resolved above (same PATH -> Homebrew ->
        # Clover-managed-node -> npx cascade _find_agent_browser uses at
        ...
        if _is_npx_agent_browser_sentinel(browser_cmd):
            ...
            # — Clover-managed-Node-only setups resolve npx only through the
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                _print_info("    Change via stt.local.model in ~/.clover/config.yaml")
            else:
        ...
        _print_info("    Switch voices by setting tts.piper.voice in ~/.clover/config.yaml")
        ...
        # Run the full `clover auth spotify` flow — if the user has no
        # client_id yet, this drops them into the interactive wizard
        # (opens the Spotify dashboard, prompts for client_id, persists
        # to ~/.clover/.env), then continues straight into PKCE. If they
        # already have an app, it skips the wizard and just does OAuth.
        from types import SimpleNamespace
        try:
            from clover_cli.auth import login_spotify_command
        except Exception as exc:
            _print_warning(f"    Could not load Spotify auth: {exc}")
            _print_info("    Run manually: clover auth spotify")
            return
        ...
        try:
            ...
            # toolset enable; they can retry with `clover auth spotify`.
            _print_warning(f"    Spotify login did not complete: {exc}")
            _print_info("    Run later: clover auth spotify")
        except Exception as exc:
        ...
        try:
            from clover_cli.plugins_cmd import _get_enabled_set, _save_enabled_set
            ...
            _print_info("    Run manually: clover plugins enable observability/langfuse")
        _print_info("    Restart Clover for tracing to take effect.")
        _print_info("    Verify: clover plugins list")
        ...
        try:
            ...
            _print_info("    Run later: clover auth add xai-oauth   (or set XAI_API_KEY)")
            return
        ...
        idx = prompt_choice(
            ...
            choices=[
                ...
                "Skip — configure later via `clover auth add xai-oauth`",
            ],
            default=0,
        )
        if idx == 0:
            if _run_xai_oauth_login_from_setup():
                ...
                _print_warning(
                    ...
                    "Run later: clover auth add xai-oauth"
                )
        elif idx == 1:
            ...
            if api_key:
                ...
                _print_warning(
                    "    No API key provided. Run later: clover auth add xai-oauth"
                )
        else:
...
def valid_post_setup_keys() -> Set[str]:
    ...
    ``post_setup``). This is the allowlist the ``clover tools post-setup``
...
def run_post_setup_command(args) -> int:
    """``clover tools post-setup <key>`` — non-interactive post-setup runner.
    ...
    if not key:
        _print_error("Usage: clover tools post-setup <key>")
        return 2
...
def _exempt_explicit_platform_native(
    ...
    the composite they chose (e.g. ``clover-discord``, which contains those
...
#:
#: Saving ``clover tools`` (or one toggle in the desktop Toolsets UI) replaces
...
#: has touched the picker, while everyone still on ``[clover-cli]`` inherits it
...
def _enable_recently_shipped_toolsets(
    ...
    Either way of saying no outlives this: unchecking in ``clover tools``
    ...
    default_ts = plat_info["default_toolset"] if plat_info else f"clover-{platform}"
    ...
    for ts_key in sorted(_RECENTLY_SHIPPED_TOOLSETS):
        ...
        # narrow composites (clover-acp, clover-webhook) stay narrow.
...
def _get_platform_tools(
    ...
    *,
    ...
    # explicit config like ``[clover-cli, a2a]`` must survive filtering just
    ...
    # "clover-cli" (which include all _CLOVER_CORE_TOOLS) cause disabled
    ...
    if has_explicit_config:
        ...
        # ``[clover-cli, spotify]`` after enabling Spotify via ``clover
        ...
        # (e.g. "clover-cli") to individual tool names and reverse-mapping.
        all_tool_names = set()
        for ts_name in toolset_names:
        ...
        # through ``clover tools`` to flip the toolset on. Only fires when
    ...
    # otherwise saving via `clover tools` (which flips has_explicit_config
    # to True) silently drops them.
    _plat_info = PLATFORMS.get(platform)
    _default_ts = _plat_info["default_toolset"] if _plat_info else f"clover-{platform}"
    ...
    skip |= {k for k in TOOLSETS if k.startswith("clover-")}
    ...
    # shipped as a bundled plugin but user must opt in via `clover tools`
    # so we don't ship 7 Spotify tool schemas to users who don't use it).
    # A plugin toolset is "known" for a platform once `clover tools`
    ...
    if plugin_ts_keys:
        ...
        for pts in plugin_ts_keys:
            if pts in toolset_names:
                ...
                # New plugin not yet seen by clover tools — default enabled
                enabled_toolsets.add(pts)
    ...
    # JSON-array string (e.g. "['memory']") from `clover config set` or a
    ...
    # is invalid (e.g. a migration or hand-edit left `clover` instead of
    # `clover-cli`), resolve_toolset() returns [] for each and the platform ends
    ...
    # at runtime, not only during the next `clover update`/`clover doctor`.
    ...
    if isinstance(_explicit, list) and _explicit:
        ...
        if (
            ...
            logger.warning(
                ...
                "name(s): %s) - tools will be unavailable. Run `clover tools` "
...
def _save_platform_tools(config: dict, platform: str, enabled_toolset_keys: Set[str]):
    ...
    # Also exclude platform default toolsets (clover-cli, clover-telegram, etc.)
    ...
    # Opening `clover tools` is the user's opt-in to reconfigure tools, so treat
...
def _estimate_tool_tokens() -> Dict[str, int]:
    ...
    from clover_constants import clover_home_key

    scope = clover_home_key()
...
# ("Clover Subscription" managed-gateway entry, "Firecrawl Self-Hosted")
...
# ("Clover Subscription", "Local Browser", "Camofox") — see the comment block
...
def _visible_providers(
    ...
    *activates* the gateway once paid access is confirmed.
    ...
    # remaining hardcoded rows ("Clover Subscription", "Firecrawl
...
_POST_SETUP_INSTALLED: dict = {
    ...
    # who toggles the toolset on via `clover tools` gets a silent no-op
...
# Python dependencies installed explicitly through ``clover tools`` are not
...
def active_restorable_python_tool_dependencies() -> list[str]:
    """Return ``clover tools`` Python dependencies present in this runtime."""
    return [
        name
...
# Kept in sync with the dashboard selects (clover_cli/web_server.py
...
def _configure_vision_backend() -> None:
    ...
    provider + model — the same surface as ``clover model`` — or point at a
...
def _configure_vision_provider_model(config: dict, vision_cfg: dict) -> None:
    ...
    substrate — so this picker lists exactly what the ``clover model`` aux-task
    ...
    """
    try:
        from clover_cli.inventory import (
    ...
    if not providers:
        _print_warning(
            ...
            "with `clover model`, then re-run this."
        )
        return
...
def _reconfigure_simple_requirements(ts_key: str):
    ...
    if ts_key == "vision":
        ...
        # `clover model`). Run it directly so reconfigure doesn't fall back to
...
def tools_command(args=None, first_install: bool = False, config: dict = None):
    """Entry point for `clover tools` and `clover setup tools`.
    ...
    if getattr(args, "summary", False):
        ...
        print()
        return
    print(color("☘ Clover Tool Configuration", Colors.CYAN, Colors.BOLD))
    print(color("  Enable or disable tools per platform.", Colors.DIM))
    print(color("  Tools that need API keys will be configured when enabled.", Colors.DIM))
    print(color("  Guide: docs/user-guide/features/tools", Colors.DIM))
    print()
    ...
    print(color(f"  Tool configuration saved to {display_clover_home()}/config.yaml", Colors.DIM))
    print(color("  Changes take effect on next 'clover' or gateway restart.", Colors.DIM))
    print()
