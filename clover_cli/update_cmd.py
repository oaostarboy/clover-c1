...
def _cmd_update_check(branch: str = "main", *, branch_explicit: bool = False):
    ...
    if method == "docker":
        ...
        # same long-form ``docker pull`` guidance ``clover update`` (apply
        # path) uses — telling the user to "reinstall via curl" or that
        # ".git is missing" would point them at the wrong remediation.
        from clover_cli.config import format_docker_update_message
    ...
    from clover_cli.gitlock import clear_stale_git_locks
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    try:
        ...
        # ``clover update`` process is the PRE-pull Python process — its
        # ``sys.modules`` cache holds the OLD ``clover_cli.config`` and
        # ``clover_cli.config_migrations`` from before ``git pull`` updated
        # the source files. A function-level ``from clover_cli.config import
