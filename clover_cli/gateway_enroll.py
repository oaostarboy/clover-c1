"""``clover gateway enroll`` — enroll a self-hosted gateway with a relay connector.
...
``docs/connector-gateway-auth-design.md``:
  ...
  4. Persist ``GATEWAY_RELAY_ID`` / ``GATEWAY_RELAY_SECRET`` /
     ...
     ``~/.clover/.env``. The per-gateway secret authenticates the WS upgrade;
...
def cmd_gateway_enroll(args) -> None:
    ...
    if is_managed():
        print(
            "✗ `clover gateway enroll` is not available in a managed/hosted install.\n"
    ...
    #    save_env_value writes them to ~/.clover/.env (0600 dir) and never logs.
    to_write = {
    ...
    # process is launched under (load_clover_dotenv exports that .env), so
...
def _warn_if_secondary_multiplex_profile() -> bool:
    ...
    in clover_cli/gateway.py. Best-effort: any failure to determine the
    ...
    try:
        ...
        default_root = Path(get_default_clover_root()).resolve()
        home = Path(get_clover_home()).resolve()
        try:
