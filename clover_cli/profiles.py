"""
Profile management for multiple isolated Clover instances.

Each profile is a fully independent CLOVER_HOME directory with its own
config.yaml, .env, memory, sessions, skills, gateway, cron, and logs.
Profiles live under ``~/.clover/profiles/<name>/`` by default.

The "default" profile is ``~/.clover`` itself — backward compatible,
zero migration needed.

Usage::

    clover profile create coder          # fresh profile + bundled skills
    clover profile create coder --clone  # also copy config, .env, SOUL.md, skills
    clover profile create coder --clone-all  # full copy of source profile
    coder chat                           # use via wrapper alias
    clover -p coder chat                 # or via flag
    clover profile use coder             # set as sticky default
    clover profile delete coder          # remove profile + alias + service
"""
...
from clover_cli.archive_safe import (
    archive_root_dirs,
    make_targz,
...
_PROFILE_DIRS = [
    ...
    # See clover_constants.get_subprocess_home().
    "home",
]
...
#   backups             — `clover backup` archives
...
# Marker file written by `clover profile create --no-skills`.  When present in
# a profile's root, callers of seed_profile_skills() (fresh-create, `clover
...
# `clover skills install` or drop SKILL.md files into the profile's skills/.
...
def _clone_all_copytree_ignore(source_dir: Path):
    ...
    Three categories:
      ...
      2. Root-level entries in ``_CLONE_ALL_DEFAULT_EXCLUDE_ROOT`` — known
         Clover infrastructure directories that only the default profile
         (``~/.clover``) ever contains.  Gated on ``source_dir`` actually
    ...
    is_default_source = source_resolved == _get_default_clover_home().resolve()
...
# Directories/files to exclude when exporting the default (~/.clover) profile.
# The default profile contains infrastructure (repo checkout, worktrees, DBs,
# caches, binaries) that named profiles don't have.  We exclude those so the
# export is a portable, reasonable-size archive of actual profile data.
_DEFAULT_EXPORT_EXCLUDE_ROOT = frozenset({
    # Infrastructure
    "clover-c1",         # repo checkout (multi-GB)
    ".worktrees",           # git worktrees
...
# Allow-list for ``export_profile("default")``: when CLOVER_HOME equals the
...
# into the export. The set below identifies the *known Clover profile
# artifacts* at the root of CLOVER_HOME; everything else is excluded.
...
# (#58394). Add new artifacts here when introduced in ``clover_constants``.
...
_RESERVED_NAMES = frozenset({
    "clover", "default", "test", "tmp", "root", "sudo",
})

# Clover subcommands that cannot be used as profile names/aliases
_CLOVER_SUBCOMMANDS = frozenset({
...
def _get_profiles_root() -> Path:
    ...
    Anchored to the clover root, NOT to the current CLOVER_HOME
    (which may itself be a profile).  This ensures ``coder profile list``
    can see all profiles.

    In Docker/custom deployments where CLOVER_HOME points outside
    ``~/.clover``, profiles live under ``CLOVER_HOME/profiles/`` so
    they persist on the mounted volume.
    """
    return _get_default_clover_home() / "profiles"


def _get_default_clover_home() -> Path:
    """Return the default (pre-profile) CLOVER_HOME path.

    In standard deployments this is ``~/.clover``.
    In Docker/custom deployments where CLOVER_HOME is outside ``~/.clover``
    (e.g. ``/opt/data``), returns CLOVER_HOME directly.
    """
...
def _get_active_profile_path() -> Path:
    ...
    return _get_default_clover_home() / "active_profile"
...
def validate_profile_name(name: str) -> None:
    ...
    if name in _RESERVED_NAMES:
        raise ValueError(
            ...
            f"the Clover installation itself or a common system binary.  "
...
def get_profile_dir(name: str) -> Path:
    """Resolve a profile name to its CLOVER_HOME directory."""
    canon = normalize_profile_name(name)
    if canon == "default":
        return _get_default_clover_home()
...
def profile_matches_home(name: str, home: "Path | None" = None) -> bool:
    ...
    ``home`` defaults to the process's current Clover home
    (:func:`clover_constants.get_clover_home`).  Used by single-profile
...
def check_alias_collision(name: str) -> Optional[str]:
    ...
    Checks: alias-name validity, reserved names, clover subcommands, existing
    binaries in PATH.
    """
    canon = normalize_profile_name(name)
    try:
    ...
    if canon in _CLOVER_SUBCOMMANDS:
        return f"'{canon}' conflicts with a clover subcommand"
...
def create_wrapper_script(name: str, target: Optional[str] = None) -> Optional[Path]:
    ...
    if is_windows:
        ...
        try:
            clover_exe = shutil.which("clover") or "clover"
            wrapper_path.write_text(f'#!/bin/sh\nexec {shlex.quote(clover_exe)} -p {profile} "$@"\n', encoding="utf-8")
...
def _migrate_profile_config_if_outdated(profile_dir: Path) -> None:
    ...
    ``_config_version``) or that is simply older than the running Clover. If we
    ...
    try:
        ...
        # not be migrated. The next `clover doctor --fix` can still surface the
...
def find_alias_for_profile(profile_name: str) -> Optional[str]:
    ...
    alias whose body invokes ``clover -p <profile>``. When the alias name equals
    the profile name this is trivial, but a custom alias (``clover profile alias
...
# Real wrappers are a few hundred bytes of shell; the needle (``clover -p X``)
...
def _read_distribution_meta(profile_dir: Path) -> tuple:
    ...
    never break ``clover profile list`` for an unrelated profile.
    """
...
def _seed_model_config(profile_dir: Path) -> None:
    ...
    try:
        ...
        source = get_clover_home() / "config.yaml"
        ...
        # Creation must not fail over this; `clover model` still sets it later.
        pass
...
def _check_gateway_running(profile_dir: Path) -> bool:
    ...
    *this* reader — which is exactly the case for a dashboard process that is
    ...
    agree.  Parameterized by ``profile_dir`` so it never mutates ``CLOVER_HOME``.
    """
    try:
...
# ``config.yaml``. ``config.yaml`` is the user-facing Clover config
...
def list_profiles() -> List[ProfileInfo]:
    ...
    # Default profile
    default_home = _get_default_clover_home()
...
def profiles_to_serve(
    ...
    """Return the ``(profile_name, clover_home)`` pairs a gateway should serve.
    ...
    - ``multiplex=True``: returns the default profile plus every valid named
      profile under ``profiles/``, each paired with its own CLOVER_HOME. When
    ...
    The returned ``clover_home`` is the path to pass to
    ``set_clover_home_override`` when scoping a turn to that profile.
    """
    active = get_active_profile_name() or "default"
    if not multiplex:
        return [(active, get_profile_dir(active))]

    serve: List[Tuple[str, Path]] = [("default", _get_default_clover_home())]
...
def create_profile(
    ...
    no_skills:
        ...
        a marker file so ``clover update`` skips re-seeding this profile's
    ...
    if canon == "default":
        raise ValueError(
            "Cannot create a profile named 'default' — it is the built-in profile (~/.clover)."
        )
    ...
    if clone_all and source_dir:
        # Full copy of source profile (exclude sibling ~/.clover/profiles/)
        shutil.copytree(
            source_dir,
            profile_dir,
            symlinks=True,
    ...
    # Keys pages, `clover -p <name> auth add`) had no file until first
    ...
    if not env_path.exists():
        try:
            env_path.write_text(
                "# Per-profile secrets for this Clover profile.\n"
    ...
    if not soul_path.exists():
        try:
            from clover_cli.default_soul import DEFAULT_SOUL_MD
            soul_path.write_text(DEFAULT_SOUL_MD, encoding="utf-8")
        except Exception:
            pass  # best-effort — don't fail profile creation over this

    # Write the opt-out marker so seed_profile_skills() and `clover update`'s
    ...
    if no_skills:
        try:
            (profile_dir / NO_BUNDLED_SKILLS_MARKER).write_text(
                ...
                "(`clover profile create --no-skills`).\n"
    ...
    # Cloned configs can be older than the running Clover (or predate schema
    ...
    if description and description.strip():
        try:
            ...
            pass  # non-fatal — user can describe later with `clover profile describe`

    # Phase 4: when running inside a container under s6, register the
    # new profile's gateway as a runtime s6 service so
    # `clover -p <profile> gateway start` can supervise it via
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    ...
    Uses subprocess because sync_skills() caches CLOVER_HOME at module level.
    Returns the sync result dict, or None on failure.

    Profiles that opted out of bundled skills (via ``clover profile create
    ...
    only the essential skills (e.g. ``clover-c1``), reporting
...
def backfill_profile_envs(quiet: bool = False) -> List[str]:
    ...
    default_env = _get_default_clover_home() / ".env"
...
def _profile_bound_backend_pids(canon: str, profile_dir: Path) -> list[int]:
    """PIDs of running Clover *backends* bound to this profile.
    ...
    names it, so find it by inspection: a Clover backend subcommand
    (``serve``/``dashboard``/``gateway``) that is bound to *this* profile either
    by a ``--profile <canon>`` / ``-p <canon>`` selector or by a ``CLOVER_HOME``
    ...
    # Never terminate ourselves or a parent (e.g. `clover -p <canon> profile
    ...
    clover_markers = ("clover_cli.main", "clover-gateway", "tui_gateway")
    ...
    # something (e.g. Electron's `findOnPath('clover')` resolution) spawns the
    ...
    # interpreter, not "clover", so the checks below would otherwise miss it.
    ...
    # interpreter.
    _CLOVER_CONSOLE_SCRIPT_NAMES = frozenset({"clover", "clover-c1", "clover-acp"})
    pids: list[int] = []
    ...
    for proc in psutil.process_iter(["pid", "name", "username", "cmdline"]):
        try:
            ...
            # Must be a Clover process: either an entrypoint marker in argv, a
            # resolved executable named `clover`, or a python interpreter
            # directly exec'ing a `clover`-named console-script shim (argv[0]
            # is the interpreter, argv[1] is the shim's path).
            joined = " ".join(argv)
            exe_name = os.path.basename(argv[0]).lower()
            is_clover = (
                any(marker in joined for marker in clover_markers)
                or exe_name == "clover"
                or exe_name.startswith("clover")
            )
            if not is_clover and len(argv) >= 2 and _python_interpreter_re.match(exe_name):
                # Match against the actual known console-script entry points
                # (pyproject.toml [project.scripts]: clover, clover-c1,
                # clover-acp) rather than a bare `startswith("clover")` --
                ...
                # "clover-notes.py" or "clover-unrelated-tool" as the shim,
                ...
                is_clover = script_stem in _CLOVER_CONSOLE_SCRIPT_NAMES
            if not is_clover:
                continue
            ...
            # ...or by CLOVER_HOME env pointing at this profile dir.
            if not bound:
                try:
                    env_home = (proc.environ() or {}).get("CLOVER_HOME", "")
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    if canon == "default":
        raise ValueError(
            "Cannot delete the default profile (~/.clover).\n"
            "To remove everything, use: clover uninstall"
        )
    ...
    # the rmtree below fail with ENOTEMPTY and — before the ensure_clover_home
...
def _maybe_register_gateway_service(profile_name: str) -> None:
    ...
    ``CLOVER_HOME`` and binds the port resolved by ``gateway/config.py``
    ...
    try:
        ...
        from clover_cli.service_manager import get_service_manager
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    # Temporarily set CLOVER_HOME so _profile_suffix resolves correctly
    old_home = os.environ.get("CLOVER_HOME")
    try:
        os.environ["CLOVER_HOME"] = str(profile_dir)
        from clover_cli.gateway import get_service_name, get_launchd_plist_path
        ...
        elif "CLOVER_HOME" in os.environ:
            del os.environ["CLOVER_HOME"]
...
def set_active_profile(name: str) -> None:
    ...
    Writes to ``~/.clover/active_profile``. Use ``"default"`` to clear.
    """
    canon = normalize_profile_name(name)
    validate_profile_name(canon)
    if canon != "default" and not profile_exists(canon):
        raise FileNotFoundError(
            ...
            f"Create it with: clover profile create {canon}"
        )
...
def get_active_profile_name() -> str:
    """Infer the current profile name from CLOVER_HOME.

    Returns ``"default"`` if CLOVER_HOME is not set or points to ``~/.clover``.
    Returns the profile name if CLOVER_HOME points into ``~/.clover/profiles/<name>``.
    Returns ``"custom"`` if CLOVER_HOME is set to an unrecognized path.
    """
    from clover_constants import get_clover_home
    clover_home = get_clover_home()
    resolved = clover_home.resolve()

    default_resolved = _get_default_clover_home().resolve()
...
def _default_export_ignore(root_dir: Path):
    ...
    * **Root-level allow-list** — only entries whose name appears in
      ...
      CLOVER_HOME equals the cwd) is excluded. Blacklisting was tried
      first and proved unable to anticipate every non-Clover file the
      user may have lying alongside CLOVER_HOME (#58394).
...
def _scrub_export_secrets(staged: Path) -> None:
    ...
    ``clover sessions export --redact``. Runs on the *staged copy only* so the
    live profile is never rewritten. ``force=True`` ignores
    ``security.redact_secrets`` / ``CLOVER_REDACT_SECRETS`` — share archives
...
def export_profile(name: str, output_path: str, extra_files: Optional[Dict[str, str]] = None) -> Path:
    ...
    if canon == "default":
        # The default profile IS ~/.clover itself — its parent is ~/ and its
        # directory name is ".clover", not "default".  We stage a clean copy
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    if not inferred_name:
        raise ValueError(
            ...
            "Specify it explicitly: clover profile import <archive> --name <name>"
        )
    ...
    # dir.  Importing as "default" would target ~/.clover itself — disallow
    # that and guide the user toward a named profile.
    canon = normalize_profile_name(inferred_name)
    validate_profile_name(canon)
    if canon == "default":
        raise ValueError(
            "Cannot import as 'default' — that is the built-in root profile (~/.clover). "
            "Specify a different name: clover profile import <archive> --name <name>"
        )
    ...
    with tempfile.TemporaryDirectory(prefix="clover_profile_import_") as tmpdir:
...
def _migrate_honcho_profile_host(old_name: str, new_name: str, new_dir: Path) -> None:
    ...
    old_host = f"clover_{old_name}"
    legacy_old_host = f"clover.{old_name}"
    new_host = f"clover_{new_name}"

    candidates = [
        new_dir / "honcho.json",
        _get_default_clover_home() / "honcho.json",
    ...
    for path in candidates:
        ...
        if isinstance(block, dict) and "aiPeer" not in block:
            if source_host.startswith("clover_"):
...
def resolve_profile_env(profile_name: str) -> str:
    """Resolve a profile name to a CLOVER_HOME path string.

    Called early in the CLI entry point, before any clover modules
    are imported, to set the CLOVER_HOME environment variable.

    When CLOVER_HOME is already set, the configured spelling IS the
