...
Used by ``clover kanban specify [task_id | --all]``. Takes a task that
...
* This module intentionally mirrors ``clover_cli/goals.py`` — same aux
...
CLOVER_KANBAN_SPECIFY_MAX_TOKENS = max(
    1500,
    env_int("CLOVER_KANBAN_SPECIFY_MAX_TOKENS", 6000),
)

logger = logging.getLogger(__name__)


_SYSTEM_PROMPT = """You are the Kanban triage specifier for the Clover Cognition board.
...
def _profile_author() -> str:
    """Mirror of ``clover_cli.kanban._profile_author``. Kept local to
    avoid a circular import when kanban.py imports this module."""
    return (
        os.environ.get("CLOVER_PROFILE")
    ...
    *,
    ...
    try:
        ...
        resp = call_llm(
            ...
            temperature=0.3,
            max_tokens=CLOVER_KANBAN_SPECIFY_MAX_TOKENS,
