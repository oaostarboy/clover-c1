"""Per-terminal session breadcrumbs for ``clover -c`` / ``--continue``.

Each CLI session writes a tiny breadcrumb file
``$CLOVER_HOME/terminal-sessions/<terminal-id>`` containing
``{"session_id": ..., "cwd": ..., "ts": ...}``.  A bare ``clover -c`` then
...
def _breadcrumbs_dir() -> Path:
    ...
    return get_clover_home() / "terminal-sessions"
