"""Normalized Clover Portal account entitlement helpers."""
...
_account_info_cache: tuple[str, float, "CloverPortalAccountInfo"] | None = None
_ACCOUNT_INFO_CACHE_LOCK = threading.Lock()


@dataclass(frozen=True)
class CloverPortalSubscriptionInfo:
...
def format_nous_portal_entitlement_message(
    ...
    *,
    ...
    """Return user-facing guidance for a missing Clover tool-gateway entitlement.
    ...
    if account_info is not None:
        if coverage_category is not None:
            ...
            if account_info.tool_gateway_entitled:
                ...
                return (
                    f"{capability} isn't included with your current Clover Portal "
    ...
    if account_info is None:
        return (
            f"Clover could not verify your Clover Portal entitlement, so {capability} "
            f"is unavailable. Run `clover model` to refresh your login, or check "
    ...
    if not account_info.logged_in:
        if account_info.inference_credential_present:
            return (
                f"Clover inference credentials are configured, but Clover cannot verify "
                f"your Clover Portal paid access for {capability}. Log in with "
                f"`clover model` to enable Portal-managed features. Billing and "
                f"credits are managed at {billing_url}."
            )
        return (
            f"Log in to Clover Portal to use {capability}: run `clover model`. "
            f"Billing and credits are managed at {billing_url}."
        )

    if account_info.paid_service_access is None:
        detail = (
            f"Clover could not verify your Clover Portal paid access, so {capability} "
            f"is unavailable."
        )
        if account_info.error:
            detail += f" Account lookup failed: {account_info.error}."
        if include_refresh_hint:
            detail += " Run `clover model` to refresh your session."
    ...
    if reason == "account_missing":
        return (
            f"Clover could not find a Clover Portal account or organisation for this "
            f"login, so {capability} is unavailable. Run `clover model` to "
            f"authenticate again; if the problem persists, contact Clover support."
        )
    ...
    if reason == "no_usable_credits" or account_info.paid_service_access is False:
        ...
        if include_refresh_hint and not account_info.fresh:
            message += " If you recently bought credits, run `clover model` to refresh Clover."
        return message

    return (
        f"Your Clover Portal account does not currently have paid service access, "
        f"so {capability} is unavailable. Add credits or update billing at {billing_url}."
    )


def _no_paid_access_message(
    account_info: CloverPortalAccountInfo,
    capability: str,
    billing_url: str,
) -> str:
    access = account_info.paid_service_access_info
    ...
    if access and access.member_spend_cap_exceeded:
        ...
        return (
            f"Your Clover Portal access is paused because you've exceeded the"
            f" per-member spend cap set by your organisation.{cap_detail}"
            f"{credit_detail} Ask your organisation admin to raise the"
            f" member spend cap at {billing_url}, then run `clover model`"
            f" to refresh."
        )
    ...
    if has_active_subscription and active_subscription_is_paid:
        credit_detail = _credit_detail(total_usable, subscription_credits, purchased_credits)
        return (
            f"Your Clover Portal credits are exhausted{credit_detail}, so {capability} "
    ...
    if has_active_subscription and active_subscription_is_paid is False:
        return (
            f"Your current Clover Portal plan does not include paid service access, "
    ...
    if has_active_subscription is False:
        credit_detail = _credit_detail(total_usable, subscription_credits, purchased_credits)
        return (
            f"Your Clover Portal account has no active subscription or usable credits"
    ...
    credit_detail = _credit_detail(total_usable, subscription_credits, purchased_credits)
    return (
        f"Your Clover Portal account has no usable paid credits{credit_detail}, so "
...
def get_nous_portal_account_info(
    *,
    ...
    """Return normalized Clover Portal account entitlement information.
...
def _info_from_valid_jwt(
    ...
    *,
    ...
    try:
        from clover_cli.auth import _decode_jwt_claims
    except Exception:
        return None
...
def _subscription_from_payload(value: Any) -> Optional[CloverPortalSubscriptionInfo]:
    if not isinstance(value, dict):
        return None
    return CloverPortalSubscriptionInfo(
