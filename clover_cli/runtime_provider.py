    ...
    if provider == "clover":
        ...
        api_key = explicit_api_key or (
            ...
            if _agent_key_is_usable(
                state,
                max(60, env_int("CLOVER_NOUS_MIN_KEY_TTL_SECONDS", 1800)),
            )
            else ""
        )
        ...
        if not api_key:
            creds = resolve_nous_runtime_credentials(
                timeout_seconds=float(_getenv("CLOVER_NOUS_TIMEOUT_SECONDS", "15")),
            )
            api_key = creds.get("api_key", "")
    ...
    *,
    ...
    if pool and pool.has_credentials():
        ...
        if provider == "clover" and entry is not None:
            min_ttl = max(60, env_int("CLOVER_NOUS_MIN_KEY_TTL_SECONDS", 1800))
            nous_state = {
