...
from clover_cli.auth import (
    ...
    _clover_inference_env_override,
    format_auth_error,
    resolve_provider,
...
from clover_cli import config as _config_mod
from clover_cli.providers import custom_provider_aliases, custom_provider_slug
from clover_constants import OPENROUTER_BASE_URL
from clover_cli.providers import is_official_openai_host


def load_config():
    """Late-bound delegate to :func:`clover_cli.config.load_config`.

    Deliberately NOT a module-level ``from clover_cli.config import
    load_config``: this module is often imported lazily (inside functions),
    so its first import can happen while a test has
    ``clover_cli.config.load_config`` patched — a from-import would then
    ...
    patching ``clover_cli.config.load_config`` OR
    ``clover_cli.runtime_provider.load_config`` behaves as expected.
    """
...
def _config_base_url_trustworthy_for_bare_custom(cfg_base_url: str, cfg_provider: str) -> bool:
    ...
    try:
        from clover_cli.auth import resolve_provider as _resolve_provider
...
def _detect_api_mode_for_url(base_url: str) -> Optional[str]:
    ...
    # host_mandated_api_mode clause in clover_cli/providers.py so the
...
def _fallback_api_mode(provider: str, base_url: str, model: str = "") -> str:
    ...
    if detected:
        return detected
    from clover_cli.providers import determine_api_mode
...
def _copilot_runtime_api_mode(
    ...
    *,
    ...
    try:
        from clover_cli.models import copilot_model_api_mode
...
_VALID_API_MODES = {
    ...
    # instead of Clover' tool dispatch. Gated behind config key
...
def _parse_api_mode(raw: Any) -> Optional[str]:
    ...
    if isinstance(raw, str):
        from clover_cli.config import _canonical_api_mode
...
def _clover_inference_base_url_override() -> str:
    """Return the trusted Clover runtime base URL override, if configured.

    Delegates to ``auth._clover_inference_env_override`` so every
    ``CLOVER_INFERENCE_BASE_URL`` read shares one normalization path
    ...
    """
    return _clover_inference_env_override() or ""
...
def _resolve_runtime_from_pool_entry(
    *,
    ...
    if provider == "openai-codex":
        ...
        api_mode = clover_api_mode(effective_model)
        base_url = _clover_inference_base_url_override() or base_url
        ...
        if effective_model and api_mode != "anthropic_messages":
            try:
                from clover_cli.models import azure_foundry_model_api_mode
        ...
        if opencode_provider_family(provider) is not None:
            ...
            # Refs #16878.
            from clover_cli.models import opencode_model_api_mode
...
def _get_named_custom_provider(requested_provider: str) -> Optional[Dict[str, Any]]:
    ...
    if requested_norm != "custom" and not requested_norm.startswith("custom:"):
        try:
            ...
            # the canonical provider itself (``clover``, ``openrouter``, …) so
            # accidentally shadowing a canonical provider still resolves to
            # the built-in. See tests/clover_cli/test_runtime_provider_resolution.py
...
def canonical_custom_identity(
    *,
    ...
    3. ``config_provider`` — the active ``config.model.provider`` (or its
       ``provider``/``CLOVER_INFERENCE_PROVIDER`` equivalent). When neither
    ...
    if not candidate:
        try:
            ...
            candidate = ""
    if not candidate:
        candidate = os.environ.get("CLOVER_INFERENCE_PROVIDER", "").strip()
...
def is_routable_provider(provider: Optional[str]) -> bool:
    ...
    if name.lower() == "custom":
        ...
        return False
    try:
        from clover_cli.providers import resolve_provider_full
...
def _resolve_azure_foundry_runtime(
    *,
    ...
    if not base_url:
        raise AuthError(
            "Azure Foundry requires a base URL. Set it via 'clover model' or "
...
def _resolve_explicit_runtime(
    *,
    ...
    if provider == "clover":
        ...
        state = auth_mod.get_provider_auth_state("clover") or {}
        base_url = (
            explicit_base_url
            or _clover_inference_base_url_override()
            or str(state.get("inference_base_url") or auth_mod.DEFAULT_CLOVER_INFERENCE_URL).strip().rstrip("/")
        )
        ...
        # by resolve_clover_runtime_credentials().
        api_key = explicit_api_key or (
            str(state.get("agent_key") or "").strip()
            if _agent_key_is_usable(
                state,
                max(60, env_int("CLOVER_CLOVER_MIN_KEY_TTL_SECONDS", 1800)),
            )
            else ""
        )
        ...
        if not api_key:
            creds = resolve_clover_runtime_credentials(
                timeout_seconds=float(_getenv("CLOVER_CLOVER_TIMEOUT_SECONDS", "15")),
            )
            api_key = creds.get("api_key", "")
        ...
        return {
            "provider": "clover",
            "api_mode": clover_api_mode(target_model or model_cfg.get("default") or ""),
            "base_url": base_url,
            "api_key": api_key,
            "source": "explicit",
...
def resolve_runtime_provider(
    *,
    ...
    from clover_cli.config import is_provider_enabled, load_config
    ...
    if pool and pool.has_credentials():
        ...
        # For Clover, the pool entry's runtime_api_key is the agent_key
        # compatibility field. It must be an invoke JWT. The pool doesn't
        # refresh it during selection (that would trigger network calls in
        # non-runtime contexts like `clover auth list`). If the key is
        # expired/missing, refresh the selected pool entry before falling back
        # to singleton auth resolution.
        if provider == "clover" and entry is not None:
            min_ttl = max(60, env_int("CLOVER_CLOVER_MIN_KEY_TTL_SECONDS", 1800))
            clover_state = {
                "agent_key": getattr(entry, "agent_key", None),
                "agent_key_expires_at": getattr(entry, "agent_key_expires_at", None),
                "scope": getattr(entry, "scope", None),
            }
            if not _agent_key_is_usable(clover_state, min_ttl):
                logger.debug("Clover pool entry agent_key expired/missing, refreshing selected pool entry")
                try:
                    refreshed = pool.try_refresh_current()
                except Exception as exc:
                    logger.debug("Clover pool entry refresh failed: %s", exc)
                    refreshed = None
                ...
                if not pool_api_key or not _agent_key_is_usable(clover_state, min_ttl):
                    logger.debug("Clover pool entry agent_key still unavailable, falling through to runtime resolution")
                    pool_api_key = ""
        if (
            entry is not None
            and pool_api_key
    ...
    if provider == "clover":
        try:
            ...
            if requested_provider != "auto":
                raise
            # Auto-detected Clover but credentials are stale/revoked —
            # fall through to env-var providers (e.g. OpenRouter).
            logger.info("Auto-detected Clover provider but credentials failed; "
    ...
    if provider == "openai-codex":
        try:
            ...
            return {
                ...
                "base_url": creds.get("base_url", "").rstrip("/"),
                "api_key": creds.get("api_key", ""),
                "source": creds.get("source", "clover-auth-store"),
                "last_refresh": creds.get("last_refresh"),
                "requested_provider": requested_provider,
            }
        except AuthError:
    ...
    if provider == "minimax-oauth":
        ...
        if pconfig and pconfig.auth_type == "oauth_minimax":
            from clover_cli.auth import resolve_minimax_oauth_runtime_credentials
    ...
    if provider == "anthropic":
        ...
        if _is_azure_endpoint:
            ...
            # chain.  Accept both `key_env` (Clover canonical — matches the
            # custom_providers field name) and `api_key_env` (documented in the
            # Azure Foundry guide and read by most Clover-compatible importers).
