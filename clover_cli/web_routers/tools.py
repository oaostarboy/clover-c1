...
late-binding seam in :mod:`clover_cli.web_deps` (``late`` for callables,
...
async def toggle_toolset(name: str, body: ToolsetToggle, profile: Optional[str] = None):
    ...
    # same background install `clover tools` runs interactively. Without this,
    ...
    # dead-end that forced users to discover `clover computer-use install`
...
async def get_toolset_config(name: str, profile: Optional[str] = None):
    ...
    Surfaces the same provider rows the CLI ``clover tools`` picker shows
...
async def get_toolset_models(
    ...
    The GUI counterpart of the model picker `clover tools` runs after a
...
async def select_toolset_provider(
    ...
    core extracted from the CLI configurator — so the GUI and ``clover tools``
...
async def save_toolset_env(name: str, body: ToolsetEnvUpdate, profile: Optional[str] = None):
    ...
    Writes each ``key: value`` to ``~/.clover/.env`` via ``save_env_value`` —
    the same store ``clover tools`` writes when it prompts for keys. Keys are
...
async def run_toolset_post_setup(
    ...
    ``clover tools post-setup <key>`` and the frontend tails the log via
    ...
    ``profile`` spawns the hook as ``clover -p <profile> tools post-setup``.
    ...
    write per-profile state must see the same CLOVER_HOME the rest of the
...
async def grant_computer_use_permissions(profile: Optional[str] = None):
    """Spawn ``clover computer-use permissions grant`` as a background action.
