...
TIPS = [
    ...
    "/title \"my project\" names your session — resume it later with /resume or clover -c.",
    ...
    "Ctrl+Z suspends Clover to the background — run fg in your shell to resume.",
    ...
    # --- CLI Flags ---
    "clover -c resumes your most recent CLI session. clover -c \"project name\" resumes by title.",
    "clover -w creates an isolated git worktree — perfect for parallel agent workflows.",
    "clover -w -q \"Fix issue #42\" combines worktree isolation with a one-shot query.",
    "clover chat -t web,terminal enables only specific toolsets for a focused session.",
    "clover chat -s github-pr-workflow preloads a skill at launch.",
    "clover chat -q \"query\" runs a single non-interactive query and exits.",
    "clover chat --max-turns 1000 overrides the default 500-iteration limit per turn.",
    "clover chat --checkpoints enables filesystem snapshots before every destructive file change.",
    "clover --yolo bypasses all dangerous command approval prompts for the entire session.",
    "clover chat --source telegram tags the session for filtering in clover sessions list.",
    "clover -p work chat runs under a specific profile without changing your default.",

    # --- CLI Subcommands ---
    "clover doctor --fix diagnoses and auto-repairs config and dependency issues.",
    "clover dump outputs a compact setup summary — great for bug reports.",
    "clover config set KEY VALUE auto-routes secrets to .env and everything else to config.yaml.",
    "clover config edit opens config.yaml in your default editor.",
    "clover config check scans for missing or stale configuration options.",
    "clover sessions browse opens an interactive session picker with search.",
    "clover sessions stats shows session counts by platform and database size.",
    "clover sessions prune --older-than 30 cleans up old sessions.",
    "clover skills search react --source skills-sh searches the skills.sh public directory.",
    "clover skills check scans installed hub skills for upstream updates.",
    "clover skills tap add myorg/skills-repo adds a custom GitHub skill source.",
    "clover skills snapshot export setup.json exports your skill configuration for backup or sharing.",
    "clover mcp add github --command npx adds MCP servers from the command line.",
    "clover mcp serve runs Clover itself as an MCP server for other agents.",
    "clover auth add lets you add multiple API keys for credential pool rotation.",
    "clover completion bash >> ~/.bashrc enables tab completion for all commands and profiles.",
    "clover logs -f follows agent.log in real time. --level WARNING --since 1h filters output.",
    "clover backup creates a zip backup of your entire Clover home directory.",
    "clover profile create coder creates an isolated profile that becomes its own command.",
    "clover profile create work --clone copies your current config and keys to a new profile.",
    "clover update syncs new bundled skills to ALL profiles automatically.",
    "clover gateway install sets up Clover as a system service (systemd/launchd).",
    "clover memory setup lets you configure an external memory provider (Honcho, Mem0, etc.).",
    "clover webhook subscribe creates event-driven webhook routes with HMAC validation.",
    "Save money: clover tools disables unused tools, clover skills config trims skills down.",
    "/reasoning low or /reasoning minimal cuts thinking depth below the default (medium) — faster, cheaper responses.",
    "clover models routes vision, compression, and aux tasks to cheaper models — cuts background token cost 85%+ without downgrading your main chat model.",
    ...
    "Set worktree: true in config.yaml to always create a git worktree (same as clover -w).",
    "Set security.website_blocklist.enabled: true to block specific domains from web tools.",
    "Set cron.wrap_response: true to add provenance metadata to scheduled-job deliveries.",
    "CLOVER_TIMEZONE overrides the server timezone with any IANA timezone string.",
    ...
    # --- Tools & Capabilities ---
    "execute_code runs Python scripts that call Clover tools programmatically — results stay out of context.",
    ...
    "Profile names become shell commands — 'clover profile create coder' creates the 'coder' command.",
    "clover profile export coder -o backup.tar.gz creates a portable profile archive.",
    ...
    "When exiting, Clover prints a resume command with session ID and stats.",
    "clover sessions export backup.jsonl exports all sessions for backup or analysis.",
    "clover -r SESSION_ID resumes any specific past session by its ID.",
    ...
    "clover skills install official/security/1password installs optional skills from the repo.",
    ...
    # --- Gateway & Messaging ---
    "Clover runs on 21 messaging platforms: Telegram, Discord, Slack, WhatsApp, Signal, Matrix, IRC, Microsoft Teams, email, and more.",
    "clover gateway install sets it up as a system service that starts on boot.",
    "DingTalk uses Stream Mode — no webhooks or public URL needed.",
    "BlueBubbles brings iMessage to Clover via a local macOS server.",
    ...
    "Context files (.clover.md, AGENTS.md) are security-scanned for prompt injection before loading.",
    ...
    "Clover loads project context from .clover.md, AGENTS.md, CLAUDE.md, or .cursorrules (first match).",
    ...
    # --- MCP ---
    "clover mcp opens an interactive picker of Clover-approved MCPs you can install in one keystroke.",
    "clover mcp catalog lists Clover-approved MCP servers shipped with the repo.",
    "clover mcp install <name> installs a catalog entry, prompts for credentials, and lets you pick which of its tools to enable.",
    ...
    "MCP servers auto-generate toolsets at runtime — clover tools can toggle them per platform.",
    ...
    "Checkpoints use shadow repos in ~/.clover/checkpoints/ — your project's .git is never touched.",

    # --- Batch & Data ---
    "batch_runner.py processes hundreds of prompts in parallel for training data generation.",
    "clover chat -Q enables quiet mode for programmatic use — suppresses banner and spinner.",
    ...
    "clover plugins install owner/repo installs plugins directly from GitHub.",
    ...
    "clover acp runs Clover as an ACP server for VS Code, Zed, and JetBrains integration.",
    "Custom providers: save named endpoints in config.yaml under custom_providers.",
    "CLOVER_EPHEMERAL_SYSTEM_PROMPT injects a system prompt that's never persisted to history.",
    "credential_pool_strategies supports fill_first, round_robin, least_used, and random rotation.",
    "clover auth add clover or clover auth add openai-codex sets up OAuth-based providers.",
    ...
    "clover status --deep runs deeper diagnostic checks across all components.",
    ...
    "Cron scripts live in ~/.clover/scripts/ and run before the agent — perfect for data collection pipelines.",
    "prefill_messages_file in config.yaml injects few-shot examples into every API call, never saved to history.",
    "SOUL.md completely replaces the agent's default identity — rewrite it to make Clover your own.",
    ...
    "Before a gateway session reset, Clover auto-flushes important facts to memory in the background.",
    ...
    "The skills audit log at ~/.clover/skills/.hub/audit.log tracks every install and removal operation.",
    ...
    "Profiles scope Clover state via CLOVER_HOME; host tool subprocesses keep your real HOME unless terminal.home_mode is profile.",
    "CLOVER_HOME_MODE env var (octal, e.g. 0701) sets custom directory permissions for web server traversal.",
    "Container mode: place .container-mode in CLOVER_HOME and the host CLI auto-execs into the container.",
    "Ctrl+C has 5 priority tiers: cancel recording → cancel prompts → cancel picker → interrupt agent → exit.",
    "Every interrupt during an agent run is logged to ~/.clover/interrupt_debug.log with timestamps.",
    ...
    "Profile names cannot collide with existing PATH binaries — 'clover profile create ls' would be rejected.",
    "clover profile create backup --clone-all copies everything (config, keys, SOUL.md, memories, skills, sessions).",
    ...
    "clover claw migrate --dry-run previews OpenClaw migration without writing anything.",
    ...
    '/goal <text> sets a standing Ralph-loop objective — Clover auto-continues turn after turn until a judge says done.',
    '/snapshot create [label] saves a full state snapshot of Clover config; /snapshot restore <id> reverts later.',
    ...
    '/busy queue|steer|interrupt controls what pressing Enter does while Clover is working.',
    ...
    '/reload reloads ~/.clover/.env into the running session — pick up new API keys without restarting.',
    ...
    "CLOVER_CRON_MAX_PARALLEL (default 4) caps how many cron jobs run per tick so bursts don't saturate your keys.",

    # --- Gateway Hooks ---
    'Gateway hooks live under ~/.clover/hooks/<name>/ with HOOK.yaml + handler.py — handler must be named `handle`.',
    'Hook events include gateway:startup, session:start, agent:step, and command:* wildcard subscriptions.',
    'Drop a ~/.clover/BOOT.md checklist and a gateway:startup hook runs it as a one-shot agent every boot.',

    # --- Curator ---
    'clover curator run --dry-run previews what the curator would archive or consolidate without mutating anything.',
    "clover curator pin <skill> hard-fences a skill against both auto-archival and the agent's skill_manage tool.",
    'clover curator rollback restores skills from a pre-run snapshot — backups live under skills/.curator_backups/.',

    # --- Credential Pools & Routing ---
    'clover auth reset <provider> clears all cooldowns and exhaustion flags on a credential pool.',
    'credential_pool_strategies.<provider>: round_robin cycles keys evenly instead of the fill_first default.',
    'use_gateway: true per-tool routes web, image, tts, or browser through your Clover subscription — no extra keys.',
    ...
    # --- TUI & Dashboard ---
    'CLOVER_TUI_RESUME=1 auto-re-attaches to the most recent TUI session on launch — handy after SSH drops.',
    "CLOVER_TUI_THEME=light|dark|<hex> forces the TUI theme on terminals that don't set COLORFGBG.",
    ...
    'clover dashboard launches a local web UI at 127.0.0.1:9119 — zero data leaves localhost.',
    'clover dashboard embeds the full Clover TUI in your browser via xterm.js and a WebSocket PTY.',
    'Drop a YAML in ~/.clover/dashboard-themes/ with two palette colors to reskin the entire dashboard.',
    'Dashboard plugins are drop-in: manifest.json + JS bundle in ~/.clover/dashboard-plugins/ — no npm build required.',
    ...
    'CLOVER_BACKGROUND_NOTIFICATIONS=result only pings when background tasks finish (vs concise/all/error/off).',
    'CLOVER_WRITE_SAFE_ROOT restricts write_file/patch to directory prefixes; multiple paths via os.pathsep (: or ;).',
    'CLOVER_IGNORE_RULES skips auto-injection of AGENTS.md, SOUL.md, .cursorrules, memory, and preloaded skills.',
    'CLOVER_ACCEPT_HOOKS auto-approves unseen shell hooks declared in config.yaml without a TTY prompt.',
    ...
    # --- TTS ---
    'tts.provider: piper runs 44-language local TTS on CPU — voices auto-download to ~/.clover/cache/piper-voices/.',
    ...
    '/reload-skills re-scans ~/.clover/skills/ so drop-in skills appear without restarting the session.',
    ...
    'clover -z "<prompt>" is the purest one-shot: final answer on stdout, nothing else — ideal for piping in scripts.',
    'clover chat --pass-session-id injects the session ID into the system prompt so the agent can self-reference it.',
    'clover chat --image path/to/pic.png attaches a local image to a single -q query without a separate upload step.',
    'clover chat --ignore-user-config skips the active user config — reproducible bug reports and CI runs.',
    "clover chat --source tool tags programmatic chats so they don't clutter clover sessions list.",
    'clover dump --show-keys includes redacted API key fingerprints for deeper support debugging.',
    'clover sessions rename <ID> "new title" renames any past session; clover sessions delete <ID> removes one.',
    'clover import restores a full Clover backup zip; session JSON/JSONL exports import from the dashboard Sessions page.',
    'clover fallback manages the fallback_model chain interactively — no hand-editing config.yaml.',
    'clover pairing rotates the DM pairing token — the first messager after rotation claims access to the bot.',
    'clover setup walks first-time users through provider, keys, and platform wiring in one interactive flow.',
    'clover status --deep runs the full health sweep across every component; plain clover status is the quick view.',

    # --- Agent Behavior Env Vars ---
    'CLOVER_AGENT_TIMEOUT=0 disables the gateway inactivity kill for a running agent — use for long research runs.',
    'CLOVER_ENABLE_PROJECT_PLUGINS=1 auto-loads repo-local plugins from ./.clover/plugins/ — trust-gated by design.',
    "CLOVER_DISABLE_FILE_STATE_GUARD=1 turns off the 'file changed since you read it' guard on patch and write_file.",
    'CLOVER_ALLOW_PRIVATE_URLS=true lets web tools hit localhost and private networks — off by default in gateway mode.',
    'CLOVER_OPTIONAL_SKILLS=name1,name2 auto-installs extra optional-catalog skills on first run per profile.',
    'CLOVER_BUNDLED_SKILLS points at a custom bundled-skill tree — used by Homebrew and Nix packaging.',
    'CLOVER_DUMP_REQUEST_STDOUT=1 dumps every API request payload to stdout instead of log files.',
    'CLOVER_OAUTH_TRACE=1 logs redacted OAuth token exchange and refresh attempts for debugging provider auth.',
    'CLOVER_STREAM_RETRIES (default 3) controls mid-stream reconnect attempts on transient network errors.',

    # --- Gateway Behavior Env Vars ---
    'CLOVER_GATEWAY_BUSY_ACK_ENABLED=false silences the ⚡/⏳/⏩ ack messages when a user messages a busy agent.',
    'CLOVER_AGENT_NOTIFY_INTERVAL (default 180s) sets how often the gateway pings with progress on long turns.',
    'CLOVER_RESTART_DRAIN_TIMEOUT (default 900s) caps how long /restart waits for in-flight runs before forcing.',
    'CLOVER_CHECKPOINT_TIMEOUT (default 30s) caps filesystem checkpoint creation — raise it on huge monorepos.',
    ...
    # --- Security ---
    'security.tirith_fail_open: false makes Clover block commands when the tirith scanner itself errors out.',
    ...
    '--source tool chats are excluded from clover sessions list by default — set --source explicitly to see them.',
    ...
    'Dashboard plugins are served from /dashboard-plugins/<name>/ — drop files into ~/.clover/dashboard-plugins/.',
]
...
# any project or none — Clover is not a coding-only agent.
# ---------------------------------------------------------------------------
