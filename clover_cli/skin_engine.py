"""Clover skin/theme engine — the theme SDK for every surface.

A data-driven skin system that lets users (and Clover itself) customize the
visual appearance across the CLI, the TUI, and the desktop GUI from a single
file. Skins are defined as YAML files in ~/.clover/skins/ or as built-in presets.
...
``resolve_skin`` / ``skin.changed``). A skin dropped in ~/.clover/skins/ therefore
...
.. code-block:: python
    ...
    set_active_skin("mytheme")            # Switch to user skin from ~/.clover/skins/
...
Drop a YAML file in ``~/.clover/skins/<name>.yaml`` following the schema above.
