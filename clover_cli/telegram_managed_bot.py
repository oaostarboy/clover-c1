...
manual BotFather token copy-paste. Clover talks only to the Clover onboarding
...
DEFAULT_MANAGER_BOT = "CloverSetupBot"

DEFAULT_BOT_NAME = "Clover Cognition"
...
def generate_username_slug(length: int = 16) -> str:
    ...
    keeping ``clover_<slug>_bot`` under Telegram's 32-character username limit.
    """
...
def generate_bot_username(profile_name: Optional[str] = None) -> str:
    """Generate a secure suggested bot username like ``clover_<slug>_bot``.
    ...
    """
    _ = profile_name
    return f"clover_{generate_username_slug()}_bot"
...
def auto_setup_telegram_bot_result(
    ...
    print()
    print(f"  Contacting Clover Telegram onboarding service: {resolved_api_url}")
    sys.stdout.flush()
    pairing = create_pairing(resolved_api_url)
    if not pairing:
        print("  ✗ Could not reach the Clover Telegram onboarding service.")
