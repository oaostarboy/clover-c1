"""``clover verify`` — detect a project's run recipe and smoke-test it.
...
``.clover/environment.json``), then runs bootstrap/build/test phases and an
...
(:mod:`agent.verification_evidence`), so a passing ``clover verify`` satisfies
