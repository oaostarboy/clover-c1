"""
Unified self-relaunch for Clover CLI.

Preserves critical flags (--tui, --dev, --profile, --model, etc.) across
process replacement so that ``clover sessions browse`` or post-setup relaunch
doesn't silently drop the user's UI mode or other preferences.

Also works when ``clover`` is not on PATH (e.g. ``nix run`` or ``python -m``).
"""
...
from clover_cli._parser import (
...
def _extract_inherited_flags(argv: Sequence[str]) -> list[str]:
    """Pull out flags that should carry over into a self-relaunched clover."""
    flags: list[str] = []
    i = 0
    while i < len(argv):
        arg = argv[i]
...
def resolve_clover_bin() -> Optional[str]:
    """Find the clover entry point.

    Priority:
      1. ``sys.argv[0]`` if it resolves to a real executable.
      2. ``shutil.which("clover")`` on PATH.
      3. ``None`` → caller should fall back to ``python -m clover_cli.main``.
    ...
    ``clover.exe`` on PATH or the ``sys.executable -m clover_cli.main``
    fallback.
    """
    argv0 = sys.argv[0]
...
def build_relaunch_argv(
    ...
    *,
    ...
    """Construct an argv list for replacing the current process with clover.
...
def relaunch(
    ...
    *,
    ...
    """Replace the current process with a fresh clover invocation.
    ...
    relaunch contract wants: "run clover again as if the user had typed
    the new argv".
    ...
    *emulates* exec by spawning the child and exiting the parent, but
    ...
    is usually ``clover.exe`` (a Python console-script shim that wraps
    ``python -m clover_cli.main``) or a ``.cmd`` batch file, and both
    ...
    That's functionally equivalent — the user sees "clover exited, then
    new clover started" — just with two PIDs in play instead of one.
    """
    ...
    if sys.platform == "win32":
        ...
        try:
            ...
            # cryptic.  Common causes: ``clover`` not on PATH yet (install
            # hasn't propagated User PATH into this shell) or a stale shim.
            print(
                f"\nClover relaunch failed: {exc}\n"
                f"Command: {' '.join(new_argv)}\n"
                f"Fix: open a new terminal so PATH picks up, then re-run clover.",
                file=sys.stderr,
            )
            sys.exit(1)
    else:
