"""``clover slack ...`` CLI subcommands.

Today only ``clover slack manifest`` is implemented — it generates the
...
Typical workflow::
    ...
    $ clover slack manifest > slack-manifest.json
    # or:
...
def _build_full_manifest(
    ...
    it stays in sync with the rest of Clover. Other manifest sections
    (display info, OAuth scopes, socket mode) are set to sensible defaults
    for a Clover deployment — users can tweak them in the Slack UI after
    pasting.

    By default, this keeps Clover on Slack's older Assistant messaging
    ...
    """
    from clover_cli.commands import slack_app_manifest
    ...
    if messaging_experience == "assistant":
        features["assistant_view"] = {
            "assistant_description": "Chat with Clover in threads and DMs.",
        }
        ...
        features["agent_view"] = {
            "agent_description": "Chat with Clover in Slack Messages.",
        }
        bot_scopes.append("assistant:write")
    ...
    display_information = {
        "name": bot_name[:35],
        "description": (bot_description or "Your Clover agent on Slack")[:140],
...
def slack_manifest_command(args) -> int:
    ...
    Flags (all parsed in ``clover_cli/main.py``):
      --write [PATH]  Write to file instead of stdout (default path:
                      ``$CLOVER_HOME/slack-manifest.json``)
      --name NAME     Override the bot display name (default: "Clover")
    ...
    """
    name = getattr(args, "name", None) or "Clover"
    description = getattr(args, "description", None) or "Your Clover agent on Slack"
    ...
    if getattr(args, "slashes_only", False) and (
        ...
        print(
            "clover slack manifest: long description options cannot be used "
            "with --slashes-only",
            file=sys.stderr,
        )
        return 2
    if long_description_file is not None:
        ...
        try:
            ...
            print(
                f"clover slack manifest: cannot read long description from "
                f"{source_arg}: {exc}",
                file=sys.stderr,
            )
            return 2
    if (
        ...
        print(
            "clover slack manifest: long description must be at least "
    ...
    if (
        ...
        print(
            "clover slack manifest: long description must be at most "
    ...
    if write_target is not None:
        ...
        print(
            "\nNext steps:\n"
            "  1. Open https://api.slack.com/apps and pick your Clover app\n"
