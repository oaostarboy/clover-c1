...
This module is imported by ``clover_cli/main.py`` BEFORE its heavy import
wall (config, argparse tree, logging, providers). Everything here must stay
**stdlib-only and cheap** (os/sys file probes; no yaml, no clover_cli.config,
...
``clover_cli/config.py``'s ``get_container_exec_info()`` reads the same
...
def active_profile_may_override_home(clover_root: str) -> bool:
    """Cheap probe: does an active non-default profile redirect CLOVER_HOME?"""
    active_profile = os.path.join(clover_root, "active_profile")
    try:
...
def _resolved_home() -> str:
    clover_home = os.environ.get("CLOVER_HOME", "").strip()
    if clover_home:
        return clover_home
    return os.path.join(os.path.expanduser("~"), ".clover")
...
def container_mode_may_be_active() -> bool:
    ...
    clover_home = os.environ.get("CLOVER_HOME", "").strip()
    if clover_home:
        if os.path.exists(os.path.join(clover_home, ".container-mode")):
            return True
        parent_name = os.path.basename(os.path.dirname(os.path.normpath(clover_home)))
        return (
            parent_name != "profiles"
            and active_profile_may_override_home(clover_home)
        )

    default_home = os.path.join(os.path.expanduser("~"), ".clover")
...
def read_install_method() -> str | None:
    ...
    non-default profile might redirect CLOVER_HOME.
    """
...
def print_fast_version_info(*, check_updates: bool = True) -> None:
    """THE canonical ``clover --version`` output (also used by /version).
    ...
    ``clover version`` subcommand. Every lazy block degrades gracefully —
    ...
    try:
        from clover_cli.banner import format_banner_version_label

        print(format_banner_version_label())
    except Exception:
        from clover_cli import __release_date__, __version__

        print(f"Clover Cognition v{__version__} ({__release_date__})")
    ...
    try:
        ...
        from clover_cli.config import detect_install_method
    ...
    try:
        from clover_cli.banner import UPDATE_AVAILABLE_NO_COUNT, check_for_updates
...
def try_fast_version(argv: list[str] | None = None) -> bool:
    """Handle ``clover --version`` before the heavy import wall.
    ...
    container. Termux keeps the CLOVER_TERMUX_DISABLE_FAST_CLI escape hatch.
    """
    if argv is None:
        argv = sys.argv[1:]
    is_termux = is_termux_env()
    if is_termux and os.environ.get("CLOVER_TERMUX_DISABLE_FAST_CLI") == "1":
        return False
    if is_termux:
