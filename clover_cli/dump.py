"""
Dump command for clover CLI.

Outputs a compact, plain-text summary of the user's Clover setup
...
from clover_cli.config import get_clover_home, get_env_path, get_project_root, load_config
...
def _dotenv_key_names() -> set[str]:
    """Return the set of env-var names assigned a non-empty value in ~/.clover/.env.
    ...
    exports. ``clover debug share`` runs in a terminal, so ``os.getenv`` reflects
...
def _get_git_commit(project_root: Path) -> str:
    ...
    to ``<project_root>/.clover_build_sha`` by the Dockerfile's
    ``CLOVER_GIT_SHA`` build-arg (see ``clover_cli/build_info.py``).
...
def _redact(value: str) -> str:
    ...
    ``clover dump`` formats empty values as blank, not as ``"(not set)"``).
    """
...
def _gateway_status() -> str:
    ...
    try:
        from clover_cli.gateway import get_gateway_runtime_snapshot
...
def _count_skills(clover_home: Path) -> int:
...
def _cron_summary(clover_home: Path) -> str:
    """Return cron jobs summary."""
    jobs_file = clover_home / "cron" / "jobs.json"
...
def run_dump(args):
    ...
    lines.append(f"clover_home:      {display_clover_home()}")
    ...
    for env_var, label in api_keys:
        ...
        # Set in this (shell) process but absent from ~/.clover/.env: a managed
        ...
        # A credential added via `clover auth add openrouter` lives in the
        # credential pool, not as an env var — surface it so the dump doesn't
        # misleadingly read "not set" while `clover auth list` shows it (#42130).
    ...
    toolsets = config.get("toolsets", ["clover-cli"])
    ...
    lines.append(f"  cron_jobs:          {_cron_summary(clover_home)}")
    lines.append(f"  skills:             {_count_skills(clover_home)}")
