"""Default configuration data for Clover Cognition.

Pure-data leaf module: DEFAULT_CONFIG and OPTIONAL_ENV_VARS, extracted
verbatim from clover_cli/config.py. Must not import from clover_cli.config.
"""
...
DEFAULT_CONFIG = {
    ...
    "toolsets": ["clover-cli"],
    # SQLite journal mode used by every Clover database opener. WAL is the
    ...
    # Soft file-descriptor limit for long-running Clover server processes.
    ...
    "session": {
        # Per-terminal `clover -c`: each CLI session drops a breadcrumb file
        # under $CLOVER_HOME/terminal-sessions/<terminal-id>, and a bare
    ...
    "agent": {
        ...
        # all normalized by clover_cli.config.resolve_turn_limit.
        "max_turns": None,
        ...
        # budget. CLI one-shot equivalent: `clover chat --run-budget N`.
        ...
        # an interactive `clover gateway restart` must never block for hours
        ...
        # the Clover-level retry loop that wraps the whole call.  Lower
        ...
        # prompt's environment-hints block. Lets a host that wraps Clover
        ...
        # identity slot (SOUL.md). Empty by default. The CLOVER_ENVIRONMENT_HINT
        # env var overrides this (build-time/container mechanism).
        "environment_hint": "",
        # Coding posture — on interactive coding surfaces (CLI, TUI, desktop
        # app, ACP) in a code workspace, Clover adds a coding operating brief
        ...
        # `clover status` / fleet monitoring). Retries never stop — this is a
        ...
        # ``CLOVER_LOCAL_STREAM_STALE_TIMEOUT`` overrides for escape-hatch use.
    ...
    "terminal": {
        ...
        # Root directory for Clover' terminal session temp files (background
        # logs/pid/exit files, code-execution sandboxes, etc.). When empty,
        # Clover uses TMPDIR/TMP/TEMP if set, otherwise a managed dir on real
        # storage at CLOVER_HOME/cache/terminal (auto-pruned after 72h) — NOT
        ...
        # bot_relay from a short-lived `clover -p <bot> chat -Q` recipient
        ...
        #             CLOVER_HOME/home for persistent state (default)
        #   real    — force the real OS-user HOME
        #   profile — force CLOVER_HOME/home when it exists (old strict
        ...
        # skipped. When empty, Clover auto-sources ``~/.profile``,
        # ``~/.bash_profile``, and ``~/.bashrc`` (in that order) if the
        # snapshot shell is bash (this is the ``auto_source_bashrc``
        # behaviour — disable with that key if you want strict login-only
        # semantics).
        "shell_init_files": [],
        # When true (default), Clover sources the user's shell rc files
        ...
        # docker_env lets you specify exact key-value pairs — useful when Clover
        ...
        # bundled Clover image, which drops to the `clover` user via
    ...
    "web": {
        ...
        # firecrawl, keenable). Set by the `clover tools` picker's
    ...
    "browser": {
        ...
        # browser tools) runs on a Clover-managed SNAPSHOT of the user's
        ...
        # fresh session launches — driven by Clover' packaged Chromium. Only
        ...
        # deletes the snapshot store (~/.clover/browser-profile/) so copied
        ...
        # approval does it run `clover browser close-profile` (terminates the
        ...
        "camofox": {
            # When true, Clover sends a stable profile-scoped userId to Camofox
            # so the server maps it to a persistent Firefox profile automatically.
            # When false (default), each session gets a random userId (ephemeral).
            "managed_persistence": False,
            # Optional externally managed Camofox identity. Useful when another
            # app owns the visible browser and Clover should operate in it.
            "user_id": "",
            "session_key": "",
    ...
    # Opt in via ``clover chat --checkpoints`` or set enabled=True here.
    "checkpoints": {
        "enabled": False,
        ...
        "max_snapshots": 20,
        # Hard ceiling on total ``~/.clover/checkpoints/`` size (MB).  When
        ...
        "max_file_size_mb": 10,
        # Auto-maintenance: clover sweeps the checkpoint base at startup
        ...
        # ``clover checkpoints prune`` command (add ``--keep-orphans`` to
    ...
    # AGENTS.md, CLAUDE.md, .clover.md, or .cursorrules before Clover applies
    ...
    # Single-query (``clover -q/-z "..."``) variant of mcp_discovery_timeout.
    ...
    # single read_file page exceeds these limits, Clover truncates the
    ...
    "compression": {
        ...
        "codex_app_server_auto": "native",  # Codex app-server (codex CLI runtime) thread
                                      ...
                                      # thread context, so Clover' summarizer cannot
                                      # shrink it (#36801). native = codex decides when
                                      # to compact its own thread (default); clover =
                                      # Clover' compression threshold triggers
        ...
        "codex_responses_native": False,  # Opt in to OpenAI's server-side compaction
                                      ...
                                      # route/model is unaffected. Clover' local
    ...
    "auxiliary": {
        ...
        "review": {
            "provider": "auto",    # auto (= inherit main model) | openrouter | clover | anthropic | ...
        ...
        # Invoked by ``clover kanban specify`` (single id or --all). Set a
        ...
        # Invoked by ``clover kanban decompose`` and the kanban
        ...
        # ``clover profile describe <name> --auto`` and the dashboard's
        ...
        # model; override via `clover model` → auxiliary → Curator to route
    ...
    "display": {
        ...
        # Which interface bare `clover` (and `clover chat`) launches by default:
        ...
        # REPL and `--tui` (or CLOVER_TUI=1) forces the TUI regardless of config.
        "interface": "cli",
        # When true, `clover --tui` auto-resumes the most recent human-
        # facing session on launch instead of forging a fresh one.
        # Mirrors `clover -c` muscle memory.  Default off so existing
        # users aren't surprised.  CLOVER_TUI_RESUME=<id> always wins.
        "tui_auto_resume_recent": False,
        # When true (default), `clover --tui` drops a one-time hint
        ...
        # model — see clover_cli/focus_view.py.
        "focus_view": False,
        ...
        # e.g. `model · 68% · ~/projects/clover`. Per-platform overrides go under
        ...
        # CLI, TUI, and desktop app. Manage with `clover pets`. Disabled until
        ...
        "pet": {
            ...
            # get_clover_home()/pets/. Empty → first installed pet.
            "slug": "",
    ...
    "dashboard": {
        ...
        # protocol ping (see clover_cli/web_server.py — an event-loop stall
        ...
        # CLOVER_TUI_WS_ORPHAN_REAP_GRACE_S env var remains an internal
        ...
        # (CLOVER_TUI_SESSION_TTL_S, default 6h) are closed with
        ...
        # (``CLOVER_DASHBOARD_BASIC_AUTH_USERNAME`` /
        ...
        # Public URL override (env: ``CLOVER_DASHBOARD_PUBLIC_URL``).
        # When set, this is the complete authority — scheme + host +
        # optional path prefix (e.g. ``https://example.com/clover``) —
    ...
    "tts": {
        ...
        "gemini": {
            ...
            # transcript section; Clover appends the live transcript when absent.
            "persona_prompt_file": "",
        },
        "xai": {
        ...
        "piper": {
            ...
            # "voices_dir": "",        # Override voice cache dir; default = ~/.clover/cache/piper-voices/
    ...
    # "Hey Clover" hands-free wake word. Always-on, on-device hotword
    ...
    "wake_word": {
        ...
        "phrase": "hey clover",       # for "sherpa" this IS the detected phrase (any text works); for other engines it's a cosmetic label — detection is keyed by the model/keyword below
        ...
        "openwakeword": {
            # "hey_clover" (the bundled, works-out-of-the-box default) OR a
    ...
    # a plugin in plugins/context_engine/<name>/ or ~/.clover/plugins/.
    "context": {
        "engine": "compressor",
    ...
    # configured providers (OpenRouter, Clover, Z.ai, Kimi, etc.) are supported.
    "delegation": {
        ...
        # trim, the full text is spilled to ~/.clover/cache/delegation/
    ...
    # response. If not, Clover feeds a continuation prompt back into the
    ...
    "goals": {
        # Max continuation turns before Clover auto-pauses the goal and
    ...
    "moa": {
        ...
        # <clover_home>/moa-traces/<session_id>.jsonl. Off by default — turn it
    ...
    "skills": {
        ...
        # with ``clover skills trust`` (run from inside it). Set to false to
        ...
        # Managed by ``clover skills trust`` / ``clover skills untrust``.
        "trusted_project_dirs": [],
        # Substitute ${CLOVER_SKILL_DIR} and ${CLOVER_SESSION_ID} in SKILL.md
        ...
        # (`clover skills install`). Runs ALONGSIDE the built-in skills
        ...
        # ~/.clover/skills/.curator_ledger.jsonl with before/after file
        # hashes; file contents are stored content-addressed (deduped) under
        # ~/.clover/.curator_backups/blobs/. Enables `clover curator ledger`
        # and single-mutation `clover curator rollback <entry-id>`.
    ...
    # cron daemon.
    #
    # See `clover curator status` for the last run summary.
    "curator": {
        "enabled": True,
        ...
        # class-level umbrellas. `clover curator run --consolidate` overrides
        ...
        # normally restored on every `clover update`, so pruning them only
        ...
        # skills are kept forever. When > 0, `clover curator purge` deletes
        ...
        # skipped), snapshot ~/.clover/skills/ into
        # ~/.clover/skills/.curator_backups/<utc-iso>/skills.tar.gz so the
        # user can roll back with `clover curator rollback`.
        "backup": {
            "enabled": True,
    ...
    # This section is only needed for clover-specific overrides; everything else
    ...
    "whatsapp": {
        ...
        # Default (None) uses the built-in "☘ *Clover Cognition*" header.
    ...
    # dangerous command. -q runs export CLOVER_INTERACTIVE=1 (for interactive
    ...
    "approvals": {
        ...
        # modal; CLOVER_TUI_NO_CONFIRM=1 force-skips that modal regardless of
    ...
    # replace Clover' built-in platform hint for a single messaging platform
    ...
    # ``clover plugins enable|disable`` and intentionally omitted here so an
    ...
    # stored approval from ~/.clover/shell-hooks-allowlist.json.
    # See `website/docs/user-guide/features/hooks.md` for schema + examples.
    "hooks": {},

    # Auto-accept shell-hook registrations without a TTY prompt.  Also
    # toggleable per-invocation via --accept-hooks or CLOVER_ACCEPT_HOOKS=1.
    ...
    "security": {
        ...
        # .cursorrules, project-local .clover config) always require human
        ...
        # longer trigger the startup banner. Add via `clover doctor --ack
        # <id>`; remove by editing the list directly. See
        # ``clover_cli/security_advisories.py`` for the catalog.
        "acked_advisories": [],
        # Allow Clover to lazy-install opt-in backend packages from PyPI
    ...
    "cron": {
        ...
        # Deliver clean agent output by default. Set to true to add a task-name
        # header, job ID, separators, and management footer for provenance.
        "wrap_response": False,
        ...
        # Also overridable via CLOVER_CRON_MAX_PARALLEL env var.
        ...
        # CLOVER_CRON_SCRIPT_TIMEOUT. Keep this in sync with
        ...
        # CLOVER_CRON_SESSION_DB_TIMEOUT env var. 0 = unlimited (skip the bound).
        ...
        # overridable via CLOVER_CRON_MEDIA_SEND_TIMEOUT env var. Keep in
    ...
    # todos to ready, and fires `clover -p <assignee> chat -q ...` for
    ...
    "kanban": {
        ...
        # one `clover` launches with no -p flag). This does not control the
        ...
        # decomposition is manual via `clover kanban decompose <id>` or
    ...
    "code_execution": {
        ...
        #     clover-c1's own python (sys.executable). Maximum isolation
    ...
    #
    # Core Clover tools (terminal, read_file, write_file, patch,
    ...
    # Logging — controls file logging to ~/.clover/logs/.
    ...
    # #8731).
    #
    # Provider keys accept the Clover provider id (as used elsewhere in
    ...
    "gateway": {
        ...
        # internal CLOVER_GATEWAY_PLATFORM_CONNECT_TIMEOUT env var, which still
        ...
        # producing ~/.clover/sessions/sessions.json entirely.
        ...
        # the CLOVER_SCALE_TO_ZERO env stamp) AND messaging is relay-only/absent
        ...
        # `launchctl kickstart ai.clover.gateway` that defenses 1-2 don't
        ...
        # `clover update` hang because it can never drain the gateway.
        "restart_loop_guard": {
            "max_restarts": 3,
            "window_seconds": 60,
            "max_gap_seconds": 300,
        ...
        # ``CLOVER_GATEWAY_MAX_STARTS`` / ``CLOVER_GATEWAY_START_WINDOW_S``
        ...
        # system-path denylist (/etc, /proc, ~/.ssh, ~/.aws, ~/.clover/.env,
        ...
        # behavior: files must live under the Clover cache, under
        ...
        # user. Bridged to CLOVER_MEDIA_DELIVERY_STRICT.
        "strict": False,
        # Extra directories from which model-emitted bare file paths may be
        # uploaded as native gateway attachments. Files inside the Clover
        # cache (~/.clover/cache/{documents,images,audio,video,screenshots})
        ...
        # to CLOVER_MEDIA_ALLOW_DIRS at gateway startup. Tilde paths are
        ...
        # CLOVER_MEDIA_TRUST_RECENT_FILES. Only consulted when ``strict``
        # is true; in default mode the denylist alone gates delivery.
        "trust_recent_files": True,
        # Recency window in seconds. 600 (10 min) comfortably covers a
        # multi-tool agent turn. Bridged to CLOVER_MEDIA_TRUST_RECENT_SECONDS.
    ...
    # Session storage — controls automatic cleanup of ~/.clover/state.db.
    ...
    "sessions": {
        ...
        # the default of ``clover sessions prune``.
        "retention_days": 90,
        ...
        # rewrites ``~/.clover/sessions/session_{sid}.json`` on every turn
        ...
        # user runs `clover sessions optimize-storage`, because the rebuild is
        # disk-heavy and long on large DBs (see that command's disk preflight).
        #
        #   "advise" (default): `clover update` prints a one-line notice with
        ...
        # ~/.clover/lib/libfts5_cjk.so), 1-2 char CJK terms (일본, 项目, ...)
        ...
        # serve the cjk index. Bridged to CLOVER_CJK_FTS (internal carrier).
        "cjk_fts": True,
        ...
        # CLOVER_SEARCH_SLOW_MS (internal carrier).
        "search_slow_ms": 1000,
        ...
        # (non-streaming) export such as `clover sessions export`. Checked
    ...
    # ``clover doctor`` behaviour.
    "doctor": {
        # Per-probe timeout (seconds) for the opt-in `clover doctor --live`
        # real-call backend probes (Firecrawl/FAL/browser/MCP/TTS/STT).
        "live_probe_timeout": 10,
    },

    # ``clover update`` behaviour.
    "updates": {
        ...
        #     DBs) into <CLOVER_HOME>/state-snapshots/ before the update.
        ...
        #     jobs) safety net.
        #   full — the quick snapshot PLUS a full ``clover backup``-style zip
        #     of CLOVER_HOME into <CLOVER_HOME>/backups/, restorable with
        #     ``clover import``. Can add minutes on large homes. This is the
        ...
        "backup_keep": 5,
        # What `clover update` does with uncommitted local changes to the
        ...
        # When `clover update` finds the source checkout parked on a feature
        ...
        #                         merge. `clover update --switch-branch`
    ...
    "lsp": {
        ...
        #              ``<CLOVER_HOME>/lsp/bin/`` on first use.
    ...
    # `clover tools`. These settings tune the backing Responses API call.
    "x_search": {
    ...
    # rather than storing them in ~/.clover/.env.
    "secrets": {
        ...
        "bitwarden": {
            ...
            # in ~/.clover/.env (or your shell) and never in config.yaml.
            ...
            # material under ~/.clover/cache/. If a later startup cannot reach
            # Bitwarden due to NETWORK/TIMEOUT, Clover may use this encrypted
            ...
            # ~/.clover/bin/ on first use.  When False you must install
            ...
            # `clover secrets bitwarden setup`.
            "server_url": "",
        },
        "onepassword": {
            ...
            # for headless auth.  Sourced from ~/.clover/.env (or the shell)
    ...
    "computer_use": {
        ...
        # by default upstream. Clover disables it for our users unless they
        # explicitly opt in here. When false (default), Clover sets
        ...
        # cua-driver ≥ 0.6.x supports --no-overlay; Clover also calls
        ...
        "no_overlay": None,
        # cua-driver permission mode for each Clover computer-use runtime.
        ...
        # manifest used when permission_mode is "bounded". Clover passes the
    ...
    #
    # Configure with `clover egress setup`.  Disabled by default — the rest of
    # Clover works exactly as before with `enabled: false`.
    "proxy": {
        ...
        "tunnel_port": 9090,
        # Auto-download the pinned iron-proxy binary into ~/.clover/bin/ on
        ...
        # Together, DeepSeek, Clover).  Wildcards (`*.foo.com`) are supported.
        "extra_allowed_hosts": [],
    },

    # Clover Desktop (Electron app) launch options. These only affect
    # `clover desktop`; they do not touch the CLI/gateway.
    "desktop": {
        ...
        # Bridged to the CLOVER_DESKTOP_DISABLE_GPU env var the Electron app reads.
        "disable_gpu": "auto",
        ...
        # Ignored on macOS/Windows. Bridged to the CLOVER_DESKTOP_PASSWORD_STORE
...
OPTIONAL_ENV_VARS = {
    ...
    "VERTEX_CREDENTIALS_PATH": {
        "description": "Path to a Google Cloud service account JSON for Vertex AI (Gemini). "
                       ...
                       "credentials Clover mints short-lived tokens from. Falls back to "
    ...
    "AZURE_FOUNDRY_BASE_URL": {
        "description": "Azure Foundry base URL (set via 'clover model' for endpoint-specific config)",
    ...
    "PORCUPINE_ACCESS_KEY": {
        "description": "Picovoice access key for the Porcupine 'Hey Clover' wake word engine (optional; openWakeWord is the free default)",
    ...
    "SLACK_ALLOWED_USERS": {
        "description": "Comma-separated Slack member IDs allowed to use Clover, e.g. U01ABC2DEF3. Without this, Slack may connect but deny messages by default.",
    ...
    "MATRIX_USER_ID": {
        "description": "Matrix user ID (e.g. @clover:example.org)",
    ...
    "MATRIX_DEVICE_ID": {
        "description": "Stable Matrix device ID for E2EE persistence across restarts (e.g. CLOVER_BOT)",
    ...
    "IRC_CHANNEL": {
        "description": "IRC channel to join (e.g. #clover)",
        "prompt": "IRC channel",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "IRC_NICKNAME": {
        "description": "Bot nickname on IRC (default: clover-bot)",
        "prompt": "IRC nickname",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "API_SERVER_MODEL_NAME": {
        "description": "Model name advertised on /v1/models. Defaults to the profile name (or 'clover-c1' for the default profile). Useful for multi-user setups with OpenWebUI.",
    ...
    "GATEWAY_PROXY_URL": {
        "description": "URL of a remote Clover API server to forward messages to (proxy mode). When set, the gateway handles platform I/O only — all agent work is delegated to the remote server. Use for Docker E2EE containers that relay to a host agent. Also configurable via gateway.proxy_url in config.yaml.",
        "prompt": "Remote Clover API server URL (e.g. http://192.168.1.100:8642)",
        "url": None,
        "password": False,
        "category": "messaging",
        "advanced": True,
    ...
    "GATEWAY_PROXY_KEY": {
        "description": "Bearer token for authenticating with the remote Clover API server (proxy mode). Must match the API_SERVER_KEY on the remote host.",
    ...
    # CLOVER_TOOL_PROGRESS_MODE is deprecated — tool progress is configured via
    # display.tool_progress in config.yaml (off|new|all|verbose|log). The
    # gateway still falls back to CLOVER_TOOL_PROGRESS_MODE for backward
    ...
    # CLOVER_TOOL_PROGRESS is fully unsupported since the v12 config support
    # floor retired its only consumer (the v3→4 migration).
    "CLOVER_PREFILL_MESSAGES_FILE": {
    ...
    "CLOVER_EPHEMERAL_SYSTEM_PROMPT": {
