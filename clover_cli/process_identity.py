...
Three layers that make every long-lived Clover process positively
identifiable, so reapers (``clover update``, Desktop startup sweeps) never
have to guess lineage from PPID archaeology or cmdline pattern-matching:

1. **Spawn tag** (``CLOVER_SPAWN`` env var): every spawner stamps its children
...
2. **Spawn ledger** (``spawn-ledger.json`` at the machine Clover root): every
...
def install_id(project_root: Optional[Path] = None) -> str:
    ...
    Lets a reaper reject processes from a different Clover install on the
    same machine without path comparisons at scan time.
    """
    if project_root is None:
        try:
            from clover_constants import PROJECT_ROOT as _root
...
def build_spawn_tag(purpose: str, *, project_root: Optional[Path] = None) -> str:
    """Value for the child's ``CLOVER_SPAWN`` env var, stamped by the spawner."""
...
def parse_spawn_tag(raw: object) -> Optional[SpawnTag]:
    """Parse a ``CLOVER_SPAWN`` value; ``None`` for anything malformed."""
...
def _ledger_path() -> Path:
    ...
    try:
        ...
        return Path(get_default_clover_root()) / LEDGER_FILENAME
    except Exception:
        from clover_cli.config import get_clover_home

        return Path(get_clover_home()) / LEDGER_FILENAME
...
def register_self(
    ...
    *,
    ...
    if spawner_pid is None:
        ...
        # CLOVER_PARENT_PID (+ optional `winms:<ms>` start marker) for its
        # parent-death watchdog. Reuse it as spawner identity so ledger
        # lineage works with every Desktop version, no TS change needed.
        try:
            raw = int(os.environ.get("CLOVER_PARENT_PID", ""))
            if raw > 0:
                spawner_pid = raw
        except (TypeError, ValueError):
            pass
        marker = os.environ.get("CLOVER_PARENT_START_MARKER", "")
    ...
    try:
        ...
        # 10 tokens (was 6): enough for `clover serve --host X --port N
...
def register_child(
    ...
    *,
    ...
    ``npx``/binary servers never import Clover code). The entry records the
