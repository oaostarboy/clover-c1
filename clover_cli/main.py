...
def cmd_gui(args: argparse.Namespace):
    ...
    # `CLOVER_DESKTOP_DISABLE_GPU=... clover desktop` keeps working.
...
def _report_dashboard_status() -> int:
    ...
    print(f"{len(live)} clover dashboard process(es) running:")
...
def _maybe_setup_dashboard_auth_interactively(args) -> None:
    ...
    ``clover dashboard register`` for OAuth.
    ...
    try:
        from clover_cli.web_server import should_require_auth
...
# so that in ``clover -m gpt5 chat``, ``gpt5`` is correctly skipped as a
# flag value rather than misclassified as a subcommand. Kept in sync with
# the top-level flags declared in ``clover_cli/_parser.py``.
#
