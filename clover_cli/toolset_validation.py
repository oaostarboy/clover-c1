...
the tool registry or launching Clover (mirrors the decoupled-helper pattern used
elsewhere in the CLI).

Motivated by #38798: a config migration silently rewrote the valid toolset name
``clover-cli`` to the non-existent ``clover``. ``resolve_toolset('clover')``
...
def validate_platform_toolsets(
    ...
    1. A toolset name that ``is_valid_toolset`` rejects — usually a corrupted or
       renamed entry. When ``clover-<platform>`` would have been valid (the exact
       #38798 shape, where ``cli`` held ``clover`` instead of ``clover-cli``),
    ...
    if valid_count == 0:
        warnings.append(
            ...
            "have no tools. Run `clover tools` to reconfigure."
        )
    return warnings
