"""clover webhook — manage dynamic webhook subscriptions from the CLI.

Usage:
    clover webhook subscribe <name> [options]
    clover webhook list
    clover webhook remove <name>
    clover webhook test <name> [--payload '{"key": "value"}']

Subscriptions persist to ~/.clover/webhook_subscriptions.json and are
...
def _subscriptions_path() -> Path:
    return _clover_home() / _SUBSCRIPTIONS_FILENAME
...
def _setup_hint() -> str:
  ...
  Then start the gateway: clover gateway run
"""
...
def webhook_command(args):
    """Entry point for 'clover webhook' subcommand."""
    sub = getattr(args, "webhook_action", None)

    if not sub:
        print("Usage: clover webhook {subscribe|list|remove|test}")
        print("Run 'clover webhook --help' for details.")
        return
...
def _cmd_subscribe(args):
    ...
    print("  The gateway must be running to receive events (clover gateway run).\n")
...
def _cmd_list(args):
    ...
    if not subs:
        ...
        print("  Create one with: clover webhook subscribe <name>")
        return
...
def _cmd_test(args):
    ...
    payload = args.payload or '{"test": true, "event_type": "test", "message": "Hello from clover webhook test"}'
    ...
    try:
        ...
        print(f"  Error: {e}")
        print("  Is the gateway running? (clover gateway run)")
