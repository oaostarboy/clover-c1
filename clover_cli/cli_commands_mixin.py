...
``cli.py``'s ``CloverCLI`` class. ``CloverCLI`` inherits ``CLICommandsMixin`` so
...
from clover_constants import display_clover_home, is_termux as _is_termux_environment
...
class CLICommandsMixin:
    ...
    def _handle_rollback_command(self, command: str):
        ...
        Syntax:
            ...
            /rollback <N> --all       — classic full restore (may overwrite
                                        files you edited after Clover did)
        ...
        if not mgr.enabled:
            ...
            print("  Enable with: clover --checkpoints")
    ...
    def _handle_diff_command(self, command: str):
        ...
        Syntax:
            ...
            /diff session          — everything Clover changed (checkpoint baseline)
    ...
    def _print_session_diff(self, cwd: str, stat_only: bool):
        ...
        if result.get("empty") or (not stat and not diff):
            print("  No changes — Clover hasn't edited any files here yet.")
            return
    ...
    def _handle_snapshot_command(self, command: str):
        """Handle /snapshot — lightweight state snapshots for Clover config/state.
        ...
        if subcmd in {"list", "ls"}:
            ...
            if not snaps:
                ...
                return
            print(f"  State snapshots ({display_clover_home()}/state-snapshots/):\n")
    ...
    def _handle_export_command(self, command: str):
        ...
        """
        from clover_cli.profiles import export_profile, get_active_profile_name
        ...
        try:
            ...
            print("  Share it: the other user runs /import or `clover profile import <archive>`.")
    ...
    def _handle_import_command(self, command: str):
        ...
        try:
            ...
            pass
        print(f"  Use it: clover -p {imported}")
    ...
    def _handle_journey_command(self, cmd_original: str) -> None:
        """Handle /journey — the learning timeline (see `clover journey`).
        ...
        from cli import _cprint
        from clover_cli.journey import register_cli
    ...
    def _handle_paste_command(self):
        ...
        from clover_cli.clipboard import has_clipboard_image
    ...
    def _handle_copy_command(self, cmd_original: str) -> None:
        ...
        try:
            from clover_cli.clipboard import (
    ...
    def _handle_image_command(self, cmd_original: str):
        ...
        if _remainder:
            ...
            _cprint(f"  {_DIM}Tip: type your next message, or run clover chat -q --image {_termux_example_image_path(image_path.name)} \"What do you see?\"{_RST}")
    ...
    def _handle_tools_command(self, cmd: str):
        ...
        def _run_capture(ns: Namespace) -> None:
            ...
            # Buffer reports isatty()=True so color() in clover_cli/colors.py
    ...
    def _handle_handoff_command(self, cmd_original: str) -> bool:
        ...
        # "Is `clover gateway` running?" is the correct diagnosis, and the
        ...
        # CLI printed "Is `clover gateway` running?" while the gateway was
        ...
        try:
            ...
            pass
        _cprint("  Timed out waiting for the gateway. Is `clover gateway` running?")
    ...
    def _handle_resume_command(self, cmd_original: str) -> None:
        ...
        if not target:
            ...
            if self._show_recent_sessions(reason="resume"):
                ...
                return
            _cprint("  Tip:   Use /history or `clover sessions list` to find sessions.")
            return
        ...
        if target.isdigit():
            ...
            from clover_cli.main import _resolve_session_by_name_or_id
        ...
        if not session_meta:
            ...
            _cprint("  Use /sessions or `clover sessions list` to see available sessions.")
            return
        ...
        # same directory as a startup `clover -c`/`--resume`. The startup resume
    ...
    def _handle_worktree_command(self, cmd_original: str) -> None:
        ...
        commits, same as ``clover -w``).

        ``prune`` is the same attended reclaim as ``clover worktree prune``
        (clover_cli/worktree_gc.py): never deletes tracked changes, unique
        ...
        if sub in {"prune", "gc", "clean"}:
            ...
            from clover_cli import worktree_gc
        ...
        if sub in {"new", "add", "create"}:
            ...
            # same way `clover -w` and session-resume cwd restore do.
            try:
                os.chdir(wt_info["path"])
            except OSError as e:
                print(f"  ⚠ Created worktree but could not enter it: {e}")
            os.environ["TERMINAL_CWD"] = wt_info["path"]
            # Register for the same keep-if-unpushed cleanup as `clover -w`.
    ...
    def _handle_personality_command(self, cmd: str):
        ...
        All resolution/persistence goes through clover_cli.personality —
        ...
        if len(parts) > 1:
            ...
            if not name:
                ...
                try:
                    from clover_cli.config import cfg_get, read_raw_config
    ...
    def _handle_pet_command(self, cmd: str):
        ...
        from clover_cli.pets import _set_active, _set_enabled, print_pet_gallery, set_pet_scale, toggle_pet_display
    ...
    def _handle_hatch_command(self, cmd: str):
        ...
        from clover_cli.pets import _set_active
    ...
    def _handle_suggestions_command(self, cmd: str):
        ...
        args = " ".join(tokens)
        try:
            from clover_cli.suggestions_cmd import handle_suggestions_command
    ...
    def _handle_blueprint_command(self, cmd: str):
        ...
        try:
            from clover_cli.blueprint_cmd import handle_blueprint_command
    ...
    def _handle_curator_command(self, cmd: str):
        ...
        Delegates to clover_cli.curator so the CLI and the `clover curator`
        ...
        try:
            from clover_cli.curator import cli_main
            cli_main(tokens)
        except SystemExit:
    ...
    def _handle_kanban_command(self, cmd: str):
        ...
        """
        from clover_cli.kanban import run_slash
    ...
    def _handle_skills_command(self, cmd: str):
        """Handle /skills slash command — delegates to clover_cli.skills_hub."""
        ...
        if args and args[0].lower() in {"pending", "approve", "apply", "reject",
            ...
            from clover_cli.write_approval_commands import handle_pending_subcommand
            ...
            if out is not None:
                print(out)
                return
        from clover_cli.skills_hub import handle_skills_slash
    ...
    def _handle_plan_command(self, cmd: str):
        ...
        ``.clover/plans/`` via ``write_file``. No engine, no model-tool
    ...
    def _handle_init_command(self, cmd: str):
        ...
        """
        from clover_cli.init_command import build_init_prompt_for_cwd
    ...
    def _handle_bundles_command(self, cmd: str) -> None:
        ...
        Mirrors ``clover bundles list`` but renders inside the running
        ...
        if not bundles:
            ...
            _cprint(
                f"  {_DIM}Create one with: clover bundles create "
        ...
        _cprint(
            ...
            f"Manage with `clover bundles`.{_RST}"
        )
        ...
        if sub.startswith("connect"):
            ...
            if hasattr(self, '_pending_input'):
                self._pending_input.put(
                    ...
                    "just because CDP is connected. This is typically a Clover-managed isolated debug "
    ...
    def _handle_heartbeat_command(self, cmd: str) -> None:
        ...
        cross-process schedules use `clover cron`.
        """
        from cli import _DIM, _RST, _cprint
        from clover_cli.heartbeat import parse_interval, format_interval
        ...
        if interval is None:
            ...
            return
        if interval < 0:
            from clover_cli.heartbeat import MIN_INTERVAL_SECONDS
        ...
        _cprint(
            ...
            f"lives only while this Clover process runs — use `clover cron` for "
    ...
    def _handle_goal_command(self, cmd: str) -> None:
        ...
        from clover_cli.goals import parse_contract
        ...
        _cprint(
            ...
            f"Clover keeps working until it is, you pause/clear it, or the budget is "
    ...
    def _handle_goal_draft(self, objective: str) -> None:
        ...
        from cli import _DIM, _RST, _cprint
        from clover_cli.goals import draft_contract
    ...
    def _handle_loop_command(self, cmd: str) -> None:
        ...
        from clover_cli.loops import dispatch_loop_command
    ...
    def _handle_skin_command(self, cmd: str):
        ...
        try:
            from clover_cli.skin_engine import list_skins, set_active_skin, get_active_skin_name
        except ImportError:
        ...
        if len(parts) < 2 or not parts[1].strip():
            ...
            print("\n  Usage: /skin <name>")
            print(f"  Custom skins: drop a YAML file in {display_clover_home()}/skins/\n")
            return
    ...
    def _compose_in_editor(self, initial_text: str = "") -> str:
        ...
        fd, path = tempfile.mkstemp(suffix=".md", prefix="clover_prompt_")
        try:
    ...
    def _set_tool_progress_mode(self, mode: str) -> None:
        ...
        """
        from clover_cli.focus_view import normalize_tool_progress_mode
    ...
    def _note_focus_hidden_line(self, function_name: str) -> None:
        ...
        if not getattr(self, "_focus_view_enabled", False):
            return
        from clover_cli.focus_view import would_display_tool_line
    ...
    def _emit_focus_recovery_line(self) -> None:
        ...
        if not getattr(self, "_focus_view_enabled", False):
            return
        from clover_cli.focus_view import format_hidden_line
    ...
    def _handle_approvals_command(self, cmd_original: str) -> None:
        ...
        from cli import _cprint
        from clover_cli.approval_mode import run_approval_mode_command
    ...
    def _handle_busy_command(self, cmd: str):
        """Handle /busy — control what Enter does while Clover is working.
        ...
        if save_config_value("display.busy_input_mode", arg):
            if arg == "queue":
                behavior = "Enter will queue follow-up input while Clover is busy."
            elif arg == "steer":
                behavior = "Enter will steer your message into the current run (after the next tool call)."
            else:
                behavior = "Enter will redirect the current run while Clover is busy; /stop still cancels it."
    ...
    def _handle_indicator_command(self, cmd: str):
        ...
        """
        from cli import _ACCENT, _DIM, _RST, _cprint, save_config_value
        from clover_constants import DEFAULT_INDICATOR_STYLE, INDICATOR_STYLES
    ...
    def _handle_fast_command(self, cmd: str):
        ...
        try:
            from clover_cli.models import _is_anthropic_fast_model
            agent = getattr(self, "agent", None)
    ...
    def _handle_debug_command(self, cmd_original: str = ""):
        ...
        - ``/debug clover``   → upload to Clover-internal storage (private, staff-only)
        - ``/debug local``  → render the report to stdout, no upload

        ``clover`` and ``local`` are mutually exclusive; if both are given,
        ...
        clover = "clover" in words and not local
        ...
        args = SimpleNamespace(
            lines=200, expire=7, local=local, clover=clover, yes=True
        )
        run_debug_share(args)

    def _handle_update_command(self) -> bool:
        """Handle /update — update Clover Cognition to the latest version.

        In the classic CLI this exits the session and relaunches as
        ``clover update`` so the user sees update output directly and gets
        ...
        if is_managed():
            print(f"  ✗ {format_managed_message('update Clover Cognition')}")
            return False
        ...
        choices = [
            ("once", "Update Now", "exit the current session and update Clover Cognition"),
    ...
    def _handle_wake_command(self, command: str):
        """Handle /wake [on|off|status] — the 'Hey Clover' hotword listener.
