...
- **Portal base URL** resolves with the same precedence as the device-flow login
  (``auth.py``): ``CLOVER_PORTAL_BASE_URL`` → ``NOUS_PORTAL_BASE_URL`` → the
...
DEFAULT_PORTAL_BASE_URL = "https://portal.clover-c1.local"
...
# clover_cli.auth.NOUS_BILLING_MANAGE_SCOPE (kept here too so this module has no
...
def resolve_portal_base_url(state: Optional[dict[str, Any]] = None) -> str:
    ...
    ``CLOVER_PORTAL_BASE_URL`` → ``NOUS_PORTAL_BASE_URL`` → stored auth-state
    ``portal_base_url`` → registry default. Trailing slash stripped.
    """
    env = os.getenv("CLOVER_PORTAL_BASE_URL") or os.getenv("NOUS_PORTAL_BASE_URL")
