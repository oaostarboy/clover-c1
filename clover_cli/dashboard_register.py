"""``clover dashboard register`` — register a self-hosted dashboard OAuth client.

Automates what a user otherwise does by hand: open the Clover Portal
``/local-dashboards`` page in a browser, click "register", copy the
resulting ``agent:{id}`` OAuth client ID, and paste it into ``~/.clover/.env``
as ``CLOVER_DASHBOARD_OAUTH_CLIENT_ID``.

This command:
  1. Resolves a fresh Clover Portal access token from the existing login
     (``~/.clover/auth.json``), refreshing it if needed. Fails fast with a
     "run `clover setup`" hint when the user isn't logged in.
  ...
  3. Writes ``CLOVER_DASHBOARD_OAUTH_CLIENT_ID`` and (if absent)
     ``CLOVER_DASHBOARD_PORTAL_URL`` into ``~/.clover/.env`` idempotently.
...
def _resolve_portal_base_url(override: Optional[str] = None) -> str:
    ...
    Precedence:
      1. ``override`` — explicit ``--portal-url`` flag or
         ``CLOVER_DASHBOARD_PORTAL_URL`` env (used for testing against a
      ...
      2. The ``portal_base_url`` stored on the Clover login — this is the
    ...
    try:
        from clover_cli.auth import DEFAULT_NOUS_PORTAL_URL, get_provider_auth_state
...
def _register_self_hosted_client(
    *,
    ...
    makes re-running ``clover dashboard register`` idempotent. The portal
    ...
    try:
        ...
        if exc.code == 401:
            raise RuntimeError(
                "Clover Portal rejected the access token (401). "
                "Try `clover auth add nous` to re-authenticate."
            ) from exc
        if exc.code == 403:
            raise RuntimeError(
                detail
        ...
        raise RuntimeError(
            f"Could not reach Clover Portal at {portal_base_url}: {exc.reason}"
        ) from exc
...
def _print_post_register_hint(
    *,
    ...
    from clover_cli.config import get_env_path

    env_path = get_env_path()
    _cid = client_id
    print()
    print(f"  Wrote to {env_path}:")
    print("    CLOVER_DASHBOARD_OAUTH_CLIENT_ID=" + str(_cid))
    if wrote_portal_url:
        print("    CLOVER_DASHBOARD_PORTAL_URL=" + str(portal_base_url))
    if public_url:
        print("    CLOVER_DASHBOARD_PUBLIC_URL=" + str(public_url))
    print()
    print(
        "  Heads up — Clover login only *engages* on a non-loopback bind. A plain\n"
        "  `clover dashboard` (localhost) leaves the gate off and serves locally\n"
    ...
    if custom_redirect_uri:
        ...
        try:
            ...
            host = "your-host"
        print("  To require Clover login on your registered host, run the dashboard")
        print(f"  bound publicly (it must be reachable at https://{host}) and log in")
        print("  at its /login page.")
    else:
        print("  To require Clover login (e.g. exposing on your LAN or a public host):")
        print("    clover dashboard --host 0.0.0.0")
...
def cmd_dashboard_register(args) -> None:
    ...
    from clover_cli.auth import AuthError, resolve_nous_access_token
    from clover_cli.config import get_env_value, is_managed, save_env_value

    # Managed (Docker/hosted) installs get their dashboard OAuth client_id
    # stamped in by the orchestrator (NAS sets CLOVER_DASHBOARD_OAUTH_CLIENT_ID
    ...
    if is_managed():
        print(
            "✗ `clover dashboard register` is not available in a managed/hosted "
            "install.\n"
            "  The dashboard OAuth client is provisioned by the hosting platform."
        )
        sys.exit(1)

    # 1. Resolve a fresh Clover access token (refreshes if near expiry). Fail fast
    ...
    # CLOVER_DASHBOARD_PORTAL_URL env var, else the stored login's portal.
    #
    ...
    # client_id locally (CLOVER_DASHBOARD_OAUTH_CLIENT_ID). Re-send it so the
    ...
    try:
        existing_client_id = get_env_value("CLOVER_DASHBOARD_OAUTH_CLIENT_ID")
    except Exception:
        existing_client_id = None
    ...
    try:
        save_env_value("CLOVER_DASHBOARD_OAUTH_CLIENT_ID", client_id)
    except Exception as exc:
        print(f"✗ Failed to write CLOVER_DASHBOARD_OAUTH_CLIENT_ID to .env: {exc}")
        print(f"  Set it manually:  CLOVER_DASHBOARD_OAUTH_CLIENT_ID={client_id}")
        sys.exit(1)

    # Persist the portal URL. Two cases:
    #   a) The user explicitly supplied a custom portal (--portal-url flag or
    #      CLOVER_DASHBOARD_PORTAL_URL env). That's an intentional choice we
    ...
    default_portal = "https://portal.clover-c1.local"
    existing_portal = None
    try:
        existing_portal = get_env_value("CLOVER_DASHBOARD_PORTAL_URL")
    except Exception:
        existing_portal = None
    ...
    if should_write_portal:
        try:
            save_env_value("CLOVER_DASHBOARD_PORTAL_URL", portal_base_url)
    ...
    # the portal, e.g. https://clover.example.com/auth/callback. At serve time
    # the dashboard auth layer (dashboard_auth/routes._redirect_uri) reconstructs
    # that same callback by taking CLOVER_DASHBOARD_PUBLIC_URL and appending
    ...
    # the supplied redirect URI and persist it as CLOVER_DASHBOARD_PUBLIC_URL so
    ...
    if public_url:
        ...
        try:
            existing_public_url = get_env_value("CLOVER_DASHBOARD_PUBLIC_URL")
        except Exception:
            existing_public_url = None
        if existing_public_url != public_url:
            try:
                save_env_value("CLOVER_DASHBOARD_PUBLIC_URL", public_url)
