...
Pure display functions with no CloverCLI state dependency.
"""
import json
import logging
import os
...
def cprint(text: str):
    ...
    try:
        ...
        # absent stdout (pythonw.exe, CI, `clover ... > file`) raises
...
_UPSTREAM_REPO_URL = "https://github.com/CloverCognition/clover-c1.git"
_OFFICIAL_REPO_CANONICAL = "github.com/clovercognition/clover-c1"
...
def _github_compare_behind(current_rev: str, target_rev: str) -> Optional[int]:
    ...
    if not (_is_full_sha(current_rev) and _is_full_sha(target_rev)):
        return None
    url = (
        "https://api.github.com/repos/clovercognition/clover-c1/"
    ...
    try:
        ...
        req = urllib.request.Request(
            ...
            headers={
                ...
                "User-Agent": "clover-cli-update-check",
            },
        )
...
def _check_via_local_git(repo_dir: Path) -> Optional[int]:
    ...
    if _is_official_ssh_remote(origin_url):
        ...
        # ahead checkout as behind nudges the user into `clover update`,
...
def check_for_updates() -> Optional[int]:
    """Check whether a Clover update is available.

    Two paths: if ``CLOVER_REVISION`` is set (nix builds embed it), compare
    ...
    cache_file = clover_home / ".update_check"
    embedded_rev = os.environ.get("CLOVER_REVISION") or None

    # Docker images have no working tree to count commits against — the
    # published image excludes `.git` (see .dockerignore) and sets no
    # CLOVER_REVISION (that's nix-only). Returning None makes both the Rich
    # banner (build_welcome_banner) and the Ink badge (branding.tsx, guarded
    # on `typeof === 'number' && > 0`) show nothing. The dashboard's REST
    # `/api/clover/update/check` endpoint short-circuits docker the same way
    # (web_server.py); mirror that here so the banner/TUI surfaces agree.
    try:
        from clover_cli.config import detect_install_method, get_project_root
    ...
    if embedded_rev:
        ...
        # $CLOVER_HOME/clover-c1/ may be a stale copy from --clone-all;
        # Path(__file__) always resolves to the actual installed checkout.
        repo_dir = Path(__file__).parent.parent.resolve()
        if not (repo_dir / ".git").exists():
            repo_dir = clover_home / "clover-c1"
        if not (repo_dir / ".git").exists():
...
def _resolve_repo_dir() -> Optional[Path]:
    """Return the active Clover git checkout, or None if this isn't a git install.

    Prefers the running code's location over the profile-scoped path
    because ``$CLOVER_HOME/clover-c1/`` may be a stale copy carried
    over by ``--clone-all``.
    """
    repo_dir = Path(__file__).parent.parent.resolve()
    if not (repo_dir / ".git").exists():
...
def get_git_banner_state(repo_dir: Optional[Path] = None) -> Optional[dict]:
    ...
    ``clover_cli/build_info.py``) and return it as a frozen
...
def _compute_git_banner_state(repo_dir: Optional[Path] = None) -> Optional[dict]:
    ...
    if repo_dir is None:
        ...
        try:
            from clover_cli.build_info import get_build_sha
...
_RELEASE_URL_BASE = "https://github.com/CloverCognition/clover-c1/releases/tag"
...
def get_latest_release_tag(repo_dir: Optional[Path] = None) -> Optional[tuple]:
    ...
    Clover checkout. Cached per-process. Release URL always points at the
    canonical CloverCognition/clover-c1 repo (forks don't get a link).
    """
...
def format_banner_version_label() -> str:
    ...
    base = f"Clover Cognition v{VERSION} ({RELEASE_DATE})"
...
def _format_update_notice(behind: int) -> str:
    ...
    from clover_cli.config import get_managed_update_command, recommended_update_command
    if behind > 0:
    ...
    # UPDATE_AVAILABLE_NO_COUNT: nix-built clover; we know an update
...
def _banner_snapshot_path() -> Path:
    return get_clover_home() / "cache" / "banner_snapshot.json"
...
def banner_snapshot_fingerprint() -> Optional[str]:
    ...
    try:
        ...
        for p in (get_config_path(), get_clover_home() / ".env"):
            try:
                st = p.stat()
...
def build_welcome_banner(console: "Console", model: str, cwd: str,
    ...
    try:
        ...
        _hero = _bskin.banner_hero if hasattr(_bskin, 'banner_hero') and _bskin.banner_hero else CLOVER_CADUCEUS
    except Exception:
        _bskin = None
    ...
    if (provider or "").strip().lower() == "moa":
        ...
        left_lines.append(f"[{accent}]MoA: {preset_name}[/]{agg_str}{ctx_str} [dim {dim}]·[/] [dim {dim}]Clover Cognition[/]")
    else:
        if not (model or "").strip() or (model or "").strip().lower() == "unknown":
            ...
            left_lines.append(
                ...
                f"[dim {dim}]— run /model or clover setup[/]"
            )
        else:
            ...
            ctx_str = f" [dim {dim}]·[/] [dim {dim}]{_format_context_length(context_length)} context[/]" if context_length else ""
            left_lines.append(f"[{accent}]{model_short}[/]{ctx_str} [dim {dim}]·[/] [dim {dim}]Clover Cognition[/]")

    if os.getenv("CLOVER_YOLO_MODE"):
    ...
    if not _has_native_mcp:
        try:
            from clover_cli.plugins import get_portable_mcp_server_names_nowait
    ...
    try:
        from clover_cli.codex_runtime_switch import get_current_runtime
        from clover_cli.config import load_config as _load_cfg
