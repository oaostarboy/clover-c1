"""``clover approvals suggest`` — mine approval history into allowlist proposals.

Clover has no dedicated approval-decision ledger: ``always`` answers land in
...
is the session DB (``~/.clover/state.db``): every assistant ``terminal`` tool
...
* **Destructive / privilege / credential / obfuscation classes are never
  ...
  restarts, clover self-management, …) are eligible.
...
def normalize_command(command: str) -> str:
    """Fold user/clover home prefixes and collapse whitespace.

    Reuses tools.approval's home-folding machinery so proposals are portable
    across machines/users (``/home/alice/x`` -> ``~/x``).
    """
    from tools.approval import (
        _rewrite_resolved_clover_home,
        _rewrite_resolved_user_home,
    )

    folded = _rewrite_resolved_user_home(_rewrite_resolved_clover_home(command))
...
def _render_text(proposals: list[Proposal], days: int) -> None:
    ...
    print(
        ...
        "  clover approvals suggest --apply 1,3\n"
        "Entries are merged into command_allowlist in ~/.clover/config.yaml."
    )


def suggest_command(args) -> int:
    """Entry point for ``clover approvals suggest``."""
...
def approvals_command(args) -> int:
    """Dispatch ``clover approvals <subcommand>``."""
    sub = getattr(args, "approvals_command", None)
    if sub == "suggest":
        return suggest_command(args)
    if sub == "test":
        from clover_cli.approvals_test import approvals_test_command
        return approvals_test_command(args)
    print(
        "usage: clover approvals <subcommand>\n"
        "\n"
        "subcommands:\n"
        ...
        "\n"
        "Run `clover approvals <subcommand> -h` for details."
    )
    return 1
