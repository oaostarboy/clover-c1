"""``clover gateway`` and ``clover proxy`` subcommand parsers.
...
def build_gateway_parser(
    ...
    gateway_run.add_argument(
        ...
        help=(
            ...
            "if CLOVER_DASHBOARD is set). Pass --no-supervise to opt out and "
    ...
    gateway_migrate_legacy = gateway_subparsers.add_parser(
        "migrate-legacy",
        help="Remove legacy clover.service units from pre-rename installs",
        description=(
            "Stop, disable, and remove legacy Clover gateway unit files "
            "(e.g. clover.service) left over from older installs. Profile "
            "units (clover-gateway-<profile>.service) and unrelated "
    ...
    gateway_enroll.add_argument(
        ...
        help=(
            ...
            "GATEWAY_RELAY_WAKE_URL in ~/.clover/.env and forwarded at provision. "
