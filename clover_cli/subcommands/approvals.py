"""``clover approvals`` subcommand parser.
...
def build_approvals_parser(subparsers, *, cmd_approvals: Callable) -> None:
    ...
    approvals_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "`clover approvals suggest` mines past approval decisions from "
    ...
    suggest_parser.add_argument(
        "--db",
        help="Path to an alternate session database (default: ~/.clover/state.db)",
    )
    ...
    test_parser = approvals_subparsers.add_parser(
        ...
        description=(
            ...
            "aren't parsed: clover approvals test -- rm -rf /tmp/x"
        ),
    )
