"""``clover cron`` subcommand parser.

Extracted verbatim from ``clover_cli/main.py:main()`` — same arguments, same
...
def build_cron_parser(subparsers, *, cmd_cron: Callable) -> None:
    ...
    cron_create.add_argument(
        "--script",
        help=(
            "Path to a script under ~/.clover/scripts/. Default mode: "
    ...
    cron_create.add_argument(
        ...
        help=(
            ...
            "~/.clover/scripts/ that runs each tick BEFORE the agent. "
    ...
    cron_create.add_argument(
        "--provider",
        dest="model_provider",
        help="Inference provider paired with --model (e.g. 'openrouter', 'clover').",
    )
    cron_create.add_argument(
        "--reasoning-effort",
    ...
    cron_edit.add_argument(
        "--script",
        help=(
            "Path to a script under ~/.clover/scripts/. Pass empty string to clear. "
    ...
    cron_edit.add_argument(
        ...
        dest="monitor_script",
        help=(
            "Set/replace the monitor source script (see `clover cron create "
