"""``clover login`` subcommand parser.
...
def build_login_parser(subparsers, *, cmd_login: Callable) -> None:
    ...
    ``clover login`` was removed in favor of ``clover auth`` / ``clover model``
    (the runtime handler in ``clover_cli/auth.py::login_command`` just prints a
    deprecation message and exits).  The subparser is kept registered so that
    old scripts/aliases invoking ``clover login [--flags]`` still receive the
    ...
    - The subparser is registered WITHOUT a ``help=`` kwarg so the row is
      omitted from ``clover --help`` (argparse only lists subcommands that
    ...
    - ``--provider`` accepts ANY value (no ``choices=``) so that, e.g.,
      ``clover login --provider anthropic`` reaches the deprecation handler and
      gets pointed at ``clover model`` instead of crashing in argparse with
      ``invalid choice: 'anthropic'`` before the handler can run.
    """
    login_parser = subparsers.add_parser(
        "login",
        description=(
            "Deprecated. Use `clover auth` to manage credentials, "
            "`clover model` to select a provider, or `clover setup` for full setup."
        ),
    )
    ...
    login_parser.add_argument(
        "--client-id", default=None, help="OAuth client id to use (default: clover-cli)"
    )
