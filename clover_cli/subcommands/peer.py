"""``clover peer`` — bot-to-bot DMs across machines/gateways.

A *peer* is another Clover gateway (any machine: homelab, Spark, Clover
...
bot on THIS machine a transport to message bots on THAT machine:
    ...
    clover peer dm spark "Message from 🤖 dixie (@dixie): disk status?"
    clover peer dm spark/researcher "..."      # named profile (multiplexed peer)
...
``clover -p <bot> chat --in ~ -c "Bot Chat" ...`` bot-messaging command, so
...
- Peer labels/URLs live in config.yaml (``bot_peers``); the peer's
  API_SERVER_KEY is a credential and lives in ``~/.clover/.env`` as
  ``CLOVER_PEER_<NAME>_KEY``.
...
def _peer_key_env(name: str) -> str:
    return f"CLOVER_PEER_{name.upper().replace('-', '_')}_KEY"
    ...
    try:
        ...
        if exc.code == 400 and "title" in detail.lower():
            ...
            raise RuntimeError(
                ...
                f"(HTTP 400: {detail}). Update the peer's clover-c1, or unhide the "
                f"session there: PATCH /api/sessions/<id> {{\"hidden\": false}}."
            ) from exc
        raise
    # Real api_server wraps the row: {"object": "clover.session", "session": {...}}.
...
def _parse_target(target: str) -> tuple[str, str | None]:
    ...
    if not peer:
        raise ValueError("Peer name required (clover peer dm <peer>[/<agent>] ...)")
...
def cmd_peer(args) -> int:
    ...
    if action in ("add", "set"):
        ...
        if key:
            ...
            print(f"Peer '{name}' saved ({url}) — key stored as {_peer_key_env(name)} in ~/.clover/.env")
        else:
            print(
                f"Peer '{name}' saved ({url}). No key given — set the peer's API_SERVER_KEY with:\n"
                f"  clover peer add {name} --url {url} --key <key>\n"
                f"  (or add {_peer_key_env(name)}=<key> to ~/.clover/.env)"
            )
        return 0
    ...
    if action in ("list", "ls", None):
        peers = _load_peers()
        if not peers:
            print("No peers registered. Add one: clover peer add <name> --url http://host:port --key <API_SERVER_KEY>")
            return 0
    ...
    if action == "dm":
        ...
        if not isinstance(peer, dict) or not peer.get("url"):
            print(f"No peer named '{peer_name}'. Run: clover peer list", file=sys.stderr)
            return 1
        key = _peer_secret(peer_name)
        if not key:
            print(
                f"No API key for peer '{peer_name}'. Set it: clover peer add {peer_name} "
                f"--url <url> --key <key> (or add {_peer_key_env(peer_name)}=<key> to ~/.clover/.env)",
                file=sys.stderr,
            )
            return 1
    ...
    print("Unknown peer action. See: clover peer --help", file=sys.stderr)
    return 2
...
def build_peer_parser(subparsers) -> None:
    ...
    parser = subparsers.add_parser(
        "peer",
        help="Bot-to-bot DMs across machines (peer Clover gateways)",
        description=(
            "Register other Clover gateways as peers and message their agents. "
            "'clover peer dm <peer>[/<agent>] \"...\"' delivers into the remote "
            "agent's canonical Bot Chat over the peer's API server and prints "
            "the reply — the cross-machine twin of 'clover -p <bot> chat'. "
            "The peer must run the api_server platform; its API_SERVER_KEY is "
            "stored locally as a credential in ~/.clover/.env."
        ),
        epilog=(
            "Examples:\n"
            "  clover peer add spark --url http://spark.lan:8377 --key <API_SERVER_KEY>\n"
            "  clover peer list\n"
            '  clover peer dm spark "Message from 🤖 dixie (@dixie): disk status?"\n'
            '  clover peer dm spark/researcher "..."   # named profile on a multiplexed peer\n'
            "  clover peer remove spark\n"
            "\n"
    ...
    add_p.add_argument("--key", default="", help="The peer's API_SERVER_KEY (stored in ~/.clover/.env)")
