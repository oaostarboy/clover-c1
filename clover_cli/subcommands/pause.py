"""``clover pause`` / ``clover resume`` — the global emergency stop.

``clover pause`` writes the ESTOP sentinel at ``$CLOVER_HOME/ESTOP``, which
halts cron dispatch, kanban dispatch, and new gateway turns on their next
check. In-flight work is never killed. ``clover resume`` removes the
...
def cmd_pause(args: argparse.Namespace) -> int:
    ...
    state = get_state() or {}
    verb = "Still paused" if already else "Clover paused"
    ...
    print(
        ...
        "    In-flight work keeps running. Run `clover resume` to lift the pause."
    )
    return 0
...
def cmd_resume(args: argparse.Namespace) -> int:
    ...
    if disengage():
        print("▶️  Clover resumed — dispatch picks up on the next tick.")
    else:
        print(f"Clover is not paused (no sentinel at {sentinel_path()}).")
    return 0
...
def build_pause_parser(subparsers) -> None:
    ...
    pause_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "`clover resume`. In-flight work is never killed."
        ),
    )
    ...
    resume_parser = subparsers.add_parser(
        "resume",
        help="Lift the emergency stop set by `clover pause`",
