"""``clover dashboard`` / ``clover serve`` subcommand parsers.
...
``clover_cli/main.py:main()`` (god-file Phase 2); handler injected to avoid
importing ``main``.
"""
...
def _add_server_runtime_args(parser) -> None:
    ...
    # `clover dashboard` / `clover serve` cmdlines and SIGTERM them directly —
    # the same path `clover update` uses to clean up stale servers.
    parser.add_argument(
        "--stop",
        action="store_true",
        help="Stop all running Clover web server processes and exit",
    )
    parser.add_argument(
        "--status",
        action="store_true",
        help="List running Clover web server processes and exit",
    )
...
def build_dashboard_parser(
    ...
    dashboard_parser = subparsers.add_parser(
        ...
        description="Launch the Clover Cognition web dashboard for managing config, API keys, and sessions",
    )
    _add_server_runtime_args(dashboard_parser)
    dashboard_parser.add_argument(
        "--no-open", action="store_true", help="Don't open browser automatically"
    )
    # Backward-compat shim: older Clover desktop app shells (<= 0.15.x) spawn the
    # backend as `clover dashboard --no-open --tui --host ... --port ...`. The
    ...
    # showed "Clover couldn't start" with no actionable cause. Accept and silently
    ...
    # browser. It exists so the Clover Desktop app (and headless remote
    ...
    serve_parser = subparsers.add_parser(
        "serve",
        help="Start the Clover backend server (headless; powers the desktop app and remote backends)",
        description=(
            "Run the Clover backend server — the JSON-RPC/WebSocket gateway the "
    ...
    # (cmd_dashboard exports CLOVER_SERVE_HEADLESS=1). `dashboard` leaves it
    # unset and serves the browser UI as before.
    serve_parser.set_defaults(func=cmd_dashboard, no_open=True, headless_backend=True)

    # `clover dashboard register` — register a self-hosted dashboard OAuth
    # client with Clover Portal and write the client_id into ~/.clover/.env.
    # Nested subparser so bare `clover dashboard` keeps launching the server
    ...
    dashboard_register_parser = dashboard_subparsers.add_parser(
        "register",
        help="Register a self-hosted dashboard with Clover Portal (writes the OAuth client ID to .env)",
        description=(
            "Register this install as a self-hosted dashboard with your Clover "
            "Portal account. Creates an OAuth client, writes "
            "CLOVER_DASHBOARD_OAUTH_CLIENT_ID into ~/.clover/.env, and prints "
            "how to engage the login gate. Requires being logged in (clover setup)."
        ),
    )
    ...
    dashboard_register_parser.add_argument(
        ...
        help=(
            ...
            "https://clover.example.com/auth/callback. Omit for localhost-only use."
        ),
    )
    dashboard_register_parser.add_argument(
        "--portal-url",
        ...
        default=None,
        help=(
            "Override the Clover Portal base URL for registration (default: the "
            "portal you logged into). The access token must be valid at this "
            "portal. Also settable via CLOVER_DASHBOARD_PORTAL_URL. Mainly for "
