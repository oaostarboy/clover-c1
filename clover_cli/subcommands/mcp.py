"""``clover mcp`` subcommand parser.

Extracted from ``clover_cli/main.py:main()`` (god-file Phase 2 follow-up).
...
from clover_cli.subcommands._shared import add_accept_hooks_flag
...
def build_mcp_parser(subparsers, *, cmd_mcp: Callable) -> None:
    ...
    mcp_parser = subparsers.add_parser(
        "mcp",
        help="Manage MCP servers and run Clover as an MCP server",
        description=(
            "Manage MCP server connections and run Clover as an MCP server.\n\n"
            "MCP servers provide additional tools via the Model Context Protocol.\n"
            "Use 'clover mcp add' to connect to a new server, or\n"
            "'clover mcp serve' to expose Clover conversations over MCP."
        ),
    )
    ...
    mcp_serve_p = mcp_sub.add_parser(
        "serve",
        help="Run Clover as an MCP server (expose conversations to other agents)",
    )
    ...
    # flag is omitted, causing `clover mcp add ...` to fall through to
    # interactive chat.
    ...
    # ── Catalog (Clover-approved MCPs shipped with the repo) ─────────────────
    mcp_sub.add_parser(
        "picker",
        help="Interactive catalog picker (also the default for `clover mcp`)",
    )
    mcp_sub.add_parser(
        "catalog",
        help="List Clover-approved MCPs available for one-click install",
    )
    mcp_install_p = mcp_sub.add_parser(
        "install",
        help="Install a catalog MCP by name (e.g. `clover mcp install n8n`)",
    )
