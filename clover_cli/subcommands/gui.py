...
def build_gui_parser(subparsers, *, cmd_gui: Callable) -> None:
    ...
    gui_parser = subparsers.add_parser(
        ...
        description=(
            "Launch the Clover Electron desktop app. By default this installs "
    ...
    gui_parser.add_argument(
        "--ignore-existing",
        action="store_true",
        help="Force Desktop to ignore any clover CLI already on PATH during backend resolution",
    )
    gui_parser.add_argument(
        "--clover-root",
        help="Override the Clover source root used by Desktop (sets CLOVER_DESKTOP_CLOVER_ROOT)",
    )
    gui_parser.add_argument(
        "--cwd",
        help="Initial project directory for Desktop chat sessions (sets CLOVER_DESKTOP_CWD)",
    )
    gui_parser.add_argument(
        "--skip-build",
        action="store_true",
    ...
    gui_parser.add_argument(
        "--identity",
        default="Clover Local Signing",
        help="Certificate name to create/use for --setup-tcc-identity (default: Clover Local Signing)",
    )
