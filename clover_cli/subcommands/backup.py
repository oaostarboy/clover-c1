"""``clover backup`` subcommand parser.
...
def build_backup_parser(subparsers, *, cmd_backup: Callable) -> None:
    ...
    backup_parser = subparsers.add_parser(
        "backup",
        help="Back up Clover home directory to a zip file",
        description="Create a zip archive of your entire Clover configuration, "
        "skills, sessions, and data (excludes the clover-c1 codebase). "
        "Use --quick for a fast snapshot of just critical state files.",
    )
    backup_parser.add_argument(
        "-o",
        "--output",
        help="Output path for the zip file (default: ~/clover-backup-<timestamp>.zip)",
    )
    backup_parser.add_argument(
        "-q",
        "--quick",
