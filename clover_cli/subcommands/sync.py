"""``clover sync`` subcommand parser — Skill Sync.

Cloned from ``clover_cli/subcommands/cron.py`` — same injected-handler shape
...
Skill Sync covers two surfaces, both under this one command for launch:
  ...
  Personal — your own skills, across your own devices:
    clover sync status                 show gate/opt-in/head state
    clover sync pull                   pull and materialize opted-in skills
    clover sync push                   push opted-in skills
    clover sync now                    reconcile: pull then push
    clover sync enable <skill>         opt a skill into sync
    clover sync disable <skill>        opt a skill out of sync
    clover sync device [--name]        show or set this device's label

  Organisation — skills shared with your team:
    clover sync propose <skill>        share a skill with your organisation

Sync is INERT unless the resolved Clover token carries the access-gate claim
...
def build_sync_parser(subparsers, *, cmd_sync: Callable) -> None:
    ...
    sync_parser = subparsers.add_parser(
        ...
        epilog=(
            "Examples:\n"
            "  clover sync status            what is synced, and from where\n"
            "  clover sync enable my-skill   include a skill in your sync\n"
            "  clover sync now               pull, then push\n"
            "  clover sync propose my-skill  share a skill with your team\n"
        ),
