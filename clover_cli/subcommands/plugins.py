"""``clover plugins`` subcommand parser.
...
def build_plugins_parser(subparsers, *, cmd_plugins: Callable) -> None:
    ...
    plugins_parser = subparsers.add_parser(
        ...
        description=(
            "Install, update, remove, list, or validate native Clover plugins "
    ...
    plugins_install.add_argument(
        "identifier",
        help=(
            "Git URL, owner/repo shorthand (e.g. anpicasso/clover-plugin-chrome-profiles), "
            "or a bare plugin name resolved through the community index "
            "(see `clover plugins search`)"
        ),
    )
    plugins_install.add_argument(
        "--force",
    ...
    _install_enable_group.add_argument(
        "--no-enable",
        action="store_true",
        help="Install disabled (skip confirmation prompt); enable later with `clover plugins enable <name>`",
    )
    ...
    plugins_pack = plugins_subparsers.add_parser(
        "pack",
        help="Declarative, shareable plugin sets (clover-pack.yaml)",
        description=(
    ...
    pack_install.add_argument(
        "source", help="Path to a clover-pack.yaml file, or an https:// URL"
    )
    pack_install.add_argument(
        "--force",
        "-f",
