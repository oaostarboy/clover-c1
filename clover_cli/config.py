"""
Configuration management for Clover Cognition.

Config files are stored in ~/.clover/ for easy access:
- ~/.clover/config.yaml  - All settings (model, toolsets, terminal, etc.)
- ~/.clover/.env         - API keys and secrets

This module provides:
- clover config          - Show current configuration
- clover config edit     - Open config in editor
- clover config get      - Print a resolved configuration value
- clover config set      - Set a specific value
- clover config unset    - Remove a user configuration value
- clover config wizard   - Re-run setup wizard
"""
...
from clover_cli.route_identity import normalize_route_base_url
...
def _backup_corrupt_config(config_path: Path) -> Optional[Path]:
    ...
    they re-run the setup wizard or ``clover config set`` (which rewrites
    ...
    ``config.yaml`` in place: clover never silently mutates the user's config,
...
def _warn_config_parse_failure(
    ...
    A YAML parse error in ``~/.clover/config.yaml`` causes ``load_config()``
    ...
    ``agent.log`` / ``errors.log`` at WARNING level so ``clover logs``
    ...
    if fallback == "last-known-good":
        ...
        msg = (
            ...
            f"Fix the YAML (clover config edit) and retry."
        )
    else:
        msg = (
            f"Failed to parse {config_path}: {exc}. "
    ...
    logger.warning(msg)
    try:
        sys.stderr.write(f"⚠️  clover config: {msg}\n")
        sys.stderr.flush()
    except Exception:
        pass
...
#   the next ``subprocess.run([...])`` Clover makes loads attacker code
#   before main().
# * ``PYTHONPATH`` / ``PYTHONHOME`` / ``PYTHONSTARTUP`` /
#   ``PYTHONUSERBASE`` — Python interpreter init. Clover itself starts
#   from one of these on every restart.
# * ``NODE_OPTIONS`` / ``NODE_PATH`` — Node interpreter; affects npm,
#   ``clover update``, the TUI build.
...
#   on every plugin install / ``clover update``.
...
# * ``CLOVER_HOME`` / ``CLOVER_PROFILE`` / ``CLOVER_CONFIG`` /
#   ``CLOVER_ENV`` — Clover runtime location flags. Writing these into
...
#
# IMPORTANT: ``CLOVER_*`` overall is NOT blocked. Many legitimate
# integration credentials follow that prefix (CLOVER_LANGFUSE_PUBLIC_KEY,
# CLOVER_SPOTIFY_CLIENT_ID, ...). The
...
_ENV_VAR_NAME_DENYLIST: frozenset[str] = frozenset({
    ...
    # Clover runtime location — never via dashboard env writer.
    # NOT a CLOVER_* blanket: integration credentials (CLOVER_GEMINI_*,
    # CLOVER_LANGFUSE_*, CLOVER_SPOTIFY_*, ...) ARE allowed.
    "CLOVER_HOME", "CLOVER_PROFILE", "CLOVER_CONFIG", "CLOVER_ENV",
    "CLOVER_CONFIG_PATH", "CLOVER_ENV_PATH",
    ...
    "CLOVER_COPILOT_ACP_COMMAND", "CLOVER_COPILOT_ACP_ARGS",
    # Clover security policy / approval-routing context. These remain available
    ...
    "CLOVER_YOLO_MODE", "CLOVER_ACCEPT_HOOKS", "CLOVER_REDACT_SECRETS",
    "CLOVER_INTERACTIVE", "CLOVER_EXEC_ASK", "CLOVER_GATEWAY_SESSION",
    "CLOVER_CRON_SESSION", "CLOVER_SINGLE_QUERY_SESSION",
    "CLOVER_SESSION_KEY", "CLOVER_SESSION_PLATFORM",
})
...
def _reject_denylisted_env_var(key: str) -> None:
    ...
    if _env_var_policy_name(key) in _ENV_VAR_NAME_DENYLIST:
        raise ValueError(
            ...
            "PYTHONPATH, PATH, EDITOR, ...) or Clover runtime location "
            "and security policy (CLOVER_HOME, CLOVER_YOLO_MODE, ...) "
...
_EXTRA_ENV_KEYS = frozenset({
    ...
    # CLOVER_TOOL_PROGRESS_MODE is deprecated (replaced by display.tool_progress
    ...
    # Activation is via plugins.enabled (opt-in through `clover plugins enable
    # observability/langfuse` or `clover tools → Langfuse`); credentials gate
    # the plugin at runtime.
    "CLOVER_LANGFUSE_ENV",
    "CLOVER_LANGFUSE_RELEASE",
    "CLOVER_LANGFUSE_SAMPLE_RATE",
    "CLOVER_LANGFUSE_MAX_CHARS",
    "CLOVER_LANGFUSE_CAPTURE",
    "CLOVER_LANGFUSE_DEBUG",
    "LANGFUSE_PUBLIC_KEY",
    "LANGFUSE_SECRET_KEY",
    "LANGFUSE_BASE_URL",
    # ACP (Agent Client Protocol) keys — profile-isolable so different
    # profiles can use different ACP backends without cross-leak.
    "CLOVER_ACP_AUTH_METHOD",
    "CLOVER_ACP_AUTO_APPROVE",
    "CLOVER_COPILOT_ACP_COMMAND",
    "CLOVER_COPILOT_ACP_ARGS",
    "COPILOT_CLI_PATH",
...
from clover_cli.default_soul import DEFAULT_SOUL_MD, is_legacy_template_soul
...
# from `nix run` / `nix profile install` (which don't set CLOVER_MANAGED).
...
def get_managed_system() -> Optional[str]:
    ...
    raw = os.getenv("CLOVER_MANAGED", "").strip()
    marker = None
    if raw:
        marker = raw.lower()
    else:
        managed_marker = get_clover_home() / ".managed"
        # An interactive shell reads the marker, because it does not see the
        # CLOVER_MANAGED variable of the service. A marker with content
...
def is_managed() -> bool:
    """Check if Clover is running in package-manager-managed mode.

    Two signals: the CLOVER_MANAGED env var (set by the systemd service),
    or a .managed marker file in CLOVER_HOME (set by the NixOS activation
...
_NIX_UPDATE_MSG = (
    "Update Clover through the Nix source that installed it "
...
def _install_method_project_root(project_root: Optional[Path] = None) -> Path:
    ...
    This is the parent of ``clover_cli/`` — i.e. the git checkout for source
    installs, ``/opt/clover`` inside the published image. It is a property of
    the running interpreter, NOT of ``$CLOVER_HOME``, which is why a
...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    """Detect how Clover was installed: 'apt', 'docker', 'nix', 'nixos',
    ...
    2. Legacy home-scoped stamp ``$CLOVER_HOME/.install_method`` — read for
       backward compatibility, but a ``docker`` value is IGNORED when we are
       not actually running inside a container (see below).
    3. CLOVER_MANAGED env / .managed marker (NixOS managed mode)
    4. /nix/store/ path detection -> 'nix' (nix run / nix profile install)
    5. .git directory presence -> 'git'
    6. Fallback -> 'unknown'

    Why the stamp is code-scoped, not home-scoped (issue: shared ``~/.clover``)
    --------------------------------------------------------------------------
    The install method describes *the binary that is running*, but
    ``$CLOVER_HOME`` is a shared DATA directory — the Docker docs deliberately
    bind-mount it (``~/.clover:/opt/data``) so config/sessions/memory persist
    and can be shared with a host-side Desktop/CLI install. When a
    containerised gateway and a host install share one ``$CLOVER_HOME``, a
    ...
    ``docker`` and ``clover update`` refuses to run ("doesn't apply inside the
    ...
    # 1. Code-scoped stamp — authoritative, immune to shared $CLOVER_HOME.
    try:
    ...
    #    host install whose shared $CLOVER_HOME was stamped by a co-located
    #    container, and honouring it wrongly blocks ``clover update``.
    try:
        method = (
            (get_clover_home() / ".install_method")
    ...
    # detect Nix installs that don't set CLOVER_MANAGED (e.g. ``nix run``,
...
def _running_in_container() -> bool:
    """Thin wrapper around ``clover_constants.is_container`` (import-safe)."""
    try:
...
def stamp_install_method(method: str, project_root: Optional[Path] = None) -> None:
    ...
    not in ``$CLOVER_HOME``, so that two installs sharing one data directory
    ...
    ``/opt/clover`` in the published image, which instead bakes the stamp at
...
def recommended_update_command_for_method(method: str) -> str:
    ...
    if method == "apt":
        ...
        return "pkg upgrade clover-c1"
    return "clover update"
...
# Long-form text for ``clover update`` / ``--check`` when running inside the
# Docker image.  Surfaced by ``cmd_update`` and ``_cmd_update_check`` in
# clover_cli/main.py; lives here so the wording stays consistent and we
...
#     — that script installs a *new* host-side Clover, it doesn't update
...
def format_docker_update_message() -> str:
    """Return the user-facing message for ``clover update`` inside Docker.
...
def format_managed_message(action: str = "modify this Clover installation") -> str:
    """Build a user-facing error for managed installs."""
    managed_system = get_managed_system() or "a package manager"
    return (
        f"Cannot {action}: this Clover installation is managed by {managed_system}.\n"
        "Use your package manager to upgrade or reinstall Clover."
    )
...
def get_container_exec_info() -> Optional[dict]:
    """Read container mode metadata from CLOVER_HOME/.container-mode.

    Returns a dict with keys: backend, container_name, exec_user, clover_bin
    or None if container mode is not active, we're already inside the
    container, or CLOVER_DEV=1 is set.

    The .container-mode file is written by the NixOS activation script when
    container.enable = true. It tells the host CLI to exec into the container
    instead of running locally.
    """
    if os.environ.get("CLOVER_DEV") == "1":
        return None
    ...
    container_mode_file = get_clover_home() / ".container-mode"
    ...
    container_name = info.get("container_name", "clover-c1")
    exec_user = info.get("exec_user", "clover")
    clover_bin = info.get("clover_bin", "/data/current-package/bin/clover")
...
# Re-export from clover_constants — canonical definition lives there.
from clover_constants import get_clover_home, get_process_clover_home  # noqa: F811,E402
from utils import atomic_replace, fast_safe_load

def get_config_path() -> Path:
    """Get the main config file path."""
    return get_clover_home() / "config.yaml"

def get_env_path() -> Path:
    """Get the .env file path (for API keys)."""
    return get_clover_home() / ".env"

def get_project_root() -> Path:
    """Get the project installation directory."""
    return Path(__file__).parent.parent.resolve()

def _resolve_clover_uid_gid() -> tuple[Optional[int], Optional[int]]:
    """Read the CLOVER_UID / CLOVER_GID env vars set by Docker deployments.

    Docker containers running Clover commonly set these to map the in-container
    user to a host user so volume-mounted state files end up with the right
    ownership. The entrypoint chowns the top-level CLOVER_HOME once, but
    subdirectories created at runtime by ``ensure_clover_home()`` (especially
    ...
    if sys.platform == "win32":
        return None, None
    uid_str = os.environ.get("CLOVER_UID", "").strip()
    gid_str = os.environ.get("CLOVER_GID", "").strip()
    try:
...
def _chown_to_clover_uid(path) -> None:
    """Chown ``path`` to ``CLOVER_UID:CLOVER_GID`` if those env vars are set.
    ...
    directories created by :func:`ensure_clover_home` on Docker deployments.
    See #34107.
    """
    uid, gid = _resolve_clover_uid_gid()
...
def _secure_dir(path):
    ...
    permissions (0750) so interactive users in the clover group can
    share state with the gateway service.

    The mode can be overridden via the CLOVER_HOME_MODE environment variable
    (e.g. CLOVER_HOME_MODE=0701) for deployments where a web server (nginx,
    caddy, etc.) needs to traverse CLOVER_HOME to reach a served subdirectory.
    The execute-only bit on a directory permits cd-through without exposing
    directory listings.

    Also applies ``CLOVER_UID``/``CLOVER_GID``-based ownership when those env
    ...
    if is_managed():
        return
    try:
        mode_str = os.environ.get("CLOVER_HOME_MODE", "").strip()
    ...
    try:
        ...
        pass
    _chown_to_clover_uid(path)


def _is_container() -> bool:
    """Detect if we're running inside a Docker/Podman/LXC container.

    When Clover runs in a container with volume-mounted config files, forcing
    ...
    """
    # Explicit opt-out
    if os.environ.get("CLOVER_CONTAINER") or os.environ.get("CLOVER_SKIP_CHMOD"):
        return True
...
def _secure_file(path):
    ...
    permissions.  Set CLOVER_SKIP_CHMOD=1 to force-skip on other systems.
    """
...
def _ensure_default_soul_md(home: Path) -> None:
    """Seed a default SOUL.md into CLOVER_HOME, upgrading legacy empty templates.
...
# ensure_clover_home(). Only successful passes are recorded, so a raised
# managed-mode/missing-profile error keeps re-checking on later loads.
_CLOVER_HOME_ENSURED: set = set()


def ensure_clover_home():
    """Ensure ~/.clover directory structure exists with secure permissions.
    ...
    After the first successful pass for a given ``CLOVER_HOME`` we only re-run
    ...
    ``get_clover_home()`` and therefore re-run for the new path.
    """
    home = get_clover_home()
    key = str(home)

    # Named profiles must be created explicitly (e.g. ``clover profile create``).
    # Check tombstones before the memo so a stale empty shell cannot skip
    # the deleted-profile guard.
    from clover_constants import assert_named_profile_home_live
    assert_named_profile_home_live(home)
    if key in _CLOVER_HOME_ENSURED and home.is_dir():
        return
    if is_managed():
        old_umask = os.umask(0o007)
        try:
            _ensure_clover_home_managed(home)
        finally:
            os.umask(old_umask)
    else:
    ...
    _CLOVER_HOME_ENSURED.add(key)


def _ensure_clover_home_managed(home: Path):
    """Managed-mode variant: verify dirs exist (activation creates them), seed SOUL.md."""
    if not home.is_dir():
        raise RuntimeError(
            f"CLOVER_HOME {home} does not exist."
        )
...
from clover_cli.config_defaults import DEFAULT_CONFIG, OPTIONAL_ENV_VARS  # noqa: F401
...
def _is_env_config_key(key: str) -> bool:
    """Return whether `clover config set` routes this key to .env."""
    if "." in key:
        return False
    key_upper = key.upper()
    api_keys = [
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    Scans all enabled skills for ``metadata.clover.config`` entries, then checks
    ...
    try:
        ...
        # should never break `clover update`.  Skill-config prompting is a
...
def _normalize_custom_provider_entry(
    ...
    *,
    ...
    _KNOWN_KEYS = {
        ...
        # unused here, but Clover' own config writer has historically emitted it
    ...
    # Clover (``_save_discovered_models_to_config``), not hand-curated.
    # Older Clover versions wrote an in-mapping ``__discovered_model_catalog__``
    ...
    if isinstance(models, dict) and models:
        ...
        # Hand-edited configs (and older Clover versions) may write
...
def get_custom_provider_context_length(
    ...
    used by:
      ...
      * ``clover_cli.model_switch.resolve_display_context_length`` (``/model`` confirmation display)
...
_EXTRA_KNOWN_ROOT_KEYS = {
    ...
    "plugins",           # plugin enable/disable lists (clover_cli/plugins_cmd.py)
    "smart_model_routing",   # written by the setup wizard (clover_cli/setup.py)
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if config is None:
        try:
            ...
            return [ConfigIssue("error", "Could not load config.yaml", "Run 'clover setup' to create a valid config")]
    ...
    if cp and not model_cfg:
        issues.append(ConfigIssue(
            "warning",
            "custom_providers defined but no 'model' section — Clover won't know which provider to use",
            "Add a model section:\n"
            "  model:\n"
            "    provider: custom\n"
    ...
    # scalars are bridged into os.environ (gateway/run.py, clover send) so
...
def print_config_warnings(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    lines.append("  \033[2mRun 'clover doctor' for fix suggestions.\033[0m")
    sys.stderr.write("\n".join(lines) + "\n\n")
...
def _persist_migration(config: Dict[str, Any]) -> None:
    ...
    DEFAULT_CONFIG dumps (the "clover update / clover -p blows up my config"
    reports).
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    from clover_cli.config_migrations import (
    ...
    if floor_refused:
        ...
        sys.stderr.write(f"⚠ clover config: {msg}\n")
        if not quiet:
            print(f"  ⚠ {msg}")
    else:
        # ── Versioned migration ladder (table-driven) ──
        # The per-version steps live in clover_cli.config_migrations as a
    ...
    if isinstance(raw_mcp_servers, dict):
        try:
            from clover_cli.mcp_security import validate_mcp_server_entry as _validate_mcp_server_entry
        except Exception:
    ...
    if new_var_names and interactive and not quiet:
        ...
        if new_and_unset:
            ...
            if answer in {"y", "yes"}:
                ...
                print("  Set later with: clover config set <key> <value>")
    ...
    # `clover update`, but only the version bump is persisted.
    ...
    # metadata.clover.config in their SKILL.md frontmatter.
    ...
    * ``${env:VAR}`` — Cursor-style SecretRef, same resolution after the
    ...
    if inner.startswith("env:"):
        ...
        if val is not None:
            return val
        logger.warning(
            "Config ref %r: %s is not set (check ~/.clover/.env); "
...
def _env_ref_snapshot(obj, snapshot=None):
    ...
    ``load_clover_dotenv()`` populated the process env, or an env var
...
def split_model_config_default(raw_default: Any) -> tuple[str, str]:
    ...
    together through ``CloverCLI`` construction: the model becomes a plain
...
def _normalize_root_model_keys(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    intuitive name OpenAI-SDK / LiteLLM users reach for, and ``clover config set``
    ...
    backends) — while display paths (``clover status``/``dump``) read ``name``
...
#   - survives the ``str() → int()`` round-trip through the CLOVER_MAX_ITERATIONS
...
def cfg_get(cfg: Optional[Dict[str, Any]], *keys: str, default: Any = None) -> Any:
    ...
    ubiquitous ``cfg_path = _clover_home / "config.yaml"`` local variable
...
# Back-compat alias — canonical set lives in clover_cli.personality.
from clover_cli.personality import NEUTRAL_PERSONALITY_NAMES as _NEUTRAL_PERSONALITY_NAMES  # noqa: F401


def _prompt_text(value: Any) -> str:
    """Normalize config prompt values from YAML before handing them to AIAgent.

    Delegates to :mod:`clover_cli.personality` — the single owner of
    personality/overlay semantics. Kept as a re-export for existing importers.
    """
    from clover_cli.personality import prompt_text
...
def render_personality_prompt(value: Any) -> str:
    ...
    from clover_cli.personality import render_personality_prompt as _render
...
def resolve_ephemeral_system_prompt_from_config(cfg: Optional[Dict[str, Any]]) -> str:
    ...
    still prefer ``CLOVER_EPHEMERAL_SYSTEM_PROMPT`` when that env var is set.

    Delegates to :mod:`clover_cli.personality` (single owner).
    """
    from clover_cli.personality import resolve_ephemeral_system_prompt

    return resolve_ephemeral_system_prompt(cfg)


def read_raw_config() -> Dict[str, Any]:
    """Read ~/.clover/config.yaml as-is, without merging defaults or migrating.
...
def read_user_config_raw(config_path: Optional[Path] = None) -> Dict[str, Any]:
    ...
    ``_clover_home``, tui profile override, multi-profile probes).
    """
...
def load_config() -> Dict[str, Any]:
    """Load configuration from ~/.clover/config.yaml.
    ...
    (which change ``CLOVER_HOME`` and therefore ``get_config_path()``)
    don't collide.
...
def _is_ssh_remote_tilde_cwd(backend: str, cwd: str) -> bool:
    ...
    Expanding ``~`` on the Clover host rewrites it to the host or container
...
def _load_config_impl(*, want_deepcopy: bool) -> Dict[str, Any]:
    with _CONFIG_LOCK:
        ...
        # cache signature so editing /etc/clover/config.yaml invalidates the
        ...
        if cached is not None and cache_sig is not None and cached[:4] == cache_sig:
            ...
            # Without this, a load_config() that ran before load_clover_dotenv()
...
def _parse_env_value(raw_value: str) -> str:
    """Parse the small .env value subset Clover writes itself."""
...
def load_env() -> Dict[str, str]:
    """Load environment variables from ~/.clover/.env.
    ...
    interactive menu render (`clover tools`, `clover setup`, status
    panels). Sanitisation is O(lines), so re-parsing the
    same file on every call was burning ~300ms of CPU per `clover tools`
...
def sanitize_env_file() -> int:
    """Read, sanitize, and rewrite ~/.clover/.env in place.
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.clover/.env."""
    if is_managed():
...
def custom_endpoint_key_env(identity: str) -> str:
    ...
    - The fixed ``CLOVER_CUSTOM_`` prefix keeps the result a valid POSIX name
    ...
    return f"CLOVER_CUSTOM_{slug}_API_KEY" if slug else "CLOVER_CUSTOM_API_KEY"


def remove_env_value(key: str) -> bool:
    """Remove a key from ~/.clover/.env and os.environ.
...
def reload_env() -> int:
    """Re-read ~/.clover/.env into os.environ. Returns count of vars updated.

    Adds/updates vars that changed and removes vars that were deleted from
    the .env file (but only vars known to Clover — OPTIONAL_ENV_VARS and
    ...
    for key, value in env_vars.items():
        if os.environ.get(key) != value:
            os.environ[key] = value
            count += 1
    # Remove known Clover vars that are no longer in .env
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ``os.environ`` or ``~/.clover/.env``, scope-aware.
...
def get_env_value_prefer_dotenv(key: str) -> Optional[str]:
    """Resolve a credential env value, preferring ``~/.clover/.env`` over ``os.environ``.

    Used for Clover-managed credentials where a deliberate edit to ``.env``
...
def show_config():
    ...
    from clover_cli.auth import get_anthropic_key
    ...
    # Warn on stale CLOVER_MAX_ITERATIONS ghost in .env that disagrees with
    ...
    try:
        _env_ghost = load_env().get("CLOVER_MAX_ITERATIONS")
        if _env_ghost is not None and str(_env_ghost).strip() != str(_cfg_max_turns).strip():
            print(color(
                f"                ⚠ .env has stale CLOVER_MAX_ITERATIONS={_env_ghost} "
                f"(run 'clover doctor --fix' to remove)",
                Colors.YELLOW,
            ))
    except Exception:
        pass
    ...
    try:
        from clover_cli.personality import active_personality_name
    ...
    print(color("  clover config edit     # Edit config file", Colors.DIM))
    print(color("  clover config set <key> <value>", Colors.DIM))
    print(color("  clover setup           # Run setup wizard", Colors.DIM))
    print()
...
def resolve_cron_model_drift_defaults(
    ...
    *,
    ...
    wins ``CLOVER_MODEL``; the environment is only a fallback. Per-job and cron
    ...
    provider = ""
    model = _model_assignment_text(env.get("CLOVER_MODEL", ""))
...
def warn_unpinned_cron_jobs_after_model_config_change(
    ...
    print(
        ...
        "changed model/provider. Inspect with `clover cron list`, then pin the "
        "intended values with `clover cron edit <job_id> --provider <provider> "
        "--model <model>`."
    )
...
_SCHEMA_DEFINED_DICT_KEYS = frozenset({
    ...
    # (clover_cli/plugins_cmd.py, clover_cli/plugin_index.py). Absent from
    # DEFAULT_CONFIG (written only when used), so listed here for
    # `clover config set plugins.index_url ...` validation.
    "plugins",
})
...
def _known_top_level_keys() -> set[str]:
    ...
    to decide whether a ``clover config set`` invocation is targeting a
    known shape.
    """
...
def _suggest_closest_key(key: str, candidates: set[str], cutoff: float = 0.6) -> Optional[str]:
    ...
    similar enough to ``key``, else None.  Used by ``clover config set``
...
def set_config_value(key: str, value: str, force: bool = False):
    ...
    Args:
        ...
        force: When True, skip the unknown-key warning — useful for scripted
            ...
            CLI exposes this via ``clover config set --force``.
    """
    if is_managed():
    ...
    if not isinstance(_default_value_for_key(key), str):
        ...
        if _lower in {'true', 'yes', 'on'}:
            ...
            #   clover config set platform_toolsets.line '["file","web"]'
            # or a multi-line YAML block:
            #   clover config set custom_providers '- name: foo
            #     base_url: https://...'
    ...
    # ``clover config set model.provider openai`` doesn't silently
    ...
    # (e.g. ``clover config set model gpt-5.6-sol`` when model already
    ...
    if "." not in key:
        ...
        if isinstance(_existing, dict):
            if key == "model":
                ...
                print(
                    f"    clover config set {key}.<sub-key> <value>",
                    file=sys.stderr,
                )
                print(
                ...
                print(
                    f"    clover config set --force {key} {value!r}",
                    file=sys.stderr,
                )
                sys.exit(1)
    _set_nested(user_config, key, value)
    # Normalize the api_base → base_url alias at set-time too (issue #8919),
    # so a fresh `clover config set model.api_base ...` lands on the canonical
    ...
    if key == "display.skin" and isinstance(value, str) and value:
        try:
            skin_file = get_clover_home() / "skins" / f"{value}.yaml"
            if skin_file.exists():
                skin_file.touch()
        except Exception:
    ...
    # — e.g. `clover config set model.api_key cfut_...` routes to config.yaml
...
def unset_config_value(key: str):
    ...
    if _is_env_config_key(key):
        ...
        # model-cache rows too, so `clover config unset <KEY>` fully removes
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if not key:
            print("Usage: clover config get <key> [--json]")
            print()
            print("Examples:")
            print("  clover config get model")
            print("  clover config get terminal.backend")
            print("  clover config get skills.config --json")
            sys.exit(1)
        ...
        if not key or value is None:
            print("Usage: clover config set [--force] <key> <value>")
            print()
            print("Examples:")
            print("  clover config set model anthropic/claude-sonnet-4")
            print("  clover config set terminal.backend docker")
            print("  clover config set OPENROUTER_API_KEY sk-or-...")
            print()
        ...
        key = getattr(args, 'key', None)
        if not key:
            print("Usage: clover config unset <key>")
            print()
            print("Examples:")
            print("  clover config unset model")
            print("  clover config unset terminal.backend")
            print("  clover config unset OPENROUTER_API_KEY")
            sys.exit(1)
        try:
        ...
        if missing_config:
            ...
            print("    Run 'clover config migrate' to add them")
        ...
        print("  clover config           Show current configuration")
        print("  clover config edit      Open config in editor")
        print("  clover config get <key>          Print a resolved config value")
        print("  clover config set <key> <value>   Set a config value")
        print("  clover config unset <key>        Remove a config value")
        print("  clover config check     Check for missing/outdated config")
        print("  clover config migrate   Update config with new options")
        print("  clover config path      Show config file path")
        print("  clover config env-path  Show .env file path")
        sys.exit(1)
...
# ``_inject_profile_env_vars`` surfaces them in ``clover config`` UI so users
