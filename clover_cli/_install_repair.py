...
- ``clover_cli._early_recovery.recover_if_needed`` — stdlib-only, runs BEFORE
  ``clover_cli.main``'s third-party imports, so it can complete a pending
  update while no native extension is mapped yet (#83569).
- ``clover_cli.main._recover_core_update_marker_locked`` — the historical
...
the corrupted-venv state it exists to repair. ``clover_cli.main`` imports
``managed_uv``, ``clover_constants``, and friends only in its late path; the
...
:func:`clover_cli._early_recovery._find_uv_binary` lookup and falls back to
...
from clover_cli import _early_recovery as _er
...
def _is_termux_env(env: dict | None = None) -> bool:
    """Stdlib Termux probe (clover_cli.main's version lives behind imports)."""
...
def _stdout_to_stderr():
    ...
    ``clover acp`` speaks JSON-RPC on stdout; an inherited-fd install child
...
def _venv_scripts_dir(root: Path) -> Path | None:
    ...
    # clover_constants is stdlib-only, so the canonical layout helpers are safe
...
#: managed binary dir (the default Clover root's ``bin``, next to uv.exe)
#: on the user PATH. Keep in lockstep with the launcher list in
#: scripts/install.ps1.
_WINDOWS_BIN_LAUNCHERS = ("clover", "clover-acp")


def _venv_is_relocatable(venv_dir: Path) -> bool:
    """True when the venv's pyvenv.cfg declares ``relocatable = true``.

    uv writes the flag; ``clover_cli.managed_uv`` builds its replacement
...
def ensure_windows_bin_launchers(
    ...
    *,
    ...
    """Re-stage the Windows ``clover`` launchers when they vanish.

    On Windows, ``clover`` resolves through launchers derived from the venv
    ...
    the managed binary dir — the default Clover root's ``bin``
    (``%LOCALAPPDATA%\\clover\\bin``, next to the managed uv) — which lives
    OUTSIDE the git checkout so no git operation can ever touch it. It is
    a per-machine dir shared by every profile: ``get_clover_home()`` would
    point inside ``profiles\\<name>`` under ``clover -p``, so the anchor
    here is :func:`clover_constants.get_default_clover_root`.

    Earlier installer versions staged them at ``<checkout>\\bin`` instead —
    inside the git working tree — where ``clover update``'s pre-update
    ...
    nothing restored them and ``clover`` stopped resolving in every new
    ...
    - canonical managed binary dir: only when *root* is the managed clone
      (``root.parent == get_default_clover_root()``), so source checkouts
    ...
    *windows* and *user_path_entries* are injectable for tests, same pattern
    as ``clover_constants.venv_bin_dir``.
    """
    if windows is None:
        windows = _is_windows()
    if not windows:
    ...
    # Per-machine anchor: the DEFAULT Clover root, not get_clover_home() —
    # under ``clover -p <name>`` that returns ``profiles\\<name>``, which
    ...
    try:
        home = Path(get_default_clover_root())
    except Exception:
        return []
    ...
    # every clover_cli.main process start (right after the profile
    ...
    if restored:
        ...
        with contextlib.suppress(OSError, ValueError):
            print(
                "  ✓ Restored clover launcher(s): " + ", ".join(restored),
                file=sys.stderr,
            )
    return restored
...
def _read_user_path_raw() -> tuple[list[str], int]:
    ...
    (same discipline as ``clover_cli.uninstall``). Only called on Windows.
    """
    import winreg
...
def migrate_windows_bin_path(
    ...
    *,
    ...
    """One-time PATH migration to the ``CLOVER_HOME\\bin`` launcher layout.

    Runs from the ``clover update`` tail (and mirrors what install.ps1's
    ...
    *read_user_path*/*write_user_path* are injectable for tests.
    ...
    from clover_constants import get_default_clover_root, venv_bin_dir
    ...
    if kept != entries:
        ...
        with contextlib.suppress(OSError, ValueError):
            print(
                f"  ✓ clover launchers now resolve from {home_bin} "
...
def _quarantine_running_clover_exe(
    ...
    """Rename live clover*.exe shims aside so the installer can rewrite them.
...
def _restore_quarantined_exes(moved: list[tuple[Path, Path]]) -> None:
    ...
    ``clover acp`` speaks JSON-RPC on stdout.
    """
...
def _run_install_cmd(cmd: list[str], *, env: dict | None, root: Path) -> None:
    ...
    failed: list[str] = []
    moved = (
        _quarantine_running_clover_exe(scripts_dir, failed_out=failed)
        if scripts_dir
        else []
    )
    if failed:
    ...
    try:
        ...
        # the quarantined shims renamed aside and `clover` gone from PATH
...
def run_core_install(root: Path) -> None:
    ...
    - quarantine live ``clover*.exe`` shims on Windows so they can be replaced
