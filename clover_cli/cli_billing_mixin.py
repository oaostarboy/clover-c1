...
This module hosts the Clover billing/subscription methods lifted out of
``cli.py``'s ``CloverCLI`` class. ``CloverCLI`` inherits
...
Import discipline mirrors ``clover_cli.cli_commands_mixin``:
...
class CLIBillingMixin:
    ...
    def _print_usage_cta(self) -> None:
        ...
        usage read with the same nudge. Only called when a Clover account is logged
        in (the balance block printed), since both commands are Clover-account only.
        """
    ...
    def _show_subscription(self):
        """`/subscription` (alias `/upgrade`) — view the Clover plan + browser hand-off.
    ...
    def _open_url_in_browser(self, url: str) -> bool:
        ...
        use (``clover_cli.auth``): ``webbrowser.open()`` returns ``True`` even when
    ...
    def _billing_overview(self, state):
        ...
        if not state.cli_billing_enabled:
            ...
            self._billing_portal_hint(
                state,
                reason="A billing admin can turn it on from the portal's Clover Cognition page to add funds here.",
            )
            return
    ...
    def _billing_require_admin(self, state) -> bool:
        ...
        if not state.cli_billing_enabled:
            ...
            self._billing_portal_hint(
                state,
                reason="A billing admin can turn it on from the portal's Clover Cognition page before adding funds.",
            )
            return False
        return True
    ...
    def _billing_confirm_and_charge(self, state, amount):
        ...
        print(f"  {'─' * 41}")
        _consent = (
            "By confirming, you allow Clover Cognition to charge your card."
        )
    ...
    def _billing_render_charge_error(self, state, exc):
        ...
        if isinstance(exc, BillingRemoteSpendingRevoked) or code == "remote_spending_revoked":
            ...
            print("  Remote spending is off for this account — a billing admin can turn it on from the portal's Clover Cognition page.")
    ...
    def _billing_handle_scope_required(self, state, *, amount=None, idempotency_key=None):
        ...
        if not (fresh.logged_in and fresh.cli_billing_enabled):
            print("  Remote Spending is allowed for this terminal, but it's still off for this org. A billing admin can turn it on from the portal's Clover Cognition page, then run /topup again.")
    ...
    def _billing_auto_reload_flow(self, state):
        ...
        print()
        _ar_consent = (
            f"By confirming, you authorize Clover Cognition to charge {card.masked} "
