...
Pure logic: walks a Clover config dict, returns issues for any reference
to a retired xAI model. No I/O, no CLI dependencies — testable in isolation
and reusable from both `clover doctor` and a future `clover migrate xai`.
"""
...
class RetirementIssue:
    """A reference to a retired xAI model found in a Clover config."""
...
def find_retired_xai_refs(config: Dict[str, Any]) -> List[RetirementIssue]:
    """Walk all model slots in a Clover config and return retirement issues.
...
def apply_migration(
    ...
    from clover_cli.config import require_readable_config_before_write
    ...
    # None``, so the next `clover migrate xai` reports nothing to migrate
