...
cookies**. It cannot be a direct OAuth client of the upstream IDP (Clover Portal): the Portal ``client_id`` is per-gateway-instance
...
def register_pending(
    *,
    ...
    upstream PKCE round trip (inside the ``clover_session_pkce`` cookie), so the
