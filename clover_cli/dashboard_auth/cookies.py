...
Three cookies in play:
  - clover_session_at:   the OAuth access token
                         (HttpOnly, lifetime = token TTL, ~15 min)
  - clover_session_rt:   the OAuth refresh token
                         (HttpOnly, lifetime = 24h, ROTATING + reuse-detected)
                         Clover Portal issues a rotating refresh token for the
                         dashboard auth-code grant (Portal NAS #293 / clover
  ...
  - clover_session_pkce: short-lived PKCE state + CSRF nonce + provider
...
https://datatracker.ietf.org/doc/html/draft-west-cookie-prefixes):
  ...
  * Gated HTTPS, direct deploy (Path=/) — ``__Host-`` prefix. Binds the
    ...
    guarantee.
  * Gated HTTPS, behind a reverse-proxy prefix (Path=/clover) —
    ``__Secure-`` prefix. ``__Host-`` is disallowed when Path != "/";
    ``__Secure-`` keeps the Secure-required hardening without the
    Path constraint, and the explicit ``Path=/clover`` covers
...
setter wrote ``__Secure-clover_session_at`` would never find the value.
...
SESSION_AT_COOKIE = "clover_session_at"
SESSION_RT_COOKIE = "clover_session_rt"
...
# auth plugins are enabled (for example Basic + Clover OAuth).
SESSION_PROVIDER_COOKIE = "clover_session_provider"
PKCE_COOKIE = "clover_session_pkce"
...
SSO_ATTEMPT_COOKIE = "clover_sso_attempt"
...
def _cookie_path(prefix: str) -> str:
    ...
    Under ``X-Forwarded-Prefix: /clover`` we want ``Path=/clover`` so:
...
def set_session_cookies(
    ...
    *,
    ...
    ``refresh_token`` is written as the RT cookie when non-empty. Clover Portal
    issues a 24h rotating refresh token (clover #37247); a provider that
    ...
    ``prefix`` is the normalised X-Forwarded-Prefix value (e.g. ``/clover``)
