...
* ``clover_cli.web_server.auth_middleware`` — loopback / ``--insecure``
  mode, gates on the ephemeral ``_SESSION_TOKEN``.
* ``clover_cli.dashboard_auth.middleware.gated_auth_middleware`` —
...
liveness probe (``clover-account-service`` ``fly-provider.ts``
