...
from ``clover_constants`` or other clover_cli modules — so it can be
...
def _resolve_log_path() -> Path:
    """``$CLOVER_HOME/logs/dashboard-auth.log``.

    Uses ``clover_constants.get_clover_home()`` (a leaf module — no import
    ...
    return get_clover_home() / "logs" / "dashboard-auth.log"
