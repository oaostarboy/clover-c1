...
from clover_cli.dashboard_auth import list_session_providers
from clover_cli.dashboard_auth.audit import AuditEvent, audit_log
from clover_cli.dashboard_auth.base import (
...
from clover_cli.dashboard_auth.public_paths import PUBLIC_API_PATHS
...
def _unauth_response(request: Request, *, reason: str) -> Response:
    ...
    Under a reverse proxy with ``X-Forwarded-Prefix: /clover``, the
    ``login_url`` is prefixed (``/clover/login?next=...``) so the
    ...
    """
    from clover_cli.dashboard_auth.prefix import prefix_from_request
...
def _auto_sso_response(request: Request) -> Response | None:
    ...
    from clover_cli.dashboard_auth.cookies import detect_https
    set_sso_attempt_cookie(
...
def _safe_next_target(request: Request) -> str:
    ...
    # the SPA's own ``sessionStorage["clover.lastLocation"]`` fallback
...
async def gated_auth_middleware(
    ...
    # ``clover_session_at``. Verify it with the identical ``verify_session``
    ...
    # sends only ``clover_session_rt``. If we bailed on ``not at`` here we'd
    ...
    if at:
        ...
        # a self-hosted-OIDC session hits the `clover` provider first, which
        # tries to reach Clover Portal's JWKS; if that's unreachable it raises,
    ...
    if session is None:
        ...
        from clover_cli.dashboard_auth.cookies import clear_session_cookies
        from clover_cli.dashboard_auth.prefix import prefix_from_request
