...
supported components into records consumed by Clover' existing skill and MCP
...
def _translate_remote(config: Mapping[str, Any]) -> Dict[str, Any]:
    ...
    The returned record targets Clover' existing URL-based MCP runtime.
...
def has_enabled_agent_plugin_mcp(raw_config: Mapping[str, Any]) -> bool:
    ...
    Directory scanning belongs to :mod:`clover_cli.plugins` so startup gating
    ...
    from clover_cli.plugins import has_enabled_agent_plugin_mcp as _probe
