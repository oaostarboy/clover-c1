...
Used by ``clover profile describe <name> --auto`` and the dashboard's
...
Design notes
------------
- Mirrors the shape of ``clover_cli/kanban_specify.py``: lazy aux
...
_SYSTEM_PROMPT = """You are a profile-describer for the Clover Cognition kanban board.
...
Rules:
  ...
  - Never write "Clover Cognition profile" or other meta-narration.
    ...
    *,
    ...
    try:
        if canon == "default":
            ...
            profile_dir = Path(get_clover_home())
        else:
