"""Cross-process mutual exclusion for in-flight Clover updates.

Three different surfaces can start an update of the same install tree:

* ``clover update`` from a terminal,
* the dashboard's Update button (``POST /api/clover/update`` →
  ``_spawn_clover_action(["update"])``, detached),
* the desktop's Update button, which hands off to the Tauri
  ``clover-setup --update`` and, on its failure screen, to install-mode
...
once — so a dashboard-spawned ``clover update`` and an installer-driven
...
unchanged and remain byte-compatible with the Rust and Electron readers:
    ...
    <CLOVER_HOME>/.clover-update-in-progress   body: "<pid>\\n<started_at_unix>"
...
then spawns ``clover update`` as a child stage. Without a handoff the child
sees its own parent's live marker and refuses — the GUI update deadlocks
against itself on every attempt ("Clover is still running", retry forever).
...
* A live holder that is a *process ancestor* of ours is likewise our own
  ...
  ``clover-setup`` binary under ``~/.clover`` is only refreshed by a full
  installer run (``copy_self_to_clover_home`` deliberately no-ops during
  ...
  ancestry check those users get exit 2 ("Clover is still running") on every
  GUI update forever, with no Clover process actually running.
"""
...
MARKER_NAME = ".clover-update-in-progress"

# Set by an orchestrating updater (the Tauri `clover-setup --update` flow) to
# its own pid before spawning `clover update` as a child stage. The parent
...
HANDOFF_PID_ENV = "CLOVER_UPDATE_HANDOFF_PID"
...
def update_marker_path() -> Path:
    ...
    Uses the *process* Clover home (never the context-local profile override):
    the Rust updater resolves ``$CLOVER_HOME`` or the platform default, and the
    ...
    return get_process_clover_home() / MARKER_NAME
...
def _is_ancestor_pid(pid: int) -> bool:
    ...
    The orchestrating updater spawns ``clover update`` as a (grand)child, so a
    ...
    parent chain. This heals the fleet of staged ``clover-setup`` binaries
...
def describe_holder(holder: UpdateHolder) -> str:
    ...
    return (
        f"✗ Another Clover update is already running (PID {holder.pid}, "
...
class UpdateLock:
    ...
    def acquire(self) -> bool:
        ...
        updater spawning `clover update` as a stage): we run under ITS claim
