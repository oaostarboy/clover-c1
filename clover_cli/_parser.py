"""
Top-level argparse construction for the clover CLI.
...
# argparse runs (it sets ``CLOVER_HOME`` and strips itself from ``sys.argv``),
...
def _inherited_flag(parser, *args, **kwargs):
    """Register a flag that ``clover_cli.relaunch`` should carry over when
...
_EPILOGUE = """
Examples:
    clover                        Start interactive chat
    clover chat -q "Hello"        Single query mode
    clover --tui                  Launch the modern TUI (or set display.interface: tui)
    clover --cli                  Force the classic REPL (overrides display.interface: tui)
    clover -c                     Resume the most recent session
    clover -c "my project"        Resume a session by name (latest in lineage)
...
def build_top_level_parser():
    ...
    # Mirrors `clover chat --model ... --provider ...` semantics.
    _inherited_flag(
        parser,
        "-m",
        "--model",
        ...
        help=(
            ...
            "Applies to -z/--oneshot and --tui. Also settable via CLOVER_INFERENCE_MODEL env var."
        ),
    )
    _inherited_flag(
        parser,
        ...
        help=(
            ...
            "under model.provider — use `clover setup` or edit the file to change it."
        ),
    )
    _inherited_flag(
        parser,
    ...
    _inherited_flag(
        ...
        help=(
            ...
            "without a TTY prompt.  Equivalent to CLOVER_ACCEPT_HOOKS=1 or "
    ...
    # `--fuckit` is the primary flag; `--yolo` stays as an alias for muscle
    # memory. dest is pinned to "yolo" explicitly (argparse would otherwise
    # derive it from the FIRST long option) so every existing
    # `getattr(args, "yolo")` call site keeps working untouched.
    _inherited_flag(
        parser,
        "--fuckit",
        "--yolo",
        ...
        action="store_true",
        default=False,
        help="Bypass all dangerous command approval prompts (alias: --yolo)",
    )
    _inherited_flag(
        parser,
        "--pass-session-id",
    ...
    _inherited_flag(
        ...
        action="store_true",
        default=False,
        help="Ignore ~/.clover/config.yaml and fall back to built-in defaults (credentials in .env are still loaded)",
    )
    _inherited_flag(
        parser,
        "--ignore-rules",
    ...
    chat_parser = subparsers.add_parser(
        ...
        description="Start an interactive chat session with Clover Cognition",
    )
    ...
    # top-level parser: when the user writes `clover -m foo chat`, argparse
    ...
    # `-w/--worktree`, `--yolo`, etc. (see tests/clover_cli/
    ...
    _inherited_flag(
        ...
        help=(
            "Auto-approve any unseen shell hooks declared in config.yaml "
            "without a TTY prompt (see also CLOVER_ACCEPT_HOOKS env var and "
    ...
    _inherited_flag(
        ...
        action="store_true",
        default=argparse.SUPPRESS,
        help="Ignore ~/.clover/config.yaml and fall back to built-in defaults (credentials in .env are still loaded). Useful for isolated CI runs, reproduction, and third-party integrations.",
    )
    _inherited_flag(
        chat_parser,
        "--ignore-rules",
    ...
    _inherited_flag(
        ...
        action="store_true",
        default=argparse.SUPPRESS,
        help="Troubleshooting mode: disable ALL customizations — user config, AGENTS.md/memory injection, plugins, and MCP servers (implies --ignore-user-config and --ignore-rules). Use to isolate whether a problem comes from your setup or from Clover itself.",
    )
    chat_parser.add_argument(
        "--source",
        default=None,
