"""Helpers for Clover subscription managed-tool capabilities."""
...
from clover_cli.config import get_env_value, load_config
...
# Maps a tools_config provider's ``managed_clover_feature`` to the tool-pool
# coverage category (clover_cli.clover_account.TOOL_COVERAGE_CATEGORIES). Lets the
# `clover tools` picker scope its entitlement gate to the selected backend, so a
# free-tool-pool user is allowed image gen but denied video gen at select time —
# consistent with the per-category feature gates in get_clover_subscription_features.
...
def _selected_provider(section: object, name_key: str = "provider") -> Optional[str]:
    ...
    an in-memory section dict: ``"clover"`` for the managed selection (stored
    ``clover`` value or legacy ``use_gateway: true``), a vendor name for BYOK
    picks, or ``None`` when no selection is stored. Keeping this in lockstep
    with the runtime resolver is what stops ``clover status`` from lying.
    """
    if not isinstance(section, dict):
        return None
...
class CloverFeatureState:
    key: str
    label: str
    included_by_default: bool
    available: bool
    active: bool
    managed_by_clover: bool
...
class CloverSubscriptionFeatures:
    ...
    @property
    def web(self) -> CloverFeatureState:
        return self.features["web"]

    @property
    def image_gen(self) -> CloverFeatureState:
        return self.features["image_gen"]

    @property
    def tts(self) -> CloverFeatureState:
        return self.features["tts"]

    @property
    def stt(self) -> CloverFeatureState:
        return self.features["stt"]

    @property
    def browser(self) -> CloverFeatureState:
        return self.features["browser"]

    @property
    def video_gen(self) -> CloverFeatureState:
        return self.features["video_gen"]

    @property
    def modal(self) -> CloverFeatureState:
        return self.features["modal"]

    def items(self) -> Iterable[CloverFeatureState]:
...
def _local_stt_backend_available() -> bool:
    ...
    ``apply_clover_managed_defaults`` from flipping a working local setup
    to the managed gateway.
    """
    if get_env_value("CLOVER_LOCAL_STT_COMMAND"):
        return True
    try:
...
def get_clover_subscription_features(
    ...
    *,
    ...
    clover_auth_present = bool(account_info and account_info.logged_in)
    ...
    # requires `pip install faster-whisper`. For Clover subscribers we'd
    # rather route through the managed OpenAI audio gateway — see
    # apply_clover_managed_defaults below.
    ...
    # "clover" (stored value or legacy use_gateway: true) = managed gateway;
    ...
    web_use_gateway = web_selected == "clover"
    tts_use_gateway = tts_selected == "clover"
    stt_use_gateway = stt_selected == "clover"
    browser_use_gateway = browser_selected == "clover"
    image_use_gateway = image_selected == "clover"
    video_use_gateway = video_selected == "clover"

    # The "clover" selection is serviced by a concrete vendor implementation —
    # normalize the current-provider labels so downstream vendor checks hold.
    if web_backend == "clover" or web_use_gateway:
        web_backend = "firecrawl"
    if tts_provider == "clover" or tts_use_gateway:
        tts_provider = "openai"
    if stt_provider == "clover" or stt_use_gateway:
        stt_provider = "openai"
    if browser_provider == "clover" or browser_use_gateway:
    ...
    # Keyless Tavily is opt-in: selecting it in `clover tools` / setup writes
    ...
    try:
        ...
        local_stt_available = bool(_HAS_FASTER_WHISPER) or bool(
            get_env_value("CLOVER_LOCAL_STT_COMMAND")
        )
    except Exception:
        local_stt_available = bool(get_env_value("CLOVER_LOCAL_STT_COMMAND"))
    ...
    features = {
        "web": CloverFeatureState(
            key="web",
            label="Web tools",
            included_by_default=True,
            available=web_available,
            active=web_active,
            managed_by_clover=web_managed,
        ...
        "image_gen": CloverFeatureState(
            key="image_gen",
            label="Image generation",
            included_by_default=True,
            available=image_available,
            active=image_active,
            managed_by_clover=image_managed,
            direct_override=image_active and not image_managed,
            toolset_enabled=image_tool_enabled,
            current_provider="FAL" if (image_selected not in (None, "clover") or (image_selected is None and direct_fal)) else ("Clover Subscription" if (image_managed or image_use_gateway) else ""),
            explicit_configured=image_selected is not None or direct_fal,
        ),
        "video_gen": CloverFeatureState(
            key="video_gen",
            label="Video generation",
            included_by_default=False,
            available=video_available,
            active=video_active,
            managed_by_clover=video_managed,
            direct_override=video_active and not video_managed,
            toolset_enabled=video_tool_enabled,
            current_provider="FAL" if (video_selected not in (None, "clover") or (video_selected is None and direct_fal_video)) else ("Clover Subscription" if (video_managed or video_use_gateway) else ""),
            explicit_configured=video_selected is not None or direct_fal_video,
        ),
        "tts": CloverFeatureState(
            key="tts",
            label="OpenAI TTS",
            included_by_default=True,
            available=tts_available,
            active=tts_active,
            managed_by_clover=tts_managed,
        ...
        "stt": CloverFeatureState(
            key="stt",
            label="Speech-to-text",
            included_by_default=True,
            available=stt_available,
            active=stt_active,
            managed_by_clover=stt_managed,
        ...
        "browser": CloverFeatureState(
            key="browser",
            label="Browser automation",
            included_by_default=True,
            available=browser_available,
            active=browser_active,
            managed_by_clover=browser_managed,
        ...
        "modal": CloverFeatureState(
            key="modal",
            label="Modal execution",
            included_by_default=False,
            available=modal_available,
            active=modal_active,
            managed_by_clover=modal_managed,
...
def apply_clover_managed_defaults(
    ...
    *,
    ...
    if "tts" in selected_toolsets and not features.tts.explicit_configured and not (
        ...
        tts_cfg["provider"] = "clover"
        tts_cfg.pop("use_gateway", None)
        changed.add("tts")

    # STT: same pattern as TTS. The DEFAULT_CONFIG seed is "local"
    # (requires `pip install faster-whisper`); for Clover subscribers we
    ...
    if "browser" in selected_toolsets and not features.browser.explicit_configured and not (
        ...
        browser_cfg["cloud_provider"] = "clover"
...
# ``get_clover_subscription_features`` reads (web uses "backend", browser
...
def get_gateway_eligible_tools(
    ...
    *,
    ...
    - unconfigured: tools with no direct credentials and no explicit
      non-clover selection (easy switch, safe to pre-check)
    - has_direct: tools where the user has their own API keys
    - explicit_configured: tools with an explicit non-clover selection stored
    ...
    All lists are empty when the user is not a paid Clover subscriber or
    is not using Clover as their provider.
    """
    ...
    # Quick provider check without the heavy get_clover_subscription_features call
    model_cfg = config.get("model")
    if not isinstance(model_cfg, dict) or str(model_cfg.get("provider") or "").strip().lower() != "clover":
        return [], [], [], []

    direct = _get_gateway_direct_credentials()

    # Check which tools the user has explicitly opted into the gateway for.
    # This is distinct from managed_by_clover which fires implicitly when
    ...
    for key in _ALL_GATEWAY_KEYS:
        ...
        elif selected is not None and selected != "clover":
            # An explicit non-clover selection (e.g. a keyless local backend
...
def prompt_enable_tool_gateway(
    ...
    *,
    ...
    # already_managed, just for a non-clover vendor.
    ...
    try:
        from clover_cli.setup import prompt_checklist
    except Exception:
        return set()
    ...
    try:
        account_info = get_clover_portal_account_info(force_fresh=False)
    except Exception:
        account_info = None
    pool_only = bool(
        account_info
    ...
    source_label = "free tool pool" if pool_only else "Clover subscription"
    ...
    # pre-checked checklist re-fired on every Clover model swap.
    ...
    if pool_only:
        title = "Your free Clover tool pool — pick the tools to enable:"
    else:
        title = (
            "Your Clover subscription includes the Tool Gateway — "
    ...
    # so the next Clover model swap doesn't pre-check it again. Cancel paths
...
# ---------------------------------------------------------------------------
# Inline Clover Portal login for the Tool Gateway picker (`clover tools`)
# ---------------------------------------------------------------------------


def ensure_clover_portal_access(
    *,
    capability: str = "the Clover Tool Gateway",
    coverage_category: Optional[str] = None,
) -> bool:
    """Make sure the user is entitled to the Clover Tool Gateway, logging in if
    needed.

    Used by ``clover tools`` when a user selects a Clover-managed Tool Gateway
    backend (e.g. "Firecrawl (Clover Portal)").  Unlike ``clover model``'s Clover
    login, this:
    ...
    It only performs the Clover Portal device-code OAuth (when the user isn't
    ...
    if info is None or not info.logged_in:
        if not _run_clover_portal_login_only(capability=capability):
            return False
        try:
            info = get_clover_portal_account_info(force_fresh=True)
        except Exception:
...
def _run_clover_portal_login_only(*, capability: str) -> bool:
    """Run the Clover Portal device-code OAuth and persist credentials only.
    ...
    try:
        from clover_cli.auth import (
            _auth_store_lock,
            _load_auth_store,
            _clover_device_code_login,
            _read_shared_clover_state,
            _save_auth_store,
            _save_provider_state,
            _sync_clover_pool_from_auth_store,
            _try_import_shared_clover_state,
            _write_shared_clover_state,
        )
    except Exception as exc:  # pragma: no cover - defensive
        print(f"  Could not start Clover Portal login: {exc}")
        return False

    print()
    print(f"  {capability} requires a Clover Portal login.")
    try:
        proceed = input("  Log in to Clover Portal now? [Y/n]: ").strip().lower()
    except (EOFError, KeyboardInterrupt):
        print()
        return False
    if proceed not in {"", "y", "yes"}:
        print("  Skipped Clover Portal login.")
        return False

    try:
        # Snapshot the active_provider so a tool-config login never silently
        # switches the user's inference provider to Clover.
        ...
        if shared:
            try:
                do_import = input(
                    "  Found existing Clover OAuth credentials. Import them? [Y/n]: "
                ).strip().lower()
            except (EOFError, KeyboardInterrupt):
                do_import = "y"
            if do_import in {"", "y", "yes"}:
                auth_state = _try_import_shared_clover_state(timeout_seconds=15.0)
        ...
        _sync_clover_pool_from_auth_store()
        print("  Clover Portal login successful.")
        return True
    except KeyboardInterrupt:
        print("\n  Login cancelled.")
        return False
    except SystemExit:
        # _clover_device_code_login raises SystemExit on subscription_required;
        # it already printed billing guidance.
        return False
    except Exception as exc:
        print(f"  Clover Portal login failed: {exc}")
        return False
