"""CLI subcommand: `clover curator <subcommand>`.
...
def _print_unmanaged_summary() -> None:
    ...
    print(
        ...
        "`clover curator adopt <name>` hands one over"
    )
...
def _cmd_run(args) -> int:
    ...
    if not synchronous:
        print("llm pass running in background — check `clover curator status` later")
    if dry:
        if synchronous:
            print(
                "dry-run: no changes applied. Read the report with "
                "`clover curator status` and run `clover curator run` (no flag) to apply."
            )
        else:
            print(
...
def _cmd_pin(args) -> int:
    ...
    if not skill_usage.set_pinned(args.skill, True):
        print(
            ...
            "`clover curator list-unmanaged` shows which skills the curator tracks."
        )
        return 1
    if not skill_usage.is_curator_managed(args.skill):
        ...
        print(
            ...
            f"`clover curator adopt {args.skill}` to put it under curator "
            "management)"
        )
        return 0
...
def _cmd_list_unmanaged(args) -> int:
    ...
    print("\nadopt one with `clover curator adopt <name>`, "
          "or all with `clover curator adopt --all-unmanaged`")
    return 0
...
def _cmd_archive(args) -> int:
    ...
    if skill_usage.get_record(args.skill).get("pinned"):
        print(
            ...
            f"`clover curator unpin {args.skill}`"
        )
        return 1
    tok = skill_ledger.set_ledger_actor("user")
    try:
...
def _cmd_backup(args) -> int:
    ...
    if snap is None:
        ...
        return 1
    print(f"curator: snapshot created at ~/.clover/skills/.curator_backups/{snap.name}")
    return 0
...
def _cmd_ledger(args) -> int:
    ...
    print(
        "\nRoll back a single mutation with `clover curator rollback <id>`; "
        "whole-tree snapshots remain available via `clover curator rollback --list`."
    )
    return 0
...
def _cmd_rollback(args) -> int:
    ...
    if entry_id:
        ...
        if entry is None:
            print(
                ...
                "See `clover curator ledger` for entry ids, or use "
    ...
    if target_path is None:
        ...
        if not rows:
            print(
                ...
                "`clover curator backup` or wait for the next curator run."
            )
        else:
            print(
    ...
    print(
        "\nThis will replace the current ~/.clover/skills/ tree (a safety "
...
def register_cli(parent: argparse.ArgumentParser) -> None:
    ...
    p_backup = subs.add_parser(
        "backup",
        help="Take a manual tar.gz snapshot of ~/.clover/skills/ "
    ...
    p_rollback = subs.add_parser(
        "rollback",
        help="Restore ~/.clover/skills/ from a curator snapshot, or a single "
             "mutation by ledger entry id (see `clover curator ledger`)",
    )
    p_rollback.add_argument(
        "entry_id", nargs="?", default=None,
        help="Ledger entry id for single-mutation rollback (from "
             "`clover curator ledger`). Omit for whole-tree snapshot rollback.",
    )
    p_rollback.add_argument(
...
def cli_main(argv=None) -> int:
    """Standalone entry (also usable by clover_cli.main fallthrough)."""
    parser = argparse.ArgumentParser(prog="clover curator")
    register_cli(parser)
