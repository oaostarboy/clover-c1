"""
Clover Cognition Uninstaller.

Provides options for:
- Full uninstall: Remove everything including configs and data
- Keep data: Remove code but keep ~/.clover/ (configs, sessions, logs)
"""
...
def remove_path_from_shell_configs():
    """Remove Clover PATH entries from shell configuration files."""
    ...
    for config_path in configs:
        try:
            ...
            # Remove lines containing clover-c1 or clover PATH entries
            new_lines = []
            skip_next = False
            
            for line in content.split('\n'):
                # Skip the "# Clover Cognition" comment and following line
                if '# Clover Cognition' in line or '# clover-c1' in line:
                    skip_next = True
                    continue
                if skip_next and ('clover' in line.lower() and 'PATH' in line):
                    skip_next = False
                    continue
                skip_next = False
                
                # Remove any PATH line containing clover
                if 'clover' in line.lower() and ('PATH=' in line or 'path=' in line.lower()):
                    continue
            ...
            if new_content != original_content:
                ...
                # This is the user's own shell rc, not a Clover-owned file, and
...
def remove_wrapper_script():
    """Remove the clover wrapper script if it exists."""
    wrapper_paths = [
        Path.home() / ".local" / "bin" / "clover",
        Path.home() / ".local" / "bin" / "clover-acp",
        Path.home() / ".local" / "bin" / "clover-c1",
        Path("/usr/local/bin/clover"),
        Path("/usr/local/bin/clover-acp"),
        Path("/usr/local/bin/clover-c1"),
    ]
    ...
    for wrapper in wrapper_paths:
        if wrapper.exists():
            try:
                # Check if it's our wrapper (contains clover_cli reference)
                content = wrapper.read_text(encoding="utf-8")
                if 'clover_cli' in content or 'clover-c1' in content:
                    wrapper.unlink()
...
def remove_node_symlinks(clover_home: Path) -> list:
    ...
    symlinks node/npm/npx into the same directory as the ``clover`` command:
    ...
    the FHS fix).  Only symlinks that resolve into this Clover home's ``node``
    directory are removed — links the user has repointed elsewhere (nvm, fnm,
    etc.) are left untouched.
    """
    node_dir = (clover_home / "node").resolve()
    removed = []
...
def uninstall_gateway_service():
    ...
    - All platforms: standalone ``clover gateway run`` processes
    ...
    try:
        from clover_cli.gateway import kill_gateway_processes, find_gateway_pids
    ...
    # 3. macOS: uninstall launchd plist
        ...
        try:
            from clover_cli.gateway import get_launchd_plist_path
...
#
#   1. Sets User-scope env vars ``CLOVER_HOME`` and ``CLOVER_GIT_BASH_PATH``
...
#      like ``%LOCALAPPDATA%\clover\git\cmd``, ``%LOCALAPPDATA%\clover\git\bin``,
#      ``%LOCALAPPDATA%\clover\git\usr\bin``, ``%LOCALAPPDATA%\clover\node``.
#      Again not in any rc file — only accessible via the registry or the
#      .NET [Environment] API.
#   3. Downloads PortableGit to ``%LOCALAPPDATA%\clover\git\`` and Node to
#      ``%LOCALAPPDATA%\clover\node\`` as user-scoped, isolated copies.
#      These are ~200MB combined and serve no purpose after uninstall.
#   4. On the ``clover dashboard`` + gateway paths, drops files into
#      ``%LOCALAPPDATA%\clover\gateway-service\`` and sometimes
...
def _clover_path_markers(clover_home: Path, *, include_managed_bin: bool = False) -> list[str]:
    """Path-entry substrings that identify Clover-owned User-PATH entries.

    ``include_managed_bin`` adds the managed binary dir (``<root>\\bin``,
    holding the clover launchers and the managed uv) — only wanted when
    ...
    """
    root = str(clover_home).rstrip("\\/")
    # Match on prefix so sub-entries (git\cmd, git\bin, git\usr\bin, node, etc.)
    # all get swept.  Also match the bare clover-c1 install dir.
    markers = [root + "\\clover-c1", root + "\\git", root + "\\node", root + "\\venv"]
    if include_managed_bin:
        markers.append(root + "\\bin")
    # Also match if CLOVER_HOME was customised to somewhere else — find-and-nuke
    # any entry whose path component contains "clover".  We don't want to catch
    # unrelated entries like "cclover-foo" or "ephermeral", so we look for
    # backslash-clover as a word-ish boundary.
    return markers


def remove_path_from_windows_registry(clover_home: Path, *, include_managed_bin: bool = False) -> list[str]:
    """Strip Clover-owned entries from User-scope PATH in the registry.
    ...
    ``include_managed_bin`` adds ``<clover_home>\\bin`` (the managed binary
    dir holding the clover launchers and the managed uv) to the sweep. Only
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, key_path, 0,
            ...
            markers = _clover_path_markers(clover_home, include_managed_bin=include_managed_bin)
            kept: list[str] = []
...
def remove_clover_env_vars_windows() -> list[str]:
    """Delete CLOVER_HOME and CLOVER_GIT_BASH_PATH from User-scope env vars."""
    try:
        import winreg
    except ImportError:
        return []
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, "Environment", 0,
                            winreg.KEY_READ | winreg.KEY_WRITE) as key:
            for name in ("CLOVER_HOME", "CLOVER_GIT_BASH_PATH"):
                try:
...
def remove_windows_bin_launchers(*, windows: bool | None = None) -> list[Path]:
    """Delete the ``clover`` launchers install.ps1 staged in the managed
    binary dir (the default Clover root's ``bin``, next to the managed uv).

    Every uninstall mode deletes the code checkout, so the launchers —
    which invoke ``<checkout>\\venv\\Scripts`` — would otherwise dangle:
    ``clover`` in a new terminal resolves to a launcher whose target is
    ...
    ``_install_repair._quarantine_running_clover_exe`` relies on), so
    ...
    *windows* is an injectable platform verdict for tests (same pattern as
    ...
    try:
        ...
        from clover_cli._install_repair import _WINDOWS_BIN_LAUNCHERS
        from clover_constants import get_default_clover_root

        bin_dir = get_default_clover_root() / "bin"
    except Exception as e:
...
def _is_default_clover_home(clover_home: Path) -> bool:
    """Return True when ``clover_home`` points at the default (non-profile) root."""
    try:
        from clover_constants import get_default_clover_root
        return clover_home.resolve() == get_default_clover_root().resolve()
    except Exception:
        return False
...
def _uninstall_profile(profile) -> None:
    ...
    remove its alias wrapper, and wipe its CLOVER_HOME directory.

    We shell out to ``clover -p <name> gateway stop|uninstall`` because
    service names, unit paths, and plist paths are all derived from the
    current CLOVER_HOME and can't be easily switched in-process.
    """
    import sys as _sys
    name = profile.name
    ...
    #    Use `python -m clover_cli.main` so we don't depend on a `clover`
    #    wrapper that may be half-removed mid-uninstall.
    clover_invocation = [_sys.executable, "-m", "clover_cli.main", "--profile", name]
    for subcmd in ("stop", "uninstall"):
        try:
            subprocess.run(
                clover_invocation + ["gateway", subcmd],
                capture_output=True,
    ...
    # 3. Wipe the profile's CLOVER_HOME directory.
    try:
...
def run_gui_uninstall(args):
    ...
    Mirrors ``clover uninstall --gui``. Removes the desktop app's built
    artifacts, the packaged app bundle (best-effort), and the Electron
    userData dir — nothing under ``$CLOVER_HOME`` config/sessions/.env, and
    never the Python agent or its venv.
    """
    from clover_cli.gui_uninstall import (
        agent_is_installed,
        gui_install_summary,
        uninstall_gui,
    )
    ...
    summary = gui_install_summary(clover_home)
    skip_confirm = bool(getattr(args, "yes", False))

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│         ⚕ Clover Chat GUI Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()

    if not summary["gui_installed"]:
        print("No Clover Chat GUI installation was found.")
        print(f"  Checked: {clover_home}, and the standard app locations for this OS.")
        return

    print(color("This removes the Chat GUI only. The Clover agent stays installed.", Colors.CYAN))
    print()
    ...
    print()
    if agent_is_installed(clover_home):
        print(color("Kept intact:", Colors.GREEN, Colors.BOLD))
        print(f"  • The Clover agent at {clover_home / 'clover-c1'}")
        print(f"  • Your config, sessions, and secrets under {clover_home}")
        print()
    ...
    print(color("└─────────────────────────────────────────────────────────┘", Colors.GREEN, Colors.BOLD))
    print()
    print("The Clover agent is still installed. Run 'clover' to use the CLI,")
    print("or 'clover uninstall' to remove the agent too.")
    print()
...
def run_uninstall(args):
    ...
    Options:
    - Full uninstall: removes code + ~/.clover/ (configs, data, logs)
    - Keep data: removes code but keeps ~/.clover/ for future reinstall
    """
    ...
    # offer to clean them up too instead of leaving zombie CLOVER_HOMEs
    # and systemd units behind.
    is_default_profile = _is_default_clover_home(clover_home)
    named_profiles = _discover_named_profiles() if is_default_profile else []

    # Non-interactive fast path (``--yes``): no prompts. ``--full`` selects a
    # full wipe (code + ~/.clover data); otherwise keep-data. Named profiles
    ...
    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│            ⚕ Clover Cognition Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()
    ...
    print(f"  Config:  {clover_home / 'config.yaml'}")
    print(f"  Secrets: {clover_home / '.env'}")
    print(f"  Data:    {clover_home / 'cron/'}, {clover_home / 'sessions/'}, {clover_home / 'logs/'}")
    print()
    ...
    # their alias wrappers, and wiping their CLOVER_HOME dirs. Otherwise
    ...
    print()
    if full_uninstall:
        print(color("⚠️  WARNING: This will permanently delete ALL Clover data!", Colors.RED, Colors.BOLD))
        ...
        print("This will remove the Clover code but keep your configuration and data.")
...
def _print_uninstall_dry_run(*, project_root: Path, clover_home: Path, full_uninstall: bool) -> None:
    ...
    print("  • Clover PATH entries from shell configs / Windows User PATH")
    print("  • Clover wrapper scripts and Clover-managed node/npm/npx symlinks")
    print("  • Desktop Chat GUI artifacts")
    print(f"  • Code checkout: {project_root}")
    if full_uninstall:
        print(f"  • Clover config/data: {clover_home}")
        if _is_default_clover_home(clover_home):
        ...
        print(f"  • Keep Clover config/data: {clover_home}")
    print()
...
def _perform_uninstall(
    *,
    ...
    ``clover`` wrapper + node symlinks → remove the desktop Chat GUI artifacts →
    delete the code checkout → (Windows) remove PortableGit/Node → optionally
    wipe ``$CLOVER_HOME`` data and named profiles on full uninstall.
    """
    print()
    ...
    if _is_windows():
        ...
        # Expand %LOCALAPPDATA% etc. in clover_home so the marker matching is
        # against fully resolved paths — installer writes literal strings
        # like C:\Users\<u>\AppData\Local\clover\git\cmd, not %LOCALAPPDATA%.
        # The managed binary dir (clover\bin: launchers + managed uv) leaves
        ...
        sweep_managed_bin = full_uninstall and _is_default_clover_home(clover_home)
        removed_path_entries = remove_path_from_windows_registry(
            Path(os.path.expandvars(str(clover_home))),
        ...
        if removed_path_entries:
            ...
            log_info("No Clover-owned PATH entries in User environment")

        log_info("Removing CLOVER_HOME / CLOVER_GIT_BASH_PATH User env vars...")
        removed_env = remove_clover_env_vars_windows()
        if removed_env:
            for name in removed_env:
                log_success(f"Removed User env var: {name}")
        else:
            log_info("No Clover-set User env vars to remove")
    
    # 3. Remove wrapper script
    log_info("Removing clover command...")
    ...
    #     would dangle — `clover` in a new terminal would resolve and then
    #     error on its missing venv target, worse than command-not-found.
    if _is_windows():
        log_info("Removing Windows clover launchers...")
        ...
        if removed_launchers:
            ...
            log_info("No Windows clover launchers found")

    # 3b. Remove node/npm/npx symlinks the installer left in ~/.local/bin
    #     (only when they still point into this Clover home's node dir, so we
    #     never clobber an existing nvm / user-managed Node).
    log_info("Removing Clover-managed node/npm/npx symlinks...")
    removed_node_links = remove_node_symlinks(clover_home)
    if removed_node_links:
        for link in removed_node_links:
            log_success(f"Removed {link}")
    else:
        log_info("No Clover-managed node/npm/npx symlinks found")
    ...
    #     step-5 rmtree(clover_home) would sweep the in-tree artifacts anyway,
    #     but the packaged app + Electron userData live OUTSIDE CLOVER_HOME and
    #     must be cleaned explicitly here.
    log_info("Removing desktop Chat GUI artifacts...")
    try:
        from clover_cli.gui_uninstall import uninstall_gui
        gui_removed = uninstall_gui(clover_home)
        if not gui_removed:
    ...
    try:
        if project_root.exists():
            # If the install is inside ~/.clover/, just remove the clover-c1 subdir
            if clover_home in project_root.parents or project_root.parent == clover_home:
    ...
    #     under CLOVER_HOME but they're install tooling, not config — safe to
    #     remove even in "keep data" mode.  If we're doing a full uninstall
    #     the step-5 rmtree(clover_home) would sweep them anyway; calling
    ...
    if _is_windows():
        ...
        removed_artifacts = remove_portable_tooling_windows(clover_home)
    ...
    # 5. Optionally remove ~/.clover/ data directory (and named profiles)
    if full_uninstall:
        # 5a. Stop and remove each named profile's gateway service and
        #     alias wrapper. The profile CLOVER_HOME dirs live under
        ...
        try:
            if clover_home.exists():
                shutil.rmtree(clover_home)
                log_success(f"Removed {clover_home}")
        except Exception as e:
            log_warn(f"Could not fully remove {clover_home}: {e}")
            log_info("You may need to manually remove it")
    else:
        log_info(f"Keeping configuration and data in {clover_home}")
    ...
    print()
    print("Thank you for using Clover Cognition! ⚕")
    print()
...
def main(argv=None) -> int:
    """Module entrypoint: ``python -m clover_cli.uninstall --mode <gui|lite|full>``.
    ...
    so ``import clover_cli`` resolves from source while the venv is torn down.

    This module imports only stdlib + ``clover_constants`` + ``clover_cli.colors``
    (and lazily ``clover_cli.gui_uninstall``), so it runs fine under a bare
    system Python with no site-packages from the venv.
    """
    import argparse

    parser = argparse.ArgumentParser(prog="python -m clover_cli.uninstall")
    parser.add_argument(
        "--mode",
