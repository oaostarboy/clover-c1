...
1. **Update receipt** — a machine-readable JSON record of what one
   ``clover update`` run discovered, did, skipped (and why), written to
   ``<CLOVER_HOME>/logs/update_receipts/``. Silent-failure classes this
...
``clover_cli.build_info.get_code_identity()``: an image build reports
...
# Module-level current receipt. ``clover update`` is a single-threaded CLI
...
class UpdateReceipt:
    """Collects the observable facts of one ``clover update`` run."""
...
def _receipt_dir() -> Path:
    ...
    return get_clover_home() / "logs" / _RECEIPT_DIR_NAME
...
def finalize_pending_update_receipt(
    ...
    ``clover update`` has many early-termination paths (Windows
