"""Install and remove the Linux desktop entry (``clover.desktop``).

``clover desktop`` builds and launches the Electron app. On Linux, a
...
DESKTOP_ENTRY_NAME = "clover.desktop"
...
def desktop_entry_path() -> Path:
    """Where the ``clover.desktop`` entry lives."""
...
def resolve_exec_command() -> str:
    """Build the absolute ``Exec=`` command line for ``clover desktop``.

    Prefer the real ``clover`` executable (argv[0] or PATH). When Clover
    runs as a module with no launcher installed, use the current
    interpreter, also absolute.
    """
    from clover_cli.relaunch import resolve_clover_bin
    ...
    if bin_path:
        ...
        if _needs_interpreter(resolved):
            ...
            # a NON-venv interpreter (e.g. the repo's `clover` script with
            ...
            # sys.executable is the interpreter actually running Clover (the
            # venv one), so prefix it explicitly.
            argv = [str(Path(sys.executable).resolve()), str(resolved), "desktop"]
        else:
            argv = [str(resolved), "desktop"]
    else:
        argv = [str(Path(sys.executable).resolve()), "-m", "clover_cli.main", "desktop"]
    return " ".join(_quote_exec_arg(a) for a in argv)


def _needs_interpreter(bin_path: Path) -> bool:
    """Whether ``bin_path`` is a Python script that must run under
    ``sys.executable`` to see Clover' venv (rather than its own shebang)."""
    try:
...
def render_desktop_entry(exec_command: str, icon: str) -> str:
    return (
        ...
        "Type=Application\n"
        "Name=Clover\n"
        "GenericName=Clover Desktop\n"
        "Comment=Launch Clover Desktop\n"
        f"Exec={exec_command}\n"
        f"Icon={icon}\n"
        "Terminal=false\n"
        "Categories=Utility;\n"
        "StartupNotify=true\n"
        "StartupWMClass=Clover\n"
    )
...
def install_desktop_entry(project_root: Path) -> Optional[Path]:
    """Write (or refresh) the Clover desktop entry. Return its path.
    ...
    icon_value = str(icon) if icon.is_file() else "clover"
