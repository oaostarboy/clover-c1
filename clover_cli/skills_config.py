"""
Skills configuration for Clover Cognition.
`clover skills` enters this module.

Toggle individual skills or categories on/off, globally or per-platform.
Config stored in ~/.clover/config.yaml under:
...
from clover_cli.config import cfg_get, load_config, save_config
from clover_cli.colors import Colors, color
from clover_cli.platforms import PLATFORMS as _PLATFORMS
...
def save_disabled_skills(config: dict, disabled: Set[str], platform: Optional[str] = None):
    ...
    Essential skills (e.g. ``clover-c1``) are silently dropped from the
