...
- ``list_authenticated_providers`` already populates each row's
  ...
  curation and pulls in non-agentic models (Clover /models returns ~400
...
def build_models_payload(
    ...
    *,
    ...
    - ``pricing``: enrich each row with formatted per-model pricing and,
      for Clover, ``free_tier``/``unavailable_models`` so the GUI picker can
      show $/Mtok columns and gate paid models on free accounts —
      mirroring the ``clover model`` CLI picker. Adds network calls
      (pricing fetch + Clover tier check); only set for interactive pickers.
    ...
    try:
        from clover_cli.providers import is_routing_aggregator as _is_routing_aggregator
    except Exception:
...
def build_aux_picker_rows(
    *,
    ...
    back. Mirrors the same filter in ``clover_cli/moa_cmd.py``.
...
def _apply_custom_aliases(rows: list[dict]) -> None:
    ...
    :func:`clover_cli.providers.custom_provider_aliases` — lets the frontend
    do a membership check instead.
    """
    from clover_cli.providers import custom_provider_aliases
...
def _append_unconfigured_rows(
    ...
    *,
    ...
    from clover_cli.models import CANONICAL_PROVIDERS, _PROVIDER_LABELS
    ...
    for entry in CANONICAL_PROVIDERS:
        ...
        if entry.slug.lower() == cur:
            ...
            warning = (
                ...
                else "Configured provider is not authenticated; run `clover model` to reactivate. "
                "Showing the saved model only."
            )
            extras.append(
                {
...
def _anthropic_oauth_credentials_present() -> bool:
    ...
    model.provider / API-key env vars: Clover' own Anthropic device flow
    ...
    try:
        ...
        clover_creds = read_clover_oauth_credentials() or {}
        if clover_creds.get("accessToken"):
            return True
...
def _filter_explicit_provider_rows(rows: list[dict], ctx: ConfigContext) -> list[dict]:
    ...
    pickers want the narrower subset the user explicitly configured for Clover.
    """
    ...
    for row in rows:
        ...
        if slug == "anthropic" and _anthropic_oauth_credentials_present():
            # Anthropic OAuth logins (Clover device flow / Claude Code) are
...
def _provider_is_keyless(slug: str) -> bool:
    """True when the provider's Clover overlay declares it keyless."""
    try:
        from clover_cli.providers import CLOVER_OVERLAYS
        overlay = CLOVER_OVERLAYS.get(slug)
...
def _apply_picker_hints(rows: list[dict]) -> None:
    ...
    for row in rows:
        ...
        row["warning"] = (
            ...
            if auth_type == "api_key" and key_env
            else f"run `clover model` to configure ({auth_type})"
        )
...
def _moa_provider_row(current_provider: str = "") -> dict | None:
    ...
    picker path (:func:`clover_cli.model_switch.list_picker_providers`) so the
