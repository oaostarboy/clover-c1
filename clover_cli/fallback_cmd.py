"""
clover fallback — manage the fallback provider chain.
...
Subcommands:
  clover fallback [list]   Show the current fallback chain (default when no subcommand)
  clover fallback add      Pick provider + model via the same picker as `clover model`,
                           then append the selection to the chain
  clover fallback remove   Pick an entry to delete from the chain
  clover fallback clear    Remove all fallback entries

Storage: ``fallback_providers`` in ``~/.clover/config.yaml`` (top-level, list of
...
def _restore_auth_active_provider(value: Any) -> None:
    ...
    try:
        from clover_cli.auth import _auth_store_lock, _load_auth_store, _save_auth_store
        ...
        # `clover model` to fix it.  Don't fail the fallback add.
        pass
...
def cmd_fallback_list(args) -> None:  # noqa: ARG001
    ...
    if not chain:
        ...
        print()
        print("  Add one with:  clover fallback add")
        print()
        return
    ...
    print("  Docs: https://clover-c1.local/docs/user-guide/features/fallback-providers")
    print()
...
def cmd_fallback_add(args) -> None:
    """Launch the same picker as `clover model`, then append the selection to the chain."""
    from clover_cli.main import _require_tty, select_provider_and_model
    ...
    print("  `clover model` — select the provider + model you want as a fallback.")
    print()
    ...
    print()
    print("  Run `clover fallback list` to view, or `clover fallback remove` to delete.")
...
def cmd_fallback(args) -> None:
    """Top-level dispatcher for ``clover fallback [subcommand]``."""
