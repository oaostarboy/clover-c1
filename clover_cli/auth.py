"""
Multi-provider authentication system for Clover Cognition.

Supports OAuth device code flows (Clover Portal, future: OpenAI Codex) and
traditional API key providers (OpenRouter, custom endpoints). Auth state
is persisted in ~/.clover/auth.json with cross-process file locking.
...
# httpx is imported lazily: it costs ~30ms at import time and clover_cli.auth
...
if TYPE_CHECKING:
    ...
    class _LazyHttpx:
        ...
        # ("clover_cli.auth.httpx.Client", ...) keeps working in tests.
...
from clover_constants import OPENROUTER_BASE_URL, secure_parent_dir
...
DEFAULT_NOUS_INFERENCE_URL = "https://inference.clover-c1.local/v1"
...
try:  # Version tag for the Codex token-endpoint User-Agent; fall back if unavailable.
    from clover_cli import __version__ as _CLOVER_CLI_VERSION
except Exception:  # pragma: no cover - version import should always succeed
    _CLOVER_CLI_VERSION = "unknown"
CODEX_OAUTH_USER_AGENT = f"clover-cli/{_CLOVER_CLI_VERSION}"
...
SPOTIFY_DOCS_URL = "https://clover-c1.local/docs/user-guide/features/spotify"
SPOTIFY_DASHBOARD_URL = "https://developer.spotify.com/dashboard"
SPOTIFY_ACCESS_TOKEN_REFRESH_SKEW_SECONDS = 120

OAUTH_OVER_SSH_DOCS_URL = "https://clover-c1.local/docs/guides/oauth-over-ssh"
...
def normalize_actual_base_url(base_url: str) -> str:
    ...
    surface for Clover' Responses transport.
    """
...
PROVIDER_REGISTRY: Dict[str, ProviderConfig] = {
    ...
    "anthropic": ProviderConfig(
        ...
        # from it), so removing it would stop Clover finding a setup-token
    ...
    "opencode-free": ProviderConfig(
        ...
        # secret to configure. Select via `clover model` / `/model free`.
        api_key_env_vars=(),
    ),
...
def get_anthropic_key() -> str:
    ...
    ``~/.clover/.env`` so a deliberate key rotation isn't shadowed by a stale
    ...
    """
    from clover_cli.config import get_env_value_prefer_dotenv
...
def _resolve_api_key_provider_secret(
    ...
    if provider_id == "copilot":
        ...
        try:
            from clover_cli.copilot_auth import resolve_copilot_token, get_copilot_api_token
    ...
    for env_var in pconfig.api_key_env_vars:
        # Prefer ~/.clover/.env over os.environ so a deliberate key rotation
...
def is_rate_limited_auth_error(error: Exception) -> bool:
    ...
    instead of prompting the operator to run ``clover auth``.
    """
    return (
...
def format_auth_error(error: Exception) -> str:
    ...
    if error.relogin_required:
        return f"{error} Run `clover model` to re-authenticate."
...
def _format_nous_entitlement_auth_error(error: AuthError) -> str:
    try:
        ...
        pass
    return f"{error} Check credits or billing in Clover Portal, then retry."
...
def _oauth_trace_enabled() -> bool:
    raw = os.getenv("CLOVER_OAUTH_TRACE", "").strip().lower()
...
# =============================================================================
# Auth Store — persistence layer for ~/.clover/auth.json
# =============================================================================

def _auth_file_path() -> Path:
    path = get_clover_home() / "auth.json"
    # Seat belt: if pytest is running and CLOVER_HOME resolves to the real
    # user's auth store, refuse rather than silently corrupt it. This catches
    # tests that forgot to monkeypatch CLOVER_HOME, tests invoked without the
    # hermetic conftest, or sandbox escapes via threads/subprocesses. In
    # production (no PYTEST_CURRENT_TEST) this is a single dict lookup.
    if os.environ.get("PYTEST_CURRENT_TEST"):
        real_home_auth = (Path.home() / ".clover" / "auth.json").resolve(strict=False)
        try:
        ...
        if resolved == real_home_auth:
            raise RuntimeError(
                ...
                "Set CLOVER_HOME to a tmp_path in your test fixture, or run "
...
def _global_auth_file_path() -> Optional[Path]:
    ...
    directory (classic mode, or custom CLOVER_HOME that is not a profile).
    ...
    try:
        ...
        global_root = get_default_clover_root()
    except Exception:
        return None
    profile_home = get_clover_home()
    try:
...
def _load_global_auth_store() -> Dict[str, Any]:
    ...
    if os.environ.get("PYTEST_CURRENT_TEST"):
        ...
        if real_home_env:
            real_root = Path(real_home_env) / ".clover" / "auth.json"
            try:
...
def _file_lock(
    ...
    (e.g. profile auth.json vs shared Clover store) don't share reentrancy
...
def _load_auth_store(auth_file: Optional[Path] = None) -> Dict[str, Any]:
    ...
    if isinstance(raw, dict) and isinstance(raw.get("systems"), dict):
        systems = raw["systems"]
        providers = {}
        if "clover_portal" in systems:
            providers["clover"] = systems["clover_portal"]
        return {"version": AUTH_STORE_VERSION, "providers": providers,
                "active_provider": "clover" if providers else None}
...
def _load_provider_state_with_source(
    ...
    global-root fallback state; persisting a rotated Clover refresh token only to
...
def mark_provider_active_if_unset(provider_id: str) -> None:
    ...
    Used by ``clover auth add`` OAuth paths that create credential-pool
...
def read_credential_pool(provider_id: Optional[str] = None) -> Dict[str, Any]:
    ...
    ``clover auth add <provider>`` inside the profile, profile entries
...
def get_provider_auth_state(provider_id: str) -> Optional[Dict[str, Any]]:
    ...
    OAuth or Clover device-code session). See issue #18594 follow-up.
    """
    auth_store = _load_auth_store()
...
def is_provider_explicitly_configured(provider_id: str) -> bool:
    ...
    try:
        ...
        # ``provider: anthropic`` as a MoA advisor/aggregator has opted Clover
    ...
    # not by the user explicitly configuring anthropic in Clover.
    ...
    if pconfig is None:
        from clover_cli.providers import get_provider
    ...
    # the user initiated inside Clover (manual add / device-code / PKCE), plus
    ...
    try:
        for entry in read_credential_pool(normalized):
            ...
            if source.startswith("env:"):
                ...
                continue
            if (
                source in {"device_code", "loopback_pkce", "clover_pkce", "manual"}
    ...
    # explicit for users who never set Clover up for it. Only Clover-scoped
...
def clear_provider_auth(provider_id: Optional[str] = None) -> bool:
    """
    Clear auth state for a provider. Used by `clover logout`.
...
def _get_config_hint_for_unknown_provider(provider_name: str) -> str:
    ...
    """
    try:
        from clover_cli.config import validate_config_structure
        issues = validate_config_structure()
        if not issues:
            return ""

        lines = ["Config issue detected — run 'clover doctor' for full diagnostics:"]
        for ci in issues:
    ...
    *,
    ...
    if normalized != "auto":
        ...
        if _config_hint:
            ...
            msg += " Check 'clover model' for available providers, or run 'clover doctor' to diagnose config issues."
    ...
    # Auto-detect an OpenRouter credential added via `clover auth add openrouter`
    ...
    # `clover auth list` showing the credential while requests go out with no
    ...
    for pid, pconfig in PROVIDER_REGISTRY.items():
        ...
        for env_var in pconfig.api_key_env_vars:
            if has_usable_secret(_scoped_key_env(env_var)):
                ...
                # OAuth but has a stale key in ~/.clover/.env isn't silently
    ...
    raise AuthError(
        "No inference provider configured. Run 'clover model' to choose a "
        "provider and model, or set an API key (OPENROUTER_API_KEY, "
        "OPENAI_API_KEY, etc.) in ~/.clover/.env.",
...
def _nous_portal_env_override() -> Optional[str]:
    ...
    ``CLOVER_PORTAL_BASE_URL=https://portal.staging-clover-c1.local`` into
...
def resolve_qwen_runtime_credentials(
    *,
    ...
    base_url = os.getenv("CLOVER_QWEN_BASE_URL", "").strip().rstrip("/") or DEFAULT_QWEN_BASE_URL
    return {
        "provider": "qwen-oauth",
        "base_url": base_url,
...
def get_qwen_auth_status() -> Dict[str, Any]:
    ...
    try:
        ...
        # and `clover model` walks users into a broken Qwen setup flow.
...
# =============================================================================
# Spotify auth — PKCE tokens stored in ~/.clover/auth.json
# =============================================================================
...
def _spotify_client_id(
    ...
    candidates = (
        explicit,
        get_env_value("CLOVER_SPOTIFY_CLIENT_ID"),
    ...
    for candidate in candidates:
        ...
        if cleaned:
            return cleaned
    raise AuthError(
        "Spotify client_id is required. Set CLOVER_SPOTIFY_CLIENT_ID or pass --client-id.",
        provider="spotify",
...
def _spotify_redirect_uri(
    ...
    candidates = (
        explicit,
        get_env_value("CLOVER_SPOTIFY_REDIRECT_URI"),
...
def _spotify_api_base_url(state: Optional[Dict[str, Any]] = None) -> str:
    ...
    candidates = (
        get_env_value("CLOVER_SPOTIFY_API_BASE_URL"),
...
def _spotify_accounts_base_url(state: Optional[Dict[str, Any]] = None) -> str:
    ...
    candidates = (
        get_env_value("CLOVER_SPOTIFY_ACCOUNTS_BASE_URL"),
...
def _refresh_spotify_oauth_state(
    ...
    *,
    ...
    if not refresh_token:
        raise AuthError(
            "Spotify refresh token missing. Run `clover auth spotify` again.",
            provider="spotify",
    ...
    if response.status_code >= 400:
        detail = response.text.strip()
        raise AuthError(
            "Spotify token refresh failed. Run `clover auth spotify` again."
            + (f" Response: {detail}" if detail else ""),
            provider="spotify",
            code="spotify_refresh_failed",
            relogin_required=True,
...
def resolve_spotify_runtime_credentials(
    *,
    ...
    with _auth_store_lock():
        ...
        if not state:
            raise AuthError(
                "Spotify is not authenticated. Run `clover auth spotify` first.",
                provider="spotify",
        ...
        if should_refresh:
            try:
                ...
                if exc.relogin_required and state.get("refresh_token"):
                    ...
                    # Mirrors the Clover / xAI-OAuth / Codex-OAuth / MiniMax pattern.
    ...
    if not access_token:
        raise AuthError(
            "Spotify access token missing. Run `clover auth spotify` again.",
            provider="spotify",
...
def _spotify_interactive_setup(redirect_uri_hint: str) -> str:
    ...
    resulting client_id to ~/.clover/.env, and return it.
    ...
    print("       App name:     anything (e.g. clover-c1)")
    ...
    # Persist so subsequent `clover auth spotify` runs skip the wizard.
    save_env_value("CLOVER_SPOTIFY_CLIENT_ID", raw)
    ...
    if redirect_uri_hint and redirect_uri_hint != DEFAULT_SPOTIFY_REDIRECT_URI:
        save_env_value("CLOVER_SPOTIFY_REDIRECT_URI", redirect_uri_hint)

    print()
    print("Saved CLOVER_SPOTIFY_CLIENT_ID to ~/.clover/.env")
    print()
    return raw
...
def login_spotify_command(args) -> None:
    ...
    # with "CLOVER_SPOTIFY_CLIENT_ID is required".
    ...
    print()
    print("Open this URL to authorize Clover:")
    print(authorize_url)
    print()
...
def _print_loopback_ssh_hint(redirect_uri: str, *, docs_url: str | None = None) -> None:
    ...
    print(divider)
    print(f"Clover is waiting for the OAuth callback on {redirect_uri}")
...
# =============================================================================
# OpenAI Codex auth — tokens stored in ~/.clover/auth.json (not ~/.codex/)
#
# Clover maintains its own Codex OAuth session separate from the Codex CLI
# and VS Code extension. This prevents refresh token rotation conflicts
# where one app's refresh invalidates the other's session.
# =============================================================================

def _read_codex_tokens(*, _lock: bool = True) -> Dict[str, Any]:
    """Read Codex OAuth tokens from Clover auth store (~/.clover/auth.json).
    ...
    if not state:
        raise AuthError(
            "No Codex credentials stored. Run `clover auth` to authenticate.",
    ...
    if not isinstance(tokens, dict):
        raise AuthError(
            "Codex auth state is missing tokens. Run `clover auth` to re-authenticate.",
            provider="openai-codex",
    ...
    if not isinstance(access_token, str) or not access_token.strip():
        raise AuthError(
            "Codex auth is missing access_token. Run `clover auth` to re-authenticate.",
            provider="openai-codex",
    ...
    if not isinstance(refresh_token, str) or not refresh_token.strip():
        raise AuthError(
            "Codex auth is missing refresh_token. Run `clover auth` to re-authenticate.",
            provider="openai-codex",
...
def _sync_codex_pool_entries(
    ...
    * ``device_code`` — the singleton-seeded entry written by the device-code
      OAuth flow when the user logged in via ``clover setup`` / the model
      picker.  Always synced with the fresh tokens.
    * ``manual:device_code`` — entries created by ``clover auth add openai-codex``
      ...
      ``clover auth add openai-codex`` and must not be overwritten by a
...
def _save_codex_tokens(tokens: Dict[str, str], last_refresh: str = None, label: str = None) -> None:
    """Save Codex OAuth tokens to Clover auth store (~/.clover/auth.json)."""
    ...
    with _auth_store_lock():
        ...
        # ``clover auth add openai-codex`` created (which must not be
...
def _recover_codex_tokens_from_cli(reason: str) -> Optional[Dict[str, str]]:
    """Adopt a valid Codex CLI token pair into Clover auth, if available."""
...
def refresh_codex_oauth_pure(
    ...
    *,
    ...
    """Refresh Codex OAuth tokens without mutating Clover auth state."""
    ...
    if response.status_code == 429:
        ...
        # "run clover auth" prompt (see issue #32790).
    ...
    if response.status_code != 200:
        ...
        if code == "refresh_token_reused":
            message = (
                ...
                "then run `clover auth` to re-authenticate."
            )
            relogin_required = True
...
def _refresh_codex_auth_tokens(
    ...
    Saves the new tokens to Clover auth store automatically.
    """
    try:
        ...
        # Self-heal cross-store refresh_token rotation. Clover keeps its OWN
        ...
        # the Codex CLI (or another Clover process) rotates the shared token,
...
def resolve_codex_runtime_credentials(
    *,
    ...
    """Resolve runtime credentials from Clover's own Codex token store.
    ...
    if data is None:
        ...
        if pool_token:
            base_url = (
                os.getenv("CLOVER_CODEX_BASE_URL", "").strip().rstrip("/")
    ...
    access_token = str(tokens.get("access_token", "") or "").strip()
    refresh_timeout_seconds = env_float("CLOVER_CODEX_REFRESH_TIMEOUT_SECONDS", 20)

    should_refresh = bool(force_refresh)
    if (not should_refresh) and refresh_if_expiring:
        should_refresh = _codex_access_token_is_expiring(access_token, refresh_skew_seconds)
    if should_refresh:
        # Re-read under lock to avoid racing with other Clover processes
...
def _probe_codex_quota_restored(
    ...
    *,
    ...
    Clover persists a Codex 429's ``reset_at`` locally and freezes the
...
# =============================================================================
# xAI Grok OAuth — tokens stored in ~/.clover/auth.json
# =============================================================================
...
def _read_xai_oauth_tokens(*, _lock: bool = True) -> Dict[str, Any]:
    ...
    if not state:
        raise AuthError(
            "No xAI OAuth credentials stored. Select xAI Grok OAuth (SuperGrok / Premium+) in `clover model`.",
            provider="xai-oauth",
    ...
    if not isinstance(tokens, dict):
        raise AuthError(
            "xAI OAuth state is missing tokens. Re-authenticate with `clover model`.",
            provider="xai-oauth",
    ...
    if not access_token:
        raise AuthError(
            "xAI OAuth state is missing access_token. Re-authenticate with `clover model`.",
            provider="xai-oauth",
    ...
    if not refresh_token:
        raise AuthError(
            "xAI OAuth state is missing refresh_token. Re-authenticate with `clover model`.",
            provider="xai-oauth",
...
def _write_through_xai_oauth_to_global_root(state: Dict[str, Any]) -> None:
    ...
    # ~/.clover/auth.json even when CLOVER_HOME points at a profile path
...
def _xai_proactive_refresh_skew_seconds(access_token: str) -> int:
    ...
    resolution (chat turn, Imagine tool call, ``clover auth status``, …),
...
def _xai_validate_oauth_endpoint(url: str, *, field: str) -> str:
    ...
    output is cached in ``~/.clover/auth.json``. A single MITM during initial
    ...
    if host != "x.ai" and not host.endswith(".x.ai"):
        raise AuthError(
            ...
            f"discovery; re-authenticate with `clover model` to re-fetch.",
            provider="xai-oauth",
            code="xai_discovery_invalid",
        )
    return url
...
def _xai_validate_inference_base_url(value: str, *, fallback: str) -> str:
    ...
    the user's SuperGrok subscription. ``XAI_BASE_URL`` / ``CLOVER_XAI_BASE_URL``
...
def refresh_xai_oauth_pure(
    ...
    *,
    ...
    if not isinstance(refresh_token, str) or not refresh_token.strip():
        raise AuthError(
            "xAI OAuth is missing refresh_token. Re-authenticate with `clover model`.",
            provider="xai-oauth",
            code="xai_auth_missing_refresh_token",
            relogin_required=True,
        )
    ...
    # written by an older Clover (or hand-edited) may carry a non-xAI
    ...
    # with a clear error so the user can re-run `clover model` to refetch.
    ...
    if response.status_code != 200:
        ...
        # on the allowlist for API access).  Re-running ``clover model``
...
def resolve_xai_oauth_runtime_credentials(
    *,
    ...
    tokens = dict(data["tokens"])
    access_token = str(tokens.get("access_token", "") or "").strip()
    refresh_timeout_seconds = env_float("CLOVER_XAI_REFRESH_TIMEOUT_SECONDS", 20)
    ...
    base_url = _xai_validate_inference_base_url(
        os.getenv("CLOVER_XAI_BASE_URL", "").strip().rstrip("/")
...
# =============================================================================
# Clover Portal — token refresh and model discovery
# =============================================================================
...
# typically live under ``<clover-root>/profiles/<name>/``) all see the
# same file.
#
...
def _nous_shared_auth_dir() -> Path:
    ...
    ``<clover-root>/shared/``, where ``<clover-root>`` is what
    :func:`clover_constants.get_default_clover_root` returns — so
    Linux/macOS classic installs land at ``~/.clover/shared/``, native
    Windows installs at ``%LOCALAPPDATA%\\clover\\shared\\``, and
    Docker / custom ``CLOVER_HOME`` deployments at
    ``<CLOVER_HOME>/shared/``. Sits outside any named profile so all
    profiles under the same root share the store.
    """
    override = os.getenv("CLOVER_SHARED_AUTH_DIR", "").strip()
    if override:
...
def _nous_shared_store_path() -> Path:
    ...
    if os.environ.get("PYTEST_CURRENT_TEST"):
        ...
        if resolved == real_home_shared:
            raise RuntimeError(
                f"Refusing to touch real user shared Clover auth store during test run: "
                f"{path}. Set CLOVER_SHARED_AUTH_DIR to a tmp_path in your test fixture."
            )
    return path
...
def _write_shared_nous_state(state: Dict[str, Any]) -> None:
    ...
    try:
        with _nous_shared_store_lock():
            ...
            # Create with 0o600 atomically via os.open(O_EXCL) — closes the TOCTOU
            # window where write_text() + post-write chmod briefly exposed Clover
...
def _read_shared_nous_state() -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        logger.debug("Shared Clover auth store at %s is unreadable: %s", path, exc)
        return None
    if not isinstance(payload, dict):
        return None
...
def _quarantine_nous_oauth_state(
    ...
    *,
    ...
    # sizes/booleans. NEVER pass a raw token/agent_key into the log call — Clover
    # has a known bug class where credential-shaped literals get corrupted in logs.
    forensic: Dict[str, Any] = {
        "reason": reason,
        "error_code": error.code,
        # No session_id field exists on Clover state; provenance is client_id +
    ...
    logger.warning(
        "Clover OAuth state quarantined (terminal auth death): %s",
...
def _quarantine_nous_pool_entries(
    ...
    *,
    ...
    """Remove singleton-seeded Clover pool entries that contain dead OAuth state."""
    pool = auth_store.get("credential_pool")
    if not isinstance(pool, dict):
        return False
    entries = pool.get("clover")
    if not isinstance(entries, list):
        return False
...
def _try_import_shared_nous_state(
    *,
    ...
    """Attempt to rehydrate Clover OAuth state from the shared store.
    ...
    *,
    ...
    if response.status_code == 200:
        ...
        if "access_token" not in payload:
            raise AuthError("Refresh response missing access_token",
                            provider="clover", code="invalid_token", relogin_required=True)
        return payload
    ...
    try:
        ...
        raise AuthError("Refresh token exchange failed",
                        provider="clover", relogin_required=True) from exc

    code = str(error_payload.get("error", "invalid_grant"))
    description = str(error_payload.get("error_description") or "Refresh token exchange failed")
    relogin = code in {"invalid_grant", "invalid_token", "refresh_token_reused"}

    # Detect the OAuth 2.1 "refresh token reuse" signal from the Clover portal
    ...
    # called POST /api/oauth/token with Clover's refresh_token without
    # persisting the rotated token back to auth.json — the server then
    # retires the original RT, Clover's next refresh uses it, and the whole
    ...
    if code == "refresh_token_reused" or "reuse" in lowered or "reuse detected" in lowered:
        description = (
            "Clover Portal detected refresh-token reuse and revoked this session.\n"
            "This usually means an external process (monitoring script, "
            "custom self-heal hook, or another Clover install sharing "
            "~/.clover/auth.json) called POST /api/oauth/token with Clover's "
...
def fetch_nous_models(
    *,
    ...
    """Fetch available model IDs from the Clover inference API."""
    ...
    if response.status_code != 200:
        ...
        raise AuthError(description, provider="clover", code="models_fetch_failed")
    ...
    for item in data:
        ...
        if isinstance(model_id, str) and model_id.strip():
            mid = model_id.strip()
            # Skip Clover models — they're not reliable for agentic tool-calling
            if "clover" in mid.lower():
                continue
...
def resolve_nous_access_token(
    *,
    ...
    """Resolve a refresh-aware Clover Portal access token for managed tool gateways."""
        ...
        with _RESOLVE_TOKEN_CACHE_LOCK:
            if _RESOLVE_TOKEN_CACHE is not None:
                ...
                if (time.monotonic() - cached_at) < _RESOLVE_TOKEN_CACHE_TTL_S:
                    return cached_token
    with _provider_state_transaction("clover") as (
        auth_store,
        state,
        state_source_path,
    ):
        ...
        if not state:
            raise AuthError(
                "Clover is not logged into Clover Portal.",
                provider="clover",
                relogin_required=True,
            )
        ...
        with _nous_shared_store_lock(timeout_seconds=max(timeout_seconds + 5.0, AUTH_LOCK_TIMEOUT_SECONDS)):
            ...
            if not isinstance(access_token, str) or not access_token:
                raise AuthError(
                    "No access token found for Clover Portal login.",
                    provider="clover",
                    relogin_required=True,
                )
...
def refresh_nous_oauth_pure(
    ...
    *,
    ...
    """Refresh Clover OAuth state without mutating auth.json directly.
    ...
    with httpx.Client(timeout=timeout, headers={"Accept": "application/json"}, verify=verify) as client:
        ...
        if force_refresh or current_invoke_jwt_status is not None:
            ...
            if not isinstance(refresh_token_value, str) or not refresh_token_value:
                ...
                raise AuthError(
                    "No refresh token is available for Clover Portal.",
                    provider="clover",
                    relogin_required=True,
                )
            refreshed = _refresh_access_token(
...
def persist_nous_credentials(
    ...
    *,
    ...
    """Persist Clover OAuth credentials as the singleton provider state
...
def resolve_nous_runtime_credentials(
    *,
    ...
    """
    Resolve Clover inference credentials for runtime use.
    ...
    with _provider_state_transaction("clover") as (
        ...
        if not state:
            raise AuthError("Clover is not logged into Clover Portal.",
    ...
    if not isinstance(api_key, str) or not api_key:
        raise AuthError("Failed to resolve a Clover inference API key",
                        provider="clover", code="server_error")
...
# which does a synchronous OAuth refresh POST to portal.clover-c1.local. That can take
...
def get_nous_session_validity() -> str:
    ...
    Returns one of:
      - ``"valid"``    — a usable Clover credential is present (login healthy).
      - ``"terminal"`` — the Clover session has taken a terminal auth failure
      ...
      - ``"unknown"``  — indeterminate (no Clover provider state, or a transient/
    ...
    # into the Clover provider state when it clears dead tokens (the exact path
...
def get_codex_auth_status() -> Dict[str, Any]:
    ...
    Checks the credential pool first (where `clover auth` stores credentials),
    then falls back to the legacy provider state.
    """
    # Check credential pool first — this is where `clover auth` and
    # `clover model` store device_code tokens.
    try:
        from agent.credential_pool import load_pool
...
def get_api_key_provider_status(provider_id: str) -> Dict[str, Any]:
    ...
    # the CloverOverlay keyless flag — the same source the provider catalog
    # and GUI contract tests use.
    try:
        from clover_cli.providers import CLOVER_OVERLAYS
        _overlay = CLOVER_OVERLAYS.get(provider_id)
    except Exception:
        _overlay = None
...
def get_external_process_provider_status(provider_id: str) -> Dict[str, Any]:
    ...
    command = (
        os.getenv("CLOVER_COPILOT_ACP_COMMAND", "").strip()
        or os.getenv("COPILOT_CLI_PATH", "").strip()
        or "copilot"
    )
    raw_args = os.getenv("CLOVER_COPILOT_ACP_ARGS", "").strip()
...
def _get_azure_foundry_auth_status() -> Dict[str, Any]:
    ...
    checks:
      ...
      * ``auth_mode == "entra_id"`` AND ``azure-identity`` is importable
        (we do NOT mint a token here; ``clover doctor`` runs the live
    ...
    try:
        from clover_cli.config import load_config, get_env_value_prefer_dotenv
        cfg = load_config()
    except Exception:
        cfg = {}
        ...
        try:
            ...
            if not installed:
                info["hint"] = (
                    ...
                    "pip install azure-identity  (or rely on Clover' "
                    "lazy-install at first use)."
                )
            else:
                info["hint"] = (
                    "azure-identity is installed; live credential validation "
                    "is skipped here. Run `clover doctor` to verify token acquisition."
                )
            return info
        except Exception as exc:
...
def resolve_api_key_provider_credentials(provider_id: str) -> Dict[str, Any]:
    ...
    if provider_id in {"kimi-coding", "kimi-coding-cn"}:
        ...
        try:
            from clover_cli.copilot_auth import (
...
def resolve_external_process_provider_credentials(provider_id: str) -> Dict[str, Any]:
    ...
    if not resolved_command and not base_url.startswith("acp+tcp://"):
        raise AuthError(
            ...
            "Install GitHub Copilot CLI or set CLOVER_COPILOT_ACP_COMMAND/COPILOT_CLI_PATH.",
            provider=provider_id,
...
def _update_config_for_provider(
    ...
    # model.api_key.
    from clover_cli.config import clear_model_endpoint_credentials
...
def _logout_default_provider_from_config() -> Optional[str]:
    ...
    `clover logout` historically keyed off auth.json.active_provider only.
    ...
    if provider in {"clover", "openai-codex", "xai-oauth"}:
        return provider
    return None
...
def _confirm_selection_guards(
    ...
    *,
    ...
    :mod:`clover_cli.model_selection_guards` and shows one [y/N] confirm with
...
def _prompt_model_selection(
    ...
    # Sale chrome (★ / -N% / was) is Clover Portal-only — never for OpenRouter
    # or other providers even if pricing.original is somehow present.
    sale_chrome = (confirm_provider or "").strip().lower() == "clover"

    def _confirmed_selection(mid: str) -> Optional[str]:
        if not mid:
            return None
        # Unified guard registry (clover_cli.model_selection_guards): the cost
    ...
    # Leave room for a leading "★ " on sale rows (Clover only).
    ...
    try:
        ...
        from clover_cli.model_search import model_search_text
    ...
    from clover_cli.curses_ui import format_radio_item_ansi
...
def _save_model_choice(model_id: str) -> None:
    ...
    """
    from clover_cli.config import save_config, load_config
...
def login_command(args) -> None:
    """Deprecated: use 'clover model' or 'clover setup' instead."""
    print("The 'clover login' command has been removed.")
    print("Use 'clover auth' to manage credentials,")
    print("'clover model' to select a provider, or 'clover setup' for full setup.")
    raise SystemExit(0)
...
def _login_openai_codex(
    ...
    *,
    ...
    """OpenAI Codex login via device code flow. Tokens stored in ~/.clover/auth.json."""

    del args, pconfig  # kept for parity with other provider login helpers

    # Check for existing Clover-owned credentials
    if not force_new_login:
        try:
            ...
            if isinstance(_resolved_key, str) and _resolved_key and not _codex_access_token_is_expiring(_resolved_key, 60):
                print("Existing Codex credentials found in Clover auth store.")
                try:
    ...
    if not force_new_login:
        ...
        if cli_tokens:
            ...
            print("Clover will create its own session to avoid conflicts with Codex CLI / VS Code.")
            try:
            ...
            if do_import in {"y", "yes"}:
                ...
                base_url = os.getenv("CLOVER_CODEX_BASE_URL", "").strip().rstrip("/") or DEFAULT_CODEX_BASE_URL
                config_path = _update_config_for_provider("openai-codex", base_url)
                print()
                print("Credentials imported. Note: if Codex CLI refreshes its token,")
                print("Clover will keep working independently with its own session.")
                print(f"  Config updated: {config_path} (model.provider=openai-codex)")
                return

    # Run a fresh device code flow — Clover gets its own OAuth session
    print()
    print("Signing in to OpenAI Codex...")
    print("(Clover creates its own session — won't affect Codex CLI or VS Code)")
    print()

    creds = _codex_device_code_login()

    # Save tokens to Clover auth store
...
def _login_xai_oauth(
    ...
    *,
    ...
    if not force_new_login:
        try:
            ...
            if isinstance(api_key, str) and api_key and not _xai_access_token_is_expiring(api_key, 60):
                print("Existing xAI OAuth credentials found in Clover auth store.")
                try:
                    reuse = input("Use existing credentials? [Y/n]: ").strip().lower()
                except (EOFError, KeyboardInterrupt):
                    reuse = "y"
    ...
    print("(Clover creates its own local OAuth session)")
    print()
    ...
    # xAI credential re-enabled. ``clover auth remove xai-oauth`` leaves a
    # ``device_code`` suppression marker that otherwise stops the singleton
    # seed from re-creating the pool entry, so ``clover auth list`` would show
...
def _minimax_save_auth_state(auth_state: Dict[str, Any]) -> None:
    """Persist MiniMax OAuth state to Clover auth store (~/.clover/auth.json)."""
    with _auth_store_lock():
        auth_store = _load_auth_store()
...
def _minimax_oauth_login(
    *, region: str = "global", open_browser: bool = True,
    ...
    print(f"Starting Clover login via MiniMax ({region}) OAuth...")
...
def _minimax_oauth_quarantine_on_terminal_refresh(state: Dict[str, Any], exc: AuthError) -> None:
    ...
    provider. Mirrors the Clover / xAI-OAuth / Codex-OAuth quarantine pattern
    so subsequent calls fail fast without a network retry.
    """
...
def build_minimax_oauth_token_provider() -> Callable[[], str]:
    ...
    def _provide() -> str:
        ...
        if not state or not state.get("access_token"):
            raise AuthError(
                "Not logged into MiniMax OAuth. Run `clover model` and select "
                "MiniMax (OAuth).",
...
def resolve_minimax_oauth_runtime_credentials(
    *, min_token_ttl_seconds: int = MINIMAX_OAUTH_REFRESH_SKEW_SECONDS,
    ...
    diagnostic call sites like ``clover status`` that just want to know
...
def _nous_device_code_login(
    *,
    ...
    """Run the Clover device-code flow and return full OAuth state without persisting."""
    ...
    print(f"Starting Clover login via {pconfig.name}...")
    print(f"Portal: {portal_base_url}")
    if insecure:
    ...
    try:
        ...
        if exc.code == "subscription_required":
            ...
            print()
            print("After subscribing, run `clover model` again to finish setup.")
            raise SystemExit(1)
        raise
...
def step_up_nous_billing_scope(
    *,
    ...
    """
    prior = get_provider_auth_state("clover") or {}
    pconfig = PROVIDER_REGISTRY["clover"]
...
def _login_nous(args, pconfig: ProviderConfig) -> None:
    ...
    try:
        ...
        if shared:
            ...
            print()
            if shared_path:
                print(f"Found existing Clover OAuth credentials at {shared_path}")
            else:
        ...
        # overwrites it to "clover".  If the user picks "Skip (keep current)"
        ...
        try:
            ...
            if model_ids:
                ...
                if free_tier:
                    ...
                    # as free so users on older Clover builds still see
        ...
        # them to Clover with a mismatched model.  The Clover OAuth tokens
        # stay saved for future use.
        if not selected_model:
            # Restore the prior active_provider that _save_provider_state
            # overwrote to "clover".  config.yaml model.provider is left
            ...
            with _auth_store_lock():
                ...
                _save_auth_store(auth_store)
            print()
            print("No provider change. Clover credentials saved for future use.")
            print("  Run `clover model` again to switch to Clover Portal.")
            return

        config_path = _update_config_for_provider(
            "clover", inference_base_url, default_model=selected_model,
        )
        if selected_model:
...
def logout_command(args) -> None:
    ...
    if clear_provider_auth(target) or should_reset_config:
        ...
        if should_reset_config and os.getenv("OPENROUTER_API_KEY"):
            print("Clover will use OpenRouter for inference.")
        elif should_reset_config:
            print("Run `clover model` or configure an API key to use Clover.")
        else:
