"""Slash command definitions and autocomplete for the Clover CLI.
...
from clover_constants import INDICATOR_STYLES
...
class CommandDef:
    ...
    # Names a key in ``clover_cli.slash_exec.EXECUTORS`` — a pure formatter
    # producing the canonical, surface-independent core text.  Surfaces
    # resolve it via ``clover_cli.slash_exec.run_execute`` and apply only
...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    CommandDef("snapshot", "Create or restore state snapshots of Clover config/state", "Session",
    ...
    CommandDef("goal", "Set a standing goal Clover works on across turns until achieved", "Session",
    ...
    # Canonical name is "fuckit"; "yolo" is kept as an alias for muscle memory.
    # telegram_bot_commands() publishes only canonical names, so the canonical
    # name is what appears in the Telegram `/` menu.
    CommandDef("fuckit", "Toggle FUCKIT mode (skip all dangerous command approvals)",
               "Configuration", aliases=("yolo",), busy_policy="dispatch"),
    ...
    CommandDef("wake", "Toggle the 'Hey Clover' wake word listener", "Configuration",
               cli_only=True, args_hint="[on|off|status]",
               subcommands=("on", "off", "status")),
    CommandDef("busy", "Control what Enter does while Clover is working", "Configuration",
    ...
    CommandDef("reload-mcp", "Reload MCP servers from config", "Tools & Skills",
               aliases=("reload_mcp",)),
    CommandDef("reload-skills", "Re-scan ~/.clover/skills/ for newly installed or removed skills",
...
def _iter_plugin_command_entries() -> list[tuple[str, str, str]]:
    ...
    :func:`clover_cli.plugins.PluginContext.register_command`. They behave
    like ``CommandDef`` entries for gateway surfacing: they appear in the
    Telegram command menu, in Slack's ``/clover`` subcommand mapping, and
...
# Telegram allows up to 100 BotCommands. Clover ships ~50 built-in commands;
...
Telegram only displays a small BotCommand menu in practice.  The full Clover
...
def _collect_gateway_skill_entries(
    ...
    try:
        ...
        # ``clover skills list`` and the agent's ``/skill-name`` dispatch but
...
def telegram_menu_commands(max_commands: int = 100) -> tuple[list[tuple[str, str]], int]:
    ...
    Skills disabled for the ``"telegram"`` platform (via ``clover skills
...
def discord_skill_commands_by_category(
    ...
    *uncategorized*.
    ...
    parity, external-dir skills are visible via ``clover skills list`` and
...
# unrelated command landed. These claim their slots right after /clover,
# ahead of both canonical names and the rest of the aliases. Anything not
# listed here still degrades gracefully (reachable via /clover <command>).
...
# native slot, the alias spelling stays reachable via /clover reset).
...
# ``/clover <command>`` on Slack only. They remain native on every other
...
#   - egress: Docker-only proxy status; reachable as /clover egress on Slack.
#   - init: repo-scan AGENTS.md bootstrap — a cwd-centric dev command that is
#     rare from Slack; reachable as /clover init. Without this entry, adding
#     /init clamps /version off the native list and breaks Telegram parity.
#   - version: low-frequency info command; reachable as /clover version on
...
#   - diff: git working-tree diff; reached via /clover diff on Slack so it
...
#     /clover update on Slack. Demoted to free the native slot /approvals now
...
#   - heartbeat: session heartbeat management; reached via /clover heartbeat
#     on Slack. Added at the 50-cap — a native slot would clamp /insights.
#   - refine: on-demand memory/skill review; reached via /clover refine on
#     Slack. Added at the 50-cap — a native slot would clamp an existing
#     native slash.
#   - pause: global emergency stop; reached via /clover pause [off] on
#     Slack. Added at the 50-cap — a native slot would clamp /platform.
#   - whoami: one-off identity lookup; reached via /clover whoami on Slack.
...
#     /clover platform on Slack. Demoted when /save became gateway-available
#     (session export is an interactive surface; platform is a rare
#     informational lookup) — without this entry /save tips the registry
#     past the 50-cap and silently clamps /platform, breaking parity.
_SLACK_VIA_CLOVER_ONLY = frozenset({"topup", "moa", "debug", "egress", "init", "version", "diff", "update", "heartbeat", "refine", "pause", "whoami", "platform"})
...
def slack_native_slashes() -> list[tuple[str, str, str]]:
    ...
    first-class slash and not a ``/clover <verb>`` subcommand.
    ...
    can still reach them via ``/clover <command>``.

    Results are clamped to Slack's 50-command limit with duplicate-name
    avoidance. ``/clover`` is always reserved as the first entry so the
    legacy ``/clover <subcommand>`` form keeps working for anything that
    ...
    # Reserve /clover as the catch-all top-level command.
    entries.append(("clover", "Talk to Clover or run a subcommand", "[subcommand] [args]"))
    seen.add("clover")
    ...
    def _add(name: str, desc: str, hint: str) -> None:
        ...
        if slack_name in _SLACK_RESERVED_COMMANDS:
            return
        if slack_name in _SLACK_VIA_CLOVER_ONLY:
            # Intentionally Slack-via-/clover only (see _SLACK_VIA_CLOVER_ONLY).
            return
    ...
    # everything except /clover, so a new canonical command can never silently
...
def slack_app_manifest(request_url: str = "slack/commands") -> dict[str, Any]:
...
def slack_subcommand_map() -> dict[str, str]:
    """Return subcommand -> /command mapping for Slack /clover handler.

    Maps both canonical names and aliases so /clover bg do stuff works
    the same as /clover background do stuff.

    Plugin-registered slash commands are included so ``/clover <plugin-cmd>``
...
class SlashCommandCompleter(Completer):
    ...
    def _personality_completions(sub_text: str, sub_lower: str):
        """Yield completions for /personality via clover_cli.personality."""
        try:
