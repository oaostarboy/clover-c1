"""clover memory setup|status — configure memory provider plugins.
...
def _provider_pip_dependencies(provider_name: str, declared: list) -> list:
    ...
    ``clover memory setup`` — if the update-time refresh only reinstalled
    ...
    if provider_name == "hindsight":
        try:
            import json
            cfg_path = get_clover_home() / "hindsight" / "config.json"
...
# ---------------------------------------------------------------------------
# Curses-based interactive picker (same pattern as clover tools)
# ---------------------------------------------------------------------------
...
def _install_dependencies(provider_name: str, *, force: bool = False) -> None:
    ...
    ranges. This is how ``clover update`` heals the active memory provider
    ...
    # data volume (CLOVER_LAZY_INSTALL_TARGET). install_specs handles the
...
def cmd_setup_provider(provider_name: str) -> None:
    ...
    if not match:
        ...
        print("  Run 'clover memory setup' to see available providers.\n")
        return
    ...
    if hasattr(provider, "post_setup"):
        ...
        provider.post_setup(clover_home, config)
        return
...
def cmd_setup(args) -> None:
    ...
    if not providers:
        ...
        print("  Install a plugin to ~/.clover/plugins/ and try again.\n")
        return
...
def cmd_status(args) -> None:
    ...
    if provider_name:
        ...
        if provider:
            ...
            if provider.is_available():
                ...
                if required_fields:
                    ...
                    for f in required_fields:
                        ...
                        print(line)
                print(
                    "  Note: systemd/gateway services do not inherit ~/.clover/.env —"
                )
                print(
            ...
            print(f"  Install the '{provider_name}' memory plugin to ~/.clover/plugins/")
