"""Pydantic request/response models for the Clover dashboard web server.

Extracted verbatim from ``clover_cli/web_server.py`` (pure schema move).
``web_server`` re-exports every name here, so existing imports like
``from clover_cli.web_server import ConfigUpdate`` keep working.
"""
...
class ModelAssignment(BaseModel):
    ...
    # ``clover model`` custom flow collects.
    api_key: str = ""
...
class ImportRequest(BaseModel):
    archive: str
    # Pass --force to `clover import`. The spawned action runs with
...
class ProfileCreate(BaseModel):
    ...
    # via a subprocess scoped to the profile (`clover -p <name> skills install`)
    # because skills_hub.SKILLS_DIR is import-time-bound and the CLOVER_HOME
...
class ProfileExport(BaseModel):
    ...
    # Where to write the archive. Empty → a staging path under CLOVER_HOME.
    output: str = ""
