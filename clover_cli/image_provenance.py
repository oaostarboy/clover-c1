"""Image-authored deployment provenance for immutable Clover runtimes.

The published image bakes ``/etc/clover/image-provenance.json`` outside both
``$CLOVER_HOME`` and the mutable checkout.  A bind-mounted checkout (including
...
IMAGE_PROVENANCE_PATH = Path("/etc/clover/image-provenance.json")
