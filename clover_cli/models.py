...
Add, remove, or reorder entries here — both `clover setup` and
`clover` provider-selection will pick up the change automatically.
"""
...
from clover_cli.urllib_security import open_credentialed_url, url_origin
...
_CLOVER_USER_AGENT = f"clover-cli/{_CLOVER_VERSION}"
...
def _custom_provider_ssl_context(base_url: str):
    ...
    if not base_url:
        return None
    try:
        from clover_cli.config import get_custom_provider_tls_settings
...
def _codex_curated_models() -> list[str]:
    ...
    This keeps the gateway /model picker in sync with the CLI `clover model`
    flow without maintaining a separate static list.
    """
    from clover_cli.codex_models import DEFAULT_CODEX_MODELS, _finalize_codex_models
...
# $CLOVER_HOME/models_dev_cache.json as of 2026-04-28. Whenever xAI renames
...
def _xai_merge_curated_extras(ids: list[str]) -> list[str]:
    """Append Clover-curated xAI models that are missing from models.dev."""
    out = list(ids)
...
def _xai_curated_models() -> list[str]:
    ...
    Reads $CLOVER_HOME/models_dev_cache.json directly (no network). Falls
...
_PROVIDER_MODELS: dict[str, list[str]] = {
    ...
    # endpoint expects (see clover_cli/model_setup_flows.py).
...
# ---------------------------------------------------------------------------
# Clover Portal free-model helper
# ---------------------------------------------------------------------------
# The Clover Portal models endpoint is the source of truth for which models
...
def partition_clover_models_by_tier(
    ...
    """Split Clover models into (selectable, unavailable) based on user tier.
...
def union_with_portal_free_recommendations(
    ...
    *,
    ...
    older Clover that doesn't ship that model in its hardcoded curated
...
def union_with_portal_paid_recommendations(
    ...
    *,
    ...
    picker even if the user is running an older Clover that doesn't ship
...
# ---------------------------------------------------------------------------
# Clover Portal recommended models
#
...
# this list:  clover model, /model, list_authenticated_providers.
#
# Fields:
#   slug        — internal provider ID (used in config.yaml, --provider flag)
#   label       — short display name
#   tui_desc    — longer description for the `clover model` interactive picker
# ---------------------------------------------------------------------------
...
#
# Some vendors expose several Clover provider slugs (one per endpoint /
# auth method: global API, China API, OAuth coding plan, ...). Listing every
# slug as a top-level row in the interactive `clover model` / setup wizard /
...
def group_providers(slugs):
    ...
    DISPLAY ONLY. Used by every interactive picker (``clover model``, the
...
# In-repo fallback for the model Clover silently lands on when the user never
...
def get_preferred_silent_default_model(provider: str = "openrouter") -> str:
    ...
    """
    try:
        from clover_cli.model_catalog import get_default_model_from_cache
...
# curated-list entry [0]. Metered aggregators (Clover Portal, OpenRouter) order
...
def get_default_model_for_provider(provider: str) -> str:
    ...
    model was ever selected (e.g. ``clover auth add openai-codex`` without
    ``clover model``, or a profile that sets ``provider`` with no ``model``).

    For most providers this is the first entry in ``_PROVIDER_MODELS`` — the
    same model the ``clover model`` picker offers first. For metered aggregators
...
def _openrouter_model_supports_tools(item: Any) -> bool:
    ...
    clover-c1 is tool-calling-first — every provider path assumes the model
    ...
    **Permissive when the field is missing.** Some OpenRouter-compatible gateways
    (Clover Portal, private mirrors, older catalog snapshots) don't populate
...
# copy, `clover -p`, a cron job, or a freshly booted gateway answers
...
# the Clover Portal list different models, and a staging Portal must not answer
# for production.
...
def _reasoning_caps_disk_path() -> Path:
    from clover_constants import get_clover_home
    return get_clover_home() / "cache" / "reasoning_caps.json"
...
def _fetch_reasoning_caps_catalog(
    ...
    (OpenRouter itself, Clover Portal). Returns None when the catalog is
    ...
    """
    headers = {"Accept": "application/json", "User-Agent": _CLOVER_USER_AGENT}
    try:
...
def clover_model_reasoning_capabilities(
    ...
    *,
    ...
    """Return live-catalog reasoning capabilities for a Clover Portal model.
...
def fetch_openrouter_models(
    ...
    *,
    ...
    try:
        from clover_cli.model_catalog import get_curated_openrouter_models
    ...
    for preferred_id in preferred_ids:
        ...
        if live_item is None:
            continue
        # Hide models that don't advertise tool-calling support — clover-c1
        ...
        if preferred_id == silent_default:
            ...
            # picker shows which model Clover lands on when none is selected.
            desc = "default"
        else:
...
def get_curated_clover_model_ids() -> list[str]:
    ...
    snapshot in ``_PROVIDER_MODELS["clover"]`` when the manifest is
    ...
    if remote:
        return list(remote)
    return list(_PROVIDER_MODELS.get("clover", []))
...
def fetch_ai_gateway_models(
    ...
    *,
    ...
    from clover_constants import AI_GATEWAY_BASE_URL
...
def compute_sale_discount(
    ...
    Clover Portal-only feature: callers gate on the provider; this helper only
    sees ``original`` because the Clover fetch path opted in via
...
def fetch_models_with_pricing(
    ...
    *,
    ...
    Works with any OpenRouter-compatible endpoint (OpenRouter, Clover Portal).

    When *include_sale_original* is true (Clover Portal only) and the gateway
    ...
    for item in payload.get("data", []):
        ...
        if mid and isinstance(pricing, dict):
            ...
            # Sale chrome is Clover Portal-only. Never copy pricing.original for
...
def fetch_ai_gateway_pricing(
    ...
    *,
    ...
    """Fetch Vercel AI Gateway /v1/models and return clover-shaped pricing.

    Vercel uses ``input`` / ``output`` field names; clover's picker expects
...
_DEFAULT_CLOVER_INFERENCE_BASE = ""
...
def _configured_custom_provider_ids() -> set[str]:
    ...
    try:
        ...
        from clover_cli.providers import custom_provider_slug
...
def list_available_providers() -> list[dict[str, str]]:
    ...
    source of truth shared with ``clover model``, ``/model``, etc.).
    """
    ...
    for pid in provider_order:
        ...
        has_creds = False
        try:
            from clover_cli.auth import get_auth_status, has_usable_secret
            if pid == "custom":
...
def probe_ollama_local_models(
    ...
    try:
        ...
        request_headers = {"User-Agent": _CLOVER_USER_AGENT}
...
def curated_models_for_provider(
    ...
    *,
    ...
    # Try live API first (Codex, Clover, etc. all support /models)
...
def _resolve_static_model_alias(
    ...
    try:
        from clover_cli.model_switch import MODEL_ALIASES
    except Exception:
        return None
...
def normalize_provider(provider: Optional[str]) -> str:
    """Normalize provider aliases to Clover' canonical provider ids.

    Note: ``"auto"`` passes through unchanged — use
    ``clover_cli.auth.resolve_provider()`` to resolve it to a concrete
...
def model_supports_fast_mode(model_id: Optional[str]) -> bool:
    """Return whether Clover should expose the /fast toggle for this model."""
...
def _resolve_copilot_catalog_api_key() -> str:
    ...
    Resolution order:
      ...
      2. ``read_credential_pool("copilot")`` — a token (typically a
         ...
         populated by ``clover auth add copilot`` and by ``_seed_from_env``
         when the env var is set in ``~/.clover/.env``.
...
#   - "clover": curated list and Portal /models endpoint are the source of
...
def _model_dedup_key(model_id: str) -> str:
    ...
    try:
        from clover_cli.model_search import model_alias_canonical
...
def provider_model_ids(provider: Optional[str], *, force_refresh: bool = False) -> list[str]:
    ...
    Tries live API endpoints for providers that support them (Codex, Clover),
    falling back to static lists. For providers in ``_MODELS_DEV_PREFERRED``
    (opencode-go/zen, xiaomi, deepseek, smaller inference providers, etc.),
    models.dev entries are merged on top of curated so new models released
    on the platform appear in ``/model`` without a Clover release.
    """
    requested = str(provider or "").strip().lower()
    if requested == "ollama":
        if force_refresh:
    ...
    if normalized == "openai-codex":
        ...
        # a Clover release). Falls back to the hardcoded catalog if no token
    ...
    if normalized in ("openai", "openai-api"):
        ...
        if api_key:
            ...
            # ``clover model`` shows.
            from clover_cli.providers import is_official_openai_host
...
#
# Cache strategy:
#   - One JSON file at $CLOVER_HOME/provider_models_cache.json
...
def _provider_models_cache_path() -> Path:
    from clover_constants import get_clover_home
    return get_clover_home() / "provider_models_cache.json"
...
def _credential_fingerprint(provider: str) -> str:
    ...
    # env var, so it must change the fingerprint too or `clover config set
...
def _lmstudio_request_headers(api_key: Optional[str] = None) -> dict:
    ...
    headers = {"User-Agent": _CLOVER_USER_AGENT}
...
_COPILOT_MODEL_ALIASES = {
    ...
    # Dash-notation fallbacks: Clover' default Claude IDs elsewhere use
...
def copilot_model_api_mode(
    ...
    *,
    ...
    # endpoint, not through Clover' native Anthropic adapter. The live catalog may
...
def opencode_zen_free_headers() -> dict:
    ...
    """
    try:
        from clover_cli import __version__ as _v
    except Exception:
        _v = "0"
    return {
        "Authorization": "",
...
def _fetch_opencode_free_models(
    ...
    from clover_cli.urllib_security import open_credentialed_url
...
def probe_api_models(
    ...
    tried: list[str] = []
    headers: dict[str, str] = {"User-Agent": _CLOVER_USER_AGENT}
    if urllib.parse.urlparse(normalized).hostname == "generativelanguage.googleapis.com":
        headers["X-Goog-Api-Client"] = f"clover-c1/{_CLOVER_VERSION}"
...
_DEEPINFRA_MODELS_QUERY = "filter=true&sort_by=clover"
...
def _ollama_cloud_cache_path() -> Path:
    ...
    from clover_constants import get_clover_home
    return get_clover_home() / "ollama_cloud_models_cache.json"
    ...
    *,
    ...
    if normalized == "moa":
        try:
            ...
            return {
                ...
                "message": f"MoA preset `{requested}` was not found. Run `clover moa list`.",
            }
        except Exception as exc:
            return {
                "accepted": False, "persist": False, "recognized": False,
        ...
        if ollama_models is None:
            return {
                ...
                "message": (
                    ...
                    "Clover will save the model name, but local Ollama model discovery could not verify it."
                ),
            }
    ...
    if normalized == "custom" or normalized.startswith("custom:"):
        ...
        message = (
            ...
            f"Clover will still save `{requested}`, but the endpoint should expose `/models` for verification."
        )
        if api_mode == "anthropic_messages":
            message += (
    ...
    if normalized in {"openai-codex", "xai-oauth"}:
        ...
        # BEFORE the hidden-slug soft-accept below: the suffix is a Clover
        ...
        if catalog_models:
            ...
            return {
                ...
                "message": (
                    ...
                    f"{suggestion_text}"
                    "\n  MiniMax does not expose a /models endpoint, so Clover cannot verify the model name."
    ...
    if api_models is not None:
        ...
        if _variant_base is not None and _variant_base in set(api_models):
            ...
            # fetch) so we only accept models Clover actually ships.  (#46850)
            #
