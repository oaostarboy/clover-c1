...
Backs ``clover kanban export|import``, the matching ``/boards/{slug}/export``
...
ARCHIVE_FORMAT = "clover-kanban-board"
    ...
    *,
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        manifest = {
            ...
            "clover_version": str(clover_version),
            "includes": {
...
def _read_manifest(root: Path) -> dict[str, Any]:
    ...
    if not path.exists():
        raise ValueError(
            "archive is not a Clover kanban board export (no manifest.json)"
        )
    try:
    ...
    if not isinstance(manifest, dict) or manifest.get("format") != ARCHIVE_FORMAT:
        raise ValueError(
            "archive is not a Clover kanban board export "
    ...
    if not isinstance(version, int) or version > ARCHIVE_FORMAT_VERSION:
        raise ValueError(
            f"archive format version {version!r} is newer than this Clover "
            f"understands (max {ARCHIVE_FORMAT_VERSION}) — update Clover and retry"
        )
    return manifest
    ...
    *,
    ...
    return {
        ...
        "source": {
            ...
            "clover_version": manifest.get("clover_version"),
        },
        "counts": counts,
