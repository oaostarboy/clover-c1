"""``clover logs`` — view and filter Clover log files.
...
Usage examples::
    ...
    clover logs                    # last 50 lines of agent.log
    clover logs -f                 # follow agent.log in real time
    clover logs errors             # last 50 lines of errors.log
    clover logs gateway -n 100    # last 100 lines of gateway.log
    clover logs gui -f            # follow gui.log (dashboard/pty/ws)
    clover logs desktop -f        # follow desktop.log (Electron app boot/backend)
    clover logs --level WARNING    # only WARNING+ lines
    clover logs --session abc123   # filter by session ID substring
    clover logs --component tools  # only tool-related lines
    clover logs --since 1h         # lines from the last hour
    clover logs --since 30m -f     # follow, starting 30 min ago
"""
...
def tail_log(
    ...
    *,
    ...
    log_path = get_clover_home() / "logs" / filename
    if not log_path.exists():
        print(f"Log file not found: {log_path}")
        print("(Logs are created when Clover runs — try 'clover chat' first)")
        sys.exit(1)
    ...
    if follow:
        print(f"--- {display_clover_home()}/logs/{filename}{filter_desc} (Ctrl+C to stop) ---")
    else:
        print(f"--- {display_clover_home()}/logs/{filename}{filter_desc} (last {num_lines}) ---")
...
def list_logs() -> None:
    ...
    if not log_dir.exists():
        print(f"No logs directory at {display_clover_home()}/logs/")
        return

    print(f"Log files in {display_clover_home()}/logs/:\n")
    found = False
    ...
    if not found:
        print("  (no log files yet — run 'clover chat' to generate logs)")
