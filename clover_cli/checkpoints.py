"""`clover checkpoints` CLI subcommand.

Gives users direct visibility and control over the filesystem checkpoint
store at ``~/.clover/checkpoints/``.  Actions:

    clover checkpoints               # same as `status`
    clover checkpoints status        # total size, project count, breakdown
    clover checkpoints list          # per-project checkpoint counts + workdir
    clover checkpoints prune [opts]  # force a sweep (ignores the 24h marker)
    clover checkpoints clear [-f]    # nuke the entire base (asks first)
    clover checkpoints clear-legacy  # delete just the legacy-* archives

Examples::

    clover checkpoints
    clover checkpoints prune --retention-days 3 --max-size-mb 200
    clover checkpoints clear -f
...
from clover_cli.sizefmt import format_bytes as _fmt_bytes
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if legacy:
        ...
        print()
        print("Clear with: clover checkpoints clear-legacy")
    return 0
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire subcommands onto the ``clover checkpoints`` parser."""
    parser.set_defaults(func=cmd_status)  # bare `clover checkpoints` → status
