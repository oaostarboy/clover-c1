"""Profile distributions — shareable, packaged Clover profiles via git.

A distribution is a Clover profile published as a git repository (or
...
* ``clover profile export/import`` — local backup / restore for a profile
  on your own machine. NOT a distribution format. Stays as-is.
* ``clover skills install <url>`` — the URL install pattern we're mirroring,
  but at the profile granularity.

Subcommands (all live under ``clover profile``, not a parallel tree):

    clover profile install <source> [--name N] [--alias] [--force] [--yes]
...
USER_OWNED_EXCLUDE: frozenset = frozenset({
    ...
    "backups", "cache",
    # Infrastructure
    "clover-c1", ".worktrees", "profiles", "bin", "node_modules",
...
class DistributionManifest:
    ...
    def from_dict(cls, data: Any) -> "DistributionManifest":
        ...
        return cls(
            ...
            description=str(data.get("description") or ""),
            clover_requires=str(data.get("clover_requires") or ""),
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        if self.clover_requires:
            out["clover_requires"] = self.clover_requires
        if self.author:
...
def check_clover_requires(spec: str, current_version: str) -> None:
    ...
    if not ok:
        raise DistributionError(
            f"This distribution requires Clover {op}{target}, "
...
def _stage_source(source: str, workdir: Path) -> Tuple[Path, str]:
    ...
    installed manifest's ``source:`` field so ``clover profile update`` can
    ...
    if _looks_like_git_url(src_str):
        ...
        if not (cloned / MANIFEST_FILENAME).is_file():
            raise DistributionError(
                ...
                "This repository is not a Clover profile distribution."
            )
...
def plan_install(
    ...
    if manifest is None:
        raise DistributionError(
            ...
            "this source is not a Clover distribution."
        )

    # Version check up-front so we fail fast
    check_clover_requires(manifest.clover_requires, clover_version)
    ...
    if canon == "default":
        raise DistributionError(
            ...
            "root profile (~/.clover).  Pass --name <name> to install under a "
            "new profile."
        )
    manifest.name = canon
...
def install_distribution(
    ...
    with tempfile.TemporaryDirectory(prefix="clover_dist_install_") as tmp:
        ...
        if plan.existing and not force:
            raise DistributionError(
                ...
                "Use `clover profile update` to upgrade in place, "
...
def update_distribution(
    ...
    if not existing_manifest.source:
        raise DistributionError(
            ...
            "`clover profile install <source> --name {canon} --force`."
        )

    with tempfile.TemporaryDirectory(prefix="clover_dist_update_") as tmp:
        plan = plan_install(
