"""``clover worktree`` — audit and reclaim accumulated git worktrees/branches.

Attended counterpart of the silent startup pruner (see
``clover_cli/worktree_gc.py`` for the policy and shared invariants). Usage:

    clover worktree list             # audit: verdict + reason per tree
    clover worktree prune            # reap safe trees + merged branches
    clover worktree prune --dry-run  # show the plan, change nothing
    clover worktree prune --trees-only / --branches-only
"""
...
def cmd_worktree(args) -> int:
    ...
    if action == "list":
        ...
        print(
            ...
            f"{_fmt_size(reapable_mb)} reclaimable now via `clover worktree prune`."
        )
