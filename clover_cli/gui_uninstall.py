"""
Clover Desktop (Chat GUI) uninstaller.
...
the Python agent or the user's config/data:
  ...
  1. Source-built GUI (``clover desktop`` / ``clover gui``)
     Built inside the agent checkout under ``$CLOVER_HOME/clover-c1/``:
       ...
       - ``$CLOVER_HOME/desktop-build-stamp.json`` (the build freshness stamp)
  ...
  2. Packaged distributable (DMG / NSIS / AppImage / deb / rpm)
     ...
     own bundled Electron + a per-user Electron ``userData`` directory:
       - macOS:   ``/Applications/Clover.app`` or ``~/Applications/Clover.app``
       - Windows: ``%LOCALAPPDATA%\\Programs\\Clover`` (NSIS per-user)
...
the app name ("Clover"), separate from ``$CLOVER_HOME``:
  - macOS:   ``~/Library/Application Support/Clover``
  - Windows: ``%APPDATA%\\Clover``
  - Linux:   ``$XDG_CONFIG_HOME/Clover`` (default ``~/.config/Clover``)
...
``clover uninstall --gui`` (and friends) without paying for the full CLI.
"""
...
def _agent_root(clover_home: Path) -> Path:
    """The agent checkout root — same layout install.sh / install.ps1 use."""
    return clover_home / "clover-c1"


def desktop_userdata_dir() -> Path:
    """Return the Electron ``userData`` directory for the desktop app.

    Mirrors Electron's ``app.getPath('userData')`` for an app named "Clover"
    ...
    if sys.platform == "darwin":
        return home / "Library" / "Application Support" / "Clover"
...
def source_built_gui_artifacts(clover_home: Path) -> "list[Path]":
    """GUI build artifacts produced by ``clover desktop`` inside the checkout.

    These are removable on a GUI uninstall without harming the agent: the
    Python agent runs from ``clover-c1/`` source + ``venv/`` and never
    needs the Electron build output or node_modules.
    """
    agent_root = _agent_root(clover_home)
    ...
    return [
        ...
        clover_home / "desktop-build-stamp.json",
    ]
...
def packaged_gui_app_paths() -> "list[Path]":
    ...
    electron-builder output locations for the "Clover" product.
    """
    home = Path.home()
    paths: list[Path] = []
    if sys.platform == "darwin":
        paths += [
            Path("/Applications/Clover.app"),
        ...
        paths += [
            # NSIS per-user install (perMachine=false → Programs\Clover).
            local_base / "Programs" / "Clover",
            # Older / alternate layout some builds used.
            local_base / "clover-desktop",
        ]
        program_files = os.environ.get("ProgramFiles")
        if program_files:
            # NSIS per-machine fallback (needs admin to remove).
            paths.append(Path(program_files) / "Clover")
    else:
        ...
        # ``uninstall_gui``.
        from clover_cli.linux_desktop_entry import desktop_entry_path

        data = os.environ.get("XDG_DATA_HOME")
        data_base = Path(data) if data else (home / ".local" / "share")
        paths += [
            # The launcher entry `clover desktop` installs. Its icon lives
            # in the checkout, not in the installed app.
            desktop_entry_path(),
            # Some packaged builds emit this casing.
            data_base / "applications" / "Clover.desktop",
        ]
    return paths


def agent_is_installed(clover_home: Path) -> bool:
    """Return True when a usable Python agent install exists under CLOVER_HOME.
    ...
    if (agent_root / "clover_cli").is_dir():
        return True
...
def gui_is_installed(clover_home: Path) -> bool:
    """Return True when any desktop GUI artifact exists (built or packaged)."""
    for p in source_built_gui_artifacts(clover_home):
        if p.exists():
            return True
...
def gui_install_summary(clover_home: "Path | None" = None) -> dict:
    ...
    """
    home: Path = clover_home if clover_home is not None else get_clover_home()
...
def uninstall_gui(clover_home: "Path | None" = None, *, remove_userdata: bool = True) -> "list[Path]":
    ...
    Never touches ``clover-c1/clover_cli`` (agent source), ``venv/``, or any
    config / sessions / .env under ``$CLOVER_HOME``.
    ...
    if sys.platform.startswith("linux"):
        ...
        # Reindex so Clover disappears from the launcher.
        try:
            from clover_cli.linux_desktop_entry import (
                desktop_entry_path,
        ...
        log_info(
            ...
            "with your package manager (e.g. 'sudo apt remove clover' or "
            "'sudo dnf remove clover'). AppImage builds are a single file you "
