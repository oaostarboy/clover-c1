"""Shell completion script generation for clover CLI.
...
def generate_bash(parser: argparse.ArgumentParser) -> str:
    ...
    for cmd in sorted(tree["subcommands"]):
        ...
        if cmd == "profile" and info["subcommands"]:
            ...
            cases.append(
                ...
                f"                    COMPREPLY=($(compgen -W \"$(_clover_profiles)\" -- \"$cur\"))\n"
                f"                    return\n"
                f"                    ;;\n"
                f"            esac\n"
                f"            ;;"
    ...
    return f"""# Clover Cognition bash completion
# Add to ~/.bashrc:
#   eval "$(clover completion bash)"

_clover_profiles() {{
    local profiles_dir="$HOME/.clover/profiles"
...
_clover_completion() {{
    ...
    if [[ "$prev" == "-p" || "$prev" == "--profile" ]]; then
        COMPREPLY=($(compgen -W "$(_clover_profiles)" -- "$cur"))
        return
    fi
...
complete -F _clover_completion clover
"""
...
def generate_zsh(parser: argparse.ArgumentParser) -> str:
    ...
    return f"""#compdef clover
# Clover Cognition zsh completion
# Add to ~/.zshrc:
...
_clover_profiles() {{
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.clover/profiles" ]]; then
        profiles+=($HOME/.clover/profiles/*(N/:t))
    fi
...
_clover() {{
    ...
    _arguments -C \\
        ...
        '(-)'{{-p,--profile}}'[Profile name]:profile:_clover_profiles' \\
        '1:command:->commands' \\
        '*::arg:->args'
...
{top_cmds_str}
            ...
            _describe 'clover command' subcmds
            ;;
        args)
            case ${{line[1]}} in
{sub_cases_str}
...
def generate_fish(parser: argparse.ArgumentParser) -> str:
    ...
    lines: list[str] = [
        "# Clover Cognition fish completion",
        "# Add to your config:",
        "#   clover completion fish | source",
        "",
        "# Helper: list available profiles",
        "function __clover_profiles",
        "    echo default",
        "    if test -d $HOME/.clover/profiles",
        "        for d in $HOME/.clover/profiles/*/",
        "            basename $d",
        "        end",
        "    end",
        "end",
        ...
        "",
        "# Complete profile names after -p / --profile",
        "complete -c clover -f -s p -l profile"
        " -d 'Profile name' -xa '(__clover_profiles)'",
        "",
        ...
        if cmd == "profile":
            for action in sorted(profile_name_actions):
                lines.append(
                    ...
                    f"-a '(__clover_profiles)' -d 'Profile name'"
                )
