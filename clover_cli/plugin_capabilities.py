...
this. Capabilities govern the *host API surfaces* Clover hands out (which
...
until the user re-consents (``clover plugins update`` surfaces the diff).
...
def parse_declared_capabilities(raw: Any, plugin_name: str = "?") -> List[str]:
    ...
    for a newer Clover may declare ids this build doesn't know; they can
