"""MCP picker — interactive `clover mcp picker` (also the default `clover mcp`).

Lists every catalog entry plus any custom MCP servers the user has added via
``clover mcp add``, lets them pick one, and routes to install / enable /
disable / uninstall / configure-tools flows.

Mirrors the `clover plugin` picker UX: arrow keys to navigate, ENTER on a row
...
from clover_cli.cli_output import prompt_yes_no
from clover_cli.curses_ui import curses_single_select
from clover_cli.mcp_catalog import (
    CatalogEntry,
    CatalogError,
    catalog_diagnostics,
    install_entry,
...
def _enable_disable(name: str, *, enable: bool) -> None:
    ...
    print(color(
        ...
        "Start a new Clover session for changes to take effect.",
        Colors.GREEN,
    ))
...
def _configure_tools(name: str) -> None:
    ...
    """
    import argparse
    from clover_cli.mcp_config import cmd_mcp_configure
...
def _print_rows_text(rows: List[_Row]) -> None:
    ...
    as the default output of `clover mcp catalog`."""
    if not rows:
        print()
    ...
    print()
    print(color(
        "  Install: clover mcp install <name>    Picker: clover mcp",
        Colors.DIM,
    ))

    # Surface manifest-version warnings so users know when their Clover is
    ...
    if future:
        ...
        for name, _, msg in future:
            print(color(
                f"  ⚠ '{name}' requires a newer Clover — run `clover update` "
...
def show_catalog() -> None:
    """`clover mcp catalog` — print the curated list + custom servers, no interaction."""
    _print_rows_text(_build_rows())


def run_picker() -> None:
    """`clover mcp picker` (and default `clover mcp`) — interactive selector.
...
def install_by_name(identifier: str) -> int:
    """`clover mcp install <name>` — non-interactive entry-point.
    ...
    if entry is None:
        print(color(
            ...
            "Run `clover mcp catalog` to see available entries.",
            Colors.RED,
        ))
        return 1
    try:
