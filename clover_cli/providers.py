"""
Single source of truth for provider identity in Clover Cognition.
...
2. **Clover overlays** — transport type, auth patterns, aggregator flags,
...
# -- Clover overlay ----------------------------------------------------------
# Clover-specific metadata that models.dev doesn't provide.

@dataclass(frozen=True)
class CloverOverlay:
    """Clover-specific provider metadata layered on top of models.dev."""
...
CLOVER_OVERLAYS: Dict[str, CloverOverlay] = {
    "moa": CloverOverlay(
        transport="openai_chat",
        auth_type="virtual",
        base_url_override="moa://local",
    ),
    "openrouter": CloverOverlay(
        transport="openai_chat",
        is_aggregator=True,
    ...
    "openai-api": CloverOverlay(
        transport="codex_responses",
        base_url_override="https://api.openai.com/v1",
        base_url_env_var="OPENAI_BASE_URL",
    ),
    "xai-oauth": CloverOverlay(
        transport="codex_responses",
        auth_type="oauth_external",
        base_url_override="https://api.x.ai/v1",
        base_url_env_var="XAI_BASE_URL",
    ),
    "qwen-oauth": CloverOverlay(
        transport="openai_chat",
        auth_type="oauth_external",
        base_url_override="https://portal.qwen.ai/v1",
        base_url_env_var="CLOVER_QWEN_BASE_URL",
    ),
    "lmstudio": CloverOverlay(
        transport="openai_chat",
        auth_type="api_key",
    ...
    "copilot-acp": CloverOverlay(
        transport="codex_responses",
        auth_type="external_process",
        base_url_override="acp://copilot",
        base_url_env_var="COPILOT_ACP_BASE_URL",
    ),
    "github-copilot": CloverOverlay(
        transport="openai_chat",
        extra_env_vars=("COPILOT_GITHUB_TOKEN", "GH_TOKEN"),
    ),
    "anthropic": CloverOverlay(
    ...
    "kimi-for-coding": CloverOverlay(
        transport="openai_chat",
        base_url_env_var="KIMI_BASE_URL",
    ),
    "stepfun": CloverOverlay(
        transport="openai_chat",
        extra_env_vars=("STEPFUN_API_KEY",),
        base_url_override="https://api.stepfun.ai/step_plan/v1",
        base_url_env_var="STEPFUN_BASE_URL",
    ),
    "minimax": CloverOverlay(
        transport="anthropic_messages",
        base_url_env_var="MINIMAX_BASE_URL",
    ),
    "minimax-oauth": CloverOverlay(
        transport="anthropic_messages",
        auth_type="oauth_external",
        base_url_override="https://api.minimax.io/anthropic",
    ),
    "minimax-cn": CloverOverlay(
        transport="anthropic_messages",
        base_url_env_var="MINIMAX_CN_BASE_URL",
    ),
    "deepseek": CloverOverlay(
        transport="openai_chat",
        base_url_env_var="DEEPSEEK_BASE_URL",
    ),
    "alibaba": CloverOverlay(
        transport="openai_chat",
        base_url_env_var="DASHSCOPE_BASE_URL",
    ),
    "alibaba-coding-plan": CloverOverlay(
        transport="openai_chat",
        base_url_env_var="ALIBABA_CODING_PLAN_BASE_URL",
    ),
    "vercel": CloverOverlay(
        transport="openai_chat",
        is_aggregator=True,
    ),
    "opencode": CloverOverlay(
        transport="openai_chat",
        is_aggregator=True,
        base_url_env_var="OPENCODE_ZEN_BASE_URL",
    ),
    "opencode-go": CloverOverlay(
        transport="openai_chat",
        is_aggregator=True,
        base_url_env_var="OPENCODE_GO_BASE_URL",
    ),
    "opencode-free": CloverOverlay(
        transport="openai_chat",
        is_aggregator=True,
    ...
    "huggingface": CloverOverlay(
        transport="openai_chat",
        is_aggregator=True,
        base_url_env_var="HF_BASE_URL",
    ),
    "novita": CloverOverlay(
        transport="openai_chat",
        is_aggregator=True,
    ...
    "nvidia": CloverOverlay(
        transport="openai_chat",
        base_url_override="https://integrate.api.nvidia.com/v1",
        base_url_env_var="NVIDIA_BASE_URL",
    ),
    "xiaomi": CloverOverlay(
        transport="openai_chat",
        base_url_env_var="XIAOMI_BASE_URL",
    ),
    "tencent-tokenhub": CloverOverlay(
        transport="openai_chat",
        base_url_env_var="TOKENHUB_BASE_URL",
    ),
    "tencent-tokenplan": CloverOverlay(
        transport="anthropic_messages",
    ...
    "azure-foundry": CloverOverlay(
        transport="openai_chat",  # default; overridden by api_mode in config
        base_url_env_var="AZURE_FOUNDRY_BASE_URL",
    ),
    "bedrock": CloverOverlay(
...
class ProviderDef:
    ...
    doc: str = ""
    source: str = ""                      # "models.dev", "clover", "user-config"
...
def get_provider(name: str, *, allow_network: bool = True) -> Optional[ProviderDef]:
    ...
    overlay = CLOVER_OVERLAYS.get(canonical)
    ...
    if mdev_info is not None:
        ...
        # Combine env vars: models.dev env + clover extra
    ...
    if overlay is not None:
        # Clover-only provider (not in models.dev)
        return ProviderDef(
            id=canonical,
    ...
    # tencent-tokenhub) are absent from models.dev and CLOVER_OVERLAYS, so
...
def clover_api_mode(model: str = "") -> str:
    ...
    historical Clover transport — so callers that don't yet know the model
...
def determine_api_mode(provider: str, base_url: str = "", model: str = "") -> str:
    ...
    Resolution order:
      ...
      2. Clover Portal dual-wire (model-derived; overlay alone is openai_chat).
      3. Known provider → transport → TRANSPORT_TO_API_MODE.
      4. Direct provider checks (bedrock).
      5. Default: 'chat_completions'.

    *model* is optional but required for dual-wire providers (Clover) whose
    ...
    # Clover is dual-wire: anthropic/* → Messages, everything else →
    # chat_completions. The Clover overlay still advertises openai_chat
    ...
    # Direct provider checks for providers not in CLOVER_OVERLAYS
    if provider == "bedrock":
        return "bedrock_converse"
...
def resolve_provider_full(
    ...
    # 0.5 Exact Clover provider IDs must win over LOSSY alias collapsing.
    ...
    if canonical != raw:
        try:
            from clover_cli.auth import PROVIDER_REGISTRY as _AUTH_PROVIDER_REGISTRY
