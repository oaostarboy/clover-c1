...
_TASK_NAME_DEFAULT = "Clover_Gateway"
_TASK_DESCRIPTION = "Clover Cognition Gateway - Messaging Platform Integration"
...
def _preserve_clover_home_path(path: str | Path) -> str:
    """Render Clover-owned paths under the configured CLOVER_HOME spelling.

    Windows installs may keep ``%LOCALAPPDATA%\\clover`` as a symlink/junction to
    ...
    path lives under CLOVER_HOME.
    """
    candidate = Path(path)
    try:
        from clover_cli.config import get_clover_home

        home = Path(get_clover_home())
...
def _current_profile_cli_args() -> list[str]:
    """Return CLI args that preserve the current Clover profile."""
    from clover_cli.gateway import _profile_arg
...
def _launch_elevated_gateway_command(command: str, extra_args: list[str] | None = None) -> bool:
    ...
    """
    _assert_windows()
    args = ["-m", "clover_cli.main", *_current_profile_cli_args(), "gateway", command]
    if extra_args:
...
def _launch_elevated_install(
    ...
    *,
    ...
    old_start_now = os.environ.get("CLOVER_GATEWAY_INSTALL_START_NOW")
    old_start_on_login = os.environ.get("CLOVER_GATEWAY_INSTALL_START_ON_LOGIN")
    old_handoff = os.environ.get("CLOVER_GATEWAY_ELEVATED_HANDOFF")
    try:
        if start_now is not None:
            os.environ["CLOVER_GATEWAY_INSTALL_START_NOW"] = "1" if start_now else "0"
        if start_on_login is not None:
            os.environ["CLOVER_GATEWAY_INSTALL_START_ON_LOGIN"] = "1" if start_on_login else "0"
        os.environ["CLOVER_GATEWAY_ELEVATED_HANDOFF"] = "1"
        ...
        for key, old in (
            ("CLOVER_GATEWAY_INSTALL_START_NOW", old_start_now),
            ("CLOVER_GATEWAY_INSTALL_START_ON_LOGIN", old_start_on_login),
            ("CLOVER_GATEWAY_ELEVATED_HANDOFF", old_handoff),
        ):
            if old is None:
...
def get_task_name() -> str:
    ...
    Default profile: ``Clover_Gateway``
    Named profile X: ``Clover_Gateway_<X>``
    """
    _assert_windows()
    # Local import to avoid circular module initialization during clover_cli boot.
    from clover_cli.gateway import _profile_suffix
...
def get_task_script_path() -> Path:
    ...
    Lives under ``%LOCALAPPDATA%\\clover\\gateway-service\\<task_name>.cmd``
    (or ``<CLOVER_HOME>/gateway-service/<task_name>.cmd`` so per-profile
    Clover installs stay self-contained).
    """
    _assert_windows()
    from clover_cli.config import get_clover_home

    script_dir = Path(get_clover_home()) / "gateway-service"
...
def _stable_gateway_working_dir(project_root: Path) -> str:
    ...
    Mirror the POSIX service invariant: anchor at ``CLOVER_HOME`` whenever it
    ...
    source checkout only if ``CLOVER_HOME`` cannot be used yet. Preserve the
...
def _build_gateway_cmd_script(
    ...
    The script:
      ...
      - exports CLOVER_HOME, PYTHONIOENCODING, VIRTUAL_ENV
      - invokes ``python -m clover_cli.main [--profile X] gateway run``
    ...
    to that terminal like a normal foreground ``clover gateway run``.
    ...
    lines.append(f'set "CLOVER_HOME={clover_home}"')
    lines.append('set "PYTHONIOENCODING=utf-8"')
    lines.append('set "CLOVER_GATEWAY_DETACHED=1"')
    python_exe_path, venv_dir, extra_pythonpath = _resolve_detached_python(python_path)
    # VIRTUAL_ENV lets the gateway's own python detection find the venv
    # if someone imports clover_constants-based logic during startup.
    lines.append(f'set "VIRTUAL_ENV={_preserve_clover_home_path(venv_dir)}"')
    pythonpath_entries = [
        _preserve_clover_home_path(Path(__file__).resolve().parent.parent),
        *[_preserve_clover_home_path(entry) for entry in extra_pythonpath],
    ]
    lines.append(f'set "PYTHONPATH={";".join([*pythonpath_entries, "%PYTHONPATH%"])}"')

    prog_args = [python_exe_path, "-m", "clover_cli.main"]
    if profile_arg:
...
def _build_gateway_vbs_script(
    ...
    repo_root = _preserve_clover_home_path(Path(__file__).resolve().parent.parent)
    static_pythonpath = os.pathsep.join(
        [repo_root, *[_preserve_clover_home_path(entry) for entry in extra_pythonpath]]
    )
    ...
    lines = [
        ...
        f"env.Item({_quote_vbs_string('CLOVER_HOME')}) = {_quote_vbs_string(clover_home)}",
        f"env.Item({_quote_vbs_string('PYTHONIOENCODING')}) = {_quote_vbs_string('utf-8')}",
        f"env.Item({_quote_vbs_string('CLOVER_GATEWAY_DETACHED')}) = {_quote_vbs_string('1')}",
        f"env.Item({_quote_vbs_string('VIRTUAL_ENV')}) = {_quote_vbs_string(_preserve_clover_home_path(venv_dir))}",
...
def _write_task_script() -> Path:
    ...
    python_path = _preserve_clover_home_path(get_python_path())
    working_dir = _stable_gateway_working_dir(PROJECT_ROOT)
    clover_home = str(Path(get_clover_home()))
    profile_arg = _profile_arg(clover_home)

    content = _build_gateway_cmd_script(python_path, working_dir, clover_home, profile_arg)
    ...
    vbs_content = _build_gateway_vbs_script(python_path, working_dir, clover_home, profile_arg)
...
def _install_scheduled_task(task_name: str, script_path: Path) -> tuple[bool, str]:
    ...
    Always recreate instead of ``/Change``. Older Clover builds and failed
...
def _build_gateway_argv() -> tuple[list[str], str, dict[str, str]]:
    ...
    python_exe, venv_dir, extra_pythonpath = _resolve_detached_python(
        _preserve_clover_home_path(get_python_path())
    )
    project_root = _preserve_clover_home_path(PROJECT_ROOT)
    working_dir = _stable_gateway_working_dir(PROJECT_ROOT)
    clover_home = str(Path(get_clover_home()))
    profile_arg = _profile_arg(clover_home)

    argv = [python_exe, "-m", "clover_cli.main"]
    if profile_arg:
    ...
    env_overlay = {
        ...
        "CLOVER_GATEWAY_DETACHED": "1",
        "VIRTUAL_ENV": _preserve_clover_home_path(venv_dir),
    }
    _prepend_pythonpath(
        env_overlay,
        [project_root, *[_preserve_clover_home_path(entry) for entry in extra_pythonpath]]
        if extra_pythonpath
        else [project_root],
    )
...
def windowless_gateway_restart_spec(
    ...
    supplies the stable cwd + env overlay (CLOVER_HOME, VIRTUAL_ENV,
    PYTHONPATH) so the respawn doesn't depend on the watcher's transient
    working directory.

    Returns ``(new_argv, working_dir, env_overlay)``.  ``new_argv``
    preserves every argument after the interpreter (``-m clover_cli.main
    ...
    from clover_cli.config import get_clover_home
    from clover_cli.gateway import PROJECT_ROOT
    ...
    try:
        clover_home = str(Path(get_clover_home()).resolve())
    except Exception:
        clover_home = ""
    ...
    if clover_home:
        env_overlay["CLOVER_HOME"] = clover_home
    _prepend_pythonpath(
        env_overlay,
...
def _spawn_detached(script_path: Path | None = None) -> int:
    ...
    We spawn ``python.exe -m clover_cli.main gateway run`` directly — NOT
    ...
    log_dir = Path(get_clover_home()) / "logs"
...
def _prompt_install_choices(
    ...
    env_start_now = _install_choice_from_env("CLOVER_GATEWAY_INSTALL_START_NOW")
    env_start_on_login = _install_choice_from_env("CLOVER_GATEWAY_INSTALL_START_ON_LOGIN")
    ...
    from clover_cli.setup import prompt_yes_no
...
def _install_startup_fallback(script_path: Path, start_now: bool, detail: str) -> None:
    ...
    # Re-running `clover -p <profile> gateway install` must be safe.
    ...
    from clover_cli.gateway import find_gateway_pids, _profile_arg
    ...
    if running_pids:
        ...
        profile_arg = _profile_arg()
        start_cmd = f"clover {profile_arg} gateway start" if profile_arg else "clover gateway start"
    ...
    *,
    ...
    if not start_on_login:
        ...
        if start_now:
            ...
            print("  Run later with: clover gateway start")
        return
    ...
    if not _is_running_as_admin() and not elevated_handoff:
        ...
        if prompt_yes_no("  Open the UAC prompt now?", False):
            if _launch_elevated_install(force=force, start_now=start_now, start_on_login=start_on_login):
                print("✓ Launched elevated Clover gateway install prompt.")
                if start_now:
                    print("  Approve the Windows UAC prompt; the elevated install will start the gateway afterwards.")
                else:
                    print("  Approve the Windows UAC prompt, then run: clover gateway status")
                return
    ...
    if ok:
        ...
        if start_now:
            ...
            print("  Start manually with: clover gateway start")
        _print_next_steps()
        return
...
def _report_gateway_start(via: str) -> None:
    ...
    if pids:
        ...
        print(f"    type {Path(get_clover_home())}\\logs\\gateway-stdio.log")


def _print_next_steps() -> None:
    from clover_cli.config import get_clover_home

    clover_home = Path(get_clover_home())
    print()
    print("Next steps:")
    print("  clover gateway status                      # Check status")
    print(f"  type {clover_home}\\logs\\gateway.log       # View logs")
        ...
        if code == 0:
            ...
            if prompt_yes_no("  Open the UAC prompt now?", False):
                if _launch_elevated_uninstall():
                    print("✓ Launched elevated Clover gateway uninstall prompt.")
                    print("  Approve the Windows UAC prompt, then run: clover gateway status")
                    return
...
def start() -> None:
    ...
    if not task_installed and not startup_installed:
        ...
        if not prompt_yes_no("  Install it now so the gateway starts on login?", True):
            print("  Run: clover gateway install")
            return
        install(force=False)
        task_installed = is_task_registered()
        startup_installed = is_startup_entry_installed()
        if not task_installed and not startup_installed:
            ...
            print("  If a UAC prompt opened, approve it, then run: clover gateway start")
            return
