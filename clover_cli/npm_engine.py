...
Scope of the repair is deliberately narrow. Clover only upgrades an npm that
lives inside its **own** managed Node tree (``$CLOVER_HOME/node``), installing
...
and their other projects; Clover never modifies those. When the failing npm is
one of those foreign installs, Clover instead provisions its own managed Node
...
def managed_npm_prefix(npm: str | os.PathLike[str] | None) -> Path | None:
    """Return the Clover-managed Node root *npm* lives in, else ``None``.

    Symlinks are resolved first: an install links ``~/.local/bin/npm`` at
    ``$CLOVER_HOME/node/bin/npm``, which itself links into
...
def upgrade_managed_npm(
    ...
    *,
    ...
    ``prefix=~/.local`` into ``$CLOVER_HOME/node/etc/npmrc`` so that global
    ...
    if not quiet:
        print(
            f"→ Upgrading Clover-managed npm to satisfy {npm_range}…",
            flush=True,
        )
    ...
    if managed_node_tree_in_use():
        if not quiet:
            print(
                "  ⚠ deferred: the Clover-managed Node.js tree is in use by a "
    ...
    try:
        ...
        with tempfile.TemporaryDirectory(prefix="clover-npm-upgrade-") as tmp:
...
def _print_manual_fix(npm: str, npm_range: str, actual: str | None) -> None:
    ...
    print(
        ...
        f"  Resolved npm: {npm}\n"
        "  Clover could not provision its own Node.js runtime and never\n"
...
def _provision_managed_npm(npm_range: str | None, *, quiet: bool = False) -> str | None:
    """Provision a Clover-managed Node tree and return a satisfying npm.

    Installs the managed tree under ``$CLOVER_HOME/node`` (reusing a healthy
    ...
    if not quiet:
        print(
            "→ Provisioning a Clover-managed Node.js runtime "
            "(the resolved npm belongs to your system and is left alone)…",
            flush=True,
        )
    managed_npm = bootstrap_clover_managed_node()
    if not managed_npm:
        if not quiet:
...
def maybe_repair_npm_engine(
    ...
    *output* is the combined stdout/stderr of the npm command that just failed.
    ...
    the same *npm* after an in-place upgrade of a Clover-managed install, or
    ...
    if prefix is not None:
        # Clover owns this npm — upgrade it in place. Only an npm-range
