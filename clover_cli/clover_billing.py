"""Clover Portal Remote Spending HTTP client (Phase 2b).

Thin, fail-loud client for the four ``/api/billing/*`` endpoints the terminal
billing screens drive. Companion to ``clover_cli/clover_account.py`` (which owns
...
- **Auth** = the OAuth bearer JWT Clover already holds for inference
  (``get_provider_auth_state("clover")["access_token"]``). No API-key auth on these.
- **Portal base URL** resolves with the same precedence as the device-flow login
  (``auth.py``): ``CLOVER_PORTAL_BASE_URL`` → ``CLOVER_PORTAL_BASE_URL`` → the
...
# clover_cli.auth.CLOVER_BILLING_MANAGE_SCOPE (kept here too so this module has no
...
def resolve_portal_base_url(state: Optional[dict[str, Any]] = None) -> str:
    ...
    ``CLOVER_PORTAL_BASE_URL`` → ``CLOVER_PORTAL_BASE_URL`` → stored auth-state
    ``portal_base_url`` → registry default. Trailing slash stripped.
    """
    env = os.getenv("CLOVER_PORTAL_BASE_URL") or os.getenv("CLOVER_PORTAL_BASE_URL")
...
# Short-lived cache for the resolved (token, base). `resolve_clover_access_token`
...
def _resolve_token_and_base(*, use_cache: bool = True) -> tuple[str, str]:
    ...
    (``resolve_clover_access_token``), so a short-lived (~15 min) access token that
    ...
    when there is no usable Clover session at all.
    ...
    try:
        token = resolve_clover_access_token()
...
def _request(
    ...
    *,
    ...
    try:
        ...
        raise BillingError(
            f"Could not reach Clover Portal: {exc.reason}", error="network_error"
        ) from exc
        ...
        raise BillingError(
            "Could not reach Clover Portal: timed out", error="network_error"
        ) from exc
