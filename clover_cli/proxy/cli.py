"""CLI handlers for the ``clover proxy`` subcommand."""
...
from clover_cli.proxy.adapters import ADAPTERS, get_adapter
from clover_cli.proxy.server import (
    AIOHTTP_AVAILABLE,
    DEFAULT_HOST,
    DEFAULT_PORT,
    run_server,
...
def _print_aiohttp_missing() -> None:
    print(
        "clover proxy requires aiohttp. Run `clover setup` to install it.",
        file=sys.stderr,
    )
...
def cmd_proxy_start(args: Any) -> int:
    ...
    provider = getattr(args, "provider", None) or "clover"
    try:
    ...
    if not adapter.is_authenticated():
        auth_hint = getattr(adapter, "auth_hint", f"clover auth add {adapter.name}")
        print(
    ...
    print(
        f"Starting Clover proxy for {adapter.display_name}\n"
...
def cmd_proxy_status(args: Any) -> int:
    ...
    print("Clover proxy upstream adapters\n")
    ...
    print(
        "\nStart the proxy with: clover proxy start [--provider <name>]"
    )
    return 0
...
def cmd_proxy(args: Any) -> int:
    """Dispatch ``clover proxy <subcommand>``."""
    ...
    print(
        "clover proxy — local OpenAI-compatible proxy that attaches your\n"
