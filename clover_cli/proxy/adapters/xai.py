...
# xAI's public API is OpenAI-compatible for the endpoints Clover commonly
# uses. The Responses endpoint is included because Clover' native xAI runtime
...
class XAIGrokAdapter(UpstreamAdapter):
    """Proxy upstream for xAI Grok via Clover-managed OAuth credentials."""

    auth_hint = "clover auth add xai-oauth --type oauth"
        ...
        with self._lock:
            ...
            if pool is None or not pool.has_credentials():
                raise RuntimeError(
                    ...
                    "`clover auth add xai-oauth --type oauth` first."
                )
            ...
            if entry is None:
                raise RuntimeError(
                    ...
                    "`clover auth reset xai-oauth` or re-authenticate with "
                    "`clover auth add xai-oauth --type oauth`."
                )
    ...
    def _credential_from_entry(self, entry: PooledCredential) -> UpstreamCredential:
        ...
        if not bearer:
            raise RuntimeError(
                ...
                "Re-authenticate with `clover auth add xai-oauth --type oauth`."
            )
