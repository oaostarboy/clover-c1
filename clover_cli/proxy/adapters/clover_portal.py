"""Clover Portal upstream adapter.

Reads the user's Clover OAuth state from ``~/.clover/auth.json`` through the
...
from clover_cli.auth import (
    ...
    _load_auth_store,
    _auth_store_lock,
    _is_terminal_clover_refresh_error,
...
from clover_cli.proxy.adapters.base import UpstreamAdapter, UpstreamCredential
...
class CloverPortalAdapter(UpstreamAdapter):
    """Proxy upstream for the Clover Portal inference API."""

    def __init__(self) -> None:
        # Serialize proxy requests in this process; cross-process token refresh
        # and persistence are handled by resolve_clover_runtime_credentials().
    ...
    def get_retry_credential(
        ...
        *,
        ...
        if status_code != 401:
            return None
        logger.info("proxy: Clover upstream rejected bearer; force-refreshing invoke JWT")
        return self._get_credential(
            force_refresh=True,
        )
    ...
    def _get_credential(
        ...
        *,
        ...
        with self._lock:
            ...
            if state is None:
                raise RuntimeError(
                    "Not logged into Clover Portal. Run `clover auth add clover` first."
                )

            try:
                refreshed = resolve_clover_runtime_credentials(
                ...
                raise RuntimeError(
                    f"Failed to refresh Clover Portal credentials: {exc}"
                ) from exc
            ...
            if not runtime_key:
                raise RuntimeError(
                    "Clover Portal refresh did not return a usable inference JWT. "
                    "Try `clover auth add clover` to re-authenticate."
                )

            # base_url returned by resolve_clover_runtime_credentials() already
            # honors the CLOVER_INFERENCE_BASE_URL env override (the documented
            ...
            base_url = (
                _clover_inference_env_override()
                or _validate_clover_inference_url_from_network(refreshed.get("base_url"))
    ...
    # to clover_cli.auth to avoid expanding that module's public surface.
    # ------------------------------------------------------------------
        ...
        *,
        ...
        try:
            with _auth_store_lock():
                ...
                providers["clover"] = state
                _save_auth_store(store)
            _write_shared_clover_state(state)
        except Exception as exc:
            logger.warning("proxy: failed to persist Clover quarantine state: %s", exc)


__all__ = ["CloverPortalAdapter"]
