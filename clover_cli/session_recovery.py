"""Offline, non-destructive recovery for a damaged Clover session database.
...
def _copy_source_bundle(source: Path, snapshot_dir: Path) -> tuple[Path, list[str]]:
    ...
    locks -- the failure class ``clover_cli.sqlite_safe_read`` exists to
    ...
    keeps this path consistent with ``clover_state._backup_db_file``.
    """
    from clover_cli.sqlite_safe_read import LiveConnectionError, offline_file_access
...
def _snapshot_and_inspect(
    ...
    temp_dir = tempfile.TemporaryDirectory(
        prefix="clover-session-recovery-",
        dir=str(work_root),
    )
    ...
    try:
        ...
        if before != after:
            raise SessionRecoverySafetyError(
                ...
                "Stop every Clover process using this profile and retry. "
                "This includes the interactive `clover` CLI session this "
                ...
                "Clover process even after the gateway is stopped. Run the "
                "recovery from a fresh shell with no `clover` session open, "
...
def _recover_via_lost_and_found(
    *,
    ...
    from clover_cli.session_lost_and_found import (
