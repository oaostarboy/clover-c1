"""``clover doctor --live`` — opt-in bounded real-call tool-backend probes.
...
def _browser_available() -> bool:
    ...
    if shutil.which("agent-browser"):
        return True
    try:
        from clover_cli.doctor import CLOVER_HOME, PROJECT_ROOT

        if (PROJECT_ROOT / "node_modules" / "agent-browser").exists():
            return True
        for candidate in (CLOVER_HOME / "node" / "bin",
                          CLOVER_HOME / "node",
                          CLOVER_HOME / "node_modules" / ".bin"):
    ...
    # clover_cli.doctor uses so this probe can't diverge from it, including
...
def _probe_mcp_server(name: str, config: dict, timeout: float):
    ...
    Reuses the exact machinery behind ``clover mcp test``.
    """
