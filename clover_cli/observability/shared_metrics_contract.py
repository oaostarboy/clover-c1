"""Bounded product contract for the first Clover shared-metrics slice."""
...
SCHEMA_KEY = "clover.metrics.schema_version"
SCHEMA_VERSION = "clover.metrics.event.v2"
MODEL_CALL_SCOPE = "clover.model_call"
MODEL_CALL_PROFILE_MODEL = "unknown"
TASK_SCOPE = "clover.task_run"
TOOL_CALL_SCOPE = "clover.tool_call"
CLIENT_ACTIVE_MARK = "clover.client.active"
TOOL_APPROVAL_MARK = "clover.tool_approval"
SKILL_LIFECYCLE_MARK = "clover.skill.lifecycle"
SKILL_LOAD_MARK = "clover.skill.load"
SUBSCRIBER_NAME = "clover.nemo_relay.shared_metrics"
CLIENT_ACTIVE_METRIC = "clover.client.active"
LEGACY_MODEL_CALL_METRIC = "clover.model_call.count"
MODEL_ROUTE_METRIC = "clover.model_route.count"
TASK_STARTED_METRIC = "clover.task_run.started"
TASK_FINISHED_METRIC = "clover.task_run.finished"
TOOL_CALL_METRIC = "clover.tool_call.count"
TOOL_APPROVAL_METRIC = "clover.tool_approval.count"
SKILL_LIFECYCLE_METRIC = "clover.skill.lifecycle.count"
SKILL_LOAD_METRIC = "clover.skill.load.count"
...
def client_install_method(value: Any) -> str:
    """Return an allowlisted Clover installation method."""
    normalized = str(value or "").strip().lower()
    if normalized == "nix":
        return "nixos"
...
def client_resource(
    ...
    *,
    ...
    normalized_version = str(clover_version or "").strip()
    ...
    return {
        ...
        "clover_version": normalized_version,
...
def client_resource_is_valid(resource: Any) -> bool:
    ...
    if not isinstance(resource, dict) or set(resource) != CLIENT_RESOURCE_KEYS:
        return False
    version = resource.get("clover_version")
    return (
...
def _auxiliary_model_call_dimensions(event: Any) -> dict[str, str] | None:
    """Project a terminal auxiliary route from its Clover logical scope."""
    metadata = getattr(event, "metadata", None)
    if (
    ...
    if relay_metadata - {"otel.status_code"} or metadata.get(
        ...
        return None
    call_role = metadata.get("clover.call_role")
...
def execution_surface(kwargs: dict[str, Any]) -> str:
    ...
    try:
        from clover_cli.platforms import get_all_platforms
...
def task_terminal_state(kwargs: dict[str, Any]) -> tuple[str, str, str]:
    """Map Clover terminal state to bounded task outcome dimensions."""
...
def tool_category(kwargs: dict[str, Any]) -> str:
    """Map Clover registry toolset metadata to a low-cardinality category."""
    ...
    if toolset.startswith(
        ("discord", "email", "feishu", "clover-yuanbao", "slack", "sms")
    ):
...
def tool_outcome(kwargs: dict[str, Any]) -> str:
    """Normalize the terminal Clover tool status without inspecting its result."""
...
def model_call_fields(kwargs: dict[str, Any]) -> dict[str, str]:
    """Return the terminal model identity and provider route known to Clover."""
