"""Durable aggregation and local export for Clover shared metrics."""
...
from clover_cli.sqlite_util import write_txn
...
_PACKAGE_SCHEMA_VERSION = "clover.shared_metrics.v2"
...
class SharedMetricsStore:
    ...
    def __init__(
        ...
        root = get_clover_home() / "telemetry" / "shared_metrics"
        ...
        return [
            {
                ...
                "resource": {
                    "clover_version": row["clover_version"],
    ...
    def _ensure_schema(self) -> None:
        with self._connection(busy_timeout_ms=_SCHEMA_BUSY_TIMEOUT_MS) as connection:
            # Serialize first-run creation and upgrades across Clover processes.
            with write_txn(connection):
    ...
    def _create_package_in_transaction(
        ...
        resource = {
            "clover_version": period_row["clover_version"],
