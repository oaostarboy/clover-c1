...
- ``clover_cli.providers``        -- canonical provider identity + overlays
- ``clover_cli.model_normalize``  -- per-provider name formatting
...
def _declared_model_ids(value: Any) -> list[str]:
    ...
    if isinstance(value, dict):
        for model_id in value:
            # Backward compat: pre-fix Clover wrote sentinel keys inside the
...
def _entry_models_discovered(entry: Any) -> bool:
    """True when the entry's ``models`` mapping was auto-discovered by Clover.

    The current shape is an entry-level ``models_discovered: true`` sibling of
    ``models``. Older Clover versions wrote an in-mapping
...
def _models_config_is_allowlist(value: Any, discovered: bool = False) -> bool:
    ...
    written by ``_save_custom_provider`` / the ``clover model`` wizard — not a
    ...
    while ``clover model`` still live-probed the full ``/v1/models`` list.
    ...
    ``_entry_models_discovered``): a catalog Clover itself persisted after a
    ...
    *,
    ...
    try:
        ...
        for entry in providers:
            ...
            # A mapping Clover itself discovered (``models_discovered: true``
...
_CLOVER_MODEL_WARNING = (
    "Clover Cognition Clover 3 & 4 models are NOT agentic and are not designed "
    "for use with Clover Cognition. They lack the tool-calling capabilities "
    "required for agent workflows. Consider using an agentic model instead "
    "(Claude, GPT, Gemini, DeepSeek, etc.)."
)

# Match only the real Clover Cognition Clover 3 / Clover 4 chat families.
# The previous substring check (`"clover" in name.lower()`) false-positived on
# unrelated local Modelfiles like ``clover-brain:qwen3-14b-ctx16k`` that just
# happen to carry "clover" in their tag but are fully tool-capable.
#
# Positive examples the regex must match:
#   cloverc1/Clover-3-Llama-3.1-70B, clover-4-405b, openrouter/clover3:70b
...
def _load_direct_aliases() -> dict[str, DirectAlias]:
    ...
    Also reads ``model.aliases`` (set by ``clover config set model.aliases.xxx``)
...
def _ensure_direct_aliases() -> None:
    ...
    the module attribute. This keeps `from clover_cli.model_switch import
...
def resolve_display_context_length(
    ...
    about Codex OAuth, Copilot, Clover, and falls back to models.dev for the
    rest.
...
async def resolve_display_context_length_async(
    ...
    ``/v1/models``, Copilot, Clover, Codex, GMI, Ollama, models.dev and
    ...
    ``clover_cli.route_identity.should_clear_context_pin_async``, which
...
def switch_model(
    ...
    if explicit_provider:
        ...
        if pdef is None:
            _switch_err = (
                ...
                f"Check 'clover model' for available providers, or define it "
            ...
            try:
                ...
                if _cfg_issues:
                    _switch_err += "\n\nRun 'clover doctor' — config issues detected:"
        ...
        from clover_cli.models import _AGGREGATOR_PROVIDERS as _AGG_PROVIDERS
        from clover_cli.providers import ALIASES as _PROVIDER_ALIAS_TABLE
    ...
    # =================================================================
        ...
        try:
            from clover_cli.config import load_config
            from clover_cli.moa_config import exact_moa_preset_name, normalize_moa_config
    ...
    if provider_changed or explicit_provider:
        ...
        if explicit_provider and user_providers:
            from clover_cli.providers import resolve_user_provider as _ruser
        ...
        if current_provider == "custom" and current_base_url:
            try:
                from clover_cli.models import should_use_ollama_native_catalog
    ...
    # clover_cli.runtime_provider.resolve_runtime_provider; without the strip,
    ...
    # kimi) the same way.
    from clover_cli.models import opencode_provider_family as _oc_family_fn
    if _oc_family_fn(target_provider) is not None and isinstance(base_url, str):
        from clover_cli.models import normalize_opencode_base_url
    ...
    clover_warn = _check_clover_model_warning(new_model)
    if clover_warn:
        warnings.append(clover_warn)
...
def _extra_headers_from_config(entry: Any) -> dict[str, str]:
    if not isinstance(entry, dict):
        return {}
    from clover_cli.config import normalize_extra_headers
...
def _prefetch_provider_models_parallel(provider_slugs: list[str]) -> None:
    ...
    :param provider_slugs: Clover provider IDs to prefetch (e.g. ``["openrouter",
        "anthropic", "deepseek"]``).  Unknown providers are silently skipped.
    """
    from clover_cli.models import cached_provider_model_ids
    ...
    def _fetch_one(slug: str) -> None:
        try:
            ...
            if models:
                from clover_cli.models import update_provider_cache_entry
...
def _collect_authed_provider_slugs(
    ...
    from clover_cli.auth import PROVIDER_REGISTRY, _load_auth_store
    from clover_cli.providers import CLOVER_OVERLAYS, ALIASES as _PROVIDER_ALIAS_TABLE
    from clover_cli.models import _AGGREGATOR_PROVIDERS as _AGG_PROVIDERS, CANONICAL_PROVIDERS

    _excluded_set = {str(p).strip().lower() for p in excluded if p}
    slugs: list[str] = []
    seen: set[str] = set()

    # --- Section 1: Clover-mapped providers (PROVIDER_TO_MODELS_DEV) ---
    for clover_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        _alias_target = _PROVIDER_ALIAS_TABLE.get(clover_id)
        if (
            _alias_target
            and _alias_target != clover_id
            and _alias_target in _AGG_PROVIDERS
        ):
            continue
        _canonical = clover_id
        try:
            from providers import get_provider_profile as _gpp
            _prof = _gpp(clover_id)
            if _prof is not None:
                _canonical = _prof.name
        except Exception:
            pass
        if _canonical != clover_id:
            continue
        if clover_id.lower() in seen:
            continue
        if clover_id.lower() in _excluded_set or mdev_id.lower() in _excluded_set:
            continue
        pdata = models_dev_data.get(mdev_id)
        if not isinstance(pdata, dict):
            continue
        pconfig = PROVIDER_REGISTRY.get(clover_id)
        if pconfig and pconfig.auth_type != "api_key":
            continue
        from clover_cli.auth import is_runtime_provider_routable
        if not is_runtime_provider_routable(clover_id):
            continue
        ...
        if not has_creds:
            try:
                ...
                raw_pool_present = bool(
                    store and store.get("credential_pool", {}).get(clover_id)
                )
                if raw_pool_present:
                    has_creds = _credential_pool_is_usable(
                        clover_id, raw_pool_present=True
                    )
            except Exception:
                pass
        if has_creds:
            slugs.append(clover_id)
            seen.add(clover_id.lower())

    # --- Section 2: Clover-only providers (CLOVER_OVERLAYS) ---
    _mdev_to_clover = {v: k for k, v in PROVIDER_TO_MODELS_DEV.items()}
    for pid, overlay in CLOVER_OVERLAYS.items():
        if pid.lower() in seen:
            continue
        clover_slug = _mdev_to_clover.get(pid, pid)
        if clover_slug.lower() in seen:
            continue
        if pid.lower() in _excluded_set or clover_slug.lower() in _excluded_set:
            continue
        has_creds = False
        ...
        if not has_creds:
            try:
                ...
                if pid in providers_store or clover_slug in providers_store:
                    has_creds = True
            except Exception:
                pass
        if not has_creds:
            try:
                if _credential_pool_is_usable(clover_slug):
                    has_creds = True
            except Exception:
                pass
        if has_creds:
            slugs.append(clover_slug)
            seen.add(pid.lower())
            seen.add(clover_slug.lower())
...
def list_authenticated_providers(
    ...
    *,
    ...
    Uses the curated model lists from clover_cli/models.py (OPENROUTER_MODELS,
    ...
    from clover_cli.config import coerce_provider_id, stringify_provider_map
    ...
    # Compared against clover_id / mdev_id (section 1), pid / clover_slug
    ...
    def _record_builtin_endpoint(slug: str) -> None:
        ...
        try:
            from clover_cli.auth import PROVIDER_REGISTRY as _reg
        except Exception:
            return
        pcfg = _reg.get(slug)
        if not pcfg:
    ...
    if "lmstudio" not in curated and (
        ...
        from clover_cli.models import fetch_lmstudio_models
    ...
    # --- 1. Check Clover-mapped providers ---
    from clover_cli.models import _AGGREGATOR_PROVIDERS as _AGG_PROVIDERS
    from clover_cli.providers import ALIASES as _PROVIDER_ALIAS_TABLE
    for clover_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        ...
        if (
            ...
            continue
        # Resolve the canonical provider profile name.  Skip clover_ids
        # that are mere aliases resolving to a different canonical profile
        # (e.g. "kimi" and "moonshot" both → "kimi-coding").  Only process
        # entries whose clover_id matches the canonical profile name so
        ...
        # same clover_id).  Distinct canonical profiles that share a
        ...
        if slug.lower() in seen_slugs:
            continue
        if clover_id.lower() in _excluded or mdev_id.lower() in _excluded:
            continue
        ...
        # section 2 (CLOVER_OVERLAYS) with proper auth store checking.
        if pconfig and pconfig.auth_type != "api_key":
            continue
        # models.dev catalogs include providers Clover may not route yet.
        ...
        if not has_creds:
            try:
                from clover_cli.auth import _load_auth_store
                store = _load_auth_store()
                raw_pool_present = bool(
                    store and store.get("credential_pool", {}).get(clover_id)
                )
        ...
        # /model picker sees the SAME list `clover model` would build, with
        # disk caching to keep the picker open snappy. Falls back to the
        # curated static list when the live fetcher returns nothing.
        model_ids = cached_provider_model_ids(clover_id)
        if not model_ids:
            model_ids = curated.get(clover_id, [])
            if clover_id in _MODELS_DEV_PREFERRED:
                model_ids = _merge_with_models_dev(clover_id, model_ids)
    ...
    # Build reverse mapping: models.dev ID → Clover provider ID.
    # CLOVER_OVERLAYS keys may be models.dev IDs (e.g. "github-copilot")
    # while _PROVIDER_MODELS and config.yaml use Clover IDs ("copilot").
    _mdev_to_clover = {v: k for k, v in PROVIDER_TO_MODELS_DEV.items()}
    ...
    for pid, overlay in CLOVER_OVERLAYS.items():
        ...
        # Resolve Clover slug — e.g. "github-copilot" → "copilot"
        clover_slug = _mdev_to_clover.get(pid, pid)
        if clover_slug.lower() in seen_slugs:
            continue
        if pid.lower() in _excluded or clover_slug.lower() in _excluded:
            continue
        ...
        if getattr(overlay, "keyless", False):
            ...
            has_creds = _has_aws_sdk_creds_for_listing(clover_slug)
        elif overlay.auth_type == "vertex":
        ...
        if not has_creds:
            try:
                ...
                if store and (pid in providers_store or clover_slug in providers_store):
                    has_creds = True
            except Exception as exc:
        ...
        if not has_creds:
            try:
                if _credential_pool_is_usable(clover_slug):
                    ...
                    try:
                        from agent.credential_pool import load_pool
                        _pool = load_pool(clover_slug)
                        if _pool.has_credentials():
                            has_creds = True
                    except Exception:
                        pass
            except Exception as exc:
                logger.debug("Credential pool check failed for %s: %s", clover_slug, exc)
        ...
        # configured.
        if not has_creds and clover_slug == "anthropic":
            try:
                ...
                if (clover_creds and clover_creds.get("accessToken")) or \
        ...
        if clover_slug in {"openai-codex", "copilot", "copilot-acp"}:
            ...
            model_ids = cached_provider_model_ids(clover_slug)
        # For aws_sdk providers (bedrock), use live discovery so the list
        # reflects the active region (eu.*, ap.*) not the static us.* list.
        elif overlay.auth_type == "aws_sdk":
            try:
                _ids = cached_provider_model_ids(clover_slug)
            ...
            if not model_ids:
                model_ids = curated.get(clover_slug, []) or curated.get(pid, [])
                if clover_slug in _MODELS_DEV_PREFERRED:
                    model_ids = _merge_with_models_dev(clover_slug, model_ids)
        total = len(model_ids)
        if clover_slug in _UNCAPPED_PICKER_PROVIDERS:
            top = model_ids  # Aggregator: show full catalog regardless of max_models
        else:
            top = model_ids[:max_models] if max_models is not None else model_ids
        ...
        results.append({
            "slug": clover_slug,
            "name": get_label(clover_slug),
            "is_current": clover_slug == current_provider or pid == current_provider,
            "is_user_defined": False,
            "models": top,
            "total_models": total,
            "source": "clover",
        ...
        seen_slugs.add(clover_slug.lower())
        _record_builtin_endpoint(clover_slug)

    # --- 2b. Cross-check canonical provider list ---
    # Catches providers that are in CANONICAL_PROVIDERS but weren't found
    # in PROVIDER_TO_MODELS_DEV or CLOVER_OVERLAYS (keeps /model in sync
    # with `clover model`).
    try:
        from clover_cli.models import CANONICAL_PROVIDERS as _canon_provs
    except ImportError:
        _canon_provs = []
    ...
    if user_providers and isinstance(user_providers, dict):
        ...
        for ep_name, ep_cfg in user_providers.items():
            ...
            # Clover writes ``models:`` as a dict keyed by model id, but older
            ...
            if group_key not in ep_groups:
                ...
                # Clover's own writer uses (mirrors section-4 grouping).
            ...
            # ``clover model``, not an allowlist — see
        ...
        for grp in ep_groups.values():
            ...
            #   Ollama/llama.cpp match ``clover model``. Pin with
            ...
            if _probe_live:
                ...
                try:
                    from clover_cli.models import cached_fetch_api_models
    ...
    if custom_providers and isinstance(custom_providers, list):
        ...
        for entry in custom_providers:
            ...
            # active model. Clover's own writer (main.py::_save_custom_provider)
            # stores every configured model as a dict under ``models:``;
            # downstream readers (agent/models_dev.py, gateway/run.py,
            # run_agent.py, clover_cli/config.py) already consume that dict.
        ...
        for grp in groups.values():
            ...
            #   ``clover model``. Pin a dict catalog with
...
def _prepend_moa_picker_provider(providers: List[dict], current_provider: str = "") -> List[dict]:
    ...
    """
    try:
        from clover_cli.inventory import _moa_provider_row
...
def list_picker_providers(
    ...
    - OpenRouter's model list is replaced with the output of
      :func:`clover_cli.models.fetch_openrouter_models`, which filters the
    ...
    """
    from clover_cli.models import fetch_openrouter_models
