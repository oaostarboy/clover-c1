...
on Japanese locales, etc.).  Clover's banners, tool output feed, and slash
...
Entry points (``cli.py`` ``main``, ``clover_cli/main.py`` CLI dispatch,
...
def _flip_console_code_page_to_utf8() -> None:
    ...
    is silent — if there's no attached console (e.g. Clover is running
...
def configure_windows_stdio() -> bool:
    ...
    Set ``CLOVER_DISABLE_WINDOWS_UTF8=1`` in the environment to opt out
    ...
    if os.environ.get("CLOVER_DISABLE_WINDOWS_UTF8") in {"1", "true", "True", "yes"}:
        _CONFIGURED = True
        return False
    ...
    # Augment PATH with the Clover-managed Git install directories so
    ...
    # already-running shells don't see the change — which means clover
    ...
    # install without Clover-managed PortableGit).
    ...
    # stdin is re-configured for completeness; Clover's interactive
...
def _default_windows_editor() -> str:
    ...
    The prompt_toolkit buffer's ``open_in_editor`` and Clover's
    ``clover config edit`` both honour ``$EDITOR``.  Users who prefer a
    ...
    Set this before launching Clover (User env var in Windows Settings, or
    export in a PowerShell profile) and Clover picks it up automatically.
    """
    import shutil
...
def _augment_path_with_known_tools() -> None:
    """Prepend well-known Clover-managed tool directories to os.environ['PATH'].
    ...
    ``%LOCALAPPDATA%\\clover\\git\\bin`` to the User PATH via
    ...
    *spawned* processes only — already-running shells (including the one the
    user invokes ``clover`` from right after install) retain their old PATH.

    Any subprocess Clover spawns — bash, ``rg``, ``grep``, ``npm`` — inherits
    ...
    Patch-up strategy: add the known Clover-managed tool directories to our
    ...
    # see on next launch.
    candidate_dirs = [
        os.path.join(local_appdata, "clover", "git", "cmd"),
        os.path.join(local_appdata, "clover", "git", "bin"),
        os.path.join(local_appdata, "clover", "git", "usr", "bin"),
        # Clover venv Scripts directory — host of the clover.exe shim itself,
        # also where any pip-installed console scripts land.  Usually already
        # on PATH when the user invokes clover, but harmless to include.
        os.path.join(local_appdata, "clover", "clover-c1", "venv", "Scripts"),
