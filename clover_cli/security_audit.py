"""On-demand supply-chain audit for Clover Cognition installs.

Scans three surfaces a Clover user actually controls and we can map to
upstream advisories without auth or extra binaries:

1. The Clover venv (every PyPI dist via ``importlib.metadata``).
2. Python deps declared by user-installed plugins under ``~/.clover/plugins``
...
def _discover_plugins(clover_home: Path) -> list[Component]:
    """Python deps declared by plugins under ``~/.clover/plugins``.
...
def _discover_components(
    *,
    ...
    home = clover_home or Path(get_clover_home())
...
def cmd_security_audit(args: argparse.Namespace) -> int:
    """Implementation of `clover security audit`."""
    ...
    components = _discover_components(
        skip_venv=skip_venv, skip_plugins=skip_plugins, skip_mcp=skip_mcp, clover_home=home
    )
