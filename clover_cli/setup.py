...
def run_setup_wizard(args):
    ...
    if is_existing:
        ...
        if quick_requested:
            _run_quick_setup(config, clover_home)
            return
        ...
        if setup_mode == 0:
            _run_first_time_quick_setup(config, clover_home, is_existing)
            return
        if setup_mode == 2:
            _run_blank_slate_setup(config, clover_home, is_existing)
            return
...
def _run_blank_slate_setup(config: dict, clover_home, is_existing: bool):
    ...
    if path == 0:
        ...
        # `clover update` runs don't re-inject them.
        try:
