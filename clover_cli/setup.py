"""
Interactive setup wizard for Clover Cognition.
...
# display_clover_home imported lazily at call sites (stale-module safety during clover update)
...
from clover_cli.cli_output import (  # noqa: E402
    print_error,
    print_info,
    print_success,
    print_warning,
)
from clover_cli.secret_prompt import masked_secret_prompt  # noqa: E402
...
def print_noninteractive_setup_guidance(reason: str | None = None) -> None:
    ...
    print()
    print(color("☘ Clover Setup — Non-interactive mode", Colors.CYAN, Colors.BOLD))
    print()
    if reason:
        print_info(reason)
    print_info("The interactive wizard cannot be used here.")
    print()
    print_info("Configure Clover using environment variables or config commands:")
    print_info("  clover config set model.provider custom")
    print_info("  clover config set model.base_url http://localhost:8080/v1")
    print_info("  clover config set model.default your-model-name")
    print()
    print_info("Or set OPENROUTER_API_KEY / OPENAI_API_KEY in your environment.")
    print_info("Run 'clover setup' in an interactive terminal to use the full wizard.")
    print()
...
_SETUP_NAVIGATION: ContextVar[_SetupNavigationState | None] = ContextVar(
    "clover_setup_navigation", default=None
)
...
def is_noninteractive() -> bool:
    ...
    ``CLOVER_NONINTERACTIVE=1`` (see ``clover_cli/web_server.py``). In that
    ...
    """
    return os.environ.get("CLOVER_NONINTERACTIVE", "").strip().lower() in {
        "1",
        "true",
        "yes",
        "on",
...
def prompt_yes_no(question: str, default: bool = True) -> bool:
    ...
    Non-interactive callers (``CLOVER_NONINTERACTIVE=1`` or a closed/redirected
...
def prompt_checklist(title: str, items: list, pre_selected: list = None) -> list:
    ...
    from clover_cli.curses_ui import curses_checklist
...
def _prompt_api_key(var: dict):
    ...
    if value:
        ...
        print_warning("  Skipped (configure later with 'clover setup')")


def _print_setup_summary(config: dict, clover_home):
    ...
    try:
        from clover_cli.auth import resolve_provider
    ...
    if _vision_backends:
        ...
        tool_status.append(("Vision (image analysis)", False, "run 'clover setup' to configure"))
    ...
    if subscription_features.tts.managed_by_clover:
        ...
        if neutts_ok:
            ...
            tool_status.append(("Text-to-Speech (NeuTTS — not installed)", False, "run 'clover setup tts'"))
        ...
        if kittentts_ok:
            ...
            tool_status.append(("Text-to-Speech (KittenTTS — not installed)", False, "run 'clover setup tts'"))
    else:
    ...
    # Spotify (OAuth via clover auth spotify — check auth.json, not env vars)
    try:
    ...
    if disabled_tools:
        print_warning(
            "Some tools are disabled. Run 'clover setup tools' to configure them,"
        )
    ...
    print(
        f"   {color('Data:', Colors.YELLOW)}      {clover_home}/cron/, sessions/, logs/"
    )
    print()
    ...
    print()
    print(f"   {color('clover setup', Colors.GREEN)}          Re-run the full wizard")
    print(f"   {color('clover setup model', Colors.GREEN)}    Change model/provider")
    print(f"   {color('clover setup terminal', Colors.GREEN)} Change terminal backend")
    print(f"   {color('clover setup gateway', Colors.GREEN)}  Configure messaging")
    print(f"   {color('clover setup tools', Colors.GREEN)}    Configure tool providers")
    print()
    print(f"   {color('clover config', Colors.GREEN)}         View current settings")
    print(
        f"   {color('clover config edit', Colors.GREEN)}    Open config in your editor"
    )
    print(f"   {color('clover config set <key> <value>', Colors.GREEN)}")
    ...
    print()
    print(f"   {color('clover', Colors.GREEN)}              Start chatting")
    print(f"   {color('clover gateway', Colors.GREEN)}      Start messaging gateway")
    print(f"   {color('clover doctor', Colors.GREEN)}       Check for issues")
    print()
...
# between `clover tools` and `clover setup tools`).
...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    Delegates to ``cmd_model()`` (the same flow used by ``clover model``)
    ...
    provider added to ``clover model`` is automatically available here.
    ...
    # Delegate to the shared clover model flow — handles provider picker,
    # credential prompting, model selection, and config persistence.
    from clover_cli.main import select_provider_and_model
    try:
...
def _xai_oauth_logged_in_for_setup() -> bool:
    ...
    through ``clover model`` -> xAI Grok OAuth (SuperGrok / Premium+).
    """
    try:
        from clover_cli.auth import get_xai_oauth_auth_status
...
def _setup_tts_provider(config: dict):
    ...
    if selected == "neutts":
        ...
        # via the Clover auth store) > existing XAI_API_KEY > prompt the user.
            ...
            if choice_idx == 0:
                ...
                if api_key:
                    ...
                    print_warning(
                        ...
                        f"via clover setup model or {_dhh()}/.env to use xAI TTS. "
                        "Falling back to Edge TTS."
                    )
                    selected = "edge"
            else:
...
def setup_tts(config: dict):
    """Standalone TTS setup (for 'clover setup tts')."""
...
def setup_terminal_backend(config: dict):
    ...
    # under ~/.clover/plugins/). Fail-soft: a broken plugin must not take the
    # setup wizard down.
    ...
    if selected_backend == "local":
        ...
        # configurable later via `clover setup terminal` / config.yaml.
        ...
        # Image and resource limits use defaults; tune via `clover setup terminal`.
        ...
        if prompt_yes_no("  Enable egress firewall for Docker sandboxes?", False):
            ...
            print_info(
                "Run `clover egress setup` then `clover egress start` to mint "
                "tokens and launch the proxy."
            )
        else:
            print_info(
                "Skipping egress firewall. You can enable it later with `clover egress setup`."
            )
        ...
        print_info("Requires the optional SDK: pip install 'clover-c1[vercel]'")
        ...
        try:
            ...
            # Managed uv first: $CLOVER_HOME/bin is never on PATH, so a bare
            # which() misses the uv Clover installed. Bootstrapping one is
            ...
            if result.returncode == 0:
                ...
                print_warning("Install failed — run manually: pip install 'clover-c1[vercel]'")
                if result.stderr:
                    print_info(f"  Error: {result.stderr.strip().splitlines()[-1]}")
...
def _apply_default_agent_settings(config: dict):
    ...
    # bridges it into CLOVER_MAX_ITERATIONS at startup. We no longer write
    ...
    remove_env_value("CLOVER_MAX_ITERATIONS")
    ...
    print_info("  Run `clover setup agent` later to customize.")
...
def setup_agent_settings(config: dict):
    ...
    try:
        ...
        if max_iter > 0:
            ...
            # CLOVER_MAX_ITERATIONS from agent.max_turns at startup.
    ...
    print_info("  log     — Silent in chat; write every tool call to ~/.clover/logs/tool_calls.log (gateway only)")
...
def _setup_telegram():
    ...
    print()
    print_info("📬 Home Channel: where Clover delivers cron job results,")
...
def _setup_bluebubbles():
    ...
    print_info("Connects Clover to iMessage via BlueBubbles — a free, open-source")
...
def _setup_qqbot():
    ...
    from clover_cli.gateway import _setup_qqbot as _gateway_setup_qqbot
...
def setup_gateway(config: dict):
    ...
    if not selected:
        print_info("No platforms selected. Run 'clover setup gateway' later to configure.")
    else:
        for idx in selected:
    ...
    if any_messaging:
        ...
        if missing_home:
            ...
            for plat in missing_home:
                print_info(
                    f"     clover config set {plat.upper()}_HOME_CHANNEL <channel_id>"
                )
    ...
    # via `clover import` or `clover setup gateway`). Gating this on
...
def setup_tools(config: dict, first_install: bool = False):
    ...
    Both `clover setup tools` and `clover tools` use the same flow:
...
def setup_telemetry(config: dict):
    ...
    print_info("Packages stay under this Clover profile and are not uploaded.")
...
def _get_section_config_summary(config: dict, section_key: str) -> Optional[str]:
    ...
    ``get_env_value`` is the module-level import from clover_cli.config
    ...
    if section_key == "model":
        ...
        from clover_cli.gateway import _all_platforms, _platform_status
...
def _load_openclaw_migration_module():
    """Load the openclaw_to_clover migration script as a module.
    ...
    spec = importlib.util.spec_from_file_location(
        "openclaw_to_clover", _OPENCLAW_SCRIPT
    )
...
# Config values may have different semantics between OpenClaw and Clover.
# Instruction/context files (.md) can contain incompatible setup procedures.
_HIGH_IMPACT_KIND_KEYWORDS = {
    "gateway": "⚠ Gateway/messaging — this will configure Clover to use your OpenClaw messaging channels",
    "telegram": "⚠ Telegram — this will point Clover at your OpenClaw Telegram bot",
    "slack": "⚠ Slack — this will point Clover at your OpenClaw Slack workspace",
    "discord": "⚠ Discord — this will point Clover at your OpenClaw Discord bot",
    "whatsapp": "⚠ WhatsApp — this will point Clover at your OpenClaw WhatsApp connection",
    "config": "⚠ Config values — OpenClaw settings may not map 1:1 to Clover equivalents",
...
def _print_migration_preview(report: dict):
    ...
    if conflict_items:
        print(color("  Would overwrite (conflicts with existing Clover config):", Colors.YELLOW))
    ...
    if warnings_shown:
        ...
        print()
        print(color("  Note: OpenClaw config values may have different semantics in Clover.", Colors.YELLOW))
        print(color("  For example, OpenClaw's tool_call_execution: \"auto\" ≠ Clover's yolo mode.", Colors.YELLOW))
        print(color("  Instruction files (.md) from OpenClaw may contain incompatible procedures.", Colors.YELLOW))
        print()


def _offer_openclaw_migration(clover_home: Path) -> bool:
    ...
    print_info("Clover can preview what would be imported before making any changes.")
    print()

    if not prompt_yes_no("Would you like to see what can be imported?", default=True):
        print_info(
            "Skipping migration. You can run it later with: clover claw migrate --dry-run"
        )
        return False
    ...
    try:
        ...
        dry_migrator = mod.Migrator(
            ...
            target_root=clover_home.resolve(),
    ...
    if not prompt_yes_no("Proceed with migration?", default=False):
        print_info(
            "Migration cancelled. You can run it later with: clover claw migrate"
        )
        print_info(
    ...
    # Execute the migration — overwrite=False so existing Clover configs are
    ...
    try:
        migrator = mod.Migrator(
            ...
            execute=True,
            workspace_target=None,
            overwrite=False,  # preserve existing Clover config
            migrate_secrets=True,
            output_dir=None,
            selected_options=selected,
            preset_name="full",
    ...
    if conflicts:
        print_info(f"Skipped {conflicts} item(s) that already exist in Clover (use clover claw migrate --overwrite to force).")
    if skipped:
...
def _setup_navigation_scope():
    ...
    from clover_cli.curses_ui import (
...
def run_setup_action_with_navigation(
    ...
    *,
    ...
    Shared commands such as ``clover model`` use the same provider/model
...
def _run_setup_wizard_impl(args):
    ...
    Supports full, quick, and section-specific setup:
      clover setup           — full or quick (auto-detected)
      clover setup model     — just model/provider
      clover setup tts       — just text-to-speech
      clover setup terminal  — just terminal backend
      clover setup gateway   — just messaging platforms
      clover setup tools     — just tool configuration
      clover setup telemetry — just local shared metrics
      clover setup agent     — just agent settings
    """
    from clover_cli.config import is_managed, managed_error
    if is_managed():
    ...
    if section:
        for key, label, func in SETUP_SECTIONS:
            if key == section:
                ...
                print(color(f"│     ☘ Clover Setup — {label:<34s} │", Colors.MAGENTA))
                print(
                    color(
                        "└─────────────────────────────────────────────────────────┘",
                        Colors.MAGENTA,
    ...
    print(
        color(
            "│             ☘ Clover Cognition Setup Wizard                │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    print(
        color(
            "│  Let's configure your Clover Cognition installation.       │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    if is_existing:
        ...
        if quick_requested:
            _run_setup_steps(
                [("Quick Setup", lambda: _run_quick_setup(config, clover_home))]
            )
            return

        print()
        print_header("Reconfigure")
        print_success("You already have Clover configured.")
        print_info("Running the full wizard — each prompt shows your current value.")
        print_info("Press Enter to keep it, or type a new value to change it.")
        print_info("")
        print_info("Tip: jump straight to a section with 'clover setup model|terminal|")
        ...
        migration_ran = _offer_openclaw_migration(clover_home)
        if migration_ran:
            config = load_config()
    ...
    print()
    print_info("You can edit these files directly or use 'clover config edit'")
    ...
    # Tune later with `clover setup agent`.
    if not is_existing:
...
def _print_macos_fda_tip() -> None:
    ...
    print_info("  your terminal (and Clover.app if you use Desktop), or run:")
    print_info("    open \"x-apple.systempreferences:com.apple.preference"
               ".security?Privacy_AllFiles\"")
    print_info("  The grant is permanent — it survives every Clover update.")
...
def _blank_slate_minimal_toolsets(config: dict):
    ...
    ``clover-c1`` skill (the agent's operating manual for driving,
    configuring, and troubleshooting Clover) is always seeded — without
    ...
    2. ``agent.disabled_toolsets`` — a global hard-suppression list (applied last
       ...
       quirks. The user re-enables any of them later via ``clover tools`` (which
    ...
    try:
        ...
        from clover_cli.tools_config import CONFIGURABLE_TOOLSETS, _get_plugin_toolset_keys
...
def _blank_slate_minimize_config(config: dict):
    ...
    Everything here is opt-in afterwards via ``clover setup agent`` /
    ``clover config set``. We keep only what's needed to run.
    """
...
def _run_blank_slate_setup(config: dict, clover_home, is_existing: bool):
    ...
    print_info("essential `clover-c1` skill is always kept so the agent")
    print_info("can help you drive and configure Clover itself.")
    print()
    ...
    if path == 0:
        ...
        # `clover update` runs don't re-inject them. Essential skills (the
        # `clover-c1` operating manual) are still seeded by the sync.
        try:
        ...
        print_info("  Enable tools:        clover tools")
        print_info("  Seed skills:         clover skills opt-in --sync")
        print_info("  Add MCP servers:     clover mcp add")
        print_info("  Enable plugins:      clover plugins")
        print_info("  Tune agent settings: clover setup agent")
        print()
        _print_setup_summary(config, clover_home)
        return

    # ── Walkthrough path — opt in to each capability ──
    _blank_slate_walkthrough(config, clover_home)


def _blank_slate_walkthrough(config: dict, clover_home):
    ...
    try:
        ...
        if seed_skills:
            ...
            set_bundled_skills_opt_out(True)
            # Essential skills (the `clover-c1` operating manual) are
            # still seeded even for an opted-out profile.
            sync_skills(quiet=True)
            print_info("No skills seeded (except the essential `clover-c1`")
            print_info("skill). A .no-bundled-skills marker keeps future")
            print_info("`clover update` runs from re-injecting them. Opt back in any")
            print_info("time with `clover skills opt-in --sync`.")
    except Exception as exc:
    ...
    if prompt_yes_no("Open the tool selector to enable more tools?", default=False):
        ...
        print_info("Keeping the minimal toolset. Add tools later with `clover tools`.")
    ...
    if prompt_yes_no("Review and enable built-in plugins now?", default=False):
        print_info("Manage plugins with `clover plugins list` / `clover plugins install`.")
    else:
        print_info("No plugins enabled. Add later with `clover plugins`.")

    # ── MCP servers (off unless chosen) ──
    print()
    print_header("MCP Servers")
    if prompt_yes_no("Add an MCP server now?", default=False):
        print_info("Add servers with `clover mcp add <name> --url ... | --command ...`.")
    else:
        print_info("No MCP servers configured. Add later with `clover mcp add`.")
    ...
    print()
    print_success("Blank Slate setup complete — minimal agent ready.")
    print_info("  Enable more tools:   clover tools")
    print_info("  Seed skills:         clover skills opt-in --sync")
    print_info("  Add MCP servers:     clover mcp add")
    print_info("  Tune agent settings: clover setup agent")
    print()
...
def _run_quick_setup(config: dict, clover_home):
    ...
    if not has_anything_missing:
        ...
        print()
        print_info("Run 'clover setup' and choose 'Full Setup' to reconfigure,")
    ...
    if missing_messaging:
        ...
        print_info("Connect Clover to messaging apps to chat from anywhere.")
        print_info("You can configure these later with 'clover setup gateway'.")
