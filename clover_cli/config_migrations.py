...
``clover_cli.config.migrate_config``. Each step is a function
...
``clover_cli.config`` imports :func:`run_migrations` lazily (inside
``migrate_config``), and every step function here resolves its helpers
(``read_raw_config``, ``_persist_migration``, ``get_env_value``, …) lazily
through the live ``clover_cli.config`` module object at call time via
:func:`_cfg`. There is deliberately NO module-level import of
``clover_cli.config`` here, so no circular import can form — and, just as
importantly, tests that monkeypatch helpers on ``clover_cli.config`` (e.g.
``patch("clover_cli.config.read_raw_config", ...)``) keep working, because
...
#: CLOVER_TOOL_PROGRESS*) forever is not worth it. Below-floor configs are
...
def support_floor_message() -> str:
    ...
    return (
        ...
        f"{display_clover_home()}/config.yaml and run `clover setup` to "
...
def _cfg():
    """Return the live ``clover_cli.config`` module (lazy, cycle-free)."""
    from clover_cli import config
...
def _migrate_to_21(results: Dict[str, Any], quiet: bool) -> None:
    ...
    read_raw_config = _c.read_raw_config
    _persist_migration = _c._persist_migration
    get_clover_home = _c.get_clover_home
    ...
    if "enabled" not in plugins_cfg:
        ...
        # Scan ``$CLOVER_HOME/plugins/`` for currently installed user plugins.
        ...
        if not quiet:
            if grandfathered:
                ...
                print(
                    ...
                    "Use `clover plugins enable <name>` to activate."
                )
...
def _migrate_to_23(results: Dict[str, Any], quiet: bool) -> None:
    ...
    # settings in their `config.yaml`, and `clover curator status` has no
    ...
    #   3. Creates `~/.clover/logs/curator/` if missing (belt-and-suspenders
    #      on top of ensure_clover_home() — old profiles that predate this
    ...
    try:
        curator_dir = get_clover_home() / "logs" / "curator"
        ...
        if added_curator:
            ...
            if not quiet:
                print(
                    ...
                    f"({', '.join(added_curator)}) — edit via `clover config set`"
                )
        if added_aux:
            results["config_added"].append(
            ...
            if not quiet:
                print(
                    ...
                    f"({', '.join(added_aux)}) — edit via `clover config set`"
                )
...
# behavior set it to true explicitly via `clover config set`.
...
def _migrate_to_38(results: Dict[str, Any], quiet: bool) -> None:
    ...
    from clover_cli.relay_plugin_cutover import legacy_relay_plugin_keys
    ...
    message = (
        ...
        "CLOVER_NEMO_RELAY_PLUGINS_TOML."
    )
...
def _migrate_to_39(results: Dict[str, Any], quiet: bool) -> None:
    ...
    # period that has since ended server-side, leaving every Clover-signed-in
    ...
    # provider surface (`video_generate`, `clover tools` → Video Generation).
