"""Windows ConPTY bridge for the `clover dashboard` chat tab.

Drop-in counterpart to ``clover_cli.pty_bridge.PtyBridge`` for native
Windows. Mirrors the exact public surface the ``/api/pty`` WebSocket
handler in ``clover_cli.web_server`` consumes: ``spawn``, ``read``,
