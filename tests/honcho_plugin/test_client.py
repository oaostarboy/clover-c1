...
class TestHonchoClientConfigDefaults:
    def test_default_values(self):
        ...
        assert config.host == "clover"
        assert config.workspace_id == "clover"
...
class TestFromGlobalConfig:
    ...
    def test_recall_mode_from_config(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "key",
            "recallMode": "tools",
            "hosts": {"clover": {"recallMode": "context"}},
        }))
        config = HonchoClientConfig.from_global_config(config_path=config_file)
    ...
    def test_base_url_host_block_overrides_root_and_env(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            ...
            "hosts": {"clover": {"baseUrl": "http://host-block:9001"}},
        }))
    ...
    def test_base_url_full_precedence_chain(self, tmp_path):
        ...
        config_file = tmp_path / "config.json"
        layers = {
            "hosts": {"clover": {"baseUrl": "http://host:1"}},
...
class TestResolveSessionName:
    ...
    def test_per_repo_uses_git_root(self):
        ...
        with patch.object(
            HonchoClientConfig, "_git_repo_name", return_value="clover-c1"
        ):
            result = config.resolve_session_name("/home/user/clover-c1/subdir")
        assert result == "clover-c1"


class TestResolveConfigPath:
    def test_prefers_clover_home_when_exists(self, tmp_path):
        clover_home = tmp_path / "clover"
        clover_home.mkdir()
        local_cfg = clover_home / "honcho.json"
    ...
    def test_falls_back_to_default_profile_when_no_local(self, tmp_path, monkeypatch):
        # Profile mode: CLOVER_HOME points at ~/.clover/profiles/<name>, so
        # _get_default_clover_home() must resolve back to ~/.clover — that's
        # the bug the HOME-anchored helper fixes (vs. blindly using Path.home()).
        fake_home = tmp_path / "fakehome"
        fake_home.mkdir()
        default_home = fake_home / ".clover"
        ...
        assert _get_default_clover_home() == default_home
        assert result == default_cfg


class TestResolveActiveHost:
    def test_profile_host_key_uses_honcho_safe_separator(self):
        assert profile_host_key("coder") == "clover_coder"
        assert profile_host_key("default") == "clover"


    def test_explicit_env_var_wins(self):
        with patch.dict(os.environ, {"CLOVER_HONCHO_HOST": "clover.coder"}):
            assert resolve_active_host() == "clover.coder"
    ...
    def test_profiles_import_failure_falls_back(self):
        ...
        with patch.dict(os.environ, {}, clear=False), patch(
            ...
            os.environ.pop("CLOVER_HONCHO_HOST", None)
            # Temporarily remove clover_cli.profiles to simulate import failure
            saved = sys.modules.get("clover_cli.profiles")
            sys.modules["clover_cli.profiles"] = None  # type: ignore
            try:
                assert resolve_active_host() == "clover"
            finally:
                if saved is not None:
                    sys.modules["clover_cli.profiles"] = saved
                else:
                    sys.modules.pop("clover_cli.profiles", None)


class TestProfileScopedConfig:
    def test_from_env_uses_profile_host(self):
        with patch.dict(os.environ, {"HONCHO_API_KEY": "key"}):
            config = HonchoClientConfig.from_env(host="clover_coder")
        assert config.host == "clover_coder"
        assert config.workspace_id == "clover"  # shared workspace
        assert config.ai_peer == "clover_coder"
...
class TestObservationModeMigration:
    ...
    def test_existing_config_defaults_to_unified(self, tmp_path):
        ...
        cfg_file.write_text(json.dumps({
            "apiKey": "k",
            "hosts": {"clover": {"enabled": True, "aiPeer": "clover"}},
        }))
...
class TestGetHonchoClient:
    ...
    def test_dot_form_legacy_host_key_keeps_local_api_key(self):
        """Regression for #37436: a legacy dot-form host block (clover.work)
        ...
        cfg = HonchoClientConfig(
            ...
            host="clover_work",
            workspace_id="clover",
            raw={"hosts": {"clover.work": {"apiKey": "explicit-local-key"}}},
        )
    ...
    def test_timeout_change_triggers_client_rebuild(self):
        ...
        cfg_yaml = get_clover_home() / "config.yaml"
    ...
    def test_managed_config_timeout_does_not_thrash_singleton(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("CLOVER_MANAGED_DIR", str(managed_dir))
...
class TestGetHonchoClientBaseUrlDoublePrefixFix:
    ...
    def test_lan_default_host_empty_key_uses_local_placeholder(self, tmp_path):
        ...
        with patch.dict(os.environ, {}, clear=True), \
             patch("clover_cli.profiles.get_active_profile_name", return_value="default"), \
