"""Shared fixtures for the clover-c1 test suite.
...
2. **Isolated CLOVER_HOME.** CLOVER_HOME points to a per-test tempdir so
   code reading ``~/.clover/*`` via ``get_clover_home()`` can't see the
   real one. (We do NOT also redirect HOME — that broke subprocesses in
   CI. Code using ``Path.home() / ".clover"`` instead of the canonical
   ``get_clover_home()`` is a bug to fix at the callsite.)
3. **Deterministic runtime.** TZ=UTC, LANG=C.UTF-8, PYTHONHASHSEED=0.
4. **No CLOVER_SESSION_* inheritance** — the agent's current gateway
...
# ── Sandbox CLOVER_HOME before ANY test module is imported ──────────────────
# `clover_cli/main.py` calls `setup_logging()` at MODULE level, which resolves
# `get_clover_home()` and attaches rotating file handlers to the ROOT logger.
...
# real `~/.clover/logs/agent.log` and `errors.log`.
#
# The `_isolate_env` fixture below also sandboxes CLOVER_HOME, but fixtures run
...
# the REAL Clover root — capture it BEFORE the sandbox rewires CLOVER_HOME,
# otherwise the deny-list would point at the throwaway tempdir and the guard
# would silently stop protecting the operator's actual ~/.clover (#69385).
_PRE_SANDBOX_KANBAN_OVERRIDE = os.environ.get("CLOVER_KANBAN_HOME", "").strip()
_PRE_SANDBOX_CLOVER_HOME = os.environ.get("CLOVER_HOME", "")


def _clover_home_points_at_production(value: str) -> bool:
    """True when a pre-set CLOVER_HOME resolves to the real production root.

    Gateway-launched shells (and developer shells that ``export
    CLOVER_HOME=~/.clover``) hand pytest the PRODUCTION home. Historically
    the session sandbox below honored any pre-set value, so collection-time
    imports (logging handlers, ``clover_state.DEFAULT_DB_PATH``) froze paths
    inside the real ``~/.clover`` — the escape vector that landed pytest
    ...
    CLOVER_HOME is honored now.
    """
    if not value:
        return True
    try:
        resolved = Path(value).expanduser().resolve()
        real_root = (Path.home() / ".clover").resolve()
    except Exception:
        return True
...
if _clover_home_points_at_production(os.environ.get("CLOVER_HOME", "")):
    _SESSION_CLOVER_HOME = tempfile.mkdtemp(prefix="clover-test-home-")
    os.environ["CLOVER_HOME"] = _SESSION_CLOVER_HOME
    atexit.register(shutil.rmtree, _SESSION_CLOVER_HOME, True)
...
# like a real CLI") — which used to disarm clover_state's live-DB guard in
# the child at the same moment the child lost the CLOVER_HOME redirect.
# CLOVER_TEST_ISOLATION is OUR marker: exported here (before any test module
# imports), inherited by every child by default, and honored by
# clover_state._running_under_pytest() as a test-context signal. A child
...
# open a real DB must export CLOVER_STATE_DB_GUARD_BYPASS=1 in that child's
# env instead of stripping markers.
os.environ["CLOVER_TEST_ISOLATION"] = os.environ.get("CLOVER_HOME", "") or "1"

#: CLOVER_HOME as it stood when conftest was imported - i.e. before any test
...
CLOVER_HOME_AT_CONFTEST_IMPORT = os.environ.get("CLOVER_HOME", "")
...
# CLOVER_* vars that change test behavior by being set. Unset all of these
# unconditionally — individual tests that need them set do so explicitly.
_CLOVER_BEHAVIORAL_VARS = frozenset({
    ...
    # final response text to ``clover_cli.voice.speak_text`` — real synthesis,
    ...
    # any test run launched FROM a Clover agent session inherits them and
    # clover_constants home-resolution helpers prefer them over monkeypatched
    # HOME (test_subprocess_home_isolation red locally, green on CI).
    "CLOVER_REAL_HOME",
    "TERMINAL_HOME_MODE",
    "CLOVER_INTERACTIVE",
    "CLOVER_QUIET",
    "CLOVER_TOOL_PROGRESS",
    "CLOVER_TOOL_PROGRESS_MODE",
    "CLOVER_MAX_ITERATIONS",
    "CLOVER_SESSION_PLATFORM",
    "CLOVER_SESSION_CHAT_ID",
    "CLOVER_SESSION_CHAT_NAME",
    "CLOVER_SESSION_CHAT_TYPE",
    "CLOVER_SESSION_THREAD_ID",
    "CLOVER_SESSION_SOURCE",
    "CLOVER_SESSION_KEY",
    "CLOVER_GATEWAY_SESSION",
    "CLOVER_CRON_SESSION",
    "_CLOVER_GATEWAY",
    "CLOVER_PLATFORM",
    "CLOVER_MODEL",
    "CLOVER_INFERENCE_MODEL",
    "CLOVER_INFERENCE_PROVIDER",
    "CLOVER_TUI_PROVIDER",
    "CLOVER_MANAGED",
    "CLOVER_MANAGED_DIR",
    "CLOVER_DEV",
    ...
    "CLOVER_TIMEZONE",
    "CLOVER_REDACT_SECRETS",
    "CLOVER_BACKGROUND_NOTIFICATIONS",
    "CLOVER_EXEC_ASK",
    "CLOVER_HOME_MODE",
    "CLOVER_AGENT_USE_LEGACY_SESSION_KEYS",
    # Kanban path/board pins must never leak from a developer shell or
    # dispatched worker into tests; otherwise tests can write fake tasks to
    # the real ~/.clover/kanban.db instead of the per-test CLOVER_HOME.
    "CLOVER_KANBAN_DB",
    "CLOVER_KANBAN_BOARD",
    "CLOVER_KANBAN_HOME",
    "CLOVER_KANBAN_WORKSPACES_ROOT",
    "CLOVER_KANBAN_LOGS_ROOT",
    "CLOVER_KANBAN_TASK",
    "CLOVER_KANBAN_WORKSPACE",
    "CLOVER_KANBAN_RUN_ID",
    "CLOVER_KANBAN_CLAIM_LOCK",
    "CLOVER_KANBAN_DISPATCH_IN_GATEWAY",
    ...
    "CLOVER_DELEGATED_CHILD_CONTEXT",
    "CLOVER_TENANT",
    ...
    # otherwise-unrelated config tests away from the default "clover" host.
    "CLOVER_HONCHO_HOST",
    # Dashboard OAuth auth gate (PR #30156). When set, the bundled
    # dashboard-auth `clover` plugin auto-registers itself on plugin discovery,
    ...
    "CLOVER_DASHBOARD_OAUTH_CLIENT_ID",
    "CLOVER_DASHBOARD_PORTAL_URL",
    "TERMINAL_CWD",
    "TERMINAL_ENV",
...
def _hermetic_environment(tmp_path, monkeypatch):
    ...
    Also redirects HOME and CLOVER_HOME to per-test tempdirs so code that
    reads ``~/.clover/*`` can't touch the real one, and pins TZ/LANG so
    ...
    # 2. Blank behavioral CLOVER_* vars that could change test semantics.
    for name in _CLOVER_BEHAVIORAL_VARS:
        monkeypatch.delenv(name, raising=False)
    ...
    monkeypatch.setenv("CLOVER_HONCHO_HOST", "clover")

    # 3. Redirect CLOVER_HOME to a per-test tempdir. Code that reads
    #    ``~/.clover/*`` via ``get_clover_home()`` now gets the tempdir.
    #
    #    NOTE: We do NOT also redirect HOME. Doing so broke CI because
    #    some tests (and their transitive deps) spawn subprocesses that
    #    inherit HOME and expect it to be stable. If a test genuinely
    #    needs HOME isolated, it should set it explicitly in its own
    #    fixture. Any code in the codebase reading ``~/.clover/*`` via
    #    ``Path.home() / ".clover"`` instead of ``get_clover_home()``
    #    is a bug to fix at the callsite.
    fake_clover_home = tmp_path / "clover_test"
    fake_clover_home.mkdir()
    (fake_clover_home / "sessions").mkdir()
    (fake_clover_home / "cron").mkdir()
    (fake_clover_home / "memories").mkdir()
    (fake_clover_home / "skills").mkdir()
    monkeypatch.setenv("CLOVER_HOME", str(fake_clover_home))
    ...
    # clover_state's live-DB guard stays armed in them even when the test
    # strips pytest's own PYTEST_* vars from the child env.
    monkeypatch.setenv("CLOVER_TEST_ISOLATION", str(fake_clover_home))
    # And never let a developer-shell (or leaked child) bypass disarm the
    # guard for in-process code under test.
    monkeypatch.delenv("CLOVER_STATE_DB_GUARD_BYPASS", raising=False)

    # 3b. clover_state computes ``DEFAULT_DB_PATH = get_clover_home() / "state.db"``
    #     at import time. When the module is first imported at collection (any
    #     test file with a top-level ``from clover_state import ...``) that
    ...
    clover_state_mod = sys.modules.get("clover_state")
    if clover_state_mod is not None and hasattr(clover_state_mod, "DEFAULT_DB_PATH"):
        monkeypatch.setattr(
            clover_state_mod, "DEFAULT_DB_PATH", fake_clover_home / "state.db"
        )
    ...
    monkeypatch.setenv("CLOVER_DISABLE_LAZY_INSTALLS", "1")

    # 5. Reset plugin singleton so tests don't leak plugins from
    #    ~/.clover/plugins/ (which, per step 3, is now empty — but the
    #    singleton might still be cached from a previous test).
    try:
        import clover_cli.plugins as _plugins_mod
        ...
        # previous test's tmp_path CLOVER_HOME can't leak forward. Paths
...
@pytest.fixture(autouse=True)
def _isolate_clover_home(_hermetic_environment):
...
def _neutralize_kanban_memory_guard(request, monkeypatch):
    ...
    if request.node.get_closest_marker("real_memory_guard"):
        return
    try:
        from clover_cli import kanban_db as _kb_mod
    except Exception:
        return
...
# invocation), kanban writes silently pollute the real ~/.clover. This autouse
...
# allow-list because test-level fixtures legitimately move CLOVER_HOME to
...
def _capture_real_kanban_root() -> Path:
    ...
    file (before the session CLOVER_HOME sandbox rewired the env), so the
    deny-list keeps pointing at the operator's actual root. Mirrors
    ``kanban_db.kanban_home()`` resolution order:
    1. ``CLOVER_KANBAN_HOME`` env var when set and non-empty
    2. the real (pre-sandbox) Clover root otherwise
    """
    if _PRE_SANDBOX_KANBAN_OVERRIDE:
        return Path(_PRE_SANDBOX_KANBAN_OVERRIDE).expanduser().resolve()
    if _PRE_SANDBOX_CLOVER_HOME and not _clover_home_points_at_production(
        _PRE_SANDBOX_CLOVER_HOME
    ):
        # CLOVER_HOME was genuinely set to a CUSTOM root before the sandbox
        ...
        return get_default_clover_root().resolve()
    # No pre-existing CLOVER_HOME: the real root is the platform default,
    # NOT the sandbox tempdir now sitting in the env.
    return (Path.home() / ".clover").resolve()
...
def _kanban_write_guard(_hermetic_environment, monkeypatch):
    ...
    ``~/.clover`` captured at import time. Hermetic tests that legitimately
    move CLOVER_HOME to sibling tempdirs are unaffected.

    Only patches when ``clover_cli.kanban_db`` is *already imported* — a
    ...
    """
    _kdb = sys.modules.get("clover_cli.kanban_db")
    if _kdb is None:
        return

    # The sys.modules probe can observe the module MID-IMPORT: a fixture
    # boundary firing while another test's lazy `import clover_cli.kanban_db`
    ...
    def _guarded_connect(db_path=None, *args, **kwargs):
        ...
        raise RuntimeError(
            ...
            f"to the real ~/.clover. See #69283."
        )
...
# ``clover_state._ensure_test_isolation`` (the single choke point every
# ``SessionDB()`` construction goes through) refuses, under pytest, any DB
# path that resolves inside the REAL Clover root. This fixture wires the
# test-side knobs:
...
#     installs where CLOVER_HOME is not ~/.clover) into the guard's
...
# so subprocess children that import clover_state directly are covered even
# without this fixture.


@pytest.fixture(autouse=True)
def _state_db_write_guard(request, monkeypatch):
    _hs = sys.modules.get("clover_state")
    ...
    if _PRE_SANDBOX_CLOVER_HOME and not _clover_home_points_at_production(
        ...
        extra_roots.append(
            Path(_PRE_SANDBOX_CLOVER_HOME).expanduser().resolve()
        )
    monkeypatch.setattr(
...
def _reset_tui_gateway_server_state():
    ...
    # A leaked context-local Clover home override redirects every later
    # ``get_clover_home()`` call (active-session registry, config paths)
    # to a stale per-test tmpdir. Force the main-thread ContextVar back
    # to its default.
    try:
        from clover_constants import get_clover_home_override, set_clover_home_override

        if get_clover_home_override() is not None:
            set_clover_home_override(None)
    except Exception:
        pass
...
@pytest.fixture()
def mock_config():
    """Return a minimal clover config dict suitable for unit tests."""
    return {
...
# environment and finds the developer's live ``clover-gateway`` process
...
# correlated with a ``tests/clover_cli/`` pytest run starting up.
#
...
#    ``check_output`` reject any ``systemctl ... <verb> clover-gateway``
...
def _wal_is_usable() -> bool:
    """True when Clover will actually put a database into WAL mode here.

    Clover refuses journal_mode=WAL on SQLite builds carrying the upstream
    ...
    running Clover can link DIFFERENT SQLite versions: a repo ``.venv`` on
    3.50.4 (vulnerable → DELETE) alongside a Clover managed runtime on 3.53.1
    (fixed → WAL). The same test then passes in one and fails in the other.

    IMPORTANT: this must NOT import ``clover_state``. That module computes
    ``DEFAULT_DB_PATH`` from ``get_clover_home()`` at import time, so importing
    it during collection — before the per-test ``_isolate_clover_home`` fixture
    redirects ``CLOVER_HOME`` — permanently caches the DEVELOPER'S REAL
    ``~/.clover/state.db`` for the whole session. Tests then read live
    production sessions instead of a tempdir. The version predicate is
    duplicated from ``clover_state._is_sqlite_wal_reset_vulnerable`` (upstream
...
#      process environment: ``os.environ["CLOVER_VOICE_TTS"] = "1"``. The
...
#      ``clover_cli.voice.speak_text(final_response)`` on a daemon thread.
...
# to mock it:
#
#  • ``clover_cli.voice.speak_text`` — the synth+playback entry point both
#    gateway call sites late-import, so patching the module attribute catches
#    them wherever they import it from.
#  • ``clover_cli.voice.play_audio_file`` — the module-level binding
#    ``speak_text`` actually plays through. Patching the binding inside
#    ``clover_cli.voice`` (not ``tools.voice_mode``) keeps the real function
...
# OS gating
#
# Clover runs on Linux, macOS and native Windows, and a lot of its behaviour
# genuinely differs per host: PTY vs pywinpty, taskkill vs SIGTERM, launchd
# vs systemd, Keychain vs libsecret, ``%LOCALAPPDATA%`` vs ``~/.clover``.
#
...
def pytest_configure(config):  # noqa: D401 — pytest hook
    ...
    config.addinivalue_line(
        ...
        "SQLite WAL mode; skipped on builds where Clover falls back to "
...
def pytest_collection_modifyitems(config, items):  # noqa: D401 — pytest hook
    ...
    reason = (
        f"SQLite {sqlite3.sqlite_version} has the WAL-reset bug — Clover uses "
...
def _live_system_guard(request, monkeypatch):
    ...
    tokens[0]), so ``bash -c "systemctl restart clover-gateway"``,
    ``sudo systemctl ...``, ``env systemctl ...``, ``setsid systemctl ...``
    are all caught. ``pkill``/``killall``/``taskkill`` invocations
    targeting clover/python patterns are also blocked.
    """
    ...
    def _matches_clover_gateway(cmd_str: str) -> bool:
        low = cmd_str.lower()
        return any(tok in low for tok in _CLOVER_TOKENS)

    def _is_blocked_systemctl(cmd) -> bool:
        cmd_str = _cmd_to_string(cmd)
        if "systemctl" not in cmd_str:
            return False
        if not _matches_clover_gateway(cmd_str):
            return False
        try:
    ...
    def _is_process_killer(cmd) -> bool:
        ...
        for tok in killer_tokens:
            ...
            if head in _PROCESS_KILLERS:
                low = cmd_str.lower()
                # pkill -f pattern: catch clover-themed patterns + a
                # plain "python" -f which would catch the live gateway
                # whose cmdline contains "python -m clover_cli.main".
                if (
                    "clover" in low
                    or "gateway" in low
    ...
    def _check_subprocess_cmd(name, cmd):
        if _is_blocked_systemctl(cmd):
            raise RuntimeError(
                ...
                "live clover-gateway systemd unit. Mock "
        ...
        if _is_process_killer(cmd):
            raise RuntimeError(
                ...
                "targeting clover/python could hit the live gateway. "
                "Mark with @pytest.mark.live_system_guard_bypass if "
                "intentional."
            )
        # Block any subprocess that would run `clover update` (or the
        # equivalent `python -m clover_cli.main update`).  These commands
        ...
        if "update" in low and (
            # clover update / clover update --gateway / setsid bash -c ... clover update
            ("clover" in low and "update" in low.split())
            or
            # python -m clover_cli.main update --gateway
            ("clover_cli" in low and "update" in low.split())
            or
            # venv/bin/clover update  (absolute path variant used in tests)
            (".venv/bin/clover" in low and "update" in low)
        ):
            raise RuntimeError(
                f"tests/conftest.py live-system guard: blocked "
                f"subprocess.{name}({cmd!r}) — this command would run "
                "`clover update` against the real checkout, fetching "
...
def _audio_playback_guard(request, monkeypatch):
    ...
    depth behind ``_CLOVER_BEHAVIORAL_VARS``: the env blanking stops the flag
    ...
    try:
        import clover_cli.voice as _voice
    except Exception:
