...
from clover_cli.plugins import PluginManager, VALID_HOOKS


def _make_enabled_plugin(clover_home: Path, name: str, register_body: str) -> Path:
    """Create a plugin under <clover_home>/plugins/<name> and opt it in."""
    plugin_dir = clover_home / "plugins" / name
...
def test_no_plugins_returns_empty_results(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("CLOVER_HOME", str(tmp_path / "clover_empty"))
