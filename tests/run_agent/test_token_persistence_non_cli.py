...
def test_session_search_lazily_opens_db_when_entrypoint_did_not_pass_one(monkeypatch):
    ...
    clover_state = ModuleType("clover_state")
    clover_state.SessionDB = FakeSessionDB
    monkeypatch.setitem(sys.modules, "clover_state", clover_state)
    ...
    result = json.loads(agent._invoke_tool(
        "session_search",
        {"query": "Clover", "detail": "full"},
        "task-id",
    ))

    assert result["success"] is True
    assert captured["db"] is sentinel_db
    assert captured["query"] == "Clover"
...
def test_sequential_session_search_forwards_detail(monkeypatch):
    ...
    tool_call = SimpleNamespace(
        ...
        function=SimpleNamespace(
            name="session_search",
            arguments=json.dumps({"query": "Clover", "detail": "full"}),
        ),
    )
