"""Regression guard: a genuine Clover 429 must re-enter the retry loop so the
top-of-loop Clover rate-limit guard can activate the fallback chain.
...
``record_clover_rate_limit()`` moments earlier and either activates a fallback
