"""Tests for Clover fallback local-availability suppression.

Blocker if Clover token material is missing locally: the fallback chain
should not repeatedly attempt Clover resolution; it must skip and continue
...
class TestNousFallbackLocalAvailability:
    def test_missing_clover_token_is_skipped_once(self):
        """Clover fallback is skipped when no access/refresh token is stored."""
        agent = _make_agent(
            fallback_model=[
    ...
    def test_clover_unavailable_not_retried_in_same_session(self):
        """After Clover is skipped once, subsequent activations continue further."""
        agent = _make_agent(
            fallback_model=[
                {"provider": "clover", "model": "anthropic/claude-sonnet-4.6"},
                {"provider": "openai-codex", "model": "gpt-5.5"},
