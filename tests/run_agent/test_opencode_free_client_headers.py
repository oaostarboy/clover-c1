...
@patch("run_agent.OpenAI")
def test_opencode_free_sends_clover_attribution(mock_openai):
    """Keyless requests still identify as Clover (attribution headers match
    ...
    headers = _zen_call_headers(mock_openai)
    assert headers.get("X-Title") == "Clover Cognition"
    assert str(headers.get("User-Agent", "")).startswith("CloverAgent/")
