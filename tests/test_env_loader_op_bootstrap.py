...
1. ``load_clover_dotenv()`` auto-loads ``~/.clover/.op.env`` so the
...
def test_op_env_autoloads_bootstrap_token_in_cron_context(tmp_path, monkeypatch):
    ...
    env_loader.load_clover_dotenv(clover_home=home)
...
def _seed_openrouter_token(monkeypatch, dotenv_value, environ_value):
    ...
    monkeypatch.setattr(
        "clover_cli.auth.is_source_suppressed", lambda _p, _s: False
    )
