...
def test_registered_observable_metric_names_cover_snapshot_metrics(monkeypatch):
    ...
    gateway_snapshot = type("S", (), {"metrics": [
        _M("clover.gateway.up"), _M("clover.gateway.active_agents"),
        _M("clover.gateway.busy"), _M("clover.gateway.drainable"),
        _M("clover.gateway.restart_requested"),
        _M("clover.platform.up"), _M("clover.platform.degraded"),
    ]})()
    cron_snapshot = type("S", (), {"metrics": [
        _M("clover.cron.scheduler.heartbeat_age_seconds"),
        _M("clover.cron.scheduler.last_success_age_seconds"),
        _M("clover.cron.scheduler.catch_up_occurrences"),
        _M("clover.cron.jobs.enabled"), _M("clover.cron.jobs.running"),
        _M("clover.cron.jobs.overdue"),
    ]})()
...
def test_monitoring_docs_distinguish_relay_health_scope_and_terminal_flush():
    ...
    assert "Clover Cognition-owned Relay transport health" in text
