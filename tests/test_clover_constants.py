...
from clover_constants import (
    ...
    find_clover_node_executable,
    find_node_executable,
    find_node_executable_on_path,
    get_default_clover_root,
    get_clover_dir,
    get_clover_home,
    get_process_clover_home,
    heal_clover_managed_node,
    ...
    with_clover_node_path,
)


class TestGetDefaultCloverRoot:
    """Tests for get_default_clover_root() — Docker/custom deployment awareness."""

    def test_no_clover_home_returns_native(self, tmp_path, monkeypatch):
        """When CLOVER_HOME is not set, returns ~/.clover."""
        monkeypatch.delenv("CLOVER_HOME", raising=False)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)

        assert get_default_clover_root() == tmp_path / ".clover"
    ...
    def test_docker_profile_active(self, tmp_path, monkeypatch):
        """When a Docker profile is active (CLOVER_HOME=<root>/profiles/<name>),
        returns the Docker root, not the profile dir."""
        docker_root = tmp_path / "opt" / "data"
        profile = docker_root / "profiles" / "coder"
        profile.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("CLOVER_HOME", str(profile))
        assert get_default_clover_root() == docker_root

    @pytest.mark.windows_only
    def test_no_clover_home_returns_localappdata_root_on_windows(self, tmp_path, monkeypatch):
        """Native Windows falls back to %LOCALAPPDATA%\\clover, not ~/.clover."""
        local_appdata = tmp_path / "LocalAppData"
        monkeypatch.delenv("CLOVER_HOME", raising=False)
        monkeypatch.setenv("LOCALAPPDATA", str(local_appdata))
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "Home")

        assert get_default_clover_root() == local_appdata / "clover"

    def test_result_memoised_until_env_or_home_changes(self, tmp_path, monkeypatch):
        """Repeated calls reuse the memo; CLOVER_HOME / home changes invalidate.

        get_default_clover_root() resolves CLOVER_HOME against the native
        ...
        (native home, CLOVER_HOME) compared for free each call.
        """
        # CLOVER_HOME set to a Docker-profile path: every call resolves the
        ...
        # _get_platform_default_clover_home() on every call (even hits), so
        ...
        monkeypatch.setattr(
            clover_constants, "_default_clover_root_memo", None, raising=False
        )

        first = get_default_clover_root()
        ...
        # CLOVER_HOME change invalidates the memo (fresh resolution).
        other_profile = docker_root / "profiles" / "writer"
        other_profile.mkdir(parents=True)
        monkeypatch.setenv("CLOVER_HOME", str(other_profile))
        before = resolve_calls["n"]
        assert get_default_clover_root() == docker_root
        assert resolve_calls["n"] > before, (
            "CLOVER_HOME change must force a fresh resolution"
        )
...
class TestGetCloverHome:
    """Tests for get_clover_home() platform-aware fallback."""

    @pytest.mark.windows_only
    def test_windows_fallback_uses_localappdata(self, tmp_path, monkeypatch):
        """When CLOVER_HOME is unset on Windows, use %LOCALAPPDATA%\\clover."""
        local_appdata = tmp_path / "LocalAppData"
        monkeypatch.delenv("CLOVER_HOME", raising=False)
        monkeypatch.setenv("LOCALAPPDATA", str(local_appdata))
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "Home")
        monkeypatch.setattr(clover_constants, "_profile_fallback_warned", False)

        assert get_clover_home() == local_appdata / "clover"


class TestGetProcessCloverHome:
    """Tests for get_process_clover_home() — process launch scope.
    ...
    def test_env_set_returns_that_path(self, tmp_path, monkeypatch):
        ...
        assert get_process_clover_home() == home




class TestCloverManagedNode:
    @pytest.mark.windows_only
    def test_windows_node_dir_prefers_portable_root(self, tmp_path, monkeypatch):
        ...
        assert iter_clover_node_dirs() == [node_dir, bin_dir]
    ...
    def test_windows_finds_npm_cmd_before_path(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("CLOVER_HOME", str(home))
        monkeypatch.setattr(clover_constants, "node_tool_runnable", lambda path: True)

        assert find_clover_node_executable("npm") == str(npm_cmd)
    ...
    def test_windows_skips_broken_managed_npm_without_path_fallback(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(clover_constants, "_managed_node_heal_attempted", False)
        monkeypatch.setattr(clover_constants, "heal_clover_managed_node", lambda: False)
        monkeypatch.setattr(
            clover_constants,
            "node_tool_runnable",
            lambda path: False,
        ...
        assert clover_managed_node_tree_present() is True
...
class TestNodeToolRunnable:
    """node_tool_runnable() rejects broken Clover-managed npm/node wrappers."""
    ...
    def test_broken_managed_npm_heals_when_node_still_runs(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(clover_constants, "heal_clover_managed_node", _heal)
    ...
    def test_outdated_managed_node_heals_to_target_major(self, tmp_path, monkeypatch):
        ...
        target = clover_constants._CLOVER_NODE_TARGET_MAJOR
        profile_home = tmp_path / "profiles" / "assistant"
        managed_bin = profile_home / "node" / "bin"
        managed_bin.mkdir(parents=True)
        old_node = self._stub(
        ...
        resolved = clover_constants.find_clover_node_executable("node")
        assert heal_called["value"] is True
    ...
    def test_outdated_managed_node_survives_failed_heal(self, tmp_path, monkeypatch):
        ...
        assert clover_constants.find_clover_node_executable("node") == str(old_node)
    ...
    def test_target_major_managed_node_does_not_heal(self, tmp_path, monkeypatch):
        ...
        assert clover_constants.find_clover_node_executable("node") == str(node)
...
class TestIsContainer:
    ...
    def _reset_cache(self, monkeypatch):
        ...
        monkeypatch.setattr(clover_constants, "_container_detected", None)
    ...
    def test_caches_result(self, monkeypatch):
        ...
        monkeypatch.setattr(clover_constants, "_container_detected", True)
        assert is_container() is True
...
class TestResolvePerModelReasoningEffort:
    ...
    def test_exact_match(self):
        ...
        from clover_constants import resolve_per_model_reasoning_effort
...
class TestSecureParentDir:
    ...
    def test_safe_path_calls_chmod(self, tmp_path, monkeypatch):
        ...
        safe_dir = tmp_path / "home" / "user" / ".clover"
    ...
    def test_install_tree_skipped(self, monkeypatch):
        ...
        Regression test for #93050: secure_parent_dir() chmod'd /opt/clover to
        0700 because it has 3 path parts and passed the ``< 3`` guard, locking
        out UID 10000 (clover user) from traversing the install dir.
        """
        install_root = Path(clover_constants.__file__).resolve().parent

        # Directly under the install root (e.g. /opt/clover/auth.json)
    ...
    def test_install_tree_siblings_still_hardened(self, monkeypatch):
        ...
        # Prefix-named sibling of the install root (/opt/clover-data/...).
...
class TestAgentBrowserRunnable:
    ...
    def test_version_probe_uses_windows_hide_flags(self, tmp_path, monkeypatch):
        ...
        import clover_cli._subprocess_compat as subprocess_compat
...
class TestGetCloverDir:
    """Tests for ``get_clover_dir(new_subpath, old_name)``.
    ...
    def test_neither_exists_returns_new(self, tmp_path, monkeypatch):
        ...
        result = get_clover_dir("platforms/pairing", "pairing")
    ...
    def test_legacy_is_file_treated_as_content(self, tmp_path, monkeypatch):
        ...
        result = get_clover_dir("cache/images", "image_cache")
...
class TestWslPathTranslation:
    ...
    def test_windows_drive_to_wsl_mount(self):
        assert clover_constants.windows_path_to_wsl(r"C:\Users\alex") == "/mnt/c/Users/alex"
        assert clover_constants.windows_path_to_wsl("C:/Users/alex") == "/mnt/c/Users/alex"
        assert clover_constants.windows_path_to_wsl("D:\\") == "/mnt/d/"

    def test_windows_drive_ignores_non_drive_paths(self):
        assert clover_constants.windows_path_to_wsl("/home/alex") is None
        assert clover_constants.windows_path_to_wsl("relative\\dir") is None




    def test_translate_maps_windows_and_unc_on_wsl(self, monkeypatch):
        monkeypatch.setattr(clover_constants, "is_wsl", lambda: True)
        assert clover_constants.translate_cwd_for_wsl_backend(r"C:\Users\alex") == "/mnt/c/Users/alex"
        assert clover_constants.translate_cwd_for_wsl_backend(r"\\wsl.localhost\Ubuntu\home\alex") == "/home/alex"
        # Already-POSIX paths pass through untouched.
        assert clover_constants.translate_cwd_for_wsl_backend("/home/alex") == "/home/alex"
...
class TestManagedNodeTreeInUse:
    ...
    def test_always_false_off_windows(self, tmp_path, monkeypatch):
        monkeypatch.setattr(clover_constants.sys, "platform", "darwin")
        monkeypatch.setenv("CLOVER_HOME", str(tmp_path))
        assert clover_constants.managed_node_tree_in_use() is False
    ...
    def test_exe_under_node_dir_counts(self, tmp_path, monkeypatch):
        ...
        assert clover_constants.managed_node_tree_in_use() is True
...
class TestWindowsHealStageSwap:
    ...
    def _stub_env(
        ...
        monkeypatch.setenv("CLOVER_HOME", str(home))
        monkeypatch.setenv(
            "CLOVER_NODE_TARGET_MAJOR",
            str(clover_constants._CLOVER_NODE_TARGET_MAJOR),
        )
        monkeypatch.setattr(clover_constants, "_managed_node_heal_attempted", False)
        monkeypatch.setattr(
            clover_constants, "_managed_node_in_use_notice_printed", False
        )
        monkeypatch.setattr(
            clover_constants,
        ...
        monkeypatch.setattr(
            clover_constants, "node_tool_runnable", lambda path: True
        )
    ...
    def test_in_use_defers_without_touching_tree(self, tmp_path, monkeypatch):
        ...
        zip_name, zip_bytes = _make_node_zip(clover_constants._CLOVER_NODE_TARGET_MAJOR)
        ...
        result = clover_constants._heal_managed_node_windows()
...
class TestHealAttemptFlagSemantics:
    ...
    def test_deferral_keeps_flag_clear_and_retries(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(clover_constants, "_heal_managed_node_windows", fake_heal)

        assert heal_clover_managed_node() is False
        assert clover_constants._managed_node_heal_attempted is False
    ...
    def test_real_failure_records_attempt(self, tmp_path, monkeypatch):
        ...
        assert heal_clover_managed_node() is False
        assert clover_constants._managed_node_heal_attempted is True
