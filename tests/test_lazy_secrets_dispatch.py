...
1. `clover secrets bitwarden setup --help` works (dispatch path)
2. `clover update --check` works (update path)
3. `clover secrets bitwarden disable` works (handler execution)
4. `clover secrets onepassword status` works (lazy backend loads on demand)
...
def _run_clover(args: list[str], timeout: int = 30) -> subprocess.CompletedProcess[str]:
    """Run clover CLI as a subprocess from repo root.
    ...
    connectivity — but ``clover update --check`` really does ``git fetch``
    ...
    env.update(
        {
            ...
            "GIT_CONFIG_KEY_0": "url.file:///nonexistent-clover-test-remote/.insteadOf",
            "GIT_CONFIG_VALUE_0": "https://",
            "GIT_CONFIG_KEY_1": "url.file:///nonexistent-clover-test-remote/.insteadOf",
...
class TestSecretsDispatchE2E:
    ...
    def test_bitwarden_status(self) -> None:
        """`clover secrets bitwarden status` must exit 0 (runs lazy backend)."""
        result = _run_clover(["secrets", "bitwarden", "status"])
    ...
    def test_bitwarden_disable(self) -> None:
        """`clover secrets bitwarden disable` must exit 0."""
        result = _run_clover(["secrets", "bitwarden", "disable"])
        assert result.returncode == 0, (
    ...
    def test_onepassword_status(self) -> None:
        """`clover secrets onepassword status` must exit 0 (1Password lazy backend)."""
        result = _run_clover(["secrets", "onepassword", "status"])
        assert result.returncode in (0, 1), (
    ...
    def test_onepassword_setup_help(self) -> None:
        """`clover secrets onepassword setup --help` must exit 0."""
        result = _run_clover(["secrets", "onepassword", "setup", "--help"])
...
class TestUpdatePathE2E:
    ...
    These tests invoke the real `clover update --check` path as a subprocess.
    ...
    def test_update_check_clean(self) -> None:
        """`clover update --check` must not load cryptography._rust."""
        result = _run_clover(["update", "--check"])
    ...
    def test_main_update_check_crypto_absent_in_sys_modules(self) -> None:
        """Decisive invariant: invoking main() with argv=['clover','update','--check']
...
sys.argv = ['clover', 'update', '--check']

import clover_cli.main as m
...
# resolves the clover_cli.main module's lazily re-exported attribute —
# so the patch must land on clover_cli.main._cmd_update_check.
with patch('clover_cli.main._cmd_update_check', capture_update_check):
    try:
        m.main()
