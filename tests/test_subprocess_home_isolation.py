...
Clover state stays profile-scoped through CLOVER_HOME. Host subprocesses should
...
class TestGetSubprocessHome:
    """Unit tests for clover_constants.get_subprocess_home()."""

    def _host_mode(self, monkeypatch):
        monkeypatch.setattr(clover_constants, "is_container", lambda: False)
        monkeypatch.delenv("TERMINAL_HOME_MODE", raising=False)
        monkeypatch.delenv("CLOVER_REAL_HOME", raising=False)
    ...
    def test_host_auto_keeps_real_home_when_profile_home_exists(self, tmp_path, monkeypatch):
        ...
        clover_home = real_home / ".clover" / "profiles" / "coder"
        profile_home = clover_home / "home"
    ...
    def test_real_mode_repairs_parent_home_already_pointing_at_profile(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("CLOVER_REAL_HOME", str(real_home))

        from clover_constants import get_subprocess_home, get_real_home
    ...
    def test_two_profiles_get_different_homes(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("CLOVER_HOME", str(base / "alpha"))
        home_a = get_subprocess_home()

        monkeypatch.setenv("CLOVER_HOME", str(base / "beta"))
...
class TestMakeRunEnvHomeInjection:
    ...
    def test_host_auto_preserves_real_home_when_profile_home_exists(self, tmp_path, monkeypatch):
        ...
        assert result["CLOVER_REAL_HOME"] == str(real_home)
    ...
    def test_profile_mode_injects_profile_home_when_profile_home_exists(self, tmp_path, monkeypatch):
        ...
        assert result["HOME"] == str(clover_home / "home")
        assert result["CLOVER_REAL_HOME"] == str(real_home)
    ...
    def test_no_injection_when_clover_home_unset(self, monkeypatch):
...
class TestProfileBootstrap:
    ...
    def test_profile_dirs_includes_home(self):
        from clover_cli.profiles import _PROFILE_DIRS
