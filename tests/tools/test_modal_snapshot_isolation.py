...
def _restore_tool_modules():
    original_clover_home = os.environ.get("CLOVER_HOME")
    original_modules = {
        name: module
    ...
    try:
        ...
        if original_clover_home is None:
            os.environ.pop("CLOVER_HOME", None)
        else:
            os.environ["CLOVER_HOME"] = original_clover_home
        _reset_modules(("tools", "clover_cli", "modal"))
...
def _install_modal_test_modules(
    ...
    *,
    ...
    clover_cli = types.ModuleType("clover_cli")
    clover_cli.__path__ = []  # type: ignore[attr-defined]
    sys.modules["clover_cli"] = clover_cli
    clover_home = tmp_path / "clover-home"
    os.environ["CLOVER_HOME"] = str(clover_home)
    sys.modules["clover_cli.config"] = types.SimpleNamespace(
        get_clover_home=lambda: clover_home,
    )
    ...
    async def _lookup_aio(_name: str, create_if_missing: bool = False):
        return types.SimpleNamespace(name="clover-c1", create_if_missing=create_if_missing)
    ...
    return {
        "snapshot_store": clover_home / "modal_snapshots.json",
