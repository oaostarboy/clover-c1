...
class TestGating:
    ...
    def test_a_profile_sees_a_credential_held_at_the_global_root(self, tmp_path, monkeypatch):
        ...
        # clover_cli.auth, not here.
        monkeypatch.delenv("TOOL_GATEWAY_USER_TOKEN", raising=False)
        root = tmp_path / "root"
        (root / "profiles" / "work").mkdir(parents=True)
        (root / "auth.json").write_text(
            json.dumps({"version": 1, "providers": {"clover": {"access_token": "root-token"}}}),
            encoding="utf-8",
        )
        monkeypatch.setenv("CLOVER_HOME", str(root / "profiles" / "work"))
...
class TestPollTransport:
    ...
    def test_a_cli_session_is_recognised_by_its_source(self, tmp_path, monkeypatch):
        # The CLI, TUI, and desktop leave CLOVER_SESSION_PLATFORM empty and
        # identify themselves on CLOVER_SESSION_SOURCE instead, so keying only
        # on the platform would miss them.
        monkeypatch.delenv("CLOVER_SESSION_PLATFORM", raising=False)
        monkeypatch.setenv("CLOVER_SESSION_SOURCE", "tui")
        response = _FakeResponse(200, {
            "id": "bfl_job_1",
            "status": "Ready",
            "result": {"sample": "https://cdn.example/x/flux3-clip.mp4?sig=a"},
