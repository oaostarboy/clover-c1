...
Covers the canonical fix for issues #4146, #27303, #30882, #33057:
  ...
  4. tools.code_execution_tool._scrub_child_env — broad CLOVER_ prefix dropped,
...
def gw_session(monkeypatch):
    """A clean gateway session: CLOVER_GATEWAY_SESSION set, a bound session
...
def test_env_scrub_clover_allowlist_and_secret_blocks():
    from tools.code_execution_tool import _scrub_child_env

    env = {
        # operational allowlist → kept
        "CLOVER_HOME": "/h", "CLOVER_PROFILE": "p",
        "CLOVER_CONFIG": "/c.yaml", "CLOVER_ENV": "/e",
        "CLOVER_DELEGATED_CHILD_CONTEXT": "1",
        # other CLOVER_* → dropped (broad prefix removed)
        "CLOVER_BASE_URL": "https://x", "CLOVER_INTERACTIVE": "1",
        "CLOVER_KANBAN_DB": "postgres://u:p@h/db",
    ...
    for kept in (
        ...
        "CLOVER_DELEGATED_CHILD_CONTEXT", "PATH",
    ):
        assert kept in out, f"{kept} should be kept"
    for dropped in (
        "CLOVER_BASE_URL", "CLOVER_INTERACTIVE", "CLOVER_KANBAN_DB",
...
def test_env_scrub_no_log_when_nothing_dropped(caplog):
    """No diagnostic noise when there are no dropped CLOVER_* vars."""
    import logging
