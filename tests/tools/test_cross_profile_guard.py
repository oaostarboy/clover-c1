...
@pytest.fixture
def fake_clover(tmp_path, monkeypatch):
    """Build a two-profile Clover layout and point CLOVER_HOME at
    the clover-security profile (matching the original-incident shape).
    """
    root = tmp_path / "fake-clover"
    (root / "skills" / "shared-skill").mkdir(parents=True)
    (root / "skills" / "shared-skill" / "SKILL.md").write_text(
        "---\nname: shared-skill\ndescription: default copy.\n---\n"
    )

    sec_home = root / "profiles" / "clover-security"
    (sec_home / "skills").mkdir(parents=True)

    coder_home = root / "profiles" / "coder"
    (coder_home / "skills").mkdir(parents=True)

    monkeypatch.setenv("CLOVER_HOME", str(sec_home))

    import clover_constants
    monkeypatch.setattr(clover_constants, "get_default_clover_root", lambda: root)

    import agent.file_safety as fs
    monkeypatch.setattr(fs, "_clover_home_path", lambda: sec_home)
    monkeypatch.setattr(fs, "_clover_root_path", lambda: root)
...
class TestWriteFileCrossProfileGuard:
    def test_in_profile_write_allowed(self, fake_clover):
        from tools.file_tools import write_file_tool
        target = fake_clover["sec_home"] / "skills" / "new-skill" / "SKILL.md"
    ...
    def test_cross_profile_write_blocked_by_default(self, fake_clover):
        """The May 2026 incident — security-profile session edits default
        profile's skill. Must be blocked."""
        from tools.file_tools import write_file_tool
        target = fake_clover["root"] / "skills" / "shared-skill" / "SKILL.md"
        ...
        assert "clover-security" in result["error"]
        # File untouched.
        assert target.read_text() == original


    def test_non_clover_path_unaffected(self, fake_clover, tmp_path):
        from tools.file_tools import write_file_tool
...
class TestPatchCrossProfileGuard:
    def test_cross_profile_patch_blocked(self, fake_clover):
    ...
    def test_cross_profile_patch_bypass(self, fake_clover):
        from tools.file_tools import patch_tool
        target = fake_clover["root"] / "skills" / "shared-skill" / "SKILL.md"
        result_json = patch_tool(
            mode="replace",
    ...
    def test_v4a_patch_extracts_path_for_guard(self, fake_clover):
...
class TestSkillManageCrossProfileErrorUX:
    ...
    def test_error_names_other_profile_when_skill_lives_there(
        self, fake_clover, monkeypatch
    ):
        """The original incident shape — model expects 'foo' in active
        profile, but 'foo' lives in default. Error must point at default."""
        self._make_skill_in_profile(fake_clover["root"], "default-only-skill")

        # Re-import the module so SKILLS_DIR picks up CLOVER_HOME (set in
        ...
        assert "not found in active profile 'clover-security'" in err
...
class TestSystemPromptActiveProfile:
    def test_default_profile_line_in_prompt(self, tmp_path, monkeypatch):
        ...
        about ~/.clover/profiles/<name>/."""
        # Don't set CLOVER_HOME — falls back to default.
        import agent.file_safety as fs
        monkeypatch.setattr(fs, "_clover_home_path", lambda: tmp_path / "fake")
        monkeypatch.setattr(fs, "_clover_root_path", lambda: tmp_path / "fake")
    ...
    def test_named_profile_line_in_prompt_text(self, fake_clover):
        """When active profile is 'clover-security', the prompt warns
        ...
        assert "Active Clover profile" in src
        assert "cross_profile=True" in src
        assert "~/.clover/profiles/" in src
        # Both branches present (default and named profile).
        assert "Active Clover profile: default" in src
        assert "Active Clover profile: {active_profile}" in src
