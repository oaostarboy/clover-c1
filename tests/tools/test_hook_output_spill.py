...
class GetSpillConfigTests(unittest.TestCase):
    ...
    def test_load_config_exception_is_swallowed(self):
        with patch("clover_cli.config.load_config", side_effect=RuntimeError("bad")):
            cfg = hos.get_spill_config()
        self.assertEqual(cfg["max_chars"], hos.DEFAULT_MAX_CHARS)
        self.assertTrue(cfg["enabled"])
...
class SpillIfOversizedTests(unittest.TestCase):
    def setUp(self):
        self.tmpdir = tempfile.mkdtemp(prefix="clover-spill-test-")
    ...
    def test_default_directory_uses_clover_home(self):
        """When no directory override, spill under CLOVER_HOME/hook_outputs."""
        test_home = tempfile.mkdtemp(prefix="clover-home-")
        try:
            with patch.dict(os.environ, {"CLOVER_HOME": test_home}):
                # Also patch get_clover_home to the env var to mirror production.
            ...
            # ~/.clover/hook_outputs depending on get_clover_home behaviour.
            candidates = [
                Path(test_home) / "hook_outputs" / "sess",
                Path(os.path.expanduser("~/.clover/hook_outputs/sess")),
            ]
