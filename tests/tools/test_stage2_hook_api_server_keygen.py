...
was gated on ``[ -f "$CLOVER_HOME/.env" ]`` while the first-boot seed that
...
def _run_keygen(
    ...
    script = (
        ...
        f"{env_setup}"
        f'CLOVER_HOME="{home}"\n'
        # In tests we run unprivileged; as_clover is a passthrough then.
        'as_clover() { "$@"; }\n'
...
def test_keygen_skips_when_container_env_provides_key(
    ...
    Clover loads $CLOVER_HOME/.env with override=True, so a key generated
...
def test_dockerignore_keeps_env_example_template() -> None:
    """The first-boot seed copies /opt/clover/.env.example -> $CLOVER_HOME/.env.
    ...
    assert verdict == "included", (
        ...
        "seeds $CLOVER_HOME/.env from it on first boot"
    )
