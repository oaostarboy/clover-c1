...
class TestWriteDenyExactPaths:
    ...
    def test_clover_root_env_when_running_under_profile(self, tmp_path, monkeypatch):
        ...
        ``~/.clover/.env`` (whose credentials are inherited by every profile)
        could be silently overwritten by ``write_file`` while a profile was
        active.
        """
        root = tmp_path / "clover_root"
        ...
        # Sanity check: CLOVER_HOME does point to the profile dir, not the root.
...
class TestWriteAllowed:
    ...
    def test_clover_control_files_requested_writable(self):
