...
Covers:
- :class:`CloverTokenStorage` ``.meta.json`` roundtrip (save / load / remove)
- The production manager provider
  (:class:`tools.mcp_oauth_manager.CloverMCPOAuthProvider`) restoring metadata
...
from tools.mcp_oauth_manager import _CLOVER_PROVIDER_CLS
...
# ---------------------------------------------------------------------------
# CloverTokenStorage metadata roundtrip
# ---------------------------------------------------------------------------
...
class TestMetadataStorage:
    def test_save_and_load_roundtrip(self, tmp_path, monkeypatch):
        ...
        storage = CloverTokenStorage("example-server")
    ...
    def test_remove_deletes_meta_file(self, tmp_path, monkeypatch):
        monkeypatch.setenv("CLOVER_HOME", str(tmp_path))
        storage = CloverTokenStorage("cleanup-server")
...
# ---------------------------------------------------------------------------
# Manager-path provider (CloverMCPOAuthProvider) — production code path
# ---------------------------------------------------------------------------


def _manager_provider_with_context(storage: CloverTokenStorage, **context_attrs):
    ...
    provider = _CLOVER_PROVIDER_CLS.__new__(_CLOVER_PROVIDER_CLS)
    provider._clover_server_name = context_attrs.get("server_name", "srv")
    context = MagicMock()
...
class TestManagerOAuthProviderMetadata:
    def test_initialize_restores_metadata_from_disk(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("CLOVER_HOME", str(tmp_path))
        storage = CloverTokenStorage("mgr-srv")
        ...
        with patch.object(
            _CLOVER_PROVIDER_CLS.__bases__[0], "_initialize", new=AsyncMock()
        ):
    ...
    def test_async_auth_flow_persists_on_completion(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("CLOVER_HOME", str(tmp_path))
        storage = CloverTokenStorage("flow-srv")
        ...
        with patch.object(
            _CLOVER_PROVIDER_CLS.__bases__[0],
            "async_auth_flow",
