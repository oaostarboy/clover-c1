...
Older images made /opt/clover/ui-tui and /opt/clover/node_modules writable so a
...
def test_dockerfile_uses_prebuilt_tui_instead_of_writable_node_modules() -> None:
    ...
    assert "ENV CLOVER_TUI_DIR=/opt/clover/ui-tui" in text
    assert "cd ../ui-tui && npm run build" in text
    assert "chown -R clover:clover /opt/clover/ui-tui" not in text
    assert "chown -R clover:clover /opt/clover/node_modules" not in text
