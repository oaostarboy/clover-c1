...
class TestDirHash:
    ...
    def test_generated_bytecode_is_not_a_user_edit(self, tmp_path):
        """Running a skill's script must not make the skill look user-edited.

        Python writes __pycache__/*.pyc next to any script it imports. The user
        never wrote those bytes. When _dir_hash counted them, `clover update`
        classified the skill as user-modified and skipped updating it, which is
        how a shipped fix could be held back on a skill nobody had touched.
        """
        skill = tmp_path / "skill"
        (skill / "scripts").mkdir(parents=True)
        (skill / "SKILL.md").write_text("# Stock, never edited")
        (skill / "scripts" / "helper.py").write_text("print(1)")

        stock = _dir_hash(skill)

        # Python generates these by itself, on import. No user action.
        cache = skill / "scripts" / "__pycache__"
        cache.mkdir()
        (cache / "helper.cpython-314.pyc").write_bytes(b"\x00bytecode\x00")

        assert _dir_hash(skill) == stock, (
            "generated bytecode changed the skill hash, so an untouched skill "
            "reads as user-modified and stops receiving updates"
        )

    def test_other_generated_caches_are_ignored_too(self, tmp_path):
        """The same holds for the other caches tools drop inside a directory."""
        skill = tmp_path / "skill"
        skill.mkdir()
        (skill / "SKILL.md").write_text("# Stock")
        stock = _dir_hash(skill)

        for cache_dir in (".pytest_cache", ".mypy_cache", ".ruff_cache"):
            junk = skill / cache_dir
            junk.mkdir()
            (junk / "data").write_text("generated")

        assert _dir_hash(skill) == stock

    def test_a_real_edit_still_registers(self, tmp_path):
        """The guard must keep working: real edits still change the hash."""
        skill = tmp_path / "skill"
        (skill / "scripts").mkdir(parents=True)
        (skill / "SKILL.md").write_text("# Stock")
        (skill / "scripts" / "helper.py").write_text("print(1)")
        ...
        # An edit to a script the user owns must still be detected.
        (skill / "scripts" / "helper.py").write_text("print(2)")
        assert _dir_hash(skill) != stock

        # So must a new file the user adds.
        (skill / "scripts" / "helper.py").write_text("print(1)")
        assert _dir_hash(skill) == stock
...
class TestGeneratedFilesDoNotBlockUpdates:
    """Regression: an untouched skill must keep receiving updates.

    Reported after a real `clover update` on three machines. Every one
    announced "1 user-modified bundled skill (kept as-is)" for a skill the
    user had never opened. The skill was google-workspace, and it was the one
    skill still carrying the Windows-path bug the update was shipping the fix
    for: the protection meant to save user edits was preserving a bug instead.

    Cause: the skill ships scripts. Python compiles a script on import and
    writes __pycache__/*.pyc beside it. The hash counted those generated
    bytes, so the on-disk copy no longer matched its own stock hash.
    """

    def _bundled(self, tmp_path):
        bundled = tmp_path / "bundled_skills"
        skill = bundled / "productivity" / "google-workspace"
        (skill / "scripts").mkdir(parents=True)
        (skill / "SKILL.md").write_text(
            "---\nname: google-workspace\n---\n# Uses $CLOVER_HOME\n"
        )
        (skill / "scripts" / "helper.py").write_text("print('stock')\n")
        return bundled
    ...
    def test_running_a_skill_script_does_not_freeze_its_updates(self, tmp_path):
        ...
        with self._patches(bundled, skills_dir, manifest_file):
            ...
            assert "google-workspace" in first["copied"]

            dest = skills_dir / "productivity" / "google-workspace"

            # The agent runs the skill. Python writes bytecode by itself.
            cache = dest / "scripts" / "__pycache__"
            cache.mkdir(parents=True)
            (cache / "helper.cpython-314.pyc").write_bytes(b"\x00generated\x00")

            # Upstream ships a fix to the same skill.
            (bundled / "productivity" / "google-workspace" / "SKILL.md").write_text(
                "---\nname: google-workspace\n---\n# Windows path fixed\n"
            )
            ...
            assert "google-workspace" not in second["user_modified"], (
                "generated bytecode made an untouched skill look user-edited, "
                "so the update was withheld"
            )
            assert "Windows path fixed" in (dest / "SKILL.md").read_text()

    def test_a_real_user_edit_is_still_protected(self, tmp_path):
        """The guard must keep its purpose: real edits are never overwritten."""
        bundled = self._bundled(tmp_path)
        skills_dir = tmp_path / "user_skills"
        manifest_file = skills_dir / ".bundled_manifest"
        ...
        with self._patches(bundled, skills_dir, manifest_file):
            ...
            # The user genuinely edits the skill.
            (dest / "SKILL.md").write_text("---\nname: google-workspace\n---\n# MINE\n")
            ...
            assert "google-workspace" in result["user_modified"]
            assert "# MINE" in (dest / "SKILL.md").read_text()

    def test_diff_does_not_report_generated_bytecode(self, tmp_path):
        """`clover skills diff` listed a .pyc as a user addition. It is not."""
        bundled = self._bundled(tmp_path)
        skills_dir = tmp_path / "user_skills"
        manifest_file = skills_dir / ".bundled_manifest"
        ...
        with self._patches(bundled, skills_dir, manifest_file):
            ...
            result = diff_bundled_skill("google-workspace")
            reported = [d["path"] for d in result["diffs"]]

            assert not any("__pycache__" in p for p in reported), (
                f"diff reported generated bytecode as a user change: {reported}"
            )
