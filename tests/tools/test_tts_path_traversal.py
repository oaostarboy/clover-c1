...
def test_output_path_rejects_clover_oauth_store(tmp_path, monkeypatch):
    """TTS output_path must not bypass the shared protected-file write guard."""
    import agent.file_safety as file_safety

    clover_home = tmp_path / "clover-home"
    clover_home.mkdir()
    monkeypatch.setattr(file_safety, "_clover_home_path", lambda: clover_home)
    monkeypatch.setattr(file_safety, "_clover_root_path", lambda: clover_home)

    target = clover_home / ".anthropic_oauth.json"
    result = json.loads(text_to_speech_tool(
        text="hello",
...
def test_output_path_rejects_mcp_token_directory(tmp_path, monkeypatch):
    ...
    clover_home = tmp_path / "clover-home"
    token_dir = clover_home / "mcp-tokens"
