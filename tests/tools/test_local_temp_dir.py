...
Clover exposes ``terminal.temp_dir`` (mirrored to ``TERMINAL_TEMP_DIR``) so
...
def test_default_is_clover_cache_not_tmp(tmp_path, monkeypatch):
    """With no overrides at all, the default temp root is real storage under
    CLOVER_HOME (cache/terminal), NOT tmpfs /tmp."""
    import clover_constants  # noqa: F401 — resolves CLOVER_HOME per call
    ...
    assert result == str(tmp_path / ".clover" / "cache" / "terminal")
...
def test_cleanup_terminal_temp_cache(tmp_path, monkeypatch):
    ...
    assert not (root / "clover_bg_dead1.log").exists()
