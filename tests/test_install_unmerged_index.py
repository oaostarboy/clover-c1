...
The ``clover update`` Python path already clears the conflict with ``git reset``
...
def test_install_ps1_stops_venv_resident_processes_before_parking_venv() -> None:
    ...
    ``venv\\Scripts\\pythonw.exe -m clover_cli.main gateway run`` — image name
    ``pythonw``, not ``clover.exe`` — so the ``taskkill /IM clover.exe`` guard
    ...
    # The clover.exe tree-kill is preserved (kills spawned child processes too).
    assert 'taskkill /F /T /IM clover.exe' in text
