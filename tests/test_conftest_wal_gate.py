"""The conftest WAL gate must agree with clover_state, and must not import it.

``tests/conftest.py::_wal_is_usable`` duplicates the SQLite WAL-reset version
predicate instead of importing ``clover_state``. That is deliberate: importing
``clover_state`` during collection caches ``DEFAULT_DB_PATH`` from the real
``~/.clover`` before the per-test ``CLOVER_HOME`` redirect, which makes tests
...
def test_conftest_gate_agrees_with_clover_state(version_info, monkeypatch):
    ...
    assert _wal_is_usable() is not is_sqlite_wal_reset_vulnerable(version_info), (
        f"conftest gate and clover_state disagree for SQLite {version_info}"
    )


def test_conftest_does_not_import_clover_state_at_collection():
    """The gate must stay import-free of clover_state.

    Importing it during collection caches DEFAULT_DB_PATH from the real
    ~/.clover, so tests read live production sessions instead of a tempdir.
    Reading the source is not an option here (banned), so assert on behavior:
    the gate must work with ``clover_state`` absent from ``sys.modules`` and
    ...
    def guard(name, *args, **kwargs):
        if name == "clover_state" or name.startswith("clover_state."):
            blocked.append(name)
            raise AssertionError(
                "conftest._wal_is_usable imported clover_state — this caches "
                "DEFAULT_DB_PATH from the real ~/.clover during collection"
            )
        return real_import(name, *args, **kwargs)

    saved = sys.modules.pop("clover_state", None)
    ...
    try:
        ...
        if saved is not None:
            sys.modules["clover_state"] = saved
    assert not blocked
