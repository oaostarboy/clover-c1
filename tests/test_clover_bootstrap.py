"""Tests for clover_bootstrap — Windows UTF-8 stdio shim.

The bootstrap module is imported at the top of every Clover entry point
(clover, clover-c1, clover-acp, gateway, batch_runner, cli.py).  It
...
Key invariants covered by these tests:
  ...
  5. Load order: every Clover entry point imports clover_bootstrap as its
...
def _fresh_import():
    """Return a freshly-imported clover_bootstrap module.

    Drops any cached copy from sys.modules first so module-level code
    runs again and the platform check re-evaluates.
    """
    sys.modules.pop("clover_bootstrap", None)
    import clover_bootstrap  # noqa: WPS433
    return clover_bootstrap
...
class TestWindowsBehavior:
    ...
    def test_stdout_reconfigured_to_utf8_on_windows(self):
        # The live process's stdout should now be UTF-8 (the Clover CLI
...
class TestEntryPointsImportBootstrap:
    """Every Clover entry point must import clover_bootstrap as its
    ...
    # Entry points that invoke Clover as a process.  Each one must
    # import clover_bootstrap before doing any file I/O or stdout writes.
    ENTRY_POINTS = [
        "clover_cli/main.py",   # clover CLI (console_script)
        "run_agent.py",          # clover-c1 (console_script)
        "acp_adapter/entry.py",  # clover-acp (console_script)
    ...
    def test_entry_point_imports_bootstrap(self, path):
        """The file must contain 'import clover_bootstrap' and that
        ...
        half-finished ``clover update`` (git-reset landed new code but
        ``uv pip install -e .`` didn't finish re-registering
        ``clover_bootstrap`` as a top-level module) leaves clover
        ...
        """
        # Resolve relative to the clover-c1 repo root.  Tests live
        # at tests/test_clover_bootstrap.py, so go up one dir.
        import pathlib
        ...
        for node in ast.iter_child_nodes(tree):
            ...
            # clover start even when clover_bootstrap hasn't been
        ...
        assert first_import_name == "clover_bootstrap", (
            f"{path}: first top-level import is {first_import_name!r}, "
            f"but it must be 'clover_bootstrap' so UTF-8 stdio is "
            f"configured before anything else initializes.  Move the "
            f"'import clover_bootstrap' line to be the first import."
        )


class TestHardenImportPath:
    """harden_import_path() must keep a same-named package in the launch
    directory from shadowing Clover's own top-level modules — covering both
    the relative ('' / '.') and absolute-path forms the cwd can take on
    sys.path (issue #51286)."""

    def _run(self, hb, path_seed, env=None):
        original = sys.path[:]
        original_env = os.environ.get("CLOVER_PYTHON_SRC_ROOT")
        try:
            sys.path[:] = path_seed
            if env is not None:
                os.environ["CLOVER_PYTHON_SRC_ROOT"] = env
            elif "CLOVER_PYTHON_SRC_ROOT" in os.environ:
                del os.environ["CLOVER_PYTHON_SRC_ROOT"]
            hb.harden_import_path(src_root="/opt/clover")
            return sys.path[:]
        finally:
            sys.path[:] = original
            if original_env is None:
                os.environ.pop("CLOVER_PYTHON_SRC_ROOT", None)
            else:
                os.environ["CLOVER_PYTHON_SRC_ROOT"] = original_env

    def test_relative_cwd_forms_removed(self):
        hb = _fresh_import()
        result = self._run(hb, ["", ".", "/opt/clover", "/usr/lib/python"])
    ...
    def test_absolute_cwd_path_loses_to_src_root(self):
        ...
        # Clover root.  The guard must relocate Clover to the front.
        hb = _fresh_import()
        result = self._run(hb, ["/home/user/tg-ws-proxy", "/opt/clover"])
        assert result[0] == "/opt/clover"
        # The cwd absolute path may still appear (it can hold legit deps),
        # but only AFTER the Clover root.
        assert result.index("/opt/clover") < result.index("/home/user/tg-ws-proxy")
    ...
    def test_env_var_used_when_no_arg(self):
        ...
        try:
            ...
            os.environ["CLOVER_PYTHON_SRC_ROOT"] = "/env/clover"
            hb.harden_import_path()
            assert sys.path[0] == "/env/clover"
        finally:
            sys.path[:] = original
            if original_env is None:
                os.environ.pop("CLOVER_PYTHON_SRC_ROOT", None)
