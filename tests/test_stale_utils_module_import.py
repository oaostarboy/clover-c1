...
Real incident (gateway session 1518671026962174144)::
    ...
    cannot import name 'env_float' from 'utils' (~/.clover/clover-c1/utils.py)
...
2. ``clover update`` ran ``git pull``, updating ``utils.py`` (now defining
...
``clover_cli/main.py`` (the ``clover update`` flow, ~line 9326) already
acknowledges this exact hazard -- "source files on disk are newer than cached
Python modules in this process" -- and reloads ``clover_constants`` after the
