...
binary download are exercised manually by `clover secrets bitwarden
...
def clover_home(tmp_path, monkeypatch):
    """Point Clover at an isolated home directory."""
    home = tmp_path / ".clover"
    home.mkdir()
    monkeypatch.setenv("CLOVER_HOME", str(home))
    # Some modules cache get_clover_home; clear if needed.
    import clover_constants
    if hasattr(clover_constants, "_CLOVER_HOME_CACHE"):
        clover_constants._CLOVER_HOME_CACHE = None  # type: ignore[attr-defined]
    return home
...
def test_install_bws_happy_path(clover_home, monkeypatch):
...
def test_env_loader_calls_bsm_when_enabled(tmp_path, monkeypatch):
    ...
    from clover_cli.env_loader import _apply_external_secret_sources
