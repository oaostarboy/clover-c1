...
SessionDB.session_gateway_runtime (clover_state.py) — the round trip that
makes `clover --resume` reopen a session on the model/provider it actually
...
def _make_stub(**overrides):
    """Bare CloverCLI the way resume paths see it (no __init__)."""
    stub = object.__new__(cli_mod.CloverCLI)
...
def test_session_gateway_runtime_falls_back_to_top_level_keys():
    ...
    meta = _row(model_config={"provider": "clover", "api_mode": "chat_completions"})
    runtime = SessionDB.session_gateway_runtime(meta)
    assert runtime == {"provider": "clover", "api_mode": "chat_completions"}
...
def test_session_gateway_runtime_explicit_provider_wins_over_billing():
    ...
    meta = _row(model_config={"provider": "clover"})
    meta["billing_provider"] = "minimax"
    runtime = SessionDB.session_gateway_runtime(meta)
    assert runtime == {"provider": "clover"}
