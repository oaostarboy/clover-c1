...
def test_provider_flag_uses_named_custom_default_model(monkeypatch):
    ...
    monkeypatch.setitem(
        cli.CLI_CONFIG,
        "model",
        {"default": "tencent/hy3:free", "provider": "clover"},
    )
    config = {
        "model": {"default": "tencent/hy3:free", "provider": "clover"},
        "providers": {
            "gmk-lan": {
                "name": "GMK Local",
    ...
    monkeypatch.setattr("clover_cli.config.load_config", lambda: config)
    monkeypatch.setattr("clover_cli.runtime_provider.load_config", lambda: config)

    shell = cli.CloverCLI(provider="gmk-lan", compact=True, max_turns=1)
...
def test_provider_flag_logs_when_custom_default_model_cannot_resolve(monkeypatch, caplog):
    ...
    monkeypatch.setattr(
        "clover_cli.runtime_provider._get_named_custom_provider",
        _boom,
    )
...
def test_clover_cli_init_does_not_eagerly_resolve_runtime_provider(monkeypatch):
    cli = _import_cli()
    calls = {"count": 0}

    def _unexpected_runtime_resolve(**kwargs):
        calls["count"] += 1
        raise AssertionError("resolve_runtime_provider should not be called in CloverCLI.__init__")

    monkeypatch.setattr("clover_cli.runtime_provider.resolve_runtime_provider", _unexpected_runtime_resolve)
    monkeypatch.setattr("clover_cli.runtime_provider.format_runtime_provider_error", lambda exc: str(exc))

    shell = cli.CloverCLI(model="gpt-5", compact=True, max_turns=1)
...
def test_runtime_resolution_failure_is_not_sticky(monkeypatch):
    ...
    monkeypatch.setattr("clover_cli.runtime_provider.resolve_runtime_provider", _runtime_resolve)
    monkeypatch.setattr("clover_cli.runtime_provider.format_runtime_provider_error", lambda exc: str(exc))
...
def test_codex_provider_uses_config_model(monkeypatch):
    ...
    monkeypatch.setattr(
        "clover_cli.codex_models.get_codex_model_ids",
        lambda access_token=None: ["gpt-5.2-codex"],
    )

    shell = cli.CloverCLI(compact=True, max_turns=1)
...
def test_model_flow_custom_saves_verified_v1_base_url(monkeypatch, capsys):
    monkeypatch.setattr(
        "clover_cli.config.get_env_value",
        lambda key: "" if key in {"OPENAI_BASE_URL", "OPENAI_API_KEY"} else "",
    )
    saved_env = {}
    monkeypatch.setattr("clover_cli.config.save_env_value", lambda key, value: saved_env.__setitem__(key, value))
    monkeypatch.setattr("clover_cli.auth._save_model_choice", lambda model: saved_env.__setitem__("MODEL", model))
    monkeypatch.setattr("clover_cli.auth.deactivate_provider", lambda: None)
    monkeypatch.setattr("clover_cli.main._save_custom_provider", lambda *args, **kwargs: None)
    monkeypatch.setattr(
    ...
    monkeypatch.setattr("clover_cli.secret_prompt.masked_secret_prompt", lambda _prompt="": next(answers))

    clover_main._model_flow_custom({})
...
def test_model_flow_custom_persists_selected_api_mode(monkeypatch):
    ...
    monkeypatch.setattr("clover_cli.auth._save_model_choice", lambda model: None)
    monkeypatch.setattr("clover_cli.auth.deactivate_provider", lambda: None)
    monkeypatch.setattr(
        "clover_cli.models.probe_api_models",
        lambda api_key, base_url: {
    ...
    saved_env = {}
    monkeypatch.setattr("clover_cli.config.load_config", lambda: saved_cfg)
    monkeypatch.setattr("clover_cli.config.save_config", lambda cfg: saved_cfg.update(cfg))
    monkeypatch.setattr(
        "clover_cli.config.save_env_value",
        lambda key, value: saved_env.__setitem__(key, value),
    )
    monkeypatch.setattr(
        "clover_cli.main._save_custom_provider",
    ...
    monkeypatch.setattr("builtins.input", lambda _prompt="": next(answers))
    monkeypatch.setattr("clover_cli.secret_prompt.masked_secret_prompt", lambda _prompt="": "test-key")

    clover_main._model_flow_custom({"model": {"provider": "custom"}})
...
def test_auto_provider_name_localhost():
    from clover_cli.main import _auto_provider_name
...
def test_save_custom_provider_uses_provided_name(monkeypatch, tmp_path):
    ...
    import yaml
    from clover_cli.main import _save_custom_provider

    cfg_path = tmp_path / "config.yaml"
    cfg_path.write_text(yaml.dump({}))

    monkeypatch.setattr(
        "clover_cli.config.load_config", lambda: yaml.safe_load(cfg_path.read_text()) or {},
    )
    saved = {}
    def _save(cfg):
        saved.update(cfg)
    monkeypatch.setattr("clover_cli.config.save_config", _save)
...
def test_save_custom_provider_references_the_key_instead_of_inlining_it(monkeypatch, tmp_path):
    ...
    saved = {}
    monkeypatch.setattr("clover_cli.config.save_config", lambda cfg: saved.update(cfg))

    _save_custom_provider(
        "http://localhost:11434/v1",
        api_key="sk-secret",
        name="Ollama",
        key_env="CLOVER_CUSTOM_LOCALHOST_11434_API_KEY",
    )

    entry = saved["custom_providers"][0]
    assert entry["key_env"] == "CLOVER_CUSTOM_LOCALHOST_11434_API_KEY"
...
def test_custom_endpoint_key_env_is_a_valid_posix_name_for_ip_endpoints():
    ...
    from clover_cli.config import _ENV_VAR_NAME_RE, custom_endpoint_key_env
