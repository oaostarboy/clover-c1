...
class TestSaveConfigValueAtomic:
    ...
    def config_env(self, tmp_path, monkeypatch):
        ...
        # save_config_value resolves the target live via get_clover_home(), so
        # point CLOVER_HOME at the temp dir (the _clover_home import-time
    ...
    def test_model_write_runs_shared_cron_drift_warning(self, config_env, monkeypatch):
        warning = MagicMock()
        monkeypatch.setattr(
            "clover_cli.config.warn_unpinned_cron_jobs_after_model_config_change",
            warning,
        )
...
class TestSaveConfigValueTargetsUserConfig:
    """Regression: persisted runtime settings must land in CLOVER_HOME/config.yaml
    ...
    install whose CLOVER_HOME/config.yaml did not exist yet, save_config_value
    fell back to the checked-in cli-config.yaml. The toggle reported success, but
    startup read CLOVER_HOME/config.yaml and never saw the setting."""

    def test_creates_user_config_when_absent(self, tmp_path, monkeypatch):
        # Fresh CLOVER_HOME with NO config.yaml (managed/desktop first launch).
        clover_home = tmp_path / ".clover"
        clover_home.mkdir()
        monkeypatch.setenv("CLOVER_HOME", str(clover_home))
    ...
    def test_does_not_write_repo_cli_config(self, tmp_path, monkeypatch):
        ...
        result = yaml.safe_load((clover_home / "config.yaml").read_text())
        assert result["wake_word"]["enabled"] is True
