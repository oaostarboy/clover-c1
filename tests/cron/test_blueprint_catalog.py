...
the shared /blueprint command handler (clover_cli/blueprint_cmd.py), and
the docs generator. Uses an isolated CLOVER_HOME for anything that touches the
cron job store.
"""
...
class TestRenderers:
    ...
    def test_deeplink_shape(self):
        ...
        assert url.startswith("clover://blueprint/morning-brief?")
