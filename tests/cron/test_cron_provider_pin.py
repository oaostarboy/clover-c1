...
resolve_runtime_provider against a temp CLOVER_HOME) and the create_job
...
class TestProviderDriftGuard:
    ...
    def test_b_unpinned_snapshot_differs_fails_closed(self, tmp_path):
        ...
        job = _base_job(provider_snapshot="openrouter")
        success, output, final_response, error, agent_constructed = \
            _run_with_current_provider(job, "clover", tmp_path)
        ...
        assert "clover" in blob
        assert "spend" in blob
        assert "clover cron edit pin-test --provider <provider> --model <model>" in blob
        ...
        assert "host running clover" in delivered
        assert "clover cron edit pin-test --provider <provider> --model <model>" in delivered
    ...
    def test_missing_model_guides_to_user_owned_cli(self, tmp_path, monkeypatch):
        ...
        assert agent_constructed is False
        assert error is not None
        assert "clover cron edit pin-test --model <name>" in error
...
class TestCreateJobSnapshot:
    ...
    def test_pinned_job_skips_snapshot(self, monkeypatch):
        ...
        with patch("clover_cli.runtime_provider.resolve_runtime_provider", resolver):
            job = jobs.create_job(
                prompt="do a thing", schedule="every 1 hour", provider="clover"
            )

        # Explicit provider → pinned → no snapshot needed, and resolution skipped.
        assert job["provider"] == "clover"
...
def _run_with_current_provider_and_model(
    ...
    *,
    ...
    fake_db = MagicMock()
    with patch("cron.scheduler._clover_home", tmp_path), \
         patch("cron.scheduler._get_clover_home", return_value=tmp_path), \
         patch("cron.scheduler._resolve_origin", return_value=None), \
         patch("clover_cli.env_loader.load_clover_dotenv"), \
         patch("clover_cli.env_loader.reset_secret_source_cache"), \
         patch("clover_state.SessionDB", return_value=fake_db), \
