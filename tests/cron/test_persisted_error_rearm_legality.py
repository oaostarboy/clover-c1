...
def cron_store(tmp_path, monkeypatch):
    ...
    (clover_home / "cron").mkdir(parents=True)
    monkeypatch.setenv("CLOVER_HOME", str(clover_home))
    monkeypatch.setattr(J, "CLOVER_DIR", clover_home)
    monkeypatch.setattr(J, "CRON_DIR", clover_home / "cron")
    monkeypatch.setattr(J, "JOBS_FILE", clover_home / "cron" / "jobs.json")
    monkeypatch.setattr(J, "OUTPUT_DIR", clover_home / "cron" / "output")
...
class TestCronRearmRespectsScheduleLegality:
    def test_weekday_job_errored_friday_does_not_fire_on_saturday(self, cron_store):
        ...
        with mock.patch.object(J, "_clover_now", lambda: now):
            due = J.get_due_jobs()
