...
These run without Docker: the shim's CLOVER_TINI_SHIM_TARGET /
CLOVER_TINI_SHIM_WRAPPER hooks let us record the argv that would be
handed to /init.
"""
...
def _run_shim(
    ...
    env = os.environ.copy()
    env["CLOVER_TINI_SHIM_TARGET"] = str(init)
    env["CLOVER_TINI_SHIM_WRAPPER"] = str(wrapper)
