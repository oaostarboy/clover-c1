...
def progress(tmp_path):
    ...
    status = tmp_path / "clover-update-status"
...
def test_every_state_carries_a_clock(progress):
    ...
    for state, message in [
        ...
        ("done", ""),
        ("manual", "Reopen Clover to finish."),
        ("error", "Update failed."),
    ]:
...
# Stands in for `clover update`, and reports the stage that was on screen
...
n="$(cat "$CLOVER_TEST_CALLS" 2>/dev/null || echo 0)"; n=$((n + 1))
printf '%s' "$n" > "$CLOVER_TEST_CALLS"
for f in "$TMPDIR"/clover-update-status.[0-9]*; do
  case "$f" in *.tmp) continue ;; esac
  cp "$f" "$CLOVER_TEST_CAPTURE.$n" 2>/dev/null
done
exit "$(cat "$CLOVER_TEST_EXITS.$n" 2>/dev/null || echo 0)"
"""
...
def _run_handoff(tmp_path, exits: dict[int, int]) -> list[dict]:
    ...
    install_root = tmp_path / "clover-c1"
    (install_root / "venv" / "bin").mkdir(parents=True)
    clover = install_root / "venv" / "bin" / "clover"
    clover.write_text(FAKE_CLOVER)
    clover.chmod(0o755)
    ...
    env = {
        **os.environ,
        "TMPDIR": str(tmp_path),
        "CLOVER_TEST_CAPTURE": str(capture),
        "CLOVER_TEST_CALLS": str(calls),
        "CLOVER_TEST_EXITS": str(tmp_path / "exits"),
    }
    ...
    result = tmp_path / ".clover-update-result.json"
...
def test_update_gate_publishes_its_stage_before_running(tmp_path):
    """`clover update` is the longest wait in the hand-off and the one the
