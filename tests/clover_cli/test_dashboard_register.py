"""Tests for ``clover dashboard register``.
...
The portal HTTP call and the Clover token resolution are both mocked — this
...
import clover_cli.dashboard_register as dr
...
class TestFastFails:
    def test_not_logged_in_exits_1_with_setup_hint(self, capsys):
        ...
        assert "not logged into Clover Portal" in out
        assert "clover setup" in out
...
class TestHappyPath:
    def _run(self, *, args, account_token="tok_abc", portal="",
        ...
        # (idempotency key) and once for CLOVER_DASHBOARD_PORTAL_URL. Route by
        ...
        def fake_get_env(key):
            if key == "CLOVER_DASHBOARD_OAUTH_CLIENT_ID":
        ...
        with patch(
            ...
            "clover_cli.config.get_env_value", side_effect=fake_get_env
        ), patch(
            "clover_cli.config.save_env_value", side_effect=fake_save
        ), patch.object(
    ...
    def test_writes_client_id_and_posts_generated_name(self, capsys):
        ...
        assert saved["CLOVER_DASHBOARD_OAUTH_CLIENT_ID"] == "agent:selfhost-1"
...
class TestIdempotentRerun(TestHappyPath):
    ...
    ``existing_client_id`` (the CLOVER_DASHBOARD_OAUTH_CLIENT_ID a prior run
    ...
    def test_stale_id_falls_through_to_create_prints_registered(self, capsys):
        ...
        assert saved["CLOVER_DASHBOARD_OAUTH_CLIENT_ID"] == "agent:selfhost-new"
...
class TestCustomPortalPersistence:
    """`--portal-url` / CLOVER_DASHBOARD_PORTAL_URL is persisted to .env.
    ...
    def _run(self, *, args, portal, existing_portal):
        ...
        CLOVER_DASHBOARD_PORTAL_URL (None = not present in .env).
        """
        response = {
            "client_id": "agent:selfhost-1",
            "id": "selfhost-1",
        ...
        def fake_get_env_value(key, *a, **kw):
            if key == "CLOVER_DASHBOARD_PORTAL_URL":
        ...
        with patch(
            ...
            # The ambient process env may carry CLOVER_DASHBOARD_PORTAL_URL
            # (e.g. staging dev shells); drop it so `custom_portal_supplied`
            # is driven solely by the args.portal_url under test.
            dr.os.environ.pop("CLOVER_DASHBOARD_PORTAL_URL", None)
            dr.cmd_dashboard_register(args)
        return saved
...
class TestPublicUrlPersistence:
    """`--redirect-uri` derives & persists CLOVER_DASHBOARD_PUBLIC_URL in .env.

    --redirect-uri is the full public callback (e.g.
    https://clover.example.com/auth/callback). At serve time the dashboard auth
    layer reconstructs that callback by appending "/auth/callback" to
    CLOVER_DASHBOARD_PUBLIC_URL, so the value that's actually consumed is the
    ORIGIN (scheme://host). We derive the origin from the supplied redirect URI
    and persist THAT as CLOVER_DASHBOARD_PUBLIC_URL — the var the runtime reads
    ...
    def _run(self, *, args, existing_public=None):
        ...
        CLOVER_DASHBOARD_PUBLIC_URL (None = not present in .env).
        """
        response = {
            "client_id": "agent:selfhost-1",
            "id": "selfhost-1",
        ...
        def fake_get_env_value(key, *a, **kw):
            if key == "CLOVER_DASHBOARD_PUBLIC_URL":
    ...
    def test_no_redirect_flag_does_not_overwrite_existing(self, capsys):
        ...
        assert "CLOVER_DASHBOARD_PUBLIC_URL" not in saved
    ...
    def test_public_url_persisted_alongside_portal_url(self, capsys):
        ...
        response = {
            ...
            "name": "dreamy_tesla",
            "kind": "SELF_HOSTED",
            "custom_redirect_uri": "https://clover.example.com/auth/callback",
            "created_at": "2026-06-04T12:00:00.000Z",
        }
        saved: dict = {}
        ...
        with patch(
            ...
            "clover_cli.config.get_env_value", return_value=None
        ), patch(
            "clover_cli.config.save_env_value", side_effect=fake_save
        ), patch.object(
            dr.urllib.request, "urlopen", return_value=_fake_http_ok(response)
            ...
            dr.cmd_dashboard_register(
                _ns(
                    ...
                    redirect_uri="https://clover.example.com/auth/callback",
                )
            )
        assert saved["CLOVER_DASHBOARD_PORTAL_URL"] == "https://preview.example.com"
        assert saved["CLOVER_DASHBOARD_PUBLIC_URL"] == "https://clover.example.com"
