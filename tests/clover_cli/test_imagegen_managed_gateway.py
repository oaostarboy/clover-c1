...
(non-managed) routing. When a user picked FAL through the Clover-subscription
...
the user's personal FAL_KEY instead of the Clover Tool Gateway (real incident:
...
def test_image_gen_selector_preserves_managed_selection(monkeypatch):
    """Managed pick: the 'clover' provider string must survive the selector."""
    _quiet(monkeypatch)
    config = {}
...
def test_image_and_video_selectors_share_the_selection_contract(monkeypatch):
    ...
    for use_gateway, expected in ((True, "clover"), (False, "fal")):
        config = {
...
def test_reconfigure_managed_fal_row_keeps_managed_selection(monkeypatch):
    ...
    branch wrote the managed one — a Clover Subscription user re-entering the
...
def test_reconfigure_direct_fal_row_writes_vendor_selection(monkeypatch):
    ...
    legacy use_gateway key so the read-time shim can't resurrect 'clover'."""
    _quiet_reconfigure(monkeypatch)
    import clover_cli.tools_config as tc
