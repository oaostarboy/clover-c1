"""Tests for clover_cli.mcp_catalog and clover_cli.mcp_picker.
...
def catalog_dir(tmp_path, monkeypatch):
    ...
    cat.mkdir()
    monkeypatch.setenv("CLOVER_OPTIONAL_MCPS", str(cat))
    return cat


@pytest.fixture(autouse=True)
def _isolate_clover_home(tmp_path, monkeypatch):
    """Redirect all config I/O to a temp CLOVER_HOME."""
    hh = tmp_path / "clover-home"
    hh.mkdir()
    monkeypatch.setenv("CLOVER_HOME", str(hh))
    monkeypatch.setattr(
        "clover_cli.config.get_clover_home", lambda: hh
    )
    monkeypatch.setattr(
        "clover_cli.config.get_config_path", lambda: hh / "config.yaml"
    )
    monkeypatch.setattr(
        "clover_cli.config.get_env_path", lambda: hh / ".env"
    )
    # mcp_catalog grabs get_clover_home() lazily through clover_constants
    monkeypatch.setattr(
        "clover_constants.get_clover_home", lambda: hh
    )
    return hh
...
def _entry(name: str):
    ...
    from clover_cli.mcp_catalog import get_entry
...
class TestManifestParsing:
    def test_minimal_valid(self, catalog_dir):
        ...
        from clover_cli.mcp_catalog import list_catalog
    ...
    def test_suggest_empty_block_rejected(self, catalog_dir):
        ...
        from clover_cli.mcp_catalog import list_catalog, catalog_diagnostics
    ...
    def test_http_api_key_builds_bearer_headers_template(self, catalog_dir):
        ...
        _write_manifest(catalog_dir, "demo", body)
        from clover_cli.mcp_catalog import _build_server_config
    ...
    def test_http_api_key_requires_matching_env_declaration(self, catalog_dir):
        ...
        from clover_cli.mcp_catalog import CatalogError, _parse_manifest
...
class TestInstall:
    def test_install_simple_stdio_writes_config(self, catalog_dir):
        _write_manifest(catalog_dir, "demo", _basic_manifest())
        from clover_cli.mcp_catalog import install_entry
    ...
    def test_install_rejects_exfil_shaped_stdio_manifest(self, catalog_dir):
        ...
        from clover_cli.config import load_config
        from clover_cli.mcp_catalog import CatalogError, install_entry
...
class TestUninstall:
    def test_uninstall_removes_server_block(self, catalog_dir):
        _write_manifest(catalog_dir, "demo", _basic_manifest())
        from clover_cli.mcp_catalog import install_entry, uninstall_entry
        from clover_cli.config import load_config
    ...
    def test_uninstall_missing_returns_false(self):
        from clover_cli.mcp_catalog import uninstall_entry
...
class TestGitInstallShaRef:
    def test_sha_ref_skips_branch_attempt(self, catalog_dir, monkeypatch, tmp_path):
        ...
        from clover_cli import mcp_catalog
        from clover_cli.mcp_catalog import _do_git_install
...
class TestToolsConfigIncludeMode:
    def test_configure_mcp_writes_include_not_exclude(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr(
            "clover_cli.curses_ui.curses_checklist",
...
class TestShippedCatalog:
    def test_all_shipped_manifests_parse(self, monkeypatch):
        ...
        """
        # Use the actual repo's optional-mcps directory (no CLOVER_OPTIONAL_MCPS
        # override) so this test catches real manifests.
        monkeypatch.delenv("CLOVER_OPTIONAL_MCPS", raising=False)
        from clover_cli.mcp_catalog import _catalog_root, _parse_manifest
    ...
    def test_all_shipped_manifests_are_version_locked(self, monkeypatch):
        ...
        pyproject dependencies — everything Clover fetches/launches is pinned
        to an exact version.
