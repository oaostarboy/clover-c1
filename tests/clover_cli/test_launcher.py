"""Tests for the top-level `./clover` launcher script."""
...
def test_launcher_delegates_to_argparse_entrypoint(monkeypatch):
    """`./clover` should use `clover_cli.main`, not the legacy Fire wrapper."""
    launcher_path = Path(__file__).resolve().parents[2] / "clover"
    called = []

    fake_main_module = types.ModuleType("clover_cli.main")

    def fake_main():
        called.append("clover_cli.main")

    fake_main_module.main = fake_main
    monkeypatch.setitem(sys.modules, "clover_cli.main", fake_main_module)
