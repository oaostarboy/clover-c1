...
from clover_cli.main, these tests mock the delegation point and verify
...
def test_setup_model_provider_preserves_auxiliary_choices_written_by_picker(tmp_path, monkeypatch):
    """Aux choices made inside clover setup must survive the wizard's final save."""
...
def test_setup_copilot_acp_skips_same_provider_pool_step(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("clover_cli.setup.prompt_yes_no", fake_prompt_yes_no)
    monkeypatch.setattr("clover_cli.setup.prompt", lambda *args, **kwargs: "")
    monkeypatch.setattr("clover_cli.auth.get_active_provider", lambda: None)
