"""Tests for Clover subscription feature detection."""

import shutil
import sys

from clover_cli.clover_account import CloverPortalAccountInfo, CloverToolAccessInfo
from clover_cli import clover_subscription as ns
...
def _account(*, logged_in: bool, paid: bool | None = None) -> CloverPortalAccountInfo:
    return CloverPortalAccountInfo(
        logged_in=logged_in,
...
def _pool_account() -> CloverPortalAccountInfo:
    ...
    return CloverPortalAccountInfo(
        ...
        tool_access=CloverToolAccessInfo(enabled=True, coverage=_POOL_COVERAGE),
    )


def test_get_clover_subscription_features_recognizes_direct_exa_backend(monkeypatch):
    env = {"EXA_API_KEY": "exa-test"}

    monkeypatch.setattr(ns, "get_env_value", lambda name: env.get(name, ""))
    monkeypatch.setattr(
        ns, "get_clover_portal_account_info", lambda: _account(logged_in=False)
    )
    ...
    features = ns.get_clover_subscription_features({"web": {"backend": "exa"}})

    assert features.web.available is True
    assert features.web.active is True
    assert features.web.managed_by_clover is False
    assert features.web.direct_override is True
    assert features.web.current_provider == "exa"


def test_get_clover_subscription_features_recognizes_keyless_tavily_backend(monkeypatch):
    ...
    features = ns.get_clover_subscription_features({"web": {"backend": "tavily"}})
...
def test_keyless_tavily_search_backend_without_shared_backend(monkeypatch):
    ...
    features = ns.get_clover_subscription_features(
...
def test_local_browser_unavailable_without_chromium(monkeypatch):
    ...
    assert features.browser.managed_by_clover is False
...
def _capture_checklist(monkeypatch, *, selected_idx):
    ...
    import clover_cli.setup as setup_mod

    monkeypatch.setattr(setup_mod, "prompt_checklist", _fake_checklist, raising=False)
    monkeypatch.setattr(
        "clover_cli.config.save_config", lambda cfg: None, raising=False
    )
    return captured
...
def test_prompt_enable_tool_gateway_pool_offers_covered_tools_only(monkeypatch):
    ...
    monkeypatch.setattr(ns, "get_clover_portal_account_info", lambda **kw: _pool_account())
    monkeypatch.setattr(
        ns,
...
def test_get_gateway_eligible_tools_treats_explicit_backend_as_configured(monkeypatch):
    ...
    but an explicit non-clover selection must still keep it out of
    'unconfigured' — regression for #92647, where it was pre-checked and a
    single Enter during `clover model` overwrote it to `web.backend: clover`.
    """
    monkeypatch.setattr(ns, "get_clover_portal_account_info", lambda **kw: _account(logged_in=True, paid=True))
    monkeypatch.setattr(
        ns,
        "_get_gateway_direct_credentials",
        lambda: {"web": False, "image_gen": False, "video_gen": False, "tts": False, "browser": False},
    ...
    config = {"model": {"provider": "clover"}, "web": {"backend": "searxng"}}
...
def test_get_gateway_eligible_tools_treats_browser_use_selection_as_explicit(monkeypatch):
    ...
    managed selection, which is always stored as `cloud_provider: clover`
    ...
    config = {"model": {"provider": "clover"}, "browser": {"cloud_provider": "browser-use"}}
    unconfigured, has_direct, explicit_configured, already_managed = ns.get_gateway_eligible_tools(config)
...
def test_get_gateway_eligible_tools_not_entitled_returns_four_empty_lists(monkeypatch):
    """A logged-in Clover account with no paid access and no free tool pool
    ...
    monkeypatch.setattr(ns, "get_clover_portal_account_info", lambda **kw: _account(logged_in=True, paid=False))
...
def test_prompt_enable_tool_gateway_not_entitled_does_not_crash(monkeypatch):
    ...
    not raise when a Clover account is logged in but not entitled to the Tool
...
def test_prompt_enable_tool_gateway_never_offers_explicit_backend(monkeypatch):
    ...
    an explicit non-clover selection, so it can never be silently overwritten
...
def test_prompt_enable_tool_gateway_persists_decline(monkeypatch):
    ...
    captured = _capture_checklist(monkeypatch, selected_idx=[])
    monkeypatch.setattr(
        "clover_cli.config.save_config", lambda cfg: saved.append(dict(cfg)), raising=False
    )
...
def test_apply_clover_managed_defaults_writes_video_gen_config(monkeypatch):
    """apply_clover_managed_defaults must store the managed 'clover' selection
    when a Clover subscriber selects video_gen without a direct FAL_KEY."""
    monkeypatch.setattr(ns, "managed_clover_tools_enabled", lambda **kw: True)
    monkeypatch.delenv("FAL_KEY", raising=False)
    monkeypatch.setattr(ns, "fal_key_is_configured", lambda: False)
    monkeypatch.setattr(
        ns, "get_clover_portal_account_info",
        lambda **kw: _account(logged_in=True, paid=True),
    )

    config = {"model": {"provider": "clover"}}
    changed = ns.apply_clover_managed_defaults(
...
# ---------------------------------------------------------------------------
# ensure_clover_portal_access — inline login gate for `clover tools`
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# STT — managed-by-Clover detection (Phase 4 follow-up)
# ---------------------------------------------------------------------------
    ...
    return ns.CloverSubscriptionFeatures(
        ...
        features={
            key: ns.CloverFeatureState(
                ...
                available=False, active=False, managed_by_clover=False,
