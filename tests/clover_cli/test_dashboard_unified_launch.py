...
class TestUnifiedDashboardRouting:
    ...
    def test_profile_launch_reexecs_machine_dashboard(self, main_mod, monkeypatch):
        ...
        monkeypatch.setattr(
            "clover_cli.profiles.get_active_profile_name", lambda: "worker_x"
        )
        ...
        # CLOVER_HOME.  For a standard install (CLOVER_HOME unset) that root is
        # the platform-native default (~/.clover), NOT dropped — see the Docker
        # test below for why we resolve explicitly instead of popping.
        from clover_constants import get_default_clover_root
        assert env.get("CLOVER_HOME") == str(get_default_clover_root())


    def test_desktop_profile_backend_skips_machine_dashboard_reroute(self, main_mod, monkeypatch):
        """A desktop-spawned named-profile backend (CLOVER_DESKTOP=1) must NOT
