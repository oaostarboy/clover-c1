"""Tests for clover_cli.gui_uninstall — GUI-only uninstall + install discovery.
...
import clover_cli.gui_uninstall as gu


def _make_agent(clover_home: Path) -> Path:
    """Create a fake agent install: source package + venv."""
    agent_root = clover_home / "clover-c1"
    (agent_root / "clover_cli").mkdir(parents=True)
    (agent_root / "clover_cli" / "__init__.py").write_text("")
    (agent_root / "venv" / "bin").mkdir(parents=True)
    return agent_root


def _make_gui_build(clover_home: Path) -> None:
    """Create the source-built GUI artifacts a `clover desktop` run produces."""
    desktop = clover_home / "clover-c1" / "apps" / "desktop"
    ...
    (clover_home / "clover-c1" / "node_modules").mkdir(parents=True)
    (clover_home / "desktop-build-stamp.json").write_text("{}")


def _make_user_data(clover_home: Path) -> None:
    (clover_home / "config.yaml").write_text("x: 1\n")
    (clover_home / ".env").write_text("KEY=secret\n")
    (clover_home / "sessions").mkdir()
...
def test_gui_install_summary_shape(tmp_path, monkeypatch):
    ...
    _make_agent(clover_home)
    _make_gui_build(clover_home)
    monkeypatch.setattr(gu, "packaged_gui_app_paths", lambda: [])
    monkeypatch.setattr(gu, "desktop_userdata_dir", lambda: tmp_path / "none")

    summary = gu.gui_install_summary(clover_home)
    ...
    assert summary["clover_home"] == str(clover_home)
...
def test_linux_discovery_includes_launcher_entry(tmp_path, monkeypatch):
    """The launcher entry that `clover desktop` installs is removable."""
...
def test_uninstall_removes_launcher_entry_and_refreshes_cache(tmp_path, monkeypatch):
    ...
    clover_home = tmp_path / ".clover"
    _make_agent(clover_home)
    icon = lde.icon_path(clover_home / "clover-c1")
    icon.parent.mkdir(parents=True, exist_ok=True)
    icon.write_bytes(b"\x89PNG")
    monkeypatch.setattr(gu, "desktop_userdata_dir", lambda: tmp_path / "none")

    removed = gu.uninstall_gui(clover_home)
    ...
    assert lde.icon_path(clover_home / "clover-c1").exists()
    # The agent itself survives a GUI uninstall.
    assert (clover_home / "clover-c1" / "clover_cli").is_dir()
...
def test_uninstall_skips_cache_refresh_when_no_launcher_entry(tmp_path, monkeypatch):
    ...
    gu.uninstall_gui(tmp_path / ".clover")
...
def test_uninstall_args_namespace_mode_mapping():
    ...
    import clover_cli.uninstall as uninstall
