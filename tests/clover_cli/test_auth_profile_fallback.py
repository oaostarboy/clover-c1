...
When ``CLOVER_HOME`` points to a named profile, ``read_credential_pool()``
...
def profile_env(tmp_path, monkeypatch):
    """Set up a global root + an active profile under Path.home()/.clover/profiles/coder.

    * Path.home() -> tmp_path
    * Global root -> tmp_path/.clover            (has its own auth.json fixture)
    * Profile     -> tmp_path/.clover/profiles/coder   (active, CLOVER_HOME points here)
...
def test_provider_auth_state_returns_none_when_neither_has_it(profile_env):
    ...
    assert get_provider_auth_state("clover") is None
...
def classic_env(tmp_path, monkeypatch):
    """Classic single-root layout (CLOVER_HOME != ~/.clover, no profiles)."""
    fake_home = tmp_path / "home"
    fake_home.mkdir()
    monkeypatch.setattr(Path, "home", lambda: fake_home)
    clover_home = tmp_path / "classic"
    clover_home.mkdir()
