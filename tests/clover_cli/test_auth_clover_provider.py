"""Regression tests for Clover OAuth refresh and inference JWT interactions."""
...
from clover_cli.auth import AuthError, get_provider_auth_state, resolve_clover_runtime_credentials
...
def _setup_clover_auth(
    ...
    *,
    ...
    (clover_home / "auth.json").write_text(json.dumps(auth_store, indent=2))
...
def test_resolve_clover_runtime_credentials_prefers_invoke_jwt_and_mirrors(
    tmp_path,
    monkeypatch,
):
    ...
    creds = auth_mod.resolve_clover_runtime_credentials()

    assert creds["api_key"] == token
    assert creds["source"] == auth_mod.CLOVER_AUTH_PATH_INVOKE_JWT
    assert creds["auth_path"] == auth_mod.CLOVER_AUTH_PATH_INVOKE_JWT

    payload = json.loads((clover_home / "auth.json").read_text())
    ...
    pool_entries = payload["credential_pool"]["clover"]
    assert len(pool_entries) == 1
    assert pool_entries[0]["agent_key"] == token
    assert pool_entries[0]["source"] == auth_mod.CLOVER_DEVICE_CODE_SOURCE


def test_resolve_clover_runtime_credentials_invoke_jwt_is_idempotent(
    tmp_path,
    monkeypatch,
):
    import clover_cli.auth as auth_mod
    ...
    monkeypatch.setattr(auth_mod, "_write_shared_clover_state", _unexpected_shared_write)
    monkeypatch.setattr(
        auth_mod,
    ...
    assert (
        payload["providers"]["clover"]["agent_key_obtained_at"]
        == original_obtained_at
    )


def test_resolve_clover_runtime_credentials_reauths_when_invoke_scope_missing(
    tmp_path,
    monkeypatch,
):
    import clover_cli.auth as auth_mod
    ...
    payload = json.loads((clover_home / "auth.json").read_text())
    assert payload["providers"]["clover"]["agent_key"] is None
    assert "credential_pool" not in payload or not payload["credential_pool"].get("clover")
...
def test_removed_legacy_session_env_var_does_not_change_jwt_auth(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("CLOVER_HOME", str(clover_home))
    monkeypatch.setenv("CLOVER_AGENT_USE_LEGACY_SESSION_KEYS", "true")

    creds = auth_mod.resolve_clover_runtime_credentials()

    assert creds["api_key"] == token
    payload = json.loads((clover_home / "auth.json").read_text())
    assert payload["providers"]["clover"]["agent_key"] == token
    ...
    result = auth_mod._clover_device_code_login(
        portal_base_url="https://portal.example.com",
        inference_base_url="https://inference.example.com/v1",
        open_browser=False,
        timeout_seconds=1,
    ...
    assert requested_scopes == [auth_mod.DEFAULT_CLOVER_SCOPE]
    assert result["agent_key"] == login_token


def test_clover_inference_auth_logs_do_not_include_secret_values(
    tmp_path,
    monkeypatch,
    caplog,
):
    ...
    caplog.set_level(logging.DEBUG, logger="clover_cli.auth")
    auth_mod.resolve_clover_runtime_credentials(
        force_refresh=True,
    )
...
def test_get_clover_auth_status_checks_credential_pool(tmp_path, monkeypatch):
    """get_clover_auth_status() should find Clover credentials in the pool
    even when the auth store has no Clover provider entry — this is the
    case when login happened via the dashboard device-code flow which
    saves to the pool only.
    """
    from clover_cli.auth import get_clover_auth_status

    clover_home = tmp_path / "clover"
    clover_home.mkdir(parents=True, exist_ok=True)
    # Empty auth store — no Clover provider entry
    (clover_home / "auth.json").write_text(json.dumps({
        "version": 1, "providers": {},
    }))
    monkeypatch.setenv("CLOVER_HOME", str(clover_home))

    # Seed the credential pool with a Clover entry
    from agent.credential_pool import PooledCredential, load_pool
    pool = load_pool("clover")
    token = _invoke_jwt(seconds=3600)
    expires_at = _future_iso(3600)
    entry = PooledCredential.from_dict("clover", {
        "access_token": token,
    ...
    status = get_clover_auth_status()
    assert status["logged_in"] is True
    assert "example.com" in str(status.get("portal_base_url", ""))


def test_get_clover_auth_status_empty_returns_not_logged_in(tmp_path, monkeypatch):
    """get_clover_auth_status() returns logged_in=False when both pool
...
# =============================================================================
# _login_clover: "Skip (keep current)" must preserve prior provider + model
# =============================================================================


class TestLoginNousSkipKeepsCurrent:
    """When a user runs `clover model` → Clover Portal → Skip (keep current) after
    ...
    unconditionally after login, which flipped model.provider to "clover" while
    ...
    def _setup_home_with_openrouter(self, tmp_path, monkeypatch):
        ...
        return clover_home, config_path, auth_path
    ...
    def _patch_login_internals(self, monkeypatch, *, prompt_returns):
        ...
        monkeypatch.setattr(
            auth_mod, "_clover_device_code_login",
        ...
        def _check_clover_free_tier(**kwargs):
            free_tier_calls.append(kwargs)
            return None

        monkeypatch.setattr(models_mod, "check_clover_free_tier", _check_clover_free_tier)
        monkeypatch.setattr(
    ...
    def test_skip_keep_current_preserves_provider_and_model(self, tmp_path, monkeypatch):
        """User picks Skip → config.yaml untouched, Clover creds still saved."""
        import argparse
        import yaml
        from clover_cli.auth import PROVIDER_REGISTRY, _login_clover

        clover_home, config_path, auth_path = self._setup_home_with_openrouter(
        ...
        _login_clover(args, PROVIDER_REGISTRY["clover"])
        ...
        # auth.json: active_provider restored to openrouter, but Clover creds saved
        auth_after = json.loads(auth_path.read_text())
        assert auth_after["active_provider"] == "openrouter"
        assert "clover" in auth_after["providers"]
        assert auth_after["providers"]["clover"]["access_token"] == "fake-clover-token"
        # Existing openrouter creds still intact
        assert auth_after["providers"]["openrouter"]["api_key"] == "sk-or-fake"

    def test_picking_model_switches_to_clover(self, tmp_path, monkeypatch):
        """User picks a Clover model → provider flips to clover with that model."""
        import argparse
        import yaml
        from clover_cli.auth import PROVIDER_REGISTRY, _login_clover
    ...
    def test_skip_with_no_prior_active_provider_clears_it(self, tmp_path, monkeypatch):
        ...
        instead of leaving it as clover."""
        import argparse
        import yaml
        from clover_cli.auth import PROVIDER_REGISTRY, _login_clover
        ...
        auth_path = clover_home / "auth.json"
        auth_after = json.loads(auth_path.read_text())
        # active_provider should NOT be set to "clover" after Skip
        assert auth_after.get("active_provider") in {None, ""}
        # But Clover creds are still saved
        assert "clover" in auth_after.get("providers", {})


# =============================================================================
# persist_clover_credentials: shared helper for CLI + web dashboard login paths
# =============================================================================


def _full_state_fixture() -> dict:
    """Shape of the dict returned by _clover_device_code_login /
    refresh_clover_oauth_from_state. Used as helper input."""
    token = _invoke_jwt(seconds=3600)
    expires_at = _future_iso(3600)
    return {
        "portal_base_url": "https://portal.example.com",
...
def test_persist_clover_credentials_writes_both_pool_and_providers(tmp_path, monkeypatch):
    """Helper must populate BOTH credential_pool.clover AND providers.clover.

    Regression guard: before this helper existed, `clover auth add clover`
    wrote only the pool. After the Clover agent_key's 24h TTL expired, the
    401-recovery path in run_agent.py called resolve_clover_runtime_credentials
    which reads providers.clover, found it empty, raised AuthError, and the
    agent failed with "Non-retryable client error". Both stores must stay
    in sync at write time.
    """
    from clover_cli.auth import persist_clover_credentials, CLOVER_DEVICE_CODE_SOURCE
    ...
    entry = persist_clover_credentials(state)

    assert entry is not None
    assert entry.provider == "clover"
    assert entry.source == CLOVER_DEVICE_CODE_SOURCE

    payload = json.loads((clover_home / "auth.json").read_text())

    # providers.clover populated with the full state (new behaviour)
    singleton = payload["providers"]["clover"]
    ...
    # credential_pool.clover has exactly one canonical device_code entry
    pool_entries = payload["credential_pool"]["clover"]
    assert len(pool_entries) == 1, pool_entries
    pool_entry = pool_entries[0]
    assert pool_entry["source"] == CLOVER_DEVICE_CODE_SOURCE
    assert pool_entry["agent_key"] == state["agent_key"]
    assert pool_entry["inference_base_url"] == "https://inference.example.com/v1"


def test_persist_clover_credentials_idempotent_no_duplicate_pool_entries(tmp_path, monkeypatch):
    ...
    ``load_pool()``. The helper now writes providers.clover and lets seeding
    ...
    persist_clover_credentials(first)
    ...
    persist_clover_credentials(second)

    payload = json.loads((clover_home / "auth.json").read_text())

    # providers.clover reflects the latest write (singleton semantics)
    assert payload["providers"]["clover"]["access_token"] == second_token
    assert payload["providers"]["clover"]["agent_key"] == second_token

    # credential_pool.clover has exactly one entry, carrying the latest agent_key
    pool_entries = payload["credential_pool"]["clover"]
    assert len(pool_entries) == 1, pool_entries
    assert pool_entries[0]["source"] == CLOVER_DEVICE_CODE_SOURCE
...
def test_persist_clover_credentials_no_label_uses_auto_derived(tmp_path, monkeypatch):
    ...
    entry = persist_clover_credentials(_full_state_fixture())
    assert entry is not None
    ...
    # No "label" key embedded in providers.clover when the caller didn't supply one.
    payload = json.loads((clover_home / "auth.json").read_text())
    assert "label" not in payload["providers"]["clover"]


def test_refresh_token_reuse_detection_surfaces_actionable_message():
    """Regression for #15099.

    When the Clover Portal server returns ``invalid_grant`` with
    ``error_description`` containing "reuse detected", Clover must surface an
    ...
    re-authenticate" string led users to report this as a Clover persistence
    bug when the true cause is external RT consumption (monitoring scripts,
    custom self-heal hooks).
    """
    from clover_cli.auth import _refresh_access_token
    ...
    assert "clover auth add clover" in message.lower()
...
def test_refresh_token_exchange_sends_refresh_token_header():
    """Clover refresh tokens must be sent in a header so sandbox proxies can
    ...
    assert client.kwargs["headers"]["x-clover-refresh-token"] == "refresh-1"
...
# =============================================================================
# Shared Clover token store — cross-profile persistence (Codex-style auto-import)
# =============================================================================


@pytest.fixture
def shared_store_env(tmp_path, monkeypatch):
    """Redirect CLOVER_SHARED_AUTH_DIR to a tmp_path.

    Required for every test that exercises the shared Clover store — the
...
def test_shared_store_seat_belt_refuses_real_home_under_pytest(monkeypatch):
    """Without CLOVER_SHARED_AUTH_DIR override, the seat belt must trip.
    ...
    writing to the user's real ``~/.clover/shared/`` across CI runs.
    """
    from clover_cli.auth import _clover_shared_store_path

    monkeypatch.delenv("CLOVER_SHARED_AUTH_DIR", raising=False)

    with pytest.raises(RuntimeError, match="shared Clover auth store"):
        _clover_shared_store_path()
...
def test_shared_store_write_and_read_roundtrip(shared_store_env):
    ...
    loaded = _read_shared_clover_state()
...
def test_persist_clover_credentials_mirrors_to_shared_store(
    tmp_path, monkeypatch, shared_store_env,
):
    """persist_clover_credentials must populate BOTH per-profile auth.json
    AND the shared store, so a future profile's `clover auth add clover
    --type oauth` can one-tap import instead of redoing device-code.
    """
    from clover_cli.auth import (
        _clover_shared_store_path,
        _read_shared_clover_state,
        persist_clover_credentials,
    )

    clover_home = tmp_path / "clover"
    clover_home.mkdir(parents=True, exist_ok=True)
    (clover_home / "auth.json").write_text(
        json.dumps({"version": 1, "providers": {}})
    )
    monkeypatch.setenv("CLOVER_HOME", str(clover_home))

    persist_clover_credentials(_full_state_fixture())

    # Per-profile auth.json populated
    payload = json.loads((clover_home / "auth.json").read_text())
    assert "clover" in payload.get("providers", {})
    ...
    assert str(_clover_shared_store_path()).startswith(str(shared_store_env))
...
def test_try_import_shared_rehydrates_on_success(shared_store_env, monkeypatch):
    ...
    every field persist_clover_credentials() needs.
    """
    from clover_cli import auth as auth_mod

    auth_mod._write_shared_clover_state(_full_state_fixture())
    ...
    monkeypatch.setattr(auth_mod, "refresh_clover_oauth_from_state", _fake_refresh)

    result = auth_mod._try_import_shared_clover_state()
    ...
    assert result["client_id"] == "clover-cli"
...
class TestStalePortalBaseUrlMigration:
    """_migrate_stale_clover_portal_url auto-corrects stale portal_base_url on load."""

    def test_migrates_stale_portal_url_on_load(self, tmp_path, monkeypatch):
        from clover_cli.auth import _load_auth_store, DEFAULT_CLOVER_PORTAL_URL
        ...
        clover = store["providers"]["clover"]
        assert clover["portal_base_url"] == DEFAULT_CLOVER_PORTAL_URL
    ...
    def test_runtime_credentials_rejects_http_for_production_portal(
        ...
        auth_mod.resolve_clover_runtime_credentials()
        assert refresh_calls == [auth_mod.DEFAULT_CLOVER_PORTAL_URL]
...
class TestNousDeviceAuthTimeoutMessage:
    def test_timeout_message_mentions_captcha_login_and_retry(self):
        from clover_cli.auth import _clover_device_auth_timeout_message

        msg = _clover_device_auth_timeout_message("")
    ...
    def test_timeout_message_falls_back_to_default_portal(self):
        from clover_cli.auth import (
            ...
            _clover_device_auth_timeout_message,
        )

        msg = _clover_device_auth_timeout_message("")
        assert f"{DEFAULT_CLOVER_PORTAL_URL.rstrip('/')}/login" in msg
...
def test_poll_for_token_timeout_raises_actionable_message():
    ...
    so both the CLI login and the dashboard poller (web_server._clover_poller,
...
def test_clover_device_code_login_timeout_raises_actionable_message(monkeypatch):
    ...
    def _timeout(**kwargs):
        raise TimeoutError(
            auth_mod._clover_device_auth_timeout_message(
