...
def clover_home(tmp_path, monkeypatch):
    ...
    # /etc/clover on the dev/CI box can't influence the test.
    monkeypatch.setenv("CLOVER_MANAGED_DIR", str(tmp_path / "no_such_managed_dir"))
...
def test_user_config_overrides_default(clover_home, monkeypatch):
...
def test_env_expansion_in_user_config(clover_home, monkeypatch):
    from clover_cli.config import load_config, cfg_get
...
def test_user_env_overrides_shell(tmp_path, monkeypatch):
    ...
    load_clover_dotenv(clover_home=str(home))
