...
from clover_cli.auth import AuthError, get_provider_auth_state, resolve_nous_runtime_credentials
...
def test_get_nous_auth_status_checks_credential_pool(tmp_path, monkeypatch):
    """get_nous_auth_status() should find Clover credentials in the pool
    ...
    """
    from clover_cli.auth import get_nous_auth_status
...
class TestLoginNousSkipKeepsCurrent:
    ...
    def test_skip_keep_current_preserves_provider_and_model(self, tmp_path, monkeypatch):
        ...
        import argparse
        import yaml
        from clover_cli.auth import PROVIDER_REGISTRY, _login_nous
        ...
        _login_nous(args, PROVIDER_REGISTRY["clover"])
        ...
        assert auth_after["providers"]["clover"]["access_token"] == "fake-nous-token"
    ...
    def test_picking_model_switches_to_nous(self, tmp_path, monkeypatch):
        """User picks a Clover model → provider flips to nous with that model."""
        import argparse
        import yaml
        from clover_cli.auth import PROVIDER_REGISTRY, _login_nous
...
def test_persist_nous_credentials_writes_both_pool_and_providers(tmp_path, monkeypatch):
    ...
    Regression guard: before this helper existed, `clover auth add nous`
    ...
    """
    from clover_cli.auth import persist_nous_credentials, NOUS_DEVICE_CODE_SOURCE
...
def test_refresh_token_reuse_detection_surfaces_actionable_message():
    ...
    assert "clover auth add nous" in message.lower()
...
def test_shared_store_seat_belt_refuses_real_home_under_pytest(monkeypatch):
    ...
    """
    from clover_cli.auth import _nous_shared_store_path
...
def test_persist_nous_credentials_mirrors_to_shared_store(
    ...
    AND the shared store, so a future profile's `clover auth add nous
...
class TestStalePortalBaseUrlMigration:
    ...
    def test_migrates_stale_portal_url_on_load(self, tmp_path, monkeypatch):
        from clover_cli.auth import _load_auth_store, DEFAULT_NOUS_PORTAL_URL
