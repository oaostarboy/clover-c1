...
from clover_cli.web_server import _SESSION_TOKEN, app


HEADERS = {"X-Clover-Session-Token": _SESSION_TOKEN}


@pytest.fixture
def catalog_env(tmp_path: Path, monkeypatch: pytest.MonkeyPatch, _isolate_clover_home):
    ...
    monkeypatch.setenv("CLOVER_OPTIONAL_MCPS", str(catalog))
...
def test_catalog_rejects_undeclared_key_before_any_write_or_install(
    ...
    import clover_cli.mcp_catalog as mcp_catalog
...
def test_catalog_cannot_declare_reserved_control_key(
    ...
    response = client.post(
        "/api/mcp/catalog/install",
        headers=HEADERS,
        json={"name": "demo", "env": {"CLOVER_YOLO_MODE": "1"}},
    )
    ...
    assert installs == []
    env_path = catalog_env / ".env"
    assert not env_path.exists() or "CLOVER_YOLO_MODE" not in env_path.read_text(
        encoding="utf-8"
    )
...
def test_rejected_copilot_controls_do_not_change_live_resolvers(
    ...
    monkeypatch.setenv("CLOVER_COPILOT_ACP_COMMAND", "/opt/trusted/copilot")
    monkeypatch.setenv("CLOVER_COPILOT_ACP_ARGS", "--acp --stdio")
    expected_command = _resolve_command()
    expected_args = _resolve_args()

    attempts = {
        "CLOVER_COPILOT_ACP_COMMAND": "/tmp/attacker-command",
        "CLOVER_COPILOT_ACP_ARGS": "--malicious-transport",
    }
...
def test_preexisting_copilot_controls_remain_usable(
    ...
    monkeypatch.setenv("CLOVER_COPILOT_ACP_COMMAND", "parent-placeholder")
    monkeypatch.setenv("CLOVER_COPILOT_ACP_ARGS", "--parent-placeholder")
    (catalog_env / ".env").write_text(
        "CLOVER_COPILOT_ACP_COMMAND=/opt/operator/copilot\n"
        "CLOVER_COPILOT_ACP_ARGS=--acp --stdio --operator-mode\n",
        encoding="utf-8",
    )

    load_clover_dotenv(
        clover_home=catalog_env,
