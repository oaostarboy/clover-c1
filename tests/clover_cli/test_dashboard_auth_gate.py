...
@pytest.mark.parametrize("host,allow_public,expected", [
    ...
    # bind (June 2026 clover-0day hardening). Non-loopback always requires auth.
    ("0.0.0.0",   True,  True),
    ("0.0.0.0",   False, True),
    ...
    ("clover-c1-prod-abc.fly.dev", False, True),
])
...
def test_empty_provider_login_page_shows_supported_auth_paths():
    from clover_cli.dashboard_auth import clear_providers
...
def test_should_require_dashboard_auth_truth_table(
    ...
    if public_url is None:
        monkeypatch.delenv("CLOVER_DASHBOARD_PUBLIC_URL", raising=False)
        monkeypatch.setattr(
            web_server, "_dashboard_public_hosts", lambda: frozenset()
        )
    else:
        monkeypatch.setenv("CLOVER_DASHBOARD_PUBLIC_URL", public_url)
