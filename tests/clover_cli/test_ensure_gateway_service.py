"""Tests for clover_cli.gateway.ensure_gateway_service — the zero-prompt
service install/start path used by `clover setup` and `clover import`.
...
def _patch_host(monkeypatch, *, container=False, systemd=True, macos=False, windows=False):
    monkeypatch.setattr("clover_constants.is_container", lambda: container)
