"""Tests for Clover Portal reasoning-capability metadata.
...
def cold_cache(monkeypatch):
    ...
    monkeypatch.setattr(models_mod, "_clover_reasoning_caps_cache", None)
    monkeypatch.setattr(models_mod, "_clover_reasoning_caps_failed_at", None)
    monkeypatch.setattr(models_mod, "_clover_caps_disk_checked", False)
    monkeypatch.setattr(models_mod, "_clover_caps_warm_started", False)
...
class TestNousModelReasoningCapabilities:
    def test_fetch_parses_mandatory_flag(self, cold_cache, monkeypatch):
        from clover_cli.models import clover_model_reasoning_capabilities

        monkeypatch.setattr(
            cold_cache, "_urlopen_model_catalog_request",
            lambda req, *, timeout: _mock_response(_CATALOG),
        )
        optional = clover_model_reasoning_capabilities(
        ...
        mandatory = clover_model_reasoning_capabilities("arcee-ai/trinity-large-thinking")
    ...
    def test_catalog_read_sends_user_agent(self, cold_cache, monkeypatch):
        ...
        clover_model_reasoning_capabilities("deepseek/deepseek-v4-pro", allow_fetch=True)
    ...
    def test_catalog_read_follows_the_configured_endpoint(
        ...
        """
        from clover_cli.models import clover_catalog_url

        monkeypatch.setenv(
            "CLOVER_INFERENCE_BASE_URL", "https://staging./v1"
        )
        assert clover_catalog_url() == "https://staging./v1/models"

        monkeypatch.delenv("CLOVER_INFERENCE_BASE_URL")
        assert clover_catalog_url().endswith("/v1/models")
    ...
    def test_unlisted_and_empty_models_return_none(self, cold_cache, monkeypatch):
        ...
        assert clover_model_reasoning_capabilities("private/route", allow_fetch=True) is None
        assert clover_model_reasoning_capabilities("") is None
        assert clover_model_reasoning_capabilities(None) is None
    ...
    def test_cache_only_by_default_never_fetches(self, cold_cache, monkeypatch):
        ...
        assert clover_model_reasoning_capabilities("deepseek/deepseek-v4-pro") is None
    ...
    def test_unreachable_catalog_rate_limits_refetch(self, cold_cache, monkeypatch):
        ...
        monkeypatch.setattr(cold_cache, "_urlopen_model_catalog_request", _boom)
        assert clover_model_reasoning_capabilities("a/b", allow_fetch=True) is None
        assert clover_model_reasoning_capabilities("a/b", allow_fetch=True) is None
    ...
    def test_openrouter_cache_is_independent(self, cold_cache, monkeypatch):
        ...
        assert clover_model_reasoning_capabilities(
            "deepseek/deepseek-v4-pro", allow_fetch=True
        ) is not None
