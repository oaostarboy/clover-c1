"""Tests for issue #26670 — concurrent clover.exe detection and improved
quarantine retry / reboot-deferred fallback during `clover update` on Windows.
...
# Tests in this module either exercise the REAL _detect_concurrent_clover_instances
# helper (and need the autouse stub in tests/clover_cli/conftest.py disabled),
...
def _make_proc(pid: int, exe: str, name: str = "clover.exe"):
...
def test_detect_concurrent_parents_call_robust_to_one_bad_hop(_winp, tmp_path):
    ...
    rows = [
        ...
        _make_proc(launcher_pid, str(shim), "clover.exe"),
    ]
    ...
    with patch.dict(sys.modules, {"psutil": fake_psutil}):
        result = cli_main._detect_concurrent_clover_instances(scripts_dir)

    # No crash; helper completes. (Degenerate stub: launcher exe unreadable.)
    assert result == [(launcher_pid, "clover.exe")]
...
# ---------------------------------------------------------------------------
# _quarantine_running_clover_exe — retry, then report
# ---------------------------------------------------------------------------
...
def test_quarantine_succeeds_first_attempt(_winp, tmp_path):
    ...
    shim = tmp_path / "clover.exe"
    shim.write_bytes(b"old")

    pairs = cli_main._quarantine_running_clover_exe(tmp_path)

    assert len(pairs) == 1
    orig, quarantine = pairs[0]
    assert orig == shim
    assert quarantine.name.startswith("clover.exe.old.")
...
def test_quarantine_reports_a_lock_it_cannot_break(_winp, tmp_path, capsys, monkeypatch):
    ...
    monkeypatch.setattr(cli_main, "_clover_exe_shims", lambda d: [shim])
...
def test_pause_windows_gateways_for_update_stops_profile_and_unmapped_pids(
    ...
    monkeypatch.setattr(
        ...
        lambda pid: ["pythonw.exe", "-m", "clover_cli.main", "gateway", "run"]
        if pid == 202
        else None,
    )
    ...
    assert token == {
        ...
        "unmapped": [
            {
                "pid": 202,
                "argv": ["pythonw.exe", "-m", "clover_cli.main", "gateway", "run"],
            }
        ],
    }
...
def test_pause_and_resume_windows_gateway_service(
    ...
    assert token == {
        ...
        "unmapped_pids": [],
        "unmapped": [],
        "services": ["CloverGateway"],
        "expected_services": ["CloverGateway"],
        "restarted_services": [],
        "service_profiles": {"CloverGateway": "default"},
    }
    assert stopped == ["CloverGateway"]

    cli_main._resume_windows_gateways_after_update(token)
    assert started == ["CloverGateway"]
...
def test_pause_windows_gateway_service_failure_restores_every_attempted_service(
    ...
    services = [
        SimpleNamespace(name="CloverGateway", service_pid=11, service_create_time=11.0, gateway_pid=101, gateway_create_time=101.0, descendant_identities=()),
        SimpleNamespace(name="CloverGatewayPicasso", service_pid=22, service_create_time=22.0, gateway_pid=202, gateway_create_time=202.0, descendant_identities=()),
    ]
    monkeypatch.setattr(gateway_mod, "find_gateway_pids", lambda **_k: [])
    monkeypatch.setattr(
    ...
    with pytest.raises(RuntimeError, match="CloverGatewayPicasso"):
        cli_main._pause_windows_gateways_for_update()

    assert restarted == ["CloverGatewayPicasso", "CloverGateway"]
...
def test_pause_windows_gateway_service_surfaces_rollback_start_failure(
    ...
    with pytest.raises(RuntimeError, match="rollback failures: CloverGateway"):
        cli_main._pause_windows_gateways_for_update()
...
def test_restore_windows_gateway_service_waits_out_stop_pending(monkeypatch):
    ...
    update_cmd._restore_windows_gateway_service("CloverGateway")

    assert restarted == ["CloverGateway"]
...
def test_resume_windows_gateway_service_failure_stays_retryable(
    ...
    with pytest.raises(RuntimeError, match="CloverGateway"):
        cli_main._resume_windows_gateways_after_update(token)

    assert token["resume_needed"] is True
    assert token["services"] == ["CloverGateway"]
...
def test_unreadable_argv_falls_back_to_the_captured_prefix(monkeypatch):
    ...
    gateway_prefix = r"venv\Scripts\python.exe -m clover_cli.main gateway run"
...
# when every concurrent clover.exe is a gateway runtime — the pause
...
def test_classify_concurrent_instance_recognises_gateway_runtimes(monkeypatch):
    ...
    launcher shape (python -m, clover.exe shim, clover-gateway.exe,
    gateway/run.py, bare `clover gateway` which defaults to run)."""
    cases = [
        [r"C:\venv\Scripts\python.exe", "-m", "clover_cli.main", "gateway", "run"],
        [r"C:\venv\Scripts\clover.exe", "gateway", "run"],
        [r"C:\venv\Scripts\clover-gateway.exe"],
        [r"C:\venv\Scripts\python.exe", "gateway/run.py"],
        ["clover.exe", "GATEWAY", "RUN"],  # matcher is case-insensitive
        ["clover.exe", "gateway"],  # bare `clover gateway` defaults to run
        # profile selector before the subcommand — canonical matcher strips it
        ["clover.exe", "--profile", "work", "gateway", "run"],
    ]
    for argv in cases:
...
def test_classify_concurrent_instance_recognises_non_gateways(monkeypatch):
    ...
    cases = [
        [r"C:\venv\Scripts\clover.exe"],  # interactive REPL
        [r"C:\venv\Scripts\clover.exe", "dashboard"],
        ["clover.exe", "gateway", "status"],  # management, not runtime
        ["clover.exe", "gateway", "stop"],
...
def test_filter_non_gateway_concurrent_instances_splits(monkeypatch):
    ...
    monkeypatch.setitem(
        ...
        _fake_psutil_classify(
            {
                100: ["clover.exe", "gateway", "run"],
                200: ["clover.exe"],  # REPL — keep
                300: ["clover.exe", "dashboard"],  # keep
    ...
    assert kept == [(200, "clover.exe"), (300, "clover.exe"), (400, "clover.exe")]
...
def test_filter_non_gateway_concurrent_instances_gateway_only(monkeypatch):
    ...
    monkeypatch.setitem(
        ...
        _fake_psutil_classify(
            {
                111: ["clover.exe", "gateway", "run"],
                222: [r"C:\venv\Scripts\clover-gateway.exe"],
            }
        ),
    )
    matches = [(111, "clover.exe"), (222, "clover-gateway.exe")]
...
def test_update_gate_skips_abort_when_only_concurrent_is_gateway(
    ...
    with patch.object(
        ...
        cli_main,
        "_detect_concurrent_clover_instances",
        return_value=[(1000, "clover.exe"), (2000, "clover-gateway.exe")],
    ), patch.object(
    ...
    captured = capsys.readouterr().out
    assert "Another clover.exe is running" not in captured
...
def test_update_gate_still_aborts_on_non_gateway_concurrent(
    ...
    with patch.object(
        ...
        cli_main,
        "_detect_concurrent_clover_instances",
        return_value=[(1000, "clover.exe"), (3000, "clover.exe")],
    ), patch.object(
        cli_main,
        "_filter_non_gateway_concurrent_instances",
        return_value=[(3000, "clover.exe")],
    ), patch.object(
        cli_main, "_run_pre_update_backup"
    ) as mock_backup:
...
def test_stop_service_refuses_pid_reuse_before_sc_stop(monkeypatch):
    ...
    with pytest.raises(RuntimeError, match="identity changed"):
        update_cmd._stop_windows_gateway_service(
            "CloverGateway", expected_service_identity=(11, 11.0)
        )
