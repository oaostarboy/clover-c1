...
Regression guard for the bug where `clover model` saved `provider: upstage`
correctly but, on re-entry, showed a different provider as active. Root cause:
`clover_cli/providers.py` (the resolver behind `resolve_provider_full`) had no
...
class TestUpstageOverlay:
    def test_overlay_exists(self):
        ...
        assert "upstage" in CLOVER_OVERLAYS
        overlay = CLOVER_OVERLAYS["upstage"]
...
class TestUpstageConfigProviderWins:
    ...
    Mirrors the display logic in `clover_cli/main.py` (cmd_model): read
