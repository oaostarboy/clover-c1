...
the SAME provider universe as ``clover model`` (the CLI/TUI picker).
...
def test_every_clover_model_provider_is_configurable_in_desktop():
    """PARITY CONTRACT: GUI (keys ∪ accounts) ⊇ `clover model` universe."""
    ...
    assert not missing, (
        "providers shown in `clover model` but not configurable in the desktop "
