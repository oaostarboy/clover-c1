"""Tests for clover_cli/skills_config.py and skills_tool disabled filtering."""
...
class TestSaveDisabledSkills:
    @patch("clover_cli.skills_config.save_config")
...
class TestIsSkillDisabled:
    ...
    @patch("clover_cli.config.load_config")
    ...
    @patch("clover_cli.config.load_config")
    @patch.dict("os.environ", {"CLOVER_PLATFORM": "discord"})
...
class TestGetDisabledSkillNames:
    ...
    def test_session_platform_env_var(self, tmp_path, monkeypatch):
        """CLOVER_SESSION_PLATFORM should be used when CLOVER_PLATFORM is unset."""
        config = tmp_path / "config.yaml"
        config.write_text(
            "skills:\n"
            "  disabled:\n"
    ...
    def test_clover_platform_takes_precedence(self, tmp_path, monkeypatch):
        """CLOVER_PLATFORM should win over CLOVER_SESSION_PLATFORM."""
        config = tmp_path / "config.yaml"
        config.write_text(
            "skills:\n"
            "  platform_disabled:\n"
...
class TestGetCategories:
    def test_extracts_unique_categories(self):
        from clover_cli.skills_config import _get_categories
        skills = [
