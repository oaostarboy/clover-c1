"""Tests for the auxiliary-model configuration UI in ``clover model``.
...
from clover_cli.config import DEFAULT_CONFIG, load_config
...
def test_save_aux_choice_persists_to_config_yaml(tmp_path, monkeypatch):
    ...
    (tmp_path / ".clover").mkdir(exist_ok=True)
...
def test_reset_aux_clears_delegation_routing_preserves_settings(tmp_path, monkeypatch):
    ...
    from clover_cli.config import load_config as _lc, save_config
