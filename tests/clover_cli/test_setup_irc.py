"""Tests for IRC gateway configuration via `clover setup gateway` UI.

Covers the full plugin-platform discovery → status → configure flow so that
a fresh Clover install (no state, no env vars) can set up IRC through the
...
class TestIRCFreshInstallDiscovery:
    """IRC appears in the setup menu on a brand-new Clover install."""
    ...
    def test_irc_status_configured_when_env_set(self, monkeypatch):
        ...
        try:
            ...
            monkeypatch.setenv("IRC_CHANNEL", "#clover")
            monkeypatch.setenv("IRC_NICKNAME", "clover-bot")
...
class TestIRCGatewaySetupFreshInstall:
    """Simulate the full `clover setup gateway` experience with IRC present."""
