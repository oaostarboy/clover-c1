"""Regression tests for #34107 — Docker UID/GID handling in ensure_clover_home.

When Clover runs in Docker with ``CLOVER_UID=1000`` / ``CLOVER_GID=911``,
the entrypoint chowns the top-level ``CLOVER_HOME`` once at startup. But
...
The fix is a ``_chown_to_clover_uid`` helper that reads the env vars and
...
# ---------------------------------------------------------------------------
# _resolve_clover_uid_gid
# ---------------------------------------------------------------------------


class TestResolveCloverUidGid:
    def test_returns_parsed_values_when_both_set(self, monkeypatch):
        monkeypatch.setenv("CLOVER_UID", "1000")
        monkeypatch.setenv("CLOVER_GID", "911")
        from clover_cli.config import _resolve_clover_uid_gid
...
# ---------------------------------------------------------------------------
# _chown_to_clover_uid
# ---------------------------------------------------------------------------


class TestChownToCloverUid:
    ...
    def test_attributeerror_swallowed_for_windows_compat(self, tmp_path, monkeypatch):
        ...
        with patch.object(cfg.os, "chown", side_effect=AttributeError("no chown on this platform")):
            cfg._chown_to_clover_uid(d)  # must not raise
...
class TestSecureDirChown:
    ...
    def test_secure_dir_no_chown_when_env_unset(self, tmp_path, monkeypatch):
        monkeypatch.delenv("CLOVER_UID", raising=False)
        monkeypatch.delenv("CLOVER_GID", raising=False)
        from clover_cli import config as cfg
