...
The bug class: on Windows, when `clover.exe`/sibling shims could not be
...
import clover_cli._install_repair as ir
import clover_cli.update_cmd as update_cmd


def _make_shims(scripts_dir: Path, names=("clover", "clover-gateway")) -> list[Path]:
...
def test_strict_quarantine_refuses_before_install(windows, tmp_path, monkeypatch):
    ...
    # clover.exe cannot be renamed; clover-gateway.exe can
    real_rename = Path.rename

    def deny_clover(self, target):
        if self.name == "clover.exe":
            raise PermissionError(13, "held open")
        return real_rename(self, target)

    monkeypatch.setattr(Path, "rename", deny_clover)
    monkeypatch.setattr(cli_main, "_clover_exe_shims", lambda d: shims)
    ...
    assert install_ran == []
    assert "clover.exe" in exc_info.value.failed_shims
    # The successful rename (clover-gateway.exe) was rolled back.
    assert (scripts / "clover-gateway.exe").exists()
    assert not list(scripts.glob("clover-gateway.exe.old.*"))


def test_non_strict_keeps_warn_and_try(windows, tmp_path, monkeypatch):
    scripts = tmp_path / "venv" / "Scripts"
    shims = _make_shims(scripts, names=("clover",))
    monkeypatch.setattr(
        Path, "rename",
...
def test_recovery_install_cmd_fail_closed(windows, tmp_path, monkeypatch):
    ...
    _make_shims(scripts, names=("clover",))
...
def test_refusal_writes_marker_and_exits_2(monkeypatch, capsys):
    ...
    exc = cli_main.ShimQuarantineError(["clover.exe"])
    with pytest.raises(SystemExit) as exit_info:
        update_cmd._refuse_update_for_contended_shims(exc)
    assert exit_info.value.code == 2
    assert wrote == [1]
    out = capsys.readouterr().out
    assert "clover.exe" in out
