...
1. The standalone daemon path (``clover kanban daemon --force`` /
   :func:`clover_cli.kanban_db.run_daemon`) never resolved
...
def test_run_daemon_resolves_and_passes_max_in_progress(
    ...
    ``clover kanban dispatch`` paths both resolved the memory-derived
    default.
    """
    captured: dict = {}
...
def test_nonspawnable_review_does_not_tax_ready_budget(
    ...
    import clover_cli.config as cfgmod
    import clover_cli.profiles as profmod
