...
Covers:
- Defense 1: gateway stop/restart refuse when _CLOVER_GATEWAY=1
...
class TestGatewayLifecyclePattern:
    ...
    @pytest.mark.parametrize("text", [
        ...
        "launchctl submit -l ai.clover.gateway-hard-restart-no-photon-notice -- /bin/sh ~/.clover/scripts/hard_restart_gateway_no_photon_notice.sh",
        "launchctl submit -l clover-gateway-restart-helper -- /bin/sh helper.sh",
        # bootstrap loads an arbitrary plist — same laundering shape.
        "launchctl bootstrap gui/501 ~/Library/LaunchAgents/ai.clover.gateway.restart-once.plist",
        "launchctl bootout gui/501/ai.clover.gateway",
        ...
        (
            "launchctl submit \\\n"
            "  -l ai.clover.gateway-hard-restart-no-photon-notice \\\n"
            "  -- /bin/sh ~/.clover/scripts/hard_restart_gateway_no_photon_notice.sh"
        ),
    ])
    ...
    def test_label_defined_before_verb_is_caught(self):
        ...
        cmd = (
            "uid=$(id -u); for item in 'ai.clover.gateway-apollo:/a.plist' "
            "'ai.clover.gateway-cronus:/c.plist' 'ai.clover.gateway-plutus:/p.plist' "
            "'ai.clover.gateway:/Users/botuser/Library/LaunchAgents/ai.clover.gateway.plist'; "
    ...
    def test_line_continuation_does_not_bridge_unrelated_lines(self):
        ...
        text = (
            ...
            "unrelated clover note on the next line"
        )
    ...
    @pytest.mark.parametrize("text", [
        ...
        # `launchctl kick"start" ... ai.clover.gateway` executes exactly as
        ...
        'launchctl kick"start" -k gui/501/ai.clover.gateway',
        "launchctl kick'start' -k gui/501/ai.clover.gateway",
        "launchctl kick\\start -k gui/501/ai.clover.gateway",
        'launchctl "kickstart" -k gui/501/ai.clover.gateway',
        # Splices on the newer/legacy unload spellings this PR added.
        'launchctl boot"out" gui/501/ai.clover.gateway',
        "launchctl dis\\able gui/501/ai.clover.gateway",
        # The gateway identifier itself can be spliced just as easily.
        'launchctl bootout gui/501/ai.clover."gateway"',
        # Same class on the systemctl and clover-CLI branches.
        'systemctl re"start" clover-gateway',
        'clover gateway re"start"',
    ])
    ...
    def test_spliced_verb_inside_shell_c_payload_is_blocked(self):
        ...
        command = 'sh -c \'launchctl kick"start" -k gui/501/ai.clover.gateway\''
    ...
    @pytest.mark.parametrize("text", [
        ...
        'launchctl kick"start" -k gui/501/ai.clover.update-checker',
        'systemctl re"start" clover-meta.service',
    ...
    @pytest.mark.parametrize("text", [
        ...
        "clover cron list",
        "clover update",
        "clover config set model claude",
        "echo 'just a normal cron job'",
        "run the backup script",
        "gateway is running fine",
        # `clover gateway start` is benign — starting a gateway from inside a
        ...
        "clover gateway start --all",
        # Tightened launchctl/systemctl branches: ops on NON-gateway clover
        # services must not be falsely blocked (the old `.*clover` matched any
        # clover token).
        "launchctl unload ai.clover.update-checker.plist",
        "launchctl restart ai.clover.daemon",
        ...
        "systemctl restart clover-meta.service",
        "systemctl restart clover-cron-helper",
        ...
        "echo after the clover gateway restarted cleanly",
        "the clover gateway stopped responding, please investigate",
        # #92372 Branch D: `p?kill` without a leading \b matched the "kill"
        # tail of "skill".
        "clover skill view gateway-notes && echo clover gateway docs",
        # #77173/#77536: a file path with embedded spaces containing the
        # lifecycle words must not match — `clover` is a path component
        # there, not a command.
        "cat '/docs/clover gateway restart-notes.md'",
        "less /home/user/notes/clover gateway restart runbook.txt",
    ])
    ...
    @pytest.mark.parametrize("text", [
        ...
        "clover gateway restart",
        "clover gateway restart; echo done",
        "clover gateway stop && echo stopped",
        # #77173 command-position anchor must not weaken separator/subshell
        # forms either.
        "true;clover gateway restart",
        "true && clover gateway stop",
        "echo $(clover gateway restart)",
        "echo `clover gateway restart`",
    ])
    ...
    @pytest.mark.parametrize("text", [
        ...
        'import subprocess\nsubprocess.run(["launchctl", "bootout", "gui/501/ai.clover.gateway"])',
        'subprocess.run(["clover", "gateway", "restart"])',
        'os.system("launchctl kickstart -k gui/501/ai.clover.gateway")',
    ])
    ...
    @pytest.mark.parametrize("text", [
        ...
        'data = ["clover", "notes"]  # unrelated list',
    ])
    ...
    def test_inert_heredoc_body_prose_not_blocked(self):
        ...
        text = (
            ...
            "If the box is wedged, a human can run: clover gateway restart\n"
            "EOF"
        )
        assert not _contains_gateway_lifecycle_command(text), f"Should NOT match: {text!r}"
...
class TestProfileFlagGatewayLifecycle:
    """#78028: `clover -p <profile> gateway restart|stop` bypasses Branch A's
    ...
    def _pin_profile_identity(self, monkeypatch):
        # The ambient test env may carry CLOVER_HOME/CLOVER_PROFILE; pin the
        # profile identity explicitly so every assertion is deterministic.
        monkeypatch.setenv("CLOVER_PROFILE", "zeus")
        monkeypatch.delenv("CLOVER_PROFILE_NAME", raising=False)

    @pytest.mark.parametrize("text", [
        "clover -p zeus gateway stop",
        "clover -p zeus gateway restart",
        "clover --profile zeus gateway restart",
        "clover --profile zeus gateway stop",
        "clover --profile=zeus gateway restart",
        # Global flags before/after the selector must not hide the shape.
        "clover -v -p zeus gateway restart",
        "clover -p zeus -v gateway restart",
        "clover --debug --profile zeus gateway stop",
    ...
    @pytest.mark.parametrize("text", [
        "clover -p venus gateway stop",
        "clover -p venus gateway restart",
        "clover --profile venus gateway restart",
        "clover --profile=venus gateway stop",
        "clover -p venus -v gateway restart",
    ])
    def test_sibling_allowed(self, text):
        assert not _contains_gateway_lifecycle_command(text), f"Should allow: {text!r}"

    @pytest.mark.parametrize("text", [
        "clover -p zeus gateway start",
        "clover -p zeus gateway start --all",
    ])
    ...
    def test_adjacent_form_still_blocked(self):
        ...
        assert _contains_gateway_lifecycle_command("clover gateway restart")
        assert _contains_gateway_lifecycle_command("clover gateway stop")

    def test_clover_home_derived_profile(self, monkeypatch):
        # Without CLOVER_PROFILE the guard falls back to the CLOVER_HOME-
        # derived profile identity (get_active_profile_name) — the signal the
        # gateway process itself carries.
        monkeypatch.delenv("CLOVER_PROFILE", raising=False)
        monkeypatch.delenv("CLOVER_PROFILE_NAME", raising=False)
        import clover_cli.profiles as profiles_mod

        monkeypatch.setattr(profiles_mod, "get_active_profile_name", lambda: "zeus")
        assert _contains_gateway_lifecycle_command("clover -p zeus gateway restart")
        assert not _contains_gateway_lifecycle_command("clover -p venus gateway restart")
    ...
    def test_no_profile_context_conservative_allow(self, monkeypatch):
        ...
        assert not _contains_gateway_lifecycle_command("clover -p zeus gateway restart")
        assert _contains_gateway_lifecycle_command("clover gateway restart")
...
class TestCronCreateLifecycleBlock:
    ...
    def test_block_clover_gateway_restart(self, capsys):
        args = Namespace(
            cron_command="create",
            schedule="30m",
            prompt="Upgrade clover then run clover gateway restart",
            name=None,
            deliver=None,
            repeat=None,
            skill=None,
    ...
    def test_block_script_with_lifecycle_command(self, tmp_path, capsys, monkeypatch):
        ...
        # restart_clover_gateway_once.sh). The script must live under
        # CLOVER_HOME/scripts so the scheduler — and the guard — resolve it.
        monkeypatch.setenv("CLOVER_HOME", str(tmp_path / ".clover"))
        scripts_dir = tmp_path / ".clover" / "scripts"
...
class TestGatewaySelfTargetingGuard:
    ...
    def test_stop_allows_outside_gateway(self, monkeypatch):
        ...
        monkeypatch.delenv("_CLOVER_GATEWAY", raising=False)
...
class TestTerminalToolGatewayLifecycleGuard:
    """terminal_tool must refuse gateway lifecycle commands when _CLOVER_GATEWAY=1.

    Issue #37453: systemctl --user restart clover-gateway runs as a child of the
    ...
    @pytest.mark.parametrize("cmd", [
        "systemctl restart clover-gateway",
        "systemctl --user restart clover-gateway",
        "systemctl stop clover-gateway.service",
        "clover gateway restart",
        "clover gateway uninstall",
        "launchctl kickstart gui/501/ai.clover.gateway",
        "launchctl bootout gui/501/ai.clover.gateway",
        # #62891 exact reported shape and its bootstrap sibling.
        "launchctl submit -l ai.clover.gateway-hard-restart-no-photon-notice -- /bin/sh ~/.clover/scripts/hard_restart_gateway_no_photon_notice.sh",
        "launchctl submit -l com.foo -- /path/gateway",
        "launchctl bootstrap gui/501 ~/Library/LaunchAgents/ai.clover.gateway.restart-once.plist",
        "pkill -f clover.*gateway",
    ])
    ...
    def test_force_true_cannot_bypass_block(self, monkeypatch):
        ...
        result = json.loads(tt.terminal_tool(
            command="systemctl restart clover-gateway", force=True
        ))
    ...
    def test_blocks_launchctl_submit_inside_gateway(self, monkeypatch, tmp_path):
        ...
        result = json.loads(tt.terminal_tool(
            command=(
                "launchctl submit -l ai.clover.delayed-ops -- "
                f"/bin/bash {script}"
            )
        ))
    ...
    @pytest.mark.parametrize("command", [
        # Neutral, non-clover label: label-independent detection is the point
        # (#62891 second reproduction used `ai.clover.svc-reload-tmp`).
        "launchctl submit -l com.foo -- /path/gateway",
        "launchctl submit -l ai.clover.svc-reload-tmp -- /bin/sh /tmp/h-svc-reload.sh",
    ...
    def test_cli_agent_session_not_blocked_by_inherited_env(
        ...
        """#92560: CLI/TUI agent sessions inherit _CLOVER_GATEWAY=1 from the
        ...
        # Simulate a CLI agent session: _CLOVER_GATEWAY=1 is in the
        ...
        self._patch_env(monkeypatch, _FakeEnv(), inside_gateway=False)
        monkeypatch.setenv("_CLOVER_GATEWAY", "1")
        monkeypatch.setattr(
            tt, "_check_all_guards", lambda cmd, env, **kwargs: {"approved": True}
        )

        result = json.loads(tt.terminal_tool(command="clover gateway restart"))

        assert result["exit_code"] == 0
        assert calls == ["clover gateway restart"]
    ...
    def test_blocks_launchctl_submit_hidden_in_referenced_script(
        ...
        script.write_text(
            "#!/bin/bash\nlaunchctl submit -l ai.clover.loop -- /bin/true\n"
        )
        self._patch_env(monkeypatch, self._make_fake_env(), inside_gateway=True)
    ...
    def test_launchctl_submit_parser_handles_shell_quoting(self, monkeypatch):
        ...
        self._patch_env(monkeypatch, self._make_fake_env(), inside_gateway=True)
        result = json.loads(tt.terminal_tool(
            command="launchctl sub\"\"mit -l ai.clover.loop -- /bin/true"
        ))
    ...
    def test_safe_systemctl_commands_pass_through(self, monkeypatch):
        """Non-clover systemctl commands must not be blocked by this guard."""
        import tools.terminal_tool as tt
...
class TestLifecycleGuardModule:
    ...
    def test_prompt_with_command_raises(self):
        ...
        with pytest.raises(GatewayLifecycleBlocked) as exc:
            check_gateway_lifecycle("please run clover gateway restart", None)
    ...
    def test_split_across_prompt_and_script_still_blocks(self, tmp_path):
        ...
        script.write_text("clover gateway stop\n", encoding="utf-8")
        with pytest.raises(GatewayLifecycleBlocked):
    ...
    def test_binary_script_does_not_silently_bypass(self, tmp_path):
        ...
        script.write_bytes(b"\xfeclover gateway restart\xff")
        with pytest.raises(GatewayLifecycleBlocked):
            check_gateway_lifecycle("", str(script))


    def test_relative_script_resolved_under_scripts_dir(self, tmp_path, monkeypatch):
        """A bare/relative script name resolves under CLOVER_HOME/scripts (the
    ...
    def test_python_script_with_literal_lifecycle_command_still_blocked(
        ...
        script.write_text('import os\nos.system("clover gateway restart")\n', encoding="utf-8")
        with pytest.raises(GatewayLifecycleBlocked):
            check_gateway_lifecycle("clean prompt", str(script))
    ...
    def test_read_referenced_script_tolerates_nul_in_path(self):
        ...
        text, unsafe = _read_referenced_script(Path("/tmp/clover\x00binary"))
        assert text is None
    ...
    def test_binary_from_remote_callback_never_false_positives(self):
        ...
        def _remote_read(_path: str):
            return "MZ\x00\x00\x90\x00 clover gateway restart \x00\x00junk"
    ...
    def test_cron_guard_total_when_home_unresolvable(self, monkeypatch):
        """`get_clover_home()` falls back to Path.home(), which raises
        RuntimeError when neither CLOVER_HOME nor HOME resolves
...
class TestRelativePathDoesNotDisableDataExemption:
    ...
    @pytest.mark.parametrize("command", [
        "grep -r 'systemctl restart clover-gateway' .",
        "grep -rn 'clover gateway restart' ./logs",
        "rg 'clover gateway restart' ../archive",
        "grep -c 'systemctl stop clover-gateway' ./var/log/syslog",
        "sqlite3 ./stats.db \"SELECT restart_reason FROM clover_gateway_restarts\"",
    ])
    ...
    @pytest.mark.parametrize("command", [
        ...
        'sqlite3 ./db ".shell clover gateway restart"',
        'sqlite3 ./db ".system systemctl restart clover-gateway"',
        'psql ./x -c "\\! systemctl restart clover-gateway"',
        "grep -r 'clover gateway restart' . | sh",
        "grep -r 'clover gateway restart' ./logs | bash",
        "grep -r 'clover gateway restart' . | sudo sh",
        "grep -r 'x' . ; clover gateway restart",
        "grep -r 'x' . && systemctl restart clover-gateway",
        'grep -r "$(clover gateway restart)" .',
        "rg 'x' ./logs | xargs systemctl restart clover-gateway",
    ])
    ...
    @pytest.mark.parametrize("command", [
        ...
        'sqlite3 db ".shell clover gateway restart"',
        'sqlite3 db ".system systemctl restart clover-gateway"',
        'psql -c "\\! systemctl restart clover-gateway"',
    ])
...
class TestCreateJobBlocksLifecycleCommands:
    ...
    clover_cli.cron.cron_create. Enforcing at create_job covers both."""
    ...
    def test_create_job_blocks_prompt_command(self):
        ...
        with pytest.raises(GatewayLifecycleBlocked):
            create_job(prompt="then run clover gateway restart", schedule="30m")
    ...
    def test_cronjob_tool_surfaces_block_as_error(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("CLOVER_HOME", str(tmp_path / ".clover"))
        (tmp_path / ".clover").mkdir(parents=True)
        from tools.cronjob_tools import cronjob
        result = json.loads(cronjob(
            action="create", schedule="0 9 * * *",
            prompt="please run clover gateway restart nightly",
        ))
...
class TestLifecycleGuardDataArgumentExemption:
    ...
    @pytest.mark.parametrize("command", [
        ...
        "'systemctl stop clover-gateway'\"",
        # grep/rg pattern arguments hunting for the lifecycle string.
        "grep -c 'systemctl restart clover-gateway' /var/log/syslog",
        "rg 'clover gateway restart' /home/user/.clover/logs/",
        "journalctl -u clover-gateway --grep 'systemctl restart clover-gateway'",
        # SQL with stop/restart column/value words but no command shape.
        'sqlite3 stats.db "SELECT stop_time, restart_reason FROM '
        'clover_gateway_restarts"',
    ...
    @pytest.mark.parametrize("command", [
        ...
        'sqlite3 db ".shell clover gateway restart"',
        'psql -c "\\! systemctl restart clover-gateway"',
        "grep 'systemctl restart clover-gateway' cmds.txt | sh",
        "grep gateway f | xargs systemctl restart clover-gateway",
        'grep "$(systemctl restart clover-gateway)" f',
        "grep 'restart' log; systemctl restart clover-gateway",
        'sqlite3 db "SELECT 1"; clover gateway stop',
        # Plain lifecycle commands are unaffected by the exemption.
        "clover gateway restart",
        "sudo systemctl stop clover-gateway",
    ])
    ...
    def test_python_script_branch_gets_the_same_exemption(self, tmp_path):
        ...
        prompt = (
            ...
            "WHERE msg LIKE '%systemctl restart clover-gateway%'\""
        )
