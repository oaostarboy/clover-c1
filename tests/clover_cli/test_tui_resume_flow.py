...
def test_termux_skips_bundled_skill_sync_when_stamp_fresh(monkeypatch, tmp_path, main_mod):
    ...
    monkeypatch.setattr(main_mod, "get_clover_home", lambda: tmp_path)
...
def test_oneshot_subprocess_exits_without_teardown_abort():
    program = textwrap.dedent(
        ...
        from clover_cli.main import _exit_after_oneshot
...
def test_oneshot_wires_session_db_for_recall(monkeypatch):
    """clover -z bypasses CloverCLI, but recall still needs SessionDB."""
    from clover_cli.oneshot import _run_agent
    ...
    monkeypatch.setitem(sys.modules, "clover_state", mod("clover_state", SessionDB=FakeSessionDB))
    monkeypatch.setitem(
        sys.modules,
        "clover_cli.config",
        mod("clover_cli.config", load_config=lambda: {"model": {"default": "m"}}),
    )
    monkeypatch.setitem(
        sys.modules,
        "clover_cli.models",
        mod("clover_cli.models", detect_provider_for_model=lambda *_args, **_kwargs: None),
    )
    monkeypatch.setitem(
        sys.modules,
    ...
    monkeypatch.setitem(
        sys.modules,
        "clover_cli.tools_config",
        mod("clover_cli.tools_config", _get_platform_tools=lambda *_args, **_kwargs: {"session_search"}),
    )
...
def test_launch_tui_exports_model_provider_and_toolsets(monkeypatch, main_mod):
    ...
    def fake_call(argv, cwd=None, env=None):
        ...
        active_path_during_call = Path(env["CLOVER_TUI_ACTIVE_SESSION_FILE"])
    ...
    active_path = Path(env["CLOVER_TUI_ACTIVE_SESSION_FILE"])
    assert active_path.name.startswith("clover-tui-active-session-")
...
def test_make_tui_argv_dev_prebuilds_clover_ink(monkeypatch, main_mod, tmp_path):
