...
- Bare `clover setup` drops straight into the full reconfigure wizard
  (every prompt shows the current value as its default).
- `clover setup --quick` runs the narrower "fill in missing items" flow.
- `clover setup --reconfigure` is a backwards-compat alias for the
  bare-setup default.
...
def _enter_existing_install_patches(stack, **extra):
    ...
    for target, kwargs in [
        ("clover_cli.setup.ensure_clover_home", {}),
        ("clover_cli.setup.is_interactive_stdin", {"return_value": True}),
        ("clover_cli.config.is_managed", {"return_value": False}),
        ("clover_cli.setup.load_config", {"return_value": {}}),
        ("clover_cli.setup.save_config", {}),
        ("clover_cli.setup.get_env_value", {"return_value": None}),
        ("clover_cli.auth.get_active_provider", {"return_value": "openrouter"}),
        ("clover_cli.setup._print_setup_summary", {}),
        ("clover_cli.setup._offer_openclaw_migration", {"return_value": False}),
    ]:
...
def _enter_fresh_install_patches(stack, **extra):
    for target, kwargs in [
        ...
        ("clover_cli.setup.load_config", {"return_value": {}}),
        ("clover_cli.setup.save_config", {}),
        ("clover_cli.auth.get_active_provider", {"return_value": None}),
        ("clover_cli.setup.get_env_value", {"return_value": None}),
        ("clover_cli.setup._offer_openclaw_migration", {"return_value": False}),
    ]:
        stack.enter_context(patch(target, **kwargs))
...
class TestExistingInstallDefault:
    """Bare `clover setup` on an existing install = full reconfigure wizard."""
    ...
    def test_bare_setup_runs_full_reconfigure_without_menu(self, existing_install):
        ...
        with ExitStack() as stack:
            m = _enter_existing_install_patches(
                stack,
                prompt_choice="clover_cli.setup.prompt_choice",
                quick="clover_cli.setup._run_quick_setup",
                model="clover_cli.setup.setup_model_provider",
                terminal="clover_cli.setup.setup_terminal_backend",
                agent="clover_cli.setup.setup_agent_settings",
                gateway="clover_cli.setup.setup_gateway",
                tools="clover_cli.setup.setup_tools",
            )
...
class TestArgparse:
    ...
    def test_reconfigure_flag_reaches_cmd_setup(self, monkeypatch):
        ...
        captured = {}
        monkeypatch.setattr(
            "clover_cli.setup.run_setup_wizard",
            lambda args: captured.setdefault("args", args),
        )
        monkeypatch.setattr(sys, "argv", ["clover", "setup", "--reconfigure"])
        try:
            main()
        except SystemExit:
            pass
