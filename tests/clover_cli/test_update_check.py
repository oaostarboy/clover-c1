"""Tests for the update check mechanism in clover_cli.banner."""
...
def test_check_for_updates_uses_cache(tmp_path, monkeypatch):
    ...
    with patch("clover_cli.banner.subprocess.run") as mock_run:
...
def test_check_for_updates_does_not_cache_none(tmp_path, monkeypatch):
    ...
    cache_file = tmp_path / ".update_check"
    monkeypatch.setenv("CLOVER_HOME", str(tmp_path))
    monkeypatch.delenv("CLOVER_REVISION", raising=False)
    ...
    monkeypatch.setattr(
        "clover_cli.config.detect_install_method", lambda root: "git"
    )
    monkeypatch.setattr(
        "clover_cli.config.get_project_root", lambda: repo_dir
    )
    ...
    # Simpler: just patch the get_clover_home and the repo_dir resolution
    # by making check_for_updates find our fake repo via clover_home fallback.
    # The code checks Path(__file__).parent.parent/.git first, then falls
    # back to clover_home / "clover-c1". We ensure the fallback hits.
    # To do this, we make Path(__file__).parent.parent.resolve() return
    # a path without .git, so it falls through to clover_home / "clover-c1".
    real_resolve = Path.resolve

    def fake_resolve(self, *args, **kwargs):
        s = str(self)
        if "banner.py" in s or s.endswith("clover_cli"):
