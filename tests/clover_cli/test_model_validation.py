"""Tests for provider-aware `/model` validation in clover_cli.models."""
...
def _validate(model, provider="openrouter", api_models=FAKE_API_MODELS, **kw):
    ...
    with patch("clover_cli.models.fetch_api_models", return_value=api_models), \
         patch("clover_cli.models.probe_api_models", return_value=probe_payload):
...
class TestFetchApiModels:
    ...
    def test_probe_api_models_tries_v1_fallback(self):
        ...
        with patch("clover_cli.models._urlopen_model_catalog_request", side_effect=_fake_urlopen):
    ...
    def test_probe_api_models_uses_copilot_catalog(self):
        ...
        with patch("clover_cli.models._urlopen_model_catalog_request", return_value=_Resp()) as mock_urlopen:
...
class TestValidateApiFallback:
    ...
    def test_fetch_lmstudio_models_filters_embedding_type(self):
        ...
        with patch("clover_cli.models._urlopen_model_catalog_request", return_value=mock_resp):
    ...
    def test_validate_lmstudio_distinguishes_auth_failure(self):
        ...
        with patch("clover_cli.models._urlopen_model_catalog_request", side_effect=http_error):
...
class TestValidateCodexAutoCorrection:
    ...
    def test_missing_dash_auto_corrects(self):
        ...
        with patch("clover_cli.models.provider_model_ids", return_value=codex_models):
...
class TestValidateCodex900kVariants:
    """`-900k` is a Clover picker convention: valid variants come from the
    ...
    def test_catalog_listed_variant_accepted(self):
        with patch("clover_cli.models.provider_model_ids", return_value=self._CATALOG):
    ...
    def test_valid_variant_missing_from_catalog_still_accepted(self):
        ...
        with patch("clover_cli.models.provider_model_ids", return_value=["gpt-5.6-sol"]):
            result = validate_requested_model("gpt-5.6-sol-900k", "openai-codex")
        assert result["accepted"] is True
...
class TestProbeApiModelsUserAgent:
    """Probing custom /v1/models must send a Clover User-Agent.
    ...
    def test_probe_sends_clover_user_agent(self):
        ...
        assert ua.startswith("clover-cli/"), (
            f"User-Agent must advertise clover-cli, got {ua!r}"
        )
    ...
    def test_probe_user_agent_sent_without_api_key(self):
        ...
        assert ua and ua.startswith("clover-cli/")
...
class TestValidateOpenRouterVariantSuffixes:
    ...
    def test_static_catalog_fallback_accepts_variant(self):
        ...
        with patch("clover_cli.models.fetch_api_models", return_value=None), \
             patch(
                 "clover_cli.models.provider_model_ids",
...
class TestValidateRequestedModelNousPortalRecommendations:
    """Regression tests for issue #71312: the Clover Telegram picker (and any
    other messaging-platform /model validation, since they all share
    validate_requested_model()) rejected models that are live Clover Portal
    recommendations (/api/clover/recommended-models) but not yet in the
    hardcoded curated catalog -- even though `clover chat` already accepts
    ...
    def _validate_clover(self, model, api_models=None, portal_payload=None, portal_raises=False):
        ...
        with patch("clover_cli.models.fetch_api_models", return_value=api_models), \
             patch("clover_cli.models.probe_api_models", return_value=probe_payload), \
             patch("clover_cli.models.fetch_clover_recommended_models", side_effect=_fetch_portal), \
             patch("clover_cli.models._resolve_clover_portal_url", return_value=""), \
             patch("clover_cli.models._model_in_provider_catalog", return_value=False):
            return validate_requested_model(model, "clover")
    ...
    def test_free_portal_recommendation_accepted(self):
        ...
        result = self._validate_clover("inclusionai/ling-3.0-flash:free")
        assert result["accepted"] is True
        assert result["persist"] is True
        assert "Portal recommendation" in (result["message"] or "")

    def test_paid_portal_recommendation_accepted(self):
        result = self._validate_clover("inclusionai/ling-3.0-pro")
        assert result["accepted"] is True
    ...
    def test_model_absent_from_portal_and_catalog_still_rejected(self):
        ...
        result = self._validate_clover("totally-made-up-model-xyz")
        assert result["accepted"] is False
    ...
    def test_non_string_model_name_entries_ignored(self):
        ...
        assert self._validate_clover("5", portal_payload=payload)["accepted"] is False
        result = self._validate_clover(
            "inclusionai/ling-3.0-flash:free", portal_payload=payload
        )
        assert result["accepted"] is True

    def test_non_clover_provider_does_not_consult_portal_feed(self):
        """This fallback tier is Clover-specific; a non-Clover provider must
        not have its rejection changed by (or trigger a call to) the Clover Portal feed."""
        probe_payload = {
        ...
        with patch("clover_cli.models.fetch_api_models", return_value=["some/other-model"]), \
             patch("clover_cli.models.probe_api_models", return_value=probe_payload), \
             patch("clover_cli.models.fetch_clover_recommended_models") as mock_portal, \
             patch("clover_cli.models._model_in_provider_catalog", return_value=False):
    ...
    def test_curated_catalog_hit_short_circuits_before_portal_check(self):
        ...
        with patch("clover_cli.models.fetch_api_models", return_value=api_models), \
             patch("clover_cli.models.probe_api_models", return_value=probe_payload), \
             patch("clover_cli.models._model_in_provider_catalog", return_value=True), \
             patch("clover_cli.models.fetch_clover_recommended_models") as mock_portal:
            result = validate_requested_model("inclusionai/ling-2.6-flash", "clover")
        mock_portal.assert_not_called()
        assert result["accepted"] is True
