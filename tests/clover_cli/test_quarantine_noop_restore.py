...
``clover*.exe`` shims aside (``clover.exe.old.<ms>``) before invoking the
...
quarantined shims stayed renamed aside and ``clover`` vanished from PATH
...
def _make_scripts_dir(tmp_path: Path) -> Path:
    ...
    for name in ("clover", "clover-c1", "clover-acp", "clover-gateway"):
...
# ---------------------------------------------------------------------------
# clover_cli.main._run_quarantined_install
# ---------------------------------------------------------------------------
...
def test_main_noop_success_restores_shims(tmp_path):
    ...
    assert "clover.exe" in names, "clover.exe must be restored after a no-op install"
    assert "clover-acp.exe" in names
    assert "clover-gateway.exe" in names
    assert (scripts / "clover.exe").read_bytes() == b"MZ-old-clover"
...
def test_main_rewriting_success_keeps_fresh_shims(tmp_path):
    ...
    assert (scripts / "clover.exe").read_bytes() == b"MZ-new-clover"
...
# ---------------------------------------------------------------------------
# clover_cli._install_repair._run_install_cmd (the deferred-recovery path)
# ---------------------------------------------------------------------------
...
def test_repair_noop_success_restores_shims(tmp_path):
    ...
    names = _shim_names(scripts)
    assert "clover.exe" in names, "clover.exe must be restored after a no-op recovery install"
    assert (scripts / "clover.exe").read_bytes() == b"MZ-old-clover"
