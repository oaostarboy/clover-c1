"""Regression tests for Clover Portal inference_base_url host-allowlist validation.
...
client. ``_validate_clover_inference_url_from_network()`` blocks any URL
...
4. The env-var override path (``CLOVER_INFERENCE_BASE_URL``) is NOT
...
from clover_cli.auth import (
    ...
    _ALLOWED_CLOVER_INFERENCE_HOSTS,
...
class TestValidatorRules:
    ...
    def test_attacker_host_rejected(self, caplog):
        with caplog.at_level(logging.WARNING, logger="clover_cli.auth"):
            assert (
                _validate_clover_inference_url_from_network("https://attacker.com/v1")
                is None
            )
    ...
    def test_default_inference_url_is_in_allowlist(self):
        """Sanity check: DEFAULT_CLOVER_INFERENCE_URL must itself validate.

        If anyone retargets the default away from
        ``inference-api.``, they MUST update the allowlist
        in the same change — otherwise the allowlist would reject the
        Portal's own legitimate default and break every install.
        """
        assert (
            _validate_clover_inference_url_from_network(DEFAULT_CLOVER_INFERENCE_URL)
            == DEFAULT_CLOVER_INFERENCE_URL.rstrip("/")
        )
...
class TestCallSiteWiring:
    ...
    ``_validate_clover_inference_url_from_network`` with the un-validated
    ``_optional_base_url`` again, the test catches it.

    Each site lives inside ``resolve_clover_runtime_credentials`` and one
    ...
    def _read_auth_source(self):
        import clover_cli.auth as _auth_mod
    ...
    def test_no_unvalidated_inference_base_url_assignments_remain(self):
        ...
        for needle in (
            ...
            assert needle not in source, (
                ...
                f"Use _validate_clover_inference_url_from_network() instead."
            )
    ...
    def test_validator_wired_at_all_known_call_sites(self):
        ...
        refresh_count = source.count(
            '_validate_clover_inference_url_from_network(refreshed.get("inference_base_url"))'
        )
        mint_count = source.count(
            '_validate_clover_inference_url_from_network(mint_payload.get("inference_base_url"))'
        )
    ...
    def test_proxy_adapter_also_validates(self):
        """The Clover proxy adapter applies the validator as defense-in-depth
        ...
        boundary."""
        from pathlib import Path
        import clover_cli.proxy.adapters.clover_portal as _clover_adapter
        source = Path(_clover_adapter.__file__).read_text(encoding="utf-8")
        assert "_validate_clover_inference_url_from_network" in source


class TestEnvOverrideNotGated:
    """The documented dev/staging env-var override must keep working.

    ``CLOVER_INFERENCE_BASE_URL`` is read by ``resolve_clover_runtime_credentials``
    ...
    def test_env_override_path_does_not_call_validator(self):
        """In resolve_clover_runtime_credentials, the env override is
        ...
        for line in source.splitlines():
            if "CLOVER_INFERENCE_BASE_URL" in line and "os.getenv" in line:
                assert "_validate_clover_inference_url_from_network" not in line, (
...
class TestHealsPoisonedStoredValue:
    ...
    stale ``stg-inference-api.`` persisted before the
    ...
    staging endpoint (real incident: opus-4.8 routed to clover, clover pinned
    ...
    def test_refresh_resets_rejected_url_to_default(self, monkeypatch):
        ...
        poisoned = "https://stg-inference-api./v1"
        state = {
            "access_token": "tok",
            "refresh_token": "rtok",
            "client_id": "clover-cli",
            "portal_base_url": auth.DEFAULT_CLOVER_PORTAL_URL,
        ...
        monkeypatch.setattr(auth, "_clover_invoke_jwt_status", lambda *a, **k: "needs_refresh")
        monkeypatch.setattr(
            auth,
        ...
        monkeypatch.setattr(auth, "_assert_clover_inference_jwt_usable", lambda *a, **k: None)
        monkeypatch.setattr(auth, "_select_clover_invoke_jwt", lambda *a, **k: None)

        result = auth.refresh_clover_oauth_from_state(state, force_refresh=True)

        assert result["inference_base_url"] == auth.DEFAULT_CLOVER_INFERENCE_URL, (
...
class TestEnvOverrideWins:
    """``CLOVER_INFERENCE_BASE_URL`` must win over the stored value for the
    ...
    def _patch_no_refresh(self, monkeypatch, auth, state):
        ...
        monkeypatch.setattr(auth, "_write_shared_clover_state", lambda *a, **k: None)
        monkeypatch.setattr(auth, "_sync_clover_pool_from_auth_store", lambda *a, **k: None)
    ...
    def test_no_refresh_env_override_not_persisted(self, monkeypatch):
        ...
        state = self._base_state(auth, auth.DEFAULT_CLOVER_INFERENCE_URL)
        self._patch_no_refresh(monkeypatch, auth, state)
        monkeypatch.setenv("CLOVER_INFERENCE_BASE_URL", self.STAGING)

        auth.resolve_clover_runtime_credentials()

        assert state["inference_base_url"] == auth.DEFAULT_CLOVER_INFERENCE_URL, (
    ...
    def test_no_refresh_heals_poisoned_stored_without_env(self, monkeypatch):
        ...
        self._patch_no_refresh(monkeypatch, auth, state)
        monkeypatch.delenv("CLOVER_INFERENCE_BASE_URL", raising=False)

        result = auth.resolve_clover_runtime_credentials()
        assert result["base_url"] == auth.DEFAULT_CLOVER_INFERENCE_URL, (
...
class TestProxyAdapterEnvOverride:
    """The Clover proxy adapter is the second chokepoint: it re-validates the
    base_url returned by resolve_clover_runtime_credentials() against the prod
    allowlist. That re-validation must not clobber a legitimate
    CLOVER_INFERENCE_BASE_URL staging override.
    """
    ...
    def test_proxy_adapter_consults_env_override(self):
        ...
        source = Path(_clover_adapter.__file__).read_text(encoding="utf-8")
        assert "_clover_inference_env_override()" in source, (
