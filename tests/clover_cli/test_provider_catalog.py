"""Tests for the unified provider catalog (clover_cli.provider_catalog).

These are invariant tests, not snapshots: they assert the parity *contract*
between what ``clover model`` shows (``CANONICAL_PROVIDERS``) and what the
...
from clover_cli.provider_catalog import (
    ProviderDescriptor,
    provider_catalog,
