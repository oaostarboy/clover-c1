"""Dashboard Clover Console websocket tests."""
...
@pytest.fixture
def console_client(monkeypatch, _isolate_clover_home):
...
def test_console_ws_cancel_returns_to_prompt(console_client, monkeypatch):
    from clover_cli.console_engine import ConsoleResult, CloverConsoleEngine

    def slow_execute(self, line: str, *, confirmed: bool = False):
        time.sleep(0.2)
        return ConsoleResult("ok", output="late", command=line)

    monkeypatch.setattr(CloverConsoleEngine, "execute", slow_execute)
