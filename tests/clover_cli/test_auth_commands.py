...
def _write_auth_store(tmp_path, payload: dict) -> None:
    ...
    (clover_home / "auth.json").write_text(json.dumps(payload, indent=2))
...
def test_auth_add_api_key_persists_manual_entry(tmp_path, monkeypatch):
    ...
    from clover_cli.auth_commands import auth_add_command
    ...
    payload = json.loads((tmp_path / "clover" / "auth.json").read_text())
...
def test_auth_add_configured_provider_uses_canonical_pool_key(tmp_path, monkeypatch):
    ...
    payload = json.loads((clover_home / "auth.json").read_text(encoding="utf-8"))
...
def test_auth_add_migrates_display_name_derived_legacy_pool_key(
    ...
    with patch("clover_cli.models.clear_provider_models_cache") as clear_cache:
        auth_add_command(_Args())
...
def test_auth_list_includes_non_registry_configured_provider(
    ...
    from clover_cli.auth_commands import auth_list_command
...
def test_interactive_auth_add_accepts_non_registry_configured_provider(
    ...
    from clover_cli import auth_commands
...
def test_auth_add_clover_oauth_persists_pool_entry(tmp_path, monkeypatch):
    ...
    entries = payload["credential_pool"]["clover"]
    device_code_entries = [
...
def test_auth_add_codex_oauth_keeps_distinct_pool_accounts(tmp_path, monkeypatch):
    """Two ``clover auth add openai-codex`` runs for different ChatGPT
    ...
    second independent one. ``clover auth list`` showed two labels sharing
    ...
    monkeypatch.setattr("clover_cli.auth._codex_device_code_login", lambda: next(logins))
...
def test_codex_auth_status_reports_pool_only_credential(tmp_path, monkeypatch):
    ...
    from clover_cli.auth import get_codex_auth_status
...
def test_codex_runtime_pool_only_rate_limit_is_not_missing_auth(tmp_path, monkeypatch):
    ...
    from clover_cli.auth import AuthError, CODEX_RATE_LIMITED_CODE, resolve_codex_runtime_credentials
...
def test_auth_add_xai_oauth_sets_active_provider(tmp_path, monkeypatch):
    """clover auth add xai-oauth must set active_provider and write a pool entry.
...
def test_auth_add_xai_oauth_keeps_distinct_pool_accounts(tmp_path, monkeypatch):
    """Two ``clover auth add xai-oauth`` runs must produce independent pool entries.
...
def test_auth_remove_codex_migrates_legacy_dict_suppression(tmp_path, monkeypatch):
    ...
    payload = json.loads((tmp_path / "clover" / "auth.json").read_text(encoding="utf-8"))
...
def test_logout_resets_codex_config_when_auth_state_already_cleared(tmp_path, monkeypatch, capsys):
    """`clover logout --provider openai-codex` must still clear model.provider.
    ...
    config_text = (clover_home / "config.yaml").read_text()
...
def test_unsuppress_credential_source_clears_marker(tmp_path, monkeypatch):
    ...
    from clover_cli.auth import suppress_credential_source, unsuppress_credential_source, is_source_suppressed
...
# =============================================================================
# Unified credential-source stickiness — every source Clover reads from has a
...
def test_seed_from_singletons_respects_clover_pkce_suppression(tmp_path, monkeypatch):
    """anthropic clover_pkce must not re-seed from ~/.clover/.anthropic_oauth.json when suppressed."""
    clover_home = tmp_path / "clover"
    clover_home.mkdir(parents=True, exist_ok=True)
    monkeypatch.setenv("CLOVER_HOME", str(clover_home))

    import yaml
    (clover_home / "config.yaml").write_text(yaml.dump({"model": {"provider": "anthropic", "model": "claude"}}))
    (clover_home / "auth.json").write_text(json.dumps({
        "version": 1,
        "providers": {},
        "suppressed_sources": {"anthropic": ["clover_pkce"]},
    }))

    # Stub the readers so only clover_pkce is "available"; claude_code returns None
    import agent.anthropic_adapter as aa
    monkeypatch.setattr(aa, "read_clover_oauth_credentials", lambda: {
    ...
    # clover_pkce suppressed, claude_code returns None → nothing should be seeded
    assert entries == []
    assert "clover_pkce" not in active
...
def test_auth_remove_copilot_suppresses_all_variants(tmp_path, monkeypatch):
    ...
    from types import SimpleNamespace
    from clover_cli.auth import is_source_suppressed
    from clover_cli.auth_commands import auth_remove_command

    with patch(
        "clover_cli.copilot_auth.resolve_copilot_token",
        return_value=("ghp_fake", "gh"),
    ), patch(
        "clover_cli.copilot_auth.get_copilot_api_token",
