...
clover_cli.config.load_config, so the managed overlay has to be wired into each.
...
def test_timezone_honors_managed(homes, monkeypatch):
    home, managed = homes
    # clover_time checks an env override first; ensure it's unset so config wins.
    monkeypatch.delenv("CLOVER_TIMEZONE", raising=False)
    monkeypatch.delenv("TZ", raising=False)
    _seed(home, managed, user="timezone: America/New_York\n", mgd="timezone: Asia/Tokyo\n")
    import clover_time

    assert clover_time._resolve_timezone_name() == "Asia/Tokyo"
...
def test_gateway_env_bridge_honors_managed(homes, monkeypatch):
    ...
    every turn (CLOVER_TIMEZONE, CLOVER_REDACT_SECRETS, CLOVER_MAX_ITERATIONS,
    ...
    # writing CLOVER_TIMEZONE = cfg["timezone"]. Prove the overlay flips the value.
    import yaml
