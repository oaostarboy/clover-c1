...
Every aux picker (``clover model`` → Configure auxiliary models, the
``clover tools`` vision picker, and any future one) must route through
``clover_cli.inventory.build_aux_picker_rows()`` so it shows the same
...
def configured_home(tmp_path, monkeypatch):
    """A CLOVER_HOME with one ``providers:`` entry and one legacy
...
def test_aux_picker_surfaces_user_defined_providers(configured_home):
    ...
    """
    from clover_cli.inventory import build_aux_picker_rows
...
def test_aux_picker_requests_exhausted_pool_visibility(configured_home):
    ...
    with patch("clover_cli.model_switch.list_authenticated_providers", _capture):
...
def test_aux_pickers_route_through_the_shared_substrate(configured_home):
    ...
    """
    import clover_cli.main as main
    ...
    with (
        patch("clover_cli.model_switch.list_authenticated_providers", _direct),
        patch("clover_cli.inventory.build_aux_picker_rows", _substrate),
        patch("clover_cli.main._prompt_provider_choice", return_value=None),
    ):
    ...
    assert direct_calls == [], (
        ...
        "route through clover_cli.inventory.build_aux_picker_rows so custom "
