"""Tests for clover_cli.uninstall.remove_node_symlinks.

Regression for #34536: the POSIX installer drops node/npm/npx symlinks in
~/.local/bin pointing into $CLOVER_HOME/node and prepends ~/.local/bin to
PATH, shadowing an existing nvm. Uninstall must remove those symlinks, but
only when they still resolve into the Clover-managed node dir.
"""
...
def _make_clover_node(clover_home: Path) -> Path:
    """Create a fake $CLOVER_HOME/node/bin/{node,npm,npx} tree."""
    node_bin = clover_home / "node" / "bin"
...
def test_leaves_unrelated_symlinks_untouched(fake_home):
    ...
    clover_home = fake_home / ".clover"
    _make_clover_node(clover_home)
    ...
    removed = uninstall.remove_node_symlinks(clover_home)
...
def test_removes_fhs_symlinks_in_usr_local_bin(fake_home, tmp_path, monkeypatch):
    ...
    """
    clover_home = fake_home / ".clover"
    node_bin = _make_clover_node(clover_home)
