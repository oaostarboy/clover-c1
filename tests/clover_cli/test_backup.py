"""Tests for clover backup and import commands."""
...
def _advance_backup_clock(seconds: float = 1.1) -> None:
    """Skew clover_cli.backup's datetime forward instead of sleeping.
    ...
    import clover_cli.backup as _backup

    shim = getattr(_backup.datetime, "_clover_test_shim", None)
    if shim is None:
        class _ShimDatetime(_dt.datetime):
            _clover_test_shim = True
...
def _make_clover_tree(root: Path) -> None:
    """Create a realistic ~/.clover directory structure for testing."""
    (root / "config.yaml").write_text("model:\n  provider: openrouter\n")
    (root / ".env").write_text("OPENROUTER_API_KEY=sk-test-123\n")
    for db_name in ("memory_store.db", "clover_state.db"):
    ...
    # clover-c1 repo (should be EXCLUDED)
    (root / "clover-c1").mkdir(exist_ok=True)
    (root / "clover-c1" / "run_agent.py").write_text("# big file\n")
    (root / "clover-c1" / ".git").mkdir()
    (root / "clover-c1" / ".git" / "HEAD").write_text("ref: refs/heads/main\n")
...
class TestShouldExclude:
    def test_excludes_clover_agent(self):
        from clover_cli.backup import _should_exclude
        assert _should_exclude(Path("clover-c1/run_agent.py"))
        assert _should_exclude(Path("clover-c1/.git/HEAD"))
    ...
    def test_excludes_state_snapshots_dir(self):
        ...
        from clover_cli.backup import _EXCLUDED_DIRS, _QUICK_SNAPSHOTS_DIR, _should_exclude
...
class TestBackup:
    ...
    def test_db_snapshots_staged_beside_output_zip(self, tmp_path, monkeypatch):
        ...
        import clover_cli.backup as backup_mod
        staged_dirs = []
    ...
    def test_pre_update_db_snapshots_staged_beside_output_zip(self, tmp_path, monkeypatch):
        ...
        out_zip = clover_home / "backups" / "pre-update-test.zip"
        ...
        monkeypatch.setattr(backup_mod.tempfile, "NamedTemporaryFile", _spy)
        result = backup_mod._write_full_zip_backup(out_zip, clover_home)
    ...
    def test_skips_symlinked_files(self, tmp_path, monkeypatch):
        """Backup must not dereference symlinks and leak files outside CLOVER_HOME."""
        clover_home = tmp_path / ".clover"
        clover_home.mkdir()
        _make_clover_tree(clover_home)
        outside = tmp_path / "outside-secret.txt"
        outside.write_text("outside secret\n")
        _symlink_file_or_skip(clover_home / "skills" / "outside-link.txt", outside)
    ...
    def test_state_snapshots_not_nested_into_backup(self, tmp_path, monkeypatch):
        ...
        clover_home.mkdir()
        _make_clover_tree(clover_home)
        with sqlite3.connect(clover_home / "state.db") as conn:
            conn.execute("CREATE TABLE sessions (id TEXT PRIMARY KEY)")
            conn.execute("INSERT INTO sessions VALUES ('s1')")

        from clover_cli.backup import _QUICK_SNAPSHOTS_DIR, create_quick_snapshot, run_backup

        # Real producer, so the layout under state-snapshots/ is whatever the
        # code actually writes (manifest.json + state.db copy + ...).
        snap_id = create_quick_snapshot(clover_home=clover_home)
        assert snap_id and (clover_home / _QUICK_SNAPSHOTS_DIR / snap_id / "state.db").exists()
...
class TestValidateBackupZip:
    ...
    def test_state_db_passes(self, tmp_path):
        """A zip containing state.db is accepted as a valid Clover backup."""
        from clover_cli.backup import _validate_backup_zip
...
class TestImport:
    ...
    def test_import_survives_service_layer_import_failure(self, tmp_path, monkeypatch, capsys):
        ...
        assert "Done. Your Clover configuration has been restored." in out
        assert "clover gateway install" in out
    ...
    def test_preserves_per_profile_gateway_state(self, tmp_path, monkeypatch):
        ...
        clover_home = tmp_path / ".clover"
        (clover_home / "profiles" / "coder").mkdir(parents=True)
        monkeypatch.setenv("CLOVER_HOME", str(clover_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)

        live_state = '{"gateway_state": "running"}'
        (clover_home / "profiles" / "coder" / "gateway_state.json").write_text(live_state)
        ...
        assert (clover_home / "profiles" / "coder" / "config.yaml").read_text() == "model: anthropic\n"
        assert (
            clover_home / "profiles" / "coder" / "gateway_state.json"
    ...
    def test_preserves_runtime_pid_and_process_files(self, tmp_path, monkeypatch):
        ...
        (clover_home / "gateway.pid").write_text("4242")
        (clover_home / "processes.json").write_text('{"live": true}')
        ...
        assert (clover_home / "gateway.pid").read_text() == "4242"
        assert (clover_home / "processes.json").read_text() == '{"live": true}'
        # cron.pid / gateway.lock had no live copy and were not seeded.
        assert not (clover_home / "cron.pid").exists()
        assert not (clover_home / "gateway.lock").exists()
    ...
    def test_restores_secret_files_with_0600_perms(self, tmp_path, monkeypatch):
        ...
        self._make_backup_zip(zip_path, {
            ...
            "auth.json": '{"providers": {"clover": "token"}}',
        ...
        for rel in (".env", "auth.json", "state.db", "profiles/coder/.env"):
            mode = (clover_home / rel).stat().st_mode & 0o777
...
class TestRoundTrip:
    def test_backup_then_import(self, tmp_path, monkeypatch):
        ...
        from clover_cli.backup import run_backup, run_import

        run_backup(Namespace(output=str(out_zip)))
        assert out_zip.exists()

        # Import into a different location
        dst_home = tmp_path / "dest" / ".clover"
        dst_home.mkdir(parents=True)
        monkeypatch.setenv("CLOVER_HOME", str(dst_home))
        ...
        # clover-c1 should NOT be present
        assert not (dst_home / "clover-c1").exists()
...
class TestValidation:
    ...
    def test_detect_prefix_only_dirs(self):
        ...
        import io
        from clover_cli.backup import _detect_prefix

        buf = io.BytesIO()
        with zipfile.ZipFile(buf, "w") as zf:
            # Only directory entries (trailing slash)
            zf.writestr(".clover/", "")
            zf.writestr(".clover/skills/", "")
        buf.seek(0)
        with zipfile.ZipFile(buf, "r") as zf:
...
class TestBackupEdgeCases:
    ...
    def test_empty_clover_home(self, tmp_path, monkeypatch):
        """Backup handles empty clover home (no files to back up)."""
        clover_home = tmp_path / ".clover"
        clover_home.mkdir()
        # Only excluded dirs, no actual files
        (clover_home / "__pycache__").mkdir()
        (clover_home / "__pycache__" / "foo.pyc").write_bytes(b"\x00")
    ...
    def test_pre1980_timestamp_skipped(self, tmp_path, monkeypatch):
        ...
        clover_home = tmp_path / ".clover"
        clover_home.mkdir()
        (clover_home / "config.yaml").write_text("model: test\n")

        # Create a file with epoch timestamp (1970-01-01)
        old_file = clover_home / "ancient.txt"
...
class TestImportEdgeCases:
    ...
    def test_eof_during_confirmation(self, tmp_path, monkeypatch):
        ...
        clover_home = tmp_path / ".clover"
        clover_home.mkdir()
        (clover_home / "config.yaml").write_text("existing\n")
        monkeypatch.setenv("CLOVER_HOME", str(clover_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
    ...
    def test_progress_with_many_files(self, tmp_path, monkeypatch):
        ...
        assert (clover_home / "config.yaml").exists()
        assert (clover_home / "sessions" / "s0599.json").exists()
...
class TestImportAtomicWrites:
    """`clover import` must never leave a user's file truncated.
    ...
    tests pin the invariant for both restore branches: the CLOVER_HOME branch
    ...
    def test_failed_member_leaves_existing_file_intact(self, tmp_path, monkeypatch):
        ...
        (clover_home / "config.yaml").write_text(original)
        monkeypatch.setenv("CLOVER_HOME", str(clover_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        ...
        assert (clover_home / "config.yaml").read_text() == original
        # And the aborted write must not litter the directory it staged in.
        assert list(clover_home.glob(".config.yaml.*")) == []

    def test_failed_external_member_leaves_existing_file_intact(self, tmp_path, monkeypatch):
        """Same invariant on the `_external/` branch, which writes outside CLOVER_HOME."""
        dst_home = tmp_path / "dst"
        dst_home.mkdir()
        clover_home = dst_home / ".clover"
        clover_home.mkdir()
    ...
    def test_restore_preserves_existing_file_mode(self, tmp_path, monkeypatch):
        ...
        clover_home = tmp_path / ".clover"
        clover_home.mkdir()
        target = clover_home / "config.yaml"
    ...
    def test_restore_preserves_existing_file_owner(self, tmp_path, monkeypatch):
        ...
        ``sudo clover import`` onto a user-owned (or Docker/NAS volume-owned)
        CLOVER_HOME would hand every restored file to root. The uid/gid is
        ...
        monkeypatch.setattr(
            "clover_cli.backup._preserve_file_owner",
    ...
    def test_restore_does_not_carry_setuid_onto_archive_content(
        ...
        re-serialize content this process produced; ``clover import`` is the
...
class TestSafeCopyDb:
    def test_copies_valid_database(self, tmp_path):
        from clover_cli.backup import _safe_copy_db
    ...
    def test_aborts_when_source_remains_busy_past_deadline(
        ...
        from clover_cli import backup as backup_mod
    ...
    def test_is_zeroed_sqlite_file_detects_nul_header(self, tmp_path):
        from clover_cli.backup import is_zeroed_sqlite_file
        p = tmp_path / "state.db"
...
class TestQuickSnapshot:
    @pytest.fixture
    def clover_home(self, tmp_path):
        """Create a fake CLOVER_HOME with critical state files."""
    ...
    def test_state_db_safely_copied(self, clover_home):
        from clover_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(clover_home=clover_home)
        db_copy = clover_home / "state-snapshots" / snap_id / "state.db"
    ...
    def test_failed_state_db_copy_is_loud(self, clover_home, monkeypatch, capsys):
    ...
    def test_restore_state_db_live_connection(self, clover_home):
        ...
        """
        from clover_cli.backup import create_quick_snapshot, restore_quick_snapshot
        snap_id = create_quick_snapshot(clover_home=clover_home)

        # Open a live connection (simulating gateway/dashboard).
        live_conn = sqlite3.connect(str(clover_home / "state.db"))
        ...
        result = restore_quick_snapshot(snap_id, clover_home=clover_home)
        assert result is True
    ...
    def test_snapshot_includes_pairing_directories(self, clover_home):
        ...
        (clover_home / "platforms" / "pairing").mkdir(parents=True)
        (clover_home / "platforms" / "pairing" / "telegram-approved.json").write_text(
            '{"12345": {"user_name": "alice"}}'
        )
        (clover_home / "platforms" / "pairing" / "discord-approved.json").write_text(
            '{"67890": {"user_name": "bob"}}'
        )
        # Legacy pairing store (old location)
        (clover_home / "pairing").mkdir()
        (clover_home / "pairing" / "matrix-approved.json").write_text(
            '{"@charlie:server": {"user_name": "charlie"}}'
        )
        # Feishu's separate JSON
        (clover_home / "feishu_comment_pairing.json").write_text(
        ...
        snap_dir = clover_home / "state-snapshots" / snap_id
...
# ---------------------------------------------------------------------------
# Pre-update backup (clover update safety net)
# ---------------------------------------------------------------------------
    ...
    def test_oversized_db_suppresses_pruning(self, clover_home, capsys):
        ...
        """
        import json
        from clover_cli.backup import create_quick_snapshot, list_quick_snapshots

        # First snapshot: complete (state.db is small, under any cap)
        first_id = create_quick_snapshot(label="complete", clover_home=clover_home)
        assert first_id is not None
        first_dir = clover_home / "state-snapshots" / first_id
        ...
        second_id = create_quick_snapshot(
            label="oversized", clover_home=clover_home, max_file_size=1024, keep=1
        )
        assert second_id is not None
        second_dir = clover_home / "state-snapshots" / second_id
        ...
        all_snaps = list_quick_snapshots(limit=100, clover_home=clover_home)
...
class TestQuickSnapshotProjectsKanban:
    ...
    def test_non_default_kanban_board_snapshotted(self, clover_home):
        ...
        board_dir = clover_home / "kanban" / "boards" / "work"
        ...
        snap_id = create_quick_snapshot(clover_home=clover_home)
        copy = (
            clover_home / "state-snapshots" / snap_id
        ...
        assert restore_quick_snapshot(snap_id, clover_home=clover_home) is True
        conn = sqlite3.connect(str(board_dir / "kanban.db"))
    ...
    def test_board_db_copied_wal_safely(self, clover_home, monkeypatch):
        ...
        import clover_cli.backup as bk
        from clover_cli.backup import create_quick_snapshot

        board = clover_home / "kanban" / "boards" / "work"
        ...
        copy = clover_home / "state-snapshots" / snap_id / "kanban" / "boards" / "work" / "kanban.db"
        rows = sqlite3.connect(str(copy)).execute("SELECT * FROM tasks").fetchall()
        assert rows == [("w1", "ship")]


class TestPreUpdateBackup:
    """Tests for create_pre_update_backup — the auto-backup ``clover update``
    runs before touching anything."""


    @pytest.fixture
    def clover_home(self, tmp_path):
        root = tmp_path / ".clover"
        root.mkdir()
        _make_clover_tree(root)
        return root


    def test_backup_contents_match_full_backup(self, clover_home):
        """Pre-update backup should include the same user data that
        ``clover backup`` would, and should exclude the same directories."""
        from clover_cli.backup import create_pre_update_backup
        out = create_pre_update_backup(clover_home=clover_home)
        ...
        # clover-c1 repo excluded
        assert not any(n.startswith("clover-c1/") for n in names)
        # __pycache__ excluded
        assert not any("__pycache__" in n for n in names)
        # pid files excluded
        assert "gateway.pid" not in names

    def test_pre_update_zip_does_not_nest_the_pre_update_snapshot(self, clover_home):
        """``clover update`` in ``full`` mode takes the quick snapshot *before*
        ...
        snap_id = create_quick_snapshot(label="pre-update", clover_home=clover_home)
        assert snap_id and (clover_home / _QUICK_SNAPSHOTS_DIR / snap_id / "state.db").exists()
    ...
    def test_rotation_keeps_only_n(self, clover_home):
        ...
        created = []
        for _ in range(5):
            out = create_pre_update_backup(clover_home=clover_home, keep=3)
            created.append(out)
            _advance_backup_clock()

        remaining = sorted(
            p.name for p in (clover_home / "backups").iterdir()
    ...
    def test_skips_symlinked_files(self, clover_home, tmp_path):
        """Pre-update backups must not dereference symlinks outside CLOVER_HOME."""
        from clover_cli.backup import create_pre_update_backup
...
class TestRunPreUpdateBackup:
    ...
    def clover_home(self, tmp_path, monkeypatch):
        ...
        root.mkdir()
        _make_clover_tree(root)
        # Point CLOVER_HOME at the temp dir so config + backup paths resolve here
        monkeypatch.setenv("CLOVER_HOME", str(root))
        # Make Path.home() point at tmp_path for anything that uses it
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        # Config reads resolve CLOVER_HOME dynamically and their caches are
    ...
    @staticmethod
    def _set_mode(clover_home, value):
        import yaml
        (clover_home / "config.yaml").write_text(yaml.safe_dump({
            "_config_version": 22,
            "updates": {"pre_update_backup": value},
        }))

    @staticmethod
    def _zips(clover_home):
        d = clover_home / "backups"
        return list(d.glob("pre-update-*.zip")) if d.exists() else []

    @staticmethod
    def _snaps(clover_home):
        d = clover_home / "state-snapshots"
        return [p for p in d.iterdir() if p.is_dir()] if d.exists() else []




    def test_config_off_disables_everything_silently(self, clover_home, capsys):
        ...
        self._set_mode(clover_home, "off")
        from clover_cli.main import _run_pre_update_backup
        snap_id = _run_pre_update_backup(Namespace(no_backup=False, backup=False))
        out = capsys.readouterr().out
        assert snap_id is None
        assert out == ""
        assert not self._snaps(clover_home)
        assert not self._zips(clover_home)



    def test_config_full_mode(self, clover_home, capsys):
        self._set_mode(clover_home, "full")
        from clover_cli.main import _run_pre_update_backup
        snap_id = _run_pre_update_backup(Namespace(no_backup=False, backup=False))
        out = capsys.readouterr().out
        assert snap_id is not None
        ...
        assert len(self._zips(clover_home)) == 1




# ---------------------------------------------------------------------------
# Pre-migration backup (clover claw migrate safety net)
# ---------------------------------------------------------------------------

class TestPreMigrationBackup:
    """Tests for create_pre_migration_backup — the auto-backup
    ``clover claw migrate`` runs before mutating ~/.clover/."""
    ...
    def test_restorable_with_clover_import(self, clover_home, tmp_path):
        """The zip produced by pre-migration backup must be a valid Clover
        backup — `clover import` should accept it."""
        from clover_cli.backup import create_pre_migration_backup, _validate_backup_zip
        out = create_pre_migration_backup(clover_home=clover_home)
        assert out is not None
        with zipfile.ZipFile(out) as zf:
    ...
    def test_does_not_touch_pre_update_backups(self, clover_home):
        """Pre-migration rotation must only prune pre-migration-*.zip files,
        leaving pre-update-*.zip backups untouched."""
        from clover_cli.backup import create_pre_update_backup, create_pre_migration_backup
        update_backup = create_pre_update_backup(clover_home=clover_home, keep=5)
        assert update_backup is not None and update_backup.exists()
        # Spin up a lot of migration backups with keep=1
        for _ in range(3):
            out = create_pre_migration_backup(clover_home=clover_home, keep=1)
            assert out is not None
            _advance_backup_clock()
...
class TestRestoreCronJobsIfEmptied:
    """`clover update` config migration can leave cron/jobs.json valid-but-empty,
    ...
    def _make_snapshot(self, clover_home: Path, label="pre-update"):
        from clover_cli.backup import create_quick_snapshot
        return create_quick_snapshot(label=label, clover_home=clover_home, keep=5)

    def test_restores_when_emptied_after_migration(self, tmp_path):
        from clover_cli.backup import restore_cron_jobs_if_emptied
        clover_home = tmp_path / ".clover"
        jobs_path = clover_home / "cron" / "jobs.json"
        # Pre-update: 3 real jobs.
        self._seed_jobs(jobs_path, [{"id": "a"}, {"id": "b"}, {"id": "c"}])
        snap_id = self._make_snapshot(clover_home)
        assert snap_id

        # Migration silently empties the file (valid JSON, zero jobs).
        jobs_path.write_text(json.dumps({"jobs": []}))

        result = restore_cron_jobs_if_emptied(snap_id, clover_home=clover_home)
        assert result is not None
...
# location, NOT under CLOVER_HOME. (backup/import cycle data-loss fix)
# ---------------------------------------------------------------------------

class TestMemoryProviderExternalPaths:
    def _make_min_tree(self, clover_home: Path) -> None:
        clover_home.mkdir(parents=True, exist_ok=True)
        (clover_home / "config.yaml").write_text("model:\n  provider: openrouter\n")
        (clover_home / ".env").write_text("OPENROUTER_API_KEY=sk-test\n")
        (clover_home / "state.db").write_bytes(b"x")
    ...
    def test_backup_skips_external_paths_outside_home(self, tmp_path, monkeypatch):
        ...
        clover_home = tmp_path / ".clover"
        self._make_min_tree(clover_home)
    ...
    def test_import_restores_external_to_home_relative_location(self, tmp_path, monkeypatch):
        """_external/ members restore to ~/<relpath>, not under CLOVER_HOME,
        ...
        # External state did NOT leak into CLOVER_HOME.
        assert not (clover_home / "_external").exists()
