...
from clover_cli import nous_subscription as ns
...
def test_get_gateway_eligible_tools_treats_explicit_backend_as_configured(monkeypatch):
    ...
    single Enter during `clover model` overwrote it to `web.backend: nous`.
    """
...
def test_apply_nous_managed_defaults_writes_video_gen_config(monkeypatch):
    """apply_nous_managed_defaults must store the managed 'clover' selection
...
# ---------------------------------------------------------------------------
# ensure_nous_portal_access — inline login gate for `clover tools`
# ---------------------------------------------------------------------------
