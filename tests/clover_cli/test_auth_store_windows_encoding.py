...
``_load_auth_store`` and the Codex/Clover shared-store readers previously called
...
def clover_home(tmp_path, monkeypatch):
    """Point CLOVER_HOME at a tmp dir so we never touch the real auth store.

    Required because ``_auth_file_path()`` has a seat belt that refuses to
    resolve to the real user's ~/.clover/auth.json under pytest.
    """
...
class TestAuthStoreEncodingRoundTrip:
    def test_load_reads_utf8_with_non_ascii_label(self, clover_home, windows_default_encoding):
    ...
    def test_load_does_not_corrupt_store_on_non_ascii(self, clover_home, windows_default_encoding):
        ...
        assert not (clover_home / "auth.json.corrupt").exists()
    ...
    def test_load_handles_utf8_with_bom(self, clover_home):
...
class TestExplicitEncodingPassed:
    ...
    def test_load_auth_store_passes_utf8_encoding(self, clover_home):
        auth_path = clover_home / "auth.json"
...
# --- sibling readers of the same ~/.clover/auth.json in other modules -------
#
# _load_auth_store lives in clover_cli/auth.py, but several other modules read
# the same ~/.clover/auth.json directly. They had the same UTF-8-vs-cp1252
# asymmetry on Windows — these tests pin the sibling reads too.

class TestAuthJsonSiblingReaders:
    def test_has_xai_credentials_reads_non_ascii_store(self, clover_home, windows_default_encoding, monkeypatch):
    ...
    def test_has_any_provider_configured_reads_non_ascii_auth_store(
        ...
        """clover_cli.main._has_any_provider_configured reads auth.json.
        ...
        # clover_cli.auth; patch it on the source module so the local import
        # sees the fake.
