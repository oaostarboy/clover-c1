"""Tests for kanban board export / import (``clover_cli.kanban_transfer``).
...
from clover_cli import kanban_transfer as kt
from clover_cli.archive_safe import normalize_archive_parts, safe_extract_targz
...
def kanban_root(tmp_path, monkeypatch):
    ...
    def _use(name: str) -> Path:
        ...
        monkeypatch.setenv("CLOVER_KANBAN_HOME", str(root))
        for var in ("CLOVER_KANBAN_DB", "CLOVER_KANBAN_WORKSPACES_ROOT",
                    "CLOVER_KANBAN_ATTACHMENTS_ROOT", "CLOVER_KANBAN_BOARD"):
...
def test_import_rejects_a_future_format_version(kanban_root, tmp_path):
    ...
    kanban_root("target")
    with pytest.raises(ValueError, match="newer than this Clover"):
