"""Regression tests for the Clover Portal env-override bypassing the host
allowlist, mirroring the existing CLOVER_INFERENCE_BASE_URL /
_ALLOWED_CLOVER_INFERENCE_HOSTS treatment.

Real incident (2026-07): a hosted agent provisioned by clover-account-service
on the `staging` Vercel environment is stamped with
``CLOVER_PORTAL_BASE_URL=https://portal.staging-`` in its
...
Before this fix, ``resolve_clover_access_token`` / ``resolve_clover_runtime_
...
``_CLOVER_PORTAL_ALLOWED_HOSTS``, which only recognised the production host.
...
``_quarantine_clover_oauth_state`` and wiped the entire credential pool.

The correct fix (mirroring ``_clover_inference_env_override()``): the env
...
from clover_cli.auth import (
    ...
    _CLOVER_PORTAL_ALLOWED_HOSTS,
    _clover_portal_env_override,
)
...
class TestPortalEnvOverrideHelper:
    def test_none_when_unset(self, monkeypatch):
        ...
        monkeypatch.delenv("CLOVER_PORTAL_BASE_URL", raising=False)
        assert _clover_portal_env_override() is None


    def test_env_override_not_gated_by_allowlist(self, monkeypatch):
        """The whole point: an env-set staging host is NOT in
        _CLOVER_PORTAL_ALLOWED_HOSTS, and the helper must return it anyway —
        gating happens only for network-provenance values."""
        monkeypatch.setenv(
            "CLOVER_PORTAL_BASE_URL", "https://portal.staging-"
        )
        assert "portal.staging-" not in _CLOVER_PORTAL_ALLOWED_HOSTS
        assert (
            _clover_portal_env_override() == "https://portal.staging-"
        )


class TestResolveAccessTokenEnvOverrideWins:
    """End-to-end: resolve_clover_access_token must use the env override for
    ...
    def _run_and_capture(self, monkeypatch, auth):
        ...
        caplog_records = []
        logger = logging.getLogger("clover_cli.auth")
    ...
    def test_env_override_wins_even_with_staging_state_stored(
        ...
        a prior CLOVER_AUTH_JSON_BOOTSTRAP seed), and the env var is set to
        ...
        staging_portal = "https://portal.staging-"
    ...
    def test_no_env_no_staging_state_prod_url_used_unmodified(
        ...
        monkeypatch.delenv("CLOVER_PORTAL_BASE_URL", raising=False)
        monkeypatch.delenv("CLOVER_PORTAL_BASE_URL", raising=False)
        self._write_auth_file(tmp_path, stored_portal_url=DEFAULT_CLOVER_PORTAL_URL)

        seen_portal_urls, records = self._run_and_capture(monkeypatch, auth)

        assert seen_portal_urls == [DEFAULT_CLOVER_PORTAL_URL]
