...
the Clover package prefixes so later imports rebuild a self-consistent
...
def _restore_sys_modules():
    ...
    The purge under test evicts real Clover modules from the cache; later
...
def test_purge_evicts_clover_prefixed_modules():
    victims = [
        "clover_cli.cli_output",
        "clover_cli.gateway",
        "gateway.status",
        "tools.ansi_strip",
        "tui_gateway.server",
    ...
    for name in victims:
        if name not in sys.modules:
            ...
            added.append(name)
    try:
        cli_main._purge_stale_clover_modules()
        for name in victims:
...
def test_purge_protects_executing_modules():
    ...
    cli_main._purge_stale_clover_modules()
    assert sys.modules.get("clover_cli.update_cmd") is update_cmd
    assert sys.modules.get("clover_cli.main") is cli_main
    assert "clover_cli" in sys.modules
...
def test_purge_never_raises_on_weird_sys_modules():
    ...
    sys.modules["clover_cli._purge_test_none"] = None  # type: ignore[assignment]
    try:
        cli_main._purge_stale_clover_modules()
    finally:
        sys.modules.pop("clover_cli._purge_test_none", None)
...
def test_stale_symbol_scenario_end_to_end():
    ...
    try:
        ...
        try:
            from clover_cli.cli_output import line_input  # noqa: F401
            raised = False
        except ImportError:
            raised = True
