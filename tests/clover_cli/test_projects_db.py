"""Tests for the per-profile Projects store (clover_cli/projects_db)."""
...
def test_create_get_list(conn):
    pid = pdb.create_project(conn, name="Clover Cognition", folders=["/tmp/clover"])
    ...
    assert [f.path for f in proj.folders] == ["/tmp/clover"]
...
def test_per_profile_isolation(tmp_path):
    # Two distinct DB paths stand in for two profiles' CLOVER_HOME.
