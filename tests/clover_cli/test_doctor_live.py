"""Tests for ``clover doctor --live`` — opt-in bounded real-call tool-backend probes.
...
from clover_cli import doctor_live
from clover_cli.doctor_live import (
    ProbeResult,
...
class TestBrowserAvailableNpxRung:
    ...
    with. It must fall through to the same cascade `clover doctor` uses."""

    def _block_path_and_node_modules_checks(self, monkeypatch, tmp_path):
        monkeypatch.setattr("shutil.which", lambda *a, **k: None)
        monkeypatch.setattr("clover_cli.doctor.CLOVER_HOME", tmp_path / "home")
        monkeypatch.setattr("clover_cli.doctor.PROJECT_ROOT", tmp_path / "root")
