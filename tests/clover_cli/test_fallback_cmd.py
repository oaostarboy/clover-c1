"""Tests for `clover fallback` — chain reading, add/remove/clear, legacy migration."""
...
# ---------------------------------------------------------------------------
# Shared fixture — isolate CLOVER_HOME so save_config writes to tmp_path
# ---------------------------------------------------------------------------
...
def _write_config(home: Path, data: dict) -> None:
    config_path = home / ".clover" / "config.yaml"
...
class TestReadChain:
    def test_returns_empty_list_when_unset(self):
        from clover_cli.fallback_cmd import _read_chain
        assert _read_chain({}) == []
    ...
    def test_reads_new_list_format(self):
        ...
        cfg = {
            "fallback_providers": [
                ...
                {"provider": "clover", "model": "Clover-4-Llama-3.1-405B"},
            ]
        }
    ...
    def test_returns_copies_not_aliases(self):
        from clover_cli.fallback_cmd import _read_chain
        cfg = {"fallback_providers": [{"provider": "clover", "model": "foo"}]}
        result = _read_chain(cfg)
        result[0]["provider"] = "mutated"
        assert cfg["fallback_providers"][0]["provider"] == "clover"
...
class TestExtractFallback:
    def test_extracts_from_default_field(self):
        from clover_cli.fallback_cmd import _extract_fallback_from_model_cfg
...
class TestListCommand:
    ...
    def test_list_with_entries(self, isolated_home, capsys):
        _write_config(isolated_home, {
            ...
            "fallback_providers": [
                {"provider": "openrouter", "model": "anthropic/claude-sonnet-4.6"},
                {"provider": "clover", "model": "Clover-4"},
            ],
        })
        from clover_cli.fallback_cmd import cmd_fallback_list
        cmd_fallback_list(types.SimpleNamespace())
...
class TestAddCommand:
    def test_add_appends_new_entry(self, isolated_home, capsys):
        ...
        with patch("clover_cli.main.select_provider_and_model", side_effect=fake_picker), \
                patch("clover_cli.main._require_tty"):
            from clover_cli.fallback_cmd import cmd_fallback_add
...
class TestClearCommand:
    ...
    def test_clear_with_confirmation(self, isolated_home, capsys, monkeypatch):
        ...
        from clover_cli.fallback_cmd import cmd_fallback_clear
...
class TestArgparseWiring:
    """Verify `clover fallback` is wired into main.py's argparse tree.
    ...
    def test_fallback_help_lists_subcommands(self):
        ...
        result = subprocess.run(
            [sys.executable, "-m", "clover_cli.main", "fallback", "--help"],
            capture_output=True,
            text=True,
            timeout=30,
        )
