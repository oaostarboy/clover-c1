"""Regression tests for TOCTOU-safe credential file writers in ``clover_cli.auth``.
...
# ---------------------------------------------------------------------------
# _save_auth_store  (~/.clover/auth.json — every native OAuth provider)
# ---------------------------------------------------------------------------
...
def test_shared_nous_store_writes_0o600_with_0o700_parent(tmp_path, monkeypatch):
    ...
    # get_default_clover_root() — can't collide with our override and
    # falsely claim we're writing to the real user's shared store.
    monkeypatch.setenv("CLOVER_SHARED_AUTH_DIR", str(tmp_path / "shared_override"))
    old_umask = os.umask(0o022)
    try:
        from clover_cli import auth as auth_mod
    ...
    assert path.exists(), "shared Clover store was not written"
    mode = stat.S_IMODE(path.stat().st_mode)
    parent_mode = stat.S_IMODE(path.parent.stat().st_mode)

    assert mode == 0o600, (
        f"Clover shared store mode 0o{mode:o} != 0o600 — TOCTOU race regressed"
    )
    assert parent_mode == 0o700, (
        f"Clover shared store parent dir mode 0o{parent_mode:o} != 0o700"
    )
