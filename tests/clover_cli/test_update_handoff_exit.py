...
``_reexec_dependency_sync_off_windows_shim`` with ``CLOVER_UPDATE_REEXEC=1``)
...
def _run_cmd_update(monkeypatch, impl, *, reexec: bool):
    ...
    monkeypatch.setattr("clover_cli.config.detect_install_method", lambda root: "git")
    monkeypatch.setattr("clover_cli.update_lock.UpdateLock", lambda: _FakeLock(events["order"]))
    ...
    monkeypatch.setattr("clover_cli.update_receipt.finalize_pending_update_receipt", fake_receipt)
