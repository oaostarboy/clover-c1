...
from clover_cli import gateway as clover_gateway
from clover_cli import gateway_windows
from clover_cli import main as cli_main
from clover_cli import process_identity
...
def test_control_plane_argv_is_not_a_gateway():
    ...
    serve = "C:\\Clover\\.venv\\Scripts\\python.exe -m clover_cli.main serve --host 127.0.0.1"
    run = "C:\\Clover\\.venv\\Scripts\\python.exe -m clover_cli.main gateway run"
...
def test_control_plane_classifier_is_token_based_not_substring():
    ...
    py = "C:\\Clover\\.venv\\Scripts\\python.exe -m clover_cli.main"
...
def test_pause_skips_cold_start_plan_when_desktop_owns_lifecycle(monkeypatch):
    ...
    monkeypatch.setattr(clover_gateway, "find_gateway_pids", lambda **_k: [])
    monkeypatch.setattr(
        clover_gateway, "find_windows_gateway_services", lambda **_k: []
    )
