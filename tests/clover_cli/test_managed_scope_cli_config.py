...
clover_cli.config._load_config_impl, so the managed-scope merge has to be
applied in BOTH places or the interactive CLI/TUI surface (skin, display prefs)
silently ignores administrator-pinned values while `clover config`/`doctor`
...
def _load_cli_config(home):
    ...
    cli.py binds ``_clover_home = get_clover_home()`` at import time (module
    singleton), so monkeypatching CLOVER_HOME after import doesn't move it.
    ...
    this only matters for tests that swap CLOVER_HOME.)
    """
    import cli

    cli._clover_home = home
