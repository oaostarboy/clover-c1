"""``clover`` must survive git operations on the checkout (launcher layout).

The Windows ``clover`` command is a launcher derived from the venv console
script. Its canonical home is the managed binary dir ``CLOVER_HOME\\bin`` —
OUTSIDE the git checkout — because the earlier in-checkout home
(``clover-c1\\bin``) was swept by ``clover update``'s autostash
(``git stash push --include-untracked``) and, with the desktop updater's
``--keep-stash``, never restored: ``clover`` stopped resolving in every new
...
existing install's PATH to the canonical layout from the ``clover update``
tail. Platform verdict, PATH values, and registry I/O are injected
parameters (same pattern as ``clover_constants.venv_bin_dir``), so these
...
from clover_cli._install_repair import (
...
def _make_managed(tmp_path, monkeypatch, *, relocatable: bool = False):
    """Fake managed layout: CLOVER_HOME/clover-c1/venv/Scripts + launchers."""
...
def test_source_checkout_untouched(tmp_path, monkeypatch):
    """A checkout NOT under CLOVER_HOME gains nothing anywhere."""
    home = tmp_path / "clover-home"
    home.mkdir()
    monkeypatch.setenv("CLOVER_HOME", str(home))
    root = tmp_path / "src" / "clover-c1"
    scripts = root / "venv" / "Scripts"
    scripts.mkdir(parents=True)
    for name in _WINDOWS_BIN_LAUNCHERS:
...
def test_profile_session_still_heals_the_shared_bin(tmp_path, monkeypatch):
    """Under ``clover -p <name>`` CLOVER_HOME points inside profiles/<name>;
    ...
    monkeypatch.setenv("CLOVER_HOME", str(home / "profiles" / "work"))
...
# ---------------------------------------------------------------------------
# migrate_windows_bin_path — the `clover update` tail migration
# ---------------------------------------------------------------------------
...
def test_migration_moves_path_to_home_bin_and_strips_legacy(managed_install):
    ...
    (root / "bin").mkdir()
    (root / "bin" / "clover.exe").write_bytes(b"legacy copy")
    ...
    assert (root / "bin" / "clover.exe").read_bytes() == b"legacy copy"
...
def test_normalize_windows_path_equivalences():
    assert (
        _normalize_windows_path(r"C:\Users\Me\AppData\Local\clover\bin")
        == _normalize_windows_path("c:/users/me/appdata/local/CLOVER/BIN/")
    )
...
def test_repo_gitignores_the_legacy_bin_dir():
    ...
    launchers at ``<checkout>/bin``. ``clover update`` autostashes with
    ...
    result = subprocess.run(
        ["git", "-C", str(repo_root), "check-ignore", "-q", "bin/clover.exe"],
        capture_output=True,
    )
    assert result.returncode == 0, (
        "bin/clover.exe is not gitignored — clover update's autostash "
