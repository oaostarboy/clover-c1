...
A ``clover update`` killed after git pull advanced HEAD but before the
...
- next ``clover update`` with git already up to date still runs the
...
def _patch_update_deps(monkeypatch, tmp_path, run_side_effect):
    ...
    monkeypatch.setattr(clover_main.subprocess, "run", run_side_effect)
    monkeypatch.setattr(clover_main, "PROJECT_ROOT", tmp_path)
    (tmp_path / ".git").mkdir()
    monkeypatch.setattr(clover_main, "_resolve_update_branch", lambda args: "main")
    ...
    monkeypatch.setattr(
        clover_main, "_stash_local_changes_if_needed", lambda *a, **k: None
    )
    monkeypatch.setattr(clover_main, "_clear_bytecode_cache", lambda *a, **k: 0)
    monkeypatch.setattr(
        clover_main, "_record_bytecode_fingerprint", lambda *a, **k: None
    )
    monkeypatch.setattr(clover_main, "_run_pre_update_backup", lambda *a, **k: None)
    monkeypatch.setattr(
        clover_main, "_pause_windows_gateways_for_update", lambda: None
    )
    monkeypatch.setattr(
        clover_main, "_resume_windows_gateways_after_update", lambda *a, **k: None
    )
    monkeypatch.setattr(clover_main, "_write_update_incomplete_marker", lambda: None)
    monkeypatch.setattr(clover_main, "_clear_update_incomplete_marker", lambda: None)
    monkeypatch.setattr(
        clover_main, "_finish_dashboard_update_cleanup", lambda *a, **k: None
    )
    monkeypatch.setattr(clover_main, "_build_web_ui", lambda *a, **k: None)
    monkeypatch.setattr(
    ...
    monkeypatch.setattr(
        clover_gateway, "find_gateway_pids", lambda all_profiles=False: []
    )
    monkeypatch.setattr(clover_gateway, "supports_systemd_services", lambda: False)
    monkeypatch.setattr(
        clover_gateway, "find_profile_gateway_processes", lambda *a, **k: []
    )
    monkeypatch.setattr(
    ...
    monkeypatch.setattr(
        "clover_cli.update_inventory.collect_runtime_inventory",
...
def test_marker_round_trip_under_clover_home():
    path = update_cmd._fleet_restart_pending_marker_path()
    assert path.parent == get_clover_home()
...
def test_pending_needed_when_unfinished_receipt_runtime_sha_skews(monkeypatch):
    ...
    receipt_dir = get_clover_home() / "logs" / "update_receipts"
...
def test_run_pending_restart_true_when_no_gateways(monkeypatch, capsys):
    monkeypatch.setattr(
        "clover_cli.gateway.find_gateway_pids", lambda **k: []
    )
...
def test_interrupt_between_pull_and_restart_leaves_marker(
    ...
    monkeypatch.setattr(clover_main, "_clear_bytecode_cache", _interrupt)
...
def test_startup_warn_prints_when_marker_present(capsys):
    ...
    assert "clover gateway restart" in err
