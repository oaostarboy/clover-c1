...
columns + Free/Pro badges and gate paid models on free Clover accounts, the
same way the `clover model` CLI picker does.
"""
...
def test_apply_pricing_free_models_get_flat_100_percent_sale(monkeypatch):
    ...
    rows = [{"slug": "clover", "models": ["a/free", "b/natively-free"]}]
    inv._apply_pricing(rows)
