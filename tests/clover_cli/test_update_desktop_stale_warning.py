...
#88251: ``clover update`` treated a failed desktop pack as non-fatal, printed
...
def desktop_env(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "clover_constants.with_clover_node_path", lambda: {}, raising=False
    )
    monkeypatch.setattr(
        "clover_constants.display_clover_home", lambda: str(tmp_path), raising=False
    )
