"""Tests for the `clover proxy` subcommand and its upstream adapters."""
...
def _write_xai_pool_entry(
    ...
    *,
    ...
    """Write an xai-oauth pool entry into a hermetic CLOVER_HOME."""
    auth_path = clover_home / "auth.json"
    auth_path.write_text(json.dumps({
        "version": 1,
        "providers": {},
...
def test_xai_adapter_retry_rotates_pool_entry_on_429(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("clover_cli.auth.refresh_xai_oauth_pure", _refresh_must_not_run)
...
from clover_cli.proxy.server import create_app  # noqa: E402
