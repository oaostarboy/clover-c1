"""Tests for clover_cli.cron command handling."""
...
class TestCronCommandLifecycle:
    ...
    def test_edit_persists_user_owned_inference_pins(self, tmp_cron_dir, capsys):
        ...
        assert updated["provider"] == "clover"
...
class TestGatewayNotRunningWarning:
    ...
    def test_list_warns_when_gateway_absent(self, tmp_cron_dir, capsys, monkeypatch):
        ...
        monkeypatch.setattr("clover_cli.gateway.find_gateway_pids", lambda: [])
...
class TestExternalCronProviderStatus:
    ...
    def test_status_reports_provider_not_ticker_for_chronos(
        ...
        monkeypatch.setattr(
            "clover_cli.cron._active_cron_provider_name", lambda: "chronos"
        )
...
class TestCronRunBackgroundDispatch:
    """`clover cron run` must not report 'failed' when the run was dispatched
