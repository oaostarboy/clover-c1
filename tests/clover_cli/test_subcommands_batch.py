...
def test_config_get_unset_subcommands_parse():
    """`clover config get/unset` parse key args (and --json for get)."""
...
# ── deprecated `clover login` fails gracefully, not with argparse error ────
#
# `clover login` is a removed command; its handler (`login_command` in
# `clover_cli/auth.py`) prints a deprecation notice pointing at `clover auth` /
# `clover model` and exits 0.  Two behavior contracts guard the UX:
...
def test_login_subparser_help_is_suppressed():
    """The deprecated `login` row must not appear in `clover --help`.
