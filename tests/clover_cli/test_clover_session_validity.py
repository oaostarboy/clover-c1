"""Tests for the local-only Clover session classifier exposed on /api/status."""
...
from clover_cli.auth import (
    CLOVER_SESSION_TERMINAL,
    CLOVER_SESSION_UNKNOWN,
    CLOVER_SESSION_VALID,
    get_clover_session_validity,
)
    ...
    return ".".join(
        (
            ...
            _encode(
                {
                    "sub": "test-user",
                    "scope": auth.DEFAULT_CLOVER_SCOPE,
...
def _block_live_auth(monkeypatch):
    monkeypatch.setattr(auth, "get_clover_auth_status", _fail_if_live_auth_is_used)
    monkeypatch.setattr(
        auth,
...
# ── get_clover_auth_status_local — refresh-free display snapshot ──
...
def test_local_status_not_logged_in_after_terminal_quarantine(monkeypatch):
    ...
    status = auth.get_clover_auth_status_local()
...
def test_local_status_repeated_polling_never_uses_live_auth(monkeypatch):
    ...
    assert all(
        auth.get_clover_auth_status_local()["logged_in"] for _ in range(10)
    )
