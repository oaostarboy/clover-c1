...
Covers the three paths changed by fix/bedrock-provider-model-ids-live-discovery:
  ...
  2. list_authenticated_providers() Section 2 (CLOVER_OVERLAYS) — bedrock
...
# ---------------------------------------------------------------------------
# 2. list_authenticated_providers() — bedrock via CLOVER_OVERLAYS (Section 2)
# ---------------------------------------------------------------------------
...
class TestBedrockOverlayRegistration:
    """bedrock entry in CLOVER_OVERLAYS is correctly configured."""

    def test_bedrock_overlay_exists(self):
        from clover_cli.providers import CLOVER_OVERLAYS
        assert "bedrock" in CLOVER_OVERLAYS
