...
install``. A locked ``clover.exe`` makes the install exit 2, the handler
...
def test_zip_overlay_blocked_on_modified_file(tmp_path, monkeypatch):
    (tmp_path / ".git").mkdir()
    monkeypatch.setattr(
        update_cmd.subprocess, "run", _porcelain_run(" M clover_cli/update_cmd.py\n")
    )
...
def test_update_via_zip_aborts_before_download_when_dirty(
    ...
    monkeypatch.setattr(clover_main, "PROJECT_ROOT", fake_root)
    monkeypatch.setattr(
        update_cmd.subprocess,
        "run",
    ...
    with patch("urllib.request.urlretrieve") as download:
        with pytest.raises(SystemExit) as exc_info:
            clover_main._update_via_zip(SimpleNamespace(branch=None))
...
def test_staging_artifact_lines_are_recognized():
    ...
    assert is_artifact("?? agent.clover-update-staging/")
    assert is_artifact("?? cli.py.clover-update-staging")
    assert is_artifact("?? tools.clover-update-old/")
    ...
    assert not is_artifact(" M clover_cli/update_cmd.py")
    assert not is_artifact("?? notes.clover-update-staging.txt")
...
def test_recheck_ignores_own_staging_artifacts(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        update_cmd.subprocess,
        "run",
        _porcelain_run("?? agent.clover-update-staging/\n?? cli.py.clover-update-old\n"),
    )
    assert (
