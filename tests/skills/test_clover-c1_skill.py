"""The `clover-c1` skill is what a running Clover knows about itself.

`website/` is never packaged, so an installed Clover has no local copy of the
user guide; skills ARE synced into `$CLOVER_HOME/skills/`. The skill therefore
...
shipped feature is invisible and the agent answers "Clover can't do that."
"""
