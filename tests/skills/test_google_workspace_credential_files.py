...
class TestGoogleWorkspaceCredentialFiles:
    ...
    def test_entries_are_registered_when_files_exist(self, tmp_path):
        ...
        clover_home.mkdir()
        (clover_home / "google_token.json").write_text("{}")
        (clover_home / "google_client_secret.json").write_text("{}")
        ...
        try:
            ...
            assert "/root/.clover/google_token.json" in container_paths
            assert "/root/.clover/google_client_secret.json" in container_paths
        finally:
            clear_credential_files()
