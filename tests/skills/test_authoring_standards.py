...
def test_required_frontmatter_fields(p):
    ...
    clover = (fm.get("metadata") or {}).get("clover") or {}
    if not (clover.get("tags") or fm.get("tags")) and not _grandfathered(p, "tags"):
        pytest.fail(f"{_rel(p)}: no tags (metadata.clover.tags or top-level tags)")
...
def test_related_skills_resolve(p):
    ...
    clover = (fm.get("metadata") or {}).get("clover") or {}
    dangling = [
        rs for rs in (clover.get("related_skills") or []) if rs not in _all_names()
    ]
