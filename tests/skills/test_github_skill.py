...
def test_frontmatter_required_fields():
    ...
    clover = fm["metadata"]["clover"]
    assert clover["tags"]
    assert "related_skills" in clover
...
def test_author_credits_human_first():
    fm, _ = _frontmatter_and_body()
    assert not fm["author"].startswith("Clover Cognition"), "human contributor must be credited first"
...
def test_related_skills_resolve_in_repo():
    ...
    for name in fm["metadata"]["clover"]["related_skills"]:
        hits = (
