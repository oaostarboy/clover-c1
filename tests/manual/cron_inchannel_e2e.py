...
Uses a throwaway CLOVER_HOME so it never touches ~/.clover. No real names.
"""
...
def _fresh_home():
    """Point CLOVER_HOME at a throwaway dir BEFORE importing gateway modules
    (mirror.py binds _SESSIONS_INDEX from get_clover_home() at import time)."""
    d = tempfile.mkdtemp(prefix="cron_inchannel_e2e_")
    os.environ["CLOVER_HOME"] = d
    return Path(d)


HOME = _fresh_home()

# Import AFTER CLOVER_HOME is set.
...
# a different get_clover_home() at import if something imported it earlier).
...
def main():
    ...
    print(f"CLOVER_HOME (throwaway): {HOME}")
