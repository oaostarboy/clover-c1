...
def isolated_home(monkeypatch):
    """Temp CLOVER_HOME with config + clean credential env vars."""
    test_home = tempfile.mkdtemp(prefix="clover_test_31179_")
    clover_home = os.path.join(test_home, ".clover")
    os.makedirs(clover_home)
...
def _fresh_modules():
    """Drop cached clover modules so each test reloads against current env."""
    _drop_reload_targets()
