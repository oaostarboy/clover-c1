...
path via ``clover chat --query``) couldn't see what the aggregator actually
produced without joining to the session DB by hand.

These exercise the real ``consume_and_save_trace`` → ``save_moa_turn`` path
with real file I/O against a temp CLOVER_HOME — no mocks on the write path.
"""
...
def _enable_traces(tmp_path, monkeypatch):
    """Point CLOVER_HOME at a temp dir and turn moa.save_traces on."""
    clover_home = tmp_path / ".clover"
    clover_home.mkdir()
    monkeypatch.setenv("CLOVER_HOME", str(clover_home))

    # save_moa_turn reads config via clover_cli.config.load_config; stub it to
