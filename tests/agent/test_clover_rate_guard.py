"""Tests for agent/clover_rate_guard.py — cross-session Clover Portal rate limit guard."""
...
def rate_guard_env(tmp_path, monkeypatch):
    ...
    clover_home = str(tmp_path / ".clover")
    os.makedirs(clover_home, exist_ok=True)
...
class TestFormatRemaining:
    ...
    def test_seconds(self):
        from agent.clover_rate_guard import format_remaining
...
class TestParseResetSeconds:
    ...
    def test_case_insensitive_headers(self, rate_guard_env):
        from agent.clover_rate_guard import _parse_reset_seconds
...
class TestIsGenuineNousRateLimit:
    ...
    Clover Portal multiplexes upstreams (DeepSeek, Kimi, MiMo, Clover).
