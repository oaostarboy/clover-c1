...
manipulation. Clover keeps canonical conversation history in state.db and may
...
def fake_homes(tmp_path, monkeypatch):
    """Point CLOVER_HOME at a temp profile dir.

    Uses the real env-var resolution chain (get_clover_home /
    get_default_clover_root) instead of monkeypatching private helpers —
    ...
    CLOVER_HOME=<root>/profiles/<name> makes get_default_clover_root()
