...
def test_reconcile_absorbed_into_beats_everything_else(curator_env):
    ...
    out = curator_env._reconcile_classification(
        ...
        destinations={"clover-c1-dev"},
        absorbed_declarations={
            "pr-review-format": {"into": "clover-c1-dev", "declared": True},
        },
    )
...
# `clover curator status`). The full data has always been in REPORT.md on
...
# ---------------------------------------------------------------------------
# Pin hint — surfaces `clover curator pin <umbrella>` in the rename block so
