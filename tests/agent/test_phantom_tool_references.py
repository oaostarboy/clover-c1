...
Covers:
  * CLOVER_AGENT_HELP_GUIDANCE degrades to the docs-only variant when the
...
class TestCloverAgentHelpGuidance:
    def test_skill_variant_used_when_skill_view_present(self):
        from agent.prompt_builder import CLOVER_AGENT_HELP_GUIDANCE
        assert "skill_view(name='clover-c1')" in CLOVER_AGENT_HELP_GUIDANCE
...
class TestEssentialSkillsUndisableable:
    def test_agent_side_reader_strips_essential(self, monkeypatch, tmp_path):
        ...
        assert "clover-c1" not in disabled
...
class TestEssentialOnlySync:
    def test_opted_out_sync_seeds_only_essential(self, monkeypatch, tmp_path):
        """A profile with .no-bundled-skills still gets the clover-c1 skill."""
        ...
        for cat, name in [
            ("autonomous-ai-agents", "clover-c1"),
            ("media", "gif-search"),
        ]:
            d = bundled / cat / name
        ...
        monkeypatch.setattr(ss, "_clover_home", lambda: home)
        ...
        assert result["copied"] == ["clover-c1"]
        assert (home / "skills" / "autonomous-ai-agents" / "clover-c1" / "SKILL.md").exists()
