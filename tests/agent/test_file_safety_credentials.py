"""Tests for CLOVER_HOME credential-file read blocking in file_safety.

Regression for https://github.com/CloverCognition/clover-c1/issues/17656 —
``read_file`` was previously only sandboxed against ``CLOVER_HOME`` itself,
...
for the credential stores while leaving arbitrary ``CLOVER_HOME`` files
...
def fake_home(tmp_path, monkeypatch):
    """Point ``_clover_home_path()`` at a tmp dir for isolated checks."""
    import agent.file_safety as fs

    home = tmp_path / "clover_home"
    home.mkdir()
    monkeypatch.setattr(fs, "_clover_home_path", lambda: home)
    return home
...
def test_arbitrary_clover_home_file_not_blocked(fake_home):
    """Non-credential files inside CLOVER_HOME stay readable."""
...
def test_identically_named_clover_files_outside_home_not_blocked(
    fake_home, tmp_path
):
    """Clover-specific filenames (``auth.json``, ``mcp-tokens/``, ``google_oauth.json``)
    outside CLOVER_HOME must remain readable — the gate is per-location for
    ...
    project.mkdir()
    # auth.json outside CLOVER_HOME — readable (per-location gate).
    p = project / "auth.json"
    p.write_text("not secret here", encoding="utf-8")
    assert get_read_block_error(str(p)) is None, (
        "auth.json outside CLOVER_HOME should NOT be blocked"
    )
...
def test_profile_mode_blocks_root_credentials(tmp_path, monkeypatch):
    """Under a profile, CLOVER_HOME = <root>/profiles/<name>, but
    ...
    monkeypatch.setattr(fs, "_clover_home_path", lambda: profile)
