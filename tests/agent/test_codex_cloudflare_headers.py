...
The official Codex endpoint must receive Clover' own harness identity, rather
...
These tests pin:
- the required Clover originator
- the versioned Clover User-Agent
...
class TestCodexCloudflareHeaders:
    ...
    def test_user_agent_advertises_clover_version(self):
...
class TestPrimaryClientWiring:
    ...
    def test_apply_client_headers_on_base_url_change(self):
        ...
        with patch("run_agent.OpenAI") as mock_openai:
            ...
            assert headers.get("originator") == "clover-c1"
            assert headers.get("ChatGPT-Account-ID") == "acct-rotation"
            assert headers.get("User-Agent") == f"CloverAgent/{__version__}"
