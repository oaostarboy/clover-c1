...
def http_server(tmp_path, monkeypatch):
    """Spin up a localhost HTTP server and isolate CLOVER_HOME under tmp_path."""
    monkeypatch.setenv("CLOVER_HOME", str(tmp_path / ".clover"))
    (tmp_path / ".clover").mkdir()

    # Force the constants/image cache helpers to re-read CLOVER_HOME.
    import sys
    for mod in list(sys.modules):
        if mod.startswith("clover_constants") or mod.startswith("agent.image_gen_provider"):
...
class TestSaveUrlImage:
    def test_writes_real_bytes_to_clover_home_cache(self, http_server):
        base, _ = http_server
        ...
        # The cache directory must be under CLOVER_HOME — gateway cleanup
