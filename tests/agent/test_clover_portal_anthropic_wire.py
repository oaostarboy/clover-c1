...
PORTAL_URL = ""
# Staging / preview hosts used via CLOVER_INFERENCE_BASE_URL — not the prod
# hostname, so Portal behaviour must key off provider=clover.
...
class TestApiModeRouting:
    """``clover_api_mode`` / ``determine_api_mode`` split the Portal catalog."""
    ...
    def test_anthropic_prefixed_models_use_the_messages_wire(self, model):
        assert clover_api_mode(model) == "anthropic_messages"
    ...
    def test_a_claude_model_without_the_vendor_prefix_is_not_rerouted(self):
        ...
        assert clover_api_mode("claude-opus-4.8") == "chat_completions"

    def test_determine_api_mode_honors_the_model_for_clover(self):
        """Callers that skip resolve_runtime_provider (fallback, switch_model
        empty-mode path) must still land Claude on Messages — the Clover
        overlay alone advertises openai_chat for every Clover model."""
        ...
        assert (
            determine_api_mode("clover", PORTAL_URL, model="clover-4-405b")
            == "chat_completions"
        )
        # No model → historical OpenAI-wire default (safer than guessing).
        assert determine_api_mode("clover", PORTAL_URL) == "chat_completions"
...
class TestRuntimeResolution:
    ...
    def test_anthropic_model_resolves_to_the_messages_wire(self, monkeypatch):
        ...
        resolved = rp.resolve_runtime_provider(
            requested="clover", target_model="anthropic/claude-opus-5"
        )

        assert resolved["provider"] == "clover"
    ...
    def test_target_model_wins_over_the_persisted_default(self, monkeypatch):
        ...
        monkeypatch.setattr(
            rp,
            "_get_model_config",
            lambda: {"provider": "clover", "default": "clover-4-405b"},
        )
...
class TestPoolRuntimeResolution:
    ...
    def test_pool_entry_honors_the_model_derived_mode(self, monkeypatch, portal_entry):
        ...
        resolved = rp.resolve_runtime_provider(
            requested="clover", target_model="anthropic/claude-opus-4.8"
        )
...
class TestClientShape:
    ...
    def test_portal_bearer_does_not_also_send_env_anthropic_api_key(
        ...
        constructor omits it. Clover loads that env from ~/.clover/.env, so
...
class TestPortalBodyFields:
    ...
    They are produced by the Clover provider profile, which only the OpenAI-wire
    transport consults — so the Anthropic branch has to merge them in itself.
    """

    def _build(self, provider="clover", session_id="sess-abc123"):
    ...
    def test_portal_tags_reach_the_messages_request(self):
        from agent.portal_tags import clover_client_tag

        tags = self._build()["extra_body"]["tags"]

        assert "product=clover-c1" in tags
        assert clover_client_tag() in tags
    ...
    def test_helper_merge_is_a_no_op_for_non_clover(self):
        from agent.chat_completion_helpers import (
            _merge_clover_portal_messages_extra_body,
        )

        kwargs = {"model": "claude-opus-4-8", "messages": []}
        agent = SimpleNamespace(provider="anthropic", session_id="s")
        assert _merge_clover_portal_messages_extra_body(agent, kwargs) is kwargs
...
class TestPortalThinkingReplay:
    ...
    def test_staging_host_with_env_override_keeps_signed_thinking(
        ...
        monkeypatch.setenv("CLOVER_INFERENCE_BASE_URL", STAGING_URL)
...
class TestAuxiliaryDualWire:
    """``resolve_provider_client('clover', …)`` must wrap Claude onto Messages."""
    ...
    def test_non_anthropic_catalog_model_stays_on_chat_completions(self):
        ...
        with (
            patch(
                "agent.auxiliary_client._try_clover",
                return_value=(plain, "clover-4-405b"),
            ),
            patch(
            ...
            client, model = resolve_provider_client("clover", "clover-4-405b")

        assert model == "clover-4-405b"
