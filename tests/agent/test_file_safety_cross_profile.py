"""Tests for the cross-Clover-profile write guard in agent/file_safety.

The guard fires when a tool tries to write into another Clover profile's
...
Reference: May 2026 incident — a clover-security profile session
accidentally edited skills under both ~/.clover/profiles/clover-security/skills/
AND ~/.clover/skills/ (the default profile's skills), realizing only
...
# ---------------------------------------------------------------------------
# Helpers — set up a fake Clover root with two profiles, monkeypatch the
...
def _set_active_home(monkeypatch, clover_home: Path):
    """Point file_safety._clover_home_path at a specific profile dir."""
    import agent.file_safety as fs
...
class TestResolveActiveProfileName:
    def test_default_when_home_is_root(self, fake_clover, monkeypatch):
        _set_active_home(monkeypatch, fake_clover["default_home"])
        from agent.file_safety import _resolve_active_profile_name
        assert _resolve_active_profile_name() == "default"


    def test_falls_back_to_default_on_resolution_failure(self, fake_clover, monkeypatch):
        """If CLOVER_HOME resolution raises, return 'default' rather than crashing the tool."""
        import agent.file_safety as fs

        def _boom():
            raise RuntimeError("simulated")

        monkeypatch.setattr(fs, "_clover_home_path", _boom)
...
class TestClassifyCrossProfileTarget:
    ...
    def test_security_writing_default_skill(self, fake_clover, monkeypatch):
        ...
        assert result["active_profile"] == "clover-security"
        assert result["target_profile"] == "default"
        assert result["area"] == "skills"

    def test_default_writing_security_skill(self, fake_clover, monkeypatch):
        ...
        assert result["target_profile"] == "clover-security"


    @pytest.mark.parametrize("area", ["skills", "plugins", "cron", "memories"])
    def test_all_profile_scoped_areas_classified(self, fake_clover, monkeypatch, area):
        _set_active_home(monkeypatch, fake_clover["security_home"])
        from agent.file_safety import classify_cross_profile_target
        target = fake_clover["default_home"] / area / "foo.txt"
