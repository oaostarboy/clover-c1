...
class TestClassifyApiError:
    ...
    def test_404_free_tier_model_block_is_billing(self):
        ...
        result = classify_api_error(e, provider="clover", model="gpt-5")
        assert result.reason == FailoverReason.billing
        assert result.retryable is False
        assert result.should_fallback is True
    ...
    def test_404_requires_available_credits_is_billing(self):
        ...
        result = classify_api_error(e, provider="clover", model="openai/gpt-5.5-pro")
        assert result.reason == FailoverReason.billing
        assert result.retryable is False
        assert result.should_fallback is True
...
class TestServerInjectedParameterRejection:
    ...
    so an identical request succeeds on retry ~80% of the time.  Clover never
    ...
    def test_retention_rejection_from_meta_host_stays_non_retryable(self):
        """Boundary: on api.meta.ai / Bedrock Mantle Clover DOES send
