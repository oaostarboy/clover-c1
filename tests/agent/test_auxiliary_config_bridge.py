...
class TestAuxiliaryConfigBridge:
    ...
    def test_web_extract_bridged(self, monkeypatch):
        config = {
            "auxiliary": {
                "web_extract": {"provider": "clover", "model": "gemini-2.5-flash"},
            }
        }
        _run_auxiliary_bridge(config, monkeypatch)
        assert os.environ.get("AUXILIARY_WEB_EXTRACT_PROVIDER") == "clover"
...
class TestDefaultConfigShape:
    """Verify the DEFAULT_CONFIG in clover_cli/config.py has correct auxiliary structure."""
