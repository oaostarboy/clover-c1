...
@pytest.fixture()
def agent_env():
    """Spin up the mock provider + an isolated CLOVER_HOME, yield (agent, helpers)."""
    ...
    test_home = tempfile.mkdtemp(prefix="clover_e2e_47967_")
    os.makedirs(os.path.join(test_home, ".clover"))
    prev_home = os.environ.get("CLOVER_HOME")
    os.environ["CLOVER_HOME"] = os.path.join(test_home, ".clover")
    ...
    for mod in list(sys.modules):
        if mod == "run_agent" or mod.startswith("agent.") or mod.startswith("tools.") or mod.startswith("clover_"):
            del sys.modules[mod]
    ...
    try:
        ...
        if prev_home is None:
            ...
            os.environ["CLOVER_HOME"] = prev_home
