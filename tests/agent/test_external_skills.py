...
@pytest.fixture
def clover_home(tmp_path):
    """Create a minimal CLOVER_HOME with config."""
...
class TestGetExternalSkillsDirs:
    def test_empty_config(self, clover_home):
        (clover_home / "config.yaml").write_text("skills:\n  external_dirs: []\n")
    ...
    def test_valid_dir_returned(self, clover_home, external_skills_dir):
...
class TestGetAllSkillsDirs:
    def test_local_always_first(self, clover_home, external_skills_dir):
        (clover_home / "config.yaml").write_text(
            f"skills:\n  external_dirs:\n    - {external_skills_dir}\n"
        )
        with patch.dict(os.environ, {"CLOVER_HOME": str(clover_home)}):
        ...
        assert result[0] == clover_home / "skills"
        assert result[1] == external_skills_dir.resolve()


class TestExternalSkillsInFindAll:
    def test_external_skills_found(self, clover_home, external_skills_dir):
        (clover_home / "config.yaml").write_text(
            f"skills:\n  external_dirs:\n    - {external_skills_dir}\n"
        )
        local_skills = clover_home / "skills"
        with (
            patch.dict(os.environ, {"CLOVER_HOME": str(clover_home)}),
    ...
    def test_local_takes_precedence(self, clover_home, external_skills_dir):
...
class TestExternalSkillView:
    def test_skill_view_finds_external(self, clover_home, external_skills_dir):
        (clover_home / "config.yaml").write_text(
            f"skills:\n  external_dirs:\n    - {external_skills_dir}\n"
        )
        local_skills = clover_home / "skills"
