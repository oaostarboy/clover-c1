...
class TestNamedProfileHintIntegration:
    ...
    ``TestNamedProfileHint`` mocks ``get_clover_home``,
    ``get_default_clover_root`` and ``_resolve_active_profile_name``, so it
    ...
    ``CLOVER_HOME`` and no resolver mocks.
    """

    def test_real_clover_home_under_profiles_renders_correct_paths(
        ...
        assert get_clover_home() == profile_home
        assert get_default_clover_root() == root

        agent = _make_agent(valid_tool_names=["read_file"])
        with patch("agent.coding_context._coding_mode", return_value="off"):
            prompt = "\n\n".join(_prompt_parts(agent).values())

        assert "Active Clover profile: coder." in prompt
    ...
    def test_real_default_home_renders_default_branch(self, tmp_path, monkeypatch):
        """CLOVER_HOME at the root resolves to the default profile, unchanged."""
        root = tmp_path / ".clover"
        ...
        assert "Active Clover profile: default." in prompt
...
def test_coding_prompt_preserves_legacy_workspace_order(monkeypatch):
    ...
    monkeypatch.setattr(system_prompt, "CLOVER_AGENT_HELP_GUIDANCE", "HELP")
    monkeypatch.setattr(system_prompt, "CLOVER_AGENT_HELP_GUIDANCE_NO_SKILLS", "HELP")
    monkeypatch.setattr(system_prompt, "STEER_CHANNEL_NOTE", "STEER")
    monkeypatch.setattr(system_prompt, "get_clover_home", lambda: Path("/clover"))

    # Production renders this as str(get_clover_home()) + "/profiles/<name>/",
    # and str(Path("/clover")) is platform-dependent (backslash on Windows) —
    # build the expectation the same way instead of hardcoding "/clover".
    _home_str = str(Path("/clover"))
    expected_profile = (
    ...
    with (
        ...
        patch("clover_time.now", return_value=datetime(2026, 1, 2)),
    ):
...
class TestTelegramRichMessagesHint:
    ...
    def test_base_hint_without_rich_messages(self, monkeypatch):
        ...
        with patch("clover_cli.config.load_config_readonly") as mock_cfg:
    ...
    def test_gateway_rich_messages_integration_via_real_config(self, tmp_path, monkeypatch):
        ...
        under CLOVER_HOME with ``gateway.platforms.telegram.extra.rich_messages``
        ...
        from clover_cli import config as _cfgmod
...
def test_conversation_start_uses_session_start_not_build_time(monkeypatch):
    ...
    with (
        ...
        patch("clover_time.now", return_value=datetime(2026, 1, 2, 9, 0)),
    ):
        prompt = build_system_prompt(agent, system_message="SYSTEM_MESSAGE")
...
class TestConversationStartedTwoLine:
    ...
    def test_same_day_session_keeps_single_line(self):
        from clover_time import now as clover_now
        sid = clover_now().strftime("%Y%m%d_%H%M%S_fresh")
