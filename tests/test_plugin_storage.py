...
The contract under test: data lives under ``<clover home>/plugin-data/<name>/``
...
def test_data_dir_lives_outside_the_install_tree(clover_home):
    root = plugin_data_dir("my-plugin")

    assert root == clover_home / "plugin-data" / "my-plugin"
    assert root.is_dir()
    # The invariant that motivated the module: data must not live under the
    # install tree that `clover plugins remove` deletes.
    assert (clover_home / "plugins") not in root.parents


def test_data_dir_is_stable_across_calls(clover_home):
    assert plugin_data_dir("p") == plugin_data_dir("p")


@pytest.mark.parametrize("bad", ["", ".", "..", "../escape", "a/b", "a\\b", "x" * 65])
def test_hostile_names_are_rejected(clover_home, bad):
    with pytest.raises(ValueError):
        plugin_data_dir(bad)


def test_plugin_db_opens_wal_sqlite_in_the_data_dir(clover_home):
    ...
    assert (clover_home / "plugin-data" / "board" / "data.db").exists()


def test_plugin_db_rejects_path_shaped_filenames(clover_home):
    with pytest.raises(ValueError):
