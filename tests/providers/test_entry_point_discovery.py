"""Tests for pip entry-point provider discovery (clover-c1.plugins group).

Verifies that ``providers/__init__.py`` imports provider plugins exposed via a
distribution's ``clover-c1.plugins`` entry point, supporting both a
