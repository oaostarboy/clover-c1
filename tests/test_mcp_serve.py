"""
Tests for mcp_serve — Clover MCP server.
...
def _isolate_clover_home(tmp_path, monkeypatch):
    """Redirect CLOVER_HOME to a temp directory."""
...
def _create_test_db(db_path, session_id, messages):
    """Create a minimal SQLite DB mimicking clover_state schema."""
...
class TestEventBridgePollE2E:
    ...
    def test_poll_picks_up_new_conversation_on_db_change(
        ...
        # _poll_once reads <CLOVER_HOME>/state.db for its mtime gate; the autouse
        # fixture points CLOVER_HOME at tmp_path.
        db_path = tmp_path / "state.db"
