...
class TestToolsetConsistency:
    ...
    def test_clover_platforms_share_core_tools(self):
        """All clover-* platform toolsets share the same core tools.

        Platform-specific additions (e.g. ``discord`` / ``discord_admin``
        on clover-discord, gated on DISCORD_BOT_TOKEN) are allowed on top —
        the invariant is that the core set is identical across platforms.
        """
        platforms = ["clover-cli", "clover-telegram", "clover-discord", "clover-whatsapp", "clover-slack", "clover-signal", "clover-homeassistant"]
...
class TestDefaultPlatformWebSearchCoverage:
    def test_clover_whatsapp_toolset_includes_web_search(self):
        assert "web_search" in resolve_toolset("clover-whatsapp")
...
class TestResolveToolsetMemo:
    ...
    def test_second_resolution_is_cached(self, monkeypatch):
        ...
        first = resolve_toolset("clover-cli")
        second = resolve_toolset("clover-cli")
        ...
        assert (
            "clover-cli", True, registry_id, generation
    ...
    def test_memo_result_matches_fresh_resolution(self):
        ...
        toolsets_mod._resolve_toolset_memo.clear()
        first = resolve_toolset("clover-cli", include_registry=False)
        second = resolve_toolset("clover-cli", include_registry=False)
        assert first == second
