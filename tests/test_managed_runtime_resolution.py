"""Guard: Clover-owned subprocesses must not resolve managed runtimes by bare PATH.

Clover installs runtimes for itself — ``uv`` at ``$CLOVER_HOME/bin/uv``, Node at
``$CLOVER_HOME/node``. Neither directory is on the ambient PATH of an arbitrary
process, so ``shutil.which("uv")`` / ``shutil.which("node")`` in Clover's own
...
* when a system copy also exists, the one Clover does not own wins — which is
...
``iter_clover_node_dirs()``, ``resolve_uv()``, or ``ensure_uv()``. This test is
...
# Runtimes Clover provisions into CLOVER_HOME and must therefore resolve
# through a managed-aware helper rather than PATH.
_MANAGED_COMMANDS = frozenset({"uv", "node", "npm", "npx"})

# Directories that are not Clover-owned subprocess code: plugins ship their own
...
def _source_files() -> list[Path]:
    ...
    for path in REPO_ROOT.rglob("*.py"):
        ...
        # clover-c1-0.20.5/, build/ and *.egg-info dirs). CI jobs that
...
def test_no_unreviewed_bare_managed_runtime_lookups():
    ...
    assert not unexpected, (
        "Bare PATH lookup for a Clover-managed runtime.\n\n"
        + "\n".join(f"  {rel}:{lineno}  which({cmd!r})" for rel, cmd, lineno in unexpected)
        + "\n\n$CLOVER_HOME/bin (uv) and $CLOVER_HOME/node are not on an "
        ...
        "  node/npm -> clover_constants.find_node_executable()\n"
        "  PATH env -> clover_constants.iter_clover_node_dirs()\n"
...
def test_managed_node_helpers_exist(helper):
    ...
    assert callable(getattr(clover_constants, helper))


def test_managed_uv_helpers_exist():
    from clover_cli.managed_uv import ensure_uv, managed_uv_path, resolve_uv
