...
def test_pyproject_defines_termux_all_without_known_blockers() -> None:
    ...
    assert '"clover-c1[termux]"' in text
    assert '"clover-c1[matrix]"' not in text.split("termux-all = [", 1)[1].split("]", 1)[0]
    assert '"clover-c1[voice]"' not in text.split("termux-all = [", 1)[1].split("]", 1)[0]
