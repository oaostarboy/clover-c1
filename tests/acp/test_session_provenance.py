...
def test_root_session_no_compression(db):
    ...
    assert prov["currentCloverSessionId"] == "root1"
    assert prov["rootCloverSessionId"] == "root1"
    assert prov["parentCloverSessionId"] is None
...
def test_compression_split_continuation(db):
    ...
    prov = build_session_provenance(
        db, "acp-1", "new", previous_clover_session_id="old"
    )
    assert prov["sessionKind"] == "continuation"
    assert prov["parentCloverSessionId"] == "old"
    assert prov["rootCloverSessionId"] == "old"
    assert prov["compressionDepth"] == 1
    assert prov["previousCloverSessionId"] == "old"
...
def test_non_compression_parent_is_root_not_continuation(db):
    ...
    assert prov["sessionKind"] == "root"
    assert prov["compressionDepth"] == 0
    assert prov["rootCloverSessionId"] == "p"  # lineage root still walked
...
def test_meta_wrapper_shape(db):
    ...
    assert set(meta.keys()) == {"clover"}
    assert "sessionProvenance" in meta["clover"]
    assert meta["clover"]["sessionProvenance"]["currentCloverSessionId"] == "root1"
