...
Override the image with ``CLOVER_TEST_IMAGE`` env var to point at a pre-built
...
IMAGE_TAG = os.environ.get("CLOVER_TEST_IMAGE", "clover-c1-harness:latest")
...
def built_image() -> str:
    ...
    Override with ``CLOVER_TEST_IMAGE`` env var to point at a pre-built
    image (faster local iteration).
    """
    if os.environ.get("CLOVER_TEST_IMAGE"):
        return IMAGE_TAG
...
# ---------------------------------------------------------------------------
# docker_exec — default to the unprivileged clover user
# ---------------------------------------------------------------------------
#
# Background: every Clover runtime path inside the container drops to UID
# 10000 (the ``clover`` user) via ``s6-setuidgid clover``. ``docker exec``
...
# fails (PermissionError swallowed) for clover, so a test that ran as root
...
# helpers below run every probe under ``-u clover`` unless a specific
...
def docker_exec(
    ...
    *args: str,
    ...
    """Run a command inside ``container`` as ``user`` (default: clover).
