...
def test_no_args_starts_clover(built_image: str) -> None:
    """``docker run <image>`` should start clover cleanly.
...
def test_chat_subcommand_passthrough(built_image: str) -> None:
    """``docker run <image> chat --help`` should exec ``clover chat --help``.
...
def test_container_exit_code_matches_inner_exit(built_image: str) -> None:
    ...
    Critical for CI: ``docker run <image> clover batch ...`` returns a
