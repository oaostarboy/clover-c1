...
The shim (docker/clover-exec-shim.sh, installed at /opt/clover/bin/clover)
exists to prevent the auth.json ownership-mismatch bug where
`docker exec <c> clover login` would write /opt/data/auth.json as
...
failed: Clover is not logged into Clover Portal" on every message.

These tests verify:

1. ``docker exec <c> clover …`` (defaulting to root) gets dropped to the
   clover user before the real binary runs.
2. ``docker exec --user clover <c> clover …`` (already non-root) short-
   circuits and doesn't try to drop again.
3. Files written under $CLOVER_HOME from a ``docker exec`` session land
   as clover:clover — the actual user-visible invariant.
4. The CLOVER_DOCKER_EXEC_AS_ROOT opt-out lets diagnostic sessions keep
...
def _wait_for_cont_init(container: str) -> None:
    ...
    ``01-clover-setup`` cont-init hook (docker/stage2-hook.sh) has
    finished seeding + ``chown clover:clover`` config.yaml and running the
    ...
    clobbered the root-written file's ownership back to clover:clover,
    failing ``test_shim_opt_out_keeps_root`` non-deterministically.

    The reliable "cont-init is done" signal is
    ``$CLOVER_HOME/logs/container-boot.log``: it is written by
    ``02-reconcile-profiles`` (clover_cli.container_boot), which s6 runs
    *strictly after* ``01-clover-setup`` in lexicographic order. The
...
def test_shim_drops_root_to_clover_uid(sleep_container: str) -> None:
    """docker exec defaults to root; the shim should drop to uid 10000.

    We invoke `clover` with a Python-style `-c` shim equivalent — there's no
    pure-clover "print my uid" command, so we use the venv's python directly
    ...
    fact that the venv's `clover` is a console_scripts entry — under the
    hood it's a tiny Python wrapper. We can't easily inject "print my uid"
    into it without forking subcommands. Simplest approach: have `clover`
    do anything that writes to disk, then check the file's owner.

    Use `clover config set` which writes config.yaml under CLOVER_HOME.
    ...
    r = subprocess.run(
        ["docker", "exec", sleep_container,
         "clover", "config", "set", "_test.shim_marker", "1"],
        capture_output=True, text=True, timeout=30,
    )
    assert r.returncode == 0, f"config set failed: stdout={r.stdout!r} stderr={r.stderr!r}"

    # The written file must be owned by clover, not root.
    r = subprocess.run(
        ["docker", "exec", sleep_container,
    ...
    assert r.stdout.strip() == "clover:clover", (
        f"config.yaml owned by {r.stdout.strip()!r}, expected clover:clover. "
        "The shim did not drop privileges before invoking clover."
    )
...
def test_main_cmd_path_unaffected(built_image: str) -> None:
    ...
    The shim sits at /opt/clover/bin earliest on PATH; main-wrapper.sh
    invokes `s6-setuidgid clover clover <args>` which resolves `clover`
...
def test_e2e_login_then_supervised_gateway_can_read_auth(
    ...
    Pre-shim: ``docker exec <c> clover login`` (root) wrote
    ...
    as a parse failure, and resolve_clover_runtime_credentials raised
    "Clover is not logged into Clover Portal" on every message.

    We can't do a real OAuth login in a unit test, but we can stand in
    for it by writing the same file shape via `clover config set`-style
    writes — what matters is the *file ownership invariant* downstream
    of `_save_auth_store`. If the shim works, every file the
    `docker exec` path produces is clover-readable.

    Specifically: pretend the operator ran `clover login` (writes
    auth.json) and verify (a) the file exists and (b) it's readable by
    the clover UID. We use `clover auth list` since that touches the
    ...
    # `clover auth list` is read-only but still exercises _load_auth_store
    # under the shim's UID. We invoke `clover config set` first to
    # provoke a write into CLOVER_HOME so we have something concrete to
    # owner-check.
    r = subprocess.run(
        ["docker", "exec", sleep_container,
         "clover", "config", "set", "_test.e2e_marker", "1"],
        capture_output=True, text=True, timeout=30,
    )
    assert r.returncode == 0, f"config set failed: {r.stderr}"

    # The supervised UID (10000) must be able to read everything under
    # CLOVER_HOME that docker exec just wrote.
    r = subprocess.run(
        ["docker", "exec", "--user", "clover", sleep_container,
    ...
    assert not unreadable, (
        "Files written by `docker exec` are unreadable to the clover user "
