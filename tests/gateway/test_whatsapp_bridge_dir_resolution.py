...
(/opt/clover/scripts/whatsapp-bridge) is read-only, so `npm install` fails
with EACCES. The resolver must detect the read-only install dir and mirror the
bridge source into a writable CLOVER_HOME location instead.
"""
import importlib
...
def test_readonly_install_mirrors_to_clover_home(tmp_path, monkeypatch):
    """A read-only install tree is mirrored into a writable CLOVER_HOME."""
    ...
    monkeypatch.setattr(
        "clover_constants.get_clover_home", lambda: clover_home
    )
    ...
    expected = clover_home / "scripts" / "whatsapp-bridge"
