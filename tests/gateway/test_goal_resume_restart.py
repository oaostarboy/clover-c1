...
def clover_home(tmp_path, monkeypatch):
    ...
    # get_clover_home() prefers the context-local override over the env
    # var, so a set_clover_home_override() leaked by ANY earlier test in
...
class TestCliResumeRestartsWork:
    def test_resume_after_budget_exhaustion_queues_continuation(self, clover_home):
    ...
    def test_resume_without_goal_queues_nothing(self, clover_home):
