...
closure the PR changed, against a real temp ``CLOVER_HOME``.
"""
...
def _stub_picker_dependencies(monkeypatch):
    ...
    monkeypatch.setattr(
        "clover_cli.model_switch.list_picker_providers",
    ...
    monkeypatch.setattr(
        "clover_cli.model_switch.resolve_display_context_length",
        lambda *a, **k: 272000,
    )
...
def _setup_isolated_home(tmp_path, monkeypatch, model_yaml_value):
    ...
    # save_config writes to ``get_clover_home() / config.yaml`` — point it here.
    monkeypatch.setattr("clover_constants.get_clover_home", lambda: clover_home)
    monkeypatch.setattr("clover_cli.config.get_clover_home", lambda: clover_home)
    return cfg_path
...
async def test_multiplex_picker_global_persists_only_named_profile(
    ...
    monkeypatch.setattr(gateway_run, "_clover_home", default_home)
    _stub_picker_dependencies(monkeypatch)
