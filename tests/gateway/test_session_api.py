...
async def test_run_agent_binds_api_session_context_for_tool_env(adapter, monkeypatch):
    ...
    monkeypatch.setenv("CLOVER_SESSION_ID", "stale-session")
    observed = {}
    ...
    class FakeAgent:
        ...
        def run_conversation(self, user_message, conversation_history, task_id):
            ...
            observed["context_session_id"] = get_session_env("CLOVER_SESSION_ID")
            observed["context_platform"] = get_session_env("CLOVER_SESSION_PLATFORM")
            observed["context_session_key"] = get_session_env("CLOVER_SESSION_KEY")
            observed["child_session_id"] = _make_run_env({}).get("CLOVER_SESSION_ID")
...
async def test_session_chat_treats_pre_existing_poisoned_row_as_no_model(session_db):
    ...
    virtual model alias (e.g. "clover-c1") persisted literally as its
...
async def test_session_model_lock_endpoint_then_chat_stream_reuses_persisted_lock(
        ...
        return (
            {
                ...
                "runtime": {
                    "provider": "clover",
                    "model": "x-ai/grok-4.5",
                    "requested": {"provider": "clover", "model": "x-ai/grok-4.5"},
    ...
    assert captured["route"] == {"provider": "clover", "model": "x-ai/grok-4.5"}
    assert captured["requested_runtime"]["provider"] == "clover"
...
async def test_confirmed_runtime_lock_rejects_actual_runtime_mismatch(adapter, monkeypatch):
    ...
    with pytest.raises(RuntimeError, match="confirmed model lock runtime mismatch"):
        await adapter._run_agent(
            ...
            conversation_history=[],
            session_id="mismatch-session",
            route={"provider": "clover", "model": "x-ai/grok-4.5"},
            requested_runtime={"provider": "clover", "model": "x-ai/grok-4.5"},
            route_source="session_model_lock",
...
async def test_unconfirmed_request_does_not_replace_confirmed_session_lock(adapter, session_db):
    ...
    assert config["browser_model_lock"]["provider"] == "clover"
