...
#
# Installs a real, earlier Clover the way a user does, applies ONE update route,
# and requires the checkout to land on this commit with a working `clover`.
#
...
# Keep sandbox state out of the default .clover-sandbox so a run never clobbers
...
SANDBOX_DIR_NAME=".clover-sandbox-e2e-$ROUTE"
export CLOVER_DEV_SANDBOX_DIR="$SANDBOX_DIR_NAME"

SANDBOX_ROOT="$REPO_ROOT/$SANDBOX_DIR_NAME"
INSTALL_DIR="/home/clover/.clover/clover-c1"   # user-level layout (sandbox default)
FAKE_REMOTE="/work/repos/clover-c1.git"
...
LOG_DIR="${CLOVER_E2E_LOG_DIR:-$(mktemp -d -t clover-install-e2e-logs.XXXXXX)}"
...
# ── helpers ────────────────────────────────────────────────────────────────
# Does the INSTALLED clover accept FLAG on `clover update`?
#
...
# that silently rots. `clover update --help` is the same surface a user meets,
# and argparse prints every option it accepts.
update_supports() {
  local flag="$1"
  in_sandbox "clover update --help 2>&1" | grep -qF -- "$flag"
}
...
require_clover_works() {
  local when="$1" out
  out="$(in_sandbox "clover --version" 2>&1)" \
    || { printf '%s\n' "$out" >&2; fail "clover --version failed $when"; }
  printf '%s\n' "$out" | sed 's/^/    /'
  ok "clover runs $when"
}

# ── install the earlier Clover ─────────────────────────────────────────────
...
require_clover_works 'after install'

# ── apply exactly one update route ─────────────────────────────────────────
case "$ROUTE" in
  update)
    step 'ROUTE: clover update'
    ...
    # clover which it accepts; older ones read the prompt from stdin, so close it.
    if update_supports --yes; then
      update_cmd="clover update --yes"
    else
      update_cmd="clover update </dev/null"
    fi
    if ! in_sandbox "cd $INSTALL_DIR && $update_cmd"; then
      collect_sandbox_logs update
      fail "clover update failed ($update_cmd)"
    fi
    require_landed_on_target 'clover update'
    require_clover_works 'after clover update'
    ;;
  installer)
    ...
    require_clover_works 'after installer re-run'
    ;;
esac
