...
``set_clover_home_override`` / ``reset_clover_home_override``. Because
``_plugins_dir()`` keys off ``get_clover_home()``, the list action must then
...
def test_plugins_manage_profile_reads_that_profiles_dir(tmp_path, monkeypatch):
    ...
    import clover_cli.profiles as profiles
