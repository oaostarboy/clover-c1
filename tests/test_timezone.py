"""
Tests for timezone support (clover_time module + integration points).
...
def _reset_clover_time_cache():
    """Reset the clover_time module cache (replacement for removed reset_cache)."""
    clover_time._cached_tz = None
    clover_time._cached_tz_name = None
    clover_time._cache_resolved = False


# =========================================================================
# clover_time.now() — core helper
# =========================================================================

class TestCloverTimeNow:
    """Test the timezone-aware now() helper."""

    def setup_method(self):
        _reset_clover_time_cache()
    ...
    def test_valid_timezone_applies(self):
        ...
        os.environ["CLOVER_TIMEZONE"] = "Asia/Kolkata"
        result = clover_time.now()
    ...
    def test_utc_timezone(self):
        """UTC timezone works."""
        os.environ["CLOVER_TIMEZONE"] = "UTC"
        result = clover_time.now()
...
class TestGetTimezone:
    ...
    def test_returns_zoneinfo_for_valid(self):
        os.environ["CLOVER_TIMEZONE"] = "Europe/London"
        tz = clover_time.get_timezone()
...
class TestCodeExecutionTZ:
    ...
    def test_tz_injected_when_configured(self):
        """When CLOVER_TIMEZONE is set, child process sees TZ env var.
        ...
        #   2) CLOVER_TIMEZONE itself does NOT leak into the child env
        probe = (
            'import os; '
            'print("TZ=" + os.environ.get("TZ", "NOT_SET")); '
            'print("CLOVER_TIMEZONE=" + os.environ.get("CLOVER_TIMEZONE", "NOT_SET"))'
        )
        ...
        assert "CLOVER_TIMEZONE=NOT_SET" in result["output"], (
            "CLOVER_TIMEZONE should not leak into child env (only TZ)"
        )

    def test_tz_not_injected_when_empty(self):
        """When CLOVER_TIMEZONE is not set, child process has no TZ."""
        import json as _json
        os.environ.pop("CLOVER_TIMEZONE", None)
...
class TestCronTimezone:
    ...
    def test_ensure_aware_naive_preserves_absolute_time(self):
        ...
        Regression: the old code used replace(tzinfo=clover_tz) which shifted
        absolute time when system-local tz != Clover tz.  The fix interprets
    ...
    def test_get_due_jobs_naive_cross_timezone(self, tmp_path, monkeypatch):
        """Naive past timestamps must be detected as due even when Clover tz
        ...
        # Use a Clover timezone far behind UTC so that the numeric wall time
        # of the naive timestamp exceeds _clover_now's wall time — this would
        # have caused a false "not due" with the old replace(tzinfo=...) approach.
        os.environ["CLOVER_TIMEZONE"] = "Pacific/Midway"  # UTC-11
        _reset_clover_time_cache()
        ...
        due = get_due_jobs()
        assert len(due) == 1, (
            "Naive past timestamp should be due regardless of Clover timezone"
        )
    ...
    def test_create_job_stores_tz_aware_timestamps(self, tmp_path, monkeypatch):
        ...
        os.environ["CLOVER_TIMEZONE"] = "US/Eastern"
        _reset_clover_time_cache()
