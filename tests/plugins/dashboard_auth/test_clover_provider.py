"""Tests for the bundled Clover dashboard-auth plugin.

Covers four shapes from Phase 4 of ``.clover/plans/2026-05-21-dashboard-oauth-auth.md``:
...
import plugins.dashboard_auth.clover as clover_plugin
...
def _patched_jwks(provider: clover_plugin.CloverDashboardAuthProvider, rsa_keypair):
...
class TestConstruction:
    def test_protocol_compliance(self):
        assert_protocol_compliance(clover_plugin.CloverDashboardAuthProvider)

    def test_name_and_display(self):
        p = clover_plugin.CloverDashboardAuthProvider(
            client_id="agent:inst1", portal_url="https://portal.example.com"
        )
        assert p.name == "clover"
        assert p.display_name == "Clover Cognition"
    ...
    def test_rejects_malformed_client_id(self):
        with pytest.raises(ValueError, match="agent:"):
            clover_plugin.CloverDashboardAuthProvider(
                client_id="clover-dashboard", portal_url="https://x"
            )
...
class TestPluginRegister:
    def test_skips_when_client_id_missing(self, monkeypatch):
        monkeypatch.delenv("CLOVER_DASHBOARD_OAUTH_CLIENT_ID", raising=False)
        monkeypatch.delenv("CLOVER_DASHBOARD_PORTAL_URL", raising=False)
        ctx = MagicMock()
        clover_plugin.register(ctx)
        ctx.register_dashboard_auth_provider.assert_not_called()
        # Skip reason is surfaced for the gate's fail-closed message.
        assert "CLOVER_DASHBOARD_OAUTH_CLIENT_ID" in clover_plugin.LAST_SKIP_REASON

    def test_registers_with_default_portal_url_when_only_client_id_set(
        self, monkeypatch
    ):
        """Phase 7 follow-up: CLOVER_DASHBOARD_PORTAL_URL is optional —
        defaults to the production Clover Portal. The user shouldn't have
        to set it for the common production deployment path."""
        monkeypatch.setenv("CLOVER_DASHBOARD_OAUTH_CLIENT_ID", "agent:inst1")
        monkeypatch.delenv("CLOVER_DASHBOARD_PORTAL_URL", raising=False)
        ctx = MagicMock()
        clover_plugin.register(ctx)
        ctx.register_dashboard_auth_provider.assert_called_once()
        registered = ctx.register_dashboard_auth_provider.call_args.args[0]
        assert isinstance(registered, clover_plugin.CloverDashboardAuthProvider)
        assert registered._portal_url == ""
        # Skip reason cleared on successful registration.
        assert clover_plugin.LAST_SKIP_REASON == ""
    ...
    def test_empty_portal_url_env_uses_default(self, monkeypatch):
        ...
        monkeypatch.setenv("CLOVER_DASHBOARD_OAUTH_CLIENT_ID", "agent:inst1")
        monkeypatch.setenv("CLOVER_DASHBOARD_PORTAL_URL", "")
        ctx = MagicMock()
        clover_plugin.register(ctx)
        registered = ctx.register_dashboard_auth_provider.call_args.args[0]
        assert registered._portal_url == ""
...
class TestConfigYamlSource:
    ...
    canonical surface for these settings. ``CLOVER_DASHBOARD_OAUTH_CLIENT_ID``
    and ``CLOVER_DASHBOARD_PORTAL_URL`` are operator overrides that win when
    ...
    def patch_config(self, monkeypatch):
        """Yield a callable that replaces ``clover_cli.config.load_config``
        ...
        def _set(oauth_block: Dict[str, Any] | None) -> None:
            ...
            monkeypatch.setattr(
                "clover_cli.config.load_config", lambda: cfg
            )
    ...
    def test_config_yaml_only_client_id_registers(self, patch_config, monkeypatch):
        ...
        registers successfully. This is the path the maintainer's review pushed
    ...
    def test_env_overrides_config_client_id(self, patch_config, monkeypatch):
        ...
        CLOVER_DASHBOARD_OAUTH_CLIENT_ID at deploy time and we MUST
        honour it even if a stale config.yaml ships in the image."""
        monkeypatch.setenv("CLOVER_DASHBOARD_OAUTH_CLIENT_ID", "agent:from-env")
        patch_config({"client_id": "agent:from-config"})
        ctx = MagicMock()
        clover_plugin.register(ctx)
        registered = ctx.register_dashboard_auth_provider.call_args.args[0]
    ...
    def test_neither_source_skips_with_helpful_reason(
        ...
        assert "dashboard.oauth.client_id" in clover_plugin.LAST_SKIP_REASON, (
            f"skip reason omits the config.yaml surface — operators "
            f"won't know it exists. got: {clover_plugin.LAST_SKIP_REASON!r}"
        )
...
class TestStartLogin:
    @pytest.fixture
    def provider(self):
        return clover_plugin.CloverDashboardAuthProvider(
            client_id="agent:inst1", portal_url="https://portal.example.com"
        )
    ...
    def test_allows_http_with_arbitrary_host(self, provider):
        ...
        provider.start_login(redirect_uri="http://clover.fly.dev/auth/callback")
...
class TestCompleteLogin:
    ...
    def test_happy_path_returns_session(self, provider, rsa_keypair):
        ...
        with patch("plugins.dashboard_auth.clover.httpx.post", return_value=mock_resp):
        ...
        assert session.provider == "clover"
    ...
    def test_network_error_raises_provider_error(self, provider):
        with patch(
            "plugins.dashboard_auth.clover.httpx.post",
...
class TestVerifySession:
    ...
    def test_jwks_client_sends_explicit_http_headers(self, provider):
        ...
        client_cls.assert_called_once_with(
            provider._jwks_url,
            cache_keys=True,
            lifespan=clover_plugin._JWKS_CACHE_SECONDS,
            headers={
    ...
    def test_verification_failure_message_surfaces_token_claims(
        ...
        config drift between CLOVER_DASHBOARD_PORTAL_URL/CLIENT_ID and Portal."""
    ...
    def test_contract_version_missing_warns_but_succeeds(
        ...
        with caplog.at_level(logging.WARNING, logger="plugins.dashboard_auth.clover"):
...
class TestRefreshAndRevoke:
    ...
    def test_refresh_happy_path_returns_rotated_session(self, provider, rsa_keypair):
        ...
        with patch(
            "plugins.dashboard_auth.clover.httpx.post", return_value=mock_resp
        ) as mock_post:
        ...
        assert kwargs["headers"]["x-clover-refresh-token"] == "rt_old_value"
