...
def _isolate_env(tmp_path, monkeypatch):
    """Ensure CLOVER_HOME and RETAINDB vars are isolated."""
...
class TestRetainDBMemoryProvider:
    ...
    def test_initialize_creates_client_and_queue(self, tmp_path, monkeypatch):
        ...
        p.initialize("test-session", clover_home=str(tmp_path / ".clover"))
...
class TestPrefetch:
    ...
    def _make_initialized_provider(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("CLOVER_HOME", str(clover_home))
        p = RetainDBMemoryProvider()
        p.initialize("test-session", clover_home=str(clover_home))
        return p
