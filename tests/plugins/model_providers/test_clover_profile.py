"""Unit tests for the Clover Portal profile's reasoning wiring.
...
@pytest.fixture
def clover_profile():
    """Resolve the registered Clover profile through the real discovery path."""
    # ``model_tools`` triggers plugin discovery on import, which is what
    # registers the Clover profile in the global provider registry.
    import model_tools  # noqa: F401
    import providers

    profile = providers.get_provider_profile("clover")
    assert profile is not None, "clover provider profile must be registered"
    return profile
...
def portal_catalog(monkeypatch):
    ...
    monkeypatch.setattr(models_mod, "_clover_reasoning_caps_failed_at", None)
    monkeypatch.setattr(models_mod, "_clover_reasoning_caps_cache", {
...
class TestNousReasoningWireShape:
    ...
    def test_disable_reaches_optional_reasoning_model(self, clover_profile, portal_catalog):
        """The knob the user set is the knob the Portal receives."""
        extra_body, top_level = clover_profile.build_api_kwargs_extras(
    ...
    def test_disable_dropped_for_mandatory_reasoning_model(self, clover_profile, portal_catalog):
        """Mandatory routes 400 on a disable — send nothing instead."""
        extra_body, _ = clover_profile.build_api_kwargs_extras(
            reasoning_config={"enabled": False},
            supports_reasoning=True,
    ...
    def test_disable_dropped_for_unknown_model(self, clover_profile, portal_catalog):
    ...
    def test_disable_dropped_for_non_reasoning_route(self, clover_profile, portal_catalog):
        """A route the catalog says takes no reasoning parameter gets none.

        Clover' own ``supports_reasoning`` can disagree with the Portal about a
    ...
    def test_enabled_config_always_forwarded(self, clover_profile, portal_catalog, model):
    ...
    def test_no_config_defaults_to_medium(self, clover_profile, portal_catalog):
        extra_body, _ = clover_profile.build_api_kwargs_extras(
    ...
    def test_nothing_emitted_without_reasoning_support(self, clover_profile, portal_catalog):
        extra_body, top_level = clover_profile.build_api_kwargs_extras(
            reasoning_config={"enabled": False},
    ...
    def test_caller_config_not_mutated(self, clover_profile, portal_catalog):
        cfg = {"enabled": False}
        clover_profile.build_api_kwargs_extras(
