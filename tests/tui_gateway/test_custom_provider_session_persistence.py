...
"No LLM provider configured. Run `clover model`..." (resume failed) or
...
def _make_agent_with_override(override, monkeypatch, config, model_cfg=None):
    ...
    # Keep credential-pool resolution off the developer's real CLOVER home.
...
# provider (e.g. Clover) but who switched THIS session to a self-hosted model
...
# found" (the b200/clover-ultra-sft report). The stored MODEL NAME is the one
...
class TestModelNameRecoversEntryIdentity:
    def test_identity_by_model_from_providers_dict_models_list(self, monkeypatch):
        ...
        assert (
            rp.find_custom_provider_identity_by_model("clover-ultra-sft")
            == "custom:clover-ultra"
        )
...
# what left room bots stuck on a stale provider (e.g. "out of Clover credits"
...
#
# Regression: GH #89497 (room bots hang then report "out of Clover credits").
...
class TestRoomPlumbingRuntimeOverrides:
    def test_marked_row_returns_no_overrides(self):
        ...
        row = {
            ...
            "model_config": json.dumps(
                {"model": "openai/gpt-5.6-luna-pro", "provider": "clover", "room_plumbing": True}
            ),
        }
    ...
    def test_marked_row_dict_model_config(self):
        ...
        row = {
            "model": "openai/gpt-5.6-luna-pro",
            "model_config": {"model": "openai/gpt-5.6-luna-pro", "provider": "clover", "room_plumbing": True},
        }
        assert _stored_session_runtime_overrides(row) == {}
    ...
    def test_legacy_group_title_shape_still_skipped(self):
        ...
        row = {
            ...
            "model": "openai/gpt-5.6-luna-pro",
            "billing_provider": "clover",
            "model_config": json.dumps({"model": "openai/gpt-5.6-luna-pro", "provider": "clover"}),
        }
        assert _stored_session_runtime_overrides(row) == {}
...
# "out of Clover credits" after the profile was switched to ollama-cloud) while
...
# VeniceAI/empero endpoint under a model that should run on Clover). The sibling
...
class TestRuntimeModelConfigDropsStaleKeys:
    ...
    def test_falsy_model_drops_stale_existing_model(self):
        ...
        agent = _agent_like(model="", provider="clover")
        existing = {"model": "meituan/longcat-2.0:free", "provider": "clover"}
        config = _runtime_model_config(agent, existing)

        assert "model" not in config, config
        assert config["provider"] == "clover"
    ...
    def test_truthy_provider_overwrites_stale_existing(self):
        ...
        config = _runtime_model_config(_agent_like(provider="clover"), existing)
    ...
    def test_existing_none_returns_only_agent_identity(self):
        ...
        config = _runtime_model_config(_agent_like(provider="clover"), None)

        assert config == {"model": "deepseek/deepseek-v4-flash-0731", "provider": "clover"}
