    ...
    with patch.dict(
        "sys.modules",
        {
            "clover_constants": MagicMock(
                get_clover_home=MagicMock(return_value="/tmp/clover_test_review_summary")
            ),
...
def test_init_session_attaches_background_review_callback(server, monkeypatch):
    ...
    cb("💾 Self-improvement review: Skill 'clover-release' patched")
    ...
    assert sid == "sid-abc"
    assert payload == {
        "text": "💾 Self-improvement review: Skill 'clover-release' patched"
    }
