...
def live_home(monkeypatch):
    """A REAL isolated CLOVER_HOME with a config.yaml + state.db on disk."""
    tmp = Path(tempfile.mkdtemp(prefix="clover-live-staleprov-"))
    home = tmp / ".clover"
    ...
    # clover_constants caches the resolved home at first read — the env var
    ...
    for var in list(os.environ):
        if var.endswith("_API_KEY") or var in ("OPENROUTER_KEY", "CLOVER_KEY"):
            monkeypatch.delenv(var, raising=False)

    import clover_cli.config as hconfig
    ...
    # conftest also re-pins clover_state.DEFAULT_DB_PATH per test; pin it to
    ...
    monkeypatch.setattr(server, "_clover_home", str(home), raising=False)
    yield home, server
