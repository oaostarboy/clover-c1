...
def clover_home(tmp_path, monkeypatch):
    ...
    # Bust the goal-module DB cache so it re-resolves CLOVER_HOME.
...
@pytest.fixture()
def server(clover_home, monkeypatch):
    ...
    # Pin config resolution to the isolated CLOVER_HOME. Sibling test
    ...
    # module-level ``_clover_home = get_clover_home()`` snapshot freezes
    ...
    monkeypatch.setattr(mod, "_clover_home", clover_home)
...
def test_moa_bare_returns_usage(server, session, clover_home):
    _write_moa_config(clover_home, """
moa:
