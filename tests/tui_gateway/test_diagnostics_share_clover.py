"""diagnostics.share_clover RPC — Desktop "Send Diagnostics" upload path.

Contract pinned:
* Reuses the CLI ``--clover`` pipeline (collect_share_bundle → build_clover_bundle
  → share_to_clover) with redaction FORCED on — the client cannot disable it.
...
def _handler():
    fn = server._methods.get("diagnostics.share_clover")
    assert fn is not None, "diagnostics.share_clover not registered"
    return fn
...
def captured_upload(monkeypatch, tmp_path):
    ...
    import clover_cli.diagnostics_upload as du

    monkeypatch.setattr(du, "share_to_clover", _fake_share)
    return captured
...
def test_share_clover_uploads_redacted_bundle(captured_upload):
    ...
    assert envelope["format"].startswith("clover-debug-share/")
    assert envelope["redacted"] is True
    assert "report" in envelope["files"]


def test_share_clover_attaches_redacted_error_context(captured_upload):
...
def test_share_clover_client_text_gets_upload_safe_log_redaction(captured_upload):
...
def test_share_clover_linkless_success_is_a_failure(monkeypatch):
    ...
    monkeypatch.setattr(du, "share_to_clover", lambda blob: {})
...
def test_share_clover_extra_files_sanitized_and_redacted(captured_upload):
    secret = "sk-abc123def456ghi789jkl012mno345pqr678"
    result = _handler()(
        "rid-3",
        {
...
def test_share_clover_upload_failure_is_structured(monkeypatch):
    import clover_cli.diagnostics_upload as du

    def _boom(blob: bytes) -> dict:
        raise RuntimeError("NAS unavailable")

    monkeypatch.setattr(du, "share_to_clover", _boom)
