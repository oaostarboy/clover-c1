...
def test_outbox_drain_returns_each_envelope_once(home):
    ...
    env = bot_relay.enqueue_envelope(
        home, target=target, message="m", sender_profile="default", sender_handle="clover"
    )
...
def test_deliver_validates_profile_and_runs_transport(home, monkeypatch):
    ...
    argv = calls["argv"]
    assert argv[:3] == ["clover", "-p", "ops"]
    assert "Bot Chat" in argv and "--query-file" in argv

    # 'clover' alias resolves to default
    _result(srv._methods["bot_relay.deliver"](2, {"profile": "clover", "message": "x"}))
    assert calls["argv"][:3] == ["clover", "-p", "default"]
...
def test_deliver_write_failure_still_removes_tempfile(home, monkeypatch, tmp_path):
    ...
    assert not glob.glob(str(tmp_path / "clover-relay-dm-*")), "tempfile leaked"
