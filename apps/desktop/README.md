...
<p align="center">
  <a href="https://github.com/oaostarboy/clover-c1/releases"><img src="https://img.shields.io/badge/Download-macOS%20%C2%B7%20Windows%20%C2%B7%20Linux-00D97E?style=for-the-badge" alt="Download"></a>
...
<tr><td><b>Voice</b></td><td>Talk to Clover and hear it back.</td></tr>
...
### Install with Clover (recommended)

Already have the Clover CLI? Just run:

```bash
clover desktop
```

It builds and launches the GUI against your existing install — same config, keys, sessions, and skills. If Desktop cannot find a usable runtime or saved remote connection, first launch lets you connect to an existing Clover gateway or install Clover locally. Local onboarding then walks you through choosing a provider and model.

### Prebuilt installers

Prebuilt installers are built and distributed via [the Clover Desktop website.](https://github.com/oaostarboy/clover-c1).
...
```bash
# throwaway CLOVER_HOME, separate Electron userData, distinct app name to avoid the single-instance lock
...
first launch it can install the Clover Cognition runtime into `CLOVER_HOME`
(`~/.clover`, or `%LOCALAPPDATA%\clover` on Windows), using the same layout as a
CLI install.
...
- **Clover Cognition** runs as a headless `clover serve` process and exposes the
...
1. `CLOVER_DESKTOP_CLOVER_ROOT`
2. the current source checkout during development
3. a completed managed install
4. `CLOVER_DESKTOP_CLOVER`, or `clover` on `PATH`
5. a system Python that can import the Clover runtime
...
Desktop supports a managed local backend, explicit remote gateways, and Clover
...
screen offers **Connect to existing Clover** before starting the local installer.
...
terminal commands, and file operations run against the remote Clover host, not
...
transport- or Clover-managed header names such as `Authorization`, `Cookie`,
`Host`, `Origin`, `Referer`, and `X-Clover-Session-Token`.
...
Boot logs land in `CLOVER_HOME/logs/desktop.log` (includes backend output and recent Python tracebacks) — check it first if the app reports a boot failure.
...
```powershell
# Force a clean first-launch setup
Remove-Item "$env:LOCALAPPDATA\clover\clover-c1\.clover-bootstrap-complete"
# Rebuild a broken Python venv
Remove-Item -Recurse -Force "$env:LOCALAPPDATA\clover\clover-c1\venv"
```

> The default Clover home on Windows is `%LOCALAPPDATA%\clover`. Set the `CLOVER_HOME` env var if you've relocated it.
