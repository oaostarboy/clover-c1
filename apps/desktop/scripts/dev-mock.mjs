...
function createSandbox() {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), `clover-dev-mock-${Date.now()}`))
  ...
  return { root, cloverHome, userDataDir, cleanup: () => fs.rmSync(root, { recursive: true, force: true }) }
}

function writeMockConfig(cloverHome, mockUrl) {
  fs.writeFileSync(
    path.join(cloverHome, 'config.yaml'),
...
`,
  ...
  fs.writeFileSync(path.join(cloverHome, '.env'), 'MOCK_API_KEY=e2e-mock-key\n', 'utf8')
}
...
async function main() {
  ...
  writeMockConfig(sandbox.cloverHome, mock.url)
  console.log(`  CLOVER_HOME: ${sandbox.cloverHome}`)
  ...
  const env = {
    ...
    CLOVER_DESKTOP_APP_NAME: `CloverDevMock-${Date.now()}`,
  }
