 ...
 * product name (`Clover`). If a PREVIOUS `npm run pack` was interrupted
 ...
 *   '.../release/linux-unpacked/electron' -> '.../release/linux-unpacked/Clover'
 *
 * This is a hard failure with no obvious cause for the user — `clover desktop`
 ...
 * (the mac-unpacked Clover.app bundle) and Windows (win-unpacked), so we
 ...
 * Clover.exe that Windows can't load (truncated PE from a corrupt cached
 * Electron zip, wrong arch), the updater's integrity gate in
 * `clover desktop --build-only` (clover_cli/main.py
 ...
 */
export function preserveRollbackBackup(appOutDir, productExeName = 'Clover.exe') {
...
export default async function beforePack(context) {
  ...
  try {
    ...
    // rename fails.
    const productExe = `${(context && context.packager?.appInfo?.productFilename) || 'Clover'}.exe`
