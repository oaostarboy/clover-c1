#!/usr/bin/env node
// set-exe-identity.mjs — stamp the Clover icon + version metadata onto the
// built Clover.exe using rcedit, completely decoupled from electron-builder's
// signing path.
//
// WHY THIS EXISTS
// ---------------
...
// skips rcedit, so the unpacked Clover.exe keeps the stock Electron icon and
...
// so EVERY packed build — first install, `clover desktop`, the installer's
...
// path (which rebuilds via `clover desktop --build-only`, never install.ps1)
// shipped a stock "Electron" exe. Keeping it in afterPack closes that gap.
//
// Also runnable standalone for ad-hoc re-stamping:
//   node scripts/set-exe-identity.mjs <path-to-Clover.exe>
//
...
// Stamp the Clover icon + identity onto `exe`. Resolves on success, throws on
...
async function stampExeIdentity(exe, desktopRoot = resolve(import.meta.dirname, '..')) {
  ...
  console.log('[set-exe-identity] done — Clover icon + identity stamped')
}
