...
// no longer bundles the Clover Cognition Python payload (that's fetched at first
// launch via install.ps1 / install.sh, per the Phase 1 thin-installer flow).
const APP = (() => {
  if (PLATFORM === 'darwin') {
    const appPath = path.join(RELEASE_ROOT, `mac-${ARCH}`, 'Clover.app')
    return {
      appPath,
      binary: path.join(appPath, 'Contents', 'MacOS', 'Clover'),
  ...
  if (PLATFORM === 'win32') {
    ...
    return {
      appPath: unpacked,
      binary: path.join(unpacked, 'Clover.exe'),
  ...
  return {
    appPath: unpacked,
    binary: path.join(unpacked, 'Clover'),
    resourcesPath: path.join(unpacked, 'resources'),
    asarPath: path.join(unpacked, 'resources', 'app.asar'),
    unpackedDistIndex: path.join(unpacked, 'resources', 'app.asar.unpacked', 'dist', 'index.html')
  }
...
// Default CLOVER_HOME for non-sandboxed runs -- matches main.ts's
// resolveCloverHome(). On Windows it's %LOCALAPPDATA%\clover; elsewhere
// it's ~/.clover. The fresh-install sandbox launchFresh() sets its own
// CLOVER_HOME and never touches this.
const DEFAULT_CLOVER_HOME = (() => {
  if (PLATFORM === 'win32' && process.env.LOCALAPPDATA) {
    return path.join(process.env.LOCALAPPDATA, 'clover')
  }
  return path.join(os.homedir(), '.clover')
})()
const VENV_ROOT = path.join(DEFAULT_CLOVER_HOME, 'clover-c1', 'venv')
const FRESH_SANDBOX_ROOT = path.join(os.tmpdir(), 'clover-desktop-fresh-install')
...
function ensurePackagedApp() {
  if (process.env.CLOVER_DESKTOP_SKIP_BUILD === '1' && exists(APP.binary)) {
    return
  }
...
function resolveDmgPath() {
  if (!exists(RELEASE_ROOT)) {
    return path.join(RELEASE_ROOT, `Clover-${PACKAGE_JSON.version}-${ARCH}.dmg`)
  }

  const prefix = `Clover-${PACKAGE_JSON.version}`
  const candidates = fs
  ...
  return candidates.length > 0
    ...
    : path.join(RELEASE_ROOT, `Clover-${PACKAGE_JSON.version}-${ARCH}.dmg`)
}

function resolveNsisPath() {
  // electron-builder NSIS artifactName template is 'Clover-${version}-${os}-${arch}.${ext}'
...
function ensureDmg() {
  ...
  if (process.env.CLOVER_DESKTOP_SKIP_BUILD === '1' && exists(resolveDmgPath())) {
    return
  }
...
function ensureNsis() {
  ...
  if (process.env.CLOVER_DESKTOP_SKIP_BUILD === '1' && resolveNsisPath()) {
    return
  }
...
function launchFresh() {
  ...
  const cloverHome = path.join(sandbox, 'clover-home')
  ...
  env.CLOVER_DESKTOP_CWD = cwd
  env.CLOVER_DESKTOP_IGNORE_EXISTING = '1'
  env.CLOVER_DESKTOP_TEST_MODE = 'fresh-install'
  env.CLOVER_DESKTOP_USER_DATA_DIR = userDataDir
  env.CLOVER_HOME = cloverHome
  delete env.CLOVER_DESKTOP_CLOVER
  delete env.CLOVER_DESKTOP_CLOVER_ROOT
  ...
  console.log(`  CLOVER_HOME: ${cloverHome}`)
  console.log(`  cwd: ${cwd}`)

  return { runtimeRoot: path.join(cloverHome, 'clover-c1', 'venv') }
}

// Validate the packaged bundle matches the thin-installer architecture:
//   - The Clover Cognition Python payload is NOT shipped (it's fetched at first
...
function validateBundle() {
  ...
  // present anymore. If a stray ship of clover_cli sneaks back in we want
  // to fail loudly rather than re-introduce the 400MB delta we just removed.
  const staleFactoryMarker = path.join(APP.resourcesPath, 'clover-c1', 'clover_cli', 'main.py')
...
function help() {
  console.log(`Usage:
  npm run test:desktop:existing  # build packaged app, launch with normal PATH/existing Clover
  npm run test:desktop:fresh     # build packaged app, launch with temp userData + CLOVER_HOME
...
Fast rerun (skip rebuild if the packaged app already exists):
  CLOVER_DESKTOP_SKIP_BUILD=1 npm run test:desktop:fresh
`)
}
