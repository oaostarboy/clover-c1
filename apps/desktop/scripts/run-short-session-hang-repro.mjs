...
const UPSTREAM_URL = 'https://github.com/CloverCognition/clover-c1.git'
...
const LEGACY_STORAGE_KEY = 'clover.desktop.inflightTurnJournal.v1'
const LEGACY_MIGRATION_KEY = 'clover.desktop.inflightTurnJournal.v2.migrated'
...
async function verifyInteractiveSurfaces(cdp, timed, measure, label) {
  ...
  const version = await timed(`version.ipc.${label}`, () => cdp.eval('window.cloverDesktop.getVersion()'))
...
async function withTemporarySandbox(label, body) {
  const sandbox = mkdtempSync(join(tmpdir(), `clover-short-session-${label}-`))
...
async function executeRunInSandboxAttempt(target, index, warmup, mock, output, sandbox, attempt) {
  ...
  const cloverHome = join(sandbox, 'clover-home')
  const userData = join(sandbox, 'electron-user-data')
  const desktopLog = join(cloverHome, 'logs', 'desktop.log')
  ...
  writeSandboxConfig(cloverHome, mock.url)
  ...
  const child = spawn(
    ...
    [target.targetDesktop, `--user-data-dir=${userData}`, `--remote-debugging-port=${port}`],
    {
      ...
      env: sanitizedEnv({
        CLOVER_DESKTOP_APP_NAME: `CloverShortSession-${target.label}-${process.pid}-${index}-${warmup ? 'w' : 'm'}-${attempt}`,
        CLOVER_DESKTOP_CLOVER_ROOT: target.targetRoot,
        CLOVER_DESKTOP_IGNORE_EXISTING: '1',
        CLOVER_DESKTOP_USER_DATA_DIR: userData,
...
async function main() {
  ...
  const ephemeralRoot = mkdtempSync(join(tmpdir(), 'clover-short-session-ab-'))
  const prepared = []
