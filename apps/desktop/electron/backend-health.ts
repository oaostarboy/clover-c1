...
export interface CloverReadyOptions {
   ...
   * two very different meanings of a 401 (see `waitForCloverReady`).
   */
...
/**
 * The one factory for the actionable Clover Cloud agent-is-down error, shared by
...
export async function waitForCloverReady(baseUrl: string, options: CloverReadyOptions): Promise<void> {
  ...
  // When a Clover-managed cloud agent returns a server-side HTTP error
  ...
  throw new Error(`Clover backend did not become ready: ${detail}`)
}
