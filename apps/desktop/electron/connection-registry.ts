 ...
 * apps/shared/src/backend-scope.ts (`@clover/shared`) — tsconfig project
...
export async function reuseMatchingPrimarySshBackend({
  ...
  if (
    ...
    String(source.remoteCloverPath || '').trim() !== String(activeSsh.remoteCloverPath || '').trim() ||
...
/** Turn `ls ~/.clover/profiles` output into roster names. Always includes
...
export function normalizeConnectionInput(input: ConnectionInput, registry: ConnectionRegistry): RegistryConnection {
  ...
  if (kind === 'ssh') {
    const ssh = normalizeSshConfig({
      ...
      remoteCloverPath: input.remoteCloverPath,
