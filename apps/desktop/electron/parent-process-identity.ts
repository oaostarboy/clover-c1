export type ParentWatchdogEnv = {
  CLOVER_PARENT_PID: string
  CLOVER_PARENT_START_MARKER?: string
  CLOVER_PARENT_NONCE?: string
  /** Spawn tag consumed by clover_cli.process_identity (`v1:<install>:<purpose>:<spawner_pid>:<spawner_create_s>`).
...
export function parentWatchdogEnv(pid: number, startMarker: string | null, nonce: string): ParentWatchdogEnv {
  ...
  const env: ParentWatchdogEnv = { CLOVER_PARENT_PID: String(pid) }
  env.CLOVER_SPAWN = spawnTag(pid, startMarker)
  ...
  env.CLOVER_PARENT_START_MARKER = startMarker
  env.CLOVER_PARENT_NONCE = nonce

  return env
}

/** Build the `CLOVER_SPAWN` tag mirrored by clover_cli/process_identity.py.
