...
test('oauthGuardMayHardFail keeps the strict guard for oauth and mixed deployments', () => {
  assert.equal(oauthGuardMayHardFail([{ name: 'clover', supportsPassword: false }]), true)
  assert.equal(
    oauthGuardMayHardFail([
...
test('oauthGuardMayHardFail keeps the strict guard for string oauth providers', () => {
  assert.equal(oauthGuardMayHardFail(['clover'] as any), true)
  assert.equal(oauthGuardMayHardFail(['clover', 'basic'] as any), true)
})
