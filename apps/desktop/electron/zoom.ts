...
export const ZOOM_STORAGE_KEY = 'clover:desktop:zoomLevel'
 ...
 * but forgot to emit 'clover:zoom:changed', leaving the control stuck at 100%.
...
export function applyZoomLevel(webContents, level) {
  ...
  webContents.send('clover:zoom:changed', { level: clamped, percent: zoomLevelToPercent(clamped) })
 ...
 * only learns of changes through `clover:zoom:changed`, which never fired —
