...
// Relative, not `@clover/shared`: the electron bundle is built by esbuild with
...
function encryptDesktopSecret(value, safeStorageApi, options: { allowPlainText?: boolean } = {}) {
  ...
  if (!encryptionAvailable) {
    ...
    throw new Error(
      'Secure token storage is unavailable (no OS keyring service was found), so Clover Desktop cannot save remote gateway tokens. ' +
        ...
        'or set CLOVER_DESKTOP_REMOTE_URL and CLOVER_DESKTOP_REMOTE_TOKEN in your environment.'
    )
  }
  ...
  try {
    ...
    throw new Error(
      `Failed to encrypt the remote gateway token for secure storage${detail}. ` +
        'Set CLOVER_DESKTOP_REMOTE_URL and CLOVER_DESKTOP_REMOTE_TOKEN in your environment as a fallback.'
    )
  }
}
...
// CLOVER_DESKTOP_REMOTE_* env vars.
