// Profile-delete routing logic for the `clover:api` IPC handler.
//
...
// be rejected: spawning a fresh backend would call ensure_clover_home() and
...
/** Parse a `clover:api` request into the profile name a DELETE targets. */
 ...
 * still removing the profile and recreate its CLOVER_HOME.
 *
...
/**
 * Route the next `clover:api` request away from the primary/window backend
 ...
 * ensure_clover_home() recreates the directory the delete just removed.
 */
