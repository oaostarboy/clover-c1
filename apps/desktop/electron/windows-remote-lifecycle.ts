...
const READY_RE = /^CLOVER_(?:BACKEND|DASHBOARD)_READY port=(\d+)/gm
...
async function probeWindowsRemote(ssh, explicitCloverPath = '') {
  const explicit = psLiteral(explicitCloverPath)
  ...
  const script = [
    ...
    '$cloverHome=$env:CLOVER_HOME',
    'if(-not $cloverHome){$cloverHome=Join-Path $env:LOCALAPPDATA "clover"}',
    'Assert-NoReparse $cloverHome $true',
    '$candidate=[IO.Path]::Combine($cloverHome, "clover-c1\\venv\\Scripts\\clover.exe")',
    ...
    '$profileCandidate=[IO.Path]::Combine($HOME, "clover-c1\\.venv\\Scripts\\clover.exe")',
    ...
    '$fallbackHomeCandidate=Join-Path $cloverHome "clover-c1\\venv\\Scripts\\clover.exe"',
    '$fallbackProfileCandidate=Join-Path $HOME "clover-c1\\.venv\\Scripts\\clover.exe"',
    '$candidates=@()',
    'if($explicit){$candidates+=$explicit}',
    '$cmd=Get-Command clover.exe -ErrorAction SilentlyContinue',
    ...
    '$clover=$null',
    'foreach($candidate in $candidates){Assert-NoReparse $candidate $true;$candidatePython=[IO.Path]::Combine([IO.Path]::GetDirectoryName($candidate), "python.exe");Assert-NoReparse $candidatePython $true;try{$item=Get-Item -LiteralPath $candidate -Force -ErrorAction Stop;if(($item.Attributes -band [IO.FileAttributes]::ReparsePoint) -eq 0 -and -not $item.PSIsContainer){$clover=$item.FullName;break}}catch [Management.Automation.ItemNotFoundException]{continue}}',
    'if(-not $clover){throw "Clover is not installed on the remote Windows host."}',
    'Assert-NoReparse $clover $false',
    'if($explicit -and $clover -ne $explicit){throw "The configured Clover path is not an executable file."}',
    '$python=[IO.Path]::Combine([IO.Path]::GetDirectoryName($clover), "python.exe")',
    'Assert-NoReparse $python $false',
    '[ordered]@{os="Windows";arch=$env:PROCESSOR_ARCHITECTURE;cloverHome=$cloverHome;cloverPath=$clover;python=$python}|ConvertTo-Json -Compress'
  ].join(';')

  return JSON.parse((await ssh.exec(powerShellCommand(script))).trim())
}

function windowsUpdateMarkerProbeCommand(cloverHome) {
  const script = [
    '$ErrorActionPreference="Stop"',
...
public static class CloverMarkerNoFollow {
...
`,
    ...
    '}',
    '}',
    `$home=${psLiteral(cloverHome)}`,
    '$installRoot=$home',
    '$parent=Split-Path -Parent $home',
    'if((Split-Path -Leaf $parent) -ieq "profiles"){$installRoot=Split-Path -Parent $parent}',
    '$marker=Join-Path $installRoot ".clover-update-in-progress"',
    '$result="UNCERTAIN"',
    '$stream=$null;$memory=$null',
    'try{',
    'Assert-NoReparse $marker $true',
    'if(-not (Test-Path -LiteralPath $marker -PathType Leaf)){$result="CLEAR"}else{$stream=[CloverMarkerNoFollow]::OpenRead($marker)',
 ...
 */
async function assertWindowsRemoteInstallUpdateClear(ssh, cloverHome) {
  let observation = ''

  try {
    observation =
      String(await ssh.exec(windowsUpdateMarkerProbeCommand(cloverHome)))
        .replace(/^\uFEFF/, '')
        .trim()
        .split(/\r?\n/)
        .pop() || ''
...
async function detectRemotePlatform(ssh, explicitCloverPath = '') {
  try {
  ...
  try {
    return await probeWindowsRemote(ssh, explicitCloverPath)
  } catch (cause: any) {
...
function helperCommand(runtime, operation, args = []) {
  const argv = [runtime.python, '-m', 'clover_cli.windows_ssh_runtime', operation, ...args]
...
function atomicWindowsSpawnCommand(runtime, reservation: any = {}) {
  const argv = [runtime.python, '-m', 'clover_cli.windows_ssh_runtime', 'spawn']
  const helper = operation => [runtime.python, '-m', 'clover_cli.windows_ssh_runtime', operation]

  const script = [
    '$ErrorActionPreference="Stop"',
    `$home=${psLiteral(runtime.cloverHome)}`,
    '$installRoot=$home',
    '$parent=Split-Path -Parent $home',
    'if((Split-Path -Leaf $parent) -ieq "profiles"){$installRoot=Split-Path -Parent $parent}',
    '$marker=Join-Path $installRoot ".clover-update-in-progress"',
    ...
    reservation.ownershipId
      ...
      : '  if($LASTEXITCODE -ne 0){exit $LASTEXITCODE}',
    reservation.ownershipId
      ? `  $spawned=$spawnLines[-1]|ConvertFrom-Json; $lock=[ordered]@{schemaVersion=2;protocolVersion=1;ownershipId=${psLiteral(reservation.ownershipId)};spawnNonce=${psLiteral(reservation.spawnNonce)};pid=[int]$spawned.pid;creationTimeNs=[string]$spawned.creationTimeNs;port=0;profile=${psLiteral(reservation.profile)};cloverPath=${psLiteral(reservation.cloverPath)};cloverHome=${psLiteral(reservation.cloverHome)};tokenFingerprint=${psLiteral(reservation.tokenFingerprint)};startedAt=${psLiteral(reservation.startedAt)}}|ConvertTo-Json -Compress; ` +
...
function validLock(lock, ownershipId) {
  ...
  return Boolean(
    ...
    typeof lock.cloverPath === 'string' &&
    typeof lock.cloverHome === 'string'
  )
}
...
function reusableWindowsLock(lock, state, profile, reuseToken, runtime) {
  return Boolean(
    ...
    lock.cloverPath === runtime.cloverPath &&
    lock.cloverHome === runtime.cloverHome
  )
}
...
async function connectWindowsRemote(deps) {
  ...
  assertBootstrapNotSuperseded(signal)
  const runtime = await probeWindowsRemote(ssh, remoteCloverPath)
  await assertWindowsRemoteInstallUpdateClear(ssh, runtime.cloverHome)
  const inspection = await helper(ssh, runtime, 'inspect', [runtime.cloverPath])

  if (!inspection.supported) {
    const error: any = new Error('Update Clover on the remote Windows host before connecting with Desktop SSH.')
    error.kind = 'update-required'
    throw error
  }

  runtime.cloverPath = inspection.path
  const cloverVersion = inspection.version || ''
  rememberLog(`[ssh-lifecycle] remote platform Windows/${runtime.arch}`)
  rememberLog(`[ssh-lifecycle] located clover at ${runtime.cloverPath}`)
  ...
  try {
    ...
    spawned = await atomicWindowsSpawn(
      ssh,
      runtime,
      JSON.stringify({ ownershipId, spawnNonce, profile, cloverPath: runtime.cloverPath }),
      {
        ownershipId,
        spawnNonce,
        profile,
  ...
  try {
    ...
    const baseUrl = `http://127.0.0.1:${localPort}`
    await waitForClover(baseUrl, token)
    assertBootstrapNotSuperseded(signal)
...
function buildWindowsInteractiveCommand(remoteCwd = '') {
  ...
  script.push('$host.UI.RawUI.WindowTitle="Clover SSH"', 'powershell.exe -NoLogo')
