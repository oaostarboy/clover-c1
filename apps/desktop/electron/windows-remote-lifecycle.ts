...
async function probeWindowsRemote(ssh, explicitCloverPath = '') {
  ...
  const script = [
    ...
    '$candidates+=(Join-Path $cloverHome "clover-c1\\venv\\Scripts\\clover.exe")',
    '$candidates+=(Join-Path $HOME "clover-c1\\.venv\\Scripts\\clover.exe")',
    '$clover=$candidates|Where-Object{Test-Path -LiteralPath $_ -PathType Leaf}|Select-Object -First 1',
    ...
    '$python=Join-Path (Split-Path $clover) "python.exe"',
    'if(-not (Test-Path -LiteralPath $python -PathType Leaf)){throw "The remote Clover Python runtime was not found."}',
