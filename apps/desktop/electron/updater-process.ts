 ...
 *
 * The staged Tauri `clover-setup.exe` has no self-update path, so every
 ...
 * checkout instead: every `clover update` refreshes the code that drives the
 ...
 * and QUITS, the script waits it out, runs `clover update`, swaps/relaunches
 * the app, and writes .clover-update-result.json. With the app gone before
 * the update starts, the CLOVER_DESKTOP_CHILD_PID reaper-exclusion dance is
 ...
 * `clover update` card — old checkouts pull the script on their next update).
 */
 ...
 *
 * The Tauri installer self-copies into CLOVER_HOME on *every* platform
 * (`clover-setup.exe` on Windows, `clover-setup` elsewhere — see
 * apps/bootstrap-installer `paths::installer_dest` and
 * `bootstrap::copy_self_to_clover_home`), so finding that binary on macOS or
 ...
 * than the hand-off protocol holds the update marker, spawns `clover update`,
...
export function resolveStagedUpdaterBinary(
  ...
  const candidate = path.join(cloverHome, 'clover-setup.exe')
 ...
 *
 * `copy_self_to_clover_home` deliberately no-ops during `--update`
 ...
 * "Another Clover update is already running (PID <itself>, started 1s ago)" —
 * the observed infinite "Install didn't finish" loop. Skipping the pre-write
 * for those binaries lets them acquire cleanly and run `clover update`, which
 ...
 * the installer is written to CLOVER_HOME at install/repair time, making mtime
