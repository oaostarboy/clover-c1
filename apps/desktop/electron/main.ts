...
const USER_DATA_OVERRIDE = process.env.CLOVER_DESKTOP_USER_DATA_DIR
...
const DEV_SERVER = process.env.CLOVER_DESKTOP_DEV_SERVER
const IS_PACKAGED = app.isPackaged || Boolean(process.env.CLOVER_DESKTOP_IS_PACKAGED)
...
// switches only apply pre-launch. Override with CLOVER_DESKTOP_DISABLE_GPU
...
if (REMOTE_DISPLAY_REASON) {
  ...
  console.log(
    `[clover] remote display detected (${REMOTE_DISPLAY_REASON}); disabling GPU hardware acceleration to prevent flicker`
  )
}
...
if (DEV_CDP.port) {
  ...
  console.log(
    `[clover] renderer debugging on http://127.0.0.1:${DEV_CDP.port} — anything that can reach it ` +
      'can run code in the renderer. CLOVER_DESKTOP_CDP_PORT=off to disable.'
  )
} else {
  const why = describeDevCdpDecision(DEV_CDP)

  if (why) {
    console.warn(`[clover] ${why}`)
  }
}
...
if (IS_WSL && !REMOTE_DISPLAY_REASON && fs.existsSync('/dev/dxg')) {
  ...
  console.log('[clover] WSL GPU passthrough (/dev/dxg) detected; enabling GPU acceleration')
}
...
// it). The value arrives via CLOVER_DESKTOP_PASSWORD_STORE, bridged by the
// `clover desktop` launcher from detection or `desktop.password_store` in
...
if (PASSWORD_STORE.warning) {
  console.warn(`[clover] ${PASSWORD_STORE.warning}`)
}

if (PASSWORD_STORE.store) {
  app.commandLine.appendSwitch('password-store', PASSWORD_STORE.store)
  console.log(`[clover] using password-store backend: ${PASSWORD_STORE.store}`)
}
...
// never go through `clover desktop`; it is version-scoped so an app update
...
if (IS_WINDOWS) {
  ...
  // expose Clover sessions/config to every packaged app on the machine.
  if (shouldAttemptAclRepair(priorMarker)) {
    const exeDir = path.dirname(process.execPath)
    const acl = grantAllApplicationPackagesAcl(exeDir, { execFileSync })

    if (acl.ok) {
      console.log(`[clover] granted ALL APPLICATION PACKAGES RX on ${exeDir} (#38216)`)
    } else if (acl.error && acl.error !== 'missing-target-or-exec') {
      console.warn(`[clover] AppContainer ACL grant failed on ${exeDir}: ${acl.error}`)
    }
  }
  ...
  if (sandboxDecision.enable && sandboxDecision.reason !== 'already-enabled') {
    ...
    console.log(
      `[clover] Windows sandbox fallback enabled (${sandboxDecision.reason}); launching with --no-sandbox (#38216)`
    )
  }
  ...
  app.on('child-process-gone', (_event, details) => {
    ...
    console.warn(
      `[clover] Windows GPU sandbox crashed (exit=${details?.exitCode}); relaunching once with --no-sandbox (#38216)`
    )
    ...
    try {
      ...
      console.error(`[clover] --no-sandbox relaunch failed: ${error?.message || error}`)
    }
  })
}

ipcMain.handle('clover:get-remote-display-reason', () => REMOTE_DISPLAY_REASON)
...
// always-on timers. A completely idle, minimized Clover burned ~20% CPU
    ...
    try {
      ...
      if (parsed && typeof parsed === 'object' && typeof parsed.commit === 'string' && parsed.commit.length >= 7) {
        if (parsed.schemaVersion !== INSTALL_STAMP_SCHEMA_VERSION) {
          console.warn(
            `[clover] install-stamp.json schemaVersion ${parsed.schemaVersion} != expected ${INSTALL_STAMP_SCHEMA_VERSION}; ignoring`
          )
      ...
      console.warn(`[clover] install-stamp.json found at ${p} , but parsing failed with ${e}`)
...
if (INSTALL_STAMP) {
  console.log(
    `[clover] install stamp: ${INSTALL_STAMP.commit.slice(0, 12)}${INSTALL_STAMP.branch ? ` (${INSTALL_STAMP.branch})` : ''}${INSTALL_STAMP.dirty ? ' [DIRTY]' : ''} from ${INSTALL_STAMP.source || 'unknown'}`
  )
} else if (IS_PACKAGED) {
  ...
  console.error(
    '[clover] WARNING: no install-stamp.json found in packaged build. First-launch bootstrap will not have a pinned ref to install.'
  )
}

// CLOVER_HOME — the user-facing root for everything Clover-related. Mirrors
// scripts/install.ps1's $CloverHome and scripts/install.sh's $CLOVER_HOME.
//
// Defaults:
//   Windows: %LOCALAPPDATA%\clover (matches install.ps1)
//   macOS / Linux: ~/.clover (matches install.sh)
//
// Special case for Windows: if the user has a legacy ~/.clover directory
// (e.g., from a prior pip install or a manual setup) AND no
// %LOCALAPPDATA%\clover yet, prefer the legacy path so we don't orphan their
// existing config / sessions / .env. New installs go to %LOCALAPPDATA%.
//
// CLOVER_DESKTOP_USER_DATA_DIR (used by test:desktop:fresh) puts the sandbox
// CLOVER_HOME beneath the throwaway userData dir so a fresh-install run never
// touches the user's real ~/.clover / %LOCALAPPDATA%\clover.
function resolveCloverHome() {
  if (process.env.CLOVER_HOME) {
    return normalizeCloverHomeRoot(process.env.CLOVER_HOME)
  }

  if (USER_DATA_OVERRIDE) {
    return path.join(path.resolve(USER_DATA_OVERRIDE), 'clover-home')
  }

  if (IS_WINDOWS) {
    // A GUI app launched from Explorer inherits the environment block captured
    // at login, so a CLOVER_HOME set via `setx` AFTER login is invisible in
    // process.env even though the CLI (a fresh shell) sees it. Without this the
    // backend silently falls back to %LOCALAPPDATA%\clover and reports "No
    // inference provider configured" despite a valid configured home (#45471).
    // Consult the live User-scoped registry value before the default below.
    const fromRegistry = readWindowsUserEnvVar('CLOVER_HOME')

    if (fromRegistry) {
      return normalizeCloverHomeRoot(fromRegistry)
    }
  }

  if (IS_WINDOWS && process.env.LOCALAPPDATA) {
    const localappdata = path.join(process.env.LOCALAPPDATA, 'clover')
    const legacy = path.join(app.getPath('home'), '.clover')

    // Migrate transparently to LOCALAPPDATA, but honour an existing legacy
    // ~/.clover setup (no LOCALAPPDATA install yet) so users don't lose state.
  ...
  return path.join(app.getPath('home'), '.clover')
}

const CLOVER_HOME = resolveCloverHome()

function pathWithCloverManagedNode(...entries) {
  const managed = cloverManagedNodePathEntries(CLOVER_HOME).filter(directoryExists)

  return [...managed, ...entries, process.env.PATH].filter(Boolean).join(path.delimiter)
}

// ACTIVE_CLOVER_ROOT — the canonical mutable Clover install. Same path
// install.ps1 / install.sh use, so a desktop-only user and a CLI-only user end
// up with identical layouts and can share one install.
const ACTIVE_CLOVER_ROOT = path.join(CLOVER_HOME, 'clover-c1')
// VENV_ROOT — venv lives inside the repo, exactly like install.ps1 does it.
const VENV_ROOT = path.join(ACTIVE_CLOVER_ROOT, 'venv')
...
// boots, going straight to `resolveCloverBackend()`. Missing or stale marker
...
//
// We deliberately put the marker INSIDE ACTIVE_CLOVER_ROOT (not alongside)
...
const BOOTSTRAP_COMPLETE_MARKER = path.join(ACTIVE_CLOVER_ROOT, '.clover-bootstrap-complete')
...
// active-profile.json records which Clover profile the desktop launches its
// local backend as. When set, startClover() passes `clover --profile <name>
// dashboard …`, which deterministically pins CLOVER_HOME (see
// _apply_profile_override in clover_cli/main.py) and bypasses the sticky
// ~/.clover/active_profile file. Unset (null) preserves the legacy behavior:
// no --profile flag, so the backend honors active_profile / default.
const DESKTOP_PROFILE_CONFIG_PATH = path.join(app.getPath('userData'), 'active-profile.json')
// Mirrors clover_cli.profiles._PROFILE_ID_RE so we never hand the backend a
// value its profile resolver would reject and exit on.
const PROFILE_NAME_RE = /^[a-z0-9][a-z0-9_-]{0,63}$/
// Branch we track for self-update. The GUI work has merged to main, so this
// tracks main. User can also override at runtime via
// cloverDesktop.updates.setBranch().
const DEFAULT_UPDATE_BRANCH = 'main'
// desktop.log lives under CLOVER_HOME/logs/ so it sits next to agent.log,
// errors.log, gateway.log produced by clover_logging.setup_logging — one log
// directory per user, regardless of which UI surface produced the line.
const DESKTOP_LOG_PATH = path.join(CLOVER_HOME, 'logs', 'desktop.log')
...
//
// Mirror the Python logs (clover_logging.py RotatingFileHandler, maxBytes x
...
const BOOT_FAKE_MODE = process.env.CLOVER_DESKTOP_BOOT_FAKE === '1'
const BOOT_FAKE_ERROR = process.env.CLOVER_DESKTOP_BOOT_FAKE_ERROR || ''
...
const SKIP_QUIT_CONFIRM = process.env.CLOVER_DESKTOP_SKIP_QUIT_CONFIRM === '1'

const BOOT_FAKE_STEP_MS = (() => {
  const raw = Number.parseInt(String(process.env.CLOVER_DESKTOP_BOOT_FAKE_STEP_MS || ''), 10)
...
const APP_NAME = process.env.CLOVER_DESKTOP_APP_NAME || 'Clover'
...
// 'clover:native-theme' ('dark' | 'light' | 'system'); we pin
...
// an in-place `clover update` mid-session is reflected without an app restart;
// the seed here just covers the first open and any non-menu invocation path.
app.setAboutPanelOptions({
  applicationName: APP_NAME,
  applicationVersion: resolveCloverVersion(),
  copyright: 'Copyright © 2026 Clover Cognition'
})
...
// startClover(); this pool only holds EXTRA profile
...
const POOL_MAX_BACKENDS = Math.max(1, Number(process.env.CLOVER_DESKTOP_POOL_MAX) || 3)
const POOL_IDLE_MS = Math.max(60_000, Number(process.env.CLOVER_DESKTOP_POOL_IDLE_MS) || 10 * 60_000)
...
//   * WSL2 IPC stall  = the renderer's `clover:backend:touch` roundtrips
...
const POOL_KEEPALIVE_FRESH_MS = Math.max(
  120_000,
  Number(process.env.CLOVER_DESKTOP_POOL_KEEPALIVE_FRESH_MS) || 4 * 60_000
)
...
// onto the error so subsequent startClover() calls (e.g. the renderer's
// ensureGatewayOpen retrying after the WS won't open) return the same error
// instead of re-running install.ps1 in a hot loop. Cleared explicitly by
// the renderer's "Reload and retry" path or by quitting the app.
let bootstrapFailure = null
// Latched non-bootstrap backend spawn failure — stops getConnection() from
// respawning clover serve backend children in a tight loop while boot is broken.
...
function rememberLog(chunk) {
  ...
  cloverLog.push(...lines)

  if (cloverLog.length > 300) {
    cloverLog.splice(0, cloverLog.length - 300)
  }
...
function ensureWslWindowsFonts() {
  ...
  try {
    ...
    const confPath = path.join(confDir, '99-clover-wsl-windows-fonts.conf')
    let existing = ''
...
function broadcastBootProgress() {
  ...
  webContents.send('clover:boot-progress', bootProgressState)
}
...
//
// The snapshot is queryable via the clover:bootstrap:get IPC handler so a
...
function broadcastBootstrapEvent(ev) {
  ...
  webContents.send('clover:bootstrap:event', ev)
}
...
function promptFirstRunSetupChoice(backend) {
  broadcastBootstrapEvent({
    ...
    activeRoot: backend.activeRoot || ACTIVE_CLOVER_ROOT
  })
}
...
// The Tauri updater writes CLOVER_HOME/.clover-update-in-progress for the whole
...
// (`force_kill_other_clover`, taskkill /IM clover.exe) kills it, the launch
...
function updateGateDeps() {
  return {
    hasLiveMarker: () => Boolean(readLiveUpdateMarker(CLOVER_HOME)),
...
async function waitForUpdateToFinish() {
  ...
  const outcome = await waitForUpdateClearance(updateGateDeps(), {
    onWaitTick: async reason => {
      ...
      await advanceBootProgress(
        'backend.update-wait',
        'An update is finishing — Clover will start automatically when it completes…',
        12
      )
    },
  ...
  // "nothing happened").
  try {
    const result = readAndConsumeHandoffResult(CLOVER_HOME)
    ...
    if (result && result.ok && result.manual) {
      ...
      dialog.showErrorBox(
        'Clover update did not finish',
        `${result.message}\n\nDetails: ${path.join(CLOVER_HOME, 'logs', 'desktop-update-handoff.log')}`
      )
    }
  } catch (err) {
...
function findOnPath(command) {
  ...
  // shell-script shim named `clover` — must not shadow `clover.cmd`/`clover.exe`.
...
function unwrapWindowsVenvCloverCommand(command, backendArgs) {
  return resolveVenvCloverCommand(command, backendArgs, {
    isWindows: IS_WINDOWS,
    isCommandScript,
    fileExists,
    directoryExists,
...
// spawns `clover serve`; runtimes older than serve only have `dashboard`. We
...
// (covers a bare `clover` resolved from PATH with no known source root). Result
...
function backendSupportsServe(backend) {
  ...
  if (backend.root) {
    try {
      const src = fs.readFileSync(path.join(backend.root, 'clover_cli', 'subcommands', 'dashboard.py'), 'utf8')
  ...
  if (supported === null) {
    try {
      ...
      // `clover --version` (~10.5s measured cold), and a false negative here
      ...
      execProbeSync(backend.command, [...prefix, 'serve', '--help'], {
        ...
        env: { ...process.env, CLOVER_HOME, ...(backend.env || {}) },
        timeout: PROBE_TIMEOUT_MS,
        stdio: 'ignore',
        // `.cmd`/`.bat` shim backends carry shell: true in their descriptor
        // (see resolveCloverBackend step 4); execFileSync of a .cmd without
...
function isCloverSourceRoot(root) {
  return directoryExists(root) && fileExists(path.join(root, 'clover_cli', 'main.py'))
}

function findPythonForRoot(root) {
  const override = process.env.CLOVER_DESKTOP_PYTHON
...
function findSystemPython() {
  ...
  // CPython but several Clover deps (notably pywinpty's Rust-built
      ...
      try {
        const out = execFileSync(
          ...
          hiddenWindowsChildOptions({
            ...
            // Bare interpreter startup — much lighter than the clover-import
...
// findGitBash — locate bash.exe on Windows. Resolves CLOVER_GIT_BASH_PATH
...
// normal CLOVER_DASHBOARD_READY stdout line and no ready-file side channel is
// needed.
...
// install only has PortableGit under %LOCALAPPDATA%\clover\git (never on
...
function resolveGitBinary() {
  ...
  if (localAppData) {
    candidates.push(path.join(localAppData, 'clover', 'git', 'cmd', 'git.exe'))
    candidates.push(path.join(localAppData, 'clover', 'git', 'bin', 'git.exe'))
  }
...
function recentCloverLog() {
  return cloverLog.slice(-20).join('\n')
}

// ─── Self-update (git-pull against the running backend's clover root) ──────
...
// Dev → SOURCE_REPO_ROOT. Packaged/CLI install → ACTIVE_CLOVER_ROOT.
// CLOVER_DESKTOP_CLOVER_ROOT always wins so devs can pin a worktree.
function resolveUpdateRoot() {
  const candidates = [
    process.env.CLOVER_DESKTOP_CLOVER_ROOT && path.resolve(process.env.CLOVER_DESKTOP_CLOVER_ROOT),
    !IS_PACKAGED && isCloverSourceRoot(SOURCE_REPO_ROOT) ? SOURCE_REPO_ROOT : null,
    isCloverSourceRoot(ACTIVE_CLOVER_ROOT) ? ACTIVE_CLOVER_ROOT : null
  ].filter(Boolean)

  return candidates.find(c => directoryExists(path.join(c, '.git'))) || candidates[0] || ACTIVE_CLOVER_ROOT
}
...
function emitUpdateProgress(payload) {
  ...
  for (const window of BrowserWindow.getAllWindows()) {
    window.webContents.send('clover:updates:progress', merged)
  }
}
      ...
      const req = https.get(
        url,
        {
          headers: {
            ...
            'User-Agent': 'clover-desktop-update-check'
          },
          timeout: 10_000
        },
        res => {
...
// Windows that binary owns ALL repo mutation — running `clover update` +
...
function resolveUpdaterBinary() {
  return resolveStagedUpdaterBinary(CLOVER_HOME, { fileExists, isWindows: IS_WINDOWS })
}
...
// Path to the venv shim whose lock decides whether `clover update` can write
// fresh entry points. On Windows this is the file the running backend
// `clover.exe` holds open; on POSIX it's never mandatory-locked.
function venvCloverShimPath(updateRoot) {
  return IS_WINDOWS
    ? path.join(updateRoot, 'venv', 'Scripts', 'clover.exe')
    : path.join(updateRoot, 'venv', 'bin', 'clover')
}
...
// only signals the direct child, so on Windows a backend `clover.exe` that
// spawned its own grandchildren (a `clover` REPL, a pty terminal session, the
...
async function claimBackendChild(child, command, profile, nonce, outputTail: BackendOutputTail | null = null) {
  ...
  if (decision.action === 'fail') {
    ...
    throw new Error(
      `Clover backend (PID ${child.pid}) died before its identity could be recorded: ${decision.reason}${outputTail?.describe() ?? ''}`
    )
  }
  ...
  if (decision.action === 'degrade') {
    ...
    rememberLog(
      `WARNING: process start marker probe failed for live Clover backend PID ${child.pid}; ` +
  ...
  try {
    ...
    child.cloverBackendIdentity = identity
    ...
    await waitForBackendExit(child)
    throw new Error(
      `Could not persist ownership for the Clover backend: ${error.message}${outputTail?.describe() ?? ''}`
    )
  }
}

function releaseBackendChild(child) {
  const identity = child?.cloverBackendIdentity
...
// `cloverProcess.kill('SIGTERM')` + `app.quit()` fire-and-forget: SIGTERM on
// Windows doesn't reap the backend's grandchildren, and quit didn't wait for
// teardown, so the updater raced a still-locked `clover.exe`, the quarantine
...
// problem: the desktop's backend (and the grandchildren IT spawned — a clover
// REPL, a pty terminal, the gateway) keep `clover.exe` and other files in the
...
async function releaseBackendLock(updateRoot, tag) {
  ...
  const cloverProcess = backendConnectionState.getProcess()
  ...
  // unlock — the shim probe only covers venv\Scripts\clover.exe, but the
  // backend is `python.exe -m clover_cli.main serve`, which need not hold
  // the shim at all (#74805 first-attempt race).
  const initialPids = []

  if (cloverProcess && Number.isInteger(cloverProcess.pid)) {
    initialPids.push(cloverProcess.pid)
  }
  ...
  stopBackendTreesForUpdate(cloverProcess, {
    forceKillProcessTree,
    stopAllPoolBackends
  })

  const shim = venvCloverShimPath(updateRoot)
  ...
  const gate = await waitForBackendRelease(
    initialPids,
    {
      ...
      collectStragglerPids: () => {
        ...
        const currentCloverProcess = backendConnectionState.getProcess()

        if (currentCloverProcess && Number.isInteger(currentCloverProcess.pid)) {
          stragglers.push(currentCloverProcess.pid)
        }
...
// `clover update`). Instead we spawn the staged Clover-Setup binary with
// --update and quit, so it can run `clover update` (which refuses while we
...
async function applyUpdates(opts: { stopSafeBlockers?: boolean } = {}) {
  ...
  try {
    ...
    if (!updater && !IS_WINDOWS) {
      ...
      // Windows (quit → detached orchestrator → `clover update` → relaunch),
      ...
      // in: the CLOVER_DESKTOP_CHILD_PID reaper-exclusion dance (#37532),
      ...
      // get the manual `clover update` card once; their next update pulls it.
      return await applyUpdatesPosixHandoff(opts)
    }

    if (!updater) {
      // No staged updater binary — this is a CLI-installed user (they ran
      // `clover desktop`, never the Tauri installer that self-copies
      // clover-setup.exe into CLOVER_HOME). On Windows the repo hand-off
      ...
      if (!resolveUpdateScriptHandoff(updateRoot)) {
        // They DO have a working `clover` on PATH / in the venv, so the
        ...
        try {
          ...
          if (head.code === 0 && current && current !== 'HEAD') {
            ...
            if (branch !== 'main') {
              command = `clover update --branch ${branch}`
            }
          }
        } catch {
          // Best-effort: fall back to bare `clover update` if branch detection fails.
        }

        rememberLog(`[updates] no staged updater; surfacing manual \`${command}\` for CLI install at ${updateRoot}`)
        emitUpdateProgress({ stage: 'manual', message: command, percent: null })

        return { ok: true, manual: true, command, cloverRoot: updateRoot }
      }

      rememberLog('[updates] no staged updater; using repo hand-off script for CLI install')
    }

    const handoffConflict = updateHandoffConflict(CLOVER_HOME)
    ...
    emitUpdateProgress({
      stage: 'restart',
      message:
        'Updating Clover — this window will close and the updater will open. Don’t reopen Clover yourself; it restarts automatically when the update finishes.',
      percent: 100
    })
    ...
    preflightStateDb(CLOVER_HOME, rememberLog)

    // Stop our own backend(s) and wait for the venv shim to unlock BEFORE we
    // spawn the updater. Without this the updater races a still-locked
    // clover.exe (held by the backend child / its grandchildren) and the update
    ...
    if (!lock.unlocked) {
      ...
      // terminal running clover, an unkillable child). Handing off anyway
      ...
      const message =
        'Update aborted: another process is holding the Clover install open ' +
        '(a second Clover window or a terminal running clover?). Close it and retry.'
    ...
    // Clover processes running from this venv.  The updater normally refuses
    ...
    // `clover update` will run (the venv shim is locked while we live).
    //
    ...
    // checkout, so each `clover update` refreshes the code that drives the
    ...
    if (scriptHandoff) {
      ...
      child = spawnUpdaterProcess(wrapped.command, wrapped.args, {
        ...
        env: {
          ...process.env,
          CLOVER_HOME,
          CLOVER_UPDATE_STARTED_AT: String(updateStartedAt),
          PATH: pathWithCloverManagedNode(venvBin)
        },
        detached: true,
        stdio: 'ignore'
      })
      ...
      // The `clover update` child adopts the SCRIPT's claim via
      // update_lock.py's process-ancestry rule; no mtime heuristics needed.
      if (Number.isInteger(child.pid)) {
        writeUpdateMarker(CLOVER_HOME, child.pid, { startedAt: updateStartedAt })
      }
      ...
      // abort with "Another Clover update is already running (PID <itself>)" —
      ...
      if (Number.isInteger(child.pid) && stagedUpdaterSupportsPrewrittenMarker(updater)) {
        writeUpdateMarker(CLOVER_HOME, child.pid)
    ...
    if (!handoffOutcome.ok) {
      const message = `Update failed to start: ${handoffOutcome.message}. Clover will keep running — try again, or run \`clover update\` from a terminal.`
...
async function handOffWindowsBootstrapRecovery(reason) {
  ...
  const venvBin = path.join(updateRoot, 'venv', IS_WINDOWS ? 'Scripts' : 'bin')
  const venvClover = path.join(venvBin, IS_WINDOWS ? 'clover.exe' : 'clover')
  const venvPython = path.join(venvBin, IS_WINDOWS ? 'python.exe' : 'python')

  // The updater invokes the venv's Clover launcher, which in turn requires the
  ...
  // marker-only install through --update dead-ends at "Could not find the clover
  ...
  const updaterArgs = chooseUpdaterArgs(
    {
      hasBootstrapMarker: fileExists(path.join(updateRoot, '.clover-bootstrap-complete')),
      hasVenvClover: fileExists(venvClover),
...
// Python-level pre-update snapshot inside `clover update`.
function preflightStateDb(cloverHome, rememberLog) {
  const stateDbPath = path.join(cloverHome, 'state.db')
  ...
  try {
    ...
    if (stat.size > 100) {
      ...
      const emergencyPath = path.join(cloverHome, `state.db.pre-update-emergency-${ts}.bak`)
      ...
      try {
        ...
        try {
          const homeDir = fs.readdirSync(cloverHome)
          ...
          for (const old of backups.slice(2)) {
            try {
              fs.unlinkSync(path.join(cloverHome, old))
            } catch {
              void 0
            }
          }
...
// out, runs `clover update`, swaps/relaunches the app bundle, and writes
// .clover-update-result.json for the relaunched Desktop to surface. It shows
...
async function applyUpdatesPosixHandoff(opts: any) {
  ...
  if (!handoff) {
    emitUpdateProgress({ stage: 'manual', message: 'clover update', percent: null })

    return { ok: true, manual: true, command: 'clover update', cloverRoot: updateRoot }
  }
  ...
  const child = spawnUpdaterProcess(handoff.command, args, {
    ...
    env: {
      ...
      CLOVER_HOME,
      CLOVER_UPDATE_STARTED_AT: String(updateStartedAt),
      PATH: pathWithCloverManagedNode(path.join(updateRoot, 'venv', 'bin'))
    },
    detached: true,
    stdio: 'ignore'
  })
  ...
  emitUpdateProgress({
    stage: 'restart',
    message:
      'Updating Clover — this window will close. Don’t reopen Clover yourself; it restarts automatically when the update finishes.',
    percent: 100
  })
...
// Marker-independent: is the canonical install at ACTIVE_CLOVER_ROOT actually
...
function isActiveRuntimeUsable() {
  ...
  return (
    isCloverSourceRoot(ACTIVE_CLOVER_ROOT) &&
    fileExists(venvPython) &&
    canImportCloverCli(venvPython, {
      env: {
        PYTHONPATH: [ACTIVE_CLOVER_ROOT, process.env.PYTHONPATH].filter(Boolean).join(path.delimiter)
      }
    })
  )
}
...
function activeRuntimeState() {
  ...
  // pinned commit -- users update via the in-app update path or `clover
...
function resolveWebDist() {
  const override = process.env.CLOVER_DESKTOP_WEB_DIST
  ...
  if (IS_PACKAGED && /app\.asar(?=$|[\\/])/.test(fallback) && !directoryExists(fallback)) {
    rememberLog(
      `[web-dist] dashboard frontend dir resolved to an asar-internal path that ` +
        ...
        `Ensure dist/** is unpacked (asarUnpack) or set CLOVER_DESKTOP_WEB_DIST.`
    )
  }
...
function resolveRendererIndex() {
  ...
  if (present.length > 0) {
    ...
    rememberLog(
      `[renderer] every renderer bundle is incomplete (${present.join(', ')}). ` +
        ...
        `Repair with: clover desktop --force-build`
    )
  ...
  rememberLog(
    `[renderer] index.html not found — the desktop app was packaged without a ` +
      ...
      `Rebuild with: clover desktop --force-build`
  )
...
function resolveCloverCwd() {
  // In a packaged build, `process.cwd()` resolves to the install root (e.g.
  // `…/win-unpacked` on Windows or `/Applications/Clover.app/Contents/...`
  ...
  const candidates = [
    ...
    process.env.CLOVER_DESKTOP_CWD,
...
function sanitizeWorkspaceCwd(cwd) {
  ...
  if (!trimmed || isPackagedInstallPath(trimmed)) {
    return { cwd: resolveCloverCwd(), sanitized: Boolean(trimmed) }
  }
...
function createPythonBackend(root, label, backendArgs, options: any = {}) {
  ...
  return {
    ...
    label,
    command,
    args: ['-m', 'clover_cli.main', ...backendArgs],
...
// createActiveBackend — build a backend pointing at ACTIVE_CLOVER_ROOT, the
...
function createActiveBackend(backendArgs) {
  ...
  return {
    kind: 'python',
    label: `Clover at ${ACTIVE_CLOVER_ROOT}`,
    command,
    args: ['-m', 'clover_cli.main', ...backendArgs],
    env: buildDesktopBackendEnv({
      cloverHome: CLOVER_HOME,
      pythonPathEntries: [ACTIVE_CLOVER_ROOT, ...getVenvSitePackagesEntries(VENV_ROOT)],
      venvRoot: VENV_ROOT
    }),
    root: ACTIVE_CLOVER_ROOT,
    bootstrap: true,
    shell: false
  }
}

function resolveCloverBackend(backendArgs) {
  // 1. Explicit override -- CLOVER_DESKTOP_CLOVER_ROOT points at a developer
  //    checkout. Honour it as-is (no bootstrap; the user is driving).
  const overrideRoot = process.env.CLOVER_DESKTOP_CLOVER_ROOT && path.resolve(process.env.CLOVER_DESKTOP_CLOVER_ROOT)

  if (overrideRoot && isCloverSourceRoot(overrideRoot)) {
    const backend = createPythonBackend(overrideRoot, `Clover source at ${overrideRoot}`, backendArgs)
  ...
  //    installed `clover` on PATH so local Python edits are actually exercised.
  //    (In dev with no checkout, SOURCE_REPO_ROOT won't pass isCloverSourceRoot.)
  if (!IS_PACKAGED && isCloverSourceRoot(SOURCE_REPO_ROOT)) {
    const backend = createPythonBackend(SOURCE_REPO_ROOT, `Clover source at ${SOURCE_REPO_ROOT}`, backendArgs)
  ...
  // 3. ACTIVE_CLOVER_ROOT — the canonical install at
  //    %LOCALAPPDATA%\\clover\\clover-c1 (Windows) or ~/.clover/clover-c1.
  ...
  if (activeRuntime.shouldUseActiveRuntime && !bootstrapRepairRequested) {
    if (!activeRuntime.hasValidMarker) {
      rememberLog(
        `[bootstrap] Active Clover runtime at ${ACTIVE_CLOVER_ROOT} is usable but the bootstrap marker is missing or stale; skipping first-run bootstrap.`
      )
    }
  ...
  // 4. Existing `clover` on PATH -- installed via install.ps1 / install.sh from
  //    a previous tool-only setup, or pip-installed system-wide. Use it but
  //    do NOT write a bootstrap marker; the user did this themselves and we
  //    don't want to take ownership of an install we didn't perform.
  //    CLOVER_DESKTOP_IGNORE_EXISTING=1 forces the bootstrap path for testing.
  if (process.env.CLOVER_DESKTOP_IGNORE_EXISTING !== '1') {
    let cloverCommand = null
    const cloverOverride = process.env.CLOVER_DESKTOP_CLOVER

    if (cloverOverride) {
      const resolvedOverride = findOnPath(cloverOverride)

      if (resolvedOverride) {
        cloverCommand = resolvedOverride
      } else if (!isWindowsBinaryPathInWsl(cloverOverride, { isWsl: IS_WSL })) {
        cloverCommand = cloverOverride
      } else {
        rememberLog(`Ignoring Windows Clover override under WSL: ${cloverOverride}`)
      }
    } else {
      cloverCommand = findOnPath('clover')
    }

    if (cloverCommand) {
      if (looksLikeDesktopAppBinary(cloverCommand)) {
        rememberLog(`Ignoring desktop app executable on PATH while resolving Clover CLI: ${cloverCommand}`)
        cloverCommand = null
      }
    }

    if (cloverCommand) {
      const unwrapped = unwrapWindowsVenvCloverCommand(cloverCommand, backendArgs)

      if (unwrapped) {
        return unwrapped
      }

      // Smoke-test the candidate before trusting it. A `clover` shim
      ...
      const shellForProbe = isCommandScript(cloverCommand)

      // CLOVER_DESKTOP_CLOVER is an explicit deployment override (used by
      ...
      if (shouldTrustCloverOverride(cloverOverride) || verifyCloverCli(cloverCommand, { shell: shellForProbe })) {
        ...
        // unwrapWindowsVenvCloverCommand here: the second call repeats the
        // same un-memoized import probe, costing up to another full probe
        // timeout on the boot path for an answer we already have.
        return {
          label: `existing Clover CLI at ${cloverCommand}`,
          command: cloverCommand,
          args: backendArgs,
          bootstrap: false,
          env: {},
          kind: 'command',
      ...
      rememberLog(
        `Ignoring existing Clover CLI at ${cloverCommand}: --version probe failed; falling through to bootstrap.`
      )
    }
  }

  // 5. Last-ditch: pip-installed clover_cli module via system Python.
  ...
  if (python) {
    ...
    // having clover_cli installed -- common on dev boxes that have
    ...
    // a uv-managed 3.11 into %LOCALAPPDATA%\clover\clover-c1\venv.
    if (canImportCloverCli(python)) {
      return {
        kind: 'python',
        label: `installed clover_cli module via ${python}`,
        command: python,
        args: ['-m', 'clover_cli.main', ...backendArgs],
        bootstrap: false,
        env: {},
    ...
    rememberLog(`Ignoring system Python ${python}: clover_cli is not importable; falling through to bootstrap.`)
  }
  ...
  //    resolveCloverBackend was the old "no payload" path and forced the
  //    user into a dead end. With the bootstrap protocol, "no install yet"
  //    is a recoverable state the GUI can drive through.
  return {
    kind: 'bootstrap-needed',
    label: 'Clover Cognition not installed yet; bootstrap required',
    command: null,
    args: backendArgs,
    bootstrap: true,
    env: {},
...
async function ensureRuntime(backend) {
  ...
  // backend.kind === 'bootstrap-needed' means resolveCloverBackend couldn't
  ...
  if (backend.kind === 'bootstrap-needed') {
    rememberLog('[bootstrap] no Clover install found; starting first-launch bootstrap')

    if (await handOffWindowsBootstrapRecovery('bootstrap-needed')) {
      const handoffError: Error & { isBootstrapFailure?: boolean; bootstrapHandedOff?: boolean } = new Error(
        'Clover recovery was handed off to Clover Setup. The desktop will restart when recovery completes.'
      )
    ...
    const bootstrapResult = await runBootstrap({
      ...
      cloverHome: CLOVER_HOME,
      logRoot: path.join(CLOVER_HOME, 'logs'),
    ...
    if (bootstrapResult.cancelled) {
      const cancelledError = new Error('Clover install was cancelled.') as any
    ...
    if (!bootstrapResult.ok) {
      const bootstrapError = new Error(
        `Clover bootstrap failed${bootstrapResult.failedStage ? ` at stage '${bootstrapResult.failedStage}'` : ''}: ` +
          `${bootstrapResult.error || 'unknown error'}. ` +
          `Check ${path.join(CLOVER_HOME, 'logs', 'desktop.log')} for the full transcript.`
      ) as any

      bootstrapError.isBootstrapFailure = true
      bootstrapError.failedStage = bootstrapResult.failedStage || null
      // Latch the failure so subsequent startClover() calls return this
      // same error without re-running install.ps1.  Cleared by the
      // clover:bootstrap:reset IPC (renderer's "Reload and retry").
    ...
    return ensureRuntime(resolveCloverBackend(backend.args))
  }
  ...
  if (!isCloverSourceRoot(ACTIVE_CLOVER_ROOT)) {
    throw new Error(
      `Clover install at ${ACTIVE_CLOVER_ROOT} is missing or incomplete. ` +
        'Reinstall via the desktop installer or scripts/install.ps1.'
    )
  }

  // On Windows, preflight Git Bash. Clover' terminal tool calls bash.exe
  ...
  // %LOCALAPPDATA%\clover\git\, which findGitBash() picks up, so for any
  // user who completed the bootstrap this is a no-op. For users who got
  // here via an external `clover` on PATH, this check still helps.
  if (IS_WINDOWS && !findGitBash()) {
    throw new Error(
      'Git for Windows is required for Clover on Windows (provides Git Bash, ' +
        "which the agent's terminal tool uses). Install it from " +
        'https://git-scm.com/download/win or run `winget install -e --id Git.Git`, ' +
        'then relaunch Clover.'
    )
  }
  ...
  if (!fileExists(venvPython)) {
    ...
    // normal flow because activeRuntimeState() requires isCloverSourceRoot()
    // plus an importable clover_cli before it hands back the active runtime.
    // If we hit this, the user (or a deleted venv) broke the invariant; tell
    // them to re-run the install.
    throw new Error(
      `Clover venv missing at ${VENV_ROOT}. Re-run the desktop installer or ` + '`scripts/install.ps1` to rebuild it.'
    )
  }

  backend.command = getVenvPython(VENV_ROOT)
  backend.label = `Clover at ${ACTIVE_CLOVER_ROOT} (venv: ${VENV_ROOT})`
  updateBootProgress({
    phase: 'runtime.ready',
    message: 'Clover runtime is ready',
    progress: 82,
    running: true,
    error: null
  })
...
function multipartBody(upload) {
  const boundary = `----clover-${crypto.randomBytes(12).toString('hex')}`
    ...
    (requestState: any) =>
      new Promise((resolve, reject) => {
        ...
        if (parsed.protocol !== 'http:' && parsed.protocol !== 'https:') {
          reject(new Error(`Unsupported Clover backend URL protocol: ${parsed.protocol}`))
                ...
                reject(
                  new Error(
                    `Expected JSON from ${url} but got HTML (status ${res.statusCode}). ` +
                      'The endpoint is likely missing on the Clover backend.'
                  )
                )
        ...
        req.setTimeout(timeoutMs, () => {
          req.destroy(new Error(`Timed out connecting to Clover backend after ${timeoutMs}ms`))
        })
  ...
  return new Promise((resolve, reject) => {
    ...
    const req = client.request(
      parsed,
      {
        agent,
        method: 'GET',
        headers: options.bearer ? { Authorization: `Bearer ${options.bearer}` } : { 'X-Clover-Session-Token': token }
      },
      res => {
...
function fetchPublicJson(url, options: any = {}) {
  ...
  // NO ``X-Clover-Session-Token`` header — used by the auth-mode probe before
...
function getLinkTitleSession() {
  ...
  linkTitleSession = session.fromPartition('clover:link-titles', { cache: false })
...
async function previewFileTarget(rawTarget, baseDir) {
  ...
  const base = baseDir ? path.resolve(expandUserPath(baseDir)) : resolveCloverCwd()
...
function sendPreviewFileChanged(payload) {
  ...
  webContents.send('clover:preview-file-changed', payload)
}
...
// waitForCloverReady how to read a 401 — rejected session vs gated route.
...
async function waitForClover(baseUrl, token, signal?, authMode?, headers = {}) {
  const { probeHealth, probeIsCredentialed } = await buildReadinessHealthProbe(baseUrl, authMode, token)

  return waitForCloverReady(baseUrl, {
    token,
    signal,
    fetchPublicJson,
...
function sendBackendExit(payload) {
  ...
  webContents.send('clover:backend-exit', payload)
}
...
function sendClosePreviewRequested() {
  ...
  webContents.send('clover:close-preview-requested')
}
...
function sendPreviewNavCommand(command: 'back' | 'forward' | 'reload') {
  ...
  webContents.send('clover:preview-nav', command)
}
...
function sendOpenFolderRequested() {
  ...
  webContents.send('clover:open-folder-requested')
}

// Tell the renderer the machine just woke. Sleep silently drops the
// renderer's WebSocket to the local backend; the renderer reconnects on this
// signal so the chat composer doesn't stay stuck on "Starting Clover...".
function sendPowerResume() {
  ...
  webContents.send('clover:power-resume')
}
...
// 'clover:power-battery' push below.
ipcMain.handle('clover:power-battery:get', () => onBatteryPower === true)
...
function broadcastBatteryState(next: boolean) {
  ...
  for (const win of BrowserWindow.getAllWindows()) {
    ...
    if (webContents && !webContents.isDestroyed()) {
      webContents.send('clover:power-battery', next)
    }
  }
}
...
function sendOpenUpdatesRequested() {
  ...
  webContents.send('clover:open-updates')
...
function sendWindowStateChanged(nextIsFullscreen?: boolean, target = mainWindow) {
  ...
  webContents.send('clover:window-state-changed', state)
}
...
function installContextMenuBridge(window: BrowserWindow) {
  window.webContents.on('context-menu', (_event, params) => {
    ...
    if (params.isEditable && params.misspelledWord) {
      window.webContents.send('clover:context-menu-spellcheck', {
...
//
// Hosted Clover gateways gate the dashboard behind an OAuth provider (e.g.
// Clover Cognition) instead of a static session token. The auth model is
// fundamentally different from the token path:
//
//   * REST is authed by HttpOnly session cookies (``clover_session_at``),
...
//   * Clover Portal now issues a 24h ROTATING, reuse-detected refresh token
//     alongside the ~15-min access token (Portal NAS #293 / clover #37247).
//     Both are set as HttpOnly cookies (``clover_session_at`` ~15 min,
//     ``clover_session_rt`` 24h). When the AT cookie lapses but the RT cookie
...
// "not signed in", which on the initial boot path (startClover → the renderer's
// single-shot boot() with no retry) surfaced as the "Clover couldn't start"
...
async function hasLiveOauthSession(baseUrl) {
  ...
  // for a signed-in user — the exact race that produced the transient "Clover
  ...
  return new Promise((resolve, reject) => {
    ...
    try {
      win = new BrowserWindow({
        width: 520,
        height: 720,
        title: silent ? 'Connecting to Clover Cloud agent…' : 'Sign in to Clover gateway',
...
function fetchJsonViaOauthSession(url, options: any = {}) {
  return new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(() => {
      ...
      reject(new Error(`Timed out connecting to Clover backend after ${timeoutMs}ms`))
    }, timeoutMs)
...
// feature; the server half lives in clover_cli/dashboard_auth/native_flow.py.
// ---------------------------------------------------------------------------
...
function readDesktopConnectionsRegistry() {
  ...
  if (mtime === null) {
    ...
    try {
      ...
      // hard-failing every clover:connections:* call.
...
// a valid stored value (pins the root CLOVER_HOME explicitly); null means "no
...
async function sanitizeDesktopConnectionConfig(config = readDesktopConnectionConfig(), profile = null) {
  ...
  const envOverride = key ? false : Boolean(process.env.CLOVER_DESKTOP_REMOTE_URL)
  ...
  const remoteUrl = envOverride ? String(process.env.CLOVER_DESKTOP_REMOTE_URL || '') : String(block.url || '')
  ...
  return {
    ...
    sshRemoteCloverPath: (ssh || savedSsh)?.remoteCloverPath || '',
    sshRemoteProfile: (ssh || savedSsh)?.remoteProfile || '',
    // The env override only forces the global/primary connection; a per-profile
    // scope is never overridden by CLOVER_DESKTOP_REMOTE_URL.
    envOverride
  }
}
...
function buildSshBlock(input: any, existingBlock: any = {}) {
  ...
  const merged = normalizeSshConfig({
    ...
    remoteCloverPath: input.sshRemoteCloverPath ?? existingBlock.remoteCloverPath,
...
async function buildRemoteConnection(
  ...
  if (authMode === 'oauth') {
    ...
    // refresh token (clover #37247), and the gateway middleware transparently
    ...
    try {
      ...
      throw gatewayTicketFailure(
        ...
        'Could not reach the remote Clover gateway while refreshing its WebSocket ticket. Try reconnecting.'
      )
    }
  ...
  if (!token) {
    throw new Error(
      'Remote Clover gateway is selected, but no session token is saved. ' +
...
function activeSshTerminalTarget(webContentsId?: number) {
  ...
  const route = resolveDesktopRemoteRoute({
    config,
    env: {
      token: process.env.CLOVER_DESKTOP_REMOTE_TOKEN,
      url: process.env.CLOVER_DESKTOP_REMOTE_URL
    },
    profile,
    registry: readDesktopConnectionsRegistry()
  })
...
async function rollbackSshBootstrapResult(ssh, result, profile, sshConfig, boundaryError) {
  ...
  try {
    const expected = {
      ...
      cloverPath: result.cloverPath,
      cloverHome: result.cloverHome,
    ...
    if (result.platform?.os === 'Windows') {
      await terminateOwnedWindowsDashboardForUpdate(
        ssh,
        { cloverPath: result.cloverPath, cloverHome: result.cloverHome, python: result.pythonPath },
        expected
      )
...
async function bootstrapSshConnectionInner(profile, sshConfig, reuseToken, source, metadata, fingerprint, lease) {
  ...
  try {
    ...
    const platform = await detectRemotePlatform(ssh, sshConfig.remoteCloverPath || '')
    const lifecycle = platform.os === 'Windows' ? connectWindowsRemote : remoteLifecycle.connect
    result = await lifecycle({
      ssh,
      profile: resolveRemoteSshDashboardProfile(sshConfig.remoteProfile, profile),
      remoteCloverPath: sshConfig.remoteCloverPath || '',
      ownershipId: sshOwnershipKey(profile),
      ...
      pickLocalPort,
      waitForClover: (baseUrl, token) => waitForClover(baseUrl, token, lease.signal, 'token'),
  ...
  await fenceManagedSshBootstrapPublication({
    ...
    publish: () => {
      ...
      sshConnections.set(scope, {
        ...
        host: sshConfig.host,
        hostLabel,
        cloverVersion: result.cloverVersion || '',
        ...
        // per-profile pools also use bare keys. Only startClover' explicit call
  ...
  sshRememberLog(
    `[ssh] connection ${result.reused ? 'REUSED' : 'spawned'} dashboard: ` +
      `${result.cloverVersion || 'clover (version unknown)'} at ${result.cloverPath || '?'}`
  )
  ...
  return {
    ...connection,
    remoteCloverVersion: result.cloverVersion || '',
    ssh: {
      effectiveConfigFingerprint: sshConfig.effectiveConfigFingerprint,
      host: sshConfig.host,
...
//   2. env override (CLOVER_DESKTOP_REMOTE_URL/_TOKEN) — applies app-wide
...
function globalRemoteActive() {
  if (process.env.CLOVER_DESKTOP_REMOTE_URL) {
    return true
  }
...
// env → global) so a startClover() failure can be classified as remote (never
...
async function probeRemoteAuthMode(rawUrl) {
  ...
  // sending any credentials. ``/api/status`` is public on every Clover
  ...
  if (authRequired) {
    ...
    // button can read "Sign in with Clover Cognition" instead of a generic
...
async function testDesktopConnectionConfig(input: any = {}) {
  if (input.mode === 'ssh') {
    const sshConfig = normalizeSshConfig({
      ...
      remoteCloverPath: input.sshRemoteCloverPath
    })
    ...
    try {
      ...
      for (;;) {
        try {
          await ssh.open()
          const platform: any = await detectRemotePlatform(ssh, sshConfig.remoteCloverPath || '')
          let cloverPath
          let cloverVersion
          let supported
          ...
          if (platform.os === 'Windows') {
            ...
            cloverPath = runtime.cloverPath
            const inspection = await helper(ssh, runtime, 'inspect', [runtime.cloverPath])
            cloverVersion = inspection.version
            supported = inspection.supported
          } else {
            cloverPath = await remoteLifecycle.locateClover(ssh, sshConfig.remoteCloverPath || '')
            cloverVersion = await remoteLifecycle.probeCloverVersion(ssh, cloverPath)
            supported = await remoteLifecycle.remoteSupportsSshOwnership(ssh, cloverPath)
          }
          ...
          if (!supported) {
            return {
              ...
              error: 'Update Clover on the remote host before connecting with Desktop SSH.'
            }
          }
          ...
          return {
            ...
            remoteCloverPath: cloverPath,
            remoteCloverVersion: cloverVersion,
  ...
  if (wantRemote && block?.url) {
    ...
    const remote = (await resolveRemoteBackend(key)) || (await startClover())
  ...
  // connect to Clover gateway". Mirror the renderer's connect here so the test
...
function resetCloverConnection({ soft = false } = {}) {
  backendStartFailure = null
  remoteReauthFailure = null
  remoteLiveness.clear()
  const cloverProcess = backendConnectionState.invalidate()
  stopBackendChild(cloverProcess)
...
// startClover() spawns fresh instead of racing the dying one. Shared by the
// connection-config and profile switch flows.
async function teardownPrimaryBackendAndWait({ soft = false } = {}) {
  // Capture the reference before resetCloverConnection() invalidates it.
  const cloverProcess = backendConnectionState.getProcess()
  const dying = cloverProcess && !cloverProcess.killed ? cloverProcess : null
...
function sendConnectionApplied() {
  ...
  webContents.send('clover:connection:applied')
}
...
function broadcastConnectionsChanged(payload: { connectionId: string; reason: 'removed' | 'saved' | 'updated' }) {
  for (const win of BrowserWindow.getAllWindows()) {
    ...
    if (webContents && !webContents.isDestroyed()) {
      webContents.send('clover:connections:changed', payload)
    }
  }
}
...
async function ensureBackend(profile) {
  ...
  if (route.backend === 'primary') {
    const connection = await startClover()
  ...
  // spawning, so two children never share one profile's CLOVER_HOME.
  ...
  entry.connectionPromise = spawnPoolBackend(key, entry).catch(async error => {
    ...
    rememberLog(
      `Clover backend for profile "${key}" failed to start: ${error instanceof Error ? error.message : String(error)}`
    )
...
async function ensureRegistryBackend(connectionId, profile, managedUpdateCorrelation = '') {
  ...
  const resolveRegistrySshConfig = () => {
    ...
    if (!resolvedRegistrySshConfig) {
      resolvedRegistrySshConfig = normalizeSshConfig({
        ...
        remoteCloverPath: source.remoteCloverPath,
  ...
  if (source.kind === 'local') {
    ...
    localEntry.connectionPromise = spawnPoolBackend(profileKey, localEntry, {
      ...
      rememberLog(
        `Clover backend for profile "${profileKey}" (forced-local) failed to start: ${error instanceof Error ? error.message : String(error)}`
      )
...
async function connectRegistryBackend(
  ...
  if (source.kind === 'ssh') {
    ...
    return {
      ...
      // is only the routing label. clover:api uses it to translate explicit
      // self-profile query filters into the backend's namespace.
      remoteProfile: sshConfig.remoteProfile || '',
      logs: cloverLog.slice(-80),
      ...getWindowState()
    }
  }
  ...
  await waitForClover(connection.baseUrl, connection.token, undefined, connection.authMode, connection.headers)
  poolEntry.remoteBaseUrl = connection.baseUrl
...
function remoteUpdateTargetFromState(state): RemoteUpdateTarget {
  if (!state?.ssh || !state?.cloverPath || !state?.cloverHome) {
  ...
  return {
    ...
    cloverPath: state.cloverPath,
    cloverHome: state.cloverHome,
...
async function openManagedSshUpdateTransport(
  ...
  try {
    const platform: any = await detectRemotePlatform(ssh, config.remoteCloverPath || '')

    if (platform.os === 'Windows') {
      const runtime = platform.cloverPath ? platform : await probeWindowsRemote(ssh, config.remoteCloverPath || '')
      ...
      return {
        ...
        target: {
          ...
          platform: 'Windows',
          cloverPath: runtime.cloverPath,
          cloverHome: runtime.cloverHome,
          pythonPath: runtime.python
        }
      }
    }

    const cloverPath = await remoteLifecycle.locateClover(ssh, config.remoteCloverPath || '')
    const cloverHome = await remoteLifecycle.probeRemoteCloverHome(ssh)

    return {
      close: () => ssh.close(),
      target: { ssh, platform: platform.os, cloverPath, cloverHome }
    }
  } catch (error) {
    await ssh.close()
    throw error
...
async function drainManagedSshScope(scope) {
  ...
  try {
    ...
    if (state.remotePlatform === 'Windows') {
      await terminateOwnedWindowsDashboardForUpdate(
        state.ssh,
        { cloverPath: state.cloverPath, cloverHome: state.cloverHome, python: state.pythonPath },
        expected
      )
    ...
    if (state && state.localPort && state.remotePort && scope.reuseToken) {
      try {
        ...
        if (!forwardClosed) {
          await waitForClover(`http://127.0.0.1:${state.localPort}`, scope.reuseToken, undefined, 'token')
        } else {
...
// local-spawn portion of startClover() but without the boot-progress UI,
...
async function spawnPoolBackend(profile, entry, opts: { forceLocal?: boolean; poolKey?: string } = {}) {
  ...
  if (remote) {
    await waitForClover(remote.baseUrl, remote.token, undefined, remote.authMode, remote.headers)
  ...
  // --profile wins over the inherited CLOVER_HOME env (see _apply_profile_override
  // step 3 in clover_cli/main.py), so the child re-homes to this profile.
  // --port 0: the OS assigns an ephemeral port; the child announces it on stdout.
  const backendArgs = ['--profile', profile, 'serve', '--host', '127.0.0.1', '--port', '0']
  const backend = await ensureRuntime(resolveCloverBackend(backendArgs))
  // Route old runtimes (no `serve`) through the legacy `dashboard --no-open`.
  backend.args = getBackendArgsForRuntime(backend)
  const cloverCwd = resolveCloverCwd()
  ...
  assertLocalProfileCanStart(profile, profileDeletionGate, key =>
    directoryExists(path.join(CLOVER_HOME, 'profiles', key))
  )

  rememberLog(`Starting Clover backend for profile "${profile}" via ${backend.label}`)
  ...
  const child = spawn(
    ...
    hiddenWindowsChildOptions({
      ...
      env: {
        ...
        TERMINAL_CWD: cloverCwd,
        CLOVER_DASHBOARD_SESSION_TOKEN: token,
        ...
        ...parentIdentityEnv,
        CLOVER_WEB_DIST: webDist,
        ...(readyFile ? { CLOVER_DESKTOP_READY_FILE: readyFile } : {})
      },
      shell: backend.shell,
      stdio: ['ignore', 'pipe', 'pipe']
    })
  ...
  child.once('error', error => {
    rememberLog(`Clover backend for profile "${profile}" failed to start: ${error.message}`)
    releaseBackendChild(child)
    backendPool.delete(poolKey)
    rejectStart?.(error)
  })
  child.once('exit', (code, signal) => {
    rememberLog(`Clover backend for profile "${profile}" exited (${signal || code})`)
    releaseBackendChild(child)
    backendPool.delete(poolKey)
    ...
    if (!ready) {
      rejectStart?.(
        new Error(
          `Clover backend for profile "${profile}" exited before it became ready (${signal || code}).${outputTail.describe()}`
        )
      )
    }
  })
  ...
  await Promise.race([waitForClover(baseUrl, token), startFailed])
  ready = true

  const authToken = await adoptServedDashboardToken(baseUrl, token, {
    childAlive: () => child.exitCode === null && !child.killed,
    label: `Clover backend for profile "${profile}"`,
    rememberLog
  })
  ...
  if (!wsProbe.ok) {
    throw new Error(
      `Clover backend for profile "${profile}" is HTTP-reachable but the WebSocket (/api/ws) rejected the session token: ${wsProbe.reason}`
    )
  }
...
// backend whose ensure_clover_home() recreates the deleted profile directory.
//
...
async function startClover() {
  ...
  if (!isPrimaryInstance) {
    ...
    throw new Error('Clover Desktop is already running in another window.')
  }

  await reapOrphanedBackendsOnce()

  // Latched-failure short-circuit: once bootstrap has failed in this
  // process, every subsequent startClover() call re-throws the same error
  ...
  if (BOOT_FAKE_ERROR) {
    await advanceBootProgress('backend.resolve', 'Resolving Clover backend', 8)
  ...
  const connectionPromise = (async () => {
    const connectRemote = async remote => {
      ...
      if (!backendConnectionState.isCurrentAttempt(connectionAttempt)) {
        throw new Error('Clover backend start was superseded by a newer connection attempt.')
      }

      await advanceBootProgress('backend.remote', `Connecting to remote Clover backend at ${remote.baseUrl}`, 24)
      await waitForClover(remote.baseUrl, remote.token, undefined, remote.authMode, remote.headers)
      ...
      updateBootProgress({
        phase: 'backend.ready',
        message: 'Remote Clover backend is ready',
        progress: 94,
        running: true,
        error: null
      })

      return createPrimaryRemoteConnection(remote, cloverLog.slice(-80), getWindowState())
    }
    ...
    // deterministic (it wins over the sticky ~/.clover/active_profile file) and
    // resolves CLOVER_HOME the same way `clover -p <name>` does on the CLI. An
    ...
    const setup = await runPrimaryBackendStartup({
      ...
      prepareLocalBackend: async () => {
        await advanceBootProgress('backend.runtime', 'Resolving Clover runtime', 28)

        return resolveCloverBackend(backendArgs)
      },
      resolveRemote: () => {
    ...
    await advanceBootProgress('backend.spawn', `Starting Clover backend via ${backend.label}`, 84)
    rememberLog(`Starting Clover backend via ${backend.label}`)
    ...
    const cloverProcess = spawn(
      ...
      hiddenWindowsChildOptions({
        ...
        env: {
          ...process.env,
          // Explicitly pin CLOVER_HOME for the child so Python's get_clover_home()
          // resolves to the SAME location our resolveCloverHome() picked. Without
          // this pin, Python falls back to ~/.clover on every platform — fine on
          // mac/linux (where our default matches), but on Windows our default is
          // %LOCALAPPDATA%\clover, which differs from C:\Users\<u>\.clover.
          // Mismatch would split config / sessions / .env / logs across two
          // directories. install.ps1 sets CLOVER_HOME via setx; the desktop
    ...
    primaryOutputTail.attach(cloverProcess)
    ...
    const processOwner = backendConnectionState.attachProcess(connectionAttempt, cloverProcess)

    if (!processOwner) {
      stopBackendChild(cloverProcess)
      await waitForBackendExit(cloverProcess)
      releaseBackendChild(cloverProcess)
      throw new Error('Clover backend start was superseded by a newer connection attempt.')
    }

    cloverProcess.stdout.on('data', rememberLog)
    cloverProcess.stderr.on('data', rememberLog)
    ...
    cloverProcess.once('error', error => {
      releaseBackendChild(cloverProcess)

      if (!backendConnectionState.clearForCurrentProcess(processOwner)) {
        rememberLog(`Ignoring stale Clover backend error: ${error.message}`)
        rejectBackendStart?.(new Error('Clover backend start was superseded by a newer connection attempt.'))

        return
      }

      rememberLog(`Clover backend failed to start: ${error.message}`)
      updateBootProgress(
        {
          error: error.message,
          message: `Clover backend failed to start: ${error.message}`,
          phase: 'backend.error',
          running: false
        },
        { allowDecrease: true }
    ...
    cloverProcess.once('exit', (code, signal) => {
      releaseBackendChild(cloverProcess)

      if (!backendConnectionState.clearForCurrentProcess(processOwner)) {
        rememberLog(`Ignoring stale Clover backend exit (${signal || code})`)
      ...
      rememberLog(`Clover backend exited (${signal || code})`)
      sendBackendExit({ code, signal })

      if (!backendReady) {
        const message = `Clover backend exited before it became ready (${signal || code}).${primaryOutputTail.describe()}`
        updateBootProgress(
          {
            error: message,
            message,
        ...
        rejectBackendStart?.(
          new Error(
            `Clover backend exited before it became ready (${signal || code}). Log: ${DESKTOP_LOG_PATH}\n${recentCloverLog()}`
          )
        )
      }
    })

    await advanceBootProgress('backend.port', 'Waiting for Clover backend to launch', 86)

    // Discover the ephemeral port the child bound to
    const port = await Promise.race([
      waitForDashboardPortAnnouncement(cloverProcess, {
    ...
    const baseUrl = `http://127.0.0.1:${port}`
    await advanceBootProgress('backend.wait', 'Waiting for Clover backend to become ready', 90)
    await Promise.race([waitForClover(baseUrl, token), backendStartFailed])
    backendReady = true
    backendStartFailure = null

    const authToken = await adoptServedDashboardToken(baseUrl, token, {
      childAlive: () => cloverProcess.exitCode === null && !cloverProcess.killed,
      rememberLog
    })
    ...
    if (!wsProbe.ok) {
      throw new Error(
        `Local Clover backend is HTTP-reachable but the WebSocket (/api/ws) rejected the session token: ${wsProbe.reason}`
      )
    }

    updateBootProgress({
      phase: 'backend.ready',
      message: 'Clover backend is ready. Finalizing desktop startup',
      progress: 94,
      running: true,
      error: null
    })
...
function notifyBrowserPopoutClosed(tabId) {
  ...
  for (const other of BrowserWindow.getAllWindows()) {
    if (!other.isDestroyed()) {
      other.webContents.send('clover:browser-popout:closed', tabId)
    }
  }
}
...
// here so it can leave the app's bounds and stay visible while Clover is
...
// pushes pet state over IPC (clover:pet-overlay:state); the overlay just renders
// it. Control flows back (pop-in, composer submit) via clover:pet-overlay:control.
...
function spawnPetOverlayWindow(bounds) {
  const win = new BrowserWindow({
    ...
    // instead, leaving the main Clover app as the Dock/cmd-tab anchor.
    skipTaskbar: !IS_MAC,
    hasShadow: false,
    alwaysOnTop: true,
    ...
    // anchor and the Clover icon drops out of cmd/alt-tab — especially when the
    // main window is minimized. We flip this on only while the composer needs
    // the keyboard (see clover:pet-overlay:set-focusable).
    focusable: false,
    show: false,
  ...
  try {
    ...
    // which briefly hides the Dock/cmd-tab presence. Keep Clover in the normal
  ...
  win.on('closed', () => {
    ...
    if (mainWindow && !mainWindow.isDestroyed()) {
      mainWindow.webContents.send('clover:pet-overlay:control', { type: 'pop-in' })
    }
  })
...
// window showing only the composer and its scrollback, so Clover can be driven
...
function startHudCursorFeed(win: BrowserWindow) {
  ...
  const timer = setInterval(() => {
    ...
    last = key
    win.webContents.send('clover:hud:cursor', point)
  }, HUD_CURSOR_POLL_MS)
...
function startHudGameOverlayFeed(win: BrowserWindow) {
  ...
  const push = (state: { active: boolean; app: string }) => {
    if (!win.isDestroyed()) {
      win.webContents.send('clover:hud:game-overlay', state)
    }
  }
  ...
  const enumerate = async () => {
    ...
    if (!reported) {
      reported = true
      console.warn(`[clover] HUD cannot enumerate windows: ${windows.reason}`)
    }
...
function broadcastHudState(open) {
  ...
  for (const win of BrowserWindow.getAllWindows()) {
    if (!win.isDestroyed()) {
      win.webContents.send('clover:hud:changed', payload)
    }
  }
}
...
function spawnHudWindow(sessionId, profile) {
  const win = new BrowserWindow({
    ...
    // `clover:hud:set-bounds`, which flips resizable on for the call — the
  ...
  // grab, clover:hud:workspace-transfer temporarily makes the X11 window
...
function openHudWindow(sessionId, profile) {
  ...
  if (hudWindow && !hudWindow.isDestroyed()) {
    ...
    if (sessionId && sessionId !== hudSessionId) {
      ...
      hudWindow.webContents.send('clover:hud:goto', sessionId)
...
function spawnQuickEntryWindow() {
  ...
  win.webContents.on('did-finish-load', () => {
    if (!win.isDestroyed() && quickEntryLastState) {
      win.webContents.send('clover:quick-entry:state', quickEntryLastState)
    }
  })
...
function showQuickEntryWindow() {
  ...
  quickEntryWindow.webContents.send('clover:quick-entry:shown')
}
...
function createWindow() {
    ...
    callbacks: {
      ...
      onFailedLoadBudgetExhausted: details => {
        ...
        void loadRendererLoadErrorPage(mainWindow, {
          ...
          repairHint: 'clover desktop --force-build',
  ...
  startClover().catch(error => rememberLog(error.stack || error.message))
...
ipcMain.handle('clover:connection', async (_event, profile) => {
...
ipcMain.handle('clover:connection:for', async (_event, payload) => {
  const { connectionId, profile } = payload && typeof payload === 'object' ? (payload as any) : ({} as any)
  const registry = readDesktopConnectionsRegistry()
  const id = String(connectionId || '').trim() || registry.primary
  // Same single-owner claim as 'clover:connection', keyed by the composite
...
ipcMain.on('clover:connection:active-route', (event, route) => {
...
// "Starting Clover…". Before the renderer's backoff loop reconnects, it asks us
// to confirm the cached PRIMARY backend is still reachable; if a remote one is
// not, we drop the cache so the next getConnection() rebuilds it. Local backends
// self-heal via their child 'exit' handler, so we never touch them here.
ipcMain.handle('clover:connection:revalidate', async () => {
  ...
  return remoteRevalidation.run(connectionPromise, async () => {
    const [result] = await Promise.all([
      revalidateRemoteConnection({
        ...
        resetConnection: () => resetCloverConnection({ soft: true }),
...
ipcMain.handle('clover:backend:touch', async (_event, profile) => {
  touchPoolBackend(profile)

  return { ok: true }
})
ipcMain.handle('clover:gateway:ws-url', async (_event, profile) => {
  return gatewayWsUrlIpcResult(() => freshGatewayWsUrl(profile))
})
ipcMain.handle('clover:window:openSession', async (_event, sessionId, opts) => {
...
ipcMain.handle('clover:window:openInstance', async () => {
  createInstanceWindow()

  return { ok: true }
})
ipcMain.handle('clover:window:openBrowser', async (_event, tabId) => {
...
ipcMain.handle('clover:window:openInTerminal', async (_event, sessionId, opts) => {
  ...
  try {
    ...
    const backend = resolveCloverBackend(tuiResumeArgs(sessionId.trim(), profile || undefined))

    if (!backend.command) {
      return { ok: false, error: 'Clover is not installed yet' }
    }
    ...
    const scriptPath = path.join(
      scriptDir,
      `clover-${crypto.randomBytes(6).toString('hex')}${terminalScriptExtension()}`
    )
    ...
    fs.writeFileSync(
      ...
      buildTerminalScript({
        ...
        cwd,
        env: terminalScriptEnv(backend.env, CLOVER_HOME)
      }),
      { mode: 0o700 }
    )
...
ipcMain.handle('clover:wake-indicator:get', () => wakeIndicatorController.getState())
ipcMain.on('clover:wake-indicator:set', (_event, state) => {
...
ipcMain.handle('clover:zoom:get', event => {
...
ipcMain.on('clover:zoom:set-percent', (event, percent) => {
  const window = BrowserWindow.fromWebContents(event.sender)
...
ipcMain.handle('clover:bootstrap:reset', async () => {
  // Renderer's "Reload and retry" path. Clear the latched failure and
  // reset connection state so the next startClover() call restarts the
...
ipcMain.handle('clover:bootstrap:repair', async () => {
  // Forceful repair: force the next startClover() through the full installer
  ...
  // "bypass active runtime" path inside startClover() and fall through
...
ipcMain.handle('clover:boot-progress:get', async () => bootProgressState)
ipcMain.handle('clover:bootstrap:get', async () => getBootstrapState())
ipcMain.handle('clover:connection-config:get', async (_event, profile) =>
  sanitizeDesktopConnectionConfig(readDesktopConnectionConfig(), profile)
)
ipcMain.handle('clover:plugin-profile-routes', async (_event, rawProfileNames) => {
...
ipcMain.handle('clover:ssh-config:hosts', async () => ({ hosts: collectSshConfigHosts() }))
ipcMain.handle('clover:ssh-config:resolve', async (_event, host) => {
...
ipcMain.handle('clover:connection-config:test', async (_event, payload) => testDesktopConnectionConfig(payload))
...
ipcMain.handle('clover:secret-storage:get', async () => ({ on: secretStoragePolicy().on }))
ipcMain.handle('clover:secret-storage:set', async (_event: any, on: any) => applySecretStorageEncryption(on === true))
...
ipcMain.handle('clover:connections:list', async () => sanitizeConnectionsRegistry())
ipcMain.handle('clover:connections:save', async (_event, payload) => {
  const saved = await saveRegistryConnection(payload)

  return { ok: true, connection: saved, registry: sanitizeConnectionsRegistry() }
})
ipcMain.handle('clover:connections:remove', async (_event, id) => {
  const key = String(id || '')
...
ipcMain.handle('clover:connections:set-primary', async (_event, id) => {
...
ipcMain.handle('clover:connections:set-launch-mode', async (_event, mode) => {
  assertCanMutateManagedPrimaryRouting()
...
ipcMain.handle('clover:connections:set-last-used', async (_event, id) => {
  const registry = setLastUsedConnection(readDesktopConnectionsRegistry(), String(id || ''))
  writeDesktopConnectionsRegistry(registry)

  return { ok: true, registry: sanitizeConnectionsRegistry(registry) }
})
ipcMain.handle('clover:connections:test', async (_event, id) => {
  const registry = readDesktopConnectionsRegistry()
  ...
  if (entry.kind === 'ssh') {
    const result = await testDesktopConnectionConfig({
      ...
      sshRemoteCloverPath: entry.remoteCloverPath
    })
...
async function probeSshProfileInventory(connection) {
  ...
  const sshConfig = normalizeSshConfig({
    ...
    remoteCloverPath: connection.remoteCloverPath
  })
  ...
  try {
    await ssh.open()
    const profiles = await remoteLifecycle.listRemoteCloverProfiles(ssh)
    ...
    registry.connections.map(async connection => {
      ...
      try {
        ...
        if (connection.kind === 'ssh') {
          ...
          // The root CLOVER_HOME is an agent too; enumerations that omit it
...
ipcMain.handle('clover:agents:roster', async () => {
  const registry = readDesktopConnectionsRegistry()
  const enumerations = await enumerateRegistryAgentSources(registry)
...
// clover:gateway:ws-url. Same single-use-ticket discipline for OAuth sources.
...
ipcMain.handle('clover:gateway:ws-url-for', async (_event, payload) => {
    ...
    registry.connections
      ...
      .map(async connection => {
        ...
        try {
          ...
          const body: any = await postJsonForBackend(descriptor, '/api/clover/update', {}, { timeoutMs: 15_000 })
...
// analogue of the clover:api handler's own auth split: OAuth backends prefer a
...
ipcMain.handle('clover:connection-config:probe', async (_event, rawUrl) => probeRemoteAuthMode(rawUrl))
ipcMain.handle('clover:connection-config:oauth-login', async (_event, rawUrl) => {
  ...
  if (strategy === 'native') {
    try {
      ...
      // startClover() re-dials instead of replaying the stale rejection.
      remoteReauthFailure = null
...
ipcMain.handle('clover:connection-config:oauth-logout', async (_event, rawUrl) => {
  const baseUrl = normalizeRemoteBaseUrl(rawUrl)
...
ipcMain.handle('clover:connection-config:apply', async (_event, payload) => {
  assertCanMutateManagedPrimaryRouting()
...
ipcMain.handle('clover:profile:get', async () => ({ profile: readActiveDesktopProfile() }))
// Persistence-only sibling of clover:profile:set: records the profile the
...
// the gateway (#79886).
ipcMain.handle('clover:profile:remember', async (_event, name) => ({
  profile: writeActiveDesktopProfile(name)
}))
ipcMain.handle('clover:profile:set', async (_event, name) => {
  assertCanMutateManagedPrimaryRouting()
  const next = writeActiveDesktopProfile(name)

  // Switching profiles is a backend re-home: relaunch the dashboard under the
  // new CLOVER_HOME. Pool backends keep their own homes, so only the primary
  // is torn down.
  await teardownPrimaryBackendAndWait()
  mainWindow?.reload()
...
ipcMain.on('clover:previewShortcutActive', (_event, active) => {
  previewShortcutActive = Boolean(active)
})

ipcMain.handle('clover:requestMicrophoneAccess', async () => {
...
ipcMain.handle('clover:window:readBelow', async event => {
...
async function handleCloverApiRequest(request) {
  ...
  // backend calls ensure_clover_home() which recreates the profile directory,
...
ipcMain.handle('clover:api', async (_event, request) => {
  ...
  // respawn the old-name backend and recreate its CLOVER_HOME (#45474).
  ...
  if (!mutatingProfile) {
    return handleCloverApiRequest(request)
  }

  const releaseProfileDeletion = profileDeletionGate.acquire(mutatingProfile)

  return handleCloverApiRequest(request).finally(releaseProfileDeletion)
})
...
ipcMain.handle('clover:ambient:claim', (_event, key) => !claimedAmbientCue(String(key ?? '')))

ipcMain.handle('clover:notify', (_event, payload) => {
  ...
  notification.on('click', () => {
    ...
    if (payload?.sessionId) {
      mainWindow.webContents.send('clover:focus-session', payload.sessionId)
    }

    // Plugin / session-less activation — serializable path (+ optional notifyId
    // for renderer callbacks). Same vocabulary as clover://index-network/….
    if (payload?.activate || payload?.notifyId) {
      mainWindow.webContents.send('clover:notification-activate', {
  ...
  notification.on('action', (_actionEvent, index) => {
    ...
    if (payload?.sessionId && !payload?.notifyId && !payload?.activate) {
      mainWindow.webContents.send('clover:notification-action', { sessionId: payload.sessionId, actionId: action.id })
...
ipcMain.handle('clover:data-url-read-max:get', () => ({
...
ipcMain.handle('clover:data-url-read-max:set', (_event, maxMb) => {
...
ipcMain.handle('clover:readFileDataUrl', async (_event, filePath) => {
...
ipcMain.handle('clover:readFileDataUrlForAttach', async (_event, filePath) => {
  return readFileDataUrlForIpc(filePath, {
...
ipcMain.handle('clover:readFileText', async (_event, filePath) => {
...
// `clover:readFileText` is the *preview* read and silently truncates at
...
ipcMain.handle('clover:readPluginSource', async (_event: unknown, filePath: unknown) => {
  const { resolvedPath, stat } = await resolveReadableFileForIpc(filePath, {
...
ipcMain.handle('clover:selectPaths', async (_event, options: any = {}) => {
...
ipcMain.handle('clover:writeClipboard', (_event, text) => {
...
ipcMain.handle('clover:selectSavePath', async (_event, options: any = {}) => {
  const result = await dialog.showSaveDialog(mainWindow, {
...
ipcMain.handle('clover:readClipboard', () => clipboard.readText())

ipcMain.handle('clover:saveGatewayFile', (_event, payload) => saveGatewayFile(payload))

ipcMain.handle('clover:saveImageFromUrl', (_event, url) => saveImageFromUrl(String(url || '')))
...
ipcMain.handle('clover:context-menu:edit', (event, command) => {
...
ipcMain.handle('clover:context-menu:copy-image', event => {
...
ipcMain.handle('clover:context-menu:spellcheck', (event, action) => {
...
ipcMain.handle('clover:context-menu:guest-add-word', (_event, payload) => {
...
ipcMain.handle('clover:saveImageBuffer', async (_event, payload) => {
...
ipcMain.handle('clover:saveClipboardImage', async () => {
...
ipcMain.handle('clover:normalizePreviewTarget', (_event, target, baseDir) =>
  normalizePreviewTarget(String(target || ''), baseDir ? String(baseDir) : '')
)

ipcMain.handle('clover:watchPreviewFile', (_event, url) => watchPreviewFile(String(url || '')))

ipcMain.handle('clover:watchDirectory', (_event, dir) => watchDirectory(String(dir || '')))

ipcMain.handle('clover:stopPreviewFileWatch', (_event, id) => stopPreviewFileWatch(String(id || '')))
...
ipcMain.on('clover:active-work', (event, payload) => {
  const id = event.sender.id
...
ipcMain.on('clover:titlebar-theme', (_event, payload) => {
...
ipcMain.on('clover:native-theme', (_event, mode) => {
...
ipcMain.on('clover:translucency:support', event => {
  event.returnValue = { glass: GLASS_SUPPORTED, translucency: TRANSLUCENCY_SUPPORTED }
})

ipcMain.on('clover:translucency', (_event, payload) => {
...
ipcMain.on('clover:keep-awake', (_event, on) => {
...
ipcMain.handle('clover:quick-entry:settings:get', async () => {
...
ipcMain.handle('clover:quick-entry:settings:set', async (_event, patch) => {
...
ipcMain.on('clover:quick-entry:submit', (_event, payload) => {
  hideQuickEntryWindow()
  ...
  mainWindow.webContents.send('clover:quick-entry:submit', {
...
ipcMain.on('clover:quick-entry:state', (_event, payload) => {
  quickEntryLastState = payload ?? null

  if (quickEntryWindow && !quickEntryWindow.isDestroyed()) {
    quickEntryWindow.webContents.send('clover:quick-entry:state', payload)
  }
})

ipcMain.on('clover:quick-entry:dismiss', () => hideQuickEntryWindow())
...
ipcMain.on('clover:devtools:disable-f12', (_event, on) => {
...
ipcMain.handle('clover:openExternal', (_event, url) => {
...
// per-session secondary windows spawned via `clover:window:openSession`).
...
ipcMain.handle('clover:find-in-page', async (event, query, options) => {
  const win = BrowserWindow.fromWebContents(event.sender)
...
ipcMain.handle('clover:stop-find-in-page', event => {
  const win = BrowserWindow.fromWebContents(event.sender)
...
ipcMain.handle('clover:preview:reach', async (event, url) => reachablePreviewUrl(event.sender.id, String(url || '')))

ipcMain.handle('clover:openPreviewInBrowser', async (_event, url) => {
...
// it via writeDefaultProjectDir so resolveCloverCwd picks it up on the next
// session spawn (no app restart needed).
ipcMain.handle('clover:setting:defaultProjectDir:get', async () => ({
  dir: readDefaultProjectDir(),
  defaultLabel: app.getPath('home'),
  resolvedCwd: resolveCloverCwd()
}))

ipcMain.handle('clover:workspace:sanitize', async (_event, cwd) => sanitizeWorkspaceCwd(cwd))

ipcMain.handle('clover:setting:defaultProjectDir:set', async (_event, dir) => {
...
ipcMain.handle('clover:setting:defaultProjectDir:pick', async () => {
...
ipcMain.handle('clover:fetchLinkTitle', (_event, url) => fetchLinkTitle(url))

ipcMain.handle('clover:resolveFavicon', (_event, url) => resolveFaviconCached(url))

ipcMain.handle('clover:logs:reveal', async () => {
  try {
    await fs.promises.mkdir(path.dirname(DESKTOP_LOG_PATH), { recursive: true })
...
ipcMain.handle('clover:logs:recent', async () => ({ path: DESKTOP_LOG_PATH, lines: cloverLog.slice(-200) }))
...
ipcMain.on('clover:logs:renderer-error', (_event, report) => {
...
// Embedded terminal PTY host (clover:terminal:*) — see terminal-ipc.ts.
...
ipcMain.handle('clover:updates:check', async () =>
...
ipcMain.handle('clover:updates:apply', async (_event, payload) =>
...
ipcMain.handle('clover:updates:branch:get', async () => readDesktopUpdateConfig())

ipcMain.handle('clover:updates:branch:set', async (_event, name) => {
...
// Resolve the canonical Clover version (the one `release.py` bumps in
// clover_cli/__init__.py + pyproject.toml) so the desktop About panel shows the
// real Clover version instead of the Electron app's own package.json version,
// which historically drifted (stuck at 0.0.2). Falls back to app.getVersion()
// when the source tree can't be read (e.g. a packaged build without the repo).
function resolveCloverVersion() {
  try {
    const root = resolveUpdateRoot()
    const initPath = path.join(root, 'clover_cli', '__init__.py')
...
// Renderer-bundle skew: `clover update` moves the SOURCE TREE, but the UI
...
// Re-resolve the live Clover version and push it into the native About panel
// just before showing it, so an in-place `clover update` is reflected without
  ...
  void detectRendererSkew().then(skew => {
    app.setAboutPanelOptions({
      ...
      applicationVersion: skew.outOfSync
        ? `${resolveCloverVersion()} — app build out of date, update the desktop app`
        : resolveCloverVersion(),
      copyright: 'Copyright © 2026 Clover Cognition'
    })
    app.showAboutPanel()
  })
...
ipcMain.handle('clover:version', async () => {
  const skew = await detectRendererSkew()

  return {
    appVersion: resolveCloverVersion(),
    electronVersion: process.versions.electron,
    nodeVersion: process.versions.node,
    platform: process.platform,
    cloverRoot: resolveUpdateRoot(),
...
// the actual removal via `clover uninstall …` so the cross-platform PATH /
// registry / service / node-symlink cleanup all lives in one place
// (clover_cli/uninstall.py + clover_cli/gui_uninstall.py).
//
...
async function getUninstallSummary() {
  ...
  const agentRoot = ACTIVE_CLOVER_ROOT
  ...
  const fallback = () => ({
    ...
    agent_installed: isCloverSourceRoot(agentRoot) && fileExists(py),
    gui_installed: true,
  ...
  return new Promise(resolve => {
    ...
    try {
      const child = spawn(
        py,
        ['-m', 'clover_cli.main', 'uninstall', '--gui-summary'],
        hiddenWindowsChildOptions({
          cwd: agentRoot,
          env: { ...process.env, CLOVER_HOME, NO_COLOR: '1' },
          stdio: ['ignore', 'pipe', 'ignore']
        })
      )
...
async function runDesktopUninstall(mode) {
  ...
  if (!fileExists(venvPy)) {
    return {
      ok: false,
      error: 'agent-missing',
      message: `Can't run the uninstaller: no Clover agent venv at ${VENV_ROOT}.`
    }
  }
  ...
  // Python with PYTHONPATH=<agentRoot> so `import clover_cli` resolves from
  ...
  try {
    await releaseBackendLock(ACTIVE_CLOVER_ROOT, 'uninstall')
  } catch (error) {
  ...
  try {
    if (IS_WINDOWS) {
      scriptPath = path.join(app.getPath('temp'), `clover-uninstall-${Date.now()}.cmd`)
      ...
      scriptPath = path.join(app.getPath('temp'), `clover-uninstall-${Date.now()}.sh`)
...
ipcMain.handle('clover:uninstall:summary', async () => getUninstallSummary())
ipcMain.handle('clover:uninstall:run', async (_event, payload) => {
...
ipcMain.handle('clover:vscode-theme:fetch', async (_event, id) => fetchMarketplaceThemes(String(id || '')))

// Search the Marketplace for color-theme extensions (empty query = top installs).
ipcMain.handle('clover:vscode-theme:search', async (_event, query) => searchMarketplaceThemes(String(query || ''), 20))

// ---------------------------------------------------------------------------
// clover:// deep links (e.g. clover://blueprint/morning-brief?time=08:00,
// clover://mcp/install?name=NAME&config=B64 — the vendor "Add to Clover"
// button, or clover://plugin/install?repo=owner/repo). Dev
// (`CLOVER_DESKTOP_DEV_SERVER`) registers clover-dev:// instead — bare
// Electron or a stale OS handler often owns clover:// on dev machines.
...
// ---------------------------------------------------------------------------
const CLOVER_PROTOCOL = DEV_SERVER ? 'clover-dev' : 'clover'
/** Schemes accepted when parsing inbound URLs (dev accepts both). */
const DEEPLINK_SCHEMES = DEV_SERVER ? ['clover-dev', 'clover'] : ['clover']
...
function handleDeepLink(url) {
  ...
  // clover://blueprint/<key>?slot=val  -> host="blueprint", path="/<key>"
  ...
  try {
    ...
    mainWindow.focus()
    mainWindow.webContents.send('clover:deep-link', payload)
...
ipcMain.handle('clover:deep-link-ready', () => {
  ...
  if (_pendingDeepLink) {
    ...
    handleDeepLink(
      `${CLOVER_PROTOCOL}://${queued.kind}/${encodeURIComponent(queued.name)}` +
...
function registerDeepLinkProtocol() {
  try {
    if (process.defaultApp && process.argv.length >= 2) {
      ...
      app.setAsDefaultProtocolClient(CLOVER_PROTOCOL, process.execPath, [entry])
    } else {
      app.setAsDefaultProtocolClient(CLOVER_PROTOCOL)
    }

    rememberLog(`[deeplink] registered ${CLOVER_PROTOCOL}:// handler`)
  } catch (err) {
...
// second-instance argv. Without the lock a second `clover://` launch spawns a
...
if (!isPrimaryInstance) {
  ...
  // startup (shortcut registration, createWindow → startClover), whose
...
app.whenReady().then(() => {
  ...
  // startClover() finishes resolving the configured backend.
  const primaryProfile = primaryProfileKey()
  ...
  // Win/Linux cold start: the launching clover:// URL is in our own argv.
...
app.on('before-quit', event => {
  ...
  // quitting Clover never keeps another app's chord hostage.
