...
async function claimBackendChild(child, command, profile, nonce) {
  try {
    ...
    throw new Error(`Could not persist ownership for the Clover backend: ${error.message}`)
  }
}
...
const OAUTH_SESSION_PARTITION = 'persist:clover-remote-oauth'
...
async function bootstrapSshConnectionInner(profile, sshConfig, reuseToken, source, fingerprint, lease) {
  ...
  return { ...connection, remoteCloverVersion: result.cloverVersion || '' }
}
...
async function spawnPoolBackend(profile, entry, opts: { forceLocal?: boolean; poolKey?: string } = {}) {
  ...
  child.once('exit', (code, signal) => {
    ...
    if (!ready) {
      rejectStart?.(
        new Error(`Clover backend for profile "${profile}" exited before it became ready (${signal || code}).`)
      )
    }
  })
...
async function startClover() {
  ...
  const connectionPromise = (async () => {
    ...
    await claimBackendChild(cloverProcess, `${backend.command} ${backend.args.join(' ')}`, profile, backendNonce)
    ...
    cloverProcess.once('exit', (code, signal) => {
      ...
      if (!backendReady) {
        const message = `Clover backend exited before it became ready (${signal || code}).`
        updateBootProgress(
          {
            error: message,
            message,
    ...
    const port = await Promise.race([
      waitForDashboardPortAnnouncement(cloverProcess, { readyFile }),
      backendStartFailed
    ])
  ...
  return remoteRevalidation.run(connectionPromise, async () => {
    const [result] = await Promise.all([
      revalidateRemoteConnection({
        ...
        resetConnection: resetCloverConnection,
...
// /api/clover/update endpoint (the dashboard updater), which runs
// `clover update` on THAT machine.
...
ipcMain.handle('clover:preview:reach', async (_event, url) => reachablePreviewUrl(String(url || '')))
