 ...
 *
 * 1. TORN BUNDLE after an update (#95575): `clover update` replaces the app
...
export interface RendererLoadErrorDetails {
  ...
  /** Repair command hint, e.g. `clover desktop --force-build`. */
  repairHint?: string
  /**
...
export function buildRendererLoadErrorPage(details: RendererLoadErrorDetails = {}): string {
  ...
  const title = 'Clover couldn\u2019t start the desktop UI'
  ...
  const repair = details.repairHint ? `<p>Repair with: <code>clover desktop --force-build</code></p>` : ''
...
<main>
  ...
  <code>clover desktop --force-build</code>, then restart the app.</p>
