...
function makeRepo() {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'clover-desktop-git-status-'))

  tempDirs.push(dir)
  execFileSync('git', ['init', '-q'], { cwd: dir })
  execFileSync('git', ['config', 'user.email', 'clover-test@example.com'], { cwd: dir })
  execFileSync('git', ['config', 'user.name', 'Clover Test'], { cwd: dir })
