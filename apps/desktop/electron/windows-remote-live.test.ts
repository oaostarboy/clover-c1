...
//   CLOVER_WIN_SSH_HOST   ssh alias/host of the Windows box
//   CLOVER_WIN_SSH_USER   remote user
//   CLOVER_WIN_SSH_CLOVER absolute path to the remote clover.exe under test
const liveHost = process.env.CLOVER_WIN_SSH_HOST || ''
const liveUser = process.env.CLOVER_WIN_SSH_USER || ''
const configuredClover = process.env.CLOVER_WIN_SSH_CLOVER || ''
const ownershipId = '89abcdef0123456789abcdef01234567'

function fetchJson(url, token, path) {
  return fetch(`${url}${path}`, { headers: { 'X-Clover-Session-Token': token } }).then(async response => {
    if (!response.ok) {
...
test.skipIf(!liveHost || !liveUser || !configuredClover)(
  ...
  async () => {
    ...
    try {
      ...
      const runtimeScript = `& '${configuredClover.replace('clover.exe', 'python.exe')}' -m clover_cli.windows_ssh_runtime read-lock '${ownershipId}'`
      ...
      if (lock) {
        const python = configuredClover.replace('clover.exe', 'python.exe')
        const terminate = `& '${python}' -m clover_cli.windows_ssh_runtime terminate '${lock.pid}' '${lock.creationTimeNs}' '${lock.cloverPath}' '${lock.spawnNonce}'`
        await ssh.exec(`powershell.exe -NoProfile -NonInteractive -Command "${terminate}"`)
        await ssh.exec(
          `powershell.exe -NoProfile -NonInteractive -Command "& '${python}' -m clover_cli.windows_ssh_runtime remove-lock '${ownershipId}'"`
        )
        await ssh.exec(
          `powershell.exe -NoProfile -NonInteractive -Command "& '${python}' -m clover_cli.windows_ssh_runtime remove-log '${ownershipId}' '${lock.spawnNonce}'"`
        )
      }
