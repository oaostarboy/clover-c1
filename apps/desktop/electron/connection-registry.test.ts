...
test('primary SSH reuse rejects a descriptor with a different remote Clover path', async () => {
  const registry = migrateV1ToRegistry({
    mode: 'ssh',
    remote: { mode: 'ssh', host: 'build-host', remoteCloverPath: '/srv/clover', user: 'alice' },
    profiles: {}
  })
...
test('registry primary reuses a matching primary backend descriptor', () => {
  const registry = normalizeRegistry({
    ...
    connections: [
      ...
      { id: 'clover-vps', kind: 'ssh', label: 'Clover VPS', host: 'clover-vps' }
    ]
  })

  const descriptor = {
    connectionId: 'clover-vps',
    mode: 'remote' as const,
    remoteKind: 'ssh' as const,
    ssh: { host: 'clover-vps' }
  }

  assert.equal(registrySourceOwnsPrimaryBackend(registry, 'clover-vps', descriptor), true)
...
test('resolvedConnectionId keeps same-host SSH routes distinct by port, key, path, and profile', () => {
  ...
  const registry: ConnectionRegistry = {
    ...
    connections: [
      ...
      { ...base, id: 'ssh-path', label: 'SSH path', remoteCloverPath: '/opt/clover' },
  ...
  assert.equal(resolve({ ...base, remoteCloverPath: '/opt/clover' }), 'ssh-path')
...
// The electron and @clover/shared implementations MUST stay byte-identical —
...
test('backendScopeKey: electron and shared implementations agree everywhere', async () => {
  ...
  const shared = (await import(String('@clover/shared'))) as {
...
test('roster: source profile metadata follows the connection-qualified row', () => {
  ...
  const vpsMeta = {
    display_name: 'Emma',
    ui_meta: { 'clover-bots': { title: 'Emma', shape: 'blobatar::sun', color: '#8b5cf6' } },
    has_avatar: true
  }
...
test('merge preserves fields the editor does not carry (org, ssh extras)', () => {
  ...
  assert.equal(labelOnly.remoteCloverPath, '/opt/clover')
...
test('Apply remote preserves an existing URL identity and label without duplicates', () => {
  ...
  assert.equal(matches[0].id, 'clover-alex')
  assert.equal(matches[0].label, 'Existing gateway')
  assert.equal(matches[0].authMode, 'oauth')
  assert.equal(applied.primary, 'clover-alex')
  assert.equal(applied.lastUsed, 'clover-alex')
})
