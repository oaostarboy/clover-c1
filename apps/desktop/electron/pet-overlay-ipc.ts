...
export function registerPetOverlayIpc({
  ...
  ipcMain.handle('clover:pet-overlay:open', async (_event, request) => {
  ...
  ipcMain.handle('clover:pet-overlay:close', async () => {
    closePetOverlay()
  ...
  ipcMain.on('clover:pet-overlay:set-bounds', (_event, bounds) => {
  ...
  ipcMain.on('clover:pet-overlay:ignore-mouse', (_event, ignore) => {
    const petOverlayWindow = getPetOverlayWindow()
  ...
  ipcMain.on('clover:pet-overlay:set-focusable', (_event, focusable) => {
    const petOverlayWindow = getPetOverlayWindow()
  ...
  ipcMain.on('clover:pet-overlay:state', (_event, payload) => {
    const petOverlayWindow = getPetOverlayWindow()

    if (petOverlayWindow && !petOverlayWindow.isDestroyed()) {
      petOverlayWindow.webContents.send('clover:pet-overlay:state', payload)
    }
  })
  // Overlay → main renderer: control messages (pop back in, composer submit).
  ipcMain.on('clover:pet-overlay:control', (_event, payload) => {
    const mainWindow = getMainWindow()
    ...
    mainWindow.webContents.send('clover:pet-overlay:control', payload)
  })
}
