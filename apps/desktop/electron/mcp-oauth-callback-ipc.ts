 ...
 *   - the browser sees only a minimal "return to Clover" page.
 */
...
const DONE_HTML =
  ...
  '<p>You can close this window and return to Clover.</p>' +
...
export function registerMcpOauthCallbackIpc() {
  ...
  ipcMain.handle('clover:mcp-oauth:listen', async () => {
  ...
  ipcMain.handle('clover:mcp-oauth:wait', async (_event, id, timeoutMs) => {
  ...
  ipcMain.handle('clover:mcp-oauth:cancel', (_event, id) => {
    dispose(String(id || ''))
