...
test('Windows spawn holds the update mutex across marker check and helper spawn', () => {
  const command = atomicWindowsSpawnCommand({
    cloverHome: 'C:\\Users\\andre\\.clover',
    python: 'C:\\Users\\andre\\.clover\\python.exe'
  })

  const encoded = command.match(/-EncodedCommand\s+([^\s]+)$/)?.[1]
  const script = encoded ? Buffer.from(encoded, 'base64').toString('utf16le') : ''
  assert.match(script, /\.clover-update-in-progress/)
...
test('Windows spawn publishes the initial ownership record before releasing the mutex', () => {
  const command = atomicWindowsSpawnCommand(
    ...
    },
    {
      ...
      profile: 'default',
      cloverPath: 'C:\\Clover\\clover.exe',
      cloverHome: 'C:\\Users\\andre\\.clover',
...
test('Windows relaunch gate refuses live and uncertain markers before executing the remote runtime', async () => {
  for (const observation of ['LIVE:4242', 'UNCERTAIN']) {
    ...
    const ssh = sshWith(async command => {
      ...
      if (script.includes('Get-Command clover.exe')) {
        return JSON.stringify({
          os: 'Windows',
          arch: 'AMD64',
          cloverHome: 'C:\\Users\\alice\\.clover',
          cloverPath: 'C:\\Clover\\clover.exe',
          python: 'C:\\Clover\\python.exe'
        })
      }

      if (script.includes('.clover-update-in-progress')) {
        return observation
      }
    ...
    assert.equal(
      scripts.some(script => script.includes('clover_cli.windows_ssh_runtime')),
      false
    )
  }
})
...
test('Windows relaunch gate uses strict install-wide marker parsing and fail-closed PID probing', async () => {
  ...
  await assertWindowsRemoteInstallUpdateClear(ssh, 'C:\\Users\\alice\\.clover\\profiles\\research')
  assert.match(script, /\.clover-update-in-progress/)
...
test('Windows probe validates Clover and Python topology before selection', async () => {
  let script = ''
  ...
  const fallbackJoin = script.indexOf('Join-Path $cloverHome')
  ...
  await assert.rejects(
    detectRemotePlatform(
      sshWith(async command => {
        ...
        throw new Error('Clover is not installed on the remote Windows host.')
      })
    ),
    (err: any) => err.kind === 'unsupported-platform' && /Clover is not installed/.test(err.message)
  )
})

test('helper command uses the fixed remote Python entry point and quotes path data', () => {
  const command = helperCommand({ python: "C:\\Program Files\\Clover's\\python.exe" }, 'inspect', [
    'C:\\x y\\clover.exe'
  ])

  const encoded = command.split(' ').pop()!
  const script = Buffer.from(encoded, 'base64').toString('utf16le')
  assert.match(script, /-m' 'clover_cli\.windows_ssh_runtime' 'inspect'/)
  assert.match(script, /Clover''s/)
  assert.match(script, /C:\\x y\\clover\.exe/)
})
...
test('Windows SSH reuse requires the requested remote profile to match the lock', () => {
  ...
  const runtime = { cloverPath: lock.cloverPath, cloverHome: lock.cloverHome }
