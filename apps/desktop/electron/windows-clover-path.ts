/**
 * windows-clover-path.ts
 *
 * Pure, dependency-injected pieces of Windows `clover` resolution pulled out
 * of main.ts's findOnPath(), handOffWindowsBootstrapRecovery(), and
 * unwrapWindowsVenvCloverCommand(). Each of the three functions here pins one
 ...
 *      FIRST, so an extensionless Git-Bash `clover` shim shadowed the real
 *      clover.cmd/clover.exe; the shim then failed the --version probe and
 ...
 *      and Clover launcher. Marker-only or partial runtimes must use --repair;
 *      only a runnable pair can use --update.
 *   3. resolveVenvCloverCommand() — unwrapWindowsVenvCloverCommand() returned
 ...
 * extensionless file (e.g. a Git-Bash shell-script shim named `clover`) must
 * not shadow `clover.cmd`/`clover.exe`. The empty entry is kept LAST so
 ...
 * venv Python interpreter and the Clover launcher that drives `clover update`.
...
export interface ResolveVenvCloverCommandDeps {
  isWindows: boolean
  isCommandScript: (command: string) => boolean
  fileExists: (filePath: string) => boolean
  directoryExists: (filePath: string) => boolean
  canImportCloverCli: (python: string, opts?: { env?: Record<string, string> }) => boolean
...
/**
 * If `command` is a Windows venv `clover`/`clover.exe` console-script shim
 * (i.e. `<venvRoot>/Scripts/clover(.exe)`), resolve it to the underlying
 * venv python invoked as `python -m clover_cli.main <backendArgs>` — but
 * ONLY after smoke-testing that interpreter with canImportCloverCli(). A
 * venv whose update died mid-`pip install` still has python.exe + clover.exe
 ...
 *
 * Returns null when `command` is not a venv clover shim, the underlying
 * python doesn't exist, or the import probe fails. Otherwise returns the
 * resolved backend descriptor.
 */
export function resolveVenvCloverCommand(
  command: string,
  backendArgs: string[],
  deps: ResolveVenvCloverCommandDeps
): {
  label: string
  command: string
  args: string[]
  ...
  if (!/^clover(?:\.exe)?$/i.test(basename(resolved))) {
    return null
  }
  ...
  if (
    ...
    rememberLog?.(
      `Ignoring venv Clover at ${python}: runtime import probe failed (broken/partial venv); falling through to bootstrap.`
    )
  ...
  return {
    label: `existing Clover Python at ${python}`,
    command: python,
