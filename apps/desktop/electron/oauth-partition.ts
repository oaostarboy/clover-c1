 ...
 * (`persist:clover-remote-oauth`) — the jar was keyed on the auth *mode*, not
 ...
 * setup: one box, two dashboards) fought over the same `clover_session*`
 ...
 *     cascade deliberately shares one jar with the Clover Portal session.
...
export const LEGACY_OAUTH_PARTITION = 'persist:clover-remote-oauth'
