// Launching the Clover TUI in the user's OWN terminal emulator.
//
...
// `clover --tui --resume <id>` there. Two problems have to be solved for that
// to work anywhere:
//
//  1. WHAT to run. The desktop's Clover runtime is often a venv Python invoked
//     as `python -m clover_cli.main`, not a `clover` on PATH — so the command
 ...
 * headless child) is the wrong answer for an interactive terminal. The Clover
