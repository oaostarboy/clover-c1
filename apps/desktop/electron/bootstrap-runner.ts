 ...
 * bootstrap-runner.ts
 *
 * Drives apps/desktop's first-launch install of Clover Cognition by spawning
...
function readExistingPinnedCommit(activeRoot: string | null | undefined): string | null {
  ...
  try {
    const raw = fs.readFileSync(path.join(activeRoot, '.clover-bootstrap-complete'), 'utf8')
...
function bootstrapCacheDir(cloverHome) {
  return path.join(cloverHome, 'bootstrap-cache')
}

// The install.sh / install.ps1 that ships inside the already-installed agent
// checkout under ~/.clover/clover-c1. Used as a last-resort fallback when
// the pinned commit can't be fetched from GitHub (e.g. a locally-built desktop
// app stamped to an unpushed HEAD).
function installedAgentInstallScript(cloverHome) {
  if (!cloverHome) {
    return null
  }

  const candidate = path.join(cloverHome, 'clover-c1', 'scripts', installScriptName())
...
function cachedScriptPath(cloverHome, commit) {
  return path.join(bootstrapCacheDir(cloverHome), `install-${commit}.${process.platform === 'win32' ? 'ps1' : 'sh'}`)
}
...
function downloadInstallScript(ref, destPath) {
  ...
  const url = `https://raw.githubusercontent.com/CloverCognition/clover-c1/${ref}/scripts/${scriptName}`
...
async function resolveInstallScript({
  ...
  const cached = cachedScriptPath(cloverHome, installRef.cacheKey)
  ...
  try {
    ...
    const installed = installedAgentInstallScript(cloverHome)
...
function spawnPowerShell(scriptPath, args, { emit, stageName, abortSignal, cloverHome }: any = {}) {
  return new Promise<any>((resolve, reject) => {
    ...
    const child = spawn(
      ...
      hiddenWindowsChildOptions({
        ...
        env: {
          ...process.env,
          // Pass CLOVER_HOME through so install.ps1 respects the caller's
          // choice rather than re-computing the default.
          CLOVER_HOME: cloverHome || process.env.CLOVER_HOME || ''
        }
      })
    )
...
function spawnBash(scriptPath, args, { emit, stageName, abortSignal, cloverHome }: any = {}) {
  return new Promise<any>((resolve, reject) => {
...
function buildPosixPinArgs({ installStamp, activeRoot, cloverHome, pinCommit = true }) {
  const args = ['--dir', activeRoot, '--clover-home', cloverHome]
...
async function fetchManifest({ scriptPath, installerKind, emit, cloverHome, activeRoot, installStamp, pinCommit }) {
  const isPosix = installerKind === 'posix'

  const args = isPosix
    ? ['--manifest', ...buildPosixPinArgs({ installStamp, activeRoot, cloverHome, pinCommit })]
...
async function runStage({
  ...
  const args = isPosix
    ? [
        ...
        '--non-interactive',
        '--json',
        ...buildPosixPinArgs({ installStamp, activeRoot, cloverHome, pinCommit })
      ]
...
async function runBootstrap(opts) {
  ...
  const runLog = openRunLog(logRoot || path.join(cloverHome, 'logs'))
  ...
  try {
    ...
    const scriptInfo = await resolveInstallScript({ installStamp, sourceRepoRoot, cloverHome, emit })
