...
test('resolveLoginStrategy returns native for native_pkce gateway with non-password provider', () => {
  const statusBody = { auth_required: true, auth_flows: ['cookie', 'native_pkce'] }
  const providers = [{ name: 'clover', displayName: 'Clover Cognition', supportsPassword: false }]
...
test('resolveLoginStrategy returns native for a mixed provider deployment', () => {
  ...
  const providers = [
    ...
    { name: 'clover', supportsPassword: false }
  ]
...
test('buildNativeAuthorizeUrl encodes params and honours a path prefix', () => {
  ...
  assert.equal(parsed.searchParams.get('provider'), 'clover')
})

test('buildNativeAuthorizeUrl omits provider when not given and preserves prefix', () => {
  const url = buildNativeAuthorizeUrl('https://gw.example.com/clover', {
    challenge: 'C',
  ...
  assert.equal(parsed.pathname, '/clover/auth/native/authorize')
...
test('nativeTokenUrl / nativeRefreshUrl build the right endpoints', () => {
  ...
  assert.equal(nativeRefreshUrl('https://gw.example.com/clover'), 'https://gw.example.com/clover/auth/native/refresh')
})
...
test('parseTokenResponse maps a well-formed body', () => {
  ...
  assert.equal(t.provider, 'clover')
...
test('parseTokenResponse cannot read a persisted set (the reload bug #73271)', () => {
  ...
  const persisted = JSON.parse(
    JSON.stringify({ accessToken: 'AT', refreshToken: 'RT', expiresAt: 1, provider: 'clover', userId: 'u' })
  )
