 ...
 * `iter_clover_node_dirs()` in clover_constants.py, which the Electron main
 * process cannot import.
 */
function cloverManagedNodePathEntries(
  cloverHome,
  ...
  const root = pathModule.join(cloverHome, 'node')
...
function buildDesktopBackendPath({
  ...
  const cloverNodeDirs = cloverManagedNodePathEntries(cloverHome, { platform, pathModule })
  const venvBin = venvRoot ? pathModule.join(venvRoot, platform === 'win32' ? 'Scripts' : 'bin') : null
  const saneEntries = platform === 'win32' ? [] : POSIX_SANE_PATH_ENTRIES

  return appendUniquePathEntries([cloverNodeDirs, venvBin, currentPath, saneEntries], { delimiter })
}

function normalizeCloverHomeRoot(cloverHome, { pathModule = pathModuleForPlatform(process.platform) }: any = {}) {
  if (!cloverHome) {
    return cloverHome
  }

  const resolved = pathModule.resolve(String(cloverHome))
...
function buildDesktopBackendEnv({
  ...
  return {
    ...
    // cp1252, ...). clover_bootstrap sets this inside the child too, but only
