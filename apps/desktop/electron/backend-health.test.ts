...
test('uses lightweight /api/health for current backends', async () => {
  ...
  await waitForCloverReady('http://127.0.0.1:9000/', {
    token: 'secret-token',
...
test('does not fall back to heavyweight /api/status for transient health failures', async () => {
  ...
  await assert.rejects(
    waitForCloverReady('http://127.0.0.1:9000', {
      fetchPublicJson: async url => {
        calls.push(['public', url])
        throw new Error('Timed out connecting to Clover backend after 15000ms')
      },
      fetchJson: async url => {
        calls.push(['token', url])
      },
...
test('recognizes missing-route shapes only', () => {
  ...
  assert.equal(
    isMissingHealthEndpointError(
      new Error('Expected JSON from /api/health but got HTML. The endpoint is likely missing on the Clover backend.')
    ),
    true
  )
  assert.equal(isMissingHealthEndpointError(new Error('Timed out connecting to Clover backend after 15000ms')), false)
...
test('anonymous gate-shaped 401 falls back to /api/status (backend predates /api/health)', async () => {
  ...
  await waitForCloverReady('http://192.168.1.132:9119', {
    token: null,
    fetchPublicJson: async url => {
      calls.push(['public', url])
...
test('a credentialed probe still uses the 404 fallback for a genuinely missing route', async () => {
  ...
  await waitForCloverReady('https://gateway.example', {
    token: 'session-token',
    fetchPublicJson: async () => {
      throw new Error('public probe must not be used when credentialed')
    },
...
test('isNousCloudAgentUrl detects cloud agent hosts', () => {
  // Positive cases
  assert.equal(isNousCloudAgentUrl('https://ares-3009.agents.clover-c1.local'), true)
  assert.equal(isNousCloudAgentUrl('https://ares-3009.agents.clover-c1.local/api/health'), true)
  assert.equal(isNousCloudAgentUrl('http://test.agents.clover-c1.local'), true)
  ...
  assert.equal(isNousCloudAgentUrl('https://clover-c1.local'), false)
...
test('waitForCloverReady surfaces actionable error for cloud agent 503', async () => {
  ...
  try {
    await waitForCloverReady('https://ares-3009.agents.clover-c1.local', {
      fetchPublicJson: async () => {
        attempts++
        // Always return 503
    ...
    assert.ok(error.message.includes('portal.clover-c1.local'), `should mention portal: ${error.message}`)
    assert.ok(error.message.includes('discord.gg/CloverCognition'), `should mention Discord: ${error.message}`)
...
test('waitForCloverReady does not cloud-wrap non-cloud 503 errors', async () => {
  const currentTime = { value: 0 }
...
test('makeNousCloudBackendDownError produces the Cloud shape and preserves cause', () => {
  const err = new Error('upstream unavailable') as any
  err.statusCode = 503
  const result = makeNousCloudBackendDownError('https://ares-3009.agents.clover-c1.local', err)
  assert.ok(result)
  ...
  assert.ok(result?.message.includes('Clover Cloud agent ares-3009.agents.clover-c1.local is down'))
})

test('makeNousCloudBackendDownError returns null for a Cloud 401 (routes to reauth)', () => {
  const err = new Error('Unauthorized') as any
  err.statusCode = 401
  assert.equal(makeNousCloudBackendDownError('https://ares-3009.agents.clover-c1.local', err), null)
})
...
test('makeNousCloudBackendDownError preserves legacy string-prefix compatibility', () => {
  const result = makeNousCloudBackendDownError(
    'https://ares-3009.agents.clover-c1.local',
