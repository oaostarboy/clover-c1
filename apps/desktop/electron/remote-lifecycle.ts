 ...
 * remote-lifecycle.ts
 *
 * Pure, electron-free remote Clover dashboard lifecycle over SSH for Desktop
 ...
 *
 *   - locate the Clover install on the remote (login-shell probe),
 ...
 *     `CLOVER_DASHBOARD_READY port=<n>` readiness line,
 ...
 * the real SshConnection, fetch, adoptServedDashboardToken, and waitForClover in.
 *
 * The minted CLOVER_DASHBOARD_SESSION_TOKEN is the SPAWN credential. After
...
const READY_RE = /^CLOVER_(?:BACKEND|DASHBOARD)_READY port=(\d+)/m
const REMOTE_LOCK_DIR = '~/.clover/desktop-ssh'
...
// Resolve the remote clover executable. An EXPLICIT path is honored strictly
...
async function locateClover(ssh, remoteCloverPath) {
  const resolveLauncher = async (candidate: string) => {
    // Return the candidate path directly. The clover binary or wrapper script
  ...
  if (remoteCloverPath) {
    if (await isExecutable(remoteCloverPath)) {
      return resolveLauncher(remoteCloverPath)
    }

    const err: any = new Error(
      `The Clover path you set is not an executable on the remote host: "${remoteCloverPath}". ` +
        'Check the path (it must be the full path to the `clover` binary on the remote, e.g. ' +
        '~/clover-c1/.venv/bin/clover), or clear it to auto-detect.'
    )

    err.kind = 'clover-not-found'
    throw err
  }
  ...
  try {
    const found = (await ssh.exec(`bash -lc ${shq('command -v clover')}`)).trim()
  ...
  candidates.push('~/.local/bin/clover')
  candidates.push('/usr/local/bin/clover')
  candidates.push('~/.clover/clover-c1/venv/bin/clover')
...
// Probe the resolved binary's version string (first line of `<clover> --version`,
// e.g. "Clover Cognition v0.18.2 ..."), or '' on failure. Surfaces WHICH clover a
// connection uses, so a stale/unexpected install is visible.
async function probeCloverVersion(ssh, cloverPath) {
  try {
    const out = (await ssh.exec(`${expandRemotePath(cloverPath)} --version 2>&1`)).trim()
...
async function probeRemotePlatform(ssh) {
  ...
  if (!SUPPORTED_REMOTE_OS.has(osName)) {
    const err: any = new Error(
      `Unsupported remote platform "${osName || 'unknown'}". Clover Desktop SSH mode supports Linux, macOS, and Windows remote hosts.`
    )
...
// The CLOVER_HOME the remote dashboard will use (explicit env wins, else
// ~/.clover). Recorded in the lockfile so a future reuse can tell it's the same
// state store; best-effort.
async function probeRemoteCloverHome(ssh) {
  try {
    const out = (await ssh.exec('echo "${CLOVER_HOME:-$HOME/.clover}"')).trim().split('\n').pop()

    return out || '~/.clover'
  } catch (cause) {
    const error: any = new Error('Could not resolve the remote Clover home.')
...
marker=home/'.clover-update-in-progress'
try:
 ...
 * bytes; it never imports or executes code from the changing Clover checkout.
 ...
 */
async function assertRemoteInstallUpdateClear(ssh, cloverHome) {
  const home = assertSafeRemoteHome(cloverHome)
  let observation = ''
  ...
  try {
    ...
    const error: any = new Error('Could not prove that the remote Clover install is clear for SSH startup.')
  ...
  const error: any = new Error(
    live
      ? `Remote Clover update process ${live[1]} is still running; SSH startup is paused.`
      : 'The remote Clover update marker is unreadable or malformed; refusing SSH startup.'
  )
...
async function listRemoteCloverProfiles(ssh) {
  const home = assertSafeRemoteHome(await probeRemoteCloverHome(ssh))
  ...
  try {
    ...
    const error: any = new Error('Could not list remote Clover profiles.')
    error.kind = 'transient-transport-error'
    error.cause = cause
    throw error
  }
...
function assertSafeRemoteHome(home) {
  ...
  if (!/^(\/|~\/)[A-Za-z0-9._/+-]+$/.test(value) || value.includes('..')) {
    const error: any = new Error('Unsafe remote Clover home.')
...
async function readLockfile(ssh, ownershipId) {
  ...
  for (const field of ['profile', 'cloverPath', 'cloverHome', 'logPath', 'startedAt']) {
...
async function pidIsOurDashboard(
  ...
  if (!pid || !/^[0-9a-f]{16}$/.test(String(spawnNonce || '')) || !cloverPath) {
    return false
  }
  ...
  try {
    const script =
      ...
      `pid=${Number(pid)}\n` +
      `expected=os.path.expanduser(${shq(cloverPath)})\n` +
      // The installer-facing launcher is intentionally preserved for invocation
      // (#74411), but it may `exec python <install-dir>/clover`, leaving neither
      // launcher nor CLOVER_HOME-derived entrypoint in argv. The ownership-scoped
      // token path + random nonce + exact profile below are the alternative proof.
      `clover_home=os.path.expanduser(${shq(cloverHome)}) if ${shq(cloverHome)} else ""\n` +
      'expected_entries={expected}\n' +
      'if clover_home:\n' +
      ' expected_entries.add(os.path.join(clover_home,"clover-c1","venv","bin","clover"))\n' +
...
function buildOwnedStaleTerminationCommand(lock, ownershipId) {
  ...
  const expectedPath = expandRemotePath(lock.cloverPath)
  const expectedHome = lock.cloverHome ? expandRemotePath(lock.cloverHome) : "''"
  ...
  const executableMatch = lock.cloverHome
...
function lockMatchesManagedUpdateScope(lock, expected) {
  return Boolean(
    ...
    lock.cloverPath === expected.cloverPath &&
    lock.cloverHome === expected.cloverHome
  )
}
...
expected_path=os.path.expanduser(${py(lock.cloverPath)})
clover_home=os.path.expanduser(${py(lock.cloverHome)})
expected_entries={expected_path,os.path.join(clover_home,"clover-c1","venv","bin","clover")}
...
result=None
try:
 result=subprocess.run(["sh","-c",payload,"clover-update-mutex",str(fd)],pass_fds=(fd,),check=False)
finally:
 os.close(fd)
...
function buildSpawnCommand(cloverPath, profile, opts: any = {}) {
  const clover = expandRemotePath(cloverPath)
  ...
  const marker = expandRemotePath(`${remoteInstallRoot(opts.cloverHome || '~/.clover')}/.clover-update-in-progress`)

  const updateMutex = expandRemotePath(
    `${remoteInstallRoot(opts.cloverHome || '~/.clover')}/.clover-update-in-progress.mutex`
  )
  ...
  const dashCmd =
    ...
    `exec env CLOVER_DESKTOP=1 ${clover} ${profileArgs}${subCmd}`

  const detachedShell = `eval "exec $1>&-"; ${dashCmd} </dev/null >> ${logPath} 2>&1 & echo $!`
  const detachedSpawn = `child=$("$(command -v setsid || echo nohup)" sh -c ${shq(detachedShell)} clover-update-child "$1" & echo $!)`
...
async function remoteSupportsSshOwnership(ssh, cloverPath) {
  const clover = expandRemotePath(cloverPath)

  const out = await ssh.exec(
    `help="$(${clover} serve --help 2>&1)"; ` +
...
async function spawnRemoteDashboard(
  ssh,
  { cloverPath, profile, token, ownershipId, cloverHome = '~/.clover', assertInstallClear = async () => {} }
) {
  if (!(await remoteSupportsSshOwnership(ssh, cloverPath))) {
    const err: any = new Error(
      'The remote Clover install does not support --ssh-session-token-file and --ssh-owner-nonce. ' +
        'Update Clover on the remote host to continue using Desktop SSH mode.'
    )
  ...
  try {
    ...
    out = await ssh.exec(
      buildSpawnCommand(cloverPath, profile, {
        spawnNonce,
        tokenFilePath,
        logPath,
        cloverHome,
 ...
 * opened SshConnection, forward/pickLocalPort/waitForClover, a token-gated
...
async function connect(deps) {
  ...
  const cloverHome = await probeRemoteCloverHome(ssh)
  await assertRemoteInstallUpdateClear(ssh, cloverHome)
  const cloverPath = await locateClover(ssh, remoteCloverPath)
  log(`located clover at ${cloverPath}`)
  const cloverVersion = await probeCloverVersion(ssh, cloverPath)

  if (cloverVersion) {
    log(`remote clover version: ${cloverVersion}`)
  }
  ...
  if (isLockfileSkew(lock)) {
    ...
    const error: any = new Error(
      `The remote ownership record ${lpath} does not match this Clover Desktop build (${lock.reason}). ` +
  ...
  if (lock) {
    ...
    const reusable =
      ...
      lock.cloverPath === cloverPath &&
      lock.cloverHome === cloverHome
  ...
  const spawned = await spawnRemoteDashboard(ssh, {
    ...
    ownershipId,
    cloverHome,
    assertInstallClear: () => assertRemoteInstallUpdateClear(ssh, cloverHome)
  })
  ...
  try {
    ...
    assertBootstrapNotSuperseded(signal)
    const baseUrl = `http://127.0.0.1:${localPort}`
    await waitForClover(baseUrl, spawnToken)
    assertBootstrapNotSuperseded(signal)
