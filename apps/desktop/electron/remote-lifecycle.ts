...
async function spawnRemoteDashboard(ssh, { cloverPath, profile, token, ownershipId }) {
  ...
  try {
    out = await ssh.exec(buildSpawnCommand(cloverPath, profile, { spawnNonce, tokenFilePath, logPath }))
  } catch (error) {
    try {
      await ssh.exec(`rm -f ${expandRemotePath(tokenFilePath)}`)
    } catch {
