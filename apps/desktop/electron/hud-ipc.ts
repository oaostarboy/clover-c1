...
export function registerHudIpc({
  ...
  ipcMain.on('clover:hud:native-drag', event => {
    event.returnValue = hudWindowing().move === 'native-drag'
  })

  ipcMain.on('clover:hud:windowing', event => {
  ...
  ipcMain.on('clover:hud:workspace-transfer', (event, transferring) => {
  ...
  ipcMain.handle('clover:hud:open', async (_event, request) => {
    openHudWindow(
  ...
  ipcMain.handle('clover:hud:frost', (_event, showing) => {
  ...
  // reaches the bar.
  ipcMain.on('clover:hud:ignore-mouse', (_event, ignore) => {
    const hudWindow = getHudWindow()
  ...
  ipcMain.on('clover:hud:begin-move', event => {
    const hudWindow = getHudWindow()
  ...
  ipcMain.on('clover:hud:end-move', event => {
    const hudWindow = getHudWindow()
  ...
  ipcMain.on('clover:hud:move-by', (event, delta) => {
    const hudWindow = getHudWindow()
  ...
  ipcMain.on('clover:hud:set-bounds', (event, bounds) => {
    const hudWindow = getHudWindow()
  ...
  ipcMain.handle('clover:hud:reset-layout', event => {
    const hudWindow = getHudWindow()
  ...
  ipcMain.on('clover:hud:session', (event, sessionId) => {
    const hudWindow = getHudWindow()
  ...
  ipcMain.handle('clover:hud:close', async () => {
    closeHudWindow()
