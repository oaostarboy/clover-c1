...
// registry, and the clover:terminal:* IPC surface. Extracted from main.ts; the
...
export function registerTerminalIpc({
  ...
  function terminalShellCommand() {
    // CLOVER_DESKTOP_SHELL is the cross-platform escape hatch (a path or a bare
    ...
    const override = (process.env.CLOVER_DESKTOP_SHELL || (isWindows ? '' : process.env.SHELL) || '').trim()
  ...
  function terminalShellEnv() {
    ...
    // /TERM=dumb; some terminals set COLORFGBG which would flip Clover' TUI into
    ...
    env.TERM_PROGRAM = 'Clover'
    env.TERM_PROGRAM_VERSION = app.getVersion()

    // Let a clover/--tui launched in this pane know it's embedded in the desktop
    // GUI (build_environment_hints surfaces this). Distinct from CLOVER_DESKTOP,
    // which marks the agent *backend* and gates cron/gateway behavior.
    env.CLOVER_DESKTOP_TERMINAL = '1'

    return env
  }

  function terminalChannel(id, suffix) {
    return `clover:terminal:${id}:${suffix}`
  }
  ...
  ipcMain.handle('clover:terminal:start', async (event, payload = {}) => {
  ...
  ipcMain.handle('clover:terminal:write', (_event, id, data) => {
  ...
  ipcMain.handle('clover:terminal:resize', (_event, id, size = {}) => {
    const sessionInfo = terminalSessions.get(String(id || ''))
  ...
  ipcMain.handle('clover:terminal:cwd', async (_event, id) => {
    const sessionInfo = terminalSessions.get(String(id || ''))
  ...
  ipcMain.handle('clover:terminal:dispose', (_event, id) => disposeTerminalSession(String(id || '')))
