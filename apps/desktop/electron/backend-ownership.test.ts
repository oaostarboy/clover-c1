...
function ownershipEntry(overrides: Partial<BackendIdentity> = {}) {
  return { command: 'clover serve --port 0', ...identity(overrides) }
}
...
test('backend identity check matches only serve and dashboard invocation shapes', () => {
  assert.equal(backendCommandMatches('/venv/bin/clover serve --port 0'), true)
  assert.equal(backendCommandMatches('python -m clover_cli.main dashboard --no-open'), true)
  assert.equal(backendCommandMatches('/venv/bin/clover --profile work serve --port 0'), true)
  assert.equal(backendCommandMatches('"C:\\Clover Runtime\\clover.exe" dashboard --no-open'), true)
  assert.equal(backendCommandMatches('clover chat --query serve'), false)
