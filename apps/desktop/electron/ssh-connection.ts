...
const _REDACTIONS: Array<[RegExp, string]> = [
  [/(CLOVER_DASHBOARD_SESSION_TOKEN=)(\S+)/g, '$1<redacted>'],
  [/(X-Clover-Session-Token["']?\s*[:=]\s*["']?)([^\s"'&]+)/gi, '$1<redacted>'],
...
// root under a short per-user base (`~/.clover/desktop-ssh`) so even worst case
...
function defaultControlDir() {
  ...
  if (process.platform === 'win32') {
    return path.join(os.tmpdir(), 'clover-desktop-ssh')
  }

  return path.join(os.homedir(), '.clover', 'desktop-ssh')
}
