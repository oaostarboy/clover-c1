...
export function markerPath(cloverHome) {
  return path.join(cloverHome, '.clover-update-in-progress')
}
...
export function readLiveUpdateMarker(
  ...
  const file = markerPath(cloverHome)
  let raw
 ...
 *
 * The Tauri-based clover-setup.exe takes several seconds to initialise its
...
export function writeUpdateMarker(
  ...
  const file = markerPath(cloverHome)
  const nowMs = now()
  const owner = readLiveUpdateMarker(cloverHome, { kill, maxAgeMs, now: () => nowMs })
 ...
 * Clover to exit…") clobbers that still-running updater's claim: the
...
export function updateHandoffConflict(
  ...
  const owner = readLiveUpdateMarker(cloverHome, opts)
