 ...
 *   1. CLOVER_GIT_BASH_PATH env var override
 *   2. PortableGit under %LOCALAPPDATA%\clover\git\ (install.ps1)
...
export function findGitBash(opts: GitBashOptions): string | null {
  ...
  // Respect CLOVER_GIT_BASH_PATH if set (mirrors tools/environments/local.py:_find_bash).
  const gitBashPath = env.CLOVER_GIT_BASH_PATH
  ...
  if (localAppData) {
    candidates.push(joinWin(localAppData, 'clover', 'git', 'bin', 'bash.exe'))
    candidates.push(joinWin(localAppData, 'clover', 'git', 'usr', 'bin', 'bash.exe'))
  }
