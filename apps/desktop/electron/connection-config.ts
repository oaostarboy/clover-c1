 ...
 *     `X-Clover-Session-Token` header; WS uses `?token=`.
...
// clover_cli/dashboard_auth/cookies.py.
//
// Two cookies are in play (see that module):
//   - clover_session_at: the OAuth access token. Short-lived (~15 min); its
//     Max-Age tracks the access-token TTL, so the cookie jar drops it the
//     instant the AT expires.
//   - clover_session_rt: the OAuth refresh token. Long-lived (24h rotating,
//     reuse-detected — Portal NAS #293 / clover #37247). When the AT cookie
...
const AT_COOKIE_VARIANTS = ['__Host-clover_session_at', '__Secure-clover_session_at', 'clover_session_at']
const RT_COOKIE_VARIANTS = ['__Host-clover_session_rt', '__Secure-clover_session_rt', 'clover_session_rt']

// The Clover portal (NAS) does NOT use Clover gateway session cookies — it is a
...
// Keep this aligned with clover_cli.profiles.validate_profile_name(). `default`
// is the built-in root alias; these names cannot be created as profiles.
const RESERVED_REMOTE_PROFILES = new Set(['clover', 'test', 'tmp', 'root', 'sudo'])
...
function gatewayTicketFailure(error, authMessage, transportMessage) {
  ...
  // classification (isServerSideHttpError / makeCloverCloudBackendDownError) and
...
/** Which Clover profile the remote SSH dashboard should actually run as.
...
function normalizeSshConfig(entry) {
  ...
  const remoteCloverPath = String(entry.remoteCloverPath || '').trim()

  if (remoteCloverPath) {
    out.remoteCloverPath = remoteCloverPath
  }

  // A Desktop profile can be a local routing label rather than the profile
  // name used by the remote Clover installation. Preserve an explicit mapping
  // when it is a valid Clover profile identifier; otherwise fall back to the
...
function localPrimaryRequestScope(opts: ProfileRouteOptions): boolean | null {
  ...
  // spawned them: `_spawn_clover_action` registers the (often dynamic, e.g.
 ...
 *     `--profile`, so its `CLOVER_HOME` scopes it.
 *
 ...
 * and by the registry SSH branch of the `clover:api` handler — both routes
...
function translateSelfProfileQuery(path, profile, backendProfile) {
  ...
  try {
    parsed = new URL(rawPath, 'http://clover.local')
  } catch {
    return path
  }
...
/**
 * Resolve the two decisions made by the `clover:api` IPC handler from the same
...
/**
 * True if any cookie in `cookies` is a clover session ACCESS-token cookie
