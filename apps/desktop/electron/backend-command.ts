// Backend subcommand routing for the desktop-managed Clover process.
//
// The desktop app launches its own headless backend via `clover serve` — it
...
// app hasn't updated yet, or an older `clover` resolved from PATH) only knows
 ...
 * @param {string} [profile] optional Clover profile to pin via `--profile`.
 */
 ...
 * `-m clover_cli.main` and any `--profile <name>`). Returns a copy; if there is
...
/**
 * True when a runtime's `clover_cli/subcommands/dashboard.py` source registers
