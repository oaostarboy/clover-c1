...
test('error page names the failure and carries a Reload button', () => {
  ...
  assert.match(html, /Clover couldn.t start the desktop UI/)
  ...
  assert.match(html, /clover desktop --force-build/)
...
test('error page reload button targets the real renderer URL when provided', () => {
  const html = buildRendererLoadErrorPage({
    ...
    reloadUrl: 'file:///C:/Clover%20Agent/dist/index.html'
  })
  ...
  assert.match(html, /location\.replace\("file:\/\/\/C:\/Clover%20Agent\/dist\/index\.html"\)/)
