 ...
 * the three user-facing uninstall modes to the `clover uninstall` CLI flags,
...
/**
 * Map an uninstall mode to the `python -m clover_cli.uninstall` argv (after the
 * python executable). Uses the dedicated lightweight module entrypoint (not
 * `clover_cli.main`) so it can run under a system Python OUTSIDE the venv that
...
function uninstallArgsForMode(mode) {
  ...
  return ['-m', 'clover_cli.uninstall', '--mode', mode]
}
 ...
 *
 *   macOS:   …/Clover.app/Contents/MacOS/Clover  → …/Clover.app
 *   Windows: …\Clover\Clover.exe                 → …\Clover  (install dir)
...
function resolveRemovableAppPath(execPath, platform, env: any = {}) {
  ...
  if (platform === 'darwin') {
    // …/Clover.app/Contents/MacOS/Clover → strip 3 segments to the .app
    const macOsDir = p.dirname(exe) // …/Contents/MacOS
    const contents = p.dirname(macOsDir) // …/Contents
    const appBundle = p.dirname(contents) // …/Clover.app
  ...
  if (platform === 'win32') {
    // NSIS per-user installs Clover.exe directly in the install dir.
    const dir = p.dirname(exe)

    if (/[\\/]Clover$/i.test(dir) || /[\\/]clover-desktop$/i.test(dir)) {
      return dir
    }
  ...
  // Unpacked electron-builder tree: …/linux-unpacked/clover
  const dir = p.dirname(exe)
 ...
 * being deleted); `pythonPath` is prepended to PYTHONPATH so `import clover_cli`
 ...
 */
function buildPosixCleanupScript({ desktopPid, pythonExe, pythonPath, agentRoot, uninstallArgs, appPath, cloverHome }) {
  ...
  const lines = [
    ...
    '  done',
    'fi',
    `export CLOVER_HOME=${q(cloverHome)}`
  ]
 ...
 * modes + `pythonPath`=agentRoot so `import clover_cli` resolves from source
...
function buildWindowsCleanupScript({
  ...
  // a problem, but Clover install paths don't use them.
  const q = s => `"${String(s).replace(/"/g, '')}"`

  const lines = [
    '@echo off',
    'setlocal enableextensions',
    `set "CLOVER_HOME=${String(cloverHome).replace(/"/g, '')}"`,
    `set "PID=${pid}"`
  ]
