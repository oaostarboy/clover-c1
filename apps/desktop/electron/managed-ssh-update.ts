 ...
 * wire protocol:
 *
 *   gate dials -> drain every captured scope -> detached `clover update`
...
function buildPosixManagedUpdateLaunch(target: RemoteUpdateTarget, correlationId: string): string {
  ...
  const home = validateRemoteValue(target.cloverHome, 'Clover home')
  const cloverPath = validateRemoteValue(target.cloverPath, 'launcher path')
  ...
  const launcherWord = expandRemotePath(cloverPath)

  const updateCommand =
    `env CLOVER_HOME=${homeWord} ` +
    `CLOVER_UPDATE_CORRELATION_ID=${shq(correlation)} ` +
    'CLOVER_UPDATE_ORIGIN_PROFILE=default ' +
    `CLOVER_UPDATE_ORIGIN_HOME=${homeWord} ` +
    `CLOVER_UPDATE_OUTPUT_PATH=${outputWord} ` +
...
function buildWindowsManagedUpdateLaunch(target: RemoteUpdateTarget, correlationId: string): string {
  ...
  const wrapper = [
    ...
    `$env:CLOVER_HOME=${psLiteral(home)}`,
    `$env:CLOVER_UPDATE_CORRELATION_ID=${psLiteral(correlation)}`,
    '$env:CLOVER_UPDATE_ORIGIN_PROFILE="default"',
    `$env:CLOVER_UPDATE_ORIGIN_HOME=${psLiteral(home)}`,
    `$env:CLOVER_UPDATE_OUTPUT_PATH=${psLiteral(outputPath)}`,
    ...
    `$env:CLOVER_UPDATE_WINDOWS_DETACHED=${psLiteral(correlation)}`,
    `$env:CLOVER_UPDATE_TAURI_OUTCOME_PATH=${psLiteral(statusPath)}`,
    `$env:CLOVER_UPDATE_TAURI_READY_PATH=${psLiteral(readyPath)}`,
    ...
    `& ${psLiteral(cloverPath)} update --yes *>> ${psLiteral(outputPath)}`,
    '$rc=$LASTEXITCODE',
...
marker_path=install_root/'.clover-update-in-progress'
...
async function runManagedSshUpdate<TScope extends ManagedSshScope>(
  ...
  return {
    ...
    message:
      outcome === 'updated'
        ? 'Remote Clover updated and every managed SSH profile is ready.'
        : restoreOk
