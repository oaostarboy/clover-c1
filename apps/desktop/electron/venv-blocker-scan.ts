...
const SCAN_MODULE = 'clover_cli._scan_venv_blockers'
 ...
 * preview servers. Unknown Python/Clover processes are deliberately ignored.
 */
export async function stopSafeVenvBlockers(
  ...
  for (const process of result.processes) {
    ...
    try {
      await execFn(
        pythonPath,
        ['-m', 'clover_cli._scan_venv_blockers', '--terminate-safe', String(process.pid), String(process.createTime)],
...
export function formatBlockerMessage(result: VenvBlockerScanResult): string {
  const lines = [
    'Update aborted: another Clover process is using this installation.',
    '',
...
export function formatProbeFailedMessage(): string {
  return (
    'Update aborted: Desktop could not verify the Clover installation is free.\n' +
    '\n' +
    'Close other Clover windows and terminals, then retry.  If the problem\n' +
    'persists, run `clover update` in a terminal for detailed diagnostics.'
  )
}
