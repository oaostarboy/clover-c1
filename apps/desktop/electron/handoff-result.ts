 ...
 * console. It writes CLOVER_HOME/.clover-update-result.json on every exit
 ...
 * ever surfaces, and the user may not reopen Clover within 30 minutes.
...
export function handoffResultPath(cloverHome: string): string {
  return path.join(cloverHome, '.clover-update-result.json')
}
...
export function readAndConsumeHandoffResult(
  ...
  const file = handoffResultPath(cloverHome)
  let raw: string
