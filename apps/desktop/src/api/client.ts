import { JsonRpcGatewayClient } from '@clover/shared'

import type { CloverApiRequest } from '@/global'
...
// "Timed out connecting to Clover backend" that hangs the UI (#48504).
//
...
export class CloverGateway extends JsonRpcGatewayClient {
  constructor() {
    super({
      closedErrorMessage: 'Clover gateway connection closed',
      connectErrorMessage: 'Could not connect to Clover gateway',
      createRequestId: nextId => nextId,
      notConnectedErrorMessage: 'Clover gateway is not connected',
...
// process to route them to that host (clover:api's registry branch). Null
...
export function cloverApi<T>(request: CloverApiRequest): Promise<T> {
  return window.cloverDesktop.api<T>({ ...connectionScoped(), ...request })
}
