...
import { getApiRequestConnection, getApiRequestProfile, cloverApi, profileScoped } from './client'
...
async function activeConnection(): Promise<CloverConnection> {
  ...
  return window.cloverDesktop.getConnection(getApiRequestProfile())
}

/** Options for a plugin REST call — mirrors the app's own `cloverDesktop.api`
 *  shape, minus the path (which is namespace-derived). */
export interface PluginRestOptions {
  method?: string
  body?: unknown
  /** Single-file multipart upload (see CloverApiRequest.upload). */
...
export async function pluginRest<T>(pluginId: string, path: string, opts: PluginRestOptions = {}): Promise<T> {
  if (!window.cloverDesktop?.api) {
    throw new Error('Clover desktop bridge unavailable')
  }
