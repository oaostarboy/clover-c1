...
import { capabilityScoped, cloverApi, type ProfileScope } from './client'
...
export async function listSessions(
  ...
  const result = await cloverApi<PaginatedSessions>({
    path:
...
export async function listSidebarSessions(req: SidebarSessionsRequest): Promise<SidebarSessionsResponse> {
  ...
  try {
    result = await cloverApi<SidebarSessionsResponse>({
...
export function searchSessions(query: string): Promise<SessionSearchResponse> {
  return cloverApi<SessionSearchResponse>({
...
export function getSessionMessages(
  ...
  return cloverApi<SessionMessagesResponse>({
    ...sessionScope,
...
export function renameSession(
  ...
  return cloverApi<{ ok: boolean; title: string }>({
    ...(profile ? { profile } : {}),
    path: `/api/sessions/${encodeURIComponent(id)}`,
    method: 'PATCH',
