...
export interface I18nConfigClient {
  getConfig: () => Promise<CloverConfigRecord>
  saveConfig: (config: CloverConfigRecord) => Promise<{ ok: boolean }>
}

const defaultConfigClient: I18nConfigClient = {
  getConfig: () => {
    if (typeof window === 'undefined' || !window.cloverDesktop?.api) {
      return Promise.resolve({})
    }

    return getCloverConfigRecord()
  },
  saveConfig: config => {
    if (typeof window === 'undefined' || !window.cloverDesktop?.api) {
    ...
    return saveCloverConfig(config)
  }
}
...
export function getConfigDisplayLanguage(config: CloverConfigRecord): unknown {
  return isRecord(config.display) ? config.display.language : undefined
}

export function withConfigDisplayLanguage(config: CloverConfigRecord, locale: Locale): CloverConfigRecord {
