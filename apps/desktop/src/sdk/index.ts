/**
 * @clover/plugin-sdk — THE plugin language. The vscode-module model: plugin
 ...
 *    object as `window.__CLOVER_PLUGIN_SDK__` and maps the import to it, so a
...
import { cloverApi, type CloverGateway, deleteProfile, getLogs, getStatus } from '@/clover'
import {
  $narrowViewport,
  $newSessionTabAction,
  $paneVisible,
...
import type { PaginatedSessions, UsageStats } from '@/types/clover'
...
export interface PluginProfileRoute {
  ...
  profile: string
  /** Backend Clover profile served by that route. */
...
async function requestPluginProfile<T>(
  ...
  const getAgentRoster = window.cloverDesktop?.getAgentRoster
...
async function pluginRouteStillRegistered(route: PluginProfileRoute): Promise<boolean> {
  const getProfileRoutes = window.cloverDesktop?.getProfileRoutes
...
//
// ensureGatewayProfile awaits the store's dial, and CloverGateway.connect() has
...
export const host = {
  ...
  deleteProfile: async (profile: string | PluginProfileRoute): Promise<void> => {
    ...
    // restores on relaunch and re-creates the deleted profile (clover-c1#94235).
    dropTilesForProfile(
  ...
  connections: async () => {
    const bridge = window.cloverDesktop?.connections

    if (!bridge) {
      throw new Error('This Desktop build has no connection registry. Update Clover Desktop.')
    }
  ...
  agents: async () => {
    const roster = window.cloverDesktop?.getAgentRoster

    if (!roster) {
      throw new Error('This Desktop build cannot enumerate multi-source agents. Update Clover Desktop.')
    }
  ...
  newChat: (profile?: null | string | PluginProfileRoute, options: PluginNewChatOptions = {}): void => {
    if (options.workspaceMode === 'bots') {
      ...
      if (!openTab) {
        notify({ kind: 'error', message: 'Update Clover Desktop to open another Bot chat.' })
  ...
  profileRoutes: async () => {
    ...
    if (!getProfileRoutes) {
      throw new Error('Clover Desktop connection routing unavailable')
    }
  ...
  listPersistedSessions: async (
    ...
    return cloverApi<PaginatedSessions>({
  ...
  setPersistedSessionHidden: async (
    ...
    return cloverApi<{ ok: boolean; hidden: boolean }>({
      ...(route ? { connectionId: route.connectionId } : {}),
  ...
  request: async <T>(method: string, params: Record<string, unknown> = {}): Promise<T> => {
    ...
    if (!gateway) {
      throw new Error('Clover gateway unavailable')
    }
...
export type { CloverGateway } from '@/clover'
/** The oversized Collapse lettering an empty chat is titled with — core writes
 *  "CLOVER AGENT" with it, a `chat.empty` contribution writes its own name. */
...
export type { CloverOpenTarget } from '@/lib/clover-open-target'
...
/** Clover' reasoning levels + their compact labels, so a plugin surfacing a
...
export type { RpcEvent, StatusResponse } from '@/types/clover'
