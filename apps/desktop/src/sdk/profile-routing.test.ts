...
vi.mock('@/clover', () => ({ deleteProfile: vi.fn(), getLogs: vi.fn(), getStatus: vi.fn(), cloverApi: vi.fn() }))
...
const { deleteProfile, cloverApi } = await import('@/clover')
...
describe('connection-aware plugin host APIs', () => {
  it('retires a profile gateway before deleting it', async () => {
    ...
    // profile's backend, re-creating its CLOVER_HOME (#94235).
  ...
  it('refreshes the profile inventory before asking Electron for routes', async () => {
    ...
    ;(window as unknown as { cloverDesktop: unknown }).cloverDesktop = { getProfileRoutes }
  ...
  it('reads and hides persisted sessions through the source primary without activating the profile', async () => {
    ...
    expect(cloverApi).toHaveBeenNthCalledWith(
      1,
    ...
    expect(cloverApi).toHaveBeenNthCalledWith(2, {
      connectionId: 'source-a',
      path: '/api/sessions/bot-chat',
      method: 'PATCH',
      body: { hidden: true, profile: 'backend-worker' }
    })
    expect(vi.mocked(cloverApi).mock.calls.every(([request]) => !('profile' in request))).toBe(true)
