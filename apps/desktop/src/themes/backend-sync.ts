/**
 * Live skin sync from the Clover backend.
 *
 * The backend resolves the active skin (built-in or `$CLOVER_HOME/skins/*.yaml`)
 ...
 * stomps the user's persisted desktop theme; only a genuine name change (Clover
 ...
 */
export function ingestBackendSkin(skin: CloverSkin | undefined | null, { apply }: { apply: boolean }): void {
  ...
  // (clover), so we never register a converted theme under `default`. It is still a
  // valid apply TARGET though: a runtime switch back to `default` must repaint the
  // desktop to its own default (setTheme normalizes `default` → clover). So we only
  ...
  if (name !== 'default' && !BUILTIN_THEMES[name]) {
    const theme = skinToDesktopTheme(skin as CloverSkin)
