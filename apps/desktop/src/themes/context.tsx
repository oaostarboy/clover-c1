...
import { BUILTIN_THEME_LIST, cloverTheme, DEFAULT_SKIN_NAME, DEFAULT_TYPOGRAPHY } from './presets'
...
const SKIN_KEY = 'clover-desktop-theme-v2'
const MODE_KEY = 'clover-desktop-mode-v1'
...
const PROFILE_SKINS_KEY = 'clover-desktop-profile-themes-v1'
const PROFILE_MODES_KEY = 'clover-desktop-profile-modes-v1'
...
function applyTheme(theme: DesktopTheme, mode: 'light' | 'dark') {
  ...
  root.dataset.cloverTheme = skinName
  ...
  window.cloverDesktop?.setTitleBarTheme?.({
    background: chromeBg,
  ...
  try {
    window.localStorage.setItem('clover-boot-background', chromeBg)
    window.localStorage.setItem('clover-boot-color-scheme', rendered)
  } catch {
  ...
  if (typo.fontUrl && !INJECTED_FONT_URLS.has(typo.fontUrl)) {
    ...
    link.dataset.cloverThemeFont = 'true'
...
const syncNativeTheme = (pref: ThemeMode, rendered: 'light' | 'dark') =>
  window.cloverDesktop?.setNativeTheme?.(pref === 'system' ? 'system' : rendered)
...
export function ThemeProvider({ children }: { children: ReactNode }) {
  ...
  // Drain a backend-driven skin switch (Clover authoring/activating a skin from a
