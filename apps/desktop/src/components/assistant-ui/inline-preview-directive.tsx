 ...
 * own design.
 *
 * WIDGETS TALK BACK OFF-SCREEN. `window.clover.send(prompt)` (or declarative
 * `data-clover-send` on any clickable element) routes the prompt through the
...
const SIZE_MESSAGE_TYPE = 'clover-inline-preview-size'
const INTENT_MESSAGE_TYPE = 'clover-inline-preview-intent'
...
/** The script that gives the widget its ONE voice: `clover.send(prompt)`.
 ...
 *  wires `data-clover-send` so declarative HTML works with zero script:
 *  `<button data-clover-send="get-price eth">ETH</button>`. */
export function intentScript(token: string): string {
  return (
    ...
    ')},"*");return true}' +
    'window.clover={send:send};' +
    'addEventListener("click",function(e){var el=e.target&&e.target.closest?' +
    'e.target.closest("[data-clover-send]"):null;' +
    'if(el)send(el.getAttribute("data-clover-send")||"")},true)})()</script>'
  )
}
...
export function InlinePreviewDirective({
  ...
  // URL connection — including a same-machine `clover serve` — renders live.)
