...
const CLOVER_REF_TYPES = WIRE_REFERENCE_KINDS
type CloverRefType = (typeof CLOVER_REF_TYPES)[number]
...
const CLOVER_DIRECTIVE_RE = referenceRe()
...
export const cloverDirectiveFormatter: Unstable_DirectiveFormatter = {
...
function parseDirectiveText(text: string): Unstable_DirectiveSegment[] {
  const matches = [
    ...
    ...Array.from(text.matchAll(CLOVER_DIRECTIVE_RE)).map(match => {
...
function safeDirectiveSegments(text: string): Unstable_DirectiveSegment[] {
  try {
    return [...cloverDirectiveFormatter.parse(text)]
  } catch {
...
/**
 * Renders text containing Clover directives (`@file:...`, `@image:...`) as
...
const DirectiveImage: FC<{ id: string; label: string }> = ({ id, label }) => {
  ...
  useEffect(() => {
    ...
    const load =
      window.cloverDesktop && isRemoteGateway() ? gatewayMediaDataUrl(id) : window.cloverDesktop?.readFileDataUrl(id)
