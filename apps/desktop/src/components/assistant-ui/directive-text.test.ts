...
import { cloverDirectiveFormatter, formatRefValue } from './directive-text'
...
describe('cloverDirectiveFormatter.parse', () => {
  it('keeps quoted file paths whole when parsing', () => {
    const segments = cloverDirectiveFormatter.parse('see @image:`apple-touch-icon (1).png` for the icon')
  ...
  it('still parses unquoted paths', () => {
    const segments = cloverDirectiveFormatter.parse('@file:src/main.tsx the entry point')
  ...
  it('parses session links with profile/id values', () => {
    const segments = cloverDirectiveFormatter.parse('see @session:work/20260101_abc123 next')
...
describe('inline skill references', () => {
  const skills = (text: string) =>
    [...cloverDirectiveFormatter.parse(text)]
  ...
  it('keeps the surrounding prose as text around the chip', () => {
    const segments = cloverDirectiveFormatter.parse('tidy this with /clean thanks')
  ...
  it('parses a skill chip alongside an @ reference', () => {
    const mentions = [...cloverDirectiveFormatter.parse('run /clean on @file:`src/a.ts`')].filter(
