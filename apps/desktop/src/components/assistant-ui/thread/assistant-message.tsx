...
const ErrorRecoveryActions: FC = () => {
  ...
  // Open Logs reveals the LOCAL Electron profile's CLOVER_HOME/logs. On a
  ...
  const openLogs = useCallback(async () => {
    try {
      const root = await window.cloverDesktop?.logsRoot?.()
      ...
      const result = await window.cloverDesktop?.openDir?.(root)
