...
describe('delivery command detection', () => {
  it('matches the canonical delivery command', () => {
    const cmd = 'clover -p turqoise chat --in ~ -c "Bot Chat" -Q -q "Message from 🤖 Clover (@clover): hi there"'

    expect(deliveryTargetFromCommand(cmd)).toBe('turqoise')
  })

  it('matches with a cd prefix and timeout wrapper', () => {
    const cmd = 'cd ~ && timeout 240 clover -p mr-tester chat --in "~" -Q -q "Message from 🤖 Clover: hello"'
  ...
  it('ignores ordinary terminal commands', () => {
    ...
    expect(deliveryTargetFromCommand('clover -p turqoise chat -q "plain question"')).toBeNull()
    expect(deliveryTargetFromCommand('clover sessions list')).toBeNull()
  })
})

describe('reply extraction', () => {
  it('strips session_id bookkeeping and keeps the reply', () => {
    const output = 'session_id: 20260813_220347_f69ac6\nHi Clover! Good to hear from you.'

    expect(replyTextFromResult({ output })).toBe('Hi Clover! Good to hear from you.')
  })
