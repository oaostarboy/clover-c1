...
function failBoot() {
  $desktopBoot.set({
    error: 'Could not connect to Clover gateway',
    fakeMode: false,
    message: 'boot failed',
...
function stubDesktop(config: Record<string, unknown>, overrides: Record<string, unknown> = {}) {
  const original = window.cloverDesktop
  Object.defineProperty(window, 'cloverDesktop', {
    configurable: true,
    value: { getRecentLogs: async () => ({ lines: [] }), getConnectionConfig: async () => config, ...overrides }
  })

  return () => Object.defineProperty(window, 'cloverDesktop', { configurable: true, value: original })
}
...
describe('BootFailureOverlay', () => {
  ...
  it('recovers a cloud connection through the portal cascade instead of native OAuth', async () => {
    const gatewayUrl = 'https://agent-1.agents.'
    const logout = vi.fn().mockResolvedValue({ ok: true, connected: false })
    const nativeLogin = vi.fn().mockResolvedValue({ ok: true, connected: false })
    const cloudStatus = vi.fn().mockResolvedValue({ portalBaseUrl: '', signedIn: false })
    ...
    const restore = stubDesktop(
      ...
      },
      {
        ...
        oauthLogoutConnectionConfig: logout,
        probeConnectionConfig: vi.fn().mockResolvedValue({ providers: [{ id: 'clover', type: 'oauth' }] })
      }
    )
  ...
  it('shows the Clover Cloud down recovery when the backend flags isCloudBackendDown', async () => {
    ...
    try {
      ...
      // remote-failure copy.
      expect(await screen.findByText(/Clover Cloud agent is down/i)).toBeTruthy()
