          ...
          <span
            className={cn(
              ...
              copied
                ...
                : 'border-(--stroke-clover) text-foreground group-hover:border-(--ui-stroke-secondary)'
            )}
            key={i}
          >
            {ch}
...
function CodeBlock({ copied, onCopy, text }: { copied: boolean; onCopy: () => void; text: string }) {
  ...
  return (
    <div className="flex items-center justify-between gap-3 rounded-md border border-(--stroke-clover) px-3 py-2">
