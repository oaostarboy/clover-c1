...
export function BootFailureOverlay() {
  ...
  const retry = async () => {
    setBusy('retry')
    await window.cloverDesktop?.resetBootstrap().catch(() => undefined)
    window.location.reload()
  }

  const repair = async () => {
    setBusy('repair')
    await window.cloverDesktop?.repairBootstrap().catch(() => undefined)
    window.location.reload()
  }
  ...
  const switchToLocalGateway = async () => {
    ...
    await window.cloverDesktop?.applyConnectionConfig({ mode: 'local' }).catch(() => undefined)
    setBusy(null)
  }
  ...
  const signInRemote = async () => {
    ...
    try {
      await window.cloverDesktop?.oauthLogoutConnectionConfig?.()
      const result = await window.cloverDesktop?.oauthLoginConnectionConfig(remoteReauth.url)
  ...
  const openLogs = () => void window.cloverDesktop?.revealLogs().catch(() => undefined)
  ...
  if (view === 'connect') {
    return (
      <div
        ...
        data-glass-opaque=""
      >
        <div className="flex max-h-[86vh] w-full max-w-[46rem] flex-col overflow-hidden rounded-xl border border-(--stroke-clover) bg-(--ui-chat-bubble-background) shadow-clover">
  ...
  return (
    <div
      ...
      data-glass-opaque=""
    >
      <div className="w-full max-w-[40rem] overflow-hidden rounded-xl border border-(--stroke-clover) bg-(--ui-chat-bubble-background) shadow-clover">
