import type { GatewayWsUrlResult } from '@clover/shared'
import type { TranslucencyState } from '@clover/shared/translucency'
...
declare global {
  interface Window {
    cloverDesktop: {
      ...
      getConnection: (profile?: string | null) => Promise<CloverConnection>
      ...
      // Resume this session in the user's own terminal emulator (`clover --tui
      ...
      // Clover Cloud: one portal login powers discovery + silent per-agent
      ...
      profile: {
        ...
        // backend under the new CLOVER_HOME (reloads the window). Pass null to
        // clear the preference.
        set: (name: string | null) => Promise<DesktopActiveProfile>
      }
      api: <T>(request: CloverApiRequest) => Promise<T>
      notify: (payload: CloverNotification) => Promise<boolean>
      ...
      readFileText: (filePath: string) => Promise<CloverReadFileTextResult>
      ...
      readPluginSource?: (filePath: string) => Promise<CloverReadFileTextResult>
      selectPaths: (options?: CloverSelectPathsOptions) => Promise<string[]>
      ...
      normalizePreviewTarget: (target: string, baseDir?: string) => Promise<CloverPreviewTarget | null>
      watchPreviewFile: (url: string) => Promise<CloverPreviewWatch>
      ...
      watchDirectory?: (dir: string) => Promise<CloverPreviewWatch>
      stopPreviewFileWatch: (id: string) => Promise<boolean>
      setActiveWork?: (payload: CloverActiveWork) => void
      setTitleBarTheme?: (payload: CloverTitleBarTheme) => void
      ...
      readDir: (path: string) => Promise<CloverReadDirResult>
      ...
      // Local Desktop runtime-plugin root (<CLOVER_HOME>/desktop-plugins),
      ...
      /** LOCAL `<CLOVER_HOME>/logs` (profile-aware) — error card "Open Logs". */
      logsRoot?: () => Promise<string>
      // Local AGENT-plugin root (<CLOVER_HOME>/plugins), same Electron-local
      ...
      git?: {
        worktreeList: (repoPath: string) => Promise<CloverGitWorktree[]>
        worktreeAdd: (
          repoPath: string,
        ...
        branchList: (repoPath: string) => Promise<CloverGitBranch[]>
        ...
        baseBranchList: (repoPath: string) => Promise<CloverGitBaseBranch[]>
        ...
        repoStatus: (repoPath: string) => Promise<CloverRepoStatus | null>
        ...
        review: {
          list: (repoPath: string, scope: CloverReviewScope, baseRef?: null | string) => Promise<CloverReviewList>
          diff: (
            repoPath: string,
            filePath: string,
            scope: CloverReviewScope,
            baseRef?: null | string,
            staged?: boolean
          ) => Promise<string>
          ...
          shipInfo: (repoPath: string) => Promise<CloverReviewShipInfo>
          ...
          prList: (repoPath: string, branches: string[], numbers?: number[]) => Promise<CloverRepoPullRequests>
          ...
          fetchPrComment: (repoPath: string, url: string) => Promise<CloverPrComment | null>
      ...
      terminal: {
        ...
        onExit: (id: string, callback: (payload: CloverTerminalExit) => void) => () => void
        resize: (id: string, size: { cols: number; rows: number }) => Promise<boolean>
        start: (options?: { cols?: number; cwd?: string; rows?: number }) => Promise<CloverTerminalSession>
      ...
      onWindowStateChanged?: (callback: (payload: CloverWindowState) => void) => () => void
      ...
      onPreviewFileChanged: (callback: (payload: CloverPreviewFileChanged) => void) => () => void
...
export interface CloverTerminalSession {
  cwd: string
  id: string
  shell: string
}

export interface CloverTerminalExit {
  code: number | null
  signal: string | null
}
...
export interface DesktopUpdateApplyResult {
   ...
   *  `clover update` themselves. `command` is the exact line to run. */
  manual?: boolean
  command?: string
  cloverRoot?: string
...
export interface CloverTitleBarTheme {
  background: string
  foreground: string
}

/** Turns in flight, so the main process can confirm before a quit kills them. */
export interface CloverActiveWork {
  count: number
  titles: string[]
}

export interface CloverWindowState {
  darwinMajor?: number
  isFullscreen: boolean
  isMinimized?: boolean
  isVisible?: boolean
...
export interface DesktopConnectionConfig {
  envOverride: boolean
  // The saved connection mode. 'cloud' is a Clover Cloud connection: it carries
  ...
  remoteUrl: string
  // For a 'cloud' connection: the persisted Clover Cloud org (slug or id) the
...
export interface DesktopConnectionConfigInput {
  ...
  remoteUrl?: string
  // For a 'cloud' connection: the selected Clover Cloud org (slug or id) to
...
export interface DesktopCloudStatus {
  ...
  // Whether the OAuth partition holds a live Clover portal (Privy) session — the
  // portal authenticates via Privy, so this reflects the privy-token cookie, NOT
  // the clover gateway session cookies. See cookiesHavePrivySession.
  signedIn: boolean
}

// A discovered Clover Cloud agent — the trimmed DTO from NAS GET /api/agents.
...
export interface DesktopBootProgress {
  error: string | null
  fakeMode: boolean
  /** True when the boot failure is a Clover Cloud agent that is down (HTTP 502/503/504). */
...
export interface CloverApiRequest {
  path: string
  method?: string
  body?: unknown
...
export interface CloverNotification {
  title?: string
  body?: string
  silent?: boolean
  kind?: string
...
export interface CloverPreviewTarget {
  binary?: boolean
  byteSize?: number
  kind: 'file' | 'url'
  label: string
...
export interface CloverReadFileTextResult {
  binary?: boolean
  byteSize?: number
  language?: string
  mimeType?: string
...
export interface CloverPreviewWatch {
  id: string
  path: string
}
...
export interface CloverGitWorktree {
  path: string
  branch: null | string
  isMain: boolean
  detached: boolean
...
export interface CloverGitBranch {
  name: string
  checkedOut: boolean
  isDefault: boolean
  isRemote: boolean
...
// preselect it.
export interface CloverGitBaseBranch {
  name: string
  isRemote: boolean
  isDefault: boolean
}
...
export interface CloverRepoStatusFile {
  path: string
  staged: boolean
  unstaged: boolean
  untracked: boolean
...
export interface CloverRepoStatus {
  branch: null | string
...
// turn began.
export type CloverReviewScope = 'branch' | 'lastTurn' | 'uncommitted'

// One changed file in the review pane (status letter, +/- lines, staged flag).
export interface CloverReviewFile {
  path: string
  added: number
  removed: number
...
export interface CloverReviewList {
  files: CloverReviewFile[]
...
export interface CloverReviewPr {
  url: string
  state: string
  number: number
}
...
export interface CloverBranchPullRequest {
  branch: string
  draft: boolean
  number: number
...
export interface CloverRepoPullRequests {
  ghReady: boolean
  prs: CloverBranchPullRequest[]
}
...
export interface CloverPrComment {
  author: string
  body: string
  diffHunk: string
  kind: 'issue' | 'review'
...
export interface CloverReviewShipInfo {
  ghReady: boolean
  pr: CloverReviewPr | null
}

export interface CloverReadDirEntry {
  name: string
  path: string
  isDirectory: boolean
}

export interface CloverReadDirResult {
  entries: CloverReadDirEntry[]
  error?: string
}

export interface CloverPreviewFileChanged {
  id: string
  path: string
  url: string
}

export interface CloverSelectPathsOptions {
  title?: string
  defaultPath?: string
  directories?: boolean
  multiple?: boolean
