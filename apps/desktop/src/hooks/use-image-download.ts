...
function isMissingIpcHandler(error: unknown): boolean {
  ...
  return message.includes("No handler registered for 'clover:saveImageFromUrl'")
}
...
export function useImageDownload(src?: string) {
  ...
  const download = useCallback(async () => {
    ...
    try {
      if (window.cloverDesktop?.saveImageFromUrl) {
        if (await window.cloverDesktop.saveImageFromUrl(src)) {
