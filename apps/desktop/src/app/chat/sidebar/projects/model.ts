...
// zero Clover sessions, and those arrive continuously — so once the user dragged
...
export function useRepoWorktreeMap(
  repoPaths: string[],
  enabled: boolean
): [Record<string, CloverGitWorktree[]>, boolean] {
  const [map, setMap] = useState<Record<string, CloverGitWorktree[]>>({})
