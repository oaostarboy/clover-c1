        ...
        <>
          <a
            ...
            onClick={event => {
              ...
              void window.cloverDesktop?.openExternal(error.url)
            }}
          >
...
export function PreviewPane({ embedded = false, onRestartServer, reloadRequest = 0, tabId, target }: PreviewPaneProps) {
  ...
  const restartServer = useCallback(async () => {
    ...
    // "Ask Clover to restart the server" looked like it did nothing —
  ...
  useEffect(() => {
    if (
      ...
      !window.cloverDesktop?.watchPreviewFile ||
      !window.cloverDesktop?.onPreviewFileChanged
    ) {
      return
    }
    ...
    const unsubscribe = window.cloverDesktop.onPreviewFileChanged(payload => {
    ...
    void window.cloverDesktop
      ...
      .then(watch => {
        if (!active) {
          void window.cloverDesktop?.stopPreviewFileWatch?.(watch.id)
    ...
    return () => {
      ...
      if (watchId) {
        void window.cloverDesktop?.stopPreviewFileWatch?.(watchId)
      }
    }
  ...
  useEffect(() => {
    ...
    webview.setAttribute('partition', 'persist:clover-preview')
    ...
    const onGuestContextMenu = (event: Event) => {
      ...
      const zoom = window.cloverDesktop?.zoom?.factor?.() || 1
        ...
        },
        {
          addToDictionary: (word: string) => {
            ...
            if (typeof webContentsId === 'number') {
              void window.cloverDesktop?.contextMenuGuestAddWord?.({ webContentsId, word })
            }
          },
          copyImage: () => void window.cloverDesktop?.contextMenuCopyImage?.(),
      ...
      <div className="flex min-h-0 flex-1 flex-col overflow-hidden">
        ...
        {isWebPreview && !isRemoteHtml && (
          <PreviewBrowserBar
            ...
            onOpenExternal={() => void window.cloverDesktop?.openExternal(currentUrl)}
