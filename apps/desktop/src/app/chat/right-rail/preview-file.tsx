...
import { CLOVER_PATHS_MIME } from '@/app/chat/hooks/use-composer-actions'
...
async function readTextPreview(filePath: string) {
  try {
    ...
    if (!message.includes("No handler registered for 'clover:readFileText'")) {
      throw error
    }
  }
  ...
  const dataUrl = await window.cloverDesktop.readFileDataUrl(filePath)
...
function startLineDrag(event: ReactDragEvent<HTMLElement>, filePath: string, { end, start }: LineSelection) {
  ...
  event.dataTransfer.setData(CLOVER_PATHS_MIME, JSON.stringify([{ line: start, lineEnd, path: filePath }]))
