...
const preamble = () => `  var w = window;
  ...
  w.__cloverActHolder = w.__cloverActHolder || {};
  w.__cloverAct = ${actEngineSource()};
  w.__cloverWatch_fn = (${watchInPage.toString()});
  w.__cloverWatchTag = ${WATCH_TAG};
  var holder = w.__cloverActHolder;
  var act = function (a) { return w.__cloverAct(document, holder, a); };
  var watch = function (stage, label) {
    try { w.__cloverWatch_fn(document, holder, stage, label); } catch (err) {}
  };
...
${preamble()}
  ...
  document.addEventListener('pointerdown', function (e) {
    w.__cloverHit = { tag: e.target ? e.target.tagName : '?', trusted: e.isTrusted === true };
...
function hitNote(hit?: { tag: string; trusted: boolean } | null): string | undefined {
  return hit && hit.tag === 'CLOVER-WATCH' ? 'The action overlay intercepted the click instead of the page.' : undefined
}
