...
const canRunInTerminal = () => typeof window !== 'undefined' && Boolean(window.cloverDesktop?.terminal)
...
//      in clover_cli/provider_catalog.py) — the SAME provider identity
//      `clover model` uses. This is authoritative: a provider tagged by the
...
function ConnectedProviderRow({
  ...
  // Clover can clear this provider's creds via the API.
  const canDisconnect = provider.disconnectable ?? provider.flow !== 'external'
  // External (CLI-managed) provider Clover can't clear via the API, but ships a
...
export function ProvidersSettings({
  ...
  // Clover never deletes creds another tool owns behind a silent API call.
