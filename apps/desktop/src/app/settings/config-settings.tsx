...
import type { ConfigFieldSchema, CloverConfigRecord } from '@/types/clover'

import { cloverConfigCacheWriter, useCloverConfigRecord } from '../hooks/use-config-record'
...
function ConfigSettingsInner({
  ...
  const [config, setConfig] = useState<CloverConfigRecord | null>(null)
  const { data: loadedConfig, isError: configLoadFailed, refetch: refetchConfig } = useCloverConfigRecord(scopeProfile)
  ...
  const writeConfigCache = useMemo(() => cloverConfigCacheWriter(scopeProfile), [scopeProfile])
  ...
  const {
    ...
    queryKey:
      scopeProfile == null ? ['clover-config-schema'] : ['clover-config-schema', normalizeProfileKey(scopeProfile)],
    queryFn: () => getCloverConfigSchema(scopeProfile),
    staleTime: 5 * 60 * 1000
  })
  ...
  useEffect(() => {
    ...
    const t = window.setTimeout(() => {
      void (async () => {
        try {
          const result = await saveCloverConfig(config, scopeProfile)
  ...
  const applyConfig = (next: CloverConfigRecord) => {
    saveVersionRef.current += 1
    setConfig(next)
    setSaveVersion(saveVersionRef.current)
  }

  const updateConfig = (next: CloverConfigRecord) => {
