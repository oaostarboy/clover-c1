...
import type { CloverConfigRecord, SessionInfo } from '@/types/clover'
...
function AutoArchiveSetting() {
  ...
  const persist = useCallback(
    async (autoArchive: boolean, archiveDays: number) => {
      ...
      try {
        await saveCloverConfig(updated)
      } catch (err) {
...
// / Program Files), which buried any files Clover wrote there.
function DefaultProjectDirSetting() {
  ...
  useEffect(() => {
    ...
    // / Storybook / non-Electron context `window.cloverDesktop` is
    ...
    const settings = window.cloverDesktop?.settings
