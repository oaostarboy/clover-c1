...
import type { EnvVarInfo, OAuthProvider } from '@/types/clover'
...
function provider(id: string, loggedIn: boolean, patch: Partial<OAuthProvider> = {}): OAuthProvider {
  return {
    cli_command: `clover auth add ${id}`,
    disconnectable: true,
    docs_url: '',
    flow: 'device_code',
    id,
    name: id === 'clover' ? 'Clover Portal' : 'MiniMax',
    status: {
      logged_in: loggedIn
    },
    ...patch
...
beforeEach(() => {
  ...
  disconnectOAuthProvider.mockResolvedValue({ ok: true, provider: 'clover' })
  listOAuthProviders.mockResolvedValue({
    providers: [provider('clover', true), provider('minimax-oauth', false)]
  })
})
...
describe('ProvidersSettings', () => {
  it('disconnects a connected provider account and refreshes the accounts list', async () => {
    ...
    const remove = await screen.findByRole('button', { name: 'Remove Clover Portal' })
    await act(async () => {
    ...
    await waitFor(() => expect(disconnectOAuthProvider).toHaveBeenCalledWith('clover'))
  ...
  it('leaves the account connected when the removal prompt is dismissed', async () => {
    ...
    await act(async () => {
      fireEvent.click(await screen.findByRole('button', { name: 'Remove Clover Portal' }))
    })
  ...
  it('keeps provider selection separate from account removal', async () => {
    ...
    await act(async () => {
      fireEvent.click(await screen.findByText('Clover Portal'))
    })

    expect(startManualProviderOAuth).toHaveBeenCalledWith('clover')
    expect(disconnectOAuthProvider).not.toHaveBeenCalled()
  })
    ...
    listOAuthProviders.mockResolvedValue({
      providers: [
        provider('qwen-oauth', true, {
          ...
          disconnect_hint: "Use `clover auth add qwen-oauth` or that provider's CLI to remove it.",
