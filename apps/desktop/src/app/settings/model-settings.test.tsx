...
beforeEach(() => {
  getGlobalModelInfo.mockResolvedValue({ provider: 'clover', model: 'clover-4' })
  getGlobalModelOptions.mockResolvedValue({
    providers: [
      {
        name: 'Clover',
        slug: 'clover',
        models: ['clover-4', 'clover-4-mini'],
        authenticated: true,
        capabilities: { 'clover-4': { reasoning: true, fast: true } }
      }
    ]
  })
  getAuxiliaryModels.mockResolvedValue({
    main: { provider: 'clover', model: 'clover-4' },
  ...
  saveCloverConfig.mockResolvedValue({ ok: true })
})
...
describe('ModelSettings', () => {
  it('loads the current main model and lists configured providers only', async () => {
    ...
    // "Clover" shows in both the trigger and the open list.
    expect((await screen.findAllByText('Clover')).length).toBeGreaterThan(0)
  ...
  it('replaces the selected provider and model when the active profile changes', async () => {
    getGlobalModelInfo
      ...
      .mockResolvedValueOnce({ provider: 'clover', model: 'clover-4' })
    ...
    await waitFor(() => expect(screen.getAllByRole('combobox')[0].textContent).toContain('Clover'))
  ...
  it('writes the profile default speed (service_tier) when the fast switch is toggled', async () => {
    await renderModelSettings()
    await waitFor(() => expect(getCloverConfigRecord).toHaveBeenCalled())

    const fastSwitch = await screen.findByRole('switch')
    fireEvent.click(fastSwitch)

    await waitFor(() =>
      expect(saveCloverConfig).toHaveBeenCalledWith(
      ...
      providers: [
        {
          ...
          models: ['clover-4'],
          authenticated: true,
          capabilities: { 'clover-4': { reasoning: false, fast: false } }
        }
      ]
    })
  ...
  it('warns when a main switch leaves auxiliary tasks pinned to another provider', async () => {
    ...
    expect(screen.getByText('clover')).toBeTruthy()
  })
...
describe('ModelSettings MoA preset editor', () => {
  ...
  it('does not clear the model or save when the same provider is re-selected', async () => {
    ...
    try {
      ...
      fireEvent.click(slotSelects().ref1Provider)
      fireEvent.click(await screen.findByRole('option', { name: 'Clover' }))
      await vi.advanceTimersByTimeAsync(700)
        ...
        expect.objectContaining({
          presets: expect.objectContaining({
            default: expect.objectContaining({
              reference_models: [
                expect.objectContaining({ provider: 'clover', model: 'clover-4', enabled: false }),
