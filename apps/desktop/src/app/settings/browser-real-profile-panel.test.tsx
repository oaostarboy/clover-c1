...
vi.mock('@/clover', () => ({
  saveCloverConfigRecord: (config: Record<string, unknown>, profile?: unknown) => mocks.save(config, profile)
}))
...
vi.mock('../hooks/use-config-record', () => ({
  cloverConfigCacheWriter: () => (config: Record<string, unknown>) => mocks.cache(config),
  useCloverConfigRecord: () => ({ data: mocks.loadedConfig })
}))

describe('BrowserRealProfilePanel', () => {
  beforeEach(() => {
    mocks.loadedConfig = { browser: { allow_private_urls: false }, model: { provider: 'clover' } }
