...
export function ConnectionsRegistrySection() {
  ...
  useEffect(() => {
    if (!editorWantsOauth || !editorUrl || !window.cloverDesktop?.probeConnectionConfig) {
      setAuthProbe(null)
  ...
  const signInOauth = useCallback(async () => {
    ...
    try {
      const result = await window.cloverDesktop.oauthLoginConnectionConfig(editorUrl)
  ...
  // Fan out `clover update` to every eligible source; per-connection results
