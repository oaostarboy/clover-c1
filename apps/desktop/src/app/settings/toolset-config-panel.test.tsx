...
import type { ToolsetConfig } from '@/types/clover'
...
beforeEach(() => {
  ...
  getCloverConfigRecord.mockResolvedValue({
    tts: {
      provider: 'edge',
  ...
  getCloverConfigSchema.mockResolvedValue({ fields: {}, category_order: [] })
...
describe('ToolsetConfigPanel', () => {
  it('renders inline voice/model fields for a TTS provider row carrying tts_provider', async () => {
    ...
    await waitFor(() => expect(saveCloverConfig).toHaveBeenCalled(), { timeout: 3000 })
    const saved = saveCloverConfig.mock.calls.at(-1)?.[0] as Record<string, Record<string, Record<string, string>>>
  ...
  describe('managed Clover provider activation', () => {
    ...
    it('drives the existing Clover OAuth device-code flow from the sign-in action and refetches', async () => {
      ...
      startOAuthLogin.mockResolvedValue({
        ...
        session_id: 'sess-1',
        user_code: 'NOUS-1234',
        verification_url: 'https://portal.clover-c1.local/device?user_code=NOUS-1234',
        poll_interval: 5,
        expires_in: 600
      })
