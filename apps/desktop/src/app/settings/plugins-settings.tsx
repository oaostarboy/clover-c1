...
import { getProfiles } from '@/clover'
...
// User-installed plugins first — mirrors `clover plugins list --user`.
...
function reveal(file: string) {
  void window.cloverDesktop?.revealPath?.(file)?.catch(() => undefined)
}

async function revealPluginsDir() {
  try {
    // Electron owns the local plugin root — deriving it from the backend's
    // clover_home breaks against a remote backend (#66899).
    const dir = await window.cloverDesktop?.desktopPluginsRoot?.()
    ...
    const result = await window.cloverDesktop?.openDir?.(dir)
...
// Agent plugins live under the BACKEND's clover home (profile-aware), so the
// path comes from the gateway — not from the renderer's local CLOVER_HOME.
...
async function revealAgentPluginsDir(request: GatewayRequest) {
  try {
    ...
    const opened = await window.cloverDesktop?.openDir?.(`${home}/plugins`)
            ...
            <SelectContent>
              {profiles.map(profile => (
                <SelectItem key={profile.name} value={profile.name}>
                  {profile.is_default ? 'Clover (default)' : profile.name}
                </SelectItem>
              ))}
            </SelectContent>
          </Select>
