...
import { type ProfileScope, saveCloverConfigRecord } from '@/clover'
 ...
 * the default browser's profile (cookies/logins) into a Clover-owned copy;
...
export function BrowserRealProfilePanel({ profile }: BrowserRealProfilePanelProps) {
  ...
  const { data: config } = useCloverConfigRecord(profile)
  ...
  const toggle = useCallback(
    async (on: boolean) => {
      ...
      try {
        await saveCloverConfigRecord(next, profile)
        notify({
          kind: 'info',
