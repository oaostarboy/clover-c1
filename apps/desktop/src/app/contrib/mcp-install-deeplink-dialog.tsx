...
import { getCloverConfigRecord, saveMcpServers } from '@/clover'
...
import { setCloverConfigCache } from '../hooks/use-config-record'

/**
 * Explicit-confirm gate for `clover://mcp/install` deep links. The payload is
 ...
 * confirming lets Clover spawn that local process. An existing server name is
...
export function McpInstallDeepLinkDialog() {
  ...
  const confirm = async () => {
    ...
    try {
      ...
      const current = getServers(await getCloverConfigRecord())
      ...
      setCloverConfigCache(previous => (previous ? { ...previous, mcp_servers: nextServers } : previous))
