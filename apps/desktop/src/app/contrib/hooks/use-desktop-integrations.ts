...
import { pathFromCloverDeepLink, resolveCloverOpenPath } from '@/lib/clover-open-target'
...
export function useDesktopIntegrations({
  ...
  useEffect(() => {
    ...
    const unsubscribe = window.cloverDesktop?.onOpenUpdatesRequested?.(() => openUpdatesWindow())
  ...
  useEffect(() => {
    window.cloverDesktop?.setPreviewShortcutActive?.(true)
  }, [])
  ...
  useEffect(() => {
    const unsubscribe = window.cloverDesktop?.onFocusSession?.(sessionId => {
      if (sessionId) {
  ...
  useEffect(() => {
    const unsubscribe = window.cloverDesktop?.onNotificationAction?.(({ actionId, sessionId }) => {
  ...
  // clover://index-network/intent/1 vocabulary with deep links.
  useEffect(() => {
    const unsubscribe = window.cloverDesktop?.onNotificationActivate?.(payload => {
      if (!payload) {
        return
      }
      ...
      if (payload.activate) {
        ...
        // the pre-IPC validation — any future cloverDesktop.notify caller gets
        // funneled through the same resolver.
        const path = resolveCloverOpenPath(payload.activate)
  ...
  useEffect(() => {
    const unsubscribe = window.cloverDesktop?.onDeepLink?.(payload => {
      if (!payload?.kind) {
        return
      }
      ...
      // link (clover://index-network/intent/1, clover://open/…). The resolver
      // rejects reserved kinds and unsafe paths.
      const path = pathFromCloverDeepLink(payload.kind, payload.name || '', payload.params || {})
    ...
    void window.cloverDesktop?.signalDeepLinkReady?.()
  ...
  useEffect(() => {
    const unsubscribe = window.cloverDesktop?.onClosePreviewRequested?.(
  ...
  useEffect(() => {
    const unsubscribe = window.cloverDesktop?.onPreviewNav?.(command => {
  ...
  useEffect(() => {
    const unsubscribe = window.cloverDesktop?.onOpenFolderRequested?.(() => void openFolderAsProject())
