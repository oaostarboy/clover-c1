...
function setWorkspaceTransfer(transferring: boolean): void {
  window.cloverDesktop?.hud?.setWorkspaceTransfer?.(transferring)
}

function moveHud(state: PressState): void {
  window.cloverDesktop?.hud?.moveBy?.({
    width: state.originW,
...
function armGrab(state: PressState, workspaceTransfer: boolean): void {
  ...
  window.cloverDesktop?.hud?.beginMove?.()
}
 ...
 * (see clover:hud:move-by — a transparent frameless window drifts wider on
...
export function useHudComposerDrag(
  ...
  const reset = useCallback(() => {
    ...
    if (state) {
      if (state.armed) {
        window.cloverDesktop?.hud?.endMove?.()
      }
