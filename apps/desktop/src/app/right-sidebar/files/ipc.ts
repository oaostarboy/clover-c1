...
import type { CloverReadDirEntry, CloverReadDirResult } from '@/global'
...
export type ProjectTreeEntry = CloverReadDirEntry
...
function ignoredBy(rules: GitignoreRule[], entry: CloverReadDirEntry) {
...
async function filterIgnored(entries: CloverReadDirEntry[], rootPath: string, dirPath: string) {
...
export async function readProjectDir(dirPath: string, rootPath = dirPath): Promise<CloverReadDirResult> {
  if (!window.cloverDesktop) {
