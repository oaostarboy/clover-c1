...
beforeEach(() => {
  ...
  readDir.mockReset()
  ;(window as unknown as { cloverDesktop: { readDir: typeof readDir } }).cloverDesktop = { readDir }
})
...
function ok(entries: { name: string; path: string; isDirectory: boolean }[]): CloverReadDirResult {
  return { entries }
}
...
describe('useProjectTree', () => {
  ...
  it('reads gitignore from the real path while caching per connection', async () => {
    ...
    ;(window as unknown as { cloverDesktop: unknown }).cloverDesktop = { gitRoot, readDir, readFileDataUrl }
  ...
  it('dedupes concurrent loadChildren calls for the same id', async () => {
    ...
    let resolveChildren: ((value: CloverReadDirResult) => void) | undefined
    readDir.mockImplementationOnce(
      () =>
        new Promise<CloverReadDirResult>(resolve => {
  ...
  it('discards a stale live refresh after the active registered connection changes', async () => {
    let resolveRefreshFromA: ((result: CloverReadDirResult) => void) | undefined
  ...
  it('discards a stale child read after the active registered connection changes', async () => {
    let resolveChildFromA: ((result: CloverReadDirResult) => void) | undefined
  ...
  it('discards a stale root read after the active registered connection changes', async () => {
    let resolveFirst: ((result: CloverReadDirResult) => void) | undefined
    readDir.mockImplementationOnce(
      () =>
        new Promise<CloverReadDirResult>(resolve => {
  ...
  it('returns no-bridge gracefully when window.cloverDesktop is missing', async () => {
    delete (window as unknown as { cloverDesktop?: unknown }).cloverDesktop
  ...
  it('keeps an unreadable root on screen while it re-probes', async () => {
    ...
    let releaseProbe: ((value: CloverReadDirResult) => void) | undefined
  ...
  it('keeps loaded rows on screen while the root refreshes', async () => {
    ...
    let releaseRefresh: ((value: CloverReadDirResult) => void) | undefined
  ...
  it('clears the rows when the same path is re-read from another backend', async () => {
    ...
    let releaseFromB: ((value: CloverReadDirResult) => void) | undefined
