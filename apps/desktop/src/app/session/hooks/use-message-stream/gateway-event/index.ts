...
export function useGatewayEventHandler(deps: GatewayEventDeps) {
  const { activeSessionIdRef, compactedTurnRef, refreshCloverConfig, sessionStateByRuntimeIdRef } = deps
  ...
  // refreshCloverConfig — two REST calls (config + defaults) per event, per
