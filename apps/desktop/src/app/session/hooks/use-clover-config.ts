...
import { getCloverConfig, getCloverConfigDefaults } from '@/clover'
...
interface CloverConfigOptions {
  activeSessionIdRef: MutableRefObject<string | null>
}

export function useCloverConfig({ activeSessionIdRef }: CloverConfigOptions) {
  const [voiceMaxRecordingSeconds, setVoiceMaxRecordingSeconds] = useState(DEFAULT_VOICE_SECONDS)
  const [sttEnabled, setSttEnabled] = useState(true)
  const profileRefreshEpochRef = useRef(0)

  const refreshCloverConfig = useCallback(
    async (force = false) => {
      if (force) {
      ...
      try {
        const [config, defaults] = await Promise.all([getCloverConfig(), getCloverConfigDefaults().catch(() => ({}))])
        ...
        // rendering/applying Clover' built-in medium over the user's config.
  ...
  return { refreshCloverConfig, sttEnabled, voiceMaxRecordingSeconds }
}
