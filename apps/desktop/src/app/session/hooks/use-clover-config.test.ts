...
import { getCloverConfig } from '@/clover'
...
import { useCloverConfig } from './use-clover-config'

vi.mock('@/clover', () => ({
  getCloverConfig: vi.fn(),
  getCloverConfigDefaults: vi.fn().mockResolvedValue({})
}))

const WORKSPACE_CWD_KEY = 'clover.desktop.workspace-cwd'

const mockConfig = (config: Record<string, unknown>) =>
  vi.mocked(getCloverConfig).mockResolvedValue(config as Awaited<ReturnType<typeof getCloverConfig>>)

describe('useCloverConfig refreshCloverConfig', () => {
  beforeEach(() => {
  ...
  // `agent.reasoning_effort: high` to Clover' built-in medium.
  it('publishes the profile default effort even when a manual pick blocks the composer reseed', async () => {
    ...
    const { result } = renderHook(() => useCloverConfig({ activeSessionIdRef: { current: null } }))

    await act(async () => {
      await result.current.refreshCloverConfig()
    })
  ...
  it('does not let terminal.cwd replace an active session workspace', async () => {
    ...
    mockConfig({ terminal: { cwd: '/Users/example/new-workspace' } })
    const { result } = renderHook(() => useCloverConfig({ activeSessionIdRef: { current: 'session-1' } }))
  ...
  it('does not let a stale forced config refresh overwrite newer draft selector intent', async () => {
    const profileConfig = deferred<Awaited<ReturnType<typeof getCloverConfig>>>()
    vi.mocked(getCloverConfig).mockReturnValueOnce(profileConfig.promise)

    const { result } = renderHook(() => useCloverConfig({ activeSessionIdRef: { current: null } }))

    let pendingRefresh!: Promise<void>
    act(() => {
      pendingRefresh = result.current.refreshCloverConfig(true)
    })
    expect(getCloverConfig).toHaveBeenCalled()
    ...
    } as Awaited<ReturnType<typeof getCloverConfig>>)
  ...
  it('does not publish config after its switch loses ownership', async () => {
    const staleConfig = deferred<Awaited<ReturnType<typeof getCloverConfig>>>()
    vi.mocked(getCloverConfig).mockReturnValueOnce(staleConfig.promise)
    const { result } = renderHook(() => useCloverConfig({ activeSessionIdRef: { current: null } }))
    let ownsSwitch = true

    let refresh!: Promise<void>
    act(() => {
      refresh = result.current.refreshCloverConfig(false, () => ownsSwitch)
    })
  ...
  it('does not let an older profile config overwrite a newer profile', async () => {
    const profileB = deferred<Awaited<ReturnType<typeof getCloverConfig>>>()
    const profileC = deferred<Awaited<ReturnType<typeof getCloverConfig>>>()
    vi.mocked(getCloverConfig).mockReturnValueOnce(profileB.promise).mockReturnValueOnce(profileC.promise)
    ...
    act(() => {
      refreshB = result.current.refreshCloverConfig(true)
      refreshC = result.current.refreshCloverConfig(true)
    })
