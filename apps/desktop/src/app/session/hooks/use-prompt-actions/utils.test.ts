...
describe('renderRpcResult', () => {
  ...
  describe('session.save', () => {
    it('echoes the saved file path', () => {
      expect(renderRpcResult({ file: '/home/user/.clover/sessions/saved/x.json' }, 'save')).toBe(
        'Saved transcript to /home/user/.clover/sessions/saved/x.json'
      )
    })
  })
  ...
  describe('session.usage', () => {
    ...
    it('appends credits_lines when present', () => {
      const body = renderRpcResult(
        {
          ...
          output: 20,
          total: 30,
          credits_lines: ['Clover credits: 8,420 remaining', 'Resets: 2026-08-01']
        },
        'usage'
      )
      ...
      expect(body.split('\n')).toEqual([
        ...
        'Clover credits: 8,420 remaining',
        'Resets: 2026-08-01'
      ])
    })
  })
