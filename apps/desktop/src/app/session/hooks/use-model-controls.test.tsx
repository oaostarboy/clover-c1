...
import { getGlobalModelInfo } from '@/clover'
...
describe('useModelControls', () => {
  ...
  it('keeps a live session authoritative when Settings saves a new profile default', async () => {
    ...
    result.current.applySavedMainModel('clover', 'poolside/laguna-xs-2.1:free')
    await result.current.refreshCurrentModel()
    ...
    expect($currentProvider.get()).toBe('clover')
  ...
  it('preserves a populated model catalog when painting a saved profile default', () => {
    const queryClient = new QueryClient()
    const providers = [{ models: ['tencent/hy3:free'], name: 'Clover', slug: 'clover' }]
