...
vi.mock('@/clover', async importOriginal => {
  const actual = await importOriginal<typeof CloverModule>()

  class FakeCloverGateway {
  ...
  return { ...actual, CloverGateway: FakeCloverGateway }
})

import type * as CloverModule from '@/clover'
import type { CloverGateway } from '@/clover'
import {
  $gateway,
...
const fakeGateway = { connectionState: 'open' } as unknown as CloverGateway
...
async function activateRemoteGateway() {
  ...
  setPrimaryGateway(primary as unknown as CloverGateway, 'default')
  $gateway.set(primary as unknown as CloverGateway)
...
describe('useGatewayRequest', () => {
  ...
  it('surfaces a missing optional scoped mint bridge without falling back to a stale ticket or local lookup', async () => {
    const { desktop, gateway } = await activateRemoteGateway()
    Reflect.deleteProperty(window.cloverDesktop, 'getGatewayWsUrlFor')
  ...
  it('rejects instead of hanging forever when the reconnect getConnection() wedges (#93454)', async () => {
    ...
    } as unknown as CloverGateway

    const getConnection = vi.fn(() => new Promise(() => undefined))

    ;(window as unknown as { cloverDesktop: unknown }).cloverDesktop = { getConnection }
    $gateway.set(dropped)
