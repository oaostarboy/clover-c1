...
export function useGatewayRequest() {
  ...
  const gateway = useStore($gateway) as CloverGateway | null
  const gatewayRef = useRef<CloverGateway | null>(null)

  const connectionRef = useRef<Awaited<ReturnType<NonNullable<typeof window.cloverDesktop>['getConnection']>> | null>(
    null
  )

  const gatewayStateRef = useRef(gatewayState)
  const reconnectingRef = useRef<Promise<CloverGateway | null> | null>(null)
  ...
  useEffect(
    () =>
      $gateway.subscribe(gateway => {
        gatewayRef.current = gateway as CloverGateway | null
      }),
    []
  )
