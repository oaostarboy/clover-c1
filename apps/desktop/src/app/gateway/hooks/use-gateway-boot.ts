...
import type { RpcEvent } from '@/types/clover'
...
export function primaryRuntimeConnectionId(connection: Pick<CloverConnection, 'connectionId' | 'mode'>): null | string {
...
interface GatewayBootOptions {
  ...
  onConnectionReady: (
    connection: Awaited<ReturnType<NonNullable<typeof window.cloverDesktop>['getConnection']>> | null
  ) => void
  onGatewayReady: (gateway: CloverGateway | null) => void
...
export function useGatewayBoot({
  ...
  useEffect(() => {
    ...
    const publish = (next: CloverConnection | null) => {
    ...
    // composer stayed disabled forever on "Starting Clover...". Once the
    ...
    const attemptReconnect = async () => {
      ...
      try {
        ...
        // "Starting Clover…". The probe is a no-op for a healthy or local backend.
        ...
        const conn = await withTimeout(
          ...
          RECONNECT_ATTEMPT_TIMEOUT_MS,
          'Timed out reconnecting to Clover backend'
        )
        ...
        // as an opaque "Could not connect to Clover gateway". resolveGatewayWsUrl
        ...
        await callbacksRef.current.refreshCloverConfig().catch(() => undefined)
    ...
    const softSwitch = async () => {
      ...
      try {
        ...
        await Promise.all([
          ...
          callbacksRef.current.refreshCloverConfig(false, ownsSwitch).catch(() => undefined),
    ...
    const offBootProgress = desktop.onBootProgress(payload => {
      // Soft switch / post-boot startClover re-emits progress — ignore so the
    ...
    const gateway = adoptedFromHmr ? survivor!.gateway : new CloverGateway()
    ...
    async function boot() {
      try {
        ...
        // hanging "Starting Clover…" forever.
        const wsUrl = await withTimeout(
          resolveGatewayWsUrl(desktop, conn),
          RECONNECT_ATTEMPT_TIMEOUT_MS,
        ...
        await Promise.all([
          ...
          callbacksRef.current.refreshCloverConfig(),
          ...
          // retry it), not brick boot behind the "Clover couldn't start"
