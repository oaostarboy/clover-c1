...
import { CLOVER_CONTEXT_MENU_TRIGGER_ATTR, ContextMenu, ContextMenuTrigger } from '@/components/ui/context-menu'
...
function installBridge(partial: Partial<Window['cloverDesktop']> = {}) {
...
afterEach(() => {
  ...
  delete desktopWindow.cloverDesktop
})
...
describe('AppContextMenu', () => {
  ...
  it('offers the resolved copy only for loopback links on a remote gateway', async () => {
    ...
    installBridge({
      reachPreviewUrl: reachPreviewUrl as unknown as Window['cloverDesktop']['reachPreviewUrl'],
      writeClipboard: writeClipboard as unknown as Window['cloverDesktop']['writeClipboard']
    })
    mountMenu()
  ...
  it('runs edit verbs after the menu closed, with focus back on the editable', async () => {
    ...
    installBridge({ contextMenuEdit: contextMenuEdit as unknown as Window['cloverDesktop']['contextMenuEdit'] })
    mountMenu()
  ...
  it('keeps a modal textarea paste menu inside its dialog and restores focus', async () => {
    ...
    installBridge({
      contextMenuEdit: contextMenuEdit as unknown as Window['cloverDesktop']['contextMenuEdit'],
  ...
  it('keeps paste clickable even when the clipboard probe reports empty', async () => {
    ...
    installBridge({ readClipboard: readClipboard as unknown as Window['cloverDesktop']['readClipboard'] })
    mountMenu()
    const host = attach('<textarea>text</textarea>')
...
describe('ContextMenuTrigger asChild', () => {
  it('keeps the coordinator marker when the child overwrites data-slot', () => {
    ...
    expect(footer.hasAttribute(CLOVER_CONTEXT_MENU_TRIGGER_ATTR)).toBe(true)
  })
})
