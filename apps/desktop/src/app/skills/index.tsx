...
import type { SkillInfo, ToolsetInfo } from '@/types/clover'
...
export function SkillsView({
  ...
  const registryBridge = window.cloverDesktop?.connections
  const rosterBridge = window.cloverDesktop?.getAgentRoster
  ...
  const scopeOptions: { key: string; label: string; value: string }[] = useMemo(() => {
    ...
    return (profilesData?.profiles ?? []).map(p => ({
      key: p.name,
      label: p.is_default ? 'Clover (default)' : p.name,
      value: p.name
    }))
