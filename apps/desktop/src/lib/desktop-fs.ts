...
import type {
  CloverConnection,
  CloverReadDirResult,
  CloverReadFileTextResult,
  CloverSelectPathsOptions
} from '@/global'
...
export function desktopFsCacheKey(connection: CloverConnection | null = $connection.get()) {
...
export async function readDesktopDir(path: string): Promise<CloverReadDirResult> {
  if (!isDesktopFsRemoteMode()) {
    return bridge().readDir(path)
  }

  return remoteFsApi<CloverReadDirResult>(fsPath('list', path))
}

export async function readDesktopFileText(path: string): Promise<CloverReadFileTextResult> {
  if (!isDesktopFsRemoteMode()) {
    return bridge().readFileText(path)
  }

  return remoteFsApi<CloverReadFileTextResult>(fsPath('read-text', path))
}
...
export async function readDesktopFileDataUrlLocalFirst(path: string): Promise<string> {
  try {
    const local = await window.cloverDesktop?.readFileDataUrl?.(path)
...
export async function selectDesktopPaths(options?: CloverSelectPathsOptions): Promise<string[]> {
  const desktop = bridge()
