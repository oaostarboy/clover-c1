...
// Clover is rendering, and when the two are opposite schemes it takes over
...
function renderedMode(doc: Document): RenderedMode {
  const attr = doc.documentElement.dataset.cloverMode
 ...
 * the only carriers of Clover' palette — are dropped so the paste target's
