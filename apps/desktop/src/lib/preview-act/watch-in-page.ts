...
export function watchInPage(doc: Document, holder: WatchHolder, stage: WatchStage, label?: string): void {
  const win = doc.defaultView as (Window & { __cloverWatch?: Record<string, unknown> }) | null
  ...
  const busy = 25
  const state = win.__cloverWatch || (win.__cloverWatch = {})
  ...
  const build = () => {
    ...
    const stale = doc.querySelectorAll('clover-watch')

    for (let i = 0; i < stale.length; i++) {
      stale[i].remove()
    }

    const host = doc.createElement('clover-watch')
  ...
  // this up" in dev.
  const stamp = (win as unknown as { __cloverWatchTag?: number }).__cloverWatchTag
