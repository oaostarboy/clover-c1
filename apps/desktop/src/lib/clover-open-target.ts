 ...
 * `clover://` deep links and `host.navigate('/path?…')`.
 *
 ...
 *  - `clover://index-network/intent/1` → `/index-network/intent/1` (plugin-scoped)
 *  - `clover://open/my-page?item=x` → `/my-page?item=x` (generic open)
 *  - `/my-page?item=x` / `#/my-page?item=x` (hash-router paths)
 */

export type CloverOpenTarget = string | { href: string } | { path: string; params?: Record<string, string> }

const CLOVER_PROTOCOL = 'clover:'
...
export function normalizeCloverOpenString(raw: string): string | null {
  ...
  if (trimmed.startsWith('clover://') || trimmed.startsWith(`${CLOVER_PROTOCOL}//`)) {
    try {
      const url = new URL(trimmed)
      const host = url.hostname || ''
      const rest = decodeURIComponent((url.pathname || '').replace(/^\//, ''))

      // clover://open/<path>?… → /<path>?…
      if (host === 'open') {
        if (!rest) {
          return null
        }
      ...
      // clover://index-network/intent/1 → /index-network/intent/1
...
export function resolveCloverOpenPath(target: CloverOpenTarget | null | undefined): string | null {
  if (target == null) {
    return null
  }

  if (typeof target === 'string') {
    return normalizeCloverOpenString(target)
  }
  ...
  if ('href' in target && typeof target.href === 'string') {
    return normalizeCloverOpenString(target.href)
  }

  if ('path' in target && typeof target.path === 'string') {
    const base = normalizeCloverOpenString(target.path)
 ...
 * (`clover://<kind>/<name>?…` → kind/name/params).
 */
export function pathFromCloverDeepLink(kind: string, name: string, params: Record<string, string> = {}): string | null {
  if (!kind || !name) {
    return null
  }

  if (kind === 'open') {
    return resolveCloverOpenPath({ path: `/${name.replace(/^\//, '')}`, params })
  }
  ...
  return resolveCloverOpenPath({ path: `/${kind}/${name.replace(/^\//, '')}`, params })
}

/** Convenience for `clover://open/<name>?…` payloads. */
export function pathFromOpenDeepLink(name: string, params: Record<string, string> = {}): string | null {
  return pathFromCloverDeepLink('open', name, params)
}
