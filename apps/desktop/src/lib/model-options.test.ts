...
const globalOptions = { model: 'clover-4', provider: 'clover', providers: [] }
...
describe('requestModelOptions', () => {
  ...
  it('recovers an empty gateway catalog through profile-scoped REST without replacing the session selection', async () => {
    const gatewayPayload = { model: 'clover-local', provider: 'clover-local' }

    const restPayload = {
      model: 'profile-default',
      provider: 'openai-codex',
      providers: [{ models: ['clover-local'], name: 'Clover Local vLLM', slug: 'clover-local' }]
    }
  ...
  it('keeps the gateway result when both catalog paths have no selectable models', async () => {
    const gatewayPayload = { model: 'clover-local', provider: 'clover-local', providers: [] }
  ...
  it('prefers an owner-routed request over the ambient gateway socket', async () => {
    const gatewayPayload = {
      model: 'chrome-model',
      provider: 'clover',
      providers: [{ models: ['chrome-model'], name: 'Clover', slug: 'clover' }]
    }
  ...
  it('scopes REST recovery to the catalog owner profile', async () => {
    const restPayload = {
      model: 'berry-local',
      provider: 'clover-local',
      providers: [{ models: ['berry-local'], name: 'Clover Local', slug: 'clover-local' }]
    }
...
describe('manualPickRemoved', () => {
  const providers = [
    ...
    { name: 'Clover', slug: 'clover', models: [] } // present but unconfigured / re-auth
  ]
  ...
  it('never clobbers when the provider has an empty model list (re-auth)', () => {
    expect(manualPickRemoved(providers, 'clover', 'clover-4')).toBe(false)
  })
