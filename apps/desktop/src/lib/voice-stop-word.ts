...
//
// When someone is in a hands-free "Hey Clover" voice chat, the natural way to
...
// phrase (optionally addressed to Clover), so a real turn that merely contains
...
// Optional address prefixes so "clover stop" / "ok stop" / "hey clover, stop"
// still count. Stripped before matching the core phrase.
const ADDRESS_PREFIXES: readonly string[] = ['hey clover', 'hey clover,', 'clover', 'clover,', 'ok', 'okay', 'hey']
...
function stripAddress(text: string): string {
  for (const prefix of ADDRESS_PREFIXES) {
    if (text === prefix) {
      // Bare address ("clover") is not a stop command on its own.
      continue
    }
 ...
 * to Clover). Returns false for anything that merely contains "stop" as part of
