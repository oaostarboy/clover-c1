...
/** Why a known Clover command has no desktop UI surface. */
...
const DESKTOP_COMMAND_SPECS: readonly DesktopCommandSpec[] = [
  ...
  { name: '/profile', description: 'Switch the active Clover profile', surface: action('profile') },
  {
    name: '/skin',
  ...
  { name: '/version', description: 'Show Clover Cognition version', surface: exec() },
...
function isKnownCloverSlashCommand(command: string): boolean {
  const normalized = normalizeCommand(command)
 ...
 * Clover' built-in slash commands — i.e. skill commands (`/gif-search`,
...
export function isDesktopSlashExtensionCommand(command: string): boolean {
  ...
  return !isKnownCloverSlashCommand(normalized)
}
 ...
 * activity — the ones that ship with Clover and were never asked for. It is
