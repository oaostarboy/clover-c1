...
import { atom, host, queryClient, useQuery, useValue } from '@clover/plugin-sdk'
...
export function newBotChat(bot: RosterRow) {
  if (typeof host.newChat !== 'function') {
    host.notify?.({
      kind: 'error',
      message:
        getPluginCtx()?.i18n?.t('bot.openAnotherChatUnsupported') ?? 'Update Clover Desktop to open another Bot chat.'
    })
