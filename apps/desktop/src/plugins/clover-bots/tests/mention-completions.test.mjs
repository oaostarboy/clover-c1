...
function loadProvide({ roster, active = 'default', focused = active, meta = {} } = {}) {
  ...
  const context = {
    ...
    queryClient: {
      ...
      // key-ignoring mock.
      getQueryData: key =>
        key.length === 3 && key[0] === 'clover-bots' && key[1] === 'roster'
          ? roster
          : undefined,
      getQueriesData: () => (Array.isArray(roster?.profiles) ? [[['clover-bots', 'roster', 'local'], roster]] : []),
...
test('offers roster handles for a matching prefix, excluding the active profile', () => {
  ...
  assert.ok(inserts.includes('@clover'), 'default surfaces as @clover')
