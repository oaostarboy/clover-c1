...
function sdkNamedImports(source) {
  const match = source.match(/import\s+\{([\s\S]*?)\}\s+from '@clover\/plugin-sdk'/)
...
function writeLegacySdk(root) {
  const packageRoot = join(root, 'node_modules', '@clover', 'plugin-sdk')
  ...
  writeFileSync(
    ...
    `${JSON.stringify({ name: '@clover/plugin-sdk', type: 'module', exports: './index.js' }, null, 2)}\n`
  )
...
test('legacy SDK without optional capability exports still links Bot Mode', async t => {
  const root = mkdtempSync(join(tmpdir(), 'clover-bot-mode-legacy-sdk-'))
  ...
  assert.equal(loaded.default.id, 'clover-bots')
