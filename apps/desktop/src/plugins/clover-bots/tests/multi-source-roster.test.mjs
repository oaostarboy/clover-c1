...
function runtime() {
  ...
  const code = source
    .replace(/^import \* as sdk from '@clover\/plugin-sdk'\r?\n/m, '')
...
test('default rows use source identity without borrowing another source title', () => {
  ...
  // label. Titled: the title wins. Untitled: it stays "Clover". Regression:
  ...
  assert.equal(name(active, undefined), 'Clover')
})

test('botHandle: precomputed multi-source handle wins; default stays clover', () => {
  ...
  assert.equal(botHandle('default'), 'clover')
})
...
test('displayName: local default stays Clover; remote default uses the device label', () => {
...
test('resolveRosterMentions: @clover in this chat is not a handoff to yourself', () => {
  const { __resolveRosterMentions: resolve } = runtime()
  const roster = [
    { name: 'default', connectionId: 'local', handle: 'clover' },
    {
      name: 'default',
      connectionId: 'mac-mini',
      connectionLabel: 'Mac Mini',
  ...
  const hits = resolve('ask @clover and @default-mac-mini', roster, {
    name: 'default',
    connectionId: 'local'
  })
