...
import { host } from '@clover/plugin-sdk'
...
async function openStoredBotChat(
  ...
  // until an app restart (clover-c1#93604). A resume is cheap and
  ...
  if (route && typeof host.requestProfile !== 'function') {
    throw new Error(
      getPluginCtx()?.i18n?.t('bot.remoteConnectionsUnsupported') ??
        'Update Clover Desktop to chat with bots on other connections.'
    )
  }
