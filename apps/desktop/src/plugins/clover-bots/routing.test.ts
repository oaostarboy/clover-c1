 ...
 * the raw Cloud hostname, or as generic "Clover" when Cloud was the only
 * source.
 *
...
vi.mock('@clover/plugin-sdk', () => ({ host: hostMock }))
...
describe('alias identity survives the hosted handoff (#89131)', () => {
  ...
  it('renders the sole Cloud-only default as the alias, not "Clover"', () => {
  ...
  it('never leaks the alias onto a same-named default on another connection', () => {
    ...
    expect(displayName({ name: 'default' }, null)).toBe('Clover')
  })
