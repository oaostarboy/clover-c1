 ...
 * job for a different profile keeps the `clover -p <bot> chat` delegation
...
/** Run the delegation command under a `clover` stub that prints its argv, so
...
function argvOf(prompt: string): string[] {
  ...
  const result = spawnSync('sh', ['-c', `clover() { printf '%s\\037' "$@"; }\n${command}`], { encoding: 'utf8' })
...
describe('direct vs delegated execution', () => {
  ...
  it('keeps the delegation wrapper for a different active profile', () => {
    ...
    expect(prompt).toMatch(/clover -p 'research' chat/)
