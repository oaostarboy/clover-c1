 ...
 * `{type:'clover-skill-pick'}` back and the plugin installs via skills.manage.
 *
...
import type * as CloverSdk from '@clover/plugin-sdk'
...
vi.mock('@clover/plugin-sdk', async importOriginal => {
  const original = await importOriginal<typeof CloverSdk>()
...
describe('hub pick messages', () => {
  it('installs the picked skill when it comes from our own frame', () => {
    ...
    postPick(
      { identifier: 'clover/web-research', name: 'Web Research', type: 'clover-skill-pick' },
      {
        source: frame.contentWindow
      }
    )

    expect(installCalls()).toEqual([['skills.manage', { action: 'install', query: 'clover/web-research' }]])
  })
  ...
  it('regression: refuses identifiers outside the slug charset', () => {
    ...
    for (const identifier of ['../../etc/passwd', 'skill; rm -rf /', '-flag', 'name with spaces', '']) {
      postPick({ identifier, name: 'Web Research', type: 'clover-skill-pick' }, { source: frame.contentWindow })
    }
  ...
  it('ignores messages that are not a skill pick', () => {
    ...
    postPick({ identifier: 'clover/web-research', type: 'oauth-callback' }, { source: frame.contentWindow })
    postPick({ identifier: 'clover/web-research', type: 'clover-skill-pick' }, { source: frame.contentWindow })
