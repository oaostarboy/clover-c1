...
vi.mock('@clover/plugin-sdk', async () => {
...
function published(room: Room) {
  ...
  return (configure?.params.ui_meta as Record<string, Record<string, unknown>>)?.['clover-bots-groups']
}
...
describe('speaker labels', () => {
  it('the default profile speaks as Clover in transcripts, not @default', async () => {
    ...
    expect(line).toBe('Clover: hello room')
  ...
  it('honor friendly identity: Bot Mode title, then display_name, never a stale Clover', async () => {
    ...
    expect(chat.groupSpeakerLabel('default')).toBe('Clover')
...
describe('sync worker', () => {
  it('retries a gateway CAS conflict and publishes the merged room', async () => {
    ...
    await drain(() => (room.gateway.uiMetaRevisions['clover-bots-groups'] || 0) < 2, 60)

    const stored = room.gateway.uiMeta['clover-bots-groups'] as {
      rooms: Record<string, { log: GroupMessage[] }>
    }

    expect(room.gateway.uiMetaRevisions['clover-bots-groups']).toBe(2)
