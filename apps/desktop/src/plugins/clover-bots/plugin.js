/**
 * Clover Bot Mode — a "one chat per agent" roster for the Clover desktop.
 *
 * Left pane "Bots": one row per Clover profile (a bot = an agent profile) with
 ...
 *
 * Right tile "Routines": scheduled tasks (Clover cron jobs) scoped to the
 ...
 * bot-initiated sends use `clover -p <bot> chat --in ~ -c "Bot Chat"`.
 */

import * as sdk from '@clover/plugin-sdk'
import {
  atom,
  Button,
  Checkbox,
...
const BOT_ATTENTION_HINTS = {
  ...
  missing_config: 'Provider not configured — run clover model',
...
/** Mirrors host.paneVisibility('clover-bots:pane') — wired in register(). */
...
const GROUP_CHAT_SYNC_META_KEY = 'clover-bots-groups'
 ...
 *  clients (Clover Go, headless backends) see rooms regardless of which
...
// to (local, remote URL, SSH, Clover Cloud, docker) must be able to find
  ...
  try {
    ...
    return profiles
      .map(profile => ({
        ...
        title: String(profile?.ui_meta?.['clover-bots']?.title || profile?.display_name || ''),
...
async function saveBotMeta(owner, patch) {
  ...
  try {
    ...
    const request = route ? requestForBot(bot, 'profiles.configure', { name, ui_meta: { 'clover-bots': rest } }) :
      host.request('profiles.configure', { name, ui_meta: { 'clover-bots': rest } })
...
// handoffs (`clover -p <bot> chat --in ~ -c "Bot Chat" --create-if-missing`)
  ...
  for (const bot of roster) {
    ...
    if (image && typeof image === 'string' && image.startsWith('data:')) {
      ...
      Promise.resolve(request)
        .then(() => queryClient.invalidateQueries({ queryKey: ['clover-bots', 'roster'] }))
...
function mergeServerMeta(roster, fetchedAt = 0) {
  ...
  for (const bot of roster) {
    const server = bot.ui_meta?.['clover-bots']
...
/** Permanently delete a bot's Clover profile, then remove plugin-local state
 ...
 * directory (clover-c1#52279). That is the "can't delete a bot" error. */
...
if (typeof document !== 'undefined' && !document.getElementById('clover-bots-roster-css')) {
  const style = document.createElement('style')
  style.id = 'clover-bots-roster-css'
  style.textContent =
    '.clover-bots-roster [data-radix-scroll-area-viewport] > div {' +
    ' display: block !important; width: 100%; min-width: 0; }' +
    '.clover-scroll-cap > [data-radix-scroll-area-viewport] { max-height: inherit; }' +
    '@keyframes clover-bots-pulse { 0%,100% { opacity: 1; } 50% { opacity: 0.35; } }' +
    '.clover-bots-pulse { animation: clover-bots-pulse 1.2s ease-in-out infinite; }'
...
// and the row falls back to the "run clover mcp / Settings" hint. profile is
...
function McpSetupButton({ profile, entry, onDone, ensureProfile }) {
  ...
  const beginOAuth = async () => {
    ...
    try {
      ...
      } else if (typeof window !== 'undefined' && window.cloverDesktop && window.cloverDesktop.openExternal) {
        window.cloverDesktop.openExternal(authUrl)
      } else {
      ...
      tab === 'generate'
        ? imagen
          ...
          : jsx('div', {
              ...
              children:
                imagen === false
                  ? 'No image model available. If you just enabled one (or updated Clover), restart the gateway: Ctrl+K → "Restart gateway".'
...
function PetTab({ image, onImage }) {
  ...
  if (!pets.length) {
    return jsx('div', {
      ...
      children: 'No pets in the petdex gallery. Run `clover pets` to explore.'
    })
  }
 ...
 *  protocol into the system prompt itself (clover-c1 bot_mode_probe).
...
function useRoster() {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      // Multi-source desktops (clover-c1 #86875) also expose the union
 ...
 *  callable alias is 'clover' — the mention middleware resolves it back to
...
function botHandle(name, bot) {
  ...
  return (name || '').trim().toLowerCase() === 'default' ? 'clover' : name
}

/** Taggable @-forms derived from a bot's friendly names — the core profile
 *  display name (`clover profile rename`) and the Bot Mode title. Free text
 ...
 *  (researchbuddy). Reserved tokens are dropped so a bot renamed "Clover"
 *  can never hijack the primary profile's @clover alias. */
function mentionNameForms(value) {
  ...
  return [...new Set([slug, collapsed])].filter(
    form => /^[a-z0-9][a-z0-9_-]*$/.test(form) && !['all', 'everyone', 'user', 'default', 'clover'].includes(form)
  )
}
...
function botFriendlyNames(bot) {
  ...
  return [bot?.ui_meta?.['clover-bots']?.title, localTitle, aliasTitle, bot?.title, bot?.display_name]
}
...
function newBotChat(bot) {
  if (typeof host.newChat !== 'function') {
    host.notify?.({ kind: 'error', message: 'Update Clover Desktop to open another Bot chat.' })
...
function resolveRosterMentions(text, roster, active = {}) {
  ...
  for (const match of prose.matchAll(/(^|\s)@([a-z0-9][a-z0-9_-]*)/gi)) {
    ...
    if (token === 'clover') {
      token = byForm.has('clover') ? 'clover' : token
    }
...
// raw Cloud hostname, or to generic 'Clover' in Cloud-only mode.
//
...
// grandfather, or recover. Stored-id pins (ui_meta['clover-bots'].chat) were
...
async function openStoredBotChat(owner, storedId, summary) {
  if (!storedId || typeof host.openSession !== 'function') {
    throw new Error('This Clover Desktop version cannot open stored sessions')
  }
  ...
  // even though the session is fine (clover-c1#89617) — clicking Retry
...
async function prepareBotSource(bot) {
  ...
  if (route && typeof host.requestProfile !== 'function') {
    throw new Error('Update Clover Desktop to chat with agents on other connections.')
  }
...
async function ensureBotMetadata(bot) {
  ...
  const server = row?.ui_meta?.['clover-bots']
...
function displayName(bot, meta) {
  ...
  // "Clover". Annotated active rows carry sourceScoped too, and keying this
  ...
  // Core-profile display name (profile.yaml, set via `clover profile rename
  ...
  // never generic "Clover" or a hostname-derived label.
  if (alias) {
  ...
  // that reads like nobody bothered. Present it as Clover (the agent it is)
...
function durableGroupChatMembers(bots) {
  return (bots || []).map(bot => {
    ...
    const title = String(botRosterMeta(bot, $botMeta.get())?.title || bot.ui_meta?.['clover-bots']?.title || bot.title || '').trim()
...
// turn in its OWN persistent per-group Clover session and is fed only the
 ...
 *  renamed bot never shows up as its raw profile id or a stale "Clover"
 *  (community report, Aug 21 2026: renamed default still read "Clover is
 *  thinking…" in group rooms). The untitled primary profile is literally
 *  named "default" — render it as Clover (matching displayName and the
 *  @clover handle) so the main agent never loses its name in rooms. */
function groupSpeakerLabel(name) {
  ...
  // Core profile display_name (`clover profile rename …` / dashboard) from
  ...
  return trimmed.toLowerCase() === 'default' ? 'Clover' : trimmed
}
...
function messagingProtocolSection(name, roster) {
  ...
  return [
    ...
    '',
    '```',
    'clover -p <agent-name> chat --in ~ -c "Bot Chat" --create-if-missing -Q -q "Message from \uD83E\uDD16 ' + handle + ' (@' + handle + '): your message"',
    '',
    ...
    '',
    'The roster grows over time — run `clover profile list` for the LIVE',
...
function previewKind(preview) {
  ...
  if (match) {
    ...
    // profile reads @clover, never @default (#89484).
 ...
 *  (clover-c1#90268). Older gateways omit worker_session — always false. */
...
function AdvancedProfileConfig({ bot, state, setState }) {
  ...
  if (unsupported) {
    return jsx('div', {
      className: 'px-2 py-3 text-center text-xs text-(--ui-text-tertiary)',
      children: 'Full configuration needs a newer gateway (restart it after updating Clover).'
    })
  }
  ...
  // (clover-c1#87317): Skills (installed list + one-click hub installs +
...
function HubSkillsSection({ forProfile, onInstalled }) {
  ...
  useEffect(() => {
    ...
    const onMessage = event => {
      ...
      if (!data || data.type !== 'clover-skill-pick' || !data.name) {
        return
      }
  ...
  const ensureAgentCreated = () => {
    ...
    const flight = singleFlight(flightRef, async () => {
      ...
      if (remoteTarget) {
        ...
        try {
          void requestForTarget('profiles.configure', { name: slug, ui_meta: { 'clover-bots': look } }).catch(() => undefined)
                    ...
                    advTab === 'general'
                      ...
                      : capsFailed
                        ? jsx('div', {
                            className: 'px-2 py-3 text-center text-xs text-(--ui-text-tertiary)',
                            children:
                              'Capability catalog needs a newer gateway (restart it after updating Clover).'
                          })
                        : !caps
                          ? jsx('div', {
                              className: 'flex justify-center py-4',
...
// clover -p <bot> chat delegation wrapper so the run reaches that bot's
...
function routinePrompt(bot, title, instruction, activeProfile) {
  ...
  return (
    ...
    `clover -p ${shellQuote(bot)} chat -c ${shellQuote(`Routine: ${title}`)} -q ${shellQuote(`[Scheduled routine] ${instruction}`)}\n\n` +
...
// Clover-native schedule string; Advanced exposes it raw.
const FREQUENCIES = [
...
/** Compose the Clover schedule string from picker state. */
...
function GroupChatWorkspace({ group, members, onBack, visible = true }) {
  ...
  const renderEntry = (entry, index) => {
                  ...
                  // primary "default" profile renders as Clover, remote dupes
...
function BotsHomeView() {
  ...
  const gateway = bot.connectionLabel || (bot.connectionId === 'local' ? 'This device' : 'Clover gateway')
...
function GroupRow({ active, group, members, needsYou, onOpen, onDisband }) {
  ...
  // and the group prompt: the primary profile is @clover, not @default.
...
function BotsPane() {
  ...
  const activityOf = bot => {
    const created = botRosterMeta(bot, allMeta)?.created || bot.ui_meta?.['clover-bots']?.created || 0
        ...
        : error && !roster.length
          ? jsxs('div', {
              ...
              children: [
                jsx('div', {
                  children: gatewayUp
                    ? `Roster unavailable: ${error instanceof Error ? error.message : 'gateway error'}. If your gateway predates profiles.list, update Clover and restart the gateway.`
          ...
          : roster.length === 0
            ...
            : allBotsHidden && !hiddenExpanded
              ...
              : rosterRows.length === 0 && matchingHiddenBots.length === 0
                ...
                : jsx(ScrollArea, {
                    className: 'clover-bots-roster min-h-0 flex-1',
                    children: jsx('div', {
        ...
        description: deleting
          ? jsxs('span', {
              children: [
                ...
                ' and its associated Clover profile at ',
...
export default {
  ...
  register(ctx) {
    ...
    // excluded (a bot doesn't @ itself); 'default' surfaces as @clover.
    ctx.register({
    ...
    if (!document.getElementById('clover-bots-keyframes')) {
      const style = document.createElement('style')
      style.id = 'clover-bots-keyframes'
      style.textContent = '@keyframes clover-bots-bob { from { transform: translateY(0); } to { transform: translateY(-3px); } }'
      document.head.appendChild(style)
    }
