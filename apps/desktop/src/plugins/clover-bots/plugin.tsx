...
import { CHAT_EMPTY_AREA, COMPOSER_AREAS, host, PALETTE_AREA, translateNow } from '@clover/plugin-sdk'
import type { ChatEmptyProps, PluginContext } from '@clover/plugin-sdk'
...
export default {
  ...
  register(ctx: PluginContext) {
            ...
            try {
              const registry =
                typeof window !== 'undefined'
                  ? await Promise.resolve(window.cloverDesktop?.connections?.list?.()).catch(() => null)
                  : null
    ...
    try {
      if (typeof window !== 'undefined') {
        unbindConnectionsChanged =
          window.cloverDesktop?.connections?.onChanged?.(payload => {
    ...
    // A bot's chat before it has spoken: core's splash is Clover' wordmark and
      ...
      data: {
        handler: async (draft: ComposerDraftPayload): Promise<ComposerDraftPayload> => {
          ...
          const lines = mentionedBots.map(bot => {
            ...
            const title = String(
              botRosterMeta(bot, $botMeta.get())?.title || bot.ui_meta?.['clover-bots']?.title || bot.title || ''
            ).trim()
