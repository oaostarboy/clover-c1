...
describe('routing', () => {
  ...
  it('resolves @clover to the default member', async () => {
    const { rounds } = await loadRoom()
    ...
    const parsed = rounds.parseGroupChatMentions('@clover take a look', members)
...
describe('turn prompt', () => {
  it('addresses the default profile as @clover', async () => {
    const { rounds } = await loadRoom()
    ...
    expect(own).toMatch(/You are @clover,/)
    ...
    expect(peer).toMatch(/group chat with @clover/)
  })
