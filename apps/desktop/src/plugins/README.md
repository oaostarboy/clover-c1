...
Drop a `<name>/plugin.{ts,tsx}` here that default-exports a `CloverPlugin` and
...
`$CLOVER_HOME/desktop-plugins/<name>/plugin.js` (the disk door) — see the
`clover-desktop-plugins` skill.
