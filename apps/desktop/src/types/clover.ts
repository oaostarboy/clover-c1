...
export interface OAuthProvider {
   ...
   *  embedded terminal. Null when Clover doesn't know how to remove it. */
...
export interface EnvVarInfo {
  ...
  // in clover_cli/provider_catalog.py). When present, the Keys tab groups by
  // this provider identity — the SAME one `clover model` uses — instead of
...
export type CloverConfigRecord = Record<string, unknown>
...
export interface ModelOptionProvider {
   ...
   *  the `clover model` CLI / onboarding OAuth flow. */
  auth_type?: string
  ...
  /** Clover only: whether the current account is on the free tier. */
  free_tier?: boolean
  /** Clover only: paid models a free-tier user cannot select (shown disabled). */
...
// non-desktop) Clover simply carries none of it. See store/profile-share.ts.
export interface ProfileDesktopOverlay {
  ...
  /** Layout tree (clover.desktop.layoutTree.v2 shape). */
  layoutTree?: unknown
...
// folders. Mirrors clover_cli/projects_db.Project.to_dict().
...
export interface SkillInfo {
  ...
  usage?: number
  /** 'agent' = learned/local (editable), 'bundled' = ships with Clover, 'hub' = installed. */
 ...
 *  not Clover); elsewhere, driver health from `cua-driver doctor`. `null`
...
export interface ActionStatusResponse {
  ...
  pid: number | null
  running: boolean
  /** clover-update only: durable completion identity recovered from update.log. */
  action_id?: string
  /** clover-update only: summary of the durable update receipt (#91277 bullet 3) —
...
/** Shape of `GET /api/clover/update/check` — the backend's own update state.
...
/** One Clover-approved MCP catalog entry from `GET /api/mcp/catalog`. */
