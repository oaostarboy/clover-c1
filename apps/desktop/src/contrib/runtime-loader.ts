 ...
 *   (`@clover/plugin-sdk` / `react*` -> live shim blobs, see sdk/runtime.ts)
 *   -> blob `import()` -> validate default CloverPlugin -> register(ctx)
 *
 ...
 * and the two on-disk doors — `<clover home>/desktop-plugins/<name>/plugin.js`
 * and the unified agent-plugin half `<clover home>/plugins/<name>/desktop/
...
import { createPluginContext, type CloverPlugin } from './plugin'
...
interface LoadOptions {
   ...
   *  unified agent-plugin root sets this so `~/.clover/plugins` keeps its
...
/** Rewrite ONLY mapped import specifiers (@clover/plugin-sdk, react*) to their
...
export async function loadRuntimePlugin(
  ...
  try {
    ...
    if (unsupported.length > 0) {
      throw new Error(
        `unsupported import${unsupported.length > 1 ? 's' : ''}: ${unsupported.join(', ')} — ` +
          `runtime plugins may only import @clover/plugin-sdk and react`
      )
    }

    const url = URL.createObjectURL(new Blob([rewriteSpecifiers(source)], { type: 'text/javascript' }))

    let mod: { default?: CloverPlugin }
    ...
    if (!plugin?.id || typeof plugin.register !== 'function') {
      throw new Error(`${origin} has no valid default CloverPlugin export`)
    }

    // A disk/runtime copy of a plugin that now ships BUNDLED (e.g. a
    // standalone install of clover-bots predating its adoption in-tree) must
...
//  - `<clover home>/desktop-plugins/<name>/plugin.js` — the standalone door
//    (agent- or user-written desktop-only plugins);
//  - `<clover home>/plugins/<name>/desktop/plugin.js` — the desktop HALF of a
 ...
 *  backend's clover_home — #66899). `agentPluginsRoot` is optional: older
...
async function diskRoots(): Promise<DiskRoot[]> {
  ...
  if (unified) {
    // Opt-in by default: `~/.clover/plugins` is installed-but-inert until the
...
async function readPluginSourceText(file: string): Promise<string> {
  ...
  if (result.truncated) {
    throw new PluginSourceOversizeError(
      "plugin.js exceeds this shell's 512 KiB read limit — update Clover Desktop to load larger plugins"
    )
  }
