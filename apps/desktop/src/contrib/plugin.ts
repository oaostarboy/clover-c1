 ...
 * `CloverPlugin`; it never touches the registry directly — it receives a
...
import { pluginRest, type PluginRestOptions, pluginSocket } from '@/clover'
...
export type { PluginRestOptions } from '@/clover'
 ...
 *  under `clover.plugin.<id>.` — plugins can't read or clobber each other. */
 ...
 *  in one attributed namespace instead of the raw `window.cloverDesktop`
...
export interface PluginOs {
   ...
   *  the user is away from Clover — use `host.notify` for the in-app toast.
   *  Throttled per plugin; reserve it for genuinely notable events.
   *  Supports `icon`, `activate` (e.g. `clover://index-network/intent/1`),
...
function createPluginStorage(pluginId: string): PluginStorage {
  const scoped = (key: string) => `clover.plugin.${pluginId}.${key}`
...
function createPluginOs(pluginId: string): PluginOs {
  const attempt = async (run: (bridge: NonNullable<typeof window.cloverDesktop>) => Promise<boolean>) => {
    const bridge = typeof window === 'undefined' ? undefined : window.cloverDesktop
  ...
  const attemptPath = async (run: (bridge: NonNullable<typeof window.cloverDesktop>) => Promise<null | string>) => {
    const bridge = typeof window === 'undefined' ? undefined : window.cloverDesktop
