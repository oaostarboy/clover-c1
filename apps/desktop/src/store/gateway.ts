...
const isOpen = (gateway: CloverGateway | null): boolean => gateway?.connectionState === 'open'
...
interface RegistryConfig {
  ...
  onActiveConnectionChanged?: (connection: CloverConnection) => void
  /**
...
interface Secondary {
  ...
  connection: CloverConnection | null
...
interface GatewayRegistryState {
  ...
  primaryGateway: CloverGateway | null
  ...
  $gateway: ReturnType<typeof atom<CloverGateway | null>>
  $activeProfile: ReturnType<typeof atom<string>>
}

const STATE_KEY = Symbol.for('clover.desktop.gatewayRegistryState')
...
function createRegistryState(): GatewayRegistryState {
  return {
    ...
    $gateway: atom<CloverGateway | null>(null),
...
export function setPrimaryGateway(gateway: CloverGateway | null, profile = 'default'): void {
...
export function setPrimaryGatewayConnection(connection: Pick<CloverConnection, 'connectionId'> | null): void {
...
export function activeGateway(): CloverGateway | null {
  if (g.activeKey === g.primaryProfile) {
...
function applyActive(profile: string, activationEpoch: number): boolean {
  ...
  // Push the active scope's registry connection into the clover module (null
...
function publishActiveConnection(connection: CloverConnection): void {
...
function createSecondary(profile: string, connectionId: null | string = null): Secondary {
  const gateway = new CloverGateway()
...
async function gatewayForProfile(
  profile: string,
  leaseRequest = false
): Promise<{ gateway: CloverGateway | null; key: string; release: () => void; scopeProfile: boolean }> {
...
export async function requestGatewayForProfile<T>(
  ...
  try {
    if (!route.gateway) {
      throw new Error(`Clover gateway unavailable for profile "${route.key}"`)
    }
...
export async function requestGatewayForAgent<T>(
  ...
  if (!window.cloverDesktop?.getConnectionFor) {
    throw new Error('This Desktop build cannot dial registry connections. Update Clover Desktop.')
  }
...
export async function ensureActiveGatewayOpen(): Promise<CloverGateway | null> {
  if (g.activeKey === g.primaryProfile) {
    return g.primaryGateway
  }
  ...
  if (!isOpen(entry.gateway)) {
    ...
    // "Clover gateway is not connected" on the Sessions `+` action (#88880).
