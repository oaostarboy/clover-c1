...
import type { ProfileInfo } from '@/types/clover'
...
// /api/profiles/active `current`). "default" is the root ~/.clover. This is the
...
const PROFILE_ORDER_STORAGE_KEY = 'clover.desktop.profileOrder'
...
const PROFILE_COLORS_STORAGE_KEY = 'clover.desktop.profileColors'
...
export async function refreshActiveProfile(): Promise<void> {
  ...
  try {
    const res = await cloverApi<ActiveProfileResponse>({
...
// Persist the choice and relaunch the backend under the new CLOVER_HOME. The
...
export async function switchProfile(name: string): Promise<void> {
  ...
  await window.cloverDesktop.profile.set(name)
}
...
async function resolveConnectionForProfile(profile: string): Promise<CloverConnection | null> {
  const getConnection = window.cloverDesktop?.getConnection
...
// previous descriptor.
async function resolveConnectionForAgent(connectionId: string, profile: string): Promise<CloverConnection | null> {
  const getConnectionFor = window.cloverDesktop?.getConnectionFor
...
const SHOW_ALL_PROFILES_STORAGE_KEY = 'clover.desktop.showAllProfiles'
...
export function selectProfile(name: string): void {
  ...
  void Promise.all([activateOnCurrentSource(target), shouldRememberStartupProfile])
    .then(([, shouldRemember]) => {
      if (shouldRemember) {
        return window.cloverDesktop?.profile?.remember(target)
      }
...
async function isLocalDesktopProfile(target: string): Promise<boolean> {
  const getConnectionConfig = window.cloverDesktop?.getConnectionConfig
...
// Switch to the default (root ~/.clover) profile — bound to ⌘1.
...
export function touchActiveGatewayBackend(): void {
  ...
  void window.cloverDesktop?.touchBackend?.(target).catch(() => undefined)
}
