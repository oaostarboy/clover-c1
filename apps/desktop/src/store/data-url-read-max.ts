 ...
 * (`clover:data-url-read-max:*`). The default and the clamp are shared with
...
export async function refreshDataUrlReadMaxMb(): Promise<number> {
  const api = window.cloverDesktop?.dataUrlReadMax
...
if (typeof window !== 'undefined' && window.cloverDesktop?.dataUrlReadMax) {
