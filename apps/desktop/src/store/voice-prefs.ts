...
import { getCloverConfigRecord, saveCloverConfig } from '@/clover'
...
export async function setAutoSpeakReplies(enabled: boolean): Promise<void> {
  ...
  try {
    const record = await getCloverConfigRecord()
    const voice = record.voice && typeof record.voice === 'object' ? (record.voice as Record<string, unknown>) : {}

    await saveCloverConfig({ ...record, voice: { ...voice, auto_tts: enabled } })
  } catch (error) {
