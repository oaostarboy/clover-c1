...
import type { ModelOptionProvider, OAuthProvider, OAuthStartResponse } from '@/types/clover'
...
const CONFIGURED_CACHE_KEY = 'clover-desktop-onboarded-v1'
const SKIP_CACHE_KEY = 'clover-onboarding-skipped-v1'
const POLL_MS = 2000
...
async function fetchProviderDefaultModel(
  ...
  // `clover model` does (for Clover it honors the user's free/paid tier, so a
...
function providerResolutionFailure(reason: null | string) {
  ...
  return detail
    ? `Connected, but Clover still cannot resolve a usable provider. ${detail}`
    : 'Connected, but Clover still cannot resolve a usable provider.'
}
...
export async function refreshOnboarding(ctx: OnboardingContext) {
  ...
  if (shouldPreserveConfiguredOnFallback(runtime, state)) {
    ...
    notify({
      ...
      message:
        'Clover Desktop could not verify the running backend on startup. Some features may be unavailable until the gateway is reachable.'
    })
...
export async function recheckExternalSignin(ctx: OnboardingContext) {
  ...
  await completeWithModelConfirm(ctx, provider.name, [provider.id], reason =>
    setFlow({
      ...
      message:
        reason?.trim() ||
        `Clover still cannot reach ${provider.name}. Run \`${provider.cli_command}\` in a terminal first.`
    })
  )
}
...
export async function saveOnboardingLocalEndpoint(baseUrl: string, apiKey: string, ctx: OnboardingContext) {
  ...
  try {
    ...
    if (!runtime.ready) {
      ...
      return { ok: false, message: detail || `Saved, but Clover still cannot reach ${url}.` }
    }
