 ...
 * app's bounds and stays visible while Clover is minimized. That window carries
...
const OVERLAY_ACTIVE_KEY = 'clover.desktop.pet-overlay-active.v1'
const OVERLAY_BOUNDS_KEY = 'clover.desktop.pet-overlay-bounds.v1'
...
function pushNow(): void {
  window.cloverDesktop?.petOverlay?.pushState(currentPayload())
}
...
function openOverlay(request: PetOverlayOpenRequest): void {
  const api = window.cloverDesktop?.petOverlay
...
export function restorePetOverlay(): void {
  if (!window.cloverDesktop?.petOverlay || !$petOverlayActive.get() || stateUnsubs.length) {
    return
  }
...
export function popInPet(): void {
  ...
  stateUnsubs = []
  $petOverlayActive.set(false)
  void window.cloverDesktop?.petOverlay?.close()
}
