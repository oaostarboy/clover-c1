import type { ConnectionState } from '@clover/shared'
...
import type { SessionInfo, UsageStats } from '@/types/clover'
...
const COMPOSER_MODEL_KEY = 'clover.desktop.composer.model'
const COMPOSER_PROVIDER_KEY = 'clover.desktop.composer.provider'
const COMPOSER_MODEL_SOURCE_KEY = 'clover.desktop.composer.model-source'
const COMPOSER_EFFORT_KEY = 'clover.desktop.composer.reasoning-effort'
const COMPOSER_FAST_KEY = 'clover.desktop.composer.fast'
...
const LAST_SESSION_KEY = 'clover.desktop.lastSessionId'
const LAST_ROUTE_KEY = 'clover.desktop.lastRoute'
...
function workspaceCwdKey(connection: CloverConnection | null = $connection.get()): string {
...
export async function syncConfiguredDefaultProjectDir(shouldPublish: () => boolean = () => true): Promise<string> {
  const settings = window.cloverDesktop?.settings?.getDefaultProjectDir
...
export async function ensureDefaultWorkspaceCwd(shouldPublish: () => boolean = () => true): Promise<void> {
  const sanitize = window.cloverDesktop?.sanitizeWorkspaceCwd
...
export const $connection = atom<CloverConnection | null>(null)
...
const SESSION_OWNER_HINTS_KEY = 'clover.desktop.sessionOwnerHints.v1'
...
export const setConnection = (next: Updater<CloverConnection | null>) => {
...
// until config loads, and re-seeded on every profile switch by useCloverConfig.
