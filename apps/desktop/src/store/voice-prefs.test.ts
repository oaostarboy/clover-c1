...
vi.mock('@/clover', () => ({
  getCloverConfigRecord: vi.fn(async () => ({})),
  saveCloverConfig: vi.fn(async () => undefined)
}))
...
describe('applyVoiceStopPhraseFromConfig', () => {
  ...
  it('uses the first configured phrase so a custom phrase renders correctly', () => {
    applyVoiceStopPhraseFromConfig({ voice: { stop_phrases: ['goodbye clover', 'stop'] } })
    expect($voiceStopPhrase.get()).toBe('goodbye clover')
  })
