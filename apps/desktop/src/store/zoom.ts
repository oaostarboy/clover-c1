...
export function setZoomPercent(percent: number): void {
  window.cloverDesktop?.zoom?.setPercent(percent)
}

if (typeof window !== 'undefined' && window.cloverDesktop?.zoom) {
  void window.cloverDesktop.zoom.get().then(({ percent }) => $zoomPercent.set(percent))
  window.cloverDesktop.zoom.onChanged(({ percent }) => $zoomPercent.set(percent))
}
