...
// reaches the gateway); everything else routes through window.cloverDesktop.git,
...
function file(path: string, over: Partial<CloverReviewFile> = {}): CloverReviewFile {
  return { path, status: 'modified', staged: false, added: 1, removed: 0, ...over } as CloverReviewFile
}

type ReviewStub = Record<string, ReturnType<typeof vi.fn>>

// Install a review bridge on window.cloverDesktop. Any op not supplied defaults
...
describe('ship flow', () => {
  ...
  it('createOrOpenPr opens the existing PR without creating a new one', async () => {
    const review = stubReview()
    $reviewShipInfo.set({ ghReady: true, pr: { url: 'https://example.com/pr/9' } } as CloverReviewShipInfo)

    await createOrOpenPr()

    expect(review.createPr).not.toHaveBeenCalled()
    expect(
      (window.cloverDesktop as unknown as { openExternal: ReturnType<typeof vi.fn> }).openExternal
...
describe('refreshShipInfo', () => {
  it('populates ship info from the bridge', async () => {
    const info: CloverReviewShipInfo = {
      ghReady: true,
      pr: { url: 'https://example.com/pr/3' }
    } as CloverReviewShipInfo
  ...
  it('resets ship info when there is no bridge', async () => {
    delete (window as unknown as { cloverDesktop?: unknown }).cloverDesktop
    $reviewShipInfo.set({ ghReady: true, pr: { url: 'x' } } as CloverReviewShipInfo)
