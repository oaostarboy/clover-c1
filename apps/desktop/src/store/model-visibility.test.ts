...
describe('model visibility', () => {
  ...
  it('preserves hidden-provider sentinel without re-adding defaults', () => {
    // User explicitly hid all models for "clover" — sentinel marks this choice.
    const stored = new Set([emptyProviderSentinelKey('clover')])

    const visible = effectiveVisibleKeys(stored, [
      provider('clover', ['clover-3-llama-3.1-70b', 'clover-3-llama-3.1-8b']),
      provider('ollama', ['qwen3:latest'])
    ])

    expect(visible.has(modelVisibilityKey('clover', 'clover-3-llama-3.1-70b'))).toBe(false)
    expect(visible.has(modelVisibilityKey('clover', 'clover-3-llama-3.1-8b'))).toBe(false)
    // Sentinel itself is stripped from the result.
    expect(visible.has(emptyProviderSentinelKey('clover'))).toBe(false)
  ...
  it('restores model when toggling on after hiding all', () => {
    // Simulates: user hid all "clover" models, then toggles one back on.
    const stored = new Set([emptyProviderSentinelKey('clover'), modelVisibilityKey('ollama', 'qwen3:latest')])

    // After toggle: sentinel removed, one model added.
    const afterToggle = new Set(stored)
    afterToggle.delete(emptyProviderSentinelKey('clover'))
    afterToggle.add(modelVisibilityKey('clover', 'clover-3-llama-3.1-70b'))
    ...
    expect(visible.has(modelVisibilityKey('clover', 'clover-3-llama-3.1-70b'))).toBe(true)
    expect(visible.has(modelVisibilityKey('clover', 'clover-3-llama-3.1-8b'))).toBe(false)
  })
  ...
  it('resolveVisibleKeys preserves sentinels that effectiveVisibleKeys strips', () => {
    const stored = new Set([emptyProviderSentinelKey('clover')])
    const providers = [provider('clover', ['clover-x', 'clover-y']), provider('ollama', ['qwen3:latest'])]

    const resolved = resolveVisibleKeys(stored, providers)
    expect(resolved.has(emptyProviderSentinelKey('clover'))).toBe(true)
    expect(resolved.has(modelVisibilityKey('clover', 'clover-x'))).toBe(false)
    ...
    expect(effectiveVisibleKeys(stored, providers).has(emptyProviderSentinelKey('clover'))).toBe(false)
  })
})

describe('toggleModelVisibility', () => {
  const providers = [provider('openai', ['gpt-a', 'gpt-b']), provider('clover', ['clover-x', 'clover-y'])]
  ...
  it('keeps a hidden provider hidden when a different provider is toggled (regression for #43485)', () => {
    ...
    expect(visible.has(modelVisibilityKey('clover', 'clover-x'))).toBe(false)
    expect(visible.has(modelVisibilityKey('clover', 'clover-y'))).toBe(false)
  })

  it('clears only the toggled provider sentinel when a model is re-enabled', () => {
    let stored: Set<string> | null = new Set([emptyProviderSentinelKey('openai'), emptyProviderSentinelKey('clover')])
  ...
  it('toggling from an empty (non-null) stored set adds the model without expanding defaults', () => {
    ...
    expect(next.has(modelVisibilityKey('clover', 'clover-x'))).toBe(false)
  })
  ...
  it('toggling off one default model from null stored keeps the rest of the curated defaults', () => {
    ...
    expect(next.has(modelVisibilityKey('clover', 'clover-x'))).toBe(true)
...
describe('featured defaults', () => {
  ...
  it('defaults to the featured shortlist when a provider publishes one', () => {
    ...
    expect(visible.has(modelVisibilityKey('clover', 'google/gemini'))).toBe(true)
    expect(visible.has(modelVisibilityKey('clover', 'x-ai/grok'))).toBe(true)
    expect(visible.has(modelVisibilityKey('clover', 'anthropic/haiku'))).toBe(false)
  })
...
describe('setProviderVisibility', () => {
  ...
  it('collapses model families to one key per family when enabling', () => {
    ...
    const ps = [provider('clover', ['model', 'model-fast'])]

    const next = setProviderVisibility(null, ps, 'clover', true)

    expect(next.has(modelVisibilityKey('clover', 'model'))).toBe(true)
    // The -fast sibling is represented by its base family, not its own key.
    expect(next.has(modelVisibilityKey('clover', 'model-fast'))).toBe(false)
  })
})
