 ...
 *  Working in `~/www/clover-c1`, the draft says "clover-c1" constantly
 *  — a "Use skill: clover-c1" pill on every mention is noise (the reported
 *  annoyance that prompted this guard). Boundary containment, not exact
 *  segment equality, so worktree dirs (`clover-c1-suggest`) suppress too. */
 ...
 *  segment of a qualified form (`github/clover-c1-dev`, `plugin:skill`). */
