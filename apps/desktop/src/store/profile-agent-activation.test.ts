...
const agentConn = (over: Partial<CloverConnection> = {}): CloverConnection =>
  ({ baseUrl: 'https://homelab.invalid', mode: 'remote', profile: 'research', ...over }) as CloverConnection

const localConn = (over: Partial<CloverConnection> = {}): CloverConnection =>
  ({ baseUrl: '', mode: 'local', profile: 'default', ...over }) as CloverConnection

const getConnection = vi.fn<(profile?: string | null) => Promise<CloverConnection>>()

const getConnectionFor =
  vi.fn<(payload: { connectionId?: null | string; profile?: null | string }) => Promise<CloverConnection>>()
...
beforeEach(() => {
  ...
  vi.stubGlobal('window', { cloverDesktop: { getConnection, getConnectionFor } })
})
...
describe('ensureGatewayAgent commit hook (beforeActivate) — the Sessions switcher door (#93937)', () => {
  ...
  it('a descriptor lookup that never answers is bounded, fails open, and releases the mutex', async () => {
    ...
    try {
      getConnectionFor.mockImplementationOnce(() => new Promise<CloverConnection>(() => undefined))
