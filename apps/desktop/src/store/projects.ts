...
import type { ProjectInfo, ProjectsPayload } from '@/types/clover'
...
const PROJECT_SCOPE_KEY = 'clover.desktop.projectScope'
...
async function gatewayRequest<T>(method: string, params: Record<string, unknown> = {}): Promise<T> {
  ...
  if (!gateway) {
    throw new Error('Clover gateway is not connected')
  }
...
async function activeProjectsContext(): Promise<ActiveProjectsContext> {
  ...
  if (!gateway || gateway !== activeGateway() || profile !== projectProfile()) {
    throw new Error('Active Clover profile changed while connecting')
  }
...
async function refreshProjectTreeAcrossProfiles(): Promise<void> {
  ...
  try {
    const res = await cloverApi<ProjectTreePayload>({
...
const repoScanStates = new WeakMap<CloverGateway, RepoScanState>()
const scanningGatewayGenerations = new WeakMap<CloverGateway, number>()
...
export async function scanAndRecordRepos(force = false): Promise<void> {
  if (isDesktopFsRemoteMode()) {
    ...
    // with `scan: true` — added in #81723) so repos with zero Clover
  ...
  try {
    const policy = repoDiscoveryPolicyFromConfig(await getCloverConfig(context.profile))
...
// repo, returning where Clover should start working. Git is the source of
...
export async function listRepoBranches(repoPath: string): Promise<CloverGitBranch[]> {
  const git = desktopGit()
...
export async function listBaseBranches(repoPath: string): Promise<CloverGitBaseBranch[]> {
  const git = desktopGit()
...
export async function revealPath(path: null | string): Promise<void> {
  if (path) {
    await window.cloverDesktop?.revealPath?.(path)
  }
}
...
export async function copyPath(path: null | string): Promise<void> {
  if (path) {
    await window.cloverDesktop?.writeClipboard?.(path)
  }
}
