...
import type { CloverGitWorktree, CloverRepoStatus } from '@/global'
...
const EMPTY_WORKTREES: CloverGitWorktree[] = []
...
export const $repoStatusByCwd = atom<Record<string, CloverRepoStatus | null>>({})
export const $repoWorktreesByCwd = atom<Record<string, CloverGitWorktree[]>>({})
...
export const $repoStatus: ReadableAtom<CloverRepoStatus | null> = computed(
...
export const $repoWorktrees: ReadableAtom<CloverGitWorktree[]> = computed(
...
const statusAtomByCwd = new Map<string, ReadableAtom<CloverRepoStatus | null>>()
const worktreesAtomByCwd = new Map<string, ReadableAtom<CloverGitWorktree[]>>()
const $noRepoStatus = atom<CloverRepoStatus | null>(null)
const $noWorktrees = atom<CloverGitWorktree[]>(EMPTY_WORKTREES)

/** Reactive status for one repo cwd (a tile's worktree). Stable per cwd. */
export function repoStatusForCwd(cwd?: null | string): ReadableAtom<CloverRepoStatus | null> {
...
export function repoWorktreesForCwd(cwd?: null | string): ReadableAtom<CloverGitWorktree[]> {
  const key = normalizeCwd(cwd)
...
function setRepoStatusEntry(target: string, status: CloverRepoStatus | null): void {
...
function setRepoWorktreesEntry(target: string, worktrees: CloverGitWorktree[]): void {
...
interface RepoStatusRefreshRequest {
  probe: (cwd: string) => Promise<CloverRepoStatus | null>
  seq: number
  target: string
}
