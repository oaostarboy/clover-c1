...
const { getProfiles } = await import('@/clover')
...
const profile = (name: string, isDefault = false): ProfileInfo => ({
  ...
  model: null,
  name,
  path: `/tmp/clover/${name}`,
  provider: null,
  skill_count: 0
})

const remoteConn = (over: Partial<CloverConnection> = {}): CloverConnection =>
  ({ baseUrl: 'https://clover-roy.tail.ts.net', mode: 'remote', profile: 'vps-remote', ...over }) as CloverConnection
...
beforeEach(() => {
  ...
  $profiles.set([])
  vi.stubGlobal('window', { cloverDesktop: { getConnection } })
