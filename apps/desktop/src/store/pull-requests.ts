...
export const $pullRequestsByBranch = atom<Record<string, CloverBranchPullRequest>>({})
...
export const $prBranchBySession = persistentAtom<Record<string, string>>(
  'clover.desktop.prBranchBySession',
  {},
  Codecs.stringRecord
)
...
const $prScannedSessions = persistentAtom<string[]>('clover.desktop.prScannedSessions', [], Codecs.stringArray)
...
export function pullRequestBucket(pr: CloverBranchPullRequest | undefined): PullRequestBucket {
  if (!pr) {
    return 'none'
  }
