...
import type { CloverReviewFile, CloverReviewShipInfo } from '@/global'
...
//
// Scope is always "uncommitted" — Clover' flow is agent edits you review BEFORE
...
const OPEN_KEY = 'clover.desktop.reviewOpen'
const COMMIT_DEFAULT_KEY = 'clover.desktop.reviewCommitDefault'
const TREE_MODE_KEY = 'clover.desktop.reviewTreeMode'
const SELECTED_KEY = 'clover.desktop.reviewSelectedPath'
...
export const $reviewFiles = atom<CloverReviewFile[]>([])
...
export const $reviewShipInfo = atom<CloverReviewShipInfo>({ ghReady: false, pr: null })
...
type ReviewBridge = NonNullable<NonNullable<NonNullable<Window['cloverDesktop']>['git']>['review']>
...
export async function selectReviewFile(file: CloverReviewFile): Promise<void> {
...
function matchReviewFile(files: readonly CloverReviewFile[], path: string): CloverReviewFile | undefined {
...
export async function createOrOpenPr(): Promise<void> {
  ...
  if (existing?.url) {
    void window.cloverDesktop?.openExternal?.(existing.url)
  ...
  await runShip(async () => {
    ...
    if (url) {
      void window.cloverDesktop?.openExternal?.(url)
    }
