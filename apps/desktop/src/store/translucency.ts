 ...
 * `@clover/shared/translucency`, which owns the mapping both this store and the
 * main process read.
 *
 ...
 * material underneath the web contents to read (see the `[data-clover-glass]`
 * block in styles.css).
 */
...
} from '@clover/shared/translucency'
 ...
 * sets `cloverDesktop.glassSupported` from `os.release()` so Win10 cannot
...
export const GLASS_SUPPORTED =
  typeof window !== 'undefined' && typeof window.cloverDesktop?.glassSupported === 'boolean'
    ? window.cloverDesktop.glassSupported
...
export const TRANSLUCENCY_SUPPORTED =
  typeof window !== 'undefined' && typeof window.cloverDesktop?.translucencySupported === 'boolean'
    ? window.cloverDesktop.translucencySupported
    : isMacPlatform() || isWindowsPlatform()
...
/* Peek: while the user is actively adjusting translucency from Settings, the
   ...
   [data-clover-translucency-peek] rules in styles.css). A counter rather
   than a boolean: a held slider drag and a timed pulse from a picker click
   can overlap. */
const PEEK_ATTR = 'data-clover-translucency-peek'
...
const applyGlassSurfaces = ({ intensity, mode, scope }: TranslucencyState): void => {
  ...
  root.toggleAttribute('data-clover-glass-on', glassLive)
  root.toggleAttribute('data-clover-glass', glassOn)
  root.toggleAttribute('data-clover-clear', clearOn)
  ...
  if (glassOn) {
    root.setAttribute('data-clover-glass-scope', scope)
  } else {
    root.removeAttribute('data-clover-glass-scope')
  }
...
if (typeof window !== 'undefined') {
  ...
  $translucency.subscribe(state => {
    ...
    window.cloverDesktop?.setTranslucency?.(state)
  })
