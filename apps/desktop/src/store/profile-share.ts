 ...
 *
 * The archive is the CLI's own `clover profile export` tar.gz (config, skills,
...
import { exportProfileArchive, importProfileArchive } from '@/clover'
...
import type { ProfileDesktopOverlay } from '@/types/clover'
...
const ARCHIVE_FILTERS = [{ extensions: ['tar.gz', 'tgz'], name: 'Clover profile' }]
...
export async function runExportProfileFlow(profile?: string): Promise<null | string> {
  ...
  const pick = window.cloverDesktop?.selectSavePath
...
export async function runImportProfileFlow(): Promise<null | string> {
  const paths = await window.cloverDesktop?.selectPaths?.({
