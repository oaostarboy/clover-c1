 ...
 * composer with the reply scrolling above it, so Clover can be driven while
...
export const canUseHud = (): boolean =>
  typeof window !== 'undefined' && typeof window.cloverDesktop?.hud?.open === 'function'

export function openHud(sessionId?: null | string): void {
  const api = window.cloverDesktop?.hud
...
export function resetHudLayout(): void {
  void window.cloverDesktop?.hud?.resetLayout?.()
}
...
export const reportHudSession = (sessionId: null | string): void => window.cloverDesktop?.hud?.setSession?.(sessionId)
...
export function watchHudState(onClosed?: (sessionId: null | string) => void): () => void {
  const off = window.cloverDesktop?.hud?.onChanged?.(({ open, sessionId }) => {
    $hudActive.set(open)
