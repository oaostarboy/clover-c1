...
import { LOCAL_CONNECTION_ID, registryBackendScopeKey } from '@clover/shared'
...
const TILES_KEY = 'clover.desktop.sessionTiles.v2'
const LEGACY_TILES_KEY = 'clover.desktop.sessionTiles.v1'
...
const BOT_CHAT_SCOPE_KEY = 'clover.desktop.botChatSessions.v1'
 ...
 * restore re-dials the profile's backend, whose ensure_clover_home() re-creates
 * the profile directory the delete just removed (clover-c1#94235). Same
...
export function dropTilesForProfile(
  ...
  const ownerMatches = (owner: SessionProfileRoute | undefined): boolean => {
    ...
    // would orphan a live conversation (clover-c1#94235). Tiles persisted
...
export const $focusedStoredSessionId = computed(
  ...
  (groupId, tree, selected, workspaceMode) => {
    ...
    // it to the sidebar group, whose active pane ('clover-bots:pane') is not a
...
// Dev hook for automation (mirrors __CLOVER_LAYOUT_TREE__).
if ((import.meta.env.DEV || import.meta.env.VITE_PERF_PROBE === '1') && typeof window !== 'undefined') {
  ;(window as unknown as Record<string, unknown>).__CLOVER_SESSION_TILES__ = {
