 ...
 * packaged binary) with a real `clover serve` backend pointed at a mock
 * inference server. The full chain is exercised:
 *
 *   electron → clover serve (python) → mock provider → renderer
 *
...
test.describe('dev-mode boot with mock backend', () => {
  test('window opens with Clover title', async () => {
    const title = await fixture!.page.title()
    expect(title).toContain('Clover')
  })
  ...
  test('the preload bridge reaches the renderer', async () => {
    const bridge = await fixture!.page.evaluate(() => {
      const desktop = (window as unknown as { cloverDesktop?: Record<string, unknown> }).cloverDesktop
  ...
  test('backend boots and app becomes ready', async () => {
    ...
    // electron starts → clover serve is spawned → WS connects → config
