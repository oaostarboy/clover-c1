/**
 * Shared E2E fixtures for the Clover desktop Playwright suite.
 *
 * Two fixture modes:
 *
 ...
 *     (electron → clover serve → provider → inference → renderer) is
 ...
 * `electron-builder --dir` step and matches `clover desktop --source`. The
...
export function createSandbox(prefix: string): Sandbox {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), `clover-e2e-${prefix}-${Math.random()}`))
  const cloverHome = path.join(root, 'clover-home')
  const userDataDir = path.join(root, 'electron-user-data')

  fs.mkdirSync(cloverHome, { recursive: true })
...
export function writeMockProviderConfig(
  ...
  const configPath = path.join(cloverHome, 'config.yaml')
 ...
 */
export function writeEnvFile(cloverHome: string, apiKey = 'e2e-mock-key'): void {
  const envPath = path.join(cloverHome, '.env')
 ...
 */
function writeEmptyConfig(cloverHome: string): void {
  const configPath = path.join(cloverHome, 'config.yaml')
 ...
 *
 * Key env vars:
 *  - CLOVER_HOME → sandbox clover-home (isolated config/sessions)
 *  - CLOVER_DESKTOP_USER_DATA_DIR → sandbox electron-user-data
 *  - CLOVER_DESKTOP_IGNORE_EXISTING=1 → don't pick up `clover` from PATH
 *    (we want the dev checkout at REPO_ROOT)
 *  - CLOVER_DESKTOP_CLOVER_ROOT → REPO_ROOT (dev checkout resolution)
 *  - CLOVER_DESKTOP_APP_NAME → unique-ish per test (avoids single-instance lock)
...
export function buildAppEnv(sandbox: Sandbox, extra: Record<string, string> = {}): Record<string, string> {
  ...
  return {
    ...clean,
    CLOVER_HOME: sandbox.cloverHome,
    CLOVER_DESKTOP_USER_DATA_DIR: sandbox.userDataDir,
    CLOVER_DESKTOP_IGNORE_EXISTING: '1',
    CLOVER_DESKTOP_CLOVER_ROOT: REPO_ROOT,
    CLOVER_DESKTOP_APP_NAME: `CloverE2E-${Date.now()}`,
    ...
    CLOVER_DESKTOP_SKIP_QUIT_CONFIRM: '1',
 ...
 *
 * @param sandbox  - isolated CLOVER_HOME + userData
 * @param env      - the process environment (already has CLOVER_HOME etc.)
...
export async function setupMockBackend(options: MockBackendOptions = {}): Promise<MockBackendFixture> {
  ...
  writeEnvFile(sandbox.cloverHome)
...
export async function setupNoProvider(): Promise<NoProviderFixture> {
  ...
  writeEmptyConfig(sandbox.cloverHome)
...
export interface DeadBackendOptions {
  /**
   * When true, inject a fake boot error via CLOVER_DESKTOP_BOOT_FAKE_ERROR
 ...
 * nothing listens on). By default the backend still boots (`clover serve`
...
export async function setupDeadBackend(options: DeadBackendOptions = {}): Promise<DeadBackendFixture> {
  ...
  const configPath = path.join(sandbox.cloverHome, 'config.yaml')
  fs.writeFileSync(
    configPath,
...
`,
  ...
  const env = buildAppEnv(sandbox, options.fakeError ? { CLOVER_DESKTOP_BOOT_FAKE_ERROR: 'Failed to connect to Clover backend: connection refused' } : {})
  const { app, page } = await launchDesktop(env)
...
function resolvePackagedBinaryPath(): string {
  if (process.platform === 'win32') {
    return path.join(RELEASE_ROOT, 'win-unpacked', 'Clover.exe')
  }

  if (process.platform === 'darwin') {
    const arch = process.arch === 'arm64' ? 'arm64' : 'x64'

    return path.join(RELEASE_ROOT, `mac-${arch}`, 'Clover.app', 'Contents', 'MacOS', 'Clover')
  }

  return path.join(RELEASE_ROOT, 'linux-unpacked', 'clover')
}
 ...
 * progress without spawning a real Clover backend.
 *
 * Uses the same sandbox isolation (credential stripping, isolated
 * CLOVER_HOME + userData, unique app name) as the dev-mode fixtures.
 *
...
export async function setupPackagedApp(): Promise<PackagedAppFixture> {
  ...
  const env = buildAppEnv(sandbox, {
    ...
    CLOVER_DESKTOP_BOOT_FAKE: '1',
    CLOVER_DESKTOP_BOOT_FAKE_STEP_MS: '120',
  })

  // Clear dev-server + clover-root overrides — the packaged binary
  // should use its own bundled renderer, not the dev checkout.
  delete (env as Record<string, string | undefined>).CLOVER_DESKTOP_DEV_SERVER
  delete (env as Record<string, string | undefined>).CLOVER_DESKTOP_CLOVER
  delete (env as Record<string, string | undefined>).CLOVER_DESKTOP_CLOVER_ROOT
