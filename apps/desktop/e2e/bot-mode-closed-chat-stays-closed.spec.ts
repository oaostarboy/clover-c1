...
async function seedBot(cloverHome: string, mockUrl: string, name: string): Promise<void> {
  const dir = path.join(cloverHome, 'profiles', name)
...
test.beforeAll(async () => {
  ...
  await seedBot(sandbox.cloverHome, mock.url, 'alpha')
  await seedBot(sandbox.cloverHome, mock.url, 'beta')
