 ...
 * AIAgent loop, using the E2E mock provider configured in `cloverHome`.
 *
...
export class RealSessionBuilder {
  ...
  private constructor(cloverHome: string) {
  ...
  static async start(cloverHome: string): Promise<RealSessionBuilder> {
    const builder = new RealSessionBuilder(cloverHome)
