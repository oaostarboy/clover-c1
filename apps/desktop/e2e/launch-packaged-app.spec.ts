...
/**
 * E2E smoke tests for the packaged Clover desktop app.
 *
 ...
 * (credential stripping, isolated CLOVER_HOME + userData, unique app name).
 *
...
test('window opens with the Clover title', async () => {
...
test('HUD composer remains fully inside the transparent window', async () => {
  ...
  await fixture!.page.evaluate(() =>
    (window as typeof window & {
      cloverDesktop?: { hud?: { open: (options: { sessionId: null }) => Promise<void> } }
    }).cloverDesktop?.hud?.open({ sessionId: null })
  )
