...
function createGitRepo(root: string): string {
  ...
  execFileSync('git', ['config', 'user.name', 'Clover E2E'], { cwd: repo })
...
function configureRepoCwd(cloverHome: string, mockUrl: string, repo: string): void {
  writeMockProviderConfig(cloverHome, mockUrl)
  fs.appendFileSync(path.join(cloverHome, 'config.yaml'), `\nterminal:\n  cwd: ${repo}\n`, 'utf8')
  writeEnvFile(cloverHome)
}
...
test.beforeAll(async () => {
  ...
  configureRepoCwd(sandbox.cloverHome, mock.url, repo)
