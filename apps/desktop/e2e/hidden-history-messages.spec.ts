...
async function setupSeededMockBackend(): Promise<MockBackendFixture> {
  ...
  fs.appendFileSync(
    path.join(sandbox.cloverHome, 'config.yaml'),
    '\ncompression:\n  threshold_tokens: 1\n',
    'utf8',
  )
  writeEnvFile(sandbox.cloverHome)
  const builder = await RealSessionBuilder.start(sandbox.cloverHome)
  try {
...
test('live verify-on-stop continuations stay out of the transcript', async ({}, testInfo) => {
  ...
  writeMockProviderConfig(sandbox.cloverHome, mock.url)
  fs.appendFileSync(path.join(sandbox.cloverHome, 'config.yaml'), '\nagent:\n  verify_on_stop: true\n', 'utf8')
  writeEnvFile(sandbox.cloverHome)
  const { app, page } = await launchDesktop(buildAppEnv(sandbox))
