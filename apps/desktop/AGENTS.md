...
How to build Clover Desktop well. This is a judgment guide, not an inventory —
...
- The **backend** is authoritative for anything another Clover surface can also
...
- A **runtime home change** (switching the underlying `CLOVER_HOME` profile) is
...
its shape. "Plugin" means several unrelated things across Clover — do not assume
