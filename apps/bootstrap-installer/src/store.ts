...
export const $cloverHome = atom<string | null>(null)
...
export async function initialize(): Promise<void> {
  ...
  if (fake) {
    unlisten = () => {}
    $logPath.set('~/.clover/logs/bootstrap-installer.log')
    $cloverHome.set('~/.clover')
  ...
  try {
    const [logPath, cloverHome, mode] = await Promise.all([
      invoke<string>('get_log_path'),
      invoke<string>('get_clover_home'),
      invoke<AppMode>('get_mode')
    ])

    $logPath.set(logPath)
    $cloverHome.set(cloverHome)
    $mode.set(mode)
  } catch (err) {
  ...
  unlisten = await listen<BootstrapEvent>('bootstrap', (event) => {
    ...
    switch (payload.type) {
      ...
      case 'complete':
        ...
        // Install: show the "launch Clover" success screen. Update: this is a
...
export async function startUpdate(): Promise<void> {
  ...
  // Update is driven by the desktop handing off (Clover-Setup.exe --update);
...
export async function launchCloverDesktop(): Promise<void> {
  ...
  await invoke('launch_clover_desktop', { installRoot })
}
...
const FAKE_INSTALL_STAGES: FakeStage[] = [
  ...
  { name: 'repo', title: 'Clover repository' },
