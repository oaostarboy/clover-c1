...
export default function ProgressScreen({ bootstrap }: ProgressProps) {
  ...
  const title = bootstrap.status === 'completed' ? 'Done' : isUpdate ? 'Updating Clover' : 'Setting up Clover Cognition'

  const description = isUpdate
    ? 'Clover is updating to the latest version — this only takes a moment.'
    : 'This is a one-time setup. The Clover installer is downloading dependencies and configuring your machine. Subsequent launches will skip this step.'

  const pct = Math.round(progress.fraction * 100)

  return (
    <div className="clover-fade-in flex h-full flex-col">
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        {showLogs && (
          <div className="flex w-1/2 flex-col border-l border-(--stroke-clover)">
            <div className="flex shrink-0 items-center justify-between border-b border-(--stroke-clover) px-3 py-2 text-xs">
      ...
      <div className="flex shrink-0 items-center justify-between border-t border-(--stroke-clover) px-6 py-3">
        <button
