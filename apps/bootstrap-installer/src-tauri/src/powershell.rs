...
/// read side does not see EOF until the last of them closes it. `clover update`
...
/// stranded with it.
///
/// Same bound `Invoke-CloverStep` grew in `scripts/desktop-update/windows.ps1`
...
///
/// `clover_home_override` propagates to the child as $CLOVER_HOME so the
...
pub async fn run_script(
    ...
    if let Some(cwd) = stable_script_cwd(script_path, clover_home_override) {
        cmd.current_dir(cwd);
    }

    if let Some(home) = clover_home_override {
        cmd.env("CLOVER_HOME", home);
    }
...
fn stable_script_cwd<'a>(script_path: &'a Path, clover_home_override: Option<&'a str>) -> Option<&'a Path> {
    if let Some(home) = clover_home_override {
...
"#;
    ...
    #[test]
    fn stable_script_cwd_prefers_existing_clover_home() {
    ...
    /// child. `clover update` is this shape -- the Electron build alone is
    /// megabytes.
    #[cfg(unix)]
    #[tokio::test]
