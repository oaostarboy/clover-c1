...
//!
//! Mirrors `clover_constants.get_clover_home()` from the Python CLI:
//!   Windows: %LOCALAPPDATA%\clover
//!   macOS:   ~/.clover
//!   Linux:   ~/.clover  (override via $CLOVER_HOME)
//!
//! NOTE (macOS): Python's get_clover_home(), scripts/install.sh, and the
//! Electron desktop's resolveCloverHome() ALL use ~/.clover on macOS — there
...
/// Returns the canonical Clover home directory, respecting $CLOVER_HOME if set.
pub fn clover_home() -> PathBuf {
    if let Ok(override_path) = std::env::var("CLOVER_HOME") {
    ...
    {
        // %LOCALAPPDATA%\clover — matches scripts/install.ps1's $CloverHome.
        if let Some(local_app_data) = dirs::data_local_dir() {
            return local_app_data.join("clover");
        }
    }

    // macOS + Linux + fallback: ~/.clover (matches Python get_clover_home(),
    // install.sh, and the Electron desktop's resolveCloverHome()).
    if let Some(home) = dirs::home_dir() {
        return home.join(".clover");
    }
...
pub fn bootstrap_cache_dir() -> PathBuf {
    clover_home().join("bootstrap-cache")
}
...
/// CLOVER_HOME so it survives repo checkout deletion (unlike anything under
/// clover-c1/).
///
/// On Windows this is `%LOCALAPPDATA%\clover\clover-setup.exe`; on other
...
/// mid-update re-locks the venv shim and triggers `force_kill_other_clover`,
/// which then kills that legitimate backend in a respawn loop (#50238).
///
/// Lives directly under CLOVER_HOME (same rationale as `installer_dest`) so the
/// Electron desktop — which resolves CLOVER_HOME identically and pins it into
/// the updater's env — agrees on the exact path.
pub fn update_in_progress_marker() -> PathBuf {
    clover_home().join(".clover-update-in-progress")
}
...
pub fn copy_self_to_clover_home() -> std::io::Result<()> {
    ...
    tracing::info!(?src, ?dest, "copied installer to CLOVER_HOME");
    Ok(())
}
...
/// We don't always know ACTIVE_CLOVER_ROOT until install.ps1 reports it, so
/// this is a probe helper, not a definitive path.
pub fn likely_bootstrap_marker(install_root: &Path) -> PathBuf {
    install_root.join(".clover-bootstrap-complete")
}

/// Initializes tracing to bootstrap-installer.log under CLOVER_HOME/logs/.
...
pub fn init_logging() -> Option<WorkerGuard> {
    ...
    if let Err(err) = std::fs::create_dir_all(&dir) {
        ...
        eprintln!("[clover-setup] could not create log dir {dir:?}: {err}");
        return None;
    }
    ...
    let env_filter = tracing_subscriber::EnvFilter::try_from_env("CLOVER_BOOTSTRAP_LOG")
...
#[tauri::command]
pub fn get_clover_home() -> String {
    clover_home().to_string_lossy().into_owned()
}
