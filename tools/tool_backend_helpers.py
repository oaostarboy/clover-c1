...
def resolve_provider_secret(
    ...
    Resolution order (fixes #68003 — keys added via ``clover auth add
    ...
    2. The environment / ``~/.clover/.env``. Under a multiplexed gateway turn
       ...
       ``clover_cli.config.get_env_value`` (os.environ, then ``.env``),
       matching the tools' historical behaviour exactly.
    3. The credential pool / auth store for ``provider_id`` (``clover auth
    ...
    when omitted, ``clover_cli.config.get_env_value`` is used directly.
    """
    ...
    try:
        ...
        # `clover auth add <provider>` keys a registry provider by its plain
...
def resolve_openai_audio_api_key() -> str:
    ...
    the credential pool (``clover auth add openai-api``) via
...
def selection_error(section: str, selection_name: str, failure: str) -> str:
    ...
    return (
        f"{section} is configured to use {selection_name} (set via clover "
        f"tools), but {failure}. Run 'clover tools' to change it."
    )
...
def fal_key_is_configured() -> bool:
    ...
    Consults both ``os.environ`` and ``~/.clover/.env`` (via
    ``clover_cli.config.get_env_value`` when available) so tool-side
