...
def _terminal_env_type_for_task(task_id: str = "default") -> str:
    ...
    try:
        ...
        if env is not None:
            ...
            if "daytona" in name:
                return "daytona"
            stamped = getattr(env, "_clover_backend_name", None)
...
def _check_cross_profile_path(filepath: str, task_id: str = "default") -> str | None:
    ...
    container's sandbox mirror of Clover state.

    Two detectors (both #32049): these catch writes that would be
    SILENTLY LOST — the host Clover process never reads the mirror, so
...
def _is_openai_family_main() -> bool:
    ...
    (openrouter/clover/azure...) the MODEL slug decides (gpt-*/o-series/
