...
def _expand_tilde(path: str) -> str:
    ...
    mirrors ``clover_constants.get_subprocess_home()`` so that ``~`` resolves
    ...
    if not path or "~" not in path:
        return path
    try:
        from clover_constants import get_subprocess_home
...
def _truncate_to_char_budget(content: str, max_chars: int) -> tuple[str, int, bool]:
    ...
    ``read_file``). Where clover previously hard-rejected an oversized read
...
def _file_ops_uses_host_paths(file_ops) -> bool:
    """Return True when *file_ops* targets the same host filesystem as Clover.
...
_clover_config_resolved: str | None = None
_clover_config_resolved_loaded = False


def _get_clover_config_resolved() -> str | None:
    """Return the resolved absolute path of the Clover config file (cached)."""
    global _clover_config_resolved, _clover_config_resolved_loaded
    if _clover_config_resolved_loaded:
        return _clover_config_resolved
    _clover_config_resolved_loaded = True
    try:
        from clover_cli.config import get_config_path
        _clover_config_resolved = str(get_config_path().resolve())
    except Exception:
        try:
            _clover_config_resolved = str(Path(_expand_tilde("~/.clover/config.yaml")).resolve())
        except Exception:
            _clover_config_resolved = None
    return _clover_config_resolved
...
def _check_sensitive_path(filepath: str, task_id: str = "default") -> str | None:
    ...
    if resolved in _SENSITIVE_EXACT_PATHS or normalized in _SENSITIVE_EXACT_PATHS:
        return _err
    # Prevent agents from modifying the Clover config file directly.
    # approvals.mode and other security settings live here; a malicious or
    # prompt-injected agent could silently disable exec approval by writing to
    # this file.
    clover_config = _get_clover_config_resolved()
    if clover_config and (resolved == clover_config or normalized == clover_config):
        return (
            f"Refusing to write to Clover config file: {filepath}\n"
            "Agent cannot modify security-sensitive configuration. "
            "Edit ~/.clover/config.yaml directly or use 'clover config' instead."
        )
    return None
...
# .cursorrules (or a project-local .clover config tree) outlives the current
...
_real_clover_home_cached: str | None = None
_real_clover_home_loaded = False


def _get_real_clover_home() -> str | None:
    """Return the realpath of the authoritative Clover home (cached)."""
    global _real_clover_home_cached, _real_clover_home_loaded
    if _real_clover_home_loaded:
        return _real_clover_home_cached
    _real_clover_home_loaded = True
    try:
        from clover_constants import get_clover_home
        _real_clover_home_cached = os.path.realpath(str(get_clover_home()))
    except Exception:
        try:
            _real_clover_home_cached = os.path.realpath(_expand_tilde("~/.clover"))
        except Exception:
            _real_clover_home_cached = None
    return _real_clover_home_cached
...
def _protected_instruction_config() -> tuple[bool, list[str]]:
    ...
    """
    try:
        from clover_cli.config import load_config, cfg_get
        cfg = load_config()
...
def _protected_instruction_reason(filepath: str, task_id: str = "default",
    ...
    # The authoritative ~/.clover home is governed by its own guards
    ...
    # ``.clover`` component rule below, which would otherwise match the
    # home directory itself.
    real_home = _get_real_clover_home()
    ...
    for candidate in (normalized, resolved):
        ...
        for pattern in extra_patterns:
            if fnmatch.fnmatch(base_lower, pattern.lower()):
                return base
        # Project-local .clover config dirs (e.g. <repo>/.clover/config.yaml)
        # are loaded as project context and steer behavior the same way.
        # Scope: the file's IMMEDIATE parent must be ``.clover`` — matching
        # any ancestor named .clover would gate every write inside a
        # checkout that happens to live under ~/.clover (e.g. the
        # clover-c1 repo itself at ~/.clover/clover-c1).
        parts = candidate.replace("\\", "/").rstrip("/").split("/")
        if len(parts) >= 2 and parts[-2] == ".clover":
            return candidate
    return None
...
def _get_container_mirror_prefix_for_task(task_id: str = "default") -> str | None:
    """Return the container-side Clover mirror prefix for Docker file tools."""
    try:
        from tools.terminal_tool import (
            _active_environments,
            _env_lock,
...
def _check_cross_profile_path(filepath: str, task_id: str = "default") -> str | None:
    """Return a soft-guard warning when ``filepath`` lands in another Clover
    profile's scoped area, a host-side sandbox-mirror of authoritative profile
    state, or the Docker container's sandbox mirror of Clover state.
    ...
    * sandbox-mirror (#32049) — writes that hit the
      ``…/sandboxes/<backend>/<task>/home/.clover/…`` mirror created by a
      non-local terminal backend (Docker, Daytona, etc.), where the host
      Clover process never reads the mirror and the authoritative file is
      left untouched.
    * container-mirror (#32049 follow-up) — writes from inside a Docker
      ...
      the agent sees a plain ``/root/.clover/…`` path.

    Returns ``None`` when the write is in-scope or outside Clover scope.
    ...
    # in a session that cd'd into ``~/.clover/profiles/other/`` is
...
def read_file_tool(path: str, offset: int = 1, limit: int = 2000, task_id: str = "default") -> str:
    ...
    try:
        ...
        # ── Clover internal path guard ────────────────────────────────
        # Prevent prompt injection via catalog or hub metadata files,
        # and block credential stores under CLOVER_HOME.  Pass the
        # already-resolved path so a relative-path read against
        # TERMINAL_CWD == CLOVER_HOME (e.g. "auth.json") still hits the
...
def write_file_tool(path: str, content: str, task_id: str = "default",
    ...
    ``cross_profile`` opts out of the soft cross-Clover-profile guard. The
...
def patch_tool(mode: str = "replace", path: str = None, old_string: str = None,
    ...
    ``cross_profile`` opts out of the soft cross-Clover-profile guard for
...
WRITE_FILE_SCHEMA = {
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "cross_profile": {
                "type": "boolean",
                "description": "Opt out of the cross-profile soft guard. Defaults to false. Set true ONLY after explicit user direction to edit another Clover profile's skills/plugins/cron/memories — by default these writes are blocked with a warning because they affect a different profile than the one this session is running under.",
                "default": False,
            },
        },
...
PATCH_SCHEMA = {
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "cross_profile": {
                "type": "boolean",
                "description": "Opt out of the cross-profile soft guard. Defaults to false. Set true ONLY after explicit user direction to edit another Clover profile's skills/plugins/cron/memories.",
                "default": False,
            },
        },
        "required": ["mode"],
...
def _handle_write_file(args, **kw):
    ...
    if "content" not in args:
        return tool_error(
            ...
            "payload, or use execute_code with clover_tools.write_file() for very "
            "large files."
        )
