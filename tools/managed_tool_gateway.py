"""Generic managed-tool gateway helpers for Clover-hosted vendor passthroughs."""
...
def auth_json_path():
    """Return the Clover auth store path, respecting CLOVER_HOME overrides."""
...
def read_clover_access_token() -> Optional[str]:
    ...
    try:
        ...
        logger.debug("Clover access token refresh failed: %s", exc)
...
# managed vendor's gateway URL is pinned: adding one is a Clover release, and
...
#
# The gateway exposes a Clover-owned REST contract per vendor; it names the
...
def is_managed_clover_gateway_url(
    ...
    """True when ``url`` is on the Clover tool-gateway origin this client builds.
...
def managed_gateway_auth_headers(
    ...
    Read fresh on every call rather than cached: a Clover access token expires
...
def build_managed_media_uploader(
    ...
    Clover URL, or no ``upload_path``); callers then refuse local paths with a
    ...
    async def upload(data: bytes, mime: str) -> str:
        ...
        if not headers:
            raise RuntimeError("no Clover credential is available for the upload")
