"""
Cron job management tools for Clover Cognition.
...
# uses 30s) — comfortably below the 1800s default CLOVER_AGENT_TIMEOUT.
_CRON_RUN_HEARTBEAT_INTERVAL = 10.0

# Hard ceiling on how long the heartbeat keeps the parent watchdog at bay.
# The child cron run has its own inactivity watchdog (CLOVER_CRON_TIMEOUT,
# default 600s) that bounds a wedged job, but with CLOVER_CRON_TIMEOUT=0
...
#      saying "cat ~/.clover/.env" or "rm -rf /". `_scan_cron_prompt()` runs
...
#      `clover-c1-dev` skill contains a security postmortem mentioning
#      `cat ~/.clover/.env`, which tripped `read_secrets` and silently
...
def _origin_from_env() -> Optional[Dict[str, str]]:
    ...
    origin_platform = get_session_env("CLOVER_SESSION_PLATFORM")
    origin_chat_id = get_session_env("CLOVER_SESSION_CHAT_ID")
    if origin_platform and origin_chat_id:
        ...
        if thread_id and origin_platform == "slack":
            message_id = get_session_env("CLOVER_SESSION_MESSAGE_ID") or None
        ...
        return {
            ...
            "chat_name": get_session_env("CLOVER_SESSION_CHAT_NAME") or None,
            "thread_id": thread_id,
            ...
            # send_message, which passes CLOVER_SESSION_USER_ID to
            ...
            "scope_id": get_session_env("CLOVER_SESSION_SCOPE_ID") or None,
        }
    return None
...
def _local_delivery_notice(job: Dict[str, Any], user_deliver: Optional[str]) -> Optional[str]:
    ...
    ``CLOVER_SESSION_PLATFORM``/``CHAT_ID`` is set for them), so a
    ...
    for part in str(deliver).split(","):
        ...
        if not profile_exists(canon):
            return (
                ...
                "profile that exists here (clover profile list), or omit the "
...
def _resolve_cron_context_deliver(deliver: Optional[str]) -> Optional[str]:
    ...
    delivery target — the ``CLOVER_CRON_AUTO_DELIVER_*`` contextvars that
    ...
    * Every other element (``local``, ``all``, explicit ``platform:...``)
    ...
    if not is_truthy_value(get_session_env("CLOVER_CRON_SESSION", "")):
        return deliver

    def _creator_target() -> str:
        platform = get_session_env("CLOVER_CRON_AUTO_DELIVER_PLATFORM", "").strip()
        chat_id = get_session_env("CLOVER_CRON_AUTO_DELIVER_CHAT_ID", "").strip()
        if not platform or not chat_id:
            return "local"
        thread_id = get_session_env("CLOVER_CRON_AUTO_DELIVER_THREAD_ID", "").strip()
        if thread_id:
...
def _validate_cron_script_path(script: Optional[str]) -> Optional[str]:
    ...
    Scripts must be relative paths that resolve within CLOVER_HOME/scripts/.
    ...
    # Only relative paths within ~/.clover/scripts/ are allowed.
    if raw.startswith(("/", "~")) or (len(raw) >= 2 and raw[1] == ":"):
        return (
            f"Script path must be relative to ~/.clover/scripts/. "
            f"Got absolute or home-relative path: {raw!r}. "
            f"Place scripts in ~/.clover/scripts/ and use just the filename."
        )

    # Validate containment after resolution
    from tools.path_security import validate_within_dir

    scripts_dir = get_clover_home() / "scripts"
...
def _latest_job_output_excerpt(job_id: str, max_chars: int = 2000) -> Optional[str]:
    ...
    ``~/.clover/cron/output/``. Never raises.
    """
    try:
...
def _try_dispatch_background_run(
    ...
    None
        ...
        ``clover -z``, stateless HTTP, Kanban worker, nested cron run).
    ...
    # 'running' by a dead owner process -- e.g. a PRIOR one-shot `clover
    ...
    if not session_key:
        # Direct Python callers (`clover cron run`, tests) have no agent
    ...
    try:
        ...
        origin_ui_session_id = get_session_env("CLOVER_UI_SESSION_ID", "") or ""
    except Exception:
        pass
        ...
        if normalized == "create":
            ...
            try:
                job = create_job_with_scheduler_registration(
                    ...
                    # (clover cron create --reasoning-effort) ONLY — it is
...
Scheduling from cron-run sessions is disabled by default and enabled via cron.allow_agent_scheduling in config.yaml. When enabled, jobs created from a cron run are user-owned in the same flat job table as every other job, and their delivery resolves to the creating job's own persistent target — never to the ephemeral cron-run session. Prefer updating an existing job (list first, then update by job_id) over creating near-duplicates.""",
    "parameters": {
        ...
        "properties": {
            ...
            "script": {
                "type": "string",
                "description": f"Optional path to a script that runs each tick. In the default mode its stdout is injected into the agent's prompt as context (data-collection / change-detection pattern). With no_agent=True, the script IS the job and its stdout is delivered verbatim (classic watchdog pattern). Relative paths resolve under {display_clover_home()}/scripts/. ``.sh``/``.bash`` extensions run via bash, everything else via Python. On update, pass empty string to clear."
            },
            "monitor_script": {
                "type": "string",
                "description": f"Optional monitor-mode source script (same rules as `script`: relative to {display_clover_home()}/scripts/, .sh/.bash via bash, else Python). Each tick it runs FIRST and its output is hashed as exact bytes: UNCHANGED output suppresses the agent run entirely (no LLM, no delivery, recorded as a silent no_change tick); CHANGED output injects a MONITOR CHANGE DETECTED block (unified diff + new output) into the prompt before a normal agent run. The first tick always runs the agent (baseline). Scripts must emit STABLE output — no timestamps or random ordering — or every tick looks changed. Mutually exclusive with monitor_url; incompatible with no_agent=True. On update, pass empty string to clear."
            },
            "monitor_url": {
                "type": "string",
...
def check_cronjob_requirements() -> bool:
    ...
    return (
        env_var_enabled("CLOVER_INTERACTIVE")
        or env_var_enabled("CLOVER_GATEWAY_SESSION")
        or env_var_enabled("CLOVER_EXEC_ASK")
    )
...
registry.register(
    ...
    handler=lambda args, **kw: cronjob(
        ...
        # `clover cron create/edit --model`, or hand-edited jobs). The agent
