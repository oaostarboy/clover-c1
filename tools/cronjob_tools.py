...
def _gateway_liveness_notice(plural: bool = False) -> dict:
    ...
    Thin adapter over the shared CLI helper ``clover_cli.cron._builtin_gateway_liveness``
    ...
    """
    try:
        from clover_cli.cron import _builtin_gateway_liveness
    ...
    if _gw is False:
        return {
            ...
            "warning": (
                f"The Clover gateway is not running — {subject} "
                "but will NOT fire until the gateway is started "
                "(clover gateway install / clover gateway start). "
...
Jobs run in a fresh session with no current-chat context, so prompts must be self-contained, and the agent's FINAL RESPONSE is what gets delivered — cron runs are autonomous and cannot ask questions. Prefer updating an existing job over creating near-duplicates.""",
    "parameters": {
        ...
        "properties": {
            ...
            "script": {
                "type": "string",
                "description": f"Optional script run each tick; stdout is injected into the agent's prompt as context (with no_agent=True the script IS the job). Relative paths resolve under {display_clover_home()}/scripts/; .sh/.bash via bash, else Python. On update, '' clears."
            },
            "monitor": {
                "type": "string",
