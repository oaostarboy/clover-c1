...
Design contract (matches the Clover "no lazy-reading escape hatches / don't
...
def _check_metadata_block(frontmatter: Dict[str, Any]) -> List[LintFinding]:
    ...
    clover_meta = meta.get("clover") if isinstance(meta, dict) else None
    if not isinstance(clover_meta, dict):
        findings.append(
            LintFinding(
                WARNING,
                "missing-metadata",
                "frontmatter is missing metadata.clover.{tags, related_skills}.",
            )
        )
    else:
        if "tags" not in clover_meta:
            findings.append(
                LintFinding(
                    WARNING, "missing-metadata", "metadata.clover.tags is missing."
                )
            )
    author = str(frontmatter.get("author", ""))
    if author and author.strip().lower() in ("clover", "agent", "clover agent") and (
        ...
        findings.append(
            LintFinding(
                WARNING,
                "author-caps",
                f"author '{author}' should be 'Clover Cognition' (proper caps) "
