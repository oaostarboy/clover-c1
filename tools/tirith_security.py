...
it is automatically downloaded from GitHub releases to $CLOVER_HOME/bin/tirith.
...
def _get_clover_home() -> str:
    """Return the Clover home directory, respecting CLOVER_HOME env var."""
    return str(get_clover_home())


def _failure_marker_path() -> str:
    """Return the path to the install-failure marker file."""
    return os.path.join(_get_clover_home(), ".tirith-install-failed")
...
def _clover_bin_dir() -> str:
    """Return $CLOVER_HOME/bin, creating it if needed."""
    d = os.path.join(_get_clover_home(), "bin")
...
def _install_tirith(*, log_failures: bool = True) -> tuple[str | None, str]:
    """Download and install tirith to $CLOVER_HOME/bin/tirith.
    ...
    try:
        ...
        dest = os.path.join(_clover_bin_dir(), "tirith")
        try:
...
def _resolve_tirith_path(configured_path: str) -> str:
    ...
    2. $CLOVER_HOME/bin/tirith (previously auto-installed)
    3. Auto-install from GitHub releases → $CLOVER_HOME/bin/tirith
    ...
    clover_bin = os.path.join(_clover_bin_dir(), "tirith")
    if os.path.isfile(clover_bin) and os.access(clover_bin, os.X_OK):
        _resolved_path = clover_bin
        _install_failure_reason = ""
        _clear_install_failed()
        return clover_bin
