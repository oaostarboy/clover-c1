...
``~/.clover/skills/.curator_ledger.jsonl`` describing who changed what, with
...
Design decisions (the maintainer-approved):
  ...
  - The ledger covers ALL actors, tagged ``curator`` / ``agent`` / ``user``.
    ...
    ``clover curator rollback <entry-id>``.
...
def ledger_path() -> Path:
    return get_clover_home() / "skills" / ".curator_ledger.jsonl"


def blobs_dir() -> Path:
    return get_clover_home() / ".curator_backups" / "blobs"
...
def _validate_entry_paths(entry: Dict[str, Any]) -> Optional[str]:
    """All paths in an entry must live under CLOVER_HOME. Defense in depth —
