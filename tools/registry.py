"""Central registry for all clover-c1 tools.
...
def _discovery_cache_path() -> Optional[Path]:
    ...
    try:
        ...
        # import time (clover_constants itself is stdlib-only, so no cycle).
        from clover_constants import get_clover_home

        return Path(get_clover_home()) / "cache" / "tool_discovery_cache.json"
    except Exception:
        return None
...
# timescales. Cache results for ~30 s so env-var flips via ``clover tools``
...
def check_fn_cache_scope() -> Optional[str]:
    ...
    cache. A multiplex gateway installs a Clover-home override for every
    ...
    try:
        ...
        browser_identity = (
            get_session_env("CLOVER_SESSION_ID", ""),
...
def invalidate_check_fn_cache() -> None:
    ...
    affect tool availability (e.g. ``clover tools enable``)."""
    with _check_fn_cache_lock:
...
class ToolRegistry:
    ...
    def __init__(self):
        ...
        # Plugin registrations are overlays keyed by resolved CLOVER_HOME. A
    ...
    def _plugin_namespace_of_module(
        ...
        if module_namespace.startswith("clover_plugins."):
    ...
    def deregister(self, name: str) -> None:
        ...
        with self._lock:
            ...
            if not entry.toolset.startswith("mcp-"):
                ...
                # (``clover_plugins.{name}``), not the exact module string.
                # A handler defined in ``clover_plugins.pkg.handlers`` is
                # still owned by the ``clover_plugins.pkg`` package — exact
    ...
    def get_definitions(self, tool_names: Set[str], quiet: bool = False) -> List[dict]:
        ...
        etc.); TTL chosen so env-var changes (``clover tools enable foo``)
