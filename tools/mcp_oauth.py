...
URL of Clover' published Client ID Metadata Document (CIMD) as the
...
This module provides the glue:
    - ``CloverTokenStorage``: persists tokens/client-info to disk so they
    ...
    mcp_servers:
      my_server:
        ...
        oauth:                                  # all fields optional
          ...
          client_name: "My Custom Client"       # default: "Clover Cognition"
...
from clover_constants import secure_parent_dir
...
_USER_SKIPPED_SENTINEL = "__clover_user_skipped__"
...
def _get_token_dir(clover_home: str | Path | None = None) -> Path:
    """Return the directory for MCP OAuth token files.

    Uses CLOVER_HOME so each profile gets its own OAuth tokens.
    Layout: ``CLOVER_HOME/mcp-tokens/``
    """
    from clover_constants import get_clover_home

    base = Path(clover_home) if clover_home is not None else Path(get_clover_home())
...
def _cached_redirect_port(storage: "CloverTokenStorage | None") -> int | None:
    ...
    redirect URI that was registered with it. If Clover restarts and chooses a
...
def _cached_redirect_uri(storage: "CloverTokenStorage | None") -> str | None:
...
def _raise_if_non_interactive(lead: str) -> None:
    ...
    shared, actionable ``clover mcp login`` next-step so the guidance wording
    lives in one place across every non-interactive OAuth boundary (#57836).
    """
    if not _is_interactive():
        raise OAuthNonInteractiveError(
            f"{lead} "
            "Run `clover mcp login <server>` interactively to (re)authorize, "
...
# ---------------------------------------------------------------------------
# CloverTokenStorage -- persistent token/client-info on disk
# ---------------------------------------------------------------------------


class CloverTokenStorage:
    """Persist OAuth tokens and client registration to JSON files.

    File layout::

        CLOVER_HOME/mcp-tokens/<server_name>.json         -- tokens
        CLOVER_HOME/mcp-tokens/<server_name>.client.json   -- client info
        CLOVER_HOME/mcp-tokens/<server_name>.meta.json     -- oauth server metadata
        CLOVER_HOME/mcp-tokens/<server_name>.cimd-off      -- CIMD refused here
    """

    def __init__(self, server_name: str, *, clover_home: str | Path | None = None):
        self._server_name = _safe_filename(server_name)
        self._clover_home = Path(clover_home) if clover_home is not None else None

    def _tokens_path(self) -> Path:
        return _get_token_dir(self._clover_home) / f"{self._server_name}.json"

    def _client_info_path(self) -> Path:
        return _get_token_dir(self._clover_home) / f"{self._server_name}.client.json"

    def _meta_path(self) -> Path:
        return _get_token_dir(self._clover_home) / f"{self._server_name}.meta.json"

    def _cimd_rejected_path(self) -> Path:
        return _get_token_dir(self._clover_home) / f"{self._server_name}.cimd-off"
    ...
    async def get_tokens(self) -> "OAuthToken | None":
        ...
        if OAuthToken is None and not _ensure_sdk_loaded():
            return None
        # Clover records an absolute wall-clock ``expires_at`` alongside the
    ...
    def mark_cimd_rejected(self) -> None:
        ...
        refused. Cleared by ``remove()``, i.e. by ``clover mcp login`` /
        ``clover mcp remove``, so a fixed document gets another chance.
        """
    ...
    def restore(self, snapshot: dict[str, bytes], *, only_if_absent: bool = False) -> None:
        ...
        if not snapshot:
            return
        token_dir = _get_token_dir(self._clover_home)
    ...
    class _Handler(BaseHTTPRequestHandler):
        def do_GET(self) -> None:  # noqa: N802
            ...
            body = (
                ...
                "<p>You can close this tab and return to Clover.</p></body></html>"
            ) if code else (
        ...
        if result["auth_code"] is None:
            ...
            if cimd_url:
                hint = (
                    ...
                    f"Clover' Client ID Metadata Document ({cimd_url}); set "
...
def _paste_callback_reader(result: dict) -> None:
    ...
    if line.lower() in _SKIP_TOKENS:
        ...
        print(
            "  OAuth skipped. Run `clover mcp login <server>` later to "
...
CloverOAuthClientProvider: Any = None


def _get_clover_oauth_provider_class() -> type | None:
    global CloverOAuthClientProvider
    if CloverOAuthClientProvider is not None:
        return CloverOAuthClientProvider
    if not _ensure_sdk_loaded():
        return None

    class _CloverOAuthClientProvider(OAuthClientProvider):
        ...
        def __init__(self, *args: Any, token_user_agent: "str | None" = None, **kwargs: Any):
            ...
            self._clover_token_user_agent = token_user_agent

        def _stamp_token_user_agent(self, request):
            ua = getattr(self, "_clover_token_user_agent", None)
            if ua:
    ...
    _CloverOAuthClientProvider.__name__ = "CloverOAuthClientProvider"
    _CloverOAuthClientProvider.__qualname__ = "CloverOAuthClientProvider"
    CloverOAuthClientProvider = _CloverOAuthClientProvider
    return CloverOAuthClientProvider
...
def remove_oauth_tokens(
    ...
    *,
    ...
    storage = CloverTokenStorage(server_name, clover_home=clover_home)
    storage.remove()
...
# 2026-07-28. The SDK does the protocol work; Clover only decides whether a
...
# (section 4.2), so a CIMD flow cannot use the ephemeral port Clover picks
...
def _has_cached_client_info(storage: "CloverTokenStorage | None") -> bool:
...
def _server_declined_cimd(storage: "CloverTokenStorage | None") -> bool:
    ...
    Clover has to fix the redirect URI. Cached authorization-server metadata
...
def _maybe_use_cimd(
    cfg: dict,
    storage: "CloverTokenStorage | None" = None,
) -> "tuple[str, int] | None":
    """Return ``(client_id URL, pinned callback port)``, or None to use DCR.

    Every early return below is a case where the redirect URI Clover would
...
#   "Claude Code" → 200
#   "Codex"       → 200
#   "Clover Cognition" / "Clover" / "Cursor" / "VS Code" / … → 403
...
def _build_client_metadata(cfg: dict) -> "OAuthClientMetadata":
    ...
    if OAuthClientMetadata is None:
        _ensure_sdk_loaded()
    client_name = cfg.get("client_name", "Clover Cognition")
    scope = cfg.get("scope")
    ...
    metadata_kwargs: dict[str, Any] = {
        ...
        # rejecting loopback redirect_uris. Clover is a CLI/desktop app
...
def _invalidate_tokens_on_client_change(
    ...
    if removed:
        logger.warning(
            ...
            "Re-authorize with: clover mcp login %s",
...
def humanize_oauth_registration_error(
    ...
    *,
    ...
    Figma's remote MCP gates DCR on exact ``client_name``. Clover auto-sets
    ``Claude Code`` (known-good); this message fires when the user overrode
    that with something Figma still rejects, or an older Clover is running.
    """
    msg = str(exc)
    lowered = msg.lower()
...
def build_oauth_auth(
    ...
    storage = CloverTokenStorage(server_name)
    ...
    if not _is_interactive() and not storage.has_cached_tokens():
        raise OAuthNonInteractiveError(
            ...
            f"`clover mcp login {server_name}` interactively first to complete "
    ...
    provider_class = _get_clover_oauth_provider_class()
