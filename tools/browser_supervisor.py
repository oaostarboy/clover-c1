...
One ``CDPSupervisor`` runs per Clover ``task_id`` that has a reachable CDP
...
def _redact_cdp_error_text(exc: object) -> str:
    ...
    those credentials never reach Clover logs or tracebacks. Falls back to a
...
DIALOG_BRIDGE_HOST = "clover-dialog-bridge.invalid"
...
(() => {
  if (window.__cloverDialogBridgeInstalled) return;
  window.__cloverDialogBridgeInstalled = true;
  const ENDPOINT = "http://clover-dialog-bridge.invalid/";
