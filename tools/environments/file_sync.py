...
_FORCE_SYNC_ENV = "CLOVER_FORCE_FILE_SYNC"
...
def iter_sync_files(container_base: str = "/root/.clover") -> list[tuple[str, str]]:
    ...
    the hardcoded /root/.clover to *container_base* because the remote
...
class FileSyncManager:
    ...
    def sync(self, *, force: bool = False) -> None:
        ...
        or ``CLOVER_FORCE_FILE_SYNC=1`` is set.
    ...
    def sync_back(self, clover_home: Path | None = None) -> None:
        """Pull remote changes back to the host filesystem.

        Downloads the remote ``.clover/`` directory as a tar archive,
        ...
        """
        with self._transaction_lock:
            self._sync_back_transaction(clover_home=clover_home)

    def _sync_back_transaction(self, clover_home: Path | None = None) -> None:
    ...
    def _sync_back_impl(self) -> None:
        ...
        with tempfile.NamedTemporaryFile(suffix=".tar") as tf:
            ...
            with tempfile.TemporaryDirectory(prefix="clover-sync-back-") as staging:
    ...
    def _infer_host_path(self, remote_path: str,
        ...
        For example, if the mapping has ``/root/.clover/skills/a.md`` →
        ``~/.clover/skills/a.md``, a new remote file at
        ``/root/.clover/skills/b.md`` maps to ``~/.clover/skills/b.md``.
        """
        mapping = file_mapping if file_mapping is not None else []
