...
def _block_message(operation: str, root: Path) -> str:
    ...
    return (
        f"Blocked: `{operation}` would rewrite Clover's live source checkout "
        ...
        "checkout, stop Clover, run the command externally, then restart "
        "Clover."
    )
...
def _scratch_dir_hint() -> str:
    ...
    base = Path(clover_home).expanduser() if clover_home else Path.home() / ".clover"
