...
def _is_clover_provider_credential(name: str) -> bool:
    """True if ``name`` is a Clover-managed provider credential (API key,
    token, or similar) per ``_CLOVER_PROVIDER_ENV_BLOCKLIST``.
    ...
    Non-Clover API keys (TENOR_API_KEY, NOTION_TOKEN, etc.) are NOT
    ...
    let a skill tunnel a Clover credential into the execute_code child.
    """
    try:
        ...
        return True
    # Dynamically-generated Clover-internal secrets (AUXILIARY_*_API_KEY /
    ...
    if _is_clover_internal_secret(name):
        return True
    return name in _CLOVER_PROVIDER_ENV_BLOCKLIST
...
def register_env_passthrough(var_names: Iterable[str]) -> None:
    ...
    Variables that are Clover-managed provider credentials (from
    ``_CLOVER_PROVIDER_ENV_BLOCKLIST``) are rejected here to preserve
    the ``execute_code`` sandbox's credential-scrubbing guarantee per
    GHSA-rhgp-j443-p4rf. A skill that needs to talk to a Clover-managed
    ...
    Non-Clover third-party API keys (TENOR_API_KEY, NOTION_TOKEN, etc.)
    ...
    for name in var_names:
        ...
        if not name:
            continue
        if _is_clover_provider_credential(name):
            logger.warning(
                "env passthrough: refusing to register Clover provider "
                "credential %r (blocked by _CLOVER_PROVIDER_ENV_BLOCKLIST). "
    ...
    try:
        ...
        if isinstance(passthrough, list):
            for item in passthrough:
                ...
                # Clover-managed provider credentials must not be passed
                ...
                if _is_clover_provider_credential(name):
                    logger.warning(
                        "env passthrough: refusing to register Clover "
                        "provider credential %r from config.yaml (blocked "
                        "by _CLOVER_PROVIDER_ENV_BLOCKLIST). Operator "
