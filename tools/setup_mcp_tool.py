...
``clover mcp install <name>`` in the terminal.
"""
...
def setup_mcp_tool(
    ...
    if callback is None:
        return tool_error(
            "setup_mcp is only available in the Clover desktop app. Use the "
            "terminal instead: `clover mcp install <name>` for catalog entries, "
            "`clover mcp login <name>` for OAuth."
        )
...
SETUP_MCP_SCHEMA = {
    ...
    "description": (
        ...
        "Clover desktop chat. The card lets them install a catalog entry, "
        ...
        "continue without the server. Catalog names: run `clover mcp catalog` "
