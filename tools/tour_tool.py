#!/usr/bin/env python3
"""Run a guided tour (highlight + narrate UI elements) in the Clover desktop GUI.
...
- ``surface="app"`` — the Clover desktop app's own DOM (tours of Clover itself).
...
def tour_tool(
    ...
    if callback is None:
        return tool_error("tour is only available in the Clover desktop app.")
...
TOUR_SCHEMA = {
    "name": "tour",
    "description": (
        "Give a live guided tour in the Clover desktop GUI: dim the screen, "
        "highlight an element, and attach a popover with your own title/text. "
        "Works on two surfaces — 'app' (the Clover app itself) and 'preview' "
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "surface": {
                "type": "string",
                "enum": list(SURFACES),
                "description": "Where the tour runs: 'app' (Clover desktop UI, default) or 'preview' (the page in the in-app browser pane).",
            },
            "selector": {
                "type": "string",
