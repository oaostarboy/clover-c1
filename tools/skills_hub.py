#!/usr/bin/env python3
"""
Skills Hub — Source adapters and hub state management for the Clover Skills Hub.
...
Used by clover_cli/skills_hub.py for CLI commands and the /skills slash command.
"""
...
def _skills_dir() -> Path:
    ...
    return Path(forced) if forced is not None else _clover_home() / "skills"
...
class GitHubSource(SkillSource):
    ...
    def inspect(self, identifier: str) -> Optional[SkillMeta]:
        ...
        if isinstance(metadata, dict):
            clover_meta = metadata.get("clover", {})
            if isinstance(clover_meta, dict):
                tags = clover_meta.get("tags", [])
        if not tags:
...
class OptionalSkillSource(SkillSource):
    ...
    These skills are official (maintained by Anthony Nguyen) but not activated
    ...
    def fetch(self, identifier: str) -> Optional[SkillBundle]:
        ...
        # upstream project maintains (e.g. impeccable's Clover-native bundle
        # under pbakaus/impeccable:.clover/skills/impeccable). Install pulls
    ...
    def _fetch_from_live_repo(self, rel: str) -> Optional[SkillBundle]:
        ...
        ``clover update``. Rather than telling them to update first, resolve
    ...
    def _upstream_pointer(self, skill_dir: Path) -> Optional[Dict[str, str]]:
        ...
        A stub declares ``metadata.clover.upstream`` in its SKILL.md
        frontmatter:
    ...
    def _upstream_pointer_from_content(self, content: Union[str, bytes]) -> Optional[Dict[str, str]]:
        """Parse ``metadata.clover.upstream`` out of SKILL.md content."""
        ...
        if not isinstance(meta_block, dict):
            return None
        clover_meta = meta_block.get("clover")
        if not isinstance(clover_meta, dict):
            return None
        upstream = clover_meta.get("upstream")
...
def _category_skill_dirs(directory: Path) -> List[str]:
    ...
    ``clover_cli.skills_hub._existing_categories``.
    """
    skill_dirs: List[str] = []
...
def _clover_index_cache_file() -> Path:
    return _index_cache_dir() / "clover-index.json"


def _load_clover_index() -> Optional[dict]:
    """Fetch the centralized skills index, with local cache.

    The index is a JSON file hosted on the docs site, rebuilt daily by CI.
    We cache it locally for CLOVER_INDEX_TTL seconds to avoid repeated
    downloads within a session.
    """
    # Check local cache
    clover_index_cache_file = _clover_index_cache_file()
    if clover_index_cache_file.exists():
        try:
            age = time.time() - clover_index_cache_file.stat().st_mtime
            if age < CLOVER_INDEX_TTL:
                return json.loads(clover_index_cache_file.read_text(encoding="utf-8"))
        except (OSError, json.JSONDecodeError):
            pass
    ...
    for accept_encoding in ("gzip, deflate", "identity"):
        try:
            ...
            if resp.status_code != 200:
                logger.debug("Clover index fetch returned %d", resp.status_code)
            ...
            logger.debug(
                "Clover index decode failed (Accept-Encoding=%s): %s",
                accept_encoding,
                e,
            )
            continue
        except (httpx.HTTPError, json.JSONDecodeError) as e:
            logger.debug("Clover index fetch failed: %s", e)
            return _load_stale_index_cache()
    ...
    # Cache locally
    try:
        clover_index_cache_file.parent.mkdir(parents=True, exist_ok=True)
        clover_index_cache_file.write_text(json.dumps(data), encoding="utf-8")
    except OSError:
        pass
...
class CloverIndexSource(SkillSource):
    """Skill source backed by the centralized Clover Skills Index.
    ...
    def _ensure_loaded(self) -> dict:
        if not self._loaded:
            self._index = _load_clover_index()
            self._loaded = True
    ...
    def fetch(self, identifier: str) -> Optional[SkillBundle]:
        ...
        if resolved:
            ...
            if bundle:
                bundle.source = entry.get("source", "clover-index")
    ...
    def _to_meta(entry: dict) -> SkillMeta:
        return SkillMeta(
            name=entry.get("name", ""),
            description=entry.get("description", ""),
            source=entry.get("source", "clover-index"),
            identifier=entry.get("identifier", ""),
...
def create_source_router(auth: Optional[GitHubAuth] = None) -> List[SkillSource]:
    ...
    sources: List[SkillSource] = [
        ...
        CloverIndexSource(auth=auth), # Centralized index (search + resolved install paths)
