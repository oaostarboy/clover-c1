...
selectable via ``clover tools`` → Image Generation; the active model is
...
def _agent_cache_base_for_env(env: Any) -> str | None:
    if env is not None:
        ...
        if remote_home:
            return f"{str(remote_home).rstrip('/')}/.clover"
    ...
    # Clover cache roots can be translated without side effects. SSH can still
    ...
    try:
        ...
        if source_images and not edit_endpoint:
            raise ValueError(
                ...
                f"via `clover tools` → Image Generation."
            )
        ...
        if "image_url" in kwargs or "reference_image_urls" in kwargs:
            ...
            return json.dumps({
                ...
                "error": (
                    ...
                    f"supports editing via `clover tools` → Image Generation."
                ),
...
# model/provider via `clover tools` or `/skills`.
