...
<url>``) is spawned as a direct child of the Clover process. Clover's own
...
Clover process dies hard — ``kill -9``, an OS-level crash, a force-quit of
...
parent dies, so nothing reaps these until the next Clover startup's opt-in
