...
_PRIVATE_BROWSER_SENTINEL = "_CLOVER_BU_PRIVATE_BROWSER"
...
_OWN_TAB_PREAMBLE = """\
# clover: pin this named session to its own tab (once per daemon process)
def _clover_ensure_own_tab():
    ...
    _marker = _os.path.join(
        _tf.gettempdir(), "clover-bu-owntab-%s-%s-%s" % (_uid, _name, _dpid)
    )
    ...
    try:
        ...
        pass
_clover_ensure_own_tab()
del _clover_ensure_own_tab
"""
...
def _base_subprocess_env() -> dict:
    ...
    # may differ from Clover's venv Python. PYTHONPATH/PYTHONHOME inherited
    # from the agent process point at Clover's venv site-packages, and a
...
def default_downgrade_notice() -> Optional[str]:
    ...
    try:
        ...
        stamp = Path(get_clover_home()) / "cache" / _NOTICE_STAMP_NAME
        try:
        ...
        return (
            ...
            "Run `clover tools` (Browser Automation → Browser Use) to install it, "
...
def _managed_bin_dir() -> Optional[str]:
    """Clover' own bin dir ($CLOVER_HOME/bin) — where install.sh puts uv/uvx
    and where install_cli() links the browser-use binary."""
    try:
        from clover_constants import get_clover_home

        return str(Path(get_clover_home()) / "bin")
...
def install_cli(timeout_s: int = 600) -> Tuple[bool, str]:
    ...
    Resolution order for uv: Clover' managed uv (bootstrapped on demand via
    ``clover_cli.managed_uv.ensure_uv``) → uv on PATH. The binary is linked
    into ``$CLOVER_HOME/bin`` (``UV_TOOL_BIN_DIR``) so ``_find_cli()``
    ...
    # prevent provisioning the canonical Clover-managed copy, or resolution
    # stays pinned to a binary we don't control (version drift, no updates
    # through clover tools).
...
def _workspace_dir(task_id: Optional[str]) -> Optional[str]:
    ...
    try:
        ...
        path = Path(get_clover_home()) / "cache" / "browser-use" / "workspace" / safe
...
def _resolve_backend_cdp(
    ...
    3. A configured cloud browser provider (Browserbase, Firecrawl, Clover
    ...
    # Clover-gateway variant (use_gateway: true) DOES resolve through the
    ...
    try:
        ...
        return (
            ...
            "switch backends via `clover tools` → Browser Automation."
        )
...
def _resolve_real_profile_cdp(env: dict, force_local: bool) -> Optional[str]:
    ...
    default Chromium with their logins — a browser Clover launches on a
    SNAPSHOT of their real profile (see clover_cli.browser_connect). Two ways
    in:
...
def browser_exec(
    ...
    # Clover gateway, CDP override, local Chrome, …). Named sessions compose
...
def _dynamic_schema_overrides() -> dict:
    ...
    if _real_profile_consented():
        ...
        props["local"] = {
            "type": "boolean",
            "description": (
                "Drive the user's own local browser (a Clover-managed copy of "
