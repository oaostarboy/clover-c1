...
`clover skills publish` for sharing. No new source type, no new store, no new
...
@dataclass
class BlueprintSpec:
    """Parsed ``metadata.clover.blueprint`` automation spec for a skill."""
...
def parse_blueprint(skill_md_text: str) -> Optional[BlueprintSpec]:
    ...
    A skill is a blueprint iff ``metadata.clover.blueprint`` is a mapping containing
    ...
    clover = meta.get("clover") if isinstance(meta, dict) else None
    blueprint = clover.get("blueprint") if isinstance(clover, dict) else None
    if blueprint is None:
        return None
    if not isinstance(blueprint, dict):
        raise BlueprintError("metadata.clover.blueprint must be a mapping")
...
def export_blueprint(job: Dict[str, Any], body: str, *, blueprint_name: Optional[str] = None) -> str:
    ...
    and emit a SKILL.md (with a ``metadata.clover.blueprint`` block) they can hand
    to ``clover skills publish`` to share. ``body`` is the plain-language
