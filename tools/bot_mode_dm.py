...
teammate agent (another Clover profile on this install, or an agent on a
...
DM each other via a prompt-injected ``clover -p <bot> chat ...`` shellout.
...
- Everything here is additive. The legacy protocol transports
  (``clover -p`` / ``clover peer dm``) keep working for older prompts.

The transports themselves are unchanged and proven:
- local teammate  → ``clover -p <name> chat --in ~ -c "Bot Chat"
  --create-if-missing -Q --query-file <tmp>`` (one turn, reply on stdout)
- peer teammate   → ``clover peer dm <peer>[/<name>] < <tmp>``
                ...
                "properties": {
                    "target": {
                        ...
                        "description": (
                            ...
                            "('researcher', 'clover' for the default agent), or "
...
def _resolve_local_name(target: str, roster: list[str]) -> Optional[str]:
    """Map a target handle to a profile name ('clover' → 'default')."""
    want = target.strip()
    if not want:
        return None
    if want.lower() == "clover":
...
def cleanup_bot_dm_cache(
    ...
    locations: list[tuple[Path, str]] = [
        (temp_root, "clover-dm-*.txt"),
        (temp_root, "clover-relay-dm-*.txt"),
    ]
    try:
...
def _delivery_lock(argv: list[str], *, stdin_file: bool):
    ...
    Local deliveries (``clover -p <profile> chat …``) collide with relay
    ...
    # resolves the venv-relative clover next to this gateway's interpreter
    ...
    if (
        stdin_file
        or len(argv) < 3
        or cli not in ("clover", "clover.exe")
        or argv[1] != "-p"
    ):
        return contextlib.nullcontext()
    from tools.bot_relay import acquire_turn_lock

    home = Path(os.getenv("CLOVER_HOME") or os.path.expanduser("~/.clover"))
    return acquire_turn_lock(_clover_root(home), argv[2])
...
def _agent_home(agent: Any) -> str:
    ...
    try:
        ...
        pass
    return os.getenv("CLOVER_HOME") or os.path.expanduser("~/.clover")
