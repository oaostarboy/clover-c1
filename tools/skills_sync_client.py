...
409), and is driven by:
  ...
  * the ``clover sync status|pull|push|now`` CLI.

It lives beside ``tools/skills_sync.py`` (NOT under ``clover_cli/``) so the
...
status conflates "may administer Clover" with "has Skill Sync enabled", and has
...
is toggled via ``clover sync enable/disable`` (a ``sync`` flag on the skill's
...
user-authored skills under ``~/.clover/skills/`` are eligible; bundled and
...
# only as the local *intent* the user toggles via ``clover sync enable`` — it is
...
# namespace from clover-c1's local ``content_hash`` (skills_guard.py:846),
...
class SyncInertError(RuntimeError):
    ...
    not logged in, no bearer, or the caller is not a Clover admin.
    """
...
# sync.base_url (or the CLOVER_SYNC_BASE_URL bridge env) overrides it to point
...
DEFAULT_SYNC_BASE_URL = "https://gateway-gateway.clover-c1.local"

def resolve_sync_base_url() -> Optional[str]:
    """Resolve the sync-plane base URL.

    Order: CLOVER_SYNC_BASE_URL env bridge -> config.yaml ``sync.base_url`` ->
    ...
    """
    env = os.getenv("CLOVER_SYNC_BASE_URL")
...
# ---------------------------------------------------------------------------
# Sync feature configuration — env-first, so a Clover Cloud instance can be set
...
# precedence as base_url: the CLOVER_SYNC_* env var wins, else config.yaml
# ``sync.*``, else a built-in default.
#
#   CLOVER_SYNC_BASE_URL        -> sync.base_url        (the sync plane URL)
#   CLOVER_SYNC_ENABLED         -> sync.enabled         (master on/off; default off)
#   CLOVER_SYNC_DEFAULT_OPT_IN  -> sync.default_opt_in  (personal sync policy; default false
...
def sync_feature_enabled() -> bool:
    ...
    ``CLOVER_SYNC_ENABLED`` -> ``sync.enabled`` -> False. This is the master
    switch a Clover Cloud deployment sets to opt its instances into sync by
    default. It is checked by the gate-and-swallow entrypoints IN ADDITION to
    the Clover-admin token gate and a configured base URL — all three must hold for
    background sync to run.
    """
    return _sync_config_bool("CLOVER_SYNC_ENABLED", "enabled", default=False)


def sync_org_auto_propose() -> bool:
    """Whether an agent/user edit to an org skill is proposed automatically.

    ``CLOVER_SYNC_ORG_AUTO_PROPOSE`` -> ``sync.org_auto_propose`` -> False.

    False (default): edits to an org-shared skill stay LOCAL until the user
    runs ``clover sync propose <skill>``. The skill keeps working with the
    ...
    return _sync_config_bool(
        "CLOVER_SYNC_ORG_AUTO_PROPOSE", "org_auto_propose", default=False
    )
...
def sync_default_opt_in() -> bool:
    ...
    ``CLOVER_SYNC_DEFAULT_OPT_IN`` -> ``sync.default_opt_in`` -> False.

    False (default): opt-IN — a skill syncs only after an explicit
    ``clover sync enable`` (or a plane manifest that opted it in). True: opt-OUT
    ...
    Clover Cloud deployment wants. Per the design notes, this default is
    ...
    """
    return _sync_config_bool("CLOVER_SYNC_DEFAULT_OPT_IN", "default_opt_in", default=False)


# ---------------------------------------------------------------------------
# Local skill eligibility + the personal sync opt-in "sync" flag
#
# Only agent-created + user-authored skills under ~/.clover/skills/ sync.
...
def is_sync_eligible(skill_name: str) -> bool:
    ...
    Eligible = present locally under ~/.clover/skills/, NOT bundled, NOT
...
def list_synced_skill_names() -> List[str]:
    ...
    - **opt-out (Clover Cloud "on by default"):** every *eligible* skill syncs
      UNLESS its usage record explicitly carries ``sync: false``. This is what a
      deployment sets (via ``CLOVER_SYNC_DEFAULT_OPT_IN``) so a user's skills
...
def _all_local_skill_names() -> List[str]:
    ...
    opt-out policy). A skill is any directory under ~/.clover/skills/ containing
...
def stable_device_id() -> str:
    ...
    Use ``set_device_name()`` / ``clover sync device --name`` to set an explicit
    label."""
    ...
    # Clover Cloud (and any templated deployment) can seed the label
    # declaratively via CLOVER_SYNC_DEVICE_NAME, so a hosted instance shows a
    # recognizable name with no CLI call. Env seeds the FIRST-USE value only; it
    # is then persisted, so a later `clover sync device --name` (or editing the
    # file) still wins on that device. An explicit file (above) always wins over
    # the env.
    import os

    env_name = (os.environ.get("CLOVER_SYNC_DEVICE_NAME") or "").strip()
...
def set_device_name(name: str) -> str:
    ...
    Writes the (trimmed) name to ~/.clover/skills/.sync_device_id, overwriting
...
# the caller's three-way merge. Auth is the Clover bearer resolved above.
# ---------------------------------------------------------------------------
...
# ~/.clover/skills/.sync_state as JSON.
#
...
# path under ~/.clover/skills/ (the sync contract: "the profile root is a tree
...
def _skill_rel_path(skill_name: str) -> Optional[PurePosixPath]:
    """Return the skill's path relative to ~/.clover/skills/ (posix), or None."""
    try:
...
def _check_version(caps: Dict[str, Any]) -> None:
    ...
    if major != WIRE_VERSION:
        raise SyncError(
            f"this server speaks sync version {ver!r}, but this Clover speaks "
            f"{WIRE_VERSION} — update Clover to sync with it"
        )
    ...
    if overlaps:
        ...
        return {
            ...
            "message": (
                ...
                f"{conflict_ref}. Resolve out-of-band (clover sync / NAS UI)."
            ),
        }
...
def pull_skills(
    ...
    *,
    ...
    ~/.clover/skills/. Only paths the user has opted into (``sync: true``) are
...
# (no push, no pull, no-op) unless the signed-in user is a Clover admin.
# ---------------------------------------------------------------------------

def maybe_push_skills(*, message: str = "clover skill sync") -> Optional[Dict[str, Any]]:
...
#
# Org skills live under a DISTINCT local namespace, ~/.clover/skills/_org/
...
#
# `clover sync propose` is the org sharing surface; proposal is
...
def pull_org_skills(
    ...
    *,
    ...
    """Pull the org canonical set into ``~/.clover/skills/_org/<org_id>/``.
