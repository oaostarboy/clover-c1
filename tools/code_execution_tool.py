...
Lets the LLM write a Python script that calls Clover tools via RPC,
collapsing multi-step tool chains into a single inference turn.

Architecture (two transports):

  **Local backend (UDS):**
  1. Parent generates a `clover_tools.py` stub module with UDS RPC functions
  ...
  **Remote backends (file-based RPC):**
  1. Parent generates `clover_tools.py` with file-based RPC stubs
...
# available on every platform Clover itself runs on.
...
# match a safe prefix, the operational CLOVER_ allowlist, or (on Windows) an
# OS-essential name.  Delegate-task child context is also an exact-name
# operational marker: without it, a sandbox script that spawns/imports Clover
# code can lose the DB-layer Kanban mutation guard while still inheriting
# CLOVER_HOME.
#
# NB: the broad "CLOVER_" prefix was deliberately removed (#27303) — it leaked
# CLOVER_*-named config that lacks a secret substring (e.g. CLOVER_BASE_URL,
# CLOVER_KANBAN_DB, CLOVER_*_WEBHOOK).  The child only needs the few
# location/profile vars in _CLOVER_CHILD_ALLOWED below; CLOVER_RPC_SOCKET /
# CLOVER_RPC_DIR / TZ / HOME are injected explicitly after scrubbing.
...
# Operational CLOVER_* vars the child legitimately needs by exact name — these
# are non-secret runtime-location flags (the same set clover_cli treats as the
# runtime location) that repo-root modules a sandbox script imports may read at
# import time.  None match _SECRET_SUBSTRINGS.
_CLOVER_CHILD_ALLOWED = frozenset({
    "CLOVER_HOME",
    "CLOVER_PROFILE",
    "CLOVER_CONFIG",
    "CLOVER_ENV",
...
def _scrub_child_env(source_env, is_passthrough=None, is_windows=None):
    ...
    Rules (order matters):
      ...
      4. Operational CLOVER_* vars (_CLOVER_CHILD_ALLOWED) pass by exact name.
    ...
    scrubbed = {}
    # Non-secret CLOVER_* vars dropped by the tightened allowlist (#27303). The
    # broad "CLOVER_" prefix used to pass these through; now only the
    # operational set does. The drop is intentional (those vars can carry
    # config like CLOVER_KANBAN_DB / CLOVER_BASE_URL), but a sandbox script
    ...
    for k, v in source_env.items():
        ...
        if any(k.startswith(p) for p in _SAFE_ENV_PREFIXES):
            scrubbed[k] = v
            continue
        if k in _CLOVER_CHILD_ALLOWED:
            scrubbed[k] = v
            continue
        if is_windows and k.upper() in _WINDOWS_ESSENTIAL_ENV_VARS:
            scrubbed[k] = v
            continue
        if k.startswith("CLOVER_"):
            # Non-secret (secrets were already dropped above) and not in any
            # allowlist — a deliberately-dropped CLOVER_* var.
            _dropped_clover.append(k)
    if _dropped_clover:
        logger.debug(
            "execute_code: dropped %d non-allowlisted CLOVER_* var(s) from the "
...
# ---------------------------------------------------------------------------
# clover_tools.py code generator
# ---------------------------------------------------------------------------
...
_TOOL_STUBS = {
    ...
    "write_file": (
        ...
        '"""Write content to a file (always overwrites). Returns dict with status. cross_profile=True opts out of the cross-Clover-profile soft guard."""',
    ...
    "patch": (
        ...
        '"""Targeted find-and-replace (mode="replace") or V4A multi-file patches (mode="patch"). Returns dict with status. cross_profile=True opts out of the cross-Clover-profile soft guard."""',
...
def _sandbox_failure_hint(stderr_text: str, enabled_tools=None) -> Optional[str]:
    ...
    clover_tools import misuse (importing tools that aren't in the sandbox,
    ...
    try:
        m = re.search(
            r"cannot import name '(\w+)' from 'clover_tools'", window
        )
        if m:
            missing = m.group(1)
...
def generate_clover_tools_module(enabled_tools: List[str],
                                 transport: str = "uds") -> str:
    """
    Build the source code for the clover_tools.py stub module.
...
"""Auto-generated Clover tools RPC stubs."""
...
def _connect():
    ...
    CLOVER_RPC_SOCKET can be either:
    ...
    global _sock
    if _sock is None:
        endpoint = os.environ["CLOVER_RPC_SOCKET"]
...
def _call(tool_name, args):
    ...
    request = json.dumps({
        "tool": tool_name,
        "args": args,
        "token": os.environ.get("CLOVER_RPC_TOKEN", ""),
    }) + "\\n"
    with _call_lock:
        conn = _connect()
...
"""Auto-generated Clover tools RPC stubs (file-based transport)."""
import json, os, shlex, tempfile, threading, time

_RPC_DIR = os.environ.get("CLOVER_RPC_DIR") or os.path.join(tempfile.gettempdir(), "clover_rpc")
_seq = 0
...
def _execute_remote(
    ...
    The script and the generated clover_tools.py module are shipped to
    ...
    sandbox_dir = f"{temp_dir}/clover_exec_{sandbox_id}"
    ...
    try:
        ...
        tools_src = generate_clover_tools_module(
            list(sandbox_tools), transport="file",
        )
        _ship_file_to_remote(env, f"{sandbox_dir}/clover_tools.py", tools_src)
        ...
        env_prefix = (
            f"CLOVER_RPC_DIR={shlex.quote(f'{sandbox_dir}/rpc')} "
            f"CLOVER_RPC_TOKEN={shlex.quote(rpc_token)} "
            f"PYTHONDONTWRITEBYTECODE=1"
        )
        tz = os.getenv("CLOVER_TIMEZONE", "").strip()
        if tz:
...
def execute_code(
    ...
    # --- Set up temp directory with clover_tools.py and script.py ---
    tmpdir = tempfile.mkdtemp(prefix="clover_sandbox_")
    ...
    # selector from CLOVER_RPC_SOCKET (path vs. ``tcp://host:port``).
    ...
    if _use_tcp_rpc:
        ...
        sock_path = os.path.join(_sock_tmpdir, f"clover_rpc_{uuid.uuid4().hex}.sock")
    ...
    try:
        # Write the auto-generated clover_tools module.
        ...
        tools_src = generate_clover_tools_module(list(sandbox_tools))
        with open(os.path.join(tmpdir, "clover_tools.py"), "w", encoding="utf-8") as f:
            f.write(tools_src)
        ...
        #   connect.  CLOVER_RPC_SOCKET is set to ``tcp://127.0.0.1:<port>``
        ...
        child_env["CLOVER_RPC_SOCKET"] = rpc_endpoint
        child_env["CLOVER_RPC_TOKEN"] = rpc_token
        ...
        # CLOVER_TIMEZONE is an internal Clover setting and must not leak
        # into child processes.
        _tz_name = os.getenv("CLOVER_TIMEZONE", "").strip()
        if _tz_name:
            child_env["TZ"] = _tz_name
        child_env.pop("CLOVER_TIMEZONE", None)
        ...
        # ``clover_tools.py`` always lives in the staging directory, so that
        # directory must be importable even when project mode changes CWD.
        # Clover's own package root is useful too, but only when the child
        # uses the same Python environment. Project mode can select an
        # external venv; exposing Clover's site-packages to that interpreter
        # can mix incompatible compiled extensions (for example, Python 3.12
        # NumPy with a Python 3.9 project interpreter).
        #
        # Before re-injecting PYTHONPATH, strip Clover-owned entries that
        # leaked through _scrub_child_env (PYTHONPATH is in _SAFE_ENV_PREFIXES
        # so it passes the scrub).  They are redundant for same-Clover-
        ...
        _strip_clover_owned_pythonpath(child_env)
        _clover_root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
        _existing_pp = child_env.get("PYTHONPATH", "")
        _pp_parts = [tmpdir]
        if _uses_clover_python_environment(_child_python):
            _pp_parts.append(_clover_root)
        elif _child_python not in _external_env_logged:
            # Import behavior changes silently otherwise — surface it (once
            # per interpreter path) so "import clover_constants suddenly
            # fails" reports are diagnosable without log spam.
            _external_env_logged.add(_child_python)
            logger.info(
                "execute_code: child interpreter %s is outside the Clover "
                "environment; clover root omitted from PYTHONPATH",
                _child_python,
            )
        if _existing_pp:
        ...
        # but scripts can still read secrets from disk (e.g. open('~/.clover/.env')).
...
def _load_config() -> dict:
    ...
    ``clover --tui`` where it is never used.  Read the lightweight raw config
...
def _get_execution_mode() -> str:
    ...
    Mode semantics:
      ...
      - ``strict``: scripts run in an isolated temp directory with
        ``sys.executable`` (clover-c1's python). Reproducible and the
...
# Interpreter paths already reported as outside the Clover environment —
...
def _python_environment_prefix(python_path: str) -> str:
    ...
    sticky empty result would silently drop the clover root from every
...
def _uses_clover_python_environment(python_path: str) -> bool:
    """Whether *python_path* belongs to Clover's active Python environment.
...
def build_execute_code_schema(enabled_sandbox_tools: set = None,
    ...
    When tools are disabled via ``clover tools`` (e.g. web is turned off),
    ...
    # temp-dir staging and clover-c1's own python.
    if mode == "strict":
        cwd_note = (
            "Scripts run in their own temp dir, not the session's CWD — use absolute paths "
            "(os.path.expanduser('~/.clover/.env')) or terminal()/read_file() for user files."
        )
    else:
        cwd_note = (
    ...
    description = (
        "Run a Python script that calls Clover tools programmatically. "
        ...
        f"Available via `from clover_tools import ...`:\n\n"
        f"{tool_lines}\n\n"
            ...
            "properties": {
                "code": {
                    ...
                    "description": (
                        ...
                        f"`from clover_tools import {import_str}` "
