...
def _spill_full_stdout(stdout_text: str) -> Optional[str]:
    ...
    try:
        ...
        cache_dir = get_clover_dir("cache/exec", "exec_spill")
...
def _call(tool_name, args):
    ...
    # re-accepts (CLOVER_RPC_PERSISTENT=1); retry once on a fresh socket.
    _attempts = 2 if os.environ.get("CLOVER_RPC_PERSISTENT") == "1" else 1
    with _call_lock:
...
def _execute_remote(
    ...
    try:
        ...
        # --- Session-kernel path (clover-c1#96873) -------------------
...
def _build_child_env(*, rpc_endpoint: str, rpc_token: str, tmpdir: str,
    ...
    _pp_parts = [tmpdir]
    if _uses_clover_python_environment(child_python):
...
def execute_code(
    ...
    # terminal() path refuses `launchctl bootout ai.clover.gateway`, but the
...
def build_execute_code_schema(enabled_sandbox_tools: set = None,
    ...
    if mode == "strict":
        ...
        cwd_note = (
            ...
            "Clover's own python (the common case — stdlib plus Clover's "
    ...
    description = (
        "Run Python that calls Clover tools programmatically. Use when you "
