...
Trust levels:
  - builtin:   Ships with Clover. Never scanned, always trusted.
...
_CLOVER_CONFIG_FILES = r'\.clover/(?:config\.yaml|SOUL\.md)'
...
THREAT_PATTERNS = [
    ...
    (r'\$HOME/\.clover/\.env|\~/\.clover/\.env',
     "clover_env_access", "critical", "exfiltration",
     "directly references Clover secrets file"),
    ...
    #     For Clover/other config files it is high (caution) — legitimate
    ...
    (_prose_modify_re(_CLOVER_CONFIG_FILES),
     "clover_config_mod", "high", "persistence",
     "modification language aimed at Clover configuration files (verify intent)"),
    (_shell_write_re(_CLOVER_CONFIG_FILES),
     "clover_config_mod_shell", "critical", "persistence",
     "shell write (redirect/sed -i/tee/cp/mv) targeting Clover configuration files"),
    (r'\.clover/config\.yaml|\.clover/SOUL\.md',
     "clover_config_ref", "low", "persistence",
     "references Clover configuration files (informational; only modification intent is scored)"),
...
# scan. `.skillignore` is the Clover-native name; `.clawhubignore` is honored
