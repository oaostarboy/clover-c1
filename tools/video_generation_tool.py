...
provider** — turn on a backend by enabling a plugin (``clover plugins
enable video_gen/<name>``) and selecting it in ``clover tools`` → Video
Generation.
...
VIDEO_GENERATE_SCHEMA: Dict[str, Any] = {
    ...
    # skill at github/clover-c1-dev/references/dynamic-tool-schemas.md.
        ...
        "properties": {
            ...
            "model": {
                ...
                "description": (
                    ...
                    "configured ``video_gen.model`` (set via `clover tools` "
...
def _missing_provider_error(configured: Optional[str]) -> str:
    if configured:
        msg = (
            ...
            f"registered that name. Run `clover plugins list` to see "
            f"installed video gen backends, or `clover tools` → Video "
    ...
    msg = (
        "No video generation backend is configured. Run `clover tools` → "
...
# `clover tools` or `/skills`, the schema rebuilds automatically.
...
_GENERIC_DESCRIPTION = (
    ...
    "`clover tools` → Video Generation; the agent does not pick them. "
...
def _build_dynamic_video_schema() -> Dict[str, Any]:
    ...
    if provider is None:
        parts.append(
            ...
            "until the user picks one via `clover tools` → Video Generation."
        )
        return {"description": "\n".join(parts)}
