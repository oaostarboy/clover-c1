...
`mcp_tool.py`, and `clover_cli/mcp_config.py`. This module is the ONLY place
...
# ---------------------------------------------------------------------------
# CloverMCPOAuthProvider — OAuthClientProvider subclass with disk-watch
# ---------------------------------------------------------------------------


def _make_clover_provider_class() -> Optional[type]:
    ...
    class CloverMCPOAuthProvider(OAuthClientProvider):
        ...
        def __init__(
            ...
            **kwargs: Any,
            ...
            self._clover_server_name = server_name
            self._clover_home = ""
            ...
            self._clover_preregistered = preregistered
        ...
        async def _initialize(self) -> None:
            ...
            Paired with :class:`CloverTokenStorage` persisting an absolute
            ...
            if (
                isinstance(storage, CloverTokenStorage)
        ...
        async def _prefetch_oauth_metadata(self) -> None:
            ...
            """
            # The SDK's httpx flavour, not Clover' — mcp 2.0 builds on httpx2,
            ...
            async with httpx.AsyncClient(timeout=10.0) as client:
                ...
                for url in build_protected_resource_metadata_discovery_urls(
                    ...
                    try:
                        ...
                        logger.debug(
                            ...
                            self._clover_server_name, url, exc,
                        )
                        continue
                ...
                for url in build_oauth_authorization_server_metadata_discovery_urls(
                    ...
                    if asm:
                        ...
                        storage = self.context.storage
                        from tools.mcp_oauth import CloverTokenStorage
                        if isinstance(storage, CloverTokenStorage):
                            storage.save_oauth_metadata(asm)
                        logger.debug(
                            "MCP OAuth '%s': pre-flight ASM discovered "
                            "token_endpoint=%s",
                            self._clover_server_name, asm.token_endpoint,
                        )
                        break
        ...
        def _persist_oauth_metadata_if_changed(self) -> None:
            ...
            storage = self.context.storage
            from tools.mcp_oauth import CloverTokenStorage
            if not isinstance(storage, CloverTokenStorage):
                return
        ...
        async def _maybe_flag_poisoned_client(self, response: Any) -> None:
            ...
            and is handled by ``clover mcp reauth``.
            ...
            back to ``clover mcp reauth``.
            """
            try:
                if self._clover_preregistered:
                    return
                ...
                # walking back into the same refusal. `clover mcp login`
                ...
                if cimd_url and rejected_id == cimd_url:
                    logger.warning(
                        ...
                        self._clover_server_name, cimd_url,
                    )
    ...
    return CloverMCPOAuthProvider


# Cached at import time. Tested and used by :class:`MCPOAuthManager`.
_CLOVER_PROVIDER_CLS: Optional[type] = _make_clover_provider_class()
...
class MCPOAuthManager:
        ...
        with self._entries_lock:
            ...
            if entry.provider is None:
                ...
                if entry.provider is not None:
                    entry.provider._clover_home = key[0]
    ...
    def _build_provider(
        ...
        Constructs :class:`CloverMCPOAuthProvider` directly using the helpers
        ...
        Returns None if the MCP SDK's OAuth support is unavailable.
        """
        if _CLOVER_PROVIDER_CLS is None:
            logger.warning(
        ...
        if (
            ...
            raise OAuthNonInteractiveError(
                ...
                "cached tokens found. Run `clover mcp login "
    ...
    def remove(
        ...
        *,
        ...
        Called by ``clover mcp remove <name>`` and (indirectly) by
        ``clover mcp login <name>`` during forced re-auth.
        """
        with self._entries_lock:
            entry = self._entries.pop(self._key(server_name, clover_home), None)

        from tools.mcp_oauth import remove_oauth_tokens
        remove_oauth_tokens(server_name, clover_home=clover_home)
        logger.info(
    ...
    def restore_entry(
        ...
        *,
        ...
        if entry is None:
            return
        with self._entries_lock:
            self._entries.setdefault(self._key(server_name, clover_home), entry)
    ...
    def evict(
        ...
        *,
        ...
        with self._entries_lock:
            self._entries.pop(self._key(server_name, clover_home), None)
    ...
    async def invalidate_if_disk_changed(
        ...
        *,
        ...
        entry = self._entries.get(self._key(server_name, clover_home))
        if entry is None or entry.provider is None:
            return False

        async with entry.lock:
            tokens_path = _get_token_dir(clover_home) / f"{_safe_filename(server_name)}.json"
            try:
