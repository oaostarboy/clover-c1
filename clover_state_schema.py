...
``clover_state.SessionDB``. It defines no ``__init__`` and no state of its
...
``SessionDB.__init__``. It must never import clover_state (cycle) — shared
module-level constants live in clover_state_common.
"""
...
# Moved methods logged under the "clover_state" logger before the split;
# keep that logger identity so log filtering/capture behavior is unchanged.
logger = logging.getLogger("clover_state")
...
def schema_read_probe_statements() -> tuple:
    ...
    ``clover_cli/web_server.py``) run these probes right after a read-only
    ...
    served an empty session list after `clover update` until the user's
...
class SessionSchemaMixin:
    ...
    def _sqlite_supports_fts5(self, cursor: sqlite3.Cursor) -> bool:
        try:
            cursor.execute("CREATE VIRTUAL TABLE temp._clover_fts5_probe USING fts5(x)")
            cursor.execute("DROP TABLE temp._clover_fts5_probe")
            return True
        except sqlite3.OperationalError as exc:
            ...
            if escalated:
                ...
                if foreign_holders:
                    logger.error(
                        ...
                        "`clover sessions optimize-storage` with the gateway stopped. "
                        "`clover doctor` reports this degraded state.",
                        attempts,
                        foreign_holders,
                    )
    ...
    def _parse_schema_columns(schema_sql: str) -> Dict[str, Dict[str, str]]:
        ...
        try:
            from clover_constants import get_clover_home
            cache_path = get_clover_home() / "cache" / "schema_columns.json"
    ...
    def _init_schema(self):
        ...
        if row is None:
            ...
            if current_version < 23:
                ...
                # optimization is available. `clover sessions optimize-storage`
        ...
        if fts5_available:
            ...
            # not yet opted into `clover db optimize`) must keep its EXISTING
