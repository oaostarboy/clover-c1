# Contribuir a Clover Cognition

¡Gracias por contribuir a Clover Cognition! Esta guía cubre todo lo que necesitas: configurar tu entorno de desarrollo, entender la arquitectura, decidir qué construir y conseguir que tu PR sea aceptado.
...
2. **Compatibilidad entre plataformas** — macOS, diferentes distribuciones de Linux y WSL2 en Windows. Queremos que Clover funcione en todas partes.
...
Las habilidades incluidas (en `skills/`) se envían con cada instalación de Clover. Deben ser **ampliamente útiles para la mayoría de los usuarios**:
...
Si tu habilidad es especializada, contribuida por la comunidad o de nicho, es mejor para un **Skills Hub** — súbela a un registro de habilidades y compártela en el [GitHub Discussions](https://github.com/oaostarboy/clover-c1/discussions). Los usuarios pueden instalarla con `clover skills install`.

---

## Proveedores de Memoria: Publicar como Plugin Independiente

**Ya no aceptamos nuevos proveedores de memoria en este repositorio.** El conjunto de proveedores integrados en `plugins/memory/` (honcho, mem0, supermemory, byterover, hindsight, holographic, openviking, retaindb) está cerrado. Si quieres añadir un nuevo backend de memoria, publícalo como un **repositorio de plugin independiente** que los usuarios instalen en `~/.clover/plugins/` (o a través de un entry point de pip).

Los plugins de memoria independientes:

- Implementan el mismo ABC `MemoryProvider` (`agent/memory_provider.py`) — `sync_turn`, `prefetch`, `shutdown` y opcionalmente `post_setup(clover_home, config)` para integración con el asistente de configuración
...
- Se integran con `clover memory setup` a través de `post_setup()` — sin necesidad de tocar el código base
...
├── cli.py                    # Clase CloverCLI — TUI interactiva, integración prompt_toolkit
├── model_tools.py            # Orquestación de herramientas (capa delgada sobre tools/registry.py)
├── toolsets.py               # Agrupaciones y presets de herramientas (clover-cli, clover-telegram, etc.)
├── clover_state.py           # Base de datos de sesiones SQLite con búsqueda de texto completo FTS5, títulos de sesión
...
│
├── clover_cli/               # Implementaciones de comandos CLI
...
### Configuración del usuario (almacenada en `~/.clover/`)
...
| `~/.clover/cron/` | Datos de trabajos programados |
| `~/.clover/whatsapp/session/` | Credenciales del puente WhatsApp |
...
- **Persistencia de sesión**: Todas las conversaciones se almacenan en SQLite (`clover_state.py`) con búsqueda de texto completo y títulos de sesión únicos.
...
(por ejemplo `_CLOVER_CORE_TOOLS` o un toolset dedicado); de lo contrario la herramienta
...
2. **Las herramientas referenciadas en el cuerpo de SKILL.md deben ser herramientas nativas de Clover o servidores MCP que la habilidad espere explícitamente.** Usa los nombres de herramientas en comillas invertidas: `` `terminal` ``, `` `web_extract` ``, `` `web_search` ``, `` `read_file` ``, `` `write_file` ``, etc.
...
Clover usa un sistema de skins basado en datos — no se necesitan cambios de código para añadir una nueva skin.

**Opción A: Skin de usuario (archivo YAML)**

Crea `~/.clover/skins/<nombre>.yaml`:
...
Añade al dict `_BUILTIN_SKINS` en `clover_cli/skin_engine.py`. Usa el mismo esquema que arriba pero como dict de Python.
...
Clover se ejecuta en Linux, macOS y Windows nativo (además de WSL2). Al escribir código
...
Clover tiene acceso al terminal. La seguridad importa.
...
2. **Probar manualmente**: Ejecuta `clover` y ejercita la ruta de código que cambiaste
...
- Usa [GitHub Issues](https://github.com/oaostarboy/clover-c1)
