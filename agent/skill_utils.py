...
from clover_constants import get_config_path, get_skills_dir, is_termux
...
def _detect_environment(env: str) -> bool:
    ...
    CLOVER_KANBAN_* vars without owning them), so caching it process-wide would
    ...
    if env == "kanban":
        ...
        # dispatcher sets ``CLOVER_KANBAN_TASK`` / ``CLOVER_KANBAN_BOARD`` in the
        ...
        # tool availability.
        if os.getenv("CLOVER_KANBAN_TASK") or os.getenv("CLOVER_KANBAN_BOARD"):
        ...
        # The Clover Docker image runs s6-overlay as PID 1 (/init). s6 plants
...
def _load_raw_config() -> Dict[str, Any]:
    ...
    This module intentionally avoids importing ``clover_cli.config`` on the
...
# `clover-c1` skill is the agent's own operating manual — it drives
# configuring, extending, and troubleshooting Clover itself, and the system
# prompt unconditionally points at it. Disabling it leaves the agent unable
# to help with Clover, so disable requests for these names are ignored
# everywhere the disabled list is consulted.
ESSENTIAL_SKILLS: frozenset = frozenset({"clover-c1"})
...
def get_disabled_skill_names(platform: str | None = None) -> Set[str]:
    ...
    Args:
        platform: Explicit platform name (e.g. ``"telegram"``).  When
            *None*, resolves from ``CLOVER_PLATFORM`` or
            ``CLOVER_SESSION_PLATFORM`` env vars.  Returns the global
    ...
    resolved_platform = (
        platform
        or os.getenv("CLOVER_PLATFORM")
        or get_session_env("CLOVER_SESSION_PLATFORM")
    )
...
def parse_config_string_list(value) -> List[str]:
    ...
    ``clover config set`` and JSON-mode editor saves store lists as quoted
...
# which becomes the dominant cost of ``clover`` startup when ~120 skills
...
def get_external_skills_dirs() -> List[Path]:
    ...
    paths that resolve to the local ``~/.clover/skills/`` are silently skipped.

    Cached in-process, keyed on ``config.yaml`` mtime — the function is
    called once per skill during banner / tool-registry scans, and YAML
    parsing a non-trivial config dominates ``clover`` cold-start time
    ...
    for entry in raw_dirs:
        ...
        p = Path(expanded)
        # Resolve relative paths against CLOVER_HOME, not cwd
        if not p.is_absolute():
            p = (clover_home / p).resolve()
        else:
            p = p.resolve()
        if p == local_skills:
            continue
...
def get_all_skills_dirs() -> List[Path]:
    """Return all skill directories: local ``~/.clover/skills/`` first, then external.

    The local dir is always first (and always included even if it doesn't exist
    yet — callers handle that).  External dirs follow in config order.

    NOTE: trusted project-local dirs (``./.clover/skills`` at the git root) are
...
#   <root>/.clover/skills/   — Clover-native location
...
# a one-line "run `clover skills trust`" notice.
#
...
def _candidate_project_skills_dirs(root: Path) -> List[Path]:
    ...
    The exclusion matters when CLOVER_HOME itself lives inside a git checkout:
    ``<root>/.clover/skills`` would otherwise double as both the profile-local
...
def get_untrusted_project_skills_root() -> Optional[Tuple[Path, int]]:
    ...
    ``clover skills trust``. Returns None when there is nothing to notify
...
#
# Trust (`clover skills trust`) is a REPO-level decision made once; the repo's
...
#
# The scan cache lives under CLOVER_HOME, never inside the repo (we don't
...
def _project_scan_cache_dir() -> Path:
    ...
    return get_clover_home() / "cache" / "project_skill_scans"
...
def normalize_skill_lookup_name(identifier: str) -> str:
    ...
    under ``~/.clover/skills/`` (including via symlinks) or configured
    ...
    # ~/.clover/skills/<name> where <name> is a symlink to a checked-out
...
def is_external_skill_path(path) -> bool:
    ...
    ``skills.external_dirs`` are externally owned: Clover can discover and view
...
def extract_skill_conditions(frontmatter: Dict[str, Any]) -> Dict[str, List]:
    ...
    if not isinstance(metadata, dict):
        metadata = {}
    clover = metadata.get("clover") or {}
    if not isinstance(clover, dict):
        clover = {}
    return {
        "fallback_for_toolsets": clover.get("fallback_for_toolsets", []),
        "requires_toolsets": clover.get("requires_toolsets", []),
        "fallback_for_tools": clover.get("fallback_for_tools", []),
        "requires_tools": clover.get("requires_tools", []),
        ...
        "session_platforms": clover.get("session_platforms", []),
    }
...
def extract_skill_config_vars(frontmatter: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    if not isinstance(metadata, dict):
        return []
    clover = metadata.get("clover")
    if not isinstance(clover, dict):
        return []
    raw = clover.get("config")
    if not raw:
        return []
...
def iter_skill_index_files(skills_dir: Path, filename: str):
    ...
    Excludes Clover metadata, VCS, virtualenv/dependency, cache, and skill
