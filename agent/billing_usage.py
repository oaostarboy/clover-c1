...
Data source: the NAS account-info fetch (``CloverPortalAccountInfo``), whose
...
def usage_model_from_account(account_info: Any) -> UsageModel:
    """Build a :class:`UsageModel` from a ``CloverPortalAccountInfo``. Fail-open.
...
def build_usage_model(*, timeout: float = 10.0) -> UsageModel:
    ...
    Dev override: ``CLOVER_DEV_CREDITS_FIXTURE`` short-circuits to a fixture so
    ...
    try:
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=1) as pool:
            account = pool.submit(get_clover_portal_account_info, force_fresh=True).result(timeout=timeout)
...
def _dev_fixture_usage_model() -> Optional[UsageModel]:
    """Map ``CLOVER_DEV_CREDITS_FIXTURE`` to a usage model for offline UX work.

    Recognized names: ``free | healthy | low | topup | depleted``. Returns
    ``None`` when the env var is unset (real portal path runs).
    """
    name = (os.getenv("CLOVER_DEV_CREDITS_FIXTURE") or "").strip().lower()
    if not name:
        return None
