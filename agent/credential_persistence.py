...
``auth.json``.  They intentionally have no dependency on ``clover_cli.auth`` so
...
# Sources Clover owns and can intentionally persist in auth.json.  Everything
...
def sanitize_borrowed_credential_payload(
    ...
    Owned sources (manual entries and Clover-owned OAuth/device-code state)
