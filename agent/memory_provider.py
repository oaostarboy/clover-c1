...
Optional hooks (override to opt in):
  ...
  backup_paths() -> list[str]            — extra on-disk paths to include in `clover backup`
"""
...
class MemoryProvider(ABC):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        kwargs always include:
          - clover_home (str): The active CLOVER_HOME directory path. Use this
            for profile-scoped storage instead of hardcoding ``~/.clover``.
        ...
        kwargs may also include:
          ...
          - agent_workspace (str): Shared workspace name (e.g. "clover").
    ...
    def get_config_schema(self) -> List[Dict[str, Any]]:
        ...
        Used by 'clover memory setup' to walk the user through configuration.
    ...
    def save_config(self, values: Dict[str, Any], clover_home: str) -> None:
        ...
        Called by 'clover memory setup' after collecting user inputs.
        ``values`` contains only non-secret fields (secrets go to .env).
        ``clover_home`` is the active CLOVER_HOME directory path.
    ...
    def backup_paths(self) -> List[str]:
        """Return extra on-disk paths this provider stores OUTSIDE CLOVER_HOME.

        ``clover backup`` only walks CLOVER_HOME, so any provider state kept
        ...
        of the archive, and ``clover import`` restores them to their original
