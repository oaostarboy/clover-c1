...
# ``data:`` field before Clover receives a completion chunk.  Keeping this
...
# case: a stream dies mid-response and Clover persists a content-less
...
# Request parameters that Clover sends on SOME routes only, paired with the
...
def classify_api_error(
    ...
    *,
    ...
    # clover_cli.plugins.VALID_HOOKS for the callback contract). Callback
    ...
    try:
        from clover_cli.plugins import get_plugin_error_classification
...
def _classify_by_status(
    ...
    *,
    ...
    if status_code == 401:
        ...
        # provider-specific refresh (Codex, Anthropic, Clover) run before
    ...
    if status_code == 404:
        # Clover API currently surfaces HA/NAS credit depletion as a paid model
