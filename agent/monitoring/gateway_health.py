...
def _base_attrs(*, profile: str, install_id: str, version: str, supervision_mode: str) -> Dict[str, str]:
    ...
    return {
        ...
        "clover.supervision_mode": mode if mode in _SUPERVISION_MODES else "unknown",
    }
...
def build_gateway_health_snapshot(
    ...
    *,
    ...
    metrics: list[GatewayMetric] = [
        _metric("clover.gateway.up", 1 if gateway_running else 0, base),
        _metric("clover.gateway.active_agents", active_agents, base),
        _metric("clover.gateway.busy", 1 if busy else 0, base),
        _metric("clover.gateway.drainable", 1 if drainable else 0, base),
        _metric("clover.gateway.restart_requested", 1 if runtime.get("restart_requested") else 0, base),
    ]
    if gateway_state:
        metrics.append(_metric("clover.gateway.state", 1, base, **{"clover.gateway.state": str(gateway_state)}))
    ...
    for platform, pdata in platforms.items():
        ...
        metrics.append(_metric(
            ...
            1 if is_up else 0,
            base,
            **{"clover.platform": str(platform), "clover.platform.state": state},
        ))
        metrics.append(_metric(
            "clover.platform.degraded",
            1 if is_degraded else 0,
            base,
            **{"clover.platform": str(platform), "clover.platform.state": state, "clover.error_code": error_code},
        ))
        if is_degraded:
