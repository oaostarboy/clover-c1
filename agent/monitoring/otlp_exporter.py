...
configured under ``monitoring.export.otlp``. Lets an operator stream Clover
...
Notes:
  ...
  * ``opentelemetry-sdk`` + ``opentelemetry-exporter-otlp-proto-http`` are an
    optional extra (``pip install clover-c1[otlp]``), imported lazily so the
...
def _span_attrs(ev: Dict[str, Any]) -> Dict[str, Any]:
    ...
    kind = ev.get("event")
    attrs: Dict[str, Any] = {"clover.event": kind or "unknown"}
    keep_by_kind = {
    ...
    *,
    ...
    try:
        ...
        logger.warning("monitoring.export.otlp.enabled but the OTel SDK could not "
                       "be installed/imported; install 'clover-c1[otlp]'")
        return None
    from agent.monitoring.emitter import get_emitter
