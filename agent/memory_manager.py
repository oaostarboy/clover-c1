...
class MemoryManager:
    ...
    def add_provider(self, provider: MemoryProvider) -> None:
        ...
        _core_tool_names = set(_CLOVER_CORE_TOOLS)
    ...
    def _strip_skill_scaffolding(text: str) -> Optional[str]:
        ...
        When a user invokes a /skill or /bundle, Clover expands the turn into
    ...
    def _prefetch_provider(
        ...
        # Propagate the caller's contextvars (profile CLOVER_HOME override)
    ...
    def _submit_background(self, fn, *, kind: str = "write") -> None:
        ...
        dashboard, cron) is a ContextVar-scoped CLOVER_HOME override, and
    ...
    def initialize_all(self, session_id: str, **kwargs) -> None:
        ...
        Automatically injects ``clover_home`` into *kwargs* so that every
        provider can resolve profile-scoped storage paths without importing
        ``get_clover_home()`` themselves.
        """
        if "clover_home" not in kwargs:
            from clover_constants import get_clover_home
            kwargs["clover_home"] = str(get_clover_home())
        for provider in self._providers:
            try:
