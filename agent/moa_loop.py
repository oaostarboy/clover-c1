...
MoA-enabled; the normal Clover agent loop still owns tool calling and turn
...
def _moa_privacy_mode(moa_raw: Any) -> str:
    ...
    from clover_cli.moa_config import coerce_privacy_filter
...
def _run_references_parallel(
    ...
    *,
    ...
    # callbacks + the Clover Portal conversation tag) into each worker so
...
def _reference_messages(messages: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    conversation flow, but flatten it into clean user/assistant *text* turns:
      ...
      - system prompt: dropped (8K of Clover boilerplate, not advisory signal).
...
def _preset_temperature(preset: dict[str, Any], key: str) -> float | None:
    ...
    single-model Clover agent (which never sends temperature unless
...
def aggregate_moa_context(
    *,
    ...
    synth_prompt = (
        ...
        "Clover agent. Focus on next steps, tool-use strategy, risks, and any "
    ...
    return (
        ...
        "normal Clover agent loop. You may call tools, continue reasoning, or "
        "finish normally.]\n"
