...
2. **Command-type providers** declared under ``tts.providers.<name>:
   ...
   CLI into Clover with shell-template placeholders. **Wins over a
...
shipped today) or ``~/.clover/plugins/tts/<name>/`` (user-installed).
...
``{success: False, error: …}`` JSON envelope the rest of Clover
expects.
"""
...
class TTSProvider(abc.ABC):
    ...
    def display_name(self) -> str:
        """Human-readable label shown in ``clover tools``.
    ...
    def is_available(self) -> bool:
        ...
        Must NOT raise — used by the picker and ``clover setup`` for
