...
``clover pause`` writes a sentinel file at ``$CLOVER_HOME/ESTOP``;
``clover resume`` removes it. While the sentinel exists:
...
* new gateway turns get a brief "Clover is paused" reply instead of an
...
hold even if the file was created by ``touch ~/.clover/ESTOP``.
...
def _clover_home() -> Path:
    """Resolve the active CLOVER_HOME (profile-aware) at call time."""
    try:
...
def sentinel_path() -> Path:
    """Path of the ESTOP sentinel under the active CLOVER_HOME."""
    return _clover_home() / SENTINEL_NAME
...
def is_engaged() -> bool:
    ...
    exists (permission error, transient I/O failure on CLOVER_HOME), report
...
def paused_reply() -> Optional[str]:
    ...
    if reason:
        return (
            f"⏸️ Clover is paused ({reason}). New work is on hold; "
            "run `clover resume` to pick things back up."
        )
    return (
        "⏸️ Clover is paused. New work is on hold; "
        "run `clover resume` to pick things back up."
    )
