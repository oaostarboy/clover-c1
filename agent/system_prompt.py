...
upstream prefix cache warm.  See ``clover-c1-dev``'s
...
* ``stable``   — identity (SOUL.md or DEFAULT_AGENT_IDENTITY), tool
  guidance, computer-use guidance, clover subscription block, tool-use
...
from clover_constants import get_default_clover_root, get_clover_home
...
_PLUGIN_SECTION_FRAME_RE = re.compile(
    ...
    r"<!-- clover-plugin-section-chars:(?P<chars>[0-9]{1,4}) -->\n\n",
    re.MULTILINE,
)
...
def _tui_embedded_pane_clarifier(hint: str) -> str:
    ...
    Triggered by ``CLOVER_DESKTOP_TERMINAL=1`` (set by ``main.cjs`` only on the
    ...
    if _TUI_EMBEDDED_PANE_CLARIFIER in hint:
        return hint
    if not is_truthy_value(os.getenv("CLOVER_DESKTOP_TERMINAL")):
        return hint
...
def _plugin_session_info(agent: Any) -> Dict[str, str]:
    ...
    try:
        ...
        # the CLOVER_HOME ContextVar (#86313 class; plugin half per @helix4u).
...
def _frozen_plugin_prompt_sections(agent: Any) -> tuple:
    ...
    if isinstance(stored_prompt, str) and stored_prompt:
        ...
        return rendered
    try:
        from clover_cli.plugins import render_system_prompt_sections
...
def _plugin_section_blocks(sections: tuple, position: str) -> List[str]:
    from clover_cli.plugins import format_system_prompt_sections
...
def _agent_home(agent: Any) -> Optional[Path]:
    ...
    1. A bound CLOVER_HOME ContextVar override wins. Surfaces that multiplex
...
def _profile_name_for_home(home: Path) -> str:
    ...
    Uses :func:`get_default_clover_root` (NOT ``get_clover_home()``): on a
    correctly bound profile session the ambient home IS the profile dir, so
    ``get_clover_home()/profiles`` would never contain ``home`` and every
...
def build_system_prompt_parts(agent: Any, system_message: Optional[str] = None) -> Dict[str, str]:
    ...
    AIAgent.  Clover never re-renders parts of this string mid-
    ...
    # Some execution modes (cron) still want CLOVER_HOME persona while keeping
    # cwd project instructions disabled.
    _soul_loaded = False
    if agent.load_soul_identity or not agent.skip_context_files:
        # Scope the SOUL.md read to the agent's OWN home (see _agent_home) —
        # ambient resolution on a thread that lost the CLOVER_HOME ContextVar
    ...
    # Pointer to the docs (and, when it exists, the clover-c1 skill) for
    # user questions about Clover itself. The skill_view() pointer is a
    # dangling reference in two cases — no skill tools in the toolset
    # (Blank Slate) OR the clover-c1 skill not installed — so the
    ...
    stable_parts.append(CLOVER_AGENT_HELP_GUIDANCE_NO_SKILLS)
    ...
    # CLOVER_KANBAN_TASK env var). Normal chat sessions never see
    ...
    # the clover-c1 skill actually present in the index (gating on the
    # rendered index line keeps this a pure string check — no second
    # filesystem scan, and it inherits the index cache's stability).
    if _has_skill_view and "- clover-c1:" in skills_prompt:
        stable_parts[_help_guidance_slot] = CLOVER_AGENT_HELP_GUIDANCE
    ...
    # Coding posture (base Clover, any interactive coding surface in a code
    ...
    # `clover -p <bot> chat --in ~ -c "Bot Chat"` and the desktop pins), on
    # installs where Bot Mode manages profiles (ui_meta['clover-bots']).
    ...
    # Active-profile hint — names the Clover profile the agent is running
    # under so it doesn't conflate ~/.clover/skills/ (default profile) with
    # ~/.clover/profiles/<active>/skills/ (this profile's). Deterministic
    ...
    # ambient CLOVER_HOME: on a build thread that lost the ContextVar this
    ...
    # from get_default_clover_root(): get_clover_home() on a bound profile
    ...
    # to the legacy behavior (and patchable via this module's get_clover_home).
    if _agent_home_path is not None:
        _home_str = str(_agent_home_path)
        _root_str = str(get_default_clover_root())
    else:
        _home_str = _root_str = str(get_clover_home())
    if active_profile == "default":
        post_workspace_parts.append(
            "Active Clover profile: default. Other profiles (if any) live "
        ...
        # data sits at the ROOT (get_default_clover_root()), which in ambient
        # profile mode is NOT get_clover_home().
        profile_home = _home_str
        default_root = get_default_clover_root()
        post_workspace_parts.append(
            f"Active Clover profile: {active_profile}. This session reads "
    ...
    if not agent.skip_context_files:
        ...
        # (developing Clover). Every other surface (desktop chat panel,
    ...
    from clover_time import get_timezone as _clover_tz, now as _clover_now
    now = _clover_now()
