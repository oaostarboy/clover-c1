...
Owns one Codex thread per Clover session. Drives `turn/start`, consumes
...
# Clover' tools.terminal.security_mode → Codex's permissions profile id.
# Defaults if config is missing → workspace-write (matches Codex's own default).
_CLOVER_TO_CODEX_PERMISSION_PROFILE = {
...
def _coerce_turn_input_text(user_input: Any) -> str:
    """Collapse Clover/OpenAI rich content into app-server text input.
...
class CodexAppServerSession:
    """One Codex thread per Clover session, lifetime owned by AIAgent.
    ...
    def __init__(
        ...
        *,
        ...
        self._permission_profile = (
            permission_profile or _CLOVER_TO_CODEX_PERMISSION_PROFILE.get(
                os.environ.get("CLOVER_TERMINAL_SECURITY_MODE", "auto"),
                "workspace-write",
            )
        )
    ...
    def ensure_started(self) -> str:
        ...
        self._client.initialize(
            client_name="clover",
            client_title="Clover Cognition",
            client_version=_get_clover_version(),
        )
    ...
    def run_turn(
        ...
        *,
        ...
        # supports rich content but Clover' text path is the common case).
        try:
    ...
    def _handle_server_request(self, req: dict) -> None:
        """Translate a codex server request (approval) into Clover' approval
        ...
        if method == "item/commandExecution/requestApproval":
            ...
            # OAuth, form data). For our own clover-tools callback we
            # auto-accept — the user already approved Clover' tools
    ...
    def _decide_exec_approval(self, params: dict) -> str:
        ...
        This is protocol-level routing only — it carries NO Clover
        approval-mode/timeout logic. The Clover-side resolution happens
    ...
    def _decide_apply_patch_approval(self, params: dict) -> str:
        ...
        Protocol-level routing only; Clover approval-mode/timeout
...
def _approval_choice_to_codex_decision(choice: str) -> str:
    """Map Clover approval choices onto codex's CommandExecutionApprovalDecision
    / FileChangeApprovalDecision wire values.

    Clover returns 'once', 'session', 'always', or 'deny'.
    ...
    NOT in tools/approval.py: the Clover approval mode/timeout resolution
    ...
    # "prompt expired"; the Clover-side messaging already distinguishes them.
    return "decline"
...
def _get_clover_version() -> str:
    """Best-effort Clover version string for codex's userAgent line."""
    try:
