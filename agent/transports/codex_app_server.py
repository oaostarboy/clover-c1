...
projection into Clover' display, approval bridging, transcript projection into
...
"codex_app_server"`. Clover' default tool dispatch is unchanged when this
...
class CodexAppServerClient:
    ...
    def __init__(
        ...
        # `os.environ.copy()` also handed it every Tier-1 Clover secret — gateway
        ...
        spawn_env = clover_subprocess_env(inherit_credentials=True)
        if env:
        ...
        if spawn_env.get("CLOVER_KANBAN_TASK"):
            kanban_db = spawn_env.get("CLOVER_KANBAN_DB")
            kanban_root = (
                ...
                if kanban_db
                    ...
                    "CLOVER_KANBAN_ROOT",
                    os.path.join(
                        spawn_env.get("CLOVER_HOME", os.path.expanduser("~/.clover")),
                        "kanban",
                    ),
                )
            )
    ...
    def initialize(
        self,
        client_name: str = "clover",
        client_title: str = "Clover Cognition",
