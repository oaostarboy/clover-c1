...
# Wire-name used when Clover keeps client-side web_search on xAI Responses.
...
# avoids that while still dispatching through Clover's configured provider
# (Firecrawl / Tavily / …). Mapped back to ``web_search`` in normalize_response.
_XAI_CLIENT_WEB_SEARCH_ALIAS = "clover_web_search"
...
# rename on the wire (clover_<name>), map back in normalize_response so
# Clover dispatch is unaffected.
_OPENCODE_RESERVED_TOOL_NAMES = ("web_search", "search_files")
_RESERVED_TOOL_ALIAS_PREFIX = "clover_"
...
class ResponsesApiTransport(ProviderTransport):
    ...
    def build_kwargs(
        ...
        **params,
        ...
        # xAI server-side web search vs Clover web providers.
        #
        ...
        #    keep Clover dispatch so ``web.backend`` / ``web.search_backend``
        #    is honored, but rename the wire tool to
        #    ``clover_web_search`` so Grok cannot hijack the name. The alias
        ...
        kwargs = {
            # ``-900k`` large-context picker variants are Clover-side aliases
    ...
    def normalize_response(self, response: Any, **kwargs) -> NormalizedResponse:
        ...
        if msg and msg.tool_calls:
            ...
            for tc in msg.tool_calls:
                ...
                # Undo the xAI client-path wire alias so Clover dispatches
                ...
                # (clover_web_search / clover_search_files, #85589).
