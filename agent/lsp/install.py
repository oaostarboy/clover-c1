...
appropriate.  All installs go to a Clover-owned bin staging dir,
``<CLOVER_HOME>/lsp/bin/``, so we don't pollute the user's global
toolchain.
...
- ``manual`` — never install; if a binary is missing, the server is
  silently skipped and the user is told about it via ``clover lsp
  status``.
...
# ``<CLOVER_HOME>/lsp/bin/`` first, then on PATH.
#
# Optional fields:
...
INSTALL_RECIPES: Dict[str, Dict[str, Any]] = {
    ...
    # `clover lsp status` reports the host's presence.
...
def clover_lsp_bin_dir() -> Path:
    """Return the Clover-owned bin staging dir for LSP servers."""
    from clover_constants import get_clover_home

    p = get_clover_home() / "lsp" / "bin"
...
def _existing_binary(name: str) -> Optional[str]:
    ...
    for staged in _native_binary_candidates(clover_lsp_bin_dir() / name):
...
def _install_npm(
    ...
    """
    # Managed npm first: $CLOVER_HOME/node is not on an arbitrary process's
    # PATH, so a bare which() misses the Node that Clover installed and
    ...
    if npm is None:
        ...
        return None
    staging = clover_lsp_bin_dir().parent  # <CLOVER_HOME>/lsp/
    ...
    for c in _native_binary_candidates(nm_bin):
        if c.exists():
            ...
            link = clover_lsp_bin_dir() / c.name
            if not link.exists():
                try:
                    link.symlink_to(c)
...
def _install_pip(pkg: str, bin_name: str) -> Optional[str]:
    """Install a Python package into a clover-owned target dir.
    ...
    console script.
    """
    pip_target = clover_lsp_bin_dir().parent / "python-packages"
    ...
    for script_dir in script_dirs:
        for bin_path in _native_binary_candidates(script_dir / bin_name):
            if bin_path.exists():
                link = clover_lsp_bin_dir() / bin_path.name
                if not link.exists():
                    try:
...
def detect_status(pkg: str) -> str:
    ...
    Used by the ``clover lsp status`` CLI to give users a quick
