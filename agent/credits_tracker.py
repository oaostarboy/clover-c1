...
def is_free_tier_model(model: str, base_url: str = "") -> bool:
    """Return True when *model* is a Clover free-tier model, using ONLY local data.

    Two signals, both zero-network:

    1. The ``:free`` suffix — the canonical Clover free SKU marker (e.g.
       ``nvidia/nemotron-3-ultra:free``). Free by construction on the API side
       (spend is forced to 0 for ``:free`` ids).
    2. The ``stealth/`` prefix — Clover stealth-preview SKUs (e.g.
    ...
    3. A peek into the in-process pricing cache in ``clover_cli.models``
...
def evaluate_credits_notices(
    ...
    *,
    ...
    ``model_is_free``: True when the session's active model is a Clover free-tier
    ...
    if target_band != shown_band:
        ...
        if target_band is not None:
            ...
            # only meaningful against a Clover subscription cap (no cap → never fires),
...
def parse_credits_headers(
    ...
    try:
        ...
        # sentinel header is absent (the common case for non-Clover providers, on
        ...
        if version_val != 1:
            if version_val > 1 and not _version_warning_emitted:
                ...
                logger.warning(
                    "credits header version %d unsupported, ignoring — update Clover",
                    version_val,
                )
            return None
...
# ── Dev test fixtures (CLOVER_DEV_CREDITS_FIXTURE) ───────────────────────────
# Throwaway dev scaffolding: trigger any notice state on demand for testing,
# without real spend or Redis seeding. Set CLOVER_DEV_CREDITS_FIXTURE to either a
...
def dev_fixture_credits_state() -> Optional[CreditsState]:
    """Return a fixture CreditsState for CLOVER_DEV_CREDITS_FIXTURE, or None.
    ...
    Hard prod-leak guard: a fixture applies ONLY when the dev flag CLOVER_DEV_CREDITS
    is also on, so a stray CLOVER_DEV_CREDITS_FIXTURE (leaked into a shell profile, a
    container env, a launch plist, …) can never surface fabricated balances/notices
    on a real account.
    """
    if not is_truthy_value(os.environ.get("CLOVER_DEV_CREDITS")):
        return None
    raw = os.environ.get("CLOVER_DEV_CREDITS_FIXTURE", "").strip()
    if not raw:
        return None
    name = raw
...
def _credits_state_from_account(info) -> Optional[CreditsState]:
    """Map a CloverPortalAccountInfo into a header-shaped CreditsState for the seed.
...
def seed_credits_at_session_start(agent) -> bool:
    ...
    """
    try:
        if getattr(agent, "provider", "") != "clover":
            return False
