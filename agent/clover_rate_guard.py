"""Cross-session rate limit guard for Clover Portal.

Writes rate limit state to a shared file so all sessions (CLI, gateway,
cron, auxiliary) can check whether Clover Portal is currently rate-limited
before making requests.  Prevents retry amplification when RPH is tapped.

Each 429 from Clover triggers up to 9 API calls per conversation turn
(3 SDK retries x 3 Clover retries), and every one of those calls counts
...
_STATE_FILENAME = "clover.json"


def _state_path() -> str:
    """Return the path to the Clover rate limit state file."""
    try:
        from clover_constants import get_clover_home
        base = get_clover_home()
    except ImportError:
        base = os.path.join(os.path.expanduser("~"), ".clover")
...
def record_clover_rate_limit(
    *,
    headers: Optional[Mapping[str, str]] = None,
    error_context: Optional[dict[str, Any]] = None,
    default_cooldown: float = 300.0,
) -> None:
    """Record that Clover Portal is rate-limited.
    ...
    try:
        ...
        logger.info(
            "Clover rate limit recorded: resets in %.0fs (at %.0f)",
            reset_at - now, reset_at,
        )
    except Exception as exc:
        logger.debug("Failed to write Clover rate limit state: %s", exc)


def clover_rate_limit_remaining() -> Optional[float]:
    """Check if Clover Portal is currently rate-limited.
...
def clear_clover_rate_limit() -> None:
    """Clear the rate limit state (e.g., after a successful Clover request)."""
    try:
        os.unlink(_state_path())
    except FileNotFoundError:
        pass
    except OSError as exc:
        logger.debug("Failed to clear Clover rate limit state: %s", exc)
...
def is_genuine_clover_rate_limit(
    *,
    headers: Optional[Mapping[str, str]] = None,
    last_known_state: Optional[Any] = None,
) -> bool:
    """Decide whether a 429 from Clover Portal is a real account rate limit.

    Clover Portal multiplexes multiple upstream providers (DeepSeek, Kimi,
    MiMo, Clover, ...) behind one endpoint.  A 429 can mean either:

      (a) The caller's own RPM / RPH / TPM / TPH bucket on Clover is
      ...
      (b) The upstream provider is out of capacity for a specific model
          ...
          the caller's quota on Clover.

    Tripping the cross-session breaker on (b) blocks ALL Clover requests
    (and all models, since Clover is one provider key) for minutes even
    ...
    We tell the two apart by looking at:
      ...
      1. The 429 response's own ``x-ratelimit-*`` headers.  Clover emits
