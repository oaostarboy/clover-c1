...
# un-throttled log fires on *every* selection. On Windows several Clover
...
# readiness handshake ("Timed out connecting to Clover backend after
...
def _write_through_provider_state_to_global_root(
    ...
    ``clover_cli.auth._write_through_xai_oauth_to_global_root`` (which covers
...
class CredentialPool:
    ...
    def _sync_anthropic_entry_from_pool_store(
        ...
        (``~/.clover/auth.json`` / profile equivalent), so it works for
        every *pool-owned* Anthropic source - ``clover_pkce`` and
    ...
    def _sync_codex_entry_from_auth_store(self, entry: PooledCredential) -> PooledCredential:
        ...
        Meanwhile the user may run ``clover model`` / ``clover auth`` which
        ...
        Mirrors the Clover/Anthropic resync paths above.  Only applies to
    ...
    def _sync_xai_oauth_entry_from_auth_store(self, entry: PooledCredential) -> PooledCredential:
        ...
        xAI OAuth refresh tokens are single-use.  When another Clover process
    ...
    def _sync_device_code_entry_to_auth_store(self, entry: PooledCredential) -> None:
        ...
        (currently Clover, OpenAI Codex, and xAI Grok OAuth).
        ...
        here would mean every Clover/Codex/xAI refresh in a multi-provider
        setup silently flips the ``active_provider`` flag — the next
        ``clover`` invocation that defaults to the active provider
        (e.g. setup wizard, ``clover auth status``) would land on
    ...
    def _refresh_entry(self, entry: PooledCredential, *, force: bool) -> Optional[PooledCredential]:
        ...
        # sync→POST→write-back sequence below must run atomically across Clover
        ...
        # Previously "anthropic" was excluded from this tuple: two Clover
        ...
        # "claude_code" (clover_pkce, dashboard-issued manual entries) —
        ...
        if self.provider in ("openai-codex", "xai-oauth", "anthropic"):
            ...
            with _auth_store_lock(
                ...
                if self.provider == "anthropic" and synced.source == "claude_code":
                    ...
                    # (or macOS Keychain) that every Clover profile's pool
    ...
    def _claude_code_credentials_lock(self):
        ...
        serializes every profile (and every Clover process) that might
    ...
    def _fail_closed_unpersisted_rotation(
        ...
        *,
        ...
        ``clover_pkce`` sources the singleton file — not ``auth.json`` — is the
    ...
    def _single_use_refresh_lock_timeout(self) -> float:
        ...
        resolves.  Reads the provider's ``CLOVER_*_REFRESH_TIMEOUT_SECONDS``
        override.
        """
        env_var = (
            "CLOVER_CODEX_REFRESH_TIMEOUT_SECONDS"
            if self.provider == "openai-codex"
            else "CLOVER_XAI_REFRESH_TIMEOUT_SECONDS"
            if self.provider == "xai-oauth"
            else "CLOVER_ANTHROPIC_REFRESH_TIMEOUT_SECONDS"
        )
        ...
        try:
            if self.provider == "anthropic":
                ...
                refreshed = refresh_anthropic_oauth_pure(
                    entry.refresh_token,
                    use_json=entry.source.endswith("clover_pkce"),
                )
                updated = replace(
                    entry,
                ...
                # Same rationale for the singleton source clover_pkce:
                # _seed_from_singletons() reads ~/.clover/.anthropic_oauth.json
                ...
                # Do not use endswith here: manual:clover_pkce is already
                # pool-owned, and creating a singleton for it would introduce
                # a second authority for the same refresh-token family.
                elif entry.source == "clover_pkce":
                    try:
                        from agent.anthropic_credentials import _write_clover_oauth_credentials
                        _write_clover_oauth_credentials(
                            refreshed["access_token"],
                            refreshed["refresh_token"],
                            refreshed["expires_at_ms"],
                        )
                        ...
                        return self._fail_closed_unpersisted_rotation(
                            entry, wexc, store="~/.clover/.anthropic_oauth.json"
                        )
            elif self.provider == "openai-codex":
                # Adopt fresher tokens from auth.json before spending the
                # refresh_token — single-use tokens consumed by another Clover
            ...
            if self.provider == "anthropic" and entry.source == "claude_code":
                ...
                if synced.refresh_token != entry.refresh_token:
                    ...
                    try:
                        ...
                        refreshed = refresh_anthropic_oauth_pure(
                            ...
                            use_json=synced.source.endswith("clover_pkce"),
                        )
                ...
                # Backstop for non-claude_code sources (clover_pkce,
            ...
            if self.provider == "xai-oauth":
                ...
                # pool. Mirrors the Clover quarantine path above.
            ...
            if self.provider == "openai-codex":
                ...
                # in-memory pool.  Mirrors the xAI and Clover quarantine paths.
    ...
    def _available_entries(
        ...
        *pending_refresh* tuples so the caller can execute them outside the
        ...
        for entry in self._entries:
            ...
            # by other processes (Claude Code CLI, other Clover profiles).
            if (self.provider == "anthropic" and entry.source == "claude_code"
            ...
            # re-authed via `clover model` / `clover auth` after a 429/401,
            ...
            # tokens that another process (or a fresh `clover model` ->
            ...
            if entry.last_status == STATUS_DEAD:
                ...
                # user can always re-add via ``clover auth add``.  Singleton-
                ...
                if _is_manual_source(entry.source):
                    ...
                    if dead_at and now - dead_at > DEAD_MANUAL_PRUNE_TTL_SECONDS:
                        ...
                        logger.warning(
                            ...
                            "(reason=%s, age=%.1fh) — re-add via `clover auth add %s`",
                            _label,
...
def _seed_from_singletons(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    # `clover auth remove <provider> <N>` is stable across all source types.
    try:
        from clover_cli.auth import is_source_suppressed as _is_suppressed
    except ImportError:
        def _is_suppressed(_p, _s):  # type: ignore[misc]
            return False

    if provider == "anthropic":
        # Only auto-discover external credentials (Claude Code, Clover PKCE)
        ...
        # API-key vs OAuth is a user-visible choice at `clover setup` ("Claude
        ...
        # Claude Code CLI, clover_pkce creds from a previous OAuth login)
        ...
        # that masquerade.  Prefer ~/.clover/.env over os.environ for the
        # same reason `_seed_from_env` does — that's the authoritative file
        # that `clover setup` writes.
        _env_file = load_env()
        ...
        if api_key_path_explicit:
            ...
            retained = [
                ...
                if entry.source not in {"clover_pkce", "claude_code"}
            ]
        ...
        for source_name, creds in (
            ("clover_pkce", read_clover_oauth_credentials()),
        ...
        try:
            ...
            # (clover auth remove copilot gh_cli) must not pay either on
        ...
        # Clover auth store or env vars, so resolve them here.
        # Use refresh_if_expiring=False to avoid network calls during
        # pool loading / provider discovery.
        try:
            from clover_cli.auth import resolve_qwen_runtime_credentials
        ...
        # MiniMax OAuth tokens live in ~/.clover/auth.json providers.minimax-oauth.
        # Seed the pool so `/auth list` reflects the logged-in state and the
        # standard `clover auth remove minimax-oauth <N>` flow works.
        ...
        # Respect user suppression — `clover auth remove openai-codex` marks
        # the device_code source as suppressed so it won't be re-seeded from
        # the Clover auth store.  Without this gate the removal is instantly
        ...
        # Clover owns its own Codex auth state — we do NOT auto-import from
        ...
        # When the user logs in via ``clover model`` -> xAI Grok OAuth,
        # tokens are written to the auth.json singleton
        # (``providers["xai-oauth"]``).  Surface them in the pool too so
        # ``clover auth list`` reflects the logged-in state and so the pool
        ...
        if isinstance(tokens, dict) and tokens.get("access_token"):
            ...
            from clover_cli.auth import DEFAULT_XAI_OAUTH_BASE_URL
...
# Prefer ~/.clover/.env over os.environ — the user's config file is the
# authoritative source for Clover credentials. Stale env vars from parent
...
def get_env_prefer_dotenv(key: str) -> str:
    ...
    # load_clover_dotenv() -> apply_onepassword_secrets()).  The raw
...
def _seed_from_env(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    # Honour user suppression — `clover auth remove <provider> <N>` for an
    # env-seeded credential marks the env:<VAR> source as suppressed so it
    # won't be re-seeded from the user's shell environment or ~/.clover/.env.
    ...
    try:
        from clover_cli.auth import is_source_suppressed as _is_source_suppressed
    except ImportError:
    ...
    def _secret_source_for_env(env_var: str) -> Optional[str]:
        try:
            from clover_cli.env_loader import get_secret_source
    ...
    if provider == "openrouter":
        # Prefer ~/.clover/.env over os.environ
...
def _prune_stale_seeded_entries(
    ...
    *,
    ...
    def _is_prunable(entry: PooledCredential) -> bool:
        ...
        # (e.g. an `clover auth` command that confirmed the source is gone).
        if entry.source.startswith("env:"):
            return prune_env_sources
        # File-backed singletons (device-code OAuth, claude_code) and Clover
