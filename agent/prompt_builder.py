...
_CLOVER_MD_NAMES = (".clover.md", "CLOVER.md")


def _find_clover_md(cwd: Path) -> Optional[Path]:
    """Discover the nearest ``.clover.md`` or ``CLOVER.md``.
    ...
    # could pick up a .clover.md planted in /tmp, /home, etc.
    search_dirs = [current, *current.parents] if stop_at else [current]

    for directory in search_dirs:
        for name in _CLOVER_MD_NAMES:
...
DEFAULT_AGENT_IDENTITY = (
    "You are Clover Cognition, an intelligent AI assistant created by Anthony Nguyen. "
...
CLOVER_AGENT_HELP_GUIDANCE = (
    "You run on Clover Cognition (by Anthony Nguyen). When the user needs help with "
...
KANBAN_GUIDANCE = (
    ...
    "the shared board at `~/.clover/kanban.db`. Your task id is in "
    "`$CLOVER_KANBAN_TASK`; your workspace is `$CLOVER_KANBAN_WORKSPACE`. "
    ...
    "ground truth.\n"
    "2. **Work inside the workspace.** `cd $CLOVER_KANBAN_WORKSPACE` before "
    ...
    "\n"
    "- **Workspace.** `cd $CLOVER_KANBAN_WORKSPACE` first. For a `worktree` kind "
    "with no `.git`, `git worktree add <path> "
    "${CLOVER_KANBAN_BRANCH:-wt/$CLOVER_KANBAN_TASK}` from the main repo, then "
    "cd there. For a project-linked task the workspace is a fresh "
    "`<repo>/.worktrees/<task-id>` and `$CLOVER_KANBAN_BRANCH` a deterministic "
    ...
    "every assignee in a real profile (`clover profile list`, or ask the user), "
    "and express dependencies via `parents=[...]` on `kanban_create`, not prose.\n"
    "\n"
    "## Do NOT\n"
    "\n"
    "- Do not shell out to `clover kanban <verb>` for board operations. Use "
...
#
# The clover-c1 runtime already executes a batch of tool calls
...
def computer_use_guidance(platform_name: Optional[str] = None) -> str:
    ...
    return (
        ...
        "explicit Clover YOLO uses an unrestricted runtime after the user's "
        "launch/session risk acceptance. Permission mode and grants are fixed "
        "when Clover launches that runtime.\n\n"
        ...
        "run `clover computer-use doctor` and share the output. That command "
...
STEER_CHANNEL_NOTE = (
    ...
    "While you work, the user can send an out-of-band message that Clover "
...
def hud_surface_note(valid_tool_names: "set[str] | None" = None) -> str:
    ...
    HUD mode is a strip of Clover floating over another application, so the
    user is rarely asking about Clover — they are asking about the thing behind
    ...
    sentences = [
        "[Note: this message came from HUD mode — a small floating Clover "
        ...
        "rather than anything inside Clover. read_window_below identifies "
        "that app.",
...
PLATFORM_HINTS = {
    ...
    "tui": (
        "You are running in the Clover terminal UI (TUI). "
        "Cron jobs scheduled from this session are LOCAL-ONLY: their output is "
        "saved (viewable via cronjob action='list') but is NOT delivered back "
    ...
    "desktop": (
        "You are chatting inside the Clover desktop app — a graphical chat "
        ...
        "Widgets can talk back: window.clover.send(\"prompt\") — or a "
        "data-clover-send=\"prompt\" attribute on any clickable element — sends "
    ...
    "webui": (
        "You are in the Clover WebUI, a browser-based chat interface. "
...
# container / remote host rather than on the machine where Clover itself
...
def _probe_remote_backend(env_type: str) -> str | None:
    ...
    operate on a different machine than the host Clover runs on.
    """
...
def build_environment_hints() -> str:
    ...
    if not is_remote_backend:
        ...
        if probe:
            hints.append(
                ...
                f"where Clover itself is running. The host OS, home, and cwd "
                f"of the Clover process are irrelevant; only the following "
            ...
            hints.append(
                f"Terminal backend: {backend}. Your `terminal`, `read_file`, "
                f"`write_file`, `patch`, and `search_files` tools all operate "
                f"inside {description} — NOT on the machine where Clover "
    ...
    # Embedder-supplied environment description. Lets a host that wraps Clover
    ...
    extra = (os.getenv("CLOVER_ENVIRONMENT_HINT") or "").strip()
    if not extra:
        try:
...
def _skills_prompt_snapshot_path() -> Path:
    return get_clover_home() / ".skills_prompt_snapshot.json"
...
def _current_session_platform_hint() -> str:
    ...
    platform = os.environ.get("CLOVER_PLATFORM") or os.environ.get("CLOVER_SESSION_PLATFORM")
    if platform:
        return platform
...
def build_skills_system_prompt(
    ...
    scanned alongside the local ``~/.clover/skills/`` directory.  External dirs
    ...
    # CLOVER_HOME would otherwise fall back to the launch (default) home and
    ...
    if skills_dir_override is not None:
        ...
        _home_token = set_clover_home_override(str(skills_dir.parent))
    else:
        skills_dir = get_skills_dir()
        _home_token = None
    try:
        external_dirs = get_all_skills_dirs()[1:]  # skip local (index 0)
        # Trusted project-local dirs (./.clover/skills, ./.agents/skills at
        ...
        if _home_token is not None:
            reset_clover_home_override(_home_token)
...
def _build_skills_system_prompt_inner(
    ...
    if not skills_by_category:
        ...
        result = (
            ...
            "or troubleshoot Clover Cognition itself — its CLI, config, models, providers, tools, "
            "skills, voice, gateway, plugins, or any feature — load the `clover-c1` skill "
            "first. It has the actual commands (e.g. `clover config set …`, `clover tools`, "
            "`clover setup`) so you don't have to guess or invent workarounds.\n"
...
def build_clover_subscription_prompt(valid_tool_names: "set[str] | None" = None) -> str:
    ...
    lines = [
        "# Clover Subscription",
        "Clover subscription includes managed web tools (Firecrawl), image generation (FAL), OpenAI TTS, OpenAI Whisper STT, and browser automation (Browser Use) by default. Modal execution is optional.",
        "Current capability status:",
    ]
    lines.extend(_status_line(feature) for feature in features.items())
    lines.extend(
        [
            "When a Clover-managed feature is active, do not ask the user for Firecrawl, FAL, OpenAI TTS, OpenAI Whisper, or Browser-Use API keys.",
            "If the user is not subscribed and asks for a capability that Clover subscription would unlock or simplify, suggest Clover subscription as one option alongside direct setup or local alternatives.",
            "Do not mention subscription unless the user asks about it or it directly solves the current missing capability.",
            "Useful commands: clover setup, clover setup tools, clover setup terminal, clover status.",
        ]
    )
    return "\n".join(lines)
...
def load_soul_md(
    ...
    """Load SOUL.md from CLOVER_HOME and return its content, or None.
    ...
    ambient — which on a thread that lost the CLOVER_HOME ContextVar falls
    back to the launch home and reads the wrong profile's SOUL.md (#50233,
    same class as the skills-index leak fixed in #86313).
    """
    try:
        from clover_cli.config import ensure_clover_home
        ensure_clover_home()
    except Exception as e:
        logger.debug("Could not ensure CLOVER_HOME before loading SOUL.md: %s", e)

    _home = Path(home_override) if home_override is not None else get_clover_home()
...
def _load_clover_md(cwd_path: Path, context_length: Optional[int] = None) -> str:
    """.clover.md / CLOVER.md — walk to git root."""
    clover_md_path = _find_clover_md(cwd_path)
    if not clover_md_path:
        return ""
    try:
        content = clover_md_path.read_text(encoding="utf-8").strip()
        if not content:
            return ""
        content = _strip_yaml_frontmatter(content)
        rel = clover_md_path.name
        try:
            rel = str(clover_md_path.relative_to(cwd_path))
        except ValueError:
            pass
        content = _scan_context_content(content, rel)
        result = f"## {rel}\n\n{content}"
        return _truncate_content(
            result, ".clover.md", context_length=context_length,
            read_path=str(clover_md_path),
        )
    except Exception as e:
        logger.debug("Could not read %s: %s", clover_md_path, e)
        return ""
...
def build_context_files_prompt(
    ...
    Priority (first found wins — only ONE project context type is loaded):
      1. .clover.md / CLOVER.md  (walk to git root)
      2. AGENTS.md / agents.md   (merged chain: git root → cwd)
      3. CLAUDE.md / claude.md   (cwd only)
      4. .cursorrules / .cursor/rules/*.mdc  (cwd only)

    SOUL.md from CLOVER_HOME is independent and always included when present.
    ...
    # Never let a FALLBACK-picked directory inside the Clover install/source
    ...
    # explicitly configured cwd is honored verbatim — the Clover tree is a
    ...
    # their launch dir IS the user's shell cwd (developing Clover in-tree).
    ...
    if (
        ...
        logger.warning(
            ...
            "fell back to the Clover install tree (%s) — set terminal.cwd to "
        ...
        project_context = (
            _load_clover_md(cwd_path, context_length)
    ...
    # SOUL.md from CLOVER_HOME only — skip when already loaded as identity
    if not skip_soul:
