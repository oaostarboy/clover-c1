...
Plugins built on Clover Cognition often need to make their own LLM calls
...
Today the only stable plugin surfaces extend an existing Clover
...
:class:`~clover_cli.plugins.PluginContext`:
...
embedded slugs, no shorthands. This mirrors Clover' main config
...
handles every provider, fallback chain, and per-task override Clover
supports.
"""
...
class PluginLlm:
    ...
    Instances are constructed by :class:`clover_cli.plugins.PluginContext`
