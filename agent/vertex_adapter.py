"""Vertex AI (Google Cloud) adapter for Clover Cognition.

Provides authentication and configuration for Vertex AI's OpenAI-compatible
endpoint. This allows Clover to use Gemini models via Google Cloud with
...
# who installed plain `clover-c1` and only later selected a Gemini model.
try:
...
def has_explicit_vertex_config() -> bool:
    """True only when the user deliberately pointed Clover at Vertex.
    ...
    Clover up for Vertex would suddenly see it (and could spend against those
    credentials). So this checks only Clover-scoped signals:
      ...
      * a resolvable ``VERTEX_CREDENTIALS_PATH`` service-account file
        (the Clover-specific path var — NOT ``GOOGLE_APPLICATION_CREDENTIALS``).
    """
    if _resolve_project_override():
        return True
