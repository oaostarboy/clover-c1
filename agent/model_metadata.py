...
def _resolve_requests_verify(base_url: str = "") -> bool | str:
    ...
    3. Env vars ``CLOVER_CA_BUNDLE`` / ``REQUESTS_CA_BUNDLE`` / ``SSL_CERT_FILE``
    ...
    for env_var in ("CLOVER_CA_BUNDLE", "REQUESTS_CA_BUNDLE", "SSL_CERT_FILE"):
...
def _local_probe_disk_cache_path() -> Path:
    ...
    return get_clover_home() / "cache" / "local_endpoint_probes.json"
...
def _get_model_metadata_cache_path() -> Path:
    ...
    from clover_constants import get_clover_home
    return get_clover_home() / "cache" / "openrouter_model_metadata.json"
...
# Minimum context length required to run Clover Cognition.  Models with fewer
...
DEFAULT_CONTEXT_LENGTHS = {
    ...
    "qwen3.8-max": 1_000_000,     # 1M context (OpenRouter & Clover portal, verified 2026-08-03)
    "qwen3.8-flash": 1_000_000,   # 1M context (OpenRouter & Clover portal, verified 2026-08-28)
    ...
    # (native MiniMax-M3, OpenRouter/Clover minimax/minimax-m3).
    ...
    # so these hardcoded fallbacks prevent Clover from probing-down to
...
_URL_TO_PROVIDER: Dict[str, str] = {
    ...
    # (often 8K) makes it unusable for Clover' system prompt, but mapping
...
def _maybe_cache_local_context_length(
    ...
    """Persist a locally probed context length only when it meets Clover minimum.
...
def _resolve_endpoint_context_length(
    ...
    if not matched:
        if len(endpoint_metadata) == 1:
            ...
            # a 32K embedding model on the Clover portal) and poison the
...
def _get_context_cache_path() -> Path:
    ...
    from clover_constants import get_clover_home
    return get_clover_home() / "context_length_cache.yaml"
...
def parse_available_output_tokens_from_error(error_msg: str) -> Optional[int]:
    ...
    is_output_cap_error = (
        ...
        # OpenRouter/Clover phrasing of the same condition.
    ...
    # OpenRouter/Clover format: "maximum context length is N … (A of text input,
...
def is_output_cap_error(error_msg: str) -> bool:
    ...
    output_cap_signal = (
        ...
        or ("in the output" in error_lower                  # OpenRouter / Clover
            and "maximum context length" in error_lower)
        ...
        with httpx.Client(timeout=3.0, headers=headers) as client:
            ...
            if server_type == "ollama":
                ...
                if resp.status_code == 200:
                    ...
                    # Clover grow conversations past the runtime limit and Ollama
...
def _normalize_model_version(model: str) -> str:
    ...
    Clover uses dashes: claude-opus-4-6, claude-sonnet-4-5
...
# who never asked for it. The variant suffix is a Clover-side alias: it is
...
# Clover-side picker suffix that opts a Codex slug into the live-verified
...
def strip_codex_context_variant_suffix(model: Optional[str]) -> str:
    ...
    The suffix is a Clover picker alias (``gpt-5.6-sol-900k``); the Codex
...
def _resolve_codex_oauth_context_length_with_source(
    ...
    # ``-900k`` variants are Clover picker aliases — the Codex catalog only
...
def _resolve_clover_context_length(
    ...
    """Resolve Clover Portal model context length.

    Tries the live Clover inference endpoint first (authoritative), then falls
    back to OpenRouter metadata with suffix/version matching.

    Clover model IDs are bare after prefix-stripping (e.g. 'qwen3.6-plus',
    ...
    Returns ``(context_length, source)`` where ``source`` is one of:
      ...
      - ``""``           — could not resolve
    """
    # Portal first — the Clover /models endpoint is authoritative for what our
    ...
    def _safe_ctx(or_id: str, entry: dict) -> Optional[int]:
        ...
        resolve_context_length) so the Clover portal path does not short-circuit it.
        """
        ...
        if ctx <= 32768 and _model_name_suggests_stale_32k_underreport(or_id):
            logger.info(
                ...
                "(known 32K underreport, Clover path); falling through to hardcoded defaults",
                ctx, or_id,
            )
            return None
        return ctx
...
def get_model_context_length(
    ...
    1. Persistent cache (previously discovered via probing).  Clover URLs,
    ...
    5. Provider-aware lookups (before generic OpenRouter cache):
       ...
       b. Clover: live /v1/models probe first (authoritative), then OR
    ...
    if base_url and not _skip_persistent_context_cache(base_url, provider):
        ...
        if cached is not None:
            ...
            # Drop the stale entry and fall through to the hardcoded default.
                ...
                _invalidate_cached_context_length(model, base_url)
            # Clover Portal: the portal /v1/models endpoint is authoritative.
            ...
            elif _infer_provider_from_url(base_url) == "clover":
                logger.debug(
                    "Bypassing persistent cache for %s@%s (Clover portal authoritative)",
                    model, base_url,
                )
    ...
    if effective_provider in {"copilot", "copilot-acp", "github-copilot"}:
        try:
            from clover_cli.models import get_copilot_model_context
    ...
    # the dedicated Clover/Copilot/GMI branches above.
    ...
    # ``Clover-3-Llama-3.1-70B`` substring-match ``llama`` (131072) even when
...
def estimate_request_tokens_rough(
    ...
    *,
    ...
    Includes the major payload buckets Clover sends to providers:
