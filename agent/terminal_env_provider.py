...
Providers live in ``~/.clover/plugins/<name>/`` (user, opt-in via
...
* ``cache_path_base`` — where auto-synced ``~/.clover/cache`` files land
  inside the backend (e.g. ``"~/.clover"`` for home-synced backends,
  ``"/root/.clover"`` for root-homed containers), or ``None`` when host
...
The factory stamps ``_clover_backend_name`` on the returned object so
...
class TerminalEnvironmentProvider(abc.ABC):
    ...
    def cache_path_base(self) -> Optional[str]:
        """Base dir for synced Clover cache files inside the backend."""
        return None
    ...
    def setup_instructions(self) -> List[str]:
        """Lines printed by ``clover setup`` after this backend is selected.
