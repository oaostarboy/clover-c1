...
3. ``~/.clover/.anthropic_oauth.json`` (Clover PKCE login)
...
class CredentialPersistError(RuntimeError):
    ...
    ``~/.clover/.anthropic_oauth.json`` for ``clover_pkce``).
...
#     authority boundary of ``claude_code``/``clover_pkce`` is the shared
#     singleton file, which other Clover processes/profiles read with fresh
...
def _spent_rotation_sidecar_path(source_path: Path) -> Path:
    ...
    return source_path.with_name(source_path.name + ".clover-spent-rotations.json")
...
def spent_rotation_source_path(source: Any) -> Optional[Path]:
    ...
    if source == "clover_pkce":
        return _get_clover_oauth_file()
    return None
        ...
        payload = json.dumps(
            {
                ...
                "comment": (
                    ...
                    "never durably committed. Written by Clover so sibling "
...
def claude_code_credentials_path() -> Path:
    ...
    This file is not profile-owned: every Clover profile's credential pool
...
def _refresh_oauth_token(creds: Dict[str, Any]) -> Optional[str]:
    ...
    try:
        from clover_cli.auth import AUTH_LOCK_TIMEOUT_SECONDS, _auth_store_lock, env_float

        refresh_timeout_seconds = env_float(
            "CLOVER_ANTHROPIC_REFRESH_TIMEOUT_SECONDS", 20
        )
...
def _write_clover_oauth_credentials(
    access_token: str,
    refresh_token: Optional[str],
    expires_at_ms: Optional[int],
) -> None:
    """Write refreshed clover_pkce tokens back to ~/.clover/.anthropic_oauth.json.

    Without this, a successful pool-level refresh of a ``clover_pkce``-sourced
    ...
    try:
        ...
        logger.error(
            "Failed to write refreshed Clover OAuth credentials to %s: %s", oauth_file, e
        )
