...
# (Clover Portal → OpenAI → OpenRouter → …). OpenRouter/Clover run a quality-first
...
_REF_CAPABLE = ("clover", "openai", "openai-codex", "openrouter", "krea")
...
def _forced_provider_from_env() -> str | None:
    ...
    `CLOVER_PET_IMAGE_PROVIDER=<name>` (e.g. `openrouter`) bypasses the normal
    active/default provider resolution for pet generation only. Unknown values are
    ignored so existing users are unaffected.
    """
    forced = os.environ.get("CLOVER_PET_IMAGE_PROVIDER", "").strip().lower()
...
def resolve_provider(*, require_references: bool = True, prefer: str | None = None) -> SpriteProvider:
    ...
    raise GenerationError(
        ...
        "Open `clover tools` → Image Generation and configure Clover Portal, "
...
def generate(
    ...
    *aspect_ratio* picks the canvas: ``"square"`` for single-character base
    ...
    def _run(extra: dict) -> tuple[Path | None, str]:
        ...
        if refs:
            # Providers disagree on the ref kwarg name: our OpenRouter/Clover
