...
row taxonomy is inferred from the atlas shape so Clover can render both legacy
...
# rows (1536x1872), while older Clover/petdex sheets used 9 columns x 8 rows
...
class PetState(str, Enum):
    ...
    These are Clover' activity state names. They are not always identical to the
...
# Legacy Clover/petdex row order (top -> bottom) used by the older 8-row,
...
# Canonical Clover activity names -> accepted row-name aliases in descending
