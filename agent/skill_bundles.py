...
Storage
-------
Bundles live in ``~/.clover/skill-bundles/*.yaml`` (and the equivalent
profile-aware directory under ``CLOVER_HOME``). Each file looks like::
...
- :func:`list_bundles` — return rich info for display (``clover bundles``)
...
def _bundles_dir() -> Path:
    """Return the canonical bundles directory under CLOVER_HOME.

    Honors ``CLOVER_BUNDLES_DIR`` for tests; falls back to
    ``<CLOVER_HOME>/skill-bundles``.
    """
    override = os.environ.get("CLOVER_BUNDLES_DIR")
    if override:
        return Path(override).expanduser()
    return get_clover_home() / "skill-bundles"
...
# ---------------------------------------------------------------------------
# File-level CRUD helpers — used by `clover bundles` CLI subcommand.
# ---------------------------------------------------------------------------
