...
_AD_HOC_SCRIPT_NAME_PREFIXES = ("clover-verify-", "clover-ad-hoc-")
...
def _db_path() -> Path:
    return get_clover_home() / "verification_evidence.db"
...
def record_verify_run(
    *,
    ...
    session_id: str | None = None,
    ok: bool,
    command: str = "clover verify",
    scope: str = "full",
    output: str = "",
) -> Optional[dict[str, Any]]:
    """Record a completed ``clover verify`` run as verification evidence.

    Explicit CLI-side write: unlike :func:`record_terminal_result` there is
    nothing to classify — the caller (the ``clover verify`` command) already
    ...
    evidence = VerificationEvidence(
        command=command,
        canonical_command="clover verify",
        kind="verify",
