...
- ``set_secret_scope(mapping)`` installs the active profile's secrets for the
  ...
  via ``copy_context()`` exactly like the CLOVER_HOME override).
...
_GLOBAL_ENV_EXACT = frozenset({
    # Clover runtime / deployment
    "CLOVER_HOME", "CLOVER_PROFILE", "CLOVER_GATEWAY_LOCK_DIR",
    "CLOVER_MAX_ITERATIONS", "CLOVER_MAX_TOKENS", "CLOVER_API_TIMEOUT",
    "CLOVER_REDACT_SECRETS", "CLOVER_PORTAL_TIMEOUT_SECONDS",
    "_CLOVER_GATEWAY",
    # OS / interpreter
    "PATH", "HOME", "USER", "LANG", "LC_ALL", "TZ", "PWD", "SHELL", "TMPDIR",
    "VIRTUAL_ENV", "PYTHONPATH", "SSL_CERT_FILE",
    # Kanban paths (per-board, not per-profile-secret)
    "CLOVER_KANBAN_DB", "CLOVER_KANBAN_WORKSPACES_ROOT", "CLOVER_KANBAN_BOARD",
...
_GLOBAL_ENV_PREFIXES = (
    "CLOVER_KANBAN_",
    "CLOVER_TELEGRAM_",   # tuning knobs (batch delays, fallback toggles) — NOT the token
...
def load_env_file(env_path: Path) -> Dict[str, str]:
    ...
    ``set_secret_scope``. Parses the small KEY=VALUE subset Clover writes
    ...
    ``clover_cli.config._parse_env_value``) but never mutates the process
    ...
    # Parse values with the canonical Clover parser: save_env_value
    ...
    from clover_cli.config import _parse_env_value
...
def build_profile_secret_scope(clover_home: Path) -> Dict[str, str]:
    ...
    """
    home = Path(clover_home)
    secrets = load_env_file(home / ".env")

    try:
        from clover_cli.env_loader import get_secret_source_values
