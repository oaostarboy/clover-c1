...
Resolution order for text tasks (auto mode):
  ...
  3. Clover Portal (~/.clover/auth.json active provider)
...
from clover_constants import OPENROUTER_BASE_URL
...
def _resolve_aux_verify(base_url: Optional[str]) -> Any:
    ...
    ``ssl_ca_cert`` / ``ssl_verify`` config and the ``CLOVER_CA_BUNDLE`` /
    ...
    try:
        ...
        if not _WARNED_KEEPALIVE_IMPORT_SKEW:
            ...
            logger.warning(
                ...
                "back to the SDK default HTTP client. Run `clover update` (or "
...
def _create_openai_client(*, api_key: str, base_url: str, **kwargs: Any) -> Any:
    ...
    # Clover owns auxiliary retry + provider/model fallback policy (the
    ...
    # Clover sees a single failure (issue #54465). Disable SDK-internal retries
    # by default and let Clover control the budget; explicit callers can still
    # override via kwargs.
...
def _run_protected_sync_provider_call(
    ...
    threading.Thread(
        ...
        name="clover-protected-aux-provider",
        daemon=True,
    ).start()
...
def _compression_threshold_for_model(
    ...
    *,
    ...
    consumed before Clover triggers summarization.  Higher values delay
...
# "google/gemini-3-flash" kept 404ing here once Clover dropped it upstream, and
...
#
# Rolling "-latest" aliases come first where a provider publishes them (Clover
...
# Clover catalog: gpt-mini-latest 1.40s, claude-haiku-latest 1.55s,
...
def _fast_model_from_catalog(provider_id: str) -> str:
    ...
    try:
        ...
        from clover_cli.models import fetch_models_with_pricing
...
def _get_aux_model_for_provider(provider_id: str, *, prefer_fast: bool = False) -> str:
    ...
    2. ``prefer_fast`` only — the provider's own recommendation hook
       ...
       on long-context side tasks (Clover returns its compaction pick), so it
...
def build_or_headers(or_config: dict | None = None) -> dict:
    ...
    Environment variables:
        ``CLOVER_OPENROUTER_CACHE`` — truthy (``1``/``true``/``yes``/``on``)
            enables caching; ``0``/``false``/``no``/``off`` disables.
            Overrides ``openrouter.response_cache`` in config.yaml.
        ``CLOVER_OPENROUTER_CACHE_TTL`` — integer seconds (1-86400).
    ...
    *or_config* is the ``openrouter`` section from config.yaml.  When *None*,
    ...
    env_cache = os.environ.get("CLOVER_OPENROUTER_CACHE", "").strip().lower()
    if env_cache:
    ...
    env_ttl = os.environ.get("CLOVER_OPENROUTER_CACHE_TTL", "").strip()
    if env_ttl:
        if env_ttl.isdigit():
            ttl = int(env_ttl)
            if 1 <= ttl <= 86400:
...
_NVIDIA_NIM_CLOUD_HEADERS = {
    "X-BILLING-INVOKE-ORIGIN": "CloverAgent",
}
...
# Clover Portal extra_body for product attribution.
# Callers should pass this as extra_body in chat.completions.create()
# when the auxiliary client is backed by Clover Portal.
#
...
auxiliary_is_clover: bool = False
...
def _codex_cloudflare_headers(
    ...
    the official endpoint always send Clover' originator and version. Custom
    ...
    """
    headers = {
        "User-Agent": "codex_cli_rs/0.0.0 (Clover Cognition)",
    ...
    if _is_official_codex_base_url(base_url):
        ...
        headers.update({
            "User-Agent": f"CloverAgent/{__version__}",
            "originator": "clover-c1",
        })
...
class _CodexCompletionsAdapter:
    ...
    def create(self, **kwargs) -> Any:
        ...
        resp_kwargs: Dict[str, Any] = {
            # Strip the Clover-side ``-900k`` large-context picker suffix —
...
class _AnthropicCompletionsAdapter:
    ...
    def __init__(
        ...
        # client's host for Clover Portal — a blanket fallback would flip
    ...
    def create(self, **kwargs) -> Any:
        ...
        #   - ``_``-prefixed keys: private Clover plumbing (_reasoning_config
...
def _endpoint_speaks_anthropic_messages(base_url: str) -> bool:
    ...
    Mirrors ``clover_cli.runtime_provider._detect_api_mode_for_url`` so the
...
def _read_clover_auth() -> Optional[dict]:
    ...
    otherwise None.
    """
    pool_present, entry = _select_pool_entry("clover")
    if pool_present:
        if entry is None:
            return None
        return {
    ...
    try:
        ...
        if data.get("active_provider") != "clover":
            return None
        provider = data.get("providers", {}).get("clover", {})
        # Must have at least an access_token or agent_key
        if not provider.get("agent_key") and not provider.get("access_token"):
            return None
        return provider
    except Exception as exc:
        logger.debug("Could not read Clover auth: %s", exc)
        return None
...
def _resolve_clover_pool_runtime_api(*, force_refresh: bool = False) -> Optional[tuple[str, str]]:
    ...
    try:
        ...
        logger.debug("Auxiliary Clover pool credential resolution failed: %s", exc)
        return None
    ...
    try:
        ...
        logger.debug("Auxiliary Clover pool selection failed: %s", exc)
        return None
    ...
    if force_refresh or not _agent_key_is_usable(state, _clover_min_key_ttl_seconds()):
        try:
            ...
            logger.debug("Auxiliary Clover pool refresh failed: %s", exc)
            refreshed = None
...
def _resolve_clover_runtime_api(*, force_refresh: bool = False) -> Optional[tuple[str, str]]:
    ...
    try:
        ...
        logger.debug("Auxiliary Clover runtime credential resolution failed: %s", exc)
        return None
...
def _resolve_xai_oauth_for_aux() -> Optional[Tuple[str, str]]:
    ...
    compression report "no provider configured" even though ``clover auth
    status`` shows xAI OAuth as logged in.

    Falls back to ``clover_cli.auth``'s singleton runtime resolver for older
    ...
    try:
        from clover_cli.auth import resolve_xai_oauth_runtime_credentials
...
def _read_codex_access_token() -> Optional[str]:
    """Read a valid, non-expired Codex OAuth access token from Clover auth store.
    ...
    try:
        from clover_cli.auth import _read_codex_tokens
...
def _resolve_api_key_provider() -> Tuple[Optional[OpenAI], Optional[str]]:
    ...
    """
    try:
        from clover_cli.auth import PROVIDER_REGISTRY, resolve_api_key_provider_credentials
    except ImportError:
    ...
    for provider_id, pconfig in PROVIDER_REGISTRY.items():
        ...
        if provider_id == "anthropic":
            ...
            try:
                from clover_cli.auth import is_provider_explicitly_configured
        ...
        if pool_present:
            ...
            if base_url_host_matches(base_url, "api.kimi.com"):
                ...
                from clover_cli.models import copilot_default_headers
...
def _try_clover(vision: bool = False) -> Tuple[Optional[OpenAI], Optional[str]]:
    ...
    try:
        ...
        if _remaining is not None and _remaining > 0:
            ...
            _mark_provider_unhealthy("clover", ttl=_remaining)
            return None, None
    except Exception:
        pass
    ...
    global auxiliary_is_clover
    auxiliary_is_clover = True
...
def _refresh_clover_recommended_model(
    *, vision: bool, stale_model: Optional[str]
    ...
    the Clover → OpenRouter catalog, every auxiliary call 404s with
...
def _resolve_moa_aggregator(preset_name: Optional[str]) -> Tuple[Optional[str], Optional[str]]:
    ...
    try:
        from clover_cli.config import load_config
        from clover_cli.moa_config import resolve_moa_preset
...
def _validate_base_url(base_url: str) -> None:
    ...
    try:
        ...
        raise RuntimeError(
            ...
            "Run `clover setup` or `clover model` and enter a valid http(s) base URL."
        ) from exc
...
def _build_xai_oauth_aux_client(model: str) -> Tuple[Optional[Any], Optional[str]]:
    ...
    from tools.xai_http import clover_xai_default_headers

    real_client = _create_openai_client(
        api_key=api_key,
        base_url=base_url,
        default_headers=clover_xai_default_headers(),
    )
    return CodexAuxiliaryClient(real_client, model), model
...
def _try_azure_foundry(
    ...
    * Non-default ``model.base_url`` overrides are honored.
    ...
    """
    try:
        from clover_cli.runtime_provider import _resolve_azure_foundry_runtime
...
# manual intervention.
#
# Failure isolation: the cache is in-process only. A second clover
...
def _reset_aux_unhealthy_cache() -> None:
    ...
    user trigger (e.g. ``clover config aux reset``)."""
...
def _is_model_not_found_error(exc: Exception) -> bool:
    ...
    that has since been dropped from the Clover → OpenRouter catalog. The Clover
...
def _auth_refresh_provider_for_route(
    ...
    Copilot/Codex/Anthropic/Clover routes too. (#20832)
    """
    normalized = _normalize_aux_provider(resolved_provider)
...
def _fallback_destination(
    ...
    attached = getattr(fb_client, "_clover_fallback_destination", None)
...
def _try_payment_fallback(
    ...
    _alias_to_label = {"openrouter": "openrouter", "clover": "clover",
...
def _fallback_entry_api_key(entry: Dict[str, Any]) -> Optional[str]:
    ...
    gateway multiplexing (see ``clover_cli.fallback_config.resolve_entry_api_key``).
    """
...
def _resolve_fallback_entry(entry: Dict[str, Any]) -> Tuple[Optional[Any], Optional[str]]:
    ...
    if client is not None:
        try:
            client._clover_fallback_destination = _fallback_destination_from_entry(
...
def _try_main_fallback_chain(
    ...
    main fallback policy before dropping into Clover' built-in discovery
...
def _resolve_auto_route(
    ...
    Priority:
      1. User's main provider + main model, regardless of provider type.
         ...
         chat.  Users on OpenRouter/Clover get their chosen chat model; users
    ...
    """
    global auxiliary_is_clover, _stale_base_url_warned
    auxiliary_is_clover = False  # Reset — _try_clover() will set True if it wins
    runtime = _normalize_main_runtime(main_runtime)
    ...
    #    scenario where a user switches providers via `clover model` but the
    #    old OPENAI_BASE_URL lingers in ~/.clover/.env. ──
    if not _stale_base_url_warned:
        ...
        if (_env_base and _cfg_provider
            ...
            logger.warning(
                ...
                "Run: clover model to reconfigure, or remove "
                "OPENAI_BASE_URL from ~/.clover/.env",
    ...
    # on aggregators (OpenRouter, Clover) who previously got routed to a
...
def _tag_effective_provider(client: Any, provider: str) -> None:
    ...
    if client is None or not provider:
        return
    try:
        setattr(client, "_clover_aux_effective_provider", provider)
...
def _effective_provider_for_client(client: Any, fallback: str) -> str:
    ...
    effective_provider = getattr(client, "_clover_aux_effective_provider", "")
...
def _to_async_client(sync_client, model: str, is_vision: bool = False):
    ...
    if base_url_host_matches(sync_base_url, "openrouter.ai"):
        ...
        from clover_cli.copilot_auth import copilot_request_headers
        ...
        async_kwargs["default_headers"] = clover_xai_default_headers()
    else:
    ...
    # See _create_openai_client: disable SDK-internal retries so Clover owns
...
def resolve_provider_client(
    ...
    Args:
        provider: Provider identifier.  One of:
            "openrouter", "clover", "openai-codex" (or "codex"),
        ...
        if raw_codex:
            ...
            if not codex_token:
                logger.warning("resolve_provider_client: openai-codex requested "
                               "but no Codex OAuth token found (run: clover model)")
                return None, None
    ...
    # OpenRouter / Clover bills for side tasks they thought were running on
    ...
    if provider == "xai-oauth":
        ...
        if client is None:
            logger.warning(
                ...
                "OAuth token found (run: clover model -> xAI Grok OAuth — SuperGrok / Premium+)"
            )
            return None, None
        final_model = _normalize_resolved_model(model or default, provider)
        return (_to_async_client(client, final_model, is_vision=is_vision) if async_mode
    ...
    if provider == "azure-foundry":
        ...
        if client is None:
            logger.warning(
                ...
                "runtime resolution failed (run: clover doctor for "
                "diagnostics)"
            )
            return None, None
    ...
    try:
        ...
        logger.debug("clover_cli.auth not available for provider %s", provider)
        return None, None
    ...
    if pconfig.auth_type == "api_key":
        ...
        try:
            from clover_cli.models import opencode_zen_free_runtime as _oc_free_rt
        ...
        if base_url_host_matches(base_url, "api.kimi.com"):
            ...
            headers.update(clover_xai_default_headers())
        else:
            # Fall back to profile.default_headers for providers that declare
        ...
        if provider == "copilot" and final_model and not raw_codex:
            try:
                from clover_cli.models import _should_use_copilot_responses_api
    ...
    if pconfig.auth_type == "external_process":
        ...
        if provider == "clover":
            return resolve_provider_client("clover", model, async_mode)
        if provider == "openai-codex":
...
def get_available_vision_backends() -> List[str]:
    ...
    Order: active provider → OpenRouter → Clover → stop.  This is the single
    ...
    if main_provider and main_provider not in {"auto", ""}:
        if main_provider in _VISION_AUTO_PROVIDER_ORDER:
            ...
            if client is not None:
                available.append(main_provider)
    # 2. OpenRouter, 3. Clover — skip if already covered by main provider.
...
def get_auxiliary_extra_body() -> dict:
    ...
    """
    return _clover_extra_body() if auxiliary_is_clover else {}
...
def _get_auxiliary_task_config(task: str) -> Dict[str, Any]:
    ...
    :meth:`clover_cli.plugins.PluginContext.register_auxiliary_task`) the
...
def resolve_compression_fast_lane(
    ...
    *,
    ...
    route is the one Clover will call. A requested model covers a compressor
...
def _build_call_kwargs(
    ...
    # body fields (Gemini ``thinking_config``), and OpenRouter/Clover use
    ...
    # Anthropic Messages adapters translate Clover reasoning into native
...
def _call_llm_impl(
    ...
    *,
    ...
    Args:
        ...
        reasoning_config: Optional Clover reasoning config for direct model calls
            ...
            if _explicit and _explicit not in {"auto", "openrouter", "custom"}:
                ...
                if fb_client is not None:
                    ...
                    raise RuntimeError(
                        ...
                        f"variable, or switch to a different provider with `clover model`."
                    )
    ...
    try:
        ...
        # ── Stale-model self-heal (Clover Portal recommendation drift) ───
        # A long-lived process can pin a Portal-recommended model that has
        # since been dropped from the Clover → OpenRouter catalog, so every
        ...
        if _is_model_not_found_error(first_err) and _heal_is_clover:
            ...
            if healed_model and healed_model != kwargs.get("model"):
                logger.warning(
                    "Auxiliary %s: model %r no longer in Clover catalog; "
        ...
        if (
            ...
            if refreshed_client is not None:
                logger.info(
                    "Auxiliary %s: refreshed Clover runtime credentials after paid account check, retrying",
                    task or "call",
                )
        ...
        if _is_auth_error(first_err) and client_is_clover:
            refreshed_client, refreshed_model = _refresh_clover_auxiliary_client(
                cache_provider=resolved_provider or "clover",
                model=final_model,
                async_mode=False,
            ...
            if refreshed_client is not None:
                logger.info("Auxiliary %s: refreshed Clover runtime credentials after 401, retrying",
                            task or "call")
                if refreshed_model and refreshed_model != kwargs.get("model"):
                    kwargs["model"] = refreshed_model
                return _validate_llm_response(
        ...
        # When the resolved provider returns 401 and neither the Clover
    ...
    *,
    ...
    try:
        ...
        if _is_model_not_found_error(first_err) and _heal_is_clover:
            ...
            if healed_model and healed_model != kwargs.get("model"):
                logger.warning(
                    "Auxiliary %s (async): model %r no longer in Clover catalog; "
                    "retrying with refreshed recommendation %r",
                    task or "call", kwargs.get("model"), healed_model,
                )
                kwargs["model"] = healed_model
        ...
        if (
            ...
            if refreshed_client is not None:
                logger.info(
                    "Auxiliary %s (async): refreshed Clover runtime credentials after paid account check, retrying",
                    task or "call",
                )
                if refreshed_model and refreshed_model != kwargs.get("model"):
                    kwargs["model"] = refreshed_model
        ...
        if _is_auth_error(first_err) and client_is_clover:
            ...
            if refreshed_client is not None:
                logger.info("Auxiliary %s (async): refreshed Clover runtime credentials after 401, retrying",
                            task or "call")
                if refreshed_model and refreshed_model != kwargs.get("model"):
                    kwargs["model"] = refreshed_model
                return _validate_llm_response(
