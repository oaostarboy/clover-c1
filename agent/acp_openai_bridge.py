"""OpenAI-shape bridge shared by Clover' ACP clients.

An ACP agent (``copilot --acp``, and the ACP CLIs that reach Clover as
...
the agent's *own* tool notifications. Clover' agentic surface — ``memory``,
...
* :func:`completion_to_stream_chunks` — re-shape a one-shot ACP response as
  ...
  is inherently one-shot from Clover' perspective).
...
of its own (Copilot) forwards everything Clover offers; a CLI that is an
autonomous agent with its own read/edit/execute tools must forward only Clover'
agent-level tools, because re-offering the overlapping ones makes Clover re-run
...
class StreamChunks(list):
    ...
    Clover reads provider-level extras off the object returned by
    ``chat.completions.create`` (e.g. ``clover_projected_messages``, consumed by
...
def tool_specs_from_openai_tools(
    ...
    *,
    ...
    name is in it survive — that is how a client forwards just Clover'
