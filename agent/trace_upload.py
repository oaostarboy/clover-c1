"""Upload a Clover session transcript to Hugging Face as an agent trace.

Clover stores sessions in its own SQLite store (``clover_state.SessionDB``),
...
* **Zero LLM turn.** This is a deterministic export — it never spends a
  model call. The ``clover trace upload`` subcommand calls
...
* **Private by default.** Traces can contain prompts, tool output, local
  ...
  is passed through Clover' secret redactor (``force=True``) unless the
...
DEFAULT_DATASET_NAME = "clover-traces"
_CLOVER_VERSION = "clover-c1"
...
# ---------------------------------------------------------------------------
# Conversion: Clover OpenAI-format messages -> Claude Code JSONL
# ---------------------------------------------------------------------------
...
def _redact(text: Any, enabled: bool) -> Any:
    ...
    Non-strings pass through untouched. Uses Clover' shared redactor with
...
def build_trace_jsonl(
    ...
    *,
    ...
    """Render Clover conversation messages as Claude Code JSONL text.
...
_NO_TOKEN_MESSAGE = (
    ...
    "2. Add it to your environment as HF_TOKEN (e.g. in ~/.clover/.env):\n"
    "     HF_TOKEN=hf_xxxxxxxxxxxxxxxxxxxx\n"
    "3. Run /upload-trace again (or `clover trace upload`)."
)
...
def upload_session_trace(
    ...
    *,
    ...
    the user's private ``{user}/clover-traces`` dataset. Returns a
