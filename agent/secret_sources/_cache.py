...
cache problem must never block Clover startup.
"""
...
def resolve_cache_home(home_path: Optional[Path] = None) -> Path:
    """Resolve the Clover home used for cache paths.

    ``home_path`` is whatever ``load_clover_dotenv()`` already resolved;
    falling back to ``$CLOVER_HOME`` / ``~/.clover`` keeps direct callers
    (and tests that don't thread a home through) working.
    """
    if home_path is None:
        from clover_constants import get_clover_home

        home_path = get_clover_home()
    return home_path
...
class DiskCache(Generic[K]):
    ...
    One JSON object per backend lives at ``<clover_home>/cache/<basename>``::
    ...
    def write(
        ...
        try:
            ...
            # metadata isn't exposed if CLOVER_HOME is ever made traversable.
            try:
