...
# `export CLOVER_REDACT_SECRETS=false`) cannot disable redaction
# mid-session.  ON by default — secure default per issue #17691. Users who
# need raw credential values in tool output (e.g. working on the redactor
# itself) can opt out via `security.redact_secrets: false` in config.yaml
# (bridged to this env var in clover_cli/main.py, gateway/run.py, and
# cli.py) or `CLOVER_REDACT_SECRETS=false` in ~/.clover/.env. An opt-out
# warning is logged at gateway and CLI startup so operators see the
# downgrade — see `_log_redaction_status()` in gateway/run.py and cli.py.
_REDACT_ENABLED = os.getenv("CLOVER_REDACT_SECRETS", "true").lower() in {"1", "true", "yes", "on"}
...
_PREFIX_PATTERNS = [
    ...
    # _PREFIX_SUBSTRINGS pre-screen stays false-negative-free).
...
def mask_secret(
    ...
    *,
    ...
    Canonical helper for display-time redaction across Clover — used by
    ``clover config``, ``clover status``, ``clover dump``, and anywhere
...
def redact_sensitive_text(
    ...
    *,
    ...
    for URLs, ``"eyJ" in text`` for JWTs). On a typical clover log line
...
# (``security.redact_secrets: false`` / CLOVER_REDACT_SECRETS) applies to
