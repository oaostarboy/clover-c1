"""Lightweight internationalization (i18n) for Clover static user-facing messages.

Scope (thin slice, by design): only the highest-impact static strings shown
to the user by Clover itself -- approval prompts, a handful of gateway slash
...
Language resolution order:
    ...
    2. ``CLOVER_LANGUAGE`` environment variable (for tests / quick override)
...
def _locales_dir() -> Path:
    ...
    1. ``CLOVER_BUNDLED_LOCALES`` env var -- set by the Nix wrapper (or any
    ...
    """
    override = os.getenv("CLOVER_BUNDLED_LOCALES", "").strip()
    if override:
        candidate = Path(override)
        if candidate.is_dir():
            return candidate
        logger.warning(
            "CLOVER_BUNDLED_LOCALES points to a non-directory path (%s); "
...
def _load_catalog(lang: str) -> dict[str, str]:
    ...
    try:
        import yaml  # PyYAML is already a clover dependency
...
def reset_language_cache() -> None:
    ...
    Call after :func:`clover_cli.config.save_config` if a running process
...
def get_language() -> str:
    ...
    env_lang = os.environ.get("CLOVER_LANGUAGE")
    if env_lang:
