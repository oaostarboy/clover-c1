"""Anthropic Messages API adapter for Clover Cognition.

Translates between Clover's internal OpenAI-style message format and
...
def _getenv(name: str, default: str = "") -> str:
    ...
    read) when on. Mirrors the same wrapper in clover_cli/runtime_provider.py.
    """
...
# Clover effort → Anthropic adaptive-thinking effort (output_config.effort).
...
def _build_anthropic_client_with_bearer_hook(
    ...
    *,
    ...
    kwargs = {
        ...
        # Delegate retry to clover's outer loop (honors Retry-After); the SDK
...
def build_anthropic_client(
    ...
    * a ``Callable[[], str]`` — an Entra ID bearer token provider from
    ...
    kwargs = {
        ...
        # Delegate all rate-limit / 5xx retry to clover's outer conversation
    ...
    # ``~/.clover/.env``). The result is dual auth —
...
def _refresh_oauth_token(creds: Dict[str, Any]) -> Optional[str]:
    ...
    Clover notices an expired token, Claude Code may have already rotated it.
...
def _prefer_refreshable_claude_code_token(env_token: str, creds: Optional[Dict[str, Any]]) -> Optional[str]:
    ...
    Clover historically persisted setup tokens into ANTHROPIC_TOKEN. That makes
...
def _resolve_anthropic_pool_token() -> Optional[str]:
    ...
    ``account_usage`` and ``clover models``) never mutates ``~/.clover/auth.json``
...
def resolve_anthropic_token() -> Optional[str]:
    ...
    Priority:
      1. ANTHROPIC_TOKEN env var (OAuth/setup token saved by Clover)
      ...
      5. Anthropic credential_pool OAuth entry (~/.clover/auth.json)
    ...
    # 1. Clover-managed OAuth/setup token env var
    ...
    # 5. Clover credential_pool OAuth entry.
...
# ── Clover-native PKCE OAuth flow ────────────────────────────────────────
# Mirrors the flow used by Claude Code, pi-ai, and OpenCode.
# Stores credentials in ~/.clover/.anthropic_oauth.json (our own file).
...
def _get_clover_oauth_file() -> Path:
    return get_clover_home() / ".anthropic_oauth.json"
...
def run_clover_oauth_login_pure() -> Optional[Dict[str, Any]]:
    """Run Clover-native OAuth PKCE flow and return credential state."""
    import secrets
    import time
    import webbrowser
    ...
    print()
    print("Authorize Clover with your Claude Pro/Max subscription.")
    print()
    ...
    try:
        from clover_cli.auth import _can_open_graphical_browser as _can_open_gui
    except Exception:
...
def read_clover_oauth_credentials() -> Optional[Dict[str, Any]]:
    """Read Clover-managed OAuth credentials from ~/.clover/.anthropic_oauth.json."""
    ...
    if oauth_file.exists():
        try:
            ...
            logger.debug("Failed to read Clover OAuth credentials: %s", e)
    return None
...
def _convert_assistant_message(m: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # can validate the message history.  See clover-c1#13848.
    #
...
def _strip_orphaned_tool_blocks(result: List[Dict[str, Any]]) -> None:
    ...
    for i, m in enumerate(result):
        ...
        # replaying it verbatim.  See clover-c1: extended-thinking + parallel
...
def _manage_thinking_signatures(
    ...
    replayed assistant tool-call messages.  See clover-c1#13848 (Kimi) and
    clover-c1#16748 (DeepSeek).

    Clover Portal's ``/v1/messages`` route is the exception among third-party
...
def build_anthropic_kwargs(
    ...
    if is_oauth:
        ...
        for block in system:
            if isinstance(block, dict) and block.get("type") == "text":
                text = block.get("text", "")
                text = text.replace("Clover Cognition", "Claude Code")
                text = text.replace("Clover agent", "Claude Code")
                text = text.replace("clover-c1", "claude-code")
                text = text.replace("Clover Cognition", "Anthropic")
                block["text"] = text
        ...
        #      a) bare Clover-native tools (``read_file``)  -> ``mcp__read_file``
    ...
    # silently hides reasoning text that Clover surfaces in its CLI. We
