...
def build_turn_context(
    ...
    *,
    ...
    # Tag log records on this thread with the session ID for ``clover logs``.
    ...
    if agent.compression_enabled and _should_run_preflight_estimate(
        ...
        # Clover only initiates compaction in "clover" mode (#36801).
        _codex_native_auto = (
        ...
        if _preflight_deferred:
            ...
            logger.info(
                "Skipping Clover preflight compression for codex app-server "
                "(mode=%s); Clover will not start thread compaction here.",
    ...
    if agent._memory_manager:
        ...
        # to surface it. Rendered by Clover (via _emit_status), so it always
