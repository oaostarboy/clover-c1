"""Coding-context awareness — base Clover, every interactive surface.

When the user runs Clover inside a code workspace (CLI, TUI, desktop app, or an
editor over ACP), Clover shifts into a **coding posture**. This module is the
...
from clover_cli._subprocess_compat import bounded_git_probe

logger = logging.getLogger("clover.coding_context")
...
_EDIT_FORMAT_GUIDANCE: dict[str, tuple[tuple[str, ...], str]] = {
    ...
    "replace": (
        ("claude", "sonnet", "opus", "haiku",
         ...
         "clover", "llama", "mistral", "devstral", "minimax"),
        "- Edit format: author new files with `write_file`; for edits to "
...
# in _CLOVER_CORE_TOOLS, so they're present on every surface this fires on.
...
class RuntimeMode:
    ...
    def toolset_selection(self, config: Optional[dict[str, Any]] = None) -> Optional[list[str]]:
        ...
        selection (``--toolsets``, ``CLOVER_TUI_TOOLSETS``, …); they never
...
def is_coding_context(
    *,
    ...
    """Whether Clover should operate in its coding posture right now."""
