"""Unified removal contract for every credential source Clover reads from.

Clover seeds its credential pool from many places:
    ...
    manual        — user ran `clover auth add`
...
unify here is **removal**:
    ...
    ``clover auth remove <provider> <N>`` must make the pool entry stay gone.
...
class RemovalStep:
    ...
    Attributes:
        provider: Provider pool key (``"xai"``, ``"anthropic"``, ``"clover"``, ...).
...
def _remove_env_source(provider: str, removed) -> RemovalResult:
    ...
    Handles three user situations:
      1. Var lives only in ~/.clover/.env  → clear it
    ...
    """
    from clover_cli.config import get_env_path, remove_env_value
    ...
    try:
        ...
        if env_path.exists():
            ...
            # canonical reader in clover_cli/config.py. read_text() with no
    ...
    if shell_exported:
        result.hints.extend([
            ...
            "launchd plist, etc.) or it will keep being visible to Clover.",
            f"  The pool entry is now suppressed — Clover will ignore "
            f"{env_var} until you run `clover auth add {provider}`.",
        ])
    else:
        result.hints.append(
...
def _remove_claude_code(provider: str, removed) -> RemovalResult:
    ...
    work.  We just suppress it so Clover stops reading it.
    """
    return RemovalResult(hints=[
        "Suppressed claude_code credential — it will not be re-seeded.",
        "Note: Claude Code credentials still live in ~/.claude/.credentials.json",
        "Run `clover auth add anthropic` to re-enable if needed.",
    ])


def _remove_clover_pkce(provider: str, removed) -> RemovalResult:
    """~/.clover/.anthropic_oauth.json is ours — delete it outright."""
    from clover_constants import get_clover_home

    result = RemovalResult()
    oauth_file = get_clover_home() / ".anthropic_oauth.json"
    if oauth_file.exists():
        try:
            oauth_file.unlink()
            result.cleaned.append("Cleared Clover Anthropic OAuth credentials")
...
def _remove_minimax_oauth(provider: str, removed) -> RemovalResult:
    ...
    Same pattern as Clover: single-source OAuth state with refresh tokens.
...
def _remove_xai_oauth_device_code(provider: str, removed) -> RemovalResult:
    ...
    Without this step, ``clover auth remove xai-oauth <N>`` silently undoes
    ...
    if _clear_auth_store_provider(provider):
        result.cleaned.append(f"Cleared {provider} OAuth tokens from auth store")
    result.hints.append(
        "Run `clover model` → xAI Grok OAuth (SuperGrok / Premium+) to re-authenticate if needed."
    )
    return result
...
def _remove_codex_device_code(provider: str, removed) -> RemovalResult:
    ...
    the Clover auth store is not enough — _seed_from_singletons() would
    ...
    via ``clover auth add openai-codex``), but in both cases the re-seed
    ...
    result.hints.extend([
        ...
        "Run `clover auth add openai-codex` to re-enable if needed.",
    ])
    return result
...
def _remove_qwen_cli(provider: str, removed) -> RemovalResult:
    ...
    return RemovalResult(hints=[
        ...
        "Run `clover auth add qwen-oauth` to re-enable if needed.",
    ])
...
def _remove_copilot_gh(provider: str, removed) -> RemovalResult:
    ...
    Clover stops picking the token up.
    """
    ...
    return RemovalResult(hints=[
        ...
        "Run `clover auth add copilot` to re-enable if needed.",
    ])
...
def _register_all_sources() -> None:
    ...
    register(RemovalStep(
        provider="anthropic", source_id="clover_pkce",
        remove_fn=_remove_clover_pkce,
        description="~/.clover/.anthropic_oauth.json",
    ))
    register(RemovalStep(
