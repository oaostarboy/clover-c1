...
``kind: backend``) or ``~/.clover/plugins/browser/<name>/`` (user, opt-in via
``plugins.enabled``).
...
class BrowserProvider(abc.ABC):
    ...
    def get_setup_schema(self) -> Optional[Dict[str, Any]]:
        ...
        Used by :mod:`clover_cli.tools_config` to inject this provider as a
        ...
        expose API key prompts, badges, managed-Clover gating, and the
