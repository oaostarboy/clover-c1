"""Core NeMo Relay adapters for physical Clover provider attempts."""
...
def _relay_operation_name(provider_name: str, metadata: dict[str, Any] | None) -> str:
    """Return Relay's canonical operation name when Clover knows the API mode."""
...
def _relay_metadata(
    ...
    if not isinstance(relay_metadata, dict):
        relay_metadata = {}
    relay_metadata.setdefault("clover.provider", provider_name)
...
def execute_current(
    ...
    *,
    ...
    """Run a provider attempt under the inherited Clover turn when present."""
...
def stream_current(
    ...
    *,
    ...
    """Run a provider stream under the inherited Clover turn when present.
...
class ManagedLlmStream(Iterator[Any]):
    """Drive Relay's async stream from Clover's provider worker thread."""
    ...
    def __init__(
        ...
        *,
        ...
        def run_callback(callback: Callable[..., Any], *args: Any) -> Any:
            ...
            def guarded() -> Any:
                # Clover-side callbacks run while the native pipeline drives
...
class AnthropicStreamAccumulator:
    ...
    def response(self, base: Any = None) -> Any:
        """Return the attribute-shaped response consumed by Clover."""
    ...
    *,
    ...
    with turn.finalize_lock:
        ...
        try:
            ...
            logger.warning(
                "Clover Relay logical LLM finalization failed",
                exc_info=True,
            )
            return
        with turn.logical_llm_lock:
