...
def openclaw_residue_hint_cli() -> str:
    """Banner shown the first time Clover starts and finds ``~/.openclaw/``.

    Points users at ``clover claw migrate`` (non-destructive port of config,
    memory, and skills) first. ``clover claw cleanup`` is mentioned as the
    ...
    return (
        ...
        "To port your config, memory, and skills over to Clover, run "
        "`clover claw migrate`.\n"
        "If you've already migrated and want to archive the old directory, "
        "run `clover claw cleanup` (renames it to ~/.openclaw.pre-migration — "
