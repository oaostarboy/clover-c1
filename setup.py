...
Clover Cognition (see website/docs/getting-started/platform-support.md). The
...
build sandbox. ``nix/python.nix`` sets ``CLOVER_NIX_BUILD=1`` on the
Clover package derivation, so only that build may create an artifact.
...
_IN_NIX_BUILD = os.environ.get("CLOVER_NIX_BUILD") == "1"
...
_BLOCK_MESSAGE = (
    ...
    "the Clover Nix derivation sets CLOVER_NIX_BUILD=1. If it does, file a bug."
)
