...
1. ``flush_pending_to_file()`` — called BEFORE ``_pending_messages.clear()``
   ...
   under ``<clover_home>/pending_messages/``.
...
def _get_flush_dir():
    """Return the pending-messages flush directory under the active CLOVER_HOME."""
    from clover_constants import get_clover_home

    flush_dir = get_clover_home() / "pending_messages"
...
def spool_dropped_transcript_message(
    ...
    ``<clover_home>/pending_messages/``), so a runtime cap rotation no
