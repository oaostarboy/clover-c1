...
class WhatsAppBehaviorMixin:
    ...
    DEFAULT_REPLY_PREFIX: str = "☘ *Clover Cognition*\n────────────\n"
...
def resolve_whatsapp_bridge_dir() -> Path:
    """Resolve the WhatsApp bridge directory, mirroring to CLOVER_HOME if needed.

    When the install tree is read-only (e.g., Docker /opt/clover), this function
    mirrors the bridge source to a writable CLOVER_HOME location and returns that
    ...
    # Try CLOVER_HOME location first
    clover_home = get_clover_home()
    clover_home_bridge = clover_home / "scripts" / "whatsapp-bridge"
    ...
    # Install dir is read-only, mirror to CLOVER_HOME if needed
    if clover_home_bridge.exists():
        return clover_home_bridge

    # Mirror the bridge source to CLOVER_HOME
    try:
        clover_home_bridge.parent.mkdir(parents=True, exist_ok=True)
        shutil.copytree(
            install_bridge,
            clover_home_bridge,
            dirs_exist_ok=False,
