...
def _resolve_profile_path(path_value: Any) -> Optional[Path]:
    """Resolve a user path, mapping ~/.clover to the active profile home."""
    ...
    if raw == "~/.clover":
        return clover_home
    if raw.startswith("~/.clover/"):
        return clover_home / raw.removeprefix("~/.clover/")
    path = Path(raw).expanduser()
    if path.is_absolute():
        return path
    return clover_home / path


def _resolve_script_path(script_value: Any) -> tuple[Optional[Path], Optional[str]]:
    """Resolve a route script under CLOVER_HOME/scripts."""
    if not isinstance(script_value, str) or not script_value.strip():
        return None, "script path is empty"
    from clover_constants import get_clover_home

    scripts_root = (get_clover_home() / "scripts").resolve()
    raw_text = os.path.expandvars(script_value.strip())
    if raw_text == "~/.clover" or raw_text.startswith("~/.clover/"):
...
class WebhookRouteProcessor:
    ...
    def run_route_script(self, script_value: Any, payload: dict) -> tuple[bool, Optional[dict]]:
        ...
        if (
            ...
            or transformed.get("__clover_ignore__") is True
        ):
            return False, None
