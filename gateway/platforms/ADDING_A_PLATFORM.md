...
There are two ways to add a platform to the Clover gateway:

## Plugin Path (Recommended for Community/Third-Party)

Create a plugin directory in `~/.clover/plugins/` (or under `plugins/platforms/`
...
**zero changes to core Clover code**.
...
- `env_enablement_fn: () -> Optional[dict]` — seeds `PlatformConfig.extra`
  ...
  `clover gateway status` or `get_connected_platforms()` until the SDK
  instantiates.
...
- `plugin.yaml` `requires_env` / `optional_env` rich-dict entries —
  auto-populate `OPTIONAL_ENV_VARS` in `clover_cli/config.py` so the setup
...
Checklist for integrating a platform directly into the Clover core.
...
And add it to the `clover-gateway` composite:

```python
"clover-gateway": {
    "includes": [..., "clover-your-platform"]
}
```
...
## 12. Status Display (`clover_cli/status.py`)
...
## 13. Gateway Setup Wizard (`clover_cli/gateway.py`)
...
grep -r "telegram\|discord\|whatsapp\|slack" gateway/ tools/ agent/ cron/ clover_cli/ toolsets.py \
  --include="*.py" -l | sort -u
