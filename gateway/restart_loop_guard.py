...
Defenses 1 and 2 (the ``_CLOVER_GATEWAY`` guard on ``clover gateway
...
raw ``terminal("launchctl kickstart -k gui/<uid>/ai.clover.gateway")``,
...
State lives in ``<CLOVER_HOME>/gateway/restart_loop.json`` so it is
...
def _state_path():
    return get_clover_home() / "gateway" / "restart_loop.json"
