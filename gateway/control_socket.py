...
(the updater, `clover serve`/dashboard, the Desktop app) currently discovers
...
- ``identify`` → pid, profile label, clover_home, code_sha/code_version
...
- POSIX: Unix domain socket at ``$CLOVER_HOME/gateway.sock``. When the home
  path is too long for ``sun_path`` (~104 bytes on macOS/BSD), the socket is
  bound in the system temp dir and a pointer file
  ``$CLOVER_HOME/gateway.sock.path`` records the real location; clients
  follow the pointer transparently.
- Windows: named pipe ``\\\\.\\pipe\\clover-gateway-<home-hash>`` served via
...
Consumers (``clover update --plan`` inventory, the post-update fleet version
...
def windows_pipe_name(home: Path) -> str:
    """Per-CLOVER_HOME named pipe path (Windows transport)."""
    return rf"\\.\pipe\clover-gateway-{_home_hash(home)}"
...
def _fallback_socket_path(home: Path) -> Path:
    ...
    """
    name = f"clover-gw-{_home_hash(home)}.sock"
...
def _detect_supervisor() -> str:
    ...
    if sys.platform == "darwin" and (
        env.get("XPC_SERVICE_NAME", "").startswith("ai.clover")
        or env.get("LAUNCHD_SOCKET")
    ):
        return "launchd"
    if env.get("CLOVER_DESKTOP_MANAGED"):
        return "desktop"
...
def build_identify_payload() -> dict[str, Any]:
    ...
    payload: dict[str, Any] = {
        ...
        "clover_home": record.get("clover_home"),
        "profile": _profile_label_for_home(record.get("clover_home") or ""),
...
class GatewayControlServer:
    ...
    its CLOVER_HOME), ``stop()`` on shutdown. All failures are non-fatal —
    ...
    def __init__(
        ...
        *,
        ...
        if home is None:
            from gateway.status import _get_process_clover_home
