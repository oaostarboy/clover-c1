...
2. An event-loop heartbeat file at ``<CLOVER_HOME>/state/gateway.heartbeat`` so
...
def _process_clover_home() -> Path:
    """CLOVER_HOME for process-level identity files (ignore profile overrides)."""
...
def get_loop_heartbeat_path(home: Optional[Path] = None) -> Path:
    """Return ``<CLOVER_HOME>/state/gateway.heartbeat``."""
    base = home if home is not None else _process_clover_home()
...
def get_loop_tick_socket_path(
    ...
    ``<CLOVER_HOME>/state/gateway.loop-tick.<pid>.sock`` — PID-suffixed so a
...
async def loop_heartbeat_forever(
    ...
    *initiated* by the loop, so a frozen loop still lets the file age.
    ...
    wedged; see ``clover_cli.gateway.probe_gateway_loop_liveness`` for the
