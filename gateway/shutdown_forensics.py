...
def snapshot_shutdown_context(received_signal: Any = None) -> Dict[str, Any]:
    ...
    * Wall-clock and monotonic timestamps for cross-correlating later phases
    ...
    try:
        ...
        if clover_home_str:
            takeover_path = Path(clover_home_str) / ".gateway-takeover.json"
            if takeover_path.exists():
                try:
                    ...
                    pass
            planned_stop_path = Path(clover_home_str) / ".gateway-planned-stop.json"
...
def check_systemd_timing_alignment(drain_timeout: float) -> Optional[Dict[str, Any]]:
    ...
    upgraded clover-c1 but never re-ran ``clover setup`` to regenerate
    ...
    try:
        # /proc/self/cgroup gives us "0::/user.slice/.../clover-gateway.service"
    ...
    # that's the common case for clover.
    ...
    # constant in clover_cli/gateway.py.
    headroom = 30.0
...
def parse_systemd_duration_to_us(raw: str) -> Optional[int]:
    ...
    Public: also consumed by clover_cli.gateway's restart-wait sizing.
    """
    if not raw:
        return None
    units = {
