...
# path eagerly freezes it to whatever CLOVER_HOME/profile context existed
# at that exact import moment for the rest of the process's lifetime --
# even if a context-local override (see clover_constants.set_clover_home_override)
# is established afterward. A freshly-started, short-lived process (e.g. the
# ``clover pairing`` CLI) re-imports this module later with the final
...
def _default_pairing_dir() -> Path:
    ...
    if PAIRING_DIR is not None:
        return PAIRING_DIR
    return get_clover_dir("platforms/pairing", "pairing")
...
def _read_allowlist_env(env_var: str) -> str:
    ...
    ``clover_cli.config.save_env_value`` / ``remove_env_value``, which target
...
def _sync_allowlist_remove(platform: str, user_id: str) -> None:
    ...
    if len(remaining) == len(ids):
        return  # Not present.
    try:
        from clover_cli.config import save_env_value, remove_env_value
...
def _merge_pairing_dir(active_dir: Path, alternate_dir: Path) -> None:
    ...
    Older installs use ``{CLOVER_HOME}/pairing`` while newer code/docs may
    write ``{CLOVER_HOME}/platforms/pairing``. If both directories exist, the
...
class PairingStore:
    ...
    profile's own CLOVER_HOME using the same legacy/consolidated layout rules
    as ``clover -p <name> pairing ...``. This keeps multiplex gateways and
    profile-scoped CLI approvals on one whitelist. Without a profile, storage
    is the global pairing directory for the current CLOVER_HOME.
    """

    def __init__(self, profile: Optional[str] = None):
        # Resolve storage directory lazily — tests use a temp CLOVER_HOME
        ...
        if profile:
            ...
            # ``clover -p <profile> pairing ...`` process does. Merge the
    ...
    def _load_json(self, path: Path) -> dict:
        if path.exists():
            try:
                ...
                # the file, then the gateway process — running as `clover` after
                ...
                logger.warning(
                    ...
                    "If you ran `docker exec <container> clover pairing approve ...` as root, "
                    "re-run with `docker exec -u clover <container> ...` and "
                    "chown the existing file to the clover user, or restart the "
    ...
    def approve_request(self, platform: str, request_id: str) -> Optional[dict]:
        ...
        This is the grant path for authenticated admin surfaces (``clover
