...
class PlatformEntry:
    ...
    # status displays (``clover setup``, ``clover status``, the dashboard
    ...
    # Env vars this platform needs (for ``clover setup`` display).
    ...
    # built-ins).  Used by ``clover gateway setup`` to auto-enable the
...
class PlatformRegistry:
    ...
    def __init__(self) -> None:
        ...
        # Plugin adapters are isolated per resolved CLOVER_HOME and overlay the
        ...
        # plugin-discovery time added several seconds to *every* `clover`
        # invocation -- including plain `clover chat`, which never touches any
        ...
        # `clover setup`/`gateway status`, send_message).
    ...
    def _resolve_all(self) -> None:
        ...
        gateway startup, ``clover setup``/``gateway status``, channel
