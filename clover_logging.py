"""Centralized logging setup for Clover Cognition.

Provides a single ``setup_logging()`` entry point that both the CLI and
gateway call early in their startup path.  All log files live under
``~/.clover/logs/`` (profile-aware via ``get_clover_home()``).
...
Component separation:
    ...
    gui.log receives dashboard-side records from ``clover_cli.web_server``,
    ``clover_cli.pty_bridge``, ``tui_gateway.*``, and ``uvicorn.*``.
...
# essentially always in Clover (TUI, gateway, ``hy_memory`` server, MCP
...
from clover_constants import get_config_path, get_clover_home
...
def _install_session_record_factory() -> None:
    ...
    if getattr(current_factory, "_clover_session_injector", False):
    ...
    _session_record_factory._clover_session_injector = True  # type: ignore[attr-defined]
...
# Used by _ComponentFilter and exposed for ``clover logs --component``.
COMPONENT_PREFIXES = {
    ...
    # ``clover logs --component gateway``.
    "gateway": ("gateway", "clover_plugins", "plugins.platforms"),
    "agent": ("agent", "run_agent", "model_tools", "batch_runner"),
    "tools": ("tools",),
    "cli": ("clover_cli", "cli"),
    "cron": ("cron",),
    "gui": (
        "clover_cli.web_server",
        "clover_cli.pty_bridge",
        "tui_gateway",
        "uvicorn",
    ),
}
...
def setup_logging(
    *,
    ...
    """Configure the Clover logging subsystem.
    ...
    *force* is ``True``.
    ...
    ----------
    clover_home
        Override for the Clover home directory.  Falls back to
        ``get_clover_home()`` (profile-aware).
    log_level
    ...
    home = clover_home or get_clover_home()
    from clover_constants import mkdir_under_clover_home
    log_dir = mkdir_under_clover_home(home / "logs")
...
def setup_verbose_logging() -> None:
    ...
    for h in root.handlers:
        if isinstance(h, logging.StreamHandler) and not isinstance(h, RotatingFileHandler):
            if getattr(h, "_clover_verbose", False):
                return

    handler = logging.StreamHandler(_safe_stderr())
    handler.setLevel(logging.DEBUG)
    handler.setFormatter(RedactingFormatter(_LOG_FORMAT_VERBOSE, datefmt="%H:%M:%S"))
    handler._clover_verbose = True  # type: ignore[attr-defined]
...
class _ManagedRotatingFileHandler(RotatingFileHandler):
    ...
    1.  In managed mode (NixOS), the stateDir uses setgid (2770) so new files
        inherit the clover group. However, both ``_open()`` (initial creation)
    ...
    def __init__(self, *args, **kwargs):
        from clover_cli.config import is_managed
...
# the module header): when several Clover processes log to the same file, an
    ...
    *root* directly, so emitting threads never block on file I/O or the
    ...
    with _queue_state_lock:
        if _log_queue is None:
            ...
            qh._clover_queue = True  # type: ignore[attr-defined]
...
def drain_log_queue(timeout: float = 1.0) -> None:
    ...
    t = threading.Thread(target=_drain, name="clover-log-drain", daemon=True)
    t.start()
    t.join(timeout)
...
def _reset_queued_handlers() -> None:
    ...
    with _queue_state_lock:
        ...
        for h in list(root.handlers):
            if getattr(h, "_clover_queue", False):
...
def _add_rotating_handler(
    ...
    *,
    ...
    from clover_constants import mkdir_under_clover_home
    mkdir_under_clover_home(path.parent)
...
def _read_logging_config():
    ...
    try:
        ...
        # reuses the parse clover_cli.main's early bridge already did (one
        # config.yaml parse per process instead of 3-4). Fall back to a
        # direct parse when clover_cli.config isn't importable (bare
        # clover_logging consumers).
        try:
            from clover_cli.config import read_raw_config as _rrc
            cfg = _rrc() or {}
        except Exception:
