#!/usr/bin/env bash
# GitHub environment detection helper for Clover Cognition skills.
#
...
if command -v gh &>/dev/null && gh auth status &>/dev/null 2>&1; then
    ...
    GH_AUTH_METHOD="curl"
elif _clover_env="${CLOVER_HOME:-$HOME/.clover}/.env"; [ -f "$_clover_env" ] && grep -q "^GITHUB_TOKEN=" "$_clover_env" 2>/dev/null; then
