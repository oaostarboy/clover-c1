...
Ledger path resolution (first wins):
  --ledger PATH
  $CLOVER_CITATION_LEDGER
  $CLOVER_HOME/cache/citations/ledger.json
"""
...
from _clover_home import get_clover_home  # noqa: E402
...
def resolve_ledger_path(explicit: str | None = None) -> Path:
    ...
    env = os.environ.get("CLOVER_CITATION_LEDGER", "").strip()
    if env:
        return Path(env).expanduser()
    return get_clover_home() / "cache" / "citations" / "ledger.json"
