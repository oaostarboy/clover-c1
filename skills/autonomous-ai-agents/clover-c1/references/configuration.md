...
`clover config check` reports sections missing from an older config.

### Toolsets

Enable/disable via `clover tools` (interactive) or `clover tools enable/disable NAME`.
Full enumeration: `TOOLSETS` dict in `toolsets.py` (`_CLOVER_CORE_TOOLS` is the default bundle most platforms inherit).
