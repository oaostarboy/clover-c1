...
Config: `~/.clover/config.yaml` (settings), `~/.clover/.env` (API keys) — both under `$CLOVER_HOME` when it is set.
...
`_CLOVER_CORE_TOOLS` (every platform) or to a specific toolset.
...
1. Add `CommandDef` to `COMMAND_REGISTRY` in `clover_cli/commands.py`
...
- Tests auto-redirect `CLOVER_HOME` to temp dirs — never touch real `~/.clover/`.
...
- POSIX file modes (0o600, etc.) → `@pytest.mark.skipif(sys.platform.startswith("win"), reason="POSIX mode bits not enforced on Windows")` (see `tests/clover_cli/test_auth_toctou_file_modes.py`)
...
- Use `get_clover_home()` from `clover_constants` for all paths (profile-safe)
