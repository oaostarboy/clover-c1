# Themes / Skins — Author a Clover Color Theme
...
- The user asks for a custom look ("make me a synthwave theme", "dark forest
  vibes", "match my brand colors") for Clover itself.
...
- Write access to the Clover home dir — `~/.clover` by default, or `$CLOVER_HOME`
  / the active profile's dir. Skins live in `<clover-home>/skins/`.
- Native tools: `write_file` (create the YAML), `read_file` / `search_files`
  (inspect existing skins), `terminal` (activate via `clover config set`).
...
3. `write_file` it to `<clover-home>/skins/<name>.yaml`.
...
4. **Confirm the new look landed** and tell the user how to revert: run
   `clover config set display.skin default` (or they can `/skin default`).
...
```
clover skin set <key> <hex>      # e.g. clover skin set ui_tool "#00FFFF"
```
...
palette and resets the background. `clover skin set background "#08201f"` changes
only the background; `clover skin use <name>` / `clover skin list` switch and
enumerate.

## Pitfalls

- **Don't hardcode `~/.clover`** when a profile is active — resolve the real home
  from `$CLOVER_HOME` first, falling back to `~/.clover`.
...
- **Name collisions**: a skin named like a desktop built-in (`mono`, `slate`,
  `cyberpunk`, `clover`, `midnight`, `ember`) won't override that built-in on the
  GUI. Pick a fresh name.
- **Never hand-edit `config.yaml` to activate.** Use `clover config set
...
- **You apply it, not the user.** `clover config set display.skin <name>` is
...
- `read_file` the written `<clover-home>/skins/<name>.yaml` and confirm valid
  YAML with the intended `name` and `colors`.
- Run `clover config get display.skin` and confirm it reports `<name>`.
