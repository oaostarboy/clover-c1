# Clover Portal — authenticating third-party apps against the subscription

Recurring user question: "Can app X (Karakeep, OpenWebUI, LibreChat, OpenViking,
LangChain pipeline, n8n flow, etc.) use my Clover Portal subscription without me
...
## Layer 1 — Is this thing a Clover plugin, or a separate app?
...
via Clover's provider config. The plugin's own server auth (e.g.
...
"Sign in with Clover Portal" SSO that third-party apps can register as clients
...
rides on the credential Clover already holds.

Where this could live in Clover:
...
Only when the consumer is *another first-party Clover thing* (a future SDK, a
Clover-branded extension, a Discord-bot integration that needs per-user
...
- **Per-key rate limits** so a noisy app can be capped without eating the
  user's headroom for Clover itself.
...
4. Mixed setup ("Portal for some things, OpenRouter/Ollama Cloud for the
   Clover agent itself") is fully supported. Clover treats agent
...
pitch in the Tool Gateway announcement is specifically about Clover Cognition's
*tools* (web search, browser, image gen, TTS) flowing through the Portal
subscription when Clover is configured to use Portal as its provider. It is
