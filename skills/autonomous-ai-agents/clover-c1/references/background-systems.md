...
the `cronjob` tool, the `clover cron` CLI (`list`, `add`, `edit`,
...
User docs: https://clover-c1.local/docs/user-guide/features/cron
...
- **CLI:** `clover curator <verb>` — `status`, `usage`, `run`, `pause`,
...
User docs: https://clover-c1.local/docs/user-guide/features/curator

### Kanban (multi-agent work queue)

Durable SQLite board for multi-profile / multi-worker collaboration.
Users drive it via `clover kanban <verb>`; dispatcher-spawned workers
see a focused `kanban_*` toolset gated by `CLOVER_KANBAN_TASK`, and
...
- **Isolation:** board is the hard boundary (workers get
  `CLOVER_KANBAN_BOARD` pinned in env); tenant is a soft namespace
  within a board for workspace-path + memory-key isolation.

User docs: https://clover-c1.local/docs/user-guide/features/kanban
