...
Write plugins for the Clover desktop app: statusbar items, layout panes,
...
system (`$CLOVER_HOME/plugins/`) is otherwise documented separately.

There are TWO on-disk doors, same contract and hot reload:

- `$CLOVER_HOME/desktop-plugins/<id>/plugin.js` — standalone desktop plugin.
  Loads enabled by default.
- `$CLOVER_HOME/plugins/<id>/desktop/plugin.js` — the desktop HALF of a
...
- The Clover desktop app (it loads plugins; the CLI/gateway alone does not).
- Write access to `$CLOVER_HOME/desktop-plugins/` (usually
  `$CLOVER_HOME/desktop-plugins/`).

## How to Run

1. Create `$CLOVER_HOME/desktop-plugins/<name>/plugin.js` from
   `templates/plugin.js` (in this skill directory) — that's
   `~/.clover/...` by default, or `~/.clover/profiles/<profile>/...` under a
...
The ONLY import surface is `@clover/plugin-sdk` (plus `react` /
...
- `ctx.os` — the curated OS door, attributed to your plugin:
  ...
  posts a native OS notification. Fires only while the user is away from Clover
  ...
  (`clover://index-network/intent/1`), a hash path (`/index-network/intent/1`),
...
- Backend: if the plugin ships a Python `plugin_api.py` (under
  `$CLOVER_HOME/plugins/<id>/dashboard/`, manifest `"api": "plugin_api.py"`), reach
...
- Do not import anything except `@clover/plugin-sdk`, `react`, and
