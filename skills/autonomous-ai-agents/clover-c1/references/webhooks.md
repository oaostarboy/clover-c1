...
Create dynamic webhook subscriptions so external services (GitHub, GitLab, Stripe, CI/CD, IoT sensors, monitoring tools) can trigger Clover agent runs by POSTing events to a URL.
...
All management is via the `clover webhook` CLI command:

### Create a subscription
```bash
clover webhook subscribe <name> \
...
clover webhook subscribe todoist-clover \
  ...
  --script "todoist-clover-label.py" \
...
### GitHub: PR reviews
```bash
clover webhook subscribe github-prs \
  --events "pull_request" \
...
```bash
clover webhook subscribe ci-builds \
  --events "pipeline" \
...
```bash
clover webhook subscribe alerts \
