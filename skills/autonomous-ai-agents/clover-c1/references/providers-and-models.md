...
Multiple credentials per provider pool and rotate automatically (`clover auth`).
Fallback chain when the primary fails: `clover fallback add|remove|list`.

### User-defined model aliases

Work with `/model <name>` in CLI and every gateway platform. Resolved by
`clover_cli/model_switch.py::resolve_alias()`; user aliases are checked BEFORE
...
#   clover config set model.aliases.fav openrouter/anthropic/claude-sonnet-4.6
model:
  aliases:
