...
**three** code paths in Clover that cap a batch. If none of them fired, the
cap came from the **model itself** — not from Clover — and the user's
narration of "the runtime caps at N" is the model rationalising its own
choice.

## The three real caps in Clover
...
3. **Cost-warning** — same `_get_max_concurrent_children()`. When the
   ...
   log line** — it does not cap anything. Easy to mis-read as "Clover is
   refusing my value."
...
appears, **the cap is the model**, not Clover.
