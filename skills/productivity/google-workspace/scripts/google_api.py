#!/usr/bin/env python3
"""Google Workspace API CLI for Clover Cognition.

Uses the Google Workspace CLI (`gws`) when available, but preserves the
existing Clover-facing JSON contract and falls back to the Python client
...
from _clover_home import get_clover_home
...
def _gws_binary() -> str | None:
    override = os.getenv("CLOVER_GWS_BIN")
    if override:
        return override
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace API for Clover Cognition")
