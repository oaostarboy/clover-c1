...
## 5. Clover Configuration

### Config location

`$CLOVER_HOME/config.yaml` (defaults to `~/.clover/config.yaml` when `CLOVER_HOME` is unset)
...
Restart the Clover session for changes to take effect. The MCP connection is
