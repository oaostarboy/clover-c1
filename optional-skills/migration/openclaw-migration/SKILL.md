...
- copy OpenClaw skills into `$CLOVER_HOME/skills/openclaw-imports/`
- optionally copy the OpenClaw workspace instructions file into a chosen Clover workspace
- mirror compatible workspace assets such as `workspace/tts/` into `$CLOVER_HOME/tts/`
...
- `$CLOVER_HOME/skills/migration/openclaw-migration/scripts/openclaw_to_clover.py`

Do not guess a shorter path like `$CLOVER_HOME/skills/openclaw-migration/...`.

Before running the helper:

1. Prefer the installed path under `$CLOVER_HOME/skills/migration/openclaw-migration/`.
...
```bash
python $CLOVER_HOME/skills/migration/openclaw-migration/scripts/openclaw_to_clover.py
```

When using the terminal tool, prefer an absolute invocation pattern such as:

```json
{"command":"python /home/USER/.clover/skills/migration/openclaw-migration/scripts/openclaw_to_clover.py","workdir":"/home/USER"}
```

Dry run with the user-data preset:

```bash
python $CLOVER_HOME/skills/migration/openclaw-migration/scripts/openclaw_to_clover.py --preset user-data
```

Execute a user-data migration:

```bash
python $CLOVER_HOME/skills/migration/openclaw-migration/scripts/openclaw_to_clover.py --execute --preset user-data --skill-conflict skip
```

Execute a full compatible migration:

```bash
python $CLOVER_HOME/skills/migration/openclaw-migration/scripts/openclaw_to_clover.py --execute --preset full --migrate-secrets --skill-conflict skip
```

Execute with workspace instructions included:

```bash
python $CLOVER_HOME/skills/migration/openclaw-migration/scripts/openclaw_to_clover.py --execute --preset user-data --skill-conflict rename --workspace-target "/absolute/workspace/path"
```
...
- OpenClaw skills available under `$CLOVER_HOME/skills/openclaw-imports/`
