#!/usr/bin/env python3
"""OpenClaw -> Clover migration helper.

This script migrates the parts of an OpenClaw user footprint that map cleanly
into Clover Cognition, archives selected unmapped docs for manual review, and
...
MIGRATION_OPTION_METADATA: Dict[str, Dict[str, str]] = {
    "soul": {
        "label": "SOUL.md",
        "description": "Import the OpenClaw persona file into Clover.",
    },
    "workspace-agents": {
    ...
    "memory": {
        "label": "MEMORY.md",
        "description": "Import long-term memory entries into Clover memories.",
    },
    "user-profile": {
        "label": "USER.md",
        "description": "Import user profile entries into Clover memories.",
    },
    "messaging-settings": {
        "label": "Messaging settings",
        "description": "Import Clover-compatible messaging settings such as allowlists and working directory.",
    },
    "secret-settings": {
        "label": "Allowlisted secrets",
        "description": "Import the small allowlist of Clover-compatible secrets when explicitly enabled.",
    },
    "command-allowlist": {
        "label": "Command allowlist",
        "description": "Merge OpenClaw exec approval patterns into Clover command_allowlist.",
    },
    "skills": {
        "label": "User skills",
        "description": "Copy OpenClaw skills into ~/.clover/skills/openclaw-imports/.",
    },
    "tts-assets": {
        "label": "TTS assets",
        "description": "Copy compatible workspace TTS assets into ~/.clover/tts/.",
    },
    "discord-settings": {
        "label": "Discord settings",
        "description": "Import Discord bot token and allowlist into Clover .env.",
    },
    "slack-settings": {
        "label": "Slack settings",
        "description": "Import Slack bot/app tokens and allowlist into Clover .env.",
    },
    "whatsapp-settings": {
        "label": "WhatsApp settings",
        "description": "Import WhatsApp allowlist into Clover .env.",
    },
    "signal-settings": {
        "label": "Signal settings",
        "description": "Import Signal account, HTTP URL, and allowlist into Clover .env.",
    },
    "provider-keys": {
        "label": "Provider API keys",
        "description": "Import model provider API keys into Clover .env (requires --migrate-secrets).",
    },
    "model-config": {
        "label": "Default model",
        "description": "Import the default model setting into Clover config.yaml.",
    },
    "tts-config": {
        "label": "TTS configuration",
        "description": "Import TTS provider and voice settings into Clover config.yaml.",
    },
    "shared-skills": {
        "label": "Shared skills",
        "description": "Copy shared OpenClaw skills from ~/.openclaw/skills/ into Clover.",
    },
    "daily-memory": {
        "label": "Daily memory files",
        "description": "Merge daily memory entries from workspace/memory/ into Clover MEMORY.md.",
    },
    "archive": {
    ...
    "mcp-servers": {
        "label": "MCP servers",
        "description": "Import MCP server definitions from OpenClaw into Clover config.yaml.",
    },
    "plugins-config": {
    ...
    "cron-jobs": {
        ...
        "description": "Import cron job definitions. Archive for manual recreation via 'clover cron'.",
    },
    "hooks-config": {
    ...
    "agent-config": {
        ...
        "description": "Import agent defaults (compaction, context, thinking) into Clover config. Archive multi-agent list.",
    },
    "gateway-config": {
    ...
    "session-config": {
        ...
        "description": "Import session reset policies (daily/idle) into Clover session_reset config.",
    },
    "full-providers": {
        "label": "Full model provider definitions",
        "description": "Import custom model providers (baseUrl, apiType, headers) into Clover custom_providers.",
    },
    "deep-channels": {
    ...
    "browser-config": {
        ...
        "description": "Import browser automation settings into Clover config.yaml.",
    },
    "tools-config": {
        "label": "Tools configuration",
        "description": "Import tool settings (exec timeout, sandbox, web search) into Clover config.yaml.",
    },
    "approvals-config": {
        "label": "Approval rules",
        "description": "Import approval mode and rules into Clover config.yaml approvals section.",
    },
    "memory-backend": {
...
def load_yaml_file(path: Path) -> Dict[str, Any]:
    ...
    try:
        ...
        raise ConfigReadError(
            ...
            f"({exc}). Fix it with `clover config edit` (or move it aside), then "
    ...
    if not isinstance(data, dict):
        raise ConfigReadError(
            ...
            f"`clover config edit` (or move it aside), then re-run the migration."
        )
    return data
...
def dump_yaml_file(path: Path, data: Dict[str, Any]) -> None:
    ...
    ``~/.clover/config.yaml`` into a dotfiles repo or profile package.
    """
    if yaml is None:
        raise RuntimeError("PyYAML is required to update Clover config.yaml")
    ensure_parent(path)
...
# ── Brand rewriting ─────────────────────────────────────────
# Replace OpenClaw brand names with Clover in migrated text so that
# memory entries, user profiles, SOUL.md, and workspace instructions
# read as self-referential to the new agent identity.
#
# Case-preserving: ``OpenClaw`` → ``Clover`` (prose), but lowercase matches
# like ``openclaw`` → ``clover`` (so filesystem paths like ``~/.openclaw``
# become ``~/.clover`` — the real Clover home — not the broken ``~/.Clover``).
_REBRAND_PATTERNS: List[Tuple[re.Pattern, str]] = [
    (re.compile(r'\bOpen[\s-]?Claw\b', re.IGNORECASE), 'Clover'),
    (re.compile(r'\bClawdBot\b', re.IGNORECASE), 'Clover'),
    (re.compile(r'\bMoltBot\b', re.IGNORECASE), 'Clover'),
]
...
def _case_preserving_replacement(replacement: str):
    ...
    Keeps ``OpenClaw`` → ``Clover`` but maps ``openclaw`` → ``clover`` so a
    filesystem path like ``~/.openclaw/config.yaml`` rewrites to
    ``~/.clover/config.yaml`` (the real Clover home) instead of the broken
    ``~/.Clover/config.yaml``.
    """
...
def rebrand_text(text: str) -> str:
    """Replace OpenClaw / ClawdBot / MoltBot brand names with Clover.
...
def parse_existing_memory_entries(path: Path) -> List[str]:
    """Parse a DESTINATION Clover memory store (memories/MEMORY.md, USER.md).
...
def write_report(output_dir: Path, report: Dict[str, Any]) -> None:
    ...
    lines = [
        "# OpenClaw -> Clover Migration Report",
        "",
...
class Migrator:
    def __init__(
        ...
        # OpenClaw's extensions/migrate-clover/apply.ts "blocked by earlier
    ...
    # Option ids that mutate the Clover config.yaml file.  Once any one of
    ...
    def _build_warnings(self, summary: Dict[str, int]) -> List[str]:
        ...
        Modelled on OpenClaw's extensions/migrate-clover/plan.ts warnings[].
    ...
    def _build_next_steps(self, summary: Dict[str, int]) -> List[str]:
        ...
        if summary.get("migrated", 0) > 0:
            ...
            steps.append(
                "Start a new Clover session (or /reset) to pick up the imported config."
            )
        if summary.get("conflict", 0) > 0:
            steps.append(
    ...
    def migrate_command_allowlist(self) -> None:
        ...
        if not destination.exists():
            self.record("command-allowlist", source, destination, "skipped", "Clover config.yaml does not exist yet")
            return
    ...
    def migrate_messaging_settings(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        if isinstance(workspace, str) and workspace.strip():
            ...
            # that path will be stale after migration and would cause the Clover
        ...
        if additions:
            ...
            self.record("messaging-settings", self.source_root / "openclaw.json", self.target_root / ".env", "skipped", "No Clover-compatible messaging settings found")
    ...
    def migrate_secret_settings(self, config: Dict[str, Any]) -> None:
        ...
        if secret_additions:
            ...
            self.record(
                ...
                self.target_root / ".env",
                "skipped",
                "No allowlisted Clover-compatible secrets found",
                supported_targets=sorted(SUPPORTED_SECRET_TARGETS),
            )
    ...
    def migrate_provider_keys(self, config: Dict[str, Any]) -> None:
                ...
                if not api_key:
                    ...
                    if isinstance(raw_key, dict) and raw_key.get("source") in {"file", "exec"}:
                        self.record(
                            ...
                            f"that cannot be auto-migrated. Add this key manually via: clover config set",
                        )
                    continue
        ...
        for oc_key, clover_key in env_key_mapping.items():
            val = openclaw_env.get(oc_key, "").strip()
            if val and clover_key not in secret_additions:
                secret_additions[clover_key] = val
        ...
        if isinstance(json_env, dict):
            ...
            for src in sources:
                for oc_key, clover_key in env_key_mapping.items():
                    val = src.get(oc_key)
                    if isinstance(val, str) and val.strip() and clover_key not in secret_additions:
                        secret_additions[clover_key] = val.strip()
    ...
    def migrate_model_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        clover_config = load_yaml_file(destination)
        current_model = clover_config.get("model")
    ...
    def migrate_tts_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        if isinstance(provider, str) and provider in {"elevenlabs", "openai", "edge", "microsoft"}:
            # OpenClaw renamed "edge" to "microsoft"; Clover still uses "edge"
        ...
        clover_config = load_yaml_file(destination)
        existing_tts = clover_config.get("tts", {})
    ...
    def archive_docs(self) -> None:
        ...
        partially_extracted = [
            ...
            ("credentials/telegram-default-allowFrom.json", "Selected Clover-compatible values were extracted; raw credentials file was not copied."),
        ]
    ...
    def migrate_mcp_servers(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        clover_cfg_path = self.target_root / "config.yaml"
        clover_cfg = load_yaml_file(clover_cfg_path)
        existing_mcp = clover_cfg.get("mcp_servers") or {}
        added = 0
        ...
        for name, srv in mcp_raw.items():
            ...
            if name in existing_mcp and not self.overwrite:
                self.record("mcp-servers", f"mcp.servers.{name}", f"mcp_servers.{name}", "conflict",
                            "MCP server already exists in Clover config")
                continue

            clover_srv: Dict[str, Any] = {}
            # STDIO transport
            if srv.get("command"):
                clover_srv["command"] = srv["command"]
                if srv.get("args"):
                    clover_srv["args"] = srv["args"]
                if srv.get("env"):
                    clover_srv["env"] = srv["env"]
                if srv.get("cwd"):
                    clover_srv["cwd"] = srv["cwd"]
            # HTTP/SSE transport
            if srv.get("url"):
                clover_srv["url"] = srv["url"]
                if srv.get("headers"):
                    clover_srv["headers"] = srv["headers"]
                if srv.get("auth"):
                    clover_srv["auth"] = srv["auth"]
            # Common fields
            if srv.get("enabled") is False:
                clover_srv["enabled"] = False
            if srv.get("timeout"):
                clover_srv["timeout"] = srv["timeout"]
            if srv.get("connectTimeout"):
                clover_srv["connect_timeout"] = srv["connectTimeout"]
            # Tool filtering
            tools_cfg = srv.get("tools") or {}
            if tools_cfg.get("include") or tools_cfg.get("exclude"):
                clover_srv["tools"] = {}
                if tools_cfg.get("include"):
                    clover_srv["tools"]["include"] = tools_cfg["include"]
                if tools_cfg.get("exclude"):
                    clover_srv["tools"]["exclude"] = tools_cfg["exclude"]
            # Sampling
        ...
        if added > 0 and self.execute:
            self.maybe_backup(clover_cfg_path)
            clover_cfg["mcp_servers"] = existing_mcp
            dump_yaml_file(clover_cfg_path, clover_cfg)
        ...
        if cron:
            ...
            if self.archive_dir and self.execute:
                ...
                self.record("cron-jobs", "openclaw.json cron.*", str(dest), "archived",
                            "Cron config archived. Use 'clover cron' to recreate jobs manually.")
            else:
    ...
    def migrate_agent_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        # Map agent defaults
        agent_cfg = clover_cfg.get("agent") or {}
        ...
        if defaults.get("thinkingDefault"):
            # Map OpenClaw thinking -> Clover reasoning_effort
        ...
        if compaction:
            compression = clover_cfg.get("compression") or {}
    ...
    def migrate_gateway_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        self.record("gateway-config", "openclaw.json gateway.*", "archive/gateway-config.json",
                    "archived", "Gateway config archived. Use 'clover gateway' to configure.")

        # Extract gateway auth token to .env if present
        auth = gateway.get("auth") or {}
        if auth.get("token") and self.migrate_secrets:
            self._set_env_var("CLOVER_GATEWAY_TOKEN", auth["token"], "gateway.auth.token")
    ...
    def migrate_session_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        clover_cfg_path = self.target_root / "config.yaml"
        clover_cfg = load_yaml_file(clover_cfg_path)
        sr = clover_cfg.get("session_reset") or {}
        changes = False
    ...
    def migrate_full_providers(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        clover_cfg_path = self.target_root / "config.yaml"
        clover_cfg = load_yaml_file(clover_cfg_path)
        custom_providers = clover_cfg.get("custom_providers") or []
        added = 0
        ...
        if added > 0 and self.execute:
            self.maybe_backup(clover_cfg_path)
            clover_cfg["custom_providers"] = custom_providers
            dump_yaml_file(clover_cfg_path, clover_cfg)
    ...
    def migrate_deep_channels(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        # Map Discord-specific settings to Clover config
        discord_cfg = channels.get("discord") or {}
        if discord_cfg:
            clover_cfg_path = self.target_root / "config.yaml"
            clover_cfg = load_yaml_file(clover_cfg_path)
            discord_clover = clover_cfg.get("discord") or {}
            changed = False
            if "requireMention" in discord_cfg:
                discord_clover["require_mention"] = discord_cfg["requireMention"]
                changed = True
            if discord_cfg.get("autoThread") is not None:
                discord_clover["auto_thread"] = discord_cfg["autoThread"]
                changed = True
            if changed and self.execute:
                clover_cfg["discord"] = discord_clover
                dump_yaml_file(clover_cfg_path, clover_cfg)
    ...
    def migrate_browser_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        clover_cfg_path = self.target_root / "config.yaml"
        clover_cfg = load_yaml_file(clover_cfg_path)
        browser_clover = clover_cfg.get("browser") or {}
        changed = False

        # Map fields that have Clover equivalents
        if browser.get("cdpUrl"):
            browser_clover["cdp_url"] = browser["cdpUrl"]
            changed = True
        if browser.get("headless") is not None:
            browser_clover["headless"] = browser["headless"]
            changed = True

        if changed:
            clover_cfg["browser"] = browser_clover
            if self.execute:
                self.maybe_backup(clover_cfg_path)
                dump_yaml_file(clover_cfg_path, clover_cfg)
    ...
    def migrate_approvals_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        if mode:
            ...
            clover_mode = mode_map.get(mode, "manual")
            clover_cfg.setdefault("approvals", {})["mode"] = clover_mode
            if self.execute:
                self.maybe_backup(clover_cfg_path)
                dump_yaml_file(clover_cfg_path, clover_cfg)
            self.record("approvals-config", "openclaw.json approvals.mode",
                        "config.yaml approvals.mode", "migrated", f"Mapped '{mode}' -> '{clover_mode}'")
    ...
    def generate_migration_notes(self) -> None:
        if not self.output_dir:
            return
        notes = [
            "# OpenClaw -> Clover Migration Notes",
            "",
        ...
        if archived:
            notes.extend([
                ...
                "direct 1:1 mapping in Clover. Review each file and recreate manually:",
                "",
            ])
        ...
        if conflicts:
            notes.extend([
                "## Conflicts (Existing Clover Config Not Overwritten)",
                "",
                "These items already existed in your Clover config. Re-run with",
        ...
        notes.extend([
            ...
            "state files (todo.json, sessions, logs). If the Clover agent discovers these",
            "directories, it may read/write to them instead of the Clover state, causing",
            "confusion (e.g., cron jobs reading a different todo list than interactive sessions).",
            "",
            "**Strongly recommended:** Run `clover claw cleanup` to rename the OpenClaw",
            ...
            "or todo lists, run `clover claw cleanup` to fix it.",
            "",
            "## Clover-Specific Setup",
            "",
            "After migration, you may want to:",
            "- Run `clover claw cleanup` to archive the OpenClaw directory (prevents state confusion)",
            "- Run `clover setup` to configure any remaining settings",
            "- Run `clover mcp list` to verify MCP servers were imported correctly",
        ])

        if has_cron_config_archive:
            notes.append("- Run `clover cron` to recreate scheduled tasks (see archive/cron-config.json)")
        elif has_cron_store_archive:
            notes.append("- Run `clover cron` to recreate scheduled tasks (see archived cron-store)")
        ...
        notes.extend([
            "- Run `clover gateway install` if you need the gateway service",
            "- Review `~/.clover/config.yaml` for any adjustments",
            "",
        ])
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Migrate OpenClaw user state into Clover Cognition.")
    parser.add_argument("--source", default=str(Path.home() / ".openclaw"), help="OpenClaw home directory")
    parser.add_argument("--target", default=os.environ.get("CLOVER_HOME") or str(Path.home() / ".clover"), help="Clover home directory")
    parser.add_argument(
        "--workspace-target",
    ...
    parser.add_argument("--overwrite", action="store_true", help="Overwrite existing Clover targets after backing them up")
    parser.add_argument(
        "--migrate-secrets",
        action="store_true",
        help="Import a narrow allowlist of Clover-compatible secrets into the target env file",
    )
    parser.add_argument(
        "--skill-conflict",
...
def main() -> int:
    ...
    print()
    print("  ╔══════════════════════════════════════════════════════╗")
    print(f"  ║   OpenClaw -> Clover Migration   [{mode_label:>8s}]   ║")
    print("  ╠══════════════════════════════════════════════════════╣")
    ...
    if migrated:
        ...
        for item in migrated:
            ...
            if dest.startswith(str(report["target_root"])):
                dest = "~/.clover/" + dest[len(str(report["target_root"])) + 1:]
    ...
    if args.execute:
        print()
        print("  Next steps:")
        print("    1. Review ~/.clover/config.yaml")
        print("    2. Run: clover mcp list")
        if any(i["kind"] == "cron-jobs" and i["status"] == "archived" for i in items):
            print("    3. Recreate cron jobs: clover cron")
