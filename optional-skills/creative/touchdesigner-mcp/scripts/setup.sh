...
CLOVER_HOME_DIR="${CLOVER_HOME:-$HOME/.clover}"
CLOVER_CFG="${CLOVER_HOME_DIR}/config.yaml"
MCP_PORT=40404
...
# ── 3. Ensure Clover config has twozero_td MCP entry ──
if [[ ! -f "$CLOVER_CFG" ]]; then
    echo -e " ${FAIL} Clover config not found at ${CLOVER_CFG}"
    manual_steps+=("Create ${CLOVER_CFG} with twozero_td MCP server entry")
elif grep -q 'twozero_td' "$CLOVER_CFG" 2>/dev/null; then
    echo -e " ${OK} twozero_td MCP entry exists in Clover config"
else
    echo -e " ${WARN} Adding twozero_td MCP entry to Clover config..."
    python3 -c "
...
" 2>/dev/null && echo -e " ${OK} twozero_td MCP entry added to config" \
              || { echo -e " ${FAIL} Could not update config (is PyYAML installed?)"; \
                   manual_steps+=("Add twozero_td MCP entry to ${CLOVER_CFG} manually"); }
    manual_steps+=("Restart Clover session to pick up config change")
fi
