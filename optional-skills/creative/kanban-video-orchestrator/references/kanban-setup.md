...
the actual `setup.sh` that creates the project workspace, configures Clover
...
> [cloverc1/kanban-video-pipeline](https://github.com/cloverc1/kanban-video-pipeline).
...
2. **Create profiles** — `clover profile create <name> --clone`
3. **Configure profiles** — patch each profile's
   `$CLOVER_HOME/profiles/<name>/config.yaml` to set toolsets, always_load skills,
   and `cwd`
4. **Write SOUL.md per profile** — the personality + role definition
5. **Copy any provided assets + write `brief.md`, `TEAM.md`, and `taste/`**
6. **Fire the initial kanban task** — `clover kanban create` assigned to the director
...
```bash
clover profile create director --clone 2>/dev/null || true
```
...
Each profile has a YAML config at `$CLOVER_HOME/profiles/<name>/config.yaml`. The
...
p = os.path.join(os.environ.get("CLOVER_HOME", os.path.expanduser("~/.clover")),
...
PyYAML must be installed in the user's Python (it ships with most Clover
...
Each profile gets a `SOUL.md` at `$CLOVER_HOME/profiles/<name>/SOUL.md` that
...
```bash
clover kanban create "Direct production of <video title>" \
    --assignee director \
...
the Clover `.env` (`${CLOVER_HOME:-$HOME/.clover}/.env`) and macOS Keychain
(if on macOS):
...
check_key() {
    ...
    local kc_account="$2"
    local kc_service="$3"
    local _clover_env="${CLOVER_HOME:-$HOME/.clover}/.env"
    if grep -q "^${var}=" "$_clover_env" 2>/dev/null && \
       [ -n "$(grep "^${var}=" "$_clover_env" | cut -d= -f2-)" ]; then
        return 0
    fi
    ...
    echo "ERROR: ${var} not set in ${_clover_env} or Keychain (${kc_account}/${kc_service})"
    return 1
}

check_key ELEVENLABS_API_KEY clover ELEVENLABS_API_KEY || exit 1
check_key OPENROUTER_API_KEY clover OPENROUTER_API_KEY || exit 1
# ...
```
