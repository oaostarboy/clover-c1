...
> [CloverCognition/kanban-video-pipeline](https://github.com/CloverCognition/kanban-video-pipeline).
...
3. **Configure profiles** — patch each profile's
   `~/.clover/profiles/<name>/config.yaml` to set toolsets, always_load skills,
   and `cwd`
...
Each profile has a YAML config at `~/.clover/profiles/<name>/config.yaml`. The
...
p = os.path.expanduser(f"~/.clover/profiles/{profile}/config.yaml")
with open(p) as f:
...
Each profile gets a `SOUL.md` at `~/.clover/profiles/<name>/SOUL.md` that
