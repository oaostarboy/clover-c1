...
Maps each role archetype to the Clover skills it should `always_load` and the
toolsets it needs. Only references skills that ship in the public clover-c1
...
## Clover skills relevant to video production

### Visual / rendering skills (`clover-c1/skills/creative/`)
...
### Design / pre-production skills (`clover-c1/skills/creative/`)
...
### Audio / media skills (`clover-c1/skills/creative/` + `skills/media/`)
...
These are **not** Clover skills but external CLIs / APIs that profiles invoke.
...
## Built-in Clover tools for media review

These are native Clover tools — not invoked via terminal but through their own
...
key is present in `${CLOVER_HOME:-~/.clover}/.env` (or macOS Keychain) before firing the kanban.
...
| Anthropic | `ANTHROPIC_API_KEY` | every Clover profile (Claude) |

If a key is missing, prompt the user to add it. Storage methods, in order of
preference: macOS Keychain → `${CLOVER_HOME:-~/.clover}/.env` → environment variable.
...
When a new Clover-public video skill ships:
