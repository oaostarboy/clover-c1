...
clover kanban watch --tenant <project-slug>

# Snapshot of the board
clover kanban list --tenant <project-slug>
clover kanban list --tenant <project-slug> --json     # machine-readable

# Per-status counts + oldest-ready age
clover kanban stats --tenant <project-slug>

# Visual dashboard (browser)
clover dashboard

# Inspect a specific task (includes comments + events)
clover kanban show <task-id>

# Follow a single task's event stream
clover kanban tail <task-id>
```

Verify available subcommands with `clover kanban --help` — the kanban CLI
...
|---------|--------------|--------|
| Task RUNNING but no heartbeat in 2+ min | Worker stuck, infinite loop, blocked on input | `clover kanban show <id>` — read the worker's last events. The dispatcher SIGTERMs tasks that exceed their `max-runtime`; if you need to stop one earlier, `clover kanban block <id>` then `clover kanban archive <id>`, and create a re-run task. |
| Same task retried 2+ times | Reproducible failure (missing key, bad spec, broken tool) | `clover kanban show <id>` to read failure events. Fix root cause before re-running. |
| RUNNING longer than max_runtime | Task is slow but progressing OR genuinely stuck | Check heartbeats with `clover kanban tail <id>`. If progressing, the dispatcher will SIGTERM eventually anyway — raise `max-runtime` on a re-created task. |
...
| Tasks created but never picked up | Profile not running, or tenant mismatch, or dispatcher not running | Check `clover profile list` (profile exists?), `clover status` (gateway/dispatcher up?), and verify tenant. |
...
clover kanban comment <renderer-task-id> "Scene 3 looks too sparse \
— increase visual density. Tighten color palette to brand spec."

# 2. Create a re-render task with the original as parent
clover kanban create "Scene 3 — re-render with feedback" \
...
NEW_TASK_ID=$(clover kanban create "Generate SRT captions from voiceover" \
    --assignee captioner \
    --workspace dir:"$HOME/projects/video-pipeline/<slug>" \
    --tenant <slug> \
...
clover kanban link "$NEW_TASK_ID" <editor-task-id>
```
...
clover kanban block <task-id>
clover kanban archive <task-id>

# Diagnose what happened
clover kanban show <task-id>      # task body, comments, recent events
clover kanban tail <task-id>      # follow the live event stream
clover kanban log <task-id>       # worker process log
```
...
3. Re-fire the initial `clover kanban create` for the director
...
while true; do
    clear
    clover kanban list --tenant <slug>
    echo "---"
    clover kanban stats --tenant <slug>
    sleep 30
done
```

For a live event feed, run `clover kanban watch --tenant <slug>` in a
...
- **Profile process not running.** Tasks queue indefinitely in READY if no
  worker for that profile is online. Check `clover profile list` and start
