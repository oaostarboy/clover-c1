#!/usr/bin/env python3
"""
Monitor a running video-production kanban. Polls `clover kanban list` and
...
def clover_available() -> bool:
    return shutil.which("clover") is not None


def kanban_list(tenant: str) -> list[dict]:
    """Returns parsed task rows. Falls back to plain stdout parsing if JSON
    output isn't supported by the installed clover CLI."""
    try:
        out = subprocess.run(
        ...
        pass
    # Fallback: textual parse of `clover kanban list`
    out = subprocess.run(
...
def main():
    ...
    if not clover_available():
        print("ERROR: 'clover' CLI not found in PATH", file=sys.stderr)
        sys.exit(1)
