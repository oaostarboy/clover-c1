...
Usage (via cron with --no-agent):
    ...
    clover cron create api-events \\
      --schedule "*/1 * * * *" --no-agent \\
      --script "$CLOVER_HOME/skills/devops/watchers/scripts/watch_http_json.py" \\
...
def main() -> int:
    ...
    req = urllib.request.Request(args.url, headers={"User-Agent": "Clover-Watcher/1.0"})
