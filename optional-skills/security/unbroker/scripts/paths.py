...
All per-subject data lives under PDD_DATA_DIR (default: $CLOVER_HOME/unbroker),
which is the same trust boundary Clover uses for .env and OAuth tokens.
"""
...
def clover_home() -> Path:
    return Path(os.environ.get("CLOVER_HOME") or (Path.home() / ".clover"))


def data_dir() -> Path:
    override = os.environ.get("PDD_DATA_DIR")
    return Path(override) if override else clover_home() / "unbroker"
