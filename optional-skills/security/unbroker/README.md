...
The design is **Clover-native**: a small deterministic Python CLI (`scripts/pdd.py`) owns the state
...
```bash
clover skills install official/security/unbroker
```

Then start a new Clover session and drive it (below). The skill works zero-config; a few optional
...
Drive it from a Clover session:
...
- **PII handling.** Dossiers live under the Clover home directory (`0600`, optionally
