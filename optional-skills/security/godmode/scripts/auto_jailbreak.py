...
Usage in execute_code:
    exec(open(os.path.expanduser(
        os.path.join(os.environ.get("CLOVER_HOME", os.path.expanduser("~/.clover")), "skills/red-teaming/godmode/scripts/auto_jailbreak.py")
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("CLOVER_HOME", Path.home() / ".clover")) / "skills" / "red-teaming" / "godmode"
...
CLOVER_HOME = Path(os.getenv("CLOVER_HOME", Path.home() / ".clover"))
CONFIG_PATH = CLOVER_HOME / "config.yaml"
PREFILL_PATH = CLOVER_HOME / "prefill.json"
...
MODEL_STRATEGIES = {
    ...
    # Clover/Clover models — already uncensored, just needs clean prompt
    "clover": {
        "order": ["prefill_only"],
        "system_templates": {},
    },
...
def _detect_model_family(model: str) -> str:
    ...
    if "grok" in model_lower or "x-ai" in model_lower:
        return "grok"
    if "clover" in model_lower or "clover" in model_lower:
        return "clover"
...
def _get_current_model() -> tuple:
    """Read current model and provider from Clover config.yaml.
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.clover/prefill.json."""
    ...
    if winning_strategy:
        ...
        if not dry_run:
            ...
            if verbose:
                ...
                print()
                print("[DONE] Jailbreak locked in. Restart Clover for changes to take effect.")
        else:
            if verbose:
...
def undo_jailbreak(verbose=True):
    ...
    if verbose:
        print("[UNDO] Jailbreak removed. Restart Clover for changes to take effect.")
