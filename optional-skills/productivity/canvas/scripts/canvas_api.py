#!/usr/bin/env python3
"""Canvas LMS API CLI for Clover Cognition.
...
def _check_config():
    ...
    if missing:
        clover_env = os.path.join(
            os.environ.get("CLOVER_HOME", os.path.expanduser("~/.clover")), ".env"
        )
        print(
            f"Missing required environment variables: {', '.join(missing)}\n"
            f"Set them in {clover_env} or export them in your shell.\n"
...
def main():
    parser = argparse.ArgumentParser(
        description="Canvas LMS API CLI for Clover Cognition"
    )
