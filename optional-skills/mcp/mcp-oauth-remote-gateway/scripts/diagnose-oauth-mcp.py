...
Usage:
  ...
  <server_name> matches the files in $CLOVER_HOME/mcp-tokens/<server>.json etc.
...
def _clover_home():
    # Prefer Clover' own resolver (profile-safe); fall back to env then ~/.clover.
    try:
        from clover_constants import get_clover_home
        return str(get_clover_home())
    except Exception:
        return os.environ.get("CLOVER_HOME") or os.path.expanduser("~/.clover")


def _tokens_dir():
    return os.path.join(_clover_home(), "mcp-tokens")
    ...
    status, hdrs, body = _post(
        ...
        data={
            ...
            "params": {"protocolVersion": "2025-06-18", "capabilities": {},
                       "clientInfo": {"name": "clover-diag", "version": "1.0"}},
        },
        headers={
