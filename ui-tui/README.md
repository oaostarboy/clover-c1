...
React + Ink terminal UI for Clover. TypeScript owns the screen. Python owns sessions, tools, model calls, and most command logic.
...
Interpreter resolution order is: `CLOVER_PYTHON` → `PYTHON` → `$VIRTUAL_ENV/bin/python` → `./.venv/bin/python` → `./venv/bin/python` → `python3` (or `python` on Windows).
...
- `src/app/setupHandoff.ts` — launches external `clover setup`, suspends Ink while it runs, opens a new session on success
...
- `commands/billing.ts` — `/billing`: manage Clover remote spending — buy credits, auto-reload, limits
...
- Input history is stored in `~/.clover/.clover_history` or under `CLOVER_HOME`.
...
### Billing (`billing.ts`)
`/billing` — manage Clover remote spending — buy credits, auto-reload, limits
...
### Credits (`credits.ts`)
`/credits` — Clover credit balance and browser top-up

### Setup (`setup.ts`)
`/setup` — launches external `clover setup` wizard, suspends Ink while it runs
...
```text
ui-tui/
  packages/clover-ink/   forked Ink renderer (local dep)
  src/
    ...
    app/
      ...
      setupHandoff.ts               launches external clover setup, suspends Ink while it runs
      ...
      slash/
        ...
        commands/
          billing.ts                /billing — manage Clover remote spending
    ...
    lib/
      ...
      parentLog.ts               append-only log to ~/.clover/tui-parent.log
    ...
    types/
      clover-ink.d.ts            type declarations for @clover/ink
...
tui_gateway/
  ...
  slash_worker.py        persistent CloverCLI subprocess for slash commands
```
