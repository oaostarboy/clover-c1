import type { RunExternalProcess } from '@clover/ink'
...
export async function runExternalSetup({ args, ctx, done, launcher, suspend }: RunExternalSetupOptions) {
  ...
  transcript.sys(`launching \`clover ${args.join(' ')}\`…`)
  ...
  if (result.error) {
    transcript.sys(`error launching clover: ${result.error}`)
  ...
  if (result.code !== 0) {
    transcript.sys(`clover ${args[0]} exited with code ${result.code}`)
    patchUiState({ status: 'setup required' })
