...
import { terminalBackgroundHex } from '@clover/ink'
...
export const debugCommands: SlashCommand[] = [
  ...
  {
    help: 'rescan $CLOVER_HOME/tui-widgets and (re)register user widget apps',
    name: 'widgets-reload',
    run: (_arg, ctx) => {
  ...
  {
    help: 'write a V8 heap snapshot + memory diagnostics (see CLOVER_HEAPDUMP_DIR)',
    name: 'heapdump',
    run: (_arg, ctx) => {
      ...
      ctx.transcript.panel('Theme', [
        {
          rows: [
            ...
            ['CLOVER_TUI_BACKGROUND', process.env.CLOVER_TUI_BACKGROUND ?? '(unset)'],
            ['CLOVER_TUI_THEME', process.env.CLOVER_TUI_THEME ?? '(unset)'],
