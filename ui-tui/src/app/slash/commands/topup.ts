import { driveChargeSettlement, type SettlementOutcome } from '@clover/shared/charge-settlement'
...
const renderBillingError = (
  ...
  switch (env.error) {
    ...
    case 'remote_spending_disabled':
      ...
      // turn it on from the portal's Clover Cognition page; this is NOT a per-terminal stop.
      sys(
        "Remote spending is off for this account — a billing admin can turn it on from the portal's Clover Cognition page."
      )
      ...
      ctx.gateway
        ...
        .then(
          ctx.guarded<BillingStateResponse>(s => {
            if (!s.logged_in) {
              sys('💳 Not logged into Clover Portal — run /portal to log in, then /topup.')
