...
import { Box, Text } from '@clover/ink'
...
/**
 * User widget apps — Clover authors its own TUI widgets, mirroring the
 * Python plugin contract: drop `<name>.mjs` into `$CLOVER_HOME/tui-widgets/`,
 ...
 * React.createElement.
 *
 * Trust model matches `~/.clover/plugins/`: files under CLOVER_HOME execute
...
const widgetsDir = () => join(process.env.CLOVER_HOME?.trim() || join(homedir(), '.clover'), 'tui-widgets')
 ...
 *  every change, so a widget Clover writes appears within ~a second — no
...
export function watchUserWidgets(dir = widgetsDir()): void {
  ...
  if (!attach()) {
    ...
    // user (or Clover) ever writes must hot-load too — a 10s poll here read
