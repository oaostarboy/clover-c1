import type { SkinBranding, SkinColors } from '@clover/shared/skin'
...
// Mirrors @clover/ink's colorize.ts. Keep local: app code compiles from
// ui-tui/src, while @clover/ink is bundled separately from packages/.
...
// The classic light-mode Clover look was never hand-authored: for years the
 ...
 *  answer when known (cached in CLOVER_TUI_BACKGROUND), else the mode's
 *  assumed pole. */
function referenceBackground(isLight: boolean, env: NodeJS.ProcessEnv = process.env): string {
  const cached = (env.CLOVER_TUI_BACKGROUND ?? '').trim()
...
// Terminal. Explicit CLOVER_TUI_THEME / COLORFGBG signals above still win,
...
// `CLOVER_TUI_BACKGROUND` is intentionally generic so a future OSC11
...
//
//   1. `CLOVER_TUI_LIGHT` boolean — `1`/`true`/`yes`/`on` → light;
//      `0`/`false`/`no`/`off` → dark.  Either explicit value wins
//      regardless of any later signal.
//   2. `CLOVER_TUI_THEME` named override — `light` / `dark` win over
//      every signal below.
//   3. `CLOVER_TUI_BACKGROUND` hex hint (3- or 6-digit) — luminance
...
//
// Anything we can't decide stays dark — the default Clover palette
// is the dark one.
export function detectLightMode(
  ...
  const lightFlag = (env.CLOVER_TUI_LIGHT ?? '').trim().toLowerCase()
  ...
  const themeFlag = (env.CLOVER_TUI_THEME ?? '').trim().toLowerCase()
  ...
  const bgHint = backgroundLuminance(env.CLOVER_TUI_BACKGROUND ?? '')
 ...
 * the OSC-11 background answer cached into CLOVER_TUI_BACKGROUND after
...
export function fromSkin(
  ...
  // itself into CLOVER_TUI_BACKGROUND. See #applySkin / syncThemeToTerminalBackground).
  ...
  //    base seeds for this polarity. The base's IDENTITY FILLS (Clover navy
  ...
  // the derived value already carries the identity seeds (e.g. Clover navy).
  const activeRow =
