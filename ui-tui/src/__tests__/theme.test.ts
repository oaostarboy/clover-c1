...
// CLOVER_TUI_THEME=light (or CLOVER_TUI_BACKGROUND set to something
...
describe('DEFAULT_THEME', () => {
  it('has brand defaults', async () => {
    ...
    expect(DEFAULT_THEME.brand.name).toBe('Clover Cognition')
  ...
  it('has color palette', async () => {
    ...
    expect(DEFAULT_THEME.color.primary).toBe('#00FF87')
...
describe('LIGHT_THEME', () => {
  it('avoids bright-yellow accents unreadable on white backgrounds (#11300)', async () => {
    ...
    expect(LIGHT_THEME.color.primary).not.toBe('#00FF87')
    expect(LIGHT_THEME.color.accent).not.toBe('#00D97E')
    expect(LIGHT_THEME.color.muted).not.toBe('#1B7A4B')
    expect(LIGHT_THEME.color.statusWarn).not.toBe('#00FF87')
  })
...
describe('detectLightMode', () => {
  ...
  it('honors CLOVER_TUI_LIGHT on/off', async () => {
    const { detectLightMode } = await importThemeWithCleanEnv()

    expect(detectLightMode({ CLOVER_TUI_LIGHT: '1' })).toBe(true)
    expect(detectLightMode({ CLOVER_TUI_LIGHT: 'true' })).toBe(true)
    expect(detectLightMode({ CLOVER_TUI_LIGHT: 'on' })).toBe(true)
    expect(detectLightMode({ CLOVER_TUI_LIGHT: '0' })).toBe(false)
    expect(detectLightMode({ CLOVER_TUI_LIGHT: 'off' })).toBe(false)
  })
  ...
  it('lets CLOVER_TUI_LIGHT=0 override a light COLORFGBG', async () => {
    const { detectLightMode } = await importThemeWithCleanEnv()

    expect(detectLightMode({ COLORFGBG: '0;15', CLOVER_TUI_LIGHT: '0' })).toBe(false)
  })

  it('honors CLOVER_TUI_THEME=light/dark as a symmetric explicit override', async () => {
    const { detectLightMode } = await importThemeWithCleanEnv()

    expect(detectLightMode({ CLOVER_TUI_THEME: 'light' })).toBe(true)
    expect(detectLightMode({ CLOVER_TUI_THEME: 'dark' })).toBe(false)
    expect(detectLightMode({ COLORFGBG: '0;15', CLOVER_TUI_THEME: 'dark' })).toBe(false)
    expect(detectLightMode({ COLORFGBG: '15;0', CLOVER_TUI_THEME: 'light' })).toBe(true)
  })

  it('uses CLOVER_TUI_BACKGROUND luminance when COLORFGBG is missing', async () => {
    const { detectLightMode } = await importThemeWithCleanEnv()

    expect(detectLightMode({ CLOVER_TUI_BACKGROUND: '#ffffff' })).toBe(true)
    expect(detectLightMode({ CLOVER_TUI_BACKGROUND: '#000000' })).toBe(false)
    expect(detectLightMode({ CLOVER_TUI_BACKGROUND: '#1e1e1e' })).toBe(false)
    // Three-char hex normalises like CSS.
    expect(detectLightMode({ CLOVER_TUI_BACKGROUND: '#fff' })).toBe(true)
    // Garbage falls through to the default-dark path.
    expect(detectLightMode({ CLOVER_TUI_BACKGROUND: 'not-a-colour' })).toBe(false)
  })
  ...
  it('rejects partially-invalid hex instead of silently truncating', async () => {
    ...
    expect(detectLightMode({ CLOVER_TUI_BACKGROUND: '#fffgff' })).toBe(false)
    expect(detectLightMode({ CLOVER_TUI_BACKGROUND: 'ffggff' })).toBe(false)
    expect(detectLightMode({ CLOVER_TUI_BACKGROUND: '#xyz' })).toBe(false)
    // Wrong length also rejected (no implicit padding/truncation).
    expect(detectLightMode({ CLOVER_TUI_BACKGROUND: '#fffff' })).toBe(false)
    expect(detectLightMode({ CLOVER_TUI_BACKGROUND: '#fffffff' })).toBe(false)
  })
...
describe('fromSkin', () => {
  ...
  // CLOVER_TUI_THEME=light would flip the base palette and make these
  ...
  it('derives completion current background from resolved completion background (polarity-compatible)', async () => {
    ...
    const { fromSkin } = await importThemeWithEnv({ CLOVER_TUI_BACKGROUND: '#ffffff' })
  ...
  it('normalizes non-banner foregrounds on light Apple Terminal', async () => {
    ...
    expect(theme.color.primary).toBe('#00FF87')
  ...
  it('skinIsLight: the authored background decides; host detection only when absent', async () => {
    const { skinIsLight } = await importThemeWithEnv({ CLOVER_TUI_BACKGROUND: '#ffffff' })
...
describe('background-aware adaptation (OSC-11 light terminals)', () => {
  ...
  it('leaves the same skin untouched on a dark background', async () => {
    const { fromSkin } = await importThemeWithEnv({ CLOVER_TUI_BACKGROUND: '#1e1e2e' })
    const { color } = fromSkin(SLATE_COLORS, {})
  ...
  it('empty skin on a light background resolves to the light base palette', async () => {
    const { fromSkin, LIGHT_THEME } = await importThemeWithEnv({ CLOVER_TUI_BACKGROUND: '#ffffff' })
  ...
  it('defaultThemeForCurrentBackground follows a late CLOVER_TUI_BACKGROUND write', async () => {
    ...
    expect(defaultThemeForCurrentBackground({ CLOVER_TUI_BACKGROUND: '#ffffff' }).color).toEqual(LIGHT_THEME.color)
  })
