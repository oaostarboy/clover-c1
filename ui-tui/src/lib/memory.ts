...
export interface HeapDumpResult {
  ...
  // the heavy snapshot because CLOVER_AUTO_HEAPDUMP was not enabled (#21767).
  suppressed?: boolean
  success: boolean
}
...
export async function performHeapDump(trigger: MemoryTrigger = 'manual'): Promise<HeapDumpResult> {
  try {
    ...
    const dir = process.env.CLOVER_HEAPDUMP_DIR?.trim() || join(homedir() || tmpdir(), '.clover', 'heapdumps')

    await mkdir(dir, { recursive: true })

    const base = `clover-${new Date().toISOString().replace(/[:.]/g, '-')}-${process.pid}-${trigger}`
    ...
    const autoEnabled = /^(?:1|true|yes|on)$/i.test((process.env.CLOVER_AUTO_HEAPDUMP ?? '').trim())
...
async function pruneHeapdumps(dir: string): Promise<void> {
  const raw = process.env.CLOVER_HEAPDUMP_MAX_BYTES?.trim()
