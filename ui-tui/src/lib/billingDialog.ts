 ...
 * confirm button carries the recovery: Clover → `/topup`, other providers → their
...
export function billingDialogCopy(block: BillingBlock): BillingDialogCopy {
  if (block.is_clover) {
    return {
      cancelLabel: 'Dismiss',
      confirmLabel: 'Top up',
      detail: 'Your Clover credit balance is exhausted — top up to keep going.',
      title: 'Out of Clover credits'
    }
  }
