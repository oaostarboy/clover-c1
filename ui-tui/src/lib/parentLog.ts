...
const logDir = join(process.env.CLOVER_HOME?.trim() || join(homedir(), '.clover'), 'logs')
...
// a real ~/.clover (tests must stay hermetic — see AGENTS.md).
...
export function recordParentLifecycle(line: string): void {
  ...
  try {
    ...
    if (!warned) {
      warned = true
      process.stderr.write('clover-tui: parent lifecycle log unavailable\n')
    }
  }
}
