 ...
 * `clover-boot-background` / `clover-boot-color-scheme` localStorage keys.
 *
...
// Profile-aware: the Python launcher exports CLOVER_HOME (set by
// _apply_profile_override) before spawning the TUI. Falling back to
// ~/.clover matches get_clover_home()'s default.
const bootFilePath = () => join(process.env.CLOVER_HOME ?? join(homedir(), '.clover'), 'tui-theme-boot.json')

// Never touch the user's real ~/.clover from test runs (the TS suite has no
// CLOVER_HOME isolation fixture).
...
export interface BootSeedResult {
  ...
  /** True when the cached config pin was seeded into CLOVER_TUI_THEME. */
  seededPin: boolean
}
...
export function seedBootEnvironment(boot: BootTheme | null, env: NodeJS.ProcessEnv): BootSeedResult {
  ...
  if (!boot || env.CLOVER_TUI_THEME || env.CLOVER_TUI_LIGHT) {
    return result
  }
  ...
  if (boot.mode) {
    env.CLOVER_TUI_THEME = boot.mode
  ...
  if (
    ...
    !env.CLOVER_TUI_BACKGROUND
  ) {
    env.CLOVER_TUI_BACKGROUND = boot.background
...
export function invalidateBootBackground(env: NodeJS.ProcessEnv = process.env): boolean {
  if (!seeded.seededBackground || env.CLOVER_TUI_BACKGROUND !== seeded.seededBackground) {
    return false
  }

  delete env.CLOVER_TUI_BACKGROUND
...
/** True when this boot replayed a cached config pin into CLOVER_TUI_THEME.
