...
export function shouldForceTruecolor(env: NodeJS.ProcessEnv = process.env): boolean {
  const override = (env.CLOVER_TUI_TRUECOLOR ?? '').trim()
...
if (shouldForceTruecolor()) {
  ...
  // incorrectly. Keep Clover on the safer TERM-driven 256-color path unless
  // users explicitly opt back in via CLOVER_TUI_TRUECOLOR=1.
