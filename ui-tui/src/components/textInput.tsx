import type { InputEvent, Key } from '@clover/ink'
import * as Ink from '@clover/ink'
 ...
 * but that accepts `key.meta` on macOS — and clover-ink reports Option as
...
export function supportsFastEchoTerminal(env: NodeJS.ProcessEnv = process.env): boolean {
  ...
  if (isTermuxTuiMode(env)) {
    const override = String(env.CLOVER_TUI_TERMUX_FAST_ECHO ?? '')
      .trim()
      .toLowerCase()
  ...
  useInput(
    (inp: string, k: Key, event: InputEvent) => {
      ...
      if (actionHome) {
        ...
        // ESC d, which clover-ink decodes as meta+'d'; without this branch it
