...
clover gateway      # 启动消息网关（Telegram、Discord 等）
clover setup        # 运行完整设置向导（一次性配置所有内容）
clover claw migrate # 从 OpenClaw 迁移（如果来自 OpenClaw）
...
Clover 有两种入口：用 `clover` 启动终端 UI，或运行网关从 Telegram、Discord、Slack、WhatsApp、Signal 或 Email 与之对话。进入对话后，许多斜杠命令在两种界面中通用。

| 操作 | CLI | 消息平台 |
|------|-----|----------|
| 开始对话 | `clover` | 运行 `clover gateway setup` + `clover gateway start`，然后给机器人发消息 |
...
如果你来自 OpenClaw，Clover 可以自动导入你的设置、记忆、技能和 API 密钥。

**首次安装时：** 安装向导（`clover setup`）会自动检测 `~/.openclaw` 并在配置开始前提供迁移选项。

**安装后任意时间：**

```bash
clover claw migrate              # 交互式迁移（完整预设）
clover claw migrate --dry-run    # 预览将要迁移的内容
clover claw migrate --preset user-data   # 仅迁移用户数据，不含密钥
clover claw migrate --overwrite  # 覆盖已有冲突
```

导入内容：
- **SOUL.md** — 人格文件
- **记忆** — MEMORY.md 和 USER.md 条目
- **技能** — 用户创建的技能 → `~/.clover/skills/openclaw-imports/`
- **命令白名单** — 审批模式
- **消息设置** — 平台配置、允许用户、工作目录
...
使用 `clover claw migrate --help` 查看所有选项，或使用 `openclaw-migration` 技能进行交互式代理引导迁移（含干运行预览）。
