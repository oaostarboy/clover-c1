...
# 5. Symlinks the 'clover' CLI command into a user-facing bin dir
...
echo ""
echo -e "${CYAN}☘ Clover Cognition Setup${NC}"
echo ""
...
if is_termux; then
    ...
    if [ -n "$UV_CMD" ]; then
        ...
        _uv_log="$(mktemp 2>/dev/null || echo "/tmp/clover-uv-install.$$.log")"
...
# ============================================================================
# PATH setup — symlink clover into a user-facing bin dir
# ============================================================================

echo -e "${CYAN}→${NC} Setting up clover command..."

CLOVER_BIN="$SCRIPT_DIR/venv/bin/clover"
COMMAND_LINK_DIR="$(get_command_link_dir)"
COMMAND_LINK_DISPLAY_DIR="$(get_command_link_display_dir)"
mkdir -p "$COMMAND_LINK_DIR"
ln -sf "$CLOVER_BIN" "$COMMAND_LINK_DIR/clover"
echo -e "${GREEN}✓${NC} Symlinked clover → $COMMAND_LINK_DISPLAY_DIR/clover"
...
# ============================================================================
# Seed bundled skills into ~/.clover/skills/
# ============================================================================

CLOVER_SKILLS_DIR="${CLOVER_HOME:-$HOME/.clover}/skills"
mkdir -p "$CLOVER_SKILLS_DIR"

echo ""
echo "Syncing bundled skills to ~/.clover/skills/ ..."
if "$SCRIPT_DIR/venv/bin/python" "$SCRIPT_DIR/tools/skills_sync.py" 2>/dev/null; then
    echo -e "${GREEN}✓${NC} Skills synced"
else
    # Fallback: copy if sync script fails (missing deps, etc.)
    if [ -d "$SCRIPT_DIR/skills" ]; then
        cp -rn "$SCRIPT_DIR/skills/"* "$CLOVER_SKILLS_DIR/" 2>/dev/null || true
...
echo "Other commands:"
echo "  clover status        # Check configuration"
if is_termux; then
    echo "  clover gateway       # Run gateway in foreground"
else
    echo "  clover gateway install # Install gateway service (messaging + cron)"
fi
echo "  clover cron list     # View scheduled jobs"
echo "  clover doctor        # Diagnose issues"
echo ""
...
if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
    ...
    "$SCRIPT_DIR/venv/bin/python" -m clover_cli.main setup
fi
