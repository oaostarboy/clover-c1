...
Plugins can launch and supervise fresh Clover child sessions without importing
...
active agent turn fails closed with `No active Clover parent session`.
...
overrides, and per-launch timeouts are explicitly rejected until Clover can
support them without weakening isolation. Use `allowed_toolsets` to narrow a
child; Clover's existing unsafe-tool block remains enforced.
