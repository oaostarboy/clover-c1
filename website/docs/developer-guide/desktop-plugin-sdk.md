...
title: "Desktop Plugin SDK (@clover/plugin-sdk)"
description: "Extend the native Clover Desktop app — panes, pages, sidebar nav, status bar, palette commands, keybinds, themes, and a scoped backend namespace, with one import and no build step."
---

# Desktop Plugin SDK

The native [Clover Desktop](/user-guide/desktop) app is contribution-driven: every
...
A **desktop plugin** is a single ESM file that default-exports a `CloverPlugin`.
It imports one module — `@clover/plugin-sdk` — and gets everything: the app's
...
`$CLOVER_HOME/desktop-plugins/<id>/plugin.js` and the app loads it within seconds
and hot-reloads every save.

:::warning This is not the web-dashboard plugin SDK
"Plugin" means several unrelated things across Clover. This page is the **native
desktop app** (`clover desktop`) SDK — the `@clover/plugin-sdk` module and
`$CLOVER_HOME/desktop-plugins/`. The **web dashboard** (`clover dashboard`) has
its own, unrelated plugin system on `window.__CLOVER_PLUGIN_SDK__` with a
`manifest.json` — documented at
[Extending the Dashboard](/user-guide/features/extending-the-dashboard). Python
CLI/gateway plugins are documented at [Build a Clover Plugin](/developer-guide/plugins).
...
|------|-------|-----|------------|
| **Disk** (recommended) | `$CLOVER_HOME/desktop-plugins/<id>/plugin.js` | users, agents | none — plain ESM, loaded uncompiled |
| **Unified package** | `$CLOVER_HOME/plugins/<id>/desktop/plugin.js` | plugins that also ship agent-side code | none — same disk pipeline |
| **Bundled** | `apps/desktop/src/plugins/<id>/plugin.tsx` | in-tree, shipped with the app | the app's own Vite build |

All three take the same `CloverPlugin` contract, appear in **Settings → Plugins**,
...
Create `$CLOVER_HOME/desktop-plugins/hello/plugin.js` (that's `~/.clover/...`
by default, or `~/.clover/profiles/<name>/...` under a named profile). The folder
name must equal the plugin `id`.

```javascript
// ~/.clover/desktop-plugins/hello/plugin.js
import { host, haptic, useValue } from '@clover/plugin-sdk'
...
function HelloPane() {
  ...
  return jsxs('div', {
    ...
    children: [
      jsx('div', { className: 'font-medium', children: 'Hello, Clover' }),
      jsx('div', {
...
The only importable specifiers are `@clover/plugin-sdk`, `react`, and
`react/jsx-runtime` — everything else fails to resolve, on purpose.
:::

## The plugin contract

A plugin default-exports a `CloverPlugin`:

```ts
interface CloverPlugin {
...
interface PluginContext {
  ...
  os: PluginOs
  /** Plugin-scoped JSON persistence (keys live under `clover.plugin.<id>.`). */
...
```javascript
import { ROUTES_AREA, SIDEBAR_NAV_AREA } from '@clover/plugin-sdk'
...
```javascript
import { STATUSBAR_AREAS, TITLEBAR_AREAS } from '@clover/plugin-sdk'
...
```javascript
import { PALETTE_AREA, KEYBINDS_AREA } from '@clover/plugin-sdk'
...
```javascript
import { THEMES_AREA } from '@clover/plugin-sdk'
...
```javascript
import { Button, useTheme } from '@clover/plugin-sdk'
...
```javascript
import { host, requestTheme } from '@clover/plugin-sdk'
...
```javascript
import { TRANSCRIPT_DIRECTIVE_AREA } from '@clover/plugin-sdk'
...
`window.clover.send('get-price eth')` (or a declarative
`<button data-clover-send="get-price eth">` — no script needed) hands that
...
```javascript
import { Contribute, TITLEBAR_AREAS } from '@clover/plugin-sdk'
...
for requests; `targetProfile` is the backend Clover profile served by that route.
...
approval/turn alerts use. It fires only while the user is away from Clover
...
ctx.os.notify({
  ...
  // Body click → focus Clover + navigate. Same vocabulary as OS deep links:
  activate: 'clover://index-network/intent/1',
  ...
  actions: [
    { id: 'open', label: 'Open', activate: 'clover://index-network/intent/1' },
    { id: 'dismiss', label: 'Dismiss', onAction: () => dismiss('1') },
  ],
})
```

`activate` is deeplink-compatible: `clover://index-network/intent/1` and the
hash path `/index-network/intent/1` resolve to the same in-app route (and the
same `clover://…` URL works as an OS deep link). Action buttons only render on
...
```javascript
import { useQuery, useMutation, useQueryClient, atom, computed, useValue } from '@clover/plugin-sdk'
...
```javascript
import { queryClient } from '@clover/plugin-sdk'
...
a package into `~/.clover/plugins` is inert on every surface until the user
...
backend, the remote box's `~/.clover/plugins` is not reachable as a filesystem —
...
the `clover://` scheme — a plain anchor on your website or README:
...
`clover-dev://`. Full link reference:
...
`dashboard/` subfolder of a regular Clover plugin and declare it in a
`manifest.json`:
...
clover-c1 codebase directly (`clover_state`, `clover_cli.config`, …). See
...
`config.yaml` (and not in `plugins.disabled`). Project plugins (`./.clover/`)
...
(`clover.plugin.<id>.*`) so plugins can't read or clobber each other:
...
export a `CloverPlugin`). It's discovered by `discoverBundledPlugins()` at boot —
...
1. It goes through the app's Vite build, so you can write **real JSX** and import
   the SDK by its `@clover/plugin-sdk` alias.
2. It's still lint-fenced to `@clover/plugin-sdk` + `react` only — no `@/…` app
   internals.
...
- **Only three specifiers resolve:** `@clover/plugin-sdk`, `react`,
...
| Plugin contract | `CloverPlugin`, `PluginContext`, `PluginContribution`, `PluginStorage`, `PluginOs`, `PluginRestOptions`, `PluginNativeNotificationInput`, `PluginNotificationAction`, `CloverOpenTarget`, `Contribution` |
...
### Agents: the `clover-desktop-plugins` skill

When an agent writes a desktop plugin, it should load the bundled
**`clover-desktop-plugins`** skill — it carries the same contract as this page in
...
`$CLOVER_HOME/desktop-plugins/<id>/plugin.js` and the folder name matches the
export `id`. Run ⌘K → **Reload desktop plugins**. Check the app for an error
toast naming the failure, and tail `clover logs gui -f`.

**"unsupported import" on load.** A disk plugin may only import
`@clover/plugin-sdk`, `react`, and `react/jsx-runtime`. Remove any other import.
...
`~/.clover/plugins/<id>/dashboard/manifest.json` has `"api": "plugin_api.py"`,
that the plugin is in `plugins.enabled` in `config.yaml`, and restart the gateway
(backend routes mount at startup). Tail `~/.clover/logs/errors.log` for
