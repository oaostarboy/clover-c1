...
* **Host-owned credentials.** OAuth tokens, refresh flows, the
  ...
  concept Clover already has applies. The plugin never sees a
...
clover-c1 itself). For the async surface (`acomplete()` /
...
result = ctx.llm.complete(
    ...
    provider=None,         # optional, gated — Clover provider id (e.g. "openrouter")
...
using Clover built-in auxiliary tasks; it does not permit another plugin's
...
plugins:
  entries:
    my-plugin:
      llm:
        # Allow this plugin to choose a different Clover provider
        # (must be one Clover already knows about — same names as
        # `clover model` and config.yaml model.provider).
        ...
        # plugin sends — Clover does not look anything up.
        allowed_models:
          - openai/gpt-4o-mini
...
* **Auth.** Pulls API keys, OAuth tokens, or refresh tokens from
  `~/.clover/auth.json` / env, including the credential pool when
...
* **Fallback chain.** If the user's primary provider 5xxs or 429s,
  the request goes through Clover' usual aggregator-aware fallback
...
Existing `ctx.*` methods extend an existing Clover subsystem:
...
* Implementation: [`agent/plugin_llm.py`](https://github.com/CloverCognition/clover-c1/blob/main/agent/plugin_llm.py)
* Tests: [`tests/agent/test_plugin_llm.py`](https://github.com/CloverCognition/clover-c1/blob/main/tests/agent/test_plugin_llm.py)
* Reference plugins (companion repo):
  * [`plugin-llm-example`](https://github.com/CloverCognition/clover-example-plugins/tree/main/plugin-llm-example) — sync structured extraction with image input
  * [`plugin-llm-async-example`](https://github.com/CloverCognition/clover-example-plugins/tree/main/plugin-llm-async-example) — async with `asyncio.gather()`
