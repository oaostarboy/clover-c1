...
The ACP adapter wraps Clover' synchronous `AIAgent` in an async JSON-RPC stdio server.
...
```text
clover acp / clover-acp / python -m acp_adapter
  -> acp_adapter.entry.main()
  -> parse --version / --check / --setup before server startup
  -> load ~/.clover/.env
  -> configure stderr logging
  -> construct CloverACPAgent
...
- `allow_once` -> Clover `once`
- `allow_always` -> Clover `always`
- reject options -> Clover `deny`

Timeouts and bridge failures deny by default.

### Tool rendering helpers

`acp_adapter/tools.py` maps Clover tools to ACP tool kinds and builds editor-facing content.
...
new_session(cwd)
  -> create SessionState
  -> create AIAgent(platform="acp", enabled_toolsets=["clover-acp"])
...
Instead it reuses Clover' runtime resolver:

- `acp_adapter/auth.py`
- `clover_cli/runtime_provider.py`

So ACP advertises and uses the currently configured Clover provider/credentials. It also always advertises a terminal setup auth method (`clover-setup`, args `--setup`) so first-run ACP clients can open Clover' interactive model/provider configuration before starting a normal ACP session.
...
- ACP sessions are persisted to the shared `~/.clover/state.db` (SessionDB) and transparently restored across process restarts; they appear in `session_search`
...
- `toolsets.py` — `clover-acp` toolset definition
- `clover_cli/main.py` — `clover acp` CLI subcommand
- `pyproject.toml` — `[acp]` optional dependency + `clover-acp` script
