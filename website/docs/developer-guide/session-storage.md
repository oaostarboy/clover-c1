...
Clover Cognition uses a SQLite database (`~/.clover/state.db`) to persist session
metadata, full message history, and model configuration across CLI and gateway
sessions. This replaces the earlier per-session JSONL file approach.

Source file: `clover_state.py`


## Architecture Overview

```
~/.clover/state.db (SQLite, WAL mode)
...
Abridged — see `SCHEMA_SQL` in `clover_state.py` for the full current column list
...
indexed columns — see `SCHEMA_SQL` in `clover_state.py` for the exact SQL.
...
Multiple clover processes (gateway + CLI sessions + worktree agents) share one
...
db = SessionDB()                           # Default: ~/.clover/state.db
...
Default path: `~/.clover/state.db`

This is derived from `clover_constants.get_clover_home()` which resolves to
`~/.clover/` by default, or the value of `CLOVER_HOME` environment variable.
