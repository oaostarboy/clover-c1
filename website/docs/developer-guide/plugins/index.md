...
title: "Build a Clover Plugin"
description: "Step-by-step guide to building a complete Clover plugin with tools, hooks, data files, and skills"
---

# Build a Clover Plugin

This guide walks through building a complete Clover plugin from scratch. By the end you'll have a working plugin with multiple tools, lifecycle hooks, shipped data files, and a bundled skill — everything the plugin system supports.

:::info Not sure which guide you need?
Clover has several distinct pluggable interfaces — some use Python `register_*` APIs, others are config-driven or drop-in directories. Use this map first:
...
| An **STT backend** (custom whisper / ASR CLI) | [Voice Message Transcription](/user-guide/features/tts#voice-message-transcription-stt) — set `CLOVER_LOCAL_STT_COMMAND` to an argv-tokenized template |
...
| **Gateway event hooks** (fire on startup, session events, commands) | [Event Hooks](/user-guide/features/hooks#gateway-event-hooks) — drop `HOOK.yaml` + `handler.py` into `~/.clover/hooks/<name>/` |
...
| **Additional skill sources** (custom GitHub repos, private skill indexes) | [Skills](/user-guide/features/skills) — `clover skills tap add <repo>` · [Publishing a tap](/user-guide/features/skills#publishing-a-custom-skill-tap) |
...
Plugins that integrate **someone else's product or project** — observability/metrics backends, vendor SaaS connectors, analytics dashboards, paid-service tie-ins — are built and distributed as **standalone plugin repos**, not merged into `CloverCognition/clover-c1`. Users install them into `~/.clover/plugins/` or via a pip entry point; everything in this guide works the same way from a standalone repo. This is a coupling-and-maintenance decision (the core moves fast and we don't own your backend), not a quality bar — a plugin can be excellent and still belong in its own repo. Promote it in the Clover Cognition Discord `#plugins-skills-and-skins` channel. See [CONTRIBUTING.md](https://github.com/CloverCognition/clover-c1/blob/main/CONTRIBUTING.md) for the policy.
:::

## Portable Agent Plugins v1 packages

Clover can also install and load directory packages that target the Agent
Plugins v1.0.0 format. This is a compatibility adapter for the portable
components Clover already owns. It does not replace native `plugin.yaml` plus
...
```bash
clover plugins install owner/repository --no-enable
clover plugins list
clover plugins enable <plugin-name>
```
...
Clover validates `plugin.json`, Agent Skills frontmatter, fixed component
...
profile-scoped writable directory managed by Clover.
...
Portable `streamable-http` entries are routed through Clover' existing native
...
same full-trust posture as other installed Clover plugins.
...
records it as Published. Clover keys behavior on the canonical v1.0.0 schema
...
not by one global plugin API number. Clover does not expose a
...
continue to work after a normal Clover upgrade.
...
- **Hook payloads are keyword payloads.** New hook data is added as keyword
  ...
  Clover inspects callback signatures: a legacy callback receives the fields it
...
- **Manifests are open to additions.** Unknown `plugin.yaml` fields are ignored.
  Older Clover releases can therefore load a plugin whose manifest contains
...
Clover enforces this contract with frozen external-plugin fixtures discovered
from an isolated `CLOVER_HOME`. Those tests load and invoke the plugin through
...
```bash
mkdir -p ~/.clover/plugins/calculator
cd ~/.clover/plugins/calculator
```

### Validate with Plugin Doctor

`clover plugins doctor [path-or-id]` runs the same directory discovery,
manifest parser, namespaced import, `register(ctx)`, hook registry, and tool
registry used by Clover itself. It reports invalid hook names, callbacks that do
...
```bash
clover plugins doctor . --ci
```

Doctor uses a temporary `CLOVER_HOME`, restores plugin registration state after
...
This tells Clover: "I'm a plugin called calculator, I provide tools and hooks." The `provides_tools` and `provides_hooks` fields are lists of what the plugin registers.
...
`llm.task_override` (see `clover_cli/plugin_capabilities.py` for the
...
`clover-c1.plugin_capabilities` entry-point group. Each declaration is
named `<plugin-id>.<capability-id>` and points at the same object as your
`clover-c1.plugins` entry point:

```toml
[project.entry-points."clover-c1.plugins"]
calculator = "my_pkg:register"

[project.entry-points."clover-c1.plugin_capabilities"]
"calculator.tools.override" = "my_pkg:register"
```

Clover reads these from installed metadata without importing your code, so
`clover plugins capabilities` and the consent flow stay accurate for pip
installs.
...
this Clover understands still loads with a warning.
...
| `python_dependencies` | list of str | Declared pip requirements (e.g. `"requests>=2.0,<3"`). **Declaration seam only** — Clover validates them, and `clover plugins install` / `clover plugins doctor` surface missing ones with a `pip install` hint, but Clover **never auto-installs** them. Pin upper bounds. |
...
arbitrary packages into Clover' shared venv is a conflict and supply-chain
...
[#64165](https://github.com/CloverCognition/clover-c1/issues/64165) and
[#15220](https://github.com/CloverCognition/clover-c1/issues/15220). Plugin
...
4. **Accept `**kwargs`:** Clover may pass additional context in the future.
...
- `ctx.register_cli_command()` registers a CLI subcommand (e.g. `clover my-plugin <subcommand>`)
...
- If this function crashes, the plugin is disabled but Clover continues fine
...
Use plugin-relative config keys for user-visible behavior. Clover resolves them
...
`<CLOVER_HOME>/plugin-data/`. Neither API exposes another plugin's namespace.
...
If your plugin doesn't show up — or shows up but isn't loading — set `CLOVER_PLUGINS_DEBUG=1` to get verbose discovery logs on stderr:

```bash
CLOVER_PLUGINS_DEBUG=1 clover plugins list
```
...
The same logs are always written to `~/.clover/logs/agent.log` at WARNING level (failures only) and DEBUG level (everything) when the env var is set. So if you can't run with the env var (e.g. from inside the gateway), tail the log file instead:

```bash
clover logs --level WARNING | grep -i plugin
```

Common reasons a plugin doesn't appear:

- **Not enabled in config** — plugins are opt-in. Run `clover plugins enable <name>` (the name comes from the `plugins list` output, which can be `<category>/<plugin>` for nested layouts).
- **Wrong directory layout:** Native packages use `~/.clover/plugins/<plugin-name>/plugin.yaml` (flat) or one category level. Portable packages use root `plugin.json` in the same locations. Anything deeper is ignored.
...
tree, and `clover plugins update` / `remove` git-pull or delete it — your
...
# <clover home>/plugin-data/<name>/ — created on first use
...
**Key properties:**
- Plugin skills are **read-only** — they don't enter `~/.clover/skills/` and can't be edited via `skill_manage`.
...
:::tip Legacy pattern
The old `shutil.copy2` pattern (copying a skill into `~/.clover/skills/`) still works but creates name collision risk with built-in skills. Prefer `ctx.register_skill()` for new plugins.
:::
...
When users run `clover plugins install`, they're **prompted interactively** for any missing `requires_env` variables. Values are saved to `.env` automatically.
...
| Rule | Why |
|---|---|
| Your feature key must appear in the in-tree `LAZY_DEPS` allowlist | Prevents a malicious config from coaxing Clover into installing arbitrary packages — only specs Clover itself ships are eligible |
...
auditable in `~/.clover/logs/agent.log`. Plugins load after built-in
...
does not ship with Clover core (user, project, or pip source),
...
plugins:
  entries:
    my-plugin:                    # the plugin's registry key from `clover plugins list`
...
loader, the plugin is disabled and Clover continues. The gate exists
...
You normally never edit this key by hand. `clover plugins enable <name>`
...
This is the only hook whose return value matters. When a `pre_llm_call` callback returns a dict with a `"context"` key (or a plain string), Clover injects that text into the **current turn's user message**. This is the mechanism for memory plugins, RAG integrations, guardrails, and any plugin that needs to provide the model with additional context.

#### Return format

```python
# Dict with context key
return {"context": "Recalled memories:\n- User prefers dark mode\n- Last project: clover-c1"}
...
Per-hook context is capped at `10,000` characters by default. Anything above the cap is written to `$CLOVER_HOME/hook_outputs/<session_id>/<uuid>.txt` and replaced with a head/tail preview plus the saved path. The model can read the full content via `read_file` or `terminal` if it genuinely needs it. This keeps a runaway plugin from inflating every subsequent turn's prompt and blowing out the prompt cache prefix. Tune in `config.yaml`:
...
hooks:
  output_spill:
    ...
    # directory: null      # default: $CLOVER_HOME/hook_outputs
```
...
- **The system prompt is Clover's territory** — it contains model-specific guidance, tool enforcement rules, personality instructions, and cached skill content. Plugins contribute context alongside the user's input, not by altering the agent's core instructions.
...
The canonical list of kinds is `VALID_MIDDLEWARE` in `clover_cli/middleware.py`:
...
| `llm_request` | `request`, `original_request`, context kwargs | Return `{"request": {...}}` to replace the effective provider kwargs before Clover sends them. |
...
- Middleware payloads carry `middleware_schema_version` (`clover.middleware.v1`) alongside the observer telemetry fields.
- Unknown kinds register with a warning instead of failing, so a plugin written against a newer Clover still loads on an older one.

### Register CLI commands

Plugins can add their own `clover <plugin>` subcommand tree:

```python
def _my_command(args):
    """Handler for clover my-plugin <subcommand>."""
    ...
    if sub == "status":
        ...
        print("Usage: clover my-plugin <status|config>")

def _setup_argparse(subparser):
    """Build the argparse tree for clover my-plugin."""
...
After registration, users can run `clover my-plugin status`, `clover my-plugin config`, etc.
...
|---|---|---|
| Invoked as | `/name` in a session | `clover name` in a terminal |
...
`ctx._cli_ref` is only populated in an **interactive CLI** session. It is `None` in the gateway, in non-interactive `clover chat -q` runs, and in **kanban-spawned worker sessions** — so any plugin logic that reaches through `_cli_ref` silently no-ops in exactly those contexts. Two stable, session-agnostic APIs cover what hooks actually need:

- **`ctx.profile_name`** — the active profile name (e.g. `"default"`, or the assignee profile in a kanban worker). Derived from `CLOVER_HOME`, so it works everywhere with no `_cli_ref` dependency.
...
For running a full `clover <subcommand>` (e.g. `clover kanban show`), shell out with the `terminal` tool via `ctx.dispatch_tool("terminal", {"command": "clover kanban show ..."})` — there is no in-process slash-command bridge for headless worker sessions, and tools are the supported way to drive Clover from a hook.
...
Clover has five specialized plugin types beyond the general surface. Each ships as a directory under `plugins/<category>/<name>/` (bundled) or `~/.clover/plugins/<category>/<name>/` (user). The contract differs by category — pick the one you need, then read its full guide.
...
def register(ctx):
    ctx.register_platform(
        ...
        # show up in `clover gateway status` without SDK instantiation.
...
Clover also accepts extensions that aren't Python plugins at all. These are shown in the [Pluggable interfaces table](/user-guide/features/plugins#pluggable-interfaces--where-to-go-for-each); the sections below sketch each authoring style briefly.

### MCP servers — register external tools

Model Context Protocol (MCP) servers register their own tools into Clover without any Python plugin. Declare them in `~/.clover/config.yaml`:
...
Clover connects to each server at startup, lists its tools, and registers them alongside built-ins. The LLM sees them exactly like any other tool. **Full guide:** [MCP](/user-guide/features/mcp).

### Gateway event hooks — fire on lifecycle events

Drop a manifest + handler into `~/.clover/hooks/<name>/`:

```yaml
# ~/.clover/hooks/long-task-alert/HOOK.yaml
...
```python
# ~/.clover/hooks/long-task-alert/handler.py
...
clover skills search my-workflow --source myorg/skills-repo
clover skills install myorg/skills-repo/my-workflow
```
...
```bash
pip install clover-plugin-calculator
# Plugin auto-discovered on next clover startup
```
...
```python
# Wrong — will break if Clover passes extra context
def handler(args):
    ...
