...
| Variable | Meaning |
|----------|---------|
| `CLOVER_MAIN_CHECKOUT` | The deps checkout — where `node_modules` really lives, and whose `.venv/bin/python` runs the backend. |
| `CLOVER_GUI_DEPS_CHECKOUT` | Where the desktop deps (`apps/desktop/node_modules`) live. Defaults to `CLOVER_MAIN_CHECKOUT`; override only if you keep desktop deps elsewhere. |

Neither is read by Clover itself — they're private to these helpers. The variables Clover *does* read are covered in [Environment Variables](../reference/environment-variables.md).

## `htui` — TUI from the worktree

The Ink TUI has a dev path already: `clover --tui --dev` runs the TypeScript sources via `tsx` instead of the prebuilt bundle. `htui` is a one-liner over it that also points the run at the current worktree's `ui-tui/`:

```bash
htui() {
  local root
  root="$(_clover_root)" || { echo "htui: not in a Clover checkout" >&2; return 1; }
  ( cd "$root" && PYTHONPATH="$root" \
      "$CLOVER_MAIN_CHECKOUT/.venv/bin/python" -m clover_cli.main --tui --dev "$@" )
}
```

`--dev` compiles from source, so it links `ui-tui/node_modules` from `CLOVER_MAIN_CHECKOUT` when the root lockfile matches and installs locally otherwise (see [`_clover_root` / linking helpers](#shared-helpers)).

:::warning `--dev` and `CLOVER_TUI_DIR` are mutually exclusive
`CLOVER_TUI_DIR` points Clover at a *prebuilt* bundle (Nix, system packages), which has no source to hot-reload. If it's set in your shell, `clover --tui --dev` exits with an error. Run `unset CLOVER_TUI_DIR` before `htui`.
:::
...
hgui() {
  ...
  root="$(_clover_root)" || { echo "hgui: not in a Clover checkout" >&2; return 1; }
  deps="${CLOVER_GUI_DEPS_CHECKOUT:-$CLOVER_MAIN_CHECKOUT}"
  ...
  if cmp -s "$root/package-lock.json" "$deps/package-lock.json"; then
    _clover_link_deps "$desktop" "$deps/apps/desktop"
    _clover_link_deps "$root" "$deps"
  else
  ...
  trap '_clover_gui_cleanup "$root"' INT TERM EXIT

  ( cd "$desktop"
    export PATH="$root/node_modules/.bin:$PATH"
    CLOVER_DESKTOP_CLOVER_ROOT="$root" \
    CLOVER_DESKTOP_PYTHON="$CLOVER_MAIN_CHECKOUT/.venv/bin/python" \
    CLOVER_DESKTOP_IGNORE_EXISTING=1 \
    CLOVER_DESKTOP_CWD="$root" \
    npm run dev )
}
```
...
|----------|----------------|
| `CLOVER_DESKTOP_CLOVER_ROOT` | Runs the backend from **this worktree**, not the packaged/PATH `clover`. |
| `CLOVER_DESKTOP_PYTHON` | Reuses the deps checkout's venv instead of re-resolving a Python. |
| `CLOVER_DESKTOP_IGNORE_EXISTING` | Ignores any `clover` on `PATH` so it can't shadow the worktree. |
| `CLOVER_DESKTOP_CWD` | Opens the desktop chat rooted at the worktree. |
...
```bash
# The enclosing worktree, verified as a real Clover checkout.
_clover_root() {
  local root
  root="$(git rev-parse --show-toplevel 2>/dev/null)" || return 1
  [[ -f "$root/clover_cli/main.py" && -d "$root/ui-tui" ]] && print -r "$root"
}
...
_clover_gui_cleanup() {
  ...
  lsof -t -i:5174 >/dev/null 2>&1 && killport 5174
  pgrep -f 'clover_cli\.main.*dashboard.*--port 0' 2>/dev/null | xargs -r kill -TERM 2>/dev/null
}
```
...
- [TUI](../user-guide/tui.md) — `clover --tui --dev` and the `CLOVER_TUI_DIR` prebuild path
- [Desktop App](../user-guide/desktop.md) — building from source and the backend resolution ladder
- [`apps/desktop/README.md`](https://github.com/CloverCognition/clover-c1/blob/main/apps/desktop/README.md) — dev server, sandbox script, and packaging
- [Environment Variables](../reference/environment-variables.md) — every `CLOVER_*` variable Clover reads
