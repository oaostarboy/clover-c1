...
Clover Cognition uses a dual compression system and Anthropic prompt caching to
...
Configure via `clover plugins` → Provider Plugins → Context Engine, or edit `config.yaml` directly.

For building a context engine plugin, see [Context Engine Plugins](/developer-guide/context-engine-plugin).

## Dual Compression System

Clover has two separate compression layers that operate independently:
...
compression:
  ...
  codex_app_server_auto: native  # native|clover|off for Codex app-server thread compaction
...
auxiliary:
  compression:
    ...
    provider: auto           # Provider: "auto", "openrouter", "clover", "main", etc.
...
| `codex_app_server_auto` | `native` | `native`, `clover`, `off` | Thread-compaction mode for Codex app-server sessions (see below) |
...
OAuth (`provider: openai-codex`) and the model is gpt-5.5, Clover raises the
trigger to **85%** (~231K) and shows a notice with the opt-out command. The
notice is shown once per profile — a marker under `$CLOVER_HOME`
...
```bash
clover config set compression.codex_gpt55_autoraise false
```

To keep the 85% autoraise but hide only the one-time notice:

```bash
clover config set compression.codex_gpt55_autoraise_notice false
```
...
for ChatGPT-subscription accounts (live-verified Aug 2026). Clover keeps the
...
`gpt-5.4-900k`). These are Clover-side aliases: the suffix is stripped before
...
thread context, so Clover' auxiliary summarizer cannot shrink it — rewriting the
...
- Automatic compaction is controlled by `compression.codex_app_server_auto`:
  the default `native` lets the app-server decide when to compact and Clover
  records the resulting compaction events (compression counters, session
  events). Set `clover` to let Clover' compression threshold initiate
  app-server compaction, or `off` to disable Clover-initiated automatic
  compaction entirely (codex may still compact natively).

Clover' local transcript is never rewritten on this runtime — state.db records
the compaction boundary while the visible transcript stays intact. All other
routes (including Codex OAuth chat sessions) keep Clover' summary compressor.
...
encrypted `compaction` output item. Clover captures that item into the
...
Anthropic allows a maximum of 4 `cache_control` breakpoints per request. Clover
...
5. **Model identity is part of the cache key**: Provider-side caches are scoped
   ...
   Clover can avoid; user-facing docs for `/model`, fallback providers, and
