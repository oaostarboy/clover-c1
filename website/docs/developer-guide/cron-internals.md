...
description: "How Clover stores, schedules, edits, pauses, skill-loads, and delivers cron jobs"
---
...
| `clover_cli/cron.py` | CLI `clover cron` subcommands |
...
Jobs are stored in `~/.clover/cron/jobs.json` with atomic write semantics (write to temp file, then rename). Each job record contains:
...
In CLI mode, cron jobs only fire when `clover cron` commands are run or during active CLI sessions.
...
keep the process awake), it asks Clover infrastructure to arm exactly **one
managed one-shot per job at that job's real next-fire time**. At fire time Clover
...
The flow (the managed scheduler is provided by Clover; the agent holds no
scheduler credentials):

```
create/update a cron job
  → Chronos asks Clover to arm a one-shot at the job's next_run_at
      (authenticated with the agent's existing Clover token)
  → at fire time Clover calls the gateway: POST {callback_url}/api/cron/fire
      (authenticated with a short-lived, purpose-scoped Clover-minted JWT)
...
Config (all non-secret; on hosted agents Clover sets these at provision time):

| key | meaning |
|---|---|
| `cron.provider` | `chronos` to activate (empty = built-in ticker) |
| `cron.chronos.portal_url` | Clover base URL (arming + the fire-token issuer) |
...
If Chronos is misconfigured or the agent isn't logged into Clover,
...
agent↔Clover wire contract lives in `docs/chronos-managed-cron-contract.md`.
...
```python
# ~/.clover/scripts/check_competitors.py
...
| Local file | `local` | Save to `~/.clover/cron/output/` |
...
**Bot Chat** (`bot-chat`, `bot-chat:<profile>`) is a machine-local pseudo-platform, not a gateway adapter: the scheduler delivers by running `clover [-p <profile>] chat --in ~ -c "Bot Chat" --create-if-missing -Q --query-file <tmp>` — the same lane Bot Mode agent-to-agent messages use — so the output arrives as a real inbound turn in the profile's canonical Bot Chat and the bot runs a full agent turn on it (alternation-safe by construction; this is the chat command lane, not a transcript mirror). The bare token targets the job's own profile; the named form is validated against `~/.clover/profiles/` at create time and again at fire time, and never resolves across machines. Bot-chat targets are excluded from the `all` routing token and from delivery preflight (no gateway credentials involved). The per-delivery subprocess timeout is `cron.bot_chat_delivery_timeout_seconds` (default 600).
...
The scheduler uses cross-process file-based locking (`fcntl.flock` on Unix, `msvcrt.locking` on Windows) to prevent overlapping ticks from executing the same due-job batch twice — even between the gateway's in-process ticker and a standalone `clover cron` / manual `tick()` call. If the lock cannot be acquired, `tick()` returns 0 immediately.

## CLI Interface

The `clover cron` CLI provides direct job management:

```bash
clover cron list                    # Show all jobs
clover cron create                  # Interactive job creation (alias: add)
clover cron edit <job_id>           # Edit job configuration
clover cron pause <job_id>          # Pause a running job
clover cron resume <job_id>         # Resume a paused job
clover cron run <job_id>            # Trigger immediate execution
clover cron remove <job_id>         # Delete a job
```
