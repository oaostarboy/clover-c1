...
Clover Cognition saves conversation trajectories in ShareGPT-compatible JSONL format
...
It follows the Clover function-calling prompt template with:
...
Trajectory saving is a `run_agent.py` / library-level switch — the `clover` CLI
