...
```bash
clover teams-pipeline validate
```
...
```bash
clover teams-pipeline token-health
clover teams-pipeline token-health --force-refresh
```
...
```bash
clover teams-pipeline subscriptions
```

### Renew near-expiry subscriptions

```bash
clover teams-pipeline maintain-subscriptions
clover teams-pipeline maintain-subscriptions --dry-run
```
...
#### Option 1: Clover cron (recommended if you already run the Clover gateway)

Clover ships a built-in cron scheduler. The `--no-agent` mode runs a script as the job (rather than using an LLM), and `--script` must point at a file under `~/.clover/scripts/`. First create the script:

```bash
mkdir -p ~/.clover/scripts
cat > ~/.clover/scripts/maintain-teams-subscriptions.sh <<'EOF'
#!/usr/bin/env bash
exec clover teams-pipeline maintain-subscriptions
EOF
chmod +x ~/.clover/scripts/maintain-teams-subscriptions.sh
```

Then register a script-only cron job that runs every 12 hours (gives 6x headroom against the 72h expiry window):

```bash
clover cron create "0 */12 * * *" \
...
Create `/etc/systemd/system/clover-teams-pipeline-maintain.service`:

```ini
[Unit]
Description=Clover Teams pipeline subscription maintenance
After=network-online.target

[Service]
Type=oneshot
User=clover
EnvironmentFile=/etc/clover/env
ExecStart=/usr/local/bin/clover teams-pipeline maintain-subscriptions
```

And `/etc/systemd/system/clover-teams-pipeline-maintain.timer`:

```ini
[Unit]
Description=Run Clover Teams pipeline subscription maintenance every 12 hours
...
sudo systemctl enable --now clover-teams-pipeline-maintain.timer
systemctl list-timers clover-teams-pipeline-maintain.timer
```

#### Option 3: Plain crontab

```cron
0 */12 * * * /usr/local/bin/clover teams-pipeline maintain-subscriptions >> /var/log/clover/teams-pipeline-maintain.log 2>&1
```

Make sure the cron environment has the `MSGRAPH_*` credentials. Simplest fix: source `~/.clover/.env` at the top of a wrapper script that crontab calls.
...
```bash
clover teams-pipeline subscriptions   # should show expirationDateTime advanced
clover teams-pipeline maintain-subscriptions --dry-run   # should show "0 expiring soon" most of the time
```
...
```bash
clover teams-pipeline list
clover teams-pipeline list --status failed
clover teams-pipeline show <job-id>
```

### Replay a stored job

```bash
clover teams-pipeline run <job-id>
```

### Dry-run meeting artifact fetches

```bash
clover teams-pipeline fetch --meeting-id <meeting-id>
clover teams-pipeline fetch --join-web-url "<join-url>"
clover teams-pipeline fetch --join-web-url "<join-url>" --organizer-user-id <entra-user-id>
```
...
- run `clover teams-pipeline maintain-subscriptions --dry-run`
- inspect `clover teams-pipeline list --status failed`
...
- run `clover teams-pipeline validate`
...
Check:
- whether you manually replayed a job with `clover teams-pipeline run`
...
- [ ] `clover teams-pipeline validate` returns an OK snapshot
- [ ] `clover teams-pipeline token-health --force-refresh` succeeds
- [ ] **`maintain-subscriptions` is scheduled** (Clover cron, systemd timer, or crontab — see [Automating subscription renewal](#automating-subscription-renewal-required-for-production)). Without this, Graph subscriptions silently expire within 72 hours.
