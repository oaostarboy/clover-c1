...
Copy-paste blueprints for common automation patterns. Each blueprint uses Clover's built-in [cron scheduler](/user-guide/features/cron) for time-based triggers and [webhook platform](/user-guide/messaging/webhooks) for event-driven triggers.
...
| **GitHub Event** | Fires on PR opens, pushes, issues, CI results | Webhook platform (`clover webhook subscribe`) |
| **API Call** | External service POSTs JSON to your endpoint | Webhook platform (config.yaml routes or `clover webhook subscribe`) |
...
```bash
clover cron create "0 2 * * *" \
  "You are a project manager triaging the CloverCognition/clover-c1 GitHub repo.

1. Run: gh issue list --repo CloverCognition/clover-c1 --state open --json number,title,labels,author,createdAt --limit 30
...
```bash
clover webhook subscribe github-pr-review \
  --events "pull_request" \
...
```bash
clover cron create "0 9 * * 1" \
  "Scan the CloverCognition/clover-c1 repo for documentation drift.

1. Run: gh pr list --repo CloverCognition/clover-c1 --state merged --json number,title,files,mergedAt --limit 30
2. Filter to PRs merged in the last 7 days
3. For each merged PR, check if it modified:
   - Tool schemas (tools/*.py) — may need docs/reference/tools-reference.md update
   - CLI commands (clover_cli/commands.py, clover_cli/main.py) — may need docs/reference/cli-commands.md update
   - Config options (clover_cli/config.py) — may need docs/user-guide/configuration.md update
...
```bash
clover cron create "0 6 * * *" \
  "Run a dependency security audit on the clover-c1 project.

1. cd ~/.clover/clover-c1 && source .venv/bin/activate
...
```bash
clover webhook subscribe deploy-verify \
  --events "deployment" \
...
```bash
clover webhook subscribe alert-triage \
...
```python title="~/.clover/scripts/check-uptime.py"
...
for ep in ENDPOINTS:
    try:
        start = time.time()
        req = urllib.request.Request(ep["url"], headers={"User-Agent": "Clover-Monitor/1.0"})
...
clover cron create "every 30m" \
  ...
  --script ~/.clover/scripts/check-uptime.py \
  --name "Uptime monitor" \
  --deliver telegram
```
...
```bash
clover cron create "0 8 * * *" \
  "Search arXiv for the 3 most interesting papers on 'language model reasoning' OR 'tool-use agents' from the past day. For each paper, create an Obsidian note with the title, authors, abstract summary, key contribution, and potential relevance to Clover Cognition development." \
...
```bash
clover webhook subscribe auto-port \
  --events "pull_request" \
...
```bash
clover webhook subscribe stripe-payments \
...
```bash
clover cron create "0 3 * * 0" \
  "Run a comprehensive security audit of the clover-c1 codebase.
