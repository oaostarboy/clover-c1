---
sidebar_position: 7
title: "Use SOUL.md with Clover"
description: "How to use SOUL.md to shape Clover Cognition's default voice, what belongs there, and how it differs from AGENTS.md and /personality"
---

# Use SOUL.md with Clover

`SOUL.md` is the **primary identity** for your Clover instance. It's the first thing in the system prompt — it defines who the agent is, how it speaks, and what it avoids.

If you want Clover to feel like the same assistant every time you talk to it — or if you want to replace the Clover persona entirely with your own — this is the file to use.
...
- personality
- communication style
- how direct or warm Clover should be
- what Clover should avoid stylistically
- how Clover should relate to uncertainty, disagreement, and ambiguity

In short:
- `SOUL.md` is about who Clover is and how Clover speaks
...
Clover now uses only the global SOUL file for the current instance:

```text
~/.clover/SOUL.md
```

If you run Clover with a custom home directory, it becomes:
...
Clover automatically seeds a starter `SOUL.md` for you if one does not already exist.

That means most users now begin with a real file they can read and edit immediately.

Important:
- if you already have a `SOUL.md`, Clover does not overwrite it
- if the file exists but is empty, Clover adds nothing from it to the prompt

## How Clover uses it

When Clover starts a session, it reads `SOUL.md` from `CLOVER_HOME`, scans it for prompt-injection patterns, truncates it if needed, and uses it as the **agent identity** — slot #1 in the system prompt. This means SOUL.md completely replaces the built-in default identity text.
...
That alone can noticeably change how Clover feels.
...
Clover already tries to be helpful and clear. `SOUL.md` should add real personality and style, not restate obvious defaults.
...
# Style
How Clover should sound.

# Avoid
What Clover should not do.

# Defaults
How Clover should behave when ambiguity appears.
```
...
Then restart Clover or start a new session.
...
### I edited SOUL.md but Clover still sounds the same

Check:
- you edited `~/.clover/SOUL.md` or `$CLOVER_HOME/SOUL.md`
...
### Clover is ignoring parts of my SOUL.md
