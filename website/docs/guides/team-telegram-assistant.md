...
This tutorial walks you through setting up a Telegram bot powered by Clover Cognition that multiple team members can use. By the end, your team will have a shared AI assistant they can message for help with code, research, system administration, and anything else — secured with per-user authorization.
...
- **Clover Cognition installed** on a server or VPS (not your laptop — the bot needs to stay running). Follow the [installation guide](/getting-started/installation) if you haven't yet.
- **A Telegram account** for yourself (the bot owner)
- **An LLM provider configured** — at minimum, an API key for OpenAI, Anthropic, or another supported provider in `~/.clover/.env`

:::tip
A $5/month VPS is plenty for running the gateway. Clover itself is lightweight — the LLM API calls are what cost money, and those happen remotely.
:::
...
2. **Send `/newbot`** — BotFather will ask you two things:
   - **Display name** — what users see (e.g., `Team Clover Assistant`)
   - **Username** — must end in `bot` (e.g., `myteam_clover_bot`)
...
4. **Set a description** (optional but recommended):
   ...
   ```
   Team AI assistant powered by Clover Cognition. DM me for help with code, research, debugging, and more.
   ```
...
```
[Gateway] Starting Clover Gateway...
...
# View live logs
journalctl --user -u clover-gateway -f
...
:::tip macOS PATH
The launchd plist captures your shell PATH at install time so gateway subprocesses can find tools like Node.js and ffmpeg. If you install new tools later, re-run `clover gateway install` to update the plist.
:::
...
```bash
clover gateway stop && clover gateway start
```
...
clover pairing revoke telegram 987654321
...
**Option 2:** Set it manually in `~/.clover/.env`:
...
Control how much detail the bot shows when using tools. In `~/.clover/config.yaml`:
...
Customize how the bot communicates by editing `~/.clover/SOUL.md`:

For a full guide, see [Use SOUL.md with Clover](/guides/use-soul-with-clover).
...
```bash
# From the CLI
clover cron list          # View all scheduled jobs
clover cron status        # Check if scheduler is running
...
| What | Location |
|------|----------|
| Gateway logs | `journalctl --user -u clover-gateway` (Linux) or `~/.clover/logs/gateway.log` (macOS) |
| Cron job output | `~/.clover/cron/output/{job_id}/{timestamp}.md` |
| Cron job definitions | `~/.clover/cron/jobs.json` |
| Pairing data | `~/.clover/pairing/` |
| Session history | `~/.clover/sessions/` |
