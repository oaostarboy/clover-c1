...
description: "Send text from any shell script, cron job, CI hook, or monitoring daemon to Telegram, Discord, Slack, Signal, and other platforms using `clover send`."
---

# Pipe Script Output to Messaging Platforms

`clover send` is a small, scriptable CLI that pushes a message to any
messaging platform Clover is already configured for. Think of it as a
...
- Piping any tool's output anywhere (`make | clover send --to slack:#builds`)

The command reuses the same credentials and platform adapters that `clover
...
clover send --to slack:#eng --subject "[CI] build.log" --file build.log

# Thread target (Telegram topic, Discord thread)
clover send --to telegram:-1001234567890:17585 "threaded reply"

# List every configured target
clover send --list

# Filter by platform
clover send --list telegram
```
...
Any platform Clover ships adapters for works as a target:
...
`clover send` resolves the message body in this order:

1. **Positional argument** — `clover send --to telegram "hi"`
2. **`--file PATH`** — `clover send --to telegram --file msg.txt`
3. **Piped stdin** — `echo hi | clover send --to telegram`

When stdin is a TTY (no pipe), Clover does **not** wait for input — you'll
...
if [ "$ram_pct" -ge 85 ]; then
  clover send --to telegram --subject "⚠ MEMORY WARNING" \
    "RAM ${ram_pct}% on $(hostname)"
fi
```

Because `clover send` reuses your Clover config, the same script works on
any host where Clover is installed — no need to export bot tokens into
...
`clover send`. If the Python interpreter can't load because the box is
...
if ./scripts/deploy.sh; then
  clover send --to slack:#deploys "✅ ${CI_COMMIT_SHA:0:7} deployed"
else
  tail -n 100 deploy.log | clover send \
...
0 9 * * * /usr/local/bin/generate-metrics.sh \
  | /home/me/.clover/bin/clover send \
...
```bash
./train.py --epochs 200 && \
  clover send --to telegram "training done" || \
  clover send --to telegram "training failed (exit $?)"
```
...
clover send --to telegram --quiet "keepalive" || {
  echo "Telegram delivery failed" >&2
  exit 1
}

# Capture the message ID for later editing / threading
msg_id=$(clover send --to discord:#ops --json "build started" \
  | jq -r .message_id)
```

---

## Does `clover send` Need the Gateway Running?

**Usually no.** For any bot-token platform — Telegram, Discord, Slack,
Signal, SMS, WhatsApp Cloud API, and most others — `clover send` calls
the platform's REST endpoint directly using credentials from
`~/.clover/.env` and `~/.clover/config.yaml`. It's a standalone subprocess
...
pointing at the gateway; start it with `clover gateway start` and retry.
...
# Machine-readable
clover send --list --json
```

The listing is built from `~/.clover/channel_directory.json`, which the
gateway refreshes every few minutes while it's running. If you see
"no channels discovered yet", start the gateway once (`clover gateway
...
| Approach | Multi-platform | Reuses Clover creds | Needs gateway | Best for |
|----------|----------------|---------------------|---------------|----------|
| `clover send` | ✅ | ✅ | No (bot-token) | Everything below |
...
`clover send` is intentionally the simplest possible surface. If you need
...
If you just need to pipe a raw string, reach for `clover send`.
...
- [Gateway Internals](/developer-guide/gateway-internals) —
  the delivery router that `clover send` shares with cron delivery.
