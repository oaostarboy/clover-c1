...
description: "Diagnose and fix common Clover cron issues — jobs not firing, delivery failures, skill loading errors, and performance problems"
---
...
If you're expecting jobs to fire automatically, you need a running gateway (`clover gateway` for foreground, or `clover gateway start` for the installed service). For one-off debugging, you can manually trigger a tick with `clover cron tick`.
...
```bash
date
clover cron list   # Compare next_run times with local time
```
...
| Target | Requires |
|--------|----------|
| `telegram` | `TELEGRAM_BOT_TOKEN` in `~/.clover/.env` |
| `discord` | `DISCORD_BOT_TOKEN` in `~/.clover/.env` |
| `slack` | `SLACK_BOT_TOKEN` in `~/.clover/.env` |
...
| `local` | Write access to `~/.clover/cron/output/` |
...
If delivery fails, the job still runs — it just won't send anywhere. Check `clover cron list` for updated `last_error` field (if available).
...
Skills must be installed before they can be attached to cron jobs. If a skill is missing, install it first with `clover skills install <skill-name>` or via `/skills` in the CLI.

### Check 2: Check skill name vs. skill folder name

Skill names are case-sensitive and must match the installed skill's folder name. If your job specifies `ai-funding-report` but the skill folder is `ai-funding-daily-report`, confirm the exact name from `clover skills list`.
...
2. `~/.clover/logs/agent.log` for scheduler messages (or `errors.log` for warnings)
3. The job's `last_run` metadata via `clover cron list`

### Check 2: Common error patterns

**"No such file or directory" for scripts**
The `script` path must be an absolute path (or relative to the Clover config directory). Verify:
```bash
ls ~/.clover/scripts/your-script.py   # Must exist
clover cron edit <job_id> --script ~/.clover/scripts/your-script.py
```

**"Skill not found" at job execution**
The skill must be installed on the machine running the scheduler. If you move between machines, skills don't automatically sync — reinstall them with `clover skills install <skill-name>`.
...
Jobs are stored in `~/.clover/cron/jobs.json`. If this file is not readable/writable by your user, the scheduler will fail silently:

```bash
ls -la ~/.clover/cron/jobs.json
chmod 600 ~/.clover/cron/jobs.json   # Your user should own it
```
...
```bash
clover cron list                    # Show all jobs, states, next_run times
clover cron run <job_id>            # Schedule for next tick (for testing)
clover cron edit <job_id>           # Fix configuration issues
clover logs                         # View recent Clover logs
clover skills list                  # Verify installed skills
```
...
1. Run the job with `clover cron run <job_id>` (fires on next gateway tick) and watch for errors in the chat output
2. Check `~/.clover/logs/agent.log` for scheduler messages and `~/.clover/logs/errors.log` for warnings
3. Open an issue at [github.com/CloverCognition/clover-c1](https://github.com/CloverCognition/clover-c1) with:
