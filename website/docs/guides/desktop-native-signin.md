...
description: "How the Clover Desktop app signs in to a gated gateway using your system browser and PKCE — no embedded webview, no session cookies"
---

# Desktop Native Sign-In (RFC 8252)

When the Clover Desktop app connects to a **gated gateway** (a hosted or
...
For Clover specifically, native sign-in means:
...
```
Desktop app                Gateway (/auth/native/*)          Clover Portal (IDP)
...
desktop app* and an OAuth client *to the upstream identity provider* (Clover Portal). This is required because the upstream `client_id` and permitted
...
dashboard-auth subsystem. OAuth providers (e.g. the bundled **Clover** provider)
...
- [Run Clover with Clover Portal](./run-clover-with-nous-portal.md)
