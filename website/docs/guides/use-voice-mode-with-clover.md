---
sidebar_position: 8
title: "Use Voice Mode with Clover"
description: "A practical guide to setting up and using Clover voice mode across CLI, Telegram, Discord, and Discord voice channels"
---

# Use Voice Mode with Clover
...
:::tip
[Clover Portal](/integrations/nous-portal) bundles both the LLM and TTS through one OAuth — voice mode works end-to-end with no extra credentials.
:::
...
- you want Clover sitting in a Discord voice channel for live conversation
...
There are really three different voice experiences in Clover.
...
## Step 1: make sure normal Clover works first
...
Clover supports both local and cloud speech stacks.
...
### If you use `clover setup`

If you choose NeuTTS in the setup wizard, Clover checks whether `neutts` is already installed. If it is missing, the wizard tells you NeuTTS needs the Python package `neutts` and the system package `espeak-ng`, offers to install them for you, installs `espeak-ng` with your platform package manager, and then runs:
...
4. Clover transcribes and responds
...
- asking Clover to structure your thoughts in real time

#### Accessibility / low-typing sessions

If typing is inconvenient, voice mode is one of the fastest ways to stay in the full Clover loop.

## Tuning CLI behavior

### Silence threshold

If Clover starts/stops too aggressively, tune:
...
Clover stays a normal chat bot, but can speak replies.
...
- you want Clover to function like a portable research or ops assistant
...
Clover joins a Discord VC, listens to user speech, transcribes it, runs the normal agent pipeline, and speaks replies back into the channel.
...
- Clover detects speech boundaries
- transcripts are posted in the associated text channel
- Clover responds in text and audio
...
1. get text Clover working
2. run `clover setup tts` to enable voice support
