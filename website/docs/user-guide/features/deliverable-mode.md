...
When Clover Cognition runs inside a messaging gateway (Slack, Discord, Telegram,
...
persona in `~/.clover/SOUL.md`, or as a named preset under
`agent.personalities` in `~/.clover/config.yaml` (switchable per session
via `/personality`).
...
If you use Clover' kanban multi-agent workflow, workers can attach
...
Install MCP servers via `~/.clover/config.yaml` under the `mcp_servers`
...
back into the thread as a native attachment. Clover Cognition's deliverable
