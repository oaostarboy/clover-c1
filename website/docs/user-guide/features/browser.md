...
Clover Cognition includes a full browser automation toolset with multiple backend options:
...
If you have a paid [Clover Portal](https://portal.clover-c1.local) subscription, you can use browser automation through the **[Tool Gateway](tool-gateway.md)** without any separate API keys. New installs can run `clover setup --portal` to log in and turn on every gateway tool at once; existing installs can pick **Clover Subscription** as the browser provider via `clover model` or `clover tools`.
:::
...
The `.env` keys above supply **credentials only**. The active cloud browser is chosen by the `browser.cloud_provider` selection written by `clover tools` → Browser Automation (`browserbase`, `browser-use`, `camofox`, or `nous` for the Clover Subscription). Once a selection exists, adding or removing a key does not switch providers — and a selected provider with a missing key errors with guidance to run `clover tools` instead of silently rerouting. Never-configured setups still autodetect from available credentials.
:::
...
**This is the default browser mode**: when `browser.backend` is unset and the `browser-use` CLI is runnable (installed, or available through `uvx`), the agent gets the single `browser_exec` tool. If the CLI can't run, Clover falls back to the built-in browser tools automatically.

The mode is a **driver** that composes with your configured browser backend: it drives your local Chrome, a Clover-subscription cloud browser, Browserbase, Firecrawl, or Browser Use cloud browsers — whichever browser source is selected in `clover tools` → Browser Automation. The one exception is Camofox, which has no CDP endpoint for the harness to attach to; Camofox setups automatically keep the built-in browser tools.
...
When a cloud provider is configured, Clover auto-spawns a **local Chromium sidecar**
...
mode, so you need it installed (`clover setup tools → Browser Automation`
...
Or configure via `clover tools` → Browser Automation → Camofox.

Camofox is selected like any other browser backend: pick **Camofox** in `clover tools` → Browser Automation, which writes `browser.cloud_provider: camofox` to `config.yaml`. `CAMOFOX_URL` is only the server address — setting it no longer selects the backend by itself once a browser selection exists (never-configured setups still autodetect it).

#### Persistent browser sessions

By default, each Camofox session gets a random identity — cookies and logins don't survive across agent restarts. To enable persistent browser sessions, add the following to `~/.clover/config.yaml`:
...
Then fully restart Clover so the new config is picked up.

:::warning Nested path matters
Clover reads `browser.camofox.managed_persistence`, **not** a top-level `managed_persistence`. A common mistake is writing:

```yaml
# ❌ Wrong — Clover ignores this
managed_persistence: true
```

If the flag is placed at the wrong path, Clover silently falls back to a random ephemeral `userId` and your login state will be lost on every session.
:::
...
- Scopes the `userId` to the active Clover profile, so different Clover profiles get different browser profiles (profile isolation).

##### What Clover does not do
- It does not force persistence on the Camofox server. Clover only sends a stable `userId`; the server must honor it by mapping that `userId` to a persistent Firefox profile directory.
- If your Camofox server build treats every request as ephemeral (e.g. always calls `browser.newContext()` without loading a stored profile), Clover cannot make those sessions persist. Make sure you are running a Camofox build that implements userId-based profile persistence.

##### Verify it's working

1. Start Clover and your Camofox server.
...
If step 5 logs you out, the Camofox server isn't honoring the stable `userId`. Double-check your config path, confirm you fully restarted Clover after editing `config.yaml`, and verify your Camofox server version supports persistent per-user profiles.

##### Where state lives

Clover derives the stable `userId` from the profile-scoped directory `~/.clover/browser_auth/camofox/` (or the equivalent under `$CLOVER_HOME` for non-default profiles). The actual browser profile data lives on the Camofox server side, keyed by that `userId`. To fully reset a persistent profile, clear it on the Camofox server and remove the corresponding Clover profile's state directory.

#### Externally managed Camofox sessions

When another app drives the visible Camofox browser (a desktop assistant, a custom integration, another agent), configure Clover to operate inside that same identity instead of spawning its own isolated profile.

Three knobs control the behavior:

| Setting | Env var | Effect |
|---------|---------|--------|
| `browser.camofox.user_id` | `CAMOFOX_USER_ID` | Camofox `userId` Clover uses when creating tabs. Setting this opts the session into "externally managed" mode. |
...
| `browser.camofox.adopt_existing_tab` | `CAMOFOX_ADOPT_EXISTING_TAB` | When true, Clover calls `GET /tabs?userId=<user_id>` on first use and reuses an existing tab before creating a new one. |
...
- Clover skips destructive cleanup at task end (same as `managed_persistence: true`). The other app's tab/cookies/profile survive.
- Clover does **not** call `DELETE /sessions/<user_id>` — that endpoint wipes all user data, so it would nuke the external app's session if it fired.

**How tab adoption works (when `adopt_existing_tab: true`):**

1. On the first browser tool call after a process start, Clover issues `GET /tabs?userId=<user_id>` (5-second timeout).
2. If any tab in the response has `listItemId == session_key`, Clover adopts the most recently created one in that group.
3. Otherwise, Clover adopts the most recently created tab for the user (any `listItemId`).
4. If no tabs exist or the request fails, Clover falls back to creating a new tab on the next operation.

Adoption only fires until `tab_id` is populated for the session. If the external app closes the adopted tab mid-run, the next browser tool call will surface a Camofox error — Clover does not re-poll for a fresh tab on every call.

**Picking `session_key`:** if you want Clover to reliably attach to a *specific* existing tab, set `session_key` to the `listItemId` the external app used when creating it. If you leave `session_key` unset and only set `user_id`, Clover generates a per-task `session_key` (`task_<id>`) — Clover will share cookies and the profile with the external app, but will open its own tab alongside instead of reusing one.

**Concurrency note:** the external app and Clover can drive the same Camofox `userId` simultaneously, but Camofox does not coordinate per-tab focus between clients. Coordinate ownership at the application layer (e.g. the external app pauses while Clover runs).

#### VNC live view

When Camofox runs in headed mode (with a visible browser window), it exposes a VNC port in its health check response. Clover automatically discovers this and includes the VNC URL in navigation responses, so the agent can share a link for you to watch the browser live.
...
brave-browser \
  ...
  --user-data-dir=$HOME/.clover/chrome-debug \
  --no-first-run \
...
Then launch the Clover CLI and run `/browser connect`.
...
If Clover runs inside WSL2 but the Chrome window you want to control runs on the Windows host, `/browser connect` is often not the best path.

Why:

- `/browser connect` expects Clover itself to reach a usable CDP endpoint
...
- even when Windows Chrome is debuggable, the cleanest integration is often to let a Windows-side browser MCP server attach to Chrome and let Clover talk to that MCP server

For that setup, prefer `chrome-devtools-mcp` through Clover MCP support.
...
If you do **not** set any cloud credentials and don't use `/browser connect`, Clover can still use the browser tools through a local Chromium install driven by `agent-browser`.
...
# Extra Chromium launch flags (comma- or newline-separated). Clover auto-injects
...
# Clover doesn't add automatically; setting it disables the auto-injection.
...
:::info
The `browser` toolset must be included in your config's `toolsets` list or enabled via `clover config set toolsets '["clover-cli", "browser"]'`.
:::
...
```
Navigate to https://github.com/CloverCognition
```
...
```
Type "clover agent" into the search field @e3
```
...
Screenshots are stored in `~/.clover/cache/screenshots/` and automatically cleaned up after 24 hours.
...
When enabled, recording starts automatically on the first `browser_navigate` and saves to `~/.clover/browser_recordings/` when the session closes. Works in both local and cloud (Browserbase) modes. Recordings older than 72 hours are automatically cleaned up.
...
:::note
If paid features aren't available on your plan, Clover automatically falls back — first disabling `keepAlive`, then proxies — so browsing still works on free plans.
:::
...
- **Snapshot size** — large pages may be truncated or LLM-summarized at 15,000 characters (matching `web_extract`); the complete snapshot is saved to `~/.clover/cache/web/` and the output points at it for `read_file` paging
