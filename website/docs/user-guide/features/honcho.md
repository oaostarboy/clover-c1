...
[Honcho](https://github.com/plastic-labs/honcho) is an AI-native memory backend that adds dialectic reasoning and deep user modeling on top of Clover's built-in memory system. Instead of simple key-value storage, Honcho maintains a running model of who the user is — their preferences, communication style, goals, and patterns — by reasoning about conversations after they happen.
...
**Multi-agent profiles**: When multiple Clover instances talk to the same user (e.g., a coding assistant and a personal assistant), Honcho maintains separate "peer" profiles. Each peer sees only its own observations and conclusions, preventing cross-contamination of context.

## Setup

```bash
clover memory setup    # select "honcho" from the provider list
```
...
- `per-session` — each `clover` run gets a fresh session. Clean starts, memory via tools. Recommended for new users.
...
Server-side toggles set via the [Honcho dashboard](https://app.honcho.dev) win over local defaults — Clover syncs them back at session init.
...
The `clover honcho` subcommand is **only registered when Honcho is the active memory provider** (`memory.provider: honcho` in `config.yaml`). On a fresh install, configure Honcho directly with `clover memory setup honcho` (or run `clover memory setup` and pick it from the list); the `clover honcho` subcommand then appears on the next invocation.

```bash
clover memory setup honcho    # Configure Honcho directly (works before activation)
clover honcho status          # Connection status, config, and key settings
clover honcho setup           # Redirects to `clover memory setup` (post-activation alias)
clover honcho strategy        # Show or set session strategy (per-session/per-directory/per-repo/global)
clover honcho peer            # Show or update peer names + dialectic reasoning level
clover honcho mode            # Show or set recall mode (hybrid/context/tools)
clover honcho tokens          # Show or set token budget for context and dialectic
clover honcho identity        # Seed or show the AI peer's Honcho identity
clover honcho sync            # Sync Honcho config to all existing profiles
clover honcho peers           # Show peer identities across all profiles
clover honcho sessions        # List known Honcho session mappings
clover honcho map             # Map current directory to a Honcho session name
clover honcho enable          # Enable Honcho for the active profile
clover honcho disable         # Disable Honcho for the active profile
clover honcho migrate         # Step-by-step migration guide from openclaw-honcho
```

## Migrating from `clover honcho`

If you previously used the standalone `clover honcho setup`:
...
No re-login or re-setup needed. Run `clover memory setup` and select "honcho" — the wizard detects your existing config.
