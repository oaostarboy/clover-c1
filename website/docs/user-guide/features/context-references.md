...
Type `@` followed by a reference to inject content directly into your message. Clover expands the reference inline and appends the content under an `--- Attached Context ---` section.
...
- Clover env: `$CLOVER_HOME/.env`

These directories are fully blocked (any file inside):
- `~/.ssh/`, `~/.aws/`, `~/.gnupg/`, `~/.kube/`, `$CLOVER_HOME/skills/.hub/`
