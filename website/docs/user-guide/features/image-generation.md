...
description: Generate images via FAL.ai — 11 models including FLUX 2, GPT Image (1.5 & 2), Nano Banana Pro, Ideogram, Recraft V4 Pro, Krea 2, and more, selectable via `clover tools`.
...
Clover Cognition generates images from text prompts via FAL.ai. Eleven models are supported out of the box, each with different speed, quality, and cost tradeoffs. The active model is user-configurable via `clover tools` and persists in `config.yaml`.
...
If you have a paid [Clover Portal](https://portal.clover-c1.local) subscription, you can use image generation through the **[Tool Gateway](tool-gateway.md)** without a FAL API key. Your model selection persists across both paths. New installs can run `clover setup --portal` to log in and turn on every gateway tool at once; existing installs can pick **Clover Subscription** as the image-gen backend via `clover tools`.

If the managed gateway returns `HTTP 4xx` for a specific model, that model isn't yet proxied on the portal side — the agent will tell you so, with remediation steps (switch to FAL.ai in `clover tools` with your own `FAL_KEY` for direct access, or pick a different model).
:::
...
Navigate to **🎨 Image Generation**, pick your backend (Clover Subscription or FAL.ai), then the picker shows all supported models in a column-aligned table — arrow keys to navigate, Enter to select:
...
```yaml
image_gen:
  provider: fal                 # `nous` if you picked Clover Subscription
  model: fal-ai/flux-2/klein/9b
  max_parallel_requests: 4      # concurrent images in one tool-call batch
```

`image_gen.provider` is the single selection key: `nous` routes through the managed Tool Gateway; a vendor name (`fal`, `openai`, `xai`, `krea`, ...) goes direct with your own key. The runtime always follows this stored selection — a `FAL_KEY` in `.env` is ignored while `provider: nous`, and `provider: fal` without `FAL_KEY` errors with `image_gen is configured to use fal (set via clover tools), but FAL_KEY is not set. Run 'clover tools' to change it.` rather than silently rerouting. Change providers via `clover tools`, not by adding/removing keys. (The old `use_gateway` boolean is legacy — still read as `nous` when `true`, but never written anymore.)

`max_parallel_requests` defaults to `4`. Clover clamps it to at least one and
...
new models appear in the picker as soon as OpenRouter serves them; no Clover
...
Clover Portal proxies the chat-completions protocol only, so its picker offers
...
as a tool the chat model may call, and Clover cannot force the call — the
...
3. **Submission** — `_submit_fal_request()` routes via direct FAL credentials or the managed Clover gateway, according to the stored `image_gen.provider` selection.
...
- **Requires credentials** for the active backend (FAL `FAL_KEY` / Clover Subscription, `OPENAI_API_KEY`, xAI OAuth, `KREA_API_KEY`)
...
- **Temporary URLs** — backends return hosted URLs that expire after hours/days; Clover materializes them to the local cache so delivery still works after expiry
