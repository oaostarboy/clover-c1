...
Clover can show an animated **pet** — a small mascot sprite that reacts to what
...
- Pets are installed into your profile's `pets/` directory
  (`<CLOVER_HOME>/pets/<slug>/`), so each [profile](../profiles.md) keeps its
  own set.
...
In the terminal (CLI/TUI), Clover renders the sprite at full fidelity when your
...
clover pets list
clover pets list cat

# Install a pet and make it active in one step
clover pets install boba --select
...
## `clover pets` commands

| Goal | Command |
| --- | --- |
| Browse the gallery | `clover pets list [query] [--limit N]` |
| List installed pets | `clover pets list --installed` |
| Install a pet | `clover pets install <slug> [--select] [--force]` |
...
| Preview/animate | `clover pets show [slug] [--state <s>] [--cycle] [--once] [--mode <m>] [--scale <f>]` |
| Disable the pet | `clover pets off` |
| Remove an installed pet | `clover pets remove <slug>` |
| Diagnose setup | `clover pets doctor` |

`clover pets show` flags:
...
Beyond installing pre-made pets from the gallery, Clover can **generate a brand-new pet** from a text description — its own AI sprite-generation pipeline.

- CLI/TUI: `/hatch <description>` (alias `/generate-pet`), or `clover pets` → the generate flow.
...
2. **Hatch** — the chosen base is used as a reference image to generate one grounded animation row per Clover state (idle, thinking, tool use, etc.), which are deterministically sliced into frames and packed into a standard petdex/Codex atlas (8×9 grid of 192×208 cells). The result is a valid spritesheet you keep — and could `petdex submit`.

### Image backend

Generation uses the active [image-generation provider](/user-guide/features/image-generation), but it requires **reference-image grounding** so each animation row stays the same character as the base. Reference-capable backends: **Clover Portal**, **OpenRouter**, **OpenAI** (`gpt-image-2`), and **Krea**. OpenRouter/Clover run a quality-first model chain by default.

- Resolution order prefers Clover Portal → OpenAI → OpenRouter.
- If no reference-capable backend is configured, generation surfaces an actionable error pointing you to `clover tools` → Image Generation. (Installing/adopting existing gallery pets needs no image backend.)
- Override the backend with the `CLOVER_PET_IMAGE_PROVIDER` env var (e.g. `CLOVER_PET_IMAGE_PROVIDER=openrouter`).
...
always-on-top desktop window. Out there it stays visible while Clover is
...
Run `clover pets doctor` — it reports:
...
- The [`clover-c1` skill](../skills/bundled/autonomous-ai-agents/autonomous-ai-agents-clover-c1.md)
