...
Credential pools let you register multiple API keys or OAuth tokens for the same provider. When one key hits a rate limit or billing quota, Clover automatically rotates to the next healthy key — keeping your session alive without switching providers.
...
:::tip
Credential pools are mainly for API-key providers (OpenRouter, Anthropic). A single [Clover Portal](/integrations/nous-portal) OAuth covers 300+ models, so most users don't need a pool when on Portal.
:::
...
If you already have an API key set in `.env`, Clover auto-discovers it as a 1-key pool. To benefit from pooling, add more keys:

```bash
# Add a second OpenRouter key
clover auth add openrouter --api-key sk-or-v1-your-second-key

# Add a second Anthropic key
clover auth add anthropic --type api-key --api-key sk-ant-api03-your-second-key

# Add an Anthropic OAuth credential (requires Claude Max plan + extra usage credits)
clover auth add anthropic --type oauth
...
anthropic (3 credentials):
  #1  clover_pkce          oauth   clover_pkce ←
...
Run `clover auth` with no subcommand for an interactive wizard:
...
For providers that support both API keys and OAuth (Anthropic, Clover, Codex), the add flow asks which type:
...
| Command | Description |
|---------|-------------|
| `clover auth` | Interactive pool management wizard |
| `clover auth list` | Show all pools and credentials |
| `clover auth list <provider>` | Show a specific provider's pool |
| `clover auth add <provider>` | Add a credential (prompts for type and key) |
| `clover auth add <provider> --type api-key --api-key <key>` | Add an API key non-interactively |
| `clover auth add <provider> --type oauth` | Add an OAuth credential via browser login |
| `clover auth remove <provider> <index>` | Remove credential by 1-based index |
| `clover auth reset <provider>` | Clear all cooldowns/exhaustion status |

## Rotation Strategies

Configure via `clover auth` → "Set rotation strategy" or in `config.yaml`:
...
When you set up a custom endpoint via `clover model`, it auto-generates a name like "Together.ai" or "Local (localhost:8080)". This name becomes the pool key.

```bash
# After setting up a custom endpoint via clover model:
clover auth list
# Shows:
...
clover auth add Together.ai --api-key sk-together-second-key
```
...
Clover automatically discovers credentials from multiple sources and seeds the pool on startup:

| Source | Example | Auto-seeded? |
|--------|---------|-------------|
| Environment variables | `OPENROUTER_API_KEY`, `ANTHROPIC_API_KEY` | Yes |
| OAuth tokens (auth.json) | Codex device code, Clover device code | Yes |
| Claude Code credentials | `~/.claude/.credentials.json` | Yes (Anthropic) |
| Clover PKCE OAuth | `~/.clover/auth.json` | Yes (Anthropic) |
| Custom endpoint config | `model.api_key` in config.yaml | Yes (custom endpoints) |
| Manual entries | Added via `clover auth add` | Persisted in auth.json |

Auto-seeded entries are updated on each pool load — if you remove an env var, its pool entry is automatically pruned. Manual entries (added via `clover auth add`) are never auto-pruned.

Borrowed runtime secrets (for example env vars, Bitwarden/Vault/keyring/systemd references, and custom config values) are reference-only at the `auth.json` boundary. Clover can use the resolved value in memory for the current run, but it persists only metadata such as the source ref, label, status, request counters, and a non-reversible fingerprint. Manual entries and Clover-owned OAuth/device-code state keep the durable tokens they need to refresh.
...
2. **`clover_cli/auth_commands.py`** — CLI commands and interactive wizard
3. **`clover_cli/runtime_provider.py`** — Pool-aware credential resolution
4. **`run_agent.py`** — Error recovery: 429/402/401 → pool rotation → fallback

## Storage

Pool state is stored in `~/.clover/auth.json` under the `credential_pool` key:
...
The OpenRouter entry above was borrowed from an external source, so the raw key is not stored in `auth.json`. The manual Anthropic entry was intentionally added to Clover' credential store, so its token remains persistable.
