...
description: "Customize Clover Cognition's personality with a global SOUL.md, built-in personalities, and custom persona definitions"
---

# Personality & SOUL.md

Clover Cognition's personality is fully customizable. `SOUL.md` is the **primary identity** — it's the first thing in the system prompt and defines who the agent is.

- `SOUL.md` — a durable persona file that lives in `CLOVER_HOME` and serves as the agent's identity (slot #1 in the system prompt)
- built-in or custom `/personality` presets — session-level system-prompt overlays

If you want to change who Clover is — or replace it with an entirely different agent persona — edit `SOUL.md`.

## How SOUL.md works now

Clover now seeds a default `SOUL.md` automatically in:

```text
~/.clover/SOUL.md
```

More precisely, it uses the current instance's `CLOVER_HOME`, so if you run Clover with a custom home directory, it will use:
...
- Clover creates a starter `SOUL.md` automatically if one does not exist yet
- Existing user `SOUL.md` files are never overwritten
- Clover loads `SOUL.md` only from `CLOVER_HOME`
- Clover does not look in the current working directory for `SOUL.md`
- If `SOUL.md` exists but is empty, or cannot be loaded, Clover falls back to a built-in default identity
...
If Clover loaded `SOUL.md` from whatever directory you happened to launch it in, your personality could change unexpectedly between projects. By loading only from `CLOVER_HOME`, the personality belongs to the Clover instance itself.

That also makes it easier to teach users:
- "Edit `~/.clover/SOUL.md` to change Clover' default personality."
...
- how Clover should handle uncertainty, disagreement, or ambiguity
...
## What Clover injects into the prompt
...
If the file is empty, whitespace-only, or cannot be read, Clover falls back to a built-in default identity ("You are Clover Cognition, an intelligent AI assistant created by Clover Cognition..."). This fallback also applies when `skip_context_files` is set (e.g., in subagent/delegation contexts).
...
Clover ships with built-in personalities you can switch to with `/personality`.
...
| **pirate** | Captain Clover, tech-savvy buccaneer |
...
These are convenient overlays, but your global `SOUL.md` still gives Clover its persistent default personality unless the overlay meaningfully changes it.

## Custom personalities in config

Built-in personalities are always available on every surface (CLI, messaging platforms, TUI, and the desktop app). You can add your own — or override a built-in by reusing its name — in `~/.clover/config.yaml` under `agent.personalities`.
...
1. Keep a thoughtful global `SOUL.md` in `~/.clover/SOUL.md`
...
- [SOUL.md Guide](/guides/use-soul-with-clover)

## CLI appearance vs conversational personality

Conversational personality and CLI appearance are separate:

- `SOUL.md`, `agent.system_prompt`, and `/personality` affect how Clover speaks
- `display.skin` and `/skin` affect how Clover looks in the terminal
