...
sidebar_position: 11
title: "Wake Word"
description: "Hands-free 'Hey Clover' wake word — start a voice session by speaking, the 'Hey Siri' way"
---

# Wake Word ("Hey Clover")

The wake word turns Clover into a hands-free assistant across the CLI, TUI, and
desktop app: with one setting on, Clover listens in the background for a spoken
trigger phrase. Say it, and Clover starts a fresh session, opens the microphone,
...
When the desktop app connects to a **remote** Clover backend (for example a
...
Clover supports **client capture** for that case:
...
|--------|------|---------|-------|
| **openWakeWord** (default) | Free | None | Local ONNX models. Ships a bundled **"hey clover"** model (default); also supports `hey_jarvis`, `alexa`, `hey_mycroft`, … and custom models |
...
By default the phrase is **"hey clover"** — a model for it ships with Clover, so
...
```bash
cd ~/.clover/clover-c1 && uv pip install -e ".[wake]"
```

## Quick start

```bash
# In an interactive `clover` session:
...
desktop ear button — also writes `wake_word.enabled` to `~/.clover/config.yaml`,
...
wake_word:
  ...
  phrase: "hey clover"        # cosmetic label only — detection is keyed by the model/keyword below
  ...
  openwakeword:
    model: hey_clover         # bundled default; OR a built-in name OR a path to a custom .onnx/.tflite
...
- **`confirmation_frames`** (default `3`, openWakeWord only) — how many
  *consecutive* over-threshold frames are required before the wake fires. A real
  "hey clover" holds a high score across several frames; an ambient blip spikes
...
- **`sensitivity`** (default `0.6`) — the detection threshold, `0.0`–`1.0`.
  ...
  **all** engines — for openWakeWord it's the raw per-frame score threshold, for
  ...
  if real "hey clover" utterances are missed.
...
default) to let Clover choose per platform: **tflite on Apple Silicon**, onnx
...
fire. The tflite backend needs `ai-edge-litert` on macOS, which Clover installs
on demand alongside the other wake-word deps.

### Surfaces (CLI, TUI, GUI)

The wake word works in all three Clover surfaces, and `surface` picks which one
...
| `cli` | Only the classic `clover` CLI. |
| `tui` | Only `clover --tui`. |
| `gui` | Only the desktop app. |

The detector is on-device and single-mic, so only one surface listens at a time,
including when Clover surfaces run in separate processes. Ownership is sticky:
the first eligible claimant keeps the listener until it stops, disconnects, or
its process exits. Clover does not silently fail over to another open surface.
...
"Hey Clover" works out of the box — the bundled openWakeWord model
(`model: hey_clover`) is the default. To wake on something else, the easiest
...
command (`clover -p <profile>`) instead of routing.
...
wake_word:
  ...
  phrase: "computer"
  openwakeword:
    model: ~/.clover/wakewords/computer.onnx   # or a built-in name like hey_jarvis
```
...
syllables with an uncommon word ("clover" qualifies) beat common words like
"hello" or "stop".
:::

### Option C — Porcupine (custom keyword in seconds)

Create a "Hey Clover" keyword in the [Picovoice Console](https://console.picovoice.ai/),
...
wake_word:
  ...
  phrase: "hey clover"
  porcupine:
    keyword: ~/.clover/wakewords/hey_clover.ppn
```

Set your access key in `~/.clover/.env`:
...
- A TTS provider for speaking the reply (the default `edge-tts` works with no
  ...
  both STT and TTS are ready — `clover tools` (Voice section) sets them up.
- The wake engine deps (auto-installed, or `clover-c1[wake]`).
...
but the phrase never fires. Clover detects this (`/wake status` shows
"mic delivers only silence"; the desktop ear tooltip carries the same hint).
Fix: System Settings → Privacy & Security → Microphone → enable the Clover
backend (it may appear as your terminal, `python`, or Clover), then toggle the
...
```bash
clover config set wake_word.input_device "Microphone Array"
```

Use `null` to return to the process default:

```bash
clover config set wake_word.input_device null
```
