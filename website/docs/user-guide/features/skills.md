...
All skills live in **`~/.clover/skills/`** — the primary directory and source of truth. On fresh install, bundled skills are copied from the repo. Hub-installed and agent-created skills also go here. The agent can modify or delete any skill.

You can also point Clover at **external skill directories** — additional folders scanned alongside the local one. See [External Skill Directories](#external-skill-directories) below.
...
By default every profile is seeded with the bundled skill catalog, and each `clover update` adds any newly bundled skills. If you want a profile with **no bundled skills** — and that stays empty across updates — you have two paths:

**At install time** (applies to the default `~/.clover` profile):

```bash
curl -fsSL https://clover-c1.local/install.sh | bash -s -- --no-skills
```

**At profile-create time** (named profiles):

```bash
clover profile create research --no-skills
```

**On an already-installed profile** (default or named), toggle it at runtime:

```bash
clover skills opt-out            # stop future seeding — nothing on disk is touched
clover skills opt-out --remove   # also delete UNMODIFIED bundled skills (confirms first)
clover skills opt-in --sync      # undo: remove the marker and re-seed now
```

All three paths write a `.no-bundled-skills` marker into the profile directory. While the marker is present, the installer, `clover update`, and any skill sync all skip bundled-skill seeding for that profile. Delete the marker (or run `clover skills opt-in`) to re-enable.

:::note Safe by default
`clover skills opt-out` only stops *future* seeding — it never deletes anything already on disk. The optional `--remove` flag deletes bundled skills **only** when they are unmodified (byte-identical to the version Clover installed). Skills you have edited, skills installed from the hub, and skills you wrote yourself are always kept.
:::
...
The bundled `plan` skill is a good example. Running `/plan [request]` loads the skill's instructions, telling Clover to inspect context if needed, write a markdown implementation plan instead of executing the task, and save the result under `.clover/plans/` relative to the active workspace/backend working directory.

You can also interact with skills through natural conversation:

```bash
clover chat --toolsets skills -q "What skills do you have?"
clover chat --toolsets skills -q "Show me the axolotl skill"
```
...
description, the standard section order, Clover-tool framing, no invented
commands).
...
/home/user/.clover/cache/chart-q4-2025.png
...
When a missing value is encountered, Clover asks for it securely only when the skill is actually loaded in the local CLI. You can skip setup and keep using the skill. Messaging surfaces never ask for secrets in chat — they tell you to use `clover setup` or `~/.clover/.env` locally instead.
...
Settings are stored under `skills.config` in your config.yaml. `clover config migrate` prompts for unconfigured settings, and `clover config show` displays them. When a skill loads, its resolved config values are injected into the context so the agent knows the configured values automatically.
...
```text
~/.clover/skills/                  # Single source of truth
...
and `examples/`. Unreferenced repository files are not copied. Clover scans the
...
policy above), Clover can run [NVIDIA SkillEvaluator](https://github.com/NVIDIA/SkillEvaluator)
...
If you maintain skills outside of Clover — for example, a shared `~/.agents/skills/` directory used by multiple AI tools — you can tell Clover to scan those directories too.

Add `external_dirs` under the `skills` section in `~/.clover/config.yaml`:
...
- **Create locally, update in place**: New agent-created skills are written to `~/.clover/skills/`. Existing skills are modified where they are found, including skills under `external_dirs`, when the agent uses `skill_manage` actions such as `patch`, `edit`, `write_file`, `remove_file`, or `delete`.
- **External dirs are not a write-protection boundary**: If an external skill directory is writable by the Clover process, agent-managed skill updates can change files in that directory. Use filesystem permissions or a separate profile/toolset setup if shared external skills must stay read-only.
- **Local precedence**: If the same skill name exists in both the local dir and an external dir, the local version wins.
- **Full integration**: External skills appear in the system prompt index, `skills_list`, `skill_view`, and as `/skill-name` slash commands — no different from local skills.
- **Non-existent paths are silently skipped**: If a configured directory doesn't exist, Clover ignores it without errors. Useful for optional shared directories that may not be present on every machine.

### Example

```text
~/.clover/skills/               # Local (primary, read-write)
├── devops/deploy-k8s/
│   └── SKILL.md
└── mlops/axolotl/
    └── SKILL.md
...
Repos can carry their own skills, active only for sessions started inside that project — the same pattern other agent harnesses use for repo-local configuration. When you launch Clover inside a git checkout, it looks for skills in:

```text
<project-root>/.clover/skills/    # Clover-native location
...
Skills are procedure documents the agent follows, so Clover does **not** auto-load them from arbitrary cloned repos. The first time you run Clover in a repo with project skills, the banner shows a notice:

```text
◆ 3 project skill(s) found in /home/you/myproject but not loaded — run `clover skills trust` to enable them.
```

Trust the repo once (from inside it, or by passing the path):

```bash
clover skills trust             # trust the current repo
clover skills trust ~/myproject # or explicitly
clover skills untrust           # revoke
```

Trusted roots are stored in `skills.trusted_project_dirs` in `~/.clover/config.yaml`. Set `skills.project_discovery: false` to turn the feature off entirely (no scanning, no notices).

### Precedence

Project skills are the **highest-precedence tier**: `project → local (~/.clover/skills/) → external_dirs`. A project skill named `deploy` overrides a same-named profile or bundled skill for sessions inside that repo — that's the point: vendored repo skills win on their home turf, without touching your global profile. Project skills are tagged `[project]` in the agent's skill index so provenance stays visible.

Like external dirs, project skill directories are treated as repo-owned: autonomous skill maintenance (the curator) never modifies them, and new agent-created skills always go to `~/.clover/skills/`.

### Scan-time quarantine

Trust is a repo-level decision, but a repo's skill content changes with every `git pull`. To close that gap, every project skill is scanned with the same security scanner used for Skills Hub installs before it enters the index. A skill whose scan verdict is **dangerous** (prompt-injection directives, credential-exfiltration commands, hidden-text tricks) is quarantined: it does not appear in the skill index, `skills_list`, slash commands, and refuses to load by name with an explanatory error. Scans are content-hash cached under `~/.clover/cache/project_skill_scans/` (never inside your repo) and re-run automatically when the skill's content changes.
...
Bundles live in **`~/.clover/skill-bundles/<slug>.yaml`** and look like this:
...
- `description` (optional) — short text shown in `/bundles` and `clover bundles list`.
...
clover bundles create research

# Overwrite an existing bundle
clover bundles create backend-dev --skill ... --force

# Delete a bundle
clover bundles delete backend-dev

# Re-scan ~/.clover/skill-bundles/ and report changes
clover bundles reload
```
...
- You want to ship a team-wide "task profile" by checking the bundle YAML into a shared dotfiles repo and symlinking it into `~/.clover/skill-bundles/`.

A bundle is just a YAML alias — it doesn't install skills for you. The skills themselves must already be present (in `~/.clover/skills/` or an external skill directory). Otherwise the bundle invocation just skips the missing ones.
...
Staged writes survive restarts under `~/.clover/pending/skills/` and are
...
```bash
clover skills browse                              # Browse all hub skills (official first)
clover skills browse --source official            # Browse only official optional skills
clover skills search kubernetes                   # Search all sources
clover skills search react --source skills-sh     # Search the skills.sh directory
clover skills search https://mintlify.com/docs --source well-known
clover skills inspect openai/skills/k8s           # Preview before installing
clover skills install openai/skills/k8s           # Install with security scan
clover skills install official/security/1password
clover skills install skills-sh/vercel-labs/json-render/json-render-react --force
clover skills install well-known:https://mintlify.com/docs/.well-known/skills/mintlify
...
clover skills list --source hub                   # List hub-installed skills
clover skills check                               # Check installed hub skills for upstream updates
clover skills update                              # Reinstall hub skills with upstream changes when needed
clover skills audit                               # Re-scan all hub skills for security
clover skills uninstall k8s                       # Remove a hub skill
clover skills reset google-workspace              # Un-stick a bundled skill from "user-modified" (see below)
clover skills reset google-workspace --restore    # Also restore the bundled version, deleting your local edits
clover skills publish skills/my-skill --to github --repo owner/repo
clover skills snapshot export setup.json          # Export skill config
clover skills tap add myorg/skills-repo           # Add a custom GitHub source
```
...
|--------|---------|-------|
| `official` | `official/security/1password` | Optional skills shipped with Clover. |
| `skills-sh` | `skills-sh/vercel-labs/agent-skills/vercel-react-best-practices` | Searchable via `clover skills search <query> --source skills-sh`. Clover resolves alias-style skills when the skills.sh slug differs from the repo folder. |
...
Clover currently integrates with these skills ecosystems and discovery sources:

#### 1. Official optional skills (`official`)

These are maintained in the Clover repository itself and install with built-in trust.
...
This is Vercel's public skills directory. Clover can search it directly, inspect skill detail pages, resolve alias-style slugs, and install from the underlying source repo.
...
```bash
clover skills search https://mintlify.com/docs --source well-known
clover skills inspect well-known:https://mintlify.com/docs/.well-known/skills/mintlify
clover skills install well-known:https://mintlify.com/docs/.well-known/skills/mintlify
```

#### 4. Direct GitHub skills (`github`)

Clover can install directly from GitHub repositories and GitHub-based taps. This is useful when you already know the repo/path or want to add your own custom source repo.
...
```bash
clover skills install openai/skills/k8s
clover skills tap add myorg/skills-repo
```
...
[Skills Hub](https://clover-c1.local/docs) page — instead of a
tag-derived guess. This is generic: any tap that ships the file gets real
categorization, no Clover-side changes required.
...
- Clover source id: `clawhub`

#### 6. LobeHub (`lobehub`)

Clover can search and convert agent entries from LobeHub's public catalog into installable Clover skills.

- Site: [LobeHub](https://lobehub.com/)
- Public agents index: [chat-agents.lobehub.com](https://chat-agents.lobehub.com/)
- Backing repo: [lobehub/lobe-chat-agents](https://github.com/lobehub/lobe-chat-agents)
- Clover source id: `lobehub`

#### 7. browse.sh (`browse-sh`)

Clover integrates with [browse.sh](https://browse.sh), Browserbase's catalog of 200+ site-specific browser-automation SKILL.md files (Airbnb, Amazon, arXiv, 12306.cn, Etsy, Xero, and many more). Each skill describes how to drive one website end-to-end and is suitable for use with Clover' browser tools and any browser-automation skills you already have installed.

- Site: [browse.sh](https://browse.sh/)
- Catalog API: `https://browse.sh/api/skills`
- Clover source id: `browse-sh`
- Trust level: `community`

```bash
clover skills search airbnb --source browse-sh
clover skills inspect browse-sh/airbnb.com/search-listings-ddgioa
clover skills install browse-sh/airbnb.com/search-listings-ddgioa
```
...
Install `SKILL.md` directly from any HTTP(S) URL — useful when an author hosts a skill on their own site (no hub listing, no GitHub path to type). Clover also fetches explicitly referenced files under `references/`, `templates/`, `scripts/`, `assets/`, and `examples/`, then scans and installs the complete bundle.

- Clover source id: `url`
- Identifier: the URL itself (no prefix needed)
- Scope: `SKILL.md` plus exact referenced support files in the allowlisted directories. Clover does not enumerate or copy unrelated files from the host.

```bash
clover skills install https://sharethis.chat/SKILL.md
clover skills install https://example.com/my-skill/SKILL.md --category productivity
```
...
clover skills install https://example.com/SKILL.md --name sharethis-chat

# Or inside a chat session:
/skills install https://example.com/SKILL.md --name sharethis-chat
```

Trust level is always `community` — the same security scan runs as for every other source. The URL is stored as the install identifier, so `clover skills update` re-fetches from the same URL automatically when you want to refresh.
...
`clover skills inspect ...` now also surfaces upstream metadata when available:
- repo URL
...
| Level | Source | Policy |
|-------|--------|--------|
| `builtin` | Ships with Clover | Always trusted |
...
```bash
clover skills check          # Report which installed hub skills changed upstream
clover skills update         # Reinstall only the skills with updates available
clover skills update react   # Update one specific installed hub skill
clover skills update react --force   # Overwrite a skill you've edited locally
```

This uses the stored source identifier plus the current upstream bundle content hash to detect drift.

Skills you have edited locally (the on-disk content no longer matches the hash recorded at install time) are **skipped** by `clover skills update` so your changes are never silently overwritten. Pass `--force` to replace them with the upstream version anyway.
...
If you want to share a curated set of skills — for your team, your org, or publicly — you can publish them as a **tap**: a GitHub repository other Clover users add with `clover skills tap add <owner/repo>`. No server, no registry sign-up, no release pipeline. Just a directory of `SKILL.md` files.
...
- Each skill directory must contain a `SKILL.md` with standard [SKILL.md frontmatter](#skillmd-format) (`name`, `description`, plus optional `metadata.clover.tags`, `version`, `author`, `platforms`, `metadata.clover.config`).
...
Clover discovers skills by listing every subdirectory of the tap path and probing each for `SKILL.md`.

#### Minimal tap example

```
my-org/clover-skills
└── skills/
    └── deploy-runbook/
        └── SKILL.md
```
...
After pushing that to GitHub, any Clover user can subscribe and install:

```bash
clover skills tap add my-org/clover-skills
clover skills search deploy
clover skills install my-org/clover-skills/deploy-runbook
```

#### Non-default paths

If your skills don't live under `skills/` (common when you're adding a `skills/` subtree to an existing project), edit the tap entry in `~/.clover/skills/.hub/taps.json`:
...
The `clover skills tap add` CLI defaults new taps to `path: "skills/"`; edit the file directly if you need a different path. `clover skills tap list` shows the effective path per tap.
...
```bash
clover skills install owner/repo/skills/my-workflow
```
...
Taps are stored in `~/.clover/skills/.hub/taps.json` (created on demand).

## Bundled skill updates (`clover skills reset`)

Clover ships with a set of bundled skills in `skills/` inside the repo. On install and on every `clover update`, a sync pass copies those into `~/.clover/skills/` and records a manifest at `~/.clover/skills/.bundled_manifest` mapping each skill name to the content hash at the time it was synced (the **origin hash**).

On each sync, Clover recomputes the hash of your local copy and compares it to the origin hash:
...
clover skills reset google-workspace --restore
...
:::note Profiles
Each profile has its own `.bundled_manifest` under its own `CLOVER_HOME`, so `clover -p coder skills reset <name>` only affects that profile.
:::
