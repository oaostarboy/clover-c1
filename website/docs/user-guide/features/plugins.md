...
title: "Plugins"
description: "Extend Clover with custom tools, hooks, and integrations via the plugin system"
---

# Plugins

Clover has a plugin system for adding custom tools, hooks, and integrations without modifying core code.
...
[Adding Tools](/developer-guide/adding-tools) page is for built-in Clover
core tools that live in `tools/` and `toolsets.py`.

**→ [Build a Clover Plugin](/developer-guide/plugins)** — step-by-step guide with a complete working example.

## Quick overview

Drop a directory into `~/.clover/plugins/` with a `plugin.yaml` and Python code:

```
~/.clover/plugins/my-plugin/
...
Start Clover — your tools appear alongside built-in tools. The model can call them immediately.

### Minimal working example

Here is a complete plugin that adds a `hello_world` tool and logs every tool call via a hook.

**`~/.clover/plugins/hello-world/plugin.yaml`**
...
**`~/.clover/plugins/hello-world/__init__.py`**

```python
"""Minimal Clover plugin — registers a tool and a hook."""
...
Drop both files into `~/.clover/plugins/hello-world/`, restart Clover, and the model can immediately call `hello_world`. The hook prints a log line after every tool invocation.

The model-facing tool description belongs in `schema["description"]`. The optional `ctx.register_tool(description=...)` value is separate `ToolEntry` registry metadata: when omitted, it defaults to the schema description, but Clover does not copy it back into a schema that lacks `description`. Prefer defining the text once in the schema. If you provide both values, keep them synchronized; the model sees the schema value.

Project-local plugins under `./.clover/plugins/` are disabled by default. Enable them only for trusted repositories by setting `CLOVER_ENABLE_PROJECT_PLUGINS=true` before starting Clover.
...
| Add CLI commands | `ctx.register_cli_command(name, help, setup_fn, handler_fn)` — adds `clover <plugin> <subcommand>` |
...
| Gate on env vars | `requires_env: [API_KEY]` in plugin.yaml — prompted during `clover plugins install` |
| Distribute via pip | `[project.entry-points."clover-c1.plugins"]` |
...
|--------|------|----------|
| Bundled | `<repo>/plugins/` | Ships with Clover — see [Built-in Plugins](/user-guide/features/built-in-plugins) |
| User | `~/.clover/plugins/` | Personal plugins |
| Project | `.clover/plugins/` | Project-specific plugins (requires `CLOVER_ENABLE_PROJECT_PLUGINS=true`) |
| pip | `clover-c1.plugins` entry_points | Distributed packages |
| Nix | `services.clover-c1.extraPlugins` / `extraPythonPackages` | NixOS declarative installs — see [Nix Setup](/getting-started/nix-setup#plugins) |

Later sources override earlier ones on name collision, so a user plugin with the same name as a bundled plugin replaces it.

### Plugin sub-categories

Within each source, Clover also recognizes sub-category directories that route plugins to specialized discovery systems:
...
User plugins at `~/.clover/plugins/model-providers/<name>/` and `~/.clover/plugins/memory/<name>/` override bundled plugins of the same name — last-writer-wins in `register_provider()` / `register_memory_provider()`. Drop a directory in, and it replaces the built-in without any repo edits.

## Plugins are opt-in (with a few exceptions)

**General plugins and user-installed backends are disabled by default** — discovery finds them (so they show up in `clover plugins` and `/plugins`), but nothing with hooks or tools loads until you add the plugin's name to `plugins.enabled` in `~/.clover/config.yaml`. This stops third-party code from running without your explicit consent.
...
```bash
clover plugins                    # interactive toggle (space to check/uncheck)
clover plugins enable <name>      # add to allow-list
clover plugins disable <name>     # remove from allow-list + add to disabled
```

After `clover plugins install owner/repo`, you're asked `Enable 'name' now? [y/N]` — defaults to no. Skip the prompt for scripted installs with `--enable` or `--no-enable`.

For a reproducible install, pin a full immutable commit (tags, branches, and
abbreviated SHAs are not accepted):

```bash
clover plugins install owner/repo --ref 0123456789abcdef0123456789abcdef01234567
```

Clover checks out the commit detached, verifies that `HEAD` exactly matches the
requested SHA, and records the canonical source, installed revision, and pin
status in the current profile. `clover plugins update` refuses to move a pinned
plugin; choose a new exact commit explicitly with
`clover plugins install <source> --force --ref <new-commit>`. The
...
Several categories of plugin bypass `plugins.enabled` — they're part of Clover' built-in surface and would break basic functionality if gated off by default:
...
| **User-installed platforms** (under `~/.clover/plugins/platforms/`) | Opt-in via `plugins.enabled` — third-party gateway adapters need explicit consent. |

In short: **bundled "always-works" infrastructure loads automatically; third-party general plugins are opt-in.** The `plugins.enabled` allow-list is the gate specifically for arbitrary code a user drops into `~/.clover/plugins/`.

### Approval transports

An approval transport changes **where a human sees and answers** an existing
Clover tool-approval request. It does not decide whether a command needs
...
`present` may be synchronous or async. Clover runs it on a bounded worker and
...
`transport_fallback: builtin`. Without that exact opt-in, Clover never
materializes the prompt on another surface.

Clover still owns hardline blocks, sudo-stdin protection, user deny rules,
...
When you upgrade to a version of Clover that has opt-in plugins (config schema v21+), any user plugins already installed under `~/.clover/plugins/` that weren't already in `plugins.disabled` are **automatically grandfathered** into `plugins.enabled`. Your existing setup keeps working. Bundled standalone plugins are NOT grandfathered — even existing users have to opt in explicitly. (Bundled platform/backend plugins never needed grandfathering because they were never gated.)

## Available hooks

Plugins can register the 26 lifecycle events currently accepted by `clover_cli.plugins.VALID_HOOKS`. The **[Event Hooks catalog](/user-guide/features/hooks#shipped-plugin-hook-catalog)** is canonical for exact timing, return handling, payload fields, and privacy notes.
...
Clover has four kinds of plugins:

| Type | What it does | Selection | Location |
|------|-------------|-----------|----------|
| **General plugins** | Add tools, hooks, slash commands, CLI commands | Multi-select (enable/disable) | `~/.clover/plugins/` |
...
The table above shows the four plugin categories, but within "General plugins" the `PluginContext` exposes several distinct extension points — and Clover also accepts extensions outside the Python plugin system (config-driven backends, shell-hooked commands, external servers, etc.). Use this table to find the right doc for what you want to build:

| Want to add… | How | Authoring guide |
|---|---|---|
| A **tool** the LLM can call | Python plugin — `ctx.register_tool()` | [Build a Clover Plugin](/developer-guide/plugins) · [Adding Tools](/developer-guide/adding-tools) |
| A **lifecycle hook** (pre/post LLM, session start/end, tool filter) | Python plugin — `ctx.register_hook()` | [Hooks reference](/user-guide/features/hooks) · [Build a Clover Plugin](/developer-guide/plugins) |
| A **slash command** for the CLI / gateway | Python plugin — `ctx.register_command()` | [Build a Clover Plugin](/developer-guide/plugins) · [Extending the CLI](/developer-guide/extending-the-cli) |
| A **subcommand** for `clover <thing>` | Python plugin — `ctx.register_cli_command()` | [Extending the CLI](/developer-guide/extending-the-cli) |
...
| An **STT backend** (any CLI — whisper.cpp, custom whisper binary, local ASR CLI) | Config-driven (recommended) — declare under `stt.providers.<name>` with `type: command` in `config.yaml`, or set `CLOVER_LOCAL_STT_COMMAND` for the legacy single-command escape hatch. OR Python backend plugin — `ctx.register_transcription_provider()` for Python-SDK engines (OpenRouter, SenseAudio, Gemini-STT, etc.). | [STT Setup](/user-guide/features/tts#stt-custom-command-providers) · [Python plugin guide](/user-guide/features/tts#python-plugin-providers-stt) |
| **External tools via MCP** (filesystem, GitHub, Linear, Notion, any MCP server) | Config-driven — declare `mcp_servers.<name>` with `command:` / `url:` in `config.yaml`. Clover auto-discovers the server's tools and registers them alongside built-ins. | [MCP](/user-guide/features/mcp) |
| **Additional skill sources** (custom GitHub repos, private skill indexes) | CLI — `clover skills tap add <repo>` | [Skills Hub](/user-guide/features/skills#skills-hub) · [Publishing a custom tap](/user-guide/features/skills#publishing-a-custom-skill-tap) |
| **Gateway event hooks** (fire on `gateway:startup`, `session:start`, `agent:end`, `command:*`) | Drop `HOOK.yaml` + `handler.py` into `~/.clover/hooks/<name>/` | [Event Hooks](/user-guide/features/hooks#gateway-event-hooks) |
...
On NixOS, plugins can be installed declaratively via the module options — no `clover plugins install` needed. See the **[Nix Setup guide](/getting-started/nix-setup#plugins)** for full details.
...
```bash
clover plugins                               # unified interactive UI
clover plugins list                          # table: enabled / disabled / not enabled
clover plugins search <term>                 # search the community plugin index
clover plugins install <name>                # install by index name (resolved to repo @ pinned ref)
clover plugins install user/repo             # install from Git, then prompt Enable? [y/N]
clover plugins install user/repo --enable    # install AND enable (no prompt)
clover plugins install user/repo --no-enable # install but leave disabled (no prompt)
clover plugins update my-plugin              # pull latest (local edits are autostashed and re-applied)
clover plugins remove my-plugin              # uninstall
clover plugins enable my-plugin              # add to allow-list
clover plugins disable my-plugin             # remove from allow-list + add to disabled
clover plugins capabilities [my-plugin]      # declared vs granted capabilities
```

### One-click install links (Desktop)

Clover Desktop registers the `clover://` URL scheme, so a website, README, or
chat message can link straight to a plugin install:

```
clover://plugin/install?repo=owner/repo            # main install link
clover://plugin/install?repo=owner/repo&enable=1   # enable the agent plugin after install
clover://plugin/install?repo=owner/repo&force=1    # replace an existing install
```

Clicking one opens Clover and shows a **confirmation dialog** — the repo id,
...
Install from Git**. Legacy `clover://plugin-agent/…` and
`clover://plugin-desktop/…` URLs route into the same dialog. In dev builds
(`npm run dev`) the scheme is `clover-dev://`.

Websites need no SDK — a normal anchor works:

```html
<a href="clover://plugin/install?repo=owner/repo&enable=1">Install in Clover</a>
```

MCP servers have the equivalent link form — see
[Add to Clover link](/reference/mcp-config-reference#add-to-clover-link).
...
When a plugin declares capabilities, `clover plugins install` (and
`clover plugins enable`) shows the list with one-line risk descriptions and
...
granted, `clover plugins update` surfaces the additions and asks again. New
...
`clover plugins enable <id>` interactively to grant them later.

Inspect the state at any time:

```bash
clover plugins capabilities             # all plugins with declared/granted capabilities
clover plugins capabilities my-plugin   # one plugin, declared vs granted
```
...
not a code audit, and Clover has not reviewed the plugin's code. Only install
...
`clover plugins search <term>` searches the **community plugin index** — a
static, machine-readable JSON catalog of community plugins. Matching is fuzzy
across name, description, and tags:

```bash
clover plugins search telegram               # fuzzy search
clover plugins search                        # browse the whole index
clover plugins search --capability platform  # filter by declared capability
clover plugins search media --json           # machine-readable output
clover plugins search --refresh              # bypass the 24h local cache
```
...
```bash
clover plugins install clover-media-studio
```
...
(`https://raw.githubusercontent.com/CloverCognition/clover-plugin-index/main/index.json`,
overridable via `clover config set plugins.index_url <url>`). Fetches are
cached under `~/.clover/cache/plugin_index.json` for 24 hours; when the
remote is unreachable the stale cache is used, and when there is no cache at
all a bundled seed copy ships with Clover — so search works fully offline.

**Index entry format.** Each entry is a JSON object:

```json
{
  "name": "clover-media-studio",
  "description": "Generative media workspace plugin.",
  "author": "CloverCognition",
  "tags": ["media", "image-gen"],
  "repo": "CloverCognition/clover-media-studio",
  "ref": "<40-char commit SHA>",
  "subdir": null,
  "homepage": "https://github.com/CloverCognition/clover-media-studio",
...
(`clover_cli/data/plugin_index.json` in the repo) is the format reference.

**Submitting a plugin.** The index is maintained as a plain JSON file —
submit a pull request to the
[clover-plugin-index](https://github.com/CloverCognition/clover-plugin-index)
...
A **plugin pack** is a declarative, shareable YAML file (`clover-pack.yaml`)
...
version: 1.0.0
plugins:
  - name: clover-media-studio            # bare community-index name…
...
```bash
clover plugins pack show ./clover-pack.yaml     # dry-run review
clover plugins pack install ./clover-pack.yaml  # review → confirm → install
clover plugins pack export > clover-pack.yaml   # snapshot the current install
clover plugins pack export --enabled-only       # only plugins.enabled
```
...
same pinned install path as `clover plugins install --ref <sha>` and record
...
capability-consent prompt — identical to a single `clover plugins install`.
...
provenance (installed via `clover plugins install`). Local-only plugins are
...
auto-installed — install those manually for now (`clover skills`). Wiring
skill-hub ids into pack install is a documented follow-up seam.

### Install-time security scanning

Every `clover plugins install` and `clover plugins update` runs a static
...
On `clover plugins update`, a dangerous verdict on the updated tree
...
Running `clover plugins` with no arguments opens a composite interactive screen:
...
The default for a newly-installed or bundled plugin is `not enabled`. `clover plugins list` shows all three distinct states so you can tell what's been explicitly turned off vs. what's just waiting to be enabled.
...
- Clover reuses that session's stored platform, chat, thread, profile, and conversation history. Plugins cannot supply a new chat route through this API.
- Clover rechecks the stored route against the gateway's current authorisation rules before dispatch.
- Routes that relied only on an adapter-time or upstream authorisation decision are rejected unless Clover can revalidate them from current core allowlists, pairing, or explicit allow-all configuration.
...
- The route and conversation are pinned while dispatch is pending. Clover drops the request if topic recovery changes the route or the session rotates before handling starts.
...
:::warning
Only grant gateway injection to plugins you trust. Clover checks this host API permission and restricts it to existing session routes, but Python plugins run in-process and this setting is not a sandbox.
:::
...
`ctx.call_mcp()` lets a plugin call a tool on one of the user's configured MCP servers — synchronously, from any hook or tool handler — routing through Clover' existing native MCP client (same connections, trust-tier gates, circuit breaker, and reconnect logic as model-invoked MCP tools; never a parallel client).
