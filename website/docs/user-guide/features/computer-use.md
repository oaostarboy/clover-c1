...
Clover Cognition can drive your desktop — clicking, typing, scrolling,
...
The built-in `computer_use` toolset is the recommended Clover integration. It
...
**Fresh installs already have the driver.** The Clover installer
...
- **`clover tools`** → pick `🖱️  Computer Use` — installs the driver
...
```
clover computer-use install
```

This fetches and runs the upstream cua-driver installer — `install.sh`
on macOS/Linux, `install.ps1` on Windows. Use `clover computer-use
status` to verify the install.

Already have cua-driver? Clover reuses it when it supports the 0.20 runtime
contract. During setup, toolset enablement, `clover update`, and the first
`computer_use` call of a session, Clover checks the local version and
...
selected with `CLOVER_CUA_DRIVER_CMD` stays
under your control, so Clover reports the incompatibility and leaves it
unchanged.

If you install Cua Driver first, `cua-driver skills install` installs Cua's
skill pack under `~/.cua-driver/skills/cua-driver`. Clover autodetection is a
planned cua-driver follow-up, so currently point Clover at that directory or
...
interface. The built-in toolset provides Clover actions, configuration,
...
| Platform | Prereqs |
|---|---|
| **macOS** | System Settings → Privacy & Security → **Accessibility** + **Screen Recording**. Grant the identity named by `clover computer-use doctor`. Standard mode uses CuaDriver.app; bounded and unrestricted modes use the Clover host identity. |
...
```
clover -t computer_use chat
```

or add `computer_use` to your enabled toolsets in `~/.clover/config.yaml`.

## Permission modes and logged-in browser profiles

Clover maps its existing approval UX onto cua-driver's immutable runtime
...
| Clover session | cua-driver mode | Human intervention | `existing_profile` |
|---|---|---|---|
| Manual or smart approvals (default) | `standard` | Normal Clover approvals; Cua stops at its protected boundary | Refuses unless `computer_use.grant_existing_profile: true` (one-time config opt-in) |
...
| `--yolo`, `/yolo`, or `approvals.mode: off` | private `unrestricted` daemon | One explicit Clover risk acceptance; no runtime Cua prompts | Refuses unless `computer_use.grant_existing_profile: true`; YOLO does not substitute for this grant |
...
Clover then launches the cua-driver runtime with the trusted-launcher grant
...
computer_use:
  ...
  capability_manifest: ~/.clover/cua-manifest.yaml
```
...
for the format). Clover launches a private runtime with
...
resetting or closing the Clover session, cancellation cleanup, or process exit
closes that transport session. Clover also stops private runtimes that it
launched for bounded, unrestricted, or existing-profile access. One Clover
...
embedded service under the Clover host identity.
...
## `clover computer-use doctor` — your first triage stop

`clover computer-use doctor` runs cua-driver's structured
`health_report` MCP tool and prints a per-check matrix. It's the single
fastest way to find out *why* an action isn't working.

```
$ clover computer-use doctor
...
Clover run declares a public cua-driver **session name** (something like
`clover-3a7b9c14d2e8`). The name labels cursor identity and related state, so
...
Clover keeps its wrapper skill (`skills/autonomous-ai-agents/computer-use/SKILL.md`)
focused on the Clover-side `computer_use` workflow and action vocabulary. For
...
The command installs the pack under `~/.cua-driver/skills/cua-driver`. Clover
autodetection is a planned cua-driver follow-up, so currently point Clover at
...
These are **platform deep dives, not duplicates of the Clover skill** —
...
Code, Codex, OpenCode, OpenClaw, and Antigravity; **Clover
...
image capture also saves a bounded, shareable copy under Clover' image cache
...
Clover applies multi-layer guardrails:
...
Pair with `approvals.mode: manual` in `~/.clover/config.yaml` if you
want every action confirmed.

## Token efficiency

Screenshots are expensive. Clover applies four layers of optimisation:
...
- **Windows: elevated (admin) windows can't be driven from a normal
  ...
  Clover agent) cannot enumerate the UIA tree of, or inject mouse input
  ...
  run the Clover agent itself at High integrity (launch from an
...
- **Platform-specific deployment gotchas:**
  - **macOS** uses private SkyLight SPIs. Apple can change them in any
    OS update. Clover warns when the installed cua-driver is older than
    the version it was tested against.
  - **Windows** SSH sessions run in **Session 0**, which has no
    interactive desktop. Drive Clover from inside the RDP / console
...
```
CLOVER_CUA_DRIVER_CMD=/path/to/your/cua-driver
```

Swap the backend entirely (for testing):

```
CLOVER_COMPUTER_USE_BACKEND=noop   # records calls, no side effects
```

### Telemetry

cua-driver ships with anonymous usage telemetry (PostHog) enabled by default
upstream. **Clover disables it for you** — on every cua-driver invocation
(the MCP backend, `status`, `doctor`, and install) Clover sets
...
When it's on, `clover computer-use doctor` reports `telemetry: enabled`;
...
unreleased fix — point Clover at a binary you built from source instead
of the published release. Clover resolves the driver with
`shutil.which("cua-driver")` and **does not enforce
`CLOVER_CUA_DRIVER_VERSION`**, so a local build (reported as
...
- `-NoAutoStart` skips registering the `cua-driver-serve` logon daemon
  — you don't need it for Clover testing (see notes).
...
### Option B — point Clover straight at the built binary (fastest loop)

Skip the install ceremony entirely: `cargo build` and set
`CLOVER_CUA_DRIVER_CMD` to the resulting binary. Best for rapid
edit/build/test.
...
```
# Windows (.env)
CLOVER_CUA_DRIVER_CMD=C:\path\to\cua\libs\cua-driver\rust\target\debug\cua-driver.exe
# macOS / Linux (.env)
CLOVER_CUA_DRIVER_CMD=/path/to/cua/libs/cua-driver/rust/target/debug/cua-driver
```

### Confirm Clover is using your build

- `clover computer-use status` prints the resolved binary path and
  version.
- `clover computer-use doctor` confirms the binary is reachable and
...
- **Clover spawns a `cua-driver mcp` stdio proxy.** In a normal session the
  proxy connects to (and may start) the standard machine daemon. In explicit
  Clover YOLO, Clover instead owns a private `cua-driver serve --embedded`
...
- **Rebuild loop.** After editing cua-driver source, re-run
  ...
  for Option A, or just re-`cargo build` for Option B — no Clover
  change needed either way.
- **Local builds skip the version check.** Clover warns when the
...
**First action when anything's off: run `clover computer-use doctor`.**
...
Run `clover computer-use install` to fetch the cua-driver binary, or
run `clover tools` and enable the Computer Use toolset.
...
Wayland without an XWayland bridge. `clover computer-use doctor` will
...
- **Clover-side skill** — `skills/autonomous-ai-agents/computer-use/SKILL.md` — teaches the
  Clover `computer_use` action vocabulary; this is what the agent loads.
- **cua-driver skill pack** — for platform-specific deep dives
  ...
  `LINUX.md` / `RECORDING.md` / `WEB_APPS.md`. Clover autodetection is a
  planned follow-up; currently point Clover at the installed pack directory
...
- **cua.ai/docs** — the cua-driver project's documentation:
  ...
  - [Connect your agent](https://cua.ai/docs/how-to-guides/driver/connect-your-agent) — register cua-driver with various harnesses (Clover among them)
