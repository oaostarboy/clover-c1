...
description: "Use your Clover Portal subscription (or other OAuth provider) as an OpenAI-compatible endpoint for external apps"
---
...
chat completions — use your Clover-managed provider subscription as their
...
| Use case | "Use Clover as a chat backend" | "Use my Portal sub from another app" |
...
This opens your browser for the Clover Portal OAuth flow. Clover stores
the refresh token in `~/.clover/auth.json` — the same place all Clover
...
Model:      Clover-4-70B    # or Clover-4.3-36B, Clover-4-405B
```
...
Currently shipped: `nous` (Clover Portal) and `xai` (xAI / Grok). More
OAuth providers can be added by implementing the `UpstreamAdapter`
interface in `clover_cli/proxy/adapters/`.
...
If you see `not logged in`, run `clover portal`. If you see
...
The proxy only forwards paths the upstream actually serves. For Clover Portal:
...
supports; check `portal.clover-c1.local/models`.
...
[portal.clover-c1.local](https://portal.clover-c1.local).
...
`clover_cli/proxy/adapters/<provider>.py` and registering it in
