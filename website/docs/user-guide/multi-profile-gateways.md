...
If you only run one Clover agent, you don't need this page — see
...
[Connecting Desktop to Many Clover Instances](./multi-connection-desktop.md).

## When to use this

You want this setup when you have two or more Clover agents that should all
...
(`ai.clover.gateway-<name>.plist`) or systemd user service
(`clover-gateway-<name>.service`). This guide adds the patterns for managing
them collectively.
...
clover profile create coder
clover profile create personal-bot
clover profile create research
...
```bash
clover config set gateway.multiplex_profiles true
clover gateway restart
```

Equivalently, in the default profile's `~/.clover/config.yaml`:
...
You do **not** run `clover gateway start` for the secondary profiles — the
...
With a multiplexer running, a named-profile `clover gateway start` / `run` is a
...
- `/p/coder/...` API-server requests must use `API_SERVER_KEY` from
  `~/.clover/profiles/coder/.env`; the default listener key is rejected.
...
home). `clover status` reports the multiplexer and the profiles it serves;
`clover status -p <name>` slices to one profile. Each profile still writes its
...
`~/.local/bin/clover-gateways` and `chmod +x` it:
...
usage() {
  echo "Usage: clover-gateways {start|stop|restart|status|list}"
}
...
run_for_profile() {
  ...
  if [ "$profile" = "default" ]; then
    clover gateway "$action"
  else
    clover -p "$profile" gateway "$action"
  fi
}
...
```bash
clover-gateways start      # start every configured profile
clover-gateways stop       # stop every configured profile
clover-gateways restart    # restart all
clover-gateways status     # status across all
clover-gateways list       # delegates to `clover gateway list`
```

:::tip
The `default` profile is targeted with `clover gateway <action>` (no `-p`),
not `clover -p default gateway <action>`. The wrapper above handles both forms.
:::
...
These are equivalent to `clover -p coder gateway <action>` — useful if a
...
| Platform | Path                                                              |
| -------- | ----------------------------------------------------------------- |
| macOS    | `~/Library/LaunchAgents/ai.clover.gateway-<profile>.plist`        |
| Linux    | `~/.config/systemd/user/clover-gateway-<profile>.service`         |

The default profile keeps the historical names: `ai.clover.gateway.plist` /
`clover-gateway.service`.
...
```bash
# Default profile
tail -f ~/.clover/logs/gateway.log
tail -f ~/.clover/logs/gateway.error.log

# Named profile
tail -f ~/.clover/profiles/<name>/logs/gateway.log
tail -f ~/.clover/profiles/<name>/logs/gateway.error.log
```

Stream every profile's log simultaneously:

```bash
tail -f ~/.clover/logs/gateway.log ~/.clover/profiles/*/logs/gateway.log
```

The CLI also has a structured log viewer:

```bash
clover logs -f                  # follow default profile
clover -p coder logs -f         # follow one profile
clover logs --help              # filters, levels, JSON output
```

## Identify what's actually running

```bash
clover profile list             # profiles + model + gateway state
clover-gateways status          # full status across every profile
launchctl list | grep clover    # macOS — PIDs and labels
systemctl --user list-units 'clover-gateway-*'   # Linux — units
```
...
```
~/.clover/profiles/<name>/
├── .env              # API keys, bot tokens (chmod 600)
├── config.yaml       # model, provider, toolsets, gateway settings
└── SOUL.md           # personality / system prompt
```

The default profile uses `~/.clover/` directly with the same three files.

Edit them with any editor or via the CLI:

```bash
clover config set model.model anthropic/claude-sonnet-4    # default profile
...
caffeinate -i -w $(cat ~/.clover/gateway.pid) &   # awake while default gateway runs
...
systemd-inhibit --what=idle:sleep --who=clover --why="gateways running" \
  sleep infinity &
...
`clover-gateway-<profile>.service`) continue running across SSH disconnects
and reboots.
...
grep -H 'TELEGRAM_BOT_TOKEN\|DISCORD_BOT_TOKEN' \
     ~/.clover/.env ~/.clover/profiles/*/.env
```

## Updating the code

`clover update` pulls the latest code once and syncs new bundled skills into
every profile:
...
You ran `clover gateway start` after a previous `clover gateway stop`. The
...
```bash
ps -ef | grep "clover_cli.*-p <profile>"
cat ~/.clover/profiles/<profile>/gateway.pid
...
```bash
# macOS
launchctl unload ~/Library/LaunchAgents/ai.clover.gateway-<profile>.plist
launchctl load   ~/Library/LaunchAgents/ai.clover.gateway-<profile>.plist

# Linux
systemctl --user restart clover-gateway-<profile>.service
```

### Health check

```bash
clover doctor                  # default profile
clover -p <profile> doctor     # one profile
```
