...
if command -v gh &>/dev/null && gh auth status &>/dev/null; then
  ...
  if [ -z "$GITHUB_TOKEN" ]; then
    if _clover_env="${CLOVER_HOME:-$HOME/.clover}/.env"; [ -f "$_clover_env" ] && grep -q "^GITHUB_TOKEN=" "$_clover_env"; then
      GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" "$_clover_env" | head -1 | cut -d= -f2 | tr -d '\n\r')
    elif grep -q "github.com" ~/.git-credentials 2>/dev/null; then
      GITHUB_TOKEN=$(uv run python3 "${CLOVER_HOME:-$HOME/.clover}/skills/github/github-auth/scripts/git-credential-token.py")
    fi
  fi
fi
...
curl -s -X POST \
  ...
  -d "{
    \"commit_id\": \"$HEAD_SHA\",
    \"event\": \"COMMENT\",
    \"body\": \"Code review from Clover Cognition\",
    \"comments\": [
...
```bash
source "${CLOVER_HOME:-$HOME/.clover}/skills/github/github-auth/scripts/gh-env.sh"
...
gh pr review $PR_NUMBER --approve --body "Reviewed by Clover Cognition. Code looks clean — good test coverage, no security concerns."
