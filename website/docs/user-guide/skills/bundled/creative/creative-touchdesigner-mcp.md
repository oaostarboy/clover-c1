...
```
Clover Cognition -> MCP (Streamable HTTP) -> twozero.tox (port 40404) -> TD Python
```
...
```bash
bash "${CLOVER_HOME:-$HOME/.clover}/skills/creative/touchdesigner-mcp/scripts/setup.sh"
```

The script will:
1. Check if TD is running
2. Download twozero.tox if not already cached
3. Add `twozero_td` MCP server to Clover config (if missing)
...
3. **Restart Clover session** to pick up the new MCP server
