...
| Related skills | [`claude-code`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-claude-code), [`clover-c1`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-clover-c1) |
...
Delegate coding tasks to [Codex](https://github.com/openai/codex) via the Clover terminal. Codex is OpenAI's autonomous coding agent CLI.
...
For Clover itself, `model.provider: openai-codex` uses Clover-managed Codex
OAuth from `~/.clover/auth.json` after `clover auth add openai-codex`. For the
...
## Clover Gateway Caveat

When invoking the Codex CLI from a Clover gateway/service context (for example,
