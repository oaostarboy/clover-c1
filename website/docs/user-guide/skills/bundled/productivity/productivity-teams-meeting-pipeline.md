...
Everything operator-facing is a `clover teams-pipeline` subcommand run via the terminal tool. There are no new model tools for this pipeline — the CLI is the surface.
...
Before using the pipeline, verify these are set in `${CLOVER_HOME:-~/.clover}/.env`:
...
```bash
clover teams-pipeline validate              # config snapshot — run first after any change
clover teams-pipeline token-health          # Graph token status
clover teams-pipeline token-health --force-refresh   # force a fresh token acquisition
clover teams-pipeline list                  # recent meeting jobs
clover teams-pipeline list --status failed  # only failed jobs
clover teams-pipeline show <job-id>         # full detail of one job
clover teams-pipeline subscriptions         # current Graph webhook subscriptions
```

### Re-running / debugging

```bash
clover teams-pipeline run <job-id>          # replay a stored job (re-summarize, re-deliver)
clover teams-pipeline fetch --meeting-id <id>   # dry-run: resolve meeting + transcript without persisting
clover teams-pipeline fetch --join-web-url "<url>"   # dry-run by join URL
clover teams-pipeline fetch --join-web-url "<url>" --organizer-user-id <id>   # organizer-scoped lookup (required for /meet/ short URLs)
```

### Subscription management

```bash
clover teams-pipeline subscribe \
  --resource communications/onlineMeetings/getAllTranscripts \
  --notification-url https://<your-public-host>/msgraph/webhook \
  --client-state "$MSGRAPH_WEBHOOK_CLIENT_STATE"

clover teams-pipeline renew-subscription <sub-id> --expiration <iso-8601>
clover teams-pipeline delete-subscription <sub-id>
clover teams-pipeline maintain-subscriptions            # renew near-expiry ones
clover teams-pipeline maintain-subscriptions --dry-run  # show what would be renewed
```
...
1. Run `clover teams-pipeline subscriptions` — if it's empty or all entries show `expirationDateTime` in the past, that's the cause.
2. Recreate with `subscribe` as shown above.
3. **Set up automated renewal immediately** via `clover cron add`, a systemd timer, or plain crontab. The operator runbook at `/docs/guides/operate-teams-meeting-pipeline#automating-subscription-renewal-required-for-production` has all three options. 12-hour interval is safe (6x headroom against the 72h limit).
