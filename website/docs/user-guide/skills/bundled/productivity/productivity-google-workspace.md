...
Gmail, Calendar, Drive, Contacts, Sheets, and Docs — through Clover-managed OAuth and a thin CLI wrapper. When `gws` is installed, the skill uses it as the execution backend for broader Google Workspace coverage; otherwise it falls back to the bundled Python client implementation.
...
- `scripts/google_api.py` — compatibility wrapper CLI. It prefers `gws` for operations when available, while preserving Clover' existing JSON output contract.
...
```bash
GSETUP="python ${CLOVER_HOME:-$HOME/.clover}/skills/productivity/google-workspace/scripts/setup.py"
```
...
>
> Important Clover CLI note: if the file path starts with `/`, do NOT send only the bare path as its own message in the CLI, because it can be mistaken for a slash command. Send it in a sentence instead, like:
...
explicit (for example `~/Downloads/clover-google-client-secret.json`), then run
...
```bash
GAPI="python ${CLOVER_HOME:-$HOME/.clover}/skills/productivity/google-workspace/scripts/google_api.py"
```
