...
| List/filter by metadata (age, source, cost, tokens, workspace) | `clover sessions list` / `stats` via terminal |
| Rename | `clover sessions rename <session_id> <title...>` |
| Bulk soft-hide (reversible) | `clover sessions archive <filters>` |
| Delete (destructive) | `clover sessions delete` / `clover sessions prune <filters>` |
| Export before deleting anything valuable | `clover sessions export --session-id <id> --format md` |
...
`clover sessions list --source telegram --limit 50` instead.
...
- Before deleting anything with meaningful content, offer
  `clover sessions export --format md` as a backup.
...
After a cleanup pass, re-run the discovery query and `clover sessions list`
