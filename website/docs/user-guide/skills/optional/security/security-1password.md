...
| | |
|---|---|
| Source | Optional — install with `clover skills install official/security/1password` |
| Path | `optional-skills/security/1password` |
| Version | `1.0.0` |
| Author | arceus77-7, enhanced by Clover Cognition |
| License | MIT |
...
- `tmux` available for stable authenticated sessions during Clover terminal calls (desktop app flow only)
...
### Service Account (recommended for Clover)

Set `OP_SERVICE_ACCOUNT_TOKEN` in `${CLOVER_HOME:-~/.clover}/.env` (the skill will prompt for this on first load).
...
## Clover Execution Pattern (desktop app flow)

Clover terminal commands are non-interactive by default and can lose auth context between calls.
...
```bash
SOCKET_DIR="${TMPDIR:-/tmp}/clover-tmux-sockets"
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/clover-op.sock"
