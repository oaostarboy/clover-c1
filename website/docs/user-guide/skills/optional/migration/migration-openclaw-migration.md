---
title: "Openclaw Migration — Import an OpenClaw setup (memories, skills) into Clover"
sidebar_label: "Openclaw Migration"
description: "Import an OpenClaw setup (memories, skills) into Clover"
---
...
Import an OpenClaw setup (memories, skills) into Clover.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `clover skills install official/migration/openclaw-migration` |
| Path | `optional-skills/migration/openclaw-migration` |
| Version | `1.0.0` |
| Author | Clover Cognition (Clover Cognition) |
| License | MIT |
| Platforms | linux, macos, windows |
| Tags | `Migration`, `OpenClaw`, `Clover`, `Memory`, `Persona`, `Import` |
...
# OpenClaw -> Clover Migration

Use this skill when a user wants to move their OpenClaw setup into Clover Cognition with minimal manual cleanup.
...
```bash
clover claw migrate              # Full interactive migration
...
clover claw migrate --source /custom/path/.openclaw  # Custom source
```
...
**First-time setup:** The `clover setup` wizard automatically detects `~/.openclaw` and offers migration before configuration begins.

## What this skill does

It uses `scripts/openclaw_to_clover.py` to:

- import `SOUL.md` into the Clover home directory as `SOUL.md`
- transform OpenClaw `MEMORY.md` and `USER.md` into Clover memory entries
- merge OpenClaw command approval patterns into Clover `command_allowlist`
- migrate Clover-compatible messaging settings such as `TELEGRAM_ALLOWED_USERS`, and map OpenClaw workspace settings to Clover working-directory configuration
- copy OpenClaw skills into `~/.clover/skills/openclaw-imports/`
- optionally copy the OpenClaw workspace instructions file into a chosen Clover workspace
- mirror compatible workspace assets such as `workspace/tts/` into `~/.clover/tts/`
- archive non-secret docs that do not have a direct Clover destination
...
- `~/.clover/skills/migration/openclaw-migration/scripts/openclaw_to_clover.py`

Do not guess a shorter path like `~/.clover/skills/openclaw-migration/...`.

Before running the helper:

1. Prefer the installed path under `~/.clover/skills/migration/openclaw-migration/`.
...
With `--migrate-secrets`, it will also import a small allowlisted set of Clover-compatible secrets, currently:
...
Clover CLI supports the `clarify` tool for interactive prompts, but it is limited to:
...
- `{"question":"One or more imported OpenClaw skills already exist in Clover. How should I handle those skill conflicts?","choices":["keep existing skills","overwrite conflicting skills with backup","import conflicting skills under renamed folders"]}`
...
- `{"question":"Do you want to copy the OpenClaw workspace instructions file into a Clover workspace?","choices":["skip workspace instructions","copy to a workspace path","decide later"]}`
...
13. If a renamed imported skill has an empty `details.backup`, do not imply the existing Clover skill was renamed or backed up. Say only that the imported copy was placed in the new destination and reference `details.renamed_from` as the pre-existing folder that remained in place.
...
```bash
python3 ~/.clover/skills/migration/openclaw-migration/scripts/openclaw_to_clover.py
```

When using the terminal tool, prefer an absolute invocation pattern such as:

```json
{"command":"python3 /home/USER/.clover/skills/migration/openclaw-migration/scripts/openclaw_to_clover.py","workdir":"/home/USER"}
```

Dry run with the user-data preset:

```bash
python3 ~/.clover/skills/migration/openclaw-migration/scripts/openclaw_to_clover.py --preset user-data
```

Execute a user-data migration:

```bash
python3 ~/.clover/skills/migration/openclaw-migration/scripts/openclaw_to_clover.py --execute --preset user-data --skill-conflict skip
```

Execute a full compatible migration:

```bash
python3 ~/.clover/skills/migration/openclaw-migration/scripts/openclaw_to_clover.py --execute --preset full --migrate-secrets --skill-conflict skip
```

Execute with workspace instructions included:

```bash
python3 ~/.clover/skills/migration/openclaw-migration/scripts/openclaw_to_clover.py --execute --preset user-data --skill-conflict rename --workspace-target "/absolute/workspace/path"
```
...
2. Do not migrate secrets by default. Tokens, auth blobs, device credentials, and raw gateway config should stay out of Clover unless the user explicitly asks for secret migration.
3. Do not silently overwrite non-empty Clover targets unless the user explicitly wants that. The helper script will preserve backups when overwriting is enabled.
...
6. Even in secret-migration mode, only migrate secrets with a clean Clover destination. Unsupported auth blobs must still be reported as skipped.
...
- Clover persona state imported
- Clover memory files populated with converted OpenClaw knowledge
- OpenClaw skills available under `~/.clover/skills/openclaw-imports/`
