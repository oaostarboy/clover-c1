---
title: "Honcho — Configure and troubleshoot Honcho memory for Clover"
sidebar_label: "Honcho"
description: "Configure and troubleshoot Honcho memory for Clover"
---
...
Configure and troubleshoot Honcho memory for Clover.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `clover skills install official/autonomous-ai-agents/honcho` |
...
# Honcho Memory for Clover

Honcho provides AI-native cross-session user modeling. It learns who the user is across conversations and gives every Clover profile its own peer identity while sharing a unified view of the user.
...
See: https://docs.honcho.dev/v3/guides/integrations/clover#running-honcho-locally-with-clover

### Verify

```bash
clover honcho status    # shows resolved config, connection test, peer info
```
...
3. **AI peer card** -- the identity card for this Clover profile's AI peer
...
Honcho models conversations as interactions between **peers**. Clover creates two peers per session:

- **User peer** (`peerName`): represents the human. Honcho builds a user representation from observed messages.
- **AI peer** (`aiPeer`): represents this Clover instance. Each profile gets its own AI peer so agents develop independent views.
...
| `per-session` | New Honcho session each Clover run |
| `global` | Single session across all directories |

Manual override: `clover honcho map my-project-name`
...
Each Clover profile gets its own Honcho AI peer while sharing the same workspace (user context). This means:
...
# creates host block clover.coder, AI peer "coder", inherits config from default
```

What `--clone` does for Honcho:
1. Creates a `clover.coder` host block in `honcho.json`
...
```bash
clover honcho sync    # creates host blocks for all profiles that don't have one yet
```
...
Guidelines for Clover when Honcho memory is active.
...
Config file: `$CLOVER_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global).
...
Use `--clone` when creating: `clover profile create <name> --clone`. For existing profiles: `clover honcho sync`.
...
| Command | Description |
|---------|-------------|
| `clover honcho setup` | Interactive setup wizard (cloud/local, identity, observation, recall, sessions) |
| `clover honcho status` | Show resolved config, connection test, peer info for active profile |
| `clover honcho enable` | Enable Honcho for the active profile (creates host block if needed) |
| `clover honcho disable` | Disable Honcho for the active profile |
| `clover honcho peer` | Show or update peer names (`--user <name>`, `--ai <name>`, `--reasoning <level>`) |
| `clover honcho peers` | Show peer identities across all profiles |
| `clover honcho mode` | Show or set recall mode (`hybrid`, `context`, `tools`) |
| `clover honcho tokens` | Show or set token budgets (`--context <N>`, `--dialectic <N>`) |
| `clover honcho sessions` | List known directory-to-session-name mappings |
| `clover honcho map <name>` | Map current working directory to a Honcho session name |
| `clover honcho identity` | Seed AI peer identity or show both peer representations |
| `clover honcho sync` | Create host blocks for all Clover profiles that don't have one yet |
| `clover honcho migrate` | Step-by-step migration guide from OpenClaw native memory to Clover + Honcho |
| `clover memory setup` | Generic memory provider picker (selecting "honcho" runs the same wizard) |
| `clover memory status` | Show active memory provider and config |
| `clover memory off` | Disable external memory provider |
