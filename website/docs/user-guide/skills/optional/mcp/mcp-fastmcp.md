...
| | |
|---|---|
| Source | Optional — install with `clover skills install official/mcp/fastmcp` |
...
| Related skills | [`clover-c1`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-clover-c1), [`mcporter`](/docs/user-guide/skills/optional/mcp/mcp-mcporter) |
...
- smoke-test a server with the FastMCP CLI before wiring it into Clover or another client
...
Use `native-mcp` when the server already exists and only needs to be connected to Clover. Use `mcporter` when the goal is ad-hoc CLI access to an existing MCP server instead of building one.
