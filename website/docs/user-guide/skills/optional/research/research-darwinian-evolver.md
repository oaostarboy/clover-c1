...
| | |
|---|---|
| Source | Optional — install with `clover skills install official/research/darwinian-evolver` |
| Path | `optional-skills/research/darwinian-evolver` |
| Version | `0.1.0` |
| Author | Bihruze (Asahi0x), Clover Cognition |
| License | MIT |
...
import upstream classes into Clover itself.
...
```bash
mkdir -p ~/.clover/cache/darwinian-evolver && cd ~/.clover/cache/darwinian-evolver
...
```bash
cd ~/.clover/cache/darwinian-evolver/darwinian_evolver \
...
Open `~/.clover/cache/darwinian-evolver/darwinian_evolver/darwinian_evolver/lineage_visualizer.html`
...
SKILL_DIR=~/.clover/skills/research/darwinian-evolver
DE_DIR=~/.clover/cache/darwinian-evolver/darwinian_evolver
...
7. **AGPL.** Never `from darwinian_evolver import ...` inside Clover core.
   Custom driver scripts under `~/.clover/skills/...` are user-side and fine.
