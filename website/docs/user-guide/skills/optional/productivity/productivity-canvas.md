...
| | |
|---|---|
| Source | Optional — install with `clover skills install official/productivity/canvas` |
...
4. Name the token (e.g., "Clover Cognition"), set an optional expiry, and click **Generate Token**
5. Copy the token and add to `${CLOVER_HOME:-~/.clover}/.env`:
...
```bash
CANVAS="python $CLOVER_HOME/skills/productivity/canvas/scripts/canvas_api.py"
