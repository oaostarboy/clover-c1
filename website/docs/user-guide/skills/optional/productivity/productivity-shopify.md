...
| | |
|---|---|
| Source | Optional — install with `clover skills install official/productivity/shopify` |
...
4. Save to `${CLOVER_HOME:-~/.clover}/.env`:
   ```
...
}' '{"input":{"title":"Test Hoodie","status":"DRAFT","vendor":"Clover","productType":"Apparel","tags":["test"]}}'
```
