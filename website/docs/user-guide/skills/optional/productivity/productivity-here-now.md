...
| | |
|---|---|
| Source | Optional — install with `clover skills install official/productivity/here-now` |
...
- Skill helper paths:
  - `${CLOVER_SKILL_DIR}/scripts/publish.sh` for publishing sites
  - `${CLOVER_SKILL_DIR}/scripts/drive.sh` for private Drive storage

## Create a site

```bash
PUBLISH="${CLOVER_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --client clover
```
...
```bash
PUBLISH="${CLOVER_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --slug {slug} --client clover
```
...
```bash
DRIVE="${CLOVER_SKILL_DIR}/scripts/drive.sh"
bash "$DRIVE" default
...
| `--client {name}`      | Agent name for attribution (e.g. `clover`)    |
