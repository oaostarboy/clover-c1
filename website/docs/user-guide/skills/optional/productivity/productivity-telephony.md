...
| | |
|---|---|
| Source | Optional — install with `clover skills install official/productivity/telephony` |
...
This optional skill gives Clover practical phone capabilities while keeping telephony out of the core tool list.

It ships with a helper script, `scripts/telephony.py`, that can:
- save provider credentials into `${CLOVER_HOME:-~/.clover}/.env`
...
It does **not** turn Clover into a real-time inbound phone gateway. Inbound SMS is handled by polling the Twilio REST API. That is enough for many workflows, including notifications and some one-time-code retrieval, without adding core webhook infrastructure.
...
4. Treat third-party phone numbers as sensitive operational data:
   - do not save them to Clover memory
...
### 1) "I want Clover to own a real phone number"
Use **Twilio**.
...
- pairs well with Clover `text_to_speech` plus a public file host or tunnel

## Files and persistent state

The skill persists telephony state in two places:

### `${CLOVER_HOME:-~/.clover}/.env`
...
```bash
SCRIPT="$(find ~/.clover/skills -path '*/telephony/scripts/telephony.py' -print -quit)"
```
...
```bash
clover skills search telephony
clover skills install official/productivity/telephony
```
...
Then save credentials into Clover:
...
3. Buy it and save it into `${CLOVER_HOME:-~/.clover}/.env` + state:
```bash
python3 "$SCRIPT" twilio-buy "+17025551234" --save-env
```
...
```bash
python3 "$SCRIPT" twilio-call "+15551230000" --message "Hello! This is Clover calling with your status update." --voice Polly.Joanna
```

### E. Call with a prerecorded / custom voice message

This is the main path for reusing Clover's existing `text_to_speech` support.

Use this when:
- you want the call to use Clover's configured TTS voice rather than Twilio `<Say>`
...
Recommended Clover TTS -> Twilio Play workflow:

1. Generate the audio with Clover `text_to_speech`.
2. Make the resulting MP3 publicly reachable.
3. Place the Twilio call with `--audio-url`.

Example agent flow:
- Ask Clover to create the message audio with `text_to_speech`
...
Important note:
- Clover TTS is great for prerecorded outbound messages
...
- Clover STT/TTS alone is not being used here as a full duplex phone conversation engine; that would require a much heavier streaming/webhook integration than this skill is trying to introduce
...
7. Summarize the outcome without persisting third-party numbers to Clover memory.
...
- Do not store arbitrary third-party phone numbers in Clover memory.
...
3. persist that number to `${CLOVER_HOME:-~/.clover}/.env`
