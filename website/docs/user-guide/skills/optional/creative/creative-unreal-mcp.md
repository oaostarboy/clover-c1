...
| | |
|---|---|
| Source | Optional — install with `clover skills install official/creative/unreal-mcp` |
...
Companion skill for the `unreal-engine` entry in the Clover MCP catalog. The
...
Two halves, in this order: the editor side must be up before Clover connects.
...
### One-time, Clover side

    clover mcp install unreal-engine
...
`~/.clover/config.yaml` under `mcp_servers.unreal-engine` to match.

Do NOT use `ModelContextProtocol.GenerateClientConfig` for Clover — that
writes `.mcp.json`-style files for Claude Code/Cursor/etc. Clover connects
...
2. Start the Clover session. Tools register as `mcp_unreal_engine_*`. If
...
three meta-tools, and every real tool is reached through them. Through Clover
they appear as:
...
then happens at `clover mcp install`/`configure` time instead. Tool-search
...
3. **NEVER issue overlapping calls.** Do not batch multiple
   `mcp_unreal_engine_*` calls in one turn — Clover runs batched calls
...
- **Start order matters.** Editor + server up first, then the Clover
...
- **Timeouts on long operations.** Clover' per-call default is 120 s; asset
  imports, big level saves, and renders can exceed it. Raise
  `mcp_servers.unreal-engine.timeout` in `~/.clover/config.yaml` for
