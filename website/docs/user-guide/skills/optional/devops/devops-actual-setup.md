---
title: "Actual Setup — Set up Actual Computer (actual.inc) inference in Clover"
sidebar_label: "Actual Setup"
description: "Set up Actual Computer (actual.inc) inference in Clover"
---
...
Set up Actual Computer (actual.inc) inference in Clover.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `clover skills install official/devops/actual-setup` |
| Path | `optional-skills/devops/actual-setup` |
| Version | `2.0.0` |
| Author | shl0ms + Clover Cognition |
| License | MIT |
...
Sets up [actual.inc](https://actual.inc) (Actual Computer) as a Clover inference
...
- User has an `ac_` key and wants Clover routed through their Actual cluster.
...
- Clover has **first-class `actual` provider support** (provider id `actual`,
  aliases `actual-computer`, `actualcomputer`, `aci`). Do NOT configure Actual
  as a `custom_providers` / `providers.actual.*` entry on current Clover — the
...
3. Select provider + model:
   ...
   clover config set model.default "MODEL_ID_FROM_DISCOVERY"
   ```
4. Verify end-to-end:
   ```bash
   clover chat -Q -q "Reply with exactly: ACTUAL_OK" --provider actual -m MODEL_ID
   ```
...
4. Verify (reduced toolset — see context-window pitfall below):
   ```bash
   clover chat -Q -q "Reply with exactly: LOCAL_OK" --provider actual -m INSTALLED_NAME -t file,web
   ```
...
1. **reasoning_effort trap (handled by Clover since the first-class provider).**
   ...
   request still 400s this way on an old Clover, set a per-model cap:
   `agent.reasoning_overrides.<model>: high` in config.yaml.
2. **Context-window overflow on small local models.** Clover' default toolset
   ...
   servers emit a bare `data: [DONE]` — Clover reports
...
5. **Do not create a custom provider named `actual`.** Older setup guides
   ...
   current Clover the built-in provider wins the name; stale custom blocks
...
```bash
# Relay:
clover chat -Q -q "Reply with exactly: ACTUAL_OK" --provider actual -m MODEL
# Local (small model — reduced toolset):
clover chat -Q -q "Reply with exactly: LOCAL_OK" --provider actual -m MODEL -t file,web
