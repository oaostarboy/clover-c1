...
Clover Cognition automatically saves every conversation as a session. Sessions enable conversation resume, cross-session search, and full conversation history management.
...
1. **SQLite database** (`~/.clover/state.db`) — structured session metadata with FTS5 full-text search, plus full message history
...
Clover stores session history so it can resume conversations, but it does not
...
Clover explicitly injects for that turn.
...
For example, if a user sends an image and asks Clover to make a meme from it,
Clover may inspect that image once with vision and run an image-processing
...
`clover sessions prune` only when you want to delete old ended sessions from
storage. If `state.db` has simply grown large, start with the non-destructive
option first: `clover sessions optimize` merges FTS5 index segments and
...
| Source | Description |
|--------|-------------|
| `cli` | Interactive CLI (`clover` or `clover chat`) |
...
A bare `-c` is terminal-aware: each CLI session drops a small breadcrumb file under `~/.clover/terminal-sessions/` keyed by the terminal it runs in (tty device, tmux pane, kitty window, wezterm pane, Zellij pane, Windows Terminal session, ...). When you run `clover -c` again in the *same* terminal, Clover resumes that terminal's own session — so two panes side by side each continue their own conversation instead of both grabbing the globally most-recent one. If there's no breadcrumb for the terminal (first use, deleted session, or a stale breadcrumb older than 30 days), `-c` falls back to the most-recent-session behavior. `-c "name"` and `--resume` are unaffected. Disable with `session.terminal_continue: false` in `config.yaml`.
...
clover -c "my project"   # → resumes "my project #3"
```
...
clover --resume 20250305_091523_a1b2c3d4
clover -r 20250305_091523_a1b2c3d4

# Resume by title
clover --resume "refactoring auth"

# Resume the most recent session — same lookup as -c
clover --resume latest

# Or with the chat subcommand
clover chat --resume 20250305_091523_a1b2c3d4
```

Session IDs are shown when you exit a CLI session, and can be found with `clover sessions list`.
...
clover --resume latest --in ./my-project

# Works with the TUI too
clover --tui --resume latest --in ./my-project
```
...
```bash
clover --resume 20250305_091523_a1b2c3 --no-restore-cwd
```
...
`clover sessions list` accepts `--workspace <needle>` to show only sessions whose workspace key (git repo root, else cwd) matches — by path substring or exact directory basename:

```bash
clover sessions list --workspace my-project
clover sessions list --workspace ~/code/clover-c1
```

### Conversation Recap on Resume

When you resume a session, Clover displays a compact recap of the previous conversation in a styled panel before the input prompt:

<img className="docs-terminal-figure" src={useBaseUrl('/img/docs/session-recap.svg')} alt="Stylized preview of the Previous Conversation recap panel shown when resuming a Clover session." />
...
To disable the recap and keep the minimal one-liner behavior, set in `~/.clover/config.yaml`:
...
**Resume back to CLI:** when you want to come back to a desktop, just run `/resume <title>` (or `clover -r "<title>"` from the shell) and pick up where the platform left off.
...
Clover automatically generates a short descriptive title (3–7 words) for each session after the first exchange. This runs in a background thread using a fast auxiliary model, so it adds no latency. You'll see auto-generated titles when browsing sessions with `clover sessions list` or `clover sessions browse`.
...
```bash
clover sessions rename 20250305_091523_a1b2c3d4 "refactoring auth module"
```
...
When a session's context is compressed (manually via `/compress` or automatically), Clover creates a new continuation session. If the original had a title, the new session automatically gets a numbered title:

```
"my project" → "my project #2" → "my project #3"
```

When you resume by name (`clover -c "my project"`), it automatically picks the most recent session in the lineage.
...
Clover provides a full set of session management commands via `clover sessions`:

### List Sessions

```bash
# List recent sessions (default: last 20)
clover sessions list

# Filter by platform
clover sessions list --source telegram

# Show more sessions
clover sessions list --limit 50
```
...
`clover sessions export` is one surface for every export format, selected with `--format`:
...
clover sessions export backup.jsonl

# Export sessions from a specific platform
clover sessions export telegram-history.jsonl --source telegram

# Export a single session
clover sessions export session.jsonl --session-id 20250305_091523_a1b2c3d4

# Redact API keys/tokens/credentials from the exported content
clover sessions export backup.jsonl --redact
```
...
clover sessions export --format html --session-id 20250305_091523_a1b2c3d4 transcript.html

# All Telegram sessions from the last week in one file, secrets redacted
clover sessions export --format html --newer-than 1w --source telegram --redact archive.html
```
...
clover sessions export prompts.jsonl --session-id 20250305_091523_a1b2c3d4 --only user-prompts

# Markdown, straight to stdout
clover sessions export - --session-id 20250305_091523_a1b2c3d4 --only user-prompts --format md
```
...
`--format trace` emits Claude Code JSONL — the transcript shape the Hugging Face Hub auto-detects for its [Agent Trace Viewer](https://huggingface.co/docs/hub/agent-traces). Write it locally, or add `--upload` to push it to your own private `clover-traces` dataset (reads `HF_TOKEN`):

```bash
# Trace of the most recent session, to stdout
clover sessions export --format trace

# One session to a local trace file
clover sessions export --format trace --session-id 20250305_091523_a1b2c3d4 trace.jsonl

# Upload straight to your private HF traces dataset
clover sessions export --format trace --session-id 20250305_091523_a1b2c3d4 --upload
```
...
Pass `--format md` or `--format qmd` when you want a readable, file-based archive before hiding or deleting old sessions. Markdown/QMD exports write one file per session into a directory (default: `~/.clover/session-exports`).

```bash
# Export one session to Markdown
clover sessions export --format md --session-id 20250305_091523_a1b2c3d4

# Export a compression lineage as one logical document
clover sessions export --format md --session-id 20250305_091523_a1b2c3d4 --lineage logical

# Preview ended sessions older than 90 days without writing files
clover sessions export --format md --older-than 90 --dry-run

# Export ended Telegram sessions older than 2 weeks to QMD files
clover sessions export --format qmd --older-than 2w --source telegram

# Export long Claude sessions, secrets redacted
clover sessions export --format md --model sonnet --min-messages 50 --redact

# Only after verification, export and delete one explicitly named session
clover sessions export --format md --session-id 20250305_091523_a1b2c3d4 --delete-after-verified --yes
```
...
clover sessions delete 20250305_091523_a1b2c3d4

# Delete without confirmation
clover sessions delete 20250305_091523_a1b2c3d4 --yes
```
...
clover sessions rename 20250305_091523_a1b2c3d4 "debugging auth flow"
...
clover sessions pin 20250305_091523_a1b2c3d4
clover sessions pin 20250305 20250306

# Remove the pin
clover sessions unpin 20250305_091523_a1b2c3d4

# List pinned sessions
clover sessions pinned

# Machine-readable output, e.g. for a nightly backup of your pin set
clover sessions pinned --json > pinned-sessions.json
```
...
clover sessions prune

# Custom age threshold — bare numbers are days
clover sessions prune --older-than 30

# Durations work too: 5h, 30m, 2d, 1w
clover sessions prune --older-than 12h

# Delete only a specific time window (e.g. a batch of test sessions
# created in the last 5 hours)
clover sessions prune --newer-than 5h

# Explicit window with absolute timestamps
clover sessions prune --after "2026-07-05 09:00" --before "2026-07-05 14:30"

# Only prune sessions from a specific platform (all ages — any filter
# disables the implicit 90-day default)
clover sessions prune --source telegram
clover sessions prune --source cron --older-than 60   # add a time flag to narrow

# More filters — all AND together
clover sessions prune --newer-than 5h --title "smoke test"   # title substring
clover sessions prune --older-than 30 --max-messages 3        # tiny sessions
clover sessions prune --cwd ~/scratch --end-reason done       # by cwd / end reason
clover sessions prune --model gpt-5 --older-than 1w           # by model (substring)
clover sessions prune --provider openrouter --older-than 60   # by billing provider
clover sessions prune --branch feature/old-experiment         # by git branch
clover sessions prune --user 12345678 --chat-type group       # by messaging origin
clover sessions prune --max-tokens 500 --older-than 7         # by token usage
clover sessions prune --max-cost 0.01 --max-tool-calls 0      # cheap, tool-less runs

# Preview what would be deleted, without deleting anything
clover sessions prune --newer-than 5h --dry-run

# Skip confirmation
clover sessions prune --older-than 30 --yes
```
...
the implicit 90-day default, so `clover sessions prune --source cron` or
`--model gpt-4o` matches all ages — add a time flag to narrow it. Only a
completely bare `clover sessions prune` keeps the 90-day cutoff. Every
...
`clover sessions archive` takes the same filters as `prune` but soft-hides
...
clover sessions archive --newer-than 5h

# Archive by title substring, preview first
clover sessions archive --title "dry run" --dry-run
clover sessions archive --title "dry run" --yes
```

At least one filter is required — a bare `clover sessions archive` refuses to
archive your entire history. Archived sessions are hidden from
`clover sessions list` and `/resume` but remain in the database and can be
unarchived from the Desktop/Dashboard session list.

### Session Statistics

```bash
clover sessions stats
```
...
For deeper analytics — token usage, cost estimates, tool breakdown, and activity patterns — use [`clover insights`](/reference/cli-commands#clover-insights).
...
`clover sessions repair-routing` finds message-bearing session rows with no
...
clover sessions repair-routing
...
clover sessions repair-routing --apply

# Widen/narrow the contiguity window (default 900 seconds)
clover sessions repair-routing --max-gap-seconds 300
```
...
(`cp ~/.clover/state.db ~/.clover/state.db.bak`).


## Importing Sessions from Claude Code and Codex CLI

Started a conversation in another agent CLI? You can pull it into Clover and
continue it here. Clover reads Claude Code's session logs
...
clover sessions import

# Limit to one tool, or point at a specific file
clover sessions import --from claude
clover sessions import --from codex ~/.codex/sessions/2026/08/15/rollout-....jsonl

# Import-and-resume in one step
clover --resume @claude
clover --resume @codex
```

`clover sessions import` creates a new Clover session titled
`Imported from Claude Code: <first user message>` (or Codex CLI) and prints
the id plus a ready-to-paste `clover --resume <id>` command.
...
When Clover cannot get a participant identifier for a shared chat, it falls back to one shared session for that room.

### Shared vs Isolated Group Sessions

By default, Clover uses `group_sessions_per_user: true` in `config.yaml`. That means:

- Alice and Bob can both talk to Clover in the same Discord channel without sharing transcript history
...
|------|------|-------------|
| SQLite database | `~/.clover/state.db` | All session metadata + messages with FTS5 |
| Gateway messages    | `~/.clover/state.db`   | SQLite — canonical store for all session messages |
| Gateway routing index | `gateway_routing` table in `~/.clover/state.db` | Maps session keys to active session IDs (origin metadata, expiry flags) |
| Legacy routing mirror | `~/.clover/sessions/sessions.json` | Backward-compat mirror of the routing index, written when `gateway.write_sessions_json: true` (the default) |
...
`state.db`; `~/.clover/sessions/sessions.json` is a **legacy mirror** of it,
...
`clover sessions list`, `/sessions`, and the dashboard all read `state.db`,
which holds **every** session (CLI, TUI, and gateway). The `/save` snapshots
under `~/.clover/sessions/saved/*.json` are convenience exports, not the index.

If CLI sessions genuinely don't appear in `clover sessions list`, the cause is
`state.db` not receiving them — run `clover sessions repair` and watch for a
...
`*.jsonl` files in `~/.clover/sessions/`. They are no longer written or
read by Clover. Safe to delete after verifying the corresponding session
exists in state.db.
:::
...
- Pruning runs at most once per `sessions.min_interval_hours` (default 24); the last-run timestamp is tracked inside `state.db` itself so it's shared across every Clover process in the same `CLOVER_HOME`

Default is **off** — session history is valuable for `session_search` recall, and silently deleting it could surprise users. Enable in `~/.clover/config.yaml`:
...
clover sessions delete <session_id>
