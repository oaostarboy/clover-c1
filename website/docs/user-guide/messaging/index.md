...
description: "Chat with Clover from Telegram, Discord, Slack, WhatsApp, Signal, SMS, Email, Home Assistant, Mattermost, Matrix, DingTalk, Yuanbao, Microsoft Teams, LINE, Raft, Webhooks, or any OpenAI-compatible frontend via the API server — architecture and setup overview"
---

# Messaging Gateway

Chat with Clover from Telegram, Discord, Slack, WhatsApp, Signal, SMS, Email, Home Assistant, Mattermost, Matrix, DingTalk, Feishu/Lark, WeCom, Weixin, BlueBubbles (iMessage), QQ, Yuanbao, Microsoft Teams, LINE, ntfy, or your browser. The gateway is a single background process that connects to all your configured platforms, handles sessions, runs cron jobs, and delivers voice messages.

For the full voice feature set — including CLI microphone mode, spoken replies in messaging, and Discord voice-channel conversations — see [Voice Mode](/user-guide/features/voice-mode) and [Use Voice Mode with Clover](/guides/use-voice-mode-with-clover).

:::tip
Bots need both a model provider and tool providers (TTS, web). A [Clover Portal](/integrations/nous-portal) subscription bundles all of them.
:::
...
```mermaid
flowchart TB
    subgraph Gateway["Clover Gateway"]
...
For group chats, hooks, and automation flows, Clover supports explicit silence tokens. If the agent's final response is exactly one supported token, the gateway suppresses outbound delivery and sends nothing to the chat.
...
Silence is a delivery decision only. Clover keeps the assistant silence turn in the session transcript, so the conversation still alternates normally:
...
Failed turns still surface as errors; Clover does not hide failures just because the text resembles a silence token.
...
```bash
clover gateway setup        # Interactive setup for all messaging platforms
```
...
```bash
clover gateway              # Run in foreground
clover gateway setup        # Configure messaging platforms interactively
clover gateway install      # Install as a user service (Linux) / launchd service (macOS)
sudo clover gateway install --system   # Linux only: install a boot-time system service
clover gateway start        # Start the default service
clover gateway stop         # Stop the default service
clover gateway status       # Check default service status
clover gateway status --system         # Linux only: inspect the system service explicitly
```
...
```bash
clover gateway install --force
```

A positive value makes the generated unit use `Type=notify`,
`NotifyAccess=main`, and the matching `WatchdogSec`. Clover sends heartbeats
...
| `/update` | Update Clover Cognition to the latest version |
...
with the `session_reset` section in `~/.clover/config.yaml`:
...
Configure per-platform overrides in `~/.clover/gateway.json`:
...
Different channels can run different models and personas from a **single gateway** — e.g. a cheap fast model in `#daily` and a frontier model with a specialist prompt in `#dev`. Configure `channel_overrides` under the platform in `~/.clover/gateway-config.yaml`:
...
clover pairing approve telegram XKGH5N7P

# Other pairing commands:
clover pairing list          # View pending + approved users
clover pairing revoke telegram 123456789  # Remove access
```
...
The first time you message a busy agent on any platform, Clover appends a one-line reminder to the busy-ack explaining the knob (`"💡 First-time tip — …"`). The reminder fires once per install — a flag under `onboarding.seen.busy_input_prompt` latches it. Delete that key to see the tip again.
...
Control how much tool activity is displayed in `~/.clover/config.yaml`:
...
Setting `display.tool_progress: log` sends **no** progress bubbles to chat. Instead, each tool call is appended as a line to `~/.clover/logs/tool_calls.log` — a rotating audit file (5 MB × 3 backups) run through the same secret-redacting formatter as regular logs, so credentials never land on disk. Use it when you want a full tool-call trail without any chat noise.

### Configurable status phrases

Long-running gateway status lines ("still working…"-style heartbeats) draw from a phrase catalog. Built-in defaults ship in `gateway/assets/status_phrases.yaml`; you can add your own with profile-portable files under `CLOVER_HOME`:

- `~/.clover/status_phrases.yaml` or any `*.yaml` in `~/.clover/status_phrases/` (conventional paths, auto-loaded), or
- point config at a relative path:

```yaml
display:
  status_phrases:
    path: status_phrases/whatsapp.yaml  # relative to CLOVER_HOME
...
Off by default. When enabled, Clover prepends a human-readable timestamp
...
When the agent running a background session uses `terminal(background=true)` to start long-running processes (servers, builds, etc.), the gateway can push status updates to your chat. Control this with `display.background_process_notifications` in `~/.clover/config.yaml`:
...
```bash
CLOVER_BACKGROUND_NOTIFICATIONS=result
```
...
```bash
clover gateway install               # Install as user service
clover gateway start                 # Start the service
clover gateway stop                  # Stop the service
clover gateway status                # Check status
journalctl --user -u clover-gateway -f  # View logs

# Enable lingering (keeps running after logout)
sudo loginctl enable-linger $USER

# Or install a boot-time system service that still runs as your user
sudo clover gateway install --system
sudo clover gateway start --system
sudo clover gateway status --system
journalctl -u clover-gateway -f
```

Use the user service on laptops and dev boxes. Use the system service on VPS or headless hosts that should come back at boot without relying on systemd linger.

:::danger Don't add a custom `ExecStopPost` kill drop-in
The unit Clover installs already shuts the gateway down cleanly with `KillMode=mixed` + `KillSignal=SIGTERM`, and uses `Restart=always` with `RestartForceExitStatus` so updates and `/restart` respawn correctly. Do **not** add a systemd drop-in such as `ExecStopPost=/bin/kill -9 $MAINPID` — `ExecStopPost` fires on *every* stop, including clean restarts, so it `SIGKILL`s the freshly spawned instance before it stabilizes and `Restart=always` immediately respawns it. The result is an infinite restart loop (and, on Telegram, a flood of restart messages). If you've added such a drop-in, remove it: `systemctl --user edit clover-gateway` (or `sudo systemctl edit clover-gateway` for a system service) and delete the `ExecStopPost` line, then `systemctl --user daemon-reload`.
:::

:::tip Headless VMs: user service + linger avoids root prompts
A system service needs root for every restart — including the automatic gateway restart at the end of `clover update`. When `clover update` runs as a non-root user, it tries passwordless `sudo systemctl`; if that's unavailable, it skips the restart and prints the manual `sudo systemctl restart clover-gateway` command (it never blocks on an interactive password prompt).
...
```bash
clover gateway install          # user service
sudo loginctl enable-linger $USER   # one-time: start at boot, survive logout
```

After that, `clover update` can restart the gateway without any privileges. If you prefer to keep the system service, either run updates with `sudo clover update`, or grant the service account passwordless sudo for systemctl, e.g. in `sudo visudo -f /etc/sudoers.d/clover-gateway`:

```
clover ALL=(root) NOPASSWD: /usr/bin/systemctl --no-ask-password reset-failed clover-gateway*, /usr/bin/systemctl --no-ask-password start clover-gateway*, /usr/bin/systemctl --no-ask-password restart clover-gateway*
```
:::

Avoid keeping both the user and system gateway units installed at once unless you really mean to. Clover will warn if it detects both because start/stop/status behavior gets ambiguous.

:::info Multiple installations
If you run multiple Clover installations on the same machine (with different `CLOVER_HOME` directories), each gets its own systemd service name. The default `~/.clover` uses `clover-gateway`; other installations use `clover-gateway-<hash>`. The `clover gateway` commands automatically target the correct service for your current `CLOVER_HOME`.
:::

### macOS (launchd)

```bash
clover gateway install               # Install as launchd agent
clover gateway start                 # Start the service
clover gateway stop                  # Stop the service
clover gateway status                # Check status
tail -f ~/.clover/logs/gateway.log   # View logs
```

The generated plist lives at `~/Library/LaunchAgents/ai.clover.gateway.plist`. It includes three environment variables:
...
- **CLOVER_HOME** — scopes the gateway to your Clover installation.

:::tip PATH changes after install
launchd plists are static — if you install new tools (e.g. a new Node.js version via nvm, or ffmpeg via Homebrew) after setting up the gateway, run `clover gateway install` again to capture the updated PATH. The gateway will detect the stale plist and reload automatically.
:::

:::info Multiple installations
Like the Linux systemd service, each `CLOVER_HOME` directory gets its own launchd label. The default `~/.clover` uses `ai.clover.gateway`; other installations use `ai.clover.gateway-<suffix>`.
:::
...
|----------|---------|--------------|
| CLI | `clover-cli` | Full access |
| Telegram | `clover-telegram` | Full tools including terminal |
| Discord | `clover-discord` | Full tools including terminal |
| WhatsApp | `clover-whatsapp` | Full tools including terminal |
| WhatsApp Cloud API | `clover-whatsapp` | Full tools including terminal (shares toolset with the Baileys bridge) |
| Slack | `clover-slack` | Full tools including terminal |
| Google Chat | `clover-google_chat` | Full tools including terminal |
| Signal | `clover-signal` | Full tools including terminal |
| SMS | `clover-sms` | Full tools including terminal |
| Email | `clover-email` | Full tools including terminal |
| Home Assistant | `clover-homeassistant` | Full tools + HA device control (ha_list_entities, ha_get_state, ha_call_service, ha_list_services) |
| Mattermost | `clover-mattermost` | Full tools including terminal |
| Matrix | `clover-matrix` | Full tools including terminal |
| DingTalk | `clover-dingtalk` | Full tools including terminal |
| Feishu/Lark | `clover-feishu` | Full tools including terminal |
| WeCom | `clover-wecom` | Full tools including terminal |
| WeCom Callback | `clover-wecom-callback` | Full tools including terminal |
| Weixin | `clover-weixin` | Full tools including terminal |
| BlueBubbles | `clover-bluebubbles` | Full tools including terminal |
| QQBot | `clover-qqbot` | Full tools including terminal |
| Yuanbao | `clover-yuanbao` | Full tools including terminal |
| Microsoft Teams | `clover-teams` | Full tools including terminal |
| API Server | `clover-api-server` | Full tools (drops `clarify`, `text_to_speech` — programmatic access doesn't have an interactive user) |
| Webhooks | `clover-webhook` | Full tools including terminal |
| Raft | `clover-raft` | Wake-only channel; agent uses Raft CLI for message I/O |
...
1. **Gateway log** (`~/.clover/logs/gateway.log` or the systemd / launchd unit log). Search for the platform name and `circuit breaker`, `paused`, or `disabled`. The trip event includes the failure count and the last error.
