...
description: Control your smart home with Clover Cognition via Home Assistant integration.
...
Clover Cognition integrates with [Home Assistant](https://www.home-assistant.io/) in two ways:
...
4. Click **Create Token**, give it a name like "Clover Cognition"
5. Copy the token
...
Clover Cognition registers four tools for smart home control:
...
Configure which events the agent sees in `~/.clover/config.yaml` under the Home Assistant platform's `extra` section:
...
Outbound messages from the agent are delivered as **Home Assistant persistent notifications** (via `persistent_notification.create`). These appear in the HA notification panel with the title "Clover Cognition".
...
The adapter reads credentials from `~/.clover/.env` (auto-merged at startup) or
from `config.yaml`. Double-check the file lives under the active Clover profile
...
running Clover — `curl -H "Authorization: Bearer <token>" <url>/api/` should
