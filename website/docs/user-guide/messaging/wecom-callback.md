...
Connect Clover to WeCom (Enterprise WeChat) as a self-built enterprise application using the callback/webhook model.

:::info WeCom Bot vs WeCom Callback
Clover supports two WeCom integration modes:
...
> Run `clover gateway setup` and pick **WeCom Callback** for a guided walk-through.
...
3. Clover decrypts the message, queues it for the agent
...
(Use `clover gateway start` only after `clover gateway install` has registered the systemd/launchd service.)
...
console. A mismatch between the token configured in Clover and the token the
...
in `~/.clover/.env` values around `=` will also break signature checks. After
fixing, restart `clover gateway run`.
...
Check `clover gateway run` logs for the bound host/port. If the adapter bound to
