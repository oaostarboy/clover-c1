...
sidebar_position: 30
title: "Clover Relay"
description: "Connect Clover to messaging platforms through a relay connector that owns the platform credentials — enrollment, capabilities, config, and troubleshooting"
---

# Clover Relay (Connector)
...
Clover Relay is not a chat platform itself — it is a **connector system** that
...
1. Resolves a fresh Clover Portal access token from your existing login
   (`~/.clover/auth.json`) — this proves which Clover org (tenant) you own. If
   `gateway.idp.token_url` is configured, your own IdP is used instead (the
   air-gapped / self-hosted-IdP path, no Clover Portal involved): with
...
3. The connector verifies the token (signature, single-use, tenant match),
   ...
   once.
4. Persists the credentials into `~/.clover/.env`:
...
`clover gateway enroll` refuses to run in managed/hosted installs — there the
...
|---------|-------|---------|
| `GATEWAY_RELAY_URL` | env (`~/.clover/.env`) | Connector relay WebSocket URL. Presence enables the relay platform. |
...
| `gateway.idp.token_url` | `config.yaml` | When set, enrollment/provisioning authenticates against your own IdP instead of Clover Portal: OAuth2 client-credentials when `gateway.idp.client_id`/`client_secret` are also set; otherwise an ambient token endpoint (plain GET returning the token, raw or `{"access_token": …}`). |
...
token. Re-login with `clover auth add nous` (or `clover setup`) and retry.
...
startup. Restart it (`clover gateway restart`).
