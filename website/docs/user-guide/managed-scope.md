...
`~/.clover/config.yaml`, `~/.clover/.env`, and even the shell environment — for
...
Managed scope is read from a system-level directory, default `/etc/clover`:

```text
/etc/clover/
├── config.yaml     # managed config layer (wins over ~/.clover/config.yaml)
└── .env            # managed env layer (wins over ~/.clover/.env + shell)
```
...
The location can be relocated with the `CLOVER_MANAGED_DIR` environment variable
(for containers or non-`/etc` deployments). This is a deployment/bootstrap path
knob — like `CLOVER_HOME` — set by the same administrator who owns the managed
files. It is **never persisted** to any `.env` by Clover.

```bash
# Point managed scope at a custom directory (set by IT / the deployment, not the user)
export CLOVER_MANAGED_DIR=/opt/org/clover-policy
```

:::warning
A user who can set `CLOVER_MANAGED_DIR` can repoint managed scope at a directory
...
left user-settable. `clover doctor` reports the *resolved* managed directory so
...
|---|---|---|
| 1 | `/etc/clover/config.yaml` (managed) | `/etc/clover/.env` (managed) |
| 2 | `~/.clover/config.yaml` (user) | `~/.clover/.env` (user) |
...
```bash
clover config        # shows a header naming the managed source + the pinned keys
clover doctor        # reports the resolved managed dir + pinned key counts
```

If you try to change a managed value, Clover refuses and names the source:

```bash
$ clover config set model.default my/model
Cannot set 'model.default': it is managed by your administrator
(/etc/clover/config.yaml) and cannot be changed.
```

The same applies to managed secrets — `clover config set` / setup will not write
a user value for an env key pinned by the managed `.env`.

## Setting up a managed scope (administrators)

```bash
sudo mkdir -p /etc/clover

# Pin some config values for every user on this machine
sudo tee /etc/clover/config.yaml >/dev/null <<'YAML'
model:
  provider: nous
security:
  redact_secrets: true
...
sudo tee /etc/clover/.env >/dev/null <<'ENV'
OPENAI_API_BASE=https://inference.example.com/v1
ENV

sudo chmod 0755 /etc/clover
sudo chmod 0644 /etc/clover/config.yaml /etc/clover/.env
```

Changes take effect on the next Clover start (a malformed managed file is logged
loudly and ignored — it never blocks startup, but the admin should check
`clover doctor` to confirm the policy is being applied).

## Security model and limitations (v1)

- **Enforcement is filesystem permissions only.** If a user has write access to
  the managed directory (or runs Clover as `root`), managed scope is advisory.
