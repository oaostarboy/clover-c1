...
title: "Git Worktrees"
description: "Run multiple Clover agents safely on the same repository using git worktrees and isolated checkouts"
---

# Git Worktrees

Clover Cognition is often used on large, long‑lived repositories. When you want to:
...
This page shows how to combine worktrees with Clover so each session has a clean, isolated working directory.

## Why Use Worktrees with Clover?

Clover treats the **current working directory** as the project root:

- CLI: the directory where you run `clover` or `clover chat`
- Messaging gateways: the directory set by `terminal.cwd` in `~/.clover/config.yaml`
...
Clover creates `.worktrees/my-experiment/` inside the repo (branch
`clover/my-experiment`, based on the freshly-fetched remote tip unless
`worktree_sync: false`), and retargets the session's terminal and file tools
into it — no restart needed. Omit the name to get a random `clover-<id>`
...
git worktree add ../repo-feature feature/clover-experiment
```

This creates:

- A new directory: `../repo-feature`
- A new branch: `feature/clover-experiment` checked out in that directory

Now you can `cd` into the new worktree and run Clover there:

```bash
cd ../repo-feature

# Start Clover in the worktree
clover
```
...
git worktree add ../repo-experiment-a feature/clover-a
git worktree add ../repo-experiment-b feature/clover-b
```
...
- Works on its own branch (`feature/clover-a` vs `feature/clover-b`).
...
- Clover checkpoint data under `~/.clover/checkpoints/` is not automatically pruned when you remove a worktree, but it is usually very small.

## Best Practices

- **One worktree per Clover experiment**
...
- **Name branches after the experiment**
  - e.g. `feature/clover-checkpoints-docs`, `feature/clover-refactor-tests`.
- **Commit frequently**
...
- **Avoid running Clover from the bare repo root when using worktrees**
  - Prefer the worktree directories instead, so each agent has a clear scope.

## Using `clover -w` (Automatic Worktree Mode)

Clover has a built‑in `-w` flag that **automatically creates a disposable git worktree** with its own branch. You don't need to set up worktrees manually — just `cd` into your repo and run:
...
- Check out an isolated branch (e.g. `clover/clover-<hash>`).
...
```bash
clover -w -z "Fix issue #123"
```

For parallel agents, open multiple terminals and run `clover -w` in each — every invocation gets its own worktree and branch automatically.

## Putting It All Together

- Use **git worktrees** to give each Clover session its own clean checkout.
