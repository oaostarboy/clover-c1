...
sidebar_position: 1
title: "CLI Interface"
description: "Master the Clover Cognition terminal interface — commands, keybindings, personalities, and more"
---

# CLI Interface

Clover Cognition's CLI is a full terminal user interface (TUI) — not a web UI. It features multiline editing, slash-command autocomplete, conversation history, interrupt-and-redirect, and streaming tool output. Built for people who live in the terminal.

:::tip First-time setup
One command — `clover setup --portal` — and you're ready to `clover chat`. See [Clover Portal](/integrations/nous-portal).
:::

:::tip
Clover also ships a modern TUI with modal overlays, mouse selection, and non-blocking input. Launch it with `clover --tui` — see the [TUI](tui.md) guide.
:::
...
clover chat --query-file prompt.txt
clover chat --query-file - < prompt.txt

# With a specific model
clover chat --model "anthropic/claude-sonnet-4"

# With a specific provider
clover chat --provider nous        # Use Clover Portal
clover chat --provider openrouter  # Force OpenRouter
...
clover chat -s github-pr-workflow -q "open a draft PR"

# Resume previous sessions
clover --continue             # Resume the most recent CLI session (-c)
clover --resume <session_id>  # Resume a specific session by ID (-r)
clover --resume latest        # Resume the most recent session (same as -c)
clover --resume latest --in ./dir  # Resume ./dir's latest session, staying in ./dir

# Verbose mode (debug output)
clover chat --verbose

# Isolated git worktree (for running multiple agents in parallel)
clover -w                         # Interactive mode in worktree
clover -w -z "Fix issue #123"     # Single query in worktree
```

### Worktree cleanup

`clover -w` sessions create disposable worktrees under `<repo>/.worktrees/`.
...
```bash
clover worktree list              # audit: age, size, verdict, reason per tree
clover worktree prune             # remove safe trees + delete merged branches
clover worktree prune --dry-run   # show the plan without changing anything
clover worktree prune --trees-only     # leave local branches alone
clover worktree prune --branches-only  # leave worktrees alone
```
...
- Trees **in use by a running clover session** are never touched.
- **Untracked-only scratch** (PR body drafts, notes) is archived to
  `~/.clover/archive/worktree-prune/` before its tree is removed — never
  destroyed.
...
The `clover plugins` commands manage native Clover plugins and portable Agent
...
clover plugins disable <plugin-name>
clover plugins update <plugin-name>
clover plugins remove <plugin-name>
```

Portable packages remain disabled until explicitly enabled. Clover currently
...
<img className="docs-terminal-figure" src="/docs/img/docs/cli-layout.svg" alt="Stylized preview of the Clover CLI layout showing the banner, conversation area, and fixed input prompt." />
<p className="docs-figure-caption">The Clover CLI banner, conversation stream, and fixed input prompt rendered as a stable docs figure instead of fragile text art.</p>
...
| ⚠ YOLO | **YOLO mode warning** — shown whenever `CLOVER_YOLO_MODE` is on (either `clover --yolo` at launch or `/yolo` toggled mid-session). Mirrors the banner-line warning so you can't forget you're in auto-approve mode. |
...
| `Ctrl+D` | Exit |
| `Ctrl+Z` | Suspend Clover to background (Unix only). Run `fg` in the shell to resume. |
...
- **Approvals still apply.** A dangerous command (`rm -rf`, writes to `~/.clover/config.yaml`, etc.) goes through the same approval prompt the agent's `terminal` tool uses. `!` is a cost/latency shortcut, not a security bypass.
...
Type `/` to see the autocomplete dropdown. Clover supports a large set of CLI slash commands, dynamic skill commands, and user-defined quick commands.
...
| `/voice tts` | Toggle spoken playback for Clover replies |
...
```bash
clover -s clover-c1-dev,github-auth
clover chat -s github-pr-workflow -s github-auth
```

Clover loads each named skill into the session prompt before the first turn. The same flag works in interactive mode and single-query mode.

## Skill Slash Commands

Every installed skill in `~/.clover/skills/` is automatically registered as a slash command. The skill name becomes the command:
...
You can also define custom personalities in `~/.clover/config.yaml`:

```yaml
personalities:
  helpful: "You are a helpful, friendly AI assistant."
  kawaii: "You are a kawaii assistant! Use cute expressions..."
  pirate: "Arrr! Ye be talkin' to Captain Clover..."
  # Add your own!
```
...
`Ctrl+J` and backslash continuation are enabled by default, matching Claude Code / Codex / OpenCode multiline shortcuts. On supported terminals such as iTerm2, Clover also requests extended key reporting so `Shift+Enter` arrives as a distinct newline key. If your terminal sends LF for plain `Enter` and you need the legacy `Ctrl+J`-as-submit fallback, opt out:
...
Most terminals send the same byte sequence for `Enter` and `Shift+Enter` by default, so applications cannot distinguish them. Clover recognises `Shift+Enter` only when the terminal sends a distinct sequence via the [Kitty keyboard protocol](https://sw.kovidgoyal.net/kitty/keyboard-protocol/) or xterm's `modifyOtherKeys` mode.
...
Where the terminal cannot distinguish them, `Alt+Enter` and `Ctrl+J` continue to work by default. **On Windows Terminal specifically, `Alt+Enter` is captured by the terminal (toggles fullscreen) and never reaches Clover — use `Ctrl+Enter` (delivered as `Ctrl+J`) or `Ctrl+J` directly for a newline.**
...
The first time you press Enter while Clover is working, Clover prints a one-line reminder explaining the `/busy` knob. It only fires once per install; `onboarding.seen.busy_input_prompt` in `config.yaml` records that it was shown. Delete that key to see the tip again.
:::

### Suspending to Background

On Unix systems, press **`Ctrl+Z`** to suspend Clover to the background — just like any terminal process. The shell prints a confirmation:

```
Clover Cognition has been suspended. Run `fg` to bring Clover Cognition back.
```
...
Resume this session with:
  clover --resume 20260225_143052_a1b2c3
...
```bash
clover --continue                          # Resume the most recent CLI session
clover -c                                  # Short form
clover -c "my project"                     # Resume a named session (latest in lineage)
clover --resume 20260225_143052_a1b2c3     # Resume a specific session by ID
clover --resume "refactoring auth"         # Resume by title
clover --resume latest                     # Resume the most recent session (same as -c)
clover --resume latest --in ./my-project   # Latest session for ./my-project's workspace
clover -r 20260225_143052_a1b2c3           # Short form
```

Resuming restores the full conversation history from SQLite. The agent sees all previous messages, tool calls, and responses — just as if you never left.

Use `/title My Session Name` inside a chat to name the current session, or `clover sessions rename <id> <title>` from the command line. Use `clover sessions list` to browse past sessions.

### Session Storage

CLI sessions are stored in Clover's SQLite state database under `~/.clover/state.db`. The database keeps:
...
Clover immediately confirms the task and gives you back the prompt:
