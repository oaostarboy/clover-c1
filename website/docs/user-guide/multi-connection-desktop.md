...
# Connecting Desktop to Many Clover Instances

Register every Clover backend you own — the local runtime, remote gateways on
your LAN or VPS, SSH hosts, and Clover Cloud instances — in one desktop app,
...
- **The sidebar profile rail** — the plug button at the right end of the rail
  (tooltip: **"Connect another Clover gateway…"**) deep-links straight to
...
list of Clover gateways. Its intro says it plainly: *"Manage this device and
every Clover gateway it can reach through remote, SSH, or Cloud connections."*
Each entry is a *connection*:

| Kind | What it is | Auth |
|---|---|---|
| **Local** | "The Clover runtime managed by this app." | automatic |
| **Remote gateway** | "A Clover gateway reachable over HTTP(S) — LAN, Tailscale, or the internet." | session token or OAuth |
| **SSH** | "A Clover install reached over SSH." The app opens the tunnel and starts the dashboard for you | SSH key + adopted token |
| **Clover Cloud** | "A hosted instance discovered through your Clover Cloud account." | portal sign-in |
...
- **Duplicates are rejected when you save**: there is only ever one **local**
  ...
  **SSH** entries are deduplicated on the normalized `user@host:port` plus
  remote profile.
- Cloud entries normally come from the Clover Cloud sign-in/discovery flow at
  the top of the Gateways page — the **Clover Cloud** kind in the add-connection
...
3. Pick the kind: **Local**, **Clover Cloud**, **Remote gateway**, or **SSH**.
   (**Local** is disabled while the app-managed local entry exists — which is
   almost always; **Clover Cloud** directs you to the cloud sign-in/discovery
   flow above.)
4. Fill the fields:
   - **Name** — required, unique; the "device name" shown everywhere this
     instance appears (placeholder: `Homelab`). Max 64 characters.
   - *Remote gateway only:*
     - **Gateway URL** — the base URL of a running `clover serve` backend,
     ...
     - **Authentication** — choose **Session token** or **OAuth**:
       - **Session token** — paste the dashboard session token from the
         ...
         token."*
       - **OAuth** — sign in through the Clover Portal browser flow; no token
         to paste.
   - *SSH only:*
...
Gateways are machines or hosted backends; profiles are isolated Clover agents
...
connection is registered) dispatches `clover update` to every eligible
...
- **Clover Cloud** instances are skipped with a *"Managed by Clover Cloud"*
...
panel, ⌘K **Update Clover**, the update-ready toast) run the same fan-out
...
- **"Connection test failed"** — the backend isn't reachable at that URL from
  this machine. Check that `clover serve` is running on the remote host, the
...
- **"Update Clover Desktop to chat with agents on other connections"** — the
