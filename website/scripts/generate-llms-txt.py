#!/usr/bin/env python3
"""Generate llms.txt and llms-full.txt for the Clover docs site.
...
LLM reads to learn what Clover does.

Both publish at:
  https://clover-c1.local/docs/llms.txt
  https://clover-c1.local/docs/llms-full.txt
...
SITE_BASE = "https://clover-c1.local/docs"
...
SECTIONS: list[tuple[str, list[tuple[str, str, str | None]]]] = [
    ...
    ("Guides & Tutorials", [
        ...
        ("guides/python-library", "Use Clover as a Python Library", None),
        ("guides/use-mcp-with-clover", "Use MCP with Clover", None),
        ("guides/use-voice-mode-with-clover", "Use Voice Mode with Clover", None),
        ("guides/use-soul-with-clover", "Use SOUL.md with Clover", None),
    ...
    ("Reference", [
        ...
        ("reference/skills-catalog", "Bundled Skills Catalog", "Table of all ~90 skills bundled with Clover"),
...
def emit_llms_index() -> str:
    ...
    lines: list[str] = []
    lines.append("# Clover Cognition")
    lines.append("")
    lines.append(
        "> The self-improving AI agent built by Clover Cognition. A terminal-native "
        ...
        "backends. Works with Clover Portal, OpenRouter, OpenAI, Anthropic, Google, or any "
        "OpenAI-compatible endpoint."
    )
    lines.append("")
    lines.append(
        "Install: `curl -fsSL https://raw.githubusercontent.com/CloverCognition/"
        "clover-c1/main/scripts/install.sh | bash`  "
        "(Linux, macOS, WSL2, Termux)"
    )
    lines.append("")
    lines.append("Repo: https://github.com/CloverCognition/clover-c1")
    lines.append("")
...
def emit_llms_full() -> str:
    ...
    seen: set[Path] = set()
    chunks: list[str] = [
        "# Clover Cognition — Full Documentation\n",
        (
            "This file is the entire Clover Cognition documentation concatenated for LLM "
            "context ingestion. Section order reflects docs-site navigation: Getting "
            "Started, Using Clover, Features, Messaging, Integrations, Guides, "
            "Developer Guide, Reference, then everything else.\n"
        ),
        "Canonical site: https://clover-c1.local/docs\n",
        "Short index: https://clover-c1.local/docs/llms.txt\n",
        "\n---\n\n",
    ]
