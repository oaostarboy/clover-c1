...
def rewrite_relative_links(body: str, meta: dict[str, Any]) -> str:
    ...
    base = f"https://github.com/CloverCognition/clover-c1/blob/main/{source_dir}/{meta['rel_path']}"
...
def render_skill_page(
    ...
    clover_meta = (fm.get("metadata") or {}).get("clover") or {}
    tags = clover_meta.get("tags") or []
    related = clover_meta.get("related_skills") or []
    ...
    if meta["source_kind"] == "bundled":
        ...
        info_rows.append(
            (
                "Source",
                "Optional — install with `clover skills install official/"
                + meta["category"]
                + "/"
                + meta["slug"]
                + "`",
...
def build_catalog_md_bundled(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        'description: "Catalog of bundled skills that ship with Clover Cognition"',
        "---",
        "",
        "# Bundled Skills Catalog",
        "",
        "Clover ships with a large built-in skill library copied into `~/.clover/skills/` on install. Each skill below links to a dedicated page with its full definition, setup, and usage.",
        "",
        "Clover also syncs bundled skills on `clover update`, but the sync manifest respects local deletions and user edits. If a skill listed here is missing from your profile's `~/.clover/skills/` tree, it is still shipped with Clover; restore it with `clover skills reset <name> --restore`.",
        "",
...
def build_catalog_md_optional(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        'description: "Official optional skills shipped with clover-c1 — install via clover skills install official/<category>/<skill>"',
        "---",
        "",
        "# Optional Skills Catalog",
        "",
        "Optional skills ship with clover-c1 under `optional-skills/` but are **not active by default**. Install them explicitly:",
        "",
        "```bash",
        "clover skills install official/<category>/<skill>",
        "```",
        "",
        "For example:",
        "",
        "```bash",
        "clover skills install official/blockchain/solana",
        "clover skills install official/mlops/flash-attention",
        "```",
        "",
        ...
        "",
        "```bash",
        "clover skills uninstall <skill-name>",
        "```",
        "",
    ]
    for category in sorted(by_cat):
