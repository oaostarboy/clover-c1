...
This plugin connects Clover Cognition to iMessage (and other Spectrum
...
TypeScript-only, Clover runs it inside a small supervised Node sidecar and
...
There is no separate `login` command; like every other Clover channel,
...
Run `clover photon status` to see what's configured.

## Credentials

Runtime SDK credentials live in `~/.clover/.env` (the same place every other
...
Management metadata lives in `~/.clover/auth.json` under `credential_pool`:
...
| `PHOTON_TELEMETRY`        | false                      | Spectrum SDK telemetry — toggle with `clover photon telemetry on\|off` (restart the gateway to apply) |
...
- **Inbound attachments and voice notes are downloaded.** The sidecar reads
  ...
  something arrived. If Spectrum emits a `richlink` content object, Clover
  ...
  `.pluginPayloadAttachment` images immediately after the URL; Clover coalesces
...
- **Native polls are supported.** Clover posts poll content through
...
- **Cron/standalone sends require a running gateway.** Processes outside
  the gateway (cron subprocesses, `clover send`) cannot spawn the sidecar;
  they authenticate to the gateway's live sidecar via the runtime record at
  `<clover-home>/runtime/photon-sidecar.json` (written after the sidecar's
...
6. Verify end-to-end: `clover photon status`, a DM and a group roundtrip,
