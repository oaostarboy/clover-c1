// Clover Cognition — Photon Spectrum sidecar
//
...
//
// Inbound  (gRPC -> Clover): the SDK's `app.messages` async iterator is a
...
//   gateway connects.
// Outbound (Clover -> gRPC): `/send` drives `space.send(...)`; `/typing`
//   sends the documented `typing("start" | "stop")` content builder.
//
// Protocol (all requests require `X-Clover-Sidecar-Token: ${TOKEN}`):
...
//                          default off — toggle with `clover photon telemetry`)
  ...
  streamRestartTimer = setTimeout(() => {
    ...
    console.error(
      `photon-sidecar: upstream stream degraded for ${degradedForMs}ms; ` +
        "exiting so Clover can restart the Photon adapter"
    );
    process.exit(75);
...
const PROBE_MSG_PREFIX = "clover-liveness-probe-";
...
// instead of a cryptic module-resolution error during import. Apply Clover'
...
function inboundStreamErrorMessage(e) {
  ...
  // iMessage internal error. Local Clover allowlists cannot cause or fix this:
  ...
  if (
    ...
    out +=
      ...
      "this is upstream of Clover, so inbound iMessages may not be delivered " +
...
const server = http.createServer(async (req, res) => {
  if (!tokenOk(req.headers["x-clover-sidecar-token"])) {
  ...
  try {
    ...
    if (req.url === "/send-attachment") {
      ...
      // overrides only when Clover supplied them so a known-good
