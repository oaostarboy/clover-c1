...
Small Node helper that bridges Clover Cognition to Photon's Spectrum SDK
(`spectrum-ts`).  Clover is Python; Photon has no public HTTP
...
- exposes a loopback-only HTTP control channel for the Python adapter
  to push send/typing requests (auth via `X-Clover-Sidecar-Token`)
- drains the inbound message stream so `spectrum-ts` keeps its
  reconnect/heartbeat machinery alive and Clover can receive inbound messages
...
The Clover plugin's `clover photon setup` command runs `npm install`
here automatically.
...
`Space.send(...)` API. Clover is Python, so replies go through this sidecar
