...
under an immutable ``/opt/clover`` (read-only for the clover user), which
...
4. Source dir read-only and deps missing or stale → mirror the sidecar
   source files to ``$CLOVER_HOME/photon/sidecar`` (the durable data volume,
...
def resolve_sidecar_dir(source_dir: Optional[Path] = None) -> Path:
    ...
    mirror = get_clover_home() / "photon" / "sidecar"
    try:
