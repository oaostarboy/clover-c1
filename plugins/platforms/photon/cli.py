"""
``clover photon ...`` CLI subcommands — registered by the plugin via
...
there is no standalone ``login`` verb (matching how every other Clover
...
# Writable sidecar runtime dir (mirrors to CLOVER_HOME on immutable
...
# wiring imports this module for `clover --help`).
...
# when called later (gateway start, clover status). Cleared on success.
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire up `clover photon ...` subcommands."""
    ...
    p_setup.add_argument("--project-name", default=None,
                         help="Project name (default: 'Clover Cognition')")
...
def _run_device_login(args: argparse.Namespace) -> int:
    ...
    no standalone ``clover photon login`` command; Photon onboards
...
def _cmd_setup(args: argparse.Namespace) -> int:
    ...
    # 2. Find or create the "Clover Cognition" project.
    ...
    # 3. Provision Spectrum credentials (runtime -> ~/.clover/.env,
    ...
    try:
        from clover_cli.config import write_platform_config_field
    ...
    print("  Start the gateway:  clover gateway start")
    return 0
...
def _cmd_status(_args: argparse.Namespace) -> int:
    ...
    print(f"  sidecar deps        : {'✓ installed' if sidecar_installed else '✗ run `clover photon install-sidecar`'}")
    print(f"  telemetry           : {'on' if _telemetry_enabled() else 'off'} (`clover photon telemetry on|off`)")
    return 0
...
def _telemetry_enabled() -> bool:
    """Read PHOTON_TELEMETRY from the env / ~/.clover/.env.
...
def _cmd_telemetry(args: argparse.Namespace) -> int:
    ...
    if state is None:
        ...
        print("  Toggle with `clover photon telemetry on` / `clover photon telemetry off`.")
        return 0
    try:
        ...
        return 1
    print(f"✓ Spectrum telemetry turned {state} (PHOTON_TELEMETRY in ~/.clover/.env)")
    print("  Restart the gateway for the sidecar to pick it up:  clover gateway restart")
    return 0
...
#
# `clover gateway setup` discovers platforms via the registry and calls each
...
# project + user + sidecar flow as ``clover photon setup`` with interactive
# defaults (phone is prompted when stdin is a TTY).

def gateway_setup() -> None:
    """Run Photon first-time setup from the `clover gateway setup` wizard."""
