"""
Email platform adapter for the Clover gateway.

Allows users to interact with Clover by sending emails.
...
class EmailAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # send-helper, and `clover config show` already read. Without the
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        if missing:
            message = (
                ...
                + ", ".join(missing)
                + ". Set it via `clover gateway setup` (env) or platforms.email "
                "in config.yaml."
            )
    ...
    def _send_email(
        ...
        subject = ctx.get("subject", "Clover Cognition")
        ...
        msg_id = f"<clover-{uuid.uuid4().hex[:12]}@{self._message_id_domain()}>"
...
# clover_cli/gateway.py, and the _send_email dispatch in
