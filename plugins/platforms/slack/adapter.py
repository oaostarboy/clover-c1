...
# identify CloverAgent traffic — matching other Clover outbound surfaces
# that already set ``CloverAgent/<version>`` for platform-partner attribution.
try:
    from clover_cli import __version__ as _CLOVER_VERSION
except Exception:
    _CLOVER_VERSION = "unknown"
_CLOVER_SLACK_USER_AGENT_PREFIX = f"CloverAgent/{_CLOVER_VERSION}"
...
def _slack_unfurl_kwargs(extra: Optional[Dict[str, Any]]) -> Dict[str, bool]:
    ...
    ``_slack_unfurl_hints``: ``clover config set`` and Railway persist YAML
...
def _rewrite_known_bang_command(text: str) -> str:
    ...
    try:
        from clover_cli.commands import is_gateway_known_command
...
def _normalize_slack_text_for_dedupe(text: str, bot_uid: str = "") -> str:
    ...
    # After the label, so that ``<@U…|clover>`` is stripped like ``<@U…>``.
    if bot_uid:
...
class SlackAdapter(BasePlatformAdapter):
        ...
        try:
            ...
            if "im:history" in granted and "mpim:history" not in granted:
                ...
                logger.warning(
                    ...
                    "Regenerating the app from `clover slack` produces a "
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        if not raw_token:
            logger.error(
                ...
                "error; set SLACK_BOT_TOKEN via `clover gateway setup` "
                "or in the active profile's ~/.clover/.env file, then restart "
                "the gateway.",
            )
            self._set_fatal_error(
                "missing_slack_bot_token",
                "SLACK_BOT_TOKEN not configured. Use `clover gateway setup` "
                "or add it to your active profile's ~/.clover/.env file, "
        ...
        if not app_token:
            logger.error(
                ...
                "error; set SLACK_APP_TOKEN via `clover gateway setup` "
                "or in the active profile's ~/.clover/.env file, then restart "
                "the gateway.",
            )
            self._set_fatal_error(
                "missing_slack_app_token",
                "SLACK_APP_TOKEN not configured. Use `clover gateway setup` "
                "or add it to your active profile's ~/.clover/.env file, "
                "then restart the gateway.",
                retryable=False,
            )
        ...
        tokens_file = get_clover_home() / "slack_tokens.json"
        ...
        try:
            ...
            primary_client = AsyncWebClient(
                token=primary_token,
                user_agent_prefix=_CLOVER_SLACK_USER_AGENT_PREFIX,
            )
            self._app = AsyncApp(
                token=primary_token,
            ...
            # Clover has no listener for (e.g. user_change,
            ...
            async def handle_unhandled_event(event, body, logger):
                logger.debug(
                    ...
                    "registered; subscribed events not handled by Clover can "
                    "be removed from the Slack app manifest via "
                    "`clover slack manifest`)",
                    (event or {}).get(
                        "type",
            ...
            # /model work directly without /clover prefix). A single regex
            ...
            # manifest (see `clover slack manifest`). In Socket Mode, Slack
            ...
            from clover_cli.commands import slack_native_slashes
            import re as _re
            ...
            if _slash_names:
                ...
                _slash_pattern = _re.compile(r"^/clover$")

            @self._app.command(_slash_pattern)
            async def handle_clover_command(ack, command):
            ...
            for _action_id in (
                "clover_approve_once",
                "clover_approve_session",
                "clover_approve_always",
                "clover_deny",
            ):
            ...
            for _action_id in (
                "clover_confirm_once",
                "clover_confirm_always",
                "clover_confirm_cancel",
            ):
                self._app.action(_action_id)(self._handle_slash_confirm_action)

            self._app.action("clover_feedback")(self._handle_feedback_action)
            ...
            self._app.action(
                _re.compile(r"^clover_clarify_choice_\d+$")
            )(self._handle_clarify_action)
            self._app.action("clover_clarify_other")(self._handle_clarify_action)
            ...
            if _allow_bots_cfg != "none":
                logger.info(
                    ...
                    "'clover slack manifest' if unsure), and (b) the other bot's "
    ...
    async def create_handoff_thread(
        ...
        try:
            ...
            if client is None:
                return None
            seed_text = (
                f":thread: Clover handoff — *{(name or 'session').strip()[:80]}*"
            )
    ...
    def _dm_top_level_threads_as_sessions(self) -> bool:
        ...
        own Clover session — matching the per-thread behavior channels already
    ...
    async def _handle_app_context_changed(
        ...
        # Clover owns. Do not write it into _channel_team: channel IDs can be
    ...
    async def _handle_slack_reaction(self, event: dict, removed: bool = False) -> None:
        ...
        if target_channel:
            ...
            synthetic["_clover_reaction_source_channel"] = channel_id
            if target_thread:
                synthetic["thread_ts"] = target_thread
            else:
                synthetic.pop("thread_ts", None)
                synthetic["_clover_no_thread_response"] = True
    ...
    async def _handle_slack_message_impl(
        ...
        if is_dm:
            ...
            if not thread_ts and self._dm_top_level_threads_as_sessions():
                thread_ts = ts
        elif event.get("_clover_no_thread_response"):
        ...
        force_process = bool(event.get("_clover_force_process"))
        ...
        # the shape produced by peer Clover agents in Socket Mode on some
        ...
        try:
            ...
            if not smart_denied and allow_session:
                actions.append({
                    ...
                    "action_id": "clover_approve_session",
                    "value": session_key,
                })
                if allow_permanent:
                    actions.append({
                        ...
                        "action_id": "clover_approve_always",
                        "value": session_key,
                    })
            actions.append({
                "type": "button",
                    ...
                    "elements": [
                        {
                            ...
                            "style": "primary",
                            "action_id": "clover_confirm_once",
                            "value": value,
                        },
                        {
                            "type": "button",
                            "text": {"type": "plain_text", "text": "Always Approve"},
                            "action_id": "clover_confirm_always",
                            "value": value,
                        },
                        {
                            "type": "button",
                            ...
                            "style": "danger",
                            "action_id": "clover_confirm_cancel",
                            "value": value,
                        },
                    ],
                },
    ...
    async def send_clarify(
        ...
        (unique ``clover_clarify_choice_<idx>`` action_id, ``value`` packs
        ``clarify_id|idx``) plus a final "✏️ Other…" button
        (``clover_clarify_other``).  A choice click resolves the clarify
        ...
        try:
            ...
            for idx, choice in enumerate(choices):
                ...
                elements.append({
                    ...
                    "action_id": f"clover_clarify_choice_{idx}",
                    "value": f"{clarify_id}|{idx}",
                })
            elements.append({
                "type": "button",
                "text": {"type": "plain_text", "text": "✏️ Other…", "emoji": True},
                "action_id": "clover_clarify_other",
    ...
    async def _handle_slash_confirm_action(self, ack, body, action) -> None:
        ...
        choice_map = {
            "clover_confirm_once": "once",
            "clover_confirm_always": "always",
            "clover_confirm_cancel": "cancel",
        }
    ...
    async def _handle_approval_action(self, ack, body, action) -> None:
        ...
        choice_map = {
            "clover_approve_once": "once",
            "clover_approve_session": "session",
            "clover_approve_always": "always",
            "clover_deny": "deny",
        }
    ...
    async def _handle_clarify_action(self, ack, body, action) -> None:
        ...
        if action_id == "clover_clarify_other" or token == "other":
    ...
    async def _handle_slash_command(self, command: dict) -> None:
        ...
        The legacy ``/clover <subcommand> [args]`` form is preserved for
        backward compatibility with older workspace manifests and for users
        who want a single entry point for free-form questions (``/clover
        ...
        if slash_name in {"clover", ""}:
            # Legacy /clover <subcommand> [args] routing + free-form questions.
            # Empty slash_name falls into this branch for backward compat
            # with any caller that didn't populate command["command"].
            legacy_text = raw_text.strip()
            from clover_cli.commands import slack_subcommand_map

            subcommand_map = slack_subcommand_map()
            subcommand_map["compact"] = "/compress"
            # Guard against whitespace-only text where ``text`` is truthy but
            # ``text.split()`` returns ``[]`` (e.g. user sends ``/clover   ``).
        ...
        # "Running /cmd…" ack shown by handle_clover_command).
        # Only stash for COMMAND events (text starts with "/") — free-form
        # questions via "/clover <question>" must produce public replies so
...
# ``clover_cli/{setup,gateway}.py``, and the ``_send_slack`` dispatch in
...
async def _standalone_send(
    ...
    *,
    ...
    try:
        ...
        _tokens_file = get_clover_home() / "slack_tokens.json"
...
def interactive_setup() -> None:
    ...
    offers to set a home channel. Replaces ``clover_cli/setup.py::_setup_slack``.
    """
    from pathlib import Path
    from clover_cli.config import get_env_value, remove_env_value, save_env_value
    from clover_cli.cli_output import (
        prompt,
        prompt_yes_no,
        print_header,
        print_info,
    ...
    def _write_slack_manifest_and_instruct() -> None:
        """Generate the Slack manifest, write it under CLOVER_HOME, and print
        paste-into-Slack instructions. Failures are non-fatal."""
        try:
            from clover_cli.slack_cli import _build_full_manifest
            from clover_constants import get_clover_home
            import json as _json

            manifest = _build_full_manifest(
                bot_name="Clover",
                bot_description="Your Clover agent on Slack",
            )
            target = Path(get_clover_home()) / "slack-manifest.json"
            ...
            print_info(
                "   Re-run `clover slack manifest --write` anytime to refresh after "
                "Clover adds new commands."
            )
        except Exception as e:
    ...
    if existing:
        ...
        if not prompt_yes_no("Reconfigure Slack?", False):
            ...
            if prompt_yes_no(
                ...
                "list? (recommended after `clover update`)",
                True,
            ):
    ...
    print()
    print_info("   Full guide: docs/user-guide/messaging/slack/")
    print()
...
def _is_connected(config) -> bool:
    ...
    Looks up via ``clover_cli.gateway.get_env_value`` at call time (not via the
...
def register(ctx) -> None:
    """Plugin entry point — called by the Clover plugin system."""
    ctx.register_platform(
        ...
        install_hint="Run `clover setup` to install Slack support.",
        # Interactive setup wizard — replaces clover_cli/setup.py::_setup_slack
        # and the static _PLATFORMS["slack"] dict in clover_cli/gateway.py.
