"""ntfy platform adapter (Clover plugin).
...
``plugins/platforms/ntfy/``. The Clover plugin loader scans the
    ...
    platforms:
      ntfy:
        ...
        extra:
          ...
          topic: "clover-in"              # subscribe topic (incoming)
          publish_topic: "clover-out"     # optional — defaults to topic
...
_ECHO_TAG = "clover-c1"  # tag added to outgoing messages for echo-loop prevention
...
async def _standalone_send(
    ...
    *,
    ...
    ``clover cron`` running standalone). Without this hook,
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="pip install httpx   # already a Clover dependency",
        # Env-driven auto-configuration: seeds PlatformConfig.extra so
        # env-only setups show up in `clover gateway status` without
