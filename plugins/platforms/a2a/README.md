# A2A — Agent-to-Agent protocol for Clover
...
peer (another Clover, LangChain, CrewAI, Google ADK, OpenClaw, …). Stdlib only —
no `a2a-sdk` dependency.

## Enable

```bash
clover gateway setup      # pick A2A, or:
```
...
When the `a2a` platform is enabled, Clover serves a v1.0 Agent Card at
...
- Every exchange is logged to `~/.clover/a2a_audit.jsonl`.
- Conversations persist to `~/.clover/a2a_conversations/` — they survive context
