"""SimpleX Chat platform adapter (Clover plugin).
...
This adapter ships as a Clover platform plugin under
``plugins/platforms/simplex/``. The Clover plugin loader scans the
...
Optional environment variables:
    ...
    CLOVER_SIMPLEX_TEXT_BATCH_DELAY
...
discoverable and ``clover setup`` can describe it even when websockets is
...
# Imported at module top because they're stdlib-only inside Clover — no
...
class SimplexAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig, **kwargs):
        ...
        self._text_batch_delay = float(
            os.getenv("CLOVER_SIMPLEX_TEXT_BATCH_DELAY", "0.8")
        )
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``clover setup gateway`` → SimpleX.

    Prompts for the WebSocket URL and the optional allowlist / groups /
    auto-accept / home channel. Writes to ``~/.clover/.env`` via
    ``clover_cli.config``.
    """
    print()
    ...
    try:
        ...
        print(
            "clover_cli.config not available; set SIMPLEX_* vars manually in "
            "~/.clover/.env"
        )
        return
