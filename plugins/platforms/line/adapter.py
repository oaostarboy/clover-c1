"""
LINE Messaging API platform adapter for Clover Cognition.
...
to Clover Cognition. It deliberately ports the *strongest* idea from each into
...
class LineAdapter(BasePlatformAdapter):
    ...
    async def _handle_media(self, request) -> Any:
        ...
        ``/private/tmp`` on macOS), and ``CLOVER_HOME``. PR #8398.
        """
        from aiohttp import web
        ...
        try:
            ...
            clover_home = Path(get_clover_home()).resolve()
        except Exception:
            clover_home = Path.home().joinpath(".clover").resolve()
...
def is_connected(config) -> bool:
    """Surface in ``clover status`` even before the adapter is instantiated."""
...
def _env_enablement() -> Optional[Dict[str, Any]]:
    ...
    Lets ``clover status`` reflect a LINE configuration that lives entirely
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``clover setup line``.

    Mirrors the irc/teams style: prompts for the two required vars, plus
    one optional public URL. Writes to ``~/.clover/.env`` via ``clover_cli.config``.
    """
    print()
    ...
    try:
        from clover_cli.config import get_env_value as _get_env, save_env_value as _set_env
    except ImportError:
        print("clover_cli.config not available; set LINE_* vars manually in ~/.clover/.env")
        return
