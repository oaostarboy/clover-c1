...
# ``model_tools`` import time, so ``clover status``, ``clover chat``, etc.
...
# ``clover_plugins.platforms__google_chat.adapter`` once the plugin
...
#     fresh "Clover is thinking…" card during the small window between
...
def _mime_for_message_type(mime: str) -> MessageType:
    """Map a MIME string to a clover MessageType.
...
def card_spec_to_cards_v2(card_spec: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {"cardId": str(card_spec.get("card_id") or "clover-card"), "card": card}
...
class GoogleChatAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # Persisted to ${CLOVER_HOME}/google_chat_thread_counts.json so
        ...
        try:
            ...
            _clover_home = _get_clover_home()
        except (ModuleNotFoundError, ImportError):
            _clover_home = _Path.home() / ".clover"
        self._thread_count_store = _ThreadCountStore(
            _clover_home / "google_chat_thread_counts.json"
        )
        ...
        # they don't sit in the chat forever as "Clover is thinking…".
    ...
    def _bot_id_cache_path(self) -> _Path:
        ...
        base = os.getenv("CLOVER_HOME", str(_Path.home() / ".clover"))
    ...
    async def _dispatch_message(self, msg: Dict[str, Any], envelope: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if text.startswith("/setup-files") and event.source is not None:
                ...
                # ${CLOVER_HOME}/google_chat_user_tokens/<sanitized>.json
    ...
    async def _build_message_event(
        ...
        """Parse a Chat API message into a clover MessageEvent."""
        space = envelope.get("space") or msg.get("space") or {}
    ...
    async def send_typing(self, chat_id: str, metadata: Any = None) -> None:
        """Post a visible 'Clover is thinking…' marker message.
        ...
        "Clover is thinking…" stuck in chat forever, plus one card that
    ...
    async def stop_typing(self, chat_id: str) -> None:
        ...
        Google Chat has no separate typing API: the "Clover is thinking…"
    ...
    async def on_processing_complete(
        ...
        see "Clover is thinking…" stuck forever.
        """
    ...
    async def _consume_typing_card_with_text(
        ...
        creating a fresh "Clover is thinking…" card during any subsequent
...
def interactive_setup() -> None:
    """Walk the user through Google Chat configuration via ``clover setup``.

    The setup wizard at ``clover_cli/gateway.py`` calls this for plugin
    ...
    prompt for env vars, persist them to ``~/.clover/.env`` so the next
    ...
    print_info("  3. Create a Pub/Sub topic (e.g. clover-chat-events) and a Pull subscription.")
    ...
    print()
    print_success("Google Chat configuration saved to ~/.clover/.env")
    print_info("Restart the gateway: clover gateway restart")
...
async def _standalone_send(
    ...
    *,
    ...
    runner is not in this process (e.g. ``clover cron`` running as a
    separate process from ``clover gateway``).  Without this hook,
...
def register(ctx) -> None:
    """Plugin entry point — called by the Clover plugin system at startup.
    ...
    ctx.register_platform(
        ...
        install_hint="Run `clover setup` to install Google Chat support.",
        ...
        # the "Clover is thinking..." marker patches and edit overhead.
        ...
        platform_hint=(
            ...
            "are generating a response, a 'Clover is thinking…' marker message "
