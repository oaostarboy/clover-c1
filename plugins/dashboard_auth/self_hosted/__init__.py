...
A standards-compliant OpenID Connect Relying Party for the ``clover dashboard``
OAuth gate. Unlike the bundled ``clover`` provider (which encodes Clover Portal's
bespoke contract — ``agent:{instance_id}`` client ids, a custom access-token
JWT, the ``x-clover-refresh-token`` header, an ``oauth_contract_version`` claim),
...
:class:`~clover_cli.dashboard_auth.DashboardAuthProvider` methods and touches
...
``clover_cli/dashboard_auth/routes.py``; this provider only:
  ...
  4. verifies the **ID token** (RS256/ES256) against the discovered
     ...
     onto a :class:`~clover_cli.dashboard_auth.Session`.
...
only choice that is universally correct across self-hosted IDPs. (The ``clover``
provider verifies its *access* token because Clover Portal mints a custom JWT
...
same precedence convention as the ``clover`` plugin)::
    ...
    dashboard:
      oauth:
        ...
        self_hosted:
          ...
          # credential — prefer the env var / ~/.clover/.env over config.yaml.

    # Environment overrides (Docker/Fly secret injection)
    CLOVER_DASHBOARD_OIDC_ISSUER
    CLOVER_DASHBOARD_OIDC_CLIENT_ID
    CLOVER_DASHBOARD_OIDC_SCOPES        # optional; defaults to "openid profile email"
    CLOVER_DASHBOARD_OIDC_CLIENT_SECRET # optional; set for a confidential client
...
# JWKS cache (PyJWKClient handles its own caching; this mirrors the clover
# provider's 5-minute lifespan so key rotation is picked up promptly).
_JWKS_CACHE_SECONDS = 300


# ---------------------------------------------------------------------------
# Skip-reason channel (mirrors the clover plugin)
# ---------------------------------------------------------------------------
...
class SelfHostedOIDCProvider(DashboardAuthProvider):
    ...
    def _session_from_tokens(
        ...
        *,
        ...
        OAuth access token is intentionally NOT stored — Clover does not call
    ...
    def _validate_redirect_uri(self, redirect_uri: str) -> None:
        ...
        redirect_uris are permitted. Mirrors the clover provider.
        """
...
def register(ctx) -> None:
    ...
    client_id are configured (via ``CLOVER_DASHBOARD_OIDC_*`` env vars or the
    ...
    issuer = _resolve_setting(
        "CLOVER_DASHBOARD_OIDC_ISSUER", oidc_cfg.get("issuer")
    )
    client_id = _resolve_setting(
        "CLOVER_DASHBOARD_OIDC_CLIENT_ID", oidc_cfg.get("client_id")
    )
    scopes = (
        _resolve_setting("CLOVER_DASHBOARD_OIDC_SCOPES", oidc_cfg.get("scopes"))
        or _DEFAULT_SCOPES
    )
    # Optional — set only for a confidential client. A credential, so the
    # canonical home is the env var / ~/.clover/.env; config.yaml is supported
    # for precedence symmetry. Empty ⇒ public client (unchanged behaviour).
    client_secret = _resolve_setting(
        "CLOVER_DASHBOARD_OIDC_CLIENT_SECRET", oidc_cfg.get("client_secret")
    )
    ...
    if not issuer or not client_id:
        LAST_SKIP_REASON = (
            ...
            "(CLOVER_DASHBOARD_OIDC_ISSUER + CLOVER_DASHBOARD_OIDC_CLIENT_ID) "
