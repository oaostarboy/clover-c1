"""CloverDashboardAuthProvider — Clover Portal OAuth (authorization-code + PKCE).

Implements ``clover-account-service/docs/agent-dashboard-oauth-contract.md``
...
Configuration surfaces (env wins over config.yaml when set non-empty):
  ...
  per-deploy values don't need to bake into ``config.yaml``:
      ...
      CLOVER_DASHBOARD_OAUTH_CLIENT_ID  — shape ``agent:{agent_instance_id}``
      CLOVER_DASHBOARD_PORTAL_URL       — defaults to
                                          ````
...
Key contract points encoded here:
  ...
  - audience claim is the bare ``client_id`` (no ``clover-cli:`` prefix).
...
"state": …}`` and the route serializes those into the ``clover_session_pkce``
cookie.
...
Skip reasons:
  ...
  ("Set CLOVER_DASHBOARD_OAUTH_CLIENT_ID …") instead of the bare "no
...
# Production Portal URL. Override via CLOVER_DASHBOARD_PORTAL_URL for
...
class CloverDashboardAuthProvider(DashboardAuthProvider):
    """Clover Portal OAuth via authorization-code + PKCE (S256)."""
    ...
    def start_login(self, *, redirect_uri: str) -> LoginStart:
        ...
        # The auth-route layer expects ``cookie_payload[\"clover_session_pkce\"]``
        ...
        cookie_payload = {
            "clover_session_pkce": f"state={state};verifier={code_verifier}",
        }
    ...
    def complete_login(
        ...
        *,
        ...
        # we just receive it for symmetry with the protocol. Clover Portal
    ...
    def refresh_session(self, *, refresh_token: str) -> Session:
        ...
        try:
            response = httpx.post(
                ...
                # ``x-clover-refresh-token`` header. Portal's token endpoint
                ...
                # Portal's ``REFRESH_TOKEN_HEADER`` exactly (``x-clover-refresh-
                ...
                headers={
                    ...
                    "x-clover-refresh-token": refresh_token,
                },
                timeout=_TOKEN_ENDPOINT_TIMEOUT_SEC,
            )
        except httpx.RequestError as exc:
    ...
    def _verify_jwt(self, access_token: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # between CLOVER_DASHBOARD_PORTAL_URL / CLOVER_DASHBOARD_OAUTH_CLIENT_ID
    ...
    def _check_contract_version(self, claims: Dict[str, Any]) -> None:
        ...
        if contract_version is None:
            logger.warning(
                "Clover Portal token missing oauth_contract_version claim "
...
def _load_config_oauth_section() -> dict:
    ...
    try:
        ...
        logger.debug(
            "dashboard-auth-clover: load_config() raised %s; "
...
def _resolve_client_id() -> str:
    ...
    Order:
      1. ``CLOVER_DASHBOARD_OAUTH_CLIENT_ID`` env var (when non-empty
    ...
    """
    env = os.environ.get("CLOVER_DASHBOARD_OAUTH_CLIENT_ID", "").strip()
    if env:
        return env
...
def _resolve_portal_url() -> str:
    ...
    Order:
      1. ``CLOVER_DASHBOARD_PORTAL_URL`` env var (non-empty after strip).
      2. ``dashboard.oauth.portal_url`` in ``config.yaml``.
      3. :data:`_DEFAULT_PORTAL_URL` (production Portal).
    """
    env = os.environ.get("CLOVER_DASHBOARD_PORTAL_URL", "").strip()
    if env:
        return env
    cfg_value = str(
...
def register(ctx) -> None:
    ...
    Registers ``CloverDashboardAuthProvider`` only when a client_id is
    configured (either via ``CLOVER_DASHBOARD_OAUTH_CLIENT_ID`` env var
    ...
    can surface "Set CLOVER_DASHBOARD_OAUTH_CLIENT_ID …" instead of the
    ...
    engagement layer (``clover_cli.web_server.should_require_auth`` +
    ...
    if not client_id:
        LAST_SKIP_REASON = (
            "CLOVER_DASHBOARD_OAUTH_CLIENT_ID is not set (and "
            "dashboard.oauth.client_id in config.yaml is empty). The "
            "Clover Portal provisions this env var (shape "
            "'agent:{instance_id}') when it deploys a Clover Cognition "
        ...
        logger.debug("dashboard-auth-clover: %s", LAST_SKIP_REASON)
        return

    if not client_id.startswith("agent:"):
        LAST_SKIP_REASON = (
            f"CLOVER_DASHBOARD_OAUTH_CLIENT_ID={client_id!r} doesn't match "
            f"the contract shape 'agent:{{instance_id}}'. The Clover Portal "
            f"provisions this value at deploy time; check your Fly app's "
            f"secrets or override with the value from the Portal admin UI."
        )
        logger.warning("dashboard-auth-clover: %s", LAST_SKIP_REASON)
        return

    try:
        provider = CloverDashboardAuthProvider(
            client_id=client_id, portal_url=portal_url
        )
    except ValueError as exc:
        LAST_SKIP_REASON = f"CloverDashboardAuthProvider construction failed: {exc}"
        logger.warning("dashboard-auth-clover: %s", LAST_SKIP_REASON)
        return

    ctx.register_dashboard_auth_provider(provider)
    logger.info(
        "dashboard-auth-clover: registered provider (client_id=%s, portal=%s)",
        client_id,
        portal_url,
    )
