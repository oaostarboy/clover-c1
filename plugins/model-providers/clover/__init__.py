"""Clover Portal provider profile."""

from typing import Any

from agent.portal_tags import get_conversation_context, clover_portal_tags
from agent.transports.codex import _cache_scope_from_session_id
from providers import register_provider
from providers.base import ProviderProfile


class CloverProfile(ProviderProfile):
    """Clover Portal — product tags, reasoning with Clover-specific omission."""

    def resolve_aux_model(self, *, vision: bool = False) -> str:
        """Ask the Portal which cheap model it currently recommends.

        ``/api/clover/recommended-models`` is the authoritative, tier-aware
        source (free vs paid), so the auxiliary fast tier tracks the live
        catalog instead of a hardcoded id that 404s the day Clover retires it.
        ...
        try:
            ...
            return get_clover_recommended_aux_model(vision=vision) or ""
        except Exception:
            return ""
    ...
    def build_extra_body(
        ...
        body: dict[str, Any] = {"tags": clover_portal_tags(session_id=session_id)}
        ...
        #
        # Resolve it exactly like ``clover_portal_tags`` resolves the
    ...
    def _cannot_disable_reasoning(model: str | None) -> bool:
        ...
        try:
            ...
            caps = clover_model_reasoning_capabilities(model)
            if caps is None:
                warm_clover_reasoning_caps_async()
                return True
        except Exception:
    ...
    def build_api_kwargs_extras(
        ...
        **context,
        ...
        """Clover: passes the full reasoning_config, disable included.
...
clover = CloverProfile(
    name="clover",
    aliases=("clover-portal", "cloverc1"),
    env_vars=("CLOVER_API_KEY",),
