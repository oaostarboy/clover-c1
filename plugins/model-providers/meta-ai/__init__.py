"""Meta Model API (Muse Spark) provider plugin for Clover Cognition.
...
Bundled from https://github.com/albertodepaola/clover-meta-provider. Clover'
...
* **Zero core edits.** Everything rides on ``ProviderProfile`` hooks. No changes
  to clover' ``model_metadata.py`` / ``models.py`` / ``run_agent.py`` are needed:
...
def _resolve_effort(reasoning_config: dict | None) -> str:
    """Map Clover' reasoning_config to a Meta-safe ``reasoning_effort`` value.
...
meta_ai = MetaAIProfile(
    ...
    # in clover_cli/providers.py and the retention hint in
