...
``<clover home>/plugins/<name>/``. That tree is the plugin *install* dir —
``clover plugins remove`` deletes it and ``clover plugins update`` git-pulls
...
``<clover home>/plugin-data/<name>/``, owned by the user, untouched by
...
through ``agent.secret_scope`` / ``.env`` like everywhere else in Clover.
...
# Mirrors the plugin-name shape `clover plugins install` accepts. Anything
...
def plugin_data_dir(name: str) -> Path:
    ...
    ``<clover home>/plugin-data/<name>/`` — survives plugin update and
    removal, and follows the active profile because it resolves through
    :func:`clover_constants.get_clover_home` on every call. Don't cache the
    result across profile switches.
    """
    from clover_constants import get_clover_home

    root = get_clover_home() / "plugin-data" / _validate_name(name)
