...
>
> **Share cards (2026-05-04, vendored in clover-c1 v0.4.0):** Unlocked achievement cards now have a "Share" button that renders a 1200×630 PNG share card (client-side canvas, no backend, no network) with Download + Copy-to-clipboard actions. Fits X/Twitter, Discord, LinkedIn, Bluesky link-preview dimensions.

## What it does

Clover Achievements scans local Clover sessions and unlocks badges based on real agent behavior:

- autonomous tool chains
- debugging and recovery patterns
- vibe-coding file edits
- Clover-native skills, memory, cron, and plugin usage
...
- **Secret** — hidden until Clover detects the first related signal
...
```bash
git clone https://github.com/PCinkusz/clover-achievements ~/.clover/plugins/clover-achievements
```

For local development, keep the repo elsewhere and symlink it:

```bash
git clone https://github.com/PCinkusz/clover-achievements ~/clover-achievements
...
If backend API routes 404, restart `clover dashboard`; plugin APIs are mounted at dashboard startup.
...
```bash
cd ~/.clover/plugins/clover-achievements
git pull --ff-only
curl http://127.0.0.1:9119/api/dashboard/plugins/rescan
```

If the update changes backend routes or `plugin_api.py`, restart `clover dashboard` after pulling.
...
Achievement unlock state is stored locally in `state.json` and is not overwritten by git updates. New achievements are evaluated from your existing Clover session history. Achievement IDs are stable and should not be renamed casually because they are the unlock-state keys.
...
```text
/api/plugins/clover-achievements/
```
