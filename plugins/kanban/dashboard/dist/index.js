/**
 * Clover Kanban — Dashboard Plugin
 *
 * Board view for the multi-agent collaboration board backed by
 * ~/.clover/kanban.db. Calls the plugin's backend at /api/plugins/kanban/
 * and tails task_events over a WebSocket for live updates.
 *
 * Plain IIFE, no build step. Uses window.__CLOVER_PLUGIN_SDK__ for React +
...
(function () {
  ...
  const SDK = window.__CLOVER_PLUGIN_SDK__;
  if (!SDK) return;
  ...
  const COLUMN_DOT = {
    triage: "clover-kanban-dot-triage",
    todo: "clover-kanban-dot-todo",
    ready: "clover-kanban-dot-ready",
    running: "clover-kanban-dot-running",
    blocked: "clover-kanban-dot-blocked",
    review: "clover-kanban-dot-review",
    done: "clover-kanban-dot-done",
    archived: "clover-kanban-dot-archived",
  };
  ...
  const MIME_TASK = "text/x-clover-task";
  ...
  const DOCS_URL = "https://clover-c1.local/docs/user-guide/features/kanban";
  const DOCS_TUTORIAL_URL = "https://clover-c1.local/docs/user-guide/features/kanban-tutorial";
  ...
  const LS_BOARD_KEY = "clover.kanban.selectedBoard";
  ...
  function renderMarkdown(src) {
    ...
    html = html.replace(/\u0000CODE(\d+)\u0000/g, (_m, i) =>
      `<pre class="clover-kanban-md-code"><code>${escapeHtml(blocks[Number(i)])}</code></pre>`,
    );
    return html;
  }
  ...
  function sanitizeMarkdownAttrs(tag, attrs) {
    ...
    if (tag === "pre" && /\sclass=(["'])clover-kanban-md-code\1/i.test(attrs)) {
      return ' class="clover-kanban-md-code"';
    }
    return "";
  }
  ...
  function MarkdownBlock(props) {
    ...
    if (!enabled) {
      return h("pre", { className: "clover-kanban-pre" }, props.source || "");
    }
    return h("div", {
      className: "clover-kanban-md",
  ...
  // standard `drop` event and our `clover-kanban:drop` event.
  // -------------------------------------------------------------------------
  ...
  function attachTouchDrag(el, taskId) {
    ...
    function onDown(e) {
      ...
      proxy.classList.add("clover-kanban-touch-proxy");
      ...
      function move(ev) {
        ...
        if (target !== lastTarget) {
          if (lastTarget) lastTarget.classList.remove("clover-kanban-column--drop");
          if (target) target.classList.add("clover-kanban-column--drop");
          lastTarget = target;
        }
      }
      function up() {
        ...
        if (lastTarget) {
          lastTarget.classList.remove("clover-kanban-column--drop");
          const status = lastTarget.getAttribute("data-kanban-column");
          const isTrash = lastTarget.hasAttribute("data-kanban-trash");
          if (isTrash) {
            lastTarget.dispatchEvent(new CustomEvent("clover-kanban:delete", {
              detail: { taskId },
              bubbles: true,
            }));
          } else if (status) {
            lastTarget.dispatchEvent(new CustomEvent("clover-kanban:drop", {
  ...
  function KanbanPage() {
    ...
    useEffect(function () {
      ...
      function openWs() {
        ...
        // loopback. Reading window.__CLOVER_SESSION_TOKEN__ directly (the old
   ...
   const deleteTask = useCallback(function (taskId) {
    ...
    return h(ErrorBoundary, null,
      h("div", { className: "clover-kanban flex flex-col gap-4" },
        h(BoardSwitcher, {
          board: board,
          boardList: boardList,
  ...
  function AttentionStrip(props) {
    ...
    return h("div", {
      className: cn(
        "clover-kanban-attention",
        "clover-kanban-attention--" + topSev,
      ),
    },
      h("div", { className: "clover-kanban-attention-bar" },
        h("span", { className: "clover-kanban-attention-icon" },
          topSev === "critical" ? "!!!" : topSev === "error" ? "!!" : "⚠"),
        h("span", { className: "clover-kanban-attention-text" },
          diagTasks.length === 1
      ...
      expanded
        ? h("div", { className: "clover-kanban-attention-list" },
            diagTasks.map(function (task) {
              ...
              return h("div", {
                key: task.id,
                className: cn(
                  "clover-kanban-attention-row",
                  "clover-kanban-attention-row--" + sev,
                ),
              },
                h("span", { className: "clover-kanban-attention-row-sev" },
                  sev === "critical" ? "!!!" : sev === "error" ? "!!" : "⚠"),
                h("span", { className: "clover-kanban-attention-row-id" }, task.id),
                h("span", { className: "clover-kanban-attention-row-title" },
                  task.title || tx(t, "untitled", "(untitled)")),
                h("span", { className: "clover-kanban-attention-row-meta" },
                  task.assignee ? "@" + task.assignee : tx(t, "unassigned", "unassigned"),
                  " \u00b7 ",
                  kinds.length > 0 ? kinds.join(", ") : tx(t, "diagnostic", "diagnostic"),
                ),
                h("button", {
                  className: "clover-kanban-attention-row-btn",
  ...
  function DiagnosticActionButton(props) {
    ...
    const cls = cn(
      "clover-kanban-diag-action-btn",
      action.suggested ? "clover-kanban-diag-action-btn--suggested" : "",
    );
    ...
    return h("span", { className: cls + " clover-kanban-diag-action-btn--unknown" },
      label);
  }
  ...
  function DiagnosticCard(props) {
    ...
    const execAction = function (action) {
      ...
      if (action.kind === "comment") {
        ...
        const ta = document.querySelector(".clover-kanban-drawer-comment-row input, .clover-kanban-drawer-comment-row textarea");
        if (ta) {
    ...
    const sevClass = "clover-kanban-diag--" + (diag.severity || "warning");
    return h("div", { className: cn("clover-kanban-diag", sevClass) },
      h("div", { className: "clover-kanban-diag-header" },
        h("span", { className: "clover-kanban-diag-sev" },
          diag.severity === "critical" ? "!!!" :
          diag.severity === "error" ? "!!" : "\u26a0"),
        h("span", { className: "clover-kanban-diag-title" },
          diag.title),
      ),
      h("div", { className: "clover-kanban-diag-detail" },
        diag.detail),
      diag.data && Object.keys(diag.data).length > 0
        ? h("div", { className: "clover-kanban-diag-data" },
            Object.keys(diag.data).map(function (k) {
              const v = diag.data[k];
              if (Array.isArray(v) && v.length > 0 && typeof v[0] === "string" &&
                  v[0].indexOf("t_") === 0) {
                // Task-id list — render as chips.
                return h("div", { key: k, className: "clover-kanban-diag-data-row" },
                  h("span", { className: "clover-kanban-diag-data-key" }, k + ":"),
                  v.map(function (x) {
                    return h("code", {
                      key: x, className: "clover-kanban-event-phantom-chip",
                    }, x);
                  }),
                );
              }
              return h("div", { key: k, className: "clover-kanban-diag-data-row" },
                h("span", { className: "clover-kanban-diag-data-key" }, k + ":"),
                h("span", { className: "clover-kanban-diag-data-val" },
      ...
      reassignAction
        ? h("div", { className: "clover-kanban-diag-reassign-row" },
            h("span", { className: "clover-kanban-diag-reassign-label" },
              tx(t, "reassignTo", "Reassign to:")),
            h("select", {
              className: "clover-kanban-recovery-select",
        ...
        : null,
      h("div", { className: "clover-kanban-diag-actions" },
      ...
      msg
        ? h("div", {
            className: cn(
              "clover-kanban-diag-msg",
              msg.ok ? "clover-kanban-diag-msg--ok" : "clover-kanban-diag-msg--err",
            ),
          }, msg.text)
        : null,
    );
  ...
  function DiagnosticsSection(props) {
    ...
    return h("div", { className: "clover-kanban-section" },
      h("div", { className: "clover-kanban-section-head-row" },
        h("span", { className: "clover-kanban-section-head" },
          hasOpenDiags
            ? h("span", { className: "clover-kanban-section-head-warning" },
                `\u26a0 ${tx(t, "diagnostics", "Diagnostics")} (${diags.length})`)
            : tx(t, "diagnostics", "Diagnostics"),
        ),
        h("button", {
          className: "clover-kanban-section-toggle",
          onClick: function () { setOpen(function (x) { return !x; }); },
          type: "button",
        }, open ? tx(t, "hide", "Hide") : tx(t, "show", "Show")),
      ),
      open
        ? h("div", { className: "clover-kanban-diag-list" },
  ...
  function DocsLink() {
    return h("a", {
      ...
      className: "clover-kanban-docs-link",
      title: "Open Clover Kanban docs in a new tab",
      "aria-label": "Clover Kanban documentation",
    }, "?");
  }
  ...
  function OrchestrationPanel() {
    ...
    // each triage card (or runs `clover kanban decompose <id>`) and tasks
    ...
    return h(Card, { className: "p-3" },
      h(CardContent, { className: "p-2 flex flex-col gap-3" },
        ...
        msg ? h("div", {
          className: msg.ok ? "clover-kanban-msg-ok" : "clover-kanban-msg-err",
        }, msg.text) : null,
  ...
  function BoardSwitcher(props) {
    ...
    if (!shouldShow) {
      return h("div", {
        className: "clover-kanban-boardswitcher-compact",
    ...
    return h("div", { className: "clover-kanban-boardswitcher" },
      h("div", { className: "clover-kanban-boardswitcher-inner" },
  ...
  function NewBoardDialog(props) {
    ...
    return h("div", {
      className: "clover-kanban-dialog-backdrop",
      onClick: function (e) { if (e.target === e.currentTarget) props.onCancel(); },
    },
      h("form", {
        className: "clover-kanban-dialog",
        onSubmit: onSubmit,
      },
        h("div", { className: "clover-kanban-dialog-title" },
        ...
        h("div", { className: "clover-kanban-dialog-actions" },
          h(Button, {
            type: "button",
  ...
  function BoardToolbar(props) {
    ...
    return h("div", { className: "flex flex-wrap items-end gap-3" },
      ...
      h("div", { className: "flex flex-col gap-1",
                 title: "Filter by assigned Clover profile. Profiles are the named agent identities that claim and work on tasks." },
  ...
  function BulkActionBar(props) {
    ...
    return h("div", { className: "clover-kanban-bulk" },
      h("span", { className: "clover-kanban-bulk-count" },
      ...
      h("div", { className: "clover-kanban-bulk-priority",
      ...
      h("div", { className: "clover-kanban-bulk-reassign",
                 title: "Reassign selected tasks to a different Clover profile. Pick a profile (or unassign) and click Apply." },
        h(Select, Object.assign({
          value: assignee,
          className: "h-7 text-xs",
      ...
      h("label", { className: "clover-kanban-bulk-reclaim-first", title: "Reclaim any active claims before reassigning" },
        h(Checkbox, {
  ...
  function TrashDropZone(props) {
    ...
    useEffect(function () {
      ...
      el.addEventListener("clover-kanban:delete", onTouchDelete);
      return function () { el.removeEventListener("clover-kanban:delete", onTouchDelete); };
    }, [props.onDelete]);
    ...
    return h("div", {
      ...
      className: cn(
        "clover-kanban-trash",
        dragOver ? "clover-kanban-trash--drop" : "",
        props.draggingTaskId ? "clover-kanban-trash--active" : "",
      ),
      onDragOver: handleDragOver,
      onDragLeave: handleDragLeave,
      onDrop: handleDrop,
    },
      h("span", { className: "clover-kanban-trash-icon" }, "🗑️"),
      h("span", { className: "clover-kanban-trash-label" },
  ...
  function BoardColumns(props) {
    ...
    const isPanBlockedTarget = useCallback(function (target) {
      ...
      if (target.closest && target.closest(".clover-kanban-card")) return true;
      if (target.closest && target.closest(".clover-kanban-column-add")) return true;
      if (target.closest && target.closest(".clover-kanban-col-check")) return true;
    ...
    const stopPan = useCallback(function () {
      ...
      if (el) {
        ...
        el.classList.remove("clover-kanban-columns--panning");
        el.style.userSelect = "";
      }
    ...
    const handleMouseDown = useCallback(function (e) {
      ...
      setIsPanning(true);
      el.classList.add("clover-kanban-columns--panning");
    ...
    const handleDragStart = useCallback(function (e) {
      const card = e.target.closest && e.target.closest(".clover-kanban-card");
      if (!card) return;
    ...
    return h("div", {
      ref: columnsRef,
      className: cn(
        "clover-kanban-columns",
        isScrollable ? "clover-kanban-columns--scrollable" : "",
        isPanning ? "clover-kanban-columns--panning" : "",
      ),
      onDragStart: handleDragStart,
      onDragEnd: handleDragEnd,
  ...
  function Column(props) {
    ...
    useEffect(function () {
      ...
      el.addEventListener("clover-kanban:drop", onTouchDrop);
      return function () { el.removeEventListener("clover-kanban:drop", onTouchDrop); };
    ...
    return h("div", {
      ...
      className: cn(
        "clover-kanban-column",
        dragOver ? "clover-kanban-column--drop" : "",
      ),
      onDragOver: handleDragOver,
      onDragLeave: handleDragLeave,
      onDrop: handleDrop,
    },
      h("div", { className: "clover-kanban-column-header",
                 title: colHelp || "" },
        h(Checkbox, {
          className: "clover-kanban-col-check",
        ...
        h("span", { className: cn("clover-kanban-dot", COLUMN_DOT[props.column.name]) }),
        h("span", { className: "clover-kanban-column-label" },
          colLabel || props.column.name),
        h("span", { className: "clover-kanban-column-count",
                    title: `${props.column.tasks.length} task${props.column.tasks.length === 1 ? "" : "s"} in this column` },
          props.column.tasks.length),
        h("button", {
          type: "button",
          className: "clover-kanban-column-add",
          title: tx(t, "createTask", "Create task in this column"),
          onClick: function () { setShowCreate(function (v) { return !v; }); },
        }, showCreate ? "×" : "+"),
      ),
      h("div", { className: "clover-kanban-column-sub" },
        colHelp || ""),
      ...
      h("div", { className: "clover-kanban-column-body" },
        props.column.tasks.length === 0
          ? h("div", { className: "clover-kanban-empty" }, tx(t, "noTasks", "— no tasks —"))
          : lanes
            ? lanes.map(function (lane) {
                return h("div", { key: lane.assignee, className: "clover-kanban-lane" },
                  h("div", { className: "clover-kanban-lane-head" },
                    h("span", { className: "clover-kanban-lane-name" }, lane.assignee),
                    h("span", { className: "clover-kanban-lane-count" }, lane.tasks.length),
                  ),
  ...
  function stalenessClass(task) {
    ...
    if (age >= tier.red)   return "clover-kanban-card--stale-red";
    if (age >= tier.amber) return "clover-kanban-card--stale-amber";
    return "";
  }
  ...
  function TaskCard(props) {
    ...
    const handleDragStart = function (e) {
      ...
      const selectedCards = document.querySelectorAll(".clover-kanban-card--selected");
      if (selectedCards.length > 1 && props.selected) {
        const ghost = document.createElement("div");
        ghost.className = "clover-kanban-drag-ghost";
    ...
    return h("div", {
      ...
      className: cn(
        "clover-kanban-card",
        props.selected ? "clover-kanban-card--selected" : "",
        props.failed ? "clover-kanban-card--failed" : "",
        props.draggingSource ? "clover-kanban-card--dragging-source" : "",
        stalenessClass(t),
      ),
      draggable: true,
      tabIndex: 0,
      ...
      h(Card, null,
        h(CardContent, { className: "clover-kanban-card-content" },
          h("div", { className: "clover-kanban-card-row" },
            h("label", {
              className: "clover-kanban-card-check-wrap",
              title: tx(i18n, "selectForBulk", "Select for bulk actions"),
              onClick: function (e) { e.stopPropagation(); },
            },
              h(Checkbox, {
                className: "clover-kanban-card-check",
            ...
            h("span", { className: "clover-kanban-card-id",
                        title: `Task id: ${t.id}. Use this id with kanban_show, /kanban show, or clover kanban show.` }, t.id),
            t.warnings && t.warnings.count > 0
              ? h("span", {
                  className: cn(
                    "clover-kanban-warning-badge",
                    "clover-kanban-warning-badge--" + (t.warnings.highest_severity || "warning"),
                  ),
                  title: (
            ...
            needsAssignee
              ? h(Badge, {
                  variant: "outline",
                  className: "clover-kanban-needs-assignee",
          ...
          h("div", { className: "clover-kanban-card-title" },
            t.title || tx(i18n, "untitled", "(untitled)")),
          h("div", { className: "clover-kanban-card-row clover-kanban-card-meta" },
            t.assignee
              ? h("span", { className: "clover-kanban-assignee",
                            title: `Assigned to Clover profile @${t.assignee}` }, "@", t.assignee)
              : h("span", { className: "clover-kanban-unassigned",
                            title: needsAssignee
            ...
            t.comment_count > 0
              ? h("span", { className: "clover-kanban-count",
            ...
            t.link_counts && (t.link_counts.parents + t.link_counts.children) > 0
              ...
              : null,
            h("span", { className: "clover-kanban-ago",
  ...
  // centered modal reusing the clover-kanban-dialog chrome so the form is
  ...
  function InlineCreate(props) {
    ...
    return h("div", {
      ...
      h("form", {
        className: "clover-kanban-dialog clover-kanban-create-dialog",
            ...
            h("div", { className: "flex flex-col gap-1 flex-1" },
              ...
              h(Input, {
                ...
                title: props.columnName === "triage"
                  ? "Clover profile that will spec this task (default: the dispatcher's configured specifier). Leave blank to let the dispatcher pick."
                  : "Clover profile to assign. Leave blank and the dispatcher will pick from available profiles when the task is Ready.",
  ...
  function TaskDrawer(props) {
    ...
    return h("div", { className: "clover-kanban-drawer-shade", onClick: props.onClose },
      h("div", {
        className: "clover-kanban-drawer",
        onClick: function (e) { e.stopPropagation(); },
      },
        h("div", { className: "clover-kanban-drawer-head" },
          h("span", { className: "text-xs text-muted-foreground" }, props.taskId),
          h("button", {
            type: "button",
            onClick: props.onClose,
            className: "clover-kanban-drawer-close",
        ...
        data ? h("div", { className: "clover-kanban-drawer-comment-foot" },
          h("div", {
            className: "clover-kanban-comment-hint text-xs text-muted-foreground",
          ...
          h("div", { className: "clover-kanban-drawer-comment-row" },
            h(Input, {
              value: newComment,
  ...
  function AttachmentsSection(props) {
    ...
    return h("div", { className: "clover-kanban-section" },
      h("div", { className: "clover-kanban-section-head" },
      ...
      atts.length === 0
        ...
        : atts.map(function (a) {
            return h("div", {
              ...
              h("button", {
                type: "button",
                className: "clover-kanban-attachment-link truncate",
                title: a.filename,
  ...
  function TaskDetail(props) {
    ...
    return h("div", { className: "clover-kanban-drawer-body" },
      h("div", { className: "clover-kanban-drawer-title" },
        h("span", { className: cn("clover-kanban-dot", COLUMN_DOT[t.status]) }),
        props.editing
          ? h(TitleEditor, {
              initial: t.title || "",
          ...
          : h("span", {
              className: "clover-kanban-drawer-title-text",
              title: tx(i18n, "clickToEdit", "Click to edit"),
              onClick: function () { props.setEditing(true); },
            }, t.title || tx(i18n, "untitled", "(untitled)")),
      ),
      h("div", { className: "clover-kanban-drawer-meta" },
      ...
      (function () {
        ...
        if (finalResult) {
          ...
          return h("div", { className: "clover-kanban-section" },
            h("div", { className: "clover-kanban-section-head" }, label),
            h(MarkdownBlock, { source: finalResult, enabled: props.renderMarkdown }),
          );
        }
        if (isDone && isParent) {
          return h("div", { className: "clover-kanban-section" },
            h("div", { className: "clover-kanban-section-head" }, tx(i18n, "result", "Result")),
            h("div", { className: "clover-kanban-done-no-result clover-kanban-done-parent-note" },
        ...
        if (isDone) {
          return h("div", { className: "clover-kanban-section" },
            h("div", { className: "clover-kanban-section-head" }, tx(i18n, "result", "Result")),
            h("div", { className: "clover-kanban-done-no-result" },
      ...
      childResults.length > 0 ? h("div", { className: "clover-kanban-section" },
        h("div", { className: "clover-kanban-section-head" },
        ...
        childResults.map(function (child) {
          ...
          return h("div", { key: child.id, className: "clover-kanban-comment" },
            h("div", { className: "clover-kanban-comment-head" },
              h("span", { className: "clover-kanban-comment-author" },
                `${child.id} · ${child.title || tx(i18n, "untitled", "(untitled)")}`),
              h(Badge, { variant: "outline" }, child.status),
              h("button", {
                type: "button",
                className: "clover-kanban-diag-action-btn",
      ...
      h("div", { className: "clover-kanban-section" },
        h("div", { className: "clover-kanban-section-head" },
        ...
        comments.length === 0
          ...
          : comments.map(function (c) {
              return h("div", { key: c.id, className: "clover-kanban-comment" },
                h("div", { className: "clover-kanban-comment-head" },
                  h("span", { className: "clover-kanban-comment-author" }, c.author || "anon"),
                  h("span", { className: "clover-kanban-comment-ago" },
      ...
      h("div", { className: "clover-kanban-section" },
        ...
        events.slice().reverse().slice(0, 20).map(function (e) {
          ...
          return h("div", {
            ...
            className: cn(
              "clover-kanban-event",
              isDiag ? "clover-kanban-event--hallucination" : "",
            ),
          },
            isDiag
              ? h("div", { className: "clover-kanban-event-header" },
                  h("span", { className: "clover-kanban-event-warning-icon" }, "⚠"),
                  h("span", { className: "clover-kanban-event-warning-label" },
                    getDiagnosticEventLabel(i18n, e.kind) || e.kind),
                  h("span", { className: "clover-kanban-event-ago" },
                    timeAgo ? timeAgo(e.created_at) : ""),
                )
              : h("div", { className: "clover-kanban-event-header-plain" },
                  h("span", { className: "clover-kanban-event-kind" }, e.kind),
                  h("span", { className: "clover-kanban-event-ago" },
                    timeAgo ? timeAgo(e.created_at) : ""),
                ),
            isDiag && phantoms.length > 0
              ? h("div", { className: "clover-kanban-event-phantom-row" },
                  h("span", { className: "clover-kanban-event-phantom-label" },
                    tx(i18n, "phantomIds", "Phantom ids:")),
                  phantoms.map(function (pid) {
                    return h("code", {
                      key: pid,
                      className: "clover-kanban-event-phantom-chip",
                    }, pid);
                  }),
                )
              : null,
            e.payload && !isDiag
              ? h("code", { className: "clover-kanban-event-payload" },
    ...
    return h("div", { className: "clover-kanban-section" },
      h("div", { className: "clover-kanban-section-head-row" },
        ...
        !showAll
          ? h("button", {
              type: "button",
              onClick: function () { setExpanded(true); },
              className: "clover-kanban-edit-link",
      ...
      visible.map(function (r) {
        const outcomeClass = r.ended_at
          ? `clover-kanban-run--${r.outcome || r.status || "ended"}`
          : "clover-kanban-run--active";
        return h("div", { key: r.id, className: cn("clover-kanban-run", outcomeClass) },
          h("div", { className: "clover-kanban-run-head" },
            h("span", { className: "clover-kanban-run-outcome" },
              r.ended_at ? (r.outcome || r.status || tx(t, "ended", "ended")) : tx(t, "active", "active")),
            h("span", { className: "clover-kanban-run-profile" },
              r.profile ? `@${r.profile}` : tx(t, "noProfile", "(no profile)")),
            h("span", { className: "clover-kanban-run-elapsed" }, fmtElapsed(r)),
            h("span", { className: "clover-kanban-run-ago" },
              timeAgo ? timeAgo(r.started_at) : ""),
          ),
          r.summary
            ? h("div", { className: "clover-kanban-run-summary" }, r.summary)
            : null,
          r.error
            ? h("div", { className: "clover-kanban-run-error" }, r.error)
            : null,
          (r.metadata && Object.keys(r.metadata).length > 0)
            ? (function () {
                ...
                return h("details", {
                    className: "clover-kanban-run-meta-block",
                    open: !collapsed,
                  },
                  h("summary", { className: "clover-kanban-run-meta-label" }, "Metadata"),
                  h("code", { className: "clover-kanban-run-meta" }, json),
                );
              })()
            : null,
        );
  ...
  function WorkerLogSection(props) {
    ...
    if (state.loading) {
      ...
      body = h("pre", { className: "clover-kanban-pre clover-kanban-log" },
        data.content || "(empty)");
    }
  ...
  function MetaRow(props) {
    return h("div", { className: "clover-kanban-meta-row" },
      h("span", { className: "clover-kanban-meta-label" }, props.label),
      h("span", { className: "clover-kanban-meta-value" }, props.value),
    );
  }
  ...
  function TitleEditor(props) {
    ...
    return h("div", { className: "clover-kanban-edit-row" },
      h(Input, {
  ...
  function AssigneeEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "clover-kanban-meta-row" },
        h("span", { className: "clover-kanban-meta-label" }, tx(t, "assignee", "Assignee")),
        h("span", {
          className: "clover-kanban-meta-value clover-kanban-editable",
  ...
  function PriorityEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "clover-kanban-meta-row" },
        h("span", { className: "clover-kanban-meta-label" }, tx(t, "priority", "Priority")),
        h("span", {
          className: "clover-kanban-meta-value clover-kanban-editable",
          onClick: function () { setEditing(true); },
  ...
  function ModelEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "clover-kanban-meta-row" },
        h("span", { className: "clover-kanban-meta-label" }, tx(t, "model", "Model")),
        h("span", {
          className: cn(
            "clover-kanban-meta-value clover-kanban-editable",
    ...
    return h("div", { className: "clover-kanban-meta-row" },
      h("span", { className: "clover-kanban-meta-label" }, tx(t, "model", "Model")),
      loading
        ? h("span", { className: "clover-kanban-meta-value text-muted-foreground" },
  ...
  function BodyEditor(props) {
    ...
    return h("div", { className: "clover-kanban-section" },
      h("div", { className: "clover-kanban-section-head-row" },
        h("span", { className: "clover-kanban-section-head" }, tx(t, "description", "Description")),
        editing
      ...
      editing
        ? h("textarea", {
            className: "clover-kanban-textarea",
            value: v,
            rows: 8,
            onChange: function (e) { setV(e.target.value); },
          })
  ...
  function DependencyEditor(props) {
    ...
    return h("div", { className: "clover-kanban-section" },
      h("div", { className: "clover-kanban-section-head" }, tx(t, "dependencies", "Dependencies")),
      h("div", { className: "clover-kanban-deps-row" },
        h("span", { className: "clover-kanban-deps-label" }, tx(t, "parents", "Parents:")),
        h("div", { className: "clover-kanban-deps-chips" },
          (links.parents || []).length === 0
            ? h("span", { className: "clover-kanban-deps-empty" }, tx(t, "none", "none"))
            : (links.parents || []).map(function (id) {
                return h("span", { key: id, className: "clover-kanban-dep-chip" },
                  id,
                  h("button", {
                    type: "button",
                    className: "clover-kanban-dep-chip-x",
      ...
      h("div", { className: "clover-kanban-deps-row" },
        h("span", { className: "clover-kanban-deps-label" }, tx(t, "children", "Children:")),
        h("div", { className: "clover-kanban-deps-chips" },
  ...
  function StatusActions(props) {
    ...
    return h("div", null,
      h("div", { className: "clover-kanban-actions" },
        specifyButton,
        decomposeButton,
      ...
      specifyMsg ? h("div", {
        className: specifyMsg.ok
          ? "clover-kanban-msg-ok"
          : "clover-kanban-msg-err",
  ...
  function HomeSubsSection(props) {
    ...
    return h("div", { className: "clover-kanban-section" },
      ...
      h("div", { className: "clover-kanban-home-subs" },
        channels.map(function (hc) {
          ...
          return h(Button, {
            ...
            className: hc.subscribed
              ? "clover-kanban-home-sub clover-kanban-home-sub--on"
              : "clover-kanban-home-sub",
          }, label);
        })
      )
    );
  ...
  if (window.__CLOVER_PLUGINS__ && typeof window.__CLOVER_PLUGINS__.register === "function") {
    window.__CLOVER_PLUGINS__.register("kanban", KanbanPage);
  }
})();
