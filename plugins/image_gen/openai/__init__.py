...
three virtual model IDs so the ``clover tools`` model picker and the
    ...
    def generate(
        ...
        **kwargs: Any,
        ...
        if not api_key:
            return error_response(
                error=(
                    "OPENAI_API_KEY not set. Run `clover tools` → Image "
                    "Generation → OpenAI to configure, or `clover setup` "
                    "to add the key."
                ),
