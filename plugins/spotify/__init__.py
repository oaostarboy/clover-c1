...
``_check_spotify_available()`` — when the user has not run ``clover auth
spotify``, the tools remain registered (so they appear in ``clover
...
The Spotify auth flow (``clover auth spotify``), CLI plumbing, and docs
