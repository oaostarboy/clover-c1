...
Let the clover agent join a Google Meet call, transcribe it, optionally speak
...
```
┌─ gateway (Linux box, where clover runs) ────────────────────────────┐
│                                                                      │
...
│                                                                      │
│   NodeServer (from `clover meet node run`)                           │
│     │                                                                │
...
| `__init__.py` | `register(ctx)` — registers 5 tools + `on_session_end` hook + `clover meet` CLI |
...
| `cli.py` | `clover meet setup / auth / join / status / transcript / say / stop / node ...` |
...
| `node/registry.py` | v3: `$CLOVER_HOME/workspace/meetings/nodes.json` |
| `node/server.py` | v3: `NodeServer` (runs on host machine) |
| `node/client.py` | v3: `NodeClient` (used by tool handlers + CLI on gateway) |
| `node/cli.py` | v3: `clover meet node {run,list,approve,remove,status,ping}` |
...
clover meet install                                      # pip + Chromium
clover meet setup                                        # preflight
clover meet auth                                         # optional
clover meet join https://meet.google.com/abc-defg-hij    # transcribe
```
...
```bash
clover meet install --realtime                     # installs pulseaudio-utils
echo 'OPENAI_API_KEY=sk-...' >> ~/.clover/.env
clover meet join https://meet.google.com/abc-defg-hij --mode realtime
# then from the agent or CLI:
clover meet say "Good morning everyone, I'm the note-taker bot."
```

macOS:
```bash
clover meet install --realtime     # runs: brew install blackhole-2ch ffmpeg
# then — manually! — open System Settings → Sound → Input → BlackHole 2ch
echo 'OPENAI_API_KEY=sk-...' >> ~/.clover/.env
clover meet join https://meet.google.com/abc-defg-hij --mode realtime
```

On macOS, clover will **not** switch your system audio input automatically — the
...
clover plugins enable google_meet
clover meet node run --display-name my-mac --host 0.0.0.0 --port 18789
