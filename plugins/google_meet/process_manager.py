...
session-scoped state file under ``$CLOVER_HOME/workspace/meetings/.active.json``
...
# File + directory layout (under $CLOVER_HOME):
#
#   workspace/meetings/
...
def _root() -> Path:
    return Path(get_clover_home()) / "workspace" / "meetings"
...
def start(
    ...
    *,
    ...
    guest_name: str = "Clover Cognition",
    ...
    If a bot is already running for this clover install, leave it first —
    ...
    env = os.environ.copy()
    env["CLOVER_MEET_URL"] = url
    env["CLOVER_MEET_OUT_DIR"] = str(out)
    env["CLOVER_MEET_GUEST_NAME"] = guest_name
    if headed:
        env["CLOVER_MEET_HEADED"] = "1"
    if auth_state:
        env["CLOVER_MEET_AUTH_STATE"] = auth_state
    if duration:
        env["CLOVER_MEET_DURATION"] = duration
    # v2: realtime mode + passthroughs. The bot defaults to transcribe
    # mode if CLOVER_MEET_MODE isn't set, matching v1 behavior.
    if mode:
        env["CLOVER_MEET_MODE"] = mode
    if realtime_model:
        env["CLOVER_MEET_REALTIME_MODEL"] = realtime_model
    if realtime_voice:
        env["CLOVER_MEET_REALTIME_VOICE"] = realtime_voice
    if realtime_instructions:
        env["CLOVER_MEET_REALTIME_INSTRUCTIONS"] = realtime_instructions
    ...
    # meet_bot checks CLOVER_MEET_REALTIME_KEY before OPENAI_API_KEY.
    if not realtime_api_key:
        try:
            from agent.secret_scope import get_secret

            realtime_api_key = (
                get_secret("CLOVER_MEET_REALTIME_KEY")
                or get_secret("OPENAI_API_KEY")
            )
        except ImportError:  # pragma: no cover — secret_scope is in-repo
            pass
    if realtime_api_key:
        env["CLOVER_MEET_REALTIME_KEY"] = realtime_api_key
