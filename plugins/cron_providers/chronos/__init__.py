...
Chronos (the Greek god of time, alongside Clover) is the first non-default
...
NAS's ``agent-cron`` endpoints with the agent's existing Clover token.
...
class ChronosCronScheduler(CronScheduler):
    ...
    def is_available(self) -> bool:
        ...
        callback URL (for NAS→agent fires), and a usable Clover token (the agent
