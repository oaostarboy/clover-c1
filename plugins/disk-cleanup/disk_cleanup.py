"""disk_cleanup — ephemeral file cleanup for Clover Cognition.
...
Rules:
  ...
  - empty dirs    → always delete (under CLOVER_HOME)
  - research      → keep 10 newest, prompt for older (deep only)
  - chrome-profile→ prompt after 14 days (deep only)
  - >500 MB files → prompt always (deep only)

Scope: strictly CLOVER_HOME and /tmp/clover-*
Never touches: ~/.clover/logs/ or any system directory.
"""
...
try:
    ...
    def get_clover_home() -> Path:  # type: ignore[no-redef]
        val = (os.environ.get("CLOVER_HOME") or "").strip()
        return Path(val).resolve() if val else (Path.home() / ".clover").resolve()
...
def get_state_dir() -> Path:
    """State dir — separate from ``$CLOVER_HOME/logs/``."""
    return get_clover_home() / "disk-cleanup"
...
def get_log_file() -> Path:
    """Audit log — intentionally NOT under ``$CLOVER_HOME/logs/``."""
...
def is_safe_path(path: Path) -> bool:
    """Accept only paths under CLOVER_HOME or ``/tmp/clover-*``.

    Rejects Windows mounts (``/mnt/c`` etc.) and any system directory.
    """
    clover_home = get_clover_home()
    try:
        path.resolve().relative_to(clover_home)
        return True
    except (ValueError, OSError):
        pass
    # Allow /tmp/clover-* explicitly
    parts = path.parts
    if len(parts) >= 3 and parts[1] == "tmp" and parts[2].startswith("clover-"):
        return True
    return False
...
_EMPTY_DIR_PROTECTED_TOP_LEVEL = frozenset({
    ...
    "clover-c1", "backups", "profiles", ".worktrees",
...
# Paths under $CLOVER_HOME that must NEVER be deleted by quick(),
...
def _is_protected_cron_path(p: Path) -> bool:
    ...
    history at once.
    """
    # Lazily build the set once per process so CLOVER_HOME is resolved
    # exactly once.
    if not _PROTECTED_CRON_PATHS:
        clover_home = get_clover_home()
        for parent in ("cron", "cronjobs"):
            base = clover_home / parent
...
def track(path_str: str, category: str, silent: bool = False) -> bool:
    ...
    if not is_safe_path(path):
        _log(f"REJECT: {path} (outside CLOVER_HOME)")
        return False
...
def quick() -> Dict[str, Any]:
    ...
    # Remove empty dirs under CLOVER_HOME, but never recurse into known
    # durable state trees.  Some installs place the Clover checkout, venv,
    # and desktop build under CLOVER_HOME; a full rglob over that tree can
    # stall the gateway event loop for minutes.
    clover_home = get_clover_home()
    empty_removed = 0
    sweep_stack: List[Tuple[Path, bool]] = []
    try:
        for top in clover_home.iterdir():
            if (
                top.is_dir()
...
def guess_category(path: Path) -> Optional[str]:
    ...
    clover_home = get_clover_home()
    try:
        rel = path.resolve().relative_to(clover_home)
        ...
        # Path isn't under CLOVER_HOME (e.g. /tmp/clover-*) — fall through.
        pass
