...
Auto-tracks and cleans up ephemeral files created during Clover Cognition
sessions — test scripts, temp outputs, cron logs, stale chrome profiles.
Scoped strictly to `$CLOVER_HOME` and `/tmp/clover-*`.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `CLOVER_HOME`, track it silently as `test` / `temp` / `cron-output`. |
...
- `is_safe_path()` rejects anything outside `CLOVER_HOME` or `/tmp/clover-*`
- Windows mounts (`/mnt/c` etc.) are rejected
- The state directory `$CLOVER_HOME/disk-cleanup/` is itself excluded
- `$CLOVER_HOME/logs/`, `memories/`, `sessions/`, `skills/`, `plugins/`,
