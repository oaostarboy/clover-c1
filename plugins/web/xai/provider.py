...
``{title, url, description, position}`` rows every other Clover web
provider produces.
...
prefers Clover-managed xAI Grok OAuth (via ``clover auth``) and falls back
to ``XAI_API_KEY`` (resolved through ``~/.clover/.env``, then
``os.environ``).
"""
...
class XAIWebSearchProvider(WebSearchProvider):
    ...
    def is_available(self) -> bool:
        ...
        every ``clover tools`` repaint and at tool-registration time.
    ...
    def search(self, query: str, limit: int = 5) -> Dict[str, Any]:
        ...
        if not api_key:
            return {
                "success": False,
                "error": (
                    "No xAI credentials found. Run `clover auth` to sign in with "
