...
_API_KEY_URL = "http://app.supermemory.ai/integrations?connect=clover"
...
def _load_supermemory_config(clover_home: str) -> dict:
    config = _default_config()
    config_path = Path(clover_home) / "supermemory.json"
...
def _save_supermemory_config(values: dict, clover_home: str) -> None:
    config_path = Path(clover_home) / "supermemory.json"
    existing = {}
    if config_path.exists():
        try:
...
class _SupermemoryClient:
    def __init__(self, api_key: str, timeout: float, container_tag: str,
        ...
        self._client = Supermemory(
            ...
            timeout=timeout,
            max_retries=0,
            default_headers={"x-sm-source": "clover"},
        )

    def _merge_metadata(self, metadata: Optional[dict]) -> dict:
        # sm_source routes Clover writes into the "Clover" Space in the Supermemory
        ...
        merged = {"sm_source": "clover", **(metadata or {})}
...
def _resolve_container_tag_for_setup(clover_home: str, *, identity: str = "default") -> str:
    config = _load_supermemory_config(clover_home)
...
def _probe_supermemory_connection(api_key: str, clover_home: str, *, identity: str = "default") -> dict:
    config = _load_supermemory_config(clover_home)
    base_url = _resolve_base_url(config["base_url"])
    status = {
        "ok": False,
        "error": "",
        "container_tag": _resolve_container_tag_for_setup(clover_home, identity=identity),
        "profile_facts": 0,
...
class SupermemoryMemoryProvider(MemoryProvider):
    ...
    def get_config_schema(self):
        # Only prompt for the API key during `clover memory setup`.
        # All other options are documented for $CLOVER_HOME/supermemory.json
    ...
    def save_config(self, values, clover_home):
        ...
        _save_supermemory_config(sanitized, clover_home)
    ...
    def get_status_config(self, provider_config: dict) -> dict:
        ...
        status = _probe_supermemory_connection(api_key, clover_home)
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        from clover_constants import get_clover_home
        self._clover_home = kwargs.get("clover_home") or str(get_clover_home())
        self._session_id = session_id
        self._turn_count = 0
        self._config = _load_supermemory_config(self._clover_home)
