...
- Hosted: API key from [app.supermemory.ai/integrations?connect=clover](http://app.supermemory.ai/integrations?connect=clover)
...
Before running `clover memory setup`, add the local endpoint to
...
Then run `clover memory setup` and enter the local server's API key. Configuring
the endpoint first ensures the setup connection probe also stays local.

## Config

Config file: `$CLOVER_HOME/supermemory.json`
...
| `container_tag` | `clover` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags (e.g. `clover-{identity}` → `clover-coder`). |
...
`https://api.supermemory.ai`. Clover resolves it once and uses the same endpoint
...
All Supermemory API calls send `x-sm-source: clover`, and document writes stamp
`metadata.sm_source: clover`. This is a **functional routing key, not telemetry**:
it groups Clover-written memories into a dedicated "Clover" Space in the
...
Use `{identity}` in the `container_tag` to scope memories per Clover profile:

```json
{
  "container_tag": "clover-{identity}"
}
```

For a profile named `coder`, this resolves to `clover-coder`. The default profile resolves to `clover-default`. Without `{identity}`, all profiles share the same container.
