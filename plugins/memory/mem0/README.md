...
```bash
clover memory setup    # select "mem0"
```
...
Behavioral settings live in `$CLOVER_HOME/mem0.json` (set them via `clover memory setup`). Only the secret `MEM0_API_KEY` belongs in `~/.clover/.env`.
...
| `user_id` | `clover-user` | User identifier on Mem0 |
...
2. Point the plugin at it — via the setup wizard:
   ...
   ```
3. Start a fresh Clover session and call `mem0_search` — it connects to your server.
...
```bash
clover memory setup mem0 --mode oss \
...
```bash
clover memory setup mem0 --mode oss --oss-llm-key sk-...
```

Or edit `$CLOVER_HOME/mem0.json` directly:
```json
{
  "mode": "oss",
  "oss": {
    ...
    "vector_store": {"provider": "qdrant", "config": {"path": "~/.clover/mem0_qdrant"}}
  }
}
```
...
```bash
clover memory setup mem0 --mode platform --api-key sk-...
```
...
- Check `user_id` matches between sessions (`$CLOVER_HOME/mem0.json`).
