...
- OpenViking server running and reachable from Clover

OpenViking 0.2.10 or newer is recommended. For backward compatibility,
Clover can identify older servers that expose the legacy status-only health
...
connection values into Clover, or create a minimal `ovcli.conf` when one does
not exist.
...
default profile that is `~/.clover/.env`; for a named profile use
...
OpenViking's server config is separate from Clover:
...
Clover-side provider config is read from environment variables in the active
profile's `.env`:
...
| `OPENVIKING_AGENT` | `clover` | Clover peer ID in OpenViking, used for peer-scoped memories |

When `OPENVIKING_API_KEY` is set, Clover lets OpenViking derive account/user
identity from the key. In local or trusted deployments without an API key,
Clover sends `OPENVIKING_ACCOUNT` and `OPENVIKING_USER` as identity headers.
Clover also sends `User-Agent: openviking-memory-clover/<version>` on
OpenViking requests. This standard harness identifier contains the Clover
...
current user). Clover caches a confirmed user only for the active connection.
If the probe fails, Clover uses the configured user, or `default`, for that
...
Clover built-in `memory` tool additions are mirrored to OpenViking after the
local memory operation succeeds:

| Clover action | OpenViking operation |
|---------------|----------------------|
| `add` | `content/write` with `mode=create` under the configured peer memory namespace |

Built-in `replace` and `remove` operations are not mirrored because Clover
...
file URIs, such as
`viking://user/default/peers/clover/memories/preferences/mem_abc123.md` (any
