"""clover-memory-store — holographic memory plugin using MemoryProvider interface.
...
Config in $CLOVER_HOME/config.yaml (profile-scoped):
  plugins:
    clover-memory-store:
      db_path: $CLOVER_HOME/memory_store.db   # omit to use the default
      auto_extract: false
      default_trust: 0.5
...
def _load_plugin_config() -> dict:
    try:
        ...
        return cfg_get(all_config, "plugins", "clover-memory-store", default={}) or {}
    except Exception:
        return {}
...
class HolographicMemoryProvider(MemoryProvider):
    ...
    def save_config(self, values, clover_home):
        """Write config to config.yaml under plugins.clover-memory-store."""
        from pathlib import Path
        config_path = Path(clover_home) / "config.yaml"
        try:
            import yaml
            ...
            existing["plugins"]["clover-memory-store"] = values
    ...
    def get_config_schema(self):
        ...
        _default_db = f"{display_clover_home()}/memory_store.db"
        return [
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        _clover_home = str(get_clover_home())
        _default_db = _clover_home + "/memory_store.db"
        db_path = self._config.get("db_path", _default_db)
        # Expand $CLOVER_HOME in user-supplied paths so config values like
        # "$CLOVER_HOME/memory_store.db" or "~/.clover/memory_store.db" both
        # resolve to the active profile's directory.
        if isinstance(db_path, str):
            db_path = db_path.replace("$CLOVER_HOME", _clover_home)
            db_path = db_path.replace("${CLOVER_HOME}", _clover_home)
