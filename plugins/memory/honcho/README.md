...
> **Honcho docs:** <https://docs.honcho.dev/v3/guides/integrations/clover>
...
```bash
clover memory setup honcho   # configure Honcho directly (works on a fresh install)
clover memory setup          # generic picker, choose Honcho from the list
```
...
Or manually:
```bash
clover config set memory.provider honcho
echo "HONCHO_API_KEY=***" >> ~/.clover/.env
```

> `clover honcho setup` also works, but only **after** Honcho is the active
> memory provider — the `honcho` subcommand is registered for the active
> provider only. On a fresh install, use `clover memory setup honcho`.
...
cycle (not per pass). Select a fast, inexpensive model under `clover model`
...
| Priority | Path | Scope |
|----------|------|-------|
| 1 | `$CLOVER_HOME/honcho.json` | Profile-local (isolated Clover instances) |
| 2 | `~/.clover/honcho.json` | Default profile (shared host blocks) |
| 3 | `~/.honcho/config.json` | Global (cross-app interop) |

Host key is derived from the active Clover profile: `clover` (default) or `clover_<profile>`.
...
> **Deprecated:** `pinPeerName` is a legacy alias for `pinUserPeer`, still read for back-compat (`pinUserPeer` wins where both are set). `clover honcho setup` migrates it onto `pinUserPeer` on touch and never writes it.
...
| 4 | `per-session` strategy | Clover session ID (`20260415_a3f2b1`) |
| 5 | `per-repo` strategy | Git root directory name (`clover-c1`) |
| 6 | `per-directory` strategy | Current directory basename (`src`) |
| 7 | `global` strategy | Workspace name (`clover`) |

Gateway platforms always resolve via priority 3 (per-chat isolation) regardless of `sessionStrategy`. The strategy setting only affects CLI sessions.

If `sessionPeerPrefix` is `true`, the peer name is prepended: `alice-clover-c1`.

#### What each strategy produces

- **`per-directory`** — basename of `$PWD`. Opening clover in `~/code/myapp` and `~/code/other` gives two separate sessions. Same directory = same session across runs.
...
- **`per-session`** — Clover session ID (timestamp + hex). Every `clover` invocation starts a fresh Honcho session. Falls back to `per-directory` if no session ID is available.
- **`global`** — workspace name. One session for everything. Memory accumulates across all directories and runs.

### Multi-Profile Pattern

Multiple Clover profiles can share one workspace while maintaining separate AI identities. Config resolution is **host block > root > env var > default** — host blocks inherit from root, so shared settings only need to be declared once:
...
Both profiles see the same user (`yourname`) in the same shared environment (`clover`), but each AI peer builds its own observations, conclusions, and behavior patterns. The coder's memory stays code-oriented; the main agent's stays broad.

Host key is derived from the active Clover profile: `clover` (default) or `clover_<profile>` (e.g. `clover -p coder` -> host key `clover_coder`). Older `clover.<profile>` host blocks are still read for compatibility and are migrated when the CLI writes profile-scoped Honcho config.
...
| `CLOVER_HONCHO_HOST` | Host key override |
...
| `HONCHO_OAUTH_CLIENT_ID` | OAuth client (default `clover-c1`) |
...
|---------|-------------|
| `clover memory setup honcho` | Configure Honcho directly — works on a fresh install |
| `clover honcho setup` | Interactive setup wizard (only registered once Honcho is the active provider; redirects to `clover memory setup`) |
| `clover honcho status` | Show resolved config for active profile |
| `clover honcho enable` / `disable` | Toggle Honcho for active profile |
| `clover honcho mode <mode>` | Change recall or observation mode |
| `clover honcho peer --user <name>` | Update user peer name |
| `clover honcho peer --ai <name>` | Update AI peer name |
| `clover honcho tokens --context <N>` | Set context token budget |
| `clover honcho tokens --dialectic <N>` | Set dialectic max chars |
| `clover honcho map <name>` | Map current directory to a session name |
| `clover honcho sync` | Create host blocks for all Clover profiles |
