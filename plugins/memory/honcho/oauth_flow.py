...
``clover://`` handler. Endpoints are env-overridable with local-dev defaults
...
# The loopback redirect registered for the Clover OAuth client. IP-literal so
...
def begin_authorization(
    ...
    *,
    ...
    (``clover-desktop`` / ``clover-cli``) so the consent side can attribute
...
_CALLBACK_HTML = (
    ...
    b"<div>Connected to Honcho. You can close this tab and return to Clover.</div>"
)
...
def start_loopback_flow_background(
    *,
    ...
    # the request, where a context-local CLOVER_HOME override can't reach.
