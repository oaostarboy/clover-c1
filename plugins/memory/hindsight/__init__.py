...
Config via environment variables:
  ...
  HINDSIGHT_BANK_ID                — memory bank identifier (default: clover)
  ...
  HINDSIGHT_RETAIN_SOURCE          — metadata source value attached to retained memories (default: clover)
  HINDSIGHT_RETAIN_USER_PREFIX     — label used before user turns in retained transcripts
  HINDSIGHT_RETAIN_ASSISTANT_PREFIX — label used before assistant turns in retained transcripts

Or via $CLOVER_HOME/hindsight/config.json (profile-scoped), falling back to
...
# via the ``retain_source`` config key or HINDSIGHT_RETAIN_SOURCE (e.g. "clover").
...
def _check_local_runtime() -> tuple[bool, str | None]:
    ...
    so Clover can degrade gracefully instead of repeatedly trying to start
    ...
    healthy and ``clover memory status`` would stay green while the daemon
...
def _local_runtime_hint(reason: str | None) -> str:
    ...
    selected local_embedded without going through ``clover memory setup`` — a
    ...
    if "no module named" in text and ("hindsight'" in text or 'hindsight"' in text
        ...
        return (
            ...
            f"{sys.executable} hindsight-all — or run 'clover memory setup'. "
...
def _load_config() -> dict:
    ...
    Resolution order:
      1. $CLOVER_HOME/hindsight/config.json  (profile-scoped)
    ...
    profile_path = get_clover_home() / "hindsight" / "config.json"
    ...
    return {
        ...
        "banks": {
            "clover": {
                "bankId": os.environ.get("HINDSIGHT_BANK_ID", "clover"),
...
def _event_timestamp() -> str:
    """Return the configured Clover event time with an explicit UTC offset."""
    event_time = _clover_now()
    # clover_time.now() guarantees an aware datetime. Keep this fallback so a
...
def _embedded_profile_name(config: dict[str, Any]) -> str:
    """Return the Hindsight embedded profile name for this Clover config."""
    profile = config.get("profile", "clover")
    return str(profile or "clover")
...
def _load_simple_env(path) -> dict[str, str]:
    ...
    values: dict[str, str] = {}
    # utf-8-sig, not plain utf-8: this is also used on the Clover .env during
...
def _resolve_bank_id_template(template: str, fallback: str, **placeholders: str) -> str:
    ...
    Supported placeholders (each is sanitized before substitution):
      {profile}   — active Clover profile name (from agent_identity)
      {workspace} — Clover workspace name (from agent_workspace)
      {platform}  — "cli", "telegram", "discord", etc.
      {user}      — platform user id (gateway sessions)
      {session}   — current session id

    Missing/empty placeholders are rendered as the empty string and then
    collapsed — e.g. ``clover-{user}`` with no user becomes ``clover``.
...
class HindsightMemoryProvider(MemoryProvider):
    ...
    def __init__(self):
        ...
        self._retain_context = "conversation between Clover Cognition and the User"
    ...
    def save_config(self, values, clover_home):
        """Write config to $CLOVER_HOME/hindsight/config.json."""
        import json
        from pathlib import Path
        config_dir = Path(clover_home) / "hindsight"
    ...
    def post_setup(self, clover_home: str, config: dict) -> None:
        ...
        from clover_cli.memory_setup import _CANCELLED, _curses_select, _print_cancelled_setup
        ...
        provider_config.setdefault("bank_id", "clover")
        ...
        self.save_config(provider_config, clover_home)
        ...
        if mode == "local_embedded":
            ...
            config_path = Path(clover_home) / "hindsight" / "config.json"
            try:
            ...
            if not llm_api_key:
                llm_api_key = _load_simple_env(Path(clover_home) / ".env").get("HINDSIGHT_LLM_API_KEY", "")
            if not llm_api_key:
    ...
    def _offer_starter_template(self, mode: str, provider_config: dict, env_writes: dict) -> None:
        """Offer to seed the bank with a Clover starter template (best-effort)."""
        from clover_cli.memory_setup import _CANCELLED, _curses_select
        ...
        bank_id = provider_config.get("bank_id", "clover")
    ...
    def get_config_schema(self):
        return [
            ...
            {"key": "bank_id", "description": "Memory bank name (static fallback when bank_id_template is unset)", "default": "clover"},
            {"key": "bank_id_template", "description": "Optional template to derive bank_id dynamically. Placeholders: {profile}, {workspace}, {platform}, {user}, {session}. Example: clover-{profile}", "default": ""},
            ...
            {"key": "retain_context", "description": "Context label for retained memories", "default": "conversation between Clover Cognition and the User"},
        ...
        if self._client is None:
            if self._mode == "local_embedded":
                ...
                logger.debug("Creating HindsightEmbedded client (profile=%s, provider=%s)",
                             self._config.get("profile", "clover"), llm_provider)
                kwargs = dict(
                    profile=self._config.get("profile", "clover"),
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        try:
            ...
            if Version(installed) < Version(_MIN_CLIENT_VERSION):
                ...
                # durable data-volume target instead of /opt/clover (NS-605).
                from tools.lazy_deps import install_specs
        ...
        banks = cfg_get(self._config, "banks", "clover", default={})
        static_bank_id = self._config.get("bank_id") or banks.get("bankId", "clover")
        ...
        self._retain_context = self._config.get("retain_context", "conversation between Clover Cognition and the User")
        ...
        # Clover emits a "👁️ Hindsight — recalled N memories" status line so the
        ...
        if self._mode == "local_embedded":
            ...
            if hasattr(os, "geteuid") and os.geteuid() == 0:
                msg = (
                    ...
                    "memory daemon. Run Clover as a non-root user, or switch "
                    "to cloud / local_external mode via 'clover memory setup'."
                )
                logger.warning(msg)
                # Surface to the terminal too — a daemon that never starts
                # would otherwise fail silently and the user would only see
                # Clover get sluggish. (issue #13125)
                try:
    ...
    def prefetch(self, query: str, *, session_id: str = "") -> str:
        ...
        # turn's queued recall. See clover-c1#5820.
        ...
        if self._client is not None:
            try:
                if self._mode == "local_embedded":
                    ...
                    # When Clover created/used that client on the shared async loop,
