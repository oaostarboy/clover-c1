...
the ``clover`` integration, and applies a chosen manifest to the user's bank
...
def fetch_clover_templates(url: str | None = None) -> list[dict]:
    """Return catalog entries tagged for the ``clover`` integration."""
    catalog = _get_json(url or catalog_url())
    entries = catalog.get("templates", []) if isinstance(catalog, dict) else []
    return [e for e in entries if "clover" in (e.get("integrations") or [])]
...
def run_template_step(
    *,
    ...
    """
    try:
        entries = fetch_clover_templates()
