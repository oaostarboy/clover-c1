...
> **توجہ فرمائیں:** مقامی ونڈوز (Native Windows) پر کلوور بغیر WSL کے چلتا ہے — CLI، گیٹ وے، TUI، اور ٹولز سب مقامی طور پر کام کرتے ہیں۔ اگر آپ WSL2 استعمال کرنا پسند کرتے ہیں، تو اوپر دی گئی لینکس/میک او ایس کی کمانڈ وہاں بھی کام کرے گی۔ کوئی مسئلہ نظر آیا؟ براہ کرم [مسائل (issues) درج کریں](https://github.com/oaostarboy/clover-c1)۔
...
انسٹالر سب کچھ خود سنبھالتا ہے: uv، Python 3.11، Node.js، ripgrep، ffmpeg، **اور ایک پورٹ ایبل (portable) گٹ بیش (Git Bash)** (یعنی MinGit، جو `%LOCALAPPDATA%\clover\git` میں ان پیک ہوتا ہے — اس کے لیے ایڈمن کی اجازت درکار نہیں، اور یہ سسٹم کے کسی بھی گٹ انسٹال سے بالکل الگ ہے)۔ کلوور اس بنڈل شدہ گٹ بیش کو شیل کمانڈز چلانے کے لیے استعمال کرتا ہے۔
...
> **اینڈرائیڈ (Android) / ٹرمکس (Termux):** ٹیسٹ کیا گیا مینوئل طریقہ [Termux گائیڈ](docs/getting-started/termux) میں موجود ہے۔ ٹرمکس پر کلوور ایک مخصوص `.[termux]` ایکسٹرا انسٹال کرتا ہے کیونکہ مکمل `.[all]` ایکسٹرا میں ایسی وائس ڈیپینڈینسیز شامل ہیں جو اینڈرائیڈ کے ساتھ مطابقت نہیں رکھتیں۔
>
> **ونڈوز (Windows):** مقامی ونڈوز کی مکمل سپورٹ موجود ہے — اوپر دی گئی پاور شیل کی کمانڈ سب کچھ انسٹال کر دیتی ہے۔ اگر آپ WSL2 استعمال کرنا چاہتے ہیں، تو لینکس کی کمانڈ وہاں کام کرتی ہے۔ مقامی ونڈوز میں انسٹالیشن `%LOCALAPPDATA%\clover` میں ہوتی ہے؛ جبکہ WSL2 میں لینکس کی طرح `~/.clover` میں ہوتی ہے۔ کلوور کا وہ واحد فیچر جسے فی الحال خاص طور پر WSL2 کی ضرورت ہے وہ براؤزر پر مبنی ڈیش بورڈ چیٹ پین ہے (یہ POSIX PTY استعمال کرتا ہے — کلاسک CLI اور گیٹ وے دونوں مقامی طور پر چلتے ہیں)۔
...
```bash
clover              # انٹرایکٹو CLI — بات چیت شروع کریں
clover model        # اپنا LLM پرووائیڈر اور ماڈل منتخب کریں
clover tools        # کنفیگر کریں کہ کون سے ٹولز ایکٹو ہیں
clover config set   # انفرادی کنفگ (config) ویلیوز سیٹ کریں
clover gateway      # میسجنگ گیٹ وے شروع کریں (ٹیلی گرام، ڈسکارڈ، وغیرہ)
clover setup        # مکمل سیٹ اپ وزرڈ چلائیں (یہ سب کچھ ایک ساتھ کنفیگر کر دے گا)
clover claw migrate # OpenClaw سے مائیگریٹ کریں (اگر آپ OpenClaw سے آ رہے ہیں)
clover update       # لیٹسٹ ورژن پر اپ ڈیٹ کریں
clover doctor       # کسی بھی مسئلے کی تشخیص کریں
```
...
کلوور آپ کے پسندیدہ پرووائیڈر کے ساتھ کام کرتا ہے — یہ چیز تبدیل نہیں ہو رہی۔ لیکن اگر آپ ماڈل، ویب سرچ، امیج جنریشن، TTS، اور کلاؤڈ براؤزر کے لیے پانچ الگ الگ API کیز جمع نہیں کرنا چاہتے، تو **[Clover Portal](https://github.com/oaostarboy/clover-c1)** ان سب کو ایک ہی سبسکرپشن کے تحت کور کرتا ہے:
...
کلوور کے دو بنیادی انٹر فیس ہیں: آپ ٹرمینل UI کو `clover` کے ساتھ شروع کریں، یا گیٹ وے چلا کر اس کے ساتھ ٹیلی گرام، ڈسکارڈ، سلیک، واٹس ایپ، سگنل، یا ای میل کے ذریعے بات کریں۔ جب آپ کسی بات چیت میں ہوتے ہیں، تو بہت سی سلیش (slash) کمانڈز دونوں انٹرفیسز میں ایک جیسی ہوتی ہیں۔

<div dir="ltr">

| کارروائی (Action)                         | سی ایل آئی (CLI)                              | میسجنگ پلیٹ فارمز (Messaging platforms)                                          |
| --------------------------------------- | --------------------------------------------- | -------------------------------------------------------------------------------- |
| بات چیت شروع کریں                       | `clover`                                      | `clover gateway setup` اور `clover gateway start` چلائیں، پھر بوٹ کو میسج بھیجیں |
...
اگر آپ OpenClaw سے منتقل ہو رہے ہیں، تو کلوور آپ کی سیٹنگز، یادیں (memories)، مہارتیں (skills)، اور API کیز کو خود بخود امپورٹ کر سکتا ہے۔

**پہلی بار سیٹ اپ کے دوران:** سیٹ اپ وزرڈ (`clover setup`) خود بخود `~/.openclaw` کو پہچان لیتا ہے اور کنفیگریشن شروع ہونے سے پہلے مائیگریٹ (migrate) کرنے کا آپشن دیتا ہے۔
...
```bash
clover claw migrate              # انٹرایکٹو مائیگریشن (مکمل پری سیٹ)
clover claw migrate --dry-run    # جائزہ لیں کہ کیا کیا مائیگریٹ ہوگا
clover claw migrate --preset user-data   # حساس معلومات (secrets) کے بغیر مائیگریٹ کریں
clover claw migrate --overwrite  # موجودہ متصادم فائلوں کو اوور رائٹ کریں
```
...
- **مہارتیں (Skills)** — صارف کی بنائی گئی مہارتیں → `~/.clover/skills/openclaw-imports/`
...
تمام آپشنز دیکھنے کے لیے `clover claw migrate --help` استعمال کریں، یا انٹرایکٹو ایجنٹ کی مدد سے مائیگریٹ کرنے کے لیے `openclaw-migration` سکل کا استعمال کریں (جس میں ڈرائی رن (dry-run) پریویوز شامل ہیں)۔
...
[نوس ریسرچ (Clover Cognition)](https://github.com/oaostarboy/clover-c1) کی جانب سے تیار کردہ۔
