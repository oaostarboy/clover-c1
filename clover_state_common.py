...
Extracted verbatim from clover_state.py so the SessionDB mixin modules
(clover_state_search / clover_state_schema / clover_state_portability) can
reference them without importing clover_state (which would be a cycle).
clover_state re-imports every name here for backward compatibility.
"""
...
# optimized via ``clover sessions optimize-storage``. A legacy DB sits at
...
# `clover sessions optimize-storage` rebuilds it on a capable host.
...
# triggers repairable UNTIL the user opts into `clover db optimize`. This is
...
#
# Several independent Clover processes routinely share one state.db (gateway
# service, the Desktop app's `clover serve` backend, interactive CLI sessions,
...
#
# Semantics mirror `clover_state._cross_process_repair_lock` (the schema-
...
# rebuilder cannot wedge future rebuilds. It lives here (not clover_state)
# because the search/schema mixins cannot import clover_state (cycle).
#
