...
`clover config set gateway.profile_routes ...` writes).
...
| `profile` | yes | Target profile name (must exist under `~/.clover/profiles/<name>`). |
...
activates the per-profile runtime scope (per-profile `CLOVER_HOME`, secret scope, and
