...
(`clover-account-service`) and anyone debugging the managed-cron path.

Chronos lets a hosted Clover gateway **scale to zero** while idle and still
...
ChronosCronScheduler.reconcile()        ── agent computes next_run_at
  │  POST {portal}/api/agent-cron/provision   (auth: agent's Clover access token)
  ▼
...
|---|---|---|---|
| 1 | agent → NAS (`provision`/`cancel`/`list`) | the agent's existing **Clover Portal access token** (Bearer) — for a hosted agent this is the **bootstrap-session token** NAS planted in `auth.json` (client `clover-cli-vps`), NOT an `agent:*` client token | NAS (its normal agent-token path) |
...
> agent uses the **bootstrap-session access token** (`resolve_clover_access_token`),
> minted under the bootstrap-only client `clover-cli-vps` and seeded into the
...
- **Auth:** `Authorization: Bearer <agent Clover access token>`. NAS validates via
...
1. **Dashboard app** (`clover_cli/web_server.py`) — the agent's only public
...
- **Multi-replica agents:** the store CAS makes the fire at-most-once across N
  gateway replicas sharing one `CLOVER_HOME` — exactly one replica runs each
  fire.
...
If `callback_url` / `portal_url` is blank or the agent has no Clover login,
