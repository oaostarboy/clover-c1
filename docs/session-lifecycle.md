...
The canonical transcript store is SQLite via `SessionDB` (from `clover_state`). The
...
Counts consecutive restarts via a JSON file (`{CLOVER_HOME}/restart_counts.json`). If a
...
This prevents unwanted auto-resets after `clover update`, `clover gateway restart`,
or `/restart`.
