...
When running Clover inside Docker, the default `network_mode: host` gives the
...
The Clover [SECURITY.md](../../SECURITY.md) §2 defines the trust model. The
...
#
# Usage:
#   CLOVER_UID=$(id -u) CLOVER_GID=$(id -g) docker compose up -d
#
...
services:
  gateway:
    ...
    environment:
      ...
      - NO_PROXY=clover,clover-dashboard,localhost
...
docker compose exec gateway \
  curl -sf --max-time 5 http://clover-dashboard:9119/health && echo "OK: internal reachable" || echo "FAIL"
...
- [SECURITY.md](../../SECURITY.md) — Clover trust model and vulnerability reporting
