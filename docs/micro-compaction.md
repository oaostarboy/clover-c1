...
has to be thrown away or summarized. Clover has always done this in one batch:
...
Clover folds the single oldest un-absorbed exchange into a running summary. The
...
The in-memory splice alone isn't enough. Clover's normal session flush is
...
With `micro_compact` unset or `false` Clover behaves exactly as it always has:
...
Defaults to `$CLOVER_HOME/logs/agent.log`. It reports passes, outcome mix, net
