...
or change arbitrary Clover configuration.
...
small host fact requested by the RFC. Settings writes use Clover'
...
`CLOVER_HOME`, so one globally loaded plugin context follows multiplexed
...
```text
<CLOVER_HOME>/plugin-data/<plugin-data-namespace>/state.json
```
...
- **Profile isolation:** the data root resolves from the active context-local
  Clover home on every operation.
...
The implementation is covered with real temporary-Clover-home tests for:
...
- [Issue #64227](https://github.com/CloverCognition/clover-c1/issues/64227)
- [RFC PR #58542](https://github.com/CloverCognition/clover-c1/pull/58542) by Topher Ross
