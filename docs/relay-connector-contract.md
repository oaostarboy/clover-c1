...
`CloverCognition/gateway-gateway`). The connector implementer's first action is to
read this file.
...
- **Registration.** The gateway registers a **wake URL** at enroll/provision —
  ...
  a dashboard host). Self-hosted: `clover gateway enroll --wake-url <url>` (or
...
**per-gateway secret** (`clover gateway enroll` → connector `/relay/enroll`, or
...
(`CloverCognition/gateway-gateway`); this section is the gateway-facing summary.
...
driven by the managed Portal / `clover` CLI.
