# Clover Observer Hooks

Clover observer hooks are the read-only telemetry contract for plugins that
...
They do not replace Clover' planner, model providers, memory, tool registry,
...
Clover also has a first-party NeMo Relay shared-metrics path. It uses these
...
```text
telemetry_schema_version = "clover.observer.v1"
```

Hook callbacks are fail-open. Clover catches callback exceptions, logs a
...
payload construction is gated behind `has_hook(...)`, so Clover only builds
...
The native NeMo Relay SDK integration maps Clover session, turn, LLM, and tool
