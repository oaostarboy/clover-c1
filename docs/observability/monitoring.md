...
Clover gateway daemon, exported over OTLP/HTTP to an operator-configured
...
capture is a separate plane served by Clover's native NeMo Relay SDK
...
| --- | --- | --- |
| Gateway gauges | `/v1/metrics` | `clover.gateway.up/state/busy/drainable/active_agents/background_work/background_delegations/restart_requested`, `clover.platform.up/degraded` with bounded `error_code` attributes |
...
`clover.gateway.active_agents`, `clover.gateway.background_work`, and
`clover.gateway.background_delegations` are complementary. `active_agents`
...
```bash
clover monitoring status
```

The OpenTelemetry SDK is an optional extra (`pip install 'clover-c1[otlp]'`),
...
a plain `clover gateway run`: the exporter lives in the gateway process, so
...
`clover.gateway.up`, `clover.platform.up`, and `clover.platform.degraded`.
...
absent_over_time(clover_gateway_up[5m])

# Locally owned bridge is explicitly down.
clover_platform_up == 0

# Scheduler thread is stale even though the gateway may still be alive.
clover_cron_scheduler_heartbeat_age_seconds > 180

# Ticker loops but has not completed a successful tick recently.
clover_cron_scheduler_last_success_age_seconds > 300

# One or more jobs are beyond their existing scheduler grace window.
clover_cron_jobs_overdue > 0

# Catch-up counter increased, proving at least one stale occurrence was
# collapsed and run once after a delay.
increase(clover_cron_scheduler_catch_up_occurrences[15m]) > 0
```

Cron execution lifecycle records arrive as `clover.cron_execution` spans.
Alert or derive events from bounded attributes such as:

```text
clover.status = failed|unknown
clover.delivery_outcome = failed|not_configured
clover.error_class = auth_failed|rate_limited|timeout|network_error|
...
3. a cron lifecycle feed keyed only by opaque `clover.job_key`;
...
Clover Cognition-owned Relay transport health remains in scope. A separate gateway
...
python scripts/observability/otel_capture_collector.py \
  --host 127.0.0.1 --port 4318 --log /tmp/clover_otel_capture.jsonl
...
python scripts/observability/gateway_health_export_probe.py \
  ...
  --log /tmp/clover_otel_capture.jsonl --wait 8
...
`clover.error_class = ...` list in this file's alert section and the enum's unit
...
```bash
clover monitoring status                 # posture
python scripts/observability/gateway_health_export_probe.py \
  --endpoint http://127.0.0.1:4318/v1/traces \
...
The `clover monitoring` CLI intentionally exposes `status` only. This first
release covers only Clover Cognition-owned service-health and operational-diagnostic
signals, including Clover Cognition-owned Relay transport health. Team Gateway's
