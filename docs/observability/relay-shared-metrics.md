...
Clover includes NeMo Relay as a normal runtime dependency on platforms for
which Relay publishes a native wheel. The shared-metrics integration is built
into Clover and does not require a Clover observability plugin. Clover remains
importable without Relay on other native targets. Those targets use an
explicit reduced-capability no-op host:
Clover execution remains available, while Relay scopes, middleware, plugins,
and subscribers are unavailable. The `clover-c1[nemo-relay]` extra remains
as a no-op compatibility alias for existing installation commands.

> [!WARNING]
> This removes the Clover `observability/nemo_relay` plugin. Existing users
...
> `plugins.toml` selected with `CLOVER_NEMO_RELAY_PLUGINS_TOML`. The legacy
> `CLOVER_NEMO_RELAY_ATOF_*` and `CLOVER_NEMO_RELAY_ATIF_*` variables no
> longer activate exporters. Without the new variable, Clover does not run
> Relay plugin discovery, configuration layering, middleware, or exporters.

Clover requires NeMo Relay 0.7.1 or later within the 0.7 release line. That
...
Clover installs the platform-specific `nemo-relay` native wheel from the
...
through that native module in the Clover process so configured interceptors can
...
Collection remains off unless Clover policy enables it:
...
opt-in. Set `CLOVER_NEMO_RELAY_PLUGINS_TOML` to a selected `plugins.toml` to
activate configured middleware, exporters, or dynamic plugins. When the
variable is unset, Clover does not invoke Relay's plugin initializer, so Relay
...
from the selected file only. If the selected file cannot be loaded, Clover
...
retain the same `session_id` and add `clover.session.segment` plus
`clover.session.segment_reason` (`compaction` or `max_turns`).

## Process-Wide Plugin Policy and Profile Isolation

Relay plugin configuration is a process-level deployment choice, not a Clover
profile setting. The first hosted profile triggers lazy initialization, and
every additional profile hosted by that Clover process shares the resulting
static middleware, dynamic plugins, subscribers, exporters, and guardrail
policy. After initialization succeeds, Clover logs:

```text
Relay plugins are active process-wide and apply to all profiles hosted by this Clover process.
```
...
profile's Relay scope stack. Native dynamic plugins are loaded into the Clover
process and share the same policy boundary.

Run profiles in separate Clover processes when they require different trust
...
Clover core owns one Relay host and one isolated Relay session scope per Clover
session. Core lifecycle producers use
`clover_cli.observability.relay_runtime` to obtain the shared session handle or
run Relay scope, LLM, tool, and mark APIs in that session context. New product
marks do not require Clover plugin registration. Shared-metrics marks must
...
```text
Clover turn, API, tool, and approval hooks
  -> Relay session, task, LLM, tool, and mark lifecycle
  -> Clover shared-metrics subscriber
  -> SQLite counters
  -> immutable JSON delta package
```

Clover sends an empty `LLMRequest` into the metrics-owned lifecycle. This does
...
provider route that Clover used for the logical call, such as
...
New calls use `clover.model_route.count`. The previous
`clover.model_call.count` contract remains readable only so pending local
counters created by older builds can be exported without losing data.

The first consented session start emits an empty `clover.client.active` Relay
...
counter in any rolling 24-hour window. The metric has no dimensions; Clover
version, OS family, architecture, and install method remain bounded package
resources. Concurrent Clover processes share the SQLite latch, so simultaneous
...
Each task run is a Relay `Function` scope named `clover.task_run`, parented to
the owning Clover session. The start counter contains only bounded execution
...
provider attempts for the same Clover API request ID; they do not inflate the
logical model-call count. Tool calls are deduplicated by their Clover tool-call
...
cancellations. Active task ownership follows the task ID if Clover rotates its
conversation session during context compression.

Each tool invocation is represented by a Relay tool lifecycle named
`clover.tool_call`. The terminal counter contains only bounded tool category,
outcome, approval outcome, latency, and explicit retry-count buckets. Clover
...
tool or plugin names. Clover does not infer retries from repeated tool names or
adjacent calls; when the
hook does not provide an explicit retry relationship, the retry bucket is
`unknown`. Approval decisions are emitted as `clover.tool_approval` marks and
...
Successful skill mutations emit `clover.skill.lifecycle` marks with only a
bounded action and provenance. Successful loads emit `clover.skill.load`
marks with bounded provenance, first-use or reuse state, reuse-after-patch
state, and a use-count bucket. Clover derives reuse and patch-generation
...
```text
$CLOVER_HOME/telemetry/shared_metrics/metrics.sqlite3
$CLOVER_HOME/telemetry/shared_metrics/outbox/*.json
```
...
are written with atomic replacement. Each package records the Clover version,
...
schema field name, its current scope is one `CLOVER_HOME`, so it is more
...
Deleting `$CLOVER_HOME/telemetry/shared_metrics` resets the identifier together
...
The install identity is scoped to one `CLOVER_HOME`. To reset it, stop Clover
processes and remove `$CLOVER_HOME/telemetry/shared_metrics`. This deliberately
...
Run a real Clover CLI turn against the deterministic local model server:
