# Clover Cognition Security Policy

This document describes Clover Cognition's trust model, names the one
security boundary the project treats as load-bearing, and defines the
scope for vulnerability reports.

## 1. Reporting a Vulnerability

Report privately via [GitHub Security Advisories](https://github.com/CloverCognition/clover-c1/security/advisories/new)
or **security@clover-c1.local**. Do not open public issues for
...
- Environment details (`clover --version`, commit SHA, OS, Python
  version).
...
Clover Cognition is a single-tenant personal agent. Its posture is
layered, and the layers are not equally load-bearing. Reporters and
operators should reason about them in the same terms.

### 2.1 Definitions

- **Agent process.** The Python interpreter running Clover Cognition,
...
- **Trust envelope.** The set of resources an operator has implicitly
  granted Clover Cognition access to by running it — typically, whatever
  the operator's own user account can reach on the host.
- **Stance.** An explicit statement in Clover Cognition's documentation
...
Clover Cognition supports two OS-level isolation postures. They address
...
Clover Cognition supports this in two ways:

- **Clover Cognition's own Docker image and Compose setup.** Lighter-
...
Under a whole-process wrapper, Clover Cognition's in-process heuristics
...
Clover Cognition filters the environment it passes to its lower-trust
...
A malicious or buggy plugin is not a vulnerability in Clover Cognition
itself. Bugs in Clover Cognition's plugin-install or plugin-discovery
...
4. **Within the authorized set, all callers are equally trusted.**
   Clover Cognition does not model per-caller capabilities inside a
...
- Trust-model documentation violations: code behaving contrary to
  what this policy, Clover Cognition's own documentation, or reasonable
  operator expectations would predict — including cases where
  Clover Cognition has documented a stance about how its output should
...
- **Documented break-glass settings.** Operator-selected trade-offs
  ...
  clover-home security, and similar. Reports against those
...
- **Community-contributed skills and plugins.** Third-party skills
  ...
  plugins are in the operator's review surface, not Clover Cognition's
  trust surface (§2.4, §2.5). A skill or plugin doing something
  malicious is the expected failure mode of one that wasn't
  reviewed, not a vulnerability in Clover Cognition. Bugs in Clover Cognition's skill-install or plugin-install path that prevent the
...
- Clover Cognition includes supply-chain guards for MCP server
