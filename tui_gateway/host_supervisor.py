...
def _default_registry_path() -> Path:
    return get_clover_home() / "state" / _REGISTRY_NAME
...
class HostSupervisor:
    ...
    def __init__(
        ...
        *,
        ...
        expected_clover_home: str | None = None,
        autostart: bool = True,
    ) -> None:
        ...
        self.expected_clover_home = expected_clover_home if expected_clover_home is not None else str(get_clover_home())
    ...
    def _spawn_locked(self, *, reason: str) -> None:
        ...
        if self.env:
            env.update(self.env)
        env["CLOVER_COMPUTE_HOST_HEARTBEAT_SECS"] = str(self.heartbeat_secs)
    ...
    def _validate_hello(self) -> None:
        ...
        got_home = str(hello.get("clover_home") or "")
        if got_home and got_home != self.expected_clover_home:
            raise RuntimeError(f"compute host CLOVER_HOME mismatch: {got_home} != {self.expected_clover_home}")
