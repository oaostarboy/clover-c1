    ...
    try:
        with _profile_db(params) as db:
            ...
            with pdb.connect_closing() as conn:
                ...
                # zero Clover sessions still surface. The desktop's native scan
...
def _(rid, params: dict) -> dict:
    ...
    if key == "provider":
        try:
            from clover_cli.models import list_available_providers, normalize_provider
    ...
    if key == "profile":
        ...
        return _ok(rid, {"home": str(_clover_home), "display": display_clover_home()})
    if key == "project":
    ...
    if key == "mtime":
        cfg_path = _clover_home / "config.yaml"
        try:
        ...
        if (
            ...
            return _ok(
                rid,
                {
                    ...
                    "source": source,
                    "error": "No Clover provider is configured.",
                },
            )
