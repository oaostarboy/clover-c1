...
def _(rid, params: dict) -> dict:
    from clover_cli.input_sanitize import sanitize_user_prompt_text
    ...
    # over another app when they are back in Clover.
        ...
        if (
            ...
            if not is_truthy_value(params.get("confirm_truncate")):
                ...
                return _err(
                    ...
                    "(update your Clover client if a rewind was intended)",
                )
...
def _(rid, params: dict) -> dict:
    ...
    if err:
        return err
    try:
        from clover_cli.clipboard import has_clipboard_image, save_clipboard_image
...
def _(rid, params: dict) -> dict:
    ...
    def run():
        ...
        try:
            ...
            # CLOVER_HOME override (the ContextVar set on the session-create
            ...
            home_token = (
                set_clover_home_override(_profile_home_str)
...
def _(rid, params: dict) -> dict:
    ...
    prompt = "\n".join(
        ...
        for line in [
            ...
            "Restart exactly the app intended for the Preview URL, not Clover Desktop itself.",
            ...
            "First inspect what process, if any, owns the Preview URL port. If a stale server exists, inspect its cwd and prefer that cwd over the Clover/Desktop process cwd.",
            ...
            "Before declaring success, verify the Preview URL responds with the intended app, not Clover Desktop. If it serves Clover/Desktop UI or another unrelated app, stop that process and report failure.",
    ...
    def run():
        ...
        try:
            ...
            # the session's CLOVER_HOME override (the ContextVar set on the
...
def _(rid, params: dict) -> dict:
    # `text` is a JSON string describing the OS window underneath the Clover
