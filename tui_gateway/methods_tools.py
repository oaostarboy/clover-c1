...
@method("reload.env")
def _(rid, params: dict) -> dict:
    """Re-read ``~/.clover/.env`` into the gateway process via
    ``clover_cli.config.reload_env``, matching classic CLI's ``/reload``
...
@method("cli.exec")
def _(rid, params: dict) -> dict:
    """Run `python -m clover_cli.main` with argv; capture stdout/stderr (non-interactive only)."""
    ...
    try:
        ...
        r = subprocess.run(
            [sys.executable, "-m", "clover_cli.main", *argv],
            capture_output=True,
            text=True,
            ...
            cwd=os.getcwd(),
            # cli.exec runs `python -m clover_cli.main` (can drive the agent) →
            # needs provider credentials. Tier-1 secrets still stripped (#29157).
            env=clover_subprocess_env(inherit_credentials=True),
...
def _(rid, params: dict) -> dict:
    ...
    if name == "plan":
        ...
        # .clover/plans/ via write_file. Works on any backend.
    ...
    if name == "moa":
        ...
        try:
            from clover_cli.moa_config import moa_usage, normalize_moa_config
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        # Re-bind CLOVER_HOME to the session's profile so get_skill_commands()
        ...
        _home_token = (
            set_clover_home_override(_profile_home) if _profile_home else None
        )
        try:
        ...
        sections = [
            ...
            },
            {
                ...
                "rows": [
                    ...
                    ["Config File", str(_clover_home / "config.yaml")],
                ],
            },
        ]
...
def _(rid, params: dict) -> dict:
    ...
    # Optional profile scoping: cronjob() keys off CLOVER_HOME, so scoping the
    ...
    if profile:
        try:
            ...
            from clover_constants import set_clover_home_override
    ...
    try:
        ...
        if token is not None:
            try:
                from clover_constants import reset_clover_home_override
...
def _(rid, params: dict) -> dict:
    ...
    gateway. Shares its renderer with the ``clover journey`` CLI.
    """
    try:
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        if action == "install":
            from clover_cli.skills_hub import do_install
        ...
        if action == "browse":
            from clover_cli.skills_hub import browse_skills
        ...
        if action == "inspect":
            from clover_cli.skills_hub import inspect_skill
...
def _(rid, params: dict) -> dict:
    ...
    keys], transport}]}`` — the same catalog `clover mcp` offers, so
    ...
    try:
        ...
        from clover_cli import mcp_catalog
...
# (clover_cli/web_routers/mcp.py) so a desktop plugin can manage MCP servers for
# ANY profile, not just the launch profile. Each accepts an optional ``profile``
# param that scopes CLOVER_HOME via set_clover_home_override (omitted/None = the
# launch profile) in a try/finally, exactly like ``skills.manage`` / ``mcp.catalog``.
# All persistence reuses clover_cli/mcp_config.py helpers — no logic is duplicated.
...
def _(rid, params: dict) -> dict:
    ...
    if err:
        return err
    try:
        from clover_cli.mcp_config import _get_mcp_servers
...
def _(rid, params: dict) -> dict:
    ...
    if err:
        return err
    try:
        from clover_cli.mcp_config import _remove_mcp_server
...
def _(rid, params: dict) -> dict:
    ...
    (``window.cloverDesktop.openExternal``) and then polls
    ...
    the SAME interactive MCP OAuth machinery ``clover mcp login`` uses
    ...
    try:
        ...
        clover_home = str(get_clover_home().expanduser().resolve(strict=False))
        result = mcp_oauth_sessions.start_flow(
            clover_home, name, cfg, client_redirect_uri=client_redirect_uri
        )
        return _ok(
            rid,
            {
...
def _(rid, params: dict) -> dict:
    ...
    primitives as ``clover plugins`` / the dashboard, so the three surfaces
    ...
    Actions:
      ...
      - ``install`` → git-clone into ``~/.clover/plugins/`` (non-interactive).
    ...
    plugins live under each profile's CLOVER_HOME, so a client can list or
                ...
                out.append(
                    {
                        ...
                        # skills/MCP format) vs a native Clover plugin.
