...
def _(rid, params: dict) -> dict:
    ...
    def _profile_mention_items(prefix: str) -> list[dict]:
        ...
        under the 'clover' alias when no real profile claims that name.
        """
        out: list[dict] = []
        try:
            ...
            if "clover".startswith(prefix.lower()) and "clover" not in seen:
                out.append(
                    {
                        "text": "@clover",
                        "display": "@clover",
...
def _(rid, params: dict) -> dict:
    ...
    try:
        from clover_cli.commands import SlashCommandCompleter
...
def _(rid, params: dict) -> dict:
    try:
        from clover_cli.inventory import build_model_options_payload
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        from clover_cli.inventory import build_models_payload
        ...
        if pconfig.auth_type != "api_key":
            return _err(
                ...
                f"run `clover model` to configure",
            )
        if not pconfig.api_key_env_vars:
            return _err(rid, 4004, f"no env var defined for {pconfig.name}")

        # Save the key to ~/.clover/.env via the unified credential lifecycle
...
def _(rid, params: dict) -> dict:
    ...
    """
    try:
        from clover_cli.auth import PROVIDER_REGISTRY, clear_provider_auth
