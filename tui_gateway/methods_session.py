...
def _(rid, params: dict) -> dict:
    ...
    # and each turn re-bind CLOVER_HOME. None/own profile → launch (unchanged).
...
def _(rid, params: dict) -> dict:
    with _profile_db(params) as db:
        ...
        try:
            ...
            # custom `CLOVER_SESSION_SOURCE` values, and older installs with
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        _enable_gateway_prompts()
        home_token = (
            set_clover_home_override(str(profile_home)) if profile_home is not None else None
        )
        secret_token = (
        ...
        with _session_resume_lock:
            ...
            try:
                init_home_token = (
                    set_clover_home_override(str(profile_home))
                    if profile_home is not None
                    else None
                )
                init_secret_token = (
                ...
                try:
                    ...
                    if init_home_token is not None:
                        reset_clover_home_override(init_home_token)
                ...
                if sid in _sessions:
                    ...
                    # Remember the profile home so each turn re-binds CLOVER_HOME (the
        ...
        # atexit.register(_drain_token_queue_at_exit) (clover_state.py), which only
...
def _(rid, params: dict) -> dict:
    ...
    # ``clover --tui`` session legitimately rides the real stdio transport and
    # must stay visible.
...
def _(rid, params: dict) -> dict:
    ...
    transfer is performed by the separate ``clover gateway`` process, whose
...
def _(rid, params: dict) -> dict:
    ...
    # Clover credits block — agent-independent (a portal fetch), so it shows even
...
def _(rid, params: dict) -> dict:
    ...
    try:
        from agent.pet import store
        from clover_cli.pets import _clear_active_if
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        if new_slug != slug:
            try:
                from clover_cli.pets import _rename_active_if
...
def _(rid, params: dict) -> dict:
    ...
    try:
        from clover_cli.pets import _set_enabled
...
def _(rid, params: dict) -> dict:
    ...
    """
    try:
        from clover_cli.pets import set_pet_scale
...
def _(rid, params: dict) -> dict:
    ...
    True only when a reference-capable image backend (Clover Portal / OpenRouter /
...
def _(rid, params: dict) -> dict:
    ...
    agent = session["agent"]
    # Mirror the classic CLI /save: snapshot under the Clover profile home
    # (~/.clover/sessions/saved/) rather than the project/workspace CWD, and
    ...
    path = saved_dir / f"clover_conversation_{timestamp}.json"
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        if parent_home:
            ...
            branch_owns_db = True
        home_token = (
            set_clover_home_override(parent_home) if parent_home else None
        )
