#!/usr/bin/env bash
# Canonical test runner for clover-c1. Run this instead of calling
...
#   * Proper venv activation (probes .venv, venv, then ~/.clover/...)
#
# Usage:
...
# (CLOVER_PYTHON is exported by the devShell hook and ships [dev] extras:
# pytest, pytest-asyncio, pytest-timeout, ruff, ty).
#
# A candidate must have pytest INSTALLED, not merely exist. The release venv
# at ~/.clover/clover-c1/venv has bin/activate but no pytest, so an
...
VENV_PYTHON=""
SKIPPED_VENVS=""
for candidate in "$REPO_ROOT/.venv" "$REPO_ROOT/venv" "$HOME/.clover/clover-c1/venv"; do
...
if [ -n "$VENV" ]; then
  PYTHON="$VENV_PYTHON"
elif [ -n "${CLOVER_PYTHON:-}" ] && [ -x "$CLOVER_PYTHON" ] \
    && "$CLOVER_PYTHON" -c 'import pytest' 2>/dev/null; then
  # Guard with an import check: CLOVER_PYTHON may point at the RELEASE
  # venv (no pytest) when inherited from a wrapped `clover` binary rather
  # than the devShell hook.
  PYTHON="$CLOVER_PYTHON"
  echo "▶ no local venv — using Nix dev venv via CLOVER_PYTHON: $PYTHON"
else
  echo "error: no virtualenv with pytest found in $REPO_ROOT/.venv or $REPO_ROOT/venv," >&2
  echo "       and CLOVER_PYTHON is not a python with pytest (enter the Nix devShell or create a venv)" >&2
  if [ -n "$SKIPPED_VENVS" ]; then
...
if [ -f "$HOME/.clover/pytest_live_guard.py" ]; then
  EXTRA_PYTHONPATH="$HOME/.clover"
...
#
#   * CLOVER_TEST_WORKERS / PATHS / FILE_TIMEOUT / FILE_RETRIES / SLICE are
#     read by run_tests_parallel.py at argparse-default time — inside the
#     stripped environment.
#   * CLOVER_TEST_IMAGE is read by tests/docker/conftest.py to skip its
...
#     CLOVER_GIT_SHA build-arg the workflow bakes in).
#
# These are test-infrastructure knobs, not credentials — same class as the
# CLOVER_RUN_SLOW_PET_TESTS / CLOVER_E2E_BROWSER opt-ins already forwarded.
# Keep this an explicit allowlist (no CLOVER_TEST_* glob) so the "no
# credential can leak" property stays auditable at a glance.
TEST_ENV=()
for _test_var in CLOVER_TEST_IMAGE CLOVER_TEST_WORKERS CLOVER_TEST_PATHS \
  CLOVER_TEST_FILE_TIMEOUT CLOVER_TEST_FILE_RETRIES CLOVER_TEST_SLICE; do
  if [ -n "${!_test_var:-}" ]; then
...
exec env -i \
  ...
  PYTHONHASHSEED=0 \
  PYTHONUTF8=1 \
  ${CLOVER_RUN_SLOW_PET_TESTS:+CLOVER_RUN_SLOW_PET_TESTS="$CLOVER_RUN_SLOW_PET_TESTS"} \
  ${CLOVER_E2E_BROWSER:+CLOVER_E2E_BROWSER="$CLOVER_E2E_BROWSER"} \
