#!/usr/bin/env python3
"""Clover Cognition Release Script
...
VERSION_FILE = REPO_ROOT / "clover_cli" / "__init__.py"
...
LEGACY_AUTHOR_MAP = {
    ...
    "pi@clover.local": "Elektrofussel",  # PR #61675 salvage (honcho: defaultHost + private-range local URL detection)
    ...
    "kavi@local.clover": "kavioavio",  # Issue #46544 / PR #47705 evolution (smart DENY exact-operation owner override)
    ...
    "sunsky.lau@gmail.com": "liuhao1024",  # PR #56993 salvage (gateway: process-level CLOVER_HOME for pid/lock/status identity files; #56986)
    ...
    "wyuebei@gmail.com": "wyuebei-cloud",  # PR #56640 salvage (clover journey: replace GNU-only %-d strftime with dt.day for Windows)
    ...
    "clover.wanderer@yahoo.com": "trismegistus-wanderer",  # PR #31856 salvage (gateway: defer idle-TTL agent-cache eviction until the session store says the session actually expired, so the expiry watcher can still fire MemoryProvider.on_session_end with the live transcript; #11205)
    ...
    "me@keslerm.com": "keslerm",  # PR #3459 salvage (gateway: 'log' tool_progress mode — silent in chat, tool calls appended to ~/.clover/logs/tool_calls.log via rotating handler; duplicate of #3458 by @dlkakbs who submitted 4 min earlier — both credited)
    ...
    "hodlclone@gmail.com": "HODLCLONE",  # PR #49351 salvage (Clover Portal token resilience: rotate refresh tokens write-through to the source auth store in profile mode, skip Clover fallback when no local token, sync gateway session model after fallback)
    ...
    "moonsong@clovercognition.local": "Tranquil-Flow",  # PR #52623 salvage (auxiliary Anthropic base_url host validation; #52608)
    ...
    "121278003+Cossackx@users.noreply.github.com": "Cossackx",  # PR #52528 salvage (Windows clover-shim resolution + prefer --update on recovery; #52378)
    ...
    "clover@marian.local": "Schrotti77",
    ...
    "274096618+clover-c1-dhabibi@users.noreply.github.com": "dhabibi",
    ...
    "sberan@gmail.com": "sberan",  # PR #54494 salvage (--connect-timeout flag on clover mcp add)
    ...
    "clover@example.com": "shellybotmoyer",  # PR #18915 (bot-committed)
    ...
    "github@nadyaclover.anonaddy.com": "ruangraung",  # PR #42308
    ...
    "sylw3st3rr@gmail.com": "Sylw3ster",  # salvage of #23252 (CLOVER_KANBAN_BOARD restore)
    ...
    "58116817+TurgutKural@users.noreply.github.com": "TurgutKural",  # salvage of #23356 (CLOVER_HOME inject)
    ...
    "sauravsejal40@gmail.com": "Saurav0989",  # PR #27071 (docs: clover-eval community link)
    ...
    "cloveragent26@gmail.com": "cloveragent26",  # PR #26438 (kimi model-name reasoning pad)
    "276067471+cloveragent26@users.noreply.github.com": "cloveragent26",
    ...
    "vigo@clover": "hawknewton",  # PR #26294 (bedrock boto3 lazy_deps)
    ...
    "hex.clover@agentmail.to": "oseftg",
    ...
    "kdkcfp@gmail.com": "slowtokki0409",  # PR #29025 (ignore local Clover runtime files)
    ...
    "2663402852@qq.com": "x1am1",  # PR #35098 (chown root-owned top-level CLOVER_HOME state files)
...
def update_version_files(semver: str, calver_date: str):
    ...
    # Python package version. The desktop About panel reads the live Clover
...
def generate_changelog(commits, tag_name, semver, repo_url="",
    ...
    lines.append(f"# Clover Cognition v{semver} ({tag_name})")
    lines.append("")
    ...
    if first_release:
        ...
        lines.append("> for Clover Cognition. See below for everything included in this initial release.")
        lines.append("")
...
def main():
    parser = argparse.ArgumentParser(description="Clover Cognition Release Tool")
    ...
    print(f"{'='*60}")
    print("  Clover Cognition Release Preview")
    print(f"{'='*60}")
    ...
    if args.publish:
        ...
        tag_result = git_result(
            "tag", "-a", tag_name, "-m",
            f"Clover Cognition v{new_version} ({calver_date})\n\nWeekly release"
        )
        ...
        gh_cmd = [
            ...
            "--title", f"Clover Cognition v{new_version} ({calver_date})",
        ...
        if result and result.returncode == 0:
            ...
            print(
                f"    gh release create {tag_name} --title 'Clover Cognition v{new_version} ({calver_date})' "
