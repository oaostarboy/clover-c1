...
# Python-driven tool session (e.g. Clover terminal tool). A pre-set PYTHONPATH
...
# Configuration
REPO_URL_SSH="git@github.com:CloverCognition/clover-c1.git"
REPO_URL_HTTPS="https://github.com/CloverCognition/clover-c1.git"
CLOVER_HOME="${CLOVER_HOME:-$HOME/.clover}"
# INSTALL_DIR is resolved AFTER arg parsing and OS detection so we can pick an
# FHS-style layout for root installs.  Track whether the user gave us an
# explicit directory — if so we never override it.
if [ -n "${CLOVER_INSTALL_DIR:-}" ]; then
    INSTALL_DIR="$CLOVER_INSTALL_DIR"
...
#   code at /usr/local/lib/clover-c1, command at /usr/local/bin/clover,
#   data still at /root/.clover (CLOVER_HOME).  Matches Claude Code / Codex CLI
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo "                   write \$CLOVER_HOME/.no-bundled-skills so future"
            echo "                   'clover update' runs never inject bundled skills either"
            ...
            echo "  --include-desktop  Also build the desktop app (apps/desktop -> Clover.app)"
            echo "  --dir PATH     Installation directory"
            echo "                   default (non-root):  ~/.clover/clover-c1"
            echo "                   default (root, Linux): /usr/local/lib/clover-c1"
            echo "  --clover-home PATH  Data directory (default: ~/.clover, or \$CLOVER_HOME)"
            echo "  -h, --help     Show this help"
            echo ""
            echo "Notes:"
            echo "  When running as root on Linux, Clover installs the code under"
            echo "  /usr/local/lib/clover-c1 and links the command into"
            echo "  /usr/local/bin/clover (FHS layout — matches Claude Code / Codex CLI)."
            echo "  Data, config, sessions, and logs still live in \$CLOVER_HOME"
            echo "  (default /root/.clover).  This keeps Docker bind-mounted volumes"
            echo "  small and ensures the command is on PATH for all shells."
            echo "  Existing installs at \$CLOVER_HOME/clover-c1 are preserved in-place."
...
print_banner() {
    ...
    echo "│             ⚕ Clover Cognition Installer                    │"
    echo "├─────────────────────────────────────────────────────────┤"
    echo "│  An open source AI agent by Clover Cognition.              │"
    echo "└─────────────────────────────────────────────────────────┘"
    echo -e "${NC}"
}
...
# intentional, but they leave the checkout dirty — which forces `clover update`
...
emit_manifest() {
    ...
    # install.ps1: the signed bootstrap installer (Clover-Setup) passes it so
    ...
    printf '%s' '{"protocol_version":1,"stages":[{"name":"prerequisites","title":"System prerequisites","category":"runtime","needs_user_input":false},{"name":"repository","title":"Download Clover Cognition","category":"runtime","needs_user_input":false},{"name":"venv","title":"Create Python virtual environment","category":"runtime","needs_user_input":false},{"name":"python-deps","title":"Install Python dependencies","category":"runtime","needs_user_input":false},{"name":"node-deps","title":"Install browser-tool dependencies","category":"runtime","needs_user_input":false},{"name":"path","title":"Install clover command","category":"runtime","needs_user_input":false},{"name":"config","title":"Prepare config and skills","category":"configuration","needs_user_input":false},{"name":"setup","title":"Configure API keys and settings","category":"configuration","needs_user_input":true},{"name":"gateway","title":"Configure gateway service","category":"configuration","needs_user_input":true},'"$desktop_stage"'{"name":"complete","title":"Finish install","category":"runtime","needs_user_input":false}]}'
    printf '\n'
}
...
#
# Defaults:
#   - Non-root, any OS:       INSTALL_DIR = $CLOVER_HOME/clover-c1
#                             command link in $HOME/.local/bin
#   - Termux (any uid):       INSTALL_DIR = $CLOVER_HOME/clover-c1
#                             command link in $PREFIX/bin (already on PATH)
#   - Root on Linux (new):    INSTALL_DIR = /usr/local/lib/clover-c1
#                             command link in /usr/local/bin
#                             (unless a legacy install already exists at
#                              $CLOVER_HOME/clover-c1 — then preserve it)
#
# Always no-op when the user set --dir or $CLOVER_INSTALL_DIR.
resolve_install_layout() {
    ...
    # Termux: package manager manages /data/data/..., keep code in CLOVER_HOME.
    if is_termux; then
        INSTALL_DIR="$CLOVER_HOME/clover-c1"
        return 0
    fi
    ...
    if [ "$OS" = "linux" ] && [ "$(id -u)" -eq 0 ]; then
        if [ -d "$CLOVER_HOME/clover-c1/.git" ]; then
            INSTALL_DIR="$CLOVER_HOME/clover-c1"
            log_info "Existing install detected at $INSTALL_DIR — keeping legacy layout"
            log_info "  (new root installs use /usr/local/lib/clover-c1)"
            return 0
        fi
        INSTALL_DIR="/usr/local/lib/clover-c1"
        ...
        # /usr/local/bin/clover wrapper unable to exec the bad-interpreter venv
        # python.  See #21457.
        ...
        log_info "  Command: /usr/local/bin/clover"
        log_info "  Data:    $CLOVER_HOME (unchanged)"
...
# Point a Clover-managed Node's `npm install -g` at a directory that is on
# PATH. npm's default global prefix for a bundled Node is the Node dir itself,
# so global package binaries land in $CLOVER_HOME/node/bin — which is NOT on
...
# Clover's own global installs pass an explicit --prefix and are unaffected.
# Idempotent and a no-op when there is no Clover-managed npm, so calling it on
# every install run repairs pre-existing installs, not just fresh ones.
configure_managed_node_npm_prefix() {
    [ -x "$CLOVER_HOME/node/bin/npm" ] || return 0
    local link_dir
    link_dir="$(get_command_link_dir)"
    mkdir -p "$CLOVER_HOME/node/etc"
    printf 'prefix=%s\n' "$(dirname "$link_dir")" > "$CLOVER_HOME/node/etc/npmrc"
}

get_clover_command_path() {
    local link_dir
    link_dir="$(get_command_link_dir)"
    if [ -x "$link_dir/clover" ]; then
        echo "$link_dir/clover"
    else
        echo "clover"
    fi
}
...
detect_os() {
    case "$(uname -s)" in
        ...
        CYGWIN*|MINGW*|MSYS*)
            ...
            log_info "  iex (irm https://clover-c1.local/install.ps1)"
            exit 1
            ;;
        *)
            OS="unknown"
...
install_uv() {
    ...
    # Clover owns its own uv at $CLOVER_HOME/bin/uv.  Always install there —
    # no PATH probing, no conda guards, no multi-location resolution chains.
    # The runtime update path (clover_cli/managed_uv.py) looks in the same
    # place, so install.sh and `clover update` stay in sync.
    local _managed_uv="$CLOVER_HOME/bin/uv"
    ...
    log_info "Installing managed uv into $CLOVER_HOME/bin ..."
    mkdir -p "$CLOVER_HOME/bin"

    # Two-stage: download the installer, then run it.  Piping
    # `curl | sh` masks curl failures (sh exits 0 on empty stdin)
    # and conflates network errors with installer errors.
    local _uv_install_log _uv_installer
    _uv_install_log="$(mktemp 2>/dev/null || echo "/tmp/clover-uv-install.$$.log")"
    _uv_installer="$(mktemp 2>/dev/null || echo "/tmp/clover-uv-installer.$$.sh")"
    ...
    # directly into $CLOVER_HOME/bin instead of ~/.local/bin.
    if UV_UNMANAGED_INSTALL="$CLOVER_HOME/bin" sh "$_uv_installer" >>"$_uv_install_log" 2>&1; then
        rm -f "$_uv_installer"
        if [ -x "$_managed_uv" ]; then
            UV_CMD="$_managed_uv"
        else
...
# with the Clover-managed Node $NODE_VERSION.
...
check_node() {
    ...
    # Repair pre-existing Clover-managed installs where `npm install -g` lands
    ...
    if command -v node &> /dev/null && command -v npm &> /dev/null \
        ...
        log_warn "min-release-age-exclude) — installing Clover-managed Node $NODE_VERSION instead..."
        install_node
        return
    fi

    # Prefer a Clover-managed Node from a previous run over a too-old system one.
    if [ -x "$CLOVER_HOME/node/bin/node" ] && [ -x "$CLOVER_HOME/node/bin/npm" ] \
        && node_satisfies_build "$("$CLOVER_HOME/node/bin/node" --version)"; then
        export PATH="$CLOVER_HOME/node/bin:$PATH"
        log_success "Node.js $("$CLOVER_HOME/node/bin/node" --version) found (Clover-managed)"
        HAS_NODE=true
        return 0
    fi
    ...
    if command -v node &> /dev/null && ! command -v npm &> /dev/null; then
        ...
        log_warn "Node.js $(node --version) is too old (Clover requires Node >=26) — installing Clover-managed Node $NODE_VERSION..."
...
install_node() {
    ...
    log_info "Extracting to ~/.clover/node/..."
    ...
    # Place into ~/.clover/node/ and symlink binaries into the same bin dir
    # the clover command uses (get_command_link_dir): /usr/local/bin for root
    # FHS installs, $PREFIX/bin on Termux, ~/.local/bin otherwise.
    rm -rf "$CLOVER_HOME/node"
    mkdir -p "$CLOVER_HOME"
    mv "$extracted_dir" "$CLOVER_HOME/node"
    rm -rf "$tmp_dir"

    local node_link_dir
    node_link_dir="$(get_command_link_dir)"
    mkdir -p "$node_link_dir"
    ln -sf "$CLOVER_HOME/node/bin/node" "$node_link_dir/node"
    ln -sf "$CLOVER_HOME/node/bin/npm"  "$node_link_dir/npm"
    ln -sf "$CLOVER_HOME/node/bin/npx"  "$node_link_dir/npx"
    ...
    local installed_ver
    installed_ver=$("$CLOVER_HOME/node/bin/node" --version 2>/dev/null)
    log_success "Node.js $installed_ver installed to ~/.clover/node/"
    HAS_NODE=true
}
...
check_network_prerequisites() {
    ...
    if [ "$DISTRO" = "termux" ]; then
        ...
        log_warn "Network checks failed. Clover install may complete, but web search and dependency downloads can fail."
    ...
    if [ -n "$pkg_install" ]; then
        ...
        # sudo needs password — ask once for everything
            ...
            if [ "$IS_INTERACTIVE" = true ]; then
                ...
                log_info "Clover Cognition itself does not require or retain root access."
    ...
    if [ -d "$INSTALL_DIR" ]; then
        if [ -d "$INSTALL_DIR/.git" ]; then
            ...
            if [ -n "$(git status --porcelain)" ]; then
                ...
                # index-level conflict state. Mirrors the `clover update` path
                # (#4735).
                ...
                local stash_name
                stash_name="clover-install-autostash-$(date -u +%Y%m%d-%H%M%S)"
            ...
            # cannot succeed — mirror ``clover update`` and reset to the
            ...
            if [ -n "$autostash_ref" ]; then
                ...
                if [ "$restore_now" = "yes" ]; then
                    ...
                    if [ "$restore_ok" = "yes" ] && [ -z "$conflicted_files" ]; then
                        ...
                        log_warn "Review git diff / git status if Clover behaves unexpectedly."
                    else
...
setup_path() {
    log_info "Setting up clover command..."

    if [ "$USE_VENV" = true ]; then
        CLOVER_BIN="$INSTALL_DIR/venv/bin/python"
        CLOVER_ENTRYPOINT="$INSTALL_DIR/clover"
    else
        CLOVER_BIN="$(which clover 2>/dev/null || echo "")"
        if [ -z "$CLOVER_BIN" ]; then
            log_warn "clover not found on PATH after install"
            return 0
        fi
    fi

    # Verify the interpreter and the checked-in entrypoint needed by the launcher.
    if [ ! -x "$CLOVER_BIN" ] || { [ "$USE_VENV" = true ] && [ ! -f "$CLOVER_ENTRYPOINT" ]; }; then
        log_warn "Clover launcher prerequisites not found"
    ...
    # Create a user-facing shim for the clover command.
    # We intentionally clear PYTHONPATH/PYTHONHOME here so inherited env vars
    # can't make this launcher import modules from another checkout.
    mkdir -p "$command_link_dir"
    # Older installs created this path as a symlink to $CLOVER_BIN. Without
    # the rm, `cat >` follows the symlink and overwrites the venv pip entry
    # point with this shim — making `exec "$CLOVER_BIN"` self-recurse. (#21454)
    rm -f "$command_link_dir/clover"
    if [ "$USE_VENV" = true ]; then
        ...
        cat > "$command_link_dir/clover" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
...
EOF
    ...
    chmod +x "$command_link_dir/clover"
    log_success "Installed clover launcher → $command_link_display_dir/clover"

    # Also expose `clover-c1`. The `clover-c1` console script declared in
    ...
    rm -f "$command_link_dir/clover-c1"
    if [ "$USE_VENV" = true ]; then
        cat > "$command_link_dir/clover-c1" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
...
unset PYTHONPATH
unset PYTHONHOME
exec "$CLOVER_BIN" run_agent.py "\$@"
EOF
    fi
    chmod +x "$command_link_dir/clover-c1"
    log_success "Installed clover-c1 launcher → $command_link_display_dir/clover-c1"

    # Also expose `clover-acp`. ACP hosts (Zed, JetBrains, Buzz) resolve the
    # agent by command name on the login-shell PATH, and the `clover-acp`
    # console script lives inside the venv, which is not on that PATH. Without
    # this launcher those hosts report Clover as not installed. (#21454 applies
    # here too: clear the path first so `cat >` cannot follow an old symlink
    # into the venv and overwrite the console script.)
    rm -f "$command_link_dir/clover-acp"
    if [ "$USE_VENV" = true ]; then
        cat > "$command_link_dir/clover-acp" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$CLOVER_BIN" "$CLOVER_ENTRYPOINT" acp "\$@"
EOF
    else
        cat > "$command_link_dir/clover-acp" <<EOF
#!/usr/bin/env bash
...
EOF
    ...
    chmod +x "$command_link_dir/clover-acp"
    log_success "Installed clover-acp launcher → $command_link_display_dir/clover-acp"

    if [ "$DISTRO" = "termux" ]; then
        export PATH="$command_link_dir:$PATH"
        log_info "$command_link_display_dir is the native Termux command path"
        log_success "clover command ready"
        return 0
    fi
    ...
    if [ "$ROOT_FHS_LAYOUT" = true ]; then
        ...
        if env -i HOME="$HOME" TERM="${TERM:-dumb}" bash -i -c 'command -v clover' \
                >/dev/null 2>&1; then
        ...
        log_info "clover not on PATH in non-login shells (common on RHEL-family)"
        PATH_LINE='export PATH="/usr/local/bin:$PATH"'
        PATH_COMMENT='# Clover Cognition — ensure /usr/local/bin is on PATH (RHEL non-login shells)'
    ...
    if ! echo "$PATH" | tr ':' '\n' | grep -q "^$command_link_dir$"; then
        ...
        for SHELL_CONFIG in "${SHELL_CONFIGS[@]}"; do
            if ! grep -v '^[[:space:]]*#' "$SHELL_CONFIG" 2>/dev/null | grep -qE 'PATH=.*\.local/bin'; then
                echo "" >> "$SHELL_CONFIG"
                echo "# Clover Cognition — ensure ~/.local/bin is on PATH" >> "$SHELL_CONFIG"
                echo "$PATH_LINE" >> "$SHELL_CONFIG"
        ...
        if [ "$IS_FISH" = "true" ]; then
            if ! grep -q 'fish_add_path.*\.local/bin' "$FISH_CONFIG" 2>/dev/null; then
                echo "" >> "$FISH_CONFIG"
                echo "# Clover Cognition — ensure ~/.local/bin is on PATH" >> "$FISH_CONFIG"
    ...
    # Export for current session so clover works immediately
    export PATH="$command_link_dir:$PATH"
...
copy_config_templates() {
    ...
    if [ ! -f "$CLOVER_HOME/.env" ]; then
        if [ -f "$INSTALL_DIR/.env.example" ]; then
            cp "$INSTALL_DIR/.env.example" "$CLOVER_HOME/.env"
            log_success "Created ~/.clover/.env from template"
        else
            touch "$CLOVER_HOME/.env"
            log_success "Created ~/.clover/.env"
        fi
    else
        log_info "~/.clover/.env already exists, keeping it"
    fi
    ...
    chmod 600 "$CLOVER_HOME/.env"
    configure_browser_env_from_system_browser

    # Create config.yaml at ~/.clover/config.yaml (top level, easy to find)
    if [ ! -f "$CLOVER_HOME/config.yaml" ]; then
        if [ -f "$INSTALL_DIR/cli-config.yaml.example" ]; then
            cp "$INSTALL_DIR/cli-config.yaml.example" "$CLOVER_HOME/config.yaml"
            log_success "Created ~/.clover/config.yaml from template"
        fi
    else
        log_info "~/.clover/config.yaml already exists, keeping it"
    fi

    # Create SOUL.md if it doesn't exist (global persona file).
    # This MUST match DEFAULT_SOUL_MD in clover_cli/default_soul.py — the
    ...
    if [ ! -f "$CLOVER_HOME/SOUL.md" ]; then
        cat > "$CLOVER_HOME/SOUL.md" << 'SOUL_EOF'
...
SOUL_EOF
        log_success "Created ~/.clover/SOUL.md (edit to customize personality)"
    fi

    log_success "Configuration directory ready: ~/.clover/"

    # Seed bundled skills into ~/.clover/skills/ (manifest-based, one-time per skill)
    if [ "$NO_SKILLS" = true ]; then
        # Blank-slate install: write the opt-out marker and skip seeding.
        # skills_sync.py and `clover update` both honor this marker, so the
        ...
        printf '%s\n' \
            ...
            "Delete this file to re-enable sync on the next 'clover update'." \
            > "$CLOVER_HOME/.no-bundled-skills" 2>/dev/null || true
        log_info "Skipping bundled skills (--no-skills). Wrote $CLOVER_HOME/.no-bundled-skills"
        log_info "  Future 'clover update' runs will not inject bundled skills. Delete the marker to opt back in."
    else
        log_info "Syncing bundled skills to ~/.clover/skills/ ..."
        if "$INSTALL_DIR/venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" 2>/dev/null; then
            log_success "Skills synced to ~/.clover/skills/"
        else
            # Fallback: simple directory copy if Python sync fails
            if [ -d "$INSTALL_DIR/skills" ] && [ ! "$(ls -A "$CLOVER_HOME/skills/" 2>/dev/null | grep -v '.bundled_manifest')" ]; then
                cp -r "$INSTALL_DIR/skills/"* "$CLOVER_HOME/skills/" 2>/dev/null || true
                log_success "Skills copied to ~/.clover/skills/"
            fi
        fi
    fi
}
...
strip_snap_browser_override() {
    ...
    local env_file="$CLOVER_HOME/.env"
    ...
    local tmp
    tmp="$(mktemp)" || return 0
    if grep -Ev '^AGENT_BROWSER_EXECUTABLE_PATH=/snap/|^# Clover Cognition browser tools' "$env_file" > "$tmp"; then
        mv "$tmp" "$env_file"
        log_warn "Removed stale Snap browser override (AGENT_BROWSER_EXECUTABLE_PATH=/snap/...) from $env_file"
        log_info "Clover will use the bundled Chromium instead."
...
configure_browser_env_from_system_browser() {
    ...
    {
        echo ""
        echo "# Clover Cognition browser tools — explicit browser override."
...
install_node_deps() {
    ...
    # Keep the checkout clean so `clover update` doesn't autostash every run.
    restore_dirty_lockfiles "$INSTALL_DIR"
}
...
install_browser_use_cli() {
    ...
    # `clover tools` can install it later.
    if [ "$SKIP_BROWSER" = true ]; then
    ...
    if [ -x "$CLOVER_HOME/bin/browser-use" ]; then
        log_success "Browser Use CLI already installed"
        return 0
    fi

    log_info "Installing Browser Use CLI (default browser backend)..."
    # UV_TOOL_BIN_DIR keeps the binary inside Clover' managed bin dir, where
    # the browser tool resolves it — no reliance on the user's PATH.
    if run_with_timeout 600 env UV_NO_CONFIG=1 UV_TOOL_BIN_DIR="$CLOVER_HOME/bin" \
        ...
        log_info "Install later with: $UV_CMD tool install browser-use  (or via 'clover tools')"
    fi
}
...
install_computer_use_driver() {
    ...
    # `clover tools`, the dashboard, or the desktop app — is a config flip,
    # not a surprise multi-minute binary fetch (the confusion this fixes:
    # users had to discover `clover computer-use install` on their own).
    ...
    # Same upstream installer `clover computer-use install` runs; time-boxed
    # so a stalled GitHub download can't hang the Clover install. The
    # upstream installer serializes with its own lock (600s stale window),
    # so give it a ceiling above that — matching Clover'
    ...
    if run_with_timeout 660 /bin/bash -c \
        ...
        >"$cua_log" 2>&1; then
        log_success "Computer Use driver installed (enable via 'clover tools' → Computer Use)"
    else
        log_warn "Computer Use driver install failed — it will install on demand when you enable the tool."
        log_info "Install later with: clover computer-use install"
...
run_setup_wizard() {
    ...
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Setup wizard skipped (no terminal available). Run 'clover setup' after install."
        return 0
    fi
    ...
    if [ "$USE_VENV" = true ]; then
        "$INSTALL_DIR/venv/bin/python" -m clover_cli.main setup < /dev/tty
    else
        python -m clover_cli.main setup < /dev/tty
    fi
}

maybe_start_gateway() {
    # Check if any messaging platform tokens were configured
    ENV_FILE="$CLOVER_HOME/.env"
    if [ ! -f "$ENV_FILE" ]; then
        return 0
    fi
    ...
    log_info "The gateway needs to be running for Clover to send/receive messages."

    # If WhatsApp is enabled and no session exists yet, run foreground first for QR scan
    WHATSAPP_VAL=$(grep "^WHATSAPP_ENABLED=" "$ENV_FILE" 2>/dev/null | cut -d'=' -f2-)
    WHATSAPP_SESSION="$CLOVER_HOME/whatsapp/session/creds.json"
    if [ "$WHATSAPP_VAL" = "true" ] && [ ! -f "$WHATSAPP_SESSION" ]; then
        if [ "$IS_INTERACTIVE" = true ]; then
            echo ""
            log_info "WhatsApp is enabled but not yet paired."
            log_info "Running 'clover whatsapp' to pair via QR code..."
            echo ""
            if prompt_yes_no "Pair WhatsApp now?" "yes"; then
                CLOVER_CMD="$(get_clover_command_path)"
                $CLOVER_CMD whatsapp || true
            fi
        else
            log_info "WhatsApp pairing skipped (non-interactive). Run 'clover whatsapp' to pair."
        fi
    fi
    ...
    # but opening fails with ENXIO. See #16746.
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Gateway setup skipped (no terminal available). Run 'clover gateway install' later."
        return 0
    fi
    ...
    if [ "$should_install_gateway" = true ]; then
        ...
        if [ "$DISTRO" != "termux" ] && command -v systemctl &> /dev/null; then
            ...
            if $CLOVER_CMD gateway install 2>/dev/null; then
                log_success "Gateway service installed"
                if $CLOVER_CMD gateway start 2>/dev/null; then
                    log_success "Gateway started! Your bot is now online."
                else
                    log_warn "Service installed but failed to start. Try: clover gateway start"
                fi
            else
                log_warn "Systemd install failed. You can start manually: clover gateway"
            fi
        else
            if [ "$DISTRO" = "termux" ]; then
            ...
            nohup $CLOVER_CMD gateway > "$CLOVER_HOME/logs/gateway.log" 2>&1 &
            GATEWAY_PID=$!
            log_success "Gateway started (PID $GATEWAY_PID). Logs: ~/.clover/logs/gateway.log"
            log_info "To stop: kill $GATEWAY_PID"
            log_info "To restart later: clover gateway"
            if [ "$DISTRO" = "termux" ]; then
                log_warn "Android may stop background processes when Termux is suspended or the system reclaims resources."
            fi
        fi
    else
        log_info "Skipped. Start the gateway later with: clover gateway"
    fi
}

write_bootstrap_marker() {
    # Writes $INSTALL_DIR/.clover-bootstrap-complete, which tells the Clover
    ...
    local marker_path="$INSTALL_DIR/.clover-bootstrap-complete"
...
print_success() {
    ...
    echo -e "   ${YELLOW}Data:${NC}      $CLOVER_HOME/cron/, sessions/, logs/"
    ...
    echo ""
    if [ "$DISTRO" = "termux" ]; then
        echo -e "${YELLOW}⚡ 'clover' was linked into $(get_command_link_display_dir), which is already on PATH in Termux.${NC}"
        echo ""
    elif [ "$ROOT_FHS_LAYOUT" = true ]; then
...
ensure_browser() {
    if ! command -v node >/dev/null 2>&1; then
        local node_bin="$CLOVER_HOME/node/bin/node"
        if [ -x "$node_bin" ]; then
            export PATH="$CLOVER_HOME/node/bin:$PATH"
        else
    ...
    local npm_bin
    npm_bin="$(command -v npm 2>/dev/null || echo "$CLOVER_HOME/node/bin/npm")"
    if [ ! -x "$npm_bin" ]; then
    ...
    if ! run_with_timeout "$NODE_DEPS_TIMEOUT" "$npm_bin" install -g --prefix "$CLOVER_HOME/node" --silent --ignore-scripts \
...
# extract a tree MISSING the electron binary, so the `electron`->`Clover` rename
...
# _purge_electron_build_cache() used by `clover desktop`; install.sh was the only
...
install_desktop() {
    ...
    # floor (Node >=26) and prepends the Clover-managed Node to PATH, so
    ...
    #         retry (matches install.ps1 / `clover desktop`),
    ...
    local app=""
    if [ "$OS" = "linux" ]; then
        if [ -x "$desktop_dir/release/linux-unpacked/Clover" ]; then
            app="$desktop_dir/release/linux-unpacked/Clover"
        elif [ -x "$desktop_dir/release/linux-unpacked/clover" ]; then
            app="$desktop_dir/release/linux-unpacked/clover"
        fi
    else
        local cand
        for cand in \
            "$desktop_dir/release/mac-arm64/Clover.app" \
            "$desktop_dir/release/mac/Clover.app"; do
            if [ -d "$cand" ]; then
                app="$cand"
                break
            fi
    ...
    # `clover desktop`, so install/repair and self-update agree about the app's
    ...
    # importing Python cannot reverse it by loading CLOVER_HOME/.env. If the
    ...
    if [ "$OS" = "macos" ] && [ -z "${CSC_LINK:-}" ] && [ -z "${APPLE_SIGNING_IDENTITY:-}" ] && command -v codesign >/dev/null 2>&1; then
        ...
        if [ -x "$config_python" ]; then
            if CLOVER_HOME="$CLOVER_HOME" "$config_python" - "$desktop_dir" <<'PYEOF'
import sys
from pathlib import Path
from clover_cli.main import _desktop_macos_relaunchable_fixup
...
PYEOF
    ...
    # checkout stays clean for the next `clover update`.
    restore_dirty_lockfiles "$INSTALL_DIR"
}
...
run_stage_body() {
    ...
    case "$stage" in
        ...
        desktop)
            ...
            resolve_install_layout
            require_install_dir
            # Each stage runs in its own process, so the Clover-managed Node
            # provisioned during prerequisites/node-deps (at $CLOVER_HOME/node/bin)
...
main() {
    ...
    # Code-scoped stamp: write next to the install tree, not into $CLOVER_HOME.
    # $CLOVER_HOME is a shared data dir (it can be bind-mounted into a Docker
    # gateway too), so a stamp there gets clobbered by the container's 'docker'
    # stamp and wrongly blocks 'clover update' on this host install.
