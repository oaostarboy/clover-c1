#!/usr/bin/env python3
"""Live test harness for Clover Cognition's Tool Search feature.
...
# Force-isolate the test environment BEFORE any clover imports.
ORIGINAL_HOME = os.environ.get("CLOVER_HOME")
ORIGINAL_AUTH = Path.home() / ".clover" / "auth.json"
...
def setup_isolated_home(enabled: bool, listing: str = "off",
    ...
    """Create a fresh ~/.clover/ for one test, copying minimal credentials.

    Also reads OPENROUTER_API_KEY from the user's real ``~/.clover/.env`` so
    the agent can authenticate against OpenRouter inside the isolated home.
    """
    home_dir = Path(tempfile.mkdtemp(prefix="clover_ts_live_"))
    clover_home = home_dir / ".clover"
    clover_home.mkdir(parents=True)

    if ORIGINAL_AUTH.exists():
        shutil.copy(ORIGINAL_AUTH, clover_home / "auth.json")

    # Copy .env so OPENROUTER_API_KEY (or others) are visible to the agent
    # running inside the isolated home.
    real_env_file = Path.home() / ".clover" / ".env"
    if real_env_file.exists():
        shutil.copy(real_env_file, clover_home / ".env")
        ...
        load_clover_dotenv(clover_home=str(Path.home() / ".clover"))
    ...
    (clover_home / "config.yaml").write_text(_yaml_dump(cfg), encoding="utf-8")
    return clover_home
...
def reset_module_state():
    """Drop cached modules so the new CLOVER_HOME takes effect."""
    keys = [k for k in sys.modules.keys()
            if k.startswith(("tools.", "model_tools", "toolsets",
                             "clover_cli", "agent.", "run_agent"))]
    for k in keys:
        del sys.modules[k]
...
def run_one_scenario(scenario: Dict[str, Any], enabled: bool, out_dir: Path) -> Dict[str, Any]:
    ...
    os.environ["CLOVER_HOME"] = str(home)
...
def main():
    ...
    # Restore original CLOVER_HOME
    if ORIGINAL_HOME is not None:
        os.environ["CLOVER_HOME"] = ORIGINAL_HOME
    else:
