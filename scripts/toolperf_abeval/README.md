...
batch (tracker: [#77056](https://github.com/CloverCognition/clover-c1/issues/77056)).
...
- **Two arms, one variable.** `baseline` and `fixes` runs differ ONLY by
  ...
  Clover home, same model, same tasks, same reps.
...
1. Create a dedicated Clover home with credentials for the models under test:
   ...
   integration at it; no Clover observability plugin needs to be enabled.
