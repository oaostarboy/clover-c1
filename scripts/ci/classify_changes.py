...
# tree generates llms.txt — the index every LLM (Clover included, via the
# clover-c1 skill) reads to learn what Clover can do — and
...
_MCP_CATALOG_FILES = {"clover_cli/mcp_catalog.py"}
...
# Rust crates — currently just the Tauri bootstrap installer (Clover-Setup).
...
def _py_test_only(p: str) -> bool:
    ...
    Product jobs (Desktop E2E's ``clover serve`` backend, the Docker image)
