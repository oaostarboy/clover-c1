# Behavioral test for install.ps1's clover launcher staging (PR #92092,
...
#
# The test lifts the real Install-CloverCommandLaunchers function from the
...
# the caller (Set-PathVariable passes $CloverHome\bin -- the managed binary
...
$fn = $ast.Find({
    ...
    $n.Name -eq 'Install-CloverCommandLaunchers'
}, $true)

if (-not $fn) {
    throw "Install-CloverCommandLaunchers not found in $installPs1"
}
...
$caseRoot = [System.IO.Path]::GetFullPath((Join-Path $tempBase (
    'clover-cli-launcher-test-' + [guid]::NewGuid().ToString('N')
)))
...
try {
    $installRoot = Join-Path $caseRoot 'clover-c1'
    ...
    $missingThrew = $false
    try {
        Install-CloverCommandLaunchers -Root $installRoot -Destination $binDir | Out-Null
    } catch {
        $missingThrew = $_.Exception.Message -like '*required launcher not found*'
    }
    Assert-True $missingThrew 'missing clover.exe fails the launcher stage'
    ...
    $cloverV1 = [byte[]](77, 90, 1)
    $cloverV2 = [byte[]](77, 90, 2)
    $acp = [byte[]](77, 90, 3)
    [System.IO.File]::WriteAllBytes((Join-Path $scriptsDir 'clover.exe'), $cloverV1)
    Set-Content -Path (Join-Path $installRoot 'venv\pyvenv.cfg') `
        -Value "home = X" -Encoding Ascii

    $staged = Install-CloverCommandLaunchers -Root $installRoot -Destination $binDir
    Assert-True ($staged -eq $binDir) 'returns the destination it staged into'
    Assert-BytesEqual $cloverV1 `
        ([System.IO.File]::ReadAllBytes((Join-Path $binDir 'clover.exe'))) `
        'normal venv: exe copy lands in the destination'
    Assert-True (-not (Test-Path -LiteralPath (Join-Path $binDir 'clover-acp.exe'))) `
        'optional ACP launcher may be absent'

    [System.IO.File]::WriteAllBytes((Join-Path $scriptsDir 'clover.exe'), $cloverV2)
    [System.IO.File]::WriteAllBytes((Join-Path $scriptsDir 'clover-acp.exe'), $acp)
    Install-CloverCommandLaunchers -Root $installRoot -Destination $binDir | Out-Null
    Assert-BytesEqual $cloverV2 `
        ([System.IO.File]::ReadAllBytes((Join-Path $binDir 'clover.exe'))) `
        'installer refreshes an existing Clover launcher'
    Assert-BytesEqual $acp `
        ([System.IO.File]::ReadAllBytes((Join-Path $binDir 'clover-acp.exe'))) `
    ...
    Install-CloverCommandLaunchers -Root $installRoot -Destination $binDir | Out-Null
    Assert-True (Test-Path -LiteralPath (Join-Path $binDir 'clover.cmd')) `
        'relocatable venv: .cmd delegator staged'
    Assert-True (-not (Test-Path -LiteralPath (Join-Path $binDir 'clover.exe'))) `
        'relocatable venv: stale exe copy removed'
    $cmdBody = [System.IO.File]::ReadAllText((Join-Path $binDir 'clover.cmd'))
    Assert-True ($cmdBody.Contains((Join-Path $scriptsDir 'clover.exe')) -and $cmdBody.Contains('%*')) `
