    ...
    if ($htmlPath -and $edge) {
        ...
        if ($server) {
            try {
                ...
                $edgeProfile = Join-Path $TempDir ("clover-update-ui-{0}" -f $PID)
                $edgeArgs = @(
...
function Invoke-CloverStep([string]$Exe, [string[]]$CloverArgs, [string]$Tag) {
    ...
    $psi.Arguments = ($CloverArgs | ForEach-Object { '"{0}"' -f ($_ -replace '"', '\"') }) -join ' '
    $psi.UseShellExecute = $false
    $psi.RedirectStandardOutput = $true
    $psi.RedirectStandardError = $true
    # clover update prints UTF-8 (checkmarks, arrows, box glyphs). PS 5.1
