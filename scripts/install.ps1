# ============================================================================
# Clover Cognition Installer for Windows
# ============================================================================
...
param(
    ...
    [switch]$ForceCommit,
    [string]$Tag = "",
    [string]$CloverHome = $(if ($env:CLOVER_HOME) { $env:CLOVER_HOME } else { "$env:LOCALAPPDATA\clover" }),
    [string]$InstallDir = $(if ($env:CLOVER_HOME) { "$env:CLOVER_HOME\clover-c1" } else { "$env:LOCALAPPDATA\clover\clover-c1" }),
    ...
    # builds apps/desktop into a launchable Clover.exe.
    #
    # Why opt-in:
    #   * Clover-Setup.exe (the signed Tauri bootstrap installer) passes
    ...
    #     from inside an already-launched Clover.exe; if THAT recursively
    #     built apps/desktop it would try to overwrite the live Clover.exe
    ...
    #     `clover desktop` already builds on demand.
...
# CLOVER_HOME and InstallDir -- in that short form:
...
function Write-PathDiag {
    ...
    [Console]::Error.WriteLine("[clover] $Message")
}
...
function ConvertTo-LongPath {
    ...
    try {
        if (-not ([System.Management.Automation.PSTypeName]'CloverInstall.LongPath').Type) {
            Add-Type -Namespace 'CloverInstall' -Name 'LongPath' -MemberDefinition @'
...
'@
        ...
        $length = [CloverInstall.LongPath]::GetLongPathNameW($Path, $buffer, $buffer.Capacity)
...
# long. An explicitly passed -CloverHome / -InstallDir is normalized in place
...
if ($PSBoundParameters.ContainsKey('CloverHome')) {
    $CloverHome = ConvertTo-LongPath $CloverHome
} else {
    $CloverHome = ConvertTo-LongPath $(
        if ($env:CLOVER_HOME) { $env:CLOVER_HOME } else { "$env:LOCALAPPDATA\clover" }
    )
}
if ($PSBoundParameters.ContainsKey('InstallDir')) {
    ...
    $InstallDir = ConvertTo-LongPath $(
        if ($env:CLOVER_HOME) { "$env:CLOVER_HOME\clover-c1" } else { "$env:LOCALAPPDATA\clover\clover-c1" }
    )
}
if ($script:NormalizedProfilePaths) {
    ...
    Write-PathDiag "resolved install paths: CloverHome=$CloverHome InstallDir=$InstallDir"
}
...
function Write-Banner {
    ...
    Write-Host "|             * Clover Cognition Installer                    |" -ForegroundColor Magenta
    Write-Host "+---------------------------------------------------------+" -ForegroundColor Magenta
    Write-Host "|  An open source AI agent by Anthony Nguyen.              |" -ForegroundColor Magenta
    Write-Host "+---------------------------------------------------------+" -ForegroundColor Magenta
    Write-Host ""
}
...
function Write-BrowserEnv {
    ...
    if (-not (Test-Path $CloverHome)) {
        New-Item -ItemType Directory -Force -Path $CloverHome | Out-Null
    }
    $envFile = Join-Path $CloverHome ".env"
...
function Install-AgentBrowser {
    ...
    # which every consumer (tools/browser_tool.py, `clover update`'s npx
    ...
    $prefixDir = Join-Path $CloverHome "node"
...
function Install-Uv {
    # Clover owns its own uv at $CloverHome\bin\uv.exe.  Always install there --
    # no PATH probing, no conda guards, no multi-location resolution chains.
    # The runtime update path (clover_cli/managed_uv.py) looks in the same
    # place, so install.ps1 and `clover update` stay in sync.
    $managedUv = Join-Path $CloverHome "bin\uv.exe"
    ...
    Write-Info "Installing managed uv into $CloverHome\bin ..."
    New-Item -ItemType Directory -Path (Join-Path $CloverHome "bin") -Force | Out-Null

    # UV_INSTALL_DIR tells the astral installer to place the binary
    # directly into $CloverHome\bin instead of ~/.local/bin.
    $prevEAP = $ErrorActionPreference
    try {
        $ErrorActionPreference = "Continue"
        $env:UV_INSTALL_DIR = Join-Path $CloverHome "bin"
        ...
        # (clover_cli/managed_uv.py looks only at $CloverHome\bin\uv.exe).
...
# Put the Clover-managed Node dir at the FRONT of the persisted User PATH.
#
...
# environment (a standalone clover-setup.exe run, a user typing `npm`) silently
...
# Upgrade the Clover-managed Node tree's bundled npm into $NpmRange when
...
function Update-ManagedNpm {
    ...
    if (Test-ManagedNodeInUse $NodeDir) {
        Write-Warn "Clover-managed Node.js is in use by a running app; skipping the bundled npm upgrade (applies on a later update with the app closed)."
        return $false
    }

    Write-Info "Upgrading bundled npm to satisfy $range ..."

    $tmpCwd = Join-Path $env:TEMP ("clover-npm-upgrade-" + [Guid]::NewGuid().ToString("N"))
...
function Resolve-UvCmd {
    ...
    # sibling process and CLOVER_HOME wasn't propagated).
...
function Resolve-AvailablePythonVersion {
    ...
    # ``$script:PythonVersion = $fallbackVer`` mutation.  Under Clover-Setup.exe
...
function New-GitBashAslrFailureReason {
    ...
    return @(
        ...
        "Then rerun Clover setup. If the override is blocked or later re-applied, ask your Windows administrator to allow this per-program exception."
...
function Install-Git {
    ...
    which Clover uses to run shell commands.
    ...
    package manager):
      ...
      2. Download **PortableGit** from the official git-for-windows GitHub
         ...
         ``%LOCALAPPDATA%\clover\git`` -- never touches system Git, never
    ...
    **Why PortableGit, not MinGit:**  MinGit is the minimal-automation
    ...
    Clover needs ``bash.exe`` to run shell commands.  PortableGit is the
    ...
    blocked).  Owning the Clover copy of Git ourselves is predictable and
    recoverable: if it ever breaks, ``Remove-Item %LOCALAPPDATA%\clover\git``
    and re-running this installer fully recovers.

    After install we locate ``bash.exe`` and persist the path in
    ``CLOVER_GIT_BASH_PATH`` (User scope) so Clover can find it in a fresh
    ...
    if (Get-Command git -ErrorAction SilentlyContinue) {
        ...
        Write-Info "Trying a Clover-managed PortableGit install instead..."
    }

    # Download PortableGit into $CloverHome\git.  Always works as long as
    # we can reach github.com -- no admin, no winget, no reliance on the
    # user's possibly-broken system Git install.
    Write-Info "Git not found -- downloading PortableGit to $CloverHome\git\ ..."
    ...
    try {
        ...
        if ($arch -eq "32-bit-mingit") {
            Write-Warn "32-bit Windows detected -- PortableGit is 64-bit only.  Installing MinGit 32-bit as a last resort; bash-dependent Clover features (terminal tool, agent-browser) will not work on this machine."
        ...
        Write-Info "then re-run this installer.  Clover needs Git Bash on Windows to run"
...
function Set-GitBashEnvVar {
    ...
    ``CLOVER_GIT_BASH_PATH`` (User env scope) so Clover can find it even before
    ...
    #
    # Layouts:
    #   PortableGit (our default): $CloverHome\git\bin\bash.exe
    #   MinGit (32-bit fallback):  $CloverHome\git\usr\bin\bash.exe
    $candidates += "$CloverHome\git\bin\bash.exe"       # PortableGit layout (primary)
    $candidates += "$CloverHome\git\usr\bin\bash.exe"   # MinGit / PortableGit usr\bin fallback
    ...
    foreach ($candidate in $candidates) {
        if ($candidate -and (Test-Path $candidate)) {
            [Environment]::SetEnvironmentVariable("CLOVER_GIT_BASH_PATH", $candidate, "User")
            $env:CLOVER_GIT_BASH_PATH = $candidate
            $script:GitBashPath = $candidate
            Write-Info "Set CLOVER_GIT_BASH_PATH=$candidate"
            return
        }
    }

    Write-Warn "Could not locate bash.exe -- Clover may not find Git Bash."
...
# range used to provision the Clover-managed tree. Keeping this probe separate
...
function Test-SystemNodeReady {
    ...
    if ($npmVersion) {
        Write-Warn "Node.js $version uses npm $npmVersion, which does not satisfy Clover requirement $npmRange"
    } else {
...
function Test-Node {
    ...
    Write-Info "Using a Clover-managed Node.js installation instead..."

    # Prefer a Clover-managed Node from a previous run over a too-old system one.
    $managedNode = "$CloverHome\node\node.exe"
    if ((Test-Path $managedNode) -and (Test-NodeVersionOk (& $managedNode --version))) {
        $version = & $managedNode --version
        $env:Path = "$CloverHome\node;$env:Path"
        Set-ManagedNodeFirstOnUserPath "$CloverHome\node"
        Write-Success "Node.js $version found (Clover-managed)"
        ...
        Update-ManagedNpm "$CloverHome\node" | Out-Null
        $script:HasNode = $true
        return $true
    }

    Write-Info "Installing Clover-managed Node.js $NodeVersion LTS..."
    ...
    # The portable zip path drops node.exe + npm into $CloverHome\node\
    ...
    Write-Info "Downloading portable Node.js $NodeVersion to $CloverHome\node\ ..."
    ...
    try {
        ...
        if ($zipName) {
            ...
            $tmpDir = "$env:TEMP\clover-node-extract"
            ...
            if ($extractedDir) {
                ...
                Get-ChildItem "$CloverHome" -Directory -Filter "node.old-*" -ErrorAction SilentlyContinue |
                    Where-Object { $_.LastWriteTime -lt (Get-Date).AddMinutes(-10) } |
                    Remove-Item -Recurse -Force -ErrorAction SilentlyContinue
                Get-ChildItem "$CloverHome" -Directory -Filter "node.new-*" -ErrorAction SilentlyContinue |
                    Where-Object { $_.LastWriteTime -lt (Get-Date).AddMinutes(-10) } |
                    Remove-Item -Recurse -Force -ErrorAction SilentlyContinue
                ...
                if (Test-Path "$CloverHome\node") {
                    try {
                        Rename-Item "$CloverHome\node" $backup -ErrorAction Stop
                    } catch {
                        Write-Warn "Clover-managed Node.js is in use by a running app; deferring its upgrade. Close the app and re-run the update."
                ...
                $version = & "$CloverHome\node\node.exe" --version
                Write-Success "Node.js $version installed to $CloverHome\node\ (portable, user-scoped)"
...
function Install-SystemPackages {
    ...
    if ($hasWinget) {
        ...
        foreach ($pkg in $wingetPkgs) {
            $log = "$env:TEMP\clover-winget-$($pkg -replace '[^A-Za-z0-9]','_')-$(Get-Random).log"
            $pkgLogs[$pkg] = $log
...
function Install-Repository {
        ...
        if ($repoValid) {
            ...
            try {
                ...
                if (-not [string]::IsNullOrWhiteSpace(($statusOut -join "`n"))) {
                    ...
                    # the index conflict state is dropped. Mirrors the `clover
                    # update` path (#4735).
                    ...
                    $stashName = "clover-install-autostash-" + (Get-Date -Format "yyyyMMdd-HHmmss")
                ...
                if ($Commit) {
                    ...
                    # BACKWARDS. clover-setup.exe bakes its build-time commit
                    ...
                    # ff-only pull cannot succeed -- mirror ``clover update`` and
                ...
                if ($autostashRef) {
                    ...
                    if ($restoreNow) {
                        ...
                        if (($restoreExit -eq 0) -and ($conflictedFiles.Count -eq 0)) {
                            ...
                            Write-Warn "Review git diff / git status if Clover behaves unexpectedly."
                        } else {
            ...
            try {
                ...
                Write-Info "terminals, running clover processes) and try again."
                throw
            }
        }
    }
    ...
    if (-not $didUpdate) {
        ...
        if (-not $cloneSuccess) {
            ...
            try {
                ...
                $extractPath = "$env:TEMP\clover-c1-extract"
                ...
                if ($extractedDir) {
                    ...
                    # `clover update` (see the notes at the shared clone-path
    ...
    # next `clover update` checkout aborts on a "dirty" tree the user never
...
function Install-Venv {
    ...
    # Re-resolve the interpreter before creating the venv.  Under Clover-Setup.exe
    ...
    if (Test-Path -LiteralPath "venv") {
        ...
        # speedups.pyd) are loaded as DLLs by any running clover process.
        ...
        if ($env:OS -eq "Windows_NT") {
            $myPid = $PID
            Write-Info "Stopping any running clover processes before recreating venv..."
            ...
            try {
                schtasks /Query /FO CSV 2>$null | ConvertFrom-Csv | Where-Object { $_.TaskName -like '*Clover_Gateway*' } | ForEach-Object {
                    $tn = $_.TaskName
            ...
            # The launcher CLI (clover.exe) plus its child tree.
            & taskkill /F /T /IM clover.exe /FI "PID ne $myPid" 2>$null | Out-Null
            # taskkill /IM clover.exe is NOT enough: the gateway/agent that a
            # scheduled task or watchdog autostarts runs as
            # `pythonw.exe -m clover_cli.main gateway run` straight out of
            # venv\Scripts\, so its image name is python/pythonw, not clover.exe.
            ...
            # Clover children re-exec through .clover-runtime, so killing only
            ...
            # user (see clover_cli/gateway_windows.py), so the installer always
        ...
        try {
            ...
            throw (
                ...
                "A process still has the install directory open (often a non-Clover " +
    ...
    $env:UV_PYTHON = $venvPythonExe
        ...
        # the task fires normally on next logon and `clover update` / the
...
function Restore-VenvBackup {
    ...
    # venv so Clover (and the venv-blocker probe) stay usable (#83149).
    $backupName = Get-PendingVenvBackup
    if (-not $backupName) { return }
    try {
...
function Install-Dependencies {
    ...
    if (Test-Path "uv.lock") {
        ...
        # empty and producing the broken state where `clover.exe` exists
...
"@ 2>$null
    ...
    if (-not $installed) {
        throw "Failed to install clover-c1 package even with no extras. Inspect the uv pip install output above."
    }
    ...
    # isn't set, leaving venv\ empty and clover.exe broken with
    ...
    # here, not 30 seconds later when the user runs `clover`.
    if (-not $NoVenv) {
        $venvPython = "$InstallDir\venv\Scripts\python.exe"
        if (-not (Test-Path $venvPython)) {
            throw "Install reported success but $venvPython does not exist. The dependency sync likely landed in a sibling .venv\ directory. Re-run the installer; if it persists, close Clover processes and preserve existing venv directories before retrying. Do not delete venv in place."
        }
        ...
        if ($importExitCode -ne 0) {
            ...
            $hint = if (Test-Path $sibling) {
                "Detected sibling .venv\ at $sibling -- uv synced there instead of venv\. Close Clover processes, preserve the existing venv, and rerun the installer so the transactional recovery path can move directories safely."
            } else {
    ...
    Complete-VenvTransaction
        ...
        # a failed update leaves Clover and its blocker probe usable.
        Restore-VenvBackup
        Pop-Location
        throw
    }
    ...
    if (-not $NoVenv) {
        # uv on Windows can register clover.exe in dist-info/RECORD but fail to
        ...
        # `clover` command while clover-c1.exe / clover-acp.exe exist
...
"@ 2>$null
            if ($LASTEXITCODE -eq 0 -and $scriptNames) {
                ...
                if ($missing.Count -gt 0) {
                    ...
                    if ($stillMissing.Count -gt 0) {
                        ...
                        Write-Info "Workaround: `"$pythonExe`" -m clover_cli.main <command>"
                    } else {
    ...
    # users hit and lazy-import errors from `clover dashboard` are confusing.
    ...
    if (Test-Path $pythonExe) {
        ...
        try {
            & $pythonExe -m py_compile "$InstallDir\clover_cli\web_server.py" 2>&1 | Out-Null
            if ($LASTEXITCODE -eq 0) { $webServerSyntaxOk = $true }
        } catch { }
        $ErrorActionPreference = $prevEAP
        if (-not $webOk) {
            Write-Warn "fastapi/uvicorn not importable -- `clover dashboard` will not work."
            Write-Info "Attempting targeted install of [web] extra as last resort..."
            & $UvCmd pip install -e ".[web]"
            if ($LASTEXITCODE -eq 0) {
                Write-Success "[web] extra installed; `clover dashboard` should now work."
            } else {
        ...
        if (-not $webServerSyntaxOk) {
            throw "dashboard backend source failed syntax check: clover_cli/web_server.py"
        }
    }
...
function Install-CloverCommandLaunchers {
    param(
        [Parameter(Mandatory=$true)] [string]$Root,
        [Parameter(Mandatory=$true)] [string]$Destination
    )

    # Expose ONLY the clover launchers on PATH -- never the whole
    # venv\Scripts directory, which contains python.exe / pip.exe and
    # silently hijacks the `python` command in every terminal (#83797).
    # Requiring clover.exe before creating the destination keeps the PATH
    # stage from reporting success with an unusable command (PR #92092).
    $scriptsDir = Join-Path $Root "venv\Scripts"
    $requiredSource = Join-Path $scriptsDir "clover.exe"
    if (-not (Test-Path -LiteralPath $requiredSource -PathType Leaf)) {
        throw "Cannot set up the clover command: required launcher not found: $requiredSource"
    }

    New-Item -ItemType Directory -Force -Path $Destination | Out-Null

    # Launcher form depends on the venv (keep in lockstep with
    # clover_cli/_install_repair.py): a normal venv's exe trampoline
    ...
    foreach ($launcher in @("clover", "clover-acp")) {
    ...
    $requiredExe = Join-Path $Destination "clover.exe"
    $requiredCmd = Join-Path $Destination "clover.cmd"
    if (-not ((Test-Path -LiteralPath $requiredExe -PathType Leaf) -or
              (Test-Path -LiteralPath $requiredCmd -PathType Leaf))) {
        throw "Cannot set up the clover command: launcher was not installed: $requiredExe"
    }
    return $Destination
}

function Set-PathVariable {
    Write-Info "Setting up clover command..."
    
    if ($NoVenv) {
        $cloverBin = "$InstallDir"
    } else {
        # $CloverHome\bin is the managed binary dir (shared with the managed
        # uv), OUTSIDE the git checkout: `clover update`'s autostash
        ...
        # installer staged under clover-c1\bin. No git operation can ever
        # touch this dir. Staging and verification live in
        # Install-CloverCommandLaunchers, which throws BEFORE any PATH
        # mutation when the launchers cannot be staged.
        $cloverBin = "$CloverHome\bin"
        Install-CloverCommandLaunchers -Root $InstallDir -Destination $cloverBin | Out-Null
    }
    ...
    #   clover-c1\bin -- lived inside the git checkout, where the update
    #                       autostash could sweep the launchers off disk
    # The clover-c1\bin FILES are left in place on purpose: editor/ACP
    ...
    if (-not $NoVenv) {
        ...
        if ($cleaned.Count -ne $items.Count) {
            ...
            Write-Info "Removed legacy launcher entries from user PATH (kept clover via $cloverBin)"
        }
    }
    
    if ($currentPath -notlike "*$cloverBin*") {
        [Environment]::SetEnvironmentVariable(
            "Path",
            "$cloverBin;$currentPath",
            "User"
        )
        Write-Success "Added to user PATH: $cloverBin"
    } else {
        Write-Info "PATH already configured"
    }
    
    # Set CLOVER_HOME so the Python code finds config/data in the right place.
    # Only needed on Windows where we install to %LOCALAPPDATA%\clover instead
    # of the Unix default ~/.clover
    $currentCloverHome = [Environment]::GetEnvironmentVariable("CLOVER_HOME", "User")
    if (-not $currentCloverHome -or $currentCloverHome -ne $CloverHome) {
        [Environment]::SetEnvironmentVariable("CLOVER_HOME", $CloverHome, "User")
        Write-Success "Set CLOVER_HOME=$CloverHome"
    }
    $env:CLOVER_HOME = $CloverHome
    
    # Update current session
    $env:Path = "$cloverBin;$env:Path"
    
    Write-Success "clover command ready"
}

function Write-BootstrapMarker {
    # Writes $InstallDir\.clover-bootstrap-complete which tells the Clover
    ...
    # Clover-Setup.exe) or fall back to whatever git resolves in the
    # checkout. The desktop validates schemaVersion + pinnedCommit
    # length but doesn't enforce that HEAD matches the pin (users
    # update via `clover update` which moves HEAD legitimately).
    ...
    $markerPath = Join-Path $InstallDir ".clover-bootstrap-complete"
    $marker = [ordered]@{
        schemaVersion = 1
...
function Copy-ConfigTemplates {
    ...
    # Create the CLOVER_HOME directory structure ($CloverHome, default %LOCALAPPDATA%\clover)
    New-Item -ItemType Directory -Force -Path "$CloverHome\cron" | Out-Null
    New-Item -ItemType Directory -Force -Path "$CloverHome\sessions" | Out-Null
    New-Item -ItemType Directory -Force -Path "$CloverHome\logs" | Out-Null
    New-Item -ItemType Directory -Force -Path "$CloverHome\pairing" | Out-Null
    New-Item -ItemType Directory -Force -Path "$CloverHome\hooks" | Out-Null
    New-Item -ItemType Directory -Force -Path "$CloverHome\image_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$CloverHome\audio_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$CloverHome\memories" | Out-Null
    New-Item -ItemType Directory -Force -Path "$CloverHome\skills" | Out-Null

    
    # Create .env
    $envPath = "$CloverHome\.env"
    ...
    # Create config.yaml
    $configPath = "$CloverHome\config.yaml"
    ...
    # (the default PS5 behaviour), and Clover's prompt-injection scanner
    ...
    # PowerShell version.
    $soulPath = "$CloverHome\SOUL.md"
    if (-not (Test-Path $soulPath)) {
        # MUST match DEFAULT_SOUL_MD in clover_cli/default_soul.py. The runtime
...
"@
    ...
    Write-Success "Configuration directory ready: $CloverHome"
    
    # Seed bundled skills into $CloverHome\skills (manifest-based, one-time per skill)
    Write-Info "Syncing bundled skills to $CloverHome\skills ..."
    ...
    if (Test-Path $pythonExe) {
        try {
            ...
            Write-Success "Skills synced to $CloverHome\skills"
        } catch {
            # Fallback: simple directory copy
            $bundledSkills = "$InstallDir\skills"
            $userSkills = "$CloverHome\skills"
            if ((Test-Path $bundledSkills) -and -not (Get-ChildItem $userSkills -Exclude '.bundled_manifest' -ErrorAction SilentlyContinue)) {
                Copy-Item -Path "$bundledSkills\*" -Destination $userSkills -Recurse -Force -ErrorAction SilentlyContinue
                Write-Success "Skills copied to $CloverHome\skills"
            }
        }
    }
}
...
function Install-NodeDeps {
    if (-not $HasNode) {
        # Cross-process driver mode (Clover-Setup.exe runs each -Stage NAME
    ...
    if (-not $npmCmd) {
        ...
        Write-Info "Open a new PowerShell window and re-run 'clover setup tools' later."
        return
    }
    $npmExe = $npmCmd.Source
    if ($npmExe -like "*.ps1") {
    ...
    if (Test-Path "$InstallDir\package.json") {
        ...
        $browserLog = "$env:TEMP\clover-npm-browser-$(Get-Random).log"
        ...
        if ($browserNpmOk) {
            ...
            if (-not $npxExe) {
                ...
                $pwLog = "$env:TEMP\clover-playwright-install-$(Get-Random).log"
                Push-Location $InstallDir
    ...
    if (Test-Path "$tuiDir\package.json") {
        ...
        $tuiLog = "$env:TEMP\clover-npm-tui-$(Get-Random).log"
...
# failure is non-fatal (browser_exec can still run via uvx, and `clover tools`
...
function Install-BrowserUseCli {
    ...
    $managedBin = Join-Path $CloverHome "bin"
    $managedBu = Join-Path $managedBin "browser-use.exe"
    # MANAGED-FIRST: only Clover' managed copy short-circuits. A browser-use
    ...
    $ErrorActionPreference = "Continue"
    try {
        # UV_TOOL_BIN_DIR keeps the binary inside Clover' managed bin dir,
        ...
        if ($LASTEXITCODE -eq 0) {
            ...
            Write-Info "Install later with: uv tool install browser-use  (or via 'clover tools')"
        }
    } catch {
...
# Provision it at install time so enabling the tool later -- via `clover
...
# paths still lazy-install via install_cua_driver() (clover_cli/tools_config)
...
function Install-CuaDriver {
    ...
    $ErrorActionPreference = "Continue"
    try {
        # Same upstream installer `clover computer-use install` runs. Bounded
        ...
        # Clover' _CUA_INSTALLER_TIMEOUT (660s).
        ...
        if (Wait-Job $job -Timeout 660) {
            ...
            if ($installedCuaDriver -and (Test-CuaDriverRuntimeContract -DriverPath $installedCuaDriver.Source)) {
                Write-Success "Computer Use driver installed (enable via 'clover tools' -> Computer Use)"
            } else {
                Write-Warn "Computer Use driver install did not produce a compatible runtime -- repair it before enabling the tool."
                Write-Info "Install later with: clover computer-use install"
            }
        } else {
...
# the final `electron` -> `Clover` rename dies with ENOENT and every re-run
...
function Install-DesktopVoiceDeps {
    ...
    # leaving them to lazy first-use install. Policy change (the maintainer, July
...
function Install-Desktop {
    # Build apps/desktop into a launchable Clover.exe. Only called from
    ...
    #
    # The Tauri bootstrap installer's launch_clover_desktop command
    # resolves apps/desktop/release/win-unpacked/Clover.exe directly,
    ...
    # enforces the supported Node lines and prepends the Clover-managed Node to
    ...
    # The --dir mode produces an unpacked Clover.exe in
    ...
    # are NOT trying to work around). The Clover icon + product name are
    # stamped onto Clover.exe by our own rcedit step (Set-DesktopExeIdentity)
    ...
    $buildLog = "$env:TEMP\clover-desktop-build-$(Get-Random).log"
    ...
    $exeCandidates = @(
        "$desktopDir\release\win-unpacked\Clover.exe",
        "$desktopDir\release\win-arm64-unpacked\Clover.exe"
    )
    $found = $false
    $desktopExe = $null
    ...
    if (-not $found) {
        throw "Desktop build completed but no Clover.exe was found under $desktopDir\release\*-unpacked\"
    }

    # 3b. The Clover icon + identity are stamped onto Clover.exe by the
    ...
    #     SIDs under %LOCALAPPDATA% (electron/electron#51761, clover-c1#38216).
    ...
    #    Clover.exe. We deliberately do NOT point them at `clover desktop`: that
...
function New-DesktopShortcuts {
    ...
    try {
        ...
        $targets = @(
            (Join-Path ([Environment]::GetFolderPath('Programs')) 'Clover.lnk'),
            (Join-Path ([Environment]::GetFolderPath('Desktop')) 'Clover.lnk')
        )
        ...
        foreach ($lnkPath in $targets) {
            try {
                ...
                $sc.Description = 'Clover Cognition'
                $sc.Save()
        ...
        # with the Clover icon, but without this the shortcut can keep drawing
...
function Install-PlatformSdks {
    ...
    # ~/.clover/.env are importable.  Two problems this solves:
    #
...
function Invoke-SetupWizard {
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Skipping setup wizard (non-interactive). Configure via the GUI or 'clover setup'."
        return
    }
    ...
    # Run clover setup using the venv Python directly (no activation needed)
    if (-not $NoVenv) {
        & ".\venv\Scripts\python.exe" -m clover_cli.main setup
    } else {
        python -m clover_cli.main setup
    }
...
function Start-GatewayIfConfigured {
    ...
    $cloverCmd = "$InstallDir\venv\Scripts\clover.exe"
    if (-not (Test-Path $cloverCmd)) {
        $cloverCmd = "clover"
    }

    # If WhatsApp is enabled but not yet paired, run foreground for QR scan
    $whatsappEnabled = $content | Where-Object { $_ -match "^WHATSAPP_ENABLED=true" }
    $whatsappSession = "$CloverHome\whatsapp\session\creds.json"
    if ($whatsappEnabled -and -not (Test-Path $whatsappSession)) {
        Write-Host ""
        Write-Info "WhatsApp is enabled but not yet paired."
        Write-Info "Running 'clover whatsapp' to pair via QR code..."
        Write-Host ""
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Start the gateway later with: clover gateway"
        return
    }
    ...
    if ($response -eq "" -or $response -match "^[Yy]") {
        ...
        try {
            $logFile = "$CloverHome\logs\gateway.log"
            Start-Process -FilePath $cloverCmd -ArgumentList "gateway" `
                -RedirectStandardOutput $logFile `
                -RedirectStandardError "$CloverHome\logs\gateway-error.log" `
                -WindowStyle Hidden
            Write-Success "Gateway started! Your bot is now online."
            Write-Info "Logs: $logFile"
            Write-Info "To stop: close the gateway process from Task Manager"
        } catch {
            Write-Warn "Failed to start gateway. Run manually: clover gateway"
        }
    } else {
        Write-Info "Skipped. Start the gateway later with: clover gateway"
    }
}
...
function Write-Completion {
    ...
    Write-Host "$CloverHome\config.yaml"
    Write-Host "   API Keys:  " -NoNewline -ForegroundColor Yellow
    Write-Host "$CloverHome\.env"
    Write-Host "   Data:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$CloverHome\cron\, sessions\, logs\"
    Write-Host "   Code:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$CloverHome\clover-c1\"
    Write-Host ""
    ...
    Write-Host ""
    Write-Host "   clover              " -NoNewline -ForegroundColor Green
    Write-Host "Start chatting"
    Write-Host "   clover setup        " -NoNewline -ForegroundColor Green
    Write-Host "Configure API keys & settings"
    Write-Host "   clover config       " -NoNewline -ForegroundColor Green
    Write-Host "View/edit configuration"
    Write-Host "   clover config edit  " -NoNewline -ForegroundColor Green
    Write-Host "Open config in editor"
    Write-Host "   clover gateway      " -NoNewline -ForegroundColor Green
    Write-Host "Start messaging gateway (Telegram, Discord, etc.)"
    Write-Host "   clover update       " -NoNewline -ForegroundColor Green
...
$InstallStages = @(
    ...
    @{ Name = "repository";       Title = "Cloning Clover repository";            Category = "install";      NeedsUserInput = $false; Worker = "Stage-Repository" }
...
if ($IncludeDesktop) {
    ...
    # (Clover-Setup.exe), never via the irm|iex CLI one-liner.
    $InstallStages += @{ Name = "desktop"; Title = "Building desktop app"; Category = "install"; NeedsUserInput = $false; Worker = "Stage-Desktop" }
}
$InstallStages += @(
    @{ Name = "path";             Title = "Adding Clover to PATH";                Category = "finalize";     NeedsUserInput = $false; Worker = "Stage-Path" }
