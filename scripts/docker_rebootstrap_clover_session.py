#!/usr/bin/env python3
"""Boot-time re-seed of a terminally-dead Clover bootstrap session.

Background
----------
A Clover bootstrap session (client_id ``clover-cli-vps``) can take a terminal
``invalid_grant`` and be quarantined locally — the refresh path clears the dead
tokens from ``auth.json`` and stamps
``providers.clover.last_auth_error.relogin_required = true``. From then on every
...
``stage2-hook.sh`` seeds ``auth.json`` from ``CLOVER_AUTH_JSON_BOOTSTRAP`` only
...
``CLOVER_AUTH_JSON_REBOOTSTRAP`` (plus a restart). On boot we re-seed the Clover
...
Design constraints
------------------
- Pure stdlib, no clover_cli imports: runs early in the boot hook, before the
  app venv/modules are guaranteed importable, as its own subprocess.
- Surgical: replaces ONLY ``providers.clover`` in the existing auth.json, leaving
...
# from CLOVER_AUTH_JSON_BOOTSTRAP (create-only, blank-volume seed) so the two
# paths can never be confused: BOOTSTRAP seeds a fresh volume; REBOOTSTRAP
# overwrites a terminally-dead Clover entry on an existing volume.
REBOOTSTRAP_ENV = "CLOVER_AUTH_JSON_REBOOTSTRAP"
BOOTSTRAP_CLIENT_ID = "clover-cli-vps"


def _clover_entry_is_terminal(clover_state: Any) -> bool:
    """True iff the on-disk Clover provider entry is in the terminal/quarantined
    state AND holds no usable credential.

    Mirrors the ``terminal`` predicate in ``clover_cli.auth.get_clover_session_validity``:
    ...
    """
    if not isinstance(clover_state, dict):
        return False
    last_err = clover_state.get("last_auth_error")
    ...
    if clover_state.get("access_token") or clover_state.get("refresh_token"):
        return False
    return True


def _extract_clover_from_seed(seed_raw: str) -> Optional[dict]:
    """Pull the ``providers.clover`` block out of a CLOVER_AUTH_JSON_REBOOTSTRAP
    payload. The payload is a full auth.json document (same shape as
    CLOVER_AUTH_JSON_BOOTSTRAP). Returns None unless it carries the expected VPS
    ...
    if not isinstance(clover, dict) or not clover:
        return None
    if clover.get("client_id") != BOOTSTRAP_CLIENT_ID:
        return None
    if not (
        isinstance(clover.get("access_token"), str)
        and clover["access_token"].strip()
        and isinstance(clover.get("refresh_token"), str)
        and clover["refresh_token"].strip()
    ):
        return None
    return clover
...
def _seed_is_newer(local_clover: Any, seed_clover: dict) -> bool:
    ...
    """
    if not isinstance(local_clover, dict):
        return False
    local_obtained = _parse_timestamp(local_clover.get("obtained_at"))
    seed_obtained = _parse_timestamp(seed_clover.get("obtained_at"))
    return bool(
...
def reseed_if_terminal(auth_path: str, seed_raw: str) -> str:
    """Core logic. Returns a short status string for logging/testing:
      ...
      - "bad_seed"         — seed present but unparseable / no clover entry
      - "no_auth_file"     — auth.json absent (blank volume → let the normal
                             CLOVER_AUTH_JSON_BOOTSTRAP path handle it)
    ...
    seed_clover = _extract_clover_from_seed(seed_raw)
    if seed_clover is None:
        return "bad_seed"
    ...
    local_clover = providers.get("clover")
    terminal = _clover_entry_is_terminal(local_clover)
    newer_seed = _seed_is_newer(local_clover, seed_clover)
    ...
    # Surgical replacement: swap ONLY providers.clover, preserve everything else.
    providers["clover"] = seed_clover
...
def main() -> int:
    ...
    if result == "reseeded":
        print("[rebootstrap] Clover bootstrap session was terminal; re-seeded auth.json from "
              f"{REBOOTSTRAP_ENV}")
    elif result == "reseeded_newer":
        print("[rebootstrap] Applied newer orchestrator-issued Clover bootstrap session from "
              f"{REBOOTSTRAP_ENV}")
    else:
