"""Run a real Clover CLI turn and validate the Relay shared-metrics output."""
...
def _resolve_clover_executable(clover_repo: Path) -> Path:
    for relative_path in (
        Path(".venv") / "bin" / "clover",
        Path(".venv") / "Scripts" / "clover.exe",
    ):
        candidate = clover_repo / relative_path
        if candidate.is_file():
            return candidate
    discovered = shutil.which("clover")
    if discovered:
        return Path(discovered)
    raise SystemExit(
        "Clover executable not found in the repository virtual environment "
        "or on PATH"
    )
...
def _arguments() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=Path.cwd(),
        help="Clover source checkout containing .venv/bin/clover",
    )
    parser.add_argument(
        "--relay-python",
        type=Path,
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Directory for the isolated CLOVER_HOME and captured output",
    )
...
def _validate_store(database_path: Path) -> list[dict[str, Any]]:
    ...
    if by_name["clover.client.active"] != [
        {
        ...
        raise AssertionError(
            f"Unexpected client-active counter: {by_name['clover.client.active']}"
        )
    [model] = by_name["clover.model_route.count"]
    expected_model = {
        "name": "clover.model_route.count",
        "dimensions": {
            "model": MODEL_CANARY,
            "provider": "custom",
        },
    ...
    if model != expected_model:
        raise AssertionError(
            f"Unexpected model counter: {by_name['clover.model_route.count']}"
        )
    expected_start = {
    ...
    if by_name["clover.task_run.started"] != [expected_start]:
        raise AssertionError(
            f"Unexpected task start: {by_name['clover.task_run.started']}"
        )
    [terminal] = by_name["clover.task_run.finished"]
    ...
    [tool] = by_name["clover.tool_call.count"]
    ...
    lifecycle = by_name["clover.skill.lifecycle.count"]
    expected_actions = {
        "archived",
        "created",
        "edited",
    ...
    loads = by_name["clover.skill.load.count"]
...
def _validate_packages(
    ...
    try:
        ...
        raise RuntimeError(
            "The Clover development environment requires jsonschema"
        ) from exc
    ...
    if metrics["clover.client.active"] != [
        {
            "name": "clover.client.active",
            "type": "counter",
            "dimensions": {},
        ...
        raise AssertionError(
            f"Unexpected client-active metric: {metrics['clover.client.active']}"
        )
    [model] = metrics["clover.model_route.count"]
    if model["dimensions"] != {
        "model": MODEL_CANARY,
        "provider": "custom",
    } or model["value"] != 2:
        raise AssertionError(
            f"Unexpected model metric: {metrics['clover.model_route.count']}"
        )
    [terminal] = metrics["clover.task_run.finished"]
    ...
    [tool] = metrics["clover.tool_call.count"]
    if (
        tool["dimensions"]
        != {
            "approval_outcome": "not_required",
    ...
    lifecycle = metrics["clover.skill.lifecycle.count"]
    ...
    loads = metrics["clover.skill.load.count"]
    if {
        (
...
def main() -> int:
    args = _arguments()
    clover_repo = args.clover_repo.resolve()
    relay_python = args.relay_python.resolve() if args.relay_python else None
    clover = _resolve_clover_executable(clover_repo)
    ...
    if args.output_dir:
        ...
        root = Path(tempfile.mkdtemp(prefix="clover-relay-shared-metrics-"))
    home = root / "clover-home"
    ...
    (root / "clover.stdout.txt").write_text(result.stdout, encoding="utf-8")
    (root / "clover.stderr.txt").write_text(result.stderr, encoding="utf-8")
    if result.returncode != 0:
        raise AssertionError(
            f"Clover exited with {result.returncode}\n"
            f"stdout:\n{result.stdout}\nstderr:\n{result.stderr}"
        )
    ...
    if PROMPT_CANARY not in json.dumps(request.get("messages", [])):
        raise AssertionError("Clover model request did not contain the prompt canary")
    ...
    if TOOL_CALL_CANARY not in follow_up or TOOL_RESULT_CANARY not in follow_up:
        raise AssertionError("Clover did not return the tool result to the model")
    if RESPONSE_CANARY not in result.stdout:
        raise AssertionError("Clover did not print the mock model response")
    ...
    skill_result = subprocess.run(
        [
            ...
            "-c",
            "\n".join([
                "from clover_cli.observability import relay_shared_metrics",
    ...
    package_paths, packages = _validate_packages(
        ...
        / "observability"
        / "schemas"
        / "clover.shared_metrics.v2.schema.json",
    )

    print("Clover -> NeMo Relay shared-metrics smoke test passed")
