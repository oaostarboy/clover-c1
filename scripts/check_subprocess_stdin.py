...
When Clover runs in TUI mode, the gateway child process communicates with
...
# ``get_clover_home() / "plugins"`` (user) and ``./.clover/plugins/`` (project,
# gated behind ``CLOVER_ENABLE_PROJECT_PLUGINS``) — see
# ``clover_cli/plugins.py:10-12``.  The guard only checked the bundled
...
# this script runs as a standalone subprocess — ``clover_constants`` isn't
...
def main() -> int:
    ...
    # Add repo root to sys.path so we can import clover_constants (this script
    ...
    # plugins in get_clover_home()/plugins/ and project plugins in
    # ./.clover/plugins/, where code like ori/hooks.py can spawn
    # subprocesses with inherited stdin — #67639).
    plugin_roots: list[Path] = [get_clover_home() / "plugins"]
    if os.environ.get("CLOVER_ENABLE_PROJECT_PLUGINS"):
        plugin_roots.append(Path.cwd() / ".clover" / "plugins")
