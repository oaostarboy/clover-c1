...
function Invoke-Normalization {
    ...
    try {
        ...
        # its diagnostics are all `[clover] `-prefixed, so the two separate
        ...
        $stderr = ($raw | Where-Object { $_ -like '`[clover`]*' }) -join "`n"
        $stdout = ($raw | Where-Object { $_ -notlike '`[clover`]*' }) -join "`n"
    } finally {
    ...
    return @{
        ...
        CloverHome = $(if ($paths) { $paths.clover_home } else { $null })
...
$expectedInstallDir = "$($longRoot)${sep}AppData${sep}Local" + '\clover\clover-c1'
...
$result = Invoke-Normalization -Environment @{ TEMP = $shortTemp; TMP = $shortTemp } `
    -ExtraArgs @('-InstallDir', (Join-Path $shortProfile 'custom-clover'))
Assert-Equal -Expected (Join-Path $longRoot 'custom-clover') -Actual $result.InstallDir -Label "explicit -InstallDir keeps the caller's directory, on the long root"
