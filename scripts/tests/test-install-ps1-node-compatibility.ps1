...
# changing PATH, installing software, or touching the user's Clover home.

$repoRoot = Split-Path -Parent (Split-Path -Parent (Split-Path -Parent $MyInvocation.MyCommand.Path))
$installScript = Join-Path $repoRoot 'scripts\install.ps1'
$testRoot = Join-Path $env:TEMP ("clover-node-compatibility-test-" + [Guid]::NewGuid().ToString('N'))
$CloverHome = Join-Path $testRoot 'home'
$InstallDir = Join-Path $testRoot 'missing-checkout'
. $installScript -CloverHome $CloverHome -InstallDir $InstallDir
