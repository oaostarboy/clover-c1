...
unprivileged `clover` user, giving the layout most people have —
$CLOVER_HOME/clover-c1 plus a ~/.local/bin launcher. Pass --root for the FHS
...
Environment:
  CLOVER_DEV_SANDBOX_DIR    Sandbox directory name, relative to the repo root
                            (default: .clover-sandbox).

Examples:
  # create a sandbox, install this branch as `main`, and then drop to a shell,
  # skipping `clover setup` & the browser tools for speed.
...
INSTALL_REF=""
UPSTREAM_URL="${CLOVER_DEV_SANDBOX_UPSTREAM:-https://github.com/CloverCognition/clover-c1.git}"
...
GIT_ROOT="${CLOVER_SANDBOX_SOURCE_ROOT:-$(git rev-parse --show-toplevel)}"
...
SANDBOX_DIR_NAME="${CLOVER_DEV_SANDBOX_DIR:-.clover-sandbox}"
...
if [ "$PERSISTENT" = true ]; then
  ...
  SANDBOX_ROOT="$(mktemp -d -t clover-sandbox.XXXXXX)"
...
if [ -n "$INSTALL_REF" ]; then
  ...
  UPSTREAM_REPO="$(mktemp -d -t clover-sandbox-upstream.XXXXXX)"
...
if [ "$INSTALL_SHORTCUT" = true ]; then
  mkdir -p "$SANDBOX_ROOT/root/http/clover-c1.clover-c1.local"
  if [ -n "$INSTALL_REF" ]; then
    git -C "$UPSTREAM_REPO" show "$UPSTREAM_COMMIT:scripts/install.sh" \
      > "$SANDBOX_ROOT/root/http/clover-c1.clover-c1.local/install.sh"
  else
    cp -a "$INSTALLER_PATH" "$SANDBOX_ROOT/root/http/clover-c1.clover-c1.local/install.sh"
  fi
  set -- bash -c '
    set +e
    curl -fsSL https://clover-c1.local/install.sh | bash -s -- "$@"
    install_status=$?
    if [ "$install_status" -eq 0 ] && [ -f /work/promote-main ]; then
      next_main=$(cat /work/promote-main)
      if git --git-dir=/work/repos/clover-c1.git update-ref refs/heads/main "$next_main"; then
...
# A user-level install writes the `clover` launcher to ~/.local/bin and the
# checkout to $CLOVER_HOME; both live under the sandbox HOME, which is bound
...
SOURCE_REF="$COMMIT"
SNAPSHOT_REPO=""
FAKE_REPO="$SANDBOX_ROOT/root/repos/clover-c1.git"
git -C "$SANDBOX_ROOT/root/repos" init --bare -q clover-c1.git
if [ -n "$INSTALL_REF" ]; then
...
if [ -n "$(git -C "$GIT_ROOT" status --porcelain)" ]; then
  ...
  SNAPSHOT_REPO="$(mktemp -d -t clover-sandbox-snapshot.XXXXXX)"
  git -C "$SNAPSHOT_REPO" init -q
  git -C "$SNAPSHOT_REPO" fetch -q "$GIT_ROOT" "$COMMIT"
  git -C "$SNAPSHOT_REPO" config user.name 'Clover sandbox'
...
if [ -n "$INSTALL_REF" ]; then
  git --git-dir="$FAKE_REPO" fetch -q --force "$SOURCE_REPO" \
    "$SOURCE_REF:refs/clover-sandbox/next"
