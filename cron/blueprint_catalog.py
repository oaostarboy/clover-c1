...
renders natively:
  ...
  * Docs catalog          -> a copy-paste command + a ``clover://`` deep-link
...
def blueprint_deeplink(blueprint: AutomationBlueprint, values: Optional[Dict[str, Any]] = None) -> str:
    """Build the ``clover://blueprint/<key>?slot=val`` deep-link URL."""
    ...
    return f"clover://blueprint/{quote(blueprint.key)}{qs}"
