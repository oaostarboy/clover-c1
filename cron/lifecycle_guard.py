...
``clover gateway restart`` (or ``launchctl kickstart ai.clover.gateway``
or ``systemctl restart clover-gateway``).  When the cron fires, the
...
``clover cron create`` CLI subcommand AND the agent's ``cronjob`` model
...
command identifier (``clover gateway``, ``launchctl ... clover-gateway``,
``systemctl ... clover-gateway``, ``pkill`` against the gateway) so it
...
commands and shell scripts they reference when ``_CLOVER_GATEWAY=1``. It also
rejects ``launchctl submit`` in gateway sessions because launchd treats that
primitive as a persistent KeepAlive job, not a one-shot task. ``clover gateway
...
The profile-flag form (``clover -p <profile> gateway restart|stop``, #78028)
...
_GATEWAY_LIFECYCLE_PATTERN = re.compile(
    r"(?i)"
    # Branch A: destructive `clover gateway` operations.
    # The destructive operations are restart, stop, and uninstall.
    # `start` is intentionally excluded: starting a gateway from inside a
    # gateway is benign (a no-op or "already running" error), and a
    # legitimate cron job might start a sibling profile's gateway.
    # The lookbehind (#77173): `clover` must not be a path component or a
    # word tail. Excluding `/`, word chars, `.` and `-` keeps file paths
    # with embedded spaces (`/docs/clover gateway restart-notes.md`) from
    # matching via the `/clover` tail, while every real command position
    # (start of text, whitespace, `;`/`&`/`|`, `$(`, backtick, even a
    # U+FFFD from binary-content decoding) still matches.
    r"(?:(?<![/\w.\-])clover\s+gateway\s+(?:restart|stop|uninstall)\b)"
    # Branch B: launchctl ops on a clover-gateway label. macOS launchd
    # labels look like `ai.clover.gateway` / `clover-gateway`. Requiring the
    # gateway identifier prevents blocking unrelated clover services (e.g.
    # `launchctl unload ai.clover.update-checker.plist`).
    # `submit` and `bootstrap` are included alongside the direct verbs
    # (kickstart/etc.): `launchctl submit -l ai.clover.gateway-<suffix> --
    ...
    # Branch C: systemctl ops on a clover-gateway unit.
    r"|(?:systemctl\s+(?:-\S+\s+)*(?:restart|stop|start)\b[^\n]*\bclover[.\-]?gateway)"
    # Branch D: pkill / kill targeting the clover gateway process. Both
...
# lines (e.g. `launchctl submit \` / `  -l ai.clover.gateway-... \` / `  -- ...`,
...
# selector — `clover -p <profile> gateway restart|stop` / `--profile <name>`
# / `--profile=<name>`. The selector token between `clover` and `gateway`
...
# `clover -p venus gateway restart` operates on a sibling profile's gateway
...
def _current_profile_name() -> Optional[str]:
    ...
    Prefers the explicit ``CLOVER_PROFILE_NAME`` / ``CLOVER_PROFILE`` env
    (set by the profile launcher and kanban worker spawns), falling back to
    ``clover_cli.profiles.get_active_profile_name`` (derived from
    ``CLOVER_HOME``, which the gateway process inherits from its launch
    profile). Returns ``None`` when neither source yields a name.
    """
    for env_name in ("CLOVER_PROFILE_NAME", "CLOVER_PROFILE"):
...
# Branch B only catches `launchctl <verb> ... clover[.-]?gateway` when the
...
# command — `for item in 'ai.clover.gateway-apollo:...' 'ai.clover.gateway:...';
...
# is the unexpanded variable `$label`, not the string "clover.gateway". This
...
# EXISTING job, so anchoring to the clover-gateway label is still correct —
# `test_safe_commands` requires unrelated-label ops (e.g. `launchctl unload
# ai.clover.update-checker.plist`) to stay unblocked. The fix is checking
...
_CLOVER_GATEWAY_LABEL_RE = re.compile(r"(?i)\bclover[.\-]?gateway\b")
...
def _contains_launchctl_gateway_lifecycle(normalized_text: str) -> bool:
    ...
    return bool(_LAUNCHCTL_LIFECYCLE_VERBS_RE.search(normalized_text)) and bool(
        _CLOVER_GATEWAY_LABEL_RE.search(normalized_text)
    )
...
def contains_gateway_lifecycle_command(text: str) -> bool:
    ...
    a ``clover``/``gateway`` token) — this function is the single choke
    ...
    # a runbook line "a human can run: clover gateway restart" inside such a
    ...
    if _GATEWAY_LIFECYCLE_PATTERN.search(normalized):
        return True
    # Profile-flag form (#78028): `clover -p <profile> gateway restart|stop`
    # bypasses Branch A because the selector sits between `clover` and
    ...
    if profile_match:
        ...
        if named:
            # Profile ids cannot contain quotes (clover_cli.profiles
...
# for `systemctl restart clover-gateway` in syslog, a SQL LIKE literal over a
...
# blocking `grep -r 'systemctl restart clover-gateway' .` outright. Require a
...
# to execution (`grep 'systemctl restart clover-gateway' f | sh`); never mask
# such a line.
...
def contains_launchctl_submit_command(command: str) -> bool:
    ...
    chosen by whoever writes it, so a neutral name (``ai.clover.svc-reload-tmp``)
...
def _mask_data_sink_arguments(text: str) -> str:
    ...
    ``systemctl restart clover-gateway`` from the same characters appearing
    ...
    (false positives blocking legitimate cron prompts), e.g.::
        ...
        sqlite3 db "SELECT msg FROM log WHERE msg LIKE '%systemctl restart clover-gateway%'"
...
def _references_at(
    ...
    # (e.g. `Path.home() / ".clover"`), not an executable reference.
...
def _resolve_script_path(script_path: str) -> Optional[Path]:
    ...
    under ``<CLOVER_HOME>/scripts/`` and only accepts absolute paths as-is.
    ...
    (``~/.clover/scripts/restart.sh``) but is passed as the bare name
    ...
    if raw.is_absolute():
        return raw
    try:
        return get_clover_home() / "scripts" / raw
    except (RuntimeError, OSError):
        # get_clover_home() falls back to Path.home(), which raises when
        # neither CLOVER_HOME nor HOME is resolvable (launchd/systemd
...
def check_gateway_lifecycle(
        ...
        if resolved_script is not None:
            ...
            if _is_cloud_placeholder_path(resolved_script) or _is_cloud_placeholder_path(
                ...
                raise GatewayLifecycleBlocked(
                    ...
                    "(e.g. ~/.clover/scripts/) and recreate the job."
                )
    ...
    if python_script:
        ...
        # `clover gateway restart` embedded in a .py script is still
    ...
    if unsafe:
        raise GatewayLifecycleBlocked(
            ...
            "(#30719). Run `clover gateway restart` from a shell outside "
