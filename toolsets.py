...
_CLOVER_CORE_TOOLS = [
    ...
    # spawned as a kanban worker (CLOVER_KANBAN_TASK env set) or the current
...
TOOLSETS = {
    ...
    "x_search": {
        "description": (
            ...
            "or XAI_API_KEY). Off by default; enable in `clover tools` → "
            "X (Twitter) Search."
        ),
        "tools": ["x_search"],
        "includes": []
    ...
    "bfl": {
        "description": (
            "Black Forest Labs FLUX 3 video generation through the Clover tool "
    ...
    "kanban": {
        "description": (
            ...
            "is spawned by the kanban dispatcher (CLOVER_KANBAN_TASK env "
    ...
    # Coding posture (base Clover — CLI/TUI/desktop/ACP). Auto-selected in a
    ...
    "coding": {
        ...
        # non-configurable-toolset recovery loop in clover_cli/tools_config.py).
        "posture": True,
    },
    
    # ==========================================================================
    # Full Clover toolsets (CLI + messaging platforms)
    #
    ...
    # the `clover send` CLI), not by the model deciding to send on its own.
    # ==========================================================================
    ...
    "clover-cli": {
        ...
        "tools": _CLOVER_CORE_TOOLS,
        "includes": []
    },

    "clover-cron": {
        # Mirrors clover-cli so cron's "default" toolset is the same set of
        # core tools users see interactively — then `clover tools` filters
        # them down per the platform config. _DEFAULT_OFF_TOOLSETS (moa,
        # homeassistant) are excluded by _get_platform_tools() unless
        # the user explicitly enables them.
        "description": "Default cron toolset - same core tools as clover-cli; gated by `clover tools`",
        "tools": _CLOVER_CORE_TOOLS,
        "includes": []
    },
    ...
    "clover-email": {
        "description": "Email bot toolset - interact with Clover via email (IMAP/SMTP)",
        "tools": _CLOVER_CORE_TOOLS,
        "includes": []
    },
    ...
    "clover-sms": {
        "description": "SMS bot toolset - interact with Clover via SMS (Twilio)",
        "tools": _CLOVER_CORE_TOOLS,
        "includes": []
    },
    ...
    "clover-webhook": {
        ...
        "tools": _CLOVER_WEBHOOK_SAFE_TOOLS,
        "includes": []
    },
    ...
    "clover-gateway": {
        ...
        "tools": [],
        "includes": ["clover-telegram", "clover-discord", "clover-whatsapp", "clover-slack", "clover-signal", "clover-bluebubbles", "clover-homeassistant", "clover-email", "clover-sms", "clover-mattermost", "clover-matrix", "clover-dingtalk", "clover-feishu", "clover-wecom", "clover-wecom-callback", "clover-weixin", "clover-qqbot", "clover-webhook", "clover-yuanbao"]
    }
}
...
def bundle_non_core_tools(toolset_name: str) -> Set[str]:
    """Return a ``clover-*`` bundle's platform-specific tools, excluding core.

    Platform bundles are defined as ``_CLOVER_CORE_TOOLS + [platform extras]``.
    ...
    Bundle nesting is one level deep in practice (only ``clover-gateway``
...
def resolve_toolset(name: str, visited: Set[str] = None, *, include_registry: bool = True) -> List[str]:
    ...
    if not toolset:
        # Auto-generate a toolset for plugin platforms (clover-<name>).
        # Gives them _CLOVER_CORE_TOOLS plus any tools the plugin registered
        ...
        if include_registry and name.startswith("clover-"):
            platform_name = name[len("clover-"):]
            try:
                from gateway.platform_registry import platform_registry
                if platform_registry.is_registered(platform_name):
                    plugin_tools = set(_CLOVER_CORE_TOOLS)
                    try:
                        from tools.registry import registry
                        plugin_tools.update(
