"""Run the read-tool eval through the REAL Clover AIAgent.

For each task: fresh temp CLOVER_HOME, fresh fixture workspace, real
...
def run_task(task, model: str, provider: str, timeout_mult: float,
    ...
    clover_home = Path(tempfile.mkdtemp(prefix="readtool-home-")) / ".clover"
    ...
    try:
        ...
        shutil.rmtree(clover_home.parent, ignore_errors=True)
    return result
...
def main() -> int:
    ...
    if not os.environ.get("OPENROUTER_API_KEY"):
        raise SystemExit(
            ...
            "source ~/.clover/.env; set +a  — then relaunch."
        )
