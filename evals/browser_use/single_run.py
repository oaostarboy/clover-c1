...
The run gets a throwaway CLOVER_HOME so no local config leaks in, and the
...
os.environ["CLOVER_HOME"] = hh
# Strip web-fetch shortcuts: every arm must drive the browser.
os.environ.pop("BROWSER_USE_API_KEY", None)
for k in ("FIRECRAWL_API_KEY", "CLOVER_API_KEY", "TAVILY_API_KEY", "SERPER_API_KEY"):
...
# Clover-subscription path on boxes without an OpenRouter key. Credentials are
...
if _or_key:
    ...
    # Resolved by the orchestrator BEFORE CLOVER_HOME is redirected to the
    # throwaway home (auth state lives in the real profile). Never printed.
    _tok = os.environ.get("BUBENCH_CLOVER_TOKEN", "").strip()
    if not _tok:
        raise SystemExit("no OPENROUTER_API_KEY and no Clover auth available")
    _agent_auth = dict(
        base_url=os.environ.get("BUBENCH_CLOVER_BASE_URL", ""),
        api_key=_tok,
        provider="clover",
    )
