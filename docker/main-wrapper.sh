#!/bin/sh
# shellcheck shell=sh
# /opt/clover/docker/main-wrapper.sh — wraps the container's CMD with
...
# with-contenv before touching CLOVER_HOME / PATH. On the non-PID-1
# fallback path the Dockerfile env is still intact, so we skip the
# re-exec and continue directly.
#
# Routing:
#   no args                       → exec `clover` (the default)
#   first arg is an executable    → exec it directly (sleep, bash, sh, …)
#   first arg is anything else    → exec `clover <args>` (subcommand passthrough)
#
# Drop to clover via s6-setuidgid, but skip it when already non-root.
set -e

if [ -z "${CLOVER_MAIN_WRAPPER_ENV_READY:-}" ] && \
   [ -z "${CLOVER_HOME:-}" ] && \
   [ -x /command/with-contenv ]; then
    export CLOVER_MAIN_WRAPPER_ENV_READY=1
    exec /command/with-contenv sh "$0" "$@"
fi
unset CLOVER_MAIN_WRAPPER_ENV_READY

drop() { [ "$(id -u)" = 0 ] && set -- s6-setuidgid clover "$@"; exec "$@"; }
...
# an arbitrary non-root, non-clover UID, the bootstrap was skipped and the
# baked image dirs (owned by the clover build UID) are unwritable, so fail
...
if [ "$cur_uid" != 0 ] && [ "$cur_uid" != "$(id -u clover)" ]; then
    cat >&2 <<EOF
[clover] ERROR: container started with --user $cur_uid (an arbitrary, non-clover UID) — not supported.
...
# to the clover user's home before dropping privileges so libraries that
...
_clover_orig_cwd="${CLOVER_ORIG_CWD:-$PWD}"
...
# the user's command so `clover chat` starts in the Docker -w
...
# Clover subcommand pass-through.
drop clover "$@"
