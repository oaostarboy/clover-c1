...
#
# Wired into the image as /etc/cont-init.d/01-clover-setup by the
...
CLOVER_HOME="${CLOVER_HOME:-/opt/data}"
INSTALL_DIR="/opt/clover"

# Drop to clover via s6-setuidgid, but skip it when already non-root.
as_clover() { [ "$(id -u)" = 0 ] || { "$@"; return; }; s6-setuidgid clover "$@"; }
...
# starts non-root. The baked install tree under /opt/clover is intentionally
# root-owned and non-writable; mutable runtime state must live under
# $CLOVER_HOME. An arbitrary `--user` UID therefore cannot repair or populate
...
# default) and pass CLOVER_UID/CLOVER_GID — or the PUID/PGID aliases — which the
...
# `id -u clover` is the unremapped build UID (10000) because no root-only remap
# could run. root starts (id -u = 0) and the normal supervised drop to the
# clover UID are both unaffected.
cur_uid="$(id -u)"
if [ "$cur_uid" != 0 ] && [ "$cur_uid" != "$(id -u clover)" ]; then
    cat >&2 <<EOF
[stage2] ERROR: container started with --user $cur_uid (an arbitrary, non-clover UID).
...
and the baked /opt/clover install tree is intentionally root-owned and
...
Start the container as root (the default) and pass your host UID/GID instead:
    ...
    docker run -e CLOVER_UID=\$(id -u) -e CLOVER_GID=\$(id -g) ...

NAS users (Synology / unRAID / UGOS) can use the PUID/PGID aliases:

    docker run -e PUID=\$(id -u) -e PGID=\$(id -g) ...

The image remaps the clover user to that UID/GID at boot and chowns the data
...
# --- Bootstrap CLOVER_HOME as root ---
# Create the directory (and any missing parents) while we still have root
# privileges so the chown checks below see real metadata and the later
# `s6-setuidgid clover mkdir -p` block doesn't EACCES on root-owned
# ancestors. Without this, custom CLOVER_HOME paths whose parents only
# root can create (e.g. `CLOVER_HOME=/home/clover/.clover` in a Compose
...
# --- UID/GID remap ---
# Accept PUID/PGID as aliases for CLOVER_UID/CLOVER_GID.  NAS users (UGOS,
...
# UID 10000 leaves the runtime unable to read the volume.  CLOVER_UID/
# CLOVER_GID still win when both are set.  See #15290, salvages #25872.
CLOVER_UID="${CLOVER_UID:-${PUID:-}}"
CLOVER_GID="${CLOVER_GID:-${PGID:-}}"

if [ -n "${CLOVER_UID:-}" ] && validate_uid_gid "$CLOVER_UID" && [ "$CLOVER_UID" != "$(id -u clover)" ]; then
    echo "[stage2] Changing clover UID to $CLOVER_UID"
    usermod -u "$CLOVER_UID" clover
fi
if [ -n "${CLOVER_GID:-}" ] && validate_uid_gid "$CLOVER_GID" && [ "$CLOVER_GID" != "$(id -g clover)" ]; then
    echo "[stage2] Changing clover GID to $CLOVER_GID"
    # -o allows non-unique GID (e.g. macOS GID 20 "staff" may already
    # exist as "dialout" in the Debian-based container image).
    groupmod -o -g "$CLOVER_GID" clover 2>/dev/null || true
fi
...
# host's `docker` group (or root). The supervised clover user (UID 10000)
...
# clover process with `Groups: 10000` (998 gone); after this hook adds
...
# matching entry that includes clover. Idempotent across container
...
#   - clover is already a member of the right group (idempotent restart)
...
for sock in /var/run/docker.sock /run/docker.sock; do
    ...
    if id -G clover 2>/dev/null | tr ' ' '\n' | grep -qx "$sock_gid"; then
        echo "[stage2] clover already in group $sock_gid for $sock"
        break
    fi
    ...
    if usermod -aG "$sock_group" clover 2>/dev/null; then
        echo "[stage2] Added clover to group $sock_group (GID $sock_gid) for $sock"
    else
        echo "[stage2] Warning: usermod -aG $sock_group clover failed; docker backend may fail with EACCES"
    fi
    break
done

# --- Fix ownership of data volume ---
# When CLOVER_UID is remapped or the top-level $CLOVER_HOME isn't owned by
# the runtime clover UID, restore ownership to clover — but ONLY for the
# directories clover actually writes to. The full $CLOVER_HOME may be a
# host-mounted bind containing unrelated user files; `chown -R` would
# silently destroy host ownership of those (see issue #19788).
#
# The canonical list of clover-owned subdirs is the same one the s6-setuidgid
# mkdir -p block below seeds. Keep them in sync if the seed list changes.
actual_clover_uid=$(id -u clover)
...
chown_clover_tree() {
    ...
    chown -R clover:clover "$target" 2>/dev/null || \
        echo "[stage2] Warning: chown $target failed (rootless container?) — continuing"
}

tree_has_non_clover_owner() {
    target="$1"
    find "$target" \( ! -user clover -o ! -group clover \) -print -quit 2>/dev/null | grep -q .
}

needs_chown=false
if [ "$(stat -c %u "$CLOVER_HOME" 2>/dev/null)" != "$actual_clover_uid" ]; then
    needs_chown=true
fi
if [ "$needs_chown" = true ]; then
    echo "[stage2] Fixing ownership of $CLOVER_HOME (targeted) to clover ($actual_clover_uid)"
    # In rootless Podman the container's "root" is mapped to an
    # unprivileged host UID — chown will fail. That's fine: the volume
    # is already owned by the mapped user on the host side.
    #
    # Top-level $CLOVER_HOME: chown the directory itself (not its contents)
    # so clover can mkdir new subdirs but bind-mounted host files keep
    # their existing ownership.
    if refuse_symlinked_path "chown" "$CLOVER_HOME"; then
        :
    else
        chown clover:clover "$CLOVER_HOME" 2>/dev/null || \
            echo "[stage2] Warning: chown $CLOVER_HOME failed (rootless container?) — continuing"
    fi
    # Clover-owned subdirs: recursive chown is safe here because these are
    # created and managed exclusively by clover (see the s6-setuidgid mkdir
    ...
    for sub in cron sessions logs hooks memories skills skins plans workspace home profiles pairing platforms/pairing lazy-packages; do
        if [ -e "$CLOVER_HOME/$sub" ] && tree_has_non_clover_owner "$CLOVER_HOME/$sub"; then
            chown_clover_tree "$CLOVER_HOME/$sub"
        fi
    done
fi
...
# clover user. Hosted/container instances keep mutable state under
# $CLOVER_HOME (/opt/data) and run with PYTHONDONTWRITEBYTECODE plus
# CLOVER_DISABLE_LAZY_INSTALLS=1. Keeping /opt/clover root-owned and
...
# $CLOVER_HOME/lazy-packages dir on the data volume (Dockerfile sets
# CLOVER_LAZY_INSTALL_TARGET). That dir is appended to the END of sys.path,
...
# even though installs are re-enabled. The dir is seeded + chowned to clover
...
# Always reset ownership of $CLOVER_HOME/profiles to clover on every
# boot. Profile dirs and files can land owned by root when commands
# are invoked via `docker exec <container> clover …` (which defaults
# to root unless `-u` is passed), and that breaks the cont-init
# reconciler (02-reconcile-profiles) which runs as clover and walks
...
if [ -d "$CLOVER_HOME/profiles" ] && tree_has_non_clover_owner "$CLOVER_HOME/profiles"; then
    chown_clover_tree "$CLOVER_HOME/profiles"
fi

# Always reset ownership of $CLOVER_HOME/cron on every boot for the same
# docker-exec/root-write reason as profiles/. The cron scheduler state
# (jobs.json) must stay readable by the unprivileged clover runtime even
...
# gate as profiles/).
if [ -d "$CLOVER_HOME/cron" ] && tree_has_non_clover_owner "$CLOVER_HOME/cron"; then
    chown_clover_tree "$CLOVER_HOME/cron"
fi

# Always ensure logs/gateways is clover-owned (#45258). Formerly healed by
...
# top-level $CLOVER_HOME is mis-owned, so a warm volume with clover-owned
# CLOVER_HOME but root-owned logs/gateways would otherwise leave
# s6-setuidgid clover mkdir failing with Permission denied. Non-recursive:
# profile leaf dirs are each created/owned by their own log/run as clover.
if [ -d "$CLOVER_HOME/logs/gateways" ]; then
    if refuse_symlinked_path "chown" "$CLOVER_HOME/logs/gateways"; then
        :
    else
        chown clover:clover "$CLOVER_HOME/logs/gateways" 2>/dev/null || true
    fi
fi
...
# clover pairing approve …` defaults to uid=0 and writes 0600 root-owned
# approval files that the unprivileged clover gateway cannot read,
# silently leaving the approved user unauthorized (#10270). The targeted
# data-volume chown above only runs when the top-level $CLOVER_HOME is
...
# and cron/.
if [ -d "$CLOVER_HOME/platforms/pairing" ] && tree_has_non_clover_owner "$CLOVER_HOME/platforms/pairing"; then
    chown_clover_tree "$CLOVER_HOME/platforms/pairing"
fi
# Legacy location (pre-consolidated layout).
if [ -d "$CLOVER_HOME/pairing" ] && tree_has_non_clover_owner "$CLOVER_HOME/pairing"; then
    chown_clover_tree "$CLOVER_HOME/pairing"
fi

# Reset ownership of clover-owned top-level state files on every boot.
# The targeted data-volume chown above only covers clover-owned
# *subdirectories*; loose state files living directly under $CLOVER_HOME
# are missed. When those files are created or rewritten by
# `docker exec <container> clover …` (root unless `-u` is passed) they
# land root-owned, and the unprivileged clover runtime then hits
...
# sweep so host-owned files in a bind-mounted $CLOVER_HOME are never
...
# entries of clover_cli.profile_distribution.USER_OWNED_EXCLUDE plus the
...
for f in \
    ...
    active_profile; do
    if [ -e "$CLOVER_HOME/$f" ]; then
        if refuse_symlinked_path "chown" "$CLOVER_HOME/$f"; then
            :
        else
            chown clover:clover "$CLOVER_HOME/$f" 2>/dev/null || true
        fi
    fi
done

# --- config.yaml permissions ---
# Ensure config.yaml is readable by the clover runtime user even if it
# was edited on the host after initial ownership setup.
if [ -f "$CLOVER_HOME/config.yaml" ]; then
    if refuse_symlinked_path "chown/chmod" "$CLOVER_HOME/config.yaml"; then
        :
    else
        chown clover:clover "$CLOVER_HOME/config.yaml" 2>/dev/null || true
        chmod 640 "$CLOVER_HOME/config.yaml" 2>/dev/null || true
    fi
fi

# --- Seed directory structure as clover user ---
# Run as clover via s6-setuidgid so dirs end up owned correctly (matters
# under rootless Podman where chown back to root would fail).
#
# Use direct `mkdir -p` invocation (no `sh -c "..."` wrapper) so the
# shell isn't a second interpreter — defends against $CLOVER_HOME values
# containing shell metacharacters. PR #30136 review item O2.
as_clover mkdir -p \
    "$CLOVER_HOME/backups" \
    "$CLOVER_HOME/cron" \
    "$CLOVER_HOME/sessions" \
    "$CLOVER_HOME/logs" \
    "$CLOVER_HOME/logs/gateways" \
    "$CLOVER_HOME/hooks" \
    "$CLOVER_HOME/memories" \
    "$CLOVER_HOME/skills" \
    "$CLOVER_HOME/skins" \
    "$CLOVER_HOME/plans" \
    "$CLOVER_HOME/workspace" \
    "$CLOVER_HOME/home" \
    "$CLOVER_HOME/pairing" \
    "$CLOVER_HOME/platforms/pairing" \
    "$CLOVER_HOME/lazy-packages"

# --- Install-method stamp ---
# The 'docker' stamp is baked into the immutable install tree at
# /opt/clover/.install_method (see Dockerfile), NOT written here into
# $CLOVER_HOME. detect_install_method() reads the code-scoped stamp first.
#
# Why we no longer stamp $CLOVER_HOME: it is a shared DATA volume, commonly
# bind-mounted from the host (~/.clover:/opt/data) and sometimes shared with a
...
# 'clover update'. To heal homes already poisoned by older images, remove a
# stale 'docker' stamp from $CLOVER_HOME if one is present (the host install's
# own installer re-creates its code-scoped stamp; a genuine container relies on
# the baked /opt/clover stamp, so deleting the data-dir copy is safe).
if [ -f "$CLOVER_HOME/.install_method" ]; then
    stamped="$(tr -d '[:space:]' < "$CLOVER_HOME/.install_method" 2>/dev/null || true)"
    if [ "$stamped" = "docker" ]; then
        rm -f "$CLOVER_HOME/.install_method" 2>/dev/null || true
    fi
fi
...
seed_one() {
    dest=$1
    src=$2
    if [ ! -f "$CLOVER_HOME/$dest" ] && [ -f "$INSTALL_DIR/$src" ]; then
        if refuse_symlinked_path "seed" "$CLOVER_HOME/$dest"; then
            :
        else
            as_clover cp "$INSTALL_DIR/$src" "$CLOVER_HOME/$dest"
        fi
    fi
}
...
# the first-boot seed above depends on /opt/clover/.env.example being present
...
# not generate one. Clover loads $CLOVER_HOME/.env with override=True, so
...
if [ -n "${API_SERVER_KEY:-}" ]; then
    if [ -f "$CLOVER_HOME/.env" ] && grep -q '^API_SERVER_KEY=..*' "$CLOVER_HOME/.env" 2>/dev/null; then
        echo "[stage2] Warning: API_SERVER_KEY is set in both the container environment and $CLOVER_HOME/.env — the .env value wins at runtime (loaded with override=True)"
    else
        ...
        if [ -f "$CLOVER_HOME/.env" ] && ! refuse_symlinked_path "clean" "$CLOVER_HOME/.env"; then
            sed -i '/^API_SERVER_KEY=$/d' "$CLOVER_HOME/.env" 2>/dev/null || true
        fi
        echo "[stage2] API_SERVER_KEY provided via container environment — skipping generation"
    fi
elif ! grep -q '^API_SERVER_KEY=..*' "$CLOVER_HOME/.env" 2>/dev/null; then
    if refuse_symlinked_path "append" "$CLOVER_HOME/.env"; then
        :
    else
        if [ ! -f "$CLOVER_HOME/.env" ]; then
            ...
            (umask 077 && as_clover touch "$CLOVER_HOME/.env") 2>/dev/null || true
        fi
        if [ -f "$CLOVER_HOME/.env" ]; then
            ...
            if [ -n "$_gen_key" ]; then
                ...
                sed -i '/^API_SERVER_KEY=$/d' "$CLOVER_HOME/.env" 2>/dev/null || true
                if printf 'API_SERVER_KEY=%s\n' "$_gen_key" >> "$CLOVER_HOME/.env" 2>/dev/null; then
                    echo "[stage2] Generated API_SERVER_KEY for the loopback gateway api_server"
                else
                    echo "[stage2] Warning: could not write API_SERVER_KEY to $CLOVER_HOME/.env (read-only volume?) — gateway api_server (cron fires) will be unavailable"
                fi
            fi
            unset _gen_key
        else
            echo "[stage2] Warning: could not create $CLOVER_HOME/.env — gateway api_server (cron fires) will be unavailable"
        fi
    fi
fi
...
if [ -f "$CLOVER_HOME/.env" ]; then
    if refuse_symlinked_path "chown/chmod" "$CLOVER_HOME/.env"; then
        :
    else
        chown clover:clover "$CLOVER_HOME/.env" 2>/dev/null || true
        chmod 600 "$CLOVER_HOME/.env" 2>/dev/null || true
    fi
fi
...
# as the unprivileged `clover` user — without this it gets EACCES on every
...
# clover group (any group member could e.g. stop/suspend this machine), not
...
if [ -S /.fly/api ]; then
    if refuse_symlinked_path "chgrp/chmod" /.fly/api; then
        :
    elif chgrp clover /.fly/api 2>/dev/null && chmod g+w /.fly/api 2>/dev/null; then
        echo "[stage2] Granted clover group access to the Fly Machines API socket"
    else
...
# $CLOVER_HOME on the mounted volume. Run the same safe, non-interactive
# config-schema migrations that `clover update` runs for non-Docker installs,
# after first-boot seeding and before supervised gateway services start.
# Set CLOVER_SKIP_CONFIG_MIGRATION=1 for controlled/manual migrations.
if [ -f "$CLOVER_HOME/config.yaml" ]; then
    s6-setuidgid clover "$INSTALL_DIR/.venv/bin/python" "$INSTALL_DIR/scripts/docker_config_migrate.py" \
...
if [ ! -f "$CLOVER_HOME/auth.json" ] && [ -n "${CLOVER_AUTH_JSON_BOOTSTRAP:-}" ]; then
    if refuse_symlinked_path "seed" "$CLOVER_HOME/auth.json"; then
        :
    else
        printf '%s' "$CLOVER_AUTH_JSON_BOOTSTRAP" > "$CLOVER_HOME/auth.json"
        chown clover:clover "$CLOVER_HOME/auth.json" 2>/dev/null || true
        chmod 600 "$CLOVER_HOME/auth.json"
    fi
fi
...
# can set CLOVER_GATEWAY_BOOTSTRAP_STATE=running; we seed the state file
...
# CLOVER_AUTH_JSON_BOOTSTRAP seeds auth.json. The [ ! -f ] guard is the
...
if [ ! -f "$CLOVER_HOME/gateway_state.json" ] && \
        [ "${CLOVER_GATEWAY_BOOTSTRAP_STATE:-}" = "running" ]; then
    if refuse_symlinked_path "seed" "$CLOVER_HOME/gateway_state.json"; then
        :
    else
        printf '{"gateway_state":"running"}\n' > "$CLOVER_HOME/gateway_state.json"
        chown clover:clover "$CLOVER_HOME/gateway_state.json" 2>/dev/null || true
        chmod 644 "$CLOVER_HOME/gateway_state.json"
    fi
fi
...
if [ -d "$INSTALL_DIR/skills" ]; then
    as_clover "$INSTALL_DIR/.venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" \
        || echo "[stage2] Warning: skills_sync.py failed; continuing"
fi

# --- Discover agent-browser's Chromium binary ---
# The image's Dockerfile runs `npx playwright install chromium`, which
# populates ``$PLAYWRIGHT_BROWSERS_PATH`` (=/opt/clover/.playwright) with
# a ``chromium_headless_shell-<build>/chrome-headless-shell-linux64/``
# directory. agent-browser (the runtime CLI Clover spawns for the
...
# main-wrapper.sh propagates it into the supervised ``clover`` process
...
if [ -z "${AGENT_BROWSER_EXECUTABLE_PATH:-}" ] && \
    ...
    if [ -n "$browser_bin" ]; then
        ...
        # up for all supervised services (main-clover, dashboard, etc.).
