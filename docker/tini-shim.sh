...
# Clover WebUI, NAS "update" flows that preserve an old entrypoint, etc.)
...
#
# Test hook: CLOVER_TINI_SHIM_TARGET overrides the `/init` path (and
# CLOVER_TINI_SHIM_WRAPPER the main-wrapper path) so unit tests can
# record argv without a real s6 tree.

set -e

INIT_TARGET="${CLOVER_TINI_SHIM_TARGET:-/init}"
WRAPPER="${CLOVER_TINI_SHIM_WRAPPER:-/opt/clover/docker/main-wrapper.sh}"
