...
const BACKEND = process.env.CLOVER_DASHBOARD_URL ?? "http://127.0.0.1:9119";

/**
 * In production the Python `clover dashboard` server injects a one-shot
 * session token into `index.html` (see `clover_cli/web_server.py`). The
 * Vite dev server serves its own `index.html`, so unless we forward that
 * token, every protected `/api/*` call 401s.
 *
 * This plugin fetches the running dashboard's `index.html` on each dev page
 * load, scrapes the `window.__CLOVER_SESSION_TOKEN__` assignment, and
 * re-injects it into the dev HTML. No-op in production builds.
 */
function cloverDevToken(): Plugin {
  const TOKEN_RE = /window\.__CLOVER_SESSION_TOKEN__\s*=\s*"([^"]+)"/;
  const EMBEDDED_RE =
    /window\.__CLOVER_DASHBOARD_EMBEDDED_CHAT__\s*=\s*(true|false)/;

  return {
    name: "clover:dev-session-token",
    apply: "serve",
    async transformIndexHtml() {
      try {
        ...
        if (!match) {
          console.warn(
            `[clover] Could not find session token in ${BACKEND} — ` +
              `is \`clover dashboard\` running? /api calls will 401.`,
          );
          return;
        }
        ...
        return [
          {
            ...
            injectTo: "head",
            children:
              `window.__CLOVER_SESSION_TOKEN__="${match[1]}";` +
              `window.__CLOVER_DASHBOARD_EMBEDDED_CHAT__=${embeddedJs};`,
          },
        ];
      } catch (err) {
        console.warn(
          `[clover] Dashboard at ${BACKEND} unreachable — ` +
            `start it with \`clover dashboard\` or set CLOVER_DASHBOARD_URL. ` +
...
export default defineConfig({
  ...
  resolve: {
    alias: {
      ...
      "@clover/shared": path.resolve(__dirname, "../apps/shared/src"),
    },
    // When @clover/ui is symlinked via `file:../../design-language`,
  ...
  server: {
    proxy: {
      ...
      // Same host as `clover dashboard` must serve these; Vite has no
