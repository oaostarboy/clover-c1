import { buildCloverWebSocketUrl } from "@clover/shared";

// The dashboard can be served either at the root of its host (e.g.
// https://kanban.tilos.com/) or under a URL prefix when reverse-proxied
// (e.g. https://mission-control.tilos.com/clover/). The Python backend
// injects ``window.__CLOVER_BASE_PATH__`` into index.html based on the
// incoming ``X-Forwarded-Prefix`` header so the SPA can address its own
// ``/api/...`` and ``/dashboard-plugins/...`` URLs correctly without a
// rebuild. Empty string means "served at root".
function readBasePath(): string {
  if (typeof window === "undefined") return "";
  const raw = window.__CLOVER_BASE_PATH__ ?? "";
  if (!raw) return "";
...
export const CLOVER_BASE_PATH = readBasePath();
const BASE = CLOVER_BASE_PATH;
...
declare global {
  interface Window {
    __CLOVER_SESSION_TOKEN__?: string;
    __CLOVER_BASE_PATH__?: string;
    ...
    __CLOVER_AUTH_REQUIRED__?: boolean;
  }
}
const SESSION_HEADER = "X-Clover-Session-Token";
...
export async function fetchJSON<T>(
  ...
  const token = window.__CLOVER_SESSION_TOKEN__;
  if (token) {
  ...
  if (res.status === 401) {
    ...
    // (``clover update``, ``clover gateway restart``, etc.). A tab kept
    // open across the restart holds the OLD token in
    // ``window.__CLOVER_SESSION_TOKEN__`` from the previous HTML render,
    ...
    if (!window.__CLOVER_AUTH_REQUIRED__ && !options?.allowUnauthorized) {
  ...
  if (res.ok) {
    ...
    // current ``window.__CLOVER_SESSION_TOKEN__`` is valid, so the next
 ...
 *
 * The dashboard's gated-mode WS auth (``clover_cli.web_server._ws_auth_ok``)
...
export async function buildWsAuthParam(): Promise<[string, string]> {
  if (window.__CLOVER_AUTH_REQUIRED__) {
 ...
 *  - loopback / ``--insecure``: attach the ``X-Clover-Session-Token`` header.
 *  - gated OAuth: no token header (it's absent by design); the
 *    ``clover_session_at`` cookie rides along via ``credentials: 'include'``.
 *
 ...
 * ``window.__CLOVER_SESSION_TOKEN__`` directly, so the gated-mode ticket
...
export async function buildWsUrl(
  ...
  return buildCloverWebSocketUrl({
...
export const api = {
  ...
  updateClover: () =>
    fetchJSON<ActionResponse>("/api/clover/update", { method: "POST" }),
  checkCloverUpdate: (force = false) =>
    fetchJSON<UpdateCheckResponse>(
      `/api/clover/update/check${force ? "?force=true" : ""}`,
    ),
...
export interface SkillHubSource {
  ...
  /** clover-index only: whether the centralized index loaded. */
  available?: boolean;
}
...
export interface StatusResponse {
  ...
  /** Phase 7: registered ``DashboardAuthProvider`` names (e.g. ``["clover"]``).
   ...
   * updates are handled by the outer launcher instead of ``clover update``. */
  can_update_clover?: boolean;
  config_path: string;
  ...
  /** NS-656: disk-usage rollup for the CLOVER_HOME volume. Absent on
   * older gateways. */
 ...
 * sample of the CLOVER_HOME volume — no staleness dimension, so no
 * sampled_at. */
