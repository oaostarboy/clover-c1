/**
 * Clover Dashboard Plugin SDK — typed contract (SPIKE)
 * ====================================================
 *
 * This is the public type surface for ``window.__CLOVER_PLUGIN_SDK__`` and
 * ``window.__CLOVER_PLUGINS__``, the globals the dashboard host exposes to
 ...
 * window.__CLOVER_PLUGIN_SDK__`` because:
 *   1. The runtime object is assembled from many internal modules
 *      (``@/lib/api``, ``@clover/ui``, …). Deriving the type would
 ...
 *
 * Versioning: bump ``CloverPluginSDK["sdkVersion"]`` (and the
 ...
 *   - Ship as a published ``@clover/dashboard-plugin-sdk`` types package, or
 ...
 *     (pulls @clover/ui types into the contract) or is the loose
 ...
 * of calling ``fetch`` with a hand-read ``window.__CLOVER_SESSION_TOKEN__``.
 */
...
// ---------------------------------------------------------------------------
// Registry surface (window.__CLOVER_PLUGINS__)
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// SDK surface (window.__CLOVER_PLUGIN_SDK__)
// ---------------------------------------------------------------------------

export interface CloverPluginSDK {
  ...
  /**
   * Shared UI primitives (Clover DS / shadcn). Typed permissively at the
...
declare global {
  interface Window {
    __CLOVER_PLUGIN_SDK__?: CloverPluginSDK;
    __CLOVER_PLUGINS__?: PluginRegistry;
  }
}
