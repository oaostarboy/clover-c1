...
import { api, CLOVER_BASE_PATH } from "@/lib/api";
...
export const MANIFEST_CACHE_KEY = "clover:plugin-manifests";
...
export function usePlugins() {
  ...
  useEffect(() => {
    ...
    for (const manifest of manifests) {
      ...
      if (manifest.css) {
        const cssUrl = `${CLOVER_BASE_PATH}/dashboard-plugins/${manifest.name}/${manifest.css}`;
      ...
      const baseUrl = `${CLOVER_BASE_PATH}/dashboard-plugins/${manifest.name}/${manifest.entry}`;
      const scriptSrc = import.meta.env.DEV
        ? `${baseUrl}?clover_dv=${Date.now()}`
        : baseUrl;
      ...
      script.setAttribute("data-clover-plugin", manifest.name);
