/**
 * ChatPage — embeds `clover --tui` inside the dashboard.
 *
 ...
 *          ▼                                                              .
 *     FastAPI pty_ws  (clover_cli/web_server.py)                          .
 *          ▼                                                              .
...
import { Typography } from "@clover/ui/ui/components/typography/index";
...
const PTY_ATTACH_TOKEN_KEY = "clover.pty.token.chat";
...
export default function ChatPage({ isActive = true }: { isActive?: boolean }) {
  ...
  const [banner, setBanner] = useState<string | null>(() =>
    ...
    !window.__CLOVER_SESSION_TOKEN__ &&
    !window.__CLOVER_AUTH_REQUIRED__
      ? "Session token unavailable. Open this page through `clover dashboard`, not directly."
      : null,
  );
  ...
  const [chatPanelCollapsed, setChatPanelCollapsed] = useState(
    () => localStorage.getItem("clover-chat-panel-collapsed") === "1",
  );
  const toggleChatPanel = useCallback(() => {
    setChatPanelCollapsed((prev) => {
      const next = !prev;
      localStorage.setItem("clover-chat-panel-collapsed", next ? "1" : "0");
      return next;
    });
  }, []);
  ...
  useEffect(() => {
    ...
    const gated = !!window.__CLOVER_AUTH_REQUIRED__;
    ...
    const term = new Terminal({
      ...
      // even when the inner Clover TUI has enabled xterm mouse-events
    ...
    //      payloads upload to CLOVER_HOME/images then drive `/image`.
    //
    ...
    // must upload browser bytes to CLOVER_HOME/images, then drive `/image`
    ...
    if (useWebgl) {
      try {
        ...
        console.warn(
          "[clover-chat] WebGL renderer unavailable; falling back to default",
          err,
        );
      }
    }
    ...
    // WebSocket. In gated mode (``window.__CLOVER_AUTH_REQUIRED__``) this
    ...
    void (async () => {
      ...
      // Profile-scoped chat: the PTY child gets CLOVER_HOME pointed at the
    ...
    // forwarding them into Clover. Keyboard input, paste, and resize still
    // behave normally.
    ...
    <div className="flex min-h-0 flex-1 flex-col gap-2">
      ...
      <div className="flex min-h-0 flex-1 flex-col gap-2 lg:flex-row lg:gap-3">
        ...
        >
          <div
            ref={hostRef}
            className="clover-chat-xterm-host min-h-0 min-w-0 flex-1"
          />
