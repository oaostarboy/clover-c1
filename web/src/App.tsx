...
import { SelectionSwitcher } from "@clover/ui/ui/components/selection-switcher";
...
import { useBelowBreakpoint } from "@clover/ui/hooks/use-below-breakpoint";
...
const SIDEBAR_COLLAPSED_KEY = "clover-sidebar-collapsed";
          ...
          >
            ...
            >
              ...
              {sidebarNav.pluginItems.length > 0 && (
                <div
                  aria-labelledby="clover-sidebar-plugin-nav-heading"
                ...
                >
                  <span
                    ...
                    id="clover-sidebar-plugin-nav-heading"
                  >
...
function SidebarSystemActions({
  ...
  const { activeAction, isBusy, isRunning, pendingAction, runAction } =
    useSystemActions();
  const canUpdateClover = status?.can_update_clover === true;
  ...
  const updateConfirmDescription = useMemo(() => {
    if (updateConfirmInfo?.behind && updateConfirmInfo.behind > 0) {
      ...
      return `This will run 'clover update' (${cmd}) and pull ${n} new commit${n === 1 ? "" : "s"}. The gateway restarts when the update finishes; the current session keeps its prompt cache until then.`;
    }
    const cmd = updateConfirmInfo?.update_command ?? "clover update";
    return (
      t.status.updateCloverConfirmMessage ??
      `This will run 'clover update' (${cmd}) and restart the gateway when it finishes.`
    );
  }, [t.status.updateCloverConfirmMessage, updateConfirmInfo]);
  ...
  if (canUpdateClover) {
    items.push({
      action: "update",
      icon: Download,
      label: t.status.updateClover,
      runningLabel: t.status.updatingClover,
      spin: false,
    });
  }
  ...
  return (
    ...
    <ConfirmDialog
      ...
      description={
        ...
        "This restarts the Clover gateway process. Connected channels and active sessions will reconnect afterward."
      }
    ...
    <ConfirmDialog
      cancelLabel={t.common.cancel}
      confirmLabel={t.status.updateCloverConfirmNow ?? "Update now"}
      description={
      ...
      title={t.status.updateCloverConfirmTitle ?? `${t.status.updateClover}?`}
    />
    </>
  );
}
