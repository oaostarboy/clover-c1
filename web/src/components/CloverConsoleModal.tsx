...
interface CloverConsoleModalProps {
  open: boolean;
  onClose: () => void;
}
...
export function CloverConsoleModal({ open, onClose }: CloverConsoleModalProps) {
  const modalRef = useModalBehavior({ open, onClose });
  const hostRef = useRef<HTMLDivElement | null>(null);
  const termRef = useRef<XtermTerminal | null>(null);
  const wsRef = useRef<WebSocket | null>(null);
  const lineRef = useRef("");
  const promptRef = useRef("clover> ");
  const inputPromptRef = useRef("clover> ");
  ...
  const handleFrame = useCallback(
    (frame: ConsoleFrame) => {
      ...
      if (frame.type === "ready") {
        const nextPrompt = frame.prompt || "clover> ";
  ...
  useEffect(() => {
    ...
    writeLine(term, "\x1b[2mConnecting to Clover Console...\x1b[0m");
  ...
  return createPortal(
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-labelledby="clover-console-title"
    >
      <div
        className={cn(
          themedBody,
